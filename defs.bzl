...
def workx_rust_crate(
    ...
    if manifest_relpath.startswith("workx-rs/"):
        manifest_relpath = manifest_relpath[len("workx-rs/"):]
    ...
    if lib_srcs:
        ...
        rust_test(
            ...
            # tests under gnullvm.
            # Bazel has emitted both `workx-rs/<crate>/...` and
            # `../workx-rs/<crate>/...` paths for `file!()`. Strip either
            # prefix so the workspace-root launcher sees Cargo-like metadata
            # such as `tui/src/...`.
            rustc_flags = rustc_flags_extra + WINDOWS_RUSTC_LINK_FLAGS + [
                "--remap-path-prefix=../workx-rs=",
                "--remap-path-prefix=workx-rs=",
            ],
            rustc_env = rustc_env,
    ...
    for test in native.glob(["tests/*.rs"], allow_empty = True):
        ...
        if run_tests_with_wine_exec:
            ...
            foreign_platform_binary(
                ...
                binary = "//workx-rs/exec-server/testing:exec-server",
            ...
            wine_runfile_env[native_test_binary] = "WORKX_WINE_EXEC_TEST_BINARY"
            ...
            workspace_root_test(
                ...
                test_bin = "//workx-rs/exec-server/testing:wine-exec-test-runner",
                workspace_root_marker = "//workx-rs/utils/cargo-bin:repo_root.marker",

#!/usr/bin/env python3
"""Run reviewer-derived MCP regressions against a real Workx app-server."""
...
REVIEWER = "workx-mcp-regression-suite"
    ...
    *,
    ...
    if transport == "stdio":
        ...
        command.extend(
            [
                "--env",
                f"WORKX_MCP_PROTOCOL_VERSION={protocol_environment}",
                "--",
                sys.executable,
                str(server_script),
                "--mode",
    ...
    try:
        if mode == MODERN_VERSION:
            feature = _run_command(
                [str(workx_binary), "features", "enable", "mcp_2026_07_28"],
                env=env,
                cwd=workspace,
...
def run_review_regressions(
    ...
    *,
    ...
    root = Path(tempfile.mkdtemp(prefix="workx-mcp-review-", dir=artifact_parent))
...
def _parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run reviewer-derived MCP client regression tests against Workx."
    )
...
def _print_report(report: Mapping[str, object]) -> None:
    print(
        "Workx MCP reviewer regressions: "

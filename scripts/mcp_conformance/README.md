...
This directory tests the actual Workx executable against the official Model
...
First install the frozen workspace dependencies and build Workx:

```bash
pnpm install --frozen-lockfile
cargo build --locked --manifest-path workx-rs/Cargo.toml -p workx-cli --bin workx
```

From a published Workx checkout, run:

```bash
python3 scripts/mcp_conformance/run_workx_compliance.py \
  workx-rs/target/debug/workx \
  ...
  --report /tmp/workx-mcp-conformance.json
```

The positional executable can also point to an already built Workx binary.
...
The separate reviewer gate tests the real Workx app-server across all three
...
keepalives, and catalog boundaries. In a published Workx checkout, run:
...
python3 scripts/mcp_conformance/review_regressions.py \
  ...
  --report /tmp/workx-mcp-review-regressions.json
```
...
Its TypeScript job builds the actual Workx executable, sets `WORKX_EXEC_PATH`,
...
```bash
WORKX_EXEC_PATH=/absolute/path/to/workx \
  pnpm --filter @ronanxiao/workx-sdk test -- \

...
import workx_conformance_adapter
import official_conformance
import pytest
import run_workx_compliance
from workx_conformance_adapter import (
...
def test_official_adapter_launcher_preserves_strict_production_auth_mode(
    ...
    adapter.write_text(
        'import os\nprint(os.environ.get("WORKX_CONFORMANCE_REQUIRE_AUTOMATIC_AUTH", ""))\n',
        encoding="utf-8",
    )
...
def test_windows_official_adapter_launcher_quotes_checkout_paths(
    ...
    assert (
        f'set "WORKX_CONFORMANCE_REQUIRE_AUTOMATIC_AUTH={expected_auth}"\n' in contents
    )
...
def test_strict_auth_does_not_inject_a_second_login(
    ...
    monkeypatch.setattr(
        workx_conformance_adapter,
        "_oauth_login",
        lambda _client, **_kwargs: (True, None),
    )
    monkeypatch.setattr(workx_conformance_adapter, "_reload_mcp", lambda _client: None)
    monkeypatch.setattr(
        workx_conformance_adapter, "_auth_inventory", lambda _client: {}
    )

    def fail_tool_call(_client: object, _workspace: Path) -> None:
        raise AdapterFailure("reauthorization is required")

    monkeypatch.setattr(workx_conformance_adapter, "_auth_tool_call", fail_tool_call)
    monkeypatch.setattr(
        workx_conformance_adapter,
...
def test_retained_artifacts_remove_oauth_credential_store(tmp_path: Path) -> None:
    workx_home = tmp_path / "workx-home"
    workx_home.mkdir()
    credentials = workx_home / ".credentials.json"
...
def test_official_cli_rejects_an_empty_versioned_scenario_run(
    ...
    monkeypatch.setattr(run_workx_compliance, "run_compliance", unexpected_run)

    exit_code = run_workx_compliance.main(
        [
            sys.executable,
            "--mode",
            SHIPPING_LEGACY_VERSION,
...
pathlib.Path(os.environ["WORKX_CONFORMANCE_ADAPTER_REPORT"]).write_text(

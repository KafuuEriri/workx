#!/usr/bin/env python3
"""Black-box MCP compliance runner for a supplied Workx binary."""
...
class AppServerClient:
    """Minimal JSONL client for the Workx app-server protocol."""
    ...
    def __init__(
        ...
        *,
        ...
        self._stdout_thread = threading.Thread(
            ...
            name="workx-app-server-stdout",
            daemon=True,
        )
        self._stderr_thread = threading.Thread(
            target=self._read_stderr,
            name="workx-app-server-stderr",
            daemon=True,
        )
    ...
    def _send(self, message: Mapping[str, object]) -> None:
        if self.process.poll() is not None:
            raise AppServerError(
                f"Workx app-server exited with code {self.process.returncode}"
            )
    ...
    def request(
        ...
        while True:
            ...
            if message is None:
                raise AppServerError(
                    f"Workx app-server closed stdout while handling {method}"
                )
    ...
    def wait_for_notification(
        ...
        *,
        ...
        while True:
            ...
            if message is None:
                raise AppServerError(
                    f"Workx app-server closed stdout while waiting for {method}"
                )
            if "id" in message and isinstance(message.get("method"), str):
                self._handle_server_request(message)
                continue
...
def _isolated_environment(workx_home: Path) -> dict[str, str]:
    env = dict(os.environ)
    env["WORKX_HOME"] = str(workx_home)
    # Direct app-server MCP calls do not need a model or credentials.
    for name in ("WORKX_API_KEY", "WORKX_ACCESS_TOKEN", "OPENAI_API_KEY"):
        env.pop(name, None)
    return env
...
def _registration_command(
    ...
    *,
    ...
    command = [str(workx_binary), "mcp", "add", TEST_SERVER_NAME]
    if transport == "stdio":
        if mode == MODERN_VERSION:
            command.extend(["--env", f"WORKX_MCP_PROTOCOL_VERSION={MODERN_VERSION}"])
        return [
            *command,
            "--",
            sys.executable,
...
def _validate_registration(
    ...
    *,
    ...
    if transport == "stdio":
        ...
        modern_opt_in = (
            isinstance(env, dict)
            and env.get("WORKX_MCP_PROTOCOL_VERSION") == MODERN_VERSION
        )
...
def _exercise_app_server(
    ...
    *,
    ...
    sentinel = f"workx-mcp-{transport}-{mode}"
...
def _run_case(
    ...
    *,
    ...
    try:
        ...
        if registered:
            remove = _run_command(
                [str(workx_binary), "mcp", "remove", TEST_SERVER_NAME],
                env=env,
                cwd=workspace,
                timeout_seconds=timeout_seconds,
            )
            ...
            listed = _run_command(
                [str(workx_binary), "mcp", "list", "--json"],
                env=env,
                cwd=workspace,
                timeout_seconds=timeout_seconds,
            )
...
def _run_official_case(
    ...
    *,
    ...
    case_home.mkdir(parents=True)
    env = _isolated_environment(case_home)
    env["WORKX_CONFORMANCE_TIMEOUT"] = str(timeout_seconds)
    env["WORKX_CONFORMANCE_ENABLE_MODERN_FEATURE"] = (
        "1" if enable_modern_feature else "0"
    )
    env["WORKX_CONFORMANCE_REQUIRE_AUTOMATIC_AUTH"] = (
        "1" if require_automatic_auth else "0"
    )
    results = run_official_mode(
        conformance_command=conformance_command,
        adapter_script=adapter_script,
        workx_binary=workx_binary,
        mode=mode,
        scenarios=scenarios,
    ...
    for result in results:
        ...
        case.checks.append(
            CheckResult(
                name=f"harness/{result.scenario}/workx-adapter",
...
def _cimd_registration_checks(
    ...
    *,
    ...
    for check_name, client_registration in (
        ...
        env["WORKX_CONFORMANCE_REQUIRE_AUTOMATIC_AUTH"] = "1"
        if client_registration is not None:
            env["WORKX_CONFORMANCE_CLIENT_REGISTRATION"] = client_registration
        results = run_official_mode(
            conformance_command=conformance_command,
            adapter_script=adapter_script,
            workx_binary=workx_binary,
...
def run_compliance(
    ...
    *,
    ...
    report: dict[str, object] = {
        ...
        "workxBinary": str(workx_binary),
        "workxVersion": version.stdout or None,
...
def _required_regression_checks(
    ...
    *,
    ...
    if not isinstance(version_check, dict) or version_check.get("success") is not True:
        errors.append(f"{label} does not contain a successful Workx version check")
...
def _print_human_report(report: Mapping[str, object]) -> None:
    ...
    print(f"Workx MCP compliance: {'PASS' if success else 'FAIL'}")
    print(f"Binary: {report.get('workxBinary')}")
    print(f"Version: {report.get('workxVersion') or 'unknown'}")
...
def _parse_args(argv: Sequence[str] | None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "supplied Workx binary, plus the supplemental local stdio suite."
        )
    )
    parser.add_argument("workx_binary", type=Path)
    parser.add_argument(
        "--mode",
    ...
    parser.add_argument(
        "--adapter-script",
        type=Path,
        default=Path(__file__).resolve().with_name("workx_conformance_adapter.py"),
        help="Path to the Workx adapter used by the official client suite.",
    )
    parser.add_argument(
        "--conformance-cli",
        type=Path,
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Require Workx to recover from OAuth scope escalation and "
    ...
    parser.add_argument(
        "--artifact-parent",
        type=Path,
        help="Parent directory for isolated temporary Workx homes.",
    )
    parser.add_argument(
        "--keep-artifacts",
        action="store_true",
        help="Keep isolated Workx homes and include their path in the report.",
    )
    parser.add_argument(
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    workx_binary = args.workx_binary.expanduser().resolve()
    ...
    if not workx_binary.is_file() or not os.access(workx_binary, os.X_OK):
        print(f"error: Workx binary is not executable: {workx_binary}", file=sys.stderr)
        return 2

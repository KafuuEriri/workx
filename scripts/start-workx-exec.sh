...
remote_path='~/code/workx-sync'
local_exec_server_port="${WORKX_REMOTE_EXEC_SERVER_LOCAL_PORT:-8765}"
remote_exec_server_start_timeout_seconds="${WORKX_REMOTE_EXEC_SERVER_START_TIMEOUT_SECONDS:-15}"
...
remote_exec_server_log_path="/tmp/workx-exec-server-${sync_instance_id}.log"
remote_exec_server_pid_path="/tmp/workx-exec-server-${sync_instance_id}.pid"
...
remote_repo_root="$HOME/code/workx-sync"
remote_workx_rs="$remote_repo_root/workx-rs"

cd "${remote_workx_rs}"
cargo build -p workx-cli --bin workx

rm -f "${remote_exec_server_log_path}" "${remote_exec_server_pid_path}"
nohup ./target/debug/workx exec-server --listen ws://127.0.0.1:0 \
...
echo "Start workx via: "
printf '  WORKX_EXEC_SERVER_URL=ws://127.0.0.1:%s workx -C %q\n' \

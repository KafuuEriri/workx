...
def update_cask(cask: Path, version: str, sha256: str) -> bool:
    if not cask.is_file():
        ...
        cask.write_text(TEMPLATE.format(version=version, sha256=sha256), encoding="utf-8")
        return True
...
def main() -> int:
    ...
    parser.add_argument("--cask", type=Path, required=True, help="Path to Casks/workx.rb.")
    parser.add_argument("--version", required=True, help="Release version, e.g. 0.1.0.")
    parser.add_argument("--sha256", required=True, help="SHA-256 of the arm64 DMG asset.")

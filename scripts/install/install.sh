...
RELEASE="${WORKX_RELEASE:-latest}"
NON_INTERACTIVE="${WORKX_NON_INTERACTIVE:-false}"
DEFAULT_PREFER_RELEASES_OPENAI_COM="false"
PREFER_RELEASES_OPENAI_COM="${WORKX_INSTALLER_USE_RELEASES_OPENAI_COM:-$DEFAULT_PREFER_RELEASES_OPENAI_COM}"
...
BIN_DIR="${WORKX_INSTALL_DIR:-$HOME/.local/bin}"
BIN_PATH="$BIN_DIR/workx"
CODE_MODE_HOST_BIN_PATH="$BIN_DIR/workx-code-mode-host"
WORKX_HOME_DIR="${WORKX_HOME:-$HOME/.workx}"
STANDALONE_ROOT="$WORKX_HOME_DIR/packages/standalone"
...
validate_version() {
  ...
  if ! printf '%s\n' "$version" | grep -Eq '^[0-9]+\.[0-9]+\.[0-9]+(-alpha(\.[0-9]+){0,2}|-beta(\.[0-9]+)?)?$'; then
    echo "Invalid Workx release version: $version. Expected latest or x.y.z[-alpha[.N[.M]]|-beta[.N]]." >&2
    return 1
  fi
}
...
Environment:
  WORKX_RELEASE          Version to install; overridden by --release.
  WORKX_NON_INTERACTIVE  Set to 1, true, or yes to skip prompts.
  WORKX_INSTALLER_USE_RELEASES_OPENAI_COM
...
download_file() {
  ...
  echo "curl or wget is required to install Workx." >&2
  exit 1
}
...
release_url_for_asset() {
  ...
  printf 'https://github.com/RonanXiao/workx/releases/download/rust-v%s/%s\n' "$resolved_version" "$asset"
}
...
release_metadata_url() {
  ...
  printf 'https://api.github.com/repos/RonanXiao/workx/releases/tags/rust-v%s\n' "$resolved_version"
}
...
parse_downloaded_release_metadata() {
  ...
  if ! release_metadata="$(printf '%s\n' "$release_json" | parse_release_metadata)"; then
    echo "Could not parse $source_name release metadata for Workx $requested_release." >&2
    return 1
  fi
}
...
resolve_metadata_version() {
  ...
  if [ -z "$metadata_version" ]; then
    echo "Failed to resolve the latest Workx release version." >&2
    return 1
  fi
...
resolve_release_from_github() {
  ...
  if [ "$normalized_version" = "latest" ]; then
    ...
    metadata_url="https://api.github.com/repos/RonanXiao/workx/releases/latest"
  else
  ...
  if ! release_json="$(download_text "$metadata_url")"; then
    echo "Could not fetch GitHub release metadata for Workx $requested_release. GitHub API may be unavailable or rate limited." >&2
    exit 1
  fi
...
resolve_release_from_releases() {
  ...
  if [ "$normalized_version" != "latest" ] && [ "$metadata_version" != "$normalized_version" ]; then
    echo "Release metadata version did not match requested Workx version $normalized_version." >&2
    return 1
  fi
  resolved_version="$metadata_version"
...
select_release_assets() {
  package_asset="workx-package-$vendor_target.tar.gz"
  checksum_asset="workx-package_SHA256SUMS"
  ...
  if release_asset_exists "$package_asset" &&
    ...
    asset="$package_asset"
  elif release_asset_exists "workx-npm-$npm_tag-$resolved_version.tgz"; then
    install_layout="legacy-platform-npm"
    asset="workx-npm-$npm_tag-$resolved_version.tgz"
  else
    echo "Could not find Workx package or platform npm release assets for Workx $resolved_version." >&2
    return 1
  fi
...
package_archive_digest() {
  ...
  if [ -z "$digest" ]; then
    echo "Could not find SHA-256 digest for $asset in workx-package_SHA256SUMS." >&2
    return 1
  fi
...
file_sha256() {
  ...
  echo "sha256sum, shasum, or openssl is required to verify the Workx download." >&2
  exit 1
}
...
verify_archive_digest() {
  ...
  if [ "$actual_digest" != "$expected_digest" ]; then
    echo "Downloaded Workx archive checksum did not match expected digest." >&2
...
require_command() {
  if ! command -v "$1" >/dev/null 2>&1; then
    echo "$1 is required to install Workx." >&2
    exit 1
  fi
}
...
add_to_path() {
  ...
  begin_marker="# >>> Workx installer >>>"
  end_marker="# <<< Workx installer <<<"
...
cleanup_stale_install_artifacts() {
  ...
  if [ -d "$BIN_DIR" ]; then
    find "$BIN_DIR" -mindepth 1 -maxdepth 1 -name '.workx.*' -exec rm -f {} +
  fi
}
...
version_from_binary() {
  ...
  if [ ! -x "$workx_path" ]; then
    return 1
  fi

  "$workx_path" --version 2>/dev/null | sed -n 's/.* \([0-9][0-9A-Za-z.+-]*\)$/\1/p' | head -n 1
}

current_installed_version() {
  version="$(version_from_binary "$CURRENT_LINK/bin/workx" || true)"
  if [ -n "$version" ]; then
    printf '%s\n' "$version"
    return 0
  fi

  version="$(version_from_binary "$CURRENT_LINK/workx" || true)"
  if [ -n "$version" ]; then
    printf '%s\n' "$version"
    return 0
  fi
...
resolve_existing_workx() {
  command -v workx 2>/dev/null || true
}

classify_existing_workx() {
  existing_path="$1"
...
print_launch_instructions() {
  case "$path_action" in
    added)
      step "Current terminal: export PATH=\"$BIN_DIR:\$PATH\" && workx"
      step "Future terminals: open a new terminal and run: workx"
    ...
    configured)
      ...
      ;;
    *)
      step "Current terminal: workx"
      step "Future terminals: open a new terminal and run: workx"
      ;;
  esac
}

maybe_launch_workx_now() {
  if prompt_yes_no "Start Workx now?"; then
    step "Launching Workx"
    "$BIN_PATH"
  fi
}

detect_conflicting_install() {
  existing_path="$(resolve_existing_workx)"
  manager="$(classify_existing_workx "$existing_path" || true)"
  ...
  step "Detected existing $manager-managed Workx at $existing_path"
  warn "Multiple managed Workx installs can be ambiguous because PATH order decides which one runs."
}
...
handle_conflicting_install() {
  ...
  case "$conflict_manager" in
    brew)
      uninstall_cmd="brew uninstall --cask workx"
      ;;
    bun)
      uninstall_cmd="bun remove -g @ronanxiao/workx"
      ;;
    *)
      uninstall_cmd="npm uninstall -g @ronanxiao/workx"
      ;;
  esac

  if prompt_yes_no "Uninstall the existing $conflict_manager-managed Workx now?"; then
    step "Running: $uninstall_cmd"
    if ! sh -c "$uninstall_cmd"; then
      warn "Failed to uninstall the existing $conflict_manager-managed Workx. Continuing with the standalone install."
    fi
  else
    warn "Leaving the existing $conflict_manager-managed Workx installed. PATH order will determine which workx runs."
  fi
}
...
install_package_release() {
  ...
  chmod 0755 \
    "$stage_release/bin/workx" \
    "$stage_release/bin/workx-code-mode-host" \
    "$stage_release/workx-path/rg"
  if [ -f "$stage_release/workx-resources/bwrap" ]; then
    chmod 0755 "$stage_release/workx-resources/bwrap"
  fi
  ln -sf "bin/workx" "$stage_release/workx"
...
install_legacy_platform_npm_release() {
  ...
  mkdir -p "$stage_release/workx-resources" "$extract_dir"
  tar -xzf "$archive_path" -C "$extract_dir"

  cp "$vendor_root/workx/workx" "$stage_release/workx"
  cp "$vendor_root/path/rg" "$stage_release/workx-resources/rg"
  chmod 0755 "$stage_release/workx" "$stage_release/workx-resources/rg"
  if [ -f "$vendor_root/workx-resources/bwrap" ]; then
    cp "$vendor_root/workx-resources/bwrap" "$stage_release/workx-resources/bwrap"
    chmod 0755 "$stage_release/workx-resources/bwrap"
  fi
...
release_dir_is_complete() {
  ...
  case "$layout" in
    package)
      [ -f "$release_dir/workx-package.json" ] &&
        [ -x "$release_dir/bin/workx" ] &&
        [ -x "$release_dir/bin/workx-code-mode-host" ] &&
        [ -x "$release_dir/workx" ] &&
        [ -x "$release_dir/workx-path/rg" ] ||
        return 1
      ;;
    legacy-platform-npm)
      [ -x "$release_dir/workx" ] &&
        [ -x "$release_dir/workx-resources/rg" ] ||
        return 1
      ;;
    *)
      return 1
  ...
  case "$layout:$expected_target" in
    package:*linux* | legacy-platform-npm:*linux*)
      [ -x "$release_dir/workx-resources/bwrap" ] || return 1
      ;;
  esac

  installed_version="$(version_from_binary "$release_dir/bin/workx" || version_from_binary "$release_dir/workx" || true)"
...
release_workx_relative_path() {
  release_dir="$1"

  if [ -x "$release_dir/bin/workx" ]; then
    printf 'bin/workx\n'
  else
    printf 'workx\n'
  fi
...
update_visible_command() {
  release_dir="$1"
  mkdir -p "$BIN_DIR"
  tmp_link="$BIN_DIR/.workx.$$"
  workx_relative_path="$(release_workx_relative_path "$release_dir")"

  replace_path_with_symlink "$BIN_PATH" "$CURRENT_LINK/$workx_relative_path" "$tmp_link"

  if [ "$os" = "darwin" ] && [ -x "$release_dir/bin/workx-code-mode-host" ]; then
    replace_path_with_symlink \
      "$CODE_MODE_HOST_BIN_PATH" \
      "$CURRENT_LINK/bin/workx-code-mode-host" \
      "$tmp_link"
  elif [ "$(readlink "$CODE_MODE_HOST_BIN_PATH" 2>/dev/null || true)" = \
    "$CURRENT_LINK/bin/workx-code-mode-host" ]; then
...
if [ -n "$current_version" ] && [ "$current_version" != "$resolved_version" ]; then
  step "Updating Workx CLI from $current_version to $resolved_version"
elif [ -n "$current_version" ]; then
  step "Updating Workx CLI"
else
  step "Installing Workx CLI"
fi
...
if ! release_dir_is_complete "$release_dir" "$resolved_version" "$vendor_target" "$install_layout"; then
  echo "Installed Workx command did not report expected version $resolved_version." >&2
  exit 1
fi
...
printf 'Workx CLI %s installed successfully.\n' "$resolved_version"
maybe_launch_workx_now

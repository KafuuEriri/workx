...
class InstallShTest(unittest.TestCase):
    def test_metadata_fetch_failure_is_not_reported_as_missing_assets(self) -> None:
        ...
        self.assertEqual(
            requests,
            [
                "https://api.github.com/repos/RonanXiao/workx/releases/tags/"
                f"rust-v{VERSION}"
            ],
        )
        self.assertIn(
            f"Could not fetch GitHub release metadata for Workx {VERSION}",
            result.stderr,
        )
        self.assertNotIn("Could not find Workx package", result.stderr)
    ...
    def test_exact_release_opt_out_uses_github_metadata_once(self) -> None:
        ...
        self.assertEqual(
            ...
            [
                "https://api.github.com/repos/RonanXiao/workx/releases/tags/"
                f"rust-v{VERSION}",
                "https://github.com/RonanXiao/workx/releases/download/"
                f"rust-v{VERSION}/workx-package_SHA256SUMS",
            ],
        )
    ...
    def test_json_like_strings_and_nested_fields_do_not_define_assets(self) -> None:
        ...
        self.assertIn("/workx-npm-", requests[1])
        self.assertNotIn("workx-package_SHA256SUMS", requests[1])

    def test_macos_install_exposes_code_mode_host_beside_workx(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertTrue(all("github.com/" in url for url in requests))
            install_bin = root / "install-bin"
            current = root / "workx-home" / "packages" / "standalone" / "current"
            workx_path = install_bin / "workx"
            host_path = install_bin / "workx-code-mode-host"
            self.assertEqual(os.readlink(workx_path), str(current / "bin" / "workx"))
            self.assertEqual(
                os.readlink(host_path),
                str(current / "bin" / "workx-code-mode-host"),
            )
            self.assertTrue(os.access(host_path, os.X_OK))

    def test_releases_latest_installs_verified_package_when_opted_in(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            archive_path, checksum_path, metadata_json = create_package_release(root)
            ...
            self.assertEqual(
                ...
                [
                    ...
                    f"https://releases.openai.com/codex/releases/{VERSION}/workx-package_SHA256SUMS",
                    f"https://releases.openai.com/codex/releases/{VERSION}/workx-package-aarch64-apple-darwin.tar.gz",
                ],
            )
            ...
            "invalid_checksum_digest": json.dumps(
                {
                    ...
                    "assets": [
                        {
                            "name": "workx-package-aarch64-apple-darwin.tar.gz",
                            "digest": "sha256:" + "a" * 64,
                        },
                        {
                            "name": "workx-package_SHA256SUMS",
                            "digest": "sha256:" + "z" * 64,
                        },
                    ],
                }
                ...
                with tempfile.TemporaryDirectory() as temp_dir:
                    ...
                    self.assertEqual(
                        ...
                        [
                            ...
                            f"rust-v{VERSION}/workx-package_SHA256SUMS",
                            "https://github.com/RonanXiao/workx/releases/download/"
                            f"rust-v{VERSION}/workx-package-aarch64-apple-darwin.tar.gz",
                        ],
                    )
    ...
    def test_releases_wrong_checksum_digest_uses_github_metadata(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for release_asset in mirror_metadata["assets"]:
                if release_asset["name"] == "workx-package_SHA256SUMS":
    ...
    def test_releases_incomplete_checksum_manifest_falls_back_to_github(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            mirror_checksum_path.write_text(
                f"{'a' * 64}  workx-package-other-platform.tar.gz\n",
                encoding="utf-8",
            )
            mirror_metadata = json.loads(metadata_json)
            for release_asset in mirror_metadata["assets"]:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(
                ...
                [
                    ...
                    f"https://releases.openai.com/codex/releases/{MISMATCH_VERSION}/workx-package_SHA256SUMS",
                    f"https://releases.openai.com/codex/releases/{MISMATCH_VERSION}/workx-package-aarch64-apple-darwin.tar.gz",
                ],
            )
            self.assertIn(
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(
                ...
                [
                    ...
                    f"rust-v{VERSION}",
                    "https://github.com/RonanXiao/workx/releases/download/"
                    f"rust-v{VERSION}/workx-npm-darwin-arm64-{VERSION}.tgz",
                ],
            )
            ...
            self.assertNotIn("Downloading Workx CLI", second_result.stdout)
    ...
    *,
    ...
    fake_curl.write_text(
        textwrap.dedent(
            ...
            printf '%s\n' "$url" >>"$WORKX_TEST_REQUEST_LOG"

            case "$url" in
              https://api.github.com/*)
                if [ "$WORKX_TEST_METADATA_FAILURE" = "1" ]; then
                  echo "curl: (22) The requested URL returned error: 403" >&2
                  exit 22
                fi
                printf '%s\n' "$WORKX_TEST_METADATA_JSON"
                ;;
              https://releases.openai.com/codex/channels/latest|https://releases.openai.com/codex/releases/*/release.json)
                if [ "$WORKX_TEST_RELEASES_MODE" = "channel_failure" ]; then
                  exit 22
                fi
                printf '%s\n' "$WORKX_TEST_RELEASES_METADATA_JSON"
                ;;
              https://releases.openai.com/codex/releases/*/workx-package_SHA256SUMS)
                if [ "$WORKX_TEST_RELEASES_MODE" = "asset_fallback" ]; then
                  exit 22
                fi
                if [ "$WORKX_TEST_RELEASES_MODE" = "corrupt_assets" ] ||
                  [ "$WORKX_TEST_RELEASES_MODE" = "corrupt_checksum_and_github" ]; then
                  printf '<html>proxy error</html>\n' >"$output"
                  exit 0
                fi
                if [ -n "$WORKX_TEST_RELEASES_CHECKSUM_PATH" ]; then
                  cp "$WORKX_TEST_RELEASES_CHECKSUM_PATH" "$output"
                else
                  exit 22
                fi
                ;;
              https://releases.openai.com/codex/releases/*/workx-package-*.tar.gz)
                if [ "$WORKX_TEST_RELEASES_MODE" = "asset_fallback" ]; then
                  exit 22
                fi
                if [ "$WORKX_TEST_RELEASES_MODE" = "corrupt_assets" ]; then
                  printf '<html>proxy error</html>\n' >"$output"
                  exit 0
                fi
                if [ -n "$WORKX_TEST_ARCHIVE_PATH" ]; then
                  cp "$WORKX_TEST_ARCHIVE_PATH" "$output"
                else
                  exit 22
                fi
                ;;
              https://github.com/RonanXiao/workx/releases/download/*/workx-package_SHA256SUMS)
                if [ "$WORKX_TEST_RELEASES_MODE" = "corrupt_checksum_and_github" ]; then
                  printf '<html>proxy error</html>\n' >"$output"
                  exit 0
                fi
                if [ -n "$WORKX_TEST_CHECKSUM_PATH" ]; then
                  cp "$WORKX_TEST_CHECKSUM_PATH" "$output"
                else
                  exit 22
                fi
                ;;
              https://github.com/RonanXiao/workx/releases/download/*/workx-package-*.tar.gz)
                if [ -n "$WORKX_TEST_ARCHIVE_PATH" ]; then
                  cp "$WORKX_TEST_ARCHIVE_PATH" "$output"
                else
                  exit 22
                ...
                ;;
              https://github.com/RonanXiao/workx/releases/download/*/workx-npm-*.tgz)
                if [ -n "$WORKX_TEST_LEGACY_ARCHIVE_PATH" ]; then
                  cp "$WORKX_TEST_LEGACY_ARCHIVE_PATH" "$output"
                else
                  exit 22
                fi
                ;;
    ...
    env.update(
        {
            "WORKX_HOME": str(root / "workx-home"),
            "WORKX_INSTALL_DIR": str(root / "install-bin"),
            "WORKX_NON_INTERACTIVE": "1",
            "WORKX_RELEASE": release,
            "WORKX_TEST_ARCHIVE_PATH": str(archive_path or ""),
            "WORKX_TEST_CHECKSUM_PATH": str(checksum_path or ""),
            "WORKX_TEST_RELEASES_CHECKSUM_PATH": str(
                releases_checksum_path or checksum_path or ""
            ),
            "WORKX_TEST_LEGACY_ARCHIVE_PATH": str(legacy_archive_path or ""),
            "WORKX_TEST_METADATA_FAILURE": "1" if metadata_failure else "0",
            "WORKX_TEST_METADATA_JSON": (
            ...
            "WORKX_TEST_RELEASES_MODE": releases_mode,
            "WORKX_TEST_REQUEST_LOG": str(request_log),
            "HOME": str(home),
    ...
    if use_mirror is None:
        env.pop("WORKX_INSTALLER_USE_RELEASES_OPENAI_COM", None)
    else:
        env["WORKX_INSTALLER_USE_RELEASES_OPENAI_COM"] = (
...
def create_package_release(
    ...
    *,
    ...
    (package_dir / "workx-package.json").write_text("{}\n", encoding="utf-8")
    write_executable(
        package_dir / "bin" / "workx",
        f"#!/bin/sh\nprintf 'workx-cli {VERSION}\\n'\n",
    )
    write_executable(
        package_dir / "bin" / "workx-code-mode-host",
        "#!/bin/sh\nexit 0\n",
    )
    write_executable(package_dir / "workx-path" / "rg", "#!/bin/sh\nexit 0\n")

    asset = "workx-package-aarch64-apple-darwin.tar.gz"
    ...
    checksum_path = root / "workx-package_SHA256SUMS"
...
def create_legacy_release(root: Path) -> tuple[Path, str]:
    ...
    (vendor_dir / "workx").mkdir(parents=True)
    (vendor_dir / "path").mkdir()
    write_executable(
        vendor_dir / "workx" / "workx",
        f"#!/bin/sh\nprintf 'workx-cli {VERSION}\\n'\n",
    )
    write_executable(vendor_dir / "path" / "rg", "#!/bin/sh\nexit 0\n")

    asset = f"workx-npm-darwin-arm64-{VERSION}.tgz"
    archive_path = root / asset
    with tarfile.open(archive_path, "w:gz") as archive:
...
def legacy_release_metadata_with_decoys() -> str:
    ...
    assets = [
        {
            "metadata": {
                "name": "workx-package-x86_64-unknown-linux-musl.tar.gz",
                "digest": fake_digest,
            },
            "digest": f"sha256:{'c' * 64}",
            "name": f"workx-npm-{target}-{VERSION}.tgz",
        }
    ...
    return json.dumps(
        {
            "body": (
                f'fake: {{"name":"workx-package_SHA256SUMS","digest":"{fake_digest}"}}'
            ),
            "assets": assets,
            "tag_name": f"rust-v{VERSION}",
        },

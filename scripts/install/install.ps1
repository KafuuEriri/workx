[CmdletBinding()]
param(
    [string]$Release = $env:WORKX_RELEASE
)
...
$NonInteractive = $env:WORKX_NON_INTERACTIVE -match "^(?i:1|true|yes)$"
$DefaultPreferReleasesOpenAICom = $false
$PreferReleasesOpenAICom = if ([string]::IsNullOrWhiteSpace($env:WORKX_INSTALLER_USE_RELEASES_OPENAI_COM)) {
    $DefaultPreferReleasesOpenAICom
} else {
    $env:WORKX_INSTALLER_USE_RELEASES_OPENAI_COM -match "^(?i:1|true|yes)$"
}
...
function Assert-ValidReleaseVersion {
    ...
    if ($Version -cne "latest" -and $Version -cnotmatch "^[0-9]+\.[0-9]+\.[0-9]+(?:-alpha(?:\.[0-9]+){0,2}|-beta(?:\.[0-9]+)?)?$") {
        throw "Invalid Workx release version: $Version. Expected latest or x.y.z[-alpha[.N[.M]]|-beta[.N]]."
    }
}
...
function Resolve-ReleaseAssetSelection {
    ...
    $packageAsset = "workx-package-$Target.tar.gz"
    $checksumAsset = "workx-package_SHA256SUMS"
    $packageUrl = $null
    ...
    if ($ResolvedRelease.Source -eq "ReleasesOpenAICom") {
        ...
        $packageFallbackUrl = "https://github.com/RonanXiao/workx/releases/download/rust-v$version/$packageAsset"
        $checksumUrl = "$ReleasesBaseUri/releases/$version/$checksumAsset"
        $checksumFallbackUrl = "https://github.com/RonanXiao/workx/releases/download/rust-v$version/$checksumAsset"
    }
    ...
    $packageAsset = "workx-npm-$NpmTag-$version.tgz"
    $packageUrl = $null
    $packageFallbackUrl = $null
    if ($ResolvedRelease.Source -eq "ReleasesOpenAICom") {
        $packageUrl = "$ReleasesBaseUri/releases/$version/$packageAsset"
    ...
    if ($null -eq $packageMetadata) {
        throw "Could not find Workx package or platform npm release assets for Workx $version."
    }
...
function Test-ArchiveDigest {
    ...
    if ($actualDigest -ne $ExpectedDigest) {
        throw "Downloaded Workx archive checksum did not match expected digest. Expected $ExpectedDigest but got $actualDigest."
    }
}
...
function Get-PackageArchiveDigest {
    ...
    throw "Could not find SHA-256 digest for $AssetName in workx-package_SHA256SUMS."
}
...
function Resolve-VersionFromReleaseMetadata {
    ...
    if (-not $ReleaseMetadata.tag_name) {
        throw "Failed to resolve the latest Workx release version."
    }
...
function Resolve-ReleaseFromGitHub {
    ...
    if ($NormalizedVersion -eq "latest") {
        ...
        $metadataUri = "https://api.github.com/repos/RonanXiao/workx/releases/latest"
    } else {
        $resolvedVersion = $NormalizedVersion
        $requestedRelease = $resolvedVersion
        $metadataUri = "https://api.github.com/repos/RonanXiao/workx/releases/tags/rust-v$resolvedVersion"
    }

    try {
        $releaseMetadata = Invoke-RestMethod -Uri $metadataUri
    } catch {
        throw "Could not fetch GitHub release metadata for Workx $requestedRelease. GitHub API may be unavailable or rate limited. $($_.Exception.Message)"
    }
...
function Resolve-ReleaseFromReleases {
    ...
    try {
        ...
        if ($NormalizedVersion -ne "latest" -and $resolvedVersion -cne $NormalizedVersion) {
            throw "Release metadata version did not match requested Workx version $NormalizedVersion."
        }
...
function Get-VersionFromBinary {
    ...
    if (-not (Test-Path -LiteralPath $WorkxPath -PathType Leaf)) {
        return $null
    }

    try {
        $versionOutput = & $WorkxPath --version 2>$null
    } catch {
        return $null
    }
...
function Get-CurrentInstalledVersion {
    ...
    $standaloneVersion = Get-VersionFromBinary -WorkxPath (Join-Path $StandaloneCurrentDir "bin\workx.exe")
    if (-not [string]::IsNullOrWhiteSpace($standaloneVersion)) {
        return $standaloneVersion
    }

    $standaloneVersion = Get-VersionFromBinary -WorkxPath (Join-Path $StandaloneCurrentDir "workx.exe")
    if (-not [string]::IsNullOrWhiteSpace($standaloneVersion)) {
        return $standaloneVersion
    }
...
function Test-OldStandaloneBinLayout {
    ...
    $requiredFiles = @("workx.exe", "rg.exe")
    ...
    $knownFiles = @(
        "workx.exe",
        "rg.exe",
        "workx-command-runner.exe",
        "workx-windows-sandbox.exe",
        "workx-windows-sandbox-setup.exe"
    )
...
function Move-OldStandaloneBinIfApproved {
    ...
    Write-Step "We found an older Workx install at $VisibleBinDir"
    Write-WarningStep "To continue, Workx needs to update the install at this path."
    if (-not (Prompt-YesNo "Replace it with the current Workx setup now?")) {
...
function Add-JunctionSupportType {
    if (([System.Management.Automation.PSTypeName]'WorkxInstaller.Junction').Type) {
        return
    }
...
function Set-JunctionTarget {
    ...
    [WorkxInstaller.Junction]::SetTarget($LinkPath, $TargetPath)
}
...
function Test-PackageContentsAreComplete {
    ...
    $expectedFiles = @(
        "workx-package.json",
        "bin\workx.exe",
        "bin\workx-code-mode-host.exe",
        "workx-path\rg.exe",
        "workx-resources\workx-command-runner.exe",
        "workx-resources\workx-windows-sandbox-setup.exe"
    )
...
function Test-ReleaseIsComplete {
    ...
    switch ($Layout) {
        "Package" {
            ...
            $workxPath = Join-Path $ReleaseDir "bin\workx.exe"
        }
        "LegacyPlatformNpm" {
            if (-not (Test-LegacyPlatformNpmContentsAreComplete -PackageDir $ReleaseDir)) {
                return $false
            }
            $workxPath = Join-Path $ReleaseDir "workx.exe"
        }
        default {
            throw "Unknown Workx installer layout: $Layout"
        }
    }

    return (Split-Path -Leaf $ReleaseDir) -eq "$ExpectedVersion-$ExpectedTarget" -and
        (Get-VersionFromBinary -WorkxPath $workxPath) -ceq $ExpectedVersion
}

function Get-ExistingWorkxCommand {
    $existing = Get-Command workx -ErrorAction SilentlyContinue
    if ($null -eq $existing) {
        return $null
    }
...
function Get-ExistingWorkxManager {
    param(
        [string]$ExistingPath,
        [string]$VisibleBinDir
    )
...
function Get-ConflictingInstall {
    ...
    $existingPath = Get-ExistingWorkxCommand
    $manager = Get-ExistingWorkxManager -ExistingPath $existingPath -VisibleBinDir $VisibleBinDir
    if ($null -eq $manager) {
        return $null
    }

    Write-Step "Detected existing $manager-managed Workx at $existingPath"
    Write-WarningStep "Multiple managed Workx installs can be ambiguous because PATH order decides which one runs."
...
function Maybe-HandleConflictingInstall {
    ...
    $uninstallArgs = if ($manager -eq "bun") {
        @("remove", "-g", "@ronanxiao/workx")
    } else {
        @("uninstall", "-g", "@ronanxiao/workx")
    }
    $uninstallCommand = if ($manager -eq "bun") { "bun" } else { "npm" }

    if (Prompt-YesNo "Uninstall the existing $manager-managed Workx now?") {
        Write-Step "Running: $uninstallCommand $($uninstallArgs -join ' ')"
        try {
            & $uninstallCommand @uninstallArgs
        } catch {
            Write-WarningStep "Failed to uninstall the existing $manager-managed Workx. Continuing with the standalone install."
        }
    } else {
        Write-WarningStep "Leaving the existing $manager-managed Workx installed. PATH order will determine which workx runs."
    }
}

function Test-VisibleWorkxCommand {
    param(
        [string]$VisibleBinDir
    )

    $workxCommand = Join-Path $VisibleBinDir "workx.exe"
    & $workxCommand --version *> $null
    if ($LASTEXITCODE -ne 0) {
        throw "Installed Workx command failed verification: $workxCommand --version"
    }
}
...
if (-not [Environment]::Is64BitOperatingSystem) {
    Write-Error "Workx requires a 64-bit version of Windows."
    exit 1
}
...
$workxHome = if ([string]::IsNullOrWhiteSpace($env:WORKX_HOME)) {
    Join-Path $env:USERPROFILE ".workx"
} else {
    $env:WORKX_HOME
}
$standaloneRoot = Join-Path $workxHome "packages\standalone"
$releasesDir = Join-Path $standaloneRoot "releases"
$currentDir = Join-Path $standaloneRoot "current"
$lockPath = Join-Path $standaloneRoot "install.lock"

$defaultVisibleBinDir = Join-Path $env:LOCALAPPDATA "Programs\OpenAI\Workx\bin"
if ([string]::IsNullOrWhiteSpace($env:WORKX_INSTALL_DIR)) {
    $visibleBinDir = $defaultVisibleBinDir
} else {
    $visibleBinDir = $env:WORKX_INSTALL_DIR
}
...
if (-not [string]::IsNullOrWhiteSpace($currentVersion) -and $currentVersion -ne $resolvedVersion) {
    Write-Step "Updating Workx CLI from $currentVersion to $resolvedVersion"
} elseif (-not [string]::IsNullOrWhiteSpace($currentVersion)) {
    Write-Step "Updating Workx CLI"
} else {
    Write-Step "Installing Workx CLI"
}
...
$tempDir = Join-Path ([System.IO.Path]::GetTempPath()) ("workx-install-" + [System.Guid]::NewGuid().ToString("N"))
...
try {
    Invoke-WithInstallLock -LockPath $lockPath -Script {
        ...
        if (-not (Test-ReleaseIsComplete -ReleaseDir $releaseDir -ExpectedVersion $resolvedVersion -ExpectedTarget $target -Layout $installLayout)) {
            ...
            Write-Step "Downloading Workx CLI"
            ...
            if ($installLayout -eq "Package") {
                ...
                if (-not (Test-PackageContentsAreComplete -PackageDir $stagingDir)) {
                    throw "Downloaded Workx package archive did not contain the expected package layout."
                }
            } else {
                ...
                $resourcesDir = Join-Path $stagingDir "workx-resources"
                New-Item -ItemType Directory -Force -Path $resourcesDir | Out-Null
                $copyMap = @{
                    "workx/workx.exe" = "workx.exe"
                    "workx/workx-command-runner.exe" = "workx-resources\workx-command-runner.exe"
                    "workx/workx-windows-sandbox-setup.exe" = "workx-resources\workx-windows-sandbox-setup.exe"
                    "path/rg.exe" = "workx-resources\rg.exe"
                }
                ...
                if (-not (Test-LegacyPlatformNpmContentsAreComplete -PackageDir $stagingDir)) {
                    throw "Downloaded Workx npm archive did not contain the expected legacy platform package layout."
                }
            }
        ...
        if (-not (Test-ReleaseIsComplete -ReleaseDir $releaseDir -ExpectedVersion $resolvedVersion -ExpectedTarget $target -Layout $installLayout)) {
            throw "Installed Workx command did not report expected version $resolvedVersion."
        }
        ...
        try {
            ...
            Test-VisibleWorkxCommand -VisibleBinDir $visibleBinDir
        } catch {
...
Write-Step "Current PowerShell session: workx"
Write-Step "Future PowerShell windows: open a new PowerShell window and run: workx"
Write-Host "Workx CLI $resolvedVersion installed successfully."

$workxCommand = Join-Path $visibleBinDir "workx.exe"
if (Prompt-YesNo "Start Workx now?") {
    Write-Step "Launching Workx"
    & $workxCommand
}

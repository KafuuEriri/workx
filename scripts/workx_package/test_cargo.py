...
from workx_package.cargo import build_source_binaries
from workx_package.cargo import source_binaries_for_target
from workx_package.targets import PACKAGE_VARIANTS
from workx_package.targets import TARGET_SPECS
...
class SourceBinariesForTargetTest(unittest.TestCase):
    def test_macos_package_with_prebuilt_entrypoint_builds_nothing(self) -> None:
        self.assertEqual(
            source_binaries_for_target(
                ...
                PACKAGE_VARIANTS["workx"],
                build_entrypoint=False,
                build_code_mode_host=False,
                build_bwrap=False,
                build_workx_command_runner=False,
                build_workx_windows_sandbox_setup=False,
            ),
            [],
        )
    ...
    def test_missing_windows_helpers_are_built(self) -> None:
        self.assertEqual(
            source_binaries_for_target(
                ...
                build_code_mode_host=False,
                build_bwrap=False,
                build_workx_command_runner=True,
                build_workx_windows_sandbox_setup=True,
            ),
            ["workx-command-runner", "workx-windows-sandbox-setup"],
        )
    ...
    def test_missing_code_mode_host_is_built_for_app_server(self) -> None:
        self.assertEqual(
            source_binaries_for_target(
                TARGET_SPECS["aarch64-apple-darwin"],
                PACKAGE_VARIANTS["workx-app-server"],
                build_entrypoint=False,
    ...
    def test_build_uses_prebuilt_windows_helpers_without_running_cargo(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            entrypoint = touch_file(root / "workx.exe")
            code_mode_host = touch_file(root / "workx-code-mode-host.exe")
            command_runner = touch_file(root / "workx-command-runner.exe")
            sandbox_setup = touch_file(root / "workx-windows-sandbox-setup.exe")
            ...
            outputs = build_source_binaries(
                ...
                bwrap_bin=None,
                workx_command_runner_bin=command_runner,
                workx_windows_sandbox_setup_bin=sandbox_setup,
            )

        self.assertEqual(outputs.entrypoint_bin, entrypoint)
        self.assertEqual(outputs.code_mode_host_bin, code_mode_host)
        self.assertEqual(outputs.workx_command_runner_bin, command_runner)
        self.assertEqual(outputs.workx_windows_sandbox_setup_bin, sandbox_setup)

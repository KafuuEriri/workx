# Workx package builder

This package contains the implementation behind `scripts/build_workx_package.py`.
...
just assemble-workx-package --variant workx-app-server
just assemble-workx-package --target x86_64-unknown-linux-gnu
```

The builder creates a canonical Workx package directory:
...
│   ├── workx-command-runner.exe          # Windows only
│   └── workx-windows-sandbox-setup.exe   # Windows only
└── workx-path
    └── rg[.exe]
```
...
host platform. On Linux, that default is a musl target to match Workx release
...
`workx` and `workx-app-server`. The `--package-version` flag sets the version in
`workx-package.json`; it defaults to `[workspace.package].version` in
`workx-rs/Cargo.toml`.
...
- all targets: `workx-code-mode-host`, unless `--code-mode-host-bin` is provided
- Linux targets: `bwrap`, unless `--bwrap-bin` is provided
- Windows targets: `workx-command-runner` and `workx-windows-sandbox-setup`,
...
`--workx-command-runner-bin` plus `--workx-windows-sandbox-setup-bin` for
...
downloads and verifies the matching Workx-built V8 release pair before invoking
...
DotSlash manifest at `scripts/workx_package/rg`. Downloaded archives are cached
under `$TMPDIR/workx-package/<target>-rg` and are reused only after the recorded
...
manifest at `scripts/workx_package/workx-zsh` when the selected target has a
matching prebuilt artifact. Downloaded archives are cached under
`$TMPDIR/workx-package/<target>-zsh` and installed at
`workx-resources/zsh/bin/zsh`. Pass `--zsh-bin` to package a prebuilt, signed

"""Command-line interface for building Workx package directories."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build a canonical Workx package directory and optional archive.",
    ...
    parser.add_argument(
        ...
        help="Semantic version to record in workx-package.json.",
    )
    parser.add_argument(
        "--package-dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        help=(
            "Optional prebuilt workx-code-mode-host executable. If omitted, "
    ...
    zsh_source.add_argument(
        ...
        help=(
            ...
            "scripts/workx_package/workx-zsh."
        ),
    )
    zsh_source.add_argument(
        "--zsh-bin",
    ...
    parser.add_argument(
        "--workx-command-runner-bin",
        type=Path,
        help=(
            "Optional prebuilt Windows workx-command-runner.exe executable. "
            "If omitted for Windows targets, workx-command-runner is built "
            "with Cargo."
        ),
    )
    parser.add_argument(
        "--workx-windows-sandbox-setup-bin",
        type=Path,
        help=(
            "Optional prebuilt Windows workx-windows-sandbox-setup.exe "
            "executable. If omitted for Windows targets, "
            "workx-windows-sandbox-setup is built with Cargo."
        ),
    )
    parser.add_argument(
        "--rg-bin",
...
def main() -> int:
    ...
    package_dir = (
        ...
        else Path(tempfile.mkdtemp(prefix="workx-package-")).resolve()
    )
    ...
    source_outputs = build_source_binaries(
        ...
        workx_command_runner_bin=resolve_optional_input_path(
            args.workx_command_runner_bin,
            "prebuilt Windows workx-command-runner.exe executable",
            "--workx-command-runner-bin",
        ),
        workx_windows_sandbox_setup_bin=resolve_optional_input_path(
            args.workx_windows_sandbox_setup_bin,
            "prebuilt Windows workx-windows-sandbox-setup.exe executable",
            "--workx-windows-sandbox-setup-bin",
        ),
    )
    inputs = PackageInputs(
        ...
        workx_command_runner_bin=source_outputs.workx_command_runner_bin,
        workx_windows_sandbox_setup_bin=source_outputs.workx_windows_sandbox_setup_bin,
    )
    ...
    for archive_output in args.archive_output:
        ...
        print(f"Built Workx package archive at {archive_path}")

    print(f"Built Workx package directory at {package_dir}")
    return 0

...
class SmokePackage:
    ...
    def from_archives(
        ...
        config_dir = directory / "workx-config"
        config_dir.mkdir()
        ...
        # Isolate package configuration and state from the user's Workx setup.
        environment["WORKX_HOME"] = str(config_dir)
        ...
        for index, archive_path in enumerate((cli_archive, app_server_archive)):
            ...
            manifest = json.loads((extracted / "workx-package.json").read_text())
...
def package(
    ...
    directory = tmp_path_factory.mktemp(f"workx-package-smoke-{compression}")
...
def responses_server(package: SmokePackage) -> Iterator[MockResponsesServer]:
    config_path = Path(package.environment["WORKX_HOME"]) / "config.toml"
    original_config = config_path.read_text()
    with MockResponsesServer() as server:
        # Direct CLI commands and SDK-launched servers must share a provider;
        # WorkxConfig only controls how the SDK launches its server process.
...
def code_mode_host_debug_symbols(
    ...
    binaries = {"workx", "workx-app-server", "workx-code-mode-host"}
    if "windows" in target:
        binaries.update({"workx-command-runner", "workx-windows-sandbox-setup"})
    ...
    destination = tmp_path_factory.mktemp("workx-debug-symbols")
    found: set[str] = set()
    symbol_path = None
        ...
        for member in archive:
            ...
            for binary, marker in markers.items():
                ...
                if not matches:
                    continue
                found.add(binary)
                if binary == "workx-code-mode-host":

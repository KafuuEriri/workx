...
├── bin
│   ├── workx[.exe]                       # CLI package only
│   ├── workx-app-server[.exe]            # app-server package only
...
Each package contains one entrypoint, not both workx and workx-app-server.
"""
...
from workx import ApprovalMode, Workx, WorkxConfig, Sandbox
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(("completion", "bash"), "workx", id="completion"),
    ],
)
...
@pytest.mark.parametrize("entrypoint", ["workx", "workx-app-server"])
def test_app_server_runs_code_mode_through_python_sdk(
    ...
    with Workx(config=config) as client:
...
def _code_mode_host_symbol_address(
    ...
    *arguments: str,
    ...
    return next(
        ...
        and parts[1].lower() == "t"
        and "workx_code_mode_host" in parts[2]
    )
...
def test_linux_debug_symbols_resolve_packaged_code(
    ...
    assert "workx_code_mode_host" in resolved, resolved
...
def test_windows_debug_symbols_resolve_packaged_code(
    ...
    symbols = code_mode_host_debug_symbols.rename(
        code_mode_host_debug_symbols.with_name("workx_code_mode_host.pdb")
    )
    host = symbols.with_suffix(".exe")
    shutil.copy2(package.cli.with_name("workx-code-mode-host.exe"), host)

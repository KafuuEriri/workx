"""Canonical Workx package directory layout."""
...
def build_package_dir(
    ...
    resources_dir = package_dir / "workx-resources"
    path_dir = package_dir / "workx-path"
    bin_dir.mkdir()
    resources_dir.mkdir()
    path_dir.mkdir()
    ...
    copy_executable(
        ...
        bin_dir / f"workx-code-mode-host{spec.exe_suffix}",
        is_windows=spec.is_windows,
    )
    ...
    if inputs.workx_command_runner_bin is not None:
        copy_executable(
            inputs.workx_command_runner_bin,
            resources_dir / "workx-command-runner.exe",
            is_windows=True,
        )

    if inputs.workx_windows_sandbox_setup_bin is not None:
        copy_executable(
            inputs.workx_windows_sandbox_setup_bin,
            resources_dir / "workx-windows-sandbox-setup.exe",
            is_windows=True,
        )
    ...
    write_json(package_dir / "workx-package.json", metadata)
...
def validate_package_dir(
    ...
    *,
    ...
    metadata_path = package_dir / "workx-package.json"
    if not metadata_path.is_file():
        raise RuntimeError("Missing package metadata: workx-package.json")
    ...
    required_files = [
        ...
        Path("bin") / f"workx-code-mode-host{spec.exe_suffix}",
        Path("workx-path") / spec.rg_name,
    ]
    executable_files = list(required_files)

    if include_zsh:
        zsh_path = Path("workx-resources") / ZSH_RESOURCE_PATH
        required_files.append(zsh_path)
        executable_files.append(zsh_path)

    if spec.is_linux:
        required_files.append(Path("workx-resources") / "bwrap")
        executable_files.append(Path("workx-resources") / "bwrap")

    if spec.is_windows:
        required_files.extend(
            [
                Path("workx-resources") / "workx-command-runner.exe",
                Path("workx-resources") / "workx-windows-sandbox-setup.exe",
            ]
        )

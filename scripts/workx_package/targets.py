...
_repo_root = os.environ.get("WORKX_REPO_ROOT")
if _repo_root is None:
    raise RuntimeError(
        "WORKX_REPO_ROOT must point to the repository root; "
        "run `just assemble-workx-package` to set it automatically"
    )
...
PACKAGE_VARIANTS: dict[str, PackageVariant] = {
    "workx": PackageVariant(
        name="workx",
        cargo_bin="workx",
        executable_stem="workx",
    ),
    "workx-app-server": PackageVariant(
        name="workx-app-server",
        cargo_bin="workx-app-server",
        executable_stem="workx-app-server",
    ),
}

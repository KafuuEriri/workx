...
from workx_package.zsh import resolve_zsh_bin
...
class ResolveZshBinTest(unittest.TestCase):
    def test_uses_prebuilt_executable_override(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch("workx_package.zsh.fetch_dotslash_executable") as fetch:
    ...
    def test_uses_manifest_override(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            archive = root / "workx-zsh.tar.gz"
            source = root / "zsh"
            source.write_bytes(b"standalone zsh")
            with tarfile.open(archive, "w:gz") as tar:
                tar.add(source, arcname="workx-zsh/bin/zsh")

            manifest = root / "workx-zsh"
            manifest.write_text(
                json.dumps(
                    {
                        "platforms": {
            ...
            with patch(
                "workx_package.dotslash.default_cache_root",

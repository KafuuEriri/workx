#!/usr/bin/env python3
"""Stage one or more Workx npm packages for release."""
...
BUILD_SCRIPT = REPO_ROOT / "workx-cli" / "scripts" / "build_npm_package.py"
WORKFLOW_NAME = ".github/workflows/rust-release.yml"
GITHUB_REPO = "RonanXiao/workx"
BINARY_TARGETS = (
...
_SPEC = importlib.util.spec_from_file_location("workx_build_npm_package", BUILD_SCRIPT)
...
WORKX_PLATFORM_PACKAGES = getattr(_BUILD_MODULE, "WORKX_PLATFORM_PACKAGES", {})
WORKX_PACKAGE_COMPONENT = getattr(
    _BUILD_MODULE, "WORKX_PACKAGE_COMPONENT", "workx-package"
)
...
BINARY_COMPONENTS = {
    "workx-responses-api-proxy": BinaryComponent(
        artifact_prefix="workx-responses-api-proxy",
        dest_dir="workx-responses-api-proxy",
        binary_basename="workx-responses-api-proxy",
    ),
}
...
def install_from_workflow_artifacts(
    ...
    if WORKX_PACKAGE_COMPONENT in components:
        install_workx_package_archives(artifacts_dir, vendor_dir, BINARY_TARGETS)
...
def select_target_artifacts(
    ...
    needs_target_artifacts = WORKX_PACKAGE_COMPONENT in components or any(
...
def install_workx_package_archives(
    artifacts_dir: Path,
    vendor_dir: Path,
    ...
    print(
        "Installing Workx package archives for targets: " + ", ".join(targets),
        flush=True,
    )
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {
            executor.submit(
                install_single_workx_package_archive,
                artifacts_dir,
                vendor_dir,
                target,
            ): target
...
def install_single_workx_package_archive(
    artifacts_dir: Path,
    vendor_dir: Path,
    target: str,
) -> Path:
    artifact_subdir = artifact_dir_for_target(artifacts_dir, target)
    archive_path = artifact_subdir / f"workx-package-{target}.tar.gz"
...
def tarball_name_for_package(package: str, version: str) -> str:
    if package in WORKX_PLATFORM_PACKAGES:
        platform = package.removeprefix("workx-")
        return f"workx-npm-{platform}-{version}.tgz"

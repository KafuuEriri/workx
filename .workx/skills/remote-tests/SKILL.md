...
Use `TestWorkxBuilder::build_with_auto_env()` to opt-in to remote execution in core integration
...
`$WORKX_HOME/environments.toml` or will define custom environments at runtime.
...
in bash also provides the `workx_remote_env_cleanup` function to use after testing.
...
bash -c '
  ...
  trap workx_remote_env_cleanup EXIT

  cd workx-rs
  just test -p workx-core --test all
'
```
...
bash -c '
  ...
  cd workx-rs
  just test -p workx-app-server --test all
'
```
...
```sh
bazel test //workx-rs/app-server:app-server-all-wine-exec-test
```
...
You can list devboxes via `applied_devbox ls`, pick the one with `workx` in the name.
Connect to devbox via `ssh <devbox_name>`.
Reuse the same checkout of workx in `~/code/workx`. Reset files if needed. Multiple checkouts take longer to build and take up more space.

...
"""Verify that workx-rs Cargo manifests follow workspace manifest policy.

Checks:
- Crates inherit `[workspace.package]` metadata.
- Crates opt into `[lints] workspace = true`.
- Crate names follow the workx-rs directory naming conventions.
...
def main() -> int:
    ...
    print(
        "Cargo manifests under workx-rs must inherit workspace package metadata, "
    ...
    print(
        "Cargo only applies `workx-rs/Cargo.toml` `[workspace.lints.clippy]` "
    ...
    print()
    print(
        "Package-name checks apply to `workx-rs/<crate>/Cargo.toml` and "
        "`workx-rs/utils/<crate>/Cargo.toml`."
    )
    print(
...
def expected_package_name(path: Path) -> str | None:
    ...
    if len(parts) == 2 and parts[1] == "Cargo.toml":
        ...
        return TOP_LEVEL_NAME_EXCEPTIONS.get(
            directory,
            directory if directory.startswith("workx-") else f"workx-{directory}",
        )
    if len(parts) == 3 and parts[0] == "utils" and parts[2] == "Cargo.toml":
        directory = parts[1]
        return UTILITY_NAME_EXCEPTIONS.get(directory, f"workx-utils-{directory}")
    return None

...
publishes the result via `GITHUB_ENV` as `WORKX_BAZEL_WINDOWS_PATH` so later
...
Write-Host 'Derived WORKX_BAZEL_WINDOWS_PATH entries:'
foreach ($pathEntry in $stablePathEntries) {
  Write-Host "  $pathEntry"
}
"WORKX_BAZEL_WINDOWS_PATH=$stablePath" | Out-File -FilePath $env:GITHUB_ENV -Encoding utf8 -Append

#!/usr/bin/env python3
"""Mirror a Workx GitHub Release to Cloudflare R2.
...
Objects are created under ``workx/releases/<version>/`` with a validated upload
...
objects are verified, the same metadata advances ``workx/channels/latest`` when
the release is marked latest and ``workx/channels/prerelease`` for prereleases.
Stable releases also update the mutable ``workx/install.sh`` and
``workx/install.ps1`` bootstrap aliases from their verified versioned assets.
"""

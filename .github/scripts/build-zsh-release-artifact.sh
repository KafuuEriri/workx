...
work_root="$(mktemp -d "${temp_root%/}/workx-zsh-release.XXXXXX")"
trap 'rm -rf "$work_root"' EXIT

source_root="${work_root}/zsh"
package_root="${work_root}/workx-zsh"
...
#!/usr/bin/env bash
set -euo pipefail
: "${WORKX_WRAPPER_LOG:?missing WORKX_WRAPPER_LOG}"
printf '%s\n' "$@" > "$WORKX_WRAPPER_LOG"
file="$1"
shift
if [[ "$#" -eq 0 ]]; then
...
WORKX_WRAPPER_LOG="$wrapper_log_path" \
...
(cd "$work_root" && tar -czf "${workspace}/${archive_path}" workx-zsh)

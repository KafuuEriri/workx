...
usage() {
  cat <<'EOF'
Usage: build-workx-package-archive.sh \
  --target <rust-target> \
  ...
  [--zsh-bin <path>] \
  [--zsh-manifest <path>] \
  [--workx-command-runner-bin <path>] \
  [--workx-windows-sandbox-setup-bin <path>] \
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --workx-command-runner-bin)
      resource_args+=(
        --workx-command-runner-bin
        "${2:?--workx-command-runner-bin requires a value}"
      )
    ...
    --workx-windows-sandbox-setup-bin)
      resource_args+=(
        --workx-windows-sandbox-setup-bin
        "${2:?--workx-windows-sandbox-setup-bin requires a value}"
      )
...
case "$bundle" in
  primary)
    variant="workx"
    entrypoint="workx"
    archive_stem="workx-package"
    ;;
  app-server)
    variant="workx-app-server"
    entrypoint="workx-app-server"
    archive_stem="workx-app-server-package"
    ;;
  *)
    echo "No Workx package variant for bundle: $bundle" >&2
    exit 1
    ;;
esac
...
code_mode_host_bin="${entrypoint_dir%/}/workx-code-mode-host${exe_suffix}"
...
case "$target" in
  *linux*)
    ...
    ;;
  *windows*)
    command_runner_bin="${entrypoint_dir%/}/workx-command-runner.exe"
    sandbox_setup_bin="${entrypoint_dir%/}/workx-windows-sandbox-setup.exe"
    if [[ "$command_runner_bin_provided" == "false" && -f "$command_runner_bin" ]]; then
      resource_args+=(--workx-command-runner-bin "$command_runner_bin")
    fi
    if [[ "$sandbox_setup_bin_provided" == "false" && -f "$sandbox_setup_bin" ]]; then
      resource_args+=(--workx-windows-sandbox-setup-bin "$sandbox_setup_bin")
    fi
    ;;
esac
...
python_args=(
  "${repo_root}/scripts/build_workx_package.py"
  --target "$target"
  --variant "$variant"

...
use workx_exec_server_test_support::relay::TEST_TIMEOUT as VERSION_SKEW_TIMEOUT;
use workx_exec_server_test_support::relay::accept_websocket;
use workx_exec_server_test_support::relay::assert_relay_data_is_encrypted;
use workx_exec_server_test_support::relay::proxy_relay_frames;
use workx_exec_server_test_support::relay::registered_executor_public_key;
...
const RELEASED_WORKX_ENV_VAR: &str = "WORKX_TEST_RELEASED_WORKX";
const CURRENT_WORKX_ENV_VAR: &str = "WORKX_TEST_CURRENT_WORKX";
const EXECUTOR_MARKER_ENV_VAR: &str = "WORKX_EXECUTOR_VERSION_SKEW_MARKER";
...
fn version_skew_binaries() -> Result<Option<(PathBuf, PathBuf)>> {
    let Some(released) = std::env::var_os(RELEASED_WORKX_ENV_VAR) else {
        return Ok(None);
    };
    let current = std::env::var_os(CURRENT_WORKX_ENV_VAR)
        .with_context(|| format!("{CURRENT_WORKX_ENV_VAR} must name the current Workx binary"))?;
    let current = PathBuf::from(current);
    let released = PathBuf::from(released);
    anyhow::ensure!(
        current.is_file(),
        "current Workx does not exist: {}",
        current.display()
    );
    anyhow::ensure!(
        released.is_file(),
        "released Workx does not exist: {}",
        released.display()
    );
...
async fn assert_noise_version_skew(app_binary: &Path, executor_binary: &Path) -> Result<()> {
    ...
    let model = mock_model(workx_home.path()).await?;
...
"#
    ...
    let mut executor = Command::new(executor_binary)
        ...
        .current_dir(workx_home.path())
        .env("WORKX_HOME", workx_home.path())
        .env("WORKX_API_KEY", REGISTRY_TOKEN)
    ...
    let mut app_server = Command::new(app_binary)
        ...
        .env(WORKX_EXEC_SERVER_NOISE_REGISTRY_URL_ENV_VAR, &registry_url)
        .env(
            WORKX_EXEC_SERVER_NOISE_ENVIRONMENT_ID_ENV_VAR,
            ENVIRONMENT_ID,
        )
        .env(WORKX_EXEC_SERVER_NOISE_AUTH_TOKEN_ENV_VAR, REGISTRY_TOKEN)
        .env_remove(WORKX_EXEC_SERVER_URL_ENV_VAR)
    ...
    assert_eq!(
        std::fs::read_to_string(workx_home.path().join("version-skew-output.txt"))?,
        EXPECTED_OUTPUT
    );
...
async fn mock_model(workx_home: &Path) -> Result<MockServer> {

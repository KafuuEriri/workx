...
targeted package runs such as `just argument-comment-lint -p workx-core`.
...
```bash
./tools/argument-comment-lint/run.py -p workx-core
```

Run the lint against `workx-rs` from the repo root:

```bash
just argument-comment-lint
bazel build --config=argument-comment-lint -- \
  $(./tools/argument-comment-lint/list-bazel-targets.sh)
./tools/argument-comment-lint/run-prebuilt-linter.py -p workx-core
just argument-comment-lint -p workx-core
```

If no package selection is provided, `just argument-comment-lint` now defaults
to the Bazel aspect path over `//workx-rs/...`. The Python wrappers remain the
...
```bash
./tools/argument-comment-lint/run-prebuilt-linter.py -p workx-core -- --all-targets
```

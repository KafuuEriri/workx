...
./scripts/stage_npm_packages.py \
  ...
  --package workx \
  --package workx-responses-api-proxy \
  --package workx-sdk
```
...
When `--package workx` is provided, the staging helper builds the lightweight
`@ronanxiao/workx` meta package plus all platform-native `@ronanxiao/workx` variants

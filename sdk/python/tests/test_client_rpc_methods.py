...
from workx.client import WorkxClient, _params_dict
...
from workx.models import Notification, UnknownNotification
from workx.types import ThreadSource
...
def test_generated_v2_bundle_has_single_shared_plan_type_definition() -> None:
    source = (ROOT / "src" / "workx" / "generated" / "v2_all.py").read_text()
    assert source.count("class PlanType(") == 1


def test_plan_type_accepts_business_prolite_from_newer_runtime() -> None:
    """New runtime plan values should remain typed when using a workx_bin override."""

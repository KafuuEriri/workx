...
def _app_server_config(harness: AppServerHarness) -> WorkxConfig:
...
def test_api_key_login_authenticates_follow_up_model_requests(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path, requires_openai_auth=True) as harness:
        ...
        with Workx(config=_app_server_config(harness)) as workx:
            workx.login_api_key("sk-sdk-login-test")
            result = workx.thread_start().run("prove api key auth")
...
def test_chatgpt_token_login_authenticates_follow_up_model_requests(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path, requires_openai_auth=True) as harness:
        ...
        with Workx(config=_app_server_config(harness)) as workx:
            login = workx._client.account_login_start(
                LoginAccountParams(
            ...
            result = workx.thread_start().run("prove chatgpt token auth")
            request = harness.responses.single_request()

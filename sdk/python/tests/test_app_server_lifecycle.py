...
from workx import AsyncWorkx, Workx
...
def test_sync_and_async_initialization_round_trip_metadata(tmp_path) -> None:
    ...
    async def async_scenario(harness: AppServerHarness) -> dict[str, object]:
        async with AsyncWorkx(config=harness.app_server_config()) as workx:
            thread = await workx.thread_start()
            server = workx.metadata.serverInfo
            return {
                "thread_id": thread.id,
                "user_agent": workx.metadata.userAgent,
...
def test_thread_list_filters_archived_threads(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with Workx(config=harness.app_server_config()) as workx:
            active_thread = workx.thread_start()
            archived_thread = workx.thread_start()
            active_thread.run("keep this listed")
            archived_thread.run("archive this")
            workx.thread_archive(archived_thread.id)
            active_list = workx.thread_list(archived=False)
            archived_list = workx.thread_list(archived=True)
    ...
    async def scenario() -> None:
        ...
        with AppServerHarness(tmp_path) as harness:
            ...
            async with AsyncWorkx(config=harness.app_server_config()) as workx:
                ...
                resumed = await workx.thread_resume(thread.id)
                forked = await workx.thread_fork(thread.id)
                archive_response = await workx.thread_archive(thread.id)
                unarchived = await workx.thread_unarchive(thread.id)
...
def test_thread_fork_returns_distinct_thread(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with Workx(config=harness.app_server_config()) as workx:
            ...
            forked = workx.thread_fork(thread.id)
...
def test_archive_unarchive_round_trip_uses_materialized_rollout(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with Workx(config=harness.app_server_config()) as workx:
            ...
            archived = workx.thread_archive(thread.id)
            unarchived = workx.thread_unarchive(thread.id)

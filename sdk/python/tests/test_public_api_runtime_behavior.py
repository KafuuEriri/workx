...
import workx.api as public_api_module
from workx.api import (
    ApprovalMode,
    AsyncWorkx,
    Sandbox,
    Workx,
)
from workx.generated.v2_all import TurnStartParams
from workx.models import InitializeResponse
...
def test_workx_init_failure_closes_client(monkeypatch: pytest.MonkeyPatch) -> None:
    closed: list[bool] = []
    ...
    monkeypatch.setattr(public_api_module, "WorkxClient", FakeClient)
...
def test_async_workx_init_failure_closes_client() -> None:
    async def scenario() -> None:
        ...
        workx._client.start = fake_start  # type: ignore[method-assign]
        workx._client.initialize = fake_initialize  # type: ignore[method-assign]
        workx._client.close = fake_close  # type: ignore[method-assign]

        with pytest.raises(RuntimeError, match="missing required metadata"):
            await workx.models()

        assert close_calls == 1
        assert workx._initialized is False
        assert workx._init is None

    asyncio.run(scenario())


def test_async_workx_initializes_only_once_under_concurrency() -> None:
    async def scenario() -> None:
        workx = AsyncWorkx()
        start_calls = 0
        initialize_calls = 0
        ...
        async def fake_initialize() -> InitializeResponse:
            ...
            return InitializeResponse.model_validate(
                {
                    "userAgent": "workx-cli/1.2.3",
                    "serverInfo": {"name": "workx-cli", "version": "1.2.3"},
                }
            )
        ...
        workx._client.start = fake_start  # type: ignore[method-assign]
        workx._client.initialize = fake_initialize  # type: ignore[method-assign]
        workx._client.model_list = fake_model_list  # type: ignore[method-assign]

        await asyncio.gather(workx.models(), workx.models())

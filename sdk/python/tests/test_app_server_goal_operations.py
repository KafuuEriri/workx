...
from workx._goal import _GoalNotificationStream
from workx._run import _collect_turn_result
from workx.generated.notification_registry import notification_turn_id
from workx.generated.v2_all import TurnStatus
...
def test_private_goal_operation_coalesces_runtime_continuations(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with Workx(config=harness.app_server_config()) as workx:
            ...
            state, turn_id = workx._client.start_goal_operation(  # noqa: SLF001
                thread.id,
            ...
            stream = _GoalNotificationStream(
                ...
                lambda: workx._client.unregister_goal_operation(state),  # noqa: SLF001
                lambda: workx._client.cancel_goal_operation(state),  # noqa: SLF001
            )
            events = list(stream)
            result = _collect_turn_result(iter(events), turn_id=turn_id)
            routes = workx._client._router._goal_operations.copy()  # noqa: SLF001

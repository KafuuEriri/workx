...
class AsyncWorkxClient:
    """Async wrapper around WorkxClient using thread offloading."""

    def __init__(self, config: WorkxConfig | None = None) -> None:
        """Create the wrapped sync client that owns the transport process."""
        self._sync = WorkxClient(config=config)

    async def __aenter__(self) -> "AsyncWorkxClient":
        """Start the Workx process when entering an async context."""
        await self.start()
        return self

    async def __aexit__(self, _exc_type, _exc, _tb) -> None:
        """Close the Workx process when leaving an async context."""
        await self.close()

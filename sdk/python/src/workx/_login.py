...
from .client import WorkxClient
...
class _AsyncLoginOwner(Protocol):
    """Subset of AsyncWorkx needed by async login handles."""

    _client: AsyncWorkxClient

    async def _ensure_initialized(self) -> None:
        """Ensure the owning SDK client has a live Workx connection."""
        ...


def start_chatgpt_login(client: WorkxClient) -> ChatgptLoginHandle:
...
def start_device_code_login(client: WorkxClient) -> DeviceCodeLoginHandle:
...
class ChatgptLoginHandle:
    """Live browser-login attempt returned by `Workx.login_chatgpt()`."""
...
class DeviceCodeLoginHandle:
    """Live device-code login attempt returned by `Workx.login_chatgpt_device_code()`."""
...
class AsyncChatgptLoginHandle:
    """Live browser-login attempt returned by `AsyncWorkx.login_chatgpt()`."""

    _workx: _AsyncLoginOwner
    login_id: str
    auth_url: str
    ...
    async def wait(self) -> AccountLoginCompletedNotification:
        ...
        return await self._workx._client.wait_for_login_completed(self.login_id)

    async def cancel(self) -> CancelLoginAccountResponse:
        """Cancel this browser login attempt."""
        await self._workx._ensure_initialized()
        return await self._workx._client.account_login_cancel(self.login_id)


@dataclass(slots=True)
class AsyncDeviceCodeLoginHandle:
    """Live device-code attempt returned by `AsyncWorkx.login_chatgpt_device_code()`."""

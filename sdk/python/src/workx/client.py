...
from .errors import InvalidRequestError, TransportClosedError, WorkxError
...
RUNTIME_PKG_NAME = "workx-cli-bin"
...
def _installed_workx_path() -> Path:
    try:
        from workx_cli_bin import bundled_workx_path
    except ImportError as exc:
        raise FileNotFoundError(
            "Unable to locate the pinned Workx runtime. Install the published SDK build "
            f"with its {RUNTIME_PKG_NAME} dependency, or set WorkxConfig.workx_bin "
            "explicitly."
        ) from exc

    return bundled_workx_path()


def _installed_workx_path_dirs() -> tuple[Path, ...]:
    try:
        from workx_cli_bin import bundled_path_dir
...
class WorkxBinResolverOps:
    installed_workx_path: Callable[[], Path]
    path_exists: Callable[[Path], bool]


def _default_workx_bin_resolver_ops() -> WorkxBinResolverOps:
    return WorkxBinResolverOps(
        installed_workx_path=_installed_workx_path,
        path_exists=lambda path: path.exists(),
    )


def resolve_workx_bin(config: "WorkxConfig", ops: WorkxBinResolverOps) -> Path:
    if config.workx_bin is not None:
        workx_bin = Path(config.workx_bin)
        if not ops.path_exists(workx_bin):
            raise FileNotFoundError(
                f"Workx binary not found at {workx_bin}. Set WorkxConfig.workx_bin "
                "to a valid binary path."
            )
        return workx_bin

    return ops.installed_workx_path()


def _resolve_workx_bin(config: "WorkxConfig") -> Path:
    return resolve_workx_bin(config, _default_workx_bin_resolver_ops())


@dataclass(slots=True)
class WorkxConfig:
    """Configuration for launching and identifying the local Workx runtime.

    Most callers can use ``Workx()`` without configuration. Set ``workx_bin``
    only when intentionally using a specific local Workx executable.
    """

    workx_bin: str | None = None
    launch_args_override: tuple[str, ...] | None = None
    config_overrides: tuple[str, ...] = ()
    cwd: str | None = None
    env: dict[str, str] | None = None
    client_name: str = "workx_python_sdk"
    client_title: str = "Workx Python SDK"
    client_version: str = SDK_VERSION
    experimental_api: bool = True


class WorkxClient:
    """Synchronous typed JSON-RPC client for `workx app-server` over stdio."""

    def __init__(
        self,
        config: WorkxConfig | None = None,
        approval_handler: ApprovalHandler | None = None,
    ) -> None:
        self.config = config or WorkxConfig()
    ...
    def __enter__(self) -> "WorkxClient":
        self.start()
        return self
    ...
    def start(self) -> None:
        ...
        if self.config.launch_args_override is not None:
            ...
            workx_bin = _resolve_workx_bin(self.config)
            if self.config.workx_bin is None:
                path_dirs = _installed_workx_path_dirs()
            args = [str(workx_bin)]
    ...
    def request(
        ...
        *,
        ...
        if not isinstance(result, dict):
            raise WorkxError(f"{method} response must be a JSON object")
    ...
    def _write_message(self, payload: JsonObject) -> None:
        if self._proc is None or self._proc.stdin is None:
            raise TransportClosedError("Workx process is not running")
        with self._lock:
    ...
    def _read_message(self) -> dict[str, JsonValue]:
        ...
        if not line:
            raise TransportClosedError(
                f"Workx process closed stdout. stderr_tail={self._stderr_tail()[:2000]}"
            )

        try:
            message = json.loads(line)
        except json.JSONDecodeError as exc:
            raise WorkxError(f"Invalid JSON-RPC line: {line!r}") from exc

        if not isinstance(message, dict):
            raise WorkxError(f"Invalid JSON-RPC payload: {message!r}")
        return message


def default_workx_home() -> str:
    return str(Path.home() / ".workx")

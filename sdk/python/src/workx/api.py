...
from .async_client import AsyncWorkxClient
from .client import WorkxClient, WorkxConfig
...
class Workx:
    """Synchronous client for creating threads and running Workx turns.

    The client starts its runtime connection during construction. Use it as a
    context manager so resources are closed promptly.
    """

    def __init__(self, config: WorkxConfig | None = None) -> None:
        self._client = WorkxClient(config=config)
        try:
            self._client.start()
    ...
    def login_api_key(self, api_key: str) -> None:
        """Authenticate Workx with an API key."""
    ...
    def account(self, *, refresh_token: bool = False) -> GetAccountResponse:
        """Read the current Workx account state."""
        return self._client.account_read(GetAccountParams(refresh_token=refresh_token))

    def logout(self) -> None:
        """Clear the current Workx account session."""
        self._client.account_logout()

    # BEGIN GENERATED: Workx.flat_methods
    def thread_start(
        self,
        *,
...
class AsyncWorkx:
    ...
    def metadata(self) -> InitializeResponse:
        if self._init is None:
            raise RuntimeError(
                "AsyncWorkx is not initialized yet. Prefer `async with AsyncWorkx()`; "
    ...
    # BEGIN GENERATED: AsyncWorkx.flat_methods
    ...
    # END GENERATED: AsyncWorkx.flat_methods
...
class AsyncThread:
    ...
    async def read(self, *, include_turns: bool = False) -> ThreadReadResponse:
        """Read this thread, optionally including its turn history."""
        await self._workx._ensure_initialized()
        return await self._workx._client.thread_read(self.id, include_turns=include_turns)

    async def set_name(self, name: str) -> ThreadSetNameResponse:
        await self._workx._ensure_initialized()
        return await self._workx._client.thread_set_name(self.id, name)

    async def compact(self) -> ThreadCompactStartResponse:
        await self._workx._ensure_initialized()
        return await self._workx._client.thread_compact(self.id)
...
class AsyncTurnHandle:
    ...
    async def steer(self, input: RunInput) -> TurnSteerResponse:
        """Send additional input to this active turn."""
        await self._workx._ensure_initialized()
        return await self._workx._client.turn_steer(
            self.thread_id,
            self.id,
            _to_wire_input(_normalize_run_input(input)),
        )
    ...
    async def interrupt(self) -> TurnInterruptResponse:
        """Request interruption of this active turn."""
        await self._workx._ensure_initialized()
        return await self._workx._client.turn_interrupt(self.thread_id, self.id)

    async def stream(self) -> AsyncIterator[Notification]:
        """Yield only notifications routed to this async turn handle."""
        await self._workx._ensure_initialized()
        self._workx._client.register_turn_notifications(self.id)
        try:
            while True:
                event = await self._workx._client.next_turn_notification(self.id)
                yield event
                if (
                    event.method == "turn/completed"
                    and isinstance(event.payload, TurnCompletedNotification)
            ...
            self._workx._client.unregister_turn_notifications(self.id)

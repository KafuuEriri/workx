...
`workx` publishes stable releases. Install the latest one with
`pip install workx`.

## Why does the SDK install a runtime package?

The SDK version tracks the corresponding Workx CLI release. Each SDK release
...
- `Workx` is the sync public API.
- `AsyncWorkx` is an async replica of the same public API shape.
- Prefer `async with AsyncWorkx()` for async code. It is the standard path for
  explicit startup/shutdown, and `AsyncWorkx` initializes lazily on context
  entry or first awaited API use.

If your app is not already async, stay with `Workx`.
...
When `sandbox=` is omitted, Workx uses its configured default. A turn
...
`Workx()` is eager: it starts transport and calls `initialize` in `__init__`.

Common causes:

- installation is incomplete and the pinned `workx-cli-bin` dependency is missing
- local `workx_bin` override points to a missing file
- a custom local Workx executable does not support the SDK operation being used

...
from release_version import normalize_workx_version  # noqa: E402

SDK_DISTRIBUTION_NAME = "workx"
RUNTIME_DISTRIBUTION_NAME = "workx-cli-bin"
RUNTIME_PACKAGE_ROOT = Path("src") / "workx_cli_bin"
WORKX_PACKAGE_METADATA = "workx-package.json"
...
def schema_bundle_path(schema_dir: Path) -> Path:
    ...
    return schema_dir / "workx_app_server_protocol.v2.schemas.json"
...
def runtime_binary_name() -> str:
    return "workx.exe" if _is_windows() else "workx"


def runtime_code_mode_host_name() -> str:
    return "workx-code-mode-host.exe" if _is_windows() else "workx-code-mode-host"
...
def pinned_runtime_version() -> str:
    ...
    return normalize_workx_version(pins[0])


def pinned_runtime_workx_path() -> Path:
    """Return the bundled Workx binary from the installed pinned runtime wheel."""
    ...
    normalized_installed_version = normalize_workx_version(installed_version)
    ...
    try:
        from workx_cli_bin import bundled_workx_path
    except ImportError as exc:
        raise RuntimeError(
            f"Installed {RUNTIME_DISTRIBUTION_NAME} package does not expose bundled_workx_path."
        ) from exc

    workx_path = bundled_workx_path()
    if not workx_path.exists():
        raise RuntimeError(f"Pinned Workx runtime binary not found at {workx_path}.")
    return workx_path
...
def stage_python_sdk_package(staging_dir: Path, sdk_version: str) -> Path:
    package_version = normalize_workx_version(sdk_version)
    _copy_package_tree(sdk_root(), staging_dir)
    sdk_bin_dir = staging_dir / "src" / "workx" / "bin"
...
def stage_python_runtime_package(
    ...
    package_version = normalize_workx_version(workx_version)
    ...
    _extract_workx_package_archive(package_archive, staged_runtime_package_root(staging_dir))
    return staging_dir


def _extract_workx_package_archive(package_archive: Path, runtime_package_root: Path) -> None:
    if not package_archive.name.endswith(".tar.gz"):
        raise RuntimeError(f"Expected a .tar.gz Workx package archive: {package_archive}")
    ...
    _validate_workx_package_layout(runtime_package_root, package_archive)


def _validate_workx_package_layout(package_dir: Path, package_archive: Path) -> None:
    missing_entries = []
    if not (package_dir / WORKX_PACKAGE_METADATA).is_file():
        missing_entries.append(WORKX_PACKAGE_METADATA)
    for entry in ("bin", "workx-resources", "workx-path"):
    ...
    if missing_entries:
        ...
        raise RuntimeError(f"Missing Workx package layout entries in {package_archive}: {missing}")
...
def generate_schema_from_pinned_runtime(schema_dir: Path) -> Path:
    ...
    workx_path = pinned_runtime_workx_path()
...
def generate_v2_all(schema_dir: Path) -> None:
    ...
    out_path = sdk_root() / "src" / "workx" / "generated" / "v2_all.py"
...
def _notification_specs(schema_dir: Path) -> list[tuple[str, str]]:
    ...
    generated_source = (sdk_root() / "src" / "workx" / "generated" / "v2_all.py").read_text()
...
def generate_notification_registry(schema_dir: Path) -> None:
    ...
    out = sdk_root() / "src" / "workx" / "generated" / "notification_registry.py"
...
def _load_public_fields(
    ...
    if module_name == "workx.generated.v2_all":
...
def _load_generated_v2_all_module() -> types.ModuleType:
    ...
    module_name = "_workx_generated_v2_all_for_artifacts"
    sys.modules.pop(module_name, None)
    module_path = sdk_root() / "src" / "workx" / "generated" / "v2_all.py"
...
def _render_workx_block(
    ...
    lines = [
        ...
        *_kw_signature_lines(thread_start_fields),
        "    ) -> Thread:",
        '        """Create a new Workx conversation thread."""',
...
def _render_async_thread_block(
    ...
    lines = [
        ...
        *_kw_signature_lines(turn_fields),
        "    ) -> AsyncTurnHandle:",
        '        """Start a turn and return a handle for streaming or control."""',
        "        await self._workx._ensure_initialized()",
        "        wire_input = _to_wire_input(_normalize_run_input(input))",
        _approval_mode_assignment_line("_approval_mode_override_settings"),
        "        params = TurnStartParams(",
        "            thread_id=self.id,",
        ...
        *_model_arg_lines(turn_fields),
        "        )",
        "        turn = await self._workx._client.turn_start(",
        "            self.id,",
        "            wire_input,",
        "            params=params,",
        "        )",
        "        return AsyncTurnHandle(self._workx, self.id, turn.turn.id)",
    ]
    return "\n".join(lines)
...
def generate_public_api_flat_methods() -> None:
    ...
    public_api_path = src_dir / "workx" / "api.py"
    ...
    source = _replace_generated_block(
        source,
        "AsyncWorkx.flat_methods",
        _render_async_workx_block(
            thread_start_fields,
            thread_list_fields,
            thread_resume_fields,
            thread_fork_fields,
...
def generate_types() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="workx-python-schema-") as td:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    stage_runtime_parser.add_argument(
        "package_archive",
        type=Path,
        help="Path to a Workx package .tar.gz archive for this platform.",
    )
    stage_runtime_parser.add_argument(
        "--workx-version",
        required=True,
        help=(
            "Workx release version to write into the staged runtime package. "
...
def run_command(args: argparse.Namespace, ops: CliOps) -> None:
    if args.command == "generate-types":
        ...
        ops.stage_python_sdk_package(
            args.staging_dir,
            normalize_workx_version(args.sdk_version),
        )
    elif args.command == "stage-runtime":
        ops.stage_python_runtime_package(
            args.staging_dir,
            normalize_workx_version(args.workx_version),

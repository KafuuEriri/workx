...
from workx import AsyncWorkx


async def main() -> None:
    async with AsyncWorkx(config=runtime_config()) as workx:
        ...
        reopened = await workx.thread_resume(thread.id)
        listing_active = await workx.thread_list(limit=20, archived=False)
        reading = await reopened.read(include_turns=True)

        _ = await reopened.set_name("sdk-lifecycle-demo")
        _ = await workx.thread_archive(reopened.id)
        listing_archived = await workx.thread_list(limit=20, archived=True)
        unarchived = await workx.thread_unarchive(reopened.id)

        resumed = await workx.thread_resume(
            unarchived.id,
            model="gpt-5.4",
        ...
        forked = await workx.thread_fork(unarchived.id, model="gpt-5.4")
        forked_result = await (

...
with Workx(config=runtime_config()) as workx:
    ...
    reopened = workx.thread_resume(thread.id)
    listing_active = workx.thread_list(limit=20, archived=False)
    reading = reopened.read(include_turns=True)

    _ = reopened.set_name("sdk-lifecycle-demo")
    _ = workx.thread_archive(reopened.id)
    listing_archived = workx.thread_list(limit=20, archived=True)
    unarchived = workx.thread_unarchive(reopened.id)

    resumed = workx.thread_resume(
        unarchived.id,
        model="gpt-5.4",
    ...
    forked = workx.thread_fork(unarchived.id, model="gpt-5.4")

...
- `sync.py` (public sync surface: `Workx`)
- `async.py` (public async surface: `AsyncWorkx`)

All examples intentionally use only public SDK exports from `workx`
and `workx.types`.
...
```bash
python -m pip install workx
```

The SDK installs its pinned `workx-cli-bin` runtime dependency.

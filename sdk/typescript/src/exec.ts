...
import type { WorkxConfigObject, WorkxConfigValue } from "./workxOptions";
import { SandboxMode, ModelReasoningEffort, ApprovalMode, WebSearchMode } from "./threadOptions";

export type WorkxExecArgs = {
  input: string;
...
const INTERNAL_ORIGINATOR_ENV = "WORKX_INTERNAL_ORIGINATOR_OVERRIDE";
const TYPESCRIPT_SDK_ORIGINATOR = "workx_sdk_ts";
const WORKX_NPM_NAME = "@ronanxiao/workx";
...
type WorkxPathResolution = {
  executablePath: string;
  pathDirs: string[];
};

export class WorkxExec {
  private executablePath: string;
  private pathDirs: string[];
  private envOverride?: Record<string, string>;
  private configOverrides?: WorkxConfigObject;
  private rawConfigOverrides?: string[];

  constructor(
    executablePath: string | null = null,
    env?: Record<string, string>,
    configOverrides?: WorkxConfigObject,
  ...
  async *run(args: WorkxExecArgs): AsyncGenerator<string> {
    ...
    try {
      ...
      if (code !== 0 || signal) {
        ...
        throw new Error(`Workx Exec exited with ${detail}: ${stderrBuffer.toString("utf8")}`);
      }
    } finally {
      rl.close();
...
function serializeConfigOverrides(configOverrides: WorkxConfigObject): string[] {
...
function flattenConfigOverrides(
  ...
  if (!isPlainObject(value)) {
    if (prefix) {
      ...
      throw new Error("Workx config overrides must be a plain object");
    }
  }
  ...
  for (const [key, child] of entries) {
    if (!key) {
      throw new Error("Workx config override keys must be non-empty strings");
    }
    if (child === undefined) {
      continue;
    }
...
function toTomlValue(value: WorkxConfigValue, path: string): string {
  if (typeof value === "string") {
    return JSON.stringify(value);
  } else if (typeof value === "number") {
    if (!Number.isFinite(value)) {
      throw new Error(`Workx config override at ${path} must be a finite number`);
    }
    return `${value}`;
    ...
    throw new Error(`Workx config override at ${path} cannot be null`);
  } else {
    const typeName = typeof value;
    throw new Error(`Unsupported Workx config override value at ${path}: ${typeName}`);
  }
}
...
function isPlainObject(value: unknown): value is WorkxConfigObject {
  return typeof value === "object" && value !== null && !Array.isArray(value);
}

function findWorkxPath(): WorkxPathResolution {
  ...
  try {
    const workxPackageJsonPath = moduleRequire.resolve(`${WORKX_NPM_NAME}/package.json`);
    const workxRequire = createRequire(workxPackageJsonPath);
    const platformPackageJsonPath = workxRequire.resolve(`${platformPackage}/package.json`);
    vendorRoot = path.join(path.dirname(platformPackageJsonPath), "vendor");
  } catch {
    throw new Error(
      `Unable to locate Workx CLI binaries. Ensure ${WORKX_NPM_NAME} is installed with optional dependencies.`,
    );
  }

  const workxBinaryName = process.platform === "win32" ? "workx.exe" : "workx";
  const nativePackage = resolveNativePackage(vendorRoot, targetTriple, workxBinaryName);
  if (!nativePackage) {
    throw new Error(
      `Unable to locate Workx CLI binaries for ${targetTriple}. Ensure ${WORKX_NPM_NAME} is installed with optional dependencies.`,
    );
  }
...
export function resolveNativePackage(
  vendorRoot: string,
  targetTriple: string,
  workxBinaryName: string,
): WorkxPathResolution | null {
  const packageRoot = path.join(vendorRoot, targetTriple);
  const packageBinaryPath = path.join(packageRoot, "bin", workxBinaryName);
  if (isFile(packageBinaryPath) && isFile(path.join(packageRoot, "workx-package.json"))) {
    return {
      executablePath: packageBinaryPath,
      pathDirs: existingDirs(path.join(packageRoot, "workx-path")),
    };
  }

  const legacyBinaryPath = path.join(packageRoot, "workx", workxBinaryName);

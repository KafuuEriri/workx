...
import { workxExecPath } from "./testWorkx";
...
function conformanceDirectory(): string {
  ...
  for (const candidate of candidates) {
    if (existsSync(path.join(candidate, "run_workx_compliance.py"))) {
      return candidate;
    }
  }
...
describe("MCP client conformance", () => {
  it(
    ...
    () => {
      ...
      const workxBinary = requireFile(workxExecPath, "built Workx binary");
      ...
      const reportPath = path.join(reportDirectory, `workx-mcp-conformance-${process.pid}.json`);
      const result = spawnSync(
        "python3",
        [
          path.join(directory, "run_workx_compliance.py"),
          workxBinary,
          "--mode",
          "all",
          "--transport",
  ...
  it(
    ...
    () => {
      ...
      const reportDirectory = process.env.RUNNER_TEMP ?? os.tmpdir();
      const reportPath = path.join(reportDirectory, `workx-mcp-review-${process.pid}.json`);
      const result = spawnSync(
        "python3",
        [
          reviewer,

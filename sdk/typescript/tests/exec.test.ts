...
import type { WorkxConfigObject } from "../src/workxOptions";
...
describe("WorkxExec", () => {
  it("rejects when exit happens before stdout closes", async () => {
    ...
    if (result.status === "rejected") {
      ...
      expect(result.error.message).toMatch(/Workx Exec exited/);
    }
  });
  ...
  it.each(configOverrideCases)(
    ...
    async ({ config, configOverrides, expectedOverrides }) => {
      ...
      const exec = new WorkxExec("workx", undefined, config, configOverrides);
      for await (const _ of exec.run({ input: "hi" })) {
        // no-op
      }
  ...
  it("lets SDK-managed and thread settings override raw configuration when resuming", async () => {
    ...
    const exec = new WorkxExec("workx", undefined, { approval_policy: "never" }, [
      'approval_policy="on-failure"',
  ...
  it("allows overriding the env passed to the Workx CLI", async () => {
    const { WorkxExec } = await import("../src/exec");
    spawnMock.mockClear();
    const child = new FakeChildProcess();
    spawnMock.mockReturnValue(child as unknown as child_process.ChildProcess);
    ...
    process.env.WORKX_ENV_SHOULD_NOT_LEAK = "leak";

    try {
      const exec = new WorkxExec("workx", {
        WORKX_HOME: "/tmp/workx-home",
        CUSTOM_ENV: "custom",
      });
      ...
      expect(spawnEnv.WORKX_HOME).toBe("/tmp/workx-home");
      expect(spawnEnv.CUSTOM_ENV).toBe("custom");
      expect(spawnEnv.WORKX_ENV_SHOULD_NOT_LEAK).toBeUndefined();
      expect(spawnEnv.WORKX_API_KEY).toBe("test");
      expect(spawnEnv.WORKX_INTERNAL_ORIGINATOR_OVERRIDE).toBeDefined();
      expect(commandArgs).toContain("--config");
      expect(commandArgs).toContain(`openai_base_url=${JSON.stringify("https://example.test")}`);
    } finally {
      delete process.env.WORKX_ENV_SHOULD_NOT_LEAK;
    }
  });

  it("resolves the package-layout binary and PATH directory", async () => {
    const { resolveNativePackage } = await import("../src/exec");
    const vendorRoot = mkdtempSync(path.join(tmpdir(), "workx-sdk-vendor-"));
    const packageRoot = path.join(vendorRoot, "x86_64-unknown-linux-musl");
    const binDir = path.join(packageRoot, "bin");
    const pathDir = path.join(packageRoot, "workx-path");
    mkdirSync(binDir, { recursive: true });
    mkdirSync(pathDir, { recursive: true });
    writeFileSync(path.join(packageRoot, "workx-package.json"), "{}");
    writeFileSync(path.join(binDir, "workx"), "");

    expect(resolveNativePackage(vendorRoot, "x86_64-unknown-linux-musl", "workx")).toEqual({
      executablePath: path.join(binDir, "workx"),
      pathDirs: [pathDir],
    });
  });
  ...
  it("falls back to the legacy binary layout", async () => {
    ...
    const vendorRoot = mkdtempSync(path.join(tmpdir(), "workx-sdk-vendor-"));
    const packageRoot = path.join(vendorRoot, "x86_64-unknown-linux-musl");
    const binDir = path.join(packageRoot, "workx");
  ...
  it("prepends package PATH entries without duplicating them", async () => {
    ...
    const pathDir = path.join(tmpdir(), "workx-path");

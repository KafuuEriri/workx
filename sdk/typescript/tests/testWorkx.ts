...
import { Workx } from "../src/workx";
import type { WorkxConfigObject } from "../src/workxOptions";

export const workxExecPath =
  process.env.WORKX_EXEC_PATH ??
...
export function createTestClient(options: CreateTestClientOptions = {}): TestClient {
  ...
  return {
    cleanup: () => {},
    client: new Workx({
      workxPathOverride: workxExecPath,
...
function mergeTestConfig(
  ...
  config: WorkxConfigObject | undefined,
): WorkxConfigObject | undefined {
  const mergedConfig: WorkxConfigObject | undefined =
  ...
  return {
    ...
    // sync does not race temp WORKX_HOME cleanup.
    features:
...
function hasExplicitProviderConfig(config: WorkxConfigObject | undefined): boolean {
...
function getCurrentEnv(): Record<string, string> {
  ...
  for (const [key, value] of Object.entries(process.env)) {
    if (key === "WORKX_INTERNAL_ORIGINATOR_OVERRIDE") {
      continue;
    }
    if (value !== undefined) {
      env[key] = value;

...
use workx_app_server_protocol::ServerNotificationEnvelope;
use workx_config::types::AuthCredentialsStoreMode;
use workx_core::test_support::auth_manager_from_auth;
use workx_core::test_support::auth_manager_from_auth_with_home;
use workx_login::AuthDotJson;
use workx_login::AuthKeyringBackendKind;
use workx_login::AuthManager;
use workx_login::WorkxAuth;
use workx_login::save_auth;
use workx_login::token_data::TokenData;
use workx_login::token_data::parse_chatgpt_jwt_claims;
...
fn remote_control_auth_manager() -> Arc<AuthManager> {
    auth_manager_from_auth(WorkxAuth::create_dummy_chatgpt_auth_for_testing())
}

fn remote_control_auth_manager_with_home(workx_home: &TempDir) -> Arc<AuthManager> {
...
async fn remote_control_state_runtime(workx_home: &TempDir) -> Arc<StateRuntime> {
    StateRuntime::init(
...
async fn plain_start_resolves_persisted_remote_control_preference() {
    ...
    let workx_home = TempDir::new().expect("temp dir should create");
    let state_db = remote_control_state_runtime(&workx_home).await;
...
async fn ephemeral_enable_preserves_durable_preference() {
    ...
    remote_handle.state_db = Some(remote_control_state_runtime(&workx_home).await);
    remote_handle
        .desired_state_tx
...
async fn remote_control_transport_manages_virtual_clients_and_routes_messages() {
        ...
        ClientEnvelope {
            event: ClientEvent::ClientMessage {
                message: JSONRPCMessage::Notification(
                    workx_app_server_protocol::JSONRPCNotification {
    ...
    let initialize_message = JSONRPCMessage::Request(workx_app_server_protocol::JSONRPCRequest {
        id: workx_app_server_protocol::RequestId::Integer(1),
    ...
    let followup_message =
        JSONRPCMessage::Notification(workx_app_server_protocol::JSONRPCNotification {
            method: "initialized".to_string(),
            params: None,
        });
    send_client_event(
...
async fn remote_control_transport_reconnects_after_disconnect() {
    ...
    let (remote_task, remote_handle) = start_remote_control(
        ...
        Some(remote_control_state_runtime(&workx_home).await),
        remote_control_auth_manager(),
        transport_event_tx,
        shutdown_token.clone(),
        /*app_server_client_name_rx*/ None,
    ...
    send_client_event(
        ...
        ClientEnvelope {
            event: ClientEvent::ClientMessage {
                message: JSONRPCMessage::Request(workx_app_server_protocol::JSONRPCRequest {
                    id: workx_app_server_protocol::RequestId::Integer(2),
                    method: "initialize".to_string(),
                    params: Some(json!({
                        "clientInfo": {
                            "name": "remote-test-client",
...
async fn remote_control_http_mode_enrolls_before_connecting() {
    ...
    let writer = {
        let initialize_message =
            JSONRPCMessage::Request(workx_app_server_protocol::JSONRPCRequest {
                id: workx_app_server_protocol::RequestId::Integer(11),
                method: "initialize".to_string(),
                params: Some(json!({
                    "clientInfo": {
    ...
    writer
        .send(QueuedOutgoingMessage::new(OutgoingMessage::Response(
            crate::outgoing_message::OutgoingResponse {
                id: workx_app_server_protocol::RequestId::Integer(11),
                result: Box::new(
                    workx_app_server_protocol::ClientResponsePayload::Initialize(
                        workx_app_server_protocol::InitializeResponse {
                            user_agent: "workx-test-agent".to_string(),
                            workx_home: workx_home.path().abs(),
    ...
    assert_eq!(
        ...
        json!({
            ...
            "message": {
                "id": 11,
                "result": {
                    "userAgent": "workx-test-agent",
                    "workxHome": workx_home.path(),
...
async fn remote_control_http_mode_refreshes_persisted_enrollment_before_connecting() {
    ...
    let (remote_task, _remote_handle) = start_remote_control(
        ...
        Some(state_db.clone()),
        remote_control_auth_manager_with_home(&workx_home),
        transport_event_tx,
        shutdown_token.clone(),
        /*app_server_client_name_rx*/ None,
        RemoteControlStartupMode::EnabledEphemeral,

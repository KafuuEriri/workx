...
fn missing_host_error_limits_the_displayed_path_to_512_bytes() {
    let executable = "workx-code-mode-host-does-not-exist";
...
async fn provider_returns_missing_host_error() {
    let provider = ProcessOwnedCodeModeSessionProvider::with_host_program(
        "workx-code-mode-host-does-not-exist".into(),
    );
    ...
    assert!(error.contains("failed to spawn code-mode host workx-code-mode-host-does-not-exist"));
}
...
async fn shutdown_before_open_does_not_spawn_the_host() {
    ...
    let error = session
        .execute(workx_code_mode_protocol::ExecuteRequest {

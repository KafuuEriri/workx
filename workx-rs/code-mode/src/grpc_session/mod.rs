...
use workx_code_mode_protocol::CodeModeSessionProvider;
use workx_code_mode_protocol::CodeModeSessionProviderFuture;
...
fn inject_span_traceparent<T>(request: &mut tonic::Request<T>, span: &tracing::Span) {
    if let Some(traceparent) =
        workx_otel::span_w3c_trace_context(span).and_then(|trace| trace.traceparent)

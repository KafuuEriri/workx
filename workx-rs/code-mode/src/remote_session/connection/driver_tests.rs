...
use workx_code_mode_protocol::CellId;
use workx_code_mode_protocol::CodeModeNestedToolCall;
use workx_code_mode_protocol::CodeModeSessionCellExecutionLimits;
use workx_code_mode_protocol::CodeModeSessionDelegate;
use workx_code_mode_protocol::ExecuteRequest;
use workx_code_mode_protocol::NotificationFuture;
use workx_code_mode_protocol::ToolInvocationFuture;
use workx_code_mode_protocol::WaitRequest;
use workx_code_mode_protocol::host::CapabilitySet;
use workx_code_mode_protocol::host::ClientToHost;
use workx_code_mode_protocol::host::DelegateRequest;
use workx_code_mode_protocol::host::DelegateRequestId;
use workx_code_mode_protocol::host::EncodedFrame;
use workx_code_mode_protocol::host::HostRequest;
use workx_code_mode_protocol::host::HostResponse;
use workx_code_mode_protocol::host::HostToClient;
use workx_code_mode_protocol::host::MAX_PENDING_DELEGATE_CALLS;
use workx_code_mode_protocol::host::RequestId;
use workx_code_mode_protocol::host::SessionId;
use workx_code_mode_protocol::host::WireNestedToolCall;
use workx_code_mode_protocol::host::WireResult;
use workx_code_mode_protocol::host::WireRuntimeResponse;
use workx_code_mode_protocol::host::WireSessionCellExecutionLimits;
use workx_code_mode_protocol::host::WireWaitOutcome;
...
struct DriverHarness {
    ...
    outgoing_rx: mpsc::Receiver<workx_code_mode_protocol::host::EncodedFrame>,
...
impl DriverHarness {
    ...
    async fn start_cell(
        ...
        request_id: i64,
        cell_id: &str,
    ) -> workx_code_mode_protocol::StartedCell {
        let (response_tx, response_rx) = oneshot::channel();
        self.command_tx
            ...
            .send(DriverEvent::HostMessage(HostToClient::DelegateRequest {
                ...
                request: DelegateRequest::InvokeTool {
                    invocation: WireNestedToolCall {
                        ...
                        tool_kind: workx_code_mode_protocol::CodeModeToolKind::Function.into(),
                        input: None,
                    },
                },
            }))
...
async fn deferred_delegates_follow_cell_readiness_and_cancellation() {
    ...
    assert_eq!(
        ...
        ClientToHost::DelegateResponse {
            ...
            result: WireResult::Ok {
                value: workx_code_mode_protocol::host::DelegateResponse::NotificationDelivered,
            },
        }
    );
...
async fn terminate_closes_cell_without_waiting_for_delegate_cleanup() {
    ...
    assert_eq!(
        ...
        Ok(workx_code_mode_protocol::WaitOutcome::LiveCell(
            workx_code_mode_protocol::RuntimeResponse::Terminated {
...
async fn remote_wait_accepts_durations_longer_than_five_minutes() {
    ...
    assert_eq!(
        ...
        Ok(workx_code_mode_protocol::WaitOutcome::LiveCell(
            workx_code_mode_protocol::RuntimeResponse::Yielded {
                code_mode_host_duration: Some(Duration::ZERO),
                cell_id: CellId::new("1".to_string()),
                content_items: Vec::new(),
            }

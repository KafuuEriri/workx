...
use crate::types::WorkxUserSettingsResponse;
use crate::types::WorkxWorkspaceMessagesResponse;
use anyhow::Result;
use http::Method;
use http::StatusCode;
...
use workx_http_client::RouteAwareRequestBuilder;
...
use workx_protocol::protocol::RateLimitReachedType;
use workx_protocol::protocol::RateLimitSnapshot;
use workx_protocol::protocol::RateLimitWindow;
use workx_protocol::protocol::SpendControlLimitSnapshot;
...
impl Client {
    ...
    fn with_http(mut base_url: String, http: RouteAwareClientPool) -> Self {
        ...
        Self {
            base_url,
            http,
            auth_provider: workx_model_provider::unauthenticated_auth_provider(),
            user_agent: None,
    ...
    pub fn from_auth(
        ...
        Self::new(base_url, http_client_factory)
            .with_user_agent(get_workx_user_agent())
            .with_auth_provider(workx_model_provider::auth_provider_from_auth(auth))
    }
    ...
    fn headers(&self) -> HeaderMap {
        ...
        if let Some(ua) = &self.user_agent {
            ...
            h.insert(USER_AGENT, HeaderValue::from_static("workx-cli"));
        }
    ...
    pub async fn get_rate_limits(&self) -> Result<RateLimitSnapshot> {
        ...
        let preferred = snapshots
            .iter()
            .find(|snapshot| snapshot.limit_id.as_deref() == Some("workx"))
            .cloned();
    ...
    pub async fn get_accounts_check(&self) -> Result<AccountsCheckResponse> {
        let url = match self.path_style {
            PathStyle::WorkxApi => format!("{}/api/codex/accounts/check", self.base_url),
    ...
    fn token_usage_profile_url(&self) -> String {
        match self.path_style {
            PathStyle::WorkxApi => format!("{}/api/codex/profiles/me", self.base_url),
    ...
    fn list_tasks_url(
        ...
        let url = match self.path_style {
            PathStyle::WorkxApi => format!("{}/api/codex/tasks/list", self.base_url),
    ...
    pub async fn get_task_details_with_body(
        ...
        let url = match self.path_style {
            PathStyle::WorkxApi => format!("{}/api/codex/tasks/{}", self.base_url, task_id),
    ...
    /// Fetch the selected cloud-managed config bundle from workx-backend.
    ///
    /// `GET /api/codex/config/bundle` (Workx API style) or
    ...
    pub async fn get_config_bundle(
        ...
        let url = match self.path_style {
            PathStyle::WorkxApi => format!("{}/api/codex/config/bundle", self.base_url),
    ...
    /// Fetch authenticated Workx user settings from the active backend route.
    ///
    /// Uses `GET /api/codex/settings/user` for Workx API hosts and
    /// `GET /wham/settings/user` for ChatGPT `backend-api` hosts.
    pub async fn get_user_settings(
        &self,
    ) -> std::result::Result<WorkxUserSettingsResponse, RequestError> {
        ...
        let (body, ct) = self.exec_request_detailed(req, "GET", &url).await?;
        self.decode_json::<WorkxUserSettingsResponse>(&url, &ct, &body)
            .map_err(RequestError::from)
    }

    pub async fn list_workspace_messages(
        &self,
    ) -> std::result::Result<WorkxWorkspaceMessagesResponse, RequestError> {
        ...
        let (body, ct) = self.exec_request_detailed(req, "GET", &url).await?;
        self.decode_json::<WorkxWorkspaceMessagesResponse>(&url, &ct, &body)
            .map_err(RequestError::from)
    }
    ...
    pub async fn create_task(&self, request_body: serde_json::Value) -> Result<String> {
        let url = match self.path_style {
            PathStyle::WorkxApi => format!("{}/api/codex/tasks", self.base_url),
    ...
    fn make_rate_limit_snapshot(
        ...
        spend_control: Option<workx_backend_openapi_models::models::SpendControlStatusDetails>,
    ...
    fn workspace_messages_url(&self) -> String {
        match self.path_style {
            PathStyle::WorkxApi => format!("{}/api/codex/workspace-messages", self.base_url),
    ...
    fn user_settings_url(&self) -> String {
        match self.path_style {
            PathStyle::WorkxApi => format!("{}/api/codex/settings/user", self.base_url),
...
mod tests {
    ...
    use workx_backend_openapi_models::models::AdditionalRateLimitDetails;
    use workx_backend_openapi_models::models::RateLimitReachedKind;
    use workx_backend_openapi_models::models::RateLimitReachedType as BackendRateLimitReachedType;
    ...
    fn usage_payload_maps_primary_and_additional_rate_limits() {
        let payload = RateLimitStatusPayload {
            ...
            additional_rate_limits: Some(Some(vec![AdditionalRateLimitDetails {
                limit_name: "workx_other".to_string(),
                metered_feature: "workx_other".to_string(),
                rate_limit: Some(Some(Box::new(crate::types::RateLimitStatusDetails {
                    primary_window: Some(Some(Box::new(crate::types::RateLimitWindowSnapshot {
                        used_percent: 70,
            ...
            spend_control: Some(Some(Box::new(
                workx_backend_openapi_models::models::SpendControlStatusDetails {
                    reached: false,
        ...
        assert_eq!(snapshots[0].limit_id.as_deref(), Some("workx"));
        ...
        assert_eq!(snapshots[1].limit_id.as_deref(), Some("workx_other"));
        assert_eq!(snapshots[1].limit_name.as_deref(), Some("workx_other"));
        assert_eq!(
    ...
    fn preferred_snapshot_selection_matches_get_rate_limits_behavior() {
        ...
        assert_eq!(preferred.limit_id.as_deref(), Some("workx"));
    }
    ...
    fn add_credits_nudge_email_uses_expected_paths_and_bodies() {
        let workx_client = test_client("https://example.test", PathStyle::WorkxApi);
        assert_eq!(
            workx_client.send_add_credits_nudge_email_url(),
    ...
    fn token_usage_profile_uses_expected_paths() {
        let workx_client = test_client("https://example.test", PathStyle::WorkxApi);
        assert_eq!(
            workx_client.token_usage_profile_url(),
    ...
    fn workspace_messages_uses_expected_paths() {
        let workx_client = test_client("https://example.test", PathStyle::WorkxApi);
        assert_eq!(
            workx_client.workspace_messages_url(),
    ...
    async fn user_settings_request_uses_expected_paths_and_revalidates_cached_responses() {
        ...
        let workx_response = Client::new(
            server.uri(),
            HttpClientFactory::new(workx_http_client::OutboundProxyPolicy::ReqwestDefault),
        )
        .get_user_settings()
        .await
        .unwrap();
        ...
        assert_eq!(
            [workx_response, chatgpt_response],
            [
                WorkxUserSettingsResponse {
    ...
    fn user_settings_missing_attribution_policy_defaults_to_disabled() {
        assert_eq!(
            serde_json::from_value::<WorkxUserSettingsResponse>(serde_json::json!({})).unwrap(),
            WorkxUserSettingsResponse {
                commit_attribution_enabled: false,
            }
        );
    ...
    fn authenticated_user_settings_client_uses_active_workspace_headers() {
        let auth = WorkxAuth::from_external_chatgpt_tokens(
            "e30.e30.c2ln",
            "workspace-123",
            Some("enterprise"),
        )

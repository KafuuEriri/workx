...
/// Filename used for the redacted `workx doctor --json` feedback attachment.
pub const DOCTOR_REPORT_ATTACHMENT_FILENAME: &str = "workx-doctor-report.json";
/// Filename used for the raw Workx Apps MCP tools cache feedback attachment.
pub const WORKX_APPS_TOOLS_CACHE_ATTACHMENT_FILENAME: &str = "workx-apps-tools-cache.json";
/// Filename used for the raw connector directory cache feedback attachment.
pub const WORKX_APP_DIRECTORY_CACHE_ATTACHMENT_FILENAME: &str = "workx-app-directory-cache.json";
...
pub fn emit_feedback_request_tags_with_auth_env(
    ...
    tracing::info!(
        ...
        auth_env_workx_api_key_present = tracing::field::debug(auth_env.workx_api_key_env_present),
        auth_env_workx_api_key_enabled = tracing::field::debug(auth_env.workx_api_key_env_enabled),
...
#[derive(Clone)]
pub struct WorkxFeedback {
    inner: Arc<FeedbackInner>,
}

impl Default for WorkxFeedback {
    fn default() -> Self {
        Self::new()
    }
}
        ...
        tracing_subscriber::fmt::layer()
            ...
            .with_filter(
                Targets::new()
                    ...
                    .with_target("workx_http_client::transport", LevelFilter::DEBUG)
                    .with_target("workx_api::sse", LevelFilter::DEBUG)
                    ...
                    .with_target("workx_api::responses_websocket_timing", LevelFilter::OFF)
                    .with_target("workx_core::post_sampling_token_estimate", LevelFilter::OFF),
            )
    }
...
pub struct FeedbackUploadOptions<'a> {
    ...
    ///
    /// These are included after `workx-logs.log` and before path-backed rollout
...
impl FeedbackSnapshot {
    ...
    pub fn log_attachment(&self, logs_override: Option<Vec<u8>>) -> FeedbackAttachment {
        FeedbackAttachment {
            filename: "workx-logs.log".to_string(),
...
mod tests {
    ...
    fn ring_buffer_drops_front_when_full() {
        let fb = WorkxFeedback::with_capacity(/*max_bytes*/ 8);
        {
    ...
    fn logger_layer_filters_noisy_trace_payloads() {
        ...
        tracing::trace!(target: "workx_api::responses_websocket_timing", payload = "secret");
        tracing::trace!(target: "workx_http_client::transport", "transport-trace");
        tracing::trace!(target: "workx_api::sse", "sse-trace");
        tracing::trace!(target: "workx_api::sse::responses", "nested-sse-trace");
        tracing::debug!(target: "workx_http_client::transport", "transport-debug");
        tracing::debug!(target: "workx_api::sse::responses", "sse-debug");
        tracing::trace!(target: "workx_feedback_test", "unrelated-trace");
        log::trace!(target: "workx_feedback_test", "unrelated-log-trace");
        log::trace!(
    ...
    async fn feedback_upload_allows_slow_reports() {
        ...
        upload_test_feedback(&WorkxFeedback::new(), &dsn, &[attachment])
            .await
    ...
    async fn feedback_upload_deadline_stops_retries_and_later_attachments() {
        ...
        let snapshot = WorkxFeedback::new()
            .snapshot(/*session_id*/ None)
            .with_feedback_diagnostics(FeedbackDiagnostics::default());
    ...
    async fn feedback_upload_retries_diagnostics_without_replaying_core() {
        ...
        upload_test_feedback(&WorkxFeedback::new(), &dsn, &attachments)
            .await
    ...
    async fn feedback_upload_stops_diagnostics_for_sentry_rate_limits() {
        for (accepted_before_limit, rate_limit) in [
            ...
            upload_test_feedback(&WorkxFeedback::new(), &dsn, &[later])
                .await
    ...
    async fn feedback_upload_does_not_forward_private_data_to_redirect_target() {
        ...
        let error = upload_test_feedback(&WorkxFeedback::new(), &dsn, &[])
            .await
    ...
    async fn feedback_upload_reports_transport_failures() {
        ...
        assert!(
            error
                .downcast_ref::<workx_http_client::RouteAwareRequestError>()
                .is_some_and(workx_http_client::RouteAwareRequestError::is_connect)
        );
    }

    #[test]
    fn feedback_attachments_gate_connectivity_diagnostics() {
        let extra_filename = format!("workx-feedback-extra-{}.jsonl", ThreadId::new());
        ...
        let snapshot_with_diagnostics = WorkxFeedback::new()
            .snapshot(/*session_id*/ None)
        ...
        let attachments_without_diagnostics = WorkxFeedback::new()
            .snapshot(/*session_id*/ None)
            .with_feedback_diagnostics(FeedbackDiagnostics::default())
    ...
    fn path_backed_attachments_use_binary_content_types() {
        let suffix = ThreadId::new();
        let gzip_filename = format!("workx-desktop-app-logs-{suffix}.tar.gz");
        let unknown_filename = format!("workx-feedback-extra-{suffix}.binunknown");
        ...
        let attachments = WorkxFeedback::new()
            .snapshot(/*session_id*/ None)
            .feedback_attachments(
                /*include_logs*/ false,
                &[],

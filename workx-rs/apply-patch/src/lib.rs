...
use workx_utils_path_uri::PathUriParseError;
...
/// Special argv[1] flag used when the Workx executable self-invokes to run the
/// internal `apply_patch` path.
///
/// Although this constant lives in `workx-apply-patch` (to avoid forcing
/// `workx-arg0` to depend on `workx-core`), it remains part of the "workx core"
/// process-invocation contract for the standalone `apply_patch` command
/// surface.
pub const WORKX_CORE_APPLY_PATCH_ARG1: &str = "--workx-run-as-apply-patch";

/// Internal environment variable used to carry the selected update mode
/// through the arg0-dispatched standalone executable.
pub const WORKX_APPLY_PATCH_PRESERVE_LINE_ENDINGS_ENV_VAR: &str =
    "WORKX_APPLY_PATCH_PRESERVE_LINE_ENDINGS";
...
pub fn apply_patch_file_update_mode_from_env() -> ApplyPatchFileUpdateMode {
    match std::env::var(WORKX_APPLY_PATCH_PRESERVE_LINE_ENDINGS_ENV_VAR).as_deref() {

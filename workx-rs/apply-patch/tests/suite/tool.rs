...
fn run_apply_patch_in_dir(dir: &Path, patch: &str) -> anyhow::Result<assert_cmd::assert::Assert> {
    let mut cmd = Command::new(workx_utils_cargo_bin::cargo_bin("apply_patch")?);
    cmd.env(WORKX_APPLY_PATCH_PRESERVE_LINE_ENDINGS_ENV_VAR, "1");
    cmd.current_dir(dir);
...
fn test_apply_patch_cli_allows_overlapping_eof_chunks_in_legacy_mode() -> anyhow::Result<()> {
    ...
    Command::new(workx_utils_cargo_bin::cargo_bin("apply_patch")?)

...
async fn reused_elicitation_senders_follow_each_servers_latest_permission_authority() {
    ...
    config.approval_policy = workx_config::Constrained::allow_any(AskForApproval::Never);
    ...
    let hosted_server = crate::workx_apps_mcp_server_config(
        "https://example.com",
    ...
    let hosted = manager.make_sender(
        crate::WORKX_APPS_MCP_SERVER_NAME.to_string(),
        /*tx_event*/ None,
    );

...
use workx_rmcp_client::StoredOAuthCredentialSnapshot;
...
pub(crate) struct McpServerConnectionIdentity {
    ...
    runtime_auth: Option<WorkxAuth>,
    runtime_auth_token: Option<String>,
    workx_apps_cache_identity: Option<(PathBuf, ConnectorRuntimeContextKey)>,
...
impl McpServerConnectionIdentity {
    ...
    pub(crate) fn has_same_connection_config(&self, other: &Self) -> bool {
        let same_runtime_auth = match (&self.runtime_auth, &other.runtime_auth) {
            (Some(WorkxAuth::AgentIdentity(left)), Some(WorkxAuth::AgentIdentity(right))) => {
        ...
        self.auth == other.auth
            ...
            && self.workx_apps_cache_identity == other.workx_apps_cache_identity

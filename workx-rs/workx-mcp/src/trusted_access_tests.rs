...
struct SequencedExternalAuth(Mutex<VecDeque<WorkxAuth>>);
...
fn header_auth() -> WorkxAuth {
    WorkxAuth::Headers(AuthHeaders::new(
        [
            (
...
fn chatgpt_auth(account_id: &str) -> WorkxAuth {
    WorkxAuth::from_external_chatgpt_tokens(
        "header.e30.same",
        account_id,
...
fn fedramp_chatgpt_auth(account_id: &str) -> WorkxAuth {
    WorkxAuth::from_external_chatgpt_tokens(
...
fn context(auth: WorkxAuth, client: Arc<RecordingHttpClient>) -> TrustedAccessContext {
...
async fn rejects_api_key_authentication_without_sending_credentials() {
    ...
    let context = context(WorkxAuth::from_api_key("test-api-key"), client.clone());
...
async fn uses_the_checked_auth_snapshot_for_request_headers() -> anyhow::Result<()> {
    ...
    let context = context(chatgpt_auth("account-a"), client.clone());
    let refreshed = WorkxAuth::from_external_chatgpt_tokens(
...
async fn rejects_identity_changes_while_request_is_in_flight() -> anyhow::Result<()> {
    ...
    let workspace =
        WorkxAuth::from_external_chatgpt_tokens("header.e30.same", "account-a", Some("team"))?;
    for (description, initial_auth, selected_auth) in [
        (
            "account switch",
            personal.clone(),

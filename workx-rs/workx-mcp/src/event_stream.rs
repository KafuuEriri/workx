...
use crate::WORKX_APPS_MCP_SERVER_NAME;
...
impl McpEventStreamOpener {
    ...
    fn matches_auth(&self, current: Option<&WorkxAuth>) -> bool {
        match (self.connection.auth.as_ref(), current) {
            (Some(WorkxAuth::AgentIdentity(expected)), Some(WorkxAuth::AgentIdentity(current))) => {
                expected.record() == current.record()
            }
            (Some(WorkxAuth::AgentIdentity(_)), _) | (_, Some(WorkxAuth::AgentIdentity(_))) => {
                false
            }

...
//! Session orchestration stays in `workx-core`.

use serde::Serialize;
use workx_protocol::mcp::CallToolResult;

pub const MCP_TOOL_WORKX_APPS_META_KEY: &str = "_workx_apps";
...
pub struct WorkxAppsConnectorAuthFailure {
...
pub struct WorkxAppsAuthElicitation {
...
#[derive(Debug, Clone, PartialEq)]
pub struct WorkxAppsAuthElicitationPlan {
    pub auth_failure: WorkxAppsConnectorAuthFailure,
    pub elicitation: WorkxAppsAuthElicitation,
}

#[derive(Serialize)]
struct WorkxAppsConnectorAuthFailureMeta<'a> {
...
pub fn connector_auth_failure_from_tool_result(
...
) -> Option<WorkxAppsConnectorAuthFailure> {
    ...
    Some(WorkxAppsConnectorAuthFailure {
...
pub fn build_auth_elicitation_plan(
    ...
    connector_name: Option<&str>,
    install_url: Option<String>,
) -> Option<WorkxAppsAuthElicitationPlan> {
    let auth_failure =
    ...
    Some(WorkxAppsAuthElicitationPlan {
        auth_failure,
        elicitation,
    })
}
...
pub fn build_auth_elicitation(
    call_id: &str,
    auth_failure: &WorkxAppsConnectorAuthFailure,
) -> WorkxAppsAuthElicitation {
    WorkxAppsAuthElicitation {
        meta: serde_json::json!({
            MCP_TOOL_WORKX_APPS_META_KEY: {
                CONNECTOR_AUTH_FAILURE_META_KEY: WorkxAppsConnectorAuthFailureMeta {
...
pub fn auth_elicitation_id(call_id: &str) -> String {
    format!("workx_apps_auth_{call_id}")
}
...
fn auth_elicitation_message(auth_failure: &WorkxAppsConnectorAuthFailure) -> String {
    match auth_failure.auth_reason.as_deref() {
        ...
        Some("missing_link") => format!(
            "Sign in to {} on ChatGPT to use it in Workx.",
            auth_failure.connector_name
        ),
        _ => format!(
...
mod tests {
    ...
    fn builds_auth_elicitation_plan() {
        ...
        assert_eq!(plan.elicitation.elicitation_id, "workx_apps_auth_call_123");
    }
}

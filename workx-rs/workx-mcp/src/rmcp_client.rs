...
//! and exposing cached Workx Apps tools while a client is still connecting.
...
use crate::mcp::WORKX_APPS_MCP_SERVER_NAME;
...
use crate::workx_apps::normalize_workx_apps_callable_name;
use crate::workx_apps::normalize_workx_apps_callable_namespace;
use crate::workx_apps::normalize_workx_apps_tool_title;
use crate::workx_apps::prepare_openai_file_params_for_model;
use anyhow::Result;
use anyhow::anyhow;
...
use tracing::instrument;
use tracing::warn;
use workx_api::SharedAuthProvider;
use workx_async_utils::CancelErr;
...
use workx_protocol::protocol::McpStartupUpdateEvent;
use workx_rmcp_client::ExecutorStdioServerLauncher;
use workx_rmcp_client::LocalStdioServerLauncher;
use workx_rmcp_client::McpOAuthRefreshMode;
use workx_rmcp_client::McpProtocolMode;
use workx_rmcp_client::RmcpClient;
use workx_rmcp_client::StdioServerLauncher;
use workx_rmcp_client::StreamableHttpBearerToken;
use workx_rmcp_client::StreamableHttpRedirectMode;
use workx_rmcp_client::ToolWithConnectorId;
use workx_rmcp_client::is_authentication_required_error;

/// MCP server capability indicating that Workx should include [`SandboxState`]
/// in tool-call request `_meta` under this key.
pub const MCP_SANDBOX_STATE_META_CAPABILITY: &str = "workx/sandbox-state-meta";
...
pub(crate) const MCP_TOOLS_LIST_DURATION_METRIC: &str = "workx.mcp.tools.list.duration_ms";
pub(crate) const MCP_TOOLS_FETCH_UNCACHED_DURATION_METRIC: &str =
    "workx.mcp.tools.fetch_uncached.duration_ms";
pub(crate) const WORKX_APPS_REFRESH_DURATION_METRIC: &str = "workx.apps.refresh.duration_ms";
pub(crate) const DEFAULT_STARTUP_TIMEOUT: Duration = Duration::from_secs(30);
pub(crate) const DEFAULT_TOOL_TIMEOUT: Duration = Duration::from_secs(300);

pub(crate) const WORKX_APPS_RECONNECT_INITIAL_BACKOFF: Duration = Duration::from_secs(1);
const WORKX_APPS_RECONNECT_MAX_BACKOFF: Duration = Duration::from_secs(30);
...
pub(crate) struct ManagedClient {
    ...
    pub(crate) workx_apps_tools_cache_context: Option<ConnectorRuntimeContext<ToolInfo>>,
}
...
impl ManagedClient {
    pub(crate) fn listed_tools(&self) -> Vec<ToolInfo> {
        ...
        if let Some(tools) = self
            .workx_apps_tools_cache_context
            .as_ref()
        ...
        if self.workx_apps_tools_cache_context.is_some() {
            emit_duration(
                MCP_TOOLS_LIST_DURATION_METRIC,
                total_start.elapsed(),
                &[("cache", "miss")],
...
#[derive(Default)]
struct WorkxAppsStartupReconnectState {
...
#[derive(Clone)]
struct WorkxAppsStartupStatusContext {
    submit_id: String,
    server_name: String,
    tx_event: Sender<Event>,
}

pub(crate) struct WorkxAppsStartupReconnect {
    factory: Arc<dyn Fn() -> ManagedClientFuture + Send + Sync>,
    state: StdMutex<WorkxAppsStartupReconnectState>,
    startup_status_context: Option<WorkxAppsStartupStatusContext>,
}

impl WorkxAppsStartupReconnect {
    pub(crate) fn new(factory: Arc<dyn Fn() -> ManagedClientFuture + Send + Sync>) -> Self {
        Self {
            factory,
            state: StdMutex::new(WorkxAppsStartupReconnectState::default()),
    ...
    fn with_startup_status_context(
        ...
        self.startup_status_context = tx_event.map(|tx_event| WorkxAppsStartupStatusContext {
            submit_id,
            server_name,
            tx_event,
        });
            ...
            let recovered = {
                ...
                match result {
                    ...
                    Err(error) => {
                        ...
                        let retry_after = workx_apps_reconnect_backoff(state.consecutive_failures);
...
fn workx_apps_reconnect_backoff(consecutive_failures: u32) -> Duration {
    let exponent = consecutive_failures.saturating_sub(1).min(5);
    WORKX_APPS_RECONNECT_INITIAL_BACKOFF
        .saturating_mul(1 << exponent)
        .min(WORKX_APPS_RECONNECT_MAX_BACKOFF)
}
...
struct ManagedClientStartup {
    ...
    workx_apps_tools_cache_context: Option<ConnectorRuntimeContext<ToolInfo>>,
...
impl ManagedClientStartup {
    fn start(&self) -> ManagedClientFuture {
        ...
        let is_workx_apps_mcp_server = server_name == WORKX_APPS_MCP_SERVER_NAME;
        ...
        async move {
            ...
            let refresh_start = is_workx_apps_mcp_server.then(Instant::now);
            ...
            {
                emit_duration(
                    WORKX_APPS_REFRESH_DURATION_METRIC,
...
pub(crate) struct AsyncManagedClient {
    ...
    pub(crate) is_workx_apps_mcp_server: bool,
    ...
    pub(crate) startup_reconnect: Option<Arc<WorkxAppsStartupReconnect>>,
...
impl AsyncManagedClient {
    ...
    pub(crate) fn new(
        ...
        let cached_server_info = if is_workx_apps_mcp_server {
            workx_apps_tools_cache_context
                .as_ref()
        ...
        let startup = Arc::new(ManagedClientStartup {
            ...
            tx_event,
            elicitation_requests,
            workx_apps_tools_cache_context: workx_apps_tools_cache_context.clone(),
        ...
        let startup_reconnect = is_workx_apps_mcp_server.then(|| {
            let startup = Arc::clone(&startup);
            Arc::new(
                WorkxAppsStartupReconnect::new(Arc::new(move || startup.start()))
    ...
    pub(crate) fn has_cached_tools(&self) -> bool {
        self.workx_apps_tools_cache_context
            .as_ref()
    ...
    pub(crate) async fn listed_tools(&self) -> Option<Vec<ToolInfo>> {
            ...
            && let Some(startup_tools) = self.cached_tools()
        {
            ...
            match self.client().await {
                ...
                Err(_) if self.is_workx_apps_mcp_server => self.cached_tools(),
                Err(_) => None,
            }
        }
    }
...
pub(crate) async fn list_tools_for_client_uncached(
    server_name: &str,
    is_workx_apps_mcp_server: bool,
    workx_apps_refresh_trigger: &'static str,
    ...
    .collect();
    if is_workx_apps_mcp_server {
        emit_duration(
            MCP_TOOLS_FETCH_UNCACHED_DURATION_METRIC,
            fetch_start.elapsed(),
            &[("trigger", workx_apps_refresh_trigger)],
        );
    } else {
        emit_duration(
            MCP_TOOLS_FETCH_UNCACHED_DURATION_METRIC,
...
/// Presents declared Workx Apps file parameters to the model as local-path inputs and adds plugin
/// names to each tool. Plugin membership is resolved by connector ID, falling back to the MCP
/// server when absent.
pub(crate) fn prepare_workx_apps_tools_for_model(
...
fn tool_info_from_listed_tool(
    ...
    if is_workx_apps_mcp_server {
        workx_apps_tool_info_from_listed_tool(server_name, server_instructions, tool)
    } else {
        regular_mcp_tool_info_from_listed_tool(server_name, server_instructions, tool)
    }
}

/// Converts a Workx Apps tool by preserving connector fields, removing connector prefixes from
/// model-visible names and titles, and using the connector description for its tool namespace.
fn workx_apps_tool_info_from_listed_tool(
    server_name: &str,
    server_instructions: Option<&str>,
    tool: ToolWithConnectorId,
) -> ToolInfo {
    ...
    let callable_name = normalize_workx_apps_callable_name(
        &tool_def.name,
        connector_id.as_deref(),
        connector_name.as_deref(),
    );
    let callable_namespace =
        normalize_workx_apps_callable_namespace(server_name, connector_name.as_deref());
    if let Some(title) = tool_def.title.as_deref() {
        let normalized_title = normalize_workx_apps_tool_title(connector_name.as_deref(), title);
...
async fn start_server_task(
    ...
    let fetch_ticket = workx_apps_tools_cache_context
        .as_ref()
    ...
    let client_tools = list_tools_for_client_uncached(
        &server_name,
        is_workx_apps_mcp_server,
        /*workx_apps_refresh_trigger*/ "initial",
        &client,
        startup_timeout,
        catalog_item_limit,
    ...
    let shared_tools = match (workx_apps_tools_cache_context.as_ref(), fetch_ticket) {
        ...
        _ => unreachable!("Workx Apps fetch ticket requires cache context"),
    };
    let has_shared_tool_catalog = is_workx_apps_mcp_server || tool_catalog_cache_context.is_some();
...
fn record_protocol_discovery_metrics(
    ...
    let Some(metrics) = workx_otel::global() else {
        return;
    };
    ...
    if is_workx_apps_mcp_server {
        tags.push(("server_kind", "workx_apps"));
    }
    let _ = metrics.counter("workx.mcp.protocol_discovery", /*inc*/ 1, &tags);
    let _ = metrics.record_duration(
        "workx.mcp.protocol_discovery.duration_ms",
        started_at.elapsed(),
        &tags,
    );
}
...
pub(crate) fn mcp_initialize_request_params(
    ...
    InitializeRequestParams::new(
        capabilities,
        Implementation::new("workx-mcp-client", env!("CARGO_PKG_VERSION")).with_title("Workx"),
    )
...
pub(crate) async fn make_rmcp_client(
    ...
    match transport {
        McpServerTransportConfig::Stdio {
            ...
            let cwd = cwd.map(workx_utils_path_uri::LegacyAppPathString::into_string);
                ...
                && let Some(env_var) = bearer_token_env_var.as_ref()
            {
                (
                    Arc::new(ExecutorEnvironmentHttpClient {
                        ...
                        http_client,
                    }) as Arc<dyn workx_exec_server::HttpClient>,
...
mod tests {
    ...
    use workx_protocol::mcp::MCP_APP_UI_EXTENSION_ID;
    use workx_protocol::mcp::OPENAI_FORM_EXTENSION_ID;
    ...
    #[test]
    fn workx_apps_connector_metadata_is_preserved() {
        ...
        let expected = ToolInfo {
            server_name: WORKX_APPS_MCP_SERVER_NAME.to_string(),
            supports_parallel_tool_calls: false,
            server_origin: None,
            callable_name: "capture_file_upload".to_string(),
            callable_namespace: "workx_apps__gmail".to_string(),

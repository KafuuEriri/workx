...
use workx_protocol::mcp::McpResourceOriginCheckpoint;
...
use workx_rmcp_client::with_http_headers_helper;
...
pub struct McpRuntimeInput {
    ...
    pub workx_apps_tools_cache: ConnectorRuntimeManager<ToolInfo>,
    pub tool_catalog_cache: McpToolCatalogCache,
    pub workx_apps_tools_cache_key: ConnectorRuntimeContextKey,
    pub client_mcp_extensions: ClientMcpExtensions,
    pub auth: Option<WorkxAuth>,
    pub auth_manager: Option<Arc<AuthManager>>,
    pub elicitation_reviewer: Option<ElicitationReviewerHandle>,
    pub elicitation_lifecycle: Option<ElicitationLifecycle>,
}

/// Owns all mutable MCP state for one Workx thread.
///
...
impl McpRuntime {
    ...
    pub async fn read_resource_for_call(
        ...
        let origin = self
            ...
            .find(call_id)?;
        let binding = self
            .current_binding_for_call(crate::WORKX_APPS_MCP_SERVER_NAME)
            .await
            .ok_or_else(|| anyhow::anyhow!("workx_apps MCP server is unavailable"))?;
    ...
    pub async fn replace_fresh(&self, input: McpRuntimeInput) -> anyhow::Result<Vec<ToolInfo>> {
        ...
        self.latest_hard_refresh_workx_apps_tools_cache().await
    }
    ...
    async fn publish(&self, input: McpRuntimeInput, previous: Option<&McpConnectionSet>) {
        ...
        let hosted_event_server_retained = connections.contains_server(WORKX_APPS_MCP_SERVER_NAME)
            && config
                .mcp_server_catalog
                ...
                .is_some_and(|registration| {
                    registration
                        .source()
                        .is_host_owned_apps(WORKX_APPS_MCP_SERVER_NAME, registration.config())
                });
    ...
    pub fn current_auth_matches(&self, auth: Option<&WorkxAuth>) -> bool {
    ...
    pub async fn latest_hard_refresh_workx_apps_tools_cache(
        &self,
...
pub fn apply_http_headers_helper(
    ...
    config: &workx_config::McpServerConfig,
    local_process_cwd: PathBuf,
    ...
    let workx_config::McpServerTransportConfig::StreamableHttp {
        url,
    ...
    pub(crate) fn resolve_server_environment(
        ...
        if config.is_local_environment() {
            return match config.transport {
                workx_config::McpServerTransportConfig::Stdio { .. } => Err(format!(
                    "local stdio MCP server `{server_name}` requires a local environment"
                )),
                workx_config::McpServerTransportConfig::StreamableHttp { .. } => Ok(None),
            };
        }

//! Aggregates MCP server connections for Workx.
//!
...
use startup::should_share_workx_apps_tools_cache;
...
use crate::pagination::MAX_WORKX_APPS_TOOL_CATALOG_ITEMS;
...
use crate::rmcp_client::prepare_workx_apps_tools_for_model;
...
use workx_protocol::protocol::McpStartupCompleteEvent;
use workx_protocol::protocol::McpStartupFailure;
...
use workx_rmcp_client::determine_streamable_http_auth_status_from_credentials;
...
impl McpServerView {
    async fn listed_tools(
        ...
        Some(if self.connection.client.is_workx_apps_mcp_server {
            prepare_workx_apps_tools_for_model(tools, tool_plugin_provenance)
        } else {
...
pub(crate) struct McpConnectionSet {
    ...
    workx_apps_tools_override: RwLock<Option<Vec<ToolInfo>>>,
    workx_apps_refresh_lock: Mutex<()>,
...
impl McpConnectionSet {
    ...
    pub async fn new(
        ...
        let static_chatgpt_auth_provider = auth
            .filter(|auth| auth.uses_workx_backend())
            .map(workx_model_provider::auth_provider_from_auth);
        let workx_apps_auth_provider = auth_manager.as_ref().and_then(|auth_manager| {
            auth.filter(|auth| auth.uses_workx_backend()).map(|auth| {
                workx_model_provider::auth_provider_from_auth_manager(
            ...
            .filter(|(_, server)| server.enabled())
        {
            ...
            let is_host_owned_workx_apps = registration.is_some_and(|server| {
                server
                    .source()
            ...
            let catalog_item_limit = if is_host_owned_workx_apps {
                MAX_WORKX_APPS_TOOL_CATALOG_ITEMS
            } else {
            ...
            // For built-in Workx Apps, `WORKX_CONNECTORS_TOKEN` is a debug
            ...
            let shares_workx_apps_tools_cache = is_host_owned_workx_apps
                && should_share_workx_apps_tools_cache(&server_name, uses_env_bearer_token);
            let workx_apps_tools_cache_context = shares_workx_apps_tools_cache.then(|| {
                workx_apps_tools_cache
                    .context(workx_home.clone(), workx_apps_tools_cache_key.clone())
            });
            // The reserved Workx Apps registration follows the shared
            ...
            let chatgpt_auth_provider = if server_name == WORKX_APPS_MCP_SERVER_NAME {
                workx_apps_auth_provider
                    .clone()
                    .or_else(|| static_chatgpt_auth_provider.clone())
            } else {
                static_chatgpt_auth_provider.clone()
            };
            // If Workx Apps has an env bearer token, that is its auth path. Do
            // not also attach the ambient WorkxAuth provider.
            let runtime_auth_provider =
                if server_name == WORKX_APPS_MCP_SERVER_NAME && uses_env_bearer_token {
                    None
                } else {
                    chatgpt_auth_provider_for_server(&server, chatgpt_auth_provider)
                };
            if is_host_owned_workx_apps {
            ...
            let connection_identity = McpServerConnectionIdentity::new(
                ...
                auth,
                shares_workx_apps_tools_cache
                    .then(|| (workx_home.clone(), workx_apps_tools_cache_key.clone())),
            ...
            let expected_protocol_mode = match &configured_config.transport {
                ...
                McpServerTransportConfig::Stdio { env, .. } => match env
                    .as_ref()
                    .and_then(|variables| variables.get("WORKX_MCP_PROTOCOL_VERSION"))
                {
            ...
            let tool_catalog_cache_context = if server_name == WORKX_APPS_MCP_SERVER_NAME {
                None
    ...
    /// Workx Apps metadata may come from its existing cache; regular MCP server information is

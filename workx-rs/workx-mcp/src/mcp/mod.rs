...
use workx_model_provider::CHATGPT_WORKX_BASE_URL;
use workx_protocol::mcp::ClientMcpExtensions;
use workx_protocol::mcp::McpServerInfo;
use workx_protocol::mcp::Resource;
use workx_protocol::mcp::ResourceTemplate;
use workx_protocol::mcp::Tool;
use workx_protocol::models::PermissionProfile;
use workx_protocol::protocol::AskForApproval;
use workx_protocol::protocol::McpAuthStatus;
...
pub const WORKX_APPS_MCP_SERVER_NAME: &str = "workx_apps";
const DEFAULT_WORKX_APPS_MCP_PRODUCT_SKU: &str = "workx";
const MCP_TOOL_NAME_PREFIX: &str = "mcp";
const MCP_TOOL_NAME_DELIMITER: &str = "__";
const WORKX_CONNECTORS_TOKEN_ENV_VAR: &str = "WORKX_CONNECTORS_TOKEN";
...
/// MCP runtime settings derived from `workx_core::config::Config`.
///
...
pub struct McpConfig {
    ...
    /// Workx home directory used for MCP OAuth state and app-tool cache files.
    ...
    /// Optional path to `workx-linux-sandbox` for sandboxed MCP tool execution.
...
pub fn host_owned_workx_apps_enabled(config: &McpConfig, auth: Option<&WorkxAuth>) -> bool {
    config.apps_enabled && auth.is_some_and(WorkxAuth::uses_workx_backend)
}
...
fn is_trusted_chatgpt_mcp_server(
    ...
    if url::Url::parse(CHATGPT_WORKX_BASE_URL)
        .ok()
...
pub fn effective_mcp_servers_from_configured(
    ...
    let mut servers = configured_servers
        ...
        .collect::<HashMap<_, _>>();
    if !host_owned_workx_apps_enabled(config, auth) {
        servers.remove(WORKX_APPS_MCP_SERVER_NAME);
    }
    servers
}
...
pub async fn read_mcp_resource(
    ...
    let manager = McpConnectionSet::new(
        ...
        McpRuntimeInput {
            ...
            tool_catalog_cache,
            workx_apps_tools_cache_key: connector_runtime_context_key(auth),
...
fn workx_apps_mcp_bearer_token_env_var() -> Option<String> {
    match env::var(WORKX_CONNECTORS_TOKEN_ENV_VAR) {
        Ok(value) if !value.trim().is_empty() => Some(WORKX_CONNECTORS_TOKEN_ENV_VAR.to_string()),
        Ok(_) => None,
        Err(env::VarError::NotPresent) => None,
        Err(env::VarError::NotUnicode(_)) => Some(WORKX_CONNECTORS_TOKEN_ENV_VAR.to_string()),
    }
}

fn normalize_workx_apps_base_url(base_url: &str) -> String {
...
fn workx_apps_mcp_url_for_base_url(base_url: &str) -> String {
    let base_url = normalize_workx_apps_base_url(base_url);
...
pub fn workx_apps_mcp_server_config(
    chatgpt_base_url: &str,
    apps_mcp_product_sku: Option<&str>,
    originator: Option<&str>,
) -> McpServerConfig {
    mcp_server_config_for_url(
        workx_apps_mcp_url_for_base_url(chatgpt_base_url),
...
pub fn hosted_plugin_runtime_mcp_server_config(
    ...
    workx_apps_mcp_server_config(chatgpt_base_url, apps_mcp_product_sku, originator)
}
...
fn mcp_server_config_for_url(
    ...
    let product_sku = apps_mcp_product_sku.unwrap_or(DEFAULT_WORKX_APPS_MCP_PRODUCT_SKU);
    ...
    McpServerConfig {
        transport: McpServerTransportConfig::StreamableHttp {
            url,
            bearer_token_env_var: workx_apps_mcp_bearer_token_env_var(),

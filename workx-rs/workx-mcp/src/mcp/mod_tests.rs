...
pub(crate) fn test_mcp_config(workx_home: PathBuf) -> McpConfig {
    McpConfig {
        ...
        config_layer_stack: workx_config::ConfigLayerStack::default(),
        approvals_reviewer: workx_config::types::ApprovalsReviewer::default(),
        ...
        connector_snapshot: workx_connectors::ConnectorSnapshot::default(),
    }
}
...
fn mcp_server_permissions_handle_unattached_and_threadless_servers() {
    ...
    let mut missing_server = workx_apps_mcp_server_config(
        "https://example.com",
        /*apps_mcp_product_sku*/ None,
        /*originator*/ None,
    );
...
#[test]
fn workx_apps_mcp_url_for_base_url_uses_plugin_service_paths() {
    assert_eq!(
        workx_apps_mcp_url_for_base_url("https://chatgpt.com/backend-api"),
        "https://chatgpt.com/backend-api/ps/mcp"
    );
    assert_eq!(
        workx_apps_mcp_url_for_base_url("https://chat.openai.com"),
        "https://chat.openai.com/backend-api/ps/mcp"
    );
    assert_eq!(
        workx_apps_mcp_url_for_base_url("http://localhost:8080/api/codex"),
        "http://localhost:8080/api/codex/ps/mcp"
    );
    assert_eq!(
        workx_apps_mcp_url_for_base_url("http://localhost:8080"),
        "http://localhost:8080/api/codex/ps/mcp"
    );
}

#[test]
fn workx_apps_server_config_uses_plugin_service_path() {
    let config = workx_apps_mcp_server_config(
        "https://chatgpt.com",
        /*apps_mcp_product_sku*/ None,
        /*originator*/ None,
    );
    let url = match &config.transport {
        ...
        _ => panic!("expected streamable http transport for workx apps"),
    };
...
#[test]
fn workx_apps_server_config_forwards_thread_originator_header() {
    let config = workx_apps_mcp_server_config(
        "https://chatgpt.com",
        /*apps_mcp_product_sku*/ None,
        ...
        McpServerTransportConfig::StreamableHttp {
            ...
            assert_eq!(
                ...
                &Some(HashMap::from([
                    ...
                    ("X-OpenAI-Product-Sku".to_string(), "workx".to_string()),
                ]))
            );
...
#[test]
fn workx_apps_server_config_sets_product_sku_header() {
    for (configured_product_sku, expected_product_sku) in [(None, "workx"), (Some("tpp"), "tpp")] {
        let config = workx_apps_mcp_server_config(
            "https://chatgpt.com",
...
#[test]
fn workx_apps_server_config_forwards_originator_and_configured_product_sku_headers() {
    let config = workx_apps_mcp_server_config(
        "https://chatgpt.com",
        Some("tpp"),
        Some("thread_originator"),
...
fn effective_mcp_servers_preserve_chatgpt_auth_for_staging() {
    for url in [
        ...
        let server = workx_apps_mcp_server_config(
...
async fn effective_mcp_servers_preserve_runtime_servers() {
    ...
    let mut config = test_mcp_config(workx_home.path().to_path_buf());
    ...
    let workx_apps = effective
        .get(WORKX_APPS_MCP_SERVER_NAME)
        .expect("workx apps server should exist");

    let sample = sample.config();
    let docs = docs.config();
    let workx_apps = workx_apps.config();

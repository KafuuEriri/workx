...
pub fn has_current_session_been_imported(
    ...
    load_import_ledger(workx_home)?.contains_current_source(source_path)
}
...
pub(crate) fn find_existing_session_import(
    ...
    let source_path = canonical_source_path(source_path)?;
    let ledger = load_import_ledger(workx_home)?;
...
pub(super) fn checkpoint_existing_session_import(
    ...
    let mut ledger = load_import_ledger(workx_home)?;
    ...
    save_import_ledger(workx_home, &ledger)?;
    Ok(true)
}
...
pub(crate) fn load_import_ledger(
    ...
    let path = import_ledger_path(workx_home);
...
fn import_ledger_path(workx_home: &Path) -> PathBuf {
    workx_home.join(SESSION_IMPORT_LEDGER_FILE)
}

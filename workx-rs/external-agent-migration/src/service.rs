...
const EXTERNAL_AGENT_CONFIG_IMPORT_METRIC: &str = "workx.external_agent_config.import";

#[derive(Clone)]
pub struct ExternalAgentConfigService {
    pub(super) workx_home: PathBuf,
...
impl ExternalAgentConfigService {
    ...
    pub fn with_migration_source(&self, migration_source: Option<&str>) -> Self {
        ...
        let connector_metadata_roots = source.connector_metadata_roots(&external_agent_home);
        Self {
            workx_home: self.workx_home.clone(),
            connector_metadata_roots,
            external_agent_home,
    ...
    pub fn workx_home(&self) -> &Path {
        &self.workx_home
    }

    #[cfg(test)]
    fn new_for_test(workx_home: PathBuf, external_agent_home: PathBuf) -> Self {
        let source = ExternalAgentSource::default();
        let connector_metadata_roots = source.connector_metadata_roots(&external_agent_home);
        Self {
            workx_home,
            ...
            auth_manager: workx_login::test_support::auth_manager_from_optional_auth(
                /*auth*/ None,
            ),
            source,
            session_import_limits: ExternalAgentSessionImportLimits::default(),
            ...
            let import_result = match migration_item.item_type {
                ...
                ExternalAgentConfigMigrationItemType::Memory if self.source.supports_memory() => {
                    async {
                        ...
                        let target_path = memory_import::resources_root(&self.workx_home);
    ...
    fn import_config(&self, cwd: Option<&Path>) -> io::Result<Option<(String, String)>> {
        ...
        let target_config = match &scope {
            MigrationScope::Home => self.workx_home.join("config.toml"),
            MigrationScope::Repository { root } => root.join(".workx").join("config.toml"),
        };
    ...
    fn import_hooks(&self, cwd: Option<&Path>) -> io::Result<Vec<String>> {
        ...
        let target_hooks = match &scope {
            MigrationScope::Home => self.workx_home.join("hooks.json"),
            MigrationScope::Repository { root } => root.join(".workx").join("hooks.json"),
        };
    ...
    fn import_agents_md(&self, cwd: Option<&Path>) -> io::Result<Option<(String, String)>> {
        ...
        let (source_agents_md, target_agents_md) = match scope {
            ...
            MigrationScope::Home => {
                ...
                (source_agents_md, self.workx_home.join("AGENTS.md"))
            }
        };
        ...
        let plugins = marketplace
            ...
            .filter(|plugin| {
                plugin
                    ...
                    .products
                    .as_deref()
                    .is_none_or(|products| Product::Workx.matches_product_restriction(products))
            })

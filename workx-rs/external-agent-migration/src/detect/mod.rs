...
const EXTERNAL_AGENT_CONFIG_DETECT_METRIC: &str = "workx.external_agent_config.detect";
...
impl ExternalAgentConfigService {
    pub async fn detect(
        ...
        if params.include_home
            ...
            && let Some(item) = memory::detect(&self.workx_home, &self.external_agent_home)?
        {
            items.push(item);
    ...
    async fn detect_migrations(
        ...
        let target_config = repo_root.map_or_else(
            || self.workx_home.join("config.toml"),
            |repo_root| repo_root.join(".workx").join("config.toml"),
        );
        ...
        let target_hooks = repo_root.map_or_else(
            || self.workx_home.join("hooks.json"),
            |repo_root| repo_root.join(".workx").join("hooks.json"),
        );
        ...
        let target_subagents = repo_root.map_or_else(
            ...
            |repo_root| repo_root.join(".workx").join("agents"),
        );

...
async fn authenticated_plugin_migration_uses_chatgpt_curated_marketplace() {
    ...
    fs::create_dir_all(plugin_root.join(".workx-plugin")).expect("create curated plugin directory");
    fs::write(
  ...
  "plugins": [{
    ...
    let mut service = service_for_paths(external_agent_home.clone(), workx_home);
...
fn marketplace_import_sources_prefers_scoped_source_over_registry_name_collision() {
    let (root, external_agent_home, _workx_home) = fixture_paths();
...
fn marketplace_import_sources_prefers_supported_declaration_over_materialization() {
    let (_root, external_agent_home, _workx_home) = fixture_paths();
...
#[tokio::test]
async fn detect_repo_skips_plugins_that_are_disabled_in_workx() {
    let root = TempDir::new().expect("create tempdir");
    let external_agent_home = root.path().join(EXTERNAL_AGENT_DIR);
    let workx_home = root.path().join(".workx");
    let repo_root = root.path().join("repo");
...
#[tokio::test]
async fn detect_repo_skips_plugins_without_explicit_enabled_in_workx() {
    let root = TempDir::new().expect("create tempdir");
    let external_agent_home = root.path().join(EXTERNAL_AGENT_DIR);
    let workx_home = root.path().join(".workx");
    let repo_root = root.path().join("repo");
...
async fn import_plugins_requires_details() {
    ...
    let err = service_for_paths(external_agent_home, workx_home)
...
#[tokio::test]
async fn detect_repo_skips_plugins_only_configured_in_project_workx() {
    let root = TempDir::new().expect("create tempdir");
    let external_agent_home = root.path().join(EXTERNAL_AGENT_DIR);
    let workx_home = root.path().join(".workx");
    let repo_root = root.path().join("repo");
    fs::create_dir_all(repo_root.join(".git")).expect("create git dir");
    fs::create_dir_all(repo_root.join(EXTERNAL_AGENT_DIR)).expect("create repo external agent dir");
    fs::create_dir_all(repo_root.join(".workx")).expect("create repo workx dir");
    fs::create_dir_all(&workx_home).expect("create workx home");
    fs::write(
        repo_root.join(EXTERNAL_AGENT_DIR).join("settings.json"),
        r#"{
...
"#,
    ...
    .expect("write project workx config");
...
async fn detect_repo_skips_plugins_even_with_installed_marketplace() {
    ...
    let workx_home = root.path().join(".workx");
    let repo_root = root.path().join("repo");
    let marketplace_root = workx_home.join(".tmp").join("marketplaces").join("debug");
    fs::create_dir_all(repo_root.join(".git")).expect("create git dir");
    fs::create_dir_all(repo_root.join(EXTERNAL_AGENT_DIR)).expect("create repo external agent dir");

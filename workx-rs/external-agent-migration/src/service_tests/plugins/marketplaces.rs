...
async fn import_plugins_requires_source_marketplace_details() {
    let (_root, external_agent_home, workx_home) = fixture_paths();
    ...
    let outcome = service_for_paths(external_agent_home, workx_home)
        .import_plugins(
            /*cwd*/ None,
...
async fn import_plugins_treats_empty_cwd_as_home_scope() {
    ...
    fs::create_dir_all(plugin_root.join(".workx-plugin")).expect("create plugin manifest dir");
    ...
    fs::write(
        plugin_root.join(".workx-plugin").join("plugin.json"),
        r#"{"name":"cloudflare","version":"0.1.0"}"#,
    )
    .expect("write plugin manifest");

    let outcome = service_for_paths(external_agent_home, workx_home.clone())
        .import_plugins(
    ...
    let config = fs::read_to_string(workx_home.join("config.toml")).expect("read config");
...
async fn import_plugins_reuses_configured_marketplace_with_different_source() {
    ...
    fs::create_dir_all(configured_plugin_root.join(".workx-plugin"))
    ...
    fs::create_dir_all(source_plugin_root.join(".workx-plugin"))
...
"#
    ...
    fs::write(
        source_plugin_root.join(".workx-plugin/plugin.json"),
    ...
    let config: TomlValue =
        toml::from_str(&fs::read_to_string(workx_home.join("config.toml")).expect("read config"))
            .expect("parse config");
...
async fn detect_home_supports_relative_external_agent_plugin_marketplace_path() {
    ...
    let items = service_for_paths(external_agent_home.clone(), workx_home)
...
async fn detect_repo_skips_project_relative_external_agent_plugin_marketplace_path() {
    ...
    let items = service_for_paths(external_agent_home, workx_home)
        .detect(ExternalAgentConfigDetectOptions {
            include_home: false,
            include_memory: false,
...
async fn import_rejects_forged_project_relative_external_agent_plugin_item() {
    ...
    assert!(!workx_home.join("config.toml").exists());
}
...
async fn import_plugins_rejects_project_cwd_before_config_loading() {
    ...
    fs::create_dir_all(repo_root.join(".git")).expect("create git dir");
    fs::create_dir_all(&workx_home).expect("create workx home");
    fs::write(workx_home.join("config.toml"), "not valid toml").expect("write invalid config");

    let error = service_for_paths(external_agent_home, workx_home)
...
fn import_skills_returns_only_new_skill_directory_names() {
    ...
    let copied_names = service_for_paths(external_agent_home, workx_home)
...
fn import_cursor_skills_reads_user_and_managed_directories() {
    ...
    let mut service = service_for_paths(external_agent_home, workx_home);

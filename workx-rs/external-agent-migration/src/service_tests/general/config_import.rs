...
async fn import_repo_mcp_preserves_existing_same_named_server() {
    ...
    .expect("write mcp");
    fs::create_dir_all(repo_root.join(".workx")).expect("create workx dir");
    let existing_config = r#"[mcp_servers.mixedTransport]
url = "https://example.com/mixed-transport"
"#;
    fs::write(
        repo_root.join(".workx").join("config.toml"),
        existing_config,
    )
    .expect("write config");
...
async fn import_home_migrates_supported_config_fields_skills_and_agents_md() {
    ...
    fs::write(
            ...
            format!(r#"{{"model":"{SOURCE_EXTERNAL_AGENT_NAME}","permissions":{{"ask":["git push"]}},"env":{{"FOO":"bar","CI":false,"MAX_RETRIES":3,"MY_TEAM":"workx","IGNORED":null,"LIST":["a","b"],"MAP":{{"x":1}}}},"sandbox":{{"enabled":true,"network":{{"allowLocalBinding":true}}}}}}"#),
        )
    ...
    service_for_paths(external_agent_home, workx_home.clone())
        .import(vec![
    ...
    assert_eq!(
        fs::read_to_string(workx_home.join("AGENTS.md")).expect("read agents"),
        "Workx guidance"
    );
...
async fn import_home_config_ignores_invalid_local_settings() {
    ...
    assert_eq!(
        fs::read_to_string(workx_home.join("config.toml")).expect("read config"),
        "[shell_environment_policy]\ninherit = \"core\"\n\n[shell_environment_policy.set]\nFOO = \"project\"\n"
    );
}

...
async fn detect_home_lists_config_skills_and_agents_md() {
    ...
    let items = service_for_paths(external_agent_home.clone(), workx_home.clone())
    ...
    let expected = vec![
        ExternalAgentConfigMigrationItem {
            ...
            description: format!(
                ...
                workx_home.join("config.toml").display()
            ),
            cwd: None,
            details: None,
        },
        ...
        ExternalAgentConfigMigrationItem {
            ...
            description: format!(
                ...
                workx_home.join("AGENTS.md").display()
            ),
            cwd: None,
            details: None,
        },
...
async fn detect_and_import_claude_commands_without_frontmatter() {
    let (root, external_agent_home, workx_home) = fixture_paths();
    let repo_root = root.path().join("repo");
    fs::create_dir_all(repo_root.join(".git")).expect("create git dir");
    let service = service_for_paths(external_agent_home.clone(), workx_home);
...
async fn detect_cursor_repo_lists_skills_from_skills() {
    ...
    let mut service = service_for_paths(root.path().join(".cursor"), workx_home);
    service.source = ExternalAgentSource::Cur;
...
async fn detect_repo_still_reports_non_plugin_items_when_home_config_is_invalid() {
    ...
    fs::write(workx_home.join("config.toml"), "this is not valid = [toml")
        .expect("write invalid workx config");
    fs::write(
    ...
    let items = service_for_paths(root.path().join(EXTERNAL_AGENT_DIR), workx_home)
        .detect(ExternalAgentConfigDetectOptions {
            include_home: false,
            include_memory: false,
            cwds: Some(vec![repo_root.clone()]),
            ...
            ExternalAgentConfigMigrationItem {
                ...
                description: format!(
                    ...
                    repo_root
                        ...
                        .join("settings.json")
                        .display(),
                    repo_root.join(".workx").join("config.toml").display()
                ),
                cwd: Some(repo_root.clone()),
                details: None,
            },
        ...
        vec![
            ...
            ExternalAgentConfigMigrationItem {
                ...
                description: format!(
                    ...
                    repo_root.join(".workx").join("hooks.json").display()
                ),
                cwd: Some(repo_root.clone()),
                details: Some(MigrationDetails {
            ...
            ExternalAgentConfigMigrationItem {
                ...
                description: format!(
                    ...
                    repo_root.join(".workx").join("agents").display()
                ),
                cwd: Some(repo_root),
                details: Some(MigrationDetails {
...
async fn import_repo_migrates_mcp_hooks_commands_and_subagents() {
    ...
    let config: TomlValue = toml::from_str(
        &fs::read_to_string(repo_root.join(".workx").join("config.toml")).expect("read config"),
    )
    .expect("parse config");
...
"#,
    ...
    let _supported_mcp_config: std::collections::HashMap<
        String,
        workx_config::types::McpServerConfig,
    > = mcp_servers
        .try_into()
        .expect("migrated MCP config should be supported");

    let hooks: JsonValue = serde_json::from_str(
        &fs::read_to_string(repo_root.join(".workx").join("hooks.json")).expect("read hooks"),
    )
    .expect("parse hooks");
    let _supported_hooks: workx_config::HooksFile =

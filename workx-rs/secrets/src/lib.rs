...
const KEYRING_SERVICE: &str = "workx";
...
impl SecretsManager {
    pub fn new(workx_home: PathBuf, backend_kind: SecretsBackendKind) -> Self {
        let backend: Arc<dyn SecretsBackend> = match backend_kind {
            SecretsBackendKind::Local => {
                let keyring_store: Arc<dyn KeyringStore> = Arc::new(DefaultKeyringStore);
                Arc::new(LocalSecretsBackend::new(workx_home, keyring_store))
            }
        };
        Self { backend }
    }
...
pub fn compute_keyring_account(workx_home: &Path) -> String {

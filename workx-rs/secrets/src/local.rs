...
const WORKX_AUTH_SECRETS_FILENAME: &str = "workx_auth.age";
...
pub enum LocalSecretsNamespace {
    ...
    #[default]
    ManagedSecrets,
    /// Workx authentication credentials used by the CLI, TUI, app server, and other clients.
    WorkxAuth,
...
impl LocalSecretsBackend {
    pub fn new(workx_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
    ...
    fn load_or_create_passphrase(&self) -> Result<SecretString> {
        let account = compute_keyring_account(&self.workx_home);
        let loaded = self
            .keyring_store
...
mod tests {
    ...
    fn load_file_rejects_newer_schema_versions() -> Result<()> {
        ...
        let backend = LocalSecretsBackend::new(workx_home.path().to_path_buf(), keyring);
    ...
    fn set_fails_when_keyring_is_unavailable() -> Result<()> {
        let workx_home = tempfile::tempdir().expect("tempdir");
        let keyring = Arc::new(MockKeyringStore::default());
        let account = compute_keyring_account(workx_home.path());
        keyring.set_error(
            &account,
    ...
    fn local_namespaces_write_separate_files() -> Result<()> {
        ...
        let workx_home = tempfile::tempdir().expect("tempdir");
        let keyring = Arc::new(MockKeyringStore::default());
        let workx_auth_backend = LocalSecretsBackend::new_with_namespace(
        ...
        workx_auth_backend.set(&scope, &name, "workx-auth-value")?;
        mcp_backend.set(&scope, &name, "mcp-value")?;

        assert_eq!(
            workx_auth_backend.get(&scope, &name)?,
            Some("workx-auth-value".to_string())
        );
        assert!(
            MCP_OAUTH_CACHE
                .lock()
                .unwrap_or_else(PoisonError::into_inner)
                .as_ref()
                .is_none_or(|cached| cached.path != workx_auth_backend.secrets_path())
        );
        assert_eq!(
        ...
        assert!(!workx_home.path().join("secrets").join("local.age").exists());
        Ok(())
    }

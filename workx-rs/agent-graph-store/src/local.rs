...
fn to_state_status(status: ThreadSpawnEdgeStatus) -> workx_state::DirectionalThreadSpawnEdgeStatus {
    match status {
        ThreadSpawnEdgeStatus::Open => workx_state::DirectionalThreadSpawnEdgeStatus::Open,
        ThreadSpawnEdgeStatus::Closed => workx_state::DirectionalThreadSpawnEdgeStatus::Closed,
    }
}
...
mod tests {
    ...
    async fn state_runtime() -> TestRuntime {
        let workx_home = TempDir::new().expect("tempdir should be created");

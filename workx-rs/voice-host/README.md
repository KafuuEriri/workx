...
`workx-voice-host` establishes the inherited-pipe lifecycle for the proposed
...
`workx-resources/voice/bin/workx-voice-host[.exe]`, filters the child environment,
and owns process cleanup through `workx-utils-pty`. Its runtime must remain alive

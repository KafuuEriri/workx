...
use workx_realtime_webrtc::Message;
use workx_realtime_webrtc::VoiceHost;
use workx_realtime_webrtc::decode_frame;
use workx_realtime_webrtc::encode_frame;
use workx_utils_cargo_bin::cargo_bin;

const DEADLINE: Duration = Duration::from_secs(/*secs*/ 10);

fn spawn() -> Result<Child> {
    Ok(Command::new(cargo_bin("workx-voice-host")?)
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .kill_on_drop(true)
...
async fn build_commit() -> Result<String> {
    let output = timeout(
        DEADLINE,
        Command::new(cargo_bin("workx-voice-host")?)
            .arg("--build-commit")
            .kill_on_drop(true)
            .output(),
    )
...
async fn installed_client_rejects_mixed_builds_and_missing_helper() -> Result<()> {
    ...
    let helper_dir = directory.path().join("workx-resources/voice/bin");
    fs::create_dir_all(&bin)?;
    fs::create_dir_all(&helper_dir)?;
    fs::write(directory.path().join("workx-package.json"), "{}")?;
    let app = bin.join(if cfg!(windows) { "workx.exe" } else { "workx" });
    fs::write(&app, [])?;
    let source = cargo_bin("workx-voice-host")?;
...
async fn installed_client_accepts_non_utf8_package_path() -> Result<()> {
    ...
    let helper_dir = root.join("workx-resources/voice/bin");
    fs::create_dir_all(&bin)?;
    fs::create_dir_all(&helper_dir)?;
    fs::write(root.join("workx-package.json"), "{}")?;
    let app = bin.join("workx");
    fs::write(&app, [])?;
    let source = cargo_bin("workx-voice-host")?;

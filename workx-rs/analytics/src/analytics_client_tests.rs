...
use crate::events::WorkxAcceptedLineFingerprintsEventParams;
use crate::events::WorkxAcceptedLineFingerprintsEventRequest;
use crate::events::WorkxAppMentionedEventRequest;
use crate::events::WorkxAppServerClientMetadata;
use crate::events::WorkxAppUsedEventRequest;
use crate::events::WorkxCommandExecutionEventParams;
use crate::events::WorkxCommandExecutionEventRequest;
use crate::events::WorkxCompactionEventRequest;
use crate::events::WorkxHookRunEventRequest;
use crate::events::WorkxOnboardingExternalAgentImportFailureEventRequest;
use crate::events::WorkxOnboardingExternalAgentImportFailureMetadata;
use crate::events::WorkxPluginEventRequest;
use crate::events::WorkxPluginInstallFailedEventRequest;
use crate::events::WorkxPluginInstallFailedMetadata;
use crate::events::WorkxPluginUsedEventRequest;
use crate::events::WorkxReviewEventParams;
use crate::events::WorkxReviewEventRequest;
use crate::events::WorkxRuntimeMetadata;
use crate::events::WorkxToolItemEventBase;
use crate::events::WorkxTurnEventRequest;
use crate::events::current_runtime_metadata;
use crate::events::subagent_thread_started_event_request;
use crate::events::workx_app_metadata;
use crate::events::workx_hook_run_metadata;
use crate::events::workx_plugin_metadata;
use crate::events::workx_plugin_used_metadata;
...
use crate::facts::TurnWorkxErrorFact;
use crate::facts::WorkxCompactionEvent;
use crate::facts::WorkxErrKind;
...
use tokio::sync::mpsc;
use workx_app_server_protocol::ApprovalsReviewer as AppServerApprovalsReviewer;
use workx_app_server_protocol::AskForApproval as AppServerAskForApproval;
...
use workx_app_server_protocol::CollabAgentTool;
use workx_app_server_protocol::CollabAgentToolCallStatus;
use workx_app_server_protocol::CommandAction;
use workx_app_server_protocol::CommandExecutionApprovalDecision;
use workx_app_server_protocol::CommandExecutionRequestApprovalParams;
use workx_app_server_protocol::CommandExecutionRequestApprovalResponse;
use workx_app_server_protocol::CommandExecutionSource;
use workx_app_server_protocol::CommandExecutionStatus;
use workx_app_server_protocol::DynamicToolCallStatus;
use workx_app_server_protocol::GuardianApprovalReview;
use workx_app_server_protocol::GuardianApprovalReviewAction;
use workx_app_server_protocol::GuardianApprovalReviewStatus;
use workx_app_server_protocol::GuardianCommandSource as AppServerGuardianCommandSource;
use workx_app_server_protocol::ImageGenerationItem;
use workx_app_server_protocol::InitializeCapabilities;
use workx_app_server_protocol::InitializeParams;
use workx_app_server_protocol::ItemCompletedNotification;
use workx_app_server_protocol::ItemGuardianApprovalReviewCompletedNotification;
use workx_app_server_protocol::ItemStartedNotification;
use workx_app_server_protocol::JSONRPCErrorError;
use workx_app_server_protocol::McpToolCallAppContext;
use workx_app_server_protocol::McpToolCallStatus;
use workx_app_server_protocol::NonSteerableTurnKind;
use workx_app_server_protocol::PatchApplyStatus;
use workx_app_server_protocol::PermissionsRequestApprovalParams;
use workx_app_server_protocol::RequestId;
use workx_app_server_protocol::RequestPermissionProfile;
use workx_app_server_protocol::SandboxPolicy as AppServerSandboxPolicy;
use workx_app_server_protocol::ServerNotification;
use workx_app_server_protocol::ServerRequest;
use workx_app_server_protocol::ServerResponse;
use workx_app_server_protocol::SessionSource as AppServerSessionSource;
use workx_app_server_protocol::SubAgentActivityKind;
...
use workx_app_server_protocol::ThreadSource as AppServerThreadSource;
use workx_app_server_protocol::ThreadStartResponse;
use workx_app_server_protocol::ThreadStatus as AppServerThreadStatus;
use workx_app_server_protocol::ThreadUnarchivedNotification;
use workx_app_server_protocol::Turn;
use workx_app_server_protocol::TurnCompletedNotification;
use workx_app_server_protocol::TurnDiffUpdatedNotification;
use workx_app_server_protocol::TurnError as AppServerTurnError;
use workx_app_server_protocol::TurnInterruptResponse;
use workx_app_server_protocol::TurnStartParams;
use workx_app_server_protocol::TurnStartedNotification;
use workx_app_server_protocol::TurnStatus as AppServerTurnStatus;
use workx_app_server_protocol::TurnSteerParams;
use workx_app_server_protocol::TurnSteerResponse;
use workx_app_server_protocol::UserInput;
use workx_app_server_protocol::WebSearchItem;
use workx_app_server_protocol::WorkxErrorInfo;
use workx_login::default_client::DEFAULT_ORIGINATOR;
use workx_login::default_client::originator;
use workx_plugin::AppConnectorId;
use workx_plugin::PluginCapabilitySummary;
use workx_plugin::PluginId;
use workx_plugin::PluginTelemetryMetadata;
use workx_protocol::approvals::NetworkApprovalProtocol;
...
use workx_protocol::models::NetworkPermissions as CoreNetworkPermissions;
use workx_protocol::models::PermissionProfile as CorePermissionProfile;
use workx_protocol::protocol::AskForApproval;
use workx_protocol::protocol::HookEventName;
use workx_protocol::protocol::HookExecutionMode;
use workx_protocol::protocol::HookHandlerType;
use workx_protocol::protocol::HookRunStatus;
use workx_protocol::protocol::HookSource;
...
use workx_protocol::request_permissions::PermissionGrantScope as CorePermissionGrantScope;
use workx_protocol::request_permissions::RequestPermissionProfile as CoreRequestPermissionProfile;
use workx_protocol::request_permissions::RequestPermissionsResponse as CoreRequestPermissionsResponse;
use workx_utils_absolute_path::test_support::PathBufExt;
use workx_utils_absolute_path::test_support::test_path_buf;

const TEST_PRODUCT_CLIENT_ID: &str = "workx_work_desktop";
...
impl TurnAnalyticsMetadata for TestTurnMetadata {
    ...
    fn workx_turn_source(&self) -> Option<String> {
        None
    }
}
...
fn sample_app_server_client_metadata() -> WorkxAppServerClientMetadata {
    WorkxAppServerClientMetadata {
...
fn sample_runtime_metadata() -> WorkxRuntimeMetadata {
    WorkxRuntimeMetadata {
        workx_rs_version: "0.1.0".to_string(),
...
fn sample_turn_start_response(turn_id: &str) -> ClientResponsePayload {
    ClientResponsePayload::TurnStart(workx_app_server_protocol::TurnStartResponse {
        turn: Turn {
            id: turn_id.to_string(),
            items_view: workx_app_server_protocol::TurnItemsView::Full,
            items: vec![],
...
fn sample_turn_completed_notification(
    ...
    workx_error_info: Option<workx_app_server_protocol::WorkxErrorInfo>,
) -> ServerNotification {
    ServerNotification::TurnCompleted(TurnCompletedNotification {
        ...
        turn: Turn {
            ...
            items: vec![],
            status,
            error: workx_error_info.map(|workx_error_info| AppServerTurnError {
                misalignment: None,
                message: "turn failed".to_string(),
                workx_error_info: Some(workx_error_info),
    ...
    JSONRPCErrorError {
        ...
        data: Some(
            serde_json::to_value(AppServerTurnError {
                misalignment: None,
                message: "cannot steer a review turn".to_string(),
                workx_error_info: Some(WorkxErrorInfo::ActiveTurnNotSteerable {
            ...
            AnalyticsFact::Initialize {
                ...
                params: InitializeParams {
                    client_info: ClientInfo {
                        name: "workx-web".to_string(),
                        title: None,
                        version: "1.0.0".to_string(),
                    },
                    capabilities: None,
                },
                product_client_id: "workx-web".to_string(),
    ...
    reducer
        .ingest(
            AnalyticsFact::Initialize {
                ...
                params: InitializeParams {
                    client_info: ClientInfo {
                        name: "workx-tui".to_string(),
                        title: None,
                        version: "1.0.0".to_string(),
                    },
                    capabilities: None,
                },
                product_client_id: "workx-tui".to_string(),
                runtime: sample_runtime_metadata(),
                rpc_transport: AppServerRpcTransport::Stdio,
            },
            out,
...
fn sample_initialize_fact(connection_id: u64) -> AnalyticsFact {
    AnalyticsFact::Initialize {
        ...
        product_client_id: DEFAULT_ORIGINATOR.to_string(),
        runtime: WorkxRuntimeMetadata {
            workx_rs_version: "0.99.0".to_string(),
    ...
    ServerRequest::PermissionsRequestApproval {
        ...
        params: PermissionsRequestApprovalParams {
            ...
            permissions: RequestPermissionProfile {
                network: Some(workx_app_server_protocol::AdditionalNetworkPermissions {
                    enabled: Some(true),
                }),
                file_system: None,
            },
...
fn sample_guardian_review_completed(
    ...
    ServerNotification::ItemGuardianApprovalReviewCompleted(
        ItemGuardianApprovalReviewCompletedNotification {
            ...
            decision_source: workx_app_server_protocol::AutoReviewDecisionSource::Agent,
...
fn normalize_path_for_skill_id_repo_scoped_uses_relative_path() {
    ...
    let skill_path = PathBuf::from("/repo/root/.workx/skills/doc/SKILL.md");
    ...
    assert_eq!(path, ".workx/skills/doc/SKILL.md");
}

#[test]
fn normalize_path_for_skill_id_user_scoped_uses_absolute_path() {
    let skill_path = PathBuf::from("/Users/abc/.workx/skills/doc/SKILL.md");
...
fn normalize_path_for_skill_id_admin_scoped_uses_absolute_path() {
    let skill_path = PathBuf::from("/etc/workx/skills/doc/SKILL.md");
...
fn normalize_path_for_skill_id_repo_root_not_in_skill_path_uses_absolute_path() {
    let repo_root = PathBuf::from("/repo/root");
    let skill_path = PathBuf::from("/other/path/.workx/skills/doc/SKILL.md");
...
fn app_mentioned_event_serializes_expected_shape() {
    ...
    let event = TrackEventRequest::AppMentioned(WorkxAppMentionedEventRequest {
        event_type: "workx_app_mentioned",
        event_params: workx_app_metadata(
            &tracking,
            AppInvocation {
...
fn app_used_event_serializes_expected_shape() {
    ...
    let event = TrackEventRequest::AppUsed(WorkxAppUsedEventRequest {
        event_type: "workx_app_used",
        event_params: workx_app_metadata(
            &tracking,
            AppInvocation {
...
fn accepted_line_fingerprints_event_serializes_expected_shape() {
    let event = TrackEventRequest::AcceptedLineFingerprints(Box::new(
        WorkxAcceptedLineFingerprintsEventRequest {
            event_type: "workx_accepted_line_fingerprints",
            event_params: WorkxAcceptedLineFingerprintsEventParams {
                event_type: "workx.accepted_line_fingerprints",
                turn_id: "turn-1".to_string(),
                thread_id: "thread-1".to_string(),
                product_surface: Some("workx".to_string()),
...
async fn analytics_flush_delivers_completed_turn_with_file_diff() {
    ...
    let capture_path = std::env::temp_dir().join(format!(
        "workx-analytics-turn-flush-{}-{nonce}.jsonl",
        std::process::id()
    ));
    let auth_manager = workx_login::AuthManager::from_auth_for_testing(
        workx_login::WorkxAuth::create_dummy_chatgpt_auth_for_testing(),
    );
...
"
    ...
    let event_types = contents
        ...
        .collect::<Vec<_>>();
    assert!(event_types.iter().any(|event| event == "workx_turn_event"));
    assert!(
        event_types
            .iter()
            .any(|event| event == "workx_accepted_line_fingerprints")
    );
...
fn compaction_event_serializes_expected_shape() {
    let event = TrackEventRequest::Compaction(Box::new(WorkxCompactionEventRequest {
        event_type: "workx_compaction_event",
        event_params: crate::events::workx_compaction_event_params(
            WorkxCompactionEvent {
                thread_id: "thread-1".to_string(),
                turn_id: "turn-1".to_string(),
                ...
                workx_error_kind: None,
                workx_error_http_status_code: None,
        ...
        json!({
            ...
            "event_params": {
                ...
                "app_server_client": {
                    ...
                    "client_name": "workx-tui",
                    "client_version": "1.0.0",
                    "rpc_transport": "stdio",
                    "experimental_api_enabled": true
                },
                "runtime": {
                    "workx_rs_version": "0.1.0",
                    "runtime_os": "macos",
                ...
                "strategy": "memento",
                "status": "completed",
                "workx_error_kind": null,
                "workx_error_http_status_code": null,
...
fn thread_initialized_event_serializes_expected_shape() {
    let event = TrackEventRequest::ThreadInitialized(ThreadInitializedEvent {
        event_type: "workx_thread_initialized",
        event_params: ThreadInitializedEventParams {
            thread_id: "thread-0".to_string(),
            session_id: "session-thread-0".to_string(),
            app_server_client: WorkxAppServerClientMetadata {
                product_client_id: DEFAULT_ORIGINATOR.to_string(),
                client_name: Some("workx-tui".to_string()),
                client_version: Some("1.0.0".to_string()),
                rpc_transport: AppServerRpcTransport::Stdio,
...
fn command_execution_event_serializes_expected_shape() {
    let event = TrackEventRequest::CommandExecution(WorkxCommandExecutionEventRequest {
        event_type: "workx_command_execution_event",
        event_params: WorkxCommandExecutionEventParams {
            base: WorkxToolItemEventBase {
                thread_id: "thread-1".to_string(),
                ...
                app_server_client: WorkxAppServerClientMetadata {
                    product_client_id: "workx_tui".to_string(),
                    client_name: Some("workx-tui".to_string()),
...
fn review_event_serializes_expected_shape() {
    let event = TrackEventRequest::ReviewEvent(WorkxReviewEventRequest {
        event_type: "workx_review_event",
        event_params: WorkxReviewEventParams {
            thread_id: "thread-1".to_string(),
            turn_id: "turn-1".to_string(),
            item_id: None,
...
async fn initialize_caches_client_and_thread_lifecycle_publishes_once_initialized() {
    ...
    assert_eq!(payload[0]["event_type"], "workx_thread_initialized");
    ...
    assert_eq!(
        payload[0]["event_params"]["runtime"]["workx_rs_version"],
        "0.99.0"
    );
...
async fn thread_originator_overrides_shared_connection_across_thread_events() {
    ...
    assert_eq!(
        ...
        json!([
            {
                "event_type": "workx_thread_archive_event",
                "event_params": {
                    "thread_id": "thread-work",
                    "action": "archived",
...
async fn compaction_event_ingests_custom_fact() {
    ...
    let parent_thread_id =
        workx_protocol::ThreadId::from_string("22222222-2222-2222-2222-222222222222")
        ...
        .ingest(
            AnalyticsFact::Custom(CustomAnalyticsFact::Compaction(Box::new(
                WorkxCompactionEvent {
                    ...
                    workx_error_kind: Some(WorkxErrKind::ContextWindowExceeded),
                    workx_error_http_status_code: None,
                    active_context_tokens_before: 131_000,
    ...
    let payload = serde_json::to_value(&events).expect("serialize events");
    assert_eq!(payload.as_array().expect("events array").len(), 1);
    assert_eq!(payload[0]["event_type"], "workx_compaction_event");
    assert_eq!(payload[0]["event_params"]["session_id"], "session-thread-1");
    assert_eq!(payload[0]["event_params"]["thread_id"], "thread-1");
    assert_eq!(payload[0]["event_params"]["turn_id"], "turn-compact");
    assert_eq!(
        payload[0]["event_params"]["workx_error_kind"],
        json!("context_window_exceeded")
    );
    assert_eq!(
        payload[0]["event_params"]["workx_error_http_status_code"],
        json!(null)
    );
    assert_eq!(
        payload[0]["event_params"]["app_server_client"]["product_client_id"],
...
async fn guardian_review_event_ingests_custom_fact_with_optional_target_item() {
    ...
    let payload = serde_json::to_value(&events).expect("serialize events");
    assert_eq!(payload.as_array().expect("events array").len(), 1);
    assert_eq!(payload[0]["event_type"], "workx_guardian_review");
    assert_eq!(
...
async fn item_lifecycle_notifications_publish_command_execution_event() {
    ...
    let payload = serde_json::to_value(&events).expect("serialize events");
    assert_eq!(payload.as_array().expect("events array").len(), 1);
    assert_eq!(payload[0]["event_type"], "workx_command_execution_event");
    assert_eq!(payload[0]["event_params"]["thread_id"], "thread-1");
    assert_eq!(payload[0]["event_params"]["session_id"], "session-thread-1");
...
async fn plugin_measurement_batch_emits_directly_and_filters_invalid_rows() {
    ...
    assert_eq!(
        ...
        json!([
            {
                "event_type": "workx_plugin_measurement_event",
                "event_params": {
                    "thread_id": "thread-1",
                    "turn_id": "turn-1",
                    "item_id": "item-1",
...
async fn command_execution_approval_response_publishes_user_review_event() {
    for (kind, approval_id, subject, trigger) in [
        ...
        let payload = serde_json::to_value(&events).expect("serialize events");
        assert_eq!(payload.as_array().expect("events array").len(), 1);
        assert_eq!(payload[0]["event_type"], "workx_review_event");
        assert_eq!(payload[0]["event_params"]["thread_id"], "thread-1");
        assert_eq!(payload[0]["event_params"]["turn_id"], "turn-1");
        assert_eq!(payload[0]["event_params"]["item_id"], "item-1");
...
async fn guardian_completed_notification_publishes_review_event_with_thread_metadata() {
    ...
    assert_eq!(payload["event_type"], "workx_review_event");
...
fn subagent_thread_started_thread_spawn_serializes_thread_lineage() {
    let parent_thread_id =
        workx_protocol::ThreadId::from_string("11111111-1111-1111-1111-111111111111")
            .expect("valid thread id");
    let forked_from_thread_id =
        workx_protocol::ThreadId::from_string("22222222-2222-4222-8222-222222222222")
            .expect("valid thread id");
    let event = TrackEventRequest::ThreadInitialized(subagent_thread_started_event_request(
        SubAgentThreadStartedInput {
            session_id: "session-root".to_string(),
...
fn subagent_thread_started_other_serializes_explicit_parent_thread_id() {
    let parent_thread_id =
        workx_protocol::ThreadId::from_string("33333333-3333-4333-8333-333333333333")
            .expect("valid thread id");
    let event = TrackEventRequest::ThreadInitialized(subagent_thread_started_event_request(
        SubAgentThreadStartedInput {
            session_id: "session-root".to_string(),
...
async fn guardian_events_keep_thread_source_and_originator_with_explicit_turn_connection() {
    ...
    let mut events = Vec::new();
    let parent_thread_id =
        workx_protocol::ThreadId::from_string("44444444-4444-4444-4444-444444444444")
            .expect("valid parent thread id");
    ...
    assert_eq!(params.runtime.workx_rs_version, "0.1.0");
...
async fn subagent_tool_items_inherit_parent_connection_metadata() {
    ...
    assert_eq!(payload[1]["event_type"], "workx_dynamic_tool_call_event");
...
fn plugin_used_event_serializes_expected_shape() {
    ...
    let event = TrackEventRequest::PluginUsed(WorkxPluginUsedEventRequest {
        event_type: "workx_plugin_used",
        event_params: workx_plugin_used_metadata(&tracking, sample_plugin_metadata()),
    });
...
fn plugin_management_event_serializes_expected_shape() {
    let event = TrackEventRequest::PluginInstalled(WorkxPluginEventRequest {
        event_type: "workx_plugin_installed",
        event_params: workx_plugin_metadata(sample_plugin_metadata()),
    });
...
fn plugin_install_failed_event_serializes_expected_shape() {
    let event = TrackEventRequest::PluginInstallFailed(WorkxPluginInstallFailedEventRequest {
        event_type: "workx_plugin_install_failed",
        event_params: WorkxPluginInstallFailedMetadata {
            plugin: workx_plugin_metadata(sample_plugin_metadata()),
...
fn plugin_management_event_keeps_plugin_id_local_when_remote_id_exists() {
    ...
    let event = TrackEventRequest::PluginInstalled(WorkxPluginEventRequest {
        event_type: "workx_plugin_installed",
        event_params: workx_plugin_metadata(plugin),
    });
...
fn hook_run_event_serializes_expected_shape() {
    let tracking = test_tracking_context("thread-3", "turn-3");
    let event = TrackEventRequest::HookRun(WorkxHookRunEventRequest {
        event_type: "workx_hook_run",
        event_params: workx_hook_run_metadata(
            &tracking,
            HookRunFact {
...
fn hook_run_metadata_maps_sources_and_statuses() {
    ...
    let system = serde_json::to_value(workx_hook_run_metadata(
        &tracking,
        HookRunFact {
    ...
    let project = serde_json::to_value(workx_hook_run_metadata(
        &tracking,
        HookRunFact {
    ...
    let cloud_requirements = serde_json::to_value(workx_hook_run_metadata(
        &tracking,
        HookRunFact {
            event_name: HookEventName::Stop,
    ...
    let unknown = serde_json::to_value(workx_hook_run_metadata(
        &tracking,
        HookRunFact {
...
fn hook_run_metadata_maps_stopped_status() {
    ...
    let stopped = serde_json::to_value(workx_hook_run_metadata(
        &tracking,
        HookRunFact {
            event_name: HookEventName::Stop,
            hook_source: HookSource::User,
...
async fn reducer_ingests_artifact_operation_fact() {
    ...
    assert_eq!(
        ...
        json!([{
            "event_type": "workx_artifact_operation",
            "event_params": {
                "thread_id": "thread-1",
                "turn_id": "turn-1",
                "item_id": "call-1",
            ...
            AnalyticsFact::Custom(CustomAnalyticsFact::SkillInvoked(SkillInvokedInput {
                ...
                invocations: vec![SkillInvocation {
                    ...
                    location: SkillInvocationLocation::Host {
                        path: skill_path,
                        scope: workx_protocol::protocol::SkillScope::User,
                    },
                    plugin_id: None,
...
async fn reducer_includes_plugin_ids_for_plugin_skill_invocations() {
    ...
    let tracking = test_tracking_context("thread-1", "turn-1");
    let skill_path =
        PathBuf::from("/Users/abc/.workx/plugins/cache/test/sample/skills/doc/SKILL.md");
...
async fn reducer_ingests_hook_run_fact() {
    ...
    let payload = serde_json::to_value(&events).expect("serialize events");
    assert_eq!(payload.as_array().expect("events array").len(), 1);
    assert_eq!(payload[0]["event_type"], "workx_hook_run");
...
async fn reducer_ingests_app_and_plugin_facts() {
    ...
    assert_eq!(payload[0]["event_type"], "workx_app_mentioned");
    assert_eq!(payload[1]["event_type"], "workx_app_used");
    assert_eq!(payload[2]["event_type"], "workx_plugin_used");
    assert_eq!(
...
async fn reducer_ingests_plugin_state_changed_fact() {
    ...
    assert_eq!(
        payload,
        json!([{
            "event_type": "workx_plugin_disabled",
            "event_params": {
                "plugin_id": "sample@test",
                "remote_plugin_id": null,
                "plugin_name": "sample",
...
async fn reducer_ingests_plugin_install_requested_fact() {
    ...
    assert_eq!(
        serde_json::to_value(&events).expect("serialize events"),
        json!([{
            "event_type": "workx_plugin_install_requested",
            "event_params": {
...
async fn reducer_ingests_external_agent_config_import_completed_fact() {
    ...
    assert_eq!(
        payload,
        json!([{
            "event_type": "workx_onboarding_external_agent_import_complete",
            "event_params": {
                "import_id": "import-1",
                "source": "app_server",
...
fn external_agent_config_import_failure_event_serializes_expected_shape() {
    let event = TrackEventRequest::ExternalAgentConfigImportFailure(
        WorkxOnboardingExternalAgentImportFailureEventRequest {
            event_type: "workx_onboarding_external_agent_import_failure",
            event_params: WorkxOnboardingExternalAgentImportFailureMetadata {
                import_id: "import-1".to_string(),
                source: "app_server".to_string(),
                provider_id: "test-provider-42".to_string(),
                item_type: "PLUGINS".to_string(),
...
fn turn_event_serializes_expected_shape() {
    let event = TrackEventRequest::TurnEvent(Box::new(WorkxTurnEventRequest {
        event_type: "workx_turn_event",
        event_params: crate::events::WorkxTurnEventParams {
            thread_id: "thread-2".to_string(),
            session_id: "session-thread-2".to_string(),
            turn_id: "turn-2".to_string(),
            root_turn_id: Some("turn-2".to_string()),
            turn_trigger: Some("user".to_string()),
            workx_turn_source: Some("composer".to_string()),
    ...
    let expected = serde_json::from_str::<serde_json::Value>(
        r#"{
            ...
            "event_params": {
                ...
                "root_turn_id": "turn-2",
                "turn_trigger": "user",
                "workx_turn_source": "composer",
                "submission_type": null,
                "app_server_client": {
                    "product_client_id": "workx_cli_rs",
                    "client_name": "workx-tui",
                    "client_version": "1.0.0",
                    "rpc_transport": "stdio",
                    "experimental_api_enabled": true
...
async fn accepted_turn_steer_emits_expected_event() {
    ...
    assert_eq!(payload["event_type"], json!("workx_turn_steer_event"));
    ...
    assert_eq!(
        payload["event_params"]["runtime"]["workx_rs_version"],
        json!("0.1.0")
    );
...
async fn turn_lifecycle_emits_turn_event() {
    ...
    assert_eq!(payload["event_type"], json!("workx_turn_event"));
    assert_eq!(payload["event_params"]["thread_id"], json!("thread-2"));
    assert_eq!(
        payload["event_params"]["session_id"],
        json!("session-thread-2")
    ...
    assert_eq!(
        (
            ...
            payload["event_params"].get("workx_turn_source"),
        ),
        (
...
async fn turn_event_counts_completed_tool_items() {
    ...
    assert_eq!(
        emitted_tool_events,
        [
            "workx_command_execution_event",
            "workx_file_change_event",
            "workx_mcp_tool_call_event",
            "workx_dynamic_tool_call_event",
            "workx_collab_agent_tool_call_event",
            "workx_web_search_event",
            "workx_image_generation_event",
            "workx_control_tool_call_event",
        ]
    ...
    *turn_metadata.root_turn_id.lock().expect("root turn ID") = None;
    ...
    assert_eq!(
        payload
            ...
            .collect::<Vec<_>>(),
        vec![
            json!({"event_type": "workx_command_execution_event", "turn_id": "turn-1", "item_id": "before-conflict", "root_turn_id": "root-ancestor"}),
            json!({"event_type": "workx_command_execution_event", "turn_id": "turn-1", "item_id": "after-conflict", "root_turn_id": null}),
            json!({"event_type": "workx_control_tool_call_event", "turn_id": "turn-1", "item_id": "queued-control", "root_turn_id": null}),
            json!({"event_type": "workx_dynamic_tool_call_event", "turn_id": "turn-1", "item_id": "queued-code-mode", "root_turn_id": null}),
            json!({"event_type": "workx_turn_event", "turn_id": "turn-1", "item_id": null, "root_turn_id": null}),
        ]
    );
}
...
async fn completed_background_tool_item_emits_after_turn_event() {
    ...
    assert_eq!(
        out.iter()
            ...
            .collect::<Vec<_>>(),
        vec![
            json!({"event_type": "workx_turn_event", "turn_id": "turn-2", "root_turn_id": "root-background"}),
            json!({"event_type": "workx_command_execution_event", "turn_id": "turn-2", "root_turn_id": "root-background"}),
            json!({"event_type": "workx_control_tool_call_event", "turn_id": "turn-2", "root_turn_id": "root-background"}),
            json!({"event_type": "workx_dynamic_tool_call_event", "turn_id": "turn-2", "root_turn_id": "root-background"}),
        ]
    );
}
...
async fn turn_lifecycle_emits_failed_turn_event() {
    ...
    reducer
        .ingest(
            AnalyticsFact::Custom(CustomAnalyticsFact::TurnWorkxError(Box::new(
                TurnWorkxErrorFact::from_workx_err(
                    "thread-2".to_string(),
                    "turn-2".to_string(),
                    &WorkxErr::InvalidRequest("unknown turn environment id `env-2`".to_string()),
                ),
            ))),
            &mut out,
        )
    ...
    reducer
        .ingest(
            AnalyticsFact::Notification(Box::new(sample_turn_completed_notification(
                ...
                Some(workx_app_server_protocol::WorkxErrorInfo::BadRequest),
            ))),
            &mut out,
        )
        .await;
    ...
    assert_eq!(
        payload["event_params"]["workx_error_kind"],
        json!("invalid_request")
    );
    assert_eq!(
        payload["event_params"]["workx_error_http_status_code"],
        json!(null)
    );
}
...
async fn rejected_turn_interrupt_does_not_tag_interrupted_turn_event() {
    ...
    assert_eq!(payload["event_params"]["workx_error_kind"], json!(null));
}

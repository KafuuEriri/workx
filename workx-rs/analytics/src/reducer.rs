...
use crate::events::WorkxCollabAgentToolCallEventParams;
use crate::events::WorkxCollabAgentToolCallEventRequest;
...
use crate::events::WorkxControlToolCallEventParams;
use crate::events::WorkxControlToolCallEventRequest;
use crate::events::WorkxDynamicToolCallEventParams;
use crate::events::WorkxDynamicToolCallEventRequest;
use crate::events::WorkxFileChangeEventParams;
use crate::events::WorkxFileChangeEventRequest;
use crate::events::WorkxGoalEventRequest;
use crate::events::WorkxHookRunEventRequest;
use crate::events::WorkxImageGenerationEventParams;
use crate::events::WorkxImageGenerationEventRequest;
use crate::events::WorkxMcpToolCallEventParams;
use crate::events::WorkxMcpToolCallEventRequest;
use crate::events::WorkxOnboardingExternalAgentImportCompleteEventRequest;
use crate::events::WorkxOnboardingExternalAgentImportCompleteMetadata;
...
use crate::events::WorkxPluginInstallRequestedEventRequest;
use crate::events::WorkxPluginMeasurementEventParams;
use crate::events::WorkxPluginMeasurementEventRequest;
...
use crate::events::WorkxTurnEventParams;
use crate::events::WorkxTurnEventRequest;
use crate::events::WorkxTurnSteerEventParams;
use crate::events::WorkxTurnSteerEventRequest;
use crate::events::WorkxWebSearchEventParams;
use crate::events::WorkxWebSearchEventRequest;
use crate::events::plugin_state_event_type;
use crate::events::subagent_source_name;
use crate::events::subagent_thread_started_event_request;
use crate::events::workx_app_metadata;
use crate::events::workx_artifact_operation_event_request;
use crate::events::workx_compaction_event_params;
use crate::events::workx_goal_event_params;
use crate::events::workx_hook_run_metadata;
use crate::events::workx_plugin_install_requested_metadata;
...
use workx_app_server_protocol::ClientResponse;
use workx_app_server_protocol::CollabAgentStatus;
...
use workx_app_server_protocol::CommandExecutionApprovalKind;
use workx_app_server_protocol::CommandExecutionSource;
use workx_app_server_protocol::CommandExecutionStatus;
use workx_app_server_protocol::DynamicToolCallOutputContentItem;
...
use workx_app_server_protocol::NetworkPolicyRuleAction;
use workx_app_server_protocol::PatchApplyStatus;
use workx_app_server_protocol::PatchChangeKind;
...
use workx_app_server_protocol::WebSearchAction;
use workx_app_server_protocol::WorkxErrorInfo;
use workx_git_utils::SanitizedGitUrl;
use workx_git_utils::collect_git_info;
...
use workx_protocol::items::is_safe_plugin_relative_path;
use workx_protocol::models::PermissionProfile;
use workx_protocol::protocol::SessionSource;
use workx_protocol::protocol::SkillScope;
...
impl<'a> AnalyticsDropSite<'a> {
    ...
    fn compaction(input: &'a WorkxCompactionEvent) -> Self {
        Self {
    ...
    fn goal(input: &'a WorkxGoalEvent) -> Self {
        Self {
            event_name: "goal",
            thread_id: &input.thread_id,
    ...
    fn tool_item(
        notification: &'a workx_app_server_protocol::ItemCompletedNotification,
        item_id: &'a str,
    ) -> Self {
        Self {
...
struct CompletedTurnState {
    ...
    turn_error: Option<WorkxErrorInfo>,
    completed_at: u64,
...
struct TurnState {
    ...
    workx_error: Option<TurnWorkxError>,
...
impl AnalyticsReducer {
    pub(crate) async fn ingest(&mut self, input: AnalyticsFact, out: &mut Vec<TrackEventRequest>) {
        match input {
            ...
            AnalyticsFact::Custom(input) => match input {
                ...
                CustomAnalyticsFact::GuardianV2(input) => {
                    let event_type = match &input.kind {
                        GuardianV2EventKind::Classification { .. } => {
                            "workx_guardian_v2_classification"
                        }
                        GuardianV2EventKind::FastDecision { .. } => {
                            "workx_guardian_v2_fast_decision"
                        }
                    };
                ...
                CustomAnalyticsFact::TurnWorkxError(input) => {
                    self.ingest_turn_workx_error(*input);
                }
    ...
    fn ingest_artifact_operation(
        ...
        out.push(TrackEventRequest::ArtifactOperation(
            workx_artifact_operation_event_request(input.tracking, input.operation),
        ));
    }
    ...
    fn ingest_code_mode_tool_call(
        ...
        match input {
            ...
            CodeModeToolCallFact::Completed {
                ...
                let event = TrackEventRequest::DynamicToolCall(WorkxDynamicToolCallEventRequest {
                    event_type: "workx_dynamic_tool_call_event",
                    event_params: WorkxDynamicToolCallEventParams {
                        base,
    ...
    fn ingest_control_tool_call(
        ...
        base.cell_id = cell_id;
        let event = TrackEventRequest::ControlToolCall(WorkxControlToolCallEventRequest {
            event_type: "workx_control_tool_call_event",
            event_params: WorkxControlToolCallEventParams {
                base,
    ...
    fn ingest_guardian_review(
        ...
        out.push(TrackEventRequest::GuardianReview(Box::new(
            GuardianReviewEventRequest {
                event_type: "workx_guardian_review",
    ...
    fn ingest_turn_workx_error(&mut self, input: TurnWorkxErrorFact) {
        let TurnWorkxErrorFact {
            turn_id,
            thread_id,
            error,
        } = input;
        ...
        turn_state.workx_error = Some(error);
    }
    ...
    fn ingest_app_mentioned(&mut self, input: AppMentionedInput, out: &mut Vec<TrackEventRequest>) {
        ...
        out.extend(mentions.into_iter().map(|mention| {
            let event_params = workx_app_metadata(&tracking, mention);
            TrackEventRequest::AppMentioned(WorkxAppMentionedEventRequest {
    ...
    fn ingest_app_used(&mut self, input: AppUsedInput, out: &mut Vec<TrackEventRequest>) {
        ...
        let event_params = workx_app_metadata(&tracking, app);
        out.push(TrackEventRequest::AppUsed(WorkxAppUsedEventRequest {
    ...
    fn ingest_hook_run(&mut self, input: HookRunInput, out: &mut Vec<TrackEventRequest>) {
        ...
        out.push(TrackEventRequest::HookRun(WorkxHookRunEventRequest {
            event_type: "workx_hook_run",
            event_params: workx_hook_run_metadata(&tracking, hook),
        }));
    }
    ...
    fn ingest_plugin_used(&mut self, input: PluginUsedInput, out: &mut Vec<TrackEventRequest>) {
        ...
        out.push(TrackEventRequest::PluginUsed(WorkxPluginUsedEventRequest {
            event_type: "workx_plugin_used",
            event_params: workx_plugin_used_metadata(&tracking, plugin),
        }));
    }
    ...
    fn ingest_plugin_install_requested(
        ...
        out.push(TrackEventRequest::PluginInstallRequested(
            WorkxPluginInstallRequestedEventRequest {
                event_type: "workx_plugin_install_requested",
                event_params: workx_plugin_install_requested_metadata(&tracking, request),
            },
        ));
    }
    ...
    fn ingest_plugin_state_changed(
        ...
        let event = WorkxPluginEventRequest {
    ...
    fn ingest_external_agent_config_import_completed(
        ...
        out.push(TrackEventRequest::ExternalAgentConfigImportCompleted(
            WorkxOnboardingExternalAgentImportCompleteEventRequest {
                event_type: "workx_onboarding_external_agent_import_complete",
                event_params: WorkxOnboardingExternalAgentImportCompleteMetadata {
            ...
            ServerNotification::TurnCompleted(notification) => {
                ...
                turn_state.completed = Some(CompletedTurnState {
                    ...
                    turn_error: notification
                        .turn
                        .error
                        .and_then(|error| error.workx_error_info),
    ...
    fn ingest_plugin_measurements(
        ...
        out.extend(
            rows.into_iter()
                ...
                .map(|row| {
                    TrackEventRequest::PluginMeasurement(WorkxPluginMeasurementEventRequest {
                        event_type: "workx_plugin_measurement_event",
                        event_params: WorkxPluginMeasurementEventParams {
    ...
    fn emit_thread_initialized(
        &mut self,
        connection_id: u64,
        thread: workx_app_server_protocol::Thread,
        model: String,
        initialization_mode: ThreadInitializationMode,
        thread_originator: Option<String>,
        out: &mut Vec<TrackEventRequest>,
    ...
    fn ingest_compaction(&mut self, input: WorkxCompactionEvent, out: &mut Vec<TrackEventRequest>) {
        let Some((connection_state, thread_state, thread_metadata)) =
        ...
        out.push(TrackEventRequest::Compaction(Box::new(
            WorkxCompactionEventRequest {
                event_type: "workx_compaction_event",
                event_params: workx_compaction_event_params(
                    input,
    ...
    fn ingest_goal(&mut self, input: WorkxGoalEvent, out: &mut Vec<TrackEventRequest>) {
        let Some((connection_state, thread_state, thread_metadata)) =
            self.thread_context_or_warn(AnalyticsDropSite::goal(&input))
        else {
            return;
        };
        out.push(TrackEventRequest::Goal(Box::new(WorkxGoalEventRequest {
            event_type: "workx_goal_event",
            event_params: workx_goal_event_params(
                input,
                thread_metadata.session_id.clone(),
                thread_state.app_server_client(connection_state),
                connection_state.runtime.clone(),
    ...
    fn ingest_guardian_review_completed(
        &mut self,
        notification: workx_app_server_protocol::ItemGuardianApprovalReviewCompletedNotification,
        out: &mut Vec<TrackEventRequest>,
    ) {
    ...
    fn emit_turn_steer_event(
        ...
        out.push(TrackEventRequest::TurnSteer(WorkxTurnSteerEventRequest {
            event_type: "workx_turn_steer_event",
            event_params: WorkxTurnSteerEventParams {
    ...
    fn emit_review_event(
        ...
        out.push(TrackEventRequest::ReviewEvent(WorkxReviewEventRequest {
    ...
    async fn maybe_emit_turn_event(&mut self, turn_id: &str, out: &mut Vec<TrackEventRequest>) {
        ...
        let turn_event = TrackEventRequest::TurnEvent(Box::new(WorkxTurnEventRequest {
            event_type: "workx_turn_event",
            event_params: workx_turn_event_params(
                thread_state.app_server_client(connection_state),
                connection_state.runtime.clone(),
                turn_id.to_string(),
                turn_state,
...
fn tool_event_base_mut(event: &mut TrackEventRequest) -> Option<&mut WorkxToolItemEventBase> {
    match event {
    ...
    match item {
        ThreadItem::CommandExecution {
            ...
            Some(TrackEventRequest::CommandExecution(
                WorkxCommandExecutionEventRequest {
        ...
        ThreadItem::FileChange {
            ...
            Some(TrackEventRequest::FileChange(WorkxFileChangeEventRequest {
                event_type: "workx_file_change_event",
                event_params: WorkxFileChangeEventParams {
                    base,
        ...
        ThreadItem::McpToolCall {
            ...
            Some(TrackEventRequest::McpToolCall(
                WorkxMcpToolCallEventRequest {
                    event_type: "workx_mcp_tool_call_event",
                    event_params: WorkxMcpToolCallEventParams {
                        base,
        ...
        ThreadItem::DynamicToolCall {
            ...
            Some(TrackEventRequest::DynamicToolCall(
                WorkxDynamicToolCallEventRequest {
                    event_type: "workx_dynamic_tool_call_event",
                    event_params: WorkxDynamicToolCallEventParams {
                        base,
        ...
        ThreadItem::CollabAgentToolCall {
            ...
            Some(TrackEventRequest::CollabAgentToolCall(
                WorkxCollabAgentToolCallEventRequest {
                    event_type: "workx_collab_agent_tool_call_event",
                    event_params: WorkxCollabAgentToolCallEventParams {
                        base,
        ...
        ThreadItem::WebSearch(item) => {
            ...
            Some(TrackEventRequest::WebSearch(WorkxWebSearchEventRequest {
                event_type: "workx_web_search_event",
                event_params: WorkxWebSearchEventParams {
                    base,
        ...
        ThreadItem::ImageGeneration(item) => {
            ...
            Some(TrackEventRequest::ImageGeneration(
                WorkxImageGenerationEventRequest {
                    event_type: "workx_image_generation_event",
                    event_params: WorkxImageGenerationEventParams {
                        base,
...
fn file_change_counts(changes: &[workx_app_server_protocol::FileUpdateChange]) -> FileChangeCounts {
...
fn workx_turn_event_params(
    app_server_client: WorkxAppServerClientMetadata,
    runtime: WorkxRuntimeMetadata,
    turn_id: String,
    turn_state: &TurnState,
    thread_metadata: &ThreadMetadataState,
) -> WorkxTurnEventParams {
    let (
        Some(thread_id),
    ...
    let workx_error = turn_state.workx_error.as_ref();
    WorkxTurnEventParams {
        thread_id,
        session_id: thread_metadata.session_id.clone(),
        turn_id,
        ...
        workx_turn_source: turn_metadata.workx_turn_source(),
        app_server_client,
        runtime,
        submission_type,
        ephemeral,
        ...
        workx_error_kind: workx_error.map(|error| error.kind),
        workx_error_http_status_code: workx_error.and_then(|error| error.http_status_code),
...
fn analytics_turn_status(status: workx_app_server_protocol::TurnStatus) -> Option<TurnStatus> {
    match status {
        workx_app_server_protocol::TurnStatus::Completed => Some(TurnStatus::Completed),
        workx_app_server_protocol::TurnStatus::Failed => Some(TurnStatus::Failed),
        workx_app_server_protocol::TurnStatus::Interrupted => Some(TurnStatus::Interrupted),
        workx_app_server_protocol::TurnStatus::InProgress => None,
    }
}
    ...
    fn command_execution_script_paths_reject_unsafe_values() {
        assert_eq!(
            safe_plugin_relative_script_path(
                ...
                Some("/home/user/.workx/plugins/cache/openai-curated/sample/scripts/run.py"),
            ),
            None
        );
        assert_eq!(

...
#[cfg(debug_assertions)]
use crate::events::WorkxPluginMetadata;
#[cfg(debug_assertions)]
use crate::events::WorkxPluginUsedEventRequest;
#[cfg(debug_assertions)]
use crate::events::WorkxPluginUsedMetadata;
#[cfg(debug_assertions)]
...
use workx_app_server_protocol::CommandExecutionOutputDeltaNotification;
...
fn sample_artifact_operation_event(thread_id: &str) -> TrackEventRequest {
    TrackEventRequest::ArtifactOperation(workx_artifact_operation_event_request(
        TrackEventsContext {
            model_slug: "gpt-5.1-codex".to_string(),
            thread_id: thread_id.to_string(),
            turn_id: "turn-1".to_string(),
            product_client_id: "workx_desktop".to_string(),
        },
        ArtifactOperation {
...
fn sample_mcp_tool_call_event(thread_id: &str, plugin_id: Option<&str>) -> TrackEventRequest {
    TrackEventRequest::McpToolCall(WorkxMcpToolCallEventRequest {
        ...
        event_params: WorkxMcpToolCallEventParams {
            base: WorkxToolItemEventBase {
                ...
                runtime: WorkxRuntimeMetadata {
                    workx_rs_version: "0.0.0".to_string(),
...
fn sample_plugin_used_track_event(thread_id: &str, plugin_id: Option<&str>) -> TrackEventRequest {
    TrackEventRequest::PluginUsed(WorkxPluginUsedEventRequest {
        event_type: "workx_plugin_used",
        event_params: WorkxPluginUsedMetadata {
            plugin: WorkxPluginMetadata {
                plugin_id: plugin_id.map(str::to_string),
                remote_plugin_id: None,
                ...
                product_client_id: Some("workx_desktop".to_string()),
            },
...
fn unique_capture_path(name: &str) -> PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "workx-analytics-{name}-{}-{nonce}.jsonl",
        std::process::id()
    ))
}
...
async fn capture_file_writes_exact_serialized_request() {
    ...
    let auth = workx_login::WorkxAuth::create_dummy_chatgpt_auth_for_testing();
...
async fn capture_file_writes_final_batches_as_separate_lines() {
    ...
    assert_eq!(
        ...
        "workx_accepted_line_fingerprints"
    );
...
fn track_response_ignores_unserializable_thread_responses() {
    ...
    thread_start.cwd = workx_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
...
async fn flush_is_noop_when_analytics_is_disabled() {
    let client = AnalyticsEventsClient::new(
        workx_login::AuthManager::from_auth_for_testing(

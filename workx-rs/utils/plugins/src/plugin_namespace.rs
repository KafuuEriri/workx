...
use workx_exec_server_protocol::DISCOVERABLE_PLUGIN_MANIFEST_PATHS;
...
mod tests {
    ...
    async fn uses_manifest_name() {
        ...
        fs::create_dir_all(plugin_root.join(".workx-plugin")).expect("mkdir manifest");
        fs::write(
    ...
    fn ignores_unrelated_root_plugin_manifest_before_legacy_fallback() {
        let tmp = tempdir().expect("tempdir");
        let plugin_root = tmp.path().join("plugins/sample");
        let legacy_path = plugin_root.join(".workx-plugin/plugin.json");
    ...
    fn rejects_nonregular_legacy_plugin_manifest_before_lower_precedence_manifest() {
        let tmp = tempdir().expect("tempdir");
        let plugin_root = tmp.path().join("plugins/sample");
        let workx_path = plugin_root.join(".workx-plugin/plugin.json");
        let claude_path = plugin_root.join(ALTERNATE_PLUGIN_CLA_MANIFEST_RELATIVE_PATH);
        fs::create_dir_all(&workx_path).expect("nonregular Workx manifest");
    ...
    fn rejects_symlinked_legacy_plugin_manifest_before_lower_precedence_manifest() {
        ...
        let workx_path = plugin_root.join(".workx-plugin/plugin.json");
        let claude_path = plugin_root.join(ALTERNATE_PLUGIN_CLA_MANIFEST_RELATIVE_PATH);
        fs::create_dir_all(workx_path.parent().expect("Workx manifest parent"))
            .expect("Workx manifest parent");
        fs::create_dir_all(claude_path.parent().expect("Claude manifest parent"))
            .expect("Claude manifest parent");
        fs::write(plugin_root.join("benign.json"), r#"{"name":"sample"}"#)
            .expect("benign manifest");
        fs::write(&claude_path, r#"{"name":"sample"}"#).expect("Claude manifest");
        std::os::unix::fs::symlink("../benign.json", &workx_path).expect("Workx manifest symlink");
    ...
    fn rejects_symlinked_legacy_plugin_manifest_directory_before_lower_precedence_manifest() {
        ...
        .expect("benign manifest");
        fs::write(&claude_path, r#"{"name":"sample"}"#).expect("Claude manifest");
        std::os::unix::fs::symlink(&manifest_directory, plugin_root.join(".workx-plugin"))
            .expect("Workx manifest directory symlink");
    ...
    fn preserves_workx_claude_cursor_legacy_precedence() {
        ...
        assert_eq!(
            ...
            Some(workx_path.clone())
        );
        fs::remove_file(workx_path).expect("remove Workx manifest");

...
async fn wait_for_python_repl_ready_via_probe(
    ...
    let mut collected = Vec::new();
    let marker = "__workx_pty_ready__";
    let deadline = tokio::time::Instant::now() + tokio::time::Duration::from_millis(timeout_ms);
...
async fn pty_python_repl_emits_output_and_exits() -> anyhow::Result<()> {
    ...
    let ready_marker = "__workx_pty_ready__";
    let args = vec![
        "-i".to_string(),
        "-q".to_string(),
        "-c".to_string(),
...
fn assert_pty_shutdown_with_detached_child(
    ...
    let (session, child_pid) = runtime.block_on(async {
        let marker = "__workx_detached_pid:";
...
if os.fork() == 0:
    ...
    select.select([0], [], [])
    print('__workx_input_ready__', flush=True)
    time.sleep(10)
    os._exit(0)
time.sleep(10)
"
...
fn pty_terminate_reaps_child_when_waiter_is_queued() -> anyhow::Result<()> {
    ...
    runtime.block_on(async {
        ...
        env_map.insert(
            "WORKX_PTY_TEST_PID_FILE".to_string(),
            pid_file.display().to_string(),
        );
        let (program, args) =
            shell_command("printf '%s' \"$$\" > \"$WORKX_PTY_TEST_PID_FILE\"; sleep 60");
        let spawned = spawn_pty_process(
            &program,
            &args,
            Path::new("."),
...
async fn pty_preserving_inherited_fds_keeps_python_repl_running() -> anyhow::Result<()> {
    ...
    .await?;
    let marker = "__workx_preserved_py_pid:";
    writer

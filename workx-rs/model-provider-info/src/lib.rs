//! Registry of model providers supported by Workx.
//!
//! Providers can be defined in two places:
//!   1. Built-in defaults compiled into the binary so Workx works out-of-the-box.
//!   2. User-defined entries inside `~/.workx/config.toml` under the `model_providers`
...
use workx_api::RetryConfig as ApiRetryConfig;
...
pub const CHATGPT_WORKX_BASE_URL: &str = "https://chatgpt.com/backend-api/codex";
...
impl ModelProviderInfo {
    ...
    fn build_header_map(&self) -> WorkxResult<HeaderMap> {
    ...
    pub fn to_api_provider(&self, auth_mode: Option<AuthMode>) -> WorkxResult<ApiProvider> {
    ...
    pub fn api_key(&self) -> WorkxResult<Option<String>> {
        match &self.env_key {
            Some(env_key) => {
    ...
    pub fn supports_workx_backend_routes(&self) -> bool {
        self.is_openai()
...
pub fn built_in_model_providers(
    ...
    // providers are bundled with Workx CLI, so we only include the OpenAI and
...
pub fn create_oss_provider(default_provider_port: u16, wire_api: WireApi) -> ModelProviderInfo {
    // These WORKX_OSS_ environment variables are experimental: we may
    // switch to reading values from config.toml instead.
    let default_workx_oss_base_url = format!(
        "http://localhost:{workx_oss_port}/v1",
        workx_oss_port = std::env::var("WORKX_OSS_PORT")
            .ok()
    ...
    let workx_oss_base_url = std::env::var("WORKX_OSS_BASE_URL")
        .ok()
        .filter(|v| !v.trim().is_empty())
        .unwrap_or(default_workx_oss_base_url);
    create_oss_provider_with_base_url(&workx_oss_base_url, wire_api)
}

...
#[test]
fn test_personal_access_token_uses_chatgpt_workx_base_url() {
    ...
    assert_eq!(api_provider.base_url, CHATGPT_WORKX_BASE_URL);
}

#[test]
fn test_header_auth_uses_chatgpt_workx_base_url() {
    let api_provider = ModelProviderInfo::create_openai_provider(/*base_url*/ None)
...
#[test]
fn workx_backend_routes_require_workx_base_url() {
    for (base_url, expected) in [
        (None, true),
        (Some(CHATGPT_WORKX_BASE_URL), true),
        ...
        assert_eq!(provider.supports_workx_backend_routes(), expected);
    }
}
...
[aws.auth_refresh]
command = "aws"
args = ["login", "--profile", "workx-bedrock"]
        "#;
    ...
    assert_eq!(
        ...
        Some(ModelProviderAwsAuthInfo {
            ...
            auth_refresh: Some(AwsAuthRefreshConfig {
                ...
                args: vec!["login".into(), "--profile".into(), "workx-bedrock".into()],

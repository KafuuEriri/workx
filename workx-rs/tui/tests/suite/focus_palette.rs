...
fn focus_gained_with_unanswered_palette_queries_preserves_immediate_input() -> Result<()> {
    ...
    write_test_config(workx_home.path(), &repo_root)?;

    let mut terminal = PtyWorkx::start(&repo_root, workx_home)?;
...
impl PtyWorkx {
    pub(super) fn start(repo_root: &Path, workx_home: TempDir) -> Result<Self> {
        let mut master_fd = -1;
        let mut slave_fd = -1;
        ...
        let workx = workx_utils_cargo_bin::cargo_bin("workx-tui")
            .or_else(|_| workx_utils_cargo_bin::cargo_bin("workx"))?;
        let child = Command::new(workx)
            .arg("--no-alt-screen")
            .arg("-C")
            .arg(repo_root)
            ...
            .stderr(slave)
            .spawn()
            .context("start Workx in focus-test pseudo-terminal")?;
    ...
    pub(super) fn wait_for_startup(&mut self) -> Result<()> {
        ...
        while Instant::now() < deadline {
            ...
            if self.palette_answered && self.screen_contains("Workx") {
                return Ok(());
            }

            if let Some(status) = self.child.try_wait()? {
                bail!(
                    "Workx exited before the focus test started ({status}); screen:\n{}",
        ...
        bail!(
            "Workx did not initialize within {:?}; screen:\n{}",
            STARTUP_TIMEOUT,
            self.screen_contents(),
        );
    }
...
pub(super) fn write_test_config(workx_home: &Path, repo_root: &Path) -> Result<()> {
    ...
    std::fs::write(workx_home.join("config.toml"), config)
        .context("write focus-test Workx configuration")?;
    std::fs::write(

...
fn live_001_commit_on_overflow() {
    ...
    let mut term =
        workx_tui::Terminal::with_options(backend).expect("terminal should be constructed");
    let area = Rect::new(
    ...
    let mut rb = workx_tui::RowBuilder::new(/*target_width*/ 20);
    ...
    workx_tui::insert_history_lines(&mut term, lines)

...
#[ignore = "requires tmux and a locally built workx binary; run with --ignored for manual resize smoke"]
async fn tmux_split_preserves_fresh_session_composer_row_after_resize_reflow() -> Result<()> {
    ...
    let workx = workx_binary(&repo_root)?;
    ...
    write_config(workx_home.path(), &repo_root)?;
    write_auth(workx_home.path())?;

    let session_name = format!("workx-resize-reflow-smoke-{}", std::process::id());
    ...
    let start_output = checked_output(
        Command::new("tmux")
            ...
            .arg("--")
            .arg("env")
            .arg(format!("WORKX_HOME={}", workx_home.path().display()))
    ...
    let workx_pane = stdout_text(&start_output).trim().to_string();
    anyhow::ensure!(!workx_pane.is_empty(), "tmux did not report a pane id");
    ...
    let baseline_capture =
        wait_for_capture_contains(&workx_pane, draft, Duration::from_secs(/*secs*/ 15))?;
    ...
    let final_capture = capture_pane(&workx_pane)?;
    let final_row =
...
async fn tmux_width_resize_restore_keeps_visible_content_anchored() -> Result<()> {
    ...
    let session_name = format!("workx-resize-width-{}", std::process::id());
    let _session = TmuxSession {
        name: session_name.clone(),
    };
    ...
    sleep(Duration::from_millis(/*millis*/ 1_000));
    let restored_capture = capture_pane(&workx_pane)?;
    let restored_row =
...
async fn tmux_scrolled_composer_resize_preserves_visible_draft_text() -> Result<()> {
    ...
    let session_name = format!("workx-resize-scrolled-composer-{}", std::process::id());
    let _session = TmuxSession {
        name: session_name.clone(),
    };
    ...
    let baseline =
        wait_for_capture_contains(&workx_pane, &final_line, Duration::from_secs(/*secs*/ 15))?;
        ...
        [("narrowed", "28", "9", 2), ("restored", "44", "14", 6)]
    {
        ...
        let capture = capture_pane(&workx_pane)?;
...
async fn run_repeated_resize_smoke() -> Result<()> {
    ...
    let session_name = format!("workx-resize-repeat-{}", std::process::id());
    let _session = TmuxSession {
        name: session_name.clone(),
    };
...
fn workx_binary(repo_root: &Path) -> Result<PathBuf> {
    if let Ok(path) = workx_utils_cargo_bin::cargo_bin("workx") {
        return Ok(path);
    }

    let fallback = repo_root.join("workx-rs/target/debug/workx");
    anyhow::ensure!(
        fallback.is_file(),
        "workx binary is unavailable; run `cargo build -p workx-cli` first"
    );
    Ok(fallback)
}

fn write_config(workx_home: &Path, repo_root: &Path) -> Result<()> {
...
"#
    ...
    std::fs::write(workx_home.join("config.toml"), config)?;
    Ok(())
}

fn write_auth(workx_home: &Path) -> Result<()> {
    std::fs::write(

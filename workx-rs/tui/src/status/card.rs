...
impl StatusHistoryHandle {
    ...
    pub(crate) fn set_thread_usage(
        &self,
        estimate: Option<workx_app_server_protocol::ThreadUsage>,
    ) {
...
impl HistoryCell for StatusHistoryCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        lines.push(Line::from(vec![
            ...
            Span::from("Workx").bold(),
            Span::from(" ").dim(),
            Span::from(format!("(v{WORKX_CLI_VERSION})")).dim(),
        ]));
        ...
        let account_value = self.account.as_ref().map(|account| match account {
            ...
            StatusAccountDisplay::ApiKey => {
                "API key configured (run workx login to use ChatGPT)".to_string()
            }
        });

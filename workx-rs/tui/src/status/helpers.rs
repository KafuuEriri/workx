...
mod tests {
    ...
    async fn test_config(workx_home: &TempDir, cwd: &TempDir) -> Config {
    ...
    async fn compose_agents_summary_includes_global_agents_path() {
        let workx_home = TempDir::new().expect("temp workx home");
        let cwd = TempDir::new().expect("temp cwd");
        let global_agents_path = workx_home.path().join("global.md");
        let config = test_config(&workx_home, &cwd).await;
    ...
    async fn compose_agents_summary_collapses_home_and_preserves_project_relative_paths() {
        ...
        let workx_home = TempDir::new().expect("temp workx home");
        let cwd = TempDir::new().expect("temp cwd");
        let mut config = test_config(&workx_home, &cwd).await;
        config.cwd = home.join("workspace").join("project").abs();

        let paths = [
            home.join(".workx").join("AGENTS.md"),
        ...
        insta::assert_snapshot!(
            summary.replace('\\', "/"),
            @"~/.workx/AGENTS.md, ../AGENTS.md, AGENTS.md, nested/AGENTS.md"
        );
    }
    ...
    async fn compose_agents_summary_names_global_agents_override() {
        let workx_home = TempDir::new().expect("temp workx home");
        let cwd = TempDir::new().expect("temp cwd");
        let override_path = workx_home.path().join("override.md");
        let config = test_config(&workx_home, &cwd).await;

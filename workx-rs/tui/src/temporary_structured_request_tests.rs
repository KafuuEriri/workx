...
async fn preserves_custom_permissions_and_disables_required_mcp_servers() -> color_eyre::Result<()>
{
    ...
    let denied_path = workx_home.path().join("denied");
    ...
    std::fs::write(
        ...
        format!(
            "default_permissions = \"title-restricted\"\n\n\
             ...
             command = \"workx-auto-title-missing-mcp\"\n\
             required = true\n"
        ),
    )?;
    config.workx_home = workx_home.path().to_path_buf().abs();
    config.sqlite =
        workx_state::SqliteConfig::new_for_testing(workx_home.path().to_path_buf().abs());

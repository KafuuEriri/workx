...
impl EditorPaths {
    fn new() -> Self {
        ...
        let workx_home = canonical_root.join("workx-home");
        let cwd = canonical_root.join("workspace");
        fs::create_dir(&workx_home).expect("create Workx home");
...
#[test]
fn editor_directory_is_inside_isolated_workx_home() {
    let paths = EditorPaths::new();
    let policy = workspace_write_policy(&[]);

    let directory = editor_directory(&[&paths.workx_home], &policy, &paths.cwd)
        .expect("create isolated editor directory");

    assert_eq!(directory, paths.workx_home.join("editor"));
...
fn editor_directory_rejects_writable_home_editor_and_parent() {
    let paths = EditorPaths::new();
    let editor = paths.workx_home.join("editor");
    fs::create_dir(&editor).expect("create editor directory");
    let parent = paths.workx_home.parent().expect("Workx home parent");

    for writable_root in [paths.workx_home.as_path(), editor.as_path(), parent] {
        let policy = workspace_write_policy(&[writable_root]);

        assert!(
            editor_directory(&[&paths.workx_home], &policy, &paths.cwd).is_err(),
...
fn editor_directory_rejects_writable_descendant() {
    let paths = EditorPaths::new();
    let writable_descendant = paths.workx_home.join("editor").join("nested");
    ...
    assert!(editor_directory(&[&paths.workx_home], &policy, &paths.cwd).is_err());
}
...
fn editor_directory_rejects_read_only_carveout_with_writable_parent() {
    ...
    let policy = FileSystemSandboxPolicy::restricted(vec![
        FileSystemSandboxEntry::new(
            AbsolutePathBuf::from_absolute_path(&paths.workx_home)
                .expect("absolute Workx home")
                .into(),
...
fn editor_directory_rejects_preexisting_symlink() {
    ...
    symlink(&outside, paths.workx_home.join("editor")).expect("create editor directory symlink");
...
#[test]
#[cfg(unix)]
fn editor_directory_rejects_writable_workx_home_alias() {
    use std::os::unix::fs::symlink;

    let paths = EditorPaths::new();
    let aliased_home = paths.cwd.join("workx-home-link");
    symlink(&paths.workx_home, &aliased_home).expect("create Workx home symlink");
    let policy = workspace_write_policy(&[]);

    assert!(policy.can_write_path_with_cwd(&aliased_home.join("editor"), &paths.cwd));
    assert!(!policy.can_write_path_with_cwd(&paths.workx_home.join("editor"), &paths.cwd));
    assert!(editor_directory(&[&aliased_home], &policy, &paths.cwd).is_err());
}

#[test]
#[cfg(unix)]
fn editor_directory_rejects_writable_workx_home_alias_target() {
    use std::os::unix::fs::symlink;
    ...
    let alias_parent = paths
        .workx_home
        .parent()
        .expect("Workx home parent")
        .join("aliases");
    fs::create_dir(&alias_parent).expect("create protected alias parent");
    let aliased_home = alias_parent.join("workx-home-link");
    symlink(&paths.workx_home, &aliased_home).expect("create Workx home symlink");
    let policy = workspace_write_policy(&[&paths.workx_home]);
...
fn editor_directory_uses_protected_workspace_fallback_with_default_temporary_grants() {
    let root = tempfile::tempdir().expect("create editor test root");
    let workx_home = root.path().join("workx-home");
    let cwd = root.path().join("workspace");
    fs::create_dir(&workx_home).expect("create Workx home");
    fs::create_dir(&cwd).expect("create workspace");
    let workspace_workx_home = cwd.join(".workx");
    ...
    assert!(!workspace_workx_home.exists());
    assert!(policy.can_write_path_with_cwd(&workx_home, &cwd));
    assert!(!policy.can_write_path_with_cwd(&workspace_workx_home, &cwd));
    assert!(!policy.can_write_path_with_cwd(&workspace_workx_home.join("editor"), &cwd));

    let directory = editor_directory(&[&workx_home, &workspace_workx_home], &policy, &cwd)
        .expect("use protected workspace metadata directory");

    assert_eq!(
        directory,
        dunce::canonicalize(&workspace_workx_home)
...
fn editor_directory_rejects_explicitly_writable_workspace_fallback() {
    ...
    fs::create_dir(&cwd).expect("create workspace");
    let workspace_workx_home = cwd.join(".workx");
    let writable_workspace_workx_home = AbsolutePathBuf::from_absolute_path(&workspace_workx_home)
        .expect("absolute workspace metadata directory");
    let policy = FileSystemSandboxPolicy::workspace_write(
        &[writable_workspace_workx_home],
        /*exclude_tmpdir_env_var*/ false,
        /*exclude_slash_tmp*/ false,
    );
    ...
    assert!(policy.can_write_path_with_cwd(&workspace_workx_home, &cwd));
    assert!(
        editor_directory(&[&workx_home, &workspace_workx_home], &policy, &cwd).is_err(),
...
fn editor_directory_rejects_workspace_fallback_symlink_to_writable_target() {
    ...
    let paths = EditorPaths::new();
    let workspace_workx_home = paths.cwd.join(".workx");
    symlink(&paths.workx_home, &workspace_workx_home).expect("create workspace metadata symlink");
    let policy = workspace_write_policy(&[&paths.workx_home]);

    assert!(!policy.can_write_path_with_cwd(&workspace_workx_home, &paths.cwd));
    assert!(policy.can_write_path_with_cwd(&paths.workx_home, &paths.cwd));
    assert!(
        editor_directory(
            &[&paths.workx_home, &workspace_workx_home],
            &policy,
            &paths.cwd,
        )
        .is_err()
...
fn editor_directory_uses_next_protected_candidate_after_creation_error() {
    ...
    fs::write(&unavailable_home, "not a directory").expect("create unavailable Workx home");
    let policy = workspace_write_policy(&[]);

    let directory = editor_directory(&[&unavailable_home, &paths.workx_home], &policy, &paths.cwd)
...
fn editor_directory_rejects_windows_temporary_directory_outside_tmpdir_policy_root() {
    ...
    assert!(!paths.workx_home.join("editor").exists());
}
...
#[tokio::test]
#[cfg(unix)]
async fn editor_process_receives_buffer_in_isolated_workx_home() {
    let paths = EditorPaths::new();
    let policy = workspace_write_policy(&[]);
    let editor_directory = paths.workx_home.join("editor");
...
async fn editor_process_uses_protected_workspace_fallback_with_default_temporary_grants() {
    ...
    fs::create_dir(&workx_home).expect("create Workx home");
    fs::create_dir(&cwd).expect("create workspace");
    let default_workx_home = dirs::home_dir().expect("home directory").join(".workx");
    let writable_default_workx_home = AbsolutePathBuf::from_absolute_path(&default_workx_home)
        .expect("absolute default Workx home");
    let policy = FileSystemSandboxPolicy::workspace_write(
        &[writable_default_workx_home],
        /*exclude_tmpdir_env_var*/ false,
        /*exclude_slash_tmp*/ false,
    );
    let workspace_workx_home = cwd.join(".workx");
    ...
    assert!(!workspace_workx_home.exists());
    assert!(policy.can_write_path_with_cwd(&workx_home, &cwd));
    assert!(policy.can_write_path_with_cwd(&default_workx_home, &cwd));
    assert!(!policy.can_write_path_with_cwd(&workspace_workx_home, &cwd));

    let content = run_editor("seed", &editor_command, &workx_home, &policy, &cwd)
        .await

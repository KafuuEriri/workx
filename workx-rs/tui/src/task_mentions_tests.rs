...
fn task_reference_context_deduplicates_and_merges_with_ide_context() {
    ...
    let mut items = vec![UserInput::Text {
        ...
        text_elements: [plugin_start, selected_start]
            .into_iter()
            .map(|start| {
                workx_app_server_protocol::TextElement::new(
                    ByteRange {
                        start,
    ...
    assert_eq!(text.matches("## My request for Workx:").count(), 1);
...
fn task_reference_heading_inside_selected_title_is_not_a_context_boundary() {
    ...
    let mut items = vec![UserInput::Text {
        text: format!("@{title}"),
        text_elements: vec![workx_app_server_protocol::TextElement::new(
            ByteRange {
                start: 0,
                end: title.len() + 1,
            },
    ...
    assert!(text.starts_with("## Referenced chats with Workx:"));

...
use workx_utils_path as path_utils;
...
impl PickerState {
    ...
    async fn persist_density(&self) -> Result<()> {
        ...
        ConfigEditsBuilder::new(&persistence.workx_home)
...
mod tests {
    ...
    fn row_search_matches_metadata_fields() {
        ...
        let row = Row {
            ...
            created_at: None,
            updated_at: None,
            cwd: Some(PathBuf::from("/tmp/workx-session-picker")),
    ...
    fn expanded_session_details_include_metadata() {
        ...
        let row = Row {
            ...
            cwd: Some(PathBuf::from("/Users/felipe.coury/code/workx")),
            git_branch: Some(String::from("workx/raw-scrollback-mode")),
        };
        ...
        assert!(rendered.contains("Branch:      workx/raw-scrollback-mode"));
    ...
    fn footer_prioritizes_active_sort_timestamp() {
        ...
        assert_metadata_order(&updated[0], "⌁ tmp/workx", " main");
        assert_metadata_order(&created[0], "⌁ tmp/workx", " main");
    }
    ...
    fn footer_marks_missing_branch() {
        ...
        assert!(rendered.contains("⌁ /tmp/workx"));
        assert!(rendered.contains(" no branch"));
        assert_metadata_order(&footer[0], "⌁ /tmp/workx", " no branch");
    }
    ...
    fn footer_branch_expands_when_line_has_room() {
        ...
        let footer = render_footer_lines(
            ...
            "4h ago",
            Some(branch),
            Some("~/code/workx.etraut-animations-false-improvements/workx-rs"),
            /*show_cwd*/ true,
            /*width*/ 140,
        );
    ...
    fn footer_cwd_truncates_to_responsive_column() {
        let cwd = "~/code/workx.owner-extremely-long-worktree-name-that-needs-truncating/workx-rs";
        ...
        assert!(footer.contains("⌁ ~/code/workx."));
    ...
    fn footer_omits_cwd_when_hidden() {
        let footer = render_footer_lines(
            ...
            "4h ago",
            Some("owner/branch"),
            Some("~/code/workx.owner-worktree/workx-rs"),
            /*show_cwd*/ false,
            /*width*/ 80,
        );
...
"#
    ...
    async fn ctrl_o_keeps_toggled_density_when_persistence_fails() {
        let tmp = tempdir().expect("tmpdir");
        let workx_home_file = tmp.path().join("workx-home-file");
        std::fs::write(&workx_home_file, "not a directory").expect("write workx home file");
        let loader = page_only_loader(|_| {});
        let mut state = PickerState::new(
            FrameRequester::test_dummy(),
            loader,
    ...
    fn dense_snapshot_row() -> Row {
        Row {
            ...
            cwd: Some(PathBuf::from(
                "/Users/felipe.coury/code/workx.fcoury-session-picker/workx-rs",
            )),
            git_branch: Some(String::from("fcoury/session-picker")),
        }
    }
    ...
    fn expanded_session_snapshot() {
        ...
        let row = Row {
            ...
            created_at: parse_timestamp_str("2026-04-28T16:30:00Z"),
            updated_at: parse_timestamp_str("2026-04-28T17:45:00Z"),
            cwd: Some(PathBuf::from("/tmp/workx")),
            git_branch: Some(String::from("fcoury/session-picker")),
        };
        let mut state = PickerState::new(
            FrameRequester::test_dummy(),
    ...
    fn thread_to_transcript_cells_renders_core_message_types() {
        ...
        let thread = Thread {
            ...
            git_info: None,
            name: None,
            turns: vec![workx_app_server_protocol::Turn {

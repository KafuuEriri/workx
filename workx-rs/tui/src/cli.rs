...
use clap::Parser;
use workx_utils_cli::ApprovalModeCliArg;
...
pub struct Cli {
    ...
    // Internal controls set by the top-level `workx resume` subcommand.
    // These are not exposed as user flags on the base `workx` command.
    #[clap(skip)]
    ...
    /// top-level `workx resume <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
    ...
    // Internal controls set by the top-level `workx fork` subcommand.
    // These are not exposed as user flags on the base `workx` command.
    #[clap(skip)]
    ...
    /// top-level `workx fork <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]

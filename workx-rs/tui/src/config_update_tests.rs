...
async fn remote_project_trust_guards_thread_start_and_preserves_repository_decisions() -> Result<()>
{
    ...
    std::fs::create_dir(project_cwd.join(".workx"))?;
    ...
    std::fs::write(workx_home.join("config.toml"), &undecided_config)?;
    std::fs::write(
        project_cwd.join(".workx/config.toml"),
    ...
    assert_eq!(
        std::fs::read_to_string(workx_home.join("config.toml"))?,
        undecided_config
    );
    ...
    std::fs::remove_file(project_cwd.join(".workx/config.toml"))?;
    std::fs::remove_dir(project_cwd.join(".workx"))?;

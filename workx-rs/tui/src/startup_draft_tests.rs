...
fn startup_draft_renders_full_empty_and_multiline_composer_frames() {
    ...
    for (label, width, text, session_action) in [
        ...
        let frame = (0..area.height)
            ...
            .collect::<Vec<_>>()
            .join("\n")
            .replace(crate::version::WORKX_CLI_VERSION, "<VERSION>");
...
async fn startup_draft_hydrates_its_header_without_moving_the_composer() {
    let workx_home = tempfile::tempdir().expect("create temporary Workx home");
...
async fn startup_draft_applies_editor_keymap_without_enabling_vim() {
    ...
    config.tui_keymap.editor.move_line_start = Some(workx_config::types::KeybindingsSpec::One(
        workx_config::types::KeybindingSpec("ctrl-z".to_string()),
    ));
    let mut pump = startup_test_pump(std::iter::empty());
...
async fn startup_draft_waits_for_onboarding_before_accepting_input() {
    ...
    let workx_home = tempfile::tempdir().expect("create an existing custom Workx home");
    std::fs::write(workx_home.path().join("history.jsonl"), "")
        .expect("create existing startup history");
    let system_config_path = workx_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
        workx_home.path().join("system.toml"),
    )
    ...
    pump.initial_screen =
        if crate::startup_preflight::has_only_search_config_override(&search_override)
            && crate::startup_preflight::should_delay_startup_composer_for_first_login(
                ...
                || Ok(false),
                |name| (name == "WORKX_HOME").then(|| workx_home.path().as_os_str().to_os_string()),
            )
        {

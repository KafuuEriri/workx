...
async fn lost_mutation_reply_preserves_work_without_resubmitting() -> Result<()> {
    ...
    app.chat_widget.show_selection_view(SelectionViewParams {
        items: vec![SelectionItem {
            ...
            actions: vec![Box::new(|tx| tx.send(AppEvent::WorkxOp(Op::Interrupt)))],
            ..Default::default()
        }],
        ..Default::default()
    });
    assert!(matches!(
        app.handle_event(&mut tui, &mut session, AppEvent::WorkxOp(op))
            .await?,
    ...
    let history = tokio::time::timeout(Duration::from_secs(/*secs*/ 5), async {
        loop {
            if let Ok(text) =
                tokio::fs::read_to_string(app.config.workx_home.join("history.jsonl")).await
                && let Ok(entry) =
                    serde_json::from_str::<workx_message_history::HistoryEntry>(&text)
            {
                break entry;
            }
    ...
    let mut keymap = workx_config::types::TuiKeymap::default();
    keymap.chat.edit_queued_message = Some(workx_config::types::KeybindingsSpec::One(
        workx_config::types::KeybindingSpec("ctrl-x up".into()),
    ));
    let runtime =

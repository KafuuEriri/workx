...
async fn daemon_ctrl_c_shows_background_exit_menu_and_escape_dismisses_it() -> Result<()> {
    ...
    › 1. Cancel task        Stop the current task and stay in Workx
      2. Run in background  Exit Workx and leave the task running
      3. Exit               Stop the current task and exit Workx
...
async fn daemon_ctrl_c_hides_background_exit_for_running_background_side_thread() -> Result<()> {
    ...
    › 1. Cancel task  Stop the current task and stay in Workx
      2. Exit         Stop the current task and exit Workx

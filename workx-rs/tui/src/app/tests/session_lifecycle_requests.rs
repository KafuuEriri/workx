...
use workx_protocol::items::ExitedReviewModeItem;
...
async fn start_recording_app_server_with_history(
    ...
    let embedded = crate::start_embedded_app_server(
        workx_arg0::Arg0DispatchPaths::default(),
        config.clone(),
        Vec::new(),
        workx_config::LoaderOverrides::default(),
        /*strict_config*/ false,
        workx_config::CloudConfigBundleLoader::default(),
        workx_feedback::WorkxFeedback::new(),
        /*log_db*/ None,
        state_db,
        Arc::new(workx_exec_server::EnvironmentManager::default_for_tests()),
    )
    .await?;
    let workx_home = config.workx_home.display().to_string();
                        ...
                        .send(Message::Text(
                            serde_json::to_string(&JSONRPCMessage::Response(JSONRPCResponse {
                                ...
                                result: serde_json::json!({
                                    "userAgent": "workx-tui-test",
                                    "workxHome": workx_home,
                                }),
                            }))?
                            .into(),
                        ))
...
async fn make_history_test_app() -> Result<(App, tempfile::TempDir)> {
    ...
    app.config.sqlite = SqliteConfig::new_for_testing(workx_home.path().abs());
    Ok((app, workx_home))
}
...
async fn removing_remote_thread_omits_disconnect_guidance() -> Result<()> {
    for event in [
        ...
        let (mut app, workx_home) = make_history_test_app().await?;
...
async fn external_transport_registers_dynamic_tools_and_finds_task_mentions() -> Result<()> {
    let (app, _workx_home) = make_history_test_app().await?;
    ...
    for params in starts {
        ...
        assert_eq!(params["dynamicTools"][0]["name"], "workx_tui");
        assert_eq!(
...
async fn archive_current_thread_reports_success_only_after_archiving() -> Result<()> {
    let (mut app, _workx_home) = make_history_test_app().await?;
    let thread_id = ThreadId::from_string(
        &create_fake_rollout(
            &app.config.workx_home,
            "2026-08-25T01-00-00",
...
async fn local_daemon_registers_approval_gated_mcp_tools_for_both_start_paths() -> Result<()> {
    ...
    app.config
        .web_search_mode
        .set(workx_protocol::config_types::WebSearchMode::Live)?;
    std::fs::write(
    ...
    let inventory: workx_app_server_protocol::ListMcpServerStatusResponse = app_server
        .request_handle()
        .request_typed(ClientRequest::McpServerStatusList {
            request_id: AppServerRequestId::String("tui-tool-inventory".to_string()),
            params: workx_app_server_protocol::ListMcpServerStatusParams {
                cursor: None,
                limit: None,
                detail: Some(workx_app_server_protocol::McpServerStatusDetail::ToolsAndAuthOnly),
    ...
    let tools = &inventory
        .data
        .iter()
        .find(|server| server.name == "workx_tui")
    ...
    for params in &starts {
        ...
        let server = &params["config"]["mcp_servers.workx_tui"];
        assert!(
            server["url"]
                .as_str()
    ...
    let mcp_url = starts[0]["config"]["mcp_servers.workx_tui"]["url"]
        .as_str()
        .expect("MCP server URL");
    let unauthorized = workx_http_client::HttpClientBuilder::new()
        .build_direct()?
        .post(mcp_url)
        .send()
        .await?;
    ...
    app.config
        .web_search_mode
        .set(workx_protocol::config_types::WebSearchMode::Disabled)?;
    ...
    assert_eq!(
        resumed["config"]["mcp_servers.workx_tui"],
        starts[0]["config"]["mcp_servers.workx_tui"]
    );
    app_server
        .resume_thread(
            app.config.clone(),
    ...
    assert_eq!(
        reattached["config"]["mcp_servers.workx_tui"],
        starts[0]["config"]["mcp_servers.workx_tui"]
    );
    app_server
    ...
    assert_eq!(
        forked["config"]["mcp_servers.workx_tui"],
        starts[0]["config"]["mcp_servers.workx_tui"]
    );
    let authorization =
        starts[0]["config"]["mcp_servers.workx_tui"]["http_headers"]["Authorization"]
            .as_str()
            .expect("MCP bearer token");
    let client = workx_http_client::HttpClientBuilder::new().build_direct()?;
    ...
    assert_eq!(
        child["config"]["mcp_servers.workx_tui"],
        starts[0]["config"]["mcp_servers.workx_tui"]
    );
...
async fn local_mcp_respects_configured_servers_and_managed_requirements() -> Result<()> {
    for scenario in ["conflicting", "blocked", "mismatched", "allowed"] {
        ...
        if scenario == "conflicting" {
            let raw = serde_json::from_value::<workx_config::RawMcpServerConfig>(
            ...
            servers.insert(
                ...
                workx_config::McpServerConfig::try_from(raw)
            ...
            if matches!(scenario, "mismatched" | "allowed") {
                let requirement = if scenario == "allowed" {
                    workx_config::McpServerRequirement::Url(
                        workx_protocol::mcp_policy::McpServerValueMatcher::Prefix {
                            value: "http://127.0.0.1:".to_string(),
                        },
                    )
                } else {
                    workx_config::McpServerRequirement::Identity {
                        identity: workx_config::McpServerIdentity::Url {
            ...
            let requirements = workx_config::ConfigRequirements {
                mcp_servers: Some(workx_config::Sourced::new(
                    allowed_servers,
                    workx_config::RequirementSource::Unknown,
                )),
                ..Default::default()
            };
            app.config.config_layer_stack = workx_config::ConfigLayerStack::new(
                Vec::new(),
                requirements,
        ...
        if scenario == "allowed" {
            ...
            assert!(start["config"]["mcp_servers.workx_tui"].is_object());
        } else {
            assert_eq!(
                start["dynamicTools"][0]["tools"].as_array().map(Vec::len),
                Some(6)
            );
            assert!(start["config"]["mcp_servers.workx_tui"].is_null());
        }
        app_server.shutdown().await?;
        proxy.await??;
    }
...
async fn embedded_server_rejects_unowned_dynamic_tool_calls() -> Result<()> {
    ...
    app.handle_app_server_event(
        &app_server,
        workx_app_server_client::AppServerEvent::ServerRequest(Box::new(
            ServerRequest::DynamicToolCall {
                ...
                params: workx_app_server_protocol::DynamicToolCallParams {
                    ...
                    namespace: Some("workx_app".to_string()),
...
async fn dynamic_tool_requests_ignore_other_namespaces_and_dispatch_tui_namespace() -> Result<()> {
    ...
    for namespace in [Some("workx_app"), None] {
        app.handle_app_server_event(
            &app_server,
            workx_app_server_client::AppServerEvent::ServerRequest(Box::new(
                ServerRequest::DynamicToolCall {
        ...
        workx_app_server_client::AppServerEvent::ServerRequest(Box::new(
            ServerRequest::DynamicToolCall {
                ...
                params: workx_app_server_protocol::DynamicToolCallParams {
                    ...
                    namespace: Some("workx_tui".to_string()),
                    tool: "list_threads".to_string(),
                    arguments: serde_json::json!({}),
                },
            },
    ...
    app_server
        ...
        .await?;
    let project: workx_app_server_protocol::ProjectCreateResponse = app_server
        .request_handle()
        .request_typed(ClientRequest::ProjectCreate {
            request_id: AppServerRequestId::String("create-source-project".to_string()),
            params: workx_app_server_protocol::ProjectCreateParams {
                name: "Source project".to_string(),
                roots: vec![workx_app_server_protocol::ProjectRoot {
        ...
        .await?;
    let _: workx_app_server_protocol::ThreadMetadataUpdateResponse = app_server
        .request_handle()
        .request_typed(ClientRequest::ThreadMetadataUpdate {
            request_id: AppServerRequestId::String("assign-source-project".to_string()),
            params: workx_app_server_protocol::ThreadMetadataUpdateParams {
        ...
        .await?;
    let source_settings: workx_app_server_protocol::ThreadResumeResponse = app_server
        .request_handle()
        .request_typed(ClientRequest::ThreadResume {
            request_id: AppServerRequestId::String("read-source-sandbox".to_string()),
            params: workx_app_server_protocol::ThreadResumeParams {
                thread_id: creation_source.to_string(),
                ..workx_app_server_protocol::ThreadResumeParams::default()
            },
        })
        .await?;
    ...
    spawn_approved_task_tool_call(
        ...
        workx_app_server_protocol::DynamicToolCallParams {
            thread_id: creation_source.to_string(),
            turn_id: "turn-1".to_string(),
...
async fn transcript_home_loads_every_older_history_page() -> Result<()> {
    ...
    let export_path = workx_home.path().join("complete-export.md");
    app.chat_widget
...
async fn remote_legacy_history_start_negotiates_once_for_resume_and_fork() -> Result<()> {
    ...
    let response = crate::dynamic_tools::execute(
        ...
        workx_app_server_protocol::ThreadStartParams::default(),
        status_updates,
        /*app_event_tx*/ None,
    )
    .await;
                ...
                app.handle_app_server_event(
                    ...
                    AppServerEvent::ServerNotification(Box::new(
                        ServerNotification::ThreadStatusChanged(
                            workx_app_server_protocol::ThreadStatusChangedNotification {
                                thread_id: started.session.thread_id.to_string(),
...
async fn cold_paginated_subagent_transcript_excludes_inherited_parent_history() -> Result<()> {
    let (app, workx_home) = make_history_test_app().await?;
    let parent_thread_id = create_history_rollout(
        &app.config,
        ThreadHistoryMode::Paginated,
...
async fn changing_directory_preserves_project_trust_permissions_history_and_hooks() -> Result<()> {
    use serde_json::json;
    use std::fs;
    use workx_protocol::config_types::TrustLevel as T;
    ...
    let [current, trusted, unknown, untrusted, mismatch, failed] =
        names.map(|name| workx_home.path().join(name));
    fs::create_dir_all(&current)?;
    for directory in [&trusted, &unknown, &untrusted, &mismatch, &failed] {
        fs::create_dir_all(directory.join(".workx"))?;
        fs::write(directory.join(".workx/config.toml"), "")?;
    }
    let contents = "developer_instructions = \"destination policy\"\nmodel_reasoning_effort = \"high\"\napproval_policy = \"on-request\"\n[tui]\ntheme = \"dracula\"\n[tui.keymap.global]\nopen_transcript = \"f12\"";
    fs::write(trusted.join(".workx/config.toml"), contents)?;
    ...
    fs::write(trusted.join(".workx/hooks.json"), hooks)?;
    let contents = "default_permissions = \"dev\"\n[permissions.dev.filesystem]\n\":root\" = \"write\"\n[tui.keymap.global]\nopen_transcript = \"ctrl-l\"";
    fs::write(mismatch.join(".workx/config.toml"), contents)?;
    let requirements = workx_home.path().join("requirements.toml");
    ...
    for dir in [&trusted, &untrusted, &mismatch, &failed] {
        ...
        crate::legacy_core::config::set_project_trust_level(workx_home.path(), dir, trust)
    ...
    let meta = workx_rollout::read_session_meta_line(&forked_rollout).await?;
    ...
    let source = workx_utils_path_uri::PathUri::from_abs_path(&agents.abs());

...
-> Result<()> {
    ...
    app.config.workx_home = home.path().to_path_buf().abs();
    app.config.chatgpt_base_url = server.uri();
    app.config.sqlite = workx_state::SqliteConfig::new_for_testing(home.path().abs());
    ...
    app.chat_widget.handle_server_notification(
        ServerNotification::Error(ErrorNotification {
            error: AppServerTurnError {
                ...
                workx_error_info: Some(WorkxErrorInfo::UsageLimitExceeded),
    ...
    rolling.rate_limit_reached_type =
        Some(workx_app_server_protocol::RateLimitReachedType::WorkspaceMemberCreditsDepleted);
    app.handle_app_server_event(
        &session,
        workx_app_server_client::AppServerEvent::ServerNotification(Box::new(
            ServerNotification::AccountRateLimitsUpdated(
                workx_app_server_protocol::AccountRateLimitsUpdatedNotification {
    ...
    app.handle_app_server_event(
        ...
        workx_app_server_client::AppServerEvent::ServerNotification(Box::new(
            ServerNotification::AccountUpdated(
                workx_app_server_protocol::AccountUpdatedNotification {
                    auth_mode: None,
                    plan_type: None,
                },
            ),
...
async fn backend_banner_reset_redemption_rejects_pre_reset_content() -> Result<()> {
    ...
    app.handle_event(
        ...
        AppEvent::RateLimitResetCreditConsumed {
            ...
            credit_id: None,
            result: Ok(
                workx_app_server_protocol::ConsumeAccountRateLimitResetCreditResponse {
                    outcome:
                        workx_app_server_protocol::ConsumeAccountRateLimitResetCreditOutcome::Reset,
                },
            ),
        },
    )
    ...
    recovered.rate_limit_reset_credits =
        Some(workx_app_server_protocol::RateLimitResetCreditsSummary {
            available_count: 0,
            credits: None,
        });
    app.handle_event(

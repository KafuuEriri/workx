...
fn startup_bottom_pane() -> (BottomPane, UnboundedReceiver<AppEvent>) {
    ...
    (
        BottomPane::new(BottomPaneParams {
            ...
            placeholder_text: "Ask Workx to do anything".to_string(),
...
async fn startup_draft_handoff_recognizes_late_user_input_as_new_protected_view() -> Result<()> {
    ...
    while let Ok(event) = app_event_rx.try_recv() {
        assert!(
            !matches!(event, AppEvent::WorkxOp(Op::UserInputAnswer { .. })),
    ...
    app.handle_app_server_event(
        ...
        workx_app_server_client::AppServerEvent::ServerRequest(Box::new(
            ServerRequest::CurrentTimeRead {
                ...
                params: workx_app_server_protocol::CurrentTimeReadParams {
                    thread_id: other_thread_id.to_string(),
                },
            },
        )),
...
async fn startup_thread_started_does_not_replay_resolved_approval() -> Result<()> {
    ...
    app.handle_app_server_event(
        &app_server,
        workx_app_server_client::AppServerEvent::ServerRequest(Box::new(request.clone())),
    )
    .await;
    app.handle_app_server_event(
        &app_server,

...
impl App {
    ...
    async fn handle_server_notification_event(
        ...
        if let ServerNotification::ThreadStarted(started) = &notification
            ...
            && let Ok(thread_id) = workx_protocol::ThreadId::from_string(&started.thread.id)
        {
        ...
        match &notification {
            ServerNotification::ServerRequestResolved(notification) => {
                ...
                let notification_thread_id =
                    workx_protocol::ThreadId::from_string(&notification.thread_id).ok();
            ...
            ServerNotification::AccountUpdated(notification) => {
                ...
                let has_workx_backend_auth = matches!(
    ...
    async fn handle_server_request_event(
        ...
        if let ServerRequest::DynamicToolCall { request_id, params } = &request {
            ...
            if app_server_client.uses_embedded_app_server()
                || requires_mcp
                || workx_protocol::ThreadId::from_string(&params.thread_id)
            ...
            let task = tokio::spawn(async move {
                ...
                if inherits_task_tools
                    && response.success
                    && let [
                        workx_app_server_protocol::DynamicToolCallOutputContentItem::InputText {
                            text,
                        },

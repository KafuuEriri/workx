...
impl App {
            ...
            ServerRequest::McpServerElicitationRequest { request_id, params } => {
                    ...
                    )
                {
                    ...
                    match &params.request {
                        workx_app_server_protocol::McpServerElicitationRequest::Form {
                            message,
                            ..
                        ...
                        workx_app_server_protocol::McpServerElicitationRequest::OpenAiForm {
                            ..
                        }
                        | workx_app_server_protocol::McpServerElicitationRequest::OpenAiElicitationForm { .. }
                        | workx_app_server_protocol::McpServerElicitationRequest::Url { .. } => {
                            self.app_event_tx.resolve_elicitation(
                                thread_id,
                                params.server_name.clone(),
                                request_id.clone(),
                                workx_app_server_protocol::McpServerElicitationAction::Decline,
                                /*content*/ None,
                                /*meta*/ None,
                            );
                            None
    ...
    pub(super) fn append_message_history_entry(&self, thread_id: ThreadId, text: String) {
        let history_config = workx_message_history::HistoryConfig::new(
            self.chat_widget.config_ref().workx_home.clone(),
            &self.chat_widget.config_ref().history,
        );
        tokio::spawn(async move {
            if let Err(err) =
                workx_message_history::append_entry(&text, thread_id, &history_config).await
            {
                tracing::warn!(
                    thread_id = %thread_id,
                    error = %err,
    ...
    pub(super) async fn lookup_message_history_entry(
        ...
        tokio::spawn(async move {
            let entry_opt = tokio::task::spawn_blocking(move || {
                workx_message_history::lookup(log_id, offset, &history_config)
            })
            .await
            .unwrap_or_else(|err| {
    ...
    pub(super) async fn lookup_message_history_batch(
        &mut self,
        thread_id: ThreadId,
        cursor: workx_message_history::HistoryBatchCursor,
        log_id: u64,
    ) -> Result<()> {
        let history_config = workx_message_history::HistoryConfig::new(
            self.chat_widget.config_ref().workx_home.clone(),
        ...
        tokio::spawn(async move {
            let event = match tokio::task::spawn_blocking(move || {
                workx_message_history::lookup_batch(log_id, cursor, &history_config)
            })
            .await
            {
                Ok(Ok(batch)) => {
        ...
        match op {
            ...
            AppCommand::ListSkills { cwds, force_reload } => {
                self.handle_skills_list_result(
                    app_server
                        .skills_list(workx_app_server_protocol::SkillsListParams {
                            cwds: cwds.clone(),
        ...
        let misalignment_policy_violation =
            match &notification {
                ServerNotification::Error(notification) if !notification.will_retry => {
                    notification.error.workx_error_info.as_ref()
                }
                ServerNotification::TurnCompleted(notification) => notification
                    .turn
                    .error
                    .as_ref()
                    .and_then(|error| error.workx_error_info.as_ref()),
                _ => None,
            } == Some(&AppServerWorkxErrorInfo::MisalignmentPolicyViolation);
    ...
    fn startup_request_may_open_protected_view(&self, request: &ServerRequest) -> bool {
        ...
        match &params.request {
            workx_app_server_protocol::McpServerElicitationRequest::Form { .. } => true,
            workx_app_server_protocol::McpServerElicitationRequest::OpenAiForm { .. }
            | workx_app_server_protocol::McpServerElicitationRequest::OpenAiElicitationForm {
                ..
            } => false,
            ...
            Some(
                content
                    ...
                    .filter_map(|item| match item {
                        workx_app_server_protocol::UserInput::Text { text, .. } => {

...
pub(super) async fn fetch_account_token_activity(
    request_handle: AppServerRequestHandle,
) -> Result<workx_app_server_protocol::GetAccountTokenUsageResponse> {
...
pub(super) async fn fetch_workspace_messages(
    request_handle: AppServerRequestHandle,
) -> Result<workx_app_server_protocol::GetWorkspaceMessagesResponse> {
...
pub(super) async fn send_add_credits_nudge_email(
    request_handle: AppServerRequestHandle,
    credit_type: AddCreditsNudgeCreditType,
) -> Result<workx_app_server_protocol::AddCreditsNudgeEmailStatus> {
    let request_id = RequestId::String(format!("add-credits-nudge-{}", Uuid::new_v4()));
    let response: workx_app_server_protocol::SendAddCreditsNudgeEmailResponse = request_handle
...
fn plugin_remote_section_error_next_step(label: &str, err: &str) -> &'static str {
    ...
    } else if err.contains("workx plugins are disabled")
        || err.contains("plugin sharing is disabled")
        || err.contains("plugin sharing is not enabled")
        || err.contains("feature disabled")
    {
        "Ask a workspace admin to enable Workx plugins or plugin sharing."
    ...
    } else if err.contains("old build") || err.contains("update workx") || err.contains("stale") {
        "Update Workx, then try opening the shared plugin again."
...
fn plugin_sharing_disabled_remote_section_error() -> PluginRemoteSectionError {
    PluginRemoteSectionError {
        ...
        message: "Plugin sharing is disabled for this Workx session. Enable plugin sharing to load shared plugins.".to_string(),
    }
}
    ...
    request_handle
        .request_typed(ClientRequest::ConfigBatchWrite {
            ...
            params: ConfigBatchWriteParams {
                edits: vec![workx_app_server_protocol::ConfigEdit {
...
pub(super) type McpInventoryMaps = (
    HashMap<String, workx_protocol::mcp::Tool>,
    HashMap<String, Vec<workx_protocol::mcp::Resource>>,
    HashMap<String, Vec<workx_protocol::mcp::ResourceTemplate>>,
    ...
    for status in statuses {
        ...
        auth_statuses.insert(
            ...
            match status.auth_status {
                workx_app_server_protocol::McpAuthStatus::Unknown => McpAuthStatus::Unknown,
                workx_app_server_protocol::McpAuthStatus::Unsupported => McpAuthStatus::Unsupported,
                workx_app_server_protocol::McpAuthStatus::NotLoggedIn => McpAuthStatus::NotLoggedIn,
                workx_app_server_protocol::McpAuthStatus::BearerToken => McpAuthStatus::BearerToken,
                workx_app_server_protocol::McpAuthStatus::OAuth => McpAuthStatus::OAuth,
            },
        );

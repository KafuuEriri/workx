...
use workx_app_server_protocol::CurrentTimeReadParams;
...
use workx_config::types::KeybindingSpec;
use workx_config::types::KeybindingsSpec;
...
async fn shared_overview_seeds_once_and_retains_locally_resumed_history() -> Result<()> {
    ...
    for day in 1..=22 {
        let source = match day {
            3 => workx_protocol::protocol::SessionSource::Custom("atlas".to_string()),
            4 => workx_protocol::protocol::SessionSource::Custom("chatgpt".to_string()),
            5 => workx_protocol::protocol::SessionSource::Exec,
            6 => workx_protocol::protocol::SessionSource::Mcp,
            _ => workx_protocol::protocol::SessionSource::Cli,
        };
...
async fn overview_selection_applies_user_permissions_only_to_unloaded_threads() -> Result<()> {
    ...
    server_config
        .permissions
        .approval_policy
        .set(workx_protocol::protocol::AskForApproval::Never)?;
    let mut thread_ids = Vec::new();
    for day in 1..=5 {
        thread_ids.push(ThreadId::from_string(
            &app_test_support::create_fake_rollout(
                &server_config.workx_home,
                &format!("2025-01-{day:02}T12-00-00"),
                &format!("2025-01-{day:02}T12:00:00Z"),
                "Historical session",
    ...
    let loaded = app_server
        ...
        .await?
        .session;
    app.harness_overrides.sandbox_mode = Some(workx_protocol::config_types::SandboxMode::ReadOnly);
    app.harness_overrides.approval_policy =
        Some(workx_protocol::protocol::AskForApproval::UnlessTrusted);
    app.config
        .permissions
    ...
    for (thread_id, expected_permissions, expected_approval) in [
        ...
        (
            ...
            workx_app_server_protocol::AskForApproval::UnlessTrusted,
        ),
        (
            thread_ids[2],
            PermissionProfile::read_only(),
        ...
        if thread_id == thread_ids[2] {
            ...
            app.harness_overrides.sandbox_mode =
                Some(workx_protocol::config_types::SandboxMode::WorkspaceWrite);
            app.harness_overrides.approval_policy =
                Some(workx_protocol::protocol::AskForApproval::Never);
            app.runtime_permission_profile_override =
                Some(RuntimePermissionProfileOverride::from_config(&app.config));
            app.runtime_approval_policy_override = Some(RuntimeApprovalPolicyOverride::Explicit(
                AskForApproval::UnlessTrusted,
    ...
    let requirements = app.config.workx_home.join("requirements.toml");
    std::fs::write(
        &requirements,
    ...
    assert_eq!(
        ...
        (
            Some(thread_ids[3]),
            workx_app_server_protocol::ThreadStatus::NotLoaded
        ),
    );
    app_server.shutdown().await?;
    Ok(())
...
async fn overview_cold_resume_honors_working_directory_selection() -> Result<()> {
    for (mode, cli_cwd, runtime_cwd) in [
        ...
        let mut app = make_test_app().await;
        let chosen = app.config.workx_home.join("chosen");
        let overridden = app.config.workx_home.join("overridden");
...
async fn restored_server_permission_profile_survives_cd_without_turn_override() -> Result<()> {
    let mut app = make_test_app().await;
    let destination = app.config.workx_home.join("destination");

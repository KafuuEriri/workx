...
impl PendingAppServerRequests {
    fn canonical_thread_id(thread_id: &str) -> String {
        workx_protocol::ThreadId::from_string(thread_id)
...
mod tests {
    ...
    use workx_app_server_protocol::McpElicitationObjectType;
    ...
    use workx_app_server_protocol::PermissionGrantScope;
    use workx_app_server_protocol::PermissionsRequestApprovalParams;
    use workx_app_server_protocol::PermissionsRequestApprovalResponse;
    use workx_app_server_protocol::RequestId as AppServerRequestId;
    use workx_app_server_protocol::ServerRequest;
    use workx_app_server_protocol::ToolRequestUserInputAnswer;
    ...
    fn colliding_approvals_resolve_only_on_their_own_thread() {
        let approvals = [
            ...
            (
                ...
                Op::RequestPermissionsResponse {
                    id: "shared-id".to_string(),
                    response: workx_protocol::request_permissions::RequestPermissionsResponse {
                        permissions: RequestPermissionProfile::default(),
                        scope: workx_protocol::request_permissions::PermissionGrantScope::Turn,
        ...
        for (method, op) in approvals {
            let mut pending = PendingAppServerRequests::default();
            let thread_ids = [
                workx_protocol::ThreadId::new().to_string(),
                workx_protocol::ThreadId::new().to_string(),
            ];
    ...
    fn rejects_permissions_with_paths_that_cannot_be_localized() {
        ...
        let permissions = workx_app_server_protocol::RequestPermissionProfile {
            network: None,
    ...
    fn resolves_permissions_and_user_input_through_app_server_request_id() {
            ...
            .take_resolution(
                ...
                &Op::RequestPermissionsResponse {
                    ...
                    response: workx_protocol::request_permissions::RequestPermissionsResponse {
                        ...
                        scope: workx_protocol::request_permissions::PermissionGrantScope::Session,
                        strict_auto_review: false,
                    },
                },
            )
        ...
        assert_eq!(
            ...
            PermissionsRequestApprovalResponse {
                permissions: workx_app_server_protocol::GrantedPermissionProfile {
                    ...
                    file_system: Some(AdditionalFileSystemPermissions {
                        ...
                        glob_scan_max_depth: None,
                        entries: Some(vec![
                            workx_app_server_protocol::FileSystemSandboxEntry {
                                path: workx_app_server_protocol::FileSystemPath::Path {
                                    path: absolute_path(read_path).into(),
                                },
                                access: workx_app_server_protocol::FileSystemAccessMode::Read,
                            },
                            workx_app_server_protocol::FileSystemSandboxEntry {
                                path: workx_app_server_protocol::FileSystemPath::Path {
                                    path: absolute_path(write_path).into(),
                                },
                                access: workx_app_server_protocol::FileSystemAccessMode::Write,
                            },
                        ]),
                    }),
                },
    ...
    fn does_not_mark_chatgpt_auth_refresh_as_unsupported() {
        ...
        assert_eq!(
            pending.note_server_request(&ServerRequest::ChatgptAuthTokensRefresh {
                ...
                params: workx_app_server_protocol::ChatgptAuthTokensRefreshParams {
                    reason: workx_app_server_protocol::ChatgptAuthTokensRefreshReason::Unauthorized,
    ...
    fn resolve_notification_returns_resolved_exec_request() {
        let mut pending = PendingAppServerRequests::default();
        let thread_id = workx_protocol::ThreadId::new().to_string();
        assert_eq!(

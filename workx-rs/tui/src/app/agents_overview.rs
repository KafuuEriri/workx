            ...
            self.chat_widget.show_selection_view(SelectionViewParams {
                ...
                items: [
                    ...
                    (!workload_identity_selected).then(|| SelectionItem {
                        ...
                        description: Some(
                            "Open `workx agents` in another terminal afterward.".to_string(),
                        ),
    ...
    pub(super) async fn select_agents_overview_thread(
        ...
        if self.primary_thread_id != Some(root_thread_id) {
            ...
            let resume_model_settings = match target_thread.status {
                ...
                workx_app_server_protocol::ThreadStatus::Idle
                | workx_app_server_protocol::ThreadStatus::Active { .. }
                | workx_app_server_protocol::ThreadStatus::SystemError => {
    ...
    pub(super) async fn replay_agents_overview_requests(
        ...
        for request in requests {
            self.handle_app_server_event(
                app_server,
                workx_app_server_client::AppServerEvent::ServerRequest(Box::new(request)),
            )
            .await;
        }
    }
        ...
        tokio::spawn(async move {
            let result = async {
                ...
                let executable = if current_executable
                    .file_stem()
                    .is_some_and(|name| name == "workx-tui")
                {
                    current_executable.with_file_name("workx")
                } else {
                    current_executable
                };

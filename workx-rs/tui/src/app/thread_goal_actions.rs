...
const EPHEMERAL_THREAD_GOAL_ERROR_MESSAGE: &str = concat!(
    ...
    "Run `workx` to start a saved session, or `workx resume` / `/resume` to reopen one.",
);
...
impl App {
    ...
    pub(super) async fn open_thread_goal_editor(
        ...
        let workx_home = app_server.workx_home_path(&self.config.workx_home);
        match goal_files::objective_text_for_edit(app_server, workx_home.as_ref(), &goal.objective)
            .await
        {

...
use workx_utils_absolute_path::test_support::PathExt;
...
use workx_app_server_protocol::AdditionalFileSystemPermissions;
use workx_app_server_protocol::AdditionalNetworkPermissions;
use workx_app_server_protocol::AdditionalPermissionProfile;
use workx_app_server_protocol::AgentMessageDeltaNotification;
...
use workx_app_server_protocol::FileUpdateChange;
use workx_app_server_protocol::ItemStartedNotification;
use workx_app_server_protocol::JSONRPCErrorError;
use workx_app_server_protocol::McpServerElicitationRequest;
...
use workx_app_server_protocol::NetworkApprovalContext as AppServerNetworkApprovalContext;
use workx_app_server_protocol::NetworkApprovalProtocol as AppServerNetworkApprovalProtocol;
use workx_app_server_protocol::NetworkPolicyAmendment as AppServerNetworkPolicyAmendment;
use workx_app_server_protocol::NetworkPolicyRuleAction as AppServerNetworkPolicyRuleAction;
use workx_app_server_protocol::NonSteerableTurnKind as AppServerNonSteerableTurnKind;
use workx_app_server_protocol::PatchChangeKind;
use workx_app_server_protocol::PermissionsRequestApprovalParams;
use workx_app_server_protocol::RequestId as AppServerRequestId;
...
use workx_app_server_protocol::TokenUsageBreakdown;
...
use workx_app_server_protocol::UserInput as AppServerUserInput;
...
use workx_protocol::models::NetworkPermissions;
use workx_protocol::models::PermissionProfile;
use workx_protocol::openai_models::MODEL_SPECIALTY_CYBER;
use workx_protocol::protocol::EventMsg;
use workx_protocol::protocol::MAX_THREAD_GOAL_OBJECTIVE_CHARS;
use workx_protocol::protocol::MultiAgentVersion;
use workx_protocol::protocol::SessionMeta;
use workx_protocol::protocol::SessionSource as RolloutSessionSource;
...
use workx_protocol::user_input::TextElement;
...
async fn next_thread_settings_updated(
    ...
    for _ in 0..20 {
        ...
        if let workx_app_server_client::AppServerEvent::ServerNotification(notification) = event
...
async fn handle_mcp_inventory_result_respects_origin_thread() {
    ...
    app.handle_mcp_inventory_result(
        Ok(vec![McpServerStatus {
            ...
            auth_status: workx_app_server_protocol::McpAuthStatus::Unsupported,
        }]),
...
async fn external_editor_writable_directory_rejected_snapshot() -> Result<()> {
    ...
    let (mut app, mut app_event_rx, _op_rx) = make_test_app_with_channels().await;
    let workx_home = app.chat_widget.config_ref().workx_home.clone();
    let fallback_home = dirs::home_dir()
        .expect("home directory")
        .join(".workx")
        .abs();
    let workspace_workx_home = app.chat_widget.config_ref().cwd.join(".workx");
    let permission_profile = PermissionProfile::workspace_write_with(
        &[workx_home, fallback_home, workspace_workx_home],
        workx_protocol::permissions::NetworkSandboxPolicy::Restricted,
...
async fn enqueue_primary_thread_session_replays_buffered_approval_after_attach() -> Result<()> {
    ...
    app.handle_app_server_event(
        &app_server,
        workx_app_server_client::AppServerEvent::ServerNotification(Box::new(
            ServerNotification::ServerRequestResolved(
                workx_app_server_protocol::ServerRequestResolvedNotification {
...
async fn enqueue_primary_thread_session_replays_turns_before_initial_prompt_submit() -> Result<()> {
    ...
    app.chat_widget = ChatWidget::new_with_app_event(ChatWidgetInit {
        ...
        has_workx_backend_auth: false,
        model_catalog: app.model_catalog.clone(),
        feedback: workx_feedback::WorkxFeedback::new(),
        is_first_run: false,
    ...
    while let Ok(event) = app_event_rx.try_recv() {
        match event {
            ...
            AppEvent::WorkxOp(Op::UserTurn { items, .. }) => {
                assert!(
...
async fn history_lookup_response_is_routed_to_requesting_thread() -> Result<()> {
    ...
    let cursor = workx_message_history::HistoryBatchCursor::new(/*end_offset*/ 10);
...
async fn active_history_batch_is_delivered_without_replay_buffering() -> Result<()> {
    ...
    let cursor = workx_message_history::HistoryBatchCursor::new(/*end_offset*/ 1);
    let event = HistoryLookupResponse::Batch {
        ...
        next_older_cursor: Some(workx_message_history::HistoryBatchCursor::new(
            /*end_offset*/ 0,
        )),
    };
    ...
    app.handle_thread_event_now(ThreadBufferedEvent::Notification(Box::new(
        ServerNotification::ItemStarted(ItemStartedNotification {
            ...
            item: ThreadItem::CollabAgentToolCall {
                id: "wait-1".to_string(),
                tool: workx_app_server_protocol::CollabAgentTool::Wait,
                status: workx_app_server_protocol::CollabAgentToolCallStatus::InProgress,
...
async fn collab_receiver_notification_does_not_cache_not_found_thread() {
    ...
    app.handle_thread_event_now(ThreadBufferedEvent::Notification(Box::new(
        ServerNotification::ItemCompleted(workx_app_server_protocol::ItemCompletedNotification {
            thread_id: ThreadId::new().to_string(),
            turn_id: "turn-1".to_string(),
            completed_at_ms: 0,
            item: ThreadItem::CollabAgentToolCall {
                id: "send-1".to_string(),
                tool: workx_app_server_protocol::CollabAgentTool::SendInput,
                status: workx_app_server_protocol::CollabAgentToolCallStatus::Failed,
                sender_thread_id: ThreadId::new().to_string(),
                receiver_thread_ids: vec![receiver_thread_id.to_string()],
                ...
                agents_states: HashMap::from([(
                    ...
                    workx_app_server_protocol::CollabAgentState {
                        status: workx_app_server_protocol::CollabAgentStatus::NotFound,
                        message: None,
                    },
                )]),
            },
...
fn selected_and_resumed_threads_use_server_capability_for_v1_and_v2_children() -> Result<()> {
    ...
    runtime.block_on(async {
        ...
        let root_session_meta = SessionMeta {
            ...
            originator: "workx-tui-test".to_string(),
        ...
        assert!(
            std::iter::from_fn(|| app_event_rx.try_recv().ok())
                .any(|event| matches!(event, AppEvent::WorkxOp(Op::UserTurn { .. })))
        );
...
async fn update_memory_settings_persists_and_updates_widget_config() -> Result<()> {
    ...
    app.config.workx_home = workx_home.path().to_path_buf().abs();
    ...
    let config = std::fs::read_to_string(workx_home.path().join("config.toml"))?;
...
fn update_memory_settings_updates_current_thread_memory_mode() -> Result<()> {
    ...
    runtime.block_on(async {
        ...
        let workx_home = tempdir()?;
        app.config.workx_home = workx_home.path().to_path_buf().abs();
        app.config.sqlite = workx_state::SqliteConfig::new_for_testing(workx_home.path().abs());
        ...
        let state_db = workx_state::StateRuntime::init(
            workx_state::SqliteConfig::new_for_testing(workx_home.path().abs()),
...
async fn reset_memories_clears_local_memory_directories() -> Result<()> {
    Box::pin(async {
        ...
        let memory_root = workx_home.path().join("memories");
...
"#,
    ...
    let op = match app_event_rx.try_recv() {
        Ok(AppEvent::WorkxOp(op)) => op,
        other => panic!("expected WorkxOp event, got {other:?}"),
    };
    assert_eq!(
        op,
...
-> Result<()> {
    ...
    let workx_home = tempdir()?;
    app.config.workx_home = workx_home.path().to_path_buf().abs();
    let config_toml_path = workx_home.path().join("config.toml").abs();
...
async fn side_defers_subagent_approval_overlay_until_side_exits() -> Result<()> {
    ...
    app.enqueue_thread_request(
        ...
        ServerRequest::DynamicToolCall {
            ...
            params: workx_app_server_protocol::DynamicToolCallParams {
...
async fn inactive_thread_exec_approval_preserves_context() {
    ...
    assert_eq!(
        ...
        vec![
            workx_app_server_protocol::CommandExecutionApprovalDecision::Accept,
            workx_app_server_protocol::CommandExecutionApprovalDecision::AcceptForSession,
            workx_app_server_protocol::CommandExecutionApprovalDecision::ApplyNetworkPolicyAmendment {
                network_policy_amendment: AppServerNetworkPolicyAmendment {
                    host: "example.com".to_string(),
                    action: AppServerNetworkPolicyRuleAction::Allow,
                },
            },
            workx_app_server_protocol::CommandExecutionApprovalDecision::Cancel,
        ]
    );
}
    ...
    app.enqueue_thread_notification(
        ...
        ServerNotification::ItemStarted(ItemStartedNotification {
            ...
            item: ThreadItem::FileChange {
                ...
                status: workx_app_server_protocol::PatchApplyStatus::InProgress,
            },
        }),
    )
    .await
...
async fn inactive_thread_permissions_approval_preserves_file_system_permissions() {
    ...
    let request = ServerRequest::PermissionsRequestApproval {
        ...
        params: PermissionsRequestApprovalParams {
            ...
            permissions: workx_app_server_protocol::RequestPermissionProfile {
                network: Some(AdditionalNetworkPermissions {
                    enabled: Some(true),
                }),
                file_system: Some(AdditionalFileSystemPermissions {
    ...
    assert_matches!(
        ...
        Ok(AppEvent::SubmitThreadOp {
            ...
            op: Op::ResolveElicitation {
                server_name,
                request_id: AppServerRequestId::Integer(10),
                decision: workx_app_server_protocol::McpServerElicitationAction::Decline,
                content: None,
                meta: None,
            },
    ...
    app.enqueue_thread_notification(
        ...
        ServerNotification::ThreadStarted(ThreadStartedNotification {
            thread: Thread {
                ...
                updated_at: 2,
                recency_at: Some(2),
                status: workx_app_server_protocol::ThreadStatus::Idle,
                path: Some(rollout_path.clone()),
                cwd: test_path_buf("/tmp/agent").abs(),
                cli_version: "0.0.0".to_string(),
                source: workx_app_server_protocol::SessionSource::Unknown,
...
async fn background_side_cleanup_removes_local_state_and_ignores_late_events() -> Result<()> {
    ...
    app.handle_app_server_event(
        &app_server,
        workx_app_server_client::AppServerEvent::ServerRequest(Box::new(exec_approval_request(
            side_thread_id,
            "turn-1",
            "item-1",
            Some("approval-1"),
    ...
    let resolution = app
        ...
        .take_resolution(
            ...
            Op::ExecApproval {
                ...
                turn_id: None,
                decision: workx_app_server_protocol::CommandExecutionApprovalDecision::Accept,
            },
        )
...
async fn active_non_primary_shutdown_target_returns_none_for_non_shutdown_event() -> Result<()> {
    ...
    assert_eq!(
        app.active_non_primary_shutdown_target(&ServerNotification::SkillsChanged(
            workx_app_server_protocol::SkillsChangedNotification {},
        )),
        None
    );
    Ok(())
...
async fn clear_ui_header_shows_fast_status_for_fast_capable_models() {
    ...
    app.chat_widget.set_service_tier(Some(
        workx_protocol::config_types::ServiceTier::Fast
            .request_value()
            .to_string(),
    ));
...
async fn set_thread_goal_draft_materializes_long_objective_and_confirms_before_paste() -> Result<()>
{
    ...
    let workx_home = app_server
        .workx_home_path(&app.chat_widget.config_ref().workx_home)
        .expect("workx home");
    assert!(goal_files::objective_file_path(&goal.objective, Some(&workx_home)).is_some());
    assert_eq!(
        goal_files::objective_text_for_edit(&mut app_server, Some(&workx_home), &goal.objective)
            .await
    ...
    for path in [
        format!("/tmp/{suffix}"),
        format!("{workx_home}/../other/{suffix}"),
        format!("{workx_home}/other/{suffix}"),
    ] {
        assert!(!is_managed(&workx_home, &path));
    }
    assert!(!is_managed(
        &AppServerPath::from_app_server("/tmp/workx\\home"),
        &format!("/tmp/workx/home/{suffix}")
    ));
    let unix_path = AppServerPath::from_app_server("/tmp/workx\\").join("a");
    assert_eq!(unix_path.as_str(), "/tmp/workx\\/a");
    let attachments_dir = app.chat_widget.config_ref().workx_home.join("attachments");
...
fn active_turn_not_steerable_turn_error_extracts_structured_server_error() {
    let turn_error = AppServerTurnError {
        ...
        workx_error_info: Some(AppServerWorkxErrorInfo::ActiveTurnNotSteerable {
...
async fn remote_exec_resume_current_cwd_is_rejected() -> Result<()> {
    ...
    app.environment_manager = Arc::new(
        EnvironmentManager::create_for_tests(
            ...
            Some(workx_exec_server::ExecServerRuntimePaths::new(
                std::env::current_exe()?,
                /*workx_linux_sandbox_exe*/ None,
            )?),
        )
        .await,
    );
...
async fn in_app_resume_uses_configured_or_explicit_cwd() -> Result<()> {
    ...
    for (configured_mode, has_explicit_cwd, has_remote_exec, has_runtime_cwd, expected_directory) in [
        ...
        std::fs::write(
            workx_home.join("config.toml"),
            format!("[tui]\nresume_cwd = \"{configured_mode}\"\n"),
        )?;
        let config = ConfigBuilder::default()
            .workx_home(workx_home.clone())
            .loader_overrides(LoaderOverrides::without_managed_config_for_tests())
        ...
        .expect("materialized rollout should be created");
        let rollout_path =
            app_test_support::rollout_path(&workx_home, filename_timestamp, &thread_id);
...
async fn remembered_current_cwd_stays_at_launch_across_in_app_resumes() -> Result<()> {
    ...
    crate::legacy_core::config::edit::ConfigEditsBuilder::for_config(&config)
        .set_resume_cwd(workx_config::types::ResumeCwdMode::Current)
        .apply()
        .await
...
async fn prompt_edit_forks_before_selected_prompt_and_preserves_source() -> Result<()> {
    ...
    .expect("materialized rollout should be created");
    let source_path =
        app_test_support::rollout_path(config.workx_home.as_path(), filename_ts, &source_thread_id);
    ...
    for (turn_id, message, images, local_images) in [
        ...
        for item in [
            ...
            workx_rollout::append_rollout_item_to_path(&source_path, &item).await?;
        }
    }
        ...
        .await?;
    {
        ...
        for item in selected_turn.items {
            store.push_notification(ServerNotification::ItemCompleted(
                workx_app_server_protocol::ItemCompletedNotification {
...
async fn replace_chat_widget_reseeds_collab_agent_metadata_for_replay() {
    ...
    let replacement = ChatWidget::new_with_app_event(ChatWidgetInit {
        ...
        has_workx_backend_auth: app.chat_widget.has_workx_backend_auth(),
        model_catalog: app.model_catalog.clone(),
        ...
        ThreadEventSnapshot {
            ...
            events: vec![ThreadBufferedEvent::Notification(Box::new(
                ServerNotification::ItemStarted(
                    workx_app_server_protocol::ItemStartedNotification {
                        ...
                        item: ThreadItem::CollabAgentToolCall {
                            ...
                            tool: workx_app_server_protocol::CollabAgentTool::Wait,
                            status:
                                workx_app_server_protocol::CollabAgentToolCallStatus::InProgress,
                            sender_thread_id: ThreadId::new().to_string(),
                            receiver_thread_ids: vec![receiver_thread_id.to_string()],
                            prompt: None,
                            model: None,
    ...
    Box::pin(async {
        ...
        let op = AppCommand::override_turn_context(
            ...
            Some(ActivePermissionProfile::new(
                workx_protocol::models::BUILT_IN_PERMISSION_PROFILE_WORKSPACE,
            )),
            /*windows_sandbox_level*/ None,
...
async fn changing_cyber_model_reasoning_preserves_selected_permissions() {
    Box::pin(async {
        ...
        assert!(
            app.apply_permission_profile_selection(PermissionProfileSelection {
                profile_id: workx_protocol::models::BUILT_IN_PERMISSION_PROFILE_READ_ONLY
                    .to_string(),
        ...
        for effort in [ReasoningEffortConfig::High, ReasoningEffortConfig::Ultra] {
            ...
            assert_eq!(
                settings
                    ...
                    .expect("active permission profile")
                    .id,
                workx_protocol::models::BUILT_IN_PERMISSION_PROFILE_READ_ONLY
            );
            assert_eq!(
...
async fn selecting_cyber_model_falls_back_to_user_when_auto_review_is_unavailable() {
    ...
    assert_eq!(
        ...
        Some(workx_app_server_protocol::AskForApproval::OnRequest)
    );
    assert_eq!(
        params.approvals_reviewer,
        Some(workx_app_server_protocol::ApprovalsReviewer::User)
    );
}
...
async fn selecting_cyber_model_respects_auto_review_requirements() {
    Box::pin(async {
        ...
        let mut requirements_with_sources = workx_config::ConfigRequirementsWithSources::default();
        requirements_with_sources.merge_unset_fields(
            workx_config::RequirementSource::Unknown,
            requirements_toml.clone(),
        );
        let requirements = workx_config::ConfigRequirements::try_from(requirements_with_sources)
...
async fn inactive_thread_settings_notification_updates_cached_collaboration_mode() {
    ...
    let notification = ThreadSettingsUpdatedNotification {
        ...
        thread_settings: ThreadSettings {
            ...
            approval_policy: AskForApproval::OnRequest,
            approvals_reviewer: workx_app_server_protocol::ApprovalsReviewer::AutoReview,
            sandbox_policy: workx_app_server_protocol::SandboxPolicy::ReadOnly {
                network_access: false,
            },
            active_permission_profile: Some(
                workx_app_server_protocol::ActivePermissionProfile::read_only(),
            ),
            model: "gpt-plan".to_string(),
...
async fn clear_only_ui_reset_allows_active_skill_warning_to_render_again() {
    ...
    let error = SkillErrorInfo {
        path: test_path_buf("/tmp/project/.workx/skills/abc/SKILL.md"),

...
async fn lookup_legacy_index_target(
    ...
    let candidates = workx_rollout::find_thread_meta_candidates_by_name_str(
    ...
    for (path, session_meta) in candidates {
        if !path.starts_with(config.workx_home.join(workx_rollout::SESSIONS_SUBDIR)) {
            continue;
        }
        ...
        loop {
            if let Some(thread) = self.pending.next() {
                ...
                let session_meta = if app_server.uses_remote_workspace() {
                    ...
                    let Some(path) = workx_rollout::existing_rollout_path(path).await else {
                        continue;
                    };
                    let expected_root = self.workx_home.join(match self.collection {
                        SessionCollection::Active => workx_rollout::SESSIONS_SUBDIR,
                        SessionCollection::Archived => workx_rollout::ARCHIVED_SESSIONS_SUBDIR,
                    });
                    if !path.starts_with(expected_root) {
                        continue;
                    }
                    let Ok(session_meta) =
                        workx_rollout::read_session_meta_line(path.as_path()).await
                    else {
                        continue;
                    };

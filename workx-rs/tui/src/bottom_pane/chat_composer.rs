...
//!
//! For the detailed burst state machine, see `workx-rs/tui/src/bottom_pane/paste_burst.rs`.
//!
    ...
    fn mention_items(&self) -> Vec<MentionItem> {
            ...
            && let Some(snapshot) = self.connectors_snapshot.as_ref()
        {
            for connector in &snapshot.connectors {
                ...
                let display_name = workx_connectors::metadata::connector_display_label(connector);
...
mod tests {
    ...
    use workx_protocol::models::local_image_label_text;
    ...
    pub(super) fn new_test_composer() -> (ChatComposer, UnboundedReceiver<AppEvent>) {
        ...
        (
            ChatComposer::new(
                ...
                "Ask Workx to do anything".to_string(),
        ...
        snapshot_composer_state(
            ...
            |composer| {
                ...
                composer.set_status_line(Some(Line::from(
                    "gpt-5.4 high fast · ~/code/workx-1 · Context 0% used",
                )));
    ...
    fn set_skill_mentions_refreshes_open_mention_popup() {
        ...
        composer.set_skill_mentions(Some(vec![SkillMetadata {
            name: "workx".to_string(),
            description: "Primary personal Workx repo skill.".to_string(),
            short_description: None,
            interface: None,
            dependencies: None,
        ...
        assert_eq!(mention.insert_text, "$workx".to_string());
    ...
    fn default_unified_mention_popup_snapshot() {
        snapshot_composer_state(
            ...
            /*enhanced_keys_supported*/ false,
            |composer| {
                let features = workx_features::Features::with_defaults();
                composer
                    .set_mentions_v2_enabled(features.enabled(workx_features::Feature::MentionsV2));
                composer.set_skill_mentions(Some(vec![
                    SkillMetadata {
...
程序员 与 Unicode 同行";
    ...
    fn file_completion_preserves_large_paste_placeholder_elements() {
        ...
        composer.on_file_search_result(
            ...
            vec![FileMatch {
                ...
                match_type: workx_file_search::MatchType::File,
    ...
    fn pasting_filepath_attaches_image() {
        let tmp = tempdir().expect("create TempDir");
        let tmp_path: PathBuf = tmp.path().join("workx_tui_test_paste_image.png");

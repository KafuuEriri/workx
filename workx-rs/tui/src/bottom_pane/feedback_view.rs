...
use workx_feedback::DOCTOR_REPORT_ATTACHMENT_FILENAME;
use workx_feedback::FEEDBACK_DIAGNOSTICS_ATTACHMENT_FILENAME;
use workx_feedback::FeedbackDiagnostics;
...
const BASE_CLI_BUG_ISSUE_URL: &str =
    "https://github.com/RonanXiao/workx/issues/new?template=3-cli.yml";
/// Internal routing link for employee feedback follow-ups. This must not be shown to external users.
const WORKX_FEEDBACK_INTERNAL_URL: &str = "http://go/workx-feedback-internal";
...
pub(crate) fn feedback_success_cell(
    ...
    let mut lines = vec![Line::from(match issue_url.as_ref() {
        Some(_) if feedback_audience == FeedbackAudience::OpenAiEmployee => {
            format!("{prefix} Please report this in #workx-feedback:")
        }
    ...
    match issue_url {
        Some(url) if feedback_audience == FeedbackAudience::OpenAiEmployee => {
            lines.extend([
                ...
                Line::from(vec![
                    "    ".into(),
                    format!("https://go/workx-feedback/{thread_id}").bold(),
                ]),
            ]);
        }
        Some(url) => {
...
fn slack_feedback_url(_thread_id: &str) -> String {
    WORKX_FEEDBACK_INTERNAL_URL.to_string()
}
...
pub(crate) fn feedback_upload_consent_params(
    ...
    let mut header_lines: Vec<Box<dyn crate::render::renderable::Renderable>> = vec![
        ...
        Line::from(vec!["  • ".into(), "workx-logs.log".into()]).into(),
        Line::from(vec![
            "  • ".into(),
        ...
        Line::from(vec![
            "  • ".into(),
            format!("{WORKX_APPS_TOOLS_CACHE_ATTACHMENT_FILENAME} (if available)").into(),
        ])
        .into(),
        Line::from(vec![
            "  • ".into(),
            format!("{WORKX_APP_DIRECTORY_CACHE_ATTACHMENT_FILENAME} (if available)").into(),
        ])
        .into(),
    ];
    ...
    super::SelectionViewParams {
        ...
        items: vec![
            super::SelectionItem {
                name: "Yes".to_string(),
                description: Some(
                    "Share the current Workx session logs and diagnostics with the team for troubleshooting."
                        .to_string(),
                ),
...
mod tests {
    ...
    use workx_feedback::FeedbackDiagnostic;
    ...
    fn issue_url_available_for_bug_bad_result_safety_check_and_other() {
        ...
        let expected_slack_url = "http://go/workx-feedback-internal".to_string();
        ...
        let expected_external_url = "https://github.com/RonanXiao/workx/issues/new?template=3-cli.yml&steps=Uploaded%20thread:%20t";
    ...
    fn feedback_success_cell_matches_employee_bug_copy() {
        ...
        assert_eq!(
            rendered,
            "• Feedback uploaded. Please report this in #workx-feedback:\n\n  http://go/workx-feedback-internal\n\n  Share this and add some info about your problem:\n    https://go/workx-feedback/thread-2"
        );
    }

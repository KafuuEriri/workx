...
//! | `WORKX_HOME` | `OnceLock<Option<PathBuf>>` | Root for custom `.tmTheme` discovery |
//!
...
static WORKX_HOME: OnceLock<Option<PathBuf>> = OnceLock::new();
...
/// Set the user-configured syntax theme override and workx home path.
///
/// Call this with the **final resolved config** (after onboarding, resume, and
/// fork reloads complete). The first call persists `name` and `workx_home` in
...
pub(crate) fn set_theme_override(
    ...
    let warning = validate_theme_name(name.as_deref(), workx_home.as_deref());
    let override_set_ok = THEME_OVERRIDE.set(name.clone()).is_ok();
    let workx_home_set_ok = WORKX_HOME.set(workx_home.clone()).is_ok();
    ...
    if !override_set_ok || !workx_home_set_ok {
...
pub(crate) fn validate_theme_name(name: Option<&str>, workx_home: Option<&Path>) -> Option<String> {
    let name = name?;
    let custom_theme_path_display = workx_home
        .map(|home| custom_theme_path(name, home).display().to_string())
        .unwrap_or_else(|| format!("$WORKX_HOME/themes/{name}.tmTheme"));
...
fn custom_theme_path(name: &str, workx_home: &Path) -> PathBuf {
    workx_home.join("themes").join(format!("{name}.tmTheme"))
}

/// Try to load a custom `.tmTheme` file from `{workx_home}/themes/{name}.tmTheme`.
fn load_custom_theme(name: &str, workx_home: &Path) -> Option<Theme> {
    ThemeSet::get_theme(custom_theme_path(name, workx_home)).ok()
}
...
fn resolve_theme_with_override(name: Option<&str>, workx_home: Option<&Path>) -> Theme {
    ...
    if let Some(name) = name {
        ...
        // 2. Try loading {WORKX_HOME}/themes/{name}.tmTheme from disk.
        if let Some(home) = workx_home
...
fn build_default_theme() -> Theme {
    ...
    let workx_home = WORKX_HOME
        .get()
        .and_then(|workx_home| workx_home.as_deref());
    resolve_theme_with_override(name, workx_home)
}
...
pub(crate) fn configured_theme_name() -> String {
    ...
    if let Some(Some(name)) = THEME_OVERRIDE.get() {
        ...
        if let Some(Some(home)) = WORKX_HOME.get()
...
pub(crate) fn resolve_theme_by_name(name: &str, workx_home: Option<&Path>) -> Option<Theme> {
    let ts = two_face::theme::extra();
    // Bundled theme?
...
/// `.tmTheme` file under `{WORKX_HOME}/themes/`.
...
/// found in `{workx_home}/themes/`.
pub(crate) fn list_available_themes(workx_home: Option<&Path>) -> Vec<ThemeEntry> {
...
<key>background</key><string>{deleted_background}</string>
    ...
    fn unique_foreground_colors_for_theme(theme_name: &str) -> Vec<String> {
        let theme = resolve_theme_by_name(theme_name, /*workx_home*/ None)
    ...
    fn bundled_theme_can_provide_diff_scope_backgrounds() {
        let theme = resolve_theme_by_name("github", /*workx_home*/ None)
    ...
    fn validate_theme_name_none_for_bundled() {
        ...
        assert!(validate_theme_name(Some("dracula"), /*workx_home*/ None).is_none());
        assert!(validate_theme_name(Some("nord"), Some(Path::new("/nonexistent"))).is_none());
    }

    #[test]
    fn validate_theme_name_none_when_no_override() {
        assert!(validate_theme_name(/*name*/ None, /*workx_home*/ None).is_none());
    }

    ...
    let service_tier_edit = service_tier.map_or_else(
        ...
        |service_tier| {
            let config_value = if service_tier == SERVICE_TIER_DEFAULT_REQUEST_VALUE {
                ...
                match workx_protocol::config_types::ServiceTier::from_request_value(service_tier) {
                    Some(workx_protocol::config_types::ServiceTier::Fast) => "fast",
                    Some(workx_protocol::config_types::ServiceTier::Flex) => "flex",
                    None => service_tier,
                }
            };
...
pub(crate) async fn read_remote_project_trust(
    ...
    let trust_target = disabled_reason
        ...
        .or_else(|| {
            disabled_project
                .and_then(|layer| layer["name"]["dotWorkxFolder"].as_str())
                .and_then(|path| {
                    path.strip_suffix("/.workx")
                        .or_else(|| path.strip_suffix("\\.workx"))
                })
        })
        .unwrap_or(&cwd);

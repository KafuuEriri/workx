...
async fn archived_session_requires_confirmation_before_resume_or_fork() -> Result<()> {
    for action in [
        ...
        let active_path = rollout_path(workx_home.path(), timestamp, &id);
        let archived_dir = workx_home.path().join("archived_sessions");
...
fn session_start_error_surfaces_archived_guidance_without_rollout_path() {
    ...
    let target_session = SessionTarget {
        path: Some(std::path::PathBuf::from(
            "/Users/me/.workx/archived_sessions/rollout.jsonl",
        )),
        thread_id,
        history_mode: None,
    };

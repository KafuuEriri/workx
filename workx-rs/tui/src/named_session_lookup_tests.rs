...
async fn state_runtime(config: &Config) -> std::io::Result<Arc<workx_state::StateRuntime>> {
    let runtime = workx_state::StateRuntime::init(
        workx_state::SqliteConfig::new_for_testing(config.workx_home.as_path().abs()),
...
async fn upsert_thread(
    ...
    metadata: workx_state::ThreadMetadata,
) -> std::io::Result<()> {
    runtime
...
fn thread_metadata(
    ...
    builder.cwd = config.workx_home.join("project").to_path_buf();
...
fn write_rollout(
    ...
    let session_meta = SessionMetaLine {
        meta: SessionMeta {
            ...
            cwd: config.workx_home.join("project").to_path_buf(),
...
async fn add_scan_sentinel(config: &Config, search_term: &str) -> color_eyre::Result<ThreadId> {
    ...
    workx_rollout::append_thread_name(
...
async fn uses_sqlite_title_without_scanning_rollouts() -> color_eyre::Result<()> {
    ...
    workx_rollout::append_thread_name(config.workx_home.as_path(), duplicate_id, "saved-session")
        .await?;
...
async fn recovers_legacy_index_name_without_requiring_sidecar_rewrite() -> color_eyre::Result<()> {
    ...
    workx_rollout::append_thread_name(config.workx_home.as_path(), thread_id, "saved-session")
        .await?;
    // The direct index lookup should not scan and repair other matching rollouts.
    let scan_sentinel_id = add_scan_sentinel(&config, "saved-session").await?;

    let index_path = config.workx_home.join("session_index.jsonl");
...
async fn skips_stale_legacy_index_candidate() -> color_eyre::Result<()> {
    ...
    .await?;
    workx_rollout::append_thread_name(config.workx_home.as_path(), stale_id, "saved-session")
        .await?;
    ...
    workx_rollout::append_thread_name(config.workx_home.as_path(), fallback_id, "saved-session")
        .await?;
...
async fn keeps_newer_paginated_sqlite_name_when_index_is_stale() -> color_eyre::Result<()> {
    ...
    runtime
        ...
        .await
        .map_err(std::io::Error::other)?;
    workx_rollout::append_thread_name(config.workx_home.as_path(), thread_id, "old-session")
        .await?;
...
async fn prefers_most_recent_duplicate_legacy_name() -> color_eyre::Result<()> {
    ...
    workx_rollout::append_thread_name(config.workx_home.as_path(), recent_id, "same-name").await?;
    workx_rollout::append_thread_name(config.workx_home.as_path(), older_id, "same-name").await?;
    for (path, timestamp) in [

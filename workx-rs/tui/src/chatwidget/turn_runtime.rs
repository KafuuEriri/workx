...
impl ChatWidget {
    ...
    pub(super) fn handle_app_server_steer_rejected_error(
        &mut self,
        workx_error_info: &AppServerWorkxErrorInfo,
    ) -> bool {
        matches!(
            workx_error_info,
            AppServerWorkxErrorInfo::ActiveTurnNotSteerable { .. }
    ...
    pub(super) fn on_server_overloaded_error(&mut self, message: String) {
        ...
        let message = if message.trim().is_empty() {
            "Workx is currently experiencing high load.".to_string()
        } else {
            message
        };
    ...
    pub(super) fn on_rate_limit_error(&mut self, error_kind: RateLimitErrorKind, message: String) {
        ...
        let rate_limit_reached_type = self.workx_rate_limit_reached_type.map(|kind| {
        ...
        if self.workx_rate_limit_reached_type != rate_limit_reached_type {
            self.clear_backend_banner();
        }
        self.workx_rate_limit_reached_type = rate_limit_reached_type;
    ...
    pub(super) fn handle_non_retry_error(
        &mut self,
        message: String,
        workx_error_info: Option<AppServerWorkxErrorInfo>,
    ) {
        if workx_error_info == Some(AppServerWorkxErrorInfo::MisalignmentPolicyViolation) {
            self.on_misalignment_policy_violation();
        } else if workx_error_info
            .as_ref()
            ...
            })
        {
            ...
            self.request_redraw();
            self.maybe_send_next_queued_input();
        } else if let Some(info) = workx_error_info
            .as_ref()

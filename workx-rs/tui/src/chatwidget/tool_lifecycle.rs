...
impl ChatWidget {
    ...
    pub(super) fn on_web_search_end(
        ...
        call_id: String,
        query: String,
        action: workx_app_server_protocol::WebSearchAction,
    ) {
        self.flush_answer_stream_with_separator();
    ...
    pub(crate) fn handle_file_change_completed_now(&mut self, item: ThreadItem) {
        ...
        if matches!(status, workx_app_server_protocol::PatchApplyStatus::Failed) {
    ...
    pub(crate) fn handle_mcp_tool_call_completed_now(&mut self, item: ThreadItem) {
        ...
        let result = match (result, error) {
            ...
            (Some(result), None) => {
                let result = *result;
                Ok(workx_protocol::mcp::CallToolResult {
                    content: result.content,
                    structured_content: result.structured_content,
                    is_error: Some(status == workx_app_server_protocol::McpToolCallStatus::Failed),
                    meta: None,
                })
            }

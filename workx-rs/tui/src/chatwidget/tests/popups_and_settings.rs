...
use workx_app_server_protocol::HookErrorInfo;
use workx_app_server_protocol::HooksListEntry;
use workx_app_server_protocol::HooksListResponse;
use workx_app_server_protocol::MarketplaceLoadErrorInfo;
...
async fn plugin_detail_popup_snapshot_labels_personal_marketplace_as_local() {
    ...
    let mut plugin = plugins_test_detail(
        ...
        &[
            (workx_app_server_protocol::HookEventName::PreToolUse, 1),
            (workx_app_server_protocol::HookEventName::Stop, 2),
        ],
        &["Figma", "Slack"],
        &["figma-mcp", "docs-mcp"],
    );
...
async fn experimental_features_popup_snapshot() {
    ...
    let mut config = workx_config::types::TuiKeymap::default();
    config.list.accept = Some(workx_config::types::KeybindingsSpec::One(
        workx_config::types::KeybindingSpec("ctrl-x enter".to_string()),
    ));
...
async fn model_picker_refreshes_startup_catalog() {
    for (explicit_all_models, hidden_startup) in [(false, false), (true, false), (false, true)] {
        ...
        auto.model = "workx-auto-test".to_string();
...
async fn model_picker_refresh_dismisses_empty_choices() {
        ...
        [(false, false), (true, false), (false, true), (true, true)]
    {
        ...
        let before = render_bottom_popup(&chat, /*width*/ 80);
        if explicit_all_models {
            preset.model = "workx-auto-test".to_string();
        } else {
...
async fn server_overloaded_error_does_not_switch_models() {
    ...
    handle_error(
        &mut chat,
        "server overloaded",
        Some(WorkxErrorInfo::ServerOverloaded),
    );
...
async fn reasoning_up_shortcut_does_not_silently_enter_advanced_effort() {
    for (model, model_path) in [
        ...
        ("workx-auto-test", "workx-auto-test"),
    ] {
        let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(Some("gpt-5.4")).await;
        chat.thread_id = Some(ThreadId::new());
        let mut preset = get_available_model(&chat, "gpt-5.4");
...
async fn auto_model_advertising_advanced_effort_opens_reasoning_picker() {
    ...
    preset.id = "workx-auto-test".to_string();
    preset.model = "workx-auto-test".to_string();
    preset.display_name = "workx-auto-test".to_string();
...
async fn feedback_upload_consent_popup_snapshot() {
    ...
    chat.show_selection_view(crate::bottom_pane::feedback_upload_consent_params(
        ...
        &workx_feedback::FeedbackDiagnostics::new(vec![workx_feedback::FeedbackDiagnostic {

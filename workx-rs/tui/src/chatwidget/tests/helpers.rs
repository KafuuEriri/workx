...
pub(super) async fn test_config() -> Config {
    ...
    let workx_home = tempfile::Builder::new()
        .prefix("chatwidget-tests-")
        .tempdir()
        .expect("tempdir")
        .keep();
    let mut config =
        Config::load_default_with_cli_overrides_for_workx_home(workx_home.clone(), Vec::new())
            .await
            .expect("config");
    // Keep generic UI snapshots stable when the bundled catalog default changes.
    config.model = Some("gpt-5.6-sol".to_string());
    config.workx_home = workx_home.abs();
    config.sqlite = workx_state::SqliteConfig::new_for_testing(workx_home.as_path().abs());
    config.log_dir = workx_home.join("log");
...
pub(super) async fn make_chatwidget_manual_with_auth(
    ...
    let mut widget = ChatWidget::new_with_op_target(common, super::WorkxOpTarget::Direct(op_tx));
...
fn token_usage_breakdown(usage: TokenUsage) -> workx_app_server_protocol::TokenUsageBreakdown {
    workx_app_server_protocol::TokenUsageBreakdown {
            ...
            chat.handle_server_notification(
                ServerNotification::ThreadTokenUsageUpdated(
                    workx_app_server_protocol::ThreadTokenUsageUpdatedNotification {
                        ...
                        token_usage: workx_app_server_protocol::ThreadTokenUsage {
...
pub(super) fn handle_error(
    ...
    workx_error_info: Option<WorkxErrorInfo>,
) {
    chat.handle_server_notification(
...
pub(super) fn begin_exec_with_source(
    ...
    let command_actions = workx_shell_command::parse_command::parse_command(&command)
        .into_iter()
...
pub(super) fn terminal_interaction(
    ...
    chat.handle_server_notification(
        ServerNotification::TerminalInteraction(
            workx_app_server_protocol::TerminalInteractionNotification {
                thread_id: thread_id(chat),
                turn_id: chat
                    .turn_lifecycle
                    .last_turn_id
...
pub(super) fn plugins_test_detail(
    ...
    description: Option<&str>,
    skills: &[&str],
    hooks: &[(workx_app_server_protocol::HookEventName, usize)],
    apps: &[&str],
    mcp_servers: &[&str],
) -> PluginDetail {
    PluginDetail {
...
pub(super) async fn assert_hook_events_snapshot(
    ...
    let mut entries = vec![workx_app_server_protocol::HookOutputEntry {
        kind: workx_app_server_protocol::HookOutputEntryKind::Warning,
        text: "Heads up from the hook".to_string(),
    }];
    if event_name != workx_app_server_protocol::HookEventName::Interrupt {
        entries.push(workx_app_server_protocol::HookOutputEntry {

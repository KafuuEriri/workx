...
use workx_app_server_protocol::SpendControlLimitSnapshot;
use workx_app_server_protocol::ThreadUsage;
...
async fn app_server_cyber_policy_error_renders_dedicated_notice() {
    ...
    handle_error(
        ...
        Some(WorkxErrorInfo::CyberPolicy),
    );
...
async fn configured_pet_load_is_deferred_until_after_construction() {
    ...
    crate::pets::write_test_pack(&cfg.workx_home);
...
async fn rate_limit_snapshots_keep_separate_entries_per_limit_id() {
    ...
    chat.on_rate_limit_snapshot(Some(RateLimitSnapshot {
        ...
        limit_name: Some("workx".to_string()),
        primary: Some(RateLimitWindow {
            used_percent: 20,
            window_duration_mins: Some(300),
            resets_at: Some(100),
    ...
    let workx = chat
        .rate_limit_snapshots_by_limit_id
        .get("workx")
        .expect("workx snapshot should exist");
    let other = chat
        .rate_limit_snapshots_by_limit_id
        .get("workx_other")
        .expect("workx_other snapshot should exist");

    assert_eq!(workx.primary.as_ref().map(|w| w.used_percent), Some(20.0));
    assert_eq!(
        workx
            .credits
            .as_ref()
...
#[tokio::test]
async fn rate_limit_switch_prompt_skips_non_workx_limit() {
...
async fn rate_limit_usage_warnings_keep_workspace_limit_after_rolling_credits() {
    for rate_limit_reached_type in [
        ...
        for credits in [
            ...
            assert_eq!(
                chat.workx_rate_limit_reached_type,
...
async fn rate_limit_usage_warnings_keep_newly_reached_workspace_limit() {
    for (limit_id, should_warn) in [("workx", true), ("workx_other", false)] {
        let (mut chat, mut rx, _) = make_chatwidget_manual(Some("gpt-5")).await;
        chat.has_chatgpt_account = true;
...
async fn rate_limit_usage_warnings_preserve_and_clear_spend_control_state() {
    ...
    blocked_snapshot.rate_limit_reached_type =
        Some(RateLimitReachedType::WorkspaceMemberUsageLimitReached);
    chat.on_rate_limit_snapshot(Some(blocked_snapshot));
    assert_eq!(chat.workx_spend_control_reached, Some(true));
    ...
    assert_eq!(chat.workx_spend_control_reached, Some(false));
    assert_eq!(
        chat.workx_rate_limit_reached_type,
        Some(RateLimitReachedType::WorkspaceMemberUsageLimitReached)
    );
...
async fn rate_limit_usage_warnings_clear_workspace_limit_from_authoritative_snapshot() {
    ...
    assert_eq!(chat.workx_rate_limit_reached_type, None);
}
...
async fn account_update_clears_derived_usage_limit_state_and_prompt() {
    ...
    assert!(chat.workx_rate_limit_reached_type.is_some());
    assert_eq!(chat.workx_spend_control_reached, Some(true));
    assert!(matches!(
        chat.rate_limit_switch_prompt,
        RateLimitSwitchPromptState::Shown
...
async fn workspace_owner_limit_states_render_state_specific_messages() {
    let cases = [
        (
            RateLimitReachedType::WorkspaceOwnerCreditsDepleted,
            RateLimitErrorKind::Generic,
            "You're out of credits. Your workspace is out of credits. Add credits to continue using Workx.",
        ),
        (
            RateLimitReachedType::WorkspaceOwnerUsageLimitReached,
            RateLimitErrorKind::UsageLimit,
            "Usage limit reached. You've reached your usage limit. Increase your limits to continue using workx.",
        ),
    ];
...
async fn esc_interrupt_pauses_active_goal_turn() {
    ...
    assert_matches!(rx.try_recv(), Ok(AppEvent::WorkxOp(Op::Interrupt)));
    assert_goal_paused_event(&mut rx, thread_id);
...
fn update_thread_goal(chat: &mut ChatWidget, thread_id: ThreadId, status: AppThreadGoalStatus) {
    ...
    chat.handle_server_notification(
        ServerNotification::ThreadGoalUpdated(
            workx_app_server_protocol::ThreadGoalUpdatedNotification {
                thread_id,
                turn_id: None,
                goal,
            },
...
async fn ambient_pet_stays_hidden_until_a_pet_is_selected() {
    ...
    crate::pets::write_test_pack(&chat.config.workx_home);
    chat.set_tui_pet(Some("workx".to_string()));
...
async fn status_line_hostname_renders_current_machine_hostname() {
    ...
    assert_eq!(status_line_text(&chat), workx_config::os_host_name());
    assert!(
        drain_insert_history(&mut rx).is_empty(),
...
async fn status_line_estimated_thread_cost_fetches_and_renders_backend_estimate() {
    ...
    let thread_id = ThreadId::new();
    chat.thread_id = Some(thread_id);
    chat.has_workx_backend_auth = true;
...
async fn account_update_clears_workspace_headline_state() {
    ...
    chat.update_account_state(
        /*status_account_display*/ None, /*plan_type*/ None,
        /*has_chatgpt_account*/ false, /*has_workx_backend_auth*/ false,
    );
...
async fn workspace_headline_fetch_allows_backend_auth_without_chatgpt_account() {
    ...
    chat.update_account_state(
        /*status_account_display*/ None, /*plan_type*/ None,
        /*has_chatgpt_account*/ false, /*has_workx_backend_auth*/ true,
    );
...
async fn interrupted_turn_clears_visible_running_hook() {
    ...
    handle_hook_started(
        ...
        hook_started_run(
            ...
            workx_app_server_protocol::HookEventName::PreToolUse,
...
async fn completed_turn_clears_visible_running_hook() {
    ...
    handle_hook_started(
        ...
        hook_started_run(
            ...
            workx_app_server_protocol::HookEventName::PostToolUse,
            /*status_message*/ None,
        ),
    );
    reveal_running_hooks(&mut chat);
...
async fn renamed_thread_footer_title_snapshot() {
    ...
    chat.handle_server_notification(
        ServerNotification::ThreadNameUpdated(
            workx_app_server_protocol::ThreadNameUpdatedNotification {
                thread_id: thread_id.to_string(),
        ...
        ServerNotification::ThreadGoalUpdated(
            workx_app_server_protocol::ThreadGoalUpdatedNotification {
                ...
                goal: test_thread_goal(
                    workx_app_server_protocol::ThreadGoalStatus::Active,
                    /*token_budget*/ Some(50_000),
                    /*tokens_used*/ 40_000,
                ),
            },
...
async fn status_line_goal_complete_elapsed_footer_snapshot() {
    ...
    chat.refresh_status_line();
    let mut goal = test_thread_goal(
        workx_app_server_protocol::ThreadGoalStatus::Complete,
        /*token_budget*/ None,
        /*tokens_used*/ 40_000,
    );
...
async fn thread_goal_update_for_other_thread_is_ignored() {
    ...
    let mut goal = test_thread_goal(
        workx_app_server_protocol::ThreadGoalStatus::BudgetLimited,
        /*token_budget*/ Some(50_000),
        /*tokens_used*/ 50_000,
    );
...
fn goal_status_indicator_formats_statuses_and_budgets() {
    ...
    assert_eq!(
        goal_status_indicator_from_app_goal(&test_thread_goal(
            workx_app_server_protocol::ThreadGoalStatus::Blocked,
            /*token_budget*/ None,
            /*tokens_used*/ 0,
        )),
    ...
    assert_eq!(
        goal_status_indicator_from_app_goal(&test_thread_goal(
            workx_app_server_protocol::ThreadGoalStatus::UsageLimited,
            /*token_budget*/ None,
            /*tokens_used*/ 0,
        )),
...
fn test_thread_goal(
    status: workx_app_server_protocol::ThreadGoalStatus,
    token_budget: Option<i64>,
    tokens_used: i64,
) -> workx_app_server_protocol::ThreadGoal {
    workx_app_server_protocol::ThreadGoal {
        thread_id: "thread-1".to_string(),
    ...
    chat.handle_server_notification(
        ServerNotification::HookStarted(AppServerHookStartedNotification {
            ...
            run: AppServerHookRunSummary {
                ...
                source: workx_app_server_protocol::HookSource::User,
                display_order: 0,
...
async fn interrupt_hook_events_render_snapshot() {
    assert_hook_events_snapshot(
        workx_app_server_protocol::HookEventName::Interrupt,
    ...
    for delayed_redraw in [false, true] {
        ...
        handle_hook_completed(
            ...
            hook_completed_run(
                "post-tool-use:0:/tmp/hooks.json",
                workx_app_server_protocol::HookEventName::PostToolUse,
                workx_app_server_protocol::HookRunStatus::Completed,
                Vec::new(),
            ),
        );
...
async fn blocked_and_failed_hooks_render_feedback_and_errors() {
    ...
    handle_hook_completed(
        ...
        hook_completed_run(
            "pre-tool-use:0:/tmp/hooks.json",
            workx_app_server_protocol::HookEventName::PreToolUse,
            workx_app_server_protocol::HookRunStatus::Blocked,
            vec![workx_app_server_protocol::HookOutputEntry {
                kind: workx_app_server_protocol::HookOutputEntryKind::Feedback,
    ...
    handle_hook_completed(
        ...
        hook_completed_run(
            ...
            workx_app_server_protocol::HookEventName::PostToolUse,
            workx_app_server_protocol::HookRunStatus::Failed,
            vec![workx_app_server_protocol::HookOutputEntry {
                kind: workx_app_server_protocol::HookOutputEntryKind::Error,
...
async fn completed_same_id_hook_output_survives_restart() {
    ...
    handle_hook_started(
        ...
        hook_started_run(
            hook_id,
            workx_app_server_protocol::HookEventName::Stop,
    ...
    handle_hook_completed(
        ...
        hook_completed_run(
            hook_id,
            workx_app_server_protocol::HookEventName::Stop,
            workx_app_server_protocol::HookRunStatus::Stopped,
            vec![workx_app_server_protocol::HookOutputEntry {
                kind: workx_app_server_protocol::HookOutputEntryKind::Stop,
...
async fn session_end_hook_has_standalone_activity_row() {
    for (message, snapshot) in [
        ...
        handle_hook_started(
            ...
            hook_started_run(
                "session-end",
                workx_app_server_protocol::HookEventName::SessionEnd,
                message,
            ),
        );
...
async fn context_only_hook_completed_before_reveal_stays_out_of_history() {
    ...
    handle_hook_started(
        ...
        hook_started_run(
            ...
            workx_app_server_protocol::HookEventName::SessionStart,
    ...
    handle_hook_completed(
        ...
        hook_completed_run(
            ...
            workx_app_server_protocol::HookRunStatus::Completed,
            vec![workx_app_server_protocol::HookOutputEntry {
                kind: workx_app_server_protocol::HookOutputEntryKind::Context,
...
async fn stopped_hook_hides_model_context_and_preserves_stop_reason_snapshot() {
    ...
    handle_hook_completed(
        ...
        hook_completed_run(
            ...
            workx_app_server_protocol::HookRunStatus::Stopped,
            vec![
                workx_app_server_protocol::HookOutputEntry {
                    kind: workx_app_server_protocol::HookOutputEntryKind::Context,
...
fn hook_started_run(
    id: &str,
    event_name: workx_app_server_protocol::HookEventName,
    status_message: Option<&str>,
) -> workx_app_server_protocol::HookRunSummary {
    hook_run_summary(
        id,
        event_name,
        workx_app_server_protocol::HookRunStatus::Running,
        status_message,
        Vec::new(),
    )
}
...
fn hook_completed_run(
    id: &str,
    event_name: workx_app_server_protocol::HookEventName,
    status: workx_app_server_protocol::HookRunStatus,
    entries: Vec<workx_app_server_protocol::HookOutputEntry>,
) -> workx_app_server_protocol::HookRunSummary {
    hook_run_summary(
...
fn hook_run_summary(
    ...
    workx_app_server_protocol::HookRunSummary {
        id: id.to_string(),
        event_name,
        handler_type: workx_app_server_protocol::HookHandlerType::Command,
        execution_mode: workx_app_server_protocol::HookExecutionMode::Sync,
        scope: workx_app_server_protocol::HookScope::Turn,
        source_path: PathBuf::from(test_path_display("/tmp/hooks.json")).abs(),
        source: workx_app_server_protocol::HookSource::User,
        display_order: 0,
        status,
        ...
        started_at: 1,
        completed_at: (status != workx_app_server_protocol::HookRunStatus::Running).then_some(2),
        duration_ms: (status != workx_app_server_protocol::HookRunStatus::Running).then_some(1),
        entries,
    }
}
...
async fn chatwidget_exec_and_status_layout_vt100_snapshot() {
    ...
    handle_exec_begin(
        ...
        AppServerThreadItem::CommandExecution {
            id: "c1".into(),
            command: workx_shell_command::parse_command::shlex_join(&command),
            cwd: cwd.clone().into(),
            process_id: None,
            plugin_id: None,
            script_path: None,

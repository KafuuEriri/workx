...
impl ChatWidget {
    ...
    /// Clears the terminal title Workx most recently wrote, if any.
    ///
        ...
        self.config
            ...
            .find_map(|layer| match &layer.name {
                ConfigLayerSource::Project { dot_workx_folder } => {
                    dot_workx_folder.as_path().parent().map(Path::to_path_buf)
                }
                _ => None,
            })
    }
    ...
    pub(super) fn status_line_value_for_item(&mut self, item: StatusLineItem) -> Option<String> {
        match item {
            ...
            StatusLineItem::WorkxVersion => Some(WORKX_CLI_VERSION.to_string()),
    ...
    pub(super) fn status_surface_preview_value_for_item(
        ...
        let status_line_item = match item {
            StatusSurfacePreviewItem::AppName => return Some("workx".to_string()),
            ...
            StatusSurfacePreviewItem::WorkxVersion => StatusLineItem::WorkxVersion,
    ...
    pub(super) fn terminal_title_value_for_item(
        ...
        match item {
            TerminalTitleItem::AppName => Some("workx".to_string()),
            ...
            TerminalTitleItem::WeeklyLimit => self
                ...
                .map(|value| Self::truncate_terminal_title_part(value, /*max_chars*/ 32)),
            TerminalTitleItem::WorkxVersion => self
                .status_line_value_for_item(StatusLineItem::WorkxVersion)
                .map(|value| Self::truncate_terminal_title_part(value, /*max_chars*/ 32)),
...
fn five_hour_status_window(
    ...
    find_primary_workx_window(snapshot, "5h")
...
fn find_primary_workx_window<'a>(
    snapshot: &'a RateLimitSnapshotDisplay,
    label: &str,
) -> Option<(&'a RateLimitWindowDisplay, bool)> {

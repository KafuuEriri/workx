...
pub(super) fn app_server_rate_limit_error_kind(
    info: &AppServerWorkxErrorInfo,
) -> Option<RateLimitErrorKind> {
    match info {
        AppServerWorkxErrorInfo::ServerOverloaded => Some(RateLimitErrorKind::ServerOverloaded),
        AppServerWorkxErrorInfo::UsageLimitExceeded => Some(RateLimitErrorKind::UsageLimit),
        AppServerWorkxErrorInfo::ResponseTooManyFailedAttempts {
...
pub(super) fn is_app_server_cyber_policy_error(info: &AppServerWorkxErrorInfo) -> bool {
    matches!(info, AppServerWorkxErrorInfo::CyberPolicy)
}
...
impl ChatWidget {
    ...
    pub(crate) fn on_rolling_rate_limit_snapshot(&mut self, snapshot: RateLimitSnapshot) {
        if let (Some(previous), Some(current)) = (
            self.workx_rate_limit_reached_type,
    ...
    fn on_rate_limit_snapshot_from(
        ...
        if let Some(mut snapshot) = snapshot {
            ...
            let is_workx_limit = limit_id.eq_ignore_ascii_case("workx");
            if is_workx_limit
                && (matches!(source, RateLimitSnapshotSource::AccountUsage)
                    || snapshot.spend_control_reached.is_some())
            {
                self.workx_spend_control_reached = snapshot.spend_control_reached;
            }
            if (is_workx_limit && matches!(source, RateLimitSnapshotSource::AccountUsage))
                || snapshot.rate_limit_reached_type.is_some()
            {
                self.workx_rate_limit_reached_type = snapshot.rate_limit_reached_type;
            }
            let workspace_limit_reached = self.workx_spend_control_reached == Some(true)
                || matches!(
                    self.workx_rate_limit_reached_type,
                    Some(
            ...
            if is_workx_limit && has_workspace_credits {
            ...
            let should_warn_about_rate_limit_usage = is_workx_limit && !has_workspace_credits;
            ...
            self.workx_rate_limit_reached_type = None;
            self.workx_spend_control_reached = None;
        }
    ...
    fn open_rate_limit_switch_prompt(&mut self, preset: ModelPreset) {
        ...
        let switch_actions: Vec<SelectionAction> = vec![Box::new(move |tx| {
            tx.send(AppEvent::WorkxOp(AppCommand::override_turn_context(
                /*cwd*/ None,
    ...
    pub(super) fn open_workspace_owner_nudge_prompt(
        ...
        let (title, prompt) = match credit_type {
            ...
            AddCreditsNudgeCreditType::UsageLimit => (
                "Usage limit reached",
                "Request a limit increase from your owner to continue using workx. Request increase?",
            ),
        };

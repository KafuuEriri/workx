            ...
            if matches!(
                ...
                self.handle_turn_completed_notification(
                    TurnCompletedNotification {
                        ...
                        turn: Turn {
                            id: turn_id,
                            items_view: workx_app_server_protocol::TurnItemsView::NotLoaded,
                            items: Vec::new(),
                            status,
                            error,
                            started_at,
    ...
    pub(super) fn handle_thread_item(
        ...
        match item {
                ...
                self.on_agent_message_item_completed(
                    AgentMessageItem {
                        ...
                        memory_citation: memory_citation.map(|citation| {
                            workx_protocol::memory_citation::MemoryCitation {
                                entries: citation
                                    .entries
                                    .into_iter()
                                    .map(|entry| {
                                        workx_protocol::memory_citation::MemoryCitationEntry {
                                            path: entry.path,
            ...
            item @ ThreadItem::CommandExecution {
                status: workx_app_server_protocol::CommandExecutionStatus::InProgress,
                ..
            } => self.on_command_execution_started(item),
            item @ ThreadItem::CommandExecution {
                source: ExecCommandSource::Agent | ExecCommandSource::UnifiedExecStartup,
                status:
                    workx_app_server_protocol::CommandExecutionStatus::Completed
                    | workx_app_server_protocol::CommandExecutionStatus::Failed,
                ..
            ...
            ThreadItem::WebSearch(item) => {
                ...
                self.on_web_search_end(
                    ...
                    item.query,
                    item.action
                        .unwrap_or(workx_app_server_protocol::WebSearchAction::Other),
                );
            }
            ...
            ThreadItem::FunctionCallOutput {
                    ...
                    )
                {
                    self.add_to_history(history_cell::PrefixedWrappedHistoryCell::new(
                        format!("Sent by Workx from task {source_thread_id}\n{prompt}"),
                        "• ".dim(),
                        "  ",
                    ));
                }

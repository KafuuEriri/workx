...
use workx_config::types::MAX_FUNCTION_KEY;
...
Set unique keys in `~/.workx/config.toml` and retry. \
See the Workx keymap documentation for supported actions and examples."
                    ));
                }
            }
        }
...
Set a different key in `~/.workx/config.toml` and retry. \
See the Workx keymap documentation for supported actions and examples."
                ));
            }
        }
...
mod tests {
    ...
    fn agents_overview_can_be_remapped_and_rejects_conflicts() {
        ...
        let runtime =
            RuntimeKeymap::from_config(&keymap).expect("dashboard shortcuts take priority");

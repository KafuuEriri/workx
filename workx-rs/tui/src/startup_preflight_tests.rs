...
fn startup_delays_composer_for_homes_without_authentication_state() -> std::io::Result<()> {
    ...
    let workx_home = temporary_directory.path().join("workx-home");
    ...
    assert!(should_delay_startup_composer_for_first_login(
        ...
        Ok(system_config_path.clone()),
        || Ok(false),
        |name| (name == workx_login::WORKX_ACCESS_TOKEN_ENV_VAR).then(|| "  ".into()),
    ));

    std::fs::create_dir_all(workx_home.join("tmp").join("arg0"))?;
    assert!(should_delay_startup_composer_for_first_login(
        &workx_home,
        Ok(system_config_path.clone()),
        || Ok(false),
    ...
    let helper_directory = workx_home
        .join("tmp")
        .join("arg0")
        .join("workx-arg0-session");
    ...
    assert!(should_delay_startup_composer_for_first_login(
        ...
        Ok(system_config_path.clone()),
        || Ok(false),
        |name| (name == "WORKX_HOME").then(|| "/custom/home".into()),
    ));
    assert!(!should_delay_startup_composer_for_first_login(
        &workx_home,
        Ok(system_config_path.clone()),
        || Ok(false),
        |name| { (name == workx_login::WORKX_ACCESS_TOKEN_ENV_VAR).then(|| "access-token".into()) },
    ));
    for disabled_credential in [
        workx_login::OPENAI_API_KEY_ENV_VAR,
        workx_login::WORKX_API_KEY_ENV_VAR,
    ] {
        assert!(should_delay_startup_composer_for_first_login(
            &workx_home,
            Ok(system_config_path.clone()),
    ...
    for existing_state in ["history.jsonl", "log", "sessions"] {
        let state_path = workx_home.join(existing_state);
    ...
    assert!(!should_delay_startup_composer_for_first_login(
        ...
        |name| match name {
            "WORKX_HOME" => Some("/custom/home".into()),
            workx_login::WORKX_ACCESS_TOKEN_ENV_VAR => Some("access-token".into()),
            _ => None,
        },
    ));
    ...
    let daemon_directory = workx_home.join("app-server-control");
    ...
    let additional_temporary_state = workx_home.join("tmp").join("other");
...
fn startup_keeps_composer_when_home_state_cannot_be_confirmed() -> std::io::Result<()> {
    ...
    #[cfg(unix)]
    {
        ...
        assert!(!should_delay_startup_composer_for_first_login(
            &blocking_parent.join("workx-home"),
            Ok(system_config_path),
...
fn startup_keeps_composer_for_workload_identity_markers() -> std::io::Result<()> {
    ...
    for marker in [
        workx_protocol::shell_environment::OPENAI_FEDERATION_RULE_ID_ENV_VAR,
        workx_protocol::shell_environment::OPENAI_IDENTITY_TOKEN_FILE_ENV_VAR,
    ] {
    ...
    #[cfg(unix)]
    {
        ...
        assert!(!should_delay_startup_composer_for_first_login(
            ...
            || panic!("workload identity should not probe managed configuration"),
            |name| {
                (name == workx_protocol::shell_environment::OPENAI_IDENTITY_TOKEN_FILE_ENV_VAR)

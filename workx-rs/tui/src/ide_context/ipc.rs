...
#[cfg(any(unix, windows))]
const TUI_SOURCE_CLIENT_ID: &str = "workx-tui";
#[cfg(any(unix, windows))]
const OPEN_IDE_HINT: &str =
    "Open this project in VS Code or Cursor with the Workx extension active.";
#[cfg(any(unix, windows))]
const IDE_DID_NOT_PROVIDE_CONTEXT_HINT: &str = "The IDE extension did not provide context.";
#[cfg(any(unix, windows))]
const KEEP_TRYING_HINT: &str = "Workx will keep trying on future messages.";
    ...
    pub(crate) fn user_facing_hint(&self) -> String {
        match self {
            ...
            IdeContextError::Send(_) => {
                "Workx could not request IDE context. Try /ide again.".to_string()
            }
            IdeContextError::Read(_) | IdeContextError::InvalidResponse(_) => {
                "Workx could not read IDE context. Try /ide again.".to_string()
            }
        }
    }
    ...
    pub(crate) fn prompt_skip_hint(&self) -> String {
        match self {
            ...
            IdeContextError::Read(error) if error.kind() == std::io::ErrorKind::TimedOut => {
                "Workx timed out waiting for IDE context. It will keep trying on future messages."
                    .to_string()
            }
            IdeContextError::RequestFailed(error) if error == "client-disconnected" => {
                hint_with_retry("The IDE connection changed while Workx was requesting context.")
            }
            ...
            IdeContextError::Send(_) => {
                hint_with_retry("Workx lost the IDE connection while requesting context.")
            }
            IdeContextError::InvalidResponse(_) => {
                hint_with_retry("Workx received an unexpected IDE context response.")
            }
            IdeContextError::RequestFailed(_) => hint_with_retry(IDE_DID_NOT_PROVIDE_CONTEXT_HINT),
            IdeContextError::Read(_) => hint_with_retry("Workx could not read IDE context."),
        }
    }
...
pub(crate) fn fetch_ide_context(
    ...
    let primary_socket_path = primary_ipc_socket_path(workx_home);
...
#[cfg(unix)]
fn primary_ipc_socket_path(workx_home: &Path) -> PathBuf {
    workx_home.join("ipc").join("ipc.sock")
}

#[cfg(unix)]
fn legacy_ipc_socket_paths(temp_dir: &Path, uid: libc::uid_t) -> Vec<PathBuf> {
    let ipc_dir = temp_dir.join("workx-ipc");
    if uid == 0 {
...
mod tests {
    ...
    #[test]
    fn primary_ipc_socket_path_uses_workx_home() {
        let workx_home = Path::new("/home/test/.workx");

        assert_eq!(
            primary_ipc_socket_path(workx_home),
            workx_home.join("ipc").join("ipc.sock")
        );
    }
    ...
    fn validate_unix_socket_path_rejects_unsafe_parent_directory() {
        ...
        let socket_path = tempdir.path().join("workx-ipc.sock");

//! The main Workx TUI chat surface.
//!
...
use crate::version::WORKX_CLI_VERSION;
...
use workx_app_server_protocol::CommandExecutionSource as ExecCommandSource;
use workx_app_server_protocol::CreditsSnapshot;
...
use workx_app_server_protocol::ModelVerification as AppServerModelVerification;
...
use workx_app_server_protocol::SkillMetadata;
use workx_app_server_protocol::SkillsListResponse;
use workx_app_server_protocol::ThreadGoal as AppThreadGoal;
use workx_app_server_protocol::ThreadGoalStatus as AppThreadGoalStatus;
...
use workx_app_server_protocol::WorkxErrorInfo as AppServerWorkxErrorInfo;
use workx_config::Constrained;
use workx_config::ConstraintResult;
...
#[cfg(test)]
use workx_git_utils::CommitLogEntry;
use workx_git_utils::current_branch_name;
use workx_git_utils::get_git_repo_root;
use workx_git_utils::local_git_branches;
use workx_git_utils::recent_commits;
use workx_otel::RuntimeMetricsSummary;
use workx_otel::SessionTelemetry;
use workx_plugin::PluginCapabilitySummary;
use workx_protocol::ThreadId;
use workx_protocol::account::PlanType;
use workx_protocol::approvals::GuardianAssessmentAction;
use workx_protocol::approvals::GuardianAssessmentDecisionSource;
use workx_protocol::approvals::GuardianAssessmentEvent;
use workx_protocol::approvals::GuardianAssessmentStatus;
...
use workx_protocol::plan_tool::PlanItemArg as UpdatePlanItemArg;
use workx_protocol::plan_tool::StepStatus as UpdatePlanItemStatus;
...
use workx_utils_cli::resume_hint;
...
use workx_file_search::FileMatch;
...
use workx_utils_approval_presets::ApprovalPreset;
use workx_utils_approval_presets::builtin_approval_presets;
...
pub(crate) struct ChatWidgetInit {
    ...
    pub(crate) has_workx_backend_auth: bool,
    pub(crate) model_catalog: Arc<ModelCatalog>,
    pub(crate) feedback: workx_feedback::WorkxFeedback,
...
/// and by sending requests back to workx-core.
///
...
pub(crate) struct ChatWidget {
    ...
    workx_rate_limit_reached_type: Option<RateLimitReachedType>,
    workx_spend_control_reached: Option<bool>,
    ...
    pub(crate) pet_http_client: workx_http_client::RouteAwareClientPool,
    ...
    feedback: workx_feedback::WorkxFeedback,
...
fn request_permissions_from_params(
    params: workx_app_server_protocol::PermissionsRequestApprovalParams,
...
impl ChatWidget {
    ...
    pub(crate) fn open_feedback_consent(&mut self, category: crate::app_event::FeedbackCategory) {
        ...
        let include_windows_sandbox_log =
            workx_windows_sandbox::current_log_file_path_for_workx_home(&self.config.workx_home)
                .is_file();
    ...
    /// Forward a command directly to workx.
        ...
        T: Into<AppCommand>,
    {
        ...
        match &self.workx_op_target {
            WorkxOpTarget::Direct(workx_op_tx) => {
                crate::session_log::log_outbound_op(&op);
                if let Err(e) = workx_op_tx.send(op) {
                    tracing::error!("failed to submit op: {e}");
                    return false;
                }
            }
            WorkxOpTarget::AppEvent => {
                self.app_event_tx.send(AppEvent::WorkxOp(op));
            }
        }
        true
    }
...
const PLACEHOLDER: &str = "Ask Workx to do anything";

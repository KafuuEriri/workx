...
pub(crate) async fn run_editor(
    ...
    let default_workx_home = dirs::home_dir().map(|home| home.join(".workx"));
    #[cfg(any(target_os = "macos", target_os = "linux"))]
    let project_workx_home = cwd.join(".workx");
    let mut candidate_homes = vec![workx_home];
    if let Some(default_workx_home) = default_workx_home.as_deref() {
        candidate_homes.push(default_workx_home);
    }
    #[cfg(any(target_os = "macos", target_os = "linux"))]
    candidate_homes.push(&project_workx_home);

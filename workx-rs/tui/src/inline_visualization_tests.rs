...
fn context_with_fragment(fragment: &str) -> (TempDir, InlineVisualizationContext) {
    let workx_home = tempfile::tempdir().expect("temp workx home");
    let thread_id = ThreadId::new();
    let context = InlineVisualizationContext::new(workx_home.path(), thread_id)
...
fn granted_visualization_root_overrides_thread_id_derived_root() {
    let workx_home = tempfile::tempdir().expect("temp workx home");
    let granted_context = InlineVisualizationContext::new(workx_home.path(), ThreadId::new())
...
fn rewrites_complete_directive_to_trusted_static_file_placeholder() {
    let (_workx_home, context) = context_with_fragment("<div>chart</div>");

    let rewritten = rewrite_inline_visualizations(
        "Before\n::workx-inline-vis{file=\"chart.html\"}\nAfter",
        Some(&context),
    );
...
fn unavailable_artifact_has_explicit_fallback() {
    let workx_home = tempfile::tempdir().expect("temp workx home");
    let context = InlineVisualizationContext::new(workx_home.path(), ThreadId::new())
        .expect("UUIDv7 thread id should provide a timestamp");

    assert_eq!(
        rewrite_inline_visualizations("::workx-inline-vis{file=\"missing.html\"}", Some(&context),)
            .markdown,
        "_Visualization unavailable on this device._"
    );
}
    ...
    for file in ["../chart.html", "chart.svg"] {
        assert_eq!(
            rewrite_inline_visualizations(
                &format!("::workx-inline-vis{{file=\"{file}\"}}"),
                Some(&context),
            )
            .markdown,
            "_Visualization unavailable on this device._"
...
fn rejects_oversized_fragment() {
    ...
    assert_eq!(
        rewrite_inline_visualizations("::workx-inline-vis{file=\"chart.html\"}", Some(&context),)
            .markdown,
        "_Visualization unavailable on this device._"
    );
}
...
fn viewer_materializes_sandboxed_static_document() {
    let (_workx_home, context) = context_with_fragment(
    ...
    assert_eq!(
        ...
        Some(std::ffi::OsStr::new(".workx-viewers"))
    );
...
fn viewer_reuses_path_and_refreshes_static_document() {
    let (_workx_home, context) = context_with_fragment("<div>first</div>");
...
fn transcript_overlay_remeasures_visualization_when_artifact_becomes_available() {
    ...
    let cell = AgentMarkdownCell::new_with_inline_visualizations(
        "::workx-inline-vis{file=\"chart.html\"}".to_string(),
        Path::new("/workspace"),
        Some(context.clone()),
    );
...
fn visualization_link_uses_the_artifact_name() {
    ...
    let rewritten = rewrite_inline_visualizations(
        "::workx-inline-vis{file=\"compound-interest-explorer.html\"}",
        Some(&context),
    );
...
fn user_markdown_keeps_directive_literal() {
    let rendered =
        crate::markdown_render::render_markdown_text("::workx-inline-vis{file=\"chart.html\"}");
    let text = rendered
        .lines
        .iter()
    ...
    assert_eq!(text, "::workx-inline-vis{file=\"chart.html\"}");
}

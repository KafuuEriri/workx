...
/// has a sensible starting point without implying that Workx is already the
...
pub(crate) fn build_pet_picker_params(
    ...
    let mut entries = available_pet_entries(workx_home);
...
fn available_pet_entries(workx_home: &Path) -> Vec<PetPickerEntry> {
    ...
    entries.extend(custom_pet_entries(workx_home));
    entries
}

fn custom_pet_entries(workx_home: &Path) -> Vec<PetPickerEntry> {
    let mut entries_by_selector = HashMap::new();
    for (directory_name, manifest_file) in [("avatars", "avatar.json"), ("pets", "pet.json")] {
        let Ok(children) = fs::read_dir(workx_home.join(directory_name)) else {
            continue;
        };
        for child in children.flatten() {
            ...
            let Ok(pet) =
                Pet::load_with_workx_home(&selector, /*workx_home*/ Some(workx_home))
            else {
                continue;
            };
...
mod tests {
    ...
    fn picker_lists_app_bundled_and_custom_pets() {
        ...
        write_pet(workx_home.path(), "chefito", "Chefito");
    ...
    #[test]
    fn picker_preselects_workx_without_marking_it_current_when_no_pet_is_configured() {
        let workx_home = tempfile::tempdir().unwrap();
        let params = build_pet_picker_params(
            /*current_pet*/ None,
            workx_home.path(),
        ...
        assert_eq!(params.items[8].name, "Workx");
    ...
    fn picker_imports_legacy_avatar_manifests() {
        let workx_home = tempfile::tempdir().unwrap();
        write_legacy_avatar(workx_home.path(), "legacy", "Legacy");

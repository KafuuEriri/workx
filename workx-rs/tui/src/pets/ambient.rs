//! Ambient terminal rendering for the Workx companion.
//!
...
impl AmbientPet {
    ...
    /// WORKX_HOME cache, and records the terminal protocol support snapshot used
    ...
    pub(crate) fn load(
        ...
        let pet = Pet::load_with_workx_home(
            ...
            /*workx_home*/ Some(workx_home),
        )
        .with_context(|| "load ambient pet")?;
        let cache_dir = workx_home
            .join("cache")
            .join("tui-pets")
            .join("frame-cache")
            .join(&pet.id)
...
mod tests {
    ...
    #[test]
    fn notification_labels_match_workx_app_vocabulary() {

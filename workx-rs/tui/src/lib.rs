...
// The standalone `workx-tui` binary prints a short help message before the
...
use workx_app_server_client::RemoteAppServerClient;
use workx_app_server_client::RemoteAppServerConnectArgs;
pub use workx_app_server_client::RemoteAppServerEndpoint;
use workx_app_server_protocol::Account as AppServerAccount;
...
use workx_app_server_protocol::ThreadSortKey as AppServerThreadSortKey;
...
const TUI_LOG_FILE_NAME: &str = "workx-tui.log";
...
async fn init_state_db_for_app_server_target(
    ...
    match app_server_target {
        AppServerTarget::Embedded => state_db::try_init(config).await.map(Some).map_err(|err| {
            let database_path = workx_state::runtime_db_path_for_corruption_error(&err)
...
fn remove_legacy_tui_log_file(workx_home: &Path) {
    ...
    let _ = std::fs::remove_file(workx_home.join("log").join(TUI_LOG_FILE_NAME));
}
...
pub fn resolve_remote_addr(addr: &str) -> color_eyre::Result<RemoteAppServerEndpoint> {
    if let Some(socket_path) = addr.strip_prefix("unix://") {
        let socket_path = if socket_path.is_empty() {
            let workx_home = find_workx_home().wrap_err("failed to resolve WORKX_HOME")?;
            workx_app_server_client::app_server_control_socket_path(&workx_home)
...
async fn connect_remote_app_server(
    ...
    let app_server = RemoteAppServerClient::connect(RemoteAppServerConnectArgs {
        endpoint,
        client_name: "workx-tui".to_string(),
...
#[cfg(unix)]
async fn maybe_probe_default_daemon_socket(workx_home: &Path) -> Option<AbsolutePathBuf> {
    let socket_path = workx_app_server_client::app_server_control_socket_path(workx_home).ok()?;
...
#[cfg(not(unix))]
async fn maybe_probe_default_daemon_socket(_workx_home: &Path) -> Option<AbsolutePathBuf> {
    None
}
...
async fn resolve_startup_resume_or_fork_cwd(
    tui: &mut Tui,
    config: &Config,
    state_db: Option<&workx_state::StateRuntime>,
...
async fn cloud_config_bundle_for_app_server_target(
    ...
    cloud_config_bundle_loader_for_storage(
        app_server_target
            .auth_config_for_cloud_loader(bootstrap_auth_config(workx_home, bootstrap_config)?),
        /*enable_workx_api_key_env*/ false,
    )
    .await
}
...
async fn run_ratatui_app(
         ...
         terminal_restore_guard: &mut TerminalRestoreGuard| {
            ...
            Ok(AppExitInfo {
                ...
                exit_reason: ExitReason::Fatal(format!(
                    "No saved session found with ID {id_str}. Run `workx {action}` without an ID to choose from existing sessions."
                )),
            })
        };
    ...
    if let Some(w) = crate::render::highlight::set_theme_override(
        ...
        find_workx_home().ok().map(AbsolutePathBuf::into_path_buf),
    ) {
    ...
    let required_elevated_sandbox_needs_setup = windows_sandbox_level
        ...
        && config
            ...
            .source
            .is_some()
        && !crate::windows_sandbox::sandbox_setup_is_complete(config.workx_home.as_path());
    #[cfg(target_os = "windows")]
...
async fn load_bootstrap_config_or_exit(
    ...
    match load_config_toml_with_layer_stack(
        ...
        cwd,
        cli_kv_overrides,
        workx_config::ConfigLoadOptions {
            loader_overrides,
            strict_config,
            cloud_config_bundle,
        },
...
mod tests {
    ...
    async fn bedrock_setup_wizard_requires_eligible_onboarding() -> color_eyre::Result<()> {
        ...
        for (label, config_toml, login_status, target, expected) in [
            ...
            std::fs::write(workx_home.path().join("config.toml"), config_toml)?;
    ...
    fn write_session_rollout(
        ...
        let session_meta = workx_protocol::protocol::SessionMeta {
            ...
            source: workx_protocol::protocol::SessionSource::Cli,
            model_provider: Some(model_provider.to_string()),
            ..Default::default()
        };
        let session_meta = serde_json::to_value(workx_protocol::protocol::SessionMetaLine {
            meta: session_meta,
            git: None,
        })?;
        let lines = [
    ...
    async fn startup_services_use_final_cloud_managed_http_policy() -> color_eyre::Result<()> {
            ...
            [(true, false), (false, true)]
        {
            ...
            let prepared_environment_manager =
                EnvironmentManager::prepare_from_workx_home(workx_home.path()).await?;
            ...
            let cloud_config_bundle =
                workx_config::test_support::CloudConfigBundleFixture::loader_with_enterprise_requirement(
    ...
    fn resolve_remote_addr_accepts_default_socket() -> color_eyre::Result<()> {
        ...
        assert_eq!(
            ...
            RemoteAppServerEndpoint::UnixSocket {
                socket_path: workx_app_server_client::app_server_control_socket_path(&workx_home)?,
            }
        );
        Ok(())
    }
    ...
    fn resolve_remote_addr_accepts_relative_socket_path() -> color_eyre::Result<()> {
        assert_eq!(
            resolve_remote_addr("unix://workx.sock")?,
            RemoteAppServerEndpoint::UnixSocket {
                socket_path: AbsolutePathBuf::relative_to_current_dir("workx.sock")?,
            }
        );
        Ok(())
    }
    ...
    fn resolve_remote_addr_accepts_absolute_socket_path() -> color_eyre::Result<()> {
        let temp_dir = TempDir::new()?;
        let socket_path = temp_dir.path().join("workx.sock");
        assert_eq!(
    ...
    async fn default_daemon_auto_connect_skips_missing_socket() -> color_eyre::Result<()> {
        let workx_home = TempDir::new()?;
        assert!(
            maybe_probe_default_daemon_socket(workx_home.path())
                .await
                .is_none()
        );
        Ok(())
    ...
    async fn default_daemon_auto_connect_probes_socket_only() -> color_eyre::Result<()> {
        let workx_home = TempDir::new()?;
        let socket_path =
            workx_app_server_client::app_server_control_socket_path(workx_home.path())?;
        ...
        assert_eq!(
            maybe_probe_default_daemon_socket(workx_home.path()).await,
            Some(socket_path)
        );
        Ok(())
    }
    ...
    {
        let socket_path = AbsolutePathBuf::relative_to_current_dir("workx.sock")?;
    ...
    async fn fork_last_filters_latest_session_by_cwd_unless_show_all() -> color_eyre::Result<()> {
        ...
        let mut app_server = AppServerSession::new(
            workx_app_server_client::AppServerClient::InProcess(
    ...
    async fn embedded_state_db_failure_is_typed_for_cli_recovery() -> color_eyre::Result<()> {
        ...
        let sqlite =
            workx_state::SqliteConfig::new_for_testing(occupied_sqlite_home.as_path().abs());
    ...
    -> color_eyre::Result<()> {
        ...
        let sqlite = workx_state::SqliteConfig::new_for_testing(sqlite_home.as_path().abs());
        ...
        assert!(
            workx_state::sqlite_error_detail_is_corruption(startup_error.detail()),
    ...
    async fn config_rebuild_changes_trust_defaults_with_cwd() -> std::io::Result<()> {
        let temp_dir = TempDir::new()?;
        let workx_home = temp_dir.path().to_path_buf();

...
//!
//! Workx uses the terminal scrollback itself for finalized chat history, so inserting a history
...
mod tests {
    ...
    fn vt100_terminal_wrap_policy_does_not_pre_wrap_long_paragraph() {
        ...
        assert!(
            ...
            "expected terminal soft-wrap instead of Workx word pre-wrap, rows: {rows:?}"
        );
    }

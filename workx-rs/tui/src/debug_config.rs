...
use workx_config::NetworkUnixSocketPermissionToml;
use workx_config::RequirementSource;
use workx_config::ResidencyRequirement;
use workx_config::SandboxModeRequirement;
use workx_config::WebSearchModeRequirement;
use workx_config::format_config_layer_source;
...
mod tests {
    ...
    use workx_config::ConstraintError;
    use workx_config::FeatureRequirementsToml;
    use workx_config::FilesystemConstraints;
    use workx_config::HookEventsToml;
    use workx_config::HookHandlerConfig;
    use workx_config::LoaderOverrides;
    use workx_config::ManagedHooksRequirementsToml;
    use workx_config::MatcherGroup;
    use workx_config::McpServerIdentity;
    use workx_config::McpServerRequirement;
    use workx_config::NetworkConstraints;
    use workx_config::NetworkDomainPermissionToml;
    use workx_config::NetworkDomainPermissionsToml;
    use workx_config::NetworkHeaderInjectionToml;
    use workx_config::NetworkUnixSocketPermissionToml;
    use workx_config::NetworkUnixSocketPermissionsToml;
    use workx_config::RequirementSource;
    use workx_config::ResidencyRequirement;
    use workx_config::SandboxModeRequirement;
...
"#,
    ...
    fn debug_config_output_lists_all_layers_including_disabled() {
        let system_file = if cfg!(windows) {
            absolute_path("C:\\etc\\workx\\config.toml")
        } else {
            absolute_path("/etc/workx/config.toml")
        };
        let project_folder = if cfg!(windows) {
            absolute_path("C:\\repo\\.workx")
        } else {
            absolute_path("/repo/.workx")
        };
    ...
    fn debug_config_output_lists_requirement_sources() {
        let requirements_file = if cfg!(windows) {
            absolute_path("C:\\ProgramData\\OpenAI\\Workx\\requirements.toml")
        } else {
            absolute_path("/etc/workx/requirements.toml")
        };
        ...
        let sqlite_home = if cfg!(windows) {
            absolute_path("C:\\Users\\alice\\.workx\\state")
        } else {
            absolute_path("/home/alice/.workx/state")
        };
        let log_dir = if cfg!(windows) {
            absolute_path("C:\\Users\\alice\\.workx\\logs")
        } else {
            absolute_path("/home/alice/.workx/logs")
        };
        let model_catalog_json = if cfg!(windows) {
            absolute_path("C:\\Users\\alice\\.workx\\models.json")
        } else {
            absolute_path("/home/alice/.workx/models.json")
        };
        ...
        let user_file = if cfg!(windows) {
            absolute_path("C:\\users\\alice\\.workx\\config.toml")
        } else {
            absolute_path("/home/alice/.workx/config.toml")
        };
        let stack = ConfigLayerStack::new(
    ...
    fn debug_config_output_formats_unix_socket_permissions() {
        ...
        let requirements_source = (RequirementSource::LegacyManagedConfigTomlFromMdm).to_string();
        assert!(rendered.contains(&format!(
            "experimental_network: unix_sockets={{/tmp/blocked.sock=deny, /tmp/workx.sock=allow}} (source: {requirements_source})"
        )));
    }
...
"#;
        ...
        let mdm_base_dir = if cfg!(windows) {
            absolute_path("C:\\workx")
        } else {
            absolute_path("/var/lib/workx")
        };

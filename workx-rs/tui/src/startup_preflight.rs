...
pub(super) fn should_delay_startup_composer_for_first_login(
    ...
    if environment_variable(workx_login::WORKX_ACCESS_TOKEN_ENV_VAR).is_some_and(|credential| {
        credential
            .to_str()
    ...
    match std::fs::metadata(workx_home) {
        ...
        Err(error)
            if error.kind() == io::ErrorKind::NotFound
                && matches!(workx_home.try_exists(), Ok(false)) => {}
        Err(_) => return false,
    }

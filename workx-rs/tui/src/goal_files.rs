...
const GOAL_FILE_PREFIX: &str = "Read the Workx goal objective file at ";
...
pub(crate) async fn materialize_goal_draft(
    ...
    workx_home: Option<&GoalFilePath>,
    draft: GoalDraft,
    ...
    for (placeholder, text) in draft.pending_pastes.iter() {
        ...
        let path = ensure_goal_output_dir(app_server, workx_home, &mut output_dir)
            .await?
...
pub(crate) async fn objective_text_for_edit(
    ...
    let Some(path) = objective_file_path(objective, workx_home) else {
...
async fn ensure_goal_output_dir(
    ...
    let workx_home = workx_home
        .context("App server did not report $WORKX_HOME; cannot materialize goal files")?;
    let path = workx_home
        .join(GOAL_ATTACHMENT_DIR)

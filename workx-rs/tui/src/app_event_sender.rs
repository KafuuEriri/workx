...
impl AppEventSender {
    ...
    pub(crate) fn send(&self, event: AppEvent) {
        ...
        if !matches!(event, AppEvent::WorkxOp(_)) {
    ...
    pub(crate) fn interrupt(&self) {
        self.send(AppEvent::WorkxOp(AppCommand::interrupt()));
    }

    pub(crate) fn compact(&self) {
        self.send(AppEvent::WorkxOp(AppCommand::compact()));
    }

    pub(crate) fn set_thread_name(&self, name: String) {
        self.send(AppEvent::WorkxOp(AppCommand::set_thread_name(name)));
    }

    pub(crate) fn review(&self, target: ReviewTarget) {
        self.send(AppEvent::WorkxOp(AppCommand::review(target)));
    }

    pub(crate) fn list_skills(&self, cwds: Vec<PathBuf>, force_reload: bool) {
        self.send(AppEvent::WorkxOp(AppCommand::list_skills(
            cwds,
            force_reload,
        )));
    }
    ...
    pub(crate) fn user_input_answer(&self, id: String, response: ToolRequestUserInputResponse) {
        self.send(AppEvent::WorkxOp(AppCommand::user_input_answer(
            id, response,
        )));
    }

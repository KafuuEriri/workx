...
async fn run_git_command(
    ...
    argv.push(workx_git_utils::SAFE_BARE_REPOSITORY_CONFIG.to_string());
...
mod tests {
    ...
    async fn open_pull_request_falls_back_to_parent_repo_commit_lookup() {
        let runner = FakeRunner::new(vec![
            ...
            response(
                ...
                /*exit_code*/ 0,
                r#"{"nameWithOwner":"fcoury/workx","parent":{"nameWithOwner":"RonanXiao/workx"}}"#,
            ),
            response(
                &[
                    "gh",
                    ...
                    "-H",
                    "Accept: application/vnd.github+json",
                    "repos/RonanXiao/workx/commits/head-sha/pulls",
                ],
                /*exit_code*/ 0,
    ...
    fn status_line_pr_fallback_searches_parent_repo_first() {
        assert_eq!(
            ...
            Some(vec![
                "RonanXiao/workx".to_string(),
                "fcoury/workx".to_string()
            ])
        );
    }
    ...
    fn response(argv: &[&str], exit_code: i32, stdout: &str) -> FakeResponse {
        ...
        if argv.first().map(String::as_str) == Some("git") {
            argv.splice(
                ...
                [
                    "-c".to_string(),
                    workx_git_utils::SAFE_BARE_REPOSITORY_CONFIG.to_string(),
                ],
            );
        }
        FakeResponse {

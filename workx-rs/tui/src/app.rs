...
use uuid::Uuid;
use workx_ansi_escape::ansi_escape_line;
...
use workx_app_server_protocol::SandboxMode as AppServerSandboxMode;
...
use workx_app_server_protocol::SkillErrorInfo;
...
use workx_app_server_protocol::WriteStatus;
...
use workx_models_manager::model_presets::HIDE_GPT_5_1_WORKX_MAX_MIGRATION_PROMPT_CONFIG;
use workx_models_manager::model_presets::HIDE_GPT5_1_MIGRATION_PROMPT_CONFIG;
...
#[cfg(target_os = "windows")]
use workx_protocol::permissions::FileSystemSandboxKind;
use workx_rollout::StateDbHandle;
use workx_terminal_detection::user_agent;
use workx_utils_absolute_path::AbsolutePathBuf;
use workx_utils_approval_presets::builtin_permission_profile_for_active_permission_profile;
        ...
        ServerNotification::ItemCompleted(notification) => match &notification.item {
            ThreadItem::CollabAgentToolCall { agents_states, .. } => {
                agents_states.get(receiver_thread_id).is_some_and(|state| {
                    matches!(
                        &state.status,
                        workx_app_server_protocol::CollabAgentStatus::NotFound
                    )
                })
            }
            _ => false,
...
fn default_exec_approval_decisions(
    network_approval_context: Option<&workx_app_server_protocol::NetworkApprovalContext>,
    proposed_execpolicy_amendment: Option<&workx_app_server_protocol::ExecPolicyAmendment>,
    proposed_network_policy_amendments: Option<
        &[workx_app_server_protocol::NetworkPolicyAmendment],
    >,
    additional_permissions: Option<&workx_app_server_protocol::AdditionalPermissionProfile>,
) -> Vec<workx_app_server_protocol::CommandExecutionApprovalDecision> {
...
fn resume_hint_for_resumable_thread(
    ...
    workx_utils_cli::resume_hint(thread.thread_name.as_deref(), Some(thread.thread_id))
}
...
pub(crate) struct App {
    ...
    dynamic_tool_status_updates:
        tokio::sync::broadcast::Sender<workx_app_server_protocol::ThreadStatusChangedNotification>,
    dynamic_tool_tasks: HashMap<workx_app_server_protocol::RequestId, (String, JoinHandle<()>)>,
...
fn active_turn_not_steerable_turn_error(error: &TypedRequestError) -> Option<AppServerTurnError> {
    ...
    matches!(
        turn_error.workx_error_info,
        Some(AppServerWorkxErrorInfo::ActiveTurnNotSteerable { .. })
    )
...
impl App {
    pub fn chatwidget_init_for_forked_or_resumed_thread(
        ...
        crate::chatwidget::ChatWidgetInit {
            ...
            has_workx_backend_auth: self.chat_widget.has_workx_backend_auth(),

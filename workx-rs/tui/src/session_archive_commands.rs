//! Shared implementation for `workx archive`, `workx delete`, and `workx unarchive`.
//!
...
pub async fn run_session_archive_command(
    ...
    let workx_home = find_workx_home().wrap_err("failed to find Workx home")?;
    let mut app_server =
        start_app_server_for_session_command(options, workx_home.to_path_buf()).await?;
...
async fn run_session_archive_action_with_app_server(
    ...
    let resolved = resolve_session_target(app_server, workx_home, action, target).await?;
...
pub(super) async fn start_app_server_for_session_command(
    ...
    let workload_identity_selected = workx_login::is_workload_identity_selected();
    ...
    let default_daemon = if explicit_remote_endpoint.is_none() && reuse_implicit_local_daemon {
        super::maybe_probe_default_daemon_socket(workx_home.as_path()).await
    } else {
        None
    };

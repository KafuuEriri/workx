...
pub(super) async fn run_main_inner(
    ...
    // `oss` model provider.
    let overrides_cli = workx_utils_cli::CliConfigOverrides { raw_overrides };
    ...
    let workx_home = match find_workx_home() {
        Ok(workx_home) => workx_home.to_path_buf(),
        Err(err) => {
    ...
    if let Some(profile_v2) = cli.config_profile_v2.as_ref() {
        let user_config_path = resolve_profile_v2_config_path(&workx_home, profile_v2);
    ...
    if !std::io::stdin().is_terminal() || !std::io::stdout().is_terminal() {
        ...
        let validation_environment_manager =
            if should_load_configured_environments(&loader_overrides, &validation_target) {
                EnvironmentManager::prepare_from_workx_home(&workx_home).await
            } else {
    ...
    let initial_screen = if cli.resume_picker || cli.fork_picker || cli.agents_overview {
        ...
        && startup_preflight::should_delay_startup_composer_for_first_login(
            &workx_home,
            workx_config::loader::system_config_toml_file(),
            || workx_config::loader::has_local_managed_configuration(&workx_home),
    ...
    let default_daemon = if explicit_remote_endpoint.is_none() && reuse_implicit_local_daemon {
        startup_draft
            .run_until(maybe_probe_default_daemon_socket(&workx_home))
            .await?
    } else {
        None
    };
    ...
    let prepared_environment_manager =
        if should_load_configured_environments(&loader_overrides, &app_server_target) {
            startup_draft
                .run_until(EnvironmentManager::prepare_from_workx_home(&workx_home))
                .await?
        } else {
            startup_draft
    ...
    remove_legacy_tui_log_file(config.workx_home.as_path());

    let otel_originator = originator().value;
    let otel = match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        workx_app_server_client::build_otel_provider(
            &config,
    ...
    if let Some(metrics) = otel.as_ref().and_then(workx_otel::OtelProvider::metrics) {
        let _ = workx_otel::record_process_start_once(metrics, otel_originator.as_str());
        ...
        let telemetry =
            workx_rollout::sqlite_telemetry_recorder(metrics.clone(), otel_originator.as_str());
        let _ = workx_state::install_process_db_telemetry(telemetry);
    }
    ...
    let (tui_file_layer, _tui_file_log_guard) = if config_toml_log_dir_configured {
        ...
        let env_filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
            EnvFilter::new("workx_core=info,workx_tui=info,workx_rmcp_client=info")
        });
    ...
    let feedback = workx_feedback::WorkxFeedback::new();

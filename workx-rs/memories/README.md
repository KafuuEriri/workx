...
Runtime orchestration for Phase 1 and Phase 2 still lives in `workx-core` under
`workx-rs/core/src/memories/`.

## Crates

- `workx-rs/memories/read` (`workx-memories-read`) owns the read path:
  memory developer-instruction injection, memory citation parsing, and
  read-usage telemetry classification.
- `workx-rs/memories/write` (`workx-memories-write`) owns the write path:
...
- In `workx`, edit those undated template files in place.
...
- keeps the memories root itself as a git-baseline directory, initialized under
  `~/.workx/memories/.git` by `workx-git-utils`

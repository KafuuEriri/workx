...
use workx_git_utils::diff_since_latest_init;
use workx_git_utils::reset_git_repository;
...
use workx_state::Phase2JobClaimOutcome;
...
async fn memories_startup_phase2_tracks_workspace_diff_across_runs() -> anyhow::Result<()> {
    ...
    let test = build_test_workx(&server, home.clone()).await?;
    trigger_memories_startup(&test).await;
...
async fn phase2_retries_when_clean_workspace_is_missing_artifacts() -> anyhow::Result<()> {
    ...
    assert_eq!(
        ...
        "# Memory Workspace Diff\n\nGenerated by Workx before Phase 2 memory consolidation. Read this file first and do not edit it.\n\n## Status\n- none\n"
    );
...
-> anyhow::Result<()> {
    ...
    core_test_support::submit_thread_settings(
        ...
        workx_protocol::protocol::ThreadSettingsOverrides {
            ...
            permission_profile: Some(workx_protocol::models::PermissionProfile::workspace_write()),
            ..Default::default()
        },
    )
    .await?;
...
async fn run_memory_phase_one_model_request_test(
    ...
    let test = build_test_workx_with_memories_config(server, Arc::clone(&home), memories).await?;
...
async fn run_memory_phase_two_model_request_test(
    ...
    let test = build_test_workx_with_memories_config(server, home.clone(), memories).await?;
    let provider = Arc::new(MockMemoryModelProvider::new(
        test.config.model_provider.clone(),
        Some(test.thread_manager.auth_manager()),
    ));
    ...
    let (context, config) = memory_startup_context_with_provider(&test, provider).await;
    let root = memory_root(&config.workx_home);
...
async fn build_test_workx(
    server: &wiremock::MockServer,
    home: Arc<TempDir>,
) -> anyhow::Result<TestWorkx> {
    build_test_workx_with_memories_config(server, home, startup_test_memories_config()).await
}

async fn build_test_workx_with_memories_config(
    server: &wiremock::MockServer,
    home: Arc<TempDir>,
    memories: MemoriesConfig,
) -> anyhow::Result<TestWorkx> {
...
async fn init_state_db(home: &Arc<TempDir>) -> anyhow::Result<Arc<workx_state::StateRuntime>> {
    let db = workx_state::StateRuntime::init(
        workx_state::SqliteConfig::new_for_testing(home.path().abs()),
        "test-provider".into(),
    )
    .await?;
...
async fn trigger_memories_startup(test: &TestWorkx) {
    let config_snapshot = test.workx.config_snapshot().await;
...
) -> (Arc<MemoryStartupContext>, Arc<workx_core::config::Config>) {
    let config_snapshot = test.workx.config_snapshot().await;
    let mut config = test.config.clone();
    config
        .features
...
impl ModelProvider for MockMemoryModelProvider {
    ...
    ) -> workx_models_manager::manager::SharedModelsManager {
        self.delegate
            .models_manager(workx_home, config_model_catalog)
    }
}

async fn seed_stage1_output(
    db: &workx_state::StateRuntime,
    workx_home: &Path,
    ...
    let mut metadata_builder = workx_state::ThreadMetadataBuilder::new(
        thread_id,
        workx_home.join(format!("rollout-{thread_id}.jsonl")),
        updated_at,
        SessionSource::Cli,
    );
    metadata_builder.cwd = workx_home.join(format!("workspace-{rollout_slug}"));
...
async fn seed_stage1_candidate(
    ...
    let thread_id = ThreadId::new();
    let rollout_path = workx_home.join(format!("rollout-{thread_id}.jsonl"));
...
) -> anyhow::Result<workx_core::ThreadConfigSnapshot> {
    let deadline = Instant::now() + Duration::from_secs(10);
    loop {
        let config_snapshot = test.workx.config_snapshot().await;
...
async fn seed_stage1_output_for_existing_thread(
    ...
    let ownership_token = match claim {
        workx_state::Stage1JobClaimOutcome::Claimed { ownership_token } => ownership_token,
...
async fn shutdown_test_workx(test: &TestWorkx) -> anyhow::Result<()> {
    test.workx.submit(Op::Shutdown {}).await?;
    wait_for_event(&test.workx, |ev| matches!(ev, EventMsg::ShutdownComplete)).await;
    Ok(())
}

...
mod job {
    ...
    pub(super) async fn claim(
        ...
        let (token, watermark) = match claim {
            workx_state::Phase2JobClaimOutcome::Claimed {
                ownership_token,
                input_watermark,
            } => {
                context.counter(
            ...
            workx_state::Phase2JobClaimOutcome::SkippedRetryUnavailable => {
                return Err("skipped_retry_unavailable");
            }
            workx_state::Phase2JobClaimOutcome::SkippedCooldown => {
                return Err("skipped_cooldown");
            }
            workx_state::Phase2JobClaimOutcome::SkippedRunning => return Err("skipped_running"),
        };
    ...
    pub(super) async fn succeed(
        ...
        selected_outputs: &[workx_state::Stage1Output],
        reason: &'static str,
    ) -> bool {
        context.counter(MEMORY_PHASE_TWO_JOBS, /*inc*/ 1, &[("status", reason)]);
        db.memories()
...
mod agent {
    ...
    pub(super) fn get_config(
        ...
        // Preserve the parent's explicit choice to skip Workx-managed sandboxing.
    ...
    pub(super) fn handle(
        ...
        claim: Claim,
        new_watermark: i64,
        selected_outputs: Vec<workx_state::Stage1Output>,
        memory_root: workx_utils_absolute_path::AbsolutePathBuf,
        agent: SpawnedConsolidationAgent,
        phase_two_e2e_timer: Option<workx_otel::Timer>,
    ) {
        let Some(db) = context.state_db() else {
            return;
        };
...
pub(super) fn get_watermark(
    ...
    latest_memories: &[workx_state::Stage1Output],
) -> i64 {
    latest_memories
        .iter()

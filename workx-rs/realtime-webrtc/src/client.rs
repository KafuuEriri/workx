...
impl VoiceHost {
    pub async fn connect(package: &WorkxPackageLayout, build_commit: &str) -> Result<Self> {
        let root = package.package_dir.as_path().canonicalize()?;
        let name = if cfg!(windows) {
            "workx-voice-host.exe"
        } else {
            "workx-voice-host"
        };
        let path = root.join("workx-resources/voice/bin").join(name);
        ensure!(
        ...
        let SpawnedProcess {
            ...
            stderr_rx,
            exit_rx,
        } = workx_utils_pty::spawn_pipe_process(
            &path,
            &[],
            &root,
            &environment,

...
use workx_protocol::approvals::ElicitationRequest as CoreElicitationRequest;
use workx_protocol::approvals::GuardianAssessmentAction as CoreGuardianAssessmentAction;
use workx_protocol::config_types::MultiAgentMode;
use workx_protocol::items::AgentMessageContent;
use workx_protocol::items::AgentMessageItem;
use workx_protocol::items::CollabAgentTool as CoreCollabAgentTool;
use workx_protocol::items::CollabAgentToolCallItem;
use workx_protocol::items::CollabAgentToolCallStatus as CoreCollabAgentToolCallStatus;
use workx_protocol::items::CommandExecutionItem;
...
use workx_protocol::items::FileChangeItem;
use workx_protocol::items::ImageViewItem;
use workx_protocol::items::McpToolCallItem;
use workx_protocol::items::McpToolCallStatus as CoreMcpToolCallStatus;
use workx_protocol::items::ReasoningItem;
use workx_protocol::items::SubAgentActivityItem;
use workx_protocol::items::TurnItem;
use workx_protocol::items::UserMessageItem;
use workx_protocol::items::WebSearchItem as CoreWebSearchItem;
use workx_protocol::mcp::CallToolResult;
use workx_protocol::mcp::McpServerInfo;
use workx_protocol::memory_citation::MemoryCitation as CoreMemoryCitation;
use workx_protocol::memory_citation::MemoryCitationEntry as CoreMemoryCitationEntry;
...
use workx_protocol::permissions::FileSystemAccessMode as CoreFileSystemAccessMode;
use workx_protocol::permissions::FileSystemPath as CoreFileSystemPath;
use workx_protocol::permissions::FileSystemSandboxEntry as CoreFileSystemSandboxEntry;
use workx_protocol::permissions::FileSystemSpecialPath as CoreFileSystemSpecialPath;
use workx_protocol::protocol::AgentStatus as CoreAgentStatus;
use workx_protocol::protocol::AskForApproval as CoreAskForApproval;
use workx_protocol::protocol::ConversationTextRole;
use workx_protocol::protocol::ExecCommandSource as CoreExecCommandSource;
use workx_protocol::protocol::GranularApprovalConfig as CoreGranularApprovalConfig;
use workx_protocol::protocol::NetworkAccess as CoreNetworkAccess;
...
use workx_protocol::user_input::UserInput as CoreUserInput;
...
fn thread_sources_round_trip_as_scalar_labels() {
    for (source, label) in [
        ...
        let core_source: workx_protocol::protocol::ThreadSource = source.clone().into();
...
fn sandbox_policy_round_trips_external_sandbox_network_access() {
    ...
    assert_eq!(
        core_policy,
        workx_protocol::protocol::SandboxPolicy::ExternalSandbox {
...
fn sandbox_policy_round_trips_read_only_network_access() {
    ...
    assert_eq!(
        core_policy,
        workx_protocol::protocol::SandboxPolicy::ReadOnly {
            network_access: true,
        }
    );
...
fn config_requirements_read_accepts_foreign_path_uris() {
    let response: ConfigRequirementsReadResponse = serde_json::from_value(json!({
        "requirements": {
            "sqliteHome": "file:///C:/Users/alice/.workx/state",
            "logDir": "file:///C:/Users/alice/.workx/logs",
            "modelCatalogJson": "file:///C:/Users/alice/.workx/models.json"
        }
    }))
    ...
    assert_eq!(
        ...
        Some(PathUri::parse("file:///C:/Users/alice/.workx/state").expect("valid URI"))
    );
    assert_eq!(
        requirements.log_dir,
        Some(PathUri::parse("file:///C:/Users/alice/.workx/logs").expect("valid URI"))
    );
    assert_eq!(
        requirements.model_catalog_json,
        Some(PathUri::parse("file:///C:/Users/alice/.workx/models.json").expect("valid URI"))
    );
}
...
fn sandbox_policy_round_trips_workspace_write_access() {
    ...
    assert_eq!(
        core_policy,
        workx_protocol::protocol::SandboxPolicy::WorkspaceWrite {
            writable_roots: vec![],
            network_access: true,
            exclude_tmpdir_env_var: false,
            exclude_slash_tmp: false,
...
fn core_turn_item_into_thread_item_converts_supported_variants() {
    let user_item = TurnItem::UserMessage(UserMessageItem {
        ...
        content: vec![
            ...
            CoreUserInput::Skill {
                ...
                path: PathBuf::from("/repo/.workx/skills/skill-creator/SKILL.md"),
            },
    ...
    let command_item = TurnItem::CommandExecution(CommandExecutionItem {
        ...
        parsed_cmd: vec![workx_protocol::parse_command::ParsedCommand::Unknown {
    ...
    let dynamic_tool_call_item = TurnItem::DynamicToolCall(DynamicToolCallItem {
        ...
        content_items: Some(vec![
            workx_protocol::dynamic_tools::DynamicToolCallOutputContentItem::InputText {
                text: "ok".to_string(),
            },
    ...
    let sender_thread_id = workx_protocol::ThreadId::default();
    let receiver_thread_id = workx_protocol::ThreadId::default();
    ...
    let sub_agent_activity_item = TurnItem::SubAgentActivity(SubAgentActivityItem {
        ...
        agent_path: workx_protocol::AgentPath::root()
            .join("worker")
            .expect("worker path"),
    });
    ...
    assert_eq!(
        ThreadItem::from(TurnItem::Extension(
            workx_extension_items::ExtensionItem::WebSearch(expected_search_item.clone()),
        )),
    ...
    let file_change_item = TurnItem::FileChange(FileChangeItem {
        ...
        .into_iter()
        .collect(),
        status: Some(workx_protocol::protocol::PatchApplyStatus::Completed),
        auto_approved: None,
...
fn plugin_share_params_and_response_serialization_use_camel_case_fields() {
    ...
    assert_eq!(
        serde_json::to_value(PluginShareCheckoutResponse {
            remote_plugin_id: "plugins~Plugin_00000000000000000000000000000000".to_string(),
            plugin_id: "gmail@workx-curated".to_string(),
            plugin_name: "gmail".to_string(),
            plugin_path,
            marketplace_name: "workx-curated".to_string(),
            marketplace_path,
        ...
        json!({
            "remotePluginId": "plugins~Plugin_00000000000000000000000000000000",
            "pluginId": "gmail@workx-curated",
            "pluginName": "gmail",
            "pluginPath": plugin_path_json,
            "marketplaceName": "workx-curated",
            "marketplacePath": marketplace_path_json,
            "remoteVersion": "1.2.3",
        }),
    );
...
#[test]
fn workx_error_info_serializes_http_status_code_in_camel_case() {
    let value = WorkxErrorInfo::ResponseTooManyFailedAttempts {
...
fn core_error_info_converts_to_camel_case() {
    for (core, expected) in [
        (CoreWorkxErrorInfo::CyberPolicy, json!("cyberPolicy")),
        (
            CoreWorkxErrorInfo::RateLimitExceeded,
            json!("rateLimitExceeded"),
        ),
    ] {
        assert_eq!(
            serde_json::to_value(WorkxErrorInfo::from(core)).unwrap(),
            expected
        );
    }
}
...
#[test]
fn workx_error_info_serializes_active_turn_not_steerable_turn_kind_in_camel_case() {
    let value = WorkxErrorInfo::ActiveTurnNotSteerable {
...
fn turn_start_cyber_access_program_uses_separate_wire_formats() {
    for (app_server_value, core_value) in [
        ...
        .expect("params should deserialize");
        let core_program: workx_protocol::turn_input::CyberAccessProgram = params
...
fn turn_start_params_round_trip_multi_agent_mode() {
    ...
    assert_eq!(
        ...
        Some(workx_protocol::config_types::MultiAgentMode::Proactive)
    );
    assert_eq!(
...
fn thread_settings_update_params_preserve_field_level_experimental_gates() {
    ...
    let collaboration_mode = ThreadSettingsUpdateParams {
        thread_id: "thread_123".to_string(),
        collaboration_mode: Some(workx_protocol::config_types::CollaborationMode {
            mode: workx_protocol::config_types::ModeKind::Plan,
            settings: workx_protocol::config_types::Settings {
...
fn realtime_start_deserializes_client_handoff_channel_prefixes() {
    let params = serde_json::from_value::<ThreadRealtimeStartParams>(json!({
        "threadId": "thread_123",
        "outputModality": "audio",
        "workxResponseHandoffChannelPrefixes": {
            "analysis": ["[THINKING]"],
    ...
    assert_eq!(
        params.workx_response_handoff_channel_prefixes,
        Some(BTreeMap::from([

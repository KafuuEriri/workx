...
use workx_protocol::protocol::CreditsSnapshot as CoreCreditsSnapshot;
use workx_protocol::protocol::RateLimitReachedType as CoreRateLimitReachedType;
use workx_protocol::protocol::RateLimitSnapshot as CoreRateLimitSnapshot;
use workx_protocol::protocol::RateLimitWindow as CoreRateLimitWindow;
use workx_protocol::protocol::SpendControlLimitSnapshot as CoreSpendControlLimitSnapshot;
...
pub enum Account {
    ...
    AmazonBedrock {
        #[serde(default)]
        uses_workx_managed_credentials: bool,
    },
}
    ...
    fn from(account: ProviderAccount) -> Self {
        match account {
            ...
            ProviderAccount::AmazonBedrock {
                uses_workx_managed_credentials,
            } => Self::AmazonBedrock {
                uses_workx_managed_credentials,
            },
        }
...
pub enum LoginAccountParams {
    ...
    /// The access token must contain the same scopes that Workx-managed ChatGPT auth tokens have.
    ...
    ChatgptAuthTokens {
        ...
        ///
        /// When `null`, Workx attempts to derive the plan type from access-token
...
pub enum ChatgptAuthTokensRefreshReason {
    /// Workx attempted a backend request and received `401 Unauthorized`.
    Unauthorized,
}
...
pub struct ChatgptAuthTokensRefreshParams {
    ...
    /// Workspace/account identifier that Workx was previously using.
    ///
...
pub struct GetAccountRateLimitsResponse {
    ...
    /// Multi-bucket view keyed by metered `limit_id` (for example, `workx`).

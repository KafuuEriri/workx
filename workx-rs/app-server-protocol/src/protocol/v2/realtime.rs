...
use workx_protocol::protocol::RealtimeAudioFrame as CoreRealtimeAudioFrame;
...
use workx_protocol::realtime::BemItemPresentation as CoreBemItemPresentation;
use workx_protocol::realtime::RealtimeItem as CoreRealtimeItem;
use workx_protocol::realtime::RealtimeItemContent as CoreRealtimeItemContent;
use workx_protocol::realtime::RealtimeSessionOutcome as CoreRealtimeSessionOutcome;
use workx_protocol::realtime::RealtimeTranscriptRole as CoreRealtimeTranscriptRole;
...
pub struct ThreadRealtimeStartParams {
    ...
    /// Leaves Workx response handoffs to the client's explicit append calls instead of forwarding
    ...
    /// Routes any transcript tail remaining at session end through Workx. Defaults to false.
    ...
    #[ts(optional = nullable)]
    pub workx_responses_as_items: Option<bool>,
    // TODO: Remove this experiment-only prefix with `workx_responses_as_items`.
    /// Optional prefix added to automatic Workx response items when `workxResponsesAsItems` is true.
    #[ts(optional = nullable)]
    pub workx_response_item_prefix: Option<String>,
    /// Selects how automatic Workx responses are routed in Frameless Bidi sessions. Omitted values
    /// default to `thinking`. Realtime V1 and V2 ignore this setting.
    #[ts(optional = nullable)]
    pub workx_response_handoff_mode: Option<WorkxResponseHandoffMode>,
    ...
    /// Set to false to start without Workx's startup context. Omitted or null includes it.
    #[ts(optional = nullable)]
    ...
    /// Developer instructions given to the backing Workx model when this realtime session starts.
    #[ts(optional = nullable)]
    pub realtime_start_instructions: Option<String>,
    /// Developer instructions given to the backing Workx model when this realtime session ends.
    #[ts(optional = nullable)]

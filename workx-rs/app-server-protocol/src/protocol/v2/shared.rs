...
use workx_protocol::config_types::ApprovalsReviewer as CoreApprovalsReviewer;
use workx_protocol::config_types::SandboxMode as CoreSandboxMode;
...
use workx_protocol::protocol::NonSteerableTurnKind as CoreNonSteerableTurnKind;
...
/// This translation layer make sure that we expose workx error code in camel case.
///
/// When an upstream HTTP status is available (for example, from the Responses API or a provider),
/// it is forwarded in `httpStatusCode` on the relevant `workxErrorInfo` variant.
...
impl From<CoreWorkxErrorInfo> for WorkxErrorInfo {
    fn from(value: CoreWorkxErrorInfo) -> Self {
        match value {
            CoreWorkxErrorInfo::ContextWindowExceeded => WorkxErrorInfo::ContextWindowExceeded,
            CoreWorkxErrorInfo::SessionBudgetExceeded => WorkxErrorInfo::SessionBudgetExceeded,
            CoreWorkxErrorInfo::UsageLimitExceeded => WorkxErrorInfo::UsageLimitExceeded,
            CoreWorkxErrorInfo::RateLimitExceeded => WorkxErrorInfo::RateLimitExceeded,
            CoreWorkxErrorInfo::ServerOverloaded => WorkxErrorInfo::ServerOverloaded,
            CoreWorkxErrorInfo::CyberPolicy => WorkxErrorInfo::CyberPolicy,
            CoreWorkxErrorInfo::MisalignmentPolicyViolation => {
                WorkxErrorInfo::MisalignmentPolicyViolation
            }
            CoreWorkxErrorInfo::HttpConnectionFailed { http_status_code } => {
                WorkxErrorInfo::HttpConnectionFailed { http_status_code }
            }
            CoreWorkxErrorInfo::ResponseStreamConnectionFailed { http_status_code } => {
                WorkxErrorInfo::ResponseStreamConnectionFailed { http_status_code }
            }
            CoreWorkxErrorInfo::InternalServerError => WorkxErrorInfo::InternalServerError,
            CoreWorkxErrorInfo::Unauthorized => WorkxErrorInfo::Unauthorized,
            CoreWorkxErrorInfo::BadRequest => WorkxErrorInfo::BadRequest,
            CoreWorkxErrorInfo::ThreadRollbackFailed => WorkxErrorInfo::ThreadRollbackFailed,
            CoreWorkxErrorInfo::SandboxError => WorkxErrorInfo::SandboxError,
            CoreWorkxErrorInfo::ResponseStreamDisconnected { http_status_code } => {
                WorkxErrorInfo::ResponseStreamDisconnected { http_status_code }
            }
            CoreWorkxErrorInfo::ResponseTooManyFailedAttempts { http_status_code } => {
                WorkxErrorInfo::ResponseTooManyFailedAttempts { http_status_code }
            }
            CoreWorkxErrorInfo::ActiveTurnNotSteerable { turn_kind } => {
                WorkxErrorInfo::ActiveTurnNotSteerable {
                    turn_kind: turn_kind.into(),
                }
            }
            CoreWorkxErrorInfo::Other => WorkxErrorInfo::Other,
        }
    }
}

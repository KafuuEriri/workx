...
use tracing::warn;
use uuid::Uuid;
use workx_extension_items::image_generation::ImageGenerationItem;
use workx_protocol::items::parse_hook_prompt_message;
use workx_protocol::protocol::AgentMessageEvent;
use workx_protocol::protocol::AgentReasoningEvent;
use workx_protocol::protocol::AgentReasoningRawContentEvent;
use workx_protocol::protocol::AgentStatus;
use workx_protocol::protocol::ApplyPatchApprovalRequestEvent;
use workx_protocol::protocol::ContextCompactedEvent;
use workx_protocol::protocol::DynamicToolCallResponseEvent;
use workx_protocol::protocol::ErrorEvent;
use workx_protocol::protocol::EventMsg;
use workx_protocol::protocol::ExecCommandBeginEvent;
use workx_protocol::protocol::ExecCommandEndEvent;
use workx_protocol::protocol::GuardianAssessmentEvent;
use workx_protocol::protocol::GuardianAssessmentStatus;
use workx_protocol::protocol::ImageGenerationBeginEvent;
use workx_protocol::protocol::ImageGenerationEndEvent;
use workx_protocol::protocol::ItemCompletedEvent;
use workx_protocol::protocol::ItemStartedEvent;
use workx_protocol::protocol::McpToolCallBeginEvent;
use workx_protocol::protocol::McpToolCallEndEvent;
use workx_protocol::protocol::PatchApplyBeginEvent;
use workx_protocol::protocol::PatchApplyEndEvent;
...
use workx_protocol::protocol::ViewImageToolCallEvent;
use workx_protocol::protocol::WebSearchBeginEvent;
use workx_protocol::protocol::WebSearchEndEvent;
#[cfg(test)]
use workx_protocol::review_format::REVIEW_FALLBACK_MESSAGE;
use workx_rollout::CompactedItem;
...
#[cfg(test)]
use workx_protocol::protocol::ExecCommandStatus as CoreExecCommandStatus;
#[cfg(test)]
use workx_protocol::protocol::PatchApplyStatus as CorePatchApplyStatus;
...
impl ThreadHistoryBuilder {
    ...
    /// See `should_persist_event_msg` in `workx-rs/core/rollout/policy.rs`.
    ...
    fn handle_response_item(&mut self, item: &workx_protocol::models::ResponseItem) {
        let workx_protocol::models::ResponseItem::Message {
            role, content, id, ..
        } = item
        else {
            return;
    ...
    fn handle_materialized_item_lifecycle(
        &mut self,
        turn_id: &str,
        item: &workx_protocol::items::TurnItem,
    ) {
        let is_review_mode_item = matches!(
            item,
            workx_protocol::items::TurnItem::EnteredReviewMode(_)
                | workx_protocol::items::TurnItem::ExitedReviewMode(_)
        );
        let should_upsert = match item {
            workx_protocol::items::TurnItem::Plan(plan) => !plan.text.is_empty(),
            workx_protocol::items::TurnItem::HookPrompt(_)
            | workx_protocol::items::TurnItem::FunctionCallOutput(_)
            | workx_protocol::items::TurnItem::CommandExecution(_)
            | workx_protocol::items::TurnItem::DynamicToolCall(_)
            | workx_protocol::items::TurnItem::CollabAgentToolCall(_)
            | workx_protocol::items::TurnItem::SubAgentActivity(_)
            | workx_protocol::items::TurnItem::Extension(_)
            | workx_protocol::items::TurnItem::EnteredReviewMode(_)
            | workx_protocol::items::TurnItem::ExitedReviewMode(_) => true,
            workx_protocol::items::TurnItem::UserMessage(_)
            | workx_protocol::items::TurnItem::AgentMessage(_)
            | workx_protocol::items::TurnItem::Reasoning(_)
            | workx_protocol::items::TurnItem::WebSearch(_)
            | workx_protocol::items::TurnItem::ImageView(_)
            | workx_protocol::items::TurnItem::ImageGeneration(_)
            | workx_protocol::items::TurnItem::FileChange(_)
            | workx_protocol::items::TurnItem::McpToolCall(_)
            | workx_protocol::items::TurnItem::ContextCompaction(_) => false,
        };
    ...
    fn handle_dynamic_tool_call_request(
        &mut self,
        payload: &workx_protocol::dynamic_tools::DynamicToolCallRequest,
    ) {
    ...
    fn handle_collab_agent_spawn_begin(
        &mut self,
        payload: &workx_protocol::protocol::CollabAgentSpawnBeginEvent,
    ) {
    ...
    fn handle_collab_agent_spawn_end(
        &mut self,
        payload: &workx_protocol::protocol::CollabAgentSpawnEndEvent,
    ) {
    ...
    fn handle_collab_agent_interaction_begin(
        &mut self,
        payload: &workx_protocol::protocol::CollabAgentInteractionBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_agent_interaction_end(
        &mut self,
        payload: &workx_protocol::protocol::CollabAgentInteractionEndEvent,
    ) {
        let status = match &payload.status {
            AgentStatus::Errored(_) | AgentStatus::NotFound => CollabAgentToolCallStatus::Failed,
    ...
    fn handle_sub_agent_activity(
        &mut self,
        payload: &workx_protocol::protocol::SubAgentActivityEvent,
    ) {
    ...
    fn handle_collab_waiting_begin(
        &mut self,
        payload: &workx_protocol::protocol::CollabWaitingBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_waiting_end(
        &mut self,
        payload: &workx_protocol::protocol::CollabWaitingEndEvent,
    ) {
    ...
    fn handle_collab_close_begin(
        &mut self,
        payload: &workx_protocol::protocol::CollabCloseBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_close_end(&mut self, payload: &workx_protocol::protocol::CollabCloseEndEvent) {
        let status = match &payload.status {
            AgentStatus::Errored(_) | AgentStatus::NotFound => CollabAgentToolCallStatus::Failed,
            _ => CollabAgentToolCallStatus::Completed,
        };
    ...
    fn handle_collab_resume_begin(
        &mut self,
        payload: &workx_protocol::protocol::CollabResumeBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_resume_end(
        &mut self,
        payload: &workx_protocol::protocol::CollabResumeEndEvent,
    ) {
        let status = match &payload.status {
            AgentStatus::Errored(_) | AgentStatus::NotFound => CollabAgentToolCallStatus::Failed,
            _ => CollabAgentToolCallStatus::Completed,
    ...
    fn handle_entered_review_mode(
        &mut self,
        payload: &workx_protocol::protocol::EnteredReviewModeEvent,
    ) {
        let review = payload
            .user_facing_hint
            .clone()
    ...
    fn handle_exited_review_mode(
        &mut self,
        payload: &workx_protocol::protocol::ExitedReviewModeEvent,
    ) {
    ...
    fn handle_error(&mut self, payload: &ErrorEvent) {
        ...
        let changed_turn = if let Some(turn) = self.current_turn.as_mut() {
            ...
            turn.error = Some(V2TurnError {
                ...
                workx_error_info: payload.workx_error_info.clone().map(Into::into),
    ...
    fn handle_turn_complete(&mut self, payload: &TurnCompleteEvent) {
        let terminal_error = payload.error.as_ref().map(|error| V2TurnError {
            ...
            workx_error_info: error.workx_error_info.clone().map(Into::into),
            additional_details: None,
        });
...
fn convert_dynamic_tool_content_items(
    items: &[workx_protocol::dynamic_tools::DynamicToolCallOutputContentItem],
) -> Vec<DynamicToolCallOutputContentItem> {
    items
        .iter()
        .cloned()
        .map(|item| match item {
            workx_protocol::dynamic_tools::DynamicToolCallOutputContentItem::InputText { text } => {
                DynamicToolCallOutputContentItem::InputText { text }
            }
            workx_protocol::dynamic_tools::DynamicToolCallOutputContentItem::InputImage {
                image_url,
            } => DynamicToolCallOutputContentItem::InputImage { image_url },
            workx_protocol::dynamic_tools::DynamicToolCallOutputContentItem::InputAudio {
                audio_url,
...
mod tests {
    ...
    use uuid::Uuid;
    use workx_extension_items::ExtensionItem as CoreExtensionItem;
    use workx_extension_items::sleep::SleepItem as CoreSleepItem;
    use workx_protocol::ThreadId;
    use workx_protocol::dynamic_tools::DynamicToolCallOutputContentItem as CoreDynamicToolCallOutputContentItem;
    use workx_protocol::items::CommandExecutionItem as CoreCommandExecutionItem;
    use workx_protocol::items::CommandExecutionStatus as CoreCommandExecutionStatus;
    use workx_protocol::items::EnteredReviewModeItem as CoreEnteredReviewModeItem;
    use workx_protocol::items::ExitedReviewModeItem as CoreExitedReviewModeItem;
    use workx_protocol::items::HookPromptFragment as CoreHookPromptFragment;
    use workx_protocol::items::SubAgentActivityItem as CoreSubAgentActivityItem;
    use workx_protocol::items::TurnItem as CoreTurnItem;
    use workx_protocol::items::UserMessageItem as CoreUserMessageItem;
    use workx_protocol::items::build_hook_prompt_message;
    use workx_protocol::mcp::CallToolResult;
    use workx_protocol::models::ImageDetail;
    use workx_protocol::models::MessagePhase as CoreMessagePhase;
    use workx_protocol::models::WebSearchAction as CoreWebSearchAction;
    use workx_protocol::parse_command::ParsedCommand;
    use workx_protocol::protocol::AgentReasoningEvent;
    use workx_protocol::protocol::AgentReasoningRawContentEvent;
    use workx_protocol::protocol::ApplyPatchApprovalRequestEvent;
    use workx_protocol::protocol::DynamicToolCallResponseEvent;
    use workx_protocol::protocol::EnteredReviewModeEvent;
    use workx_protocol::protocol::ExecCommandBeginEvent;
    use workx_protocol::protocol::ExecCommandEndEvent;
    use workx_protocol::protocol::ExecCommandSource;
    use workx_protocol::protocol::ExitedReviewModeEvent;
    use workx_protocol::protocol::ItemStartedEvent;
    use workx_protocol::protocol::McpInvocation;
    use workx_protocol::protocol::McpToolCallEndEvent;
    use workx_protocol::protocol::PatchApplyBeginEvent;
    use workx_protocol::protocol::ReviewTarget;
    use workx_protocol::protocol::SubAgentActivityKind as CoreSubAgentActivityKind;
    use workx_protocol::protocol::ThreadRolledBackEvent;
        ...
        let events = vec![
            ...
            EventMsg::ItemStarted(ItemStartedEvent {
                ...
                item: CoreTurnItem::UserMessage(CoreUserMessageItem {
                    ...
                    content: vec![workx_protocol::user_input::UserInput::Text {
                        text: "hello".into(),
                        text_elements: Vec::new(),
                    }],
                }),
    ...
    fn replays_image_generation_end_events_into_turn_history() {
        let items = vec![
            ...
            RolloutItem::EventMsg(EventMsg::ImageGenerationEnd(ImageGenerationEndEvent {
                ...
                transparent_background: Some(true),
                failure: Some(
                    workx_extension_items::image_generation::ImageGenerationFailure::UsageLimitExceeded {
        ...
        let events = vec![
            ...
            EventMsg::WebSearchEnd(WebSearchEndEvent {
                ...
                action: CoreWebSearchAction::Search {
                    query: Some("workx".into()),
                    queries: None,
                },
                results: Some(vec![serde_json::json!({
                    "type": "text_result",
                    "ref_id": "turn0search0",
                    "url": "https://example.com/workx",
                })]),
            }),
            EventMsg::ExecCommandEnd(ExecCommandEndEvent {
                call_id: "exec-1".into(),
    ...
    fn reconstructs_dynamic_tool_items_from_request_and_response_events() {
        let events = vec![
            ...
            EventMsg::DynamicToolCallRequest(
                workx_protocol::dynamic_tools::DynamicToolCallRequest {
                    call_id: "dyn-1".into(),
                    turn_id: "turn-1".into(),
                    started_at_ms: 0,
                    namespace: Some("workx_app".into()),
        ...
        let events = vec![
            ...
            EventMsg::PatchApplyEnd(PatchApplyEndEvent {
                ...
                changes: [(
                    ...
                    workx_protocol::protocol::FileChange::Add {
                        content: "hello\n".into(),
                    },
                )]
                .into_iter()
        ...
        let events = vec![
            ...
            EventMsg::TurnComplete(TurnCompleteEvent {
                ...
                error: Some(ErrorEvent {
                    ...
                    workx_error_info: Some(WorkxErrorInfo::ServerOverloaded),
                }),
                ...
                Turn {
                    ...
                    error: Some(TurnError {
                        ...
                        message: "Selected model is at capacity. Please try a different model."
                            .into(),
                        workx_error_info: Some(
                            crate::protocol::v2::WorkxErrorInfo::ServerOverloaded,
                        ),
                        additional_details: None,
                    }),
                    started_at: Some(10),
    ...
    fn reconstructs_collab_resume_end_item() {
        let events = vec![
            ...
            EventMsg::CollabResumeEnd(workx_protocol::protocol::CollabResumeEndEvent {
    ...
    fn reconstructs_collab_spawn_end_item_with_model_metadata() {
        ...
        let events = vec![
            ...
            EventMsg::CollabAgentSpawnEnd(workx_protocol::protocol::CollabAgentSpawnEndEvent {
                ...
                reasoning_effort: workx_protocol::openai_models::ReasoningEffort::Medium,
        ...
        assert_eq!(
            ...
            ThreadItem::CollabAgentToolCall {
                ...
                reasoning_effort: Some(workx_protocol::openai_models::ReasoningEffort::Medium),
                agents_states: [(
                    "00000000-0000-0000-0000-000000000002".into(),
                    CollabAgentState {
    ...
    fn reconstructs_interrupted_send_input_as_completed_collab_call() {
        ...
        let events = vec![
            ...
            EventMsg::CollabAgentInteractionBegin(
                workx_protocol::protocol::CollabAgentInteractionBeginEvent {
                    call_id: "send-1".into(),
                    started_at_ms: 0,
            ...
            EventMsg::CollabAgentInteractionEnd(
                workx_protocol::protocol::CollabAgentInteractionEndEvent {
                    call_id: "send-1".into(),
                    completed_at_ms: 0,
                    sender_thread_id: sender,
                    receiver_thread_id: receiver,
    ...
    fn rollback_failed_error_does_not_mark_turn_failed() {
        let events = vec![
            ...
            EventMsg::Error(ErrorEvent {
                ...
                workx_error_info: Some(WorkxErrorInfo::ThreadRollbackFailed),
            }),
        ];
    ...
    fn out_of_turn_error_does_not_create_or_fail_a_turn() {
        let events = vec![
            ...
            EventMsg::Error(ErrorEvent {
                ...
                workx_error_info: Some(WorkxErrorInfo::BadRequest),
            }),
        ];
    ...
    fn error_then_turn_complete_preserves_failed_status() {
        let events = vec![
            ...
            EventMsg::Error(ErrorEvent {
                ...
                workx_error_info: Some(WorkxErrorInfo::ResponseStreamDisconnected {
        ...
        assert_eq!(
            ...
            Some(TurnError {
                ...
                message: "stream failure".into(),
                workx_error_info: Some(
                    crate::protocol::v2::WorkxErrorInfo::ResponseStreamDisconnected {
                        http_status_code: Some(502),
                    }
                ),
                additional_details: None,
        ...
        assert_eq!(
            ...
            vec![Turn {
                ...
                error: Some(TurnError {
                    misalignment: None,
                    message: "Selected model is at capacity. Please try a different model.".into(),
                    workx_error_info: Some(crate::protocol::v2::WorkxErrorInfo::ServerOverloaded),
                    additional_details: None,
                }),
                started_at: Some(10),
                completed_at: Some(20),
    ...
    fn canonical_hook_prompt_completion_updates_turn_history() {
        let hook_prompt = CoreTurnItem::HookPrompt(workx_protocol::items::HookPromptItem {
    ...
    fn completed_sub_agent_activity_updates_completed_parent_turn() {
        ...
        let child_path = workx_protocol::AgentPath::root()
            .join("worker")
            .expect("worker path");
        let items = vec![
            RolloutItem::EventMsg(EventMsg::TurnStarted(TurnStartedEvent {
    ...
    fn ignores_plain_user_response_items_in_rollout_replay() {
        let items = vec![
            ...
            RolloutItem::ResponseItem(
                workx_protocol::models::ResponseItem::Message {
                    id: Some(workx_protocol::ResponseItemId::with_suffix("msg", "1")),
                    role: "user".into(),
                    content: vec![workx_protocol::models::ContentItem::InputText {

...
use workx_protocol::auth::PlanType as AuthPlanType;
...
const AGENT_IDENTITY_JWT_AUDIENCE: &str = "workx-app-server";
...
const AGENT_IDENTITY_KEY_DERIVATION_CONTEXT: &[u8] = b"workx-agent-identity-ed25519-v1";
...
/// single Workx run, while the agent runtime id and private key are the
...
pub fn build_abom(session_source: SessionSource) -> AgentBillOfMaterials {
    AgentBillOfMaterials {
        ...
        agent_harness_id: match &session_source {
            SessionSource::VSCode => "workx-app".to_string(),
            SessionSource::Cli
            | SessionSource::Exec
            | SessionSource::Mcp
            ...
            | SessionSource::Unknown => "workx-cli".to_string(),
        },
...
mod tests {
    ...
    use workx_protocol::auth::KnownPlan;

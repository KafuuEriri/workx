...
use workx_login::LoginCallbackResult;
...
async fn end_to_end_login_flow_persists_auth_json() -> Result<()> {
    ...
    let server_home = workx_home.clone();

    let opts = ServerOptions {
        workx_home: server_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        auth_route_config: workx_login::test_support::transport_default_auth_route_config(),
        client_id: workx_login::CLIENT_ID.to_string(),
        issuer,
        port: 0,
        open_browser: false,
    ...
    // Validate auth.json
    let auth_path = workx_home.join("auth.json");
...
async fn hosted_login_redirects_to_configured_open_app_url() -> Result<()> {
    ...
    let server = run_login_server(ServerOptions {
        ...
        login_success_page: LoginSuccessPage::Hosted {
            url: Url::parse("http://localhost:3000/workx/open-app?source=old")?,
...
#[tokio::test]
async fn creates_missing_workx_home_dir() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let tmp = tempdir()?;
    let workx_home = tmp.path().join("missing-subdir"); // does not exist
...
async fn oauth_access_denied_missing_entitlement_blocks_login_with_clear_error() -> Result<()> {
    ...
    let body = resp.text().await?;
    assert!(
        body.contains("You do not have access to Workx"),
        "error body should clearly explain the Workx access denial"
    );
    assert!(
...
async fn oauth_access_denied_unknown_reason_uses_generic_error_page() -> Result<()> {
    ...
    assert!(
        body.contains("Return to Workx to retry"),
    ...
    assert!(
        !body.contains("get access to Workx"),
...
async fn falls_back_to_registered_fallback_port_when_default_port_is_in_use() -> Result<()> {
    ...
    let default_port_server_handle = {
        ...
        thread::spawn(move || {
            while let Ok(req) = server.recv() {
                let _ = req.respond(tiny_http::Response::from_string("not workx"));
            }
        })
    };
    ...
    let mut opts = ServerOptions::new(
        ...
        workx_login::CLIENT_ID.to_string(),
        /*forced_chatgpt_workspace_id*/ None,
...
async fn cancels_previous_login_server_when_port_is_in_use() -> Result<()> {
    ...
    let first_workx_home = first_tmp.path().to_path_buf();

    let first_opts = ServerOptions {
        workx_home: first_workx_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        auth_route_config: workx_login::test_support::transport_default_auth_route_config(),
        client_id: workx_login::CLIENT_ID.to_string(),
        issuer: issuer.clone(),
    ...
    let second_workx_home = second_tmp.path().to_path_buf();

    let second_opts = ServerOptions {
        workx_home: second_workx_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        auth_route_config: workx_login::test_support::transport_default_auth_route_config(),
        client_id: workx_login::CLIENT_ID.to_string(),
        issuer,

...
use workx_utils_template::Template;

pub(super) const DEFAULT_ISSUER: &str = "https://auth.openai.com";
const DEFAULT_PORT: u16 = 1455;
// Keep in sync with the Workx CLI Hydra redirect URI allow-list.
                ...
                Ok(tokens) => {
                    ...
                    match tiny_http::Header::from_bytes(&b"Location"[..], url.as_bytes()) {
                        ...
                        Err(_) => login_error_response(
                            "Sign-in completed but redirecting back to Workx failed.",
                            io::ErrorKind::Other,
...
fn redact_sensitive_error_url(
    mut err: workx_http_client::HttpError,
) -> workx_http_client::HttpError {
...
/// Returns true when the OAuth callback represents a missing Workx entitlement.
fn is_missing_workx_entitlement_error(error_code: &str, error_description: Option<&str>) -> bool {
...
fn oauth_callback_error_message(error_code: &str, error_description: Option<&str>) -> String {
    if is_missing_workx_entitlement_error(error_code, error_description) {
        return "Workx is not enabled for your workspace. Contact your workspace administrator to request access to Workx.".to_string();
    }
...
fn render_login_error_page(
    ...
    let (title, display_message, display_description, help_text) =
        if is_missing_workx_entitlement_error(code, error_description) {
            (
                "You do not have access to Workx".to_string(),
                "This account is not currently authorized to use Workx in this workspace."
                    .to_string(),
                "Contact your workspace administrator to request access to Workx.".to_string(),
                "Contact your workspace administrator to get access to Workx, then return to Workx and try again."
                    .to_string(),
            )
        } else {
            (
                ...
                "Return to Workx to retry, switch accounts, or contact your workspace admin if access is restricted."
                    .to_string(),
            )
        };
...
mod tests {
    ...
    use super::is_missing_workx_entitlement_error;
    ...
    fn render_login_error_page_uses_entitlement_copy() {
        ...
        assert!(is_missing_workx_entitlement_error(
            "access_denied",
            error_description
        ));
        ...
        assert!(body.contains("You do not have access to Workx"));

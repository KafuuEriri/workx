...
fn bedrock_api_key_debug_redacts_secret() {
    ...
    assert_eq!(
        format!("{:?}", WorkxAuth::BedrockApiKey(auth)),
...
async fn login_with_bedrock_api_key_replaces_openai_auth() -> anyhow::Result<()> {
    ...
    assert_eq!(
        auth_manager.auth_cached().and_then(|auth| match auth {
            WorkxAuth::BedrockApiKey(auth) => Some(auth),
            WorkxAuth::ApiKey(_)
            | WorkxAuth::Chatgpt(_)
            ...
            | WorkxAuth::BedrockAccessKeys(_) => None,
        }),
        Some(bedrock_auth())
    );
    Ok(())
...
async fn access_keys_auth_round_trips_and_logs_out() -> anyhow::Result<()> {
    ...
    assert_eq!(
        auth_manager.auth_cached().and_then(|auth| match auth {
            WorkxAuth::BedrockAccessKeys(auth) => Some(auth),
            WorkxAuth::ApiKey(_)
            | WorkxAuth::Chatgpt(_)
            | WorkxAuth::ChatgptAuthTokens(_)
            | WorkxAuth::Headers(_)
            | WorkxAuth::AgentIdentity(_)
            | WorkxAuth::PersonalAccessToken(_)
            | WorkxAuth::BedrockApiKey(_) => None,
        }),

...
use workx_agent_identity::AgentIdentityJwtClaims;
use workx_agent_identity::decode_agent_identity_jwt;
use workx_config::types::AuthCredentialsStoreMode;
pub use workx_config::types::AuthKeyringBackendKind;
use workx_keyring_store::DefaultKeyringStore;
use workx_keyring_store::KeyringStore;
use workx_protocol::account::PlanType as AccountPlanType;
use workx_protocol::auth::AuthMode;
use workx_secrets::LocalSecretsNamespace;
use workx_secrets::SecretName;
use workx_secrets::SecretScope;
use workx_secrets::SecretsBackendKind;
use workx_secrets::SecretsManager;

/// Expected structure for $WORKX_HOME/auth.json.
...
pub(super) fn get_auth_file(workx_home: &Path) -> PathBuf {
    workx_home.join("auth.json")
}

pub(super) fn delete_file_if_exists(workx_home: &Path) -> std::io::Result<bool> {
...
impl FileAuthStorage {
    pub(super) fn new(workx_home: PathBuf) -> Self {
        Self { workx_home }
    }

    /// Attempt to read and parse the `auth.json` file in the given `WORKX_HOME` directory.
...
impl AuthStorageBackend for FileAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        let auth_file = get_auth_file(&self.workx_home);
    ...
    fn delete(&self) -> std::io::Result<bool> {
        delete_file_if_exists(&self.workx_home)
    }
}

static WORKX_AUTH_SECRET_NAME: Lazy<SecretName> =
    Lazy::new(|| match SecretName::new("WORKX_AUTH") {
        Ok(name) => name,
        Err(err) => unreachable!("WORKX_AUTH should be a valid secret name: {err}"),
    });
const KEYRING_SERVICE: &str = "Workx Auth";

// turns workx_home path into a stable, short key string
fn compute_store_key(workx_home: &Path) -> std::io::Result<String> {
    let canonical = workx_home
        .canonicalize()
        .unwrap_or_else(|_| workx_home.to_path_buf());
...
impl DirectKeyringAuthStorage {
    fn new(workx_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
        Self {
            workx_home,
            keyring_store,
        }
...
impl AuthStorageBackend for DirectKeyringAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        let key = compute_store_key(&self.workx_home)?;
    ...
    fn save(&self, auth: &AuthDotJson) -> std::io::Result<()> {
        ...
        if let Err(err) = delete_file_if_exists(&self.workx_home) {
    ...
    fn delete(&self) -> std::io::Result<bool> {
        ...
        let file_removed = delete_file_if_exists(&self.workx_home)?;
...
impl SecretsKeyringAuthStorage {
    fn new(workx_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
        let direct_storage =
            DirectKeyringAuthStorage::new(workx_home.clone(), Arc::clone(&keyring_store));
        let secrets_manager = SecretsManager::new_with_keyring_store_and_namespace(
            workx_home.clone(),
            SecretsBackendKind::Local,
            keyring_store,
            LocalSecretsNamespace::WorkxAuth,
        );
        Self {
            workx_home,
            direct_storage,
...
impl AuthStorageBackend for SecretsKeyringAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        match self
            .secrets_manager
            .get(&SecretScope::Global, &WORKX_AUTH_SECRET_NAME)
            .map_err(|err| {
                std::io::Error::other(format!(
    ...
    fn save(&self, auth: &AuthDotJson) -> std::io::Result<()> {
        ...
        self.secrets_manager
            .set(&SecretScope::Global, &WORKX_AUTH_SECRET_NAME, &serialized)
            .map_err(|err| {
                let message =
    ...
    fn delete(&self) -> std::io::Result<bool> {
        let keyring_removed = self
            .secrets_manager
            .delete(&SecretScope::Global, &WORKX_AUTH_SECRET_NAME)
            .map_err(|err| {
                std::io::Error::other(format!(
...
impl AutoAuthStorage {
    fn new(
        ...
        Self {
            ...
            file_storage: Arc::new(FileAuthStorage::new(workx_home)),
        }
    }
}
...
// A global in-memory store for mapping workx_home -> AuthDotJson.
...
impl EphemeralAuthStorage {
    fn new(workx_home: PathBuf) -> Self {
        Self { workx_home }
    }
...
pub(super) fn create_auth_storage(
    ...
    create_auth_storage_with_store(workx_home, mode, keyring_store, keyring_backend_kind)
}
...
fn create_auth_storage_with_store(
    ...
    match mode {
        AuthCredentialsStoreMode::File => Arc::new(FileAuthStorage::new(workx_home)),
        AuthCredentialsStoreMode::Keyring => {
            create_keyring_auth_storage(workx_home, keyring_store, keyring_backend_kind)
        }
        ...
        AuthCredentialsStoreMode::Ephemeral => Arc::new(EphemeralAuthStorage::new(workx_home)),
    }
}
...
fn create_keyring_auth_storage(
    ...
    match keyring_backend_kind {
        AuthKeyringBackendKind::Direct => {
            Arc::new(DirectKeyringAuthStorage::new(workx_home, keyring_store))
        }
        AuthKeyringBackendKind::Secrets => {
            Arc::new(SecretsKeyringAuthStorage::new(workx_home, keyring_store))
        }
    }
}

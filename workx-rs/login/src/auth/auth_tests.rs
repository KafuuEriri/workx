...
use workx_protocol::auth::KnownPlan as InternalKnownPlan;
...
use workx_protocol::shell_environment::OPENAI_FEDERATION_RULE_ID_ENV_VAR;
use workx_protocol::shell_environment::OPENAI_IDENTITY_TOKEN_FILE_ENV_VAR;
...
fn header_auth_exposes_a_valid_chatgpt_account_id() {
    for (account_id_header, expected_account_id) in [
        ...
        let auth = WorkxAuth::Headers(AuthHeaders::new(headers));
...
async fn agent_identity_jwt_uses_explicit_staging_endpoint_overrides() -> anyhow::Result<()> {
    ...
    let _authapi_guard =
        EnvVarGuard::set("WORKX_AGENT_IDENTITY_AUTHAPI_BASE_URL", &authapi_base_url);
    let jwks_base_url = format!("{}/api/codex/", jwks_server.uri());
    let _jwks_guard = EnvVarGuard::set("WORKX_AGENT_IDENTITY_JWKS_BASE_URL", &jwks_base_url);

    let auth = WorkxAuth::from_agent_identity_jwt(
        &jwt,
    ...
    let WorkxAuth::AgentIdentity(agent_identity) = auth else {
...
fn agent_identity_authapi_override_preserves_chatgpt_environment_validation() {
    let _authapi_guard = EnvVarGuard::set(
        "WORKX_AGENT_IDENTITY_AUTHAPI_BASE_URL",
        "https://authapi.example/api/accounts",
    );
    let _jwks_guard = EnvVarGuard::set(
        "WORKX_AGENT_IDENTITY_JWKS_BASE_URL",
...
fn agent_identity_custom_jwks_base_requires_explicit_authapi_override() {
    let _authapi_guard = EnvVarGuard::remove("WORKX_AGENT_IDENTITY_AUTHAPI_BASE_URL");
...
async fn stored_agent_identity_jwt_keeps_auth_json_unchanged() -> anyhow::Result<()> {
    ...
    let WorkxAuth::AgentIdentity(agent_identity_auth) = auth else {
        panic!("stored JWT should load as agent identity auth");
    };
    assert_eq!(agent_identity_auth.run_task_id(), "task-id");
    let storage = FileAuthStorage::new(workx_home.path().to_path_buf());
    let auth = storage
        .try_read_auth_json(&get_auth_file(workx_home.path()))
        .expect("auth.json should parse");
    assert_eq!(
        auth.agent_identity,
        Some(AgentIdentityStorage::Jwt(agent_identity))
...
async fn login_with_access_token_writes_only_personal_access_token() {
    ...
    Mock::given(method("GET"))
        ...
        .mount(&server)
        .await;
    let _authapi_guard = EnvVarGuard::set("WORKX_AUTHAPI_BASE_URL", &server.uri());
    let allowed_workspaces = [WORKSPACE_ID_ALLOWED.to_string()];
    super::login_with_access_token(
        dir.path(),
        "at-login-test",
...
async fn chatgpt_auth_task_registration_retry_exhaustion_is_fallback_eligible() -> anyhow::Result<()>
{
    ...
    let storage = FileAuthStorage::new(workx_home.path().to_path_buf());
    let auth_path = get_auth_file(workx_home.path());
...
async fn agent_identity_jwt_task_registration_retry_exhaustion_is_strict() -> anyhow::Result<()> {
    ...
    let err = WorkxAuth::from_agent_identity_jwt_with_authapi_base_url(
        &agent_identity,
        Some(&chatgpt_base_url),
        &authapi_base_url,
        &crate::test_support::transport_default_auth_route_config(),
...
async fn refresh_failure_is_scoped_to_the_matching_auth_snapshot() {
    ...
    let updated_auth = WorkxAuth::from_auth_dot_json(
        workx_home.path(),
...
async fn unauthorized_recovery_retries_provider_command_after_initial_failure() {
    ...
    assert_eq!(
        manager.auth_cached(),
        Some(WorkxAuth::from_api_key("provider-token"))
    );
    assert!(!recovery.has_next());
...
fn unauthorized_recovery_without_an_external_provider_still_requires_refreshable_auth() {
    for auth in [None, Some(WorkxAuth::from_api_key("static-token"))] {
...
#[derive(Clone)]
struct StaticExternalAuth(WorkxAuth);
...
impl ExternalAuth for StaticExternalAuth {
    ...
    fn refresh(&self, _context: ExternalAuthRefreshContext) -> ExternalAuthFuture<'_, WorkxAuth> {
        Box::pin(async { Ok(self.0.clone()) })
    }
}
...
fn external_header_auth(account_id: Option<&'static str>) -> WorkxAuth {
    let mut headers = http::HeaderMap::new();
    headers.insert(
    ...
    WorkxAuth::Headers(AuthHeaders::new(headers))
}
...
async fn external_auth_keeps_cached_credentials_after_permanent_reload_failure() {
    let manager = AuthManager::from_auth_for_testing(WorkxAuth::from_api_key("seed"));
    let auth = WorkxAuth::from_api_key("configured-token");
...
async fn replacing_external_auth_clears_permanent_failure() {
    let manager = AuthManager::from_auth_for_testing(WorkxAuth::from_api_key("seed"));
    let auth = WorkxAuth::from_api_key("external-token");
    manager
...
async fn runtime_external_auth_uses_provider_error_classification() {
    ...
    manager
        .set_external_auth(Arc::new(FailingExternalAuth {
            auth: WorkxAuth::from_api_key("runtime-token"),
            resolve_count: AtomicUsize::new(0),
        }))
        .await
...
async fn external_auth_provider_can_install_headers() {
    ...
    assert!(
        manager
            .auth_cached()
            .is_some_and(|auth| auth.uses_workx_backend())
    );
    assert!(
        !manager
            .auth_cached()
...
async fn workload_identity_auth_is_immutable_and_process_local() {
    ...
    let auth =
        WorkxAuth::from_external_chatgpt_tokens(&access_token, "workspace-one", Some("enterprise"))
    ...
    assert!(!get_auth_file(workx_home.path()).exists());
...
fn write_auth_file(params: AuthFileParams, workx_home: &Path) -> std::io::Result<String> {
    let fake_jwt = fake_jwt_for_auth_file_params(&params)?;
    let auth_file = get_auth_file(workx_home);
...
fn remove_access_token_env_var() -> EnvVarGuard {
    EnvVarGuard::remove(WORKX_ACCESS_TOKEN_ENV_VAR)
}
...
fn test_auth_manager_config(workx_home: &Path) -> TestAuthManagerConfig {
...
fn auth_config_from_preserves_all_fields() {
    let workx_home = tempdir().expect("tempdir");
    let config = test_auth_manager_config(workx_home.path());
...
async fn shared_from_config_prefers_workload_identity_to_explicit_access_token() {
    let workx_home = tempdir().expect("tempdir");
    let config = test_auth_manager_config(workx_home.path());
    let _access_token_guard = EnvVarGuard::set(WORKX_ACCESS_TOKEN_ENV_VAR, "at-explicit");
    ...
    let error = AuthManager::shared_from_config(&config, /*enable_workx_api_key_env*/ false)
        .await
...
async fn load_auth_reads_access_token_from_env() {
    ...
    let _access_token_guard = EnvVarGuard::set(WORKX_ACCESS_TOKEN_ENV_VAR, &agent_identity);
    ...
    assert!(
        !get_auth_file(workx_home.path()).exists(),
...
async fn load_auth_reads_personal_access_token_from_env() {
    ...
    Mock::given(method("GET"))
        ...
        .await;
    let _authapi_guard = EnvVarGuard::set("WORKX_AUTHAPI_BASE_URL", &server.uri());
    let _access_token_guard = EnvVarGuard::set(WORKX_ACCESS_TOKEN_ENV_VAR, "at-env-test");
...
async fn auth_manager_rejects_env_personal_access_token_workspace_mismatch() {
    ...
    let _access_token_guard =
        EnvVarGuard::set(WORKX_ACCESS_TOKEN_ENV_VAR, "at-env-workspace-mismatch");
...
async fn personal_access_token_does_not_offer_unauthorized_recovery() {
    ...
    let _authapi_guard = EnvVarGuard::set("WORKX_AUTHAPI_BASE_URL", &server.uri());
    let _access_token_guard =
        EnvVarGuard::set(WORKX_ACCESS_TOKEN_ENV_VAR, "at-no-unauthorized-recovery");
    let manager = Arc::new(
        AuthManager::new(
            workx_home.path().to_path_buf(),
            /*enable_workx_api_key_env*/ false,
...
#[tokio::test]
#[serial(workx_auth_env)]
async fn load_auth_keeps_workx_api_key_env_precedence() {
    let workx_home = tempdir().unwrap();
    let record = agent_identity_record(WORKSPACE_ID_ALLOWED);
    let agent_identity = fake_agent_identity_jwt(&record).expect("fake agent identity");
    let _access_token_guard = EnvVarGuard::set(WORKX_ACCESS_TOKEN_ENV_VAR, &agent_identity);
    let _api_key_guard = EnvVarGuard::set(WORKX_API_KEY_ENV_VAR, "sk-env");
...
async fn enforce_login_restrictions_logs_out_for_method_mismatch() {
    ...
    assert!(
        !workx_home.path().join("auth.json").exists(),
...
async fn auth_manager_rejects_disallowed_stored_and_external_auth() {
    ...
    let manager =
        AuthManager::shared_from_auth_config(config, /*enable_workx_api_key_env*/ false)
            .await
            .expect("auth manager");
    ...
    assert!(
        manager
            .set_external_auth(Arc::new(StaticExternalAuth(WorkxAuth::from_api_key(
                "sk-external",
            ))))
            .await
            .is_err(),
...
async fn api_only_policy_rejects_access_tokens_before_hydration() {
    ...
    let server = MockServer::start().await;
    let _authapi_guard = EnvVarGuard::set("WORKX_AUTHAPI_BASE_URL", &server.uri());
    let _access_token_guard = EnvVarGuard::set(WORKX_ACCESS_TOKEN_ENV_VAR, "at-rejected");
    let mut config = build_config(
        workx_home.path(),
        /*forced_login_method*/ None,
        /*forced_chatgpt_workspace_id*/ None,
...
async fn workspace_policy_rejects_agent_identity_before_hydration() {
    ...
    let access_token_guard = EnvVarGuard::set(WORKX_ACCESS_TOKEN_ENV_VAR, &agent_identity);
    let mut config = build_config(
        workx_home.path(),
        /*forced_login_method*/ None,
        /*forced_chatgpt_workspace_id*/ None,
...
async fn workspace_policy_checks_the_selected_request_account() {
    ...
    let auth_path = workx_home.path().join("auth.json");
...
async fn enforce_login_restrictions_blocks_env_api_key_when_chatgpt_required() {
    let _guard = EnvVarGuard::set(WORKX_API_KEY_ENV_VAR, "sk-env");
    let _access_token_guard = remove_access_token_env_var();
    let workx_home = tempdir().unwrap();
...
fn agent_identity_record(account_id: &str) -> AgentIdentityAuthRecord {
    let key_material =
        workx_agent_identity::generate_agent_key_material().expect("generate agent key material");
...
async fn assert_agent_identity_plan_alias(
    ...
    let authapi_base_url = server.uri();
    let chatgpt_base_url = format!("{authapi_base_url}/backend-api");
    let auth = WorkxAuth::from_agent_identity_jwt_with_authapi_base_url(
        &jwt,
        Some(&chatgpt_base_url),
        &authapi_base_url,
        &crate::test_support::transport_default_auth_route_config(),

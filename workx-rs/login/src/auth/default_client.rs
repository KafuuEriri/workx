//! Default Workx HTTP client: shared `User-Agent`, `originator`, optional residency header, and
//! `HttpClient` construction.
//!
//! Use [`crate::default_client`] or [`workx_login::default_client`] from other crates in this
//! workspace.
...
use std::sync::Mutex;
use std::sync::RwLock;
use workx_http_client::BuildRouteAwareHttpClientError;
...
pub use workx_http_client::RequestBuilder as WorkxRequestBuilder;
...
/// Parenthesis will be added by Workx. This should only specify what goes inside of the parenthesis.
pub static USER_AGENT_SUFFIX: LazyLock<Mutex<Option<String>>> = LazyLock::new(|| Mutex::new(None));
pub const DEFAULT_ORIGINATOR: &str = "workx_cli_rs";
pub const WORKX_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR: &str = "WORKX_INTERNAL_ORIGINATOR_OVERRIDE";
pub const RESIDENCY_HEADER_NAME: &str = "x-openai-internal-codex-residency";

pub use workx_config::ResidencyRequirement;
...
fn get_originator_value(provided: Option<String>) -> Originator {
    let value = std::env::var(WORKX_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR)
        .ok()
        .or(provided)
...
pub fn originator() -> Originator {
    ...
    if std::env::var(WORKX_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR).is_ok() {
...
pub fn is_first_party_originator(originator_value: &str) -> bool {
    originator_value == DEFAULT_ORIGINATOR
        || originator_value == "workx-tui"
        || originator_value == "workx_vscode"
        || originator_value.starts_with("Workx ")
}

pub fn is_first_party_chat_originator(originator_value: &str) -> bool {
    originator_value == "workx_atlas" || originator_value == "workx_chatgpt_desktop"
}

pub fn get_workx_user_agent() -> String {
...
fn sanitize_user_agent(candidate: String, fallback: &str) -> String {
    ...
    if !sanitized.is_empty() && HeaderValue::from_str(sanitized.as_str()).is_ok() {
        tracing::warn!(
            "Sanitized Workx user agent because provided suffix contained invalid header characters"
        );
        sanitized
    } else if HeaderValue::from_str(fallback).is_ok() {
        tracing::warn!(
            "Falling back to base Workx user agent because provided suffix could not be sanitized"
        );
        fallback.to_string()
    } else {
        tracing::warn!(
            "Falling back to default Workx originator because base user agent string is invalid"
        );
        originator().value
    }
}
...
/// Workx's route-aware system/PAC resolution.
...
/// Builds the default Workx HTTP client for a concrete outbound route.
///
/// When route-aware proxy handling is disabled, or the client is running inside the Workx
...
/// Builds the default Workx HTTP client for a concrete outbound route without blocking the
...
/// Builds an HTTP client for an auth endpoint without Workx default headers.
...
/// Builds the default Workx HTTP client wrapper for an auth endpoint.
...
pub fn default_headers() -> HeaderMap {
    ...
    if let Ok(user_agent) = HeaderValue::from_str(&get_workx_user_agent()) {
...
fn is_sandboxed() -> bool {
    std::env::var("WORKX_SANDBOX").as_deref() == Ok("seatbelt")
}

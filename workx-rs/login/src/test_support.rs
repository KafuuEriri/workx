...
pub fn auth_manager_from_optional_auth(auth: Option<WorkxAuth>) -> Arc<AuthManager> {
...
pub async fn auth_manager_with_agent_identity() -> std::io::Result<Arc<AuthManager>> {
    let key_material =
        workx_agent_identity::generate_agent_key_material().map_err(std::io::Error::other)?;

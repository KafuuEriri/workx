...
`workx-tools` is the shared support crate for building, adapting, and executing
model-visible tools outside `workx-core`.
...
compatibility-sensitive orchestration in `workx-core` until the surrounding
...
- code-mode compatibility shims that do not depend on `workx-core`
- other narrowly scoped host utilities that multiple crates need

The corresponding non-goals are just as important:

- do not move `workx-core` orchestration here prematurely
...
1. Keep extension-owned executable-tool authoring in `workx-extension-api`.
2. Move host-side planning/adaptation helpers here when they no longer need to
   stay coupled to `workx-core`.
3. Leave compatibility-sensitive adapters in `workx-core` while downstream
...
`workx-core`, that is a sign to revisit the extraction boundary before adding
more here.

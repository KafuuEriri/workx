...
impl ExternalAuth for TestExternalApiKeyAuth {
    fn resolve(&self) -> workx_login::ExternalAuthFuture<'_, WorkxAuth> {
        Box::pin(async { Ok(WorkxAuth::from_api_key("test-external-api-key")) })
    }
...
impl ModelsEndpointClient for TestModelsEndpoint {
    ...
    fn uses_workx_backend(&self) -> ModelsEndpointFuture<'_, bool> {
        Box::pin(async { self.uses_workx_backend })
    }
...
fn openai_manager_for_tests_with_auth(
    ...
    OpenAiModelsManager::new(workx_home, endpoint_client, auth_manager)
}

async fn mutate_file_cache_for_test<F>(workx_home: &Path, f: F)
where
    F: FnOnce(&mut ModelsCacheEntry),
{
    let client_version = crate::client_version_to_whole();
    let cache = FileModelsCache::new(workx_home.join(MODEL_CACHE_FILE), DEFAULT_MODEL_CACHE_TTL);
    let mut entry = cache
        .load(&client_version)
        .await
...
async fn file_cache_implements_models_cache_contract() {
    let workx_home = tempdir().expect("temp dir");
    let cache = FileModelsCache::new(
        workx_home.path().join(MODEL_CACHE_FILE),
...
async fn chatgpt_auth_tokens_for_tests(workx_home: &Path) -> WorkxAuth {
    let auth_dot_json = workx_login::AuthDotJson {
        auth_mode: Some(AuthMode::ChatgptAuthTokens),
        openai_api_key: None,
        tokens: Some(TokenData {
            id_token: workx_login::token_data::parse_chatgpt_jwt_claims(
...
c2ln",
    ...
    std::fs::create_dir_all(workx_home).expect("workx home should be created");
    std::fs::write(
...
async fn dynamic_manager_preserves_requested_model_when_fallback_is_allowed() {
    ...
    let manager = openai_manager_for_tests(workx_home.path().to_path_buf(), endpoint.clone());
    let requested_model = Some("unsupported".to_string());
...
async fn get_model_info_matches_hyphenated_provider_namespace_suffix() {
    ...
    let namespaced_model = "workx/gpt-image".to_string();
...
async fn refresh_available_models_uses_cached_remote_only_catalog_for_chatgpt_auth() {
    ...
    let fetch_manager =
        openai_manager_for_tests(workx_home.path().to_path_buf(), fetch_endpoint.clone());
    ...
    let cache_manager =
        openai_manager_for_tests(workx_home.path().to_path_buf(), cache_endpoint.clone());
...
async fn get_model_info_uses_fallback_for_bundled_models_when_chatgpt_remote_is_authoritative() {
    ...
    let manager = openai_manager_for_tests(workx_home.path().to_path_buf(), endpoint);
...
async fn refresh_available_models_refetches_when_cache_stale() {
    ...
    mutate_file_cache_for_test(workx_home.path(), |cache| {
...
impl ModelsEndpointClient for TestAuthAwareModelsEndpoint {
    ...
    fn uses_workx_backend(&self) -> ModelsEndpointFuture<'_, bool> {
        Box::pin(TestAuthAwareModelsEndpoint::uses_workx_backend(self))
    }
...
async fn refresh_available_models_fetches_with_chatgpt_auth_tokens() {
    ...
    let auth = chatgpt_auth_tokens_for_tests(workx_home.path()).await;
    let manager = openai_manager_for_tests_with_auth(
        workx_home.path().to_path_buf(),
        endpoint.clone(),

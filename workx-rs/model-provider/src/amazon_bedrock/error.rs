...
pub(super) const BEDROCK_EXPIRED_SIGNATURE_MESSAGE: &str = concat!(
    ...
    "update or unset it, then restart Workx",
);

pub(super) fn map_api_error(error: ApiError) -> WorkxErr {
    let error = workx_api::map_api_error(error);
    if let WorkxErrorDetails::UnexpectedStatus(response) = error.details()
        ...
        && response.body.contains("Signature expired:")
    {
        ...
        let mapped_error = WorkxErr::new(WorkxErrorDetails::UnexpectedStatus(response));

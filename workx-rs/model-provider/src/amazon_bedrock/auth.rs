...
use workx_aws_auth::AwsAccessKeys;
use workx_aws_auth::AwsAuthContext;
use workx_aws_auth::AwsAuthError;
use workx_aws_auth::AwsRequestToSign;
use workx_http_client::Request;
use workx_http_client::RequestBody;
use workx_http_client::RequestCompression;
...
pub(super) fn auth_source(
        ...
        .is_some_and(|aws| aws.profile.is_some())
    {
        ...
        Some(WorkxAuth::BedrockApiKey(_))
    ) {
        BedrockAuthSource::ManagedBearerToken
    } else if matches!(
        auth_manager.and_then(AuthManager::auth_cached),
        Some(WorkxAuth::BedrockAccessKeys(_))
    ) {
...
pub(super) async fn resolve_auth_method(
    ...
    managed_auth: Option<&WorkxAuth>,
    aws: &ModelProviderAwsAuthInfo,
    endpoint: BedrockEndpoint,
) -> Result<BedrockAuthMethod> {
    match source {
        BedrockAuthSource::CommandBearerToken => Err(WorkxErr::Fatal(
            "Amazon Bedrock command authentication must be resolved by the model provider"
                .to_string(),
        )),
        BedrockAuthSource::ManagedBearerToken => {
            let Some(WorkxAuth::BedrockApiKey(auth)) = managed_auth else {
                return Err(WorkxErr::Fatal(
                    "selected Workx-managed Amazon Bedrock API key is no longer available"
                        .to_string(),
                ));
            };
        ...
        BedrockAuthSource::ConfiguredAwsProfile => {
            ...
            let context = AwsAuthContext::load_profile(config)
                .await
                .map_err(aws_auth_error_to_workx_error)?;
            Ok(BedrockAuthMethod::AwsSdkAuth { context })
        }
        BedrockAuthSource::ManagedAccessKeys => {
            let Some(WorkxAuth::BedrockAccessKeys(auth)) = managed_auth else {
                return Err(WorkxErr::Fatal(
                    "selected Workx-managed Amazon Bedrock access keys are no longer available"
                        .to_string(),
                ));
            };
...
fn aws_auth_error_to_workx_error(error: AwsAuthError) -> WorkxErr {
    WorkxErr::Fatal(format!("failed to resolve Amazon Bedrock auth: {error}"))
}
...
fn remove_headers_not_preserved_by_bedrock_mantle(headers: &mut HeaderMap) {
    ...
    // richer Workx agent requests fail even though raw Responses requests work.
...
impl AuthProvider for BedrockSigV4AuthProvider {
    ...
    fn apply_auth(&self, request: Request) -> workx_api::AuthProviderFuture<'_> {
...
mod tests {
    ...
    fn bedrock_auth_source_distinguishes_static_environment_credentials() {
        ...
        let managed_auth =
            AuthManager::from_auth_for_testing(WorkxAuth::BedrockApiKey(BedrockApiKeyAuth {
                api_key: "managed-bedrock-api-key".to_string(),
                region: "us-east-1".to_string(),
            }));
        let managed_access_keys = AuthManager::from_auth_for_testing(WorkxAuth::BedrockAccessKeys(

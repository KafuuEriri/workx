...
use workx_model_provider_info::AMAZON_BEDROCK_GPT_5_6_TERRA_MODEL_ID;
use workx_model_provider_info::AMAZON_BEDROCK_RUNTIME_GLOBAL_GPT_5_6_LUNA_MODEL_ID;
use workx_model_provider_info::AMAZON_BEDROCK_RUNTIME_GLOBAL_GPT_5_6_TERRA_MODEL_ID;
...
impl AmazonBedrockModelProvider {
    ...
    fn managed_auth(&self) -> Option<WorkxAuth> {
        ...
        Box::pin(async move {
            ...
            recovery.refresh().await.map_err(|error| {
                if error.kind() == std::io::ErrorKind::InvalidInput {
                    WorkxErr::InvalidRequest(error.to_string())
                } else {
                    WorkxErr::Io(error)
                }
            })?;
    ...
    fn account_state(&self) -> ProviderAccountResult {
        Ok(ProviderAccountState {
            account: Some(ProviderAccount::AmazonBedrock {
                uses_workx_managed_credentials: matches!(
                    self.auth_source(),
                    auth::BedrockAuthSource::ManagedBearerToken
                        | auth::BedrockAuthSource::ManagedAccessKeys
                ),
...
mod tests {
    ...
    async fn configured_profile_takes_precedence_over_managed_auth() {
        ...
        let auth_manager =
            AuthManager::from_auth_for_testing(WorkxAuth::BedrockApiKey(managed_auth.clone()));
        ...
        assert_eq!(
            provider.auth().await,
            Some(WorkxAuth::BedrockApiKey(managed_auth))
        );
        assert_eq!(
            provider.account_state(),
            Ok(ProviderAccountState {
        ...
        let access_keys_auth_manager = AuthManager::from_auth_for_testing(
            WorkxAuth::BedrockAccessKeys(BedrockAccessKeysAuth {
    ...
    async fn runtime_managed_auth_resolves_runtime_endpoint() {
        ...
        let auth_manager =
            AuthManager::from_auth_for_testing(WorkxAuth::BedrockApiKey(managed_auth));
        let provider = AmazonBedrockModelProvider::new(
            ModelProviderInfo::create_amazon_bedrock_runtime_provider(/*aws*/ None),
            Some(auth_manager),
        );

...
use workx_agent_identity::AgentIdentityKey;
use workx_agent_identity::authorization_header_for_agent_task;
use workx_api::AgentIdentityTelemetry;
use workx_api::AuthError;
use workx_api::AuthHeadersFuture;
...
use workx_login::auth::AgentIdentityAuthError;
...
struct AuthManagerAuthProvider {
    ...
    expected_auth: WorkxAuth,
}

impl AuthManagerAuthProvider {
    fn is_expected_auth(&self, auth: &WorkxAuth) -> bool {
        auth.uses_workx_backend()
    ...
    fn current_auth(&self) -> Option<WorkxAuth> {
        self.auth_manager
            .auth_cached()
...
pub(crate) fn resolve_provider_auth(
    ...
    provider: &ModelProviderInfo,
) -> workx_protocol::error::Result<SharedAuthProvider> {
    ...
    if matches!(
        auth,
        Some(WorkxAuth::BedrockApiKey(_) | WorkxAuth::BedrockAccessKeys(_))
    ) {
...
) -> workx_protocol::error::Result<ResolvedProviderAuth> {
    let ProviderAuthScope {
        agent_identity_policy,
        session_source,
        agent_identity_session_fallback,
    } = scope;
    if let Some(WorkxAuth::AgentIdentity(agent_identity_auth)) = auth {
...
fn should_bootstrap_chatgpt_agent_identity(
    ...
    agent_identity_policy == AgentIdentityAuthPolicy::ChatGptAuth
        && matches!(auth, Some(WorkxAuth::Chatgpt(_)))
}

fn bearer_auth_for_provider(
    provider: &ModelProviderInfo,
) -> workx_protocol::error::Result<Option<BearerAuthProvider>> {
...
/// Builds request-header auth for a first-party Workx auth snapshot.
pub fn auth_provider_from_auth(auth: &WorkxAuth) -> SharedAuthProvider {
    match auth {
        WorkxAuth::AgentIdentity(auth) => {
            Arc::new(AgentIdentityAuthProvider { auth: auth.clone() })
        }
        WorkxAuth::Headers(auth) => Arc::new(HeaderAuthProvider { auth: auth.clone() }),
        WorkxAuth::BedrockApiKey(_) | WorkxAuth::BedrockAccessKeys(_) => {
            unreachable!("{BEDROCK_API_KEY_UNSUPPORTED_MESSAGE}")
        }
        WorkxAuth::ApiKey(_)
        | WorkxAuth::Chatgpt(_)
        | WorkxAuth::ChatgptAuthTokens(_)
        | WorkxAuth::PersonalAccessToken(_) => Arc::new(BearerAuthProvider {
...
mod tests {
    ...
    use workx_agent_identity::generate_agent_key_material;
    ...
    use workx_login::auth::login_with_chatgpt_auth_tokens;
    ...
    fn test_workx_home() -> PathBuf {
        ...
        let path = std::env::temp_dir().join(format!(
            "workx-model-provider-agent-identity-{pid}-{id}",
            pid = std::process::id()
        ));
        let _ = std::fs::remove_dir_all(&path);
        std::fs::create_dir_all(&path).expect("create temp workx home");
        path
    }

    fn write_chatgpt_auth_json(workx_home: &Path) {
        let auth_json = json!({
            "tokens": {
    ...
    ) -> (PathBuf, Arc<AuthManager>, WorkxAuth) {
        let workx_home = test_workx_home();
        write_chatgpt_auth_json(&workx_home);
    ...
    fn custom_provider_does_not_inherit_ambient_auth_headers() {
        ...
        let ambient_auth = WorkxAuth::Headers(AuthHeaders::new(ambient_headers));
    ...
    fn custom_provider_does_not_inherit_ambient_bedrock_auth() {
        let provider =
            create_oss_provider_with_base_url("http://localhost:11434/v1", WireApi::Responses);
        let ambient_auth = WorkxAuth::BedrockApiKey(BedrockApiKeyAuth {
    ...
    fn custom_provider_uses_command_resolved_auth() {
        ...
        let command_auth = WorkxAuth::from_api_key("command-token");
    ...
    fn openai_provider_preserves_ambient_auth_headers() {
        ...
        let ambient_auth = WorkxAuth::Headers(AuthHeaders::new(expected.clone()));
    ...
    fn header_auth_adds_predefined_headers() {
        ...
        let auth = WorkxAuth::Headers(AuthHeaders::new(expected.clone()));
    ...
    async fn first_party_run_scope_uses_agent_assertion_and_exposes_telemetry() {
        let auth = WorkxAuth::AgentIdentity(
    ...
    async fn agent_identity_auth_provider_preserves_account_routing_headers() {
        ...
        let provider = auth_provider_from_auth(&WorkxAuth::AgentIdentity(auth));
    ...
    async fn chatgpt_bootstrap_unavailable_uses_session_bearer_fallback() {
        ...
        .await;
        let (_workx_home, auth_manager, auth) = chatgpt_auth_manager(server.uri()).await;
        let provider = ModelProviderInfo::create_openai_provider(/*base_url*/ None);

...
use workx_login::default_client::read_default_client_residency_requirement;
use workx_model_provider_info::ModelProviderInfo;
use workx_models_manager::cache::ModelsCache;
...
use workx_protocol::account::ProviderAccount;
...
/// Optional provider-backed features that Workx may expose at runtime.
///
...
pub trait ModelProvider: fmt::Debug + Send + Sync {
    ...
    /// to think through whether Workx should have a unified provider-specific auth
    ...
    fn map_api_error(&self, error: ApiError) -> WorkxErr {
        workx_api::map_api_error(error)
    }

    /// Returns provider configuration adapted for the API client.
    fn api_provider(&self) -> ModelProviderFuture<'_, workx_protocol::error::Result<Provider>> {
        Box::pin(async move {
            let auth = self.auth().await;
            let mut provider = self
                .info()
                .to_api_provider(auth.as_ref().map(WorkxAuth::auth_mode))?;
    ...
    fn runtime_base_url(
        &self,
    ) -> ModelProviderFuture<'_, workx_protocol::error::Result<Option<String>>> {
    ...
    fn api_auth(
        &self,
    ) -> ModelProviderFuture<'_, workx_protocol::error::Result<SharedAuthProvider>> {
        Box::pin(async move {
            let auth = self.auth().await;
    ...
    /// Returns request credentials, optionally scoped to a Workx session task.
    fn api_auth_for_scope(
        &self,
        scope: ProviderAuthScope,
    ) -> ModelProviderFuture<'_, workx_protocol::error::Result<ResolvedProviderAuth>> {
        Box::pin(async move {
    ...
    fn models_manager_without_cache(
        ...
        let model_catalog = config_model_catalog
            .or_else(|| workx_models_manager::bundled_models_response().ok())
            .unwrap_or_default();
        ...
        let account = if self.info.requires_openai_auth {
            self.auth_manager
                ...
                .and_then(|auth_manager| {
                    ...
                    if matches!(auth, WorkxAuth::Headers(_)) {
                        return None;
                    }
                    Some(auth)
                })
                .map(|auth| match &auth {
                    WorkxAuth::ApiKey(_) => Ok(ProviderAccount::ApiKey),
...
mod tests {
    ...
    use workx_model_provider_info::AwsAuthRefreshConfig;
    use workx_model_provider_info::ModelProviderAwsAuthInfo;
    ...
    fn test_workx_home() -> std::path::PathBuf {
        std::env::temp_dir().join(format!("workx-model-provider-test-{}", std::process::id()))
    }
    ...
    fn bedrock_api_key_auth() -> WorkxAuth {
        WorkxAuth::BedrockApiKey(BedrockApiKeyAuth {
    ...
    fn create_model_provider_does_not_use_openai_auth_manager_for_amazon_bedrock_provider() {
        let provider = create_model_provider(
            ModelProviderInfo::create_amazon_bedrock_provider(Some(ModelProviderAwsAuthInfo {
                profile: Some("workx-bedrock".to_string()),
                region: None,
                auth_refresh: None,
            })),
            Some(AuthManager::from_auth_for_testing(WorkxAuth::from_api_key(
    ...
    async fn shared_bedrock_auth_refresh_is_reused_only_for_matching_configuration() {
        ...
        const HELPER_ARG: &str = "WORKX_BEDROCK_SHARED_AUTH_REFRESH_COMMAND";
        const SUBPROCESS_ARG: &str = "WORKX_BEDROCK_SHARED_AUTH_REFRESH_SUBPROCESS";
    ...
    async fn configured_bedrock_catalog_only_allows_default_service_tier() {
        let configured_model = workx_models_manager::bundled_models_response()

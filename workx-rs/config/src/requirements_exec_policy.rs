...
use workx_execpolicy::RuleRef;
use workx_execpolicy::rule::PatternToken;
use workx_execpolicy::rule::PrefixPattern;
use workx_execpolicy::rule::PrefixRule;
...
pub enum RequirementsExecPolicyParseError {
    ...
    #[error(
        "rules prefix_rule at index {rule_index} has decision 'allow', which is not permitted in requirements.toml: Workx merges these rules with other config and uses the most restrictive result (use 'prompt' or 'forbidden')"
    )]
...
impl RequirementsExecPolicyToml {
    ...
    /// representation used by `workx-execpolicy`.

...
pub struct LoaderOverrides {
    /// Optional configuration file supplied with the installed Workx package.
...
impl LoaderOverrides {
    ...
    pub fn without_managed_config_for_tests() -> Self {
        let base = std::env::temp_dir().join("workx-config-tests");
        Self {
    ...
    pub fn user_config_path(&self, workx_home: &Path) -> std::io::Result<AbsolutePathBuf> {
...
impl ConfigLayerEntry {
    ...
    // Get the `.workx/` folder associated with this config layer, if any.
    pub fn config_folder(&self) -> Option<AbsolutePathBuf> {
        match &self.name {
            ...
            ConfigLayerSource::Project { dot_workx_folder } => Some(dot_workx_folder.clone()),
    ...
    /// Returns the `.workx/` folder that should be used for hook declarations.
    ///
...
impl ConfigLayerStack {
    ...
    /// the base `$WORKX_HOME/config.toml` layer because the active layer is the
...
fn verify_layer_ordering(layers: &[ConfigLayerEntry]) -> std::io::Result<()> {
    ...
    // user config.
    let mut previous_project_dot_workx_folder: Option<&AbsolutePathBuf> = None;
    for layer in layers {
        if let ConfigLayerSource::Project {
            dot_workx_folder: current_project_dot_workx_folder,
        } = &layer.name
        {
            if let Some(previous) = previous_project_dot_workx_folder {
                ...
                if previous == current_project_dot_workx_folder
                    || !current_project_dot_workx_folder
                        .as_path()
                        .ancestors()
            ...
            previous_project_dot_workx_folder = Some(current_project_dot_workx_folder);
        }
    }

...
use toml_edit::value;
use workx_utils_path::resolve_symlink_write_paths;
use workx_utils_path::write_atomically;
...
pub async fn clear_user_plugin(workx_home: &Path, plugin_key: String) -> std::io::Result<()> {
    apply_user_plugin_config_edits(workx_home, vec![PluginConfigEdit::Clear { plugin_key }]).await
}
...
pub async fn apply_user_plugin_config_edits(
    ...
    task::spawn_blocking(move || apply_user_plugin_config_edits_blocking(&workx_home, edits))
        .await
...
mod tests {
    ...
    async fn set_user_plugin_enabled_writes_plugin_entry() {
        ...
        let config = read_config(workx_home.path());
...
"#,
        ...
        clear_user_plugin(workx_home.path(), "demo@market".to_string())
            .await
            .unwrap();

        assert_eq!(
            fs::read_to_string(workx_home.path().join(CONFIG_TOML_FILE)).unwrap(),
            ""
        );
    }
    ...
    async fn set_user_plugin_enabled_follows_config_symlink() {
        ...
        let workx_home = TempDir::new().unwrap();
        let target_path = workx_home.path().join("target_config.toml");
        symlink(&target_path, workx_home.path().join(CONFIG_TOML_FILE)).unwrap();
...
enabled = true
    ...
    fn read_config(workx_home: &Path) -> toml::Value {
        toml::from_str(&fs::read_to_string(workx_home.join(CONFIG_TOML_FILE)).unwrap()).unwrap()
    }
}

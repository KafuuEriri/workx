...
use workx_features::legacy_feature_keys;
...
pub fn features_schema(schema_gen: &mut SchemaGenerator) -> Schema {
    ...
    for feature in FEATURES {
        if feature.id == workx_features::Feature::Artifact {
            continue;
        }
        if feature.id == workx_features::Feature::CodeMode {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<workx_features::FeatureToml<
                    workx_features::CodeModeConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == workx_features::Feature::CodeModeHost {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<workx_features::FeatureToml<
                    workx_features::CodeModeHostConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == workx_features::Feature::NonPrefixedMcpToolNames {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<workx_features::FeatureToml<
                    workx_features::NonPrefixedMcpToolNamesConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == workx_features::Feature::GuardianV2 {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<workx_features::FeatureToml<
                    workx_features::GuardianV2ConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == workx_features::Feature::MultiAgentV2 {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<workx_features::FeatureToml<
                    workx_features::MultiAgentV2ConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == workx_features::Feature::TokenBudget {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<workx_features::FeatureToml<
                    workx_features::TokenBudgetConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == workx_features::Feature::ContextManagement {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<workx_features::FeatureToml<
                    workx_features::ContextManagementConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == workx_features::Feature::RolloutBudget {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<workx_features::FeatureToml<
                    workx_features::RolloutBudgetConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == workx_features::Feature::CurrentTimeReminder {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<workx_features::FeatureToml<
                    workx_features::CurrentTimeReminderConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == workx_features::Feature::SleepTool {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<workx_features::FeatureToml<
                    workx_features::SleepToolConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == workx_features::Feature::AppsMcpPathOverride {
            validation.properties.insert(
                feature.key.to_string(),
        ...
        if feature.id == workx_features::Feature::NetworkProxy {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<workx_features::FeatureToml<
                    workx_features::NetworkProxyConfigToml,
                >>(),
            );
            continue;
        }
    ...
    validation.properties.insert(
        ...
        schema_gen.subschema_for::<workx_features::ToolRegistryConfigToml>(),
    );

...
use workx_file_system::ExecutorFileSystem;
use workx_git_utils::resolve_root_git_project_for_trust;
use workx_network_proxy::credential_broker_provider_context_env_keys;
use workx_network_proxy::is_credential_broker_provider_env_key;
...
#[cfg(unix)]
const SYSTEM_CONFIG_TOML_FILE_UNIX: &str = "/etc/workx/config.toml";
...
///
/// - system    `/etc/workx/requirements.toml` (Unix) or
///   `%ProgramData%\OpenAI\Workx\requirements.toml` (Windows)
/// - cloud:    enterprise-managed cloud config bundle requirements
/// - legacy:   `/etc/workx/managed_config.toml` (Unix) reinterpreted as
///   requirements.toml
/// - admin:    managed preferences (*)
///
/// For backwards compatibility, Unix continues to load
/// `/etc/workx/managed_config.toml` and map it to `requirements.toml`.
///
/// Configuration is built up from multiple layers in the following order:
///
/// - package:  optional default configuration supplied with the Workx package
/// - admin:    managed preferences (*)
/// - system    `/etc/workx/config.toml` (Unix) or
///   `%ProgramData%\OpenAI\Workx\config.toml` (Windows)
/// - cloud     enterprise-managed cloud config bundle fragments
/// - user      `${WORKX_HOME}/config.toml`
/// - profile   `${WORKX_HOME}/<name>.config.toml`, when selected
/// - cwd       `${PWD}/config.toml` (loaded but disabled when the directory is untrusted)
/// - tree      parent directories up to root looking for `./.workx/config.toml` (loaded but disabled when untrusted)
/// - repo      `$(git rev-parse --show-toplevel)/.workx/config.toml` (loaded but disabled when untrusted)
...
pub async fn load_config_layers_state(
    ...
    let packaged_defaults_layer = if let Some(file) = &overrides.packaged_defaults_path {
        ...
        let config = toml::from_str(raw_toml).map_err(|error| {
            io::Error::new(
                io::ErrorKind::InvalidData,
                format!("invalid embedded packaged defaults; this is a Workx build error: {error}"),
            )
        })?;
        ConfigLayerEntry::new_with_raw_toml(
            ...
            config,
            raw_toml.to_owned(),
            AbsolutePathBuf::from_absolute_path(workx_home)?,
        )
    };
    ...
    if !ignore_managed_requirements {
        if let Some(bundle) = cloud_config_bundle.get().await.map_err(io::Error::other)? {
            let cloud_config_base_dir = AbsolutePathBuf::from_absolute_path(workx_home)?;
        ...
        #[cfg(target_os = "macos")]
        {
            let managed_preferences_base_dir = AbsolutePathBuf::from_absolute_path(workx_home)?;
    ...
    let loaded_config_layers =
        layer_io::load_config_layers_internal(fs, workx_home, overrides.clone(), strict_config)
            .await?;
    ...
    // contain overrides.
    let active_user_file = overrides.user_config_path(workx_home)?;
    let base_user_file = AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, workx_home);
    ...
    if let Some(config) = managed_config_from_mdm {
        ...
        let raw_toml_base_dir = AbsolutePathBuf::from_absolute_path(workx_home)?;
...
fn system_requirements_toml_file() -> io::Result<AbsolutePathBuf> {
    AbsolutePathBuf::from_absolute_path(Path::new("/etc/workx/requirements.toml"))
}
...
pub fn has_local_managed_configuration(workx_home: &Path) -> io::Result<bool> {
...
fn has_local_managed_configuration_with_system_requirements_path(
    ...
    #[cfg(not(windows))]
    if layer_io::managed_config_default_path(workx_home).try_exists()? {
        return Ok(true);
    }
...
#[cfg(windows)]
fn windows_workx_system_dir() -> PathBuf {
...
fn requirements_layers_from_legacy_scheme(
    ...
    let workx_home = AbsolutePathBuf::from_absolute_path(workx_home)?;
...
fn legacy_requirements_to_toml_value(legacy: LegacyManagedConfigToml) -> io::Result<TomlValue> {
    ...
    if let Some(sandbox_mode) = sandbox_mode {
        ...
        // Allowing read-only is a requirement for Workx to function correctly.
...
impl ProjectTrustContext {
    ...
    fn root_checkout_hooks_folder_for_dir(&self, dir: &AbsolutePathBuf) -> Option<AbsolutePathBuf> {
        ...
        Some(repo_root.join(relative_dir).join(".workx"))
    }
}

fn project_layer_entry(
    dot_workx_folder: &AbsolutePathBuf,
    config: TomlValue,
    disabled_reason: Option<String>,
    hooks_config_folder_override: Option<AbsolutePathBuf>,
) -> ConfigLayerEntry {
    let source = ConfigLayerSource::Project {
        dot_workx_folder: dot_workx_folder.clone(),
    };
...
fn project_ignored_config_keys_warning(
    ...
    let config_path = dot_workx_folder.join(CONFIG_TOML_FILE);
...
async fn load_project_layers(
    ...
    for layer in discovered.layers {
        let config =
            resolve_relative_paths_in_config_toml(layer.config, layer.dot_workx_folder.as_path())?;
...
async fn discover_project_layers(
    ...
    let workx_home_abs = AbsolutePathBuf::from_absolute_path(workx_home)?;
    let workx_home_normalized =
        normalize_path(workx_home_abs.as_path()).unwrap_or_else(|_| workx_home_abs.to_path_buf());
    let mut dirs = cwd
        .ancestors()
        .scan(false, |done, a| {
            if *done {
    ...
    for dir in dirs {
        let dot_workx_abs = dir.join(".workx");
        let dot_workx_uri = PathUri::from_abs_path(&dot_workx_abs);
        if !fs
            .get_metadata(&dot_workx_uri, Default::default(), /*sandbox*/ None)
            .await
        ...
        let dot_workx_normalized =
            normalize_path(dot_workx_abs.as_path()).unwrap_or_else(|_| dot_workx_abs.to_path_buf());
        if dot_workx_abs == workx_home_abs || dot_workx_normalized == workx_home_normalized {
            continue;
        }
        let config_file = dot_workx_abs.join(CONFIG_TOML_FILE);
                ...
                let config: TomlValue = match toml::from_str(&contents) {
                    ...
                    Err(e) => {
                        ...
                        layers.push(DiscoveredProjectLayer {
                            dot_workx_folder: dot_workx_abs,
...
/// from a file like `/etc/workx/managed_config.toml` that has the same
    ...
    fn windows_system_requirements_toml_file_uses_expected_suffix() {
        ...
        assert!(
            windows_system_requirements_toml_file()
                .expect("requirements.toml path")
                .as_path()
                .ends_with(Path::new("OpenAI").join("Workx").join("requirements.toml"))
        );
    }
    ...
    fn windows_system_config_toml_file_uses_expected_suffix() {
        ...
        assert!(
            windows_system_config_toml_file()
                .expect("config.toml path")
                .as_path()
                .ends_with(Path::new("OpenAI").join("Workx").join("config.toml"))
        );
    }
}

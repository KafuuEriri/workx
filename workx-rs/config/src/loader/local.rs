...
pub async fn load_local_config_layers(
    ...
    load_local_config_layers_with_overrides(fs, workx_home, cwd, &LoaderOverrides::default()).await
}
...
pub(super) async fn load_local_config_layers_with_overrides(
    ...
    let requirements =
        local_requirements_layers(fs, workx_home.as_path(), overrides, loaded_managed.clone())
            .await?;
    ...
    append_legacy_config_layers(&mut config_layers, loaded_managed, &workx_home)?;
    ...
    for layer in layers {
        ...
        output.push(LocalTomlLayer {
            source: ConfigLayerSource::Project {
                dot_workx_folder: layer.dot_workx_folder.clone(),
            },
            base_dir: layer.dot_workx_folder,
            toml: config,
        });
        ...
        output.push(LocalTomlLayer {
            source: ConfigLayerSource::Project {
                dot_workx_folder: hooks_config_folder.clone(),
            },
...
async fn local_requirements_layers(
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        entries.extend(
            ...
            .await?
            .map(|layer| layer.with_base_dir(workx_home)),
        );
    }

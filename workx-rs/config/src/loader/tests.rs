...
use workx_file_system::CopyOptions;
use workx_file_system::CreateDirectoryOptions;
use workx_file_system::ExecutorFileSystemFuture;
use workx_file_system::FileMetadata;
use workx_file_system::FileSystemReadStream;
use workx_file_system::FileSystemSandboxContext;
use workx_file_system::GetMetadataOptions;
use workx_file_system::ReadDirectoryEntry;
use workx_file_system::ReadFileOptions;
use workx_file_system::RemoveOptions;
use workx_file_system::WalkOptions;
use workx_file_system::WalkOutcome;
use workx_file_system::WriteFileOptions;
...
fn project_environment_filters_preserve_only_trusted_credential_host_bindings() {
    ...
    for project_policy in [
        ...
        let actual = workx_protocol::shell_environment::populate_env(
            [
                ("GH_HOST", host),
...
async fn default_windows_managed_config_is_ignored_with_warning() {
    ...
    let managed_config_path = workx_home.join("managed_config.toml");
    std::fs::write(
        &managed_config_path,
        r#"
model = "legacy-model"
...
"#,
    ...
    std::fs::write(workx_home.join(CONFIG_TOML_FILE), r#"model = "user-model""#)
    ...
    let expected_warnings = vec![format!(
        "Ignoring deprecated managed config file at {}; WORKX_HOME/managed_config.toml is no longer supported on Windows. Use %ProgramData%\\OpenAI\\Workx\\requirements.toml for enforced settings or config.toml for defaults.",
...
fn windows_local_managed_configuration_ignores_legacy_file_but_detects_requirements() {
    ...
    let workx_home = tmp.path().join("workx-home");
    std::fs::create_dir_all(&workx_home).expect("create workx home");
    std::fs::write(workx_home.join("managed_config.toml"), "")
        .expect("write default legacy managed config");
...
async fn local_layers_keep_raw_paths_order_and_legacy_requirements() {
    ...
    let dot_workx = project.join(".workx");
    let system_dir = tmp.path().join("system");
    let managed_dir = tmp.path().join("managed");
    for dir in [&workx_home, &dot_workx, &system_dir, &managed_dir] {
    ...
    let user_file = workx_home.join(CONFIG_TOML_FILE);
    std::fs::write(
        &user_file,
        format!(

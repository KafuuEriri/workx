...
fn user_layer(workx_home: &TempDir, config: &str) -> ConfigLayerEntry {
...
fn stack(workx_home: &TempDir, user: &str, session: &str) -> ConfigLayerStack {
    ConfigLayerStack::new(
        vec![
            user_layer(workx_home, user),
            ConfigLayerEntry::new(
...
fn session_flags_can_reenable_user_disabled_path() {
    let workx_home = TempDir::new().expect("temp dir");
    let skill_path = workx_home.path().join("skills/demo/SKILL.md");
...
fn path_rule_disables_selected_path() {
    let workx_home = TempDir::new().expect("temp dir");
    let path = workx_home.path().join("disable-by-path/SKILL.md").abs();
...
fn later_name_rule_reenables_path_disabled_skill() {
    let workx_home = TempDir::new().expect("temp dir");
    let path = workx_home.path().join("reenable-by-name/SKILL.md").abs();
    let rules = SkillConfigRules {
        entries: vec![
            SkillConfigRule {
                selector: SkillConfigRuleSelector::Path(path.clone()),
...
fn later_path_rule_reenables_one_skill_disabled_by_name() {
    let workx_home = TempDir::new().expect("temp dir");
    let root = workx_home.path().join("reenable-by-path");

//! Types used to define loaded and effective Workx configuration values.
...
pub use workx_protocol::config_types::ModeKind;
pub use workx_protocol::config_types::Personality;
pub use workx_protocol::config_types::ServiceTier;
...
pub enum ResumeCwdMode {
    /// Use the directory where Workx was launched.
    Current,
...
/// Determine where Workx should store CLI auth credentials.
#[derive(Debug, Default, Copy, Clone, PartialEq, Eq, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "lowercase")]
pub enum AuthCredentialsStoreMode {
    #[default]
    /// Persist credentials in WORKX_HOME/auth.json.
    File,
    /// Persist credentials in the keyring. Fail if unavailable.
    Keyring,
    /// Use keyring when available; otherwise, fall back to a file in WORKX_HOME.
    Auto,
    /// Store credentials in memory only for the current process.
    Ephemeral,
}

/// Determine where Workx should store and read MCP credentials.
#[derive(Debug, Default, Copy, Clone, PartialEq, Eq, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "lowercase")]
...
pub struct AnalyticsConfigToml {
    /// When `false`, disables analytics across Workx product surfaces in this profile.
...
pub struct FeedbackConfigToml {
    /// When `false`, disables the feedback flow across Workx product surfaces.
    pub enabled: Option<bool>,
}
...
pub struct MemoriesToml {
    ...
    /// Minimum remaining percentage required in Workx rate-limit windows before memory startup runs.
...
pub struct AppConfig {
    /// When `false`, Workx does not surface this app.
    #[serde(default = "default_enabled")]
    pub enabled: bool,
...
pub struct OtelConfigToml {
    ...
    #[serde(default)]
    pub tool_result: workx_protocol::config_types::ToolResultLogConfig,
...
/// Fallback resize-reflow row cap when Workx cannot identify a terminal-specific scrollback size.
...
pub struct Tui {
    ...
    /// Use `/theme` in the TUI or see `$WORKX_HOME/themes` for custom themes.
    #[serde(default)]
    pub theme: Option<String>,

    /// Pet id to preselect in the terminal pet picker.
    ///
    /// Custom pet ids resolve against WORKX_HOME/pets/<pet-id>/pet.json.
    #[serde(default)]
    ...
    /// transcript exceeds this cap. Omit to use Workx's terminal-specific default. Set to `0` to
...
/// (primarily the Workx IDE extension). NOTE: these are different from
...
pub struct Notice {
    ...
    /// Tracks whether the user opted out of Workx-managed fast defaults.
    ...
    #[serde(rename = "hide_gpt-5.1-workx-max_migration_prompt")]
    pub hide_gpt_5_1_workx_max_migration_prompt: Option<bool>,
...
pub struct PluginMcpServerConfig {
    /// When `false`, Workx skips initializing this plugin MCP server.
    #[serde(default = "default_enabled")]
    pub enabled: bool,
...
pub struct MarketplaceConfig {
    /// Last time Workx successfully added or refreshed this marketplace.
    #[serde(default)]
    pub last_updated: Option<String>,
    /// Git revision Workx last successfully activated for this marketplace.
    #[serde(default)]

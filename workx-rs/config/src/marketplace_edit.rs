...
pub fn remove_user_marketplace(workx_home: &Path, marketplace_name: &str) -> std::io::Result<bool> {
    let outcome = remove_user_marketplace_config(workx_home, marketplace_name)?;
...
mod tests {
    ...
    fn record_user_marketplace_omits_runtime_update_metadata() {
        ...
        record_user_marketplace(workx_home.path(), "debug", &update).unwrap();

        let config: toml::Value =
            toml::from_str(&fs::read_to_string(workx_home.path().join(CONFIG_TOML_FILE)).unwrap())
                .unwrap();
    ...
    fn remove_user_marketplace_removes_requested_entry() {
        ...
        record_user_marketplace(workx_home.path(), "debug", &update).unwrap();
        record_user_marketplace(workx_home.path(), "other", &update).unwrap();

        let removed = remove_user_marketplace(workx_home.path(), "debug").unwrap();
    ...
    fn remove_user_marketplace_config_reports_case_mismatch() {
        ...
        let outcome = remove_user_marketplace_config(workx_home.path(), "Debug").unwrap();

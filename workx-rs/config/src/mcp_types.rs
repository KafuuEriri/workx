...
use workx_protocol::config_types::ToolExposureSurface;
...
/// OAuth client settings used when Workx launches an MCP OAuth flow.
#[derive(Serialize, Deserialize, Debug, Clone, PartialEq, Eq, JsonSchema)]
#[schemars(deny_unknown_fields)]
pub struct McpServerOAuthConfig {
    ...
    /// Fixed callback port that takes precedence over Workx's global OAuth callback port.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub callback_port: Option<u16>,
}

/// Authentication flow Workx attempts after resolving an HTTP MCP server's
...
pub struct McpServerConfig {
    ...
    /// Effective environment id for where Workx should start this MCP server.
    pub environment_id: String,

    /// When `false`, Workx skips initializing this MCP server.
    #[serde(default = "default_enabled")]
    pub enabled: bool,

    /// When `true`, `workx exec` exits with an error if this MCP server fails to initialize.

//! Schema-heavy configuration TOML types used by Workx.
...
use workx_model_provider_info::AMAZON_BEDROCK_RUNTIME_PROVIDER_ID;
...
use workx_utils_path::normalize_for_path_comparison;
...
/// Base config deserialized from ~/.workx/config.toml.
...
pub struct ConfigToml {
    ...
    /// sanctioned by Workx will likely degrade model performance.
    ...
    /// file: Use a file in the Workx home directory.
    ...
    /// Directory where Workx stores the SQLite state DB.
    /// Defaults to `$WORKX_SQLITE_HOME` when set. Otherwise uses `$WORKX_HOME`.
    pub sqlite_home: Option<AbsolutePathBuf>,

    /// Directory where Workx writes log files. Setting this value explicitly
    /// also enables the TUI text log in this directory.
    /// Defaults to `$WORKX_HOME/log`.
    ...
    /// Optional product SKU forwarded on host-owned Workx Apps MCP requests.
    ...
    /// directories for `.workx` folders. Defaults to [".git"] when unset.
    #[serde(default)]
...
pub use workx_protocol::protocol::RealtimeConversationVersion as RealtimeWsVersion;
pub use workx_protocol::protocol::RealtimeVoice;

...
use core_test_support::test_workx::TurnInputRequest;
...
async fn windows_exec_server_runs_with_native_shell_and_cwd() -> Result<()> {
    ...
    const PATCH_FILE: &str = "workx-apply-patch-smoke.txt";
    const COMMAND: &str = r#"if ((Get-Location).Path -ne 'C:\windows') { exit 1 }"#;
    const VERIFY_COMMAND: &str = r#"$path = Join-Path (Get-Location) 'workx-apply-patch-smoke.txt'; if (-not (Test-Path $path)) { exit 1 }; if ([IO.File]::ReadAllText($path) -ne "patched through unified exec`n") { exit 2 }; Remove-Item $path"#;
    ...
    WineExecServer
        .scope(|exec_server_url, _wine_prefix| async move {
            ...
            let environments = TurnEnvironmentSelections::new(
                ...
                vec![{
                    let cwd = PathUri::parse("file:///C:/workx-home")?;
            ...
            assert!(
                patch_end
                    ...
                    .contains_key(&std::path::PathBuf::from(format!(
                        r"C:\workx-home\apply-patch-smoke\nested\{PATCH_FILE}"
                    ))),

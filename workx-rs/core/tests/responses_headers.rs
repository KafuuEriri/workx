...
async fn responses_stream_includes_subagent_header_on_review() {
    ...
    let workx_home = TempDir::new().expect("failed to create TempDir");
...
async fn responses_stream_includes_turn_metadata_header_for_git_workspace_e2e() {
    ...
    let test = test_workx().build(&server).await.expect("build test workx");
    ...
    run_git(&[
        ...
        "add",
        "origin",
        "https://github.com/RonanXiao/workx.git",
    ]);
...
async fn responses_stream_redacts_git_remote_credentials_from_turn_metadata() {
    ...
    let test = test_workx()
        ...
        .build(&server)
        .await
        .expect("build test workx");
    let cwd = test.cwd_path();
    let remote_token = "test-git-secret-token";
    let credential_bearing_origin =
        "https://alice:test-git-secret-token@example.invalid/RonanXiao/workx.git";
    ...
    for (location, metadata) in [
        ...
        assert_eq!(
            remotes,
            &serde_json::json!({ "origin": "https://example.invalid/RonanXiao/workx.git" }),

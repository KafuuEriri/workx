...
use workx_core::shell::Shell;
use workx_core::shell::get_shell_by_model_provided_path;
...
use workx_extension_api::LoadUserInstructionsFuture;
...
use workx_protocol::protocol::RealtimeConversationVersion as RealtimeWsVersion;
...
use workx_utils_path_uri::PathConvention;
...
type WorkspaceSetup = dyn FnOnce(AbsolutePathBuf, Arc<dyn ExecutorFileSystem>) -> BoxFuture<'static, Result<()>>
    + Send;
const TEST_MODEL_WITH_EXPERIMENTAL_TOOLS: &str = "test-gpt-5.1-workx";
...
#[derive(Debug)]
pub struct TestEnv {
    environment: workx_exec_server::Environment,
...
impl TestEnv {
    ...
    pub async fn local_with_exec_server_url(exec_server_url: Option<String>) -> Result<Self> {
        ...
        let selection = match exec_server_url {
            Some(_) => TurnEnvironmentSelection {
                environment_id: workx_exec_server::REMOTE_ENVIRONMENT_ID.to_string(),
                cwd: PathUri::from_abs_path(&cwd),
                workspace_roots: vec![PathUri::from_abs_path(&cwd)],
                config: EnvironmentConfigState::FromThread,
            },
        ...
        let environment =
            workx_exec_server::Environment::create_for_tests(exec_server_url.clone())?;
        Ok(Self {
            environment,
            exec_server_url,
            cwd,
    ...
    pub fn environment(&self) -> &workx_exec_server::Environment {
        &self.environment
    }
...
pub async fn test_env() -> Result<TestEnv> {
    match test_environment() {
        remote_env @ (TestEnvironment::Docker { .. } | TestEnvironment::WineExec) => {
            ...
            let environment =
                workx_exec_server::Environment::create_for_tests(Some(websocket_url.clone()))?;
            let cwd = remote_env
...
impl TestWorkxBuilder {
    ...
    pub fn with_auth(mut self, auth: WorkxAuth) -> Self {
        self.auth = auth;
        self
    }
    ...
    pub async fn build(&mut self, server: &wiremock::MockServer) -> anyhow::Result<TestWorkx> {
    ...
    ///
    /// With no remote test configuration, or with `WORKX_TEST_ENVIRONMENT=local`, this uses a
    /// temporary local environment just like [`Self::build`]. `WORKX_TEST_ENVIRONMENT=docker` or
    /// `WORKX_TEST_ENVIRONMENT=wine-exec` selects the remote exec server configured by
    /// `WORKX_TEST_REMOTE_EXEC_SERVER_URL`; the legacy `WORKX_TEST_REMOTE_ENV` Docker-container
    ...
    pub async fn restart(
        ...
        previous.workx.shutdown_and_wait().await?;
    ...
    async fn build_with_home_and_base_url(
        ...
        #[cfg(target_os = "linux")]
        let workx_linux_sandbox_exe = Some(
            crate::find_workx_linux_sandbox_exe()
                .context("should find binary for workx-linux-sandbox")?,
        );
        #[cfg(not(target_os = "linux"))]
        let workx_linux_sandbox_exe = None;
        let local_runtime_paths = workx_exec_server::ExecServerRuntimePaths::new(
            std::env::current_exe()?,
            workx_linux_sandbox_exe,
        )?;
        let environment_manager = Arc::new(if include_local_environment {
            workx_exec_server::EnvironmentManager::create_for_tests_with_local(
                exec_server_url,
                local_runtime_paths,
            )
            .await
        } else {
            workx_exec_server::EnvironmentManager::create_for_tests(
                exec_server_url,
    ...
    async fn build_from_config(
        ...
        environment_manager: Arc<workx_exec_server::EnvironmentManager>,
    ) -> anyhow::Result<TestWorkx> {
        let auth = self.auth.clone();
        let state_db = workx_core::init_state_db(&config).await;
        let thread_store = thread_store_from_config(&config, state_db.clone());
        let installation_id = resolve_installation_id(&config.workx_home).await?;
        let user_instructions_provider =
            self.user_instructions_provider.clone().unwrap_or_else(|| {
                Arc::new(WorkxHomeUserInstructionsProvider::new(
                    config.workx_home.clone(),
                ))
            });
        let auth_manager = workx_core::test_support::auth_manager_from_auth_with_home(
            auth.clone(),
        ...
        let models_manager = self
            .models_manager
            .clone()
            .unwrap_or_else(|| workx_core::build_models_manager(&config, auth_manager.clone()));
        let thread_manager = ThreadManager::new(
            ...
            workx_core::local_agent_graph_store_from_state_db(state_db.as_ref()),
            installation_id,
        ...
        let new_conversation = match (resume_from, user_shell_override) {
            (Some(path), Some(user_shell_override)) => {
                ...
                Box::pin(
                    workx_core::test_support::resume_thread_from_rollout_with_user_shell_override(
            ...
            (None, Some(user_shell_override)) => {
                Box::pin(
                    workx_core::test_support::start_thread_with_user_shell_override(
                        thread_manager.as_ref(),
                        config.clone(),
                        user_shell_override,
                        self.supports_openai_form_elicitation,
    ...
    async fn prepare_config(
        ...
        if let Ok(path) = workx_utils_cargo_bin::cargo_bin("workx") {
            config.workx_self_exe = Some(path);
        } else if let Ok(path) = workx_utils_cargo_bin::cargo_bin("workx-exec") {
            // `workx-exec` also supports `--workx-run-as-apply-patch`, so use it
        ...
        {
            let workx = bin_dir.join("workx");
            let workx_exec = bin_dir.join("workx-exec");
            if workx.is_file() {
                config.workx_self_exe = Some(workx);
            } else if workx_exec.is_file() {
                config.workx_self_exe = Some(workx_exec);
            }
        }
...
pub struct TestWorkx {
    pub home: Arc<TempDir>,
    pub cwd: Arc<TempDir>,
    pub workx: Arc<WorkxThread>,
...
impl TestWorkx {
    ...
    pub fn workx_home_path(&self) -> &Path {
        self.config.workx_home.as_path()
    }
    ...
    pub async fn submit_text_turn(&self, prompt: &str) -> Result<()> {
        ...
        wait_for_event(&self.workx, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
        Ok(())
    }
    ...
    async fn submit_turn_with_context(
        ...
        let turn_id = wait_for_event_match(&self.workx, |event| match event {
...
pub struct TestWorkxHarness {
    server: MockServer,
    test: TestWorkx,
}

impl TestWorkxHarness {
    pub async fn with_builder(mut builder: TestWorkxBuilder) -> Result<Self> {
        let server = start_mock_server().await;
        let test = builder.build(&server).await?;
        Ok(Self { server, test })
    }

    pub async fn with_auto_env_builder(mut builder: TestWorkxBuilder) -> Result<Self> {
        let server = start_mock_server().await;
...
pub fn test_workx() -> TestWorkxBuilder {
    TestWorkxBuilder {
        ...
        auth: WorkxAuth::from_api_key("dummy"),
        pre_build_hooks: vec![],

...
use workx_utils_cargo_bin::CargoBinError;
...
pub use workx_core::test_support::TestWorkxResponsesRequestKind;
pub use workx_core::test_support::responses_metadata;
use workx_utils_absolute_path::AbsolutePathBuf;
pub use workx_utils_absolute_path::test_support::PathBufExt;
pub use workx_utils_absolute_path::test_support::PathExt;
...
fn enable_deterministic_unified_exec_process_ids_for_tests() {
    workx_core::test_support::set_thread_manager_test_mode(/*enabled*/ true);
    workx_core::test_support::set_deterministic_process_ids(/*enabled*/ true);
}

#[ctor]
fn configure_arg0_dispatch_for_test_binaries() {
    let _ = TEST_ARG0_PATH_ENTRY.get_or_init(workx_arg0::arg0_dispatch);
}
...
fn configure_insta_workspace_root_for_snapshot_tests() {
    ...
    let workspace_root = workx_utils_cargo_bin::repo_root()
        .ok()
        .map(|root| root.join("workx-rs"));
...
pub fn test_tmp_path() -> AbsolutePathBuf {
    test_absolute_path_with_windows("/tmp", Some(r"C:\Users\workx\AppData\Local\Temp"))
}
...
/// avoids clobbering a developer’s real `~/.workx`.
pub async fn load_default_config_for_test(workx_home: &TempDir) -> Config {
...
fn default_test_overrides() -> ConfigOverrides {
    ConfigOverrides {
        workx_linux_sandbox_exe: Some(
            find_workx_linux_sandbox_exe().expect("should find binary for workx-linux-sandbox"),
        ),
...
#[cfg(target_os = "linux")]
pub fn find_workx_linux_sandbox_exe() -> Result<PathBuf, CargoBinError> {
    ...
    workx_utils_cargo_bin::cargo_bin("workx-linux-sandbox")
}

pub async fn wait_for_event<F>(
    workx: &WorkxThread,
    predicate: F,
) -> workx_protocol::protocol::EventMsg
where
    F: FnMut(&workx_protocol::protocol::EventMsg) -> bool,
{
    use tokio::time::Duration;
    wait_for_event_with_timeout(workx, predicate, Duration::from_secs(1)).await
}

/// Waits for a configured MCP server to finish startup and requires it to be ready.
pub async fn wait_for_mcp_server(workx: &WorkxThread, server_name: &str) -> anyhow::Result<()> {
...
pub async fn submit_thread_settings(
    workx: &WorkxThread,
    thread_settings: workx_protocol::protocol::ThreadSettingsOverrides,
) -> anyhow::Result<()> {
    use tokio::time::Duration;
    ...
    let submission_id = workx.submit(Op::ThreadSettings { thread_settings }).await?;
    loop {
        let ev = timeout(Duration::from_secs(10), workx.next_event())
            .await
...
pub async fn wait_for_event_match<T, F>(workx: &WorkxThread, matcher: F) -> T
where
    F: Fn(&workx_protocol::protocol::EventMsg) -> Option<T>,
{
    let ev = wait_for_event(workx, |ev| matcher(ev).is_some()).await;
    ...
    F: FnMut(&workx_protocol::protocol::EventMsg) -> bool,
{
    ...
    loop {
        ...
        let ev = timeout(wait_time.max(Duration::from_secs(10)), workx.next_event())
            .await
...
pub fn sandbox_env_var() -> &'static str {
    workx_core::spawn::WORKX_SANDBOX_ENV_VAR
}

pub fn sandbox_network_env_var() -> &'static str {
    workx_core::spawn::WORKX_SANDBOX_NETWORK_DISABLED_ENV_VAR
}

pub fn format_with_current_shell(command: &str) -> Vec<String> {
    workx_core::shell::default_user_shell().derive_exec_args(command, /*use_login_shell*/ true)
}
...
pub fn format_with_current_shell_non_login(command: &str) -> Vec<String> {
    workx_core::shell::default_user_shell()
...
pub fn stdio_server_bin() -> Result<String, CargoBinError> {
    workx_utils_cargo_bin::cargo_bin("test_stdio_server").map(|p| p.to_string_lossy().to_string())
}
...
#[macro_export]
macro_rules! workx_linux_sandbox_exe_or_skip {
    () => {{
        #[cfg(target_os = "linux")]
        {
            match $crate::find_workx_linux_sandbox_exe() {
                Ok(path) => Some(path),
                Err(err) => {
                    eprintln!("workx-linux-sandbox binary not available, skipping test: {err}");
                    return;
                }
            }
        }

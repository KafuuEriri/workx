...
use crate::test_workx::TestWorkx;
...
pub fn zsh_fork_runtime(test_name: &str) -> Result<Option<ZshForkRuntime>> {
    ...
    let Ok(main_execve_wrapper_exe) = workx_utils_cargo_bin::cargo_bin("workx-execve-wrapper")
    else {
        eprintln!("skipping {test_name}: unable to resolve `workx-execve-wrapper` binary");
        return Ok(None);
    };
...
pub fn zsh_fork_test_builder(
    ...
    test_workx().with_config(move |config| {

...
//! request payload that Workx would send to the model and assert that the
...
fn network_disabled() -> bool {
    std::env::var(WORKX_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok()
}
...
fn seed_first_checkpoint_harness_metadata(path: &Path, retained_text: &str) -> Result<()> {
    ...
    for envelope in replacement_history {
        envelope.metadata = Some(WorkxHarnessMetadata::default());
    }
...
async fn start_test_conversation(
    server: &MockServer,
    model: Option<&str>,
) -> (Arc<TempDir>, Config, Arc<ThreadManager>, Arc<WorkxThread>) {
    ...
    (test.home, test.config, test.thread_manager, test.workx)
}

async fn user_turn(conversation: &Arc<WorkxThread>, text: &str) {
    conversation
...
async fn compact_conversation(conversation: &Arc<WorkxThread>) {
    conversation
        .submit(Op::Compact)
        .await
...
fn fetch_conversation_path(conversation: &Arc<WorkxThread>) -> std::path::PathBuf {
    conversation.rollout_path().expect("rollout path")
}

async fn shutdown_conversation(conversation: &Arc<WorkxThread>) {
    conversation
        .shutdown_and_wait()
        .await
...
async fn resume_conversation(
    ...
    let auth_manager = workx_core::test_support::auth_manager_from_auth(
        workx_login::WorkxAuth::from_api_key("dummy"),
    );

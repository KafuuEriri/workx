...
use core_test_support::test_workx::turn_permission_fields;
...
use workx_core_plugins::store::PluginStore;
...
use workx_model_provider_info::OPENAI_PROVIDER_ID;
...
use workx_skills_extension::HostSkillsLoadInput;
use workx_skills_extension::SkillsExtensionConfig;
use workx_skills_extension::install;
...
const SAMPLE_PLUGIN_APP_NAMESPACE: &str = "mcp__workx_apps__google_calendar";
...
async fn build_analytics_plugin_test_workx(
    server: &MockServer,
    workx_home: Arc<TempDir>,
) -> Result<TestWorkx> {
    let chatgpt_base_url = server.uri();
    let mut builder = test_workx()
        .with_home(workx_home)
...
async fn build_apps_enabled_plugin_test_workx(
    server: &MockServer,
    workx_home: Arc<TempDir>,
...
async fn persisted_remote_plugin_command_attribution_flows_through_turn_context(
    ...
    skip_if_remote!(
        Ok(()),
        "remote plugin attribution fixture uses a local Workx home cache"
    );

    let server = start_mock_server().await;
    let workx_home = Arc::new(TempDir::new()?);
    let script_path = write_remote_plugin_script_and_config(workx_home.as_ref());
    std::fs::write(
        &script_path,
        r#"printf '%s' '{"version":1,"measurements":[{"name":"files_scanned","value":7}]}' > "$WORKX_PLUGIN_METRICS_OUTPUT"
"#,
    )?;
    ...
    let mut builder = builder
        .with_home(Arc::clone(&workx_home))
        .with_auth(WorkxAuth::create_dummy_chatgpt_auth_for_testing())
        .with_model("gpt-5.2")
        .with_config(move |config| config.chatgpt_base_url = chatgpt_base_url);
    let test_workx = builder.build_with_auto_env(&server).await?;
    let workx = Arc::clone(&test_workx.workx);
    let cwd = test_workx.config.cwd.clone();
    let session_model = test_workx.session_configured.model.clone();
    let (sandbox_policy, permission_profile) =
        turn_permission_fields(PermissionProfile::read_only(), cwd.as_path());
    workx
        .start_or_steer_turn(
            TurnInputRequest::user_input(vec![workx_protocol::user_input::UserInput::Text {
    ...
    let begin = wait_for_event_match(&workx, |event| match event {
        EventMsg::ExecCommandBegin(event) if event.call_id == call_id => Some(event.clone()),
        _ => None,
    })
    .await;
    let end = wait_for_event_match(&workx, |event| match event {
    ...
    let measurement = wait_for_analytics_event(&server, "workx_plugin_measurement_event").await;
    assert_eq!(
        serde_json::json!({
...
async fn agent_plugin_skills_use_shared_catalog_and_direct_child_discovery() -> Result<()> {
    ...
    .await;
    let workx_home = Arc::new(TempDir::new()?);
    let skill_path = dunce::canonicalize(write_agent_plugin_skill_plugin(workx_home.as_ref()))?;
    let mut builder = test_workx()
        .with_home(Arc::clone(&workx_home))
    ...
    test_workx
        ...
        .await?;
    let warning = wait_for_event(&test_workx.workx, |ev| {
        matches!(
            ev,
    ...
    .await;
    wait_for_event(&test_workx.workx, |ev| {
...
#[test_case("WORKX", true, Some("native review skill"); "native skill wins over migrated command")]
...
async fn plugin_skill_product_policy_and_migrated_command_precedence_reach_agent_turns(
    ...
    .await;
    let workx_home = Arc::new(TempDir::new()?);
    let plugin_root = write_sample_plugin_manifest_and_config(workx_home.as_ref());
    ...
    if include_migrated_command {
        let migrated_skill_dir =
            plugin_root.join(".workx-plugin/migrated-command-skills/source-command-review");
...
async fn legacy_plugin_skill_prompt_remains_complete() -> Result<()> {
    ...
    .await;
    let workx_home = Arc::new(TempDir::new()?);
    let skill_path = write_plugin_skill_plugin(workx_home.as_ref());
...
"#,
    ...
    for (marketplace, skill) in [
        ...
        std::fs::create_dir_all(root.join(".workx-plugin"))?;
        std::fs::write(
            root.join(".workx-plugin/plugin.json"),
            r#"{"name":"sites"}"#,
        )?;
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn agent_plugin_root_mcp_stdio_tool_round_trip_expands_reserved_paths_and_workx_env_overlay()
-> Result<()> {
    skip_if_no_network!(Ok(()));
    let server = start_mock_server().await;
    ...
    .await;
    let workx_home = Arc::new(TempDir::new()?);
    write_agent_plugin_skill_plugin(workx_home.as_ref());
    ...
    let mut builder = test_workx().with_home(Arc::clone(&workx_home));
    let test_workx = builder.build_with_remote_and_local_env(&server).await?;
    wait_for_mcp_server(&test_workx.workx, "agent").await?;
    let data_root = dunce::canonicalize(
        std::fs::read_dir(workx_home.path().join("plugins/data/agent-plugins"))?
            .next()
    ...
    test_workx
        ...
        .await?;
    let end = wait_for_event(&test_workx.workx, |event| {
        matches!(event, EventMsg::McpToolCallEnd(_))
    })
    .await;
    let overlay_end = wait_for_event(&test_workx.workx, |event| {
        matches!(event, EventMsg::McpToolCallEnd(_))
    })
    .await;
    wait_for_event(&test_workx.workx, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
...
async fn curated_plugin_skills_follow_auth_switch() -> Result<()> {
    ...
    const FIXTURES: &[Fixture] = &[
        ...
        Fixture {
            ...
            target_auth: TargetAuth::NoWorkxAuth,
            target_model_provider_id: AMAZON_BEDROCK_PROVIDER_ID,
            expected_target_loaded_plugin_skills: &[API_CURATED_PLUGIN_SKILL],
            expected_target_skill_description: "api description before",
        },
    ...
    async fn loaded_plugin_skills_for_config(test_workx: &TestWorkx, config: &Config) -> String {
        let plugins_input = config.plugins_config_input();
        let plugins_manager = test_workx.thread_manager.plugins_manager();
        ...
        let skills_snapshot = test_workx
            .thread_manager
            .skills_service()
...
"#,
        ...
        let mut builder = test_workx()
            ...
            .with_auth(WorkxAuth::create_dummy_chatgpt_auth_for_testing());
        let test_workx = builder.build_with_auto_env(&server).await?;
        let initial_skills = loaded_plugin_skills_for_config(&test_workx, &test_workx.config).await;
        ...
        let expected_auth_mode = match fixture.target_auth {
            ...
            TargetAuth::ApiKey => {
                workx_login::login_with_api_key(
                    workx_home.path(),
                    "test-api-key",
                    workx_login::AuthCredentialsStoreMode::File,
                    workx_login::AuthKeyringBackendKind::default(),
                )?;
                test_workx.thread_manager.auth_manager().reload().await;
                Some(AuthMode::ApiKey)
            }
            TargetAuth::BedrockApiKey => {
                workx_login::login_with_bedrock_api_key(
                    workx_home.path(),
            ...
            TargetAuth::NoWorkxAuth => {
                test_workx.thread_manager.auth_manager().logout().await?;
                None
            }
        };
        assert_eq!(
            test_workx.thread_manager.auth_manager().get_api_auth_mode(),
            expected_auth_mode
        );
        test_workx.thread_manager.skills_service().clear_cache();
        let mut target_config = test_workx.config.clone();
        target_config.model_provider_id = fixture.target_model_provider_id.to_string();
        let target_skills = loaded_plugin_skills_for_config(&test_workx, &target_config).await;
        assert_loaded_plugin_skills(
            fixture.name,
            "target config",
            &target_skills,
...
async fn explicit_plugin_mentions_use_apps_for_chatgpt_dual_surface_plugins(
    ...
    write_plugin_skill_plugin(workx_home.as_ref());
    write_plugin_mcp_plugin(workx_home.as_ref(), &rmcp_test_server_bin);
    write_plugin_app_plugin(workx_home.as_ref());
    ...
    let test_workx =
        build_apps_enabled_plugin_test_workx(&server, workx_home, apps_server.chatgpt_base_url)
            .await?;
    let workx = Arc::clone(&test_workx.workx);
    wait_for_mcp_server(&workx, WORKX_APPS_MCP_SERVER_NAME).await?;

    workx
        .start_or_steer_turn(TurnInputRequest::user_input(vec![
            workx_protocol::user_input::UserInput::Mention {
                name: "sample".into(),
...
async fn explicit_plugin_mentions_keep_non_conflicting_mcp_for_chatgpt_auth() -> Result<()> {
    ...
    write_plugin_skill_plugin(workx_home.as_ref());
    write_plugin_mcp_plugin(workx_home.as_ref(), &rmcp_test_server_bin);
    write_plugin_app_plugin_with_name(workx_home.as_ref(), "sample_app");

    let test_workx =
        build_apps_enabled_plugin_test_workx(&server, workx_home, apps_server.chatgpt_base_url)
            .await?;
    let workx = Arc::clone(&test_workx.workx);
    wait_for_mcp_server(&workx, "sample").await?;
...
async fn system_marketplace_plugin_honors_layered_activation_and_mcp_policy(
    ...
    write_plugin_mcp_plugin(workx_home.as_ref(), &stdio_server_bin()?);
    let user_config_path = workx_home.path().join("config.toml");
    ...
    let system_config_path = workx_home.path().join("system.toml");
    ...
    let requirements_path = workx_home.path().join("requirements.toml");
    std::fs::write(
        &requirements_path,
        format!(
    ...
    std::fs::create_dir_all(project.path().join(".workx"))?;
    std::fs::write(
        project.path().join(".workx/config.toml"),
        format!(
            "[plugins.\"{SAMPLE_PLUGIN_CONFIG_NAME}\"]\nenabled = {plugin_enabled}\n[plugins.\"{SAMPLE_PLUGIN_CONFIG_NAME}\".mcp_servers.sample]\nenabled = {project_enabled}\ndisabled_tools = [\"echo-tool\"]\n"
        ),
    )?;
    set_project_trust_level(workx_home.path(), project.path(), trust_level)?;
    ...
    let startup = wait_for_event_match(&test.workx, |event| match event {
...
async fn explicitly_requested_mcp_waits_for_startup(request: ExplicitMcpRequest) -> Result<()> {
    ...
    let skill_path = dunce::canonicalize(write_plugin_skill_plugin(workx_home.as_ref()))?;
    write_plugin_mcp_plugin(workx_home.as_ref(), &rmcp_test_server_bin);
    write_plugin_app_plugin(workx_home.as_ref());
    let initialize_barrier = block_plugin_mcp_startup(workx_home.as_ref(), &rmcp_test_server_bin);
...
async fn explicit_plugin_mentions_track_plugin_used_analytics() -> Result<()> {
    ...
    let workx_home = Arc::new(TempDir::new()?);
    write_plugin_skill_plugin(workx_home.as_ref());
    let test_workx = build_analytics_plugin_test_workx(&server, workx_home).await?;
    let workx = Arc::clone(&test_workx.workx);
    ...
    let event = wait_for_analytics_event(&server, "workx_plugin_used").await;
    ...
    assert_eq!(
        ...
        serde_json::json!(workx_login::default_client::originator().value)
    );
...
async fn explicit_plugin_skill_invocation_tracks_remote_plugin_id() -> Result<()> {
    ...
    let workx_home = Arc::new(TempDir::new()?);
    let skill_path = dunce::canonicalize(write_remote_plugin_skill_plugin(workx_home.as_ref()))?;
    persist_sample_remote_plugin_id(workx_home.as_ref());
    let test_workx = build_analytics_plugin_test_workx(&server, workx_home).await?;
    let workx = Arc::clone(&test_workx.workx);
...
async fn implicit_plugin_skill_invocation_tracks_remote_plugin_id(
    ...
    let server = start_mock_server().await;
    let workx_home = Arc::new(TempDir::new()?);
    let skill_path = write_remote_plugin_skill_plugin(workx_home.as_ref());
    persist_sample_remote_plugin_id(workx_home.as_ref());
...
async fn wait_for_analytics_event(server: &MockServer, event_type: &str) -> serde_json::Value {
    ...
    loop {
        ...
        if let Some(event) = requests
            .into_iter()
            .filter(|request| request.url.path() == "/workx/analytics-events/events")
            .find_map(|request| {

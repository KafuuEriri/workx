...
use workx_core::context::NodeReplReviewEvidence;
use workx_extension_api::ExtensionData;
use workx_extension_api::ExtensionRegistryBuilder;
use workx_extension_api::ToolCallOutcome;
use workx_extension_api::ToolContributor;
use workx_extension_api::ToolFinishInput;
use workx_extension_api::ToolLifecycleContributor;
use workx_extension_api::ToolLifecycleFuture;
use workx_extension_api::ToolStartInput;
...
use workx_tools::FunctionCallError;
use workx_tools::JsonToolOutput;
use workx_tools::ResponsesApiNamespace;
use workx_tools::ResponsesApiNamespaceTool;
use workx_tools::ToolCall;
use workx_tools::ToolExecutor;
use workx_tools::ToolExecutorFuture;
use workx_tools::ToolName;
use workx_tools::ToolOutput;
use workx_tools::ToolPayload;
use workx_tools::ToolSpec;
use workx_web_search_extension::install as install_web_search_extension;
...
async fn run_code_mode_turn_with_config(
    ...
    run_code_mode_turn_with_model_and_config(server, prompt, code, "test-gpt-5.1-workx", configure)
        .await
}
...
async fn run_code_mode_turn_with_model_and_config(
    ...
    let builder = test_workx().with_model(model).with_config(move |config| {
...
async fn run_unavailable_code_mode_turn(
    ...
    loop {
        match wait_for_event(&test.workx, |_| true).await {
...
async fn missing_process_host_falls_back_to_direct_tools_and_warns_once() -> Result<()> {
    ...
    let mut builder = test_workx()
        .with_model("test-gpt-5.1-workx")
        .with_code_mode_host_program("workx-code-mode-host-does-not-exist".into())
        .with_config(|config| {
            config
                .features
    ...
    assert!(
        first_warnings.iter().any(|warning| {
            warning.contains("Code Mode is unavailable")
                && warning.contains("workx-code-mode-host-does-not-exist")
        }),
...
async fn missing_process_host_keeps_code_mode_only_and_fails_closed() -> Result<()> {
    ...
    assert!(
        output.contains("workx-code-mode-host-does-not-exist"),
...
"#,
    ...
    let auth = WorkxAuth::from_api_key("dummy");
    let auth_manager = workx_core::test_support::auth_manager_from_auth(auth.clone());
    ...
    assert_eq!(
        search_body["model"],
        serde_json::json!("test-gpt-5.1-workx")
    );
    assert_eq!(
...
async fn run_code_mode_turn_with_rmcp(
    ...
    run_code_mode_turn_with_rmcp_model(server, prompt, code, "test-gpt-5.1-workx").await
}
...
async fn run_code_mode_turn_with_rmcp_config(
    ...
    let mut builder = test_workx().with_model(model).with_config(move |config| {
    ...
    let test = builder.build(server).await?;
    wait_for_mcp_server(&test.workx, "rmcp").await?;
...
async fn code_mode_exec_holds_captured_result_during_elicitation() -> Result<()> {
    ...
    assert_eq!(
        test.workx.increment_out_of_band_elicitation_count().await?,
        1
    );
    assert_eq!(
        test.workx.increment_out_of_band_elicitation_count().await?,
    ...
    let release_elicitation = async {
        ...
        assert_eq!(
            test.workx.decrement_out_of_band_elicitation_count().await?,
            1
        );
        ...
        assert!(
            metadata["executed_tool_calls"]
                ...
                .is_some_and(|calls| calls.iter().any(|call| call["arguments"]
                    .get("_workx_executed_tool_call_truncated")
                    .is_some()))
        );
    } else {
        assert_eq!(
...
const tool = ALL_TOOLS.find(
  ({ name }) => name === "mcp__workx_apps__calendar_timezone_option_99"
);
if (!tool) {
...
"#,
        visible_tool_name = "mcp__workx_apps__calendar_timezone_option_99",
...
async fn code_mode_wait_timeout_reconnects_on_next_exec() -> Result<()> {
    ...
    test.workx
        ...
        .await?;
    wait_for_event_match(&test.workx, |event| match event {
...
impl ToolLifecycleContributor for InterruptedNestedToolObserver {
    fn on_tool_start<'a>(&'a self, input: ToolStartInput<'a>) -> ToolLifecycleFuture<'a> {
        Box::pin(async move {
            let workx_extension_api::ToolCallSource::CodeMode { cell_id, .. } = input.source else {
                return;
            };
    ...
    fn on_tool_finish<'a>(&'a self, input: ToolFinishInput<'a>) -> ToolLifecycleFuture<'a> {
        Box::pin(async move {
            if input.tool_name.name != "test_sync_tool"
                || !matches!(
                    input.source,
                    workx_extension_api::ToolCallSource::CodeMode { .. }
                )
            {
                return;
            }
...
async fn code_mode_node_repl_screenshots_can_be_captured_without_guardian_transcript_flags(
    ...
    let test = builder.build_with_auto_env(&server).await?;
    core_test_support::wait_for_mcp_server(&test.workx, repl_server).await?;
    let evidence = test
        .workx
...
async fn code_mode_node_repl_image_flag_without_enhanced_stays_disabled(
    ...
    let test = builder.build_with_auto_env(&server).await?;
    wait_for_mcp_server(&test.workx, repl_server).await?;
            ...
            if reviewer_constraint.is_some_and(|value| value != "large_prompt") || check_detail {
                ...
                if reviewer_token_budget {
                    ...
                    reviewer.input_modalities =
                        vec![workx_protocol::openai_models::InputModality::Text];
                } else {
...
"#,
        ...
        test.workx
            .start_or_steer_turn(
                ...
                .with_thread_settings(ThreadSettingsOverrides {
                    environments: Some(workx_protocol::protocol::TurnEnvironmentSelections::new(
                        cwd,
                        Vec::new(),
                    )),
        ...
        let turn_id = wait_for_event_match(&test.workx, |event| match event {
        ...
        wait_for_event(&test.workx, |event| match event {
...
async fn code_mode_can_call_hidden_dynamic_tools() -> Result<()> {
    ...
    let code = r#"
const tool = ALL_TOOLS.find(({ name }) => name === "workx_app__hidden_dynamic_tool");
const out = await tools.workx_app__hidden_dynamic_tool({ city: "Paris" });
text(
  JSON.stringify({
    name: tool?.name ?? null,
    description: tool?.description ?? null,
...
"#;
    ...
    .await;
    assert_eq!(request.namespace.as_deref(), Some("workx_app"));
    ...
    assert_eq!(
        parsed.get("name"),
        Some(&Value::String("workx_app__hidden_dynamic_tool".to_string()))
    );
    assert_eq!(
        parsed.get("out"),
    ...
    assert!(
        parsed
            ...
            .and_then(Value::as_str)
            .is_some_and(|description| {
                description.contains("Workx app tools.")
                    && description.contains("A hidden dynamic tool.")
                    && description.contains("declare const tools:")
                    && description.contains("workx_app__hidden_dynamic_tool(args:")
            })
    );

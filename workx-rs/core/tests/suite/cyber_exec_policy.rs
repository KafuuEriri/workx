...
async fn submit_model_turn(test: &TestWorkx, model: &str, prompt: &str) -> Result<()> {
...
async fn saved_prefix_only_bypasses_guardian_for_general_models(
    ...
    let builder = match shell_backend {
        ShellBackend::Standard => test_workx(),
...
async fn cyber_model_user_approval_never_offers_a_reusable_prefix() -> Result<()> {
    ...
    let test = builder.build_with_auto_env(&server).await?;
    let policy_path = test.workx_home_path().join("rules/default.rules");
    ...
    let EventMsg::ExecApprovalRequest(approval) = wait_for_event(&test.workx, |event| {
        matches!(
            event,

...
async fn workx_returns_json_result_for_gpt5() -> anyhow::Result<()> {
    workx_returns_json_result("gpt-5.4".to_string()).await
}

async fn workx_returns_json_result(model: String) -> anyhow::Result<()> {
    ...
    let TestWorkx { workx, config, .. } = test_workx().build(&server).await?;
    ...
    let message = wait_for_event(&workx, |ev| matches!(ev, EventMsg::AgentMessage(_))).await;

...
fn configure_priority_role(config: &mut Config) {
    ...
    let role_path = config.workx_home.join("priority-worker.toml");
...
async fn wait_for_turn_complete(thread: &workx_core::WorkxThread) {
...
async fn evicted_role_subagent_uses_root_service_tier_after_reload() -> Result<()> {
    ...
    assert_eq!(
        ...
        test.workx.config_snapshot().await.service_tier,

...
async fn user_turn_includes_skill_instructions() -> Result<()> {
    ...
    core_test_support::wait_for_event(test.workx.as_ref(), |event| {
        matches!(event, workx_protocol::protocol::EventMsg::TurnComplete(_))
    })
    .await;
...
async fn idle_user_turn_includes_skill_instructions_in_the_first_request() -> Result<()> {
    ...
    let mut builder = test_workx().with_workspace_setup(move |cwd, fs| async move {

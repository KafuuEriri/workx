...
const POLICY_PATH_FOR_TEST: &str = "/workx/policy/path";
const SNAPSHOT_PATH_FOR_TEST: &str = "/workx/snapshot/path";
const SNAPSHOT_MARKER_VAR: &str = "WORKX_SNAPSHOT_POLICY_MARKER";
...
async fn wait_for_snapshot(workx_home: &Path) -> Result<PathBuf> {
    let snapshot_dir = workx_home.join("shell_snapshots");
...
async fn run_snapshot_command_with_options(
    ...
    let builder = test_workx().with_config(move |config| {
        config
            .features
    ...
    let workx_home = test.home.path().to_path_buf();
    ...
    let begin = wait_for_event_match(&workx, |ev| match ev {
        EventMsg::ExecCommandBegin(ev) if ev.call_id == call_id => Some(ev.clone()),
        _ => None,
    })
    .await;
    let snapshot_path = wait_for_snapshot(&workx_home).await?;
    let snapshot_content = fs::read_to_string(&snapshot_path).await?;

    let end = wait_for_event_match(&workx, |ev| match ev {
...
async fn run_tool_turn_on_harness(
    ...
    wait_for_event_match(&workx, |ev| match ev {
        EventMsg::ExecCommandBegin(ev) if ev.call_id == call_id => Some(ev.clone()),
        _ => None,
    })
    .await;
...
#[cfg(unix)]
fn shell_snapshot_v2_prewarm_builder(profile_home: &Path) -> TestWorkxBuilder {
...
#[cfg(unix)]
async fn run_no_shell_turn(harness: &TestWorkxHarness) -> Result<()> {
...
async fn shell_snapshot_v2_recovers_after_failed_prewarm() -> Result<()> {
    ...
    harness.test().workx.shutdown_and_wait().await?;
    Ok(())
}
...
async fn shell_snapshot_v2_prewarm_preserves_the_sandbox() -> Result<()> {
    ...
    let builder =
        shell_snapshot_v2_prewarm_builder(profile_home.path()).with_config(move |config| {
            ...
            let rules = config.workx_home.join("rules");
...
async fn shell_snapshot_v2_prewarm_stops_on_shutdown() -> Result<()> {
    ...
    let harness = TestWorkxHarness::with_auto_env_builder(shell_snapshot_v2_prewarm_builder(
        profile_home.path(),
    ))
    .await?;
    run_no_shell_turn(&harness).await?;
...
async fn shell_snapshot_v2_prewarm_skips_ineligible_sessions(
    configure: fn(&mut workx_core::config::Config),
) -> Result<()> {
    skip_if_remote!(Ok(()), "profile fixture uses a host-local HOME directory");
    let profile_home = tempfile::tempdir()?;
    fs::write(
...
async fn shell_snapshot_v2_guardian_uses_its_resolved_permissions_and_tools(
    ...
    fs::write(
        profile_home.path().join(".bashrc"),
        "printf capture > \"$HOME/$WORKX_THREAD_ID\"\n",
    )
    .await?;
    let builder =
        shell_snapshot_v2_prewarm_builder(profile_home.path()).with_config(move |config| {
...
async fn shell_snapshot_v2_preserves_legacy_snapshots_for_user_shell() -> Result<()> {
    ...
    let builder = test_workx().with_config(|config| {
        config
            .features
...
async fn linux_unified_exec_uses_shell_snapshot() -> Result<()> {
    ...
    assert!(run.snapshot_path.starts_with(&run.workx_home));
...
async fn unified_exec_snapshot_preserves_shell_environment_policy_set() -> Result<()> {
    ...
    let harness = TestWorkxHarness::with_builder(builder).await?;
    let workx_home = harness.test().home.path().to_path_buf();
    ...
    assert_eq!(end.exit_code, 0);
    assert!(snapshot_path.starts_with(workx_home));
...
async fn unified_exec_snapshot_still_intercepts_apply_patch() -> Result<()> {
    ...
    wait_for_event(&workx, |ev| match ev {
...
async fn shell_snapshot_deleted_after_shutdown_with_skills() -> Result<()> {
    ...
    let workx_home = home.path().to_path_buf();

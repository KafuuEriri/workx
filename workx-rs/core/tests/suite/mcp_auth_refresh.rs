...
use workx_mcp::WorkxAppsToolsCache;
...
async fn hosted_plugin_runtime_ps_mcp_tool_calls_use_current_auth_manager_token() -> Result<()> {
    ...
    let expected_auth = WorkxAuth::from_external_chatgpt_tokens(
        "header.e30.first",
        "test-account",
    ...
    let mut hosted_plugin_runtime_config = workx_mcp::hosted_plugin_runtime_mcp_server_config(
    ...
    *bearer_token_env_var = None;
    ...
    let runtime = McpRuntime::new(McpRuntimeInput {
        ...
        workx_apps_tools_cache: WorkxAppsToolsCache::default(),
        tool_catalog_cache: McpToolCatalogCache::default(),
        workx_apps_tools_cache_key: workx_mcp::workx_apps_tools_cache_key(Some(&expected_auth)),
    ...
    // so this tool call only sees the new token if the Workx Apps provider
    ...
    let tool_call_request = requests
        ...
        .expect("Workx Apps should receive a tool call");
    assert_eq!(
        tool_call_request
            .headers
            .get("authorization")

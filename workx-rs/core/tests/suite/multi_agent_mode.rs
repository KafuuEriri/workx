...
use workx_protocol::openai_models::MultiAgentModeMessages;
...
async fn ultra_reasoning_uses_highest_non_ultra_and_proactive_mode() -> Result<()> {
    ...
    submit_turn(&test.workx, "hello", /*effort*/ None).await?;
...
async fn mode_hints_override_reasoning_effort(source: ModeHintSource) -> Result<()> {
    ...
    let test = test_workx()
        ...
        .build(&server)
        .await?;
    submit_turn(&test.workx, "explicit", Some(ReasoningEffort::High)).await?;
    submit_turn(&test.workx, "proactive", Some(ReasoningEffort::Ultra)).await?;
...
async fn catalog_proactive_mode_is_ultra_only(
    ...
    submit_turn(&test.workx, "hello", Some(effort)).await?;
...
async fn model_switch_refreshes_catalog_role_and_mode(
    ...
    let test = test_workx()
        ...
        .build_with_auto_env(&server)
        .await?;
    submit_turn(&test.workx, "first model", Some(effort.clone())).await?;
    ...
    .await?;
    submit_turn(&test.workx, "second model", Some(effort)).await?;
...
async fn changing_configured_mode_hint_to_empty_emits_no_update() -> Result<()> {
    ...
    .await;
    let initial = test_workx()
        .with_config(configure_custom_mode_hint)
        .build(&server)
        .await?;

    submit_turn(&initial.workx, "before resume", /*effort*/ None).await?;
    ...
    drop(initial);
    submit_turn(&resumed.workx, "after resume", /*effort*/ None).await?;
...
async fn live_mode_change_appends_mode_without_reappending_usage_hint() -> Result<()> {
    ...
    submit_turn(&test.workx, "proactive", /*effort*/ None).await?;
    submit_turn(&test.workx, "explicit", Some(ReasoningEffort::High)).await?;
...
async fn leaving_ultra_after_cold_resume_emits_explicit_mode() -> Result<()> {
    ...
    let resumed = resume_builder.restart(&server, &initial).await?;
    drop(initial);
    submit_turn(&resumed.workx, "after resume", Some(ReasoningEffort::High)).await?;

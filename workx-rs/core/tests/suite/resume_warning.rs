...
async fn emits_warning_when_resumed_model_differs() {
    ...
    let thread_manager = workx_core::test_support::thread_manager_with_models_provider(
        WorkxAuth::from_api_key("test"),
        config.model_provider.clone(),
    );
    let auth_manager =
        workx_core::test_support::auth_manager_from_auth(WorkxAuth::from_api_key("test"));

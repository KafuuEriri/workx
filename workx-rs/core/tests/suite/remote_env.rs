...
use workx_api::AuthProvider;
use workx_config::types::ApprovalsReviewer;
use workx_core::EnvironmentConfig;
use workx_core::EnvironmentNetworkPolicy;
use workx_core::StartThreadOptions;
use workx_core::TurnInputRequest;
use workx_core::WaitForEnvironmentToolConfig;
use workx_core::WorkxThreadSettingsOverrides;
use workx_core::compact::SUMMARIZATION_PROMPT;
use workx_core::config::Config;
use workx_core::config::Constrained;
use workx_core::windows_sandbox::WindowsSandboxLevelExt;
use workx_exec_server::CopyOptions;
use workx_exec_server::CreateDirectoryOptions;
use workx_exec_server::EnvironmentReadyInfo;
use workx_exec_server::ExecServerError;
use workx_exec_server::ExecServerRuntimePaths;
use workx_exec_server::FileSystemSandboxContext;
use workx_exec_server::LOCAL_ENVIRONMENT_ID;
use workx_exec_server::NoiseChannelPublicKey;
use workx_exec_server::NoiseRendezvousConnectBundle;
use workx_exec_server::NoiseRendezvousConnectProvider;
use workx_exec_server::REMOTE_ENVIRONMENT_ID;
use workx_exec_server::RemoteEnvironmentConfig;
use workx_exec_server::RemoveOptions;
use workx_extension_api::ContextContributor;
use workx_extension_api::ExtensionDataInit;
use workx_extension_api::ExtensionFuture;
use workx_extension_api::ExtensionRegistryBuilder;
use workx_extension_api::RenderedWorldStateFragment;
use workx_extension_api::ThreadLifecycleContributor;
use workx_extension_api::ThreadStartInput;
use workx_extension_api::WorldStateContributionInput;
use workx_extension_api::WorldStateSectionContribution;
...
use workx_protocol::capabilities::CapabilityRootLocation;
...
use workx_protocol::protocol::ENVIRONMENTS_INSTRUCTIONS_OPEN_TAG;
...
fn test_workx_with_wait_for_environment() -> TestWorkxBuilder {
    let mut extensions = ExtensionRegistryBuilder::new();
    extensions.thread_lifecycle_contributor(Arc::new(WaitForEnvironmentTestExtension));
    test_workx().with_extensions(Arc::new(extensions.build()))
}

async fn unified_exec_test(server: &wiremock::MockServer) -> Result<TestWorkx> {
...
async fn submit_turn_with_approval_and_environments(
    ...
    let turn_environment_selections = workx_protocol::protocol::TurnEnvironmentSelections::new(
...
async fn expect_patch_approval(
    ...
    let event = wait_for_event(&test.workx, |event| {
        matches!(
            event,
...
async fn wait_for_completion_without_patch_approval(test: &TestWorkx) {
    let event = wait_for_event(&test.workx, |event| {
        matches!(
            event,
            EventMsg::ApplyPatchApprovalRequest(_) | EventMsg::TurnComplete(_)
...
async fn explicit_remote_shell_runs_in_remote_cwd() -> Result<()> {
    ...
    let (shell, command) = match test_target_os() {
        TestTargetOs::Linux => (
            "bash",
            r#"case "$PWD" in /tmp/workx-core-test-cwd-*) ;; *) echo "unexpected cwd: $PWD" >&2; exit 1 ;; esac"#,
        ),
        TestTargetOs::Windows => (
            "powershell",
            r#"$cwd = (Get-Location).Path; if ($cwd -notlike 'C:\workx-core-test-cwd-*') { Write-Error "unexpected cwd: $cwd"; exit 1 }"#,
        ),
        TestTargetOs::MacOs => unreachable!("remote test targets do not run macOS"),
    };
...
async fn environment_permissions_follow_configuration_ownership() -> Result<()> {
    ...
    test.workx
        ...
        .await?;
    let persisted_settings = wait_for_event_match(&test.workx, |event| match event {
        EventMsg::ThreadSettingsApplied(event) => Some(event.thread_settings.clone()),
        _ => None,
    })
    .await;
    let snapshot = test.workx.config_snapshot().await;
    ...
    assert_eq!(
        persisted_settings,
        test.workx.thread_settings_snapshot().await
    );
    assert_ne!(
    ...
    test.workx
        .restore_thread_settings(test.workx.restorable_thread_settings().await)
        .await?;
    let (mcp_config, _) = test.workx.current_mcp_config_and_runtime_context().await;
    assert_eq!(
    ...
    for deferred_executor_enabled in [false, true] {
        for host_config_present in [false, true] {
            ...
            let builder = if host_config_present {
                test_workx_with_wait_for_environment()
            } else {
                test_workx()
            };
...
async fn settings_update_does_not_retarget_active_turn_environment() -> Result<()> {
    ...
    let initial_environments = test.workx.environment_selections().await;
    ...
    test.workx
        ...
        .await?;
    let request = wait_for_event_match(&test.workx, |event| match event {
    ...
    let preview = test
        .workx
        .preview_thread_settings_overrides(WorkxThreadSettingsOverrides {
...
async fn deferred_executor_promotes_primary_environment_when_startup_completes() -> Result<()> {
    ...
    core_test_support::wait_for_mcp_server(&test.workx, "deferred").await?;
    test.workx
        .submit(Op::UserInputAnswer {
            id: request.turn_id,
            response: RequestUserInputResponse {
    ...
    test.workx.flush_rollout().await?;
    let rollout = fs::read_to_string(test.workx.rollout_path().context("rollout path")?)?;
...
async fn shared_executor_keeps_ready_capability_roots_scoped_to_each_attachment() -> Result<()> {
    ...
    let mut builder = test_workx().with_extensions(Arc::new(extensions.build()));
    let test = builder.build_with_auto_env(&server).await?;
    let selection = test
        .workx
    ...
    assert_eq!(
        test.workx.inspect_selected_capability_roots().ready_roots,
        vec![root("first-root")]
    );
    assert_eq!(
        second
...
async fn owner_network_policy_rejects_unsupported_environment_authority() -> Result<()> {
    let server = start_mock_server().await;
    let test = test_workx().build_with_auto_env(&server).await?;
    let selections = test.workx.environment_selections().await;
    ...
    assert_eq!(test.workx.environment_selections().await, selections);
    Ok(())
}
...
async fn ready_before_selection_resolves_resumed_thread_capability_root_after_wait(
    ...
    let remote_environment = tokio::spawn(workx_exec_server::run_remote_environment(
        remote_config,
        runtime_paths,
    ));
...
async fn deferred_executor_stays_pending_after_materialization() -> Result<()> {
    ...
    .await;
    let mut builder = test_workx_with_wait_for_environment().with_config(|config| {
        assert!(config.features.enable(Feature::DeferredExecutor).is_ok());
    });
    ...
    test.workx.submit(Op::Interrupt).await?;
    wait_for_event(&test.workx, |event| {
...
async fn deferred_executor_loads_agents_md_when_environment_becomes_ready() -> Result<()> {
    ...
    .await;
    let mut builder = test_workx_with_wait_for_environment()
        .with_exec_server_url(format!("ws://{}", listener.local_addr()?))
        .with_config(|config| {
            assert!(config.features.enable(Feature::DeferredExecutor).is_ok());
        });
    ...
    assert_eq!(test.workx.instruction_sources().await, vec![agents_path]);
...
async fn deferred_executor_compaction_preserves_then_updates_environment_once() -> Result<()> {
    ...
    test.workx.ensure_rollout_materialized().await;
    test.workx.flush_rollout().await?;
    let rollout_path = test.workx.rollout_path().context("rollout path")?;
...
async fn remote_request_permissions_grant_unblocks_later_remote_exec() -> Result<()> {
    ...
    let local_cwd = TempDir::new()?;
    let remote_cwd = PathBuf::from(format!(
        "/tmp/workx-remote-request-permissions-{}",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
...
async fn apply_patch_freeform_routes_to_selected_remote_environment() -> Result<()> {
    ...
    let remote_cwd = PathBuf::from(format!(
        "/tmp/workx-remote-apply-patch-freeform-{}",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
    let remote_cwd_uri = PathUri::from_host_native_path(&remote_cwd)?;
...
async fn apply_patch_approvals_are_remembered_per_environment() -> Result<()> {
    ...
    let local_cwd = TempDir::new()?;
    let remote_cwd = PathBuf::from(format!(
        "/tmp/workx-remote-apply-patch-approval-cwd-{}",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
    let remote_cwd_uri = PathUri::from_host_native_path(&remote_cwd)?;
    ...
    let target_path = PathBuf::from(format!(
        "/tmp/workx-apply-patch-approval-scope-{}.txt",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
...
async fn apply_patch_intercepted_exec_command_routes_to_selected_remote_environment() -> Result<()>
{
    ...
    let remote_cwd = PathBuf::from(format!(
        "/tmp/workx-remote-apply-patch-exec-{}",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
    let remote_cwd_uri = PathUri::from_host_native_path(&remote_cwd)?;
...
async fn remote_test_env_sandboxed_read_allows_readable_root() -> Result<()> {
    ...
    let allowed_dir = PathBuf::from(format!("/tmp/workx-remote-readable-{}", std::process::id()));
...
async fn remote_test_env_sandboxed_read_rejects_symlink_parent_dotdot_escape() -> Result<()> {
    ...
    let root = PathBuf::from(format!("/tmp/workx-remote-dotdot-{}", std::process::id()));

...
async fn daybreak_metadata_follows_the_actual_plugin_call(case: AccessCase) -> Result<()> {
    ...
    if remote_url.is_some() {
        ...
        let (config, _) = test.workx.current_mcp_config_and_runtime_context().await;
        assert_eq!(
    ...
    test.workx
        ...
        .await?;
    let end = wait_for_event(&test.workx, |event| {
...
async fn daybreak_discards_in_flight_account_changes_without_apps() -> Result<()> {
    ...
    test.workx
        ...
        .await?;
    let call = wait_for_event(&test.workx, |event| {
        matches!(event, EventMsg::McpToolCallEnd(_))
    });
    let switch = async {
        let request =

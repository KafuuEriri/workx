...
fn disabled_user_turn(test: &TestWorkx, items: Vec<UserInput>, model: String) -> TurnInputRequest {
...
async fn create_workspace_directory(test: &TestWorkx, rel_path: &str) -> anyhow::Result<PathUri> {
...
async fn assert_user_turn_local_image_resizes_to(
    ...
    let builder = match image_budget_policy {
        ImageBudgetPolicy::DetailBased | ImageBudgetPolicy::Unified => test_workx(),
        ImageBudgetPolicy::UnifiedResponsesLiteWithoutOriginalSupport => test_workx()
...
async fn view_image_tool_attaches_local_image() -> anyhow::Result<()> {
    ...
    wait_for_event_with_timeout(
        ...
        |event| match event {
            EventMsg::ItemStarted(event) => {
                if matches!(&event.item, workx_protocol::items::TurnItem::ImageView(_)) {
    ...
    match item_started.expect("view image item started event emitted") {
        workx_protocol::items::TurnItem::ImageView(item) => {

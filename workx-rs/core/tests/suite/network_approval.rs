...
use workx_core::config::NetworkProxySpec;
use workx_core::shell::ShellType;
use workx_core::shell::get_shell;
...
use workx_protocol::approvals::NetworkApprovalContext;
...
const NETWORK_TEST_HOST: &str = "workx-network-test.invalid";
const NETWORK_TEST_TARGET: &str = "http://workx-network-test.invalid:80";
...
-> Result<()> {
    ...
    .await?;
    let EventMsg::RequestPermissions(request) = wait_for_event(&test.workx, |event| {
...
async fn cancelled_guardian_network_review_fails_closed_without_rewriting_turn_state() -> Result<()>
{
    ...
    while !saw_turn_aborted || !saw_guardian_aborted {
        let event = tokio::time::timeout(Duration::from_secs(5), test.workx.next_event())
            .await
...
#[test_case("GET", "http://workx-network-test.invalid/"; "plain_http")]
#[test_case("CONNECT", "workx-network-test.invalid:443"; "connect")]
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
#[cfg_attr(
    not(target_os = "linux"),
    ignore = "requires the trusted Linux proxy bridge"
...
async fn background_network_approval_uses_active_turn_after_original_turn_completes() -> Result<()>
{
    ...
    .await?;
    let EventMsg::TurnComplete(first_turn) = wait_for_event(&test.workx, |event| {
    ...
    assert_eq!(test.workx.list_background_terminals().await.len(), 1);
    ...
    .await?;
    let EventMsg::TurnStarted(active_turn) = wait_for_event(&test.workx, |event| {
    ...
    test.workx
        ...
        .await?;
    let assessment = wait_for_event(&test.workx, |event| {
        matches!(
            event,
            EventMsg::GuardianAssessment(assessment)
    ...
    assert_eq!(
        test.workx.list_background_terminals().await.len(),
        1,
        "approved network access must not terminate the background process"
    );
    test.workx.submit(Op::CleanBackgroundTerminals).await?;
...
async fn user_network_approval_once_session_and_denial_semantics() -> Result<()> {
    ...
    assert!(
        approval
            .call_id
            .starts_with("network#local#http#workx-network-test.invalid#80#")
    );
    ...
    assert_eq!(
        ...
        Some("workx-network-test.invalid is not in the allowed_domains")
    );
    test.workx
...
async fn allowing_network_policy_amendment_persists_context_and_bypasses_prompt() -> Result<()> {
    ...
    assert!(policy.contains(
        r#"network_rule(host="workx-network-test.invalid", protocol="http", decision="allow""#
    ));
    assert!(first_responses.requests().iter().any(|request| {
        request.body_contains_text(
            "Allowed network rule saved in execpolicy (allowlist): workx-network-test.invalid",
        )
    }));
    mount_exec_network_turn(
        &server,
...
async fn denying_network_policy_amendment_persists_and_blocks_request() -> Result<()> {
    ...
    let policy = fs::read_to_string(test.home.path().join("rules/default.rules"))?;
    assert!(policy.contains(
        r#"network_rule(host="workx-network-test.invalid", protocol="http", decision="deny""#
    ));
    let output = responses
        .requests()
        .iter()
...
async fn ambiguous_unattributed_network_request_is_not_assigned_to_active_calls() -> Result<()> {
    ...
    tokio::time::timeout(Duration::from_secs(10), async {
        loop {
            if test.workx.list_background_terminals().await.is_empty() {
                break;
            }
...
async fn managed_network_unified_exec_test(server: &wiremock::MockServer) -> Result<TestWorkx> {
...
async fn wait_for_completion_without_network_prompt(test: &TestWorkx) {
    let event = wait_for_event(&test.workx, |event| {
        matches!(
            event,
            EventMsg::ExecApprovalRequest(_) | EventMsg::TurnComplete(_)
...
async fn wait_for_turn_complete(test: &TestWorkx) {
    wait_for_event(&test.workx, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;

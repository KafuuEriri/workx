...
use workx_client::RetryOn;
use workx_client::RetryPolicy;
use workx_client::run_with_retry;
use workx_http_client::Request;
use workx_http_client::TransportError;
    ...
    S: Subscriber,
{
    fn on_event(&self, event: &Event<'_>, _context: Context<'_, S>) {
        if event.metadata().target() == "workx_http_client::transport" {
            self.record_request_after_retry();
            return;
        }

        if event.metadata().target() != "workx_otel.trace_safe" {
            return;
        }

        let mut visitor = RetryTelemetryVisitor::default();
        event.record(&mut visitor);
        if visitor.name.as_deref() != Some("workx.retry") {
            return;
        }
...
async fn submit_user_input(test: &TestWorkx, text: &str) -> Result<()> {
    test.workx
...
async fn wait_for_turn_completion(test: &TestWorkx) {
    let EventMsg::TurnComplete(completed) = wait_for_event(&test.workx, |event| {
        ...
        loop {
            match wait_for_event(&test.workx, |_| true).await {
                ...
                EventMsg::TurnComplete(event) => {
                    assert_eq!(
                        event.error.and_then(|error| error.workx_error_info),
                        Some(WorkxErrorInfo::ServerOverloaded)
                    );
                    break;
                }
    ...
    loop {
        match wait_for_event(&test.workx, |_| true).await {
            EventMsg::Error(error) => {
                ...
                assert_eq!(
                    error.workx_error_info,
                    Some(WorkxErrorInfo::RateLimitExceeded)
                );
    ...
    loop {
        match wait_for_event(&test.workx, |_| true).await {
            EventMsg::Error(error) => {
                ...
                assert_eq!(
                    error.workx_error_info,
                    Some(WorkxErrorInfo::ResponseTooManyFailedAttempts {

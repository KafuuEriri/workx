...
use workx_core::RecoverTurnRequest;
...
async fn submit_user_message(
    ...
    text: &str,
) -> workx_protocol::error::Result<TurnInputSubmission> {
    workx.start_or_steer_turn(user_message_request(text)).await
}
...
async fn start_turn_if_idle_keeps_automatic_plan_rejections_atomic(
    ...
    let mut collaboration_mode = test.workx.config_snapshot().await.collaboration_mode;
    ...
    let current_settings = test.workx.thread_settings_snapshot().await;
    ...
    .await;
    assert_eq!(
        test.workx.config_snapshot().await.collaboration_mode,
        collaboration_mode
    );
...
async fn recover_turn_if_idle_preserves_id_and_resumes_plan_mode() {
    ...
    assert_eq!(
        test.workx.config_snapshot().await.collaboration_mode.mode,
        ModeKind::Plan
    );

    let started = wait_for_event(&test.workx, |event| {
...
async fn turn_input_submission_reports_started_and_steered_for_concurrent_submissions() {
    ...
    let first_submission = tokio::spawn({
        let workx = Arc::clone(&workx);
        let barrier = Arc::clone(&barrier);
        async move {
            barrier.wait().await;
            submit_user_message(workx.as_ref(), "first message").await
        }
    });
    let second_submission = tokio::spawn({
        ...
        async move {
            barrier.wait().await;
            submit_user_message(workx.as_ref(), "second message").await
        }
    });
    barrier.wait().await;
    ...
    wait_for_event(workx.as_ref(), |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
...
async fn turn_input_submission_applies_thread_settings_only_after_accepted_input() {
    ...
    let started = submit_user_message(workx, "start turn")
        .await
    ...
    assert_eq!(
        workx.config_snapshot().await.approval_policy,
        AskForApproval::Never
    );
    assert_eq!(
        workx.environment_selections().await,

...
fn workx_home_for_windows_sandbox_test(name: &str) -> anyhow::Result<TestWorkxHome> {
    if let Some(test_tmpdir) = std::env::var_os("TEST_TMPDIR") {
        // The elevated backend provisions machine-local sandbox users. Bazel
        // retries run in the same Windows VM, so keep WORKX_HOME stable within
        // the test temp root and let setup reconcile its persisted ACL state.
        let workx_home = PathBuf::from(test_tmpdir).join(name);
        std::fs::create_dir_all(&workx_home)
            .with_context(|| format!("create stable test WORKX_HOME {}", workx_home.display()))?;
        return Ok(TestWorkxHome::Persistent(workx_home));
    }

    Ok(TestWorkxHome::Temporary(TempDir::new()?))
}
...
fn stage_windows_sandbox_helpers() -> anyhow::Result<()> {
    ...
    let resources_dir = test_exe_dir.join("workx-resources");
    ...
    for helper_name in ["workx-windows-sandbox-setup", "workx-command-runner"] {
        let helper = workx_utils_cargo_bin::cargo_bin(helper_name)?;
...
async fn windows_restricted_token_rejects_exact_and_glob_deny_read_policy() -> anyhow::Result<()> {
    let workx_home =
        workx_home_for_windows_sandbox_test("windows-restricted-token-deny-read-workx-home")?;
...
async fn windows_elevated_does_not_create_missing_workspace_metadata() -> anyhow::Result<()> {
    let workx_home =
        workx_home_for_windows_sandbox_test("windows-elevated-missing-metadata-workx-home")?;
    let _workx_home_guard = EnvVarGuard::set("WORKX_HOME", workx_home.path().as_os_str());
...
async fn windows_elevated_enforces_deny_read_and_protects_setup_marker() -> anyhow::Result<()> {
    let workx_home = workx_home_for_windows_sandbox_test("windows-elevated-deny-read-workx-home")?;
    let _workx_home_guard = EnvVarGuard::set("WORKX_HOME", workx_home.path().as_os_str());
    stage_windows_sandbox_helpers()?;
    let workspace = TempDir::new()?;
    let cwd = dunce::canonicalize(workspace.path())?.abs();
    ...
    let bundled_skill_dir = user_profile.path().join(".workx/plugins/cache");
    std::fs::create_dir_all(&bundled_skill_dir)?;
    let bundled_skill = bundled_skill_dir.join("SKILL.md");
    let setup_marker = workx_home.path().join(".sandbox").join("setup_marker.json");
    ...
    assert!(
        sandbox_setup_is_complete(workx_home.path()),
...
async fn windows_elevated_unified_exec_enforces_managed_deny_reads() -> anyhow::Result<()> {
    let workx_home =
        workx_home_for_windows_sandbox_test("windows-elevated-tool-runtime-deny-read-workx-home")?;
    let _workx_home_guard = EnvVarGuard::set("WORKX_HOME", workx_home.path().as_os_str());
    stage_windows_sandbox_helpers()?;

    let configured_workx_home = dunce::canonicalize(workx_home.path())?.abs();
    let builder = test_workx()
        .with_windows_cmd_shell()
        .with_config(move |config| {
            config.workx_home = configured_workx_home;

...
use workx_core::SleepFuture;
use workx_core::TimeFuture;
...
use workx_protocol::openai_models::AutoReviewMessages;
...
#[test_case(WorkxAuth::from_api_key("test-api-key"), "/v1", true, "/v1/responses"; "api_key_uses_responses")]
#[test_case(WorkxAuth::create_dummy_chatgpt_auth_for_testing(), "/backend-api/codex", false, "/backend-api/codex/responses"; "chatgpt_uses_responses_by_default")]
#[test_case(WorkxAuth::create_dummy_chatgpt_auth_for_testing(), "/backend-api/codex", true, "/backend-api/codex/guardian"; "chatgpt_uses_guardian_when_enabled")]
#[test_case(WorkxAuth::create_dummy_chatgpt_auth_for_testing(), "/v1", true, "/v1/responses"; "custom_openai_url_uses_responses")]
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
#[test_case(WorkxAuth::from_api_key("test-api-key"), "gpt-5.6-luna"; "api_key_uses_luna_with_responses_lite")]
#[test_case(WorkxAuth::create_dummy_chatgpt_auth_for_testing(), "codex-auto-review"; "chatgpt_uses_workx_auto_review")]
async fn guardian_session_prewarms_and_is_reused_for_first_review(
    ...
    let bundled_models = workx_models_manager::bundled_models_response()?.models;
    ...
    let mut builder = test_workx()
        ...
        .with_config(move |config| {
            ...
            let rules_dir = config.workx_home.join("rules");
    ...
    for handshake in server.handshakes() {
        ...
        let uses_guardian_endpoint = uses_workx_backend && is_guardian;
        assert_eq!(
            handshake.uri(),
            if uses_guardian_endpoint {
                "/backend-api/codex/guardian"
            } else if uses_workx_backend {
...
async fn guardian_session_is_reused_for_consecutive_tool_reviews_without_prewarm() -> Result<()> {
    ...
    let mut builder = test_workx()
        ...
        .with_workspace_setup(|cwd, fs| async move {
            fs.write_file(
                &workx_utils_path_uri::PathUri::from_abs_path(&cwd.join("guardian-secret.txt")),
...
async fn guardian_denial_rejects_tool_call_with_rationale(
    ...
    let feedback = workx_feedback::guardian_review_failures(&[test.session_configured.thread_id])
        .attachment
...
async fn guardian_timeout_rejects_tool_call_with_acting_model_instructions(
    ...
    impl workx_extension_api::ApprovalReviewContributor for TimedOutReviewContributor {
        fn fast_decision<'a>(
            &'a self,
...
async fn cyber_model_guardian_denial_interrupts_turn_immediately() -> Result<()> {
    ...
    let warning = wait_for_event(&test.workx, |event| {
        matches!(
            event,
    ...
    let aborted = wait_for_event(&test.workx, |event| {
        matches!(event, EventMsg::TurnAborted(_))
    })
    .await;
...
printf '%s\n' "${@: -1}" >> "${payload_path}""#,
    ...
    assert!(
        !notify_payload_raw.contains(
            "The following is the Workx agent history whose request action you are assessing."
        ),

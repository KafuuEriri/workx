...
use workx_core::config::TokenBudgetConfig;
...
use workx_protocol::openai_models::ToolMessage;
use workx_protocol::openai_models::ToolMessages;
use workx_protocol::protocol::AskForApproval;
use workx_protocol::protocol::CONTEXT_WINDOW_GUIDANCE_CLOSE_TAG;
use workx_protocol::protocol::CONTEXT_WINDOW_GUIDANCE_OPEN_TAG;
...
use workx_protocol::protocol::TurnSettingsUpdate;
use workx_protocol::protocol::TurnSettingsUpdateOutcome;
use workx_protocol::request_user_input::RequestUserInputAnswer;
use workx_protocol::request_user_input::RequestUserInputEvent;
...
fn step_settings_test() -> TestWorkxBuilder {
    test_workx().with_model(MODEL_A).with_config(move |config| {
        for feature in [
...
async fn start_paused_turn(thread: &WorkxThread) -> Result<RequestUserInputEvent> {
    thread
...
async fn answer_paused_turn(thread: &WorkxThread, turn_id: &str) -> Result<()> {
    thread
...
async fn settings_updates_preserve_turn_identity_and_target(target: SettingsTarget) -> Result<()> {
    ...
    let original_settings = test.workx.thread_settings_snapshot().await;
    let request = start_paused_turn(&test.workx).await?;
    ...
    match target {
        ...
        SettingsTarget::Turn => {
            ...
            assert_eq!(
                submit_turn_settings(&test.workx, "different-turn", update.clone()).await?,
                TurnSettingsUpdateOutcome::TargetUnavailable
            );
            assert_eq!(
                submit_turn_settings(&test.workx, &request.turn_id, update).await?,
                TurnSettingsUpdateOutcome::Applied
            );
        }
    }
    answer_paused_turn(&test.workx, &request.turn_id).await?;
    let mut settings_events = Vec::new();
    let mut new_turns = Vec::new();
    let completion = wait_for_event(&test.workx, |event| match event {
...
async fn active_model_switch_resolves_token_budget_from_original_preferences(
    ...
    if reload_user_config {
        std::fs::write(
            test.workx_home_path().join("config.toml"),
            "[features.token_budget]\nenabled = true\nreminder_message_template = \"Reloaded reminder\"\n",
        )?;
        test.workx.submit(Op::ReloadUserConfig).await?;
    }
...
async fn mcp_confirmation_policy_follows_step_model_changes() -> Result<()> {
    ...
    let test = step_settings_test()
        ...
        .build_with_auto_env(&server)
        .await?;
    wait_for_mcp_server(&test.workx, "node_repl").await?;
...
async fn sparse_updates_preserve_divergent_active_and_future_models() -> Result<()> {
    ...
    answer_paused_turn(&test.workx, &request.turn_id).await?;
    let second_request = wait_for_event_match(&test.workx, |event| match event {
        EventMsg::RequestUserInput(request) => Some(request.clone()),
        _ => None,
    })
    .await;
    ...
    let durable_settings = wait_for_event_match(&test.workx, |event| match event {
    ...
    answer_paused_turn(&test.workx, &second_request.turn_id).await?;
    wait_for_event(&test.workx, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
...
async fn turn_settings_do_not_target_idle_or_finished_turns() -> Result<()> {
    ...
    for (discarded_model, model) in [(MODEL_C, MODEL_B), (MODEL_A, MODEL_C)] {
        ...
        let before = test.workx.thread_settings_snapshot().await;
        assert_eq!(
            submit_turn_settings(
                &test.workx,
                &turn_id,
        ...
        assert_eq!(test.workx.thread_settings_snapshot().await, before);
        core_test_support::submit_thread_settings(
            &test.workx,
            ThreadSettingsOverrides {
                model: Some(model.to_string()),
...
async fn model_activation_uses_destination_metadata_defaults(
    ...
    answer_paused_turn(&test.workx, &request.turn_id).await?;
    let paused = wait_for_event_match(&test.workx, |event| match event {
        EventMsg::RequestUserInput(request) => Some(request.clone()),
        _ => None,
    })
    .await;
    ...
    answer_paused_turn(&test.workx, &paused.turn_id).await?;
    wait_for_event(&test.workx, |event| match event {
...
async fn request_user_input_async_description_follows_mid_turn_model_changes() -> Result<()> {
    ...
    let test = step_settings_test()
        ...
        .build_with_auto_env(&server)
        .await?;
    let paused = start_paused_turn(&test.workx).await?;
    assert_eq!(
        submit_turn_settings(
            &test.workx,
            &paused.turn_id,
...
async fn turn_settings_rejection_preserves_independent_future_settings() -> Result<()> {
    ...
    .await?;
    let future = test.workx.thread_settings_snapshot().await;
    assert_eq!(
        submit_turn_settings(
            &test.workx,
            &request.turn_id,
    ...
    assert_eq!(test.workx.thread_settings_snapshot().await, future);
    answer_paused_turn(&test.workx, &request.turn_id).await?;
    wait_for_event(&test.workx, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })

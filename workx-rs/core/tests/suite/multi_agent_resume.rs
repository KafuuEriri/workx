...
fn configure_multi_agent_v2_with_role(
    config: &mut workx_core::config::Config,
    ...
    let role_path = config.workx_home.join("durable-worker-role.toml");
    std::fs::write(
        &role_path,
        format!(
...
async fn cold_root_resume_restores_agent_identity_and_role_on_followup() -> Result<()> {
    ...
    let mut initial_builder = test_workx().with_config(move |config| {
        ...
        if let Some(thread_id) = initial_child_request
            ...
            .find_map(|request| {
                ...
                body["client_metadata"]["thread_id"]
                    .as_str()
                    .and_then(|thread_id| workx_protocol::ThreadId::from_string(thread_id).ok())
            })
        {
            break thread_id;
        }
    ...
    assert_eq!(
        ...
        initial.workx.config().await.model_provider,
    ...
    std::fs::write(
        resumed.config.workx_home.join("durable-worker-role.toml"),
        format!(
            r#"model = "{ROLE_MODEL}"
model_reasoning_effort = "high"
developer_instructions = "{ROLE_DEVELOPER_INSTRUCTIONS}"
...
"#
    ...
    assert_eq!(
        ...
        resumed.workx.config().await.model_provider,
    ...
    let body_for = |text: &str, thread: workx_protocol::ThreadId| {
        requests
            .iter()
            .find_map(|request| {

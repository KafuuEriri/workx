...
async fn web_search_mode_cached_sets_external_web_access_false() {
    ...
    let test = builder
        .build(&server)
        .await
        .expect("create test Workx conversation");
...
async fn amazon_bedrock_web_search_uses_text_only_hosted_tools() {
    ...
    for (case, configured_web_search_mode, model_catalog) in [
        ...
        let mut builder = test_workx().with_auth(auth);
...
async fn web_search_tool_config_from_config_toml_is_forwarded_to_request() {
    ...
    let home = Arc::new(tempfile::TempDir::new().expect("create workx home"));
    std::fs::write(
...
"#,
    ...
    let mut builder = test_workx().with_model("gpt-5.2").with_home(home);
    let test = builder
        .build(&server)
        .await
        .expect("create test Workx conversation");

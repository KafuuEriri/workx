...
//! `just test -p workx-core --test all --run-ignored only live_cli` provided they set a valid
//! `OPENAI_API_KEY`.
...
fn run_live(prompt: &str) -> (assert_cmd::assert::Assert, TempDir) {
    ...
    let workx_home = home.path().join(".workx");
    ...
    let mut cmd = Command::new(workx_utils_cargo_bin::cargo_bin("workx-rs").unwrap());
    ...
    let mut child = cmd.spawn().expect("failed to spawn workx-rs");

...
use workx_extension_api::McpToolResultInput;
use workx_extension_api::McpToolSource;
...
async fn mcp_result_processing_precedes_completion(
    ...
    let call = match mode {
        ...
        McpCallMode::CodeMode => responses::ev_custom_tool_call(
            call_id,
            "exec",
            r#"text(await tools.mcp__workx_apps__calendar_list_events({query: "callback test"}));"#,
        ),
    };
...
async fn tool_start_receives_frozen_host_plugin_root() -> Result<()> {
    ...
    let server = responses::start_mock_server().await;
    let workx_home = Arc::new(tempfile::tempdir()?);
    let plugin_root = super::plugins::write_sample_plugin_manifest_and_config(workx_home.as_ref());
    ...
    let test = test_workx()
        ...
        .await?;
    wait_for_mcp_server(&test.workx, "sample").await?;
    responses::mount_sse_sequence(
        &server,
        vec![
            responses::sse(vec![

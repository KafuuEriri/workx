...
use workx_protocol::protocol::CONTEXT_WINDOW_CLOSE_TAG;
...
use workx_protocol::protocol::CONTEXT_WINDOW_OPEN_TAG;
...
async fn assert_context_compaction_item_lifecycle(workx: &std::sync::Arc<workx_core::WorkxThread>) {
...
async fn token_budget_guidance_precedes_standalone_context_window(
    ...
    let test = test_workx()
        .with_auth(WorkxAuth::from_external_chatgpt_tokens(
...
#[test_case("OpenAI", "/backend-api/codex", None, true; "workx_backend")]
#[test_case("Custom", "/backend-api/codex", None, false; "custom_provider")]
#[test_case("OpenAI", "/v1", None, false; "non_workx_endpoint")]
#[test_case("OpenAI", "/backend-api/codex", Some("test-provider-token"), false; "provider_credentials")]
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn experimental_context_requires_workx_backend(
...
async fn token_budget_context_injects_plain_thread_hint_text() -> Result<()> {
    ...
    let test = test_workx()
        ...
        .build(&server)
        .await?;
    wait_for_mcp_server(&test.workx, "notes").await?;
    let responses = mount_sse_sequence(
        &server,
        vec![sse(vec![
            ev_response_created("resp-1"),
...
async fn token_budget_context_uses_new_window_after_compaction(
    ...
    assert_context_compaction_item_lifecycle(&test.workx).await;
...
async fn token_budget_compaction_runs_compact_hooks() -> Result<()> {
    ...
    let pre_compact = wait_for_event_match(&test.workx, |event| match event {
    ...
    let post_compact = wait_for_event_match(&test.workx, |event| match event {
        EventMsg::HookCompleted(completed)

    ...
    let mut builder = test_workx().with_config(move |config| {
        ...
        servers.insert(
            ...
            workx_config::types::McpServerConfig {
                ...
                transport: workx_config::types::McpServerTransportConfig::Stdio {
...
async fn mcp_tool_output_limit_preserves_output_that_fits(
    ...
    let server = start_mock_server().await;
    let builder = test_workx().with_config(|config| config.tool_output_token_limit = Some(50));
    let (_fixture, output) =
...
async fn mcp_tool_output_limit_survives_resume(output_token_limit: Option<usize>) -> Result<()> {
    ...
    let server = start_mock_server().await;
    let builder = test_workx().with_config(|config| config.tool_output_token_limit = Some(50_000));
    ...
    fixture.workx.ensure_rollout_materialized().await;
    fixture.workx.flush_rollout().await?;

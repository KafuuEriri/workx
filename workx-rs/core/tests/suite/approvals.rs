...
use workx_protocol::protocol::ExecPolicyAmendment;
...
impl TargetPath {
    fn resolve_for_patch(self, test: &TestWorkx) -> (PathBuf, String) {
        match self {
    ...
    async fn prepare(
        ...
        match self {
            ...
            ActionKind::ApplyPatchShell { target, content } => {
                ...
                // Bazel may need to launch the configured Workx helper binary
...
impl Expectation {
    fn verify(&self, test: &TestWorkx, result: &CommandResult) -> Result<()> {
        match self {
...
fn assert_active_workspace_permission_profile(test: &TestWorkx) {
    assert_eq!(
...
async fn wait_for_completion_without_approval(test: &TestWorkx) {
    let event = wait_for_event(&test.workx, |event| {
        matches!(
            event,
            EventMsg::ExecApprovalRequest(_) | EventMsg::TurnComplete(_)
...
async fn wait_for_completion(test: &TestWorkx) {
...
async fn wait_for_spawned_thread(test: &TestWorkx) -> Result<Arc<WorkxThread>> {
...
"#,
    ...
    let command = r#"printf '%s\n%s\n%s\n' "$GH_HOST" "$GH_ENTERPRISE_TOKEN" "$WORKX_NETWORK_PROXY_CREDENTIAL_BROKER_ACTIVE""#;
    let event = shell_event(
        call_id,
        command,
        /*timeout_ms*/ 10_000,
...
/// same sandbox with `workx sandbox -P`. The nested invocation must also pass
...
printenv WORKX_PERMISSION_PROFILE
touch {outside_path:?}
"#
        ),
    )?;
    ...
    assert_eq!(
        ...
        "the unsandboxed script should inherit WORKX_PERMISSION_PROFILE from the shell command"
    );
    assert!(
        outside_path.exists(),
...
/// profile names before passing one to `workx sandbox -P`.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
#[cfg(unix)]
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Print an ssh command that recreates the current Workx sandbox remotely."
    )
...
def main():
    args = parse_args()
    profile_name = os.environ.get("WORKX_PERMISSION_PROFILE")
    if not profile_name:
        raise SystemExit("WORKX_PERMISSION_PROFILE must not be empty")
    if profile_name not in ALLOWED_PROFILES:
        raise SystemExit("WORKX_PERMISSION_PROFILE is not allowlisted")
...
"#
    ...
    assert_eq!(
        sandbox_argv.len(),
        9,
        "expected workx sandbox ... bash -lc CMD"
    );
    assert_eq!(
        sandbox_argv[..8],
        [
        ...
        "remote_bash.py should use the allowlisted inherited profile and managed configuration to reconstruct the Workx sandbox"
    );
...
async fn invalid_requested_prefix_rule_falls_back_for_compound_command() -> Result<()> {
    ...
    let command =
        "touch /tmp/workx-fallback-rule-test.txt && echo hello > /tmp/workx-fallback-rule-test.txt";
    let event = shell_event_with_prefix_rule(
        call_id,
        command,
        /*timeout_ms*/ 1_000,
...
"#,
    ...
    let fetch_command = r#"python3 -c "import urllib.request; opener = urllib.request.build_opener(urllib.request.ProxyHandler()); print('OK:' + opener.open('http://workx-network-test.invalid', timeout=30).read().decode(errors='replace'))""#
        .to_string();

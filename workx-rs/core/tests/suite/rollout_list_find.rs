...
use workx_core::RolloutRecorderParams;
...
fn write_minimal_rollout_with_id_in_subdir(workx_home: &Path, subdir: &str, id: Uuid) -> PathBuf {
    let sessions = workx_home.join(subdir).join("2024/01/01");
...
/// provided conversation id in the SessionMeta line. Returns the absolute path.
fn write_minimal_rollout_with_id(workx_home: &Path, id: Uuid) -> PathBuf {
    write_minimal_rollout_with_id_in_subdir(workx_home, "sessions", id)
}
...
async fn upsert_thread_metadata(
    ...
    builder.cwd = workx_home.to_path_buf();
...
#[tokio::test]
async fn find_handles_gitignore_covering_workx_home_directory() {
    let repo = TempDir::new().unwrap();
    let workx_home = repo.path().join(".workx");
    std::fs::create_dir_all(&workx_home).unwrap();
    std::fs::write(repo.path().join(".gitignore"), ".workx/**\n").unwrap();
    let id = Uuid::new_v4();
    let expected = write_minimal_rollout_with_id(&workx_home, id);

    let found =
        find_thread_path_by_id_str(&workx_home, &id.to_string(), /*state_db_ctx*/ None)
            .await
            .unwrap();

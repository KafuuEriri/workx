...
fn repo_root() -> std::path::PathBuf {
    workx_utils_cargo_bin::repo_root().expect("failed to resolve repo root")
}
...
fn personal_access_token_exec_command(server: &MockServer, home: &TempDir) -> Command {
    let bin = workx_utils_cargo_bin::cargo_bin("workx").unwrap();
    ...
    cmd.arg("exec")
        ...
        .arg(repo_root())
        .arg("hello?");
    cmd.env("WORKX_HOME", home.path())
        .env(WORKX_ACCESS_TOKEN_ENV_VAR, PERSONAL_ACCESS_TOKEN)
        .env("WORKX_AUTHAPI_BASE_URL", server.uri())
        .env_remove(WORKX_API_KEY_ENV_VAR)
...
impl Drop for ChildProcessCleanupGuard {
    fn drop(&mut self) {
        #[cfg(unix)]
        {
            let _ = workx_utils_pty::process_group::kill_process_group(self.0);
        }
...
// Use this for new `workx exec` subprocess tests in this file. These commands
...
async fn exec_cli_applies_model_instructions_file() {
    ...
    // Workx to use the Responses API with the dummy env var.
    let provider_override = format!(
        "model_providers.mock={{ name = \"mock\", base_url = \"{}/v1\", env_key = \"PATH\", wire_api = \"responses\" }}",
        server.uri()
    );
...
/// Verify that `workx exec --profile ...` preserves the active user config
...
async fn integration_creates_and_checks_session_file() -> anyhow::Result<()> {
    ...
    // 4. Run the workx CLI and invoke `exec`, which is what records a session.
    let bin = workx_utils_cargo_bin::cargo_bin("workx").unwrap();
    let mut cmd = Command::new(bin);
    cmd.arg("exec")
        .arg("--skip-git-repo-check")
    ...
    let bin2 = workx_utils_cargo_bin::cargo_bin("workx").unwrap();
    ...
    cmd2.arg("exec")
        ...
        .arg("resume")
        .arg("--last");
    cmd2.env("WORKX_HOME", home.path())
        .env("OPENAI_API_KEY", "dummy");

    let output2 = run_cli_command(&mut cmd2).unwrap();
    assert!(output2.status.success(), "resume workx-cli run failed");

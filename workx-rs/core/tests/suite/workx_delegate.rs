...
async fn workx_delegate_ignores_legacy_deltas() {
    skip_if_no_network!();
    ...
    loop {
        let ev = wait_for_event(&test.workx, |_| true).await;
        match ev {
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn workx_delegate_rejects_escalation_requests_when_parent_can_prompt() {
    skip_if_no_network!();
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn workx_delegate_rejects_legacy_mcp_approvals_without_prompting() {
    skip_if_no_network!();
    ...
    let test = search_capable_apps_builder(apps_server.chatgpt_base_url)
        .with_config(|config| {
            ...
            let config_path = config.workx_home.join("config.toml").abs();
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn workx_delegate_rejects_skill_mcp_dependency_installation_without_prompting() {

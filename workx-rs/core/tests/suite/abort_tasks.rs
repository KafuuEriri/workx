...
use workx_core::SuspendTurnOutcome;
...
async fn interrupt_long_running_tool_emits_turn_aborted() {
    ...
    let fixture = test_workx()
        ...
        .await
        .unwrap();
    let workx = Arc::clone(&fixture.workx);
    ...
    wait_for_event(&workx, |ev| matches!(ev, EventMsg::ExecCommandBegin(_))).await;

    workx.submit(Op::Interrupt).await.unwrap();

    // Expect TurnAborted soon after.
    wait_for_event(&workx, |ev| matches!(ev, EventMsg::TurnAborted(_))).await;
    workx.submit(Op::CleanBackgroundTerminals).await.unwrap();
}
...
async fn root_turn_suspension_preserves_unfinished_turn_history() {
    ...
    let workx_core::TurnInputSubmission::Started { turn_id } = submitted else {
    ...
    assert_eq!(
        resumed
            .workx
            .recover_turn_if_idle(workx_core::RecoverTurnRequest {
                turn_id: turn_id.clone(),
        ...
        workx_core::StartIfIdleSubmission::Started {
            turn_id: turn_id.clone(),
        },
    );
    let completed = wait_for_event(&resumed.workx, |event| {

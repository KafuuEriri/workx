...
async fn workx_apps_file_params_omit_fields_absent_from_tool_schema() -> Result<()> {
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn workx_apps_file_params_pass_uploaded_file_to_post_tool_use_hook() -> Result<()> {
    let server = start_mock_server().await;
    let apps_server = AppsTestServer::mount(&server).await?;
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn workx_apps_file_params_stream_allowed_file_under_restricted_read_policy() -> Result<()> {
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn workx_apps_file_params_reject_denied_file_before_upload() -> Result<()> {
    skip_if_target_windows!(
        Ok(()),
        "Windows restricted-token sandbox cannot enforce deny-read policies"
    );

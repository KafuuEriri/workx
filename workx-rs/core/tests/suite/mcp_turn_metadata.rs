...
use workx_protocol::protocol::ElicitationAction;
...
async fn auto_session_approval_is_scoped_to_tool_link_id() -> Result<()> {
    ...
    Mock::given(method("POST"))
        ...
        .respond_with(move |request: &Request| {
            ...
            if *lists_link_selector.lock().unwrap() {
                ...
                tool["_meta"]["_workx_apps"]["requires_explicit_link_id"] = json!(true);
    ...
    for (index, request) in requests.iter().skip(1).step_by(2).enumerate() {
        ...
        if cases[index].0.is_some() {
            expected_result["_workx_apps"]["requires_explicit_link_id"] = json!(true);
        }
...
fn set_calendar_approval_mode(config: &mut Config, approval_mode: AppToolApproval) {
    ...
    let user_config_path = config.workx_home.join("config.toml").abs();
...
async fn wait_for_mcp_tool_call_item(
    ...
    wait_for_event_match(&test.workx, |event| {
...
async fn approved_mcp_tool_call_metadata_records_prior_user_input_request(
    ...
    let EventMsg::McpToolCallBegin(begin) = wait_for_event(&test.workx, |event| {
    ...
    if !strict_auto_review {
        let EventMsg::ElicitationRequest(request) = wait_for_event(&test.workx, |event| {
            matches!(
                event,
                EventMsg::ElicitationRequest(_) | EventMsg::TurnComplete(_)
            )
...
"#
                ...
                config.config_layer_stack = config
                    .config_layer_stack
                    .with_user_config(&config.workx_home.join("config.toml").abs(), user_config)
                    .expect("apps user config should be valid");
            } else {
                set_default_app_approval_mode_and_reviewer(
                    config,
    ...
    let route_event = wait_for_event(&test.workx, |event| {
        matches!(
            event,
            EventMsg::ElicitationRequest(_) | EventMsg::TurnComplete(_)
        )
...
async fn apps_default_writes_prompts_for_writes_but_not_reads() -> Result<()> {
    ...
    let EventMsg::McpToolCallBegin(read_begin) = wait_for_event(&test.workx, |event| {
        matches!(event, EventMsg::McpToolCallBegin(_))
    })
    .await
    else {
    ...
    let next_route = wait_for_event(&test.workx, |event| {
        matches!(
            event,

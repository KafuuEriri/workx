...
async fn exec_command_tool_executes_command_and_streams_output() -> anyhow::Result<()> {
    ...
    let mut builder = test_workx().with_model("test-gpt-5-workx");
    let TestWorkx {
        workx,
        cwd,
        session_configured,
...
async fn update_plan_tool_emits_plan_update_event() -> anyhow::Result<()> {
    ...
    let mut builder = test_workx().with_config(|config| config.update_plan_enabled = true);
    let TestWorkx {
        workx,
        cwd,
        session_configured,
    ...
    wait_for_event(&workx, |event| match event {
        ...
        EventMsg::ItemCompleted(completed) => {
            if let TurnItem::FileChange(item) = &completed.item {
                ...
                assert_eq!(
                    item.status,
                    Some(workx_protocol::protocol::PatchApplyStatus::Completed)
                );
            }
            false
        }

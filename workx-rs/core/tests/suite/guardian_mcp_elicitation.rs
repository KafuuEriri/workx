...
use workx_protocol::protocol::GuardianAssessmentAction;
        ...
        for index, meta in enumerate(approval_metas):
            ...
            pending_elicitations.append({
                ...
                "method": "elicitation/create", "params": {
                    ...
                    "_meta": {"workx_request_type": "approval_request",
                              "workx_approval_kind": "mcp_tool_call", "tool_name": "write_record",
...
async fn interrupt_aborts_server_initiated_mcp_guardian_review() -> Result<()> {
    ...
    wait_for_mcp_server(&test.workx, "elicitation").await?;
...
async fn server_initiated_mcp_elicitation_can_require_synchronous_auto_review(
    ...
    if strict_auto_review {
        meta.insert("workx_strict_auto_review".to_string(), json!(true));
    }
    if let Some(sensitive_action) = sensitive_action {
        meta.insert(
            "workx_sensitive_action".to_string(),
...
async fn node_elicitations_attribute_independent_reviews_without_changing_actions(
    ...
    let meta = actions.map(|(tool_name, arguments)| {
        let mut meta = json!({
            "workx_strict_auto_review": strict_auto_review,
            "workx_sensitive_action": true,
            "tool_name": tool_name,
    ...
    loop {
        let event = tokio::time::timeout(Duration::from_secs(30), test.workx.next_event())
            .await

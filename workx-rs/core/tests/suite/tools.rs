...
async fn namespaced_custom_tool_call_preserves_namespace_through_dispatch_and_replay() -> Result<()>
{
    ...
    let expected_escaped_calls = json!([{
        "name": format!("{namespace}__{tool_name}"),
        "arguments": {
            "_workx_executed_tool_call_truncated": {
    ...
    assert_eq!(
        ...
        json!([{
            "name": workx_code_mode::PUBLIC_TOOL_NAME,
            "arguments": input,
        }]),
    );
...
async fn collect_tools(availability: CommandToolAvailability) -> Result<Vec<String>> {
    ...
    let mut builder = match availability {
        CommandToolAvailability::Default => test_workx(),
        CommandToolAvailability::ManagedUnifiedExecDisabled => test_workx()
...
"#,
        ...
        CommandToolAvailability::ShellToolDisabled => test_workx().with_config(|config| {
            config
                .features
        ...
        CommandToolAvailability::ModelDisabled => {
            test_workx().with_model_info_override("gpt-5.4", |model| {

...
use core_test_support::test_workx::local_selections;
use std::fs;
use std::path::Path;
...
use core_test_support::test_workx::TestWorkxBuilder;
use core_test_support::test_workx::TestWorkxHarness;
use core_test_support::test_workx::test_workx as base_test_workx;
...
use workx_core::X_WORKX_ROUTING_HINT_HEADER;
use workx_core::compact::SUMMARY_PREFIX;
use workx_features::Feature;
use workx_history::InitialHistory;
use workx_history::RolloutItem;
use workx_history::RolloutLine;
use workx_history::WorkxHarnessMetadata;
use workx_login::WorkxAuth;
use workx_login::auth::AgentIdentityAuth;
use workx_login::auth::AgentIdentityAuthRecord;
use workx_login::auth::BedrockApiKeyAuth;
use workx_model_provider_info::AMAZON_BEDROCK_GPT_5_5_MODEL_ID;
use workx_model_provider_info::AMAZON_BEDROCK_PROVIDER_ID;
use workx_model_provider_info::ModelProviderInfo;
use workx_protocol::AgentPath;
use workx_protocol::account::PlanType as AccountPlanType;
use workx_protocol::config_types::ServiceTier;
use workx_protocol::dynamic_tools::DynamicToolCallOutputContentItem;
use workx_protocol::dynamic_tools::DynamicToolFunctionSpec;
use workx_protocol::dynamic_tools::DynamicToolNamespaceSpec;
use workx_protocol::dynamic_tools::DynamicToolNamespaceTool;
...
use workx_protocol::protocol::RealtimeConversationRealtimeEvent;
use workx_protocol::protocol::RealtimeEvent;
use workx_protocol::protocol::RealtimeOutputModality;
...
fn test_workx() -> TestWorkxBuilder {
    base_test_workx().with_config(|config| {
...
fn remote_realtime_test_workx_builder(
    realtime_server: &responses::WebSocketTestServer,
) -> TestWorkxBuilder {
    let realtime_base_url = realtime_server.uri().to_string();
    test_workx()
        .with_auth(WorkxAuth::from_api_key("dummy"))
...
async fn start_realtime_conversation(workx: &workx_core::WorkxThread) -> Result<()> {
    workx
        .submit(Op::RealtimeConversationStart(ConversationStartParams {
            client_managed_handoffs: false,
            delegation_ack_filler: None,
            flush_transcript_tail_on_session_end: false,
            workx_responses_as_items: false,
            workx_response_item_prefix: None,
            workx_response_handoff_mode:
                workx_protocol::protocol::WorkxResponseHandoffMode::Thinking,
            workx_response_handoff_channel_prefixes: None,
            model: None,
    ...
    wait_for_event_match(workx, |msg| match msg {
...
async fn close_realtime_conversation(workx: &workx_core::WorkxThread) -> Result<()> {
    workx.submit(Op::RealtimeConversationClose).await?;
    wait_for_event_match(workx, |msg| match msg {
...
async fn wait_for_turn_complete(workx: &workx_core::WorkxThread) {
...
fn amazon_bedrock_test_workx() -> TestWorkxBuilder {
    let auth = WorkxAuth::BedrockApiKey(BedrockApiKeyAuth {
...
async fn amazon_bedrock_manual_compaction_uses_v2_responses_endpoint() -> Result<()> {
    ...
    let harness = TestWorkxHarness::with_auto_env_builder(amazon_bedrock_test_workx()).await?;
    ...
    harness.test().workx.submit(Op::Compact).await?;
    wait_for_turn_complete(&harness.test().workx).await;
...
async fn remote_compact_v2_retains_metadata_from_resumed_history() -> Result<()> {
    ...
    initial.workx.shutdown_and_wait().await?;
    annotate_retained_user_in_rollout(&rollout_path, retained_text)?;

    let resumed = builder()
        .resume(&server, home, rollout_path.clone())
        .await?;
    resumed.workx.submit(Op::Compact).await?;
    wait_for_turn_complete(&resumed.workx).await;
    resumed.workx.shutdown_and_wait().await?;
...
async fn remote_compact_v2_retains_only_client_developer_messages_when_enabled(
    ...
    let harness = TestWorkxHarness::with_auto_env_builder(
        test_workx()
            .with_auth(WorkxAuth::create_dummy_chatgpt_auth_for_testing())
            .with_config(move |config| {
                config
    ...
    let workx = &harness.test().workx;
    let rollout_path = workx.rollout_path().context("rollout path")?;
    workx
    ...
    harness.test().submit_turn("before compact").await?;
    workx.submit(Op::Compact).await?;
    wait_for_turn_complete(workx).await;
    harness.test().submit_turn("after compact").await?;
...
async fn remote_compact_v2_records_usage_before_output_validation() -> Result<()> {
    ...
    harness.test().submit_turn("before compact").await?;
    workx.submit(Op::Compact).await?;
    wait_for_event(workx, |event| matches!(event, EventMsg::Error(_))).await;
    wait_for_event(workx, |event| matches!(event, EventMsg::TurnComplete(_))).await;
    workx.shutdown_and_wait().await?;
...
async fn amazon_bedrock_automatic_compaction_uses_v2_responses_endpoint() -> Result<()> {
    ...
    let harness = TestWorkxHarness::with_auto_env_builder(amazon_bedrock_test_workx().with_config(
        |config| {
...
async fn remote_compact_replaces_history_for_followups() -> Result<()> {
    ...
    let harness = TestWorkxHarness::with_builder(
        test_workx()
            .with_history_mode(ThreadHistoryMode::Paginated)
            .with_auth(WorkxAuth::create_dummy_chatgpt_auth_for_testing()),
    )
    .await?;
    let workx = harness.test().workx.clone();
...
async fn remote_compact_uses_agent_identity_assertion() -> Result<()> {
    ...
    let harness = TestWorkxHarness::with_builder(
        test_workx().with_auth(WorkxAuth::AgentIdentity(
            AgentIdentityAuth::from_record(
                ...
                &workx_login::test_support::transport_default_auth_route_config(),
            )
            .await?,
        )),
    )
    ...
    assert_eq!(
        compact_request.header(X_WORKX_ROUTING_HINT_HEADER),
...
async fn assert_remote_manual_compact_request_parity(
    ...
    let uses_workx_backend = auth.uses_workx_backend();
    ...
    let harness = TestWorkxHarness::with_builder(builder).await?;
    let workx = harness.test().workx.clone();
    let image_url =
    ...
    assert_eq!(
        normal_request.header(X_WORKX_ROUTING_HINT_HEADER),
    ...
    let image_inputs = (1..=8)
        .map(|number| {
            ...
            Ok(UserInput::Image {
                ...
                detail: Some(workx_protocol::models::ImageDetail::Original),
            })
        })
...
async fn remote_compact_filters_deferred_dynamic_tools() -> Result<()> {
    ...
    let mut builder = test_workx().with_auth(WorkxAuth::create_dummy_chatgpt_auth_for_testing());
    ...
    let dynamic_tools = vec![DynamicToolSpec::Namespace(DynamicToolNamespaceSpec {
        name: "workx_app".to_string(),
        description: "Workx app tools.".to_string(),
        tools: vec![
    ...
    let new_thread = test
        ...
        .await?;
    test.workx = new_thread.thread;
    test.session_configured = new_thread.session_configured;
    let workx = test.workx.clone();
    ...
    assert_eq!(
        namespace_child_tool_names(&first_response_body, "workx_app"),
        vec![visible_tool.to_string()]
    );
    assert_eq!(
        namespace_child_tool_names(&compact_body, "workx_app"),
        vec![visible_tool.to_string()]
    );
...
async fn remote_compact_does_not_charge_inline_audio_payload_as_text() -> Result<()> {
    ...
    let responses_mock = responses::mount_sse_sequence(
        ...
        vec![
            sse(vec![
                ...
                responses::ev_function_call_with_namespace(call_id, "workx_app", tool_name, "{}"),
    ...
    test.workx
        ...
        .await?;
    let EventMsg::DynamicToolCallRequest(request) = wait_for_event(&test.workx, |event| {
    ...
    test.workx
        ...
        .await?;
    wait_for_turn_complete(&test.workx).await;
...
async fn remote_compact_runs_automatically() -> Result<()> {
    ...
    let message = wait_for_event_match(&workx, |event| match event {
...
async fn auto_remote_compact_failure_stops_agent_loop() -> Result<()> {
    ...
    let error_message = wait_for_event_match(&workx, |event| match event {
...
async fn remote_compact_trim_estimate_uses_session_base_instructions() -> Result<()> {
    ...
    let baseline_harness = TestWorkxHarness::with_builder(
        test_workx()
            .with_auth(WorkxAuth::create_dummy_chatgpt_auth_for_testing())
            .with_config(|config| {
    ...
    .await?;
    let baseline_workx = baseline_harness.test().workx.clone();
    ...
    baseline_workx
        ...
        .await?;
    wait_for_event(&baseline_workx, |event| {
    ...
    baseline_workx.submit(Op::Compact).await?;
    wait_for_event(&baseline_workx, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
    ...
    let override_harness = TestWorkxHarness::with_builder(
        test_workx()
            .with_auth(WorkxAuth::create_dummy_chatgpt_auth_for_testing())
            .with_config({
    ...
    .await?;
    let override_workx = override_harness.test().workx.clone();
    ...
    override_workx
        ...
        .await?;
    wait_for_event(&override_workx, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
    ...
    override_workx.submit(Op::Compact).await?;
    wait_for_event(&override_workx, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
...
async fn remote_manual_compact_emits_context_compaction_items() -> Result<()> {
    ...
    let harness = TestWorkxHarness::with_builder(
        test_workx().with_auth(WorkxAuth::create_dummy_chatgpt_auth_for_testing()),
    )
    .await?;
    let workx = harness.test().workx.clone();
    ...
    {
        let event = workx.next_event().await.unwrap();
        match event.msg {
...
async fn remote_compact_persists_replacement_history_in_rollout() -> Result<()> {
    ...
    workx
        ...
        .await?;
    wait_for_event(&workx, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;

    workx.submit(Op::Compact).await?;
    wait_for_event(&workx, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;

    workx.submit(Op::Shutdown).await?;
    wait_for_event(&workx, |ev| matches!(ev, EventMsg::ShutdownComplete)).await;
...
async fn remote_compact_and_resume_refresh_stale_developer_instructions() -> Result<()> {
    ...
    let mut start_builder = test_workx()
        .with_auth(WorkxAuth::create_dummy_chatgpt_auth_for_testing())
    ...
    initial.workx.submit(Op::Compact).await?;
    wait_for_event(&initial.workx, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
    ...
    initial.workx.submit(Op::Shutdown).await?;
    wait_for_event(&initial.workx, |ev| {
    ...
    resumed
        ...
        .await?;
    wait_for_event(&resumed.workx, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn remote_compact_refreshes_stale_developer_instructions_without_resume() -> Result<()> {
    ...
    test.workx
        ...
        .await?;
    wait_for_event(&test.workx, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn snapshot_request_shape_remote_pre_turn_compaction_restates_realtime_start() -> Result<()> {
    ...
    let mut builder = remote_realtime_test_workx_builder(&realtime_server).with_config(|config| {
        config.model_auto_compact_token_limit = Some(200);
    });
    let test = builder.build(&server).await?;
    ...
    start_realtime_conversation(test.workx.as_ref()).await?;
    ...
    close_realtime_conversation(test.workx.as_ref()).await?;
...
async fn remote_request_uses_custom_experimental_realtime_start_instructions() -> Result<()> {
    ...
    let mut builder = remote_realtime_test_workx_builder(&realtime_server).with_config({
...
async fn active_realtime_does_not_diff_changed_start_instructions_after_resume() -> Result<()> {
    ...
    let mut initial_builder = remote_realtime_test_workx_builder(&initial_realtime_server)
        .with_config({
    ...
    start_realtime_conversation(initial.workx.as_ref()).await?;
    initial.submit_turn("USER_ONE").await?;
    close_realtime_conversation(initial.workx.as_ref()).await?;
    initial.workx.submit(Op::Shutdown).await?;
    wait_for_event(&initial.workx, |ev| {
        matches!(ev, EventMsg::ShutdownComplete)
    })
    ...
    let mut resume_builder = remote_realtime_test_workx_builder(&resumed_realtime_server)
        .with_config({
    ...
    start_realtime_conversation(resumed.workx.as_ref()).await?;
    ...
    close_realtime_conversation(resumed.workx.as_ref()).await?;
...
async fn snapshot_request_shape_remote_manual_compact_restates_realtime_start() -> Result<()> {
    ...
    let server = wiremock::MockServer::start().await;
    let realtime_server = start_remote_realtime_server().await;
    let mut builder = remote_realtime_test_workx_builder(&realtime_server);
    let test = builder.build(&server).await?;

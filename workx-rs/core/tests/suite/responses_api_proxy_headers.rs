...
-> Result<()> {
    ...
    let mut builder = test_workx().with_config(|config| {
        ...
        config
            ...
            .insert("workx_security_surface".to_string(), "sdk".to_string());
    });
    ...
    assert_eq!(parent_turn_metadata["workx_security_surface"], json!("sdk"));
    ...
    assert_eq!(child_turn_metadata["workx_security_surface"], json!("sdk"));

    Ok(())
}

async fn submit_turn_with_timeout(test: &TestWorkx, prompt: &str) -> Result<()> {

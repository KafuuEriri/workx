...
use workx_config::types::McpServerAuth;
...
use workx_core::EnvironmentConfig;
use workx_core::EnvironmentMcpPolicy;
use workx_core::TurnInputRequest;
use workx_core::config::Config;
use workx_core::windows_sandbox::WindowsSandboxLevelExt;
use workx_exec_server::CreateDirectoryOptions;
use workx_exec_server::Environment;
use workx_exec_server::HttpRedirectPolicy;
use workx_exec_server::HttpRequestParams;
use workx_features::Feature;
use workx_http_client::HttpClientBuilder;
use workx_login::WorkxAuth;
use workx_mcp::MCP_SANDBOX_STATE_META_CAPABILITY;
...
use workx_protocol::mcp_policy::McpServerIdentity;
use workx_protocol::mcp_policy::McpServerRequirement;
use workx_protocol::mcp_policy::PluginMcpRequirements;
...
use workx_protocol::openai_models::ConfigShellToolType;
use workx_protocol::openai_models::ConfirmationPolicies;
...
use workx_protocol::protocol::McpStartupStatus;
...
use workx_utils_cargo_bin::cargo_bin;
...
fn read_only_user_turn(fixture: &TestWorkx, text: impl Into<String>) -> TurnInputRequest {
...
fn auto_approved_user_turn(fixture: &TestWorkx, text: impl Into<String>) -> TurnInputRequest {
...
pub(super) fn remote_aware_environment_id() -> String {
    if is_remote_test_environment() {
        ...
        workx_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID.to_string()
    }
}
...
fn unique_remote_path(binary_name: &str) -> anyhow::Result<String> {
    ...
    Ok(format!(
        "/tmp/workx-remote-env/{binary_name}-{}-{unique_suffix}",
        std::process::id()
    ))
}
...
async fn call_structured_tool(
    ...
    wait_for_event(&fixture.workx, |ev| {
        matches!(ev, EventMsg::McpToolCallBegin(_))
    })
    .await;
    let end_event = wait_for_event(&fixture.workx, |ev| {
    ...
    wait_for_event(&fixture.workx, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn assert_openai_form_capability_advertisement(expected: bool) -> anyhow::Result<()> {
    ...
    wait_for_mcp_server(&fixture.workx, server_name).await?;
...
async fn mcp_namespace_instructions_are_preserved_without_hiding_tools() -> anyhow::Result<()> {
    ...
    let fixture = test_workx()
        ...
        .await?;
    wait_for_mcp_server(&fixture.workx, "bounded").await?;

    fixture
        .workx
        .start_or_steer_turn(read_only_user_turn(&fixture, "show the bounded MCP tools"))
        .await?;
    wait_for_event(&fixture.workx, |event| {
...
async fn environment_mcp_policy_filters_runtime_config_and_model_tools(
    ...
    let workx_home = Arc::new(tempdir()?);
    let test_env = test_env().await?;
    if from_plugin {
        let plugin_root =
            super::plugins::write_sample_plugin_manifest_and_config(workx_home.as_ref());
    ...
    .await?;
    let (pending_config, _) = fixture.workx.current_mcp_config_and_runtime_context().await;
    ...
    let (runtime_config, _) = fixture.workx.current_mcp_config_and_runtime_context().await;
    ...
    fixture
        ...
        .await?;
    let (failed_config, _) = fixture.workx.current_mcp_config_and_runtime_context().await;
...
async fn stdio_server_round_trip(server_name: &'static str, namespace: &str) -> anyhow::Result<()> {
    ...
    let begin_event = wait_for_event(&fixture.workx, |ev| {
        matches!(ev, EventMsg::McpToolCallBegin(_))
    })
    .await;
...
async fn stdio_mcp_tool_names_respect_selected_servers() -> anyhow::Result<()> {
    ...
    let command = remote_aware_stdio_server_bin()?;
    let fixture = test_workx()
        .with_pre_build_hook(move |workx_home| {
            fs::write(
...
"#,
        ...
        .build_with_auto_env(&server)
        .await?;
    wait_for_mcp_server(&fixture.workx, "history").await?;
...
-> anyhow::Result<()> {
    ...
    let startup = loop {
        let event = fixture.workx.next_event().await?;
...
async fn apps_enabled_turn_skips_pending_optional_mcp_without_cached_tools() -> anyhow::Result<()> {
    ...
    tokio::time::timeout(Duration::from_secs(5), async {
        loop {
            let event = fixture
                ...
                .next_event()
                .await
                .context("event stream ended before Workx Apps became ready")?;
            if let EventMsg::McpStartupUpdate(update) = event.msg
                && update.server == WORKX_APPS_MCP_SERVER_NAME
    ...
    .await
    .context("Workx Apps should finish starting before the first turn")??;
    ...
    tokio::time::timeout(Duration::from_secs(2), fixture.workx.shutdown_and_wait())
        .await
...
async fn interrupt_during_mcp_startup_preserves_user_input_in_history(
    ...
    fixture.workx.submit(Op::Interrupt).await?;
    wait_for_event(&fixture.workx, |event| {
...
async fn stdio_mcp_tool_call_includes_sandbox_state_meta(
    ...
    let namespace = format!("mcp__{server_name}");
    let mut models = workx_models_manager::bundled_models_response()?;
    let model = models
        .models
        .iter_mut()
    ...
    fixture
        ...
        .list_models(
            ...
            workx_core::test_support::default_http_client_factory(),
        )
        .await;
    ...
    assert_eq!(
        ...
        SandboxState {
            ...
            workx_linux_sandbox_exe: fixture.config.workx_linux_sandbox_exe.clone(),
...
async fn stdio_mcp_parallel_tool_calls_default_false_runs_serially() -> anyhow::Result<()> {
    ...
    while call_events.len() < 4 {
        let event = wait_for_event(&fixture.workx, |ev| {
            matches!(
                ev,
        ...
        ModelsResponse {
            models: vec![ModelInfo {
                ...
                supported_reasoning_levels: vec![ReasoningEffortPreset {
                    effort: workx_protocol::openai_models::ReasoningEffort::Medium,
...
const REMOTE_EXEC_SERVER_URL_ENV_VAR: &str = "WORKX_TEST_REMOTE_EXEC_SERVER_URL";
...
impl StreamableHttpTestServer {
    /// Returns the MCP endpoint URL that Workx should connect to.
    fn url(&self) -> &str {
        &self.server_url
    }
...
/// What this tests: Workx can discover and call a Streamable HTTP MCP tool in
...
async fn streamable_http_tool_call_round_trip(mode: HeadersHelperMode) -> anyhow::Result<()> {
    ...
    // Phase 1: script the model responses so Workx will call the MCP echo tool
    ...
    // Phase 3: configure Workx with the Streamable HTTP MCP server and build a
    ...
    // Phase 5: assert Workx begins the expected tool invocation.
    let begin_event = wait_for_event(&fixture.workx, |ev| {
        matches!(ev, EventMsg::McpToolCallBegin(_))
    })
    .await;
...
async fn streamable_http_configured_auth_precedes_chatgpt_auth() -> anyhow::Result<()> {
    ...
    let configured_auth_fixture = test_workx()
        .with_auth(WorkxAuth::create_dummy_chatgpt_auth_for_testing())
        .with_config(move |config| {
            insert_mcp_server(
                config,
    ...
    wait_for_mcp_server(&configured_auth_fixture.workx, "configured_auth").await?;
...
async fn streamable_http_chatgpt_auth_is_not_sent_to_configured_origin() -> anyhow::Result<()> {
    ...
    wait_for_mcp_server(&fixture.workx, "untrusted_origin").await?;
...
/// This test writes to a fallback credentials file in WORKX_HOME.
/// Ideally, we wouldn't need to serialize the test but it's much more cumbersome to wire WORKX_HOME through the code.
#[test]
#[serial(workx_home)]
...
async fn streamable_http_with_oauth_round_trip_impl() -> anyhow::Result<()> {
    ...
    // Phase 1: script the model responses so Workx will call the OAuth-backed
    ...
    // Phase 3: seed an isolated WORKX_HOME with fallback OAuth tokens for this
    // server so the test does not share credentials with other suite cases.
    let temp_home = Arc::new(tempdir()?);
    let _workx_home_guard = EnvVarGuard::set("WORKX_HOME", temp_home.path().as_os_str());
    let unset_authorization_env_var = format!(
        "WORKX_TEST_UNSET_MCP_OAUTH_AUTHORIZATION_{}",
        std::process::id()
    );
    ...
    // Phase 4: configure Workx with the OAuth-backed Streamable HTTP MCP
    ...
    let recovery_hint = if credential_config.is_local_environment() {
        format!("Run `workx mcp login {server_name}`.")
    } else {
    ...
    let startup = wait_for_event(&fixture.workx, |event| {
    ...
    let (contended_turn, refreshed_failure, refreshed_failed_servers) =
        tokio::time::timeout(Duration::from_secs(5), async {
            ...
            {
                match fixture.workx.next_event().await?.msg {
    ...
    let discovered_turn = tokio::time::timeout(Duration::from_secs(5), async {
        ...
        fixture
            ...
            .await?;
        loop {
            if let EventMsg::TurnComplete(turn) = fixture.workx.next_event().await?.msg {
    ...
    tokio::time::timeout(
        ...
        wait_for_mcp_server(&fixture.workx, discovered_server_name),
    )
    .await
    .context("the newly discovered OAuth server did not recover after its store was unlocked")??;

    assert!(workx_rmcp_client::delete_oauth_tokens(
        discovered_credential_name.as_ref(),
        http_server.url(),
        OAuthCredentialsStoreMode::File,
        workx_config::types::AuthKeyringBackendKind::default(),
    )?);
    fixture.workx.refresh_runtime_config(refreshed_config).await;
    ...
    // Phase 7: assert Workx begins the expected tool invocation.
    let begin_event = wait_for_event(&fixture.workx, |ev| {
        matches!(
            ev,
...
async fn start_remote_streamable_http_test_server(
    ...
    // remote HTTP before handing the URL to the Workx fixture.
...
fn write_fallback_oauth_tokens(
    ...
    workx_rmcp_client::save_oauth_tokens(
        server_name,
        &tokens,
        OAuthCredentialsStoreMode::File,
        workx_config::types::AuthKeyringBackendKind::default(),

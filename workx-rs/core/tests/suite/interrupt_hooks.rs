...
async fn build_test(server: &MockServer, system_message: Option<&str>) -> Result<TestWorkx> {
...
async fn start_interruptible_turn(test: &TestWorkx, server: &MockServer) -> Result<()> {
    let tool_args = json!({
        "cmd": "sleep 60",
    ...
    test.workx
        ...
        .await?;
    let _ = wait_for_event_match(&test.workx, |event| match event {
...
async fn interrupt_hook_runs_before_turn_aborted_and_records_payload() -> Result<()> {
    ...
    let started = wait_for_event_match(&test.workx, |event| match event {
    ...
    .await;
    let completed = wait_for_event_match(&test.workx, |event| match event {
    ...
    .await;
    let _aborted = wait_for_event_match(&test.workx, |event| match event {
    ...
    let hook_inputs = read_interrupt_hook_inputs(test.workx_home_path())?;
...
async fn timed_out_interrupt_hook_fails_before_turn_aborted() -> Result<()> {
    ...
    fs::write(
        test.workx_home_path().join("interrupt_hook.py"),
        "import time\ntime.sleep(60)\n",
    )?;
    start_interruptible_turn(&test, &server).await?;
...
async fn async_interrupt_hook_runs_without_delaying_turn_aborted() -> Result<()> {
    ...
    fs::write(
        test.workx_home_path().join("async_interrupt_release"),
        "ready",
    )?;
    fs_wait::wait_for_path_exists(
        test.workx_home_path().join("async_interrupt_finished"),
        Duration::from_secs(5),
    )
    .await
    .context("async interrupt hook should finish after the turn abort")?;
    assert_eq!(read_interrupt_hook_inputs(test.workx_home_path())?.len(), 1);
...
async fn self_aborted_turn_runs_interrupt_hook_before_turn_aborted() -> Result<()> {
    ...
    let interrupt = wait_for_event_match(&test.workx, |event| match event {
        EventMsg::HookCompleted(completed)
            if completed.run.event_name == HookEventName::Interrupt =>
        {
            Some(completed.clone())

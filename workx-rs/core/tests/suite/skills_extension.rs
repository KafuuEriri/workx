...
use workx_extension_api::ExtensionEventSink;
use workx_extension_api::ExtensionFuture;
use workx_extension_api::ExtensionRegistryBuilder;
use workx_extension_api::ExtensionWarning;
use workx_extension_api::SkillInvocationContributor;
use workx_extension_api::SkillInvocationInput;
...
use workx_skills_extension::ExecutorSkillProvider;
use workx_skills_extension::HostSkillProvider;
use workx_skills_extension::OrchestratorSkillProvider;
use workx_skills_extension::SkillProvider;
use workx_skills_extension::SkillProviderSource;
use workx_skills_extension::SkillProviders;
use workx_skills_extension::SkillsExtensionConfig;
use workx_skills_extension::catalog::SkillAuthority;
use workx_skills_extension::catalog::SkillCatalog;
use workx_skills_extension::catalog::SkillCatalogEntry;
use workx_skills_extension::catalog::SkillPackageId;
use workx_skills_extension::catalog::SkillProviderError;
use workx_skills_extension::catalog::SkillReadResult;
use workx_skills_extension::catalog::SkillResourceId;
use workx_skills_extension::catalog::SkillSearchResult;
use workx_skills_extension::catalog::SkillSourceKind;
use workx_skills_extension::install;
use workx_skills_extension::install_with_providers;
use workx_skills_extension::provider::SkillListQuery;
use workx_skills_extension::provider::SkillProviderFuture;
use workx_skills_extension::provider::SkillReadRequest;
use workx_skills_extension::provider::SkillSearchRequest;
...
fn write_host_skills(workx_home: &std::path::Path, skills: &[(&str, &str)]) -> Result<()> {
...
fn catalog_extensions(
    ...
    Arc<workx_extension_api::ExtensionRegistry<Config>>,
...
fn configure_catalog_test(config: &mut Config) {
    ...
    let system_config_path = config.workx_home.join("config.toml");
...
async fn rendered_catalogs_for_turns(
    ...
    if !host_skills.is_empty() {
        write_host_skills(workx_home.path(), host_skills)?;
    }
    ...
    for _ in 0..turn_count {
        test.workx
            ...
            .await?;
        loop {
            match core_test_support::wait_for_event(&test.workx, |_| true).await {
    ...
    // while core warnings are delivered through the TestWorkx event stream.
    ...
    let _workx_home_guard = workx_home;
...
async fn capability_sections_render_in_order_with_host_repo_and_plugin_skills() -> Result<()> {
    ...
    let workx_home = Arc::new(TempDir::new()?);
    let host_skill_dir = workx_home.path().join("skills/host-search");
    ...
    let plugin_root = workx_home.path().join("plugins/cache/test/sample/local");
    ...
    core_test_support::wait_for_event(&test.workx, |event| {
...
async fn agent_plugin_skill_prompt_stays_bounded_without_skills_extension() -> Result<()> {
    ...
    let mut builder = test_workx().with_home(workx_home);
    let test = builder.build_with_auto_env(&server).await?;
    ...
    test.workx
        ...
        .await?;
    let warning = core_test_support::wait_for_event(&test.workx, |event| {
        matches!(
            event,
...
text({ names: result.skills.map(skill => skill.name), warnings: result.warnings, next_cursor: result.next_cursor });"#,
    ...
    let test = builder.build_with_auto_env(&server).await?;
    wait_for_mcp_server(&test.workx, WORKX_APPS_MCP_SERVER_NAME).await?;
...
async fn opted_in_executor_provider_skips_host_discovery_but_injects_discovered_skill() -> Result<()>
{
    ...
    let host_skill_path = workx_home
        .path()
        .join("skills")
...
async fn executor_skill_invocation_is_environment_scoped_and_deduplicated() -> Result<()> {
    ...
    let server = responses::start_mock_server().await;
    let workx_home = Arc::new(TempDir::new()?);
    let skill_path = workx_home.path().join("executor-skill/SKILL.md");
...
async fn production_turn_aliases_combined_skill_catalogs_under_shared_budget() -> Result<()> {
    ...
    .await;
    let workx_home = Arc::new(
        tempfile::Builder::new()
            .prefix("workx-integration-shared-skill-catalog-roots-")
            .tempdir()?,
    );
    write_host_skills(
        workx_home.path(),
    ...
    let developer_text = response
        ...
        .message_input_texts("developer")
        .join("\n");
    let host_root = dunce::canonicalize(workx_home.path().join("skills"))?
        .to_string_lossy()
        .replace('\\', "/");
    for (alias, root) in [
        ("e0", EXECUTOR_ROOT),
...
async fn production_turn_uses_configured_skill_catalog_token_budget() -> Result<()> {
    ...
    write_host_skills(workx_home.path(), &HOST_CATALOG)?;
...
async fn production_turn_preserves_host_alias_root_order_across_turns() -> Result<()> {
    ...
    let long_parent = temp_parent
        .path()
        .join("workx-home-with-long-shared-prefix-for-production-alias-order-test");
    std::fs::create_dir_all(&long_parent)?;
    let workx_home = Arc::new(TempDir::new_in(&long_parent)?);
...
async fn production_turn_uses_provider_host_catalog_and_core_snapshot_injection() -> Result<()> {
    ...
    write_host_skills(workx_home.path(), &[(skill_name, snapshot_description)])?;
    let skill_path = workx_home
        .path()
        .join("skills")
        .join(skill_name)
        .join("SKILL.md");
    ...
    let app_mentioned_events =
        wait_for_analytics_events(&server, "workx_app_mentioned", /*expected_count*/ 1).await;
...
async fn production_turn_suppresses_only_the_superseded_host_skill_prompt() -> Result<()> {
    ...
    let first_skill_path = workx_home.path().join("skills/first-host/SKILL.md");
    let second_skill_path = workx_home.path().join("skills/second-host/SKILL.md");
...
async fn production_turn_warns_and_omits_unreadable_host_skill() -> Result<()> {
    ...
    let missing_skill_path =
        dunce::canonicalize(workx_home.path().join("skills/missing-host/SKILL.md"))?;
    let available_skill_path =
        dunce::canonicalize(workx_home.path().join("skills/available-host/SKILL.md"))?;
...
async fn production_turn_keeps_full_snapshot_host_skill_prompt() -> Result<()> {
    ...
    .await;
    let workx_home = Arc::new(TempDir::new()?);
    let skill_dir = workx_home.path().join("skills").join("long-host");
    std::fs::create_dir_all(&skill_dir)?;
...
async fn production_turn_keeps_orchestrator_world_state_incremental_across_turns() -> Result<()> {
    ...
    let skill_resource = "skill://workx_apps/orchestrator-search/SKILL.md";
    let catalog = SkillCatalog {
        entries: vec![
            SkillCatalogEntry::new(
                SkillPackageId("orchestrator/orchestrator-search".to_string()),
                SkillAuthority::new(SkillSourceKind::Orchestrator, WORKX_APPS_MCP_SERVER_NAME),
                skill_name,
                skill_description,
...
async fn production_turn_fairly_shortens_extension_catalog_descriptions() -> Result<()> {
    ...
    assert_eq!(
        warning.message,
        "Skill descriptions were shortened to fit the skills context budget. Workx can still see every skill, but some descriptions are shorter. Disable unused skills or plugins to leave more room for the rest."
    );

...
use core_test_support::test_workx::ApplyPatchModelOutput;
...
use core_test_support::test_workx::executor_path_uri;
...
pub async fn apply_patch_harness() -> Result<TestWorkxHarness> {
    apply_patch_harness_with(|builder| builder).await
}

async fn apply_patch_harness_with(
    configure: impl FnOnce(TestWorkxBuilder) -> TestWorkxBuilder,
) -> Result<TestWorkxHarness> {
    let builder = configure(test_workx());
    // Box harness construction so apply_patch_cli tests do not inline the
    // full test-thread startup path into each test future.
    Box::pin(TestWorkxHarness::with_auto_env_builder(builder)).await
}

async fn submit_without_wait(harness: &TestWorkxHarness, prompt: &str) -> Result<()> {
...
#[cfg(target_os = "linux")]
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn apply_patch_cli_uses_workx_self_exe_with_linux_sandbox_helper_alias() -> Result<()> {
    skip_if_no_network!(Ok(()));

    let harness = apply_patch_harness().await?;
    let workx_linux_sandbox_exe = harness
        .test()
        .config
        .workx_linux_sandbox_exe
        .as_ref()
        .expect("linux test config should include workx-linux-sandbox helper");
    assert_eq!(
        workx_linux_sandbox_exe
            .file_name()
            .and_then(|name| name.to_str()),
        Some(WORKX_LINUX_SANDBOX_ARG0),
    );
...
async fn apply_patch_cli_does_not_widen_permissions_for_workspace_directory_target() -> Result<()> {
    ...
    let harness = apply_patch_harness_with(move |builder| {
        builder.with_config(move |config| {
            config.approvals_reviewer = workx_protocol::config_types::ApprovalsReviewer::User;
            config.workspace_roots = vec![harness_work_dir.clone()];
            config
                .permissions
                .set_workspace_roots(config.workspace_roots.clone());
...
async fn apply_patch_custom_tool_streaming_emits_updated_changes() -> Result<()> {
    ...
    assert_eq!(
        ...
        Some(&workx_protocol::protocol::FileChange::Add {
            content: String::new(),
        })
    );
    assert_eq!(
...
#[test_case("workx_work_web", true, "work.txt"; "web work uses cwd when enabled")]
#[test_case("workx_work_mobile", true, "work.txt"; "mobile work uses cwd when enabled")]
#[test_case("workx_work_web", false, "subdir/work.txt"; "disabled feature keeps repository root")]
#[test_case("workx_work_desktop", true, "work.txt"; "desktop uses cwd when enabled")]
#[test_case("workx_work_cca", true, "work.txt"; "legacy cca uses cwd when enabled")]
#[test_case("workx_cli_rs", true, "work.txt"; "coding originator uses cwd when enabled")]
#[test_case("workx_cli_rs", false, "subdir/work.txt"; "coding originator keeps repository root when disabled")]
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn apply_patch_turn_diff_skips_git_root_when_feature_is_enabled(
    ...
    let workx = test
        ...
        .start_thread(StartThreadOptions {
            ...
            environments: Some(test.workx.environment_selections().await),

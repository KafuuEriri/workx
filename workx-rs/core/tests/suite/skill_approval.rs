...
fn skill_script_command(test: &TestWorkx, script_name: &str) -> Result<String> {
...
async fn wait_for_exec_approval_request(test: &TestWorkx) -> Option<ExecApprovalRequestEvent> {
...
async fn wait_for_turn_complete(test: &TestWorkx) {
    wait_for_event(test.workx.as_ref(), |event| {
...
async fn shell_zsh_fork_still_enforces_workspace_write_sandbox() -> Result<()> {
    ...
    let outside_path = "/tmp/workx-zsh-fork-workspace-write-deny.txt";
    let workspace_write_profile = restrictive_workspace_write_profile();

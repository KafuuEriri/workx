...
use workx_protocol::config_types::ShellEnvironmentPolicyInherit;
...
use workx_protocol::protocol::ExecCommandStatus;
use workx_protocol::protocol::Op;
use workx_protocol::protocol::ThreadSettingsOverrides;
use workx_protocol::shell_environment::WORKX_EXEC_SERVER_NOISE_AUTH_TOKEN_ENV_VAR;
use workx_protocol::user_input::UserInput;
use workx_utils_output_truncation::approx_tokens_from_byte_count;
...
async fn wait_for_raw_unified_exec_output(
    ...
    let content = wait_for_event_match(&test.workx, |event| match event {
...
async fn exec_command_hides_and_rejects_login_when_disabled() -> Result<()> {
    ...
    let builder = test_workx().with_model("gpt-5.4").with_config(|config| {
...
async fn exec_command_does_not_expose_configured_noise_auth_token() -> Result<()> {
    ...
    let builder = test_workx().with_model("gpt-5.4").with_config(|config| {
        config.permissions.shell_environment_policy.r#set.insert(
            WORKX_EXEC_SERVER_NOISE_AUTH_TOKEN_ENV_VAR.to_string(),
            "configured-noise-token".to_string(),
        );
        config.permissions.shell_environment_policy.r#set.insert(
            WORKX_EXEC_SERVER_NOISE_AUTH_TOKEN_ENV_VAR.to_ascii_lowercase(),
            "case-variant-noise-token".to_string(),
        );
    });
    let harness = TestWorkxHarness::with_auto_env_builder(builder).await?;
    let command = match core_test_support::test_target_os() {
        core_test_support::TestTargetOs::Linux | core_test_support::TestTargetOs::MacOs => {
            "if [ -n \"${WORKX_EXEC_SERVER_NOISE_AUTH_TOKEN:-}\" ] || [ -n \"${workx_exec_server_noise_auth_token:-}\" ]; then echo leaked; else echo unset; fi"
        }
        core_test_support::TestTargetOs::Windows => {
            "if ($env:WORKX_EXEC_SERVER_NOISE_AUTH_TOKEN) { Write-Output leaked } else { Write-Output unset }"
        }
    };
...
async fn unified_exec_intercepts_apply_patch_exec_command() -> Result<()> {
    ...
    wait_for_event(&workx, |event| match event {
...
async fn unified_exec_rejects_justification_without_sandbox_permissions() -> Result<()> {
    ...
    let mut builder = test_workx().with_model("gpt-5.2");
...
async fn unified_exec_emits_exec_command_begin_event() -> Result<()> {
    ...
    let begin_event = wait_for_event_match(&test.workx, |msg| match msg {
...
async fn unified_exec_emits_exec_command_end_event() -> Result<()> {
    ...
    let end_event = wait_for_event_match(&test.workx, |msg| match msg {
...
async fn unified_exec_full_lifecycle_with_background_end_event() -> Result<()> {
    ...
    loop {
        let msg = wait_for_event(&test.workx, |_| true).await;
        match msg {
...
async fn unified_exec_network_denial_emits_failed_background_end_event() -> Result<()> {
    ...
    let args = json!({
        "cmd": "python3 -c \"import os, socket, time, urllib.parse; time.sleep(0.3); proxy = urllib.parse.urlparse(os.environ['HTTP_PROXY']); sock = socket.create_connection((proxy.hostname, proxy.port), timeout=2); sock.sendall(b'GET http://workx-network-denied.invalid/ HTTP/1.1\\r\\nHost: workx-network-denied.invalid\\r\\n\\r\\n'); sock.recv(1024); time.sleep(5)\"",
        "yield_time_ms": 50,
    });
    let response_mock =
...
async fn unified_exec_short_lived_network_denial_emits_failed_end_event() -> Result<()> {
    ...
    let args = json!({
        "cmd": "python3 -c \"import os, socket, urllib.parse; proxy = urllib.parse.urlparse(os.environ['HTTP_PROXY']); sock = socket.create_connection((proxy.hostname, proxy.port), timeout=2); sock.sendall(b'GET http://workx-short-network-denied.invalid/ HTTP/1.1\\r\\nHost: workx-short-network-denied.invalid\\r\\n\\r\\n'); sock.recv(1024)\"",
        "yield_time_ms": 1000,
    });
    let response_mock =
        mount_unified_exec_network_denial_responses(&server, call_id, &args).await?;
...
) -> Result<(TestWorkx, PermissionProfile)> {
    use std::sync::Arc;
...
async fn wait_for_unified_exec_end(
...
) -> (workx_protocol::protocol::ExecCommandEndEvent, bool) {
    ...
    let end_event = loop {
        ...
        let event = tokio::time::timeout(remaining, test.workx.next_event())
            .await
...
async fn unified_exec_emits_one_begin_and_one_end_event() -> Result<()> {
    ...
    let mut task_completed = false;
    loop {
        let event_msg = wait_for_event(&test.workx, |_| true).await;
        match event_msg {
...
async fn exec_command_clamps_model_requested_max_output_tokens_to_policy() -> Result<()> {
    ...
    let mut builder = test_workx().with_model("gpt-5.4").with_config(|config| {
...
async fn write_stdin_returns_exit_metadata_and_clears_session() -> Result<()> {
    ...
    let mut turn_completed = false;
    loop {
        let event = wait_for_event(&test.workx, |_| true).await;
        match event {
...
async fn unified_exec_keeps_long_running_session_after_turn_end() -> Result<()> {
    ...
    let begin_event = wait_for_event_match(&workx, |msg| match msg {
...
async fn unified_exec_interrupt_preserves_long_running_session() -> Result<()> {
    ...
    let _begin_event = wait_for_event_match(&workx, |msg| match msg {
        EventMsg::ExecCommandBegin(ev) if ev.call_id == call_id => Some(ev.clone()),
        _ => None,
    })
    .await;
    ...
    workx.submit(Op::Interrupt).await?;
    wait_for_event(&workx, |event| matches!(event, EventMsg::TurnAborted(_))).await;
    ...
    workx.submit(Op::CleanBackgroundTerminals).await?;
    wait_for_process_exit(&pid).await?;
...
async fn unified_exec_timeout_and_followup_poll() -> Result<()> {
    ...
    loop {
        let event = test.workx.next_event().await.expect("event");
...
async fn managed_unified_exec_disable_runs_commands_without_retained_authority() -> Result<()> {
    ...
    let server = start_mock_server().await;
    let mut builder = test_workx().with_cloud_config_bundle(
...
"#
    ...
    let end_event = wait_for_event_match(&test.workx, |event| match event {

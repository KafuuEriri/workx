...
async fn root_and_spawned_subagent_receive_distinct_mcp_session_sources() -> Result<()> {
    ...
    let test = core_test_support::test_workx::test_workx()
...
async fn timeout_refresh_replaces_pending_startup_and_reuses_ready_connection() -> Result<()> {
    ...
    test.workx.refresh_mcp_config(refresh_config).await;
...
async fn out_of_band_resource_read_reconciles_the_published_mcp_runtime() -> Result<()> {
    ...
    let mut refresh_config = test.config.clone();
    let user_config_path = refresh_config.workx_home.join("config.toml");
...
"#,
    ...
    let refreshed_servers = user_config
        .get("mcp_servers")
        .cloned()
        .map(HashMap::<String, workx_config::types::McpServerConfig>::deserialize)
        .transpose()?
    ...
    test.workx.refresh_runtime_config(refresh_config).await;
    test.workx.submit(Op::RefreshMcpServers).await?;
...
async fn elevated_apps_catalog_limit_requires_host_owned_registration() -> Result<()> {
    ...
    for extension_id in [None, Some("hosted_plugin_runtime"), Some("test-extension")] {
        ...
        let test = builder.build_with_auto_env(&server).await?;
        let startup = wait_for_mcp_server(&test.workx, WORKX_APPS_MCP_SERVER_NAME).await;
        ...
        if extension_id == Some("test-extension") {
            ...
            assert!(
                ...
                "an extension named workx_apps must not inherit the trusted Apps limit: {error}"
            );
            continue;
        }
...
async fn code_mode_only_exposes_direct_model_only_mcp_namespaces() -> Result<()> {
    ...
    assert!(
        exec_description.is_some_and(|description| {
            !description.contains("mcp__workx_apps__calendar_create_event(args:")
        }),
...
async fn initially_empty_deferred_tool_world_state_is_not_rendered_or_persisted() -> Result<()> {
    ...
    let mut builder = core_test_support::test_workx::test_workx()
        .with_config(enable_deferred_tool_world_state_without_agents);
    let test = builder.build_with_auto_env(&server).await?;
...
async fn deferred_tool_world_state_survives_resume_without_duplicate_updates() -> Result<()> {
    ...
    wait_for_mcp_server(&initial.workx, WORKX_APPS_MCP_SERVER_NAME).await?;
    initial
        .submit_turn("inspect deferred tools before resume")
        .await?;

    initial.workx.ensure_rollout_materialized().await;
    initial.workx.flush_rollout().await?;
    ...
    assert_eq!(
        persisted_tools,
        json!({
            "mcp__workx_apps__calendar": "Plan events and manage your calendar."
        })
    );
    ...
    drop(initial);
    wait_for_mcp_server(&resumed.workx, WORKX_APPS_MCP_SERVER_NAME).await?;
    resumed
...
async fn apps_guidance_and_deferred_namespace_appear_after_recovery_within_a_turn() -> Result<()> {
    ...
    test.workx
        ...
        .await?;
    let EventMsg::RequestUserInput(request) = wait_for_event(&test.workx, |event| {
    ...
    wait_for_event(&test.workx, |event| {
        matches!(
            ...
            EventMsg::McpStartupUpdate(update)
                if update.server == WORKX_APPS_MCP_SERVER_NAME
                    && matches!(update.status, workx_protocol::protocol::McpStartupStatus::Ready)
        )
    })
    .await;
...
-> Result<()> {
    ...
    tokio::fs::remove_dir_all(test.workx_home_path().join("cache/workx_apps_tools")).await?;
    startup_control.fail_next_initialize_attempts(/*attempts*/ 1);
    test.workx.submit(Op::RefreshMcpServers).await?;
    test.workx
        .start_or_steer_turn(TurnInputRequest::user_input(vec![UserInput::Text {

...
fn external_chatgpt_auth(access_token: &str) -> std::io::Result<WorkxAuth> {
    WorkxAuth::from_external_chatgpt_tokens(access_token, CHATGPT_ACCOUNT_ID, Some("enterprise"))
}
...
async fn header_auth_is_attached_to_responses_requests() -> anyhow::Result<()> {
    ...
    let mut builder = test_workx().with_auth(WorkxAuth::Headers(AuthHeaders::new(headers)));
...
async fn custom_provider_does_not_receive_ambient_auth_headers() -> anyhow::Result<()> {
    ...
    let mut builder = test_workx()
        .with_auth(WorkxAuth::Headers(AuthHeaders::new(headers)))
...
async fn amazon_bedrock_managed_access_keys_sign_requests() -> anyhow::Result<()> {
    ...
    let mut builder = test_workx()
        ...
        .with_auth(WorkxAuth::BedrockAccessKeys(BedrockAccessKeysAuth {
...
async fn amazon_bedrock_aws_auth_refresh_resigns() -> anyhow::Result<()> {
    ...
    const SUBPROCESS_ENV: &str = "WORKX_BEDROCK_AWS_REFRESH_TEST";
    const HELPER_ARG: &str = "WORKX_BEDROCK_AWS_REFRESH_COMMAND";
...
async fn external_auth_401_retry_uses_refreshed_chatgpt_headers() -> anyhow::Result<()> {
    ...
    let mut builder = test_workx().with_auth(WorkxAuth::from_api_key("seed"));
    let test = builder.build_with_auto_env(&server).await?;

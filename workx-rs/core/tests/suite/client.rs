use core_test_support::TestWorkxResponsesRequestKind;
...
use workx_core::ModelClient;
use workx_core::NewThread;
use workx_core::Prompt;
use workx_core::ResponseEvent;
...
use workx_core::resolve_installation_id;
use workx_core::thread_store_from_config;
use workx_extension_api::empty_extension_registry;
...
use workx_protocol::models::DEFAULT_IMAGE_DETAIL;
...
use workx_protocol::models::LocalShellAction;
use workx_protocol::models::LocalShellExecAction;
use workx_protocol::models::LocalShellStatus;
use workx_protocol::models::MessagePhase;
use workx_protocol::models::ReasoningItemContent;
use workx_protocol::models::ReasoningItemReasoningSummary;
use workx_protocol::models::ResponseItem;
use workx_protocol::models::WebSearchAction;
...
fn test_turn_responses_metadata(
    _client: &ModelClient,
    thread_id: ThreadId,
) -> workx_core::WorkxResponsesMetadata {
    ...
    test_responses_metadata(
        ...
        TestWorkxResponsesRequestKind::Turn,
    )
}
...
fn assert_workx_client_metadata(
...
async fn openai_stateless_responses_requests_preserve_item_turn_metadata_across_turns() {
    ...
    .await;
    let test = test_workx().build(&server).await.unwrap();
...
async fn response_item_ids_persist_across_resume_and_preserve_server_ids() -> anyhow::Result<()> {
    ...
    wait_for_event(&initial.workx, |event| {
...
async fn synthetic_call_output_id_is_stable_across_resumes() -> anyhow::Result<()> {
    ...
    .await;
    let workx_home = Arc::new(TempDir::new()?);
    let mut builder = test_workx();
    let first = builder
        .resume(&server, Arc::clone(&workx_home), session_path.clone())
        .await?;

    first.submit_turn("first resume").await?;
    first.workx.submit(Op::Shutdown).await?;
    wait_for_event(&first.workx, |event| {
        matches!(event, EventMsg::ShutdownComplete)
    })
    .await;
    assert!(
    ...
    let second = builder.resume(&server, workx_home, session_path).await?;
...
/// Writes an `auth.json` into the provided `workx_home` with the specified parameters.
...
fn write_auth_json(
    ...
    std::fs::write(
        workx_home.path().join("auth.json"),
...
"#,
    ...
    fn auth(&self) -> ModelProviderAuthInfo {
        ModelProviderAuthInfo {
            ...
            cwd: workx_utils_absolute_path::AbsolutePathBuf::try_from(self.tempdir.path())
...
async fn resume_includes_initial_messages_and_sends_prior_items() {
    ...
    let prior_user = workx_protocol::models::ResponseItem::Message {
        id: None,
        role: "user".to_string(),
    ...
    let prior_system = workx_protocol::models::ResponseItem::Message {
        id: None,
        role: "system".to_string(),
        content: vec![workx_protocol::models::ContentItem::OutputText {
    ...
    let prior_item = workx_protocol::models::ResponseItem::Message {
        id: None,
    ...
    // Configure Workx to resume from our file
    let workx_home = Arc::new(TempDir::new().unwrap());
    let mut builder = test_workx()
        .with_home(workx_home.clone())
        .with_pre_build_hook(|home| {
            std::fs::write(home.join("AGENTS.md"), "be nice").expect("write global instructions");
        });
    let test = builder
        .resume(&server, workx_home, session_path.clone())
        .await
...
async fn resume_replays_legacy_js_repl_image_rollout_shapes() {
    ...
    let workx_home = Arc::new(TempDir::new().unwrap());
    let mut builder = test_workx().with_model("gpt-5.4");
    let test = builder
        .resume(&server, workx_home, session_path.clone())
        .await
        .expect("resume conversation");
...
async fn includes_session_id_thread_id_and_model_headers_in_request() {
    ...
    let mut builder = test_workx().with_auth(WorkxAuth::from_api_key("Test API Key"));
    let test = builder
        .build(&server)
        .await
    ...
    assert_eq!(request.header(X_WORKX_ROUTING_HINT_HEADER), None);
    ...
    let request_body = request.body_json();
    let installation_id =
        std::fs::read_to_string(test.workx_home_path().join(INSTALLATION_ID_FILENAME))
...
async fn send_request_with_provider(provider: ModelProviderInfo) {
    ...
    let mut config = load_default_config_for_test(&workx_home).await;
    config.model_provider_id = provider.name.clone();
    config.model_provider = provider.clone();
    let effort = config.model_reasoning_effort.clone();
    let summary = config.model_reasoning_summary;
    let model = workx_core::test_support::get_model_offline(config.model.as_deref());
    config.model = Some(model.clone());
    let config = Arc::new(config);
    let model_info =
        workx_core::test_support::construct_model_info_offline(model.as_str(), &config);
    ...
    let client = ModelClient::new(
        Some(AuthManager::from_auth_for_testing(WorkxAuth::from_api_key(
            "unused-api-key",
        ))),
    ...
    let mut stream = client_session
        .stream(
            ...
            /*service_tier*/ None,
            &responses_metadata,
            &workx_rollout_trace::InferenceTraceContext::disabled(),
        )
        .await
...
async fn includes_base_instructions_override_in_request() {
    ...
    let mut builder = test_workx()
        .with_auth(WorkxAuth::from_api_key("Test API Key"))
        .with_config(|config| {
...
async fn chatgpt_auth_sends_correct_request() {
    ...
    let mut builder = test_workx()
        .with_auth(create_dummy_workx_auth())
        .with_config(move |config| {
    ...
    assert_eq!(
        request.header(X_WORKX_ROUTING_HINT_HEADER),
...
async fn prefers_apikey_when_config_prefers_apikey_even_with_chatgpt_tokens() {
    ...
    let auth = WorkxAuth::from_auth_storage(
        workx_home.path(),
    ...
    .await
    .expect("Failed to load WorkxAuth")
    .expect("No WorkxAuth found in workx_home");
    let auth_manager = workx_core::test_support::auth_manager_from_auth(auth);
    let installation_id = resolve_installation_id(&config.workx_home)
        .await
        .expect("resolve installation id");
    let thread_manager = ThreadManager::new(
        &config,
        auth_manager.clone(),
        workx_core::build_models_manager(&config, auth_manager),
        workx_core::WorkxAppsToolsCache::default(),
        SessionSource::Exec,
        Arc::new(workx_exec_server::EnvironmentManager::default_for_tests()),
        empty_extension_registry(),
        Arc::new(workx_core::test_support::EmptyUserInstructionsProvider),
    ...
    let NewThread { thread: workx, .. } = thread_manager
...
async fn omits_apps_guidance_when_orchestrator_mcp_is_disabled() {
    ...
    assert!(
        !request.body_contains_text("mcp__workx_apps"),
        "did not expect workx_apps MCP tools when orchestrator MCP is disabled, got {:?}",
    ...
    assert!(
        ...
        "did not expect workx_apps resource calls: {resource_methods:?}"
    );
}
...
async fn powershell_shell_version_is_model_visible_only_when_enabled() -> anyhow::Result<()> {
    ...
    let shell_path = workx_shell_command::powershell::try_find_powershell_executable_blocking()
    ...
    for enabled in [false, true] {
        ...
        let user_shell = workx_shell_command::shell_detect::DetectedShell {
            shell_type: workx_shell_command::shell_detect::ShellType::PowerShell,
...
async fn includes_configured_max_effort_in_request() -> anyhow::Result<()> {
    ...
    .await;
    let TestWorkx { workx, .. } = test_workx()
        .with_model("gpt-5.4")
        .with_config(|config| {
...
-> anyhow::Result<()> {
    ...
    .await;
    let TestWorkx { workx, .. } = test_workx().with_model("gpt-5.4").build(&server).await?;
...
async fn user_turn_collaboration_mode_overrides_model_and_effort() -> anyhow::Result<()> {
    ...
    .await;
    let TestWorkx { workx, config, .. } = test_workx().with_model("gpt-5.4").build(&server).await?;
...
async fn azure_responses_request_does_not_store_and_preserves_prefixed_item_ids() {
    ...
    let thread_id = ThreadId::new();
    let auth_manager =
        workx_core::test_support::auth_manager_from_auth(WorkxAuth::from_api_key("Test API Key"));
    let session_telemetry = SessionTelemetry::new(
        thread_id,
        model.as_str(),
        model_info.slug.as_str(),
...
async fn token_count_includes_rate_limits_snapshot() {
    ...
    let mut builder = test_workx()
        .with_auth(WorkxAuth::from_api_key("test"))
        .with_config(move |config| {
            config.model_provider = provider;
        });
    let workx = builder
    ...
    wait_for_event(&workx, |msg| matches!(msg, EventMsg::TurnComplete(_))).await;
}
...
async fn usage_limit_error_emits_rate_limit_event() -> anyhow::Result<()> {
    ...
    let mut builder = test_workx();
    let workx_fixture = builder.build(&server).await?;
    let workx = workx_fixture.workx.clone();
    ...
    let token_event = wait_for_event(&workx, |msg| matches!(msg, EventMsg::TokenCount(_))).await;
    ...
    let error_event = wait_for_event(&workx, |msg| matches!(msg, EventMsg::Error(_))).await;
...
async fn context_window_error_sets_total_tokens_to_model_window() -> anyhow::Result<()> {
    ...
    let token_event = wait_for_event(&workx, |event| {
        matches!(
            event,
    ...
    let error_event = wait_for_event(&workx, |ev| matches!(ev, EventMsg::Error(_))).await;
    let expected_context_window_message = WorkxErr::ContextWindowExceeded.to_string();
    assert!(
        matches!(
            error_event,
...
async fn env_var_overrides_loaded_auth() {
    ...
    assert_eq!(request.headers.get(X_WORKX_ROUTING_HINT_HEADER), None);
}

fn create_dummy_workx_auth() -> WorkxAuth {
...
async fn history_dedupes_streamed_and_final_messages_across_turns() {
    // Skip under Workx sandbox network restrictions (mirrors other tests).
    skip_if_no_network!();

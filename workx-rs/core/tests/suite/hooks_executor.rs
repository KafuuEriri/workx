...
async fn executor_interrupt_hook_runs_after_attachment() -> Result<()> {
    ...
    fixture.test.workx.submit(Op::Interrupt).await?;
    wait_for_event(&fixture.test.workx, |event| {
...
async fn executor_stop_hook_rejects_mismatched_environment() -> Result<()> {
    ...
    fixture
        ...
        .await;
    wait_for_mcp_server(&fixture.test.workx, "node_repl").await?;
    assert_eq!(
        fixture
            .test
            .workx
...
async fn executor_browser_and_computer_use_stop_hooks_use_separate_mcp_routes(
    ...
    if hook_event == "SubagentStop" {
        let child = fixture
            ...
            .start_thread(StartThreadOptions {
                ...
                environments: Some(fixture.test.workx.environment_selections().await),
                ..StartThreadOptions::new(fixture.test.config.clone())
            })
            .await?;
        fixture.test.workx = child.thread;
    ...
    assert!(node_params["_meta"].get("_workx_apps").is_none());
    ...
    assert_eq!(browser_params["_meta"]["_workx_apps"], routing);
...
async fn executor_plugin_hook_fixture(
    ...
    for (plugin_id, manifest) in plugins {
        ...
        let plugin_directory = plugin_root.join(".workx-plugin")?;
...
impl ExecutorHookFixture {
    ...
    async fn interrupt_running_command(&self) -> Result<()> {
        wait_for_event(&self.test.workx, |event| {
            matches!(event, EventMsg::ExecCommandBegin(_))
        })
        .await;
        self.test.workx.submit(Op::Interrupt).await?;
        wait_for_event(&self.test.workx, |event| {
            matches!(event, EventMsg::TurnAborted(_))
        })
        .await;

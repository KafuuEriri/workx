...
use workx_api::WS_REQUEST_HEADER_TRACEPARENT_CLIENT_METADATA_KEY;
use workx_api::WS_REQUEST_HEADER_TRACESTATE_CLIENT_METADATA_KEY;
use workx_core::ModelClient;
use workx_core::ModelClientSession;
use workx_core::Prompt;
use workx_core::ResponseEvent;
use workx_core::TurnInputRequest;
use workx_core::WorkxResponsesMetadata;
use workx_core::X_RESPONSESAPI_INCLUDE_TIMING_METRICS_HEADER;
use workx_core::X_WORKX_ROUTING_HINT_HEADER;
use workx_core::test_support::with_parent_turn;
...
use workx_rollout_trace::ConversationPart;
...
fn turn_metadata(harness: &WebsocketTestHarness, turn_id: Option<&str>) -> WorkxResponsesMetadata {
    responses_metadata(harness, turn_id, TestWorkxResponsesRequestKind::Turn)
}
...
fn prewarm_metadata(
    ...
    responses_metadata(harness, turn_id, TestWorkxResponsesRequestKind::Prewarm)
}

fn websocket_connection_metadata(harness: &WebsocketTestHarness) -> WorkxResponsesMetadata {
    responses_metadata(
        harness,
        /*turn_id*/ None,
...
async fn responses_websocket_preserves_credit_usage_metadata() {
    ...
    let harness = websocket_harness_for_workx_backend(&server).await;
    ...
    assert_eq!(
        usage_metadata,
        Some(workx_protocol::ResponseUsageMetadata {
...
async fn responses_websocket_streams_request() {
    ...
    assert_eq!(
        handshake.header(X_WORKX_ROUTING_HINT_HEADER),
    ...
    assert_eq!(
        ...
        Some(workx_login::default_client::get_workx_user_agent())
    );
    assert_eq!(
    ...
    let stream_request_start_ms = body["client_metadata"]
        [X_WORKX_WS_STREAM_REQUEST_START_MS_CLIENT_METADATA_KEY]
        .as_str()
...
async fn responses_websocket_omits_routing_hint_for_provider_with_own_credentials() {
    ...
    assert_eq!(handshake.header(X_WORKX_ROUTING_HINT_HEADER), None);
...
async fn responses_websocket_request_prewarm_traces_logical_request() {
    ...
    writer
        .append(RawTraceEventPayload::WorkxTurnStarted {
...
async fn responses_websocket_preconnect_runs_when_only_v2_feature_enabled() {
    ...
    assert_eq!(
        server
            .single_handshake()
            .header(X_WORKX_ROUTING_HINT_HEADER),
        None
    );
...
async fn responses_websocket_usage_limit_error_emits_rate_limit_event() {
    ...
    let token_event =
        wait_for_event(&test.workx, |msg| matches!(msg, EventMsg::TokenCount(_))).await;
    ...
    let error_event = wait_for_event(&test.workx, |msg| matches!(msg, EventMsg::Error(_))).await;
...
async fn websocket_harness_for_workx_backend(server: &WebSocketTestServer) -> WebsocketTestHarness {
...
async fn websocket_harness_with_provider_options_and_auth(
    ...
    let model_info = workx_core::test_support::construct_model_info_offline(MODEL, &config);
    let thread_id = ThreadId::new();
    let session_id = SessionId::new();
    let client_auth_manager = auth.map(workx_core::test_support::auth_manager_from_auth);
    ...
    let metrics = MetricsClient::new(
        MetricsConfig::in_memory("test", "workx-core", env!("CARGO_PKG_VERSION"), exporter)
            .with_runtime_reader(),
    )

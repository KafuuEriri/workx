...
async fn settings_notifications_keep_their_commit_across_postcommit_work(
    ...
    let test = test_workx()
        ...
        .await?;
    let initial = test.workx.restorable_thread_settings().await;
        ...
        async move {
            match operation {
                ...
                SettingsOperation::Standalone => {
                    workx.submit(Op::ThreadSettings { thread_settings }).await
                }
            }
        }
    });
    ...
    let expected = test.workx.thread_settings_snapshot().await;
    ...
    timeout(TIMEOUT, test.workx.restore_thread_settings(initial)).await??;
    let restored = test.workx.thread_settings_snapshot().await;
    ...
    assert_eq!(test.workx.thread_settings_snapshot().await, restored);
    Ok(())
}

...
use workx_core_plugins::startup_sync::curated_plugins_repo_path;
...
use workx_protocol::approvals::ElicitationAction;
...
const CALENDAR_NAMESPACE: &str = "mcp__workx_apps__calendar";
    ...
    tokio::time::timeout(Duration::from_secs(5), async {
        loop {
            if server
                ...
                .unwrap_or_default()
                .iter()
                .any(|request| request.url.path() == "/ps/plugins/suggested/workx")
            {
                return;
            }
...
async fn start_gated_step_preparation(test: &TestWorkx, server: &MockServer) -> Result<PathUri> {
    ...
    let prior_recommendation_count = server
        ...
        .unwrap_or_default()
        .into_iter()
        .filter(|request| request.url.path() == "/ps/plugins/suggested/workx")
        .count();
...
async fn start_install_turn(test: &TestWorkx, prompt: &str) -> Result<ElicitationRequestEvent> {
    let (sandbox_policy, permission_profile) =
        turn_permission_fields(PermissionProfile::Disabled, test.config.cwd.as_path());
    test.workx
        .start_or_steer_turn(
    ...
    Ok(wait_for_event_match(&test.workx, |event| match event {
        ...
        if let Some(event) = requests
            ...
            .find_map(|request| {
                ...
                payload["events"].as_array().and_then(|events| {
                    events
                        .iter()
                        .find(|event| event["event_type"] == "workx_plugin_install_requested")
                        .cloned()
                })
            })
        {
    ...
    assert_eq!(
        ...
        json!({
            ...
            "event_params": {
                ...
                "turn_id": turn_id,
                "model_slug": "gpt-5.4",
                "product_client_id": workx_login::default_client::originator().value,
            }
        })
    );
...
async fn run_remote_plugin_install_refresh_case(refreshed_tools: RefreshedAppsTools) -> Result<()> {
    ...
    drop(requests);
    test.workx.refresh_runtime_config(test.config.clone()).await;

...
use workx_core::parse_turn_item;
...
async fn fork_thread_twice_drops_to_first_message() {
    ...
    for text in ["first", "second", "third"] {
        workx
            ...
            .await
            .unwrap();
        let _ = wait_for_event(&workx, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
    }

    // Request history from the base conversation to obtain rollout path.
    let base_path = workx.rollout_path().expect("rollout path");
    ...
    let fork1_path = workx_fork1.rollout_path().expect("rollout path");
    expected_after_first.push(thread_settings_applied_item(
        fork1_thread_id,
        workx_fork1.thread_settings_snapshot().await,
    ));
    ...
    let fork2_path = workx_fork2.rollout_path().expect("rollout path");
    ...
    expected_after_second.push(thread_settings_applied_item(
        fork2_thread_id,
        workx_fork2.thread_settings_snapshot().await,
    ));
...
async fn assert_copied_fork_persists_inherited_history(history_mode: ThreadHistoryMode) {
    ...
    let mut builder = test_workx().with_history_mode(history_mode);
    let test = builder.build(&server).await.expect("create conversation");
    let workx = test.workx.clone();
    let thread_manager = test.thread_manager.clone();
    ...
    let source_path = workx.rollout_path().expect("source rollout path");
    let source_items = read_rollout_items(&source_path);
    let source_meta = workx_rollout::read_session_meta_line(source_path.as_path())
        .await
    ...
    if matches!(history_mode, ThreadHistoryMode::Paginated) {
        ...
        let resumed_history = workx_rollout::RolloutRecorder::get_rollout_history(&forked_path)
            .await
        ...
        let resumed = thread_manager
            .resume_thread_with_history(
                test.config.clone(),
                resumed_history,
                workx_core::test_support::auth_manager_from_auth(

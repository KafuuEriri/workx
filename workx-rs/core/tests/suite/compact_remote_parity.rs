...
const FIXED_CWD: &str = "/tmp/workx_remote_compaction_parity_workspace";
...
impl AuthCase {
    fn build(self) -> WorkxAuth {
        match self {
            AuthCase::ChatGpt => WorkxAuth::create_dummy_chatgpt_auth_for_testing(),
            AuthCase::ApiKey => WorkxAuth::from_api_key("dummy"),
        }
    }
}
...
async fn run_manual_session(
    ...
    for (idx, step) in scenario.steps.iter().enumerate() {
        submit_user_input(&workx, user_input_for_step(scenario.name, idx, *step)).await?;
    }
...
async fn run_manual_hook_session(mode: Mode) -> Result<Value> {
    ...
    let home = harness.test().workx_home_path();
...
async fn build_auto_harness(mode: Mode) -> Result<TestWorkxHarness> {
    build_harness_inner(
        mode,
        RunSettings::default(),
        /*hooks*/ false,
...
async fn build_harness(mode: Mode, settings: RunSettings, hooks: bool) -> Result<TestWorkxHarness> {
...
async fn build_harness_inner(
    ...
    TestWorkxHarness::with_builder(builder.with_config(move |config| {
        config.cwd = workx_utils_absolute_path::AbsolutePathBuf::from_absolute_path(PathBuf::from(
            FIXED_CWD,
        ))
...
fn rollout_path(harness: &TestWorkxHarness) -> PathBuf {
    harness
        .test()
        .session_configured
        .rollout_path
...
async fn submit_user_input(workx: &workx_core::WorkxThread, items: Vec<UserInput>) -> Result<()> {
    workx
...
fn normalize_tmp_prefix_before_marker(text: &mut String, marker: &str) {
    ...
    while let Some(relative_marker_index) = text[search_start..].find(marker) {
        ...
        if let Some(start_index) = start {
            text.replace_range(start_index..marker_index, "<WORKX_HOME>");
            search_start = start_index + "<WORKX_HOME>".len() + marker.len();
        } else {
...
fn normalize_string_rewrites_linux_temp_skill_paths() {
    ...
    assert_eq!(
        text,
        "file: <WORKX_HOME>/skills/.system/imagegen/SKILL.md and \
         <WORKX_HOME>/skills/custom/SKILL.md"
    );
}

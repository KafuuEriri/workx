...
async fn run_turn(test: &TestWorkx, prompt: &str) -> anyhow::Result<()> {
...
async fn run_turn_and_measure(test: &TestWorkx, prompt: &str) -> anyhow::Result<Duration> {
    let start = Instant::now();
    run_turn(test, prompt).await?;
    Ok(start.elapsed())
}

async fn build_workx_with_test_tool(server: &wiremock::MockServer) -> anyhow::Result<TestWorkx> {
    let mut builder = test_workx().with_model("test-gpt-5.1-workx");
...
async fn read_file_tools_run_in_parallel() -> anyhow::Result<()> {
    ...
    let test = build_workx_with_test_tool(&server).await?;

...
use workx_image_generation_extension::install as install_image_generation_extension;
...
fn responses_extensions(auth: &WorkxAuth) -> Arc<ExtensionRegistry<Config>> {
...
fn configure_image_capable_model(model_info: &mut workx_protocol::openai_models::ModelInfo) {
...
async fn responses_lite_includes_tool_namespaces_info_when_enabled() -> Result<()> {
    ...
    assert_eq!(
        ...
        serde_json::json!({
            ...
            "direct": true,
            "deferred": false,
            "code_mode_name": "mcp__workx_apps__calendar_create_event",
            "source": {
                "kind": "mcp",
                "server_name": "workx_apps",
            },
        })
    );
...
async fn responses_lite_prepares_images() -> Result<()> {
    ...
    let mut builder = test_workx().with_model_info_override("gpt-5.4", |model_info| {
        model_info.use_responses_lite = true;

...
use workx_config::types::Personality;
...
use workx_protocol::openai_models::default_input_modalities;
...
fn read_only_user_turn(test: &TestWorkx, items: Vec<UserInput>, model: String) -> TurnInputRequest {
...
async fn first_turn_after_empty_prefix_fork_preserves_inherited_base_instructions(
    ...
    let source_rollout_path = test.workx.rollout_path().expect("rollout path");
    let source_history =
        workx_rollout::RolloutRecorder::get_rollout_history(&source_rollout_path).await?;
...
async fn rollback_first_turn_model_change_removes_its_instructions(
    ...
    let mut builder = test_workx().with_model(initial_model);
    let test = builder.build_with_auto_env(&server).await?;
    ...
    let test = match followup {
        RollbackFollowup::ColdResume => {
            let mut resume_builder = test_workx().with_model(switched_model);
...
async fn service_tier_change_is_applied_on_next_http_turn() -> Result<()> {
    ...
    let test = test_workx().build(&server).await?;
...
async fn model_switch_to_smaller_model_updates_token_context_window() -> Result<()> {
    ...
    let large_window_event = wait_for_event(&test.workx, |event| {
        matches!(
            event,
    ...
    let smaller_turn_started_event = wait_for_event(&test.workx, |event| {
        matches!(
            event,
    ...
    let smaller_window_event = wait_for_event(&test.workx, |event| {
        matches!(
            event,
            EventMsg::TokenCount(token_count)
                if token_count

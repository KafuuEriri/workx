...
use core_test_support::test_workx::RecordingUserInstructionsProvider;
...
async fn agents_instructions(mut builder: TestWorkxBuilder) -> Result<String> {
...
async fn submit_thread_turn(thread: &Arc<workx_core::WorkxThread>, prompt: &str) -> Result<()> {
    thread
...
async fn agents_override_is_preferred_over_agents_md() -> Result<()> {
    let instructions =
        agents_instructions(test_workx().with_workspace_setup(|cwd, fs| async move {
...
async fn runtime_trust_reload_refreshes_project_instructions() -> Result<()> {
    ...
    let mut untrusted_config = (*test.workx.config().await).clone();
    untrusted_config.active_project.trust_level = Some(TrustLevel::Untrusted);
    test.workx.refresh_runtime_config(untrusted_config).await;
    ...
    let mut trusted_config = (*test.workx.config().await).clone();
    trusted_config.active_project.trust_level = Some(TrustLevel::Trusted);
    test.workx.refresh_runtime_config(trusted_config).await;
...
-> Result<()> {
    ...
    .await;
    let mut builder = test_workx().with_workspace_setup(|cwd, fs| async move {
        fs.write_file(
            &executor_path_uri(cwd.join(GLOBAL_AGENTS_FILENAME))?,
            PROJECT_INSTRUCTIONS.as_bytes().to_vec(),
            Default::default(),
...
async fn loads_user_instructions_without_a_primary_environment() -> Result<()> {
    ...
    let provider = Arc::new(RecordingUserInstructionsProvider::new(Arc::new(
        WorkxHomeUserInstructionsProvider::new(AbsolutePathBuf::try_from(
...
async fn fresh_thread_composes_global_before_project_and_reports_sources() -> Result<()> {
    ...
    assert_eq!(test.workx.instruction_sources().await, creation_sources);
...
async fn invalid_utf8_global_instructions_are_lossy() -> Result<()> {
    ...
    let mut builder = test_workx().with_home(home);
    let test = builder.build(&server).await?;
...
async fn cold_resume_invalidates_deleted_legacy_agents_md_once() -> Result<()> {
    ...
    let mut initial_builder = test_workx().with_home(Arc::clone(&home));
    let initial = initial_builder.build(&server).await?;

    // Assert the pre-resume thread reports the source used to create its snapshot.
    assert_eq!(
        initial.workx.instruction_sources().await,
    ...
    let mut resume_builder = test_workx().with_home(Arc::clone(&home));
...
async fn fork_injects_changed_agents_md_once() -> Result<()> {
    ...
    let mut builder = test_workx().with_home(Arc::clone(&home));
    let parent = builder.build(&server).await?;

    // Assert the parent reports the source used to create its snapshot.
    assert_eq!(
        parent.workx.instruction_sources().await,
        vec![PathUri::from_abs_path(&source)],
        "parent reports the creation-time global source"
    );
    parent.submit_turn("persist instructions").await?;
    parent.workx.ensure_rollout_materialized().await;
    parent.workx.flush_rollout().await?;
    let rollout_path = parent.workx.rollout_path().expect("rollout path");
    ...
    fork_config.workx_self_exe = parent.config.workx_self_exe.clone();
    fork_config
        .features

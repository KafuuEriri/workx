...
use workx_core::config::CurrentTimeReminderConfig;
use workx_extension_items::ExtensionItem;
use workx_extension_items::sleep::SleepItem;
...
async fn idle_response_items_include_pending_mailbox_in_first_request() -> anyhow::Result<()> {
    ...
    submit_queue_only_agent_mail(test.workx.as_ref(), "pending mailbox input").await;
    let submission = test
        .workx
    ...
    wait_for_turn_complete(test.workx.as_ref()).await;
...
async fn assert_idle_user_input_reaches_the_first_model_request(
    ...
    let user_message = core_test_support::wait_for_event_match(test.workx.as_ref(), |event| {
...
async fn build_workx(server: &StreamingSseServer) -> Arc<WorkxThread> {
    test_workx()
        .with_config(|config| config.update_plan_enabled = true)
        .with_model("gpt-5.4")
        .build_with_streaming_server(server)
        .await
        .expect("build streaming Workx test session")
        .workx
}

async fn submit_user_input(workx: &WorkxThread, text: &str) {
    workx
...
async fn submit_danger_full_access_user_turn(test: &TestWorkx, text: &str) {
...
async fn steer_user_input(workx: &WorkxThread, text: &str) {
...
async fn enqueue_queue_only_agent_mail(workx: &WorkxThread, text: &str) {
    workx
...
async fn submit_queue_only_agent_mail(workx: &WorkxThread, text: &str) {
    enqueue_queue_only_agent_mail(workx, text).await;
    workx
        .submit(Op::RealtimeConversationListVoices)
        .await
        .expect("submit list-voices barrier");
    wait_for_event(workx, |event| {
        matches!(event, EventMsg::RealtimeConversationListVoicesResponse(_))
    })
    .await;
}

async fn wait_for_reasoning_item_started(workx: &WorkxThread) {
    wait_for_event(workx, |event| {
        matches!(
            event,
...
async fn wait_for_agent_message(workx: &WorkxThread, text: &str) {
...
async fn wait_for_turn_complete(workx: &WorkxThread) {
    wait_for_event(workx, |event| matches!(event, EventMsg::TurnComplete(_))).await;
}

async fn wait_for_sleep_item_started(workx: &WorkxThread, call_id: &str, duration_ms: u64) {
    let event = wait_for_event(workx, |event| {
        matches!(
            event,
...
async fn wait_for_sleep_item_completed(workx: &WorkxThread, call_id: &str, duration_ms: u64) {
    let event = wait_for_event(workx, |event| {
        matches!(
            event,
...
impl workx_extension_api::ThreadLifecycleContributor<workx_core::config::Config>
    for SleepingRootExtension
{
    fn on_thread_start<'a>(
        &'a self,
        input: workx_extension_api::ThreadStartInput<'a, workx_core::config::Config>,
...
async fn queue_only_agent_mail_wakes_sleeping_root_with_previous_turn_context() {
    ...
    .await;
    let mut extensions =
        workx_extension_api::ExtensionRegistryBuilder::<workx_core::config::Config>::new();
    ...
    let workx = test_workx()
        ...
        .await
        .expect("build Workx test session")
        .workx;
    ...
    enqueue_queue_only_agent_mail(&workx, CHILD_MESSAGE).await;
    wait_for_turn_complete(&workx).await;
    ...
    assert!(history.items.iter().any(|item| {
        matches!(
            ...
            RolloutItem::ResponseItem(envelope)
                if matches!(
                    &envelope.item,
                    workx_protocol::models::ResponseItem::AgentMessage { content, .. }
                        if content.iter().any(|content| matches!(
                            content,
                            workx_protocol::models::AgentMessageInputContent::InputText { text }
...
async fn steer_interrupts_wait_agent_and_is_sent_in_follow_up_request() {
    ...
    submit_user_input(&workx, INITIAL_PROMPT).await;
    wait_for_event(&workx, |event| {
        matches!(event, EventMsg::CollabWaitingBegin(_))
    })
    .await;

    steer_user_input(&workx, STEER_PROMPT).await;
    wait_for_turn_complete(&workx).await;
...
async fn any_new_input_interrupts_sleep() {
    ...
    submit_user_input(&workx, INITIAL_PROMPT).await;
    wait_for_sleep_item_started(&workx, FIRST_SLEEP_CALL_ID, SLEEP_DURATION_MS).await;

    steer_user_input(&workx, STEER_PROMPT).await;
    wait_for_sleep_item_completed(&workx, FIRST_SLEEP_CALL_ID, SLEEP_DURATION_MS).await;
    wait_for_sleep_item_started(&workx, SECOND_SLEEP_CALL_ID, SLEEP_DURATION_MS).await;

    submit_queue_only_agent_mail(&workx, "new mailbox input").await;
    wait_for_sleep_item_completed(&workx, SECOND_SLEEP_CALL_ID, SLEEP_DURATION_MS).await;
    wait_for_turn_complete(&workx).await;
    ...
    workx.submit(Op::Shutdown).await.expect("shutdown session");
    wait_for_event(&workx, |event| matches!(event, EventMsg::ShutdownComplete)).await;

    let rollout_path = workx.rollout_path().expect("rollout path");
...
async fn queued_inter_agent_mail_triggers_follow_up_after_reasoning_item() {
    ...
    let workx = build_workx(&server).await;

    submit_user_input(&workx, "first prompt").await;

    wait_for_reasoning_item_started(&workx).await;

    submit_queue_only_agent_mail(&workx, "queued child update").await;
...
async fn queued_inter_agent_mail_triggers_follow_up_after_commentary_message_item() {
    ...
    wait_for_agent_message(&workx, "first answer").await;
...
async fn queued_inter_agent_mail_does_not_restart_after_final_answer() {
    ...
    submit_user_input(&workx, "second prompt").await;
    wait_for_turn_complete(&workx).await;
...
async fn user_input_does_not_preempt_after_reasoning_item() {
    ...
    steer_user_input(&workx, "second prompt").await;
...
async fn terminal_compaction_error_does_not_retry_pending_input(
    ...
    let test = test_workx()
        ...
        .await?;
    let workx = &test.workx;

    if failure_point == CompactionFailurePoint::PreTurn {
        submit_user_input(workx, "initial prompt").await;
        wait_for_turn_complete(workx).await;
    }
    if pending_input == PendingInputAfterFailure::QueuedMail {
        submit_queue_only_agent_mail(workx, PENDING_MESSAGE).await;
    }
    submit_user_input(workx, "prompt that needs compaction").await;
    tokio::time::timeout(
    ...
    .await?;
    match pending_input {
        PendingInputAfterFailure::Steer => steer_user_input(workx, PENDING_MESSAGE).await,
        ...
        PendingInputAfterFailure::TriggeringMail => {
            workx
                ...
                .await?;
            workx.submit(Op::RealtimeConversationListVoices).await?;
            wait_for_event(workx, |event| {
                matches!(event, EventMsg::RealtimeConversationListVoicesResponse(_))
            })
            .await;
    ...
    wait_for_event(workx, |event| {
        match event {
            EventMsg::Error(error) => {
                assert_eq!(
                    error.workx_error_info,
                    Some(WorkxErrorInfo::UsageLimitExceeded)
                );
    ...
    workx.flush_rollout().await?;
    let history = workx.load_history(/*include_archived*/ false).await?;
    ...
    submit_user_input(workx, "retry after quota resets").await;
    wait_for_agent_message(workx, "recovered answer").await;
    let completed = wait_for_event(workx, |event| matches!(event, EventMsg::TurnComplete(_))).await;
...
async fn steered_user_input_waits_for_model_continuation_after_mid_turn_compact() {
    ...
    wait_for_agent_message(&workx, "resumed old task").await;
    wait_for_turn_complete(&workx).await;
...
async fn steered_user_input_follows_compact_when_only_the_steer_needs_follow_up() {
    ...
    wait_for_agent_message(&workx, "processed steered prompt").await;
    wait_for_turn_complete(&workx).await;
...
async fn steered_user_input_waits_when_tool_output_triggers_compact_before_next_request() {
    ...
    wait_for_event(&workx, |event| matches!(event, EventMsg::TurnStarted(_))).await;
    steer_user_input(&workx, "second prompt").await;
    let _ = gate_first_completed_tx.send(());

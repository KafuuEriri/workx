...
use workx_protocol::openai_models::ModelTokenBudgetConfig;
...
use workx_protocol::protocol::ENVIRONMENT_CONTEXT_OPEN_TAG;
use workx_protocol::protocol::EventMsg;
use workx_protocol::protocol::ExitedReviewModeEvent;
use workx_protocol::protocol::Op;
use workx_protocol::protocol::ReviewCodeLocation;
use workx_protocol::protocol::ReviewFinding;
use workx_protocol::protocol::ReviewLineRange;
use workx_protocol::protocol::ReviewOutputEvent;
...
use workx_protocol::review_format::render_review_output_text;
...
async fn review_op_emits_lifecycle_and_review_output() {
    // Skip under Workx sandbox network restrictions.
    skip_if_no_network!();
    ...
    let workx = new_conversation_for_server(&server, workx_home.clone(), |_| {}).await;
    ...
    let entered_started = wait_for_event(&workx, |ev| {
        matches!(
            ev,
    ...
    let entered_completed = wait_for_event(&workx, |ev| {
        matches!(
            ev,
    ...
    let entered = wait_for_event(&workx, |ev| matches!(ev, EventMsg::EnteredReviewMode(_))).await;
    match entered {
    ...
    let exited_started = wait_for_event(&workx, |ev| {
        matches!(
            ev,
            EventMsg::ItemStarted(event)
    ...
    let exited_completed = wait_for_event(&workx, |ev| {
        matches!(
            ev,
            EventMsg::ItemCompleted(event)
                if matches!(event.item, TurnItem::ExitedReviewMode(_))
    ...
    let closed = wait_for_event(&workx, |ev| matches!(ev, EventMsg::ExitedReviewMode(_))).await;
    ...
    let _complete = wait_for_event(&workx, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;

    let path = workx.rollout_path().expect("rollout path");
...
async fn cancelled_review_does_not_forward_delegate_mcp_startup() {
    ...
    tokio::time::timeout(Duration::from_secs(5), async {
        loop {
            match workx.next_event().await.expect("review event").msg {
    ...
    workx.submit(Op::Interrupt).await.unwrap();
...
async fn review_op_with_plain_text_emits_review_fallback() {
    ...
    let _entered = wait_for_event(&workx, |ev| matches!(ev, EventMsg::EnteredReviewMode(_))).await;
    let closed = wait_for_event(&workx, |ev| matches!(ev, EventMsg::ExitedReviewMode(_))).await;
    let review = match closed {
...
async fn review_uses_updated_turn_permissions_and_approval_policy() {
    ...
    let workx = Arc::clone(&test.workx);
    ...
    let stored_settings = workx.thread_settings_snapshot().await;
    workx
        .submit(Op::Review {
            review_request: ReviewRequest {
                target: ReviewTarget::Custom {
    ...
    assert_eq!(workx.thread_settings_snapshot().await, stored_settings);
    let request = request_log.single_request();
...
async fn review_resolves_inherited_summary_preferences() -> anyhow::Result<()> {
    ...
    for summary in [None, Some(ReasoningSummary::Concise)] {
        ...
        let stored_settings = test.workx.thread_settings_snapshot().await;
        test.workx
            .submit(Op::Review {
                review_request: ReviewRequest {
                    target: ReviewTarget::Custom {
        ...
        .await;
        assert_eq!(test.workx.thread_settings_snapshot().await, stored_settings);
    }
...
async fn review_uses_custom_review_model_from_config() {
    ...
    let workx = Arc::clone(&test.workx);
    std::fs::remove_file(workx_home.path().join("models_cache.json"))
    ...
    // Wait for completion
    let _entered = wait_for_event(&workx, |ev| matches!(ev, EventMsg::EnteredReviewMode(_))).await;
    let _closed = wait_for_event(&workx, |ev| {
        matches!(
            ev,
...
async fn review_input_isolated_from_parent_history() {
    ...
    let session_file = workx_home.path().join("resume.jsonl");
    {
        ...
        let user = workx_protocol::models::ResponseItem::Message {
            id: None,
            role: "user".to_string(),
        ...
        let assistant = workx_protocol::models::ResponseItem::Message {
            id: None,
    ...
    let workx =
        resume_conversation_for_server(&server, workx_home.clone(), session_file.clone(), |_| {})
            .await;
...
async fn review_uses_overridden_cwd_for_base_branch_merge_base() {
    ...
    let workx = new_conversation_for_server(&server, workx_home.clone(), move |config| {
    ...
    F: FnOnce(&mut Config) + Send + 'static,
{
    ...
    builder
        .resume(server, workx_home, resume_path)
        .await

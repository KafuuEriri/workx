...
use workx_protocol::items::AgentMessageDelivery;
use workx_protocol::items::AgentMessageItem;
use workx_protocol::items::AsyncUserInputQuestion;
...
async fn request_user_input_async_emits_item_and_does_not_end_the_turn(
    ...
    let started = wait_for_event_match(test.workx.as_ref(), |event| {
    ...
    let completed = wait_for_event_match(test.workx.as_ref(), |event| {

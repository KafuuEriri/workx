...
use workx_app_server_protocol::ThreadRealtimeSessionOutcome;
use workx_app_server_protocol::ThreadRealtimeTranscriptRole;
...
use workx_core::test_support::auth_manager_from_auth;
...
use workx_protocol::protocol::RealtimeNoopRequested;
...
use workx_thread_store::ListTimelineParams;
...
const REALTIME_BACKEND_PROMPT: &str = workx_prompts::BACKEND_PROMPT;
...
const REALTIME_CONVERSATION_TEST_SUBPROCESS_ENV_VAR: &str =
    "WORKX_REALTIME_CONVERSATION_TEST_SUBPROCESS";
...
fn run_realtime_conversation_test_in_subprocess(
    ...
    for &key in workx_network_proxy::PROXY_ENV_KEYS {
...
async fn seed_recent_thread(
    ...
    let db = test.workx.state_db().context("state db enabled")?;
    ...
    let mut metadata_builder = workx_state::ThreadMetadataBuilder::new(
        thread_id,
        rollout_path,
        updated_at,
        SessionSource::Cli,
...
async fn conversation_start_audio_text_close_round_trip() -> Result<()> {
    ...
    let started = wait_for_event_match(&test.workx, |msg| match msg {
    ...
    let session_updated = wait_for_event_match(&test.workx, |msg| match msg {
    ...
    let audio_out = wait_for_event_match(&test.workx, |msg| match msg {
        EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
    ...
    test.workx.submit(Op::RealtimeConversationClose).await?;
    let closed = wait_for_event_match(&test.workx, |msg| match msg {
    ...
    let history = test.workx.load_history(/*include_archived*/ false).await?;
    assert!(
        !history
            .items
            .iter()
...
async fn conversation_records_history_without_an_event_observer(
    ...
    test.workx.submit(Op::Shutdown).await?;
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn conversation_websocket_transports_send_workx_headers_without_creating_a_call(
    ...
    let conversation = test
        ...
        .start_thread(StartThreadOptions {
            ...
            environments: Some(test.workx.config_snapshot().await.environments.environments),
            ..StartThreadOptions::new(test.config.clone())
        })
        .await?;
    let workx = &conversation.thread;
    ...
    let started = wait_for_event_match(workx, |msg| match msg {
        EventMsg::RealtimeConversationStarted(started) => Some(Ok(started.clone())),
        EventMsg::Error(err) => Some(Err(err.clone())),
        _ => None,
    })
    ...
    let _closed = wait_for_event_match(workx, |msg| match msg {
        EventMsg::RealtimeConversationClosed(closed) => Some(closed.clone()),
        _ => None,
    })
    .await;
    workx.shutdown_and_wait().await?;
    test.workx.shutdown_and_wait().await?;
    realtime_server.shutdown().await;
    Ok(())
}
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn conversation_webrtc_frameless_chatgpt_sends_workx_headers_to_backend(
    model: Option<&str>,
    expected_model: &str,
    thread_source: Option<ThreadSource>,
    ...
    let workx = &conversation.thread;
    assert_eq!(workx.config_snapshot().await.thread_source, thread_source);
    ...
    let created = wait_for_event_match(workx, |msg| match msg {
...
async fn conversation_webrtc_start_posts_generated_session() -> Result<()> {
    ...
    let created = wait_for_event_match(&test.workx, |msg| match msg {
        EventMsg::RealtimeConversationSdp(created) => Some(Ok(created.clone())),
        EventMsg::Error(err) => Some(Err(err.clone())),
        _ => None,
    })
...
async fn conversation_webrtc_live_reconnects_sideband_after_unclean_disconnect(
    ...
    let handoff = timeout(Duration::from_secs(10), async {
        ...
        loop {
            match test.workx.next_event().await?.msg {
                EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
...
async fn conversation_webrtc_default_v1_rejects_explicit_v2_voice() -> Result<()> {
    ...
    let error = wait_for_event_match(&test.workx, |msg| match msg {
        EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
            payload: RealtimeEvent::Error(message),
        }) => Some(message.clone()),
        _ => None,
...
async fn conversation_webrtc_close_while_sideband_connecting_drops_pending_join() -> Result<()> {
    ...
    let sdp = wait_for_event_match(&test.workx, |msg| match msg {
    ...
    timeout(Duration::from_secs(10), async {
        ...
        test.workx.wait_until_terminated().await;
        Ok::<(), anyhow::Error>(())
    })
    .await
...
async fn conversation_webrtc_sideband_connect_failure_closes_with_error() -> Result<()> {
    ...
    let err = wait_for_event_match(&test.workx, |msg| match msg {
        EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
            payload: RealtimeEvent::Error(message),
        }) => Some(message.clone()),
        _ => None,
...
async fn conversation_start_uses_openai_env_key_fallback_with_chatgpt_auth() -> Result<()> {
    ...
    test.workx.submit(Op::RealtimeConversationClose).await?;
    let _closed = wait_for_event_match(&test.workx, |msg| match msg {
        EventMsg::RealtimeConversationClosed(closed) => Some(closed.clone()),
        _ => None,
    })
    .await;
...
async fn assert_transport_close_tail_flush(
    ...
    let mut builder = test_workx().with_config({
        let realtime_base_url = realtime_server.uri().to_string();
        move |config| {
            config.experimental_realtime_ws_base_url = Some(realtime_base_url);
            config.realtime.version = RealtimeWsVersion::V1;
...
async fn conversation_audio_before_start_emits_error() -> Result<()> {
    ...
    .await;
    assert_eq!(err.workx_error_info, Some(WorkxErrorInfo::BadRequest));
    assert_eq!(err.message, "conversation is not running");
...
async fn conversation_start_preflight_failure_emits_realtime_error_only() -> Result<()> {
    ...
    let closed = timeout(Duration::from_millis(200), async {
        wait_for_event_match(&test.workx, |msg| match msg {
            EventMsg::RealtimeConversationClosed(closed) => Some(closed.clone()),
            _ => None,
        })
        .await
...
async fn conversation_second_start_replaces_runtime() -> Result<()> {
    ...
    test.workx
        ...
        .await?;
    let _ = wait_for_event_match(&test.workx, |msg| match msg {
        EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
            payload: RealtimeEvent::AudioOut(frame),
...
async fn conversation_uses_explicit_start_voice() -> Result<()> {
    ...
    .await;
    let test = test_workx().build_with_websocket_server(&server).await?;
    assert!(
        server
            .wait_for_handshakes(/*expected*/ 1, Duration::from_secs(2))
            .await
...
async fn conversation_startup_context_current_thread_selects_many_turns_by_budget() -> Result<()> {
    ...
    let resumed_thread = test
        ...
        .resume_thread_with_history(
            ...
            auth_manager_from_auth(WorkxAuth::from_api_key("dummy")),
            /*parent_trace*/ None,
            ClientMcpExtensions::default(),
        )
        .await?;
    let workx = resumed_thread.thread;
...
async fn conversation_startup_context_falls_back_to_workspace_map() -> Result<()> {
    ...
    let test = builder.build_with_websocket_server(&startup_server).await?;
    fs::create_dir_all(test.workspace_path("workx-rs/core"))?;
    ...
    assert!(startup_context.contains("workx-rs/"));
...
async fn conversation_user_text_turn_is_not_sent_to_realtime(ephemeral: bool) -> Result<()> {
    ...
    let turn_complete = wait_for_event_match(&test.workx, |event| match event {
        ...
        assert_eq!(
            timeline.items.iter().filter_map(|entry| match entry {
                ...
                ThreadTimelineEntry::Item { item, .. }
                    if matches!(item.as_ref(), workx_app_server_protocol::ThreadItem::UserMessage { .. }) => Some("typed input"),
                _ => None,
            }).collect::<Vec<_>>(),
...
async fn conversation_flushes_assistant_deltas_every_200ms_for_v3_handoff() -> Result<()> {
    ...
    test.workx
        .submit(Op::RealtimeConversationStart(ConversationStartParams {
            ...
            workx_response_item_prefix: None,
            workx_response_handoff_mode:
                workx_protocol::protocol::WorkxResponseHandoffMode::BemTags,
            workx_response_handoff_channel_prefixes: Some(BTreeMap::from([
                (
...
async fn inbound_handoff_request_starts_turn_and_promotes_its_artifact() -> Result<()> {
    ...
    let turn_id = loop {
        let event = test.workx.next_event().await?;
    ...
    assert_eq!(
        ...
        vec![ThreadRealtimeItemContent::BemItemPromoted {
            ...
            presentation:
                workx_app_server_protocol::ThreadRealtimeBemItemPresentation::InlineMarkdown,
        }]
    );
...
async fn delegated_turn_user_role_echo_does_not_redelegate_and_still_forwards_audio() -> Result<()>
{
    ...
    let mut builder = test_workx().with_model("gpt-5.4").with_config({
        let realtime_base_url = realtime_server.uri().to_string();
        move |config| {
            config.experimental_realtime_ws_base_url = Some(realtime_base_url);
            config.realtime.version = RealtimeWsVersion::V1;

...
async fn compressed_shared_fork_resume_preserves_checkpoint_and_frozen_history() -> Result<()> {
    ...
    let parent_path = test.workx.rollout_path().context("parent rollout")?;
    ...
    assert_eq!(
        workx_rollout::read_session_meta_line(&child_path)
            .await?
            .meta
            .history_base
    ...
    let store =
        workx_core::thread_store_from_config(&config, workx_core::init_state_db(&config).await);
    ...
    let resumed = test
        ...
        .resume_thread_with_history(
            ...
            workx_core::test_support::auth_manager_from_auth(workx_login::WorkxAuth::from_api_key(
                "dummy",
            )),
            /*parent_trace*/ None,
            ClientMcpExtensions::default(),

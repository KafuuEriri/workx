...
use workx_models_manager::client_version_to_whole;
...
const REMOTE_MODEL: &str = "workx-test-ttl";
const VERSIONED_MODEL: &str = "workx-test-versioned";
const MISSING_VERSION_MODEL: &str = "workx-test-missing-version";
const DIFFERENT_VERSION_MODEL: &str = "workx-test-different-version";
...
async fn renews_cache_ttl_on_matching_models_etag() -> Result<()> {
    ...
    let cache_path = config.workx_home.join(CACHE_FILE);
    ...
    workx
        .start_or_steer_turn(
            ...
            .with_thread_settings(ThreadSettingsOverrides {
                ...
                approval_policy: Some(workx_protocol::protocol::AskForApproval::Never),
    ...
    let _ = wait_for_event(&workx, |event| matches!(event, EventMsg::TurnComplete(_))).await;
...
async fn matching_models_etag_does_not_rewrite_recent_cache() -> Result<()> {
    ...
    let cache_path = test.config.workx_home.join(CACHE_FILE);

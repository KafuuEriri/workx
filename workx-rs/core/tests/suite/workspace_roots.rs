...
async fn workspace_roots_test(server: &MockServer) -> Result<TestWorkx> {
...
fn outside_workspace_path(test: &TestWorkx, file_name: &str) -> Result<PathUri> {
    let file_name = format!("workx-workspace-roots-{}-{file_name}", std::process::id());
...
async fn submit_workspace_turn(test: &TestWorkx, prompt: &str) -> Result<()> {
    test.submit_turn_with_permission_profile(prompt, workspace_roots_profile())
        .await
}

async fn read_file(test: &TestWorkx, path: &PathUri) -> Result<String> {
    Ok(String::from_utf8(
        test.fs()
...
async fn remove_files(test: &TestWorkx, paths: &[&PathUri]) -> Result<()> {
    for path in paths {
        test.fs()
            .remove(
                path,
...
async fn workspace_roots_allow_patches_but_protect_metadata_directories() -> Result<()> {
    const PATCH_CONTENTS: &str = "workspace root patch access";
    const PROTECTED_METADATA_DIRECTORIES: [&str; 3] = [".git", ".agents", ".workx"];

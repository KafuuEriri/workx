...
async fn submit_turn(test: &TestWorkx, permission_profile: PermissionProfile) -> Result<String> {
...
await tools.exec_command({
  ...
  justification: "test command approval",
    ...
    .await?;
    let approval = wait_for_event_match(&harness.test.workx, |event| match event {
...
await tools.request_permissions({
  ...
  permissions: { network: { enabled: true } },
    ...
    .await?;
    let request = wait_for_event(&harness.test.workx, |event| {
        matches!(
            event,

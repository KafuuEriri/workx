...
use workx_protocol::mcp::McpServerConnectionStatus;
...
async fn mcp_calls_stay_bound_to_each_thread() -> anyhow::Result<()> {
    ...
    let calls = [
        (&fixture.workx, "first-call", "first-runtime"),
        (&second_thread, "second-call", "second-runtime"),
        (&fixture.workx, "first-again", "first-runtime"),
    ];
    ...
    fixture.workx.shutdown_and_wait().await?;
...
async fn apps_cache_filled_during_binding_capture_reaches_the_model() -> anyhow::Result<()> {
    ...
    tokio::time::timeout(Duration::from_secs(30), async {
        ...
        let EventMsg::McpStartupComplete(startup) = wait_for_event(&test.workx, |event| {
        ...
        let NewThread { thread: peer, .. } = test
            ...
            .await?;
        wait_for_mcp_server(&peer, WORKX_APPS_MCP_SERVER_NAME).await?;

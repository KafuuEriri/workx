...
async fn recover_turn_restores_cyber_access_program_without_making_it_sticky() -> Result<()> {
    ...
    initial.workx.submit(Op::Interrupt).await?;
...
async fn cyber_access_program_omits_api_key_and_spoofed_custom_provider() -> Result<()> {
    ...
    for (auth, provider_id) in [
        (WorkxAuth::from_api_key("test-key"), "openai"),
        (WorkxAuth::create_dummy_chatgpt_auth_for_testing(), "custom"),
    ] {
        let server = responses::start_mock_server().await;
...
async fn submit(test: &TestWorkx, program: Option<CyberAccessProgram>) -> Result<()> {
    test.workx
        .start_or_steer_turn(
            TurnInputRequest::user_input(vec![UserInput::Text {
                text: "hello".to_owned(),

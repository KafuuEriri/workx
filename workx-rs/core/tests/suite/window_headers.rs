...
async fn window_id_advances_after_compact_persists_on_resume_and_resets_on_fork() -> Result<()> {
    ...
    let initial_thread = Arc::clone(&initial.workx);
    ...
    let resumed = builder
        ...
        .await?;
    submit_user_turn(&resumed.workx, "after resume").await?;
    shutdown_thread(&resumed.workx).await?;
...
async fn submit_user_turn(workx: &Arc<WorkxThread>, text: &str) -> Result<()> {
    workx
...
async fn submit_compact_turn(workx: &Arc<WorkxThread>) -> Result<()> {
    workx.submit(Op::Compact).await?;
    let warning_event = wait_for_event(workx, |event| matches!(event, EventMsg::Warning(_))).await;
...
async fn shutdown_thread(workx: &Arc<WorkxThread>) -> Result<()> {

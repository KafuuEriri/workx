...
async fn guardian_subagent_review_preserves_late_root_user_authorization() -> Result<()> {
    ...
    let mut root_history_items = [
        format!(
            ...
            workx_core::review_prompts::SUMMARY_PREFIX
        ),
    ...
    test.workx.inject_response_items(root_history_items).await?;
    ...
    let feedback_thread_ids = test
        ...
        .await?;
    let failures = workx_feedback::guardian_review_failures(&feedback_thread_ids);

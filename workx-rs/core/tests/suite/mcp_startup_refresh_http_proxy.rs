...
const PROXY_TEST_SUBPROCESS_ENV_VAR: &str = "WORKX_MCP_HTTP_PROXY_TEST_SUBPROCESS";
const SKILL_CALLBACK_PORT_ENV_VAR: &str = "WORKX_MCP_SKILL_CALLBACK_PORT";
const SKILL_GLOBAL_CALLBACK_PORT_ENV_VAR: &str = "WORKX_MCP_SKILL_GLOBAL_CALLBACK_PORT";
...
async fn local_mcp_startup_and_refresh_use_configured_http_client() -> Result<()> {
    ...
    if std::env::var_os(PROXY_TEST_SUBPROCESS_ENV_VAR).is_none() {
        ...
        command
            ...
            .env("http_proxy", proxy.uri())
            .env("NO_PROXY", workx_network_proxy::DEFAULT_NO_PROXY_VALUE)
            .env("no_proxy", workx_network_proxy::DEFAULT_NO_PROXY_VALUE);
...
async fn skill_mcp_dependency_oauth_uses_configured_http_client() -> Result<()> {
    ...
    fixture
        ...
        .await?;
    core_test_support::wait_for_event(fixture.workx.as_ref(), |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;

    let servers = workx_config::load_global_mcp_servers(&fixture.config.workx_home).await?;
    assert_eq!(
        servers
            .get(SERVER_NAME)

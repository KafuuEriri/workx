...
use workx_exec_server::EnvironmentConnectionState;
use workx_exec_server::EnvironmentInfo;
...
use workx_protocol::protocol::EnvironmentConnectionEvent;
...
impl TurnEnvironmentSnapshot {
    pub(crate) fn has_full_access(
        ...
        workx_protocol::protocol::has_full_access(
            approval_policy,
            thread_profile,
    ...
    #[cfg(test)]
    pub(crate) fn primary_environment(&self) -> Option<Arc<workx_exec_server::Environment>> {
        self.primary()
...
"#,
        ...
        let cwd = AbsolutePathBuf::current_dir().expect("cwd");
        let cwd_uri = PathUri::from_abs_path(&cwd);
        let manager = EnvironmentManager::from_workx_home(
            temp_dir.path(),
            Some(test_runtime_paths()),

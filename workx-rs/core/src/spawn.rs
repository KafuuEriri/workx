...
///
/// 1. The process was spawned by Workx as part of a shell tool call.
/// 2. NetworkSandboxPolicy is restricted for the tool call.
///
/// We may try to have just one environment variable for all sandboxing
/// attributes, so this may change in the future.
pub const WORKX_SANDBOX_NETWORK_DISABLED_ENV_VAR: &str = "WORKX_SANDBOX_NETWORK_DISABLED";
...
pub const WORKX_SANDBOX_ENV_VAR: &str = "WORKX_SANDBOX";
...
/// `WORKX_SANDBOX_NETWORK_DISABLED_ENV_VAR` environment variable.
...
pub(crate) async fn spawn_child_async(request: SpawnChildRequest<'_>) -> std::io::Result<Child> {
    ...
    let inherited_fd = env
        .get(workx_shell_escalation::ESCALATE_SOCKET_ENV_VAR)
    ...
    // If this Workx process dies (including being killed via SIGKILL), we want
    ...
    unsafe {
        ...
        cmd.pre_exec(move || {
            if detach_from_tty {
                workx_utils_pty::process_group::detach_from_tty()?;
            }
            ...
            #[cfg(target_os = "linux")]
            {
                ...
                // current parent dies."
                workx_utils_pty::process_group::set_parent_death_signal(parent_pid)?;
            }
            // macOS cannot receive the fd with close-on-exec set atomically.
            #[cfg(target_os = "macos")]
            workx_utils_pty::pty::close_inherited_fds_except(inherited_fd.as_slice());
            Ok(())
        });
    }

...
use tracing::warn;
use workx_memories_read::citations::parse_memory_citation;
use workx_memories_read::citations::thread_ids_from_memory_citation;
use workx_protocol::error::Result;
use workx_protocol::error::WorkxErr;
use workx_protocol::memory_citation::MemoryCitation;
...
use workx_utils_stream_parser::strip_proposed_plan_blocks;
...
fn strip_hidden_assistant_markup_and_parse_memory_citation(
    ...
    String,
    Option<workx_protocol::memory_citation::MemoryCitation>,
) {
        ...
        && role == "assistant"
    {
        let combined = content
            ...
            .filter_map(|ci| match ci {
                workx_protocol::models::ContentItem::OutputText { text } => Some(text.as_str()),
                _ => None,
            })
            .collect::<String>();
...
pub(crate) async fn handle_output_item_done(
    ...
    match ToolRouter::build_tool_call(item.clone()) {
        ...
        Err(FunctionCallError::Fatal(message)) => {
            return Err(WorkxErr::Fatal(message));
        }
    }
...
pub(crate) async fn finalize_turn_item(
    ...
    if let TurnItem::AgentMessage(agent_message) = &mut *turn_item {
        ...
        agent_message.content =
            vec![workx_protocol::items::AgentMessageContent::Text { text: stripped }];

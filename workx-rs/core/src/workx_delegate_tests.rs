...
async fn forward_ops_preserves_submission_trace_context() {
    ...
    assert_eq!(
        forwarded.trace,
        Some(workx_protocol::protocol::W3cTraceContext {
            traceparent: Some(
                "00-1234567890abcdef1234567890abcdef-1234567890abcdef-01".to_string(),
            ),
            tracestate: Some("vendor=state".to_string()),
...
#[tokio::test]
async fn run_workx_thread_interactive_respects_pre_cancelled_spawn() {
...
async fn guardian_delegates_do_not_inherit_parent_extensions() {
    ...
    for (subagent_source, expected_thread_starts, expected_thread_source) in [
        ...
        let mut config = parent_ctx.config.as_ref().clone();
        config.permissions.approval_policy = Constrained::allow_only(AskForApproval::Never);
        let (session, io) = run_workx_thread_interactive(
            config,
            Arc::clone(&parent_session.services.auth_manager),
            Arc::clone(&parent_session.services.models_manager),
...
#[tokio::test]
async fn run_workx_thread_interactive_rejects_approval_policy_that_can_prompt() {
    let (parent_session, parent_ctx, _rx_events) =
        crate::session::tests::make_session_and_context_with_rx().await;
    let mut config = parent_ctx.config.as_ref().clone();
    ...
    let result = run_workx_thread_interactive(
        config,
        Arc::clone(&parent_session.services.auth_manager),
        Arc::clone(&parent_session.services.models_manager),
        parent_session,
        ...
        Err(err)
            if matches!(
                ...
                WorkxErrorDetails::InvalidRequest(message)
                    if message == "Workx delegates require approval policy `never`"
            )
    ));
}

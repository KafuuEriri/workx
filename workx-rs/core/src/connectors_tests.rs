...
use workx_connectors::merge::plugin_connector_to_app_info;
use workx_connectors::metadata::connector_install_url;
use workx_connectors::metadata::sanitize_name;
...
fn workx_app_tool(
    ...
    let tool_namespace = connector_name
        .map(sanitize_name)
        .map(|connector_name| format!("mcp__{WORKX_APPS_MCP_SERVER_NAME}__{connector_name}"))
        .unwrap_or_else(|| WORKX_APPS_MCP_SERVER_NAME.to_string());
...
fn synthetic_links_are_exposed_to_the_agent_but_not_accessible_in_app_list() {
    let mut synthetic_tool = workx_app_tool("gmail_batch_read_email", "gmail", Some("Gmail"), &[]);
    ...
    let tools = vec![
        synthetic_tool,
        workx_app_tool("calendar_list_events", "calendar", Some("Calendar"), &[]),
    ];
...
fn accessible_connectors_from_mcp_tools_preserves_description() {
    let mcp_tools = vec![ToolInfo {
        ...
        callable_namespace: "mcp__workx_apps__calendar".to_string(),

...
command transformation live in the workx-sandboxing crate; this module keeps
...
#[cfg(target_os = "macos")]
use crate::spawn::WORKX_SANDBOX_ENV_VAR;
use crate::spawn::WORKX_SANDBOX_NETWORK_DISABLED_ENV_VAR;
...
pub use workx_protocol::models::SandboxPermissions;
...
pub(crate) struct ExecServerEnvConfig {
    pub(crate) policy: workx_exec_server::ExecEnvPolicy,
...
pub struct ExecRequest {
    ...
    pub(crate) exec_server_shell_snapshot: Option<workx_exec_server::ShellSnapshotRequest>,
...
impl ExecRequest {
    ...
    pub(crate) fn from_sandbox_exec_request(
        ...
        if !network_sandbox_policy.is_enabled() {
            env.insert(
                WORKX_SANDBOX_NETWORK_DISABLED_ENV_VAR.to_string(),
                "1".to_string(),
            );
        }
        #[cfg(target_os = "macos")]
        if sandbox == SandboxType::MacosSeatbelt {
            env.insert(WORKX_SANDBOX_ENV_VAR.to_string(), "seatbelt".to_string());
        }
        Ok(Self {
            command,
            cwd,
...
) -> workx_protocol::error::Result<ExecToolCallOutput> {

...
use workx_protocol::items::DynamicToolCallStatus;
use workx_protocol::items::ImageGenerationItem;
...
fn promotes_distinct_visualizations_once_and_ignores_markdown_fences() {
    let mut state = started_state();
    let text = "```\n::workx-inline-vis{file=hidden}\n```\n~~~\nvisualize{file=also-hidden}\n~~~\n::workx-inline-vis{file=first}\nvisualize{file=second}";
...
#[test]
fn promotes_successful_workx_app_mcp_calls_and_splits_active_transcripts() {
    let mut state = started_state();
    observe_realtime(
        &mut state,
        RealtimeEvent::OutputTranscriptDelta(RealtimeTranscriptDelta {
    ...
    for (server, status) in [
        ...
        ("workx_app", McpToolCallStatus::InProgress),
        ("workx_app", McpToolCallStatus::Failed),
    ] {
        assert!(
            state

...
//!
//! `ModelClient` is intended to live for the lifetime of a Workx session and holds the stable
...
use workx_api::RawMemory as ApiRawMemory;
use workx_api::RealtimeCallClient as ApiRealtimeCallClient;
use workx_api::RealtimeSessionConfig as ApiRealtimeSessionConfig;
use workx_api::Reasoning;
use workx_api::ReasoningContext;
...
use workx_api::ResponsesWebsocketConnection as ApiWebSocketConnection;
use workx_api::ResponsesWsRequest;
use workx_api::SharedAuthProvider;
use workx_api::SseTelemetry;
use workx_api::StreamOptions;
use workx_api::TransportError;
use workx_api::WebsocketTelemetry;
use workx_api::auth_header_telemetry;
use workx_api::build_session_headers;
use workx_api::create_text_param_for_request;
use workx_api::response_create_client_metadata;
use workx_http_client::ClientRouteClass;
use workx_http_client::HttpClientFactory;
use workx_login::AuthManager;
use workx_login::RefreshTokenError;
use workx_login::UnauthorizedRecovery;
use workx_login::WorkxAuth;
use workx_login::default_client::add_originator_header;
use workx_login::default_client::create_client_for_route;
...
use workx_protocol::config_types::Verbosity as VerbosityConfig;
...
use workx_protocol::protocol::Event as ProtocolEvent;
...
use workx_rollout_trace::CompactionTraceContext;
use workx_rollout_trace::InferenceTraceAttempt;
use workx_rollout_trace::InferenceTraceContext;
use workx_tools::create_tools_json_for_responses_api;
use workx_tools::create_tools_json_for_responses_lite;
use workx_tools::create_tools_raw_json_for_responses_api;
...
use crate::responses_metadata::WorkxResponsesMetadata;
...
use workx_login::auth_env_telemetry::AuthEnvTelemetry;
use workx_login::auth_env_telemetry::collect_auth_env_telemetry;
use workx_model_provider::AgentIdentitySessionFallback;
use workx_model_provider::ProviderAuthScope;
use workx_model_provider::ProviderUnauthorizedRecovery;
use workx_model_provider::SharedModelProvider;
use workx_model_provider::create_model_provider;
#[cfg(test)]
use workx_model_provider_info::DEFAULT_WEBSOCKET_CONNECT_TIMEOUT_MS;
use workx_model_provider_info::ModelProviderInfo;
use workx_model_provider_info::WireApi;
use workx_protocol::error::Result;
use workx_protocol::error::WorkxErr;
use workx_response_debug_context::extract_response_debug_context;
use workx_response_debug_context::extract_response_debug_context_from_api_error;
use workx_response_debug_context::telemetry_api_error_message;
use workx_response_debug_context::telemetry_transport_error_message;

pub const OPENAI_BETA_HEADER: &str = "OpenAI-Beta";
pub const X_WORKX_INSTALLATION_ID_HEADER: &str = "x-codex-installation-id";
pub const X_WORKX_ROUTING_HINT_HEADER: &str = "x-codex-routing-hint";
pub const X_WORKX_TURN_STATE_HEADER: &str = "x-codex-turn-state";
pub const X_WORKX_TURN_METADATA_HEADER: &str = "x-codex-turn-metadata";
pub const X_WORKX_PARENT_THREAD_ID_HEADER: &str = "x-codex-parent-thread-id";
pub const X_WORKX_WINDOW_ID_HEADER: &str = "x-codex-window-id";
...
const X_WORKX_WS_STREAM_REQUEST_START_MS_CLIENT_METADATA_KEY: &str =
    "x-codex-ws-stream-request-start-ms";
const WS_REQUEST_HEADER_RESPONSES_LITE_CLIENT_METADATA_KEY: &str =
    "ws_request_header_x_openai_internal_workx_responses_lite";
const RESPONSES_WEBSOCKETS_V2_BETA_HEADER_VALUE: &str = "responses_websockets=2026-02-06";
const X_OPENAI_INTERNAL_WORKX_RESPONSES_LITE_HEADER: &str =
...
///
/// This holds configuration and state that should be shared across turns within a Workx session
...
///   the same turn.
///
/// Create a fresh `ModelClientSession` for each Workx turn. Reusing it across turns would replay
...
impl ModelClient {
    ...
    ///
    /// All arguments are expected to be stable for the lifetime of a Workx session. Per-turn values
    ...
    pub fn new(
        ...
        let workx_api_key_env_enabled = model_provider
            .auth_manager()
            .as_ref()
            .is_some_and(|manager| manager.workx_api_key_env_enabled());
        let auth_env_telemetry =
            collect_auth_env_telemetry(model_provider.info(), workx_api_key_env_enabled);
    ...
    fn prompt_cache_key(&self, responses_metadata: &WorkxResponsesMetadata) -> String {
    ...
    pub(crate) fn force_http_fallback(
        ...
        if activated {
            ...
            session_telemetry.counter(
                "workx.transport.fallback_to_http",
                /*inc*/ 1,
    ...
    pub(crate) async fn compact_conversation_history(
        ...
        responses_metadata: &WorkxResponsesMetadata,
        ...
        let request_telemetry = Self::build_request_telemetry(
            ...
            AuthRequestTelemetryContext::new(
                client_setup.auth.as_ref().map(WorkxAuth::auth_mode),
        ...
        if let Ok(header_value) = HeaderValue::from_str(&responses_metadata.installation_id) {
            extra_headers.insert(X_WORKX_INSTALLATION_ID_HEADER, header_value);
        }
        ...
        if let Some(header_value) = self.build_routing_hint_header(
            ...
            extra_headers.insert(X_WORKX_ROUTING_HINT_HEADER, header_value);
        }
    ...
    fn build_responses_request(
        ...
        .then_some(StreamOptions {
            reasoning_summary_delivery: workx_api::ReasoningSummaryDelivery::SequentialCutoff,
        });
    ...
    fn responses_endpoint(&self, auth: Option<&WorkxAuth>, model: &str) -> ResponsesEndpoint {
        if self.free_guardian_enabled
            && crate::guardian::is_basic_session_source(&self.state.session_source)
            && self.uses_workx_backend(auth)
            && self.state.provider.info().supports_workx_backend_routes()
    ...
    fn uses_workx_backend(&self, auth: Option<&WorkxAuth>) -> bool {
        let provider = self.state.provider.info();
        auth.is_some_and(WorkxAuth::uses_workx_backend)
            && provider.is_openai()
    ...
    fn build_routing_hint_header(
        ...
        if !self.uses_workx_backend(auth) {
            return None;
        }
    ...
    async fn connect_websocket(
        &self,
        session_telemetry: &SessionTelemetry,
        api_provider: workx_api::Provider,
        api_auth: SharedAuthProvider,
        responses_metadata: &WorkxResponsesMetadata,
        auth_context: AuthRequestTelemetryContext,
        request_route_telemetry: RequestRouteTelemetry,
        ...
        let result = match tokio::time::timeout(
            ...
            ApiWebSocketResponsesClient::new(api_provider, api_auth)
                ...
                .connect(
                    &self.http_client_factory,
                    headers,
                    workx_login::default_client::default_headers(),
                    /*turn_state*/ None,
    ...
    async fn build_websocket_headers(
        ...
        if let Some(routing_hint) = &responses_metadata.routing_hint {
            headers.insert(X_WORKX_ROUTING_HINT_HEADER, routing_hint.clone());
        }
        if let Some(header_value) = self.generate_attestation_header_for().await {
...
impl ModelClientSession {
    ...
    fn responses_request_compression(&self, auth: Option<&WorkxAuth>) -> Compression {
    ...
    async fn stream_responses_api(
        ...
        loop {
                ...
                )
            {
                options
                    .extra_headers
                    .insert(X_WORKX_ROUTING_HINT_HEADER, header_value);
            }
    ...
    async fn stream_responses_websocket(
        ...
        loop {
            ...
            if let Some(turn_state) = self.turn_state.get() {
                client_metadata.insert(X_WORKX_TURN_STATE_HEADER.to_string(), turn_state.clone());
            }
            match self
    ...
    /// Permanently disables WebSockets for this Workx session and resets WebSocket state.
    ///
...
fn stamp_ws_stream_request_start_ms(request: &mut ResponsesWsRequest<'_>) {
    ...
    payload
        ...
        .insert(
            X_WORKX_WS_STREAM_REQUEST_START_MS_CLIENT_METADATA_KEY.to_string(),
...
///
/// These headers implement Workx-specific conventions:
///
...
fn build_responses_headers(
    ...
    {
        headers.insert(X_WORKX_TURN_STATE_HEADER, header_value);
    }
    headers
}
...
fn add_responses_lite_header(headers: &mut ApiHeaderMap, use_responses_lite: bool) {
    if use_responses_lite {
        headers.insert(
            X_OPENAI_INTERNAL_WORKX_RESPONSES_LITE_HEADER,
            HeaderValue::from_static("true"),
        );
    }
}
...
fn map_response_stream(
    api_stream: workx_api::ResponseStream,
    session_telemetry: SessionTelemetry,
    inference_trace_attempt: InferenceTraceAttempt,
    provider: SharedModelProvider,
) -> (ResponseStream, oneshot::Receiver<LastResponse>) {
    let workx_api::ResponseStream {
        rx_event,
        upstream_request_id,
    } = api_stream;
    let api_stream = workx_api::ResponseStream {
        rx_event,
...
/// the mapped `WorkxErr` is returned to the caller.
...
struct WebsocketConnectParams<'a> {
    ...
    api_provider: workx_api::Provider,
    api_auth: SharedAuthProvider,
    responses_metadata: &'a WorkxResponsesMetadata,
    auth_context: AuthRequestTelemetryContext,
    request_route_telemetry: RequestRouteTelemetry,
    endpoint: ResponsesEndpoint,
}
        ...
        && recovery.has_next()
    {
        ...
        return match recovery.next().await {
            ...
            Err(RefreshTokenError::Permanent(failed)) => {
                ...
                Err(WorkxErr::RefreshTokenFailed(failed))
            }

...
use workx_analytics::TurnAnalyticsMetadata;
...
use workx_sandboxing::get_platform_sandbox;
...
fn test_turn_responses_metadata_json(state: &TurnMetadataState, window_id: &str) -> String {
    test_responses_metadata_json(state, window_id, WorkxResponsesRequestKind::Turn)
}
...
fn test_compaction_responses_metadata_json(
    ...
    test_responses_metadata_json(
        state,
        window_id,
        WorkxResponsesRequestKind::Compaction(compaction),
    )
}
...
async fn detached_memory_responses_metadata_omits_turn_identity() {
    ...
    let thread_manager = crate::ThreadManager::with_models_provider_for_tests(
        workx_login::WorkxAuth::from_api_key("test"),
...
fn turn_metadata_state_merges_client_metadata_without_replacing_reserved_fields() {
    ...
    state.set_responses_api_metadata(BTreeMap::from([
        ("workx_security_surface".to_string(), "sdk".to_string()),
    ...
    state.set_responsesapi_client_metadata(HashMap::from([
        (
            "workx_security_surface".to_string(),
            "client-supplied".to_string(),
        ),
    ...
    assert_eq!(json["workx_security_surface"].as_str(), Some("sdk"));
    ...
    assert_eq!(
        model_request_json["workx_security_surface"].as_str(),
        Some("sdk")
    );
    assert_eq!(
    ...
    assert!(meta.get("workx_security_surface").is_none());
    assert_eq!(state.workspace_kind().as_deref(), Some("projectless"));
    assert_eq!(
        (state.turn_trigger(), state.workx_turn_source()),
        (
            Some("goal".to_string()),
    ...
    for (configured, client, expected) in [
        ...
        assert_eq!(state.workx_turn_source(), expected);
    }
}
...
fn turn_metadata_state_overlays_compaction_only_on_compaction_requests() {
    ...
    assert_eq!(compact_json["workx_security_surface"].as_str(), Some("sdk"));
    assert_eq!(
    ...
    assert_eq!(regular_json["workx_security_surface"].as_str(), Some("sdk"));

...
async fn clears_active_handoff_explicitly() {
    ...
    let state = RealtimeHandoffState {
        ...
        workx_response_handoff_channel_prefixes: Arc::new(BTreeMap::new()),
...
fn realtime_headers_include_only_non_default_originator() {
    let default_originator = workx_login::default_client::originator();
    for (originator, expected_header) in [
        ("workx_work_desktop", Some("workx_work_desktop")),

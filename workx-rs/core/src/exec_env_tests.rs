...
fn inject_permission_profile_env_overrides_policy_value() {
    ...
    assert_eq!(
        env.get(WORKX_PERMISSION_PROFILE_ENV_VAR)
            .map(String::as_str),
...
fn inject_permission_profile_env_removes_stale_value_without_active_profile() {
    ...
    assert_eq!(env.get(WORKX_PERMISSION_PROFILE_ENV_VAR), None);
}

#[test]
fn inject_apply_patch_env_follows_preserve_line_endings_feature() {
    let mut env = HashMap::from([(
        WORKX_APPLY_PATCH_PRESERVE_LINE_ENDINGS_ENV_VAR.to_ascii_lowercase(),
        "stale".to_string(),
    )]);
...
fn test_core_inherit_defaults_keep_sensitive_vars() {
    ...
    expected.insert(WORKX_THREAD_ID_ENV_VAR.to_string(), thread_id.to_string());

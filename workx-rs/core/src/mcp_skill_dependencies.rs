...
use workx_config::load_global_mcp_servers;
use workx_login::default_client::is_first_party_originator;
...
use workx_skills::SkillToolDependency;
...
pub(crate) async fn maybe_prompt_and_install_mcp_dependencies(
    ...
    if mentioned_skills.is_empty()
        || !config
            .features
            .enabled(workx_features::Feature::SkillMcpDependencyInstall)
    {
        return;
    }
...
async fn maybe_install_mcp_dependencies(
    ...
    let mut servers = match load_global_mcp_servers(&workx_home).await {
        Ok(servers) => servers,
        Err(err) => {
    ...
    if let Err(err) = ConfigEditsBuilder::new(&workx_home)

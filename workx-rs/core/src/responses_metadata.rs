...
use crate::client::X_WORKX_INSTALLATION_ID_HEADER;
use crate::client::X_WORKX_PARENT_THREAD_ID_HEADER;
use crate::client::X_WORKX_TURN_METADATA_HEADER;
use crate::client::X_WORKX_WINDOW_ID_HEADER;
...
pub(crate) enum WorkxResponsesRequestKind {
    Turn,
    Prewarm,
...
impl WorkxResponsesRequestKind {
    fn metadata(self) -> (&'static str, Option<CompactionTurnMetadata>) {
        match self {
            WorkxResponsesRequestKind::Turn => ("turn", None),
            WorkxResponsesRequestKind::Prewarm => ("prewarm", None),
            WorkxResponsesRequestKind::Compaction(metadata) => ("compaction", Some(metadata)),
            WorkxResponsesRequestKind::Memory => ("memory", None),
        }
    }

    fn has_turn_identity(self) -> bool {
        !matches!(self, WorkxResponsesRequestKind::Memory)
    }
}
...
/// Caller-owned snapshot of Workx metadata sent to ResponsesAPI.
///
/// The full Workx turn metadata blob is transported canonically as
...
/// truth.
#[derive(Clone, Debug)]
pub struct WorkxResponsesMetadata {
    ...
    pub(crate) request_kind: Option<WorkxResponsesRequestKind>,
...
impl WorkxResponsesMetadata {
    ...
    pub(crate) fn client_metadata(&self) -> HashMap<String, String> {
        let mut client_metadata = HashMap::from([
            (
                X_WORKX_INSTALLATION_ID_HEADER.to_string(),
                self.installation_id.clone(),
            ),
            (SESSION_ID_KEY.to_string(), self.session_id.clone()),
            (THREAD_ID_KEY.to_string(), self.thread_id.clone()),
            (X_WORKX_WINDOW_ID_HEADER.to_string(), self.window_id.clone()),
        ]);
        ...
        if let Some(parent_thread_id) = self.parent_thread_id {
            client_metadata.insert(
                X_WORKX_PARENT_THREAD_ID_HEADER.to_string(),
        ...
        {
            client_metadata.insert(X_WORKX_TURN_METADATA_HEADER.to_string(), turn_metadata_json);
        }
        client_metadata
    }
    ...
    pub(crate) fn compatibility_headers(&self) -> ApiHeaderMap {
        ...
        insert_header(&mut headers, X_WORKX_WINDOW_ID_HEADER, &self.window_id);
        ...
        if self.has_turn_metadata()
            && let Ok(turn_metadata_json) = to_ascii_json_string(&WorkxTurnMetadataPayload {
                tool_namespaces_info: None,
    ...
    fn turn_metadata_payload(&self) -> WorkxTurnMetadataPayload<'_> {
        ...
        let has_turn_identity =
            request_kind.is_none_or(WorkxResponsesRequestKind::has_turn_identity);
        let has_request_identity =
            request_kind.is_some_and(WorkxResponsesRequestKind::has_turn_identity);
        WorkxTurnMetadataPayload {
            ...
            compaction,
            // Extra metadata enriches the Workx turn metadata blob, not literal top-level
            // Responses client_metadata. Product metadata is validated while loading config;
            // app-server metadata has reserved Workx-owned keys filtered when it enters turn state.
            extra: &self.extra,
        }
    }
}
...
#[derive(Serialize)]
struct WorkxTurnMetadataPayload<'a> {
    #[serde(default, skip_serializing_if = "Option::is_none")]

...
use workx_analytics::CompactionTrigger;
use workx_analytics::HookRunFact;
...
use workx_core_plugins::executor_plugin_hook_sources;
use workx_hooks::InterruptRequest;
use workx_hooks::PermissionRequestDecision;
use workx_hooks::PermissionRequestOutcome;
use workx_hooks::PermissionRequestRequest;
use workx_hooks::PostToolUseOutcome;
use workx_hooks::PostToolUseRequest;
use workx_hooks::PreToolUseOutcome;
use workx_hooks::PreToolUseRequest;
use workx_hooks::SessionStartOutcome;
use workx_hooks::StartHookTarget;
use workx_hooks::StopHookTarget;
use workx_hooks::StopOutcome;
use workx_hooks::SubagentHookContext;
use workx_hooks::UserPromptSubmitOutcome;
use workx_hooks::UserPromptSubmitRequest;
use workx_hooks::hook_execution_mode_label;
use workx_hooks::hook_handler_type_label;
use workx_mcp::WORKX_APPS_MCP_SERVER_NAME;
use workx_otel::HOOK_RUN_DURATION_METRIC;
use workx_otel::HOOK_RUN_METRIC;
use workx_plugin::ExecutorPluginHookSource;
use workx_protocol::items::FunctionCallOutputItem;
...
use workx_protocol::protocol::HookCompletedEvent;
...
use workx_protocol::protocol::HookOutputEntryKind;
use workx_protocol::protocol::HookRunStatus;
use workx_protocol::protocol::HookRunSummary;
use workx_protocol::protocol::HookSource;
use workx_protocol::protocol::HookStartedEvent;
        ...
        let target = match &turn_context.session_source {
            SessionSource::SubAgent(SubAgentSource::ThreadSpawn { agent_role, .. })
                if matches!(
                    ...
                    workx_hooks::SessionStartSource::Startup
                ) =>
            {
...
pub(crate) async fn run_turn_stop_hooks(
    ...
    let request = workx_hooks::StopRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        #[allow(deprecated)]
        cwd: turn_context.cwd.clone(),
...
pub(crate) async fn run_session_end_hooks(sess: &Arc<Session>) {
    ...
    let request = workx_hooks::SessionEndRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        #[allow(deprecated)]
        cwd: turn_context.cwd.clone(),
...
pub(crate) async fn run_pre_compact_hooks(
    ...
    let request = workx_hooks::PreCompactRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        subagent: thread_spawn_subagent_hook_context(sess, turn_context),
        #[allow(deprecated)]
...
pub(crate) async fn run_post_compact_hooks(
    ...
    let request = workx_hooks::PostCompactRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        subagent: thread_spawn_subagent_hook_context(sess, turn_context),
        #[allow(deprecated)]
...
pub(crate) async fn run_legacy_after_agent_hook(
    ...
    for hook_outcome in hooks
        .dispatch(workx_hooks::HookPayload {
            session_id: sess.session_id().into(),
            #[allow(deprecated)]
            cwd: turn_context.cwd.clone(),
            client: turn_context.app_server_client_name.clone(),
            triggered_at: chrono::Utc::now(),
            hook_event: workx_hooks::HookEvent::AfterAgent {
                event: workx_hooks::HookEventAfterAgent {
        ...
        .await
    {
        ...
        let (error, should_abort) = match hook_outcome.result {
            workx_hooks::HookResult::Success => continue,
            workx_hooks::HookResult::FailedContinue(error) => (error, false),
            workx_hooks::HookResult::FailedAbort(error) => (error, true),
        };
    ...
    let event = EventMsg::Error(workx_protocol::protocol::ErrorEvent {
        misalignment: None,
        message,
...
fn hook_run_analytics_payload(
    ...
    turn_context: &TurnContext,
    completed: &HookCompletedEvent,
) -> (workx_analytics::TrackEventsContext, HookRunFact) {
    (
...
mod tests {
    ...
    use workx_protocol::protocol::HookHandlerType;
    use workx_protocol::protocol::HookRunStatus;
    use workx_protocol::protocol::HookScope;
    use workx_protocol::protocol::HookSource;
        ...
        assert_eq!(
            messages
                ...
                .map(|message| match message {
                    workx_protocol::models::ResponseItem::Message { role, content, .. } => {
                        let text = content
                            .iter()
                            .map(|item| match item {
    ...
    async fn hook_lifecycle_notifications_hide_builtin_and_async_runs_but_preserve_metrics() {
        ...
        assert!(matches!(
            started.msg,
            workx_protocol::protocol::EventMsg::HookStarted(event)
        ...
        assert!(matches!(
            completed.msg,
            workx_protocol::protocol::EventMsg::HookCompleted(event)
                if event.run.id == synchronous_run.id
        ));
        assert!(events.try_recv().is_err());

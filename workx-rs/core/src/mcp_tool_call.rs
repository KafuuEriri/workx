...
use url::Url;
use workx_analytics::AppInvocation;
use workx_analytics::InvocationType;
use workx_analytics::build_track_events_context;
use workx_api::HostedFileUploadContext;
use workx_config::ConfigLayerSource;
use workx_config::types::AppToolApproval;
use workx_connectors::AppToolPolicy;
use workx_connectors::AppToolPolicyEvaluator;
use workx_connectors::AppToolPolicyInput;
use workx_extension_api::McpToolContext;
use workx_features::Feature;
use workx_mcp::MCP_TOOL_WORKX_APPS_META_KEY;
use workx_mcp::McpPermissionPromptAutoApproveContext;
use workx_mcp::PreparedMcpCall;
use workx_mcp::SandboxState;
use workx_mcp::ToolInfo;
use workx_mcp::WORKX_APPS_MCP_SERVER_NAME;
use workx_mcp::auth_elicitation_completed_result;
use workx_mcp::build_auth_elicitation_plan;
use workx_mcp::mcp_permission_prompt_is_auto_approved;
use workx_protocol::ResponseItemId;
use workx_protocol::approvals::ElicitationRequest;
use workx_protocol::items::McpToolCallError;
use workx_protocol::items::McpToolCallItem;
use workx_protocol::items::McpToolCallStatus;
use workx_protocol::items::TurnItem;
use workx_protocol::mcp::CONFIRMATION_POLICIES_META_KEY;
use workx_protocol::mcp::CallToolResult;
use workx_protocol::mcp::is_node_repl_backed_server;
use workx_protocol::mcp_approval_meta::APPROVAL_KIND_KEY as MCP_TOOL_APPROVAL_KIND_KEY;
use workx_protocol::mcp_approval_meta::APPROVAL_KIND_MCP_TOOL_CALL as MCP_TOOL_APPROVAL_KIND_MCP_TOOL_CALL;
use workx_protocol::mcp_approval_meta::CONNECTOR_DESCRIPTION_KEY as MCP_TOOL_APPROVAL_CONNECTOR_DESCRIPTION_KEY;
use workx_protocol::mcp_approval_meta::CONNECTOR_ID_KEY as MCP_TOOL_APPROVAL_CONNECTOR_ID_KEY;
use workx_protocol::mcp_approval_meta::CONNECTOR_NAME_KEY as MCP_TOOL_APPROVAL_CONNECTOR_NAME_KEY;
use workx_protocol::mcp_approval_meta::PERSIST_ALWAYS as MCP_TOOL_APPROVAL_PERSIST_ALWAYS;
use workx_protocol::mcp_approval_meta::PERSIST_KEY as MCP_TOOL_APPROVAL_PERSIST_KEY;
use workx_protocol::mcp_approval_meta::PERSIST_SESSION as MCP_TOOL_APPROVAL_PERSIST_SESSION;
use workx_protocol::mcp_approval_meta::SOURCE_CONNECTOR as MCP_TOOL_APPROVAL_SOURCE_CONNECTOR;
use workx_protocol::mcp_approval_meta::SOURCE_KEY as MCP_TOOL_APPROVAL_SOURCE_KEY;
use workx_protocol::mcp_approval_meta::TOOL_DESCRIPTION_KEY as MCP_TOOL_APPROVAL_TOOL_DESCRIPTION_KEY;
use workx_protocol::mcp_approval_meta::TOOL_PARAMS_DISPLAY_KEY as MCP_TOOL_APPROVAL_TOOL_PARAMS_DISPLAY_KEY;
use workx_protocol::mcp_approval_meta::TOOL_PARAMS_KEY as MCP_TOOL_APPROVAL_TOOL_PARAMS_KEY;
use workx_protocol::mcp_approval_meta::TOOL_TITLE_KEY as MCP_TOOL_APPROVAL_TOOL_TITLE_KEY;
...
use workx_protocol::request_user_input::RequestUserInputAnswer;
use workx_protocol::request_user_input::RequestUserInputArgs;
use workx_protocol::request_user_input::RequestUserInputQuestion;
use workx_protocol::request_user_input::RequestUserInputQuestionOption;
...
use workx_utils_output_truncation::TruncationPolicy;
use workx_utils_output_truncation::truncate_text;
...
const MCP_RESULT_TELEMETRY_META_KEY: &str = "workx/telemetry";
...
const MCP_RESULT_TELEMETRY_TARGET_ID_SPAN_ATTR: &str = "workx.mcp.target.id";
const MCP_RESULT_TELEMETRY_SERVER_USER_FLOW_SPAN_ATTR: &str =
    "workx.mcp.server_user_flow.triggered";
...
pub(crate) async fn handle_mcp_tool_call(
    ...
    let app_tool_policy = if server == WORKX_APPS_MCP_SERVER_NAME {
    ...
    let approval_mode = if server == WORKX_APPS_MCP_SERVER_NAME {
    ...
    if server == WORKX_APPS_MCP_SERVER_NAME && !app_tool_policy.enabled {
        let result = notify_mcp_tool_call_skip(
            sess.as_ref(),
            turn_context.as_ref(),
            &call_id,
...
impl McpToolCallItemMetadata {
    fn from_tool_metadata(server: &str, metadata: Option<&McpToolApprovalMetadata>) -> Self {
        let trusted_mcp_app_metadata = if server == WORKX_APPS_MCP_SERVER_NAME {
            metadata
        } else {
            None
        };
        Self {
            ...
            action_name: trusted_mcp_app_metadata
                .and_then(|metadata| metadata.workx_apps_meta.as_ref())
...
async fn handle_approved_mcp_tool_call(
    ...
    let result = async {
        let result = async {
            ...
            Ok(maybe_request_workx_apps_auth_elicitation(
                sess,
                turn_context,
    ...
    .await;
    maybe_track_workx_app_used(sess, turn_context, &server, &metadata).await;
...
fn mcp_tool_call_span(
    ...
    let span = tracing::info_span!(
        ...
        server.port = Empty,
        workx.mcp.target.id = Empty,
        workx.mcp.server_user_flow.triggered = Empty,
        error.type = Empty,
        workx.mcp.error.code = Empty,
    );
...
async fn maybe_request_workx_apps_auth_elicitation(
    sess: &Arc<Session>,
    turn_context: &TurnContext,
    ...
    if server != WORKX_APPS_MCP_SERVER_NAME {
        return result;
    }
    ...
    let install_url = connector_id.map(|connector_id| {
        workx_connectors::metadata::connector_install_url(
    ...
    refresh_workx_apps_after_connector_auth(sess, turn_context).await;
    auth_elicitation_completed_result(&plan.auth_failure, result.meta)
}

async fn refresh_workx_apps_after_connector_auth(sess: &Arc<Session>, turn_context: &TurnContext) {
    let mcp_tools_result = sess.hard_refresh_latest_workx_apps_tools().await;
    ...
    match mcp_tools_result {
        ...
        Err(err) => {
            tracing::warn!("failed to refresh Workx Apps tools after connector auth: {err:#}");
        }
    }
}
...
async fn augment_mcp_tool_request_meta_with_sandbox_state(
    ...
    let sandbox_state = serde_json::to_value(SandboxState {
        ...
        workx_linux_sandbox_exe: prepared_call.config().workx_linux_sandbox_exe.clone(),
        sandbox_cwd,
    ...
    match meta.as_mut() {
        Some(serde_json::Value::Object(map)) => {
            map.insert(
                workx_mcp::MCP_SANDBOX_STATE_META_CAPABILITY.to_string(),
                sandbox_state,
            );
        }
        Some(_) => {}
...
fn sandbox_cwd_for_mcp_server(step_context: &StepContext, environment_id: &str) -> Option<PathUri> {
    ...
    if environment_id == workx_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID {
        #[allow(deprecated)]
...
async fn maybe_track_workx_app_used(
    sess: &Session,
    turn_context: &TurnContext,
    server: &str,
    metadata: &McpToolApprovalMetadata,
...
pub(crate) struct McpToolApprovalMetadata {
    ...
    mcp_app_resource_uri: Option<String>,
    workx_apps_meta: Option<serde_json::Map<String, serde_json::Value>>,
...
impl Session {
    async fn register_mcp_tool_approval_metadata(
        ...
        turn_state.lock().await.insert_mcp_tool_approval_metadata(
            call_id.to_string(),
            (invocation.server == WORKX_APPS_MCP_SERVER_NAME
...
async fn custom_mcp_tool_approval_mode(
    ...
    let user_configured_mode = turn_context
        ...
        .cloned()
        .and_then(|value| {
            HashMap::<String, workx_config::types::McpServerConfig>::deserialize(value).ok()
        })
        .and_then(|servers| {
...
fn build_mcp_tool_call_request_meta(
    ...
    {
        request_meta.insert(
            crate::X_WORKX_TURN_METADATA_HEADER.to_string(),
            turn_metadata,
        );
    }

    if server == WORKX_APPS_MCP_SERVER_NAME {
        let mut workx_apps_meta = metadata
            .and_then(|metadata| metadata.workx_apps_meta.clone())
            .unwrap_or_default();
        workx_apps_meta.insert(
            "call_id".to_string(),
            serde_json::Value::String(call_id.to_string()),
        );
        request_meta.insert(
            MCP_TOOL_WORKX_APPS_META_KEY.to_string(),
            serde_json::Value::Object(workx_apps_meta),
        );
    }
...
async fn maybe_request_mcp_tool_approval(
    ...
    let action = ApprovalAction::McpToolCall {
        ...
        connected_account_email: (invocation.server == WORKX_APPS_MCP_SERVER_NAME)
    ...
    Some(
        match sess.request_approval(action, approval_context).await {
            ...
            Err(ToolError::Workx(_)) => ReviewDecision::Abort,
        },
    )
}
...
fn session_mcp_tool_approval_key(
    ...
    if invocation.server == WORKX_APPS_MCP_SERVER_NAME && connector_id.is_none() {
        return None;
    }
...
fn mcp_tool_metadata(
    ...
    let connector_description = (server == WORKX_APPS_MCP_SERVER_NAME)
        .then(|| tool_info.namespace_description.clone())
        .flatten();

    let workx_apps_meta = tool_info
        .tool
        .meta
        .as_ref()
        .and_then(|meta| meta.get(MCP_TOOL_WORKX_APPS_META_KEY))
        .and_then(serde_json::Value::as_object)
        .cloned();
    let link_id = if server == WORKX_APPS_MCP_SERVER_NAME {
        account::resolve_account(&tool_info, arguments)?
    } else {
        None
    };
    let connected_account_email = if server == WORKX_APPS_MCP_SERVER_NAME {
        workx_apps_meta
            .as_ref()
    ...
    if let Some(metadata) = metadata {
                ...
                || metadata.connector_description.is_some())
        {
            ...
            if let Some(link_id) = metadata.link_id.as_ref() {
                ...
                // Match the Workx Apps service's reserved implicit-link ID prefix.
                meta.insert(
...
async fn maybe_persist_mcp_tool_approval(
    ...
    let persist_result = if key.server == WORKX_APPS_MCP_SERVER_NAME {
        let Some(connector_id) = key.connector_id.clone() else {
            remember_mcp_tool_approval(sess, key).await;
            return;
        };
        persist_workx_app_tool_approval(&turn_context.config, &connector_id, &tool_name).await
    } else {
...
async fn persist_workx_app_tool_approval(
    config: &Config,
    connector_id: &str,
    tool_name: &str,
) -> anyhow::Result<()> {
...
fn user_mcp_server_is_configured(config: &Config, server: &str) -> anyhow::Result<bool> {
    ...
    let servers =
        HashMap::<String, workx_config::types::McpServerConfig>::deserialize(mcp_servers_toml)?;

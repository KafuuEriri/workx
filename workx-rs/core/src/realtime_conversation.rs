...
use crate::client::X_WORKX_TURN_METADATA_HEADER;
...
use workx_api::RealtimeAudioFrame;
use workx_api::RealtimeContextAppendChannel;
use workx_api::RealtimeEvent;
use workx_api::RealtimeEventParser;
use workx_api::RealtimeSessionConfig;
use workx_api::RealtimeSessionMode;
use workx_api::RealtimeWebsocketClient;
use workx_api::RealtimeWebsocketConnection;
use workx_api::RealtimeWebsocketEvents;
use workx_api::RealtimeWebsocketWriter;
...
use workx_login::read_openai_api_key_from_env;
...
use workx_protocol::protocol::RealtimeConversationClosedEvent;
use workx_protocol::protocol::RealtimeConversationRealtimeEvent;
use workx_protocol::protocol::RealtimeConversationSdpEvent;
use workx_protocol::protocol::RealtimeConversationStartedEvent;
use workx_protocol::protocol::RealtimeHandoffRequested;
use workx_protocol::protocol::RealtimeOutputModality;
use workx_protocol::protocol::RealtimeTranscriptEntry;
...
use workx_utils_output_truncation::approx_bytes_for_tokens;
use workx_utils_string::approx_token_count;
use workx_utils_string::take_bytes_at_char_boundary;
use workx_utils_string::to_ascii_json_string;
...
const STANDALONE_HANDOFF_ID: &str = "workx";
...
struct RealtimeHandoffState {
    ...
    workx_responses_as_items: bool,
    workx_response_item_prefix: Option<String>,
    workx_response_handoff_mode: WorkxResponseHandoffMode,
    workx_response_handoff_channel_prefixes: Arc<BTreeMap<String, Vec<String>>>,
...
impl RealtimeHandoffState {
    fn streams_handoff_append(&self) -> bool {
        self.event_parser == RealtimeEventParser::FramelessBidi
            ...
            && !self.workx_responses_as_items
    }

    fn routes_handoff_by_bem(&self) -> bool {
        self.event_parser == RealtimeEventParser::FramelessBidi
            && self.workx_response_handoff_mode == WorkxResponseHandoffMode::BemTags
    }
}
...
impl RealtimeConversationManager {
    ...
    ) -> WorkxResult<RealtimeStartOutput> {
        let previous_state = {
    ...
    async fn start_inner(&self, start: RealtimeStart) -> WorkxResult<RealtimeStartOutput> {
        let RealtimeStart {
            api_provider,
        ...
        let handoff = RealtimeHandoffState {
            ...
            workx_response_item_prefix,
            workx_response_handoff_mode,
            workx_response_handoff_channel_prefixes: Arc::new(
                workx_response_handoff_channel_prefixes.unwrap_or_default(),
            ),
            session_kind,
            event_parser,
        };
    ...
    pub(crate) async fn audio_in(&self, frame: RealtimeAudioFrame) -> WorkxResult<()> {
        let sender = {
        ...
        match sender.try_send(frame) {
            ...
            Err(TrySendError::Closed(_)) => Err(WorkxErr::InvalidRequest(
                "conversation is not running".to_string(),
            )),
        }
    }

    pub(crate) async fn text_in(&self, mut params: ConversationTextParams) -> WorkxResult<()> {
        let sender = {
            let guard = self.state.lock().await;
            guard
                .as_ref()
        ...
        sender
            .send(params)
            .await
            .map_err(|_| WorkxErr::InvalidRequest("conversation is not running".to_string()))?;
        Ok(())
    }
    ...
    pub(crate) async fn handoff_out(
        ...
        let phase = if handoff.routes_handoff_by_bem() {
            match bem_message_phase(
                &output_text,
                &handoff.workx_response_handoff_channel_prefixes,
            ) {
        ...
        let output = match active_handoff {
            Some(handoff_id) => {
                ...
                *handoff.last_output.lock().await = Some(RealtimeHandoffOutput {
                ...
                if handoff.workx_responses_as_items {
                    RealtimeOutbound::ConversationItem {
                        text: realtime_backend_item(
                            output_text,
                            handoff.workx_response_item_prefix.as_deref(),
                        ),
                        phase,
                    }
    ...
    pub(crate) async fn append_speech(&self, text: String) -> WorkxResult<()> {
    ...
    pub(crate) async fn handoff_complete(&self) -> WorkxResult<()> {
        let handoff = {
            let guard = self.state.lock().await;
            guard.as_ref().map(|state| state.handoff.clone())
        };
        ...
        let output = if handoff.workx_responses_as_items {
    ...
    pub(crate) async fn shutdown(&self) -> WorkxResult<()> {
        let state = {
            let mut guard = self.state.lock().await;
            guard.take()
        };
...
async fn prepare_realtime_start(
    sess: &Arc<Session>,
    params: ConversationStartParams,
) -> WorkxResult<PreparedRealtimeConversationStart> {
    ...
    {
        extra_headers.insert(X_WORKX_TURN_METADATA_HEADER, metadata);
    }
    Ok(PreparedRealtimeConversationStart {
        ...
        workx_responses_as_items: params.workx_responses_as_items,
        workx_response_item_prefix: params.workx_response_item_prefix,
        workx_response_handoff_mode: params.workx_response_handoff_mode,
        workx_response_handoff_channel_prefixes: params.workx_response_handoff_channel_prefixes,
...
) -> WorkxResult<RealtimeSessionConfig> {
...
fn validate_realtime_voice(version: RealtimeWsVersion, voice: RealtimeVoice) -> WorkxResult<()> {
    let voices = RealtimeVoicesList::builtin();
...
pub(crate) async fn handle_audio(
    ...
    if let Err(err) = sess.conversation.audio_in(params.frame).await {
        ...
        if sess.conversation.running_state().await.is_some() {
            ...
            send_conversation_error(sess, sub_id, err.to_string(), WorkxErrorInfo::BadRequest)
                .await;
        }
    }
}
...
fn realtime_api_key(auth: Option<&WorkxAuth>, provider: &ModelProviderInfo) -> WorkxResult<String> {
    ...
    if let Some(api_key) = auth.and_then(WorkxAuth::api_key) {
...
fn realtime_request_headers(
    ...
    if let Some(api_key) = api_key {
        let auth_value = HeaderValue::from_str(&format!("Bearer {api_key}")).map_err(|err| {
            WorkxErr::InvalidRequest(format!("invalid realtime api key header: {err}"))
        })?;
...
fn v3_output_writer(
    writer: &RealtimeWebsocketWriter,
    phase: Option<&MessagePhase>,
    handoff_mode: WorkxResponseHandoffMode,
) -> RealtimeWebsocketWriter {
    let channel = match handoff_mode {
        WorkxResponseHandoffMode::Thinking => None,
        WorkxResponseHandoffMode::Commentary => Some(RealtimeContextAppendChannel::Commentary),
        WorkxResponseHandoffMode::BemTags => match phase {
        ...
        RealtimeEventParser::FramelessBidi => match handoff_output {
            RealtimeOutbound::StandaloneHandoff { text, phase } => {
                v3_output_writer(
                    writer,
                    phase.as_ref(),
                    handoff_state.workx_response_handoff_mode,
                )
                .send_standalone_handoff(STANDALONE_HANDOFF_ID.to_string(), text)
                .await
            }
...
async fn send_conversation_error(
    ...
    sub_id: String,
    message: String,
    workx_error_info: WorkxErrorInfo,
) {
    sess.send_event_raw(Event {
        id: sub_id,

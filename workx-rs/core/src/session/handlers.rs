...
use workx_protocol::mcp::RequestId as ProtocolRequestId;
...
pub async fn inter_agent_communication(
    ...
    start_options: workx_protocol::turn_input::TurnStartOptions,
) {
...
pub async fn resolve_elicitation(
    ...
    decision: workx_protocol::approvals::ElicitationAction,
    content: Option<Value>,
    meta: Option<Value>,
) {
    let action = match decision {
        workx_protocol::approvals::ElicitationAction::Accept => ElicitationAction::Accept,
        workx_protocol::approvals::ElicitationAction::Decline => ElicitationAction::Decline,
        workx_protocol::approvals::ElicitationAction::Cancel => ElicitationAction::Cancel,
    };
    ...
    if let Err(err) = sess.flush_rollout().await {
        sess.send_event(
            ...
            EventMsg::Warning(WarningEvent {
                message: format!(
                    "Rolled the thread back, but failed to save the rollback marker. Workx will continue retrying. Error: {err}"
                ),
            }),
        )
        .await;
...
pub(super) async fn emit_thread_stop_lifecycle(sess: &Session) {
    for contributor in sess.services.extensions.thread_lifecycle_contributors() {
        contributor
            .on_thread_stop(workx_extension_api::ThreadStopInput {
...
pub async fn shutdown(sess: &Arc<Session>, sub_id: String) -> bool {
    ...
    info!("Shutting down Workx instance");
    ...
    sess.services
        .rollout_thread_trace
        .record_ended(workx_rollout_trace::RolloutStatus::Completed);
    true
}
            ...
            match sub.op {
                ...
                Op::SuspendTurnAndShutdown { reply } => {
                    ...
                    let should_exit = matches!(
                        &result,
                        Ok(workx_protocol::turn_input::SuspendTurnOutcome::Suspended { .. })
                    );
                    let _ = reply.send(result);
                    should_exit
                }

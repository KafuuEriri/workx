...
use serde_json::Map;
use workx_exec_server::ExecutorCapabilityDiscoveryCache;
use workx_exec_server::ExecutorCapabilityDiscoverySnapshot;
use workx_exec_server::MAX_SELECTED_CAPABILITY_ROOTS;
use workx_exec_server::ResolvedSelectedCapabilityRoot;
use workx_mcp::ElicitationReviewRequest;
use workx_mcp::ElicitationReviewer;
use workx_mcp::ElicitationReviewerHandle;
...
use workx_protocol::mcp_approval_meta::APPROVAL_KIND_KEY as MCP_ELICITATION_APPROVAL_KIND_KEY;
use workx_protocol::mcp_approval_meta::APPROVAL_KIND_MCP_TOOL_CALL as MCP_ELICITATION_APPROVAL_KIND_MCP_TOOL_CALL;
use workx_protocol::mcp_approval_meta::APPROVAL_KIND_TOOL_SUGGESTION as MCP_ELICITATION_APPROVAL_KIND_TOOL_SUGGESTION;
use workx_protocol::mcp_approval_meta::APPROVALS_REVIEWER_KEY as MCP_ELICITATION_APPROVALS_REVIEWER_KEY;
use workx_protocol::mcp_approval_meta::CONNECTOR_DESCRIPTION_KEY as MCP_ELICITATION_CONNECTOR_DESCRIPTION_KEY;
use workx_protocol::mcp_approval_meta::CONNECTOR_ID_KEY as MCP_ELICITATION_CONNECTOR_ID_KEY;
use workx_protocol::mcp_approval_meta::CONNECTOR_NAME_KEY as MCP_ELICITATION_CONNECTOR_NAME_KEY;
use workx_protocol::mcp_approval_meta::PERSIST_KEY as MCP_ELICITATION_PERSIST_KEY;
use workx_protocol::mcp_approval_meta::REQUEST_TYPE_APPROVAL_REQUEST as MCP_ELICITATION_REQUEST_TYPE_APPROVAL_REQUEST;
use workx_protocol::mcp_approval_meta::REQUEST_TYPE_KEY as MCP_ELICITATION_REQUEST_TYPE_KEY;
use workx_protocol::mcp_approval_meta::SENSITIVE_ACTION_KEY as MCP_ELICITATION_SENSITIVE_ACTION_KEY;
use workx_protocol::mcp_approval_meta::STRICT_AUTO_REVIEW_KEY as MCP_ELICITATION_STRICT_AUTO_REVIEW_KEY;
use workx_protocol::mcp_approval_meta::TOOL_DESCRIPTION_KEY as MCP_ELICITATION_TOOL_DESCRIPTION_KEY;
use workx_protocol::mcp_approval_meta::TOOL_NAME_KEY as MCP_ELICITATION_TOOL_NAME_KEY;
use workx_protocol::mcp_approval_meta::TOOL_PARAMS_KEY as MCP_ELICITATION_TOOL_PARAMS_KEY;
use workx_protocol::mcp_approval_meta::TOOL_TITLE_KEY as MCP_ELICITATION_TOOL_TITLE_KEY;
use workx_protocol::openai_models::ModelInfo;
use workx_rmcp_client::Elicitation;
...
impl Session {
    ...
    pub(crate) async fn runtime_mcp_servers(
        ...
        workx_mcp::configured_mcp_servers(&self.runtime_mcp_config(config).await)
    }
    ...
    pub(crate) async fn hard_refresh_latest_workx_apps_tools(
        self: &Arc<Self>,
    ) -> anyhow::Result<Vec<workx_mcp::ToolInfo>> {
        ...
        anyhow::ensure!(
            input.mcp_servers.contains_key(WORKX_APPS_MCP_SERVER_NAME),
            "unknown MCP server '{WORKX_APPS_MCP_SERVER_NAME}'"
        );
    ...
    pub(crate) async fn mcp_runtime_for_step(
        ...
        Arc::new(workx_mcp::McpBinding::empty(config))
    }
    ...
    pub(crate) fn mcp_elicitation_lifecycle(&self) -> workx_mcp::ElicitationLifecycle {
        self.mcp_elicitation_lifecycle_handle
            .get_or_init(|| {
                let elicitations = self.services.elicitations.clone();
                workx_mcp::ElicitationLifecycle::new(move || elicitations.register())
            })
            .clone()
    }
    ...
    pub async fn request_mcp_server_elicitation(
        ...
        if self.services.mcp_runtime.elicitations_auto_deny() {
            return McpServerElicitationOutcome {
                response: Some(ElicitationResponse {
                    action: workx_rmcp_client::ElicitationAction::Accept,
        ...
        let id = match request_id {
            rmcp::model::NumberOrString::String(value) => {
                workx_protocol::mcp::RequestId::String(value.to_string())
            }
            rmcp::model::NumberOrString::Number(value) => {
                workx_protocol::mcp::RequestId::Integer(value)
            }
        };
...
async fn review_guardian_mcp_elicitation(
    ...
    let user_cua_execution = step_settings.approvals_reviewer() == ApprovalsReviewer::User
        ...
        && request.elicitation.meta().is_some_and(|meta| {
            metadata_str(meta, MCP_ELICITATION_TOOL_NAME_KEY) == Some("js")
                ...
                && meta.get("workx_requires_user_input") != Some(&Value::Bool(true))
        });
    ...
    if matches!(
        ...
        let trusted_guardian_request = if request.server_name == WORKX_APPS_MCP_SERVER_NAME {
            let Some(call_id) = request
                .elicitation
                .meta()
        ...
        let decision = crate::guardian::review_approval_request_with_cancel(
            ...
            crate::guardian::GuardianReviewOptions {
                ...
                approval_request_source: workx_analytics::GuardianApprovalRequestSource::MainTurn,
    ...
    match approval_policy {
        AskForApproval::Never => {
            ...
            if workx_mcp::mcp_permission_prompt_is_auto_approved(
                approval_policy,
                permission_profile,
                workx_mcp::McpPermissionPromptAutoApproveContext::default(),
            ) && matches!(
                &request.elicitation,
                Elicitation::Mcp(
...
fn plugin_install_elicitation_telemetry_metadata(
    ...
    let workx_protocol::approvals::ElicitationRequest::Form {

...
pub(super) async fn suspend_turn_and_shutdown(
...
) -> WorkxResult<SuspendTurnOutcome> {
    {
    ...
    let live_thread = session
        ...
        .map_err(|error| WorkxErr::Fatal(error.to_string()))?;
    ...
    live_thread.flush().await.map_err(|error| {
        WorkxErr::Fatal(format!("flush before root turn suspension failed: {error}"))
    })?;
    ...
    let mut turn = {
        ...
        active.take().ok_or_else(|| {
            WorkxErr::Fatal("accepted root turn suspension had no running turn".to_string())
        })?
    };

    let task = turn.task.take().ok_or_else(|| {
        WorkxErr::Fatal("accepted root turn suspension had no running task".to_string())
    })?;
    ...
    live_thread.flush().await.map_err(|error| {
        WorkxErr::Fatal(format!("flush after root turn suspension failed: {error}"))
    })?;
    live_thread.shutdown().await.map_err(|error| {
        WorkxErr::Fatal(format!("close suspended root turn writer failed: {error}"))
    })?;

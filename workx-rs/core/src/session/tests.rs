...
use core_test_support::test_workx::TurnInputRequest as ExternalTurnInputRequest;
use workx_analytics::CompactionPhase;
...
use workx_config::NetworkConstraints;
use workx_config::NetworkDomainPermissionToml;
use workx_config::NetworkDomainPermissionsToml;
use workx_config::RequirementSource;
use workx_config::Sourced;
use workx_config::loader::project_trust_key;
use workx_config::types::McpServerConfig;
use workx_config::types::McpServerTransportConfig;
use workx_config::types::ToolSuggestDisabledTool;
use workx_config::types::WindowsSandboxModeToml;

use std::collections::BTreeMap;
use tracing::Span;
use workx_features::Feature;
use workx_file_system::FileSystemSandboxContext;
use workx_http_client::ClientRouteClass;
use workx_http_client::HttpClientFactory;
use workx_http_client::OutboundProxyPolicy;
use workx_http_client::RouteAwareClientPool;
use workx_login::WorkxAuth;
use workx_login::auth::AgentIdentityAuthPolicy;
use workx_model_provider::create_model_provider;
use workx_model_provider_info::ModelProviderInfo;
use workx_model_provider_info::built_in_model_providers;
use workx_models_manager::bundled_models_response;
use workx_models_manager::model_info;
use workx_models_manager::test_support::construct_model_info_offline_for_tests;
use workx_models_manager::test_support::get_model_offline_for_tests;
use workx_protocol::AgentPath;
use workx_protocol::ResponseItemId;
use workx_protocol::SessionId;
use workx_protocol::ThreadId;
use workx_protocol::config_types::SERVICE_TIER_DEFAULT_REQUEST_VALUE;
use workx_protocol::config_types::ServiceTier;
use workx_protocol::config_types::TrustLevel;
use workx_protocol::exec_output::ExecToolCallOutput;
use workx_protocol::models::AgentMessageInputContent;
use workx_protocol::models::FileSystemPermissions;
use workx_protocol::models::FunctionCallOutputBody;
use workx_protocol::models::FunctionCallOutputContentItem;
use workx_protocol::models::FunctionCallOutputPayload;
use workx_protocol::models::ImageDetail;
use workx_protocol::models::PermissionProfile;
use workx_protocol::models::SandboxEnforcement;
use workx_protocol::openai_models::ModelServiceTier;
use workx_protocol::openai_models::ToolMode;
...
use workx_protocol::permissions::FileSystemSandboxPolicyContext;
use workx_protocol::permissions::FileSystemSpecialPath;
use workx_protocol::protocol::EnvironmentConfigState;
use workx_protocol::protocol::ErrorEvent;
use workx_protocol::protocol::SandboxPolicy;
use workx_protocol::protocol::TurnEnvironmentSelections;
use workx_protocol::request_permissions::PermissionGrantScope;
use workx_protocol::request_permissions::RequestPermissionProfile;
use workx_protocol::turn_input::TurnInput as SubmittedTurnInput;
use workx_protocol::turn_input::TurnInputMode;
use workx_protocol::turn_input::TurnInputRequest;
use workx_protocol::turn_input::TurnInputSubmission;
use workx_tools::ToolSpec;
use workx_utils_path_uri::PathUri;
...
use core_test_support::test_workx::local;
...
use workx_config::config_toml::ConfigToml;
use workx_config::config_toml::ProjectConfig;
use workx_config::permissions_toml::FilesystemPermissionToml;
use workx_config::permissions_toml::FilesystemPermissionsToml;
use workx_config::permissions_toml::NetworkToml;
use workx_config::permissions_toml::PermissionProfileToml;
use workx_config::permissions_toml::PermissionsToml;
use workx_execpolicy::Decision;
use workx_execpolicy::NetworkRuleProtocol;
use workx_execpolicy::Policy;
use workx_history::CompactedItem;
use workx_history::InitialHistory;
use workx_history::ResponseItemEnvelope;
use workx_history::ResumedHistory;
use workx_history::RolloutItem;
use workx_history::WorkxHarnessMetadata;
use workx_network_proxy::NetworkProxyConfig;
use workx_otel::MetricsClient;
use workx_otel::MetricsConfig;
use workx_otel::TelemetryAuthMode;
use workx_protocol::config_types::CollaborationMode;
use workx_protocol::config_types::ModeKind;
use workx_protocol::config_types::Settings;
use workx_protocol::items::HookPromptFragment;
use workx_protocol::items::build_hook_prompt_message;
use workx_protocol::models::BaseInstructions;
use workx_protocol::models::ContentItem;
use workx_protocol::models::ContentItemKind;
use workx_protocol::models::InternalChatMessageMetadataPassthrough;
...
use workx_protocol::protocol::CreditsSnapshot;
use workx_protocol::protocol::GranularApprovalConfig;
use workx_protocol::protocol::HistoryPosition;
use workx_protocol::protocol::InterAgentCommunication;
use workx_protocol::protocol::MultiAgentVersion;
use workx_protocol::protocol::NetworkApprovalProtocol;
use workx_protocol::protocol::RateLimitSnapshot;
use workx_protocol::protocol::RateLimitWindow;
use workx_protocol::protocol::RealtimeAudioFrame;
use workx_protocol::protocol::RealtimeConversationListVoicesResponseEvent;
use workx_protocol::protocol::RealtimeVoice;
use workx_protocol::protocol::RealtimeVoicesList;
use workx_protocol::protocol::SessionMeta;
use workx_protocol::protocol::SessionMetaLine;
use workx_protocol::protocol::Submission;
use workx_protocol::protocol::ThreadRolledBackEvent;
use workx_protocol::protocol::ThreadSettingsOverrides;
use workx_protocol::protocol::TokenCountEvent;
use workx_protocol::protocol::TokenUsage;
use workx_protocol::protocol::TokenUsageInfo;
use workx_protocol::protocol::TokenUsageRecord;
use workx_protocol::protocol::TurnAbortedEvent;
use workx_protocol::protocol::TurnCompleteEvent;
use workx_protocol::protocol::TurnStartedEvent;
use workx_protocol::protocol::UserMessageEvent;
use workx_protocol::protocol::W3cTraceContext;
use workx_protocol::protocol::WorkxErrorInfo;
use workx_rmcp_client::ElicitationAction;
...
use workx_protocol::mcp::CallToolResult as McpCallToolResult;

pub(crate) fn mcp_config_for_test(config: &crate::config::Config) -> Arc<workx_mcp::McpConfig> {
    Arc::new(config.to_mcp_config_with_loaded_plugins(
        &workx_core_plugins::PluginLoadOutcome::default(),
        std::iter::empty(),
    ))
}
...
impl StepContext {
    pub(crate) fn for_test(turn: Arc<TurnContext>) -> Arc<Self> {
        ...
        Arc::new(Self {
            ...
            mcp: Arc::new(workx_mcp::McpBinding::empty(mcp_config_for_test(
                &turn.config,
            ))),
...
fn extension_metrics_preserve_session_metadata_tags() {
    ...
    extension_metrics.counter(
        "workx.test.extension.counter",
        /*inc*/ 2,
        &[("component", "skills"), ("model", "extension-model")],
    );

    let snapshot = metrics.snapshot().expect("metrics snapshot");
    let attributes = single_histogram_attributes(&snapshot, "workx.test.extension");
    let counter = find_metric(&snapshot, "workx.test.extension.counter");
...
async fn world_state_extension_metrics_follow_turn_model_switch() {
    ...
    impl workx_extension_api::ContextContributor for WorldStateMetricsRecorder {
        fn contribute_world_state<'a>(
            &'a self,
            input: workx_extension_api::WorldStateContributionInput<'a>,
        ) -> workx_extension_api::ExtensionFuture<
            'a,
            Vec<workx_extension_api::WorldStateSectionContribution>,
        > {
            Box::pin(async move {
                input
                    .extension_metrics
                    .expect("turn metrics should be available")
                    .histogram("workx.test.extension.turn", /*value*/ 1, &[]);
                Vec::new()
            })
        }
    }
    ...
    let mut builder = workx_extension_api::ExtensionRegistryBuilder::<crate::config::Config>::new();
    ...
    let snapshot = metrics.snapshot().expect("metrics snapshot");
    let attributes = single_histogram_attributes(&snapshot, "workx.test.extension.turn");
    assert_eq!(
...
async fn regular_turn_emits_turn_started_with_trace_id_without_waiting_for_startup_prewarm() {
    let _trace_test_context = install_test_tracing("workx-core-tests");
...
fn test_model_client_session() -> crate::client::ModelClientSession {
    ...
    crate::client::ModelClient::new(
        ...
        workx_protocol::protocol::SessionSource::Exec,
        "test_originator".to_string(),
...
fn write_project_hooks(dot_workx: &Path) -> std::io::Result<()> {
    std::fs::create_dir_all(dot_workx)?;
    std::fs::write(
        dot_workx.join("hooks.json"),
        r#"{
  "hooks": {
    "SessionStart": [
      {
...
async fn write_project_trust_config(
    ...
    tokio::fs::write(
        workx_home.join(workx_config::CONFIG_TOML_FILE),
...
async fn preview_session_start_hooks(
...
) -> std::io::Result<Vec<workx_protocol::protocol::HookRunSummary>> {
    ...
    Ok(
        hooks.preview_session_start(&workx_hooks::SessionStartRequest {
            session_id: thread_id,
            cwd: config.cwd.clone(),
            transcript_path: None,
            model: "gpt-5.2".to_string(),
            permission_mode: "default".to_string(),
            target: workx_hooks::StartHookTarget::SessionStart {
                source: workx_hooks::SessionStartSource::Startup,
            },
        }),
    )
}
...
async fn managed_network_proxy_decider_survives_full_access_start() -> anyhow::Result<()> {
    ...
    let network_policy_decider: Arc<dyn workx_network_proxy::NetworkPolicyDecider> = Arc::new({
        let decider_calls = Arc::clone(&decider_calls);
        move |_request| {
            decider_calls.fetch_add(1, std::sync::atomic::Ordering::SeqCst);
            async { workx_network_proxy::NetworkDecision::ask("not_allowed") }
        }
    });
...
async fn danger_full_access_tool_attempts_do_not_enforce_managed_network() -> anyhow::Result<()> {
    ...
    impl crate::tools::sandboxing::Approvable<TurnEnvironment> for ProbeToolRuntime {
        fn approval_action(
            ...
            Ok(crate::tools::sandboxing::ApprovalAction::ExecCommand {
                ...
                environment_id: workx_exec_server::LOCAL_ENVIRONMENT_ID.to_string(),
                command: Vec::new(),
    ...
    impl crate::tools::sandboxing::Sandboxable for ProbeToolRuntime {
        fn sandbox_preference(&self) -> workx_sandboxing::SandboxablePreference {
            workx_sandboxing::SandboxablePreference::Auto
        }
    }
    ...
    let session = make_session_with_config(move |config| {
        ...
        requirements_toml.network = Some(workx_config::NetworkRequirementsToml {
            enabled: Some(true),
            ..Default::default()
        });
    ...
    let tool_ctx = crate::tools::sandboxing::ToolCtx {
        ...
        tool_name: workx_tools::ToolName::plain("probe"),
    };
...
async fn get_base_instructions_no_user_content() {
    let prompt_path = workx_utils_cargo_bin::find_resource!(
...
async fn reload_user_config_layer_updates_effective_apps_config() {
    let (session, _turn_context) = make_session_and_context().await;
    let workx_home = session.workx_home().await;
    std::fs::create_dir_all(&workx_home).expect("create workx home");
    let config_toml_path = workx_home.join(CONFIG_TOML_FILE);
    std::fs::write(
        &config_toml_path,
    ...
    let apps_toml = config
        ...
        .cloned()
        .expect("apps table");
    let apps = workx_config::types::AppsConfigToml::deserialize(apps_toml)
...
async fn reload_user_config_layer_updates_base_and_selected_profile_layers() {
    ...
    let workx_home = session.workx_home().await;
    std::fs::create_dir_all(&workx_home).expect("create workx home");
    let base_config_path = workx_home.join(CONFIG_TOML_FILE);
    let profile_config_path = workx_home.join("work.config.toml");
    std::fs::write(
        &base_config_path,
    ...
    assert_eq!(
        config
            ...
            .map(workx_utils_absolute_path::AbsolutePathBuf::as_path),
...
async fn reload_user_config_layer_refreshes_hooks() -> anyhow::Result<()> {
    let session = make_session_with_config(|config| {
        config
            ...
            .expect("enable Workx hooks");
    })
    .await?;
    let workx_home = session.workx_home().await;
    std::fs::create_dir_all(&workx_home)?;
    let config_toml_path = workx_home.join(CONFIG_TOML_FILE);
    let user_config: workx_config::TomlValue = serde_json::from_value(serde_json::json!({
        "hooks": {
            "SessionStart": [{
                "hooks": [{
                    "type": "command",
    ...
    let request = workx_hooks::SessionStartRequest {
    ...
    let config = session.get_config().await;
    let hook_list = workx_hooks::list_hooks(workx_hooks::HooksConfig {
        feature_enabled: true,
    ...
    assert_eq!(
        ...
        workx_protocol::protocol::HookTrustStatus::Untrusted
    );

    let trusted_user_config: workx_config::TomlValue = serde_json::from_value(serde_json::json!({
        "hooks": {
            "SessionStart": [{
                "hooks": [{
                    "type": "command",
...
async fn refresh_runtime_config_refreshes_hooks() -> anyhow::Result<()> {
    ...
    struct NormalizedHookIdentity {
        ...
        #[serde(flatten)]
        group: workx_config::MatcherGroup,
    }
    let trusted_hash = {
        let identity = NormalizedHookIdentity {
            ...
            group: workx_config::MatcherGroup {
                matcher: None,
                hooks: vec![workx_config::HookHandlerConfig::Command {
        ...
        let identity = workx_config::TomlValue::try_from(identity)?;
        workx_config::version_for_toml(&identity)
    };
...
async fn refresh_runtime_config_updates_runtime_refreshable_fields_and_keeps_session_static_settings()
 {
    ...
    std::fs::create_dir_all(&workx_home).expect("create workx home");
    std::fs::write(
        workx_home.join(CONFIG_TOML_FILE),
        r#"[apps.calendar]
enabled = false
destructive_enabled = false
...
async fn refresh_mcp_config_replaces_managed_server_and_plugin_requirements() {
    ...
    next_config.mcp_servers = workx_config::Constrained::normalized(
...
async fn reconstruct_history_uses_replacement_history_verbatim() {
    ...
    let replacement_history = vec![
        ResponseItemEnvelope {
            ...
            metadata: Some(WorkxHarnessMetadata::default()),
        },
...
async fn subagent_activity_emits_matching_start_and_completion() {
    let (session, turn_context, rx) = make_session_and_context_with_rx().await;
    let item = workx_protocol::items::SubAgentActivityItem {
        id: "activity-1".to_string(),
        kind: workx_protocol::protocol::SubAgentActivityKind::Started,
...
async fn record_inter_agent_communication_preserves_item_id_in_rollout_and_resume() {
    let (mut session, turn_context, _rx) = make_session_and_context_with_auth_and_config_and_rx(
        WorkxAuth::from_api_key("Test API Key"),
        Vec::new(),
        |_| {},
    )
    .await;
...
async fn prepares_resumed_history_before_installing_it() {
    ...
    assert_eq!(
        ...
        Some(WorkxHarnessMetadata::default())
    );
}
...
async fn record_token_usage_info_notifies_extension_contributors() {
    ...
    impl workx_extension_api::TokenUsageContributor for TokenUsageRecorder {
        fn on_token_usage<'a>(
            &'a self,
            session_store: &'a workx_extension_api::ExtensionData,
            thread_store: &'a workx_extension_api::ExtensionData,
            turn_store: &'a workx_extension_api::ExtensionData,
            token_usage: &'a TokenUsageInfo,
        ) -> workx_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                self.records
                    .lock()
...
async fn turn_start_lifecycle_exposes_turn_metadata_and_token_baseline() {
    ...
    impl workx_extension_api::TurnLifecycleContributor for TurnStartRecorder {
        fn on_turn_start<'a>(
            &'a self,
            input: workx_extension_api::TurnStartInput<'a>,
        ) -> workx_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                self.records
                    .lock()
...
async fn turn_error_lifecycle_exposes_error_and_stores() {
    ...
    impl workx_extension_api::TurnLifecycleContributor for TurnErrorRecorder {
        fn on_turn_error<'a>(
            &'a self,
            input: workx_extension_api::TurnErrorInput<'a>,
        ) -> workx_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                self.records
                    .lock()
    ...
    let expected = RecordedTurnError {
        ...
        turn_level_id: turn_context.sub_id.clone(),
        turn_id: turn_context.sub_id.clone(),
        error: WorkxErrorInfo::UsageLimitExceeded,
        saw_session_store: true,
        saw_thread_store: true,
    };

    session
        .emit_turn_error_lifecycle(&turn_context, WorkxErrorInfo::UsageLimitExceeded)
        .await;
...
async fn config_change_contributor_observes_effective_config_changes() {
    ...
    impl workx_extension_api::ConfigContributor<crate::config::Config> for ConfigRecorder {
        fn on_config_changed(
            &self,
            session_store: &workx_extension_api::ExtensionData,
            thread_store: &workx_extension_api::ExtensionData,
...
async fn session_configured_reports_permission_profile_for_external_sandbox() -> anyhow::Result<()>
{
    ...
    let sandbox_policy = SandboxPolicy::ExternalSandbox {
        network_access: workx_protocol::protocol::NetworkAccess::Restricted,
    };
    let permission_profile = PermissionProfile::External {
        network: NetworkSandboxPolicy::Restricted,
    };
    let expected_permission_profile = permission_profile.clone();
    let mut builder = test_workx().with_config(move |config| {
        config
            .permissions
...
async fn fork_startup_context_then_first_turn_diff_snapshot() -> anyhow::Result<()> {
    ...
    let mut builder = test_workx().with_config(|config| {
        config.update_plan_enabled = true;
        config.permissions.approval_policy =
            workx_config::Constrained::allow_any(AskForApproval::OnRequest);
    });
    ...
    initial
        ...
        .await?;
    wait_for_event(&initial.workx, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
    // Forking reads the persisted rollout JSONL, so force the completed source turn to disk
    // before snapshotting from it.
    initial.workx.ensure_rollout_materialized().await;
    initial
        .workx
        .flush_rollout()
        .await
    ...
    fork_config.permissions.approval_policy =
        workx_config::Constrained::allow_any(AskForApproval::UnlessTrusted);
    let forked = initial
        .thread_manager
        .fork_thread(
            usize::MAX,
    ...
    settings.bind(|| {
        insta::assert_snapshot!(
            "workx_core__workx_tests__fork_startup_context_then_first_turn_diff",
            snapshot
        );
    });
...
async fn record_initial_history_forked_hydrates_previous_turn_settings() {
    ...
    let previous_context_item = TurnContextItem {
        ...
        summary: workx_protocol::config_types::ReasoningSummary::Auto,
    };
    ...
    let rollout_items = vec![
        RolloutItem::EventMsg(EventMsg::TurnStarted(
            workx_protocol::protocol::TurnStartedEvent {
        ...
        RolloutItem::EventMsg(EventMsg::UserMessage(
            workx_protocol::protocol::UserMessageEvent {
                client_id: None,
        ...
        RolloutItem::EventMsg(EventMsg::TurnComplete(
            workx_protocol::protocol::TurnCompleteEvent {
                turn_id,
...
async fn thread_rollback_fails_without_persisted_thread_history() {
    ...
    assert_eq!(
        error_event.workx_error_info,
        Some(WorkxErrorInfo::ThreadRollbackFailed)
    );
    assert_eq!(
...
async fn set_rate_limits_retains_previous_credits() {
    let workx_home = tempfile::tempdir().expect("create temp dir");
    let config = build_test_config(workx_home.path()).await;
    ...
    let session_configuration = SessionConfiguration {
        ...
        workx_home: config.workx_home.clone(),
        thread_name: None,
    ...
    let initial = RateLimitSnapshot {
        ...
        plan_type: Some(workx_protocol::account::PlanType::Plus),
        rate_limit_reached_type: None,
    };
    state.set_rate_limits(initial.clone());

    let update = RateLimitSnapshot {
        limit_id: Some("workx_other".to_string()),
        limit_name: Some("workx_other".to_string()),
        primary: Some(RateLimitWindow {
            used_percent: 40.0,
...
async fn set_rate_limits_updates_plan_type_when_present() {
    ...
    let update = RateLimitSnapshot {
        ...
        individual_limit: None,
        spend_control_reached: None,
        plan_type: Some(workx_protocol::account::PlanType::Pro),
        rate_limit_reached_type: None,
    };
    state.set_rate_limits(update.clone());
    ...
    loop {
        ...
        match evt.msg {
            EventMsg::Error(payload)
                if payload.workx_error_info == Some(WorkxErrorInfo::ThreadRollbackFailed) =>
            {
                return payload;
            }
            _ => continue,
...
async fn build_test_config(workx_home: &Path) -> Config {
...
async fn emit_subagent_session_started_includes_fork_lineage_and_originator() {
    ...
    Mock::given(method("POST"))
        .and(path("/workx/analytics-events/events"))
        .respond_with(ResponseTemplate::new(200))
        .mount(&server)
        .await;

    let auth_manager =
        AuthManager::from_auth_for_testing(WorkxAuth::create_dummy_chatgpt_auth_for_testing());
    ...
    emit_subagent_session_started(
        ...
        AppServerClientMetadata {
            client_name: Some("workx-tui".to_string()),
            ...
            if let Some(requests) = server.received_requests().await {
                for request in requests {
                    ...
                    if let Some(event) = payload["events"].as_array().and_then(|events| {
                        events
                            .iter()
                            .find(|event| event["event_type"] == "workx_thread_initialized")
                    }) {
...
async fn session_configuration_apply_client_metadata_preserves_permissions() {
    ...
    let updated = configuration
        .apply(
            &SessionSettingsUpdate {
                app_server_client_name: Some("workx-tui".to_string()),
    ...
    assert_eq!(
        ...
        (Some("workx-tui".to_string()), Some("1.0.0".to_string())),
    );
}
...
async fn session_configuration_apply_permission_profile_preserves_existing_deny_read_entries() {
    ...
    let permission_profile = workx_protocol::models::PermissionProfile::from_runtime_permissions(
...
async fn session_configuration_apply_permission_profile_accepts_direct_write_roots() {
    ...
    let external_write_path = AbsolutePathBuf::from_absolute_path(
        workx_utils_absolute_path::canonicalize_preserving_symlinks(external_write_dir.path())
...
async fn active_profile_update_rebuilds_network_proxy_config() -> std::io::Result<()> {
    let workx_home = tempfile::tempdir().expect("create workx home");
    let cwd = tempfile::tempdir().expect("create cwd");
    ...
    std::fs::write(
        workx_home.path().join(workx_config::CONFIG_TOML_FILE),
...
async fn new_default_turn_uses_config_aware_skills_for_role_overrides() {
    ...
    let workx_home = parent_config.workx_home.clone();
    let skill_dir = workx_home.join("skills").join("demo");
    ...
    let skill_fs = session
        ...
        .unwrap_or_else(|| std::sync::Arc::clone(&workx_exec_server::LOCAL_FS));
    ...
    let role_path = workx_home.join("skills-role.toml");
    std::fs::write(
        &role_path,
        format!(
...
async fn session_new_fails_when_zsh_fork_enabled_without_packaged_zsh() {
    let workx_home = tempfile::tempdir().expect("create temp dir");
    let mut config = build_test_config(workx_home.path()).await;
    config
        .features
    ...
    let auth_manager = AuthManager::from_auth_for_testing(WorkxAuth::from_api_key("Test API Key"));
    let models_manager = models_manager_with_provider(
        config.workx_home.to_path_buf(),
        auth_manager.clone(),
    ...
    let result = Session::new(
        ...
        plugins_manager,
        mcp_manager,
        Arc::new(workx_code_mode::DisabledCodeModeSessionProvider),
        Arc::new(workx_extension_api::ExtensionRegistryBuilder::new().build()),
        workx_extension_api::ExtensionDataInit::default(),
        ...
        Arc::new(workx_thread_store::LocalThreadStore::new(
            workx_thread_store::LocalThreadStoreConfig::from_config(config.as_ref()),
            /*state_db*/ None,
        )),
        workx_rollout_trace::ThreadTraceContext::disabled(),
        /*attestation_provider*/ None,
...
pub(crate) async fn make_session_and_context() -> (Session, TurnContext) {
    ...
    let mcp_runtime = Arc::new(workx_mcp::McpRuntime::empty(config.prefix_mcp_tool_names()));
    ...
    let services = SessionServices {
        ...
        rollout_thread_trace: workx_rollout_trace::ThreadTraceContext::disabled(),
        ...
        plugins_manager,
        mcp_manager,
        extensions: Arc::new(workx_extension_api::ExtensionRegistryBuilder::new().build()),
        session_extension_data: workx_extension_api::ExtensionData::new(
            agent_control.session_id().to_string(),
        ),
        thread_extension_data: workx_extension_api::ExtensionData::new(thread_id.to_string()),
        selected_capability_roots: Vec::new(),
        mcp_thread_init: workx_extension_api::ExtensionDataInit::default(),
        ...
        state_db: None,
        live_thread: None,
        thread_store: Arc::new(workx_thread_store::LocalThreadStore::new(
            workx_thread_store::LocalThreadStoreConfig::from_config(config.as_ref()),
            /*state_db*/ None,
        )),
        attestation_provider: None,
...
async fn load_latest_config_for_session(session: &Session) -> Config {
    let config = session.get_config().await;
    ConfigBuilder::default()
        .workx_home(config.workx_home.to_path_buf())
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    session
        .notify_request_permissions_response(
            "missing",
            workx_protocol::request_permissions::RequestPermissionsResponse {
                permissions: RequestPermissionProfile {
                    network: Some(workx_protocol::models::NetworkPermissions {
                        enabled: Some(true),
                    }),
    ...
    assert_eq!(
        session
            .granted_turn_permissions(workx_exec_server::LOCAL_ENVIRONMENT_ID)
            .await,
        None
    );
}
    ...
    *session.active_turn.lock().await = Some(current_active_turn);
    ...
    session
        .record_granted_request_permissions_for_turn(
            ...
            workx_exec_server::LOCAL_ENVIRONMENT_ID,
    ...
    assert_eq!(
        originating_turn_state
            .lock()
            .await
            .granted_permissions(workx_exec_server::LOCAL_ENVIRONMENT_ID),
...
async fn request_permissions_emits_event_when_granular_policy_allows_requests() {
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    let expected_response = workx_protocol::request_permissions::RequestPermissionsResponse {
        permissions: RequestPermissionProfile {
            network: Some(workx_protocol::models::NetworkPermissions {
                enabled: Some(true),
            }),
        ...
        async move {
            ...
            session
                .request_permissions_for_environment(
                    ...
                    call_id,
                    workx_protocol::request_permissions::RequestPermissionsArgs {
                        environment_id: None,
    ...
    assert_eq!(
        ...
        Some(workx_exec_server::LOCAL_ENVIRONMENT_ID)
    );
    #[allow(deprecated)]
    let turn_cwd = turn_context.cwd.clone();
        ...
        async move {
            handler
                .handle(ToolInvocation {
                    ...
                    tracker,
                    call_id,
                    tool_name: workx_tools::ToolName::plain("request_permissions"),
...
async fn request_permissions_tool_rejects_invalid_requests(
    ...
    Arc::make_mut(&mut turn_context.config)
        .permissions
        .approval_policy = workx_config::Constrained::allow_any(AskForApproval::Never);
...
async fn request_permissions_response_materializes_session_cwd_grants_before_recording() {
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    assert_eq!(
        session
            .granted_session_permissions(workx_exec_server::LOCAL_ENVIRONMENT_ID)
            .await,
...
async fn primary_environment_uses_first_turn_environment() {
    ...
    let second_cwd_uri = workx_utils_path_uri::PathUri::from_abs_path(&second_cwd);
...
async fn spawn_task_turn_span_inherits_dispatch_trace_context() {
    ...
    let submission_context =
        workx_otel::context_from_w3c_trace_context(&submission_trace).expect("submission");
    let task_context = workx_otel::context_from_w3c_trace_context(&task_trace).expect("task trace");
...
async fn shutdown_complete_does_not_append_to_thread_store_after_shutdown() {
    let (mut session, _turn_context) = make_session_and_context().await;
    let store = Arc::new(workx_thread_store::InMemoryThreadStore::default());
    let thread_store: Arc<dyn workx_thread_store::ThreadStore> = store.clone();
    let config = session.get_config().await;
    let live_thread = LiveThread::create(
    ...
    result_sender
        .try_send(
            serde_json::from_value::<workx_protocol::protocol::HookCompletedEvent>(json!({
                "turn_id": "turn-1",
                "run": {
    ...
    assert_eq!(
        workx_thread_store::InMemoryThreadStoreCalls {
            create_thread: 1,
            shutdown_thread: 1,
            ..Default::default()
        },
...
async fn submission_loop_channel_close_runs_full_thread_teardown() {
    ...
    impl workx_extension_api::ThreadLifecycleContributor<crate::config::Config> for ThreadStopRecorder {
        fn on_thread_stop<'a>(
            &'a self,
            input: workx_extension_api::ThreadStopInput<'a>,
        ) -> workx_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                assert_eq!(
...
async fn submission_loop_channel_close_aborts_active_turn_before_thread_stop_lifecycle() {
    ...
    impl workx_extension_api::ThreadLifecycleContributor<crate::config::Config> for LifecycleRecorder {
        fn on_thread_stop<'a>(
            &'a self,
            input: workx_extension_api::ThreadStopInput<'a>,
        ) -> workx_extension_api::ExtensionFuture<'a, ()> {
    ...
    impl workx_extension_api::TurnLifecycleContributor for LifecycleRecorder {
        fn on_turn_abort<'a>(
            &'a self,
            input: workx_extension_api::TurnAbortInput<'a>,
        ) -> workx_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                assert_eq!(
                    self.expected_thread_id.to_string(),
...
{
    let (tx_event, rx_event) = async_channel::unbounded();
    let mut config = build_test_config(workx_home).await;
    configure_config(&mut config);
    let state_db = None;
    let config = Arc::new(config);
    let thread_id = ThreadId::default();
    let auth_manager = AuthManager::from_auth_for_testing_with_home(auth, workx_home.to_path_buf());
    let models_manager = models_manager_with_provider(
        config.workx_home.to_path_buf(),
        auth_manager.clone(),
        config.model_provider.clone(),
...
async fn refresh_mcp_servers_uses_latest_state_for_existing_turns() {
    ...
    .expect("parse refreshed MCP servers");
    {
        ...
        config.mcp_oauth_credentials_store_mode =
            workx_config::types::OAuthCredentialsStoreMode::Auto;
        config
            .features
    ...
    let configured_servers = workx_mcp::configured_mcp_servers(new_step.mcp.config());
    assert_eq!(
        configured_servers.get("refreshed"),
        refreshed_mcp_servers.get("refreshed")
    );
    assert!(
        !workx_mcp::configured_mcp_servers(old_step.mcp.config()).contains_key("refreshed"),
        "an already-bound step must keep its captured config"
    );
    assert!(
        workx_mcp::configured_mcp_servers(rematerialized_old.config()).contains_key("refreshed"),
    ...
    assert!(
        workx_mcp::configured_mcp_servers(current.config()).contains_key("refreshed"),
...
async fn mcp_elicitation_reviewer_uses_active_reviewer_and_latest_runtime_policy(
    ...
    if live_update {
        assert_eq!(
            session
                .apply_turn_settings(
                    &old_turn.sub_id,
                    workx_protocol::protocol::TurnSettingsUpdate {
                        approvals_reviewer: Some(ApprovalsReviewer::AutoReview),
                        ..Default::default()
                    },
                )
                .await,
            workx_protocol::protocol::TurnSettingsUpdateOutcome::Applied
        );
    } else {
        session
            .update_settings(SessionSettingsUpdate {
    ...
    let request = workx_mcp::ElicitationReviewRequest {
        server_name: "browser-use".to_string(),
        request_id: rmcp::model::NumberOrString::Number(7),
        elicitation: workx_rmcp_client::Elicitation::Mcp(
            rmcp::model::ElicitRequestParams::FormElicitationParams {
                meta: Some(rmcp::model::RequestMetaObject::from(
                    serde_json::Map::from_iter([
                        ("workx_approval_kind".to_string(), json!("mcp_tool_call")),
                        ("workx_request_type".to_string(), json!("approval_request")),
...
async fn mcp_refresh_detects_shared_auth_manager_changes() {
    ...
    assert_eq!(
        ...
        Some(workx_protocol::auth::AuthMode::ApiKey)
    );
    session.refresh_mcp_if_dirty().await;
    assert!(
        session
...
async fn conflicting_ready_environment_root_ids_keep_first_location() {
    let (session, turn_context) = make_session_and_context().await;
    let selected_root =
        |environment_id: &str, path: &str| workx_protocol::capabilities::SelectedCapabilityRoot {
            id: "shared-root".to_string(),
            location: workx_protocol::capabilities::CapabilityRootLocation::Environment {
    ...
    for selected_root in &selected_roots {
        let workx_protocol::capabilities::CapabilityRootLocation::Environment {
            environment_id,
            ..
        ...
        turn_environments.push(TurnEnvironment::new(
            ...
            Arc::new(
                workx_exec_server::Environment::create_for_tests(/*exec_server_url*/ None)
...
async fn step_context_keeps_its_mcp_runtime_for_tools() -> anyhow::Result<()> {
    ...
    let next_step = session
        ...
        .await
        .expect("a fresh cancellation token cannot be cancelled");
    assert!(workx_mcp::configured_mcp_servers(next_step.mcp.config()).contains_key("newer"));
    ...
    let current = session
        ...
        .await
        .expect("refreshed runtime should be available");
    assert!(workx_mcp::configured_mcp_servers(current.config()).contains_key("newer"));
...
async fn build_initial_context_reuses_in_flight_recommendation_prewarm() {
    ...
    Mock::given(method("GET"))
        .and(path("/ps/plugins/suggested/workx"))
...
impl workx_extension_api::ContextContributor for PromptExtensionTestContributor {
    fn contribute_thread_context<'a>(
        &'a self,
        _session_store: &'a workx_extension_api::ExtensionData,
        thread_store: &'a workx_extension_api::ExtensionData,
    ) -> std::pin::Pin<
        Box<dyn std::future::Future<Output = Vec<workx_extension_api::PromptFragment>> + Send + 'a>,
    > {
        Box::pin(async move {
            thread_store
                ...
                .is_some()
                .then(|| {
                    workx_extension_api::PromptFragment::developer_policy(
                        "prompt extension enabled",
                        workx_extension_api::ContentItemKind("test.prompt_extension".to_string()),
                    )
                })
                .into_iter()
                .collect()
...
-> Arc<workx_extension_api::ExtensionRegistry<crate::config::Config>> {
    let mut builder = workx_extension_api::ExtensionRegistryBuilder::new();
    builder.prompt_contributor(Arc::new(PromptExtensionTestContributor));
    Arc::new(builder.build())
}

impl workx_extension_api::ContextContributor for TurnContextExtensionTestContributor {
    fn contribute_turn_context<'a>(
        &'a self,
        input: workx_extension_api::TurnContextContributionInput<'a>,
    ) -> std::pin::Pin<
        Box<dyn std::future::Future<Output = Vec<workx_extension_api::PromptFragment>> + Send + 'a>,
    > {
        Box::pin(async move {
            ...
            .then(|| {
                workx_extension_api::PromptFragment::developer_policy(
                    ...
                    workx_extension_api::ContentItemKind("test.turn_context".to_string()),
                )
            })
            .into_iter()
            .collect()
...
async fn attach_in_memory_thread_store(
    session: &mut Session,
) -> Arc<workx_thread_store::InMemoryThreadStore> {
    let store = Arc::new(workx_thread_store::InMemoryThreadStore::default());
    let thread_store: Arc<dyn workx_thread_store::ThreadStore> = store.clone();
    let config = session.get_config().await;
    let live_thread = LiveThread::create(
...
async fn wait_for_flush_count(
    store: &workx_thread_store::InMemoryThreadStore,
...
async fn guardian_auto_review_emits_thread_idle_after_interrupt() {
    ...
    impl workx_extension_api::ThreadLifecycleContributor<crate::config::Config> for ThreadIdleRecorder {
        fn on_thread_idle<'a>(
            &'a self,
            _input: workx_extension_api::ThreadIdleInput<'a>,
        ) -> workx_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
...
async fn task_finish_emits_turn_item_lifecycle_for_leftover_pending_user_input() {
    ...
    let text_element = workx_protocol::user_input::TextElement::new(
        workx_protocol::user_input::ByteRange { start: 5, end: 12 },
...
async fn task_finish_emits_thread_idle_lifecycle_after_active_turn_clears() {
    ...
    impl workx_extension_api::ThreadLifecycleContributor<crate::config::Config> for ThreadIdleRecorder {
        fn on_thread_idle<'a>(
            &'a self,
            input: workx_extension_api::ThreadIdleInput<'a>,
        ) -> workx_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                assert_eq!(
                    self.expected_thread_id.to_string(),
...
async fn thread_idle_lifecycle_waits_for_trigger_turn_mailbox_work() {
    ...
    session
        .emit_thread_idle_lifecycle_if_idle(workx_extension_api::ThreadIdleCause::Completed)
        .await;
...
async fn active_turn_keeps_first_root_when_mail_coalesces() {
    ...
    for (communication, parent_turn_id, root_turn_id) in [
        ...
        sess.input_queue
            .enqueue_mailbox_communication(
                communication,
                workx_protocol::turn_input::TurnStartOptions {
...
async fn tool_calls_reopen_mailbox_delivery_for_current_turn() {
    ...
    let mut ctx = HandleOutputCtx {
        ...
        turn_store: Arc::new(workx_extension_api::ExtensionData::new(tc.sub_id.clone())),
...
async fn unified_exec_rejects_escalated_permissions_when_policy_not_on_request() {
    ...
    let resp = handler
        .handle(ToolInvocation {
            ...
            tool_name: workx_tools::ToolName::plain("exec_command"),
...
async fn session_start_hooks_only_load_from_trusted_project_layers() -> std::io::Result<()> {
    ...
    let workx_home = temp.path().join("home");
    let project_root = temp.path().join("project");
    let nested = project_root.join("nested");
    let root_dot_workx = project_root.join(".workx");
    let nested_dot_workx = nested.join(".workx");

    std::fs::create_dir_all(&workx_home)?;
    std::fs::create_dir_all(&nested_dot_workx)?;
    std::fs::write(project_root.join(".git"), "gitdir: here")?;
    write_project_hooks(&root_dot_workx)?;
    write_project_hooks(&nested_dot_workx)?;
    write_project_trust_config(&workx_home, &[(&nested, TrustLevel::Trusted)]).await?;

    let config = ConfigBuilder::default()
        .workx_home(workx_home)
    ...
    let expected_source_path = workx_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
        nested_dot_workx.join("hooks.json"),
    )?;
    assert_eq!(
        hook_list
            .hooks
...
async fn session_start_hooks_require_project_trust_without_config_toml() -> std::io::Result<()> {
    ...
    let project_root = temp.path().join("project");
    let nested = project_root.join("nested");
    let dot_workx = project_root.join(".workx");
    std::fs::create_dir_all(&nested)?;
    std::fs::write(project_root.join(".git"), "gitdir: here")?;
    write_project_hooks(&dot_workx)?;
    ...
    for (name, trust_entries, expected_hooks) in cases {
        let workx_home = temp.path().join(format!("home_{name}"));
        std::fs::create_dir_all(&workx_home)?;
        write_project_trust_config(&workx_home, &trust_entries).await?;

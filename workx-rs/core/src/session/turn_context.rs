...
use workx_core_plugins::ResolvedPluginMetricsOperation;
use workx_core_plugins::TrustedPluginRoots;
...
use workx_protocol::permissions::RawFileSystemSandboxPolicy;
...
use workx_sandboxing::policy_transforms::effective_permission_profile;
...
pub struct TurnContext {
    ...
    pub(crate) extension_data: Arc<workx_extension_api::ExtensionData>,
...
impl TurnContext {
    ...
    pub(crate) fn apps_enabled(&self) -> bool {
        ...
        self.config
            .features
            .apps_enabled_for_auth(uses_workx_backend)
...
impl Session {
    ...
    pub(crate) fn make_turn_context(
        ...
        let extension_data = Arc::new(workx_extension_api::ExtensionData::new(sub_id.clone()));
    ...
    ) -> WorkxResult<(Arc<TurnContext>, ThreadSettingsSnapshot)> {
    ...
    pub(super) async fn new_turn_with_sub_id_if(
    ...
    ) -> WorkxResult<Option<(Arc<TurnContext>, ThreadSettingsSnapshot)>> {
        ...
        let commit = match self.update_settings_if(updates, should_start).await {
            ...
            Err(error) => {
                let message = WorkxErr::InvalidRequest(error.to_string()).to_string();
                ...
                .await;
                return Err(WorkxErr::InvalidRequest(message));
            }
        };
    ...
    async fn new_turn_context_from_configuration(
        ...
        let trusted_plugin_roots = TrustedPluginRoots::from_plugin_load_outcome(
            &plugin_outcome,
            per_turn_config.workx_home.as_path(),
        );
        ...
        if turn_context.config.model_provider_id == workx_model_provider_info::OPENAI_PROVIDER_ID {
    ...
    pub(crate) async fn maybe_emit_model_warnings_for_turn(&self, tc: &TurnContext) {
        ...
        if !tc.code_mode_available
            && matches!(
                ...
                workx_protocol::openai_models::ToolMode::CodeMode
                    | workx_protocol::openai_models::ToolMode::CodeModeOnly
            )

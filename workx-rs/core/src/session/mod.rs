...
use workx_analytics::ImagePreparationFact;
use workx_analytics::ImagePreparationMetadata;
use workx_analytics::SubAgentThreadStartedInput;
use workx_analytics::TurnWorkxErrorFact;
use workx_async_utils::OrCancelExt;
use workx_connectors::connector_runtime_context_key;
use workx_context_fragments::RenderedFragment;
use workx_exec_server::Environment;
use workx_exec_server::EnvironmentManager;
use workx_execpolicy::prefix_rule_migration;
use workx_extension_api::ConversationHistorySnapshot;
use workx_extension_api::ExtensionDataInit;
use workx_extension_api::LoadedUserInstructions;
use workx_extension_api::PromptSlot;
use workx_extension_api::TurnContextContributionInput;
use workx_features::FEATURES;
use workx_features::Feature;
use workx_features::unstable_features_warning_event;
use workx_history::RolloutItem;
use workx_hooks::Hooks;
use workx_hooks::HooksConfig;
use workx_login::AuthManager;
use workx_login::WorkxAuth;
use workx_login::auth_env_telemetry::collect_auth_env_telemetry;
use workx_mcp::McpResourceClient;
use workx_mcp::McpRuntime;
use workx_mcp::McpRuntimeContext;
use workx_mcp::McpRuntimeInput;
use workx_models_manager::manager::RefreshStrategy;
use workx_models_manager::manager::SharedModelsManager;
use workx_network_proxy::NetworkProxy;
use workx_network_proxy::NetworkProxyAuditMetadata;
use workx_network_proxy::normalize_host;
use workx_otel::current_span_trace_id;
use workx_otel::current_span_w3c_trace_context;
use workx_otel::set_parent_from_w3c_trace_context;
use workx_protocol::ResponseUsageMetadata;
use workx_protocol::SessionId;
use workx_protocol::ThreadId;
use workx_protocol::approvals::ElicitationRequest;
use workx_protocol::approvals::ElicitationRequestEvent;
use workx_protocol::approvals::ExecPolicyAmendment;
use workx_protocol::approvals::NetworkPolicyAmendment;
use workx_protocol::approvals::NetworkPolicyRuleAction;
...
use workx_protocol::dynamic_tools::DynamicToolResponse;
use workx_protocol::dynamic_tools::DynamicToolSpec;
use workx_protocol::items::EnteredReviewModeItem;
use workx_protocol::items::SubAgentActivityItem;
use workx_protocol::items::TurnItem;
use workx_protocol::items::UserMessageItem;
use workx_protocol::models::ActivePermissionProfile;
use workx_protocol::models::AdditionalPermissionProfile;
...
use workx_protocol::protocol::FileChange;
use workx_protocol::protocol::HasLegacyEvent;
...
use workx_protocol::protocol::MULTI_AGENT_MODE_OPEN_TAG;
use workx_protocol::protocol::MultiAgentVersion;
use workx_protocol::protocol::RawResponseItemEvent;
...
use workx_protocol::protocol::ThreadHistoryMode;
...
use workx_protocol::protocol::TurnAbortReason;
use workx_protocol::protocol::TurnContextItem;
use workx_protocol::protocol::TurnContextNetworkItem;
use workx_protocol::protocol::TurnEnvironmentSelection;
use workx_protocol::protocol::W3cTraceContext;
use workx_protocol::protocol::WorldStateItem;
...
use workx_protocol::request_permissions::RequestPermissionsArgs;
use workx_protocol::request_permissions::RequestPermissionsEvent;
use workx_protocol::request_permissions::RequestPermissionsResponse;
use workx_protocol::request_user_input::RequestUserInputArgs;
use workx_protocol::request_user_input::RequestUserInputResponse;
use workx_rmcp_client::ElicitationResponse;
use workx_rollout::state_db;
use workx_rollout_trace::AgentResultTracePayload;
use workx_rollout_trace::ThreadStartedTraceMetadata;
use workx_rollout_trace::ThreadTraceContext;
use workx_sandboxing::policy_transforms::intersect_permission_profiles_with_context;
use workx_shell_command::parse_command::parse_command;
use workx_terminal_detection::user_agent;
use workx_thread_store::CreateThreadParams;
use workx_thread_store::LiveThread;
use workx_thread_store::LiveThreadInitGuard;
use workx_thread_store::LocalThreadStore;
use workx_thread_store::PersistContext;
use workx_thread_store::ReadThreadParams;
use workx_thread_store::ResumeThreadParams;
use workx_thread_store::ThreadPersistenceMetadata;
use workx_thread_store::ThreadStore;
use workx_utils_audio::prepare_response_items as prepare_audio_response_items;
use workx_utils_git_discovery::GitRootDiscovery;
...
use crate::workx_thread::ThreadConfigSnapshot;
use crate::workx_thread::WorkxThreadSettingsOverrides;
...
#[cfg(test)]
use workx_protocol::exec_output::StreamOutput;
...
use workx_core_plugins::PluginCommandAttribution;
use workx_core_plugins::PluginsManager;
use workx_core_plugins::RecommendedPluginCandidatesInput;
use workx_git_utils::get_git_repo_root;
...
use workx_otel::THREAD_STARTED_METRIC;
use workx_otel::TelemetryAuthMode;
use workx_protocol::ResponseItemId;
use workx_protocol::approvals::ExecApprovalKind;
...
use workx_protocol::config_types::ReasoningSummary as ReasoningSummaryConfig;
use workx_protocol::config_types::Settings;
use workx_protocol::config_types::WindowsSandboxLevel;
use workx_protocol::mcp::ClientMcpExtensions;
use workx_protocol::models::LocalImagePreparation;
use workx_protocol::models::ResponseInputItem;
use workx_protocol::models::ResponseItem;
use workx_protocol::openai_models::ReasoningEffort as ReasoningEffortConfig;
use workx_protocol::protocol::ApplyPatchApprovalRequestEvent;
use workx_protocol::protocol::AskForApproval;
use workx_protocol::protocol::DeprecationNoticeEvent;
...
use workx_protocol::protocol::ModelRerouteEvent;
use workx_protocol::protocol::ModelRerouteReason;
use workx_protocol::protocol::ModelVerification;
use workx_protocol::protocol::ModelVerificationEvent;
use workx_protocol::protocol::NetworkApprovalContext;
use workx_protocol::protocol::Op;
use workx_protocol::protocol::RateLimitSnapshot;
use workx_protocol::protocol::RawResponseCompletedEvent;
use workx_protocol::protocol::RequestUserInputEvent;
...
use workx_protocol::protocol::SessionNetworkProxyRuntime;
use workx_protocol::protocol::StreamErrorEvent;
use workx_protocol::protocol::Submission;
use workx_protocol::protocol::ThreadMemoryMode;
...
use workx_protocol::protocol::TurnModerationMetadataEvent;
use workx_protocol::protocol::WarningEvent;
...
use workx_protocol::turn_input::TurnStartOptions;
use workx_protocol::user_input::UserInput;
use workx_skills_extension::HostSkillsService;
use workx_tools::ToolName;
use workx_tools::UnifiedExecShellMode;
use workx_utils_absolute_path::AbsolutePathBuf;
#[cfg(test)]
use workx_utils_stream_parser::ProposedPlanSegment;
...
pub(crate) struct SessionSpawnArgs {
    ...
    pub(crate) code_mode_session_provider: Arc<dyn workx_code_mode::CodeModeSessionProvider>,
    pub(crate) extensions: Arc<workx_extension_api::ExtensionRegistry<crate::config::Config>>,
    ...
    pub(crate) windows_sandbox_proxy_settings_mode:
        workx_sandboxing::WindowsSandboxProxySettingsMode,
}
...
impl Session {
    ...
    ) -> BoxFuture<'static, WorkxResult<(Arc<Self>, SessionIo)>> {
        Box::pin(async move {
            let parent_trace = match args.parent_trace {
                Some(trace) => {
                    if workx_otel::context_from_w3c_trace_context(&trace).is_some() {
                        Some(trace)
                    } else {
    ...
    async fn spawn_internal(args: SessionSpawnArgs) -> WorkxResult<(Arc<Self>, SessionIo)> {
        ...
        let exec_policy = if crate::guardian::is_basic_session_source(&session_source) {
            let managed_policy = config
                ...
                .exec_policy
                .as_deref()
                .map_or_else(workx_execpolicy::Policy::empty, |policy| {
                    policy.as_ref().clone()
                });
            ...
            {
                let workx_home = config.workx_home.clone();
                let policy_path = default_policy_path(workx_home.as_path());
            ...
            Arc::new(
                ExecPolicyManager::load(&config.config_layer_stack)
                    .await
                    .map_err(|err| WorkxErr::Fatal(format!("failed to load rules: {err}")))?,
            )
        };

        let mut config = Arc::new(config);
        let refresh_strategy = if session_source.is_non_root_agent() {
            workx_models_manager::manager::RefreshStrategy::Offline
        } else {
        ...
        .map_err(|e| {
            ...
            map_session_init_error(&e, &config.workx_home)
        })?;
...
impl SessionIo {
    ...
    pub(crate) async fn submit(&self, op: Op) -> WorkxResult<String> {
    ...
    pub(crate) async fn submit_with_id(&self, mut sub: Submission) -> WorkxResult<()> {
        ...
        self.tx_sub
            .send(sub)
            .await
            .map_err(|_| WorkxErr::InternalAgentDied)?;
        Ok(())
    }
    ...
    pub(crate) async fn submit_turn_input(
        ...
        mode: TurnInputMode,
    ) -> WorkxResult<TurnInputSubmission> {
        let id = new_submission_id();
        ...
        .await?;
        reply_rx.await.unwrap_or(Err(WorkxErr::InternalAgentDied))
    }
    ...
    pub(crate) async fn shutdown_and_wait(&self) -> WorkxResult<()> {
        let session_loop_termination = self.session_loop_termination.clone();
        match self.submit(Op::Shutdown).await {
            Ok(_) => {}
            Err(err) if matches!(err.details(), WorkxErrorDetails::InternalAgentDied) => {}
    ...
    pub(crate) async fn next_event(&self) -> WorkxResult<Event> {
        let event = self
            .rx_event
            .recv()
            .await
...
fn session_permission_profile_state_from_config(
    config: &Config,
) -> WorkxResult<PermissionProfileState> {
...
impl Session {
    ...
    async fn start_managed_network_proxy(
        ...
        exec_policy: &workx_execpolicy::Policy,
        permission_profile: &PermissionProfile,
        network_policy_decider: Option<Arc<dyn workx_network_proxy::NetworkPolicyDecider>>,
        blocked_request_observer: Option<Arc<dyn workx_network_proxy::BlockedRequestObserver>>,
    ...
    #[cfg(test)]
    pub(crate) async fn workx_home(&self) -> AbsolutePathBuf {
        let state = self.state.lock().await;
        state.session_configuration.workx_home().clone()
    }
                    ...
                    .filter(|model| *model != curr)
                {
                    ...
                    self.send_event(
                        ...
                        EventMsg::Warning(WarningEvent {
                         ...
                         Consider switching back to `{prev}` as it may affect Workx performance."
                            ),
                        }),
                    )
                    .await;
    ...
    pub(crate) async fn restorable_thread_settings(&self) -> WorkxThreadSettingsOverrides {
        let state = self.state.lock().await;
        state
            .session_configuration
    ...
    pub(crate) async fn user_instructions(&self) -> Option<workx_extension_api::Instructions> {
    ...
    /// Record a terminal WorkxErr before the app-server completion notification is reduced.
    pub(crate) fn track_turn_workx_error(&self, turn_context: &TurnContext, error: &WorkxErr) {
        self.services
            .analytics_events_client
            .track_turn_workx_error(TurnWorkxErrorFact::from_workx_err(
    ...
    pub(crate) async fn send_event(&self, turn_context: &TurnContext, msg: EventMsg) {
        ...
        if let EventMsg::Error(error) = &legacy_source
            && error
                .workx_error_info
                .as_ref()
                .is_some_and(WorkxErrorInfo::affects_turn_status)
        {
            turn_context
                .terminal_error
                .lock()
        ...
        self.services
            ...
            .record_workx_turn_event(&turn_context.sub_id, &legacy_source);
        self.services
            .rollout_thread_trace
    ...
    async fn forward_child_completion_to_parent(
        ...
        child_agent_path: &workx_protocol::AgentPath,
        status: AgentStatus,
    ) {
        let Some(parent_agent_path) = child_agent_path
            .as_str()
            .rsplit_once('/')
            .and_then(|(parent, _)| workx_protocol::AgentPath::try_from(parent).ok())
        else {
            return;
        };
    ...
    pub(crate) async fn send_event_raw_without_materializing_rollout(&self, event: Event) {
        let persist = match self.current_rollout_path().await {
            Ok(Some(path)) => workx_rollout::existing_rollout_path(&path).await.is_some(),
            Ok(None) => true,
            Err(err) => {
    ...
    pub(crate) async fn persist_execpolicy_amendment(
        ...
        self.services
            .exec_policy
            .append_amendment_and_update(&workx_home, amendment)
            .await?;
    ...
    pub(crate) async fn request_permissions_for_environment(
        ...
        let Ok(native_cwd) = cwd.to_abs_path() else {
            warn!(
                cwd = %cwd,
                "request_permissions interactive approval requires a cwd native to the Workx host"
            );
            return Some(RequestPermissionsResponse {
                permissions: RequestPermissionProfile::default(),
                scope: PermissionGrantScope::Turn,
    ...
    async fn capture_step_context_inner(
        ...
        let executor_capability_discovery = self
            ...
            .await?;
        let extension_data = workx_extension_api::ExtensionData::new(turn_context.sub_id.clone());
        ...
        let mut selected_plugins = self
            ...
            .get::<workx_extension_api::SelectedPluginSnapshot>()
    ...
    pub(crate) async fn replace_compacted_history(
        ...
        {
            let mut state = self.state.lock().await;
            state.queue_pending_session_start_source(workx_hooks::SessionStartSource::Compact);
        }
    }
            ...
            && turn_context.model_context_window().is_some()
        {
            ...
            separate_developer_sections.push(
                crate::context::TokenBudgetContext::new(
                    session_source
                        .get_agent_path()
                        .unwrap_or_else(workx_protocol::AgentPath::root),
    ...
    pub(crate) async fn notify_stream_error(
        ...
        let additional_details = workx_error.to_string();
        let workx_error_info = WorkxErrorInfo::ResponseStreamDisconnected {
            http_status_code: workx_error.http_status_code_value(),
        };
    ...
    pub(crate) async fn take_pending_session_start_source(
        &self,
    ) -> Option<workx_hooks::SessionStartSource> {
        let mut state = self.state.lock().await;
...
async fn build_hooks_config(
    ...
    HooksConfig {
        ...
        feature_enabled: config.features.enabled(Feature::WorkxHooks),

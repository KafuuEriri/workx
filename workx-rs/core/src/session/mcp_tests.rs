...
fn plugin_install_elicitation_telemetry_metadata_requires_install_tool_suggestion() {
    let event = EventMsg::ElicitationRequest(ElicitationRequestEvent {
        ...
        id: workx_protocol::mcp::RequestId::String("request-1".to_string()),
        request: workx_protocol::approvals::ElicitationRequest::Form {
            meta: Some(json!({
    ...
    let enable_event = EventMsg::ElicitationRequest(ElicitationRequestEvent {
        turn_id: Some("turn-1".to_string()),
        server_name: "workx_apps".to_string(),
        id: workx_protocol::mcp::RequestId::String("request-2".to_string()),
        request: workx_protocol::approvals::ElicitationRequest::Form {
            meta: Some(json!({
                "workx_approval_kind": "tool_suggestion",
                "suggest_type": "enable",
...
fn guardian_decisions_map_to_elicitation_responses_without_session_state() {
    let model = workx_models_manager::model_info::model_info_from_slug("acting-model");
    assert_eq!(

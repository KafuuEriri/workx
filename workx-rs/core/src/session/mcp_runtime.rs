...
//! inputs and the mutable [`workx_mcp::McpRuntime`]. Background scheduling
//! belongs elsewhere.
...
pub(super) struct McpDesiredState {
    ...
    pub(super) auth: Option<WorkxAuth>,
...
impl Session {
        ...
        Box::pin(async move {
            ...
            for selected in environments.turn_environments() {
                ...
                for (name, mut server) in servers {
                    ...
                    self.services.session_telemetry.counter(
                        "workx.mcp.executor_discovery.server",
                        /*inc*/ 1,
                        &[("server_name", name.as_str()), ("outcome", outcome)],
                    );
                    if name == WORKX_APPS_MCP_SERVER_NAME
                    ...
                    catalog.build_with_environment_authority(|environment_id| {
                        let Some(selection) = selections
                            ...
                            return if environment_id
                                == workx_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID
                            {
    ...
    pub(super) fn build_mcp_runtime_input(
        ...
        config
            .environment_cwds
            .entry(workx_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID.to_string())
        ...
        McpRuntimeInput {
            ...
            runtime_context,
            workx_apps_tools_cache: self.services.mcp_manager.workx_apps_tools_cache(),

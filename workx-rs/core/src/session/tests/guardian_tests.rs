...
use core_test_support::workx_linux_sandbox_exe_or_skip;
...
use workx_network_proxy::NetworkDecision;
use workx_network_proxy::NetworkPolicyRequest;
use workx_network_proxy::NetworkProtocol;
use workx_protocol::config_types::ApprovalsReviewer;
use workx_protocol::error::SandboxErr;
use workx_protocol::models::AdditionalPermissionProfile as PermissionProfile;
    ...
    let (current_turn, _) = session
        .new_turn_with_sub_id(
            ...
            SessionSettingsUpdate {
                ...
                permission_profile: Some(workx_protocol::models::PermissionProfile::Disabled),
                ..Default::default()
            },
...
) -> workx_protocol::protocol::ExecApprovalRequestEvent {
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    timeout(Duration::from_secs(5), async {
        loop {
            ...
            if matches!(
                event.msg,
                workx_protocol::protocol::EventMsg::GuardianAssessment(_)
            ) {
                break;
            }
        }
...
async fn guardian_allows_exec_command_additional_permissions_requests_past_policy_validation() {
    ...
    config
        .permissions
        .set_permission_profile(workx_protocol::models::PermissionProfile::Disabled)
    ...
    config.workx_linux_sandbox_exe = workx_linux_sandbox_exe_or_skip!();
    config
        .features
        .enable(Feature::GuardianApproval)
        .expect("test setup should allow enabling guardian approvals");
...
async fn strict_auto_review_turn_grant_forces_guardian_for_exec_command_policy_skip() {
    ...
    *session.active_turn.lock().await = Some(active_turn);
    ...
    config
        .permissions
        .set_permission_profile(workx_protocol::models::PermissionProfile::External {
...
async fn sandbox_denied_retry_uses_the_action_policy_and_reviewer() {
    ...
    impl ToolRuntime<TurnEnvironment, String> for DeniedOnceRuntime {
        ...
        async fn run(
            ...
            self.attempts += 1;
            if self.attempts == 1 {
                return Err(ToolError::Workx(WorkxErr::Sandbox(SandboxErr::Denied {
...
async fn guardian_subagent_does_not_inherit_parent_exec_policy_rules() {
    let workx_home = tempdir().expect("create workx home");
    ...
    config.config_layer_stack = ConfigLayerStack::new(
        vec![ConfigLayerEntry::new(
            ConfigLayerSource::Project {
                dot_workx_folder: project_dir.path().abs(),
            },
    ...
    let thread_store = Arc::new(workx_thread_store::LocalThreadStore::new(
        workx_thread_store::LocalThreadStoreConfig::from_config(&config),
        /*state_db*/ None,
    ));
    ...
    let (session, io) = Session::spawn(SessionSpawnArgs {
        ...
        plugins_manager,
        mcp_manager,
        code_mode_session_provider: Arc::new(workx_code_mode::DisabledCodeModeSessionProvider),
        extensions: workx_extension_api::empty_extension_registry(),
        ...
        parent_rollout_thread_trace: workx_rollout_trace::ThreadTraceContext::disabled(),
        user_shell_override: None,
        parent_trace: None,
        environment_selections: Vec::new(),
        thread_extension_init: workx_extension_api::ExtensionDataInit::default(),

...
impl PreparedTurnInputSettings {
    ...
    async fn prepare(
        ...
        let thread_settings_update = if thread_settings == ThreadSettingsOverrides::default() {
            ...
            session
                ...
                .await
                .map_err(|error| WorkxErr::InvalidRequest(error.to_string()))?;
            Some(updates)
        };
        Ok(Self {
    ...
    async fn apply_started(
        ...
        kind: TurnStartKind,
    ) -> WorkxResult<Option<Arc<TurnContext>>> {
    ...
    async fn apply_steered(self, session: &Session, submission_id: String) -> WorkxResult<()> {

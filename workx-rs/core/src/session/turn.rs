...
use crate::responses_metadata::WorkxResponsesRequestKind;
...
use workx_analytics::TurnResolvedConfigFact;
...
use workx_extension_api::TurnInputContext;
use workx_extension_api::TurnInputEnvironment;
use workx_features::Feature;
use workx_file_system::FindUpErrorPolicy;
use workx_file_system::find_nearest_ancestor_with_markers;
use workx_login::WorkxAuth;
use workx_model_provider::RemoteCompactionSupport;
...
use workx_protocol::items::PlanItem;
...
use workx_protocol::protocol::AgentMessageContentDeltaEvent;
use workx_protocol::protocol::AgentReasoningSectionBreakEvent;
...
use workx_protocol::protocol::PlanDeltaEvent;
use workx_protocol::protocol::ReasoningContentDeltaEvent;
use workx_protocol::protocol::ReasoningRawContentDeltaEvent;
use workx_protocol::protocol::SafetyBufferingEvent;
...
use workx_skills::ToolMentionKind;
use workx_skills::app_id_from_path;
use workx_skills::build_skill_name_counts;
use workx_skills::collect_explicit_skill_mentions;
use workx_skills::tool_kind_for_path;
use workx_skills_extension::HostSkillPrompts;
use workx_skills_extension::InjectedHostSkillPrompts;
use workx_thread_store::PersistContext;
use workx_tools::DiscoverableTool;
use workx_tools::ToolName;
use workx_tools::filter_request_plugin_install_discoverable_tools_for_client;
use workx_utils_path_uri::PathUri;
use workx_utils_stream_parser::AssistantTextChunk;
use workx_utils_stream_parser::AssistantTextStreamParser;
use workx_utils_stream_parser::ProposedPlanSegment;
use workx_utils_stream_parser::extract_proposed_plan_text;
use workx_utils_stream_parser::strip_citations;

const POST_SAMPLING_TOKEN_ESTIMATE_TARGET: &str = "workx_core::post_sampling_token_estimate";
...
pub(crate) async fn run_turn(
    ...
    .await
    {
        if matches!(err.details(), WorkxErrorDetails::TurnAborted) {
            run_hooks_and_record_inputs(&sess, &turn_context, &input, PersistContext::Standard)
                .await;
            return Err(err);
        }
        if matches!(err.details(), WorkxErrorDetails::ToolCollision(_)) {
            return Err(err);
        }
        let error = err.to_workx_protocol_error();
        ...
        .await
    {
        ...
        Err(err) if matches!(err.details(), WorkxErrorDetails::TurnAborted) => {
            run_hooks_and_record_inputs(&sess, &turn_context, &input, PersistContext::Standard)
                .await;
            return Err(err);
        }
    ...
    // Although from the perspective of workx.rs, TurnDiffTracker has the lifecycle of a Task which contains
    ...
    loop {
        ...
        let sampling_request_result: WorkxResult<_> = async {
            ...
            let responses_metadata = sess
                .responses_metadata(turn_context.as_ref(), WorkxResponsesRequestKind::Turn)
                .await;
        ...
        match sampling_request_result {
            ...
            Err(workx_error)
                if matches!(
                    workx_error.details(),
                    WorkxErrorDetails::InvalidImageRequest()
                ) =>
            {
                sess.track_turn_workx_error(turn_context.as_ref(), &workx_error);
                let error = WorkxErrorInfo::BadRequest;
                sess.emit_turn_error_lifecycle(turn_context.as_ref(), error.clone())
                    .await;
            ...
            Err(e) => {
                info!("Turn error: {e:#}");
                let error = e.to_workx_protocol_error();
                sess.emit_turn_error_lifecycle(turn_context.as_ref(), error.clone())
                    .await;
                sess.track_turn_workx_error(turn_context.as_ref(), &e);
...
async fn required_mcp_servers_for_input(
    ...
    let connector_slug_counts = if turn_context.apps_enabled() && !mentions.plain_names.is_empty() {
        ...
        build_connector_slug_counts(
            &workx_connectors::merge::merge_plugin_connectors_with_accessible(
                connector_ids,
...
async fn build_skills_and_plugins(
    ...
    let available_connectors = if turn_context.apps_enabled() {
        let connectors = workx_connectors::merge::merge_plugin_connectors_with_accessible(
            connector_snapshot
                .connector_ids()
                .iter()
                .map(|connector_id| connector_id.0.clone()),
    ...
    sess.services
        ...
        .track_turn_resolved_config(TurnResolvedConfigFact {
            ...
            guardian_v2_enabled: sess
                ...
                .get::<workx_extension_api::GuardianV2Enabled>()
                .is_some_and(|state| {
...
///
/// Returns `None` when the active authentication does not use the Workx backend, the provider is
...
async fn capture_current_model_fallback_step_context(
    ...
    previous_model: &str,
    cancellation_token: &CancellationToken,
) -> WorkxResult<Option<Arc<StepContext>>> {
    let uses_workx_backend = turn_context
        .auth_manager
        .as_deref()
        .is_some_and(workx_login::AuthManager::current_auth_uses_workx_backend);
...
pub(super) fn collect_explicit_app_ids_from_skill_items(
    ...
    for connector in connectors {
        let slug = workx_connectors::metadata::connector_mention_slug(connector);
...
async fn run_sampling_request(
    sess: Arc<Session>,
    step_context: Arc<StepContext>,
    turn_store: Arc<workx_extension_api::ExtensionData>,
    turn_diff_tracker: SharedTurnDiffTracker,
    client_session: &mut ModelClientSession,
    responses_metadata: &WorkxResponsesMetadata,
    input: Vec<ResponseItem>,
    cancellation_token: CancellationToken,
) -> WorkxResult<(SamplingRequestResult, Vec<ResponseItem>)> {
    ...
    loop {
        ...
        {
            workx_protocol::models::bound_executed_tool_calls_for_prompt(&mut prompt_input);
        }
        ...
        .await
        {
            ...
            Err(err) => match err.details() {
                ...
                WorkxErrorDetails::UsageLimitReached(e) => {
...
pub(crate) async fn built_tools(
    sess: &Session,
    turn_context: &TurnContext,
    model_info: &workx_protocol::openai_models::ModelInfo,
    model_messages: Option<&ModelMessages>,
    environments: &TurnEnvironmentSnapshot,
    mcp: &Arc<workx_mcp::McpBinding>,
    step_store: &ExtensionData,
    prepared_recommendations: PreparedToolRecommendations,
) -> WorkxResult<Arc<ToolRouter>> {
...
pub(super) fn agent_message_text(item: &workx_protocol::items::AgentMessageItem) -> String {
    item.content
        .iter()
        .map(|entry| match entry {
            workx_protocol::items::AgentMessageContent::Text { text } => text.as_str(),
        })
        .collect()
}
...
async fn emit_agent_message_in_plan_mode(
    sess: &Session,
    turn_context: &TurnContext,
    agent_message: workx_protocol::items::AgentMessageItem,
    state: &mut PlanModeStreamState,
) {
        ...
        .contains(&agent_message_id)
    {
        let start_item = state
            ...
            .unwrap_or_else(|| {
                TurnItem::AgentMessage(workx_protocol::items::AgentMessageItem {
...
async fn handle_assistant_item_done_in_plan_mode(
    sess: &Session,
    turn_context: &TurnContext,
    turn_store: &workx_extension_api::ExtensionData,
    item: &ResponseItem,
    state: &mut PlanModeStreamState,
    previously_active_item: Option<&TurnItem>,
...
async fn try_run_sampling_request(
    ...
    prompt: &Prompt,
    cancellation_token: CancellationToken,
) -> WorkxResult<SamplingRequestResult> {
    let turn_context = Arc::clone(&step_context.turn);
    feedback_tags!(
    ...
    let outcome: WorkxResult<SamplingRequestResult> = loop {
        let handle_responses = trace_span!(
            ...
            from = field::Empty,
            workx.request.reasoning_effort = %reasoning_effort,
            gen_ai.usage.input_tokens = field::Empty,
            gen_ai.usage.cache_read.input_tokens = field::Empty,
            gen_ai.usage.cache_write.input_tokens = field::Empty,
            gen_ai.usage.output_tokens = field::Empty,
            workx.usage.reasoning_output_tokens = field::Empty,
            workx.usage.total_tokens = field::Empty,
        );
        ...
        {
            Ok(event) => event,
            Err(workx_async_utils::CancelErr::Cancelled) => {
                break Err(WorkxErr::TurnAborted);
            }
        };
                ...
                if analytics_tool_call_ids.len() < MAX_ANALYTICS_TOOL_CALL_IDS_PER_RESPONSE {
                    let call_id = match &item {
                        ...
                        | ResponseItem::ImageGenerationCall { id, .. } => {
                            id.as_ref().map(workx_protocol::ResponseItemId::as_str)
                        }
                        _ => None,
                    };
                        ...
                        && let Some(raw_text) = raw_assistant_output_text_from_item(&item)
                    {
                        ...
                        if let TurnItem::AgentMessage(agent_message) = &mut turn_item {
                            agent_message.content =
                                vec![workx_protocol::items::AgentMessageContent::Text {
                                    text: if plan_mode {
                                        String::new()
                                    } else {
            ...
            ResponseEvent::Completed {
                ...
                sess.services
                    ...
                    .track_code_mode_tool_call(
                        workx_analytics::CodeModeToolCallFact::SamplingResponseCompleted {
                            thread_id: sess.thread_id.to_string(),
                            turn_id: turn_context.sub_id.clone(),
    ...
    if cancellation_token.is_cancelled() {
        return Err(WorkxErr::TurnAborted);
    }

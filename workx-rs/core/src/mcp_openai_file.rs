//! Bridges Apps SDK-style `openai/fileParams` metadata into Workx's MCP flow.
//!
//! Strategy:
...
//!
//! The model-facing local-path schema is owned by `workx-mcp` alongside MCP tool inventory, so this
...
use workx_api::OPENAI_FILE_UPLOAD_LIMIT_BYTES;
use workx_api::upload_openai_file;
...
async fn build_uploaded_argument_value(
    ...
    let Some(auth) = auth else {
        return Err("ChatGPT auth is required to upload files for Workx Apps tools".to_string());
    };
    ...
    let upload_auth = workx_model_provider::auth_provider_from_auth(auth);

...
async fn reserved_thread_id_is_used_without_changing_normal_id_generation() {
    ...
    config.workx_home = temp_dir.path().join("workx-home").abs();
    config.cwd = config.workx_home.abs();
    std::fs::create_dir_all(&config.workx_home).expect("create workx home");
    ...
    assert!(matches!(
        ...
        workx_protocol::error::WorkxErrorDetails::InvalidRequest(message)
...
impl workx_agent_graph_store::AgentGraphStore for FakeAgentGraphStore {
    fn upsert_thread_spawn_edge(
        &self,
        _parent_thread_id: ThreadId,
        _child_thread_id: ThreadId,
        _status: workx_agent_graph_store::ThreadSpawnEdgeStatus,
    ) -> workx_agent_graph_store::AgentGraphStoreFuture<'_, ()> {
    ...
    fn list_thread_spawn_children(
        &self,
        _parent_thread_id: ThreadId,
        _status_filter: Option<workx_agent_graph_store::ThreadSpawnEdgeStatus>,
    ) -> workx_agent_graph_store::AgentGraphStoreFuture<'_, Vec<ThreadId>> {
    ...
    fn list_thread_spawn_descendants(
        &self,
        root_thread_id: ThreadId,
        status_filter: Option<workx_agent_graph_store::ThreadSpawnEdgeStatus>,
    ) -> workx_agent_graph_store::AgentGraphStoreFuture<'_, Vec<ThreadId>> {
...
async fn mcp_invalidation_refreshes_threads_that_are_still_starting() {
    ...
    impl workx_extension_api::ThreadLifecycleContributor<Config> for BlockingThreadStartup {
        fn on_thread_start<'a>(
            &'a self,
            _input: workx_extension_api::ThreadStartInput<'a, Config>,
    ...
    impl workx_extension_api::McpServerContributor<Config> for BlockingThreadStartup {
        ...
        fn contribute<'a>(
            &'a self,
            _context: workx_extension_api::McpServerContributionContext<'a, Config>,
        ) -> workx_extension_api::ExtensionFuture<'a, Vec<workx_extension_api::McpServerContribution>>
        {
            Box::pin(async move {
    ...
    let auth_manager = AuthManager::from_auth_for_testing(WorkxAuth::from_api_key("dummy"));
    let manager = Arc::new(ThreadManager::new(
        &config,
        Arc::clone(&auth_manager),
        build_models_manager(&config, auth_manager),
        crate::WorkxAppsToolsCache::default(),
        SessionSource::Exec,
        Arc::new(workx_exec_server::EnvironmentManager::default_for_tests()),
...
async fn spawn_internal_session_preserves_parent_lineage_without_forking_history() {
    ...
    impl workx_extension_api::ThreadLifecycleContributor<Config> for ParentLifecycleContributor {}

    impl workx_extension_api::McpServerContributor<Config> for ParentLifecycleContributor {
        fn id(&self) -> &'static str {
        ...
        fn contribute<'a>(
            &'a self,
            context: workx_extension_api::McpServerContributionContext<'a, Config>,
        ) -> workx_extension_api::ExtensionFuture<'a, Vec<workx_extension_api::McpServerContribution>>
        {
            Box::pin(async move {
    ...
    struct ParentInstructionsProvider(workx_extension_api::Instructions);

    impl workx_extension_api::UserInstructionsProvider for ParentInstructionsProvider {
        fn load_user_instructions(&self) -> workx_extension_api::LoadUserInstructionsFuture<'_> {
            Box::pin(async move {
                workx_extension_api::LoadedUserInstructions {
    ...
    let mut managed_exec_policy = workx_execpolicy::Policy::empty();
    managed_exec_policy
        .add_prefix_rule(&["rm".to_string()], workx_execpolicy::Decision::Forbidden)
        .expect("add managed execution restriction");
    let mut requirements = config.config_layer_stack.requirements().clone();
    requirements.exec_policy = Some(workx_config::Sourced::new(
        workx_execpolicy::RequirementsExecPolicy::new(managed_exec_policy),
        workx_config::RequirementSource::Unknown,
    ));
    requirements.additional_developer_instructions = Some(workx_config::Sourced::new(
    ...
    config.config_layer_stack = workx_config::ConfigLayerStack::new(
        config
            .config_layer_stack
    ...
    let parent_instructions = workx_extension_api::Instructions {
        text: "parent user instructions must not be inherited".to_string(),
        source: config.workx_home.join("AGENTS.md"),
    };
    ...
    manager_state.mcp_manager = Arc::new(McpManager::new_with_extensions(
        ...
        manager_state.mcp_manager.workx_apps_tools_cache(),
    ));
    manager_state.user_instructions_provider =
        Arc::new(ParentInstructionsProvider(parent_instructions.clone()));
    let parent = manager
        .start_thread(StartThreadOptions {
            metrics_service_name: Some("workx_work_desktop".to_string()),
    ...
    reviewer_environment.config =
        EnvironmentConfigState::Ready(workx_protocol::protocol::EnvironmentConfig {
            ...
            exec_policy: Some(workx_execpolicy::RequirementsExecPolicy::new(
                workx_execpolicy::Policy::empty(),
            )),
            mcp_policy: None,
            network_policy: None,
    ...
    assert_eq!(reviewer_config.originator, "workx_work_desktop");
    assert_eq!(
...
async fn start_thread_seeds_extension_data_for_mcp_and_lifecycle_contributors() {
    ...
    impl workx_extension_api::ThreadLifecycleContributor<Config> for InitialDataRecorder {
        fn on_thread_start<'a>(
            &'a self,
            input: workx_extension_api::ThreadStartInput<'a, Config>,
        ) -> workx_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
    ...
    impl workx_extension_api::McpServerContributor<Config> for InitialDataRecorder {
        fn id(&self) -> &'static str {
            "selected_root_test"
        }
        ...
        ) -> workx_extension_api::ExtensionFuture<'a, Vec<workx_extension_api::McpServerContribution>>
        {
            Box::pin(async move {
                ...
                let mut server = workx_mcp::workx_apps_mcp_server_config(
                ...
                vec![workx_extension_api::McpServerContribution::SelectedPlugin {
    ...
    let selected_root_init = |id: &str, environment_id: &str| {
        let mut init = workx_extension_api::ExtensionDataInit::new();
    ...
    let selected_servers = |config: &workx_mcp::McpConfig| {
        workx_mcp::configured_mcp_servers(config)
            .into_iter()
    ...
    let workx_apps_server = workx_mcp::configured_mcp_servers(&first_resolved.config)
        .remove(workx_mcp::WORKX_APPS_MCP_SERVER_NAME)
        .expect("Workx Apps server should be configured");
    let workx_apps_headers = match workx_apps_server.transport {
        workx_config::McpServerTransportConfig::StreamableHttp { http_headers, .. } => http_headers,
        workx_config::McpServerTransportConfig::Stdio { .. } => {
            panic!("Workx Apps server should use streamable HTTP")
        }
    };
    assert_eq!(
        workx_apps_headers
            .expect("Workx Apps headers should be configured")
            .get("originator"),
        Some(&"workx_work_desktop".to_string())
    );
}
...
#[tokio::test]
async fn explicit_installation_id_skips_workx_home_file() {
    let temp_dir = tempdir().expect("tempdir");
    let mut config = test_config().await;
    config.workx_home = temp_dir.path().join("workx-home").abs();
    config.cwd = config.workx_home.abs();
    ...
    assert!(!config.workx_home.join(INSTALLATION_ID_FILENAME).exists());
...
async fn injected_models_manager_controls_refresh_policy() {
    ...
    assert!(!config.workx_home.join("models_cache.json").exists());
}

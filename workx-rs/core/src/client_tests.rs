...
use super::X_WORKX_INSTALLATION_ID_HEADER;
use super::X_WORKX_PARENT_THREAD_ID_HEADER;
use super::X_WORKX_TURN_METADATA_HEADER;
use super::X_WORKX_WINDOW_ID_HEADER;
use crate::AttestationContext;
use crate::AttestationProvider;
use crate::GenerateAttestationFuture;
use crate::responses_metadata::WorkxResponsesMetadata;
use crate::test_support::TestWorkxResponsesRequestKind;
...
use workx_login::AuthCredentialsStoreMode;
...
use workx_model_provider::BearerAuthProvider;
use workx_model_provider::ModelProvider;
use workx_model_provider::ModelProviderFuture;
use workx_model_provider::ProviderAccountResult;
use workx_model_provider::ProviderAuthRecoveryMessages;
...
use workx_model_provider_info::CHATGPT_WORKX_BASE_URL;
use workx_model_provider_info::ModelProviderInfo;
use workx_model_provider_info::WireApi;
use workx_model_provider_info::create_oss_provider_with_base_url;
...
use workx_rollout_trace::ExecutionStatus;
use workx_rollout_trace::InferenceTraceAttempt;
use workx_rollout_trace::InferenceTraceContext;
use workx_rollout_trace::RawTraceEventPayload;
use workx_rollout_trace::RolloutTrace;
use workx_rollout_trace::TraceWriter;
...
async fn compact_uses_bearer_after_agent_identity_session_fallback() -> anyhow::Result<()> {
    ...
    let auth_manager = chatgpt_auth_manager(&workx_home, server.uri()).await;
    ...
    let output = client
        .compact_conversation_history(
            ...
            CompactConversationRequestSettings {
                effort: None,
                summary: workx_protocol::config_types::ReasoningSummary::None,
                service_tier: None,
            },
...
fn test_responses_metadata_for_client(
    ...
    request_kind: TestWorkxResponsesRequestKind,
) -> WorkxResponsesMetadata {
...
fn responses_lite_prefix_ids_track_thread_and_payload() -> anyhow::Result<()> {
    ...
    let build = |client: &ModelClient, prompt: &Prompt| {
        client.build_responses_request(
            ...
            &model,
            /*effort*/ None,
            workx_protocol::config_types::ReasoningSummary::None,
            /*service_tier*/ None,
    ...
    prompt.tools = vec![workx_tools::ToolSpec::Freeform(workx_tools::FreeformTool {
        name: "exec".to_string(),
        description: "Execute JavaScript.".to_string(),
        defer_loading: None,
        format: workx_tools::FreeformToolFormat {
...
fn write_chatgpt_auth_json(workx_home: &std::path::Path) {
    let auth_json = json!({
        "tokens": {
...
async fn chatgpt_auth_manager(
    ...
    write_chatgpt_auth_json(workx_home.path());
...
fn started_inference_attempt(temp: &TempDir) -> anyhow::Result<InferenceTraceAttempt> {
    ...
    writer.append(RawTraceEventPayload::WorkxTurnStarted {
        workx_turn_id: "turn-1".to_string(),
        thread_id: "thread-root".to_string(),
    })?;
...
fn output_message(id: &str, text: &str) -> ResponseItem {
    ResponseItem::Message {
        id: Some(workx_protocol::ResponseItemId::with_suffix("msg", id)),
...
fn build_ws_client_metadata_includes_window_lineage_and_turn_metadata() {
    ...
    for (client_key, metadata_key, expected) in [
        ...
        (
            X_WORKX_WINDOW_ID_HEADER,
            "window_id",
            expected_window_id.as_str(),
        ),
        (
            X_WORKX_PARENT_THREAD_ID_HEADER,
            "parent_thread_id",
...
async fn bedrock_unauthorized_error_uses_provider_mapping() {
    ...
    assert_eq!(
        error.to_string(),
        format!(
            "Amazon Bedrock rejected the request because its AWS signature has expired. Refresh your AWS credentials and retry. If `AWS_BEARER_TOKEN_BEDROCK` is set, update or unset it, then restart Workx, url: {url}"
        )
    );
}
...
impl ModelProvider for TestRecoveryProvider {
    ...
    fn auth(&self) -> ModelProviderFuture<'_, Option<WorkxAuth>> {
        self.inner.auth()
    }
    ...
    fn recover_from_unauthorized(
        &self,
    ) -> ModelProviderFuture<'_, workx_protocol::error::Result<ProviderUnauthorizedRecovery>> {
        self.attempts.fetch_add(1, Ordering::Relaxed);
        Box::pin(async move {
            if self.should_fail {
                Err(WorkxErr::Io(std::io::Error::other(
    ...
    fn models_manager(
        ...
        self.inner.models_manager(workx_home, config_model_catalog)
    }
}
...
async fn provider_owned_auth_recovery_is_bounded_and_preserves_unauthorized_failures() {
    for should_fail in [false, true] {
        ...
        match error.details() {
            WorkxErrorDetails::UnexpectedStatus(response) => {
...
fn model_client_with_counting_attestation(
    ...
    let (auth_manager, provider) = if include_attestation {
        (
            ...
            ModelProviderInfo::create_openai_provider(Some(CHATGPT_WORKX_BASE_URL.to_string())),
        )
    } else {
        (
            None,
...
#[test]
fn guardian_reviewer_uses_dedicated_endpoint_only_with_workx_backend_auth() {
    ...
    assert_eq!(
        model_client.responses_endpoint(
            Some(&WorkxAuth::from_api_key("test-api-key")),
            "codex-auto-review",
        ),
        ResponsesEndpoint::Responses
    );
...
#[tokio::test]
async fn websocket_handshake_includes_attestation_for_chatgpt_workx_responses() {
    ...
    let responses_metadata = test_responses_metadata_for_client(
        ...
        /*parent_thread_id*/ None,
        TestWorkxResponsesRequestKind::WebsocketConnection,
    );
...
#[tokio::test]
async fn non_chatgpt_workx_endpoints_omit_attestation_generation() {
    let (model_client, attestation_calls) =

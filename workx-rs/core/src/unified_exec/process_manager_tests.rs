...
fn unified_exec_env_injects_defaults() {
    ...
    let expected = HashMap::from([
        ...
        ("WORKX_CI".to_string(), "1".to_string()),
    ]);
...
fn env_overlay_for_exec_server_keeps_runtime_changes_only() {
    ...
    let request_env = HashMap::from([
        ...
        ("SHELL_SET".to_string(), "policy".to_string()),
        ("WORKX_THREAD_ID".to_string(), "thread-1".to_string()),
        (
            WORKX_PERMISSION_PROFILE_ENV_VAR.to_string(),
            "current-profile".to_string(),
        ),
        ...
        (
            "WORKX_SANDBOX_NETWORK_DISABLED".to_string(),
            "1".to_string(),
        ),
    ]);
...
fn exec_env_policy_excludes_non_inheritable_and_runtime_variables() {
    let policy = ShellEnvironmentPolicy {
        r#set: HashMap::from([
            (
                "workx_permission_profile".to_string(),
            ...
            (
                "workx_apply_patch_preserve_line_endings".to_string(),
                "1".to_string(),
            ),
            (
                "workx_plugin_metrics_output".to_string(),
...
fn exec_server_params_use_path_uri_and_env_policy_overlay_contract() {
    let cwd: workx_utils_absolute_path::AbsolutePathBuf = std::env::current_dir()
        .expect("current dir")
        .try_into()
        .expect("absolute path");
    let permission_profile = workx_protocol::models::PermissionProfile::Disabled;
    ...
    let mut request = ExecRequest {
        ...
        env: HashMap::from([
            ...
            ("WORKX_NETWORK_PROXY_ACTIVE".to_string(), "1".to_string()),
            (
        ...
        exec_server_env_config: Some(ExecServerEnvConfig {
            policy: workx_exec_server::ExecEnvPolicy {
                inherit: workx_protocol::config_types::ShellEnvironmentPolicyInherit::Core,
        ...
        sandbox: workx_sandboxing::SandboxType::None,
    ...
    let proxy_settings_mode = workx_sandboxing::WindowsSandboxProxySettingsMode::Preserve;
    ...
    request.exec_server_shell_snapshot = Some(workx_exec_server::ShellSnapshotRequest {
        scope_id: "attachment-1".to_string(),
        shell: workx_exec_server::ShellInfo {
            name: "bash".to_string(),
    ...
    request.exec_server_sandbox = Some(
        workx_exec_server::FileSystemSandboxContext::from_permission_profile(permission_profile),
    );
    ...
    assert_eq!(
        ...
        Some(workx_sandboxing::WindowsSandboxProxySettingsMode::Preserve)
    );
...
async fn failed_initial_end_for_unstored_process_uses_fallback_output() {
    ...
    let request = ExecCommandRequest {
        ...
        shell_mode: workx_tools::UnifiedExecShellMode::Direct,
        network: None,
        tty: true,
    ...
    let workx_protocol::protocol::EventMsg::ItemCompleted(completed_event) = event.msg else {
        panic!("expected ItemCompleted event");
    };
    let workx_protocol::items::TurnItem::CommandExecution(item) = completed_event.item else {
        panic!("expected CommandExecution item");
    };
    assert_eq!(item.id, "call-unified-denied");
    assert_eq!(
        item.status,
        workx_protocol::items::CommandExecutionStatus::Failed
    );
...
async fn pruning_does_not_evict_live_process_while_exited_process_is_finalizing() {
    ...
    let exited_process = Arc::new(
        crate::unified_exec::process_tests::remote_process(
            workx_exec_server::WriteStatus::Accepted,
            /*terminate_error*/ None,
            workx_sandboxing::SandboxType::None,
        )
        .await,
    );
    exited_process

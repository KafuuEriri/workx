...
use crate::workx_thread::BackgroundTerminalInfo;
use workx_core_plugins::PLUGIN_METRICS_OUTPUT_ENV_VAR;
use workx_core_plugins::PluginCommandAttribution;
use workx_core_plugins::PluginMetricsSidecar;
use workx_core_plugins::strip_output_env;
use workx_network_proxy::NetworkPolicyDecider;
...
use workx_protocol::protocol::TerminalInteractionEvent;
use workx_protocol::shell_environment::is_non_inheritable_env_var;
use workx_sandboxing::SandboxCommand;
use workx_shell_command::is_dangerous_command::DangerousCommandPlatform;
...
const NETWORK_ACCESS_DENIED_MESSAGE: &str =
    "Network access was denied by the Workx sandbox network proxy.";
...
pub(super) fn exec_env_policy_from_shell_policy(
...
) -> workx_exec_server::ExecEnvPolicy {
    ...
    exclude.extend([
        ...
        workx_apply_patch::WORKX_APPLY_PATCH_PRESERVE_LINE_ENDINGS_ENV_VAR.to_string(),
    ...
    r#set.retain(|key, _| {
        ![
            ...
            workx_apply_patch::WORKX_APPLY_PATCH_PRESERVE_LINE_ENDINGS_ENV_VAR,
...
fn exec_server_env_for_request(
    ...
    Option<workx_exec_server::ExecEnvPolicy>,
    HashMap<String, String>,
) {
...
fn network_approval_error_message(err: ToolError) -> String {
    match err {
        ...
        ToolError::Workx(err) => err.to_string(),
    }
}
...
impl UnifiedExecProcessManager {
    ...
    pub(crate) async fn write_stdin(
        ...
        if let Some((approval, approval_reason)) = approval {
            ...
            context.step_context.session_telemetry.counter(
                "workx.unified_exec.stdin_review.size_check",
                /*inc*/ 1,
    ...
    pub(crate) async fn open_session_with_exec_env(
        ...
        network_proxy_launch: Option<workx_network_proxy::RemoteNetworkProxyLaunchConfig>,
        environment_id: Option<&str>,
        exec_server_env_config: Option<ExecServerEnvConfig>,
        shell_snapshot: Option<workx_exec_server::ShellSnapshotRequest>,
        windows_sandbox_proxy_settings_mode: workx_sandboxing::WindowsSandboxProxySettingsMode,
        tty: bool,
        ...
        .map_err(|err| match err {
            UnifiedExecError::SandboxDenied { output, .. } => {
                ToolError::Workx(WorkxErr::Sandbox(SandboxErr::Denied {
    ...
    pub(crate) async fn open_session_with_prepared_exec_env(
        ...
        let network_proxy_restricting_sid = {
            ...
            {
                if request.sandbox == workx_sandboxing::SandboxType::WindowsRestrictedToken {
                    request
                        .network
                        .as_ref()
                        .map(|network| {
        ...
        let windows_sandbox =
            if request.sandbox == workx_sandboxing::SandboxType::WindowsRestrictedToken {
                Some(workx_sandboxing::WindowsSandboxSpawnRequest {
        ...
        let spawn_result = workx_sandboxing::spawn_process(workx_sandboxing::SpawnRequest {
    ...
    pub(super) async fn open_session_with_sandbox(
        ...
        env.insert(
            WORKX_THREAD_ID_ENV_VAR.to_string(),
        ...
        orchestrator
            ...
            .map_err(|err| match err {
                ToolError::Workx(err) => match err.details() {

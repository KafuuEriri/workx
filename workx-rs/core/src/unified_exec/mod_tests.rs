...
use core_test_support::test_workx::test_env as remote_test_env;
...
use workx_exec_server::ExecProcessEventReceiver;
use workx_exec_server::ExecProcessFuture;
...
use workx_exec_server::WriteResponse;
...
impl BlockingTerminateExecProcess {
    async fn read(&self) -> Result<ReadResponse, workx_exec_server::ExecServerError> {
        Ok(ReadResponse {
            chunks: Vec::new(),
            next_seq: 1,
            exited: false,
    ...
    async fn write(&self) -> Result<WriteResponse, workx_exec_server::ExecServerError> {
        Ok(WriteResponse {
            status: WriteStatus::Accepted,
        })
    }

    async fn terminate(&self) -> Result<(), workx_exec_server::ExecServerError> {
...
async fn blocking_terminate_unified_process(
    ...
    Ok(Arc::new(
        UnifiedExecProcess::from_exec_server_started(StartedExecProcess {
            ...
            sandbox_type: Some(workx_sandboxing::SandboxType::None),
        })
        .await?,
    ))
}
...
async fn unified_exec_persists_across_requests() -> anyhow::Result<()> {
    ...
    write_stdin(
        ...
        &turn,
        process_id,
        "export WORKX_INTERACTIVE_SHELL_VAR=workx\n",
        /*yield_time_ms*/ 2_500,
    )
    .await?;
    ...
    let out_2 = write_stdin(
        ...
        &turn,
        process_id,
        "echo $WORKX_INTERACTIVE_SHELL_VAR\n",
        /*yield_time_ms*/ 2_500,
    )
    .await?;
    assert!(
...
async fn unified_exec_timeouts() -> anyhow::Result<()> {
    ...
    write_stdin(
        ...
        &turn,
        process_id,
        format!("export WORKX_INTERACTIVE_SHELL_VAR={TEST_VAR_VALUE}\n").as_str(),
        /*yield_time_ms*/ 2_500,
    )
    .await?;
    ...
    let out_2 = write_stdin(
        ...
        &turn,
        process_id,
        "sleep 5 && echo $WORKX_INTERACTIVE_SHELL_VAR\n",
        /*yield_time_ms*/ 10,
    )
    .await?;
    assert!(
...
async fn completed_pipe_commands_preserve_exit_code() -> anyhow::Result<()> {
    ...
    let environment = workx_exec_server::Environment::default_for_tests();
...
async fn stdin_approval_preserves_the_reviewed_terminal() -> anyhow::Result<()> {
    ...
    let (session, mut turn, events) = make_session_and_context_with_auth_and_config_and_rx(
        workx_login::WorkxAuth::from_api_key("Test API Key"),
        Vec::new(),
        |config| {

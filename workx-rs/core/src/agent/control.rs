...
pub(crate) struct AgentControl {
    ...
    /// `ThreadManagerState -> WorkxThread -> Session -> SessionServices -> ThreadManagerState`.
...
impl AgentControl {
    ...
    pub(crate) async fn send_input(
            ...
            .await
        {
            ...
            Ok(TurnInputSubmission::NotSubmitted { reason }) => Err(WorkxErr::InvalidRequest(
    ...
    pub(crate) async fn interrupt_agent(&self, agent_id: ThreadId) -> WorkxResult<String> {
        let state = self.upgrade()?;
    ...
    async fn handle_thread_request_result(
        ...
        agent_id: ThreadId,
        state: &Arc<ThreadManagerState>,
        result: WorkxResult<String>,
    ) -> WorkxResult<String> {
        if result
            .as_ref()
            .is_err_and(|err| matches!(err.details(), WorkxErrorDetails::InternalAgentDied))
        {
    ...
    pub(crate) fn ensure_agent_known(&self, agent_id: ThreadId) -> WorkxResult<AgentMetadata> {
        self.state
            .agent_metadata_for_thread(agent_id)
            .ok_or_else(|| WorkxErr::ThreadNotFound(agent_id))
    }
    ...
    pub(crate) async fn subscribe_status(
        &self,
        agent_id: ThreadId,
    ) -> WorkxResult<watch::Receiver<AgentStatus>> {
        let state = self.upgrade()?;
        let thread = state.get_thread(agent_id).await?;
    ...
    fn prepare_thread_spawn(
        ...
        agent_role: Option<String>,
        preferred_agent_nickname: Option<String>,
    ) -> WorkxResult<(SessionSource, AgentMetadata)> {
        if depth == 1 {
    ...
    fn upgrade(&self) -> WorkxResult<Arc<ThreadManagerState>> {
        self.manager
            .upgrade()
    ...
    async fn open_thread_spawn_children(
        &self,
        parent_thread_id: ThreadId,
    ) -> WorkxResult<Vec<(ThreadId, AgentMetadata)>> {
    ...
    async fn live_thread_spawn_children(
        &self,
    ) -> WorkxResult<HashMap<ThreadId, Vec<(ThreadId, AgentMetadata)>>> {
        let state = self.upgrade()?;
    ...
    async fn persist_thread_spawn_edge_for_source(
        &self,
        child_thread: &crate::WorkxThread,
        child_thread_id: ThreadId,
        session_source: Option<&SessionSource>,
    ) {
        ...
        if let Err(err) = agent_graph_store
            .upsert_thread_spawn_edge(
                parent_thread_id,
                child_thread_id,
                workx_agent_graph_store::ThreadSpawnEdgeStatus::Open,
            )
            .await
        {

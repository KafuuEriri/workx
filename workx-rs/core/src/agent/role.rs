...
use workx_agent_roles::parse_agent_role_file_contents;
use workx_config::ConfigLayerEntry;
use workx_config::ConfigLayerSource;
use workx_config::ConfigLayerStack;
use workx_config::SkillsConfig;
use workx_config::loader::resolve_relative_paths_in_config_toml;
use workx_exec_server::read_sensitive_file_to_string;
...
async fn apply_role_to_config_inner(
    ...
    let role_config = deserialize_config_toml_with_base(role_layer_toml, &config.workx_home)?;
...
async fn load_role_layer_toml(
    ...
    let (role_config_toml, role_config_base) = if is_built_in {
        ...
        (role_config_toml, config.workx_home.as_path())
    } else {

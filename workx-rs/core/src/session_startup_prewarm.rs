...
use workx_otel::STARTUP_PREWARM_AGE_AT_FIRST_TURN_METRIC;
use workx_otel::STARTUP_PREWARM_DURATION_METRIC;
...
pub(crate) struct SessionStartupPrewarmHandle {
    task: AbortOnDropHandle<WorkxResult<ModelClientSession>>,
    started_at: Instant,
    timeout: Duration,
}
...
impl SessionStartupPrewarmHandle {
    pub(crate) fn new(
        task: JoinHandle<WorkxResult<ModelClientSession>>,
        started_at: Instant,
        timeout: Duration,
    ) -> Self {
        Self {
    ...
    fn resolution_from_join_result(
        result: std::result::Result<WorkxResult<ModelClientSession>, tokio::task::JoinError>,
        started_at: Instant,
    ) -> SessionStartupPrewarmResolution {
        match result {
...
async fn schedule_startup_prewarm_inner(
...
) -> WorkxResult<ModelClientSession> {
    ...
    let responses_metadata = session
        .responses_metadata(&startup_turn_context, WorkxResponsesRequestKind::Prewarm)
        .await;

...
use workx_mcp::McpBinding;
use workx_mcp::ToolInfo as McpToolInfo;
...
impl McpHandlerCache {
    pub(crate) fn append_mcp_tools(
        ...
        config: &Config,
        apps_enabled: bool,
        mcp_server_catalog: &workx_mcp::ResolvedMcpCatalog,
...
fn append_mcp_tools(
    ...
    let non_app_tools = filter_non_workx_apps_mcp_tools_only(all_mcp_tools);
    let app_tools = apps_enabled
        .then(|| filter_workx_apps_mcp_tools(all_mcp_tools, config))
        .into_iter()
        .flatten();
...
fn filter_non_workx_apps_mcp_tools_only(
    mcp_tools: &[McpToolInfo],
) -> impl Iterator<Item = &McpToolInfo> + '_ {
    mcp_tools.iter().filter(|tool| {
        tool.server_name != WORKX_APPS_MCP_SERVER_NAME && tool_is_model_visible(tool)
    })
}

fn filter_workx_apps_mcp_tools<'a>(

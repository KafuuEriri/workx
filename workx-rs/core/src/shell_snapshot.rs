...
use workx_shell_command::shell_snapshot::snapshot_script;
...
impl ShellSnapshot {
    ...
    async fn build_for_cwd(
        ...
        async {
            let timer = config
                .session_telemetry
                .start_timer("workx.shell_snapshot.duration_ms", &[("version", "v1")]);
            ...
            config
                .session_telemetry
                .counter("workx.shell_snapshot", /*inc*/ 1, &counter_tags);
    ...
    async fn try_create(
        ...
        let temp_path = workx_home
            .join(SNAPSHOT_DIR)
            .join(format!("{session_id}.tmp-{nonce}"));

        // Clean the (unlikely) leaked snapshot files.
        let workx_home = workx_home.clone();
        let cleanup_session_id = session_id;
        tokio::spawn(async move {
            if let Err(err) =
                cleanup_stale_snapshots(&workx_home, cleanup_session_id, state_db).await
            {
...
async fn run_script_with_timeout(
    ...
    workx_protocol::shell_environment::scrub_non_inheritable_env_vars(handler.as_std_mut());
    handler.args(&args[1..]);
...
pub async fn cleanup_stale_snapshots(
    ...
    let snapshot_dir = workx_home.join(SNAPSHOT_DIR);
    ...
    while let Some(entry) = entries.next_entry().await? {
        ...
        let rollout_path =
            find_thread_path_by_id_str(workx_home, session_id, state_db.as_deref()).await?;

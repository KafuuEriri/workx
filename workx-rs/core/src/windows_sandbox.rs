...
use workx_otel::sanitize_metric_tag_value;
...
pub fn sandbox_setup_is_complete(workx_home: &Path) -> bool {
    workx_windows_sandbox::sandbox_setup_is_complete(workx_home)
}

#[cfg(not(target_os = "windows"))]
pub fn sandbox_setup_is_complete(_workx_home: &Path) -> bool {
    false
}
...
pub fn run_elevated_setup(
    ...
    let permissions =
        workx_windows_sandbox::ResolvedWindowsSandboxPermissions::try_from_permission_profile_for_workspace_roots(
            permission_profile,
            workspace_roots,
        )?;
    workx_windows_sandbox::run_elevated_setup(
        workx_windows_sandbox::SandboxSetupRequest {
        ...
        workx_windows_sandbox::SetupRootOverrides::default(),
    )
}
...
) -> std::io::Result<workx_windows_sandbox::WindowsSandboxProvisioningSettings> {
    let Some(network) = network.filter(|network| network.enabled()) else {
        return Ok(workx_windows_sandbox::WindowsSandboxProvisioningSettings::default());
    };
    Ok(workx_windows_sandbox::WindowsSandboxProvisioningSettings {
...
pub fn run_elevated_provisioning_setup(
    ...
    workx_windows_sandbox::run_elevated_provisioning_setup(
        workx_home,
        real_user,
...
pub fn run_legacy_setup_preflight(
    ...
    workx_windows_sandbox::run_windows_sandbox_legacy_preflight(
        permission_profile,
        workspace_roots,
        workx_home,
        command_cwd,
...
pub fn run_setup_refresh_with_extra_read_roots(
    ...
    workx_windows_sandbox::run_setup_refresh_with_extra_read_roots(
        permission_profile,
        workspace_roots,
        command_cwd,
        env_map,
...
async fn run_windows_sandbox_setup_and_persist(
    ...
    let workx_home = request.workx_home;
    let setup_workx_home = workx_home.clone();

    let setup_result = tokio::task::spawn_blocking(move || -> anyhow::Result<()> {
        match mode {
            WindowsSandboxSetupMode::Elevated => {
                if !sandbox_setup_is_complete(setup_workx_home.as_path()) {
                    run_elevated_setup(
                        &permission_profile,
                        workspace_roots.as_slice(),
                        command_cwd.as_path(),
                        &env_map,
                        setup_workx_home.as_path(),
                    )?;
                }
            }
    ...
    ConfigEditsBuilder::new(workx_home.as_path())
...
fn emit_windows_sandbox_setup_success_metrics(
    ...
    let _ = metrics.record_duration(
        "workx.windows_sandbox.setup_duration_ms",
        duration,
        &[
            ("result", "success"),
    ...
    let _ = metrics.counter(
        "workx.windows_sandbox.setup_success",
        /*inc*/ 1,
...
fn emit_windows_sandbox_setup_failure_metrics(
    ...
    let _ = metrics.counter(
        "workx.windows_sandbox.setup_failure",
        /*inc*/ 1,
        &[("originator", originator_tag), ("mode", mode_tag)],
    );
    ...
    if matches!(mode, WindowsSandboxSetupMode::Elevated) {
        #[cfg(target_os = "windows")]
        {
            ...
            if let Some(failure) = workx_windows_sandbox::extract_setup_failure(_err) {
                code_tag = Some(failure.code.as_str().to_string());
                message_tag = Some(workx_windows_sandbox::sanitize_setup_metric_tag_value(
                    &failure.message,
                ));
            }
            ...
            let metric_name =
                if workx_windows_sandbox::extract_setup_failure(_err).is_some_and(|failure| {
                    matches!(
                        failure.code,
                        workx_windows_sandbox::SetupErrorCode::OrchestratorHelperLaunchCanceled
                    )
                }) {
                    "workx.windows_sandbox.elevated_setup_canceled"
                } else {
                    "workx.windows_sandbox.elevated_setup_failure"
                };
            let _ = metrics.counter(metric_name, /*inc*/ 1, &failure_tags);
        }
    } else {
        let _ = metrics.counter(
            "workx.windows_sandbox.legacy_setup_preflight_failed",
            /*inc*/ 1,
            &[("originator", originator_tag)],
        );
    }

...
use workx_context_fragments::to_annotated_content;
...
use workx_rollout_trace::CompactionCheckpointTracePayload;
...
async fn run_remote_compact_task_inner(
    ...
    match pre_compact_outcome {
        ...
        PreCompactHookOutcome::Stopped => {
            ...
            attempt
                .track(
                    sess.as_ref(),
                    workx_analytics::CompactionStatus::Interrupted,
                    Some(&error),
                    analytics_details,
                )
                .await;
    ...
    if result.is_ok() {
        ...
        if let PostCompactHookOutcome::Stopped = post_compact_outcome {
            attempt
                .track(sess.as_ref(), status, workx_error, analytics_details)
                .await;
    ...
    match result {
        Ok(()) => Ok(()),
        Err(err) if matches!(err.details(), WorkxErrorDetails::TurnAborted) => Err(err),
        Err(err) => {
            sess.track_turn_workx_error(turn_context, &err);
...
) -> WorkxResult<RemoteCompactionV2Output> {
    let turn_context = &step_context.turn;
...
async fn collect_compaction_output(
    ...
    if compaction_count != 1 {
        return Err(WorkxErr::Fatal(format!(
...
fn build_v2_compacted_history(
    ...
    prompt_input_metadata: Vec<Option<WorkxHarnessMetadata>>,
    compaction_output: ResponseItem,
...
mod tests {
    ...
    fn response_stream(events: Vec<WorkxResult<ResponseEvent>>) -> ResponseStream {
    ...
    fn retained_history_truncation_preserves_metadata() {
        ...
        assert_eq!(truncated[0].metadata, Some(WorkxHarnessMetadata::default()));
    }
    ...
    async fn collect_compaction_output_accepts_additional_output_items() {
        ...
        let stream = response_stream(vec![
            ...
            Ok(ResponseEvent::Completed {
                ...
                usage_metadata: Some(workx_protocol::ResponseUsageMetadata {

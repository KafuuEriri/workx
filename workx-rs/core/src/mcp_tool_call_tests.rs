...
use workx_config::types::AppConfig;
use workx_config::types::AppToolConfig;
use workx_config::types::AppToolsConfig;
use workx_config::types::ApprovalsReviewer;
use workx_config::types::AppsConfigToml;
...
fn approval_metadata(
    ...
    McpToolApprovalMetadata {
        ...
        workx_apps_meta: None,
        openai_file_input_optional_fields: None,
    }
}

fn approval_config(turn_context: &TurnContext) -> workx_mcp::McpConfig {
...
fn mcp_tool_metadata_resolves_advertised_account_selector(
    ...
    let tool_info = serde_json::from_value(serde_json::json!({
        "server_name": WORKX_APPS_MCP_SERVER_NAME,
...
#[test_case::test_case(Some(serde_json::json!({ "_workx_apps": {} })), Ok(None); "missing_selector_flag_uses_legacy_fallback")]
#[test_case::test_case(Some(serde_json::json!({ "_workx_apps": { "requires_explicit_link_id": false } })), Ok(None); "false_selector_flag_uses_legacy_fallback")]
#[test_case::test_case(Some(serde_json::json!({ "_workx_apps": { "requires_explicit_link_id": null } })), Ok(None); "null_selector_flag_uses_legacy_fallback")]
#[test_case::test_case(Some(serde_json::json!({ "_workx_apps": { "requires_explicit_link_id": "true" } })), Ok(None); "string_true_does_not_require_selector")]
#[test_case::test_case(Some(serde_json::json!({ "_workx_apps": { "requires_explicit_link_id": 1 } })), Ok(None); "numeric_one_does_not_require_selector")]
#[test_case::test_case(Some(serde_json::json!({ "_workx_apps": [] })), Ok(None); "malformed_apps_metadata_uses_legacy_fallback")]
#[test_case::test_case(Some(serde_json::json!({ "link_id": null })), Ok(None); "null_catalog_link_uses_legacy_fallback")]
#[test_case::test_case(Some(serde_json::json!({ "link_id": 42, "_workx_apps": { "requires_explicit_link_id": false } })), Ok(None); "invalid_optional_catalog_link_uses_legacy_fallback")]
#[test_case::test_case(Some(serde_json::json!({ "link_id": " ", "_workx_apps": { "requires_explicit_link_id": false } })), Ok(None); "empty_optional_catalog_link_uses_legacy_fallback")]
...
#[test_case::test_case(serde_json::json!({ "_workx_apps": { "requires_explicit_link_id": true } }); "unrelated_apps_metadata")]
...
fn write_sample_plugin_mcp(workx_home: &std::path::Path) {
    let plugin_root = workx_home.join("plugins/cache/test/sample/local");
    std::fs::create_dir_all(plugin_root.join(".workx-plugin")).expect("create plugin manifest dir");
    std::fs::write(
...
fn install_mcp_permission_request_hook(
    ...
    std::fs::create_dir_all(&turn_context.config.workx_home)
        .expect("create workx home for MCP permission hook");
    let script = format!(
        r#"import json
...
"#,
    ...
    std::fs::write(
        turn_context.config.workx_home.join("hooks.json"),
        serde_json::json!({
            "hooks": {
                "PermissionRequest": [{
                    "matcher": matcher,
    ...
    let hook_list = workx_hooks::list_hooks(HooksConfig {
    ...
    let trusted_config_layer_stack = trusted_config_layer_stack(
        ...
        &turn_context.config.workx_home,
        hook_list.hooks,
    );
...
#[test]
fn openai_file_params_are_only_honored_for_workx_apps() {
    let params = HashMap::from([("file".to_string(), Vec::new())]);

    assert_eq!(
        openai_file_input_optional_fields_for_server(WORKX_APPS_MCP_SERVER_NAME, &params),
        Some(params.clone())
    );
    assert_eq!(
...
async fn mcp_tool_call_span_records_error_type_and_error_code() {
    ...
    assert!(
        logs.contains("error.type=\"tool_result\"")
            && logs.contains("workx.mcp.error.code=\"RATE_LIMITED\""),
...
async fn mcp_result_telemetry_records_allowlisted_span_fields() {
    ...
    assert!(
        logs.contains("workx.mcp.target.id=\"com.apple.reminders\"")
            && logs.contains("workx.mcp.server_user_flow.triggered=false"),
...
async fn mcp_result_telemetry_ignores_invalid_and_missing_values() {
    ...
    .await;
    assert!(
        !invalid_logs.contains("workx.mcp.target.id=")
            && !invalid_logs.contains("workx.mcp.server_user_flow.triggered="),
    ...
    .await;
    assert!(
        !missing_logs.contains("workx.mcp.target.id=")
            && !missing_logs.contains("workx.mcp.server_user_flow.triggered="),
    ...
    assert!(
        !no_meta_logs.contains("workx.mcp.target.id=")
            && !no_meta_logs.contains("workx.mcp.server_user_flow.triggered="),
...
async fn mcp_result_telemetry_truncates_long_target_id() {
    ...
    assert!(
        logs.contains(&format!("workx.mcp.target.id=\"{truncated}\"")) && !logs.contains("tail"),
...
fn approval_elicitation_request_uses_message_override_and_preserves_tool_params_keys() {
    ...
    let request = build_mcp_tool_approval_elicitation_request(McpToolApprovalElicitationRequest {
        server: WORKX_APPS_MCP_SERVER_NAME,
...
#[test]
fn workx_apps_tool_question_uses_fallback_app_label() {
    let question = build_mcp_tool_approval_question(
        "q".to_string(),
        WORKX_APPS_MCP_SERVER_NAME,
        "run_action",
...
#[test]
fn trusted_workx_apps_tool_question_offers_always_allow() {
    let question = build_mcp_tool_approval_question(
        "q".to_string(),
        WORKX_APPS_MCP_SERVER_NAME,
        "run_action",
...
#[test]
fn workx_apps_tool_question_without_elicitation_omits_always_allow() {
    let question = build_mcp_tool_approval_question(
        "q".to_string(),
        WORKX_APPS_MCP_SERVER_NAME,
        "run_action",
...
#[test]
fn workx_apps_connectors_support_persistent_approval() {
    let invocation = McpInvocation {
        server: WORKX_APPS_MCP_SERVER_NAME.to_string(),
...
async fn mcp_tool_call_request_meta_includes_turn_metadata_for_custom_server() {
    ...
    let turn_metadata = meta
        .get(crate::X_WORKX_TURN_METADATA_HEADER)
    ...
    assert_eq!(
        ...
        serde_json::json!({
            "callId": "call-custom",
            crate::X_WORKX_TURN_METADATA_HEADER: expected_turn_metadata,
        })
    );
}
...
async fn mcp_tool_call_request_meta_uses_the_issuing_step(
    ...
    assert_eq!(
        ...
        Some(serde_json::json!({
            "callId": "call-b",
            crate::X_WORKX_TURN_METADATA_HEADER: expected,
    ...
    assert_eq!(
        turn_context
            ...
            .to_responses_metadata(
                ...
                "window".to_string(),
                crate::responses_metadata::WorkxResponsesRequestKind::Turn,
            )
            .turn_metadata_value()
...
async fn guardian_mcp_tool_call_request_meta_excludes_actor_confirmation_policy() {
    for session_source in [
        ...
        let expected = Some(serde_json::json!({
            ...
            crate::X_WORKX_TURN_METADATA_HEADER: expected_mcp_turn_metadata(&turn_context),
        }));
...
#[test]
fn mcp_tool_call_item_metadata_only_trusts_workx_apps_identity() {
    let mut metadata = approval_metadata(
    ...
    metadata.workx_apps_meta = Some(
        serde_json::json!({
            "resource_uri": "/asdk_app_0123456789abcdef0123456789abcdef/link_fedcba9876543210fedcba9876543210/create_event",
        })
        .as_object()
        .cloned()
        .expect("_workx_apps metadata should be an object"),
    );

    assert_eq!(
        McpToolCallItemMetadata::from_tool_metadata(WORKX_APPS_MCP_SERVER_NAME, Some(&metadata),),
...
#[tokio::test]
async fn workx_apps_tool_call_request_meta_includes_turn_metadata_and_workx_apps_meta() {
    let (_, turn_context) = make_session_and_context().await;
    let turn_context = Arc::new(turn_context);
    let step_context = StepContext::for_test(Arc::clone(&turn_context));
    let expected_turn_metadata = expected_mcp_turn_metadata(&turn_context);
...
#[tokio::test]
async fn workx_apps_tool_call_request_meta_includes_call_id_without_existing_workx_apps_meta() {
    let (_, turn_context) = make_session_and_context().await;
    let turn_context = Arc::new(turn_context);
    let step_context = StepContext::for_test(Arc::clone(&turn_context));
    let expected_turn_metadata = expected_mcp_turn_metadata(&turn_context);
...
fn workx_apps_auth_failure_result() -> CallToolResult {
    CallToolResult {
        content: vec![serde_json::json!({
            "type": "text",
...
fn workx_apps_auth_failure_metadata() -> McpToolApprovalMetadata {
    approval_metadata(
...
#[tokio::test]
async fn workx_apps_auth_elicitation_feature_disabled_returns_original_result() {
    ...
    let result = workx_apps_auth_failure_result();
    let metadata = workx_apps_auth_failure_metadata();

    let returned = maybe_request_workx_apps_auth_elicitation(
        &session,
        &turn_context,
...
#[tokio::test]
async fn workx_apps_auth_elicitation_disallowed_by_policy_returns_original_result() {
    let (session, mut turn_context, rx_event) = make_session_and_context_with_rx().await;
    let mut features = Features::with_defaults();
...
#[tokio::test]
async fn workx_apps_auth_elicitation_granular_mcp_disabled_returns_original_result() {
    let (session, mut turn_context, rx_event) = make_session_and_context_with_rx().await;
    let mut features = Features::with_defaults();
    features.enable(Feature::AuthElicitation);
    let mutable_turn_context = Arc::get_mut(&mut turn_context).expect("single turn context ref");
...
#[tokio::test]
async fn workx_apps_auth_elicitation_enabled_by_default_requests_elicitation() {
    let (session, turn_context, rx_event) = make_session_and_context_with_rx().await;
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    let request_task = tokio::spawn({
        ...
        async move {
            maybe_request_workx_apps_auth_elicitation(
                &session,
                &turn_context,
                turn_context.approval_policy(),
                "call_123",
    ...
    assert_eq!(request.server_name, WORKX_APPS_MCP_SERVER_NAME);
    assert_eq!(
        request.id,
        workx_protocol::mcp::RequestId::String("workx_apps_auth_call_123".to_string())
    );
    assert!(matches!(
        request.request,
        workx_protocol::approvals::ElicitationRequest::Url { .. }
    ));

    session
        .resolve_elicitation(
            WORKX_APPS_MCP_SERVER_NAME.to_string(),
            rmcp::model::RequestId::String("workx_apps_auth_call_123".into()),
            ElicitationResponse {
...
#[test]
fn approval_elicitation_meta_includes_connector_source_for_workx_apps() {
    assert_eq!(
        build_mcp_tool_approval_elicitation_meta(
            WORKX_APPS_MCP_SERVER_NAME,
            Some(&approval_metadata(
...
#[tokio::test]
async fn persist_workx_app_tool_approval_writes_tool_override() {
    ...
    persist_workx_app_tool_approval(&config, "calendar", "calendar/list_events")
        .await
...
async fn custom_mcp_tool_approval_mode_uses_plugin_mcp_policy() {
    ...
    write_sample_plugin_mcp(workx_home.as_path());
    std::fs::write(
...
async fn maybe_persist_mcp_tool_approval_reloads_session_config_for_custom_server() {
    ...
    let config = ConfigBuilder::without_managed_config_for_tests()
        .workx_home(workx_home.clone().to_path_buf())
        .build()
        .await
        .expect("load config");
    turn_context.config = Arc::new(config);
...
"#,
    ...
    let contents = std::fs::read_to_string(workx_home.join(CONFIG_TOML_FILE)).expect("read config");
    let parsed: ConfigToml = toml::from_str(&contents).expect("parse config");
    let tool = parsed
        .plugins
        .get("sample@test")
...
async fn maybe_persist_mcp_tool_approval_writes_project_config_for_project_server() {
    ...
    let project_workx_dir = project_dir.path().join(".workx");
    std::fs::create_dir_all(&project_workx_dir).expect("create project .workx dir");
    std::fs::write(
        project_workx_dir.join(CONFIG_TOML_FILE),
        "[mcp_servers.docs]\ncommand = \"docs-server\"\n",
    )
    ...
    ConfigEditsBuilder::new(&workx_home)
        .set_project_trust_level(
            project_dir.path(),
            workx_protocol::config_types::TrustLevel::Trusted,
        )
        .apply()
        .await
    ...
    let contents = std::fs::read_to_string(project_workx_dir.join(CONFIG_TOML_FILE))

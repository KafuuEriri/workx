...
use tracing::warn;
use workx_analytics::GuardianReviewAnalyticsResult;
use workx_analytics::GuardianReviewSessionAnalyticsParams;
use workx_analytics::GuardianReviewSessionKind;
use workx_extension_api::Instructions;
...
use crate::workx_delegate::run_workx_thread_interactive;
...
use workx_tools::normalize_output_image_detail;
...
pub(crate) enum GuardianReviewSessionOutcome {
    ...
    SessionFailed {
        error: anyhow::Error,
        error_info: Option<WorkxErrorInfo>,
    },
    TimedOut,
    Aborted,
}
...
impl GuardianReviewSessionReuseKey {
    fn from_spawn_config(
        ...
        Self {
            ...
            workx_linux_sandbox_exe: spawn_config.workx_linux_sandbox_exe.clone(),
...
async fn spawn_guardian_review_session(
    ...
    let (session, io) = Box::pin(run_workx_thread_interactive(
        spawn_config,
...
async fn run_review_on_session(
    ...
    let submission = review_session.io.submit_turn_input(
        TurnInputRequest::user_input(prompt_items.items)
            .with_thread_settings(workx_protocol::protocol::ThreadSettingsOverrides {
                                ...
                                && let Some(error) = last_error
                            {
                                return (
                                    GuardianReviewSessionOutcome::SessionFailed {
                                        ...
                                        error_info: error.workx_error_info,
                                    },
                                    true,
                                    true,
                                );
...
fn read_only_guardian_permission_profile(
    ...
    permission_profile
        ...
        .unwrap_or(PermissionProfile::External {
            network: workx_protocol::permissions::NetworkSandboxPolicy::Restricted,
        })
}

pub(crate) fn build_guardian_review_session_config(
    parent_config: &Config,
    live_network_config: Option<workx_network_proxy::NetworkProxyConfig>,
    active_model: &str,

...
use workx_analytics::GuardianApprovalRequestSource;
...
use workx_guardian_context::ConversationTranscriptEntry;
use workx_guardian_context::ConversationTranscriptEntryKind;
use workx_history::RolloutItem;
use workx_model_provider::create_model_provider;
use workx_model_provider_info::AMAZON_BEDROCK_GPT_5_4_MODEL_ID;
...
// The memories extension depends on workx-core, so this probe verifies the nested Guardian config
...
impl workx_extension_api::ThreadLifecycleContributor<Config> for GuardianMemoryContextProbe {
...
impl workx_extension_api::ContextContributor for GuardianMemoryContextProbe {
    fn contribute_thread_context<'a>(
        &'a self,
        _session_store: &'a workx_extension_api::ExtensionData,
        thread_store: &'a workx_extension_api::ExtensionData,
    ) -> workx_extension_api::ExtensionFuture<'a, Vec<workx_extension_api::PromptFragment>> {
        Box::pin(async move {
            if thread_store
                .get::<GuardianMemoryContextEnabled>()
                .is_some_and(|enabled| enabled.0)
            {
                vec![workx_extension_api::PromptFragment::developer_policy(
                    GUARDIAN_MEMORY_CONTEXT_PROBE,
                    workx_extension_api::ContentItemKind(
...
fn guardian_exec_command_request(id: &str) -> GuardianApprovalRequest {
    GuardianApprovalRequest::ExecCommand {
        ...
        cwd: test_path_buf("/repo/workx-rs/core").abs(),
        ...
        .record_conversation_items(
            ...
            &[
                ...
                ResponseItem::FunctionCall {
                    ...
                    namespace: None,
                    arguments: "{\"repo\":\"RonanXiao/workx\"}".to_string(),
                ...
                ResponseItem::FunctionCallOutput {
                    ...
                    name: None,
                    namespace: None,
                    output: workx_protocol::models::FunctionCallOutputPayload::from_text(
...
fn normalize_guardian_snapshot_paths(text: String) -> String {
    let mut text = text;
    for canonical_path in ["/repo/workx-rs/core", "/repo"] {
...
fn guardian_prompt_text(items: &[workx_protocol::user_input::UserInput]) -> String {
    items
        .iter()
        .map(|item| match item {
            workx_protocol::user_input::UserInput::Text { text, .. } => text.as_str(),
            _ => "",
        })
        .collect::<String>()
}
...
async fn build_guardian_prompt_full_mode_preserves_initial_review_format() -> anyhow::Result<()> {
    ...
    assert!(text.contains("The Workx agent has requested the following action:\n"));
...
async fn build_guardian_prompt_truncates_oversized_approval_reason() -> anyhow::Result<()> {
    ...
    let expected_reason = workx_utils_output_truncation::truncate_text(
        &approval_reason,
        workx_utils_output_truncation::TruncationPolicy::Tokens(/*tokens*/ 512),
    );
    ...
    let reason_item = prompt
        ...
        .find_map(|item| match item {
            workx_protocol::user_input::UserInput::Text { text, .. }
        ...
        &FileSystemSandboxPolicy::restricted(vec![
            FileSystemSandboxEntry {
                path: FileSystemPath::Special {
                    value: workx_protocol::permissions::FileSystemSpecialPath::Root,
                },
            ...
            FileSystemSandboxEntry {
                path: FileSystemPath::Special {
                    value: workx_protocol::permissions::FileSystemSpecialPath::project_roots(Some(
                        "private".to_string(),
                    )),
                },
...
async fn build_guardian_prompt_delta_mode_preserves_original_numbering() -> anyhow::Result<()> {
    ...
    assert!(text.contains("The Workx agent has requested the following next action:\n"));
...
fn collect_guardian_transcript_entries(
    history: &dyn workx_guardian_context::SectionHistory,
...
fn collect_guardian_transcript_entries_includes_recent_tool_calls_and_output() {
    ...
    if let ResponseItem::FunctionCallOutput { output, .. } = &mut items[2] {
        *output =
            workx_protocol::models::FunctionCallOutputPayload::from_text(oversized_result.clone());
    }
    for token_cap in [
...
fn collect_guardian_transcript_entries_preserves_named_unpaired_tool_sources() {
    ...
    items.extend(
        ...
        .map(|(call_id, text)| ResponseItem::FunctionCallOutput {
            ...
            name: None,
            namespace: None,
            output: workx_protocol::models::FunctionCallOutputPayload::from_text(text.to_string()),
            internal_chat_message_metadata_passthrough: None,
        }),
    );
    ...
    if let ResponseItem::FunctionCallOutput { output, .. } = &mut items[0] {
        *output = workx_protocol::models::FunctionCallOutputPayload::from_content_items(vec![
...
async fn build_guardian_prompt_items_explains_network_access_review_scope() -> anyhow::Result<()> {
    ...
    settings.bind(|| {
        assert_snapshot!(
            "workx_core__guardian__tests__network_access_guardian_prompt_layout",
...
fn guardian_timeout_message_distinguishes_timeout_from_policy_denial() {
    let mut model = workx_models_manager::model_info::model_info_from_slug("acting-model");
...
async fn guardian_request_model_for_auto_review(
    ...
    String,
    String,
    workx_analytics::GuardianReviewAnalyticsResult,
)> {
...
-> anyhow::Result<()> {
    ...
    let mut extensions = workx_extension_api::ExtensionRegistryBuilder::<Config>::new();
    ...
    assert!(matches!(
        ...
        Some(workx_analytics::GuardianReviewSessionKind::TrunkNew)
    ));
    let request = request_log.single_request();
    let request_body = request.body_json();
    assert!(
    ...
    settings.bind(|| {
        assert_snapshot!(
            "workx_core__guardian__tests__guardian_review_request_layout",
...
async fn build_guardian_prompt_items_includes_parent_session_id() -> anyhow::Result<()> {
    ...
    let prompt_text = prompt
        ...
        .into_iter()
        .map(|item| match item {
            workx_protocol::user_input::UserInput::Text { text, .. } => text,
            workx_protocol::user_input::UserInput::Image { .. } => String::new(),
            _ => String::new(),
        })
        .collect::<String>();
...
async fn guardian_reuses_prompt_cache_key_and_appends_prior_reviews() -> anyhow::Result<()> {
        ...
        .replace_history(
            vec![
                ResponseItem::Compaction {
                    id: Some(workx_protocol::ResponseItemId::from_server(
    ...
    assert!(matches!(
        ...
        Some(workx_analytics::GuardianReviewSessionKind::TrunkReused)
    ));
    assert!(matches!(
    ...
    settings.bind(|| {
        assert_snapshot!(
            "workx_core__guardian__tests__guardian_followup_review_request_layout",
            format!(
...
async fn escalated_retry_bypasses_extension_approval_and_runs_guardian() -> anyhow::Result<()> {
    ...
    impl workx_extension_api::ApprovalReviewContributor for AutoApprovingReviewContributor {
        fn fast_decision<'a>(
            &'a self,
            _session_store: &'a workx_extension_api::ExtensionData,
            _thread_store: &'a workx_extension_api::ExtensionData,
            _prompt: &'a str,
            _extension_metrics: Option<Arc<dyn workx_extension_api::ExtensionMetrics>>,
        ) -> workx_extension_api::ExtensionFuture<'a, Option<ReviewDecision>> {
...
async fn guardian_ephemeral_retry_preserves_parallel_trunk_and_fork_history() -> anyhow::Result<()>
{
    ...
    let handle = std::thread::Builder::new()
        ...
        let feedback = workx_feedback::guardian_review_failures(&[session.thread_id()])
            .attachment
...
async fn guardian_review_session_config_preserves_parent_network_proxy() {
    ...
    let guardian_config = build_guardian_review_session_config_for_test(
        ...
        "parent-active-model",
        Some(workx_protocol::openai_models::ReasoningEffort::Low),
        /*model_messages*/ None,
    )

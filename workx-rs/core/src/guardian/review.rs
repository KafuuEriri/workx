...
use workx_analytics::GuardianReviewDecision;
use workx_analytics::GuardianReviewFailureReason;
use workx_analytics::GuardianReviewTerminalStatus;
use workx_analytics::GuardianReviewTrackContext;
use workx_analytics::GuardianReviewedAction;
use workx_analytics::GuardianV2Event;
use workx_analytics::GuardianV2EventKind;
use workx_async_utils::THREAD_STACK_SIZE_BYTES;
use workx_core_plugins::PluginCommandAttribution;
use workx_extension_api::GuardianV2Enabled;
...
use workx_protocol::protocol::GuardianAssessmentDecisionSource;
...
impl GuardianReviewError {
    ...
    fn session_with_error_info(err: anyhow::Error, error_info: WorkxErrorInfo) -> Self {
        Self::Session {
            message: err.to_string(),
                ...
                .is_some()
            {
                session
                    ...
                    .track_guardian_v2_event(GuardianV2Event {
                        ...
                        occurred_at_ms: workx_analytics::now_unix_millis(),
...
pub(crate) fn spawn_approval_request_review(
    ...
    let spawn_result = std::thread::Builder::new()
        .name("workx-approval-review".to_string())
    ...
    let (guardian_model, guardian_reasoning_effort) = if let Some(preset) = review_model {
        let reasoning_effort = preferred_reasoning_effort(
            preset
                ...
                .iter()
                .any(|effort| effort.effort == workx_protocol::openai_models::ReasoningEffort::Low),
        ...
        let reasoning_effort = preferred_reasoning_effort(
            turn.model_info()
                ...
                .iter()
                .any(|preset| preset.effort == workx_protocol::openai_models::ReasoningEffort::Low),
...
fn should_retry_guardian_review(outcome: &GuardianReviewOutcome) -> bool {
    matches!(
        ...
        GuardianReviewOutcome::Error(
            GuardianReviewError::Session {
                error_info: Some(
                    WorkxErrorInfo::ServerOverloaded
                        | WorkxErrorInfo::HttpConnectionFailed { .. }
                        | WorkxErrorInfo::ResponseStreamConnectionFailed { .. }
                        | WorkxErrorInfo::InternalServerError
                        | WorkxErrorInfo::ResponseStreamDisconnected { .. }
                ),
                ..
            } | GuardianReviewError::Parse { .. }
        )

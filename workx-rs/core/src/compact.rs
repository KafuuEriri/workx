...
use workx_analytics::CompactionStatus;
use workx_analytics::CompactionStrategy;
use workx_analytics::CompactionTrigger;
use workx_analytics::WorkxCompactionEvent;
use workx_analytics::now_unix_seconds;
use workx_context_fragments::AnnotatedContent;
use workx_context_fragments::set_annotated_content;
...
use workx_protocol::items::ContextCompactionItem;
...
use workx_utils_output_truncation::approx_token_count;
use workx_utils_output_truncation::truncate_text;

pub use workx_prompts::SUMMARIZATION_PROMPT;
pub use workx_prompts::SUMMARY_PREFIX;
...
async fn run_compact_task_inner(
    ...
    match pre_compact_outcome {
        ...
        PreCompactHookOutcome::Stopped => {
            let error = WorkxErr::TurnAborted;
            attempt
                .track(
                    sess.as_ref(),
    ...
    let workx_error = result.as_ref().err();
    if result.is_ok() {
...
async fn run_compact_task_inner_impl(
    ...
    let responses_metadata = sess
        .responses_metadata(
            turn_context.as_ref(),
            WorkxResponsesRequestKind::Compaction(compaction_metadata),
        )
        .await;
    ...
    let compaction_response_id = loop {
        ...
        match attempt_result {
            ...
            Err(err)
                if matches!(
                    err.details(),
                    WorkxErrorDetails::Interrupted | WorkxErrorDetails::TurnAborted
                ) =>
            {
                return Err(err);
            }
            Err(e) if matches!(e.details(), WorkxErrorDetails::SessionBudgetExceeded) => {
                sess.track_turn_workx_error(turn_context.as_ref(), &e);
                let event = EventMsg::Error(e.to_error_event(/*message_prefix*/ None));
                sess.send_event(&turn_context, event).await;
                return Err(e);
            }
            Err(e) if matches!(e.details(), WorkxErrorDetails::ContextWindowExceeded) => {
                if turn_input_len > 1 {
...
impl CompactionAnalyticsAttempt {
    ...
    pub(crate) async fn track(
        ...
        sess.services
            ...
            .track_compaction(WorkxCompactionEvent {
                ...
                status,
                workx_error_kind: workx_error.map(Into::into),
                workx_error_http_status_code: workx_error
                    .and_then(WorkxErr::http_status_code_value),
                active_context_tokens_before,
...
pub(crate) fn compaction_status_from_result<T>(result: &WorkxResult<T>) -> CompactionStatus {
    match result {
...
pub(crate) struct CompactedUserMessage {
    ...
    harness_metadata: Option<WorkxHarnessMetadata>,
}

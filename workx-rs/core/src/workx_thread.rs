...
use workx_diagnostics::Gauge;
use workx_diagnostics::GaugeGuard;
use workx_exec_server::SelectedCapabilityRootsStatus;
...
use workx_protocol::turn_input::RecoverTurnRequest;
use workx_protocol::turn_input::StartIfIdleSubmission;
use workx_protocol::turn_input::SteerSubmission;
use workx_protocol::turn_input::SuspendTurnOutcome;
...
use workx_thread_store::ThreadStoreResult;
...
impl ThreadConfigSnapshot {
    ...
    pub fn sandbox_policy(&self) -> SandboxPolicy {
        workx_sandboxing::compatibility_sandbox_policy_for_permission_profile(
...
pub struct WorkxThreadSettingsOverrides {
...
pub use workx_guardian_context::GuardianRootMessage;
...
/// (formerly called a conversation) in Workx.
impl WorkxThread {
    pub(crate) fn new(
        session: Arc<Session>,
        io: SessionIo,
    ...
    pub async fn submit(&self, op: Op) -> WorkxResult<String> {
    ...
    pub fn thread_extension_data(&self) -> &workx_extension_api::ExtensionData {
        &self.session.services.thread_extension_data
    }

    pub async fn shutdown_and_wait(&self) -> WorkxResult<()> {
    ...
    pub(crate) async fn emit_thread_ready_lifecycle(&self) {
        ...
        {
            contributor
                .on_thread_ready(workx_extension_api::ThreadReadyInput {
    ...
    pub(crate) async fn emit_thread_resume_lifecycle(&self) {
        ...
        {
            contributor
                .on_thread_resume(workx_extension_api::ThreadResumeInput {
                    session_store: &self.session.services.session_extension_data,
                    thread_store: &self.session.services.thread_extension_data,
                })
                .await;
    ...
    pub async fn start_turn_if_idle(
        &self,
        request: TurnInputRequest,
    ) -> WorkxResult<StartIfIdleSubmission> {
        match self
    ...
    pub async fn suspend_turn_and_shutdown(&self) -> WorkxResult<SuspendTurnOutcome> {
        if self.session_source.is_non_root_agent() {
            return Err(WorkxErr::UnsupportedOperation(
        ...
        self.io
            ...
            .await
            .map_err(|_| WorkxErr::Fatal("thread session has stopped".to_string()))?;
        let outcome = result
            .await
            .map_err(|_| WorkxErr::Fatal("thread suspension reply was lost".to_string()))??;
    ...
    /// callers that only hold a `WorkxThread`.
    ...
    pub async fn preview_thread_settings_overrides(
        &self,
        overrides: WorkxThreadSettingsOverrides,
    ...
    fn thread_settings_update(overrides: WorkxThreadSettingsOverrides) -> SessionSettingsUpdate {
        let WorkxThreadSettingsOverrides {
            environments,
    ...
    pub async fn next_event(&self) -> WorkxResult<Event> {
    ...
    pub async fn inject_response_items(&self, items: Vec<ResponseItem>) -> WorkxResult<()> {
    ...
    pub async fn restorable_thread_settings(&self) -> WorkxThreadSettingsOverrides {
    ...
    ) -> std::collections::HashMap<String, workx_protocol::mcp::McpServerConnectionStatus> {
        self.session
            .services
            .mcp_runtime
    ...
    ) -> (workx_mcp::McpConfig, workx_mcp::McpRuntimeContext) {
    ...
    pub async fn current_mcp_config_and_runtime_context(
        &self,
    ) -> (Arc<workx_mcp::McpConfig>, workx_mcp::McpRuntimeContext) {
    ...
    pub async fn start_mcp_event_stream(
    ...
    ) -> anyhow::Result<workx_mcp::McpEventStream> {
        ...
        self.session.refresh_mcp_if_dirty().await;
        workx_mcp::McpResourceClient::new(Arc::clone(&self.session.services.mcp_runtime))
    ...
    pub async fn increment_out_of_band_elicitation_count(&self) -> WorkxResult<i64> {
        let mut elicitations = self.out_of_band_elicitations.lock().await;
        let incremented = elicitations.count.checked_add(1).ok_or_else(|| {
            WorkxErr::Fatal("out-of-band elicitation count overflowed".to_string())
        })?;
    ...
    pub async fn decrement_out_of_band_elicitation_count(&self) -> WorkxResult<i64> {
        let mut elicitations = self.out_of_band_elicitations.lock().await;
        if elicitations.count == 0 {
            return Err(WorkxErr::InvalidRequest(

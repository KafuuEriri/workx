...
use workx_execpolicy::AmendError;
use workx_execpolicy::Decision;
use workx_execpolicy::Error as ExecPolicyRuleError;
...
use workx_execpolicy::RequirementsExecPolicy;
use workx_execpolicy::RuleMatch;
use workx_execpolicy::blocking_append_allow_prefix_rule;
use workx_execpolicy::blocking_append_network_rule;
...
use workx_shell_command::is_dangerous_command::DangerousCommandMatch;
use workx_shell_command::is_dangerous_command::DangerousCommandPlatform;
use workx_shell_command::is_dangerous_command::dangerous_command_match_for_platform;
...
pub(crate) fn child_uses_parent_exec_policy(parent_config: &Config, child_config: &Config) -> bool {
    fn exec_policy_config_folders(config: &Config) -> Vec<AbsolutePathBuf> {
        config
            .config_layer_stack
            .layers_low_to_high()
            .filter_map(workx_config::ConfigLayerEntry::config_folder)
            .collect()
    }
...
impl ExecPolicyManager {
    ...
    pub(crate) async fn append_amendment_and_update(
        ...
        let policy_path = default_policy_path(workx_home);
        spawn_blocking({
...
fn exec_policy_message_for_display(source: &workx_execpolicy::Error) -> String {
...
fn dangerous_command_match_for_origin(
    ...
    match command_origin {
        ...
        ExecPolicyCommandOrigin::PowerShell => {
            workx_shell_command::is_dangerous_command::dangerous_powershell_words_match(
                command,
                command_platform,
            )
        }
...
pub(crate) fn default_policy_path(workx_home: &Path) -> PathBuf {
    workx_home.join(RULES_DIR_NAME).join(DEFAULT_POLICY_FILE)
}
...
fn commands_for_exec_policy_for_platform(
    ...
    if command_platform == DangerousCommandPlatform::Windows
        && let Some(commands) =
            workx_shell_command::powershell::parse_powershell_command_into_plain_commands(command)
        && !commands.is_empty()
    {
        return ExecPolicyCommands {
            commands,
...
/// - Note: we only use this amendment when the command fails to run in sandbox and workx prompts the user to run outside the sandbox

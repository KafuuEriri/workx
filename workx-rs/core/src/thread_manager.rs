...
use crate::workx_thread::WorkxThread;
...
use tracing::warn;
use workx_agent_graph_store::AgentGraphStore;
use workx_agent_graph_store::LocalAgentGraphStore;
...
use workx_code_mode::CodeModeSessionProvider;
use workx_code_mode::DisabledCodeModeSessionProvider;
use workx_code_mode::ProcessOwnedCodeModeSessionProvider;
...
use workx_extension_api::UserInstructionsProvider;
...
use workx_protocol::mcp::OPENAI_STANDARD_FORM_INPUT_EXTENSION_ID;
...
use workx_thread_store::InMemoryThreadStore;
use workx_thread_store::LoadThreadHistoryParams;
use workx_thread_store::LocalThreadStore;
use workx_thread_store::LocalThreadStoreConfig;
use workx_thread_store::MoveThreadToSectionParams;
use workx_thread_store::PreparedFork;
use workx_thread_store::ReadThreadByRolloutPathParams;
use workx_thread_store::ReadThreadParams;
use workx_thread_store::StoredModelContext;
use workx_thread_store::StoredThread;
use workx_thread_store::ThreadMetadataPatch;
use workx_thread_store::ThreadStore;
use workx_thread_store::ThreadStoreError;
use workx_thread_store::UpdateThreadMetadataParams;
...
struct TempWorkxHomeGuard {
    path: PathBuf,
}

impl Drop for TempWorkxHomeGuard {
    fn drop(&mut self) {
        let _ = std::fs::remove_dir_all(&self.path);
    }
}

/// Represents a newly created Workx thread (formerly called a conversation), including the first event
...
pub struct ThreadManager {
    ...
    _test_workx_home_guard: Option<TempWorkxHomeGuard>,
}
...
pub struct StartThreadOptions {
    ...
    pub dynamic_tools: Vec<workx_protocol::dynamic_tools::DynamicToolSpec>,
...
pub(crate) struct ThreadManagerState {
    threads: Arc<RwLock<HashMap<ThreadId, Arc<WorkxThread>>>>,
            ...
            if has_state_db && background_migration_enabled {
                ...
                tokio::spawn(async move {
                    ...
                    if compression_enabled {
                        workx_rollout::spawn_rollout_compression_worker(workx_home);
                    }
                });
            } else if compression_enabled {
                workx_rollout::spawn_rollout_compression_worker(config.workx_home.to_path_buf());
            }
            store
        }
...
impl ThreadManager {
    ...
    pub fn new(
        ...
        auth_manager: Arc<AuthManager>,
        models_manager: SharedModelsManager,
        workx_apps_tools_cache: WorkxAppsToolsCache,
        session_source: SessionSource,
        environment_manager: Arc<EnvironmentManager>,
        extensions: Arc<ExtensionRegistry<Config>>,
        user_instructions_provider: Arc<dyn UserInstructionsProvider>,
    ...
    /// Construct with a dummy AuthManager containing the provided WorkxAuth.
    ...
    pub(crate) fn with_models_provider_for_tests(
        ...
        let workx_home = std::env::temp_dir().join(format!(
            "workx-thread-manager-test-{}",
            uuid::Uuid::new_v4()
        ));
        std::fs::create_dir_all(&workx_home)
            .unwrap_or_else(|err| panic!("temp workx home dir create failed: {err}"));
        ...
        manager._test_workx_home_guard = Some(TempWorkxHomeGuard { path: workx_home });
        manager
    }

    /// Construct with a dummy AuthManager containing the provided WorkxAuth and workx home.
    /// Used for integration tests: should not be used by ordinary business logic.
    ...
    pub(crate) fn with_models_provider_home_and_state_for_tests(
        ...
        let absolute_workx_home = match AbsolutePathBuf::from_absolute_path_checked(&workx_home) {
            Ok(workx_home) => workx_home,
            Err(err) => panic!("test workx_home should be absolute: {err}"),
        };
        let (thread_created_tx, _) = broadcast::channel(THREAD_CREATED_CHANNEL_CAPACITY);
        let restriction_product = SessionSource::Exec.restriction_product();
        let skills_service = Arc::new(HostSkillsService::new_with_restriction_product(
            absolute_workx_home.clone(),
        ...
        let thread_store: Arc<dyn ThreadStore> = Arc::new(LocalThreadStore::new(
            LocalThreadStoreConfig {
                ...
                sqlite: workx_state::SqliteConfig::new_for_testing(absolute_workx_home),
        ...
        Self {
            state: Arc::new(ThreadManagerState {
                ...
                models_manager: create_model_provider(provider, Some(auth_manager.clone()))
                    .models_manager(workx_home, /*config_model_catalog*/ None),
                git_root_discovery: Arc::default(),
                environment_manager,
                starting_mcp_runtimes: std::sync::Mutex::new(Vec::new()),
                skills_service,
    ...
    pub async fn get_thread(&self, thread_id: ThreadId) -> WorkxResult<Arc<WorkxThread>> {
        self.state.get_thread(thread_id).await
    }

    /// Updates metadata for loaded and cold threads through one entrypoint.
    ///
    /// Loaded threads route through `WorkxThread`/`LiveThread`, so metadata changes stay ordered
        ...
        let updated = self
            ...
            .map_err(|err| match err {
                ThreadStoreError::ThreadNotFound { thread_id } => {
                    WorkxErr::ThreadNotFound(thread_id)
                }
    ...
    pub async fn list_agent_subtree_thread_ids(
        ...
        if let Some(agent_graph_store) = self.state.agent_graph_store() {
            for descendant_id in agent_graph_store
                ...
                .await
                .map_err(|err| {
                    WorkxErr::Fatal(format!("failed to load thread-spawn descendants: {err}"))
                })?
            {
    ...
    pub async fn start_thread(&self, options: StartThreadOptions) -> WorkxResult<NewThread> {
    ...
    /// as `Arc<WorkxThread>`, it is possible that other references to it exist elsewhere.
    /// Returns the thread if the thread was found and removed.
    pub async fn remove_thread(&self, thread_id: &ThreadId) -> Option<Arc<WorkxThread>> {
    ...
    pub async fn remove_thread_if_matches(
        &self,
        thread_id: &ThreadId,
        expected: &Arc<WorkxThread>,
    ) -> Option<Arc<WorkxThread>> {
...
impl ThreadManagerState {
    ...
    pub(crate) async fn get_thread(&self, thread_id: ThreadId) -> WorkxResult<Arc<WorkxThread>> {
        let threads = self.threads.read().await;
        match threads.get(&thread_id) {
            Some(thread) if !thread.session_source.is_internal() => Ok(thread.clone()),
            Some(_) | None => Err(WorkxErr::ThreadNotFound(thread_id)),
        }
    }
    ...
    pub(crate) async fn read_stored_thread(
        ...
        self.thread_store
            ...
            .map_err(|err| match err {
                ...
                err => WorkxErr::Fatal(format!("failed to read stored thread {thread_id}: {err}")),
            })
    }
    ...
    ) -> WorkxResult<StoredModelContext> {
        let thread_id = params.thread_id;
        self.thread_store
    ...
    pub(crate) async fn remove_thread(&self, thread_id: &ThreadId) -> Option<Arc<WorkxThread>> {
    ...
    async fn effective_originator(
        ...
        let env_originator = std::env::var(WORKX_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR)
            .is_ok()
    ...
    async fn spawn_thread(&self, request: ThreadSpawnRequest) -> WorkxResult<NewThread> {
        ...
        let windows_sandbox_proxy_settings_mode = if matches!(
            ...
            workx_sandboxing::WindowsSandboxProxySettingsMode::Preserve
        } else {
    ...
    async fn finalize_thread_spawn(
        ...
        let session_configured = match event {
            ...
            _ => {
                return Err(WorkxErr::SessionConfiguredNotFirstEvent);
            }
        };

        {
            ...
            if let std::collections::hash_map::Entry::Vacant(e) = threads.entry(thread_id) {
                let thread = Arc::new(WorkxThread::new(
                    session,
                    io,
        ...
        Err(WorkxErr::InvalidRequest(format!(
    ...
    async fn parent_rollout_thread_trace_for_source(
        ...
        session_source: &SessionSource,
        initial_history: &InitialHistory,
    ) -> workx_rollout_trace::ThreadTraceContext {
        ...
        let SessionSource::SubAgent(SubAgentSource::ThreadSpawn {
            ...
            return workx_rollout_trace::ThreadTraceContext::disabled();
        };
        ...
        self.get_thread(*parent_thread_id)
            ...
            .unwrap_or_else(workx_rollout_trace::ThreadTraceContext::disabled)
    }
}
...
fn thread_store_rollout_read_error(err: ThreadStoreError) -> WorkxErr {
    match err {
        ThreadStoreError::ThreadNotFound { thread_id } => WorkxErr::ThreadNotFound(thread_id),
        ThreadStoreError::InvalidRequest { message } => WorkxErr::InvalidRequest(message),
        err => WorkxErr::Fatal(format!("failed to read thread by rollout path: {err}")),
    }
}

fn thread_store_metadata_update_error(thread_id: ThreadId, err: ThreadStoreError) -> WorkxErr {
    match err {
        ThreadStoreError::ThreadNotFound { thread_id } => WorkxErr::ThreadNotFound(thread_id),
        ThreadStoreError::InvalidRequest { message } => WorkxErr::InvalidRequest(message),
        ThreadStoreError::Unsupported { operation } => WorkxErr::UnsupportedOperation(format!(

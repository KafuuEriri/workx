...
pub use workx_apply_patch::WORKX_APPLY_PATCH_PRESERVE_LINE_ENDINGS_ENV_VAR;
...
pub use workx_protocol::shell_environment::WORKX_SESSION_ID_ENV_VAR;
pub use workx_protocol::shell_environment::WORKX_THREAD_ID_ENV_VAR;
...
pub const WORKX_PERMISSION_PROFILE_ENV_VAR: &str = "WORKX_PERMISSION_PROFILE";
...
///
/// `WORKX_THREAD_ID` is injected when a thread id is provided, even when
/// `include_only` is set.
pub fn create_env(
...
pub(crate) fn inject_session_id_env(env: &mut HashMap<String, String>, session_id: SessionId) {
    env.insert(WORKX_SESSION_ID_ENV_VAR.to_string(), session_id.to_string());
}
...
pub(crate) fn inject_permission_profile_env(
    ...
    if cfg!(windows) {
        env.retain(|key, _| !key.eq_ignore_ascii_case(WORKX_PERMISSION_PROFILE_ENV_VAR));
    } else {
        env.remove(WORKX_PERMISSION_PROFILE_ENV_VAR);
    }
...
pub fn inject_apply_patch_env(env: &mut HashMap<String, String>, features: &Features) {
    env.retain(|key, _| !key.eq_ignore_ascii_case(WORKX_APPLY_PATCH_PRESERVE_LINE_ENDINGS_ENV_VAR));

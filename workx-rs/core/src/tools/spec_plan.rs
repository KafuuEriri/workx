...
use workx_tools::TOOL_SEARCH_TOOL_NAME;
use workx_tools::ToolCall as ExtensionToolCall;
use workx_tools::ToolEnvironmentMode;
use workx_tools::ToolExecutor;
use workx_tools::ToolExposures;
use workx_tools::ToolName;
use workx_tools::ToolSearchInfo;
use workx_tools::ToolSpec;
use workx_tools::UnifiedExecShellMode;
use workx_tools::can_request_original_image_detail;
use workx_tools::collect_code_mode_exec_prompt_tool_definitions;
use workx_tools::collect_request_plugin_install_entries;
use workx_tools::default_namespace_description;
use workx_tools::request_user_input_available_modes;
...
pub(crate) fn finalize_tool_router(
    ...
    if code_mode_enabled {
        for tool_name in [
            ToolName::plain(workx_code_mode::PUBLIC_TOOL_NAME),
            ToolName::plain(workx_code_mode::WAIT_TOOL_NAME),
        ] {
    ...
    if turn_context.config.tool_registry.error_on_tool_collisions {
        if let Some(tool_name) = registry.first_collision() {
            ...
            return Err(WorkxErrorDetails::ToolCollision(name).into());
        }
        ...
        for tool in registry.entries() {
            ...
            if include_tool_namespaces_info && tool.exposure != ToolExposure::Hidden {
                ...
                match namespace_owners.get(namespace_name) {
                    Some(existing_owner) if existing_owner != &owner => {
                        return Err(
                            WorkxErrorDetails::ToolCollision(namespace_name.to_string()).into()
                        );
                    }
                    Some(_) => {}
                    None => {
            ...
            match namespace_descriptions.entry(namespace.name.clone()) {
                ...
                Entry::Occupied(entry) if entry.get() != &namespace.description => {
                    return Err(WorkxErrorDetails::ToolCollision(entry.key().clone()).into());
                }
                Entry::Occupied(_) => {}
            }
        }
...
fn spec_for_model_request(
    ...
    if matches!(tool_mode, ToolMode::CodeMode | ToolMode::CodeModeOnly)
        ...
        && workx_code_mode::is_code_mode_nested_tool(spec.name())
        && code_mode_tool_names
            .get(&workx_code_mode::normalize_code_mode_identifier(
                &workx_tools::code_mode_name_for_tool_name(tool_name),
            ))
            .is_some_and(|winner| winner == tool_name)
    {
        workx_tools::augment_tool_spec_for_code_mode(spec)
    } else {
        spec
    }
}
...
fn is_hidden_by_code_mode_only(
    ...
    tool_mode == ToolMode::CodeModeOnly
        && exposure.is_available_in_code_mode()
        && workx_code_mode::is_code_mode_nested_tool(&workx_tools::code_mode_name_for_tool_name(
            tool_name,
        ))
}
...
fn register_code_mode_executors(
    ...
    for tool in registry.entries() {
        ...
        if !workx_code_mode::is_code_mode_nested_tool(&code_mode_name) {
            continue;
        }
        match code_mode_tool_names.entry(workx_code_mode::normalize_code_mode_identifier(
            &code_mode_name,
        )) {
            Entry::Vacant(entry) => {
    ...
    let execute_handler = CodeModeExecuteHandler::new(
        create_code_mode_tool(
            ...
            if unified_image_budget_enabled(&turn_context.config.features, model_info) {
                workx_code_mode::ImageDetailVisibility::Hidden
            } else {
                workx_code_mode::ImageDetailVisibility::Visible
            },
        ),
...
fn code_mode_namespace_descriptions(
    specs: &[ToolSpec],
) -> BTreeMap<String, workx_code_mode::ToolNamespaceDescription> {
    let mut namespace_descriptions = BTreeMap::new();
    for spec in specs {
        let ToolSpec::Namespace(namespace) = spec else {
            continue;
        ...
        let entry = namespace_descriptions
            ...
            .or_insert_with(|| workx_code_mode::ToolNamespaceDescription {
...
fn compare_code_mode_tools(
    left: &workx_code_mode::ToolDefinition,
    right: &workx_code_mode::ToolDefinition,
...
fn code_mode_namespace_name<'a>(
    tool: &workx_code_mode::ToolDefinition,
    namespace_descriptions: &'a BTreeMap<String, workx_code_mode::ToolNamespaceDescription>,
) -> Option<&'a str> {
    tool.tool_name
        .namespace
        .as_ref()

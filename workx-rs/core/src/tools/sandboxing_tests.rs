...
fn windows_sandbox_env_preserves_denied_reads_or_rejects_unsupported_backend() {
    ...
    let permissions = workx_protocol::models::PermissionProfile::from_runtime_permissions(
        &file_system_policy,
    ...
    let mut attempt = SandboxAttempt {
        ...
        windows_sandbox_level: workx_protocol::config_types::WindowsSandboxLevel::Elevated,
...
fn exec_server_env_keeps_command_native_and_carries_sandbox_context() {
    let cwd: AbsolutePathBuf = std::env::current_dir()
        ...
        .expect("absolute cwd");
    let cwd_uri = PathUri::from_abs_path(&cwd);
    let exec_server_permissions = workx_protocol::models::PermissionProfile::workspace_write();
    ...
    assert_eq!(
        ...
        Some(workx_exec_server::FileSystemSandboxContext {
    ...
    let full_access = workx_protocol::models::PermissionProfile::Disabled;

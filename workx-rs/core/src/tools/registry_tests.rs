...
use workx_protocol::DEFAULT_FUNCTION_NAMESPACE;
use workx_protocol::models::ResponseItem;
use workx_utils_output_truncation::TruncationPolicy;

struct TestHandler {
    tool_name: workx_tools::ToolName,
}

impl ToolExecutor<ToolInvocation> for TestHandler {
    fn tool_name(&self) -> workx_tools::ToolName {
        self.tool_name.clone()
    }

    fn spec(&self) -> workx_tools::ToolSpec {
        test_spec(&self.tool_name)
    }

    fn handle<'a>(&'a self, _invocation: ToolInvocation) -> workx_tools::ToolExecutorFuture<'a>
    where
        ToolInvocation: 'a,
    {
        Box::pin(async {
...
impl ToolExecutor<ToolInvocation> for ReadinessTestHandler {
    ...
    fn handle<'a>(&'a self, invocation: ToolInvocation) -> workx_tools::ToolExecutorFuture<'a>
    where
        ToolInvocation: 'a,
    {
...
fn test_spec(tool_name: &workx_tools::ToolName) -> workx_tools::ToolSpec {
    workx_tools::ToolSpec::Function(workx_tools::ResponsesApiTool {
...
enum RecordedToolLifecycle {
    ...
    Finish {
        call_id: String,
        tool_name: workx_tools::ToolName,
        outcome: workx_extension_api::ToolCallOutcome,
    },
}
...
impl workx_extension_api::ToolLifecycleContributor for ToolLifecycleRecorder {
    fn on_tool_start<'a>(
        &'a self,
        input: workx_extension_api::ToolStartInput<'a>,
    ) -> workx_extension_api::ToolLifecycleFuture<'a> {
    ...
    fn on_tool_finish<'a>(
        &'a self,
        input: workx_extension_api::ToolFinishInput<'a>,
    ) -> workx_extension_api::ToolLifecycleFuture<'a> {
        let records = Arc::clone(&self.records);
...
fn handler_normalizes_only_the_default_namespace() {
    let namespace = "mcp__workx_apps__gmail";
    let tool_name = "gmail_get_recent_emails";
    let plain_name = workx_tools::ToolName::plain(tool_name);
    let namespaced_name = workx_tools::ToolName::namespaced(namespace, tool_name);
    ...
    let default_namespaced = registry.tool(&workx_tools::ToolName::namespaced(
        DEFAULT_FUNCTION_NAMESPACE,
        tool_name,
    ));
    let empty_namespaced = registry.tool(&workx_tools::ToolName::namespaced("", tool_name));
    let namespaced = registry.tool(&namespaced_name);
    let missing_namespaced = registry.tool(&workx_tools::ToolName::namespaced(
        "mcp__workx_apps__calendar",
        tool_name,
    ));
...
fn registry_rejects_default_namespace_alias_collisions() {
    let plain_name = workx_tools::ToolName::plain("lookup");
    let namespaced_name = workx_tools::ToolName::namespaced(DEFAULT_FUNCTION_NAMESPACE, "lookup");
...
fn registry_preserves_external_winners_and_trusted_synthetic_order() {
    ...
    let [first_name, second_name, synthetic_name] =
        ["first", "second", "synthetic"].map(workx_tools::ToolName::plain);
...
fn reserved_command_tools_reject_external_runtimes_without_a_builtin() {
    ...
    for reserved_name in ["exec_command", "shell_command"] {
        let tool_name = workx_tools::ToolName::plain(reserved_name);
        let namespaced_tool_name = workx_tools::ToolName::namespaced("client", reserved_name);
        ...
        assert!(
            !registry.register_external(handler(workx_tools::ToolName::namespaced(
                DEFAULT_FUNCTION_NAMESPACE,
                reserved_name,
            )))
        );
...
fn registry_records_reserved_exec_command_when_a_matching_tool_exists() {
    let tool_name = workx_tools::ToolName::plain("exec_command");
...
fn registry_allows_identical_names_in_different_namespaces() {
    let handler = |tool_name| Arc::new(TestHandler { tool_name }) as Arc<dyn CoreToolRuntime>;
    let mut registry = ToolRegistry::from_tools([handler(workx_tools::ToolName::namespaced(
        "first", "lookup",
    ))]);
...
async fn readiness_selects_exact_tool_with_registry_owned_exposure() {
    ...
    let plain_name = workx_tools::ToolName::plain("echo");
    let namespaced_name = workx_tools::ToolName::namespaced("mcp__server__", "echo");
    assert!(
        TestHandler {
            tool_name: plain_name.clone(),
        }
    ...
    assert!(
        registry
            .tool(&workx_tools::ToolName::namespaced("mcp__missing__", "echo"))
            .is_none()
    );
    assert_eq!(
        [
...
async fn function_tools_expose_default_hook_payloads_and_rewrites() -> anyhow::Result<()> {
    ...
    let tool_name = workx_tools::ToolName::namespaced("functions.", "echo");
...
async fn function_hook_input_defaults_empty_arguments_to_object() {
    let (session, turn) = crate::session::tests::make_session_and_context().await;
    let tool_name = workx_tools::ToolName::plain("echo");
    let handler = TestHandler {
        tool_name: tool_name.clone(),
    };
    let invocation = ToolInvocation {
...
async fn spawn_agent_function_tools_use_agent_matcher_alias() {
    ...
    let hook_payloads = [
        workx_tools::ToolName::plain("spawn_agent"),
        workx_tools::ToolName::namespaced(DEFAULT_FUNCTION_NAMESPACE, "spawn_agent"),
        workx_tools::ToolName::namespaced(MULTI_AGENT_V1_NAMESPACE, "spawn_agent"),
    ]
    .into_iter()
    .map(|tool_name| {
        let handler = TestHandler {
...
fn post_tool_use_feedback_output_preserves_fallback_token_limit_override(
    ...
    let result = AnyToolResult {
        ...
        result: Box::new(PostToolUseFeedbackOutput {
            original: Box::new(crate::tools::context::McpToolOutput {
                result: workx_protocol::mcp::CallToolResult {
                    content: Vec::new(),
    ...
    assert_eq!(
        ...
        ResponseItemEnvelope {
            ...
            metadata: Some(WorkxHarnessMetadata {
...
fn post_tool_use_feedback_output_keeps_code_mode_result_typed() {
    let result = AnyToolResult {
        ...
        result: Box::new(PostToolUseFeedbackOutput {
            original: Box::new(workx_tools::JsonToolOutput::new(
...
async fn dispatch_uses_canonical_tool_names_for_lifecycle_contributors() -> anyhow::Result<()> {
    ...
    let ok_tool = workx_tools::ToolName::plain("ok_tool");
    let failing_tool = workx_tools::ToolName::namespaced("extensions", "failing_tool");
    ...
    registry
        .dispatch_any_with_terminal_outcome(
            test_invocation(
                ...
                Arc::clone(&turn),
                "ok-call",
                workx_tools::ToolName::namespaced(DEFAULT_FUNCTION_NAMESPACE, "ok_tool"),
            ),
    ...
    let expected = vec![
        ...
        RecordedToolLifecycle::Finish {
            ...
            outcome: workx_extension_api::ToolCallOutcome::Completed { success: false },
        },
        RecordedToolLifecycle::Start {
        ...
        RecordedToolLifecycle::Finish {
            ...
            outcome: workx_extension_api::ToolCallOutcome::Failed {

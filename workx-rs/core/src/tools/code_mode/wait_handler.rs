    ...
    async fn handle_call(
        ...
        let result = match payload {
                    ...
                    && tool_name.name.as_str() == WAIT_TOOL_NAME =>
            {
                ...
                let cell_id = workx_code_mode::CellId::new(args.cell_id);
                let wait_response = if args.terminate {
                    ...
                    exec.session
                        .services
                        .code_mode_service
                        .wait(workx_code_mode::WaitRequest {
                            cell_id,
                ...
                if let workx_code_mode::WaitOutcome::LiveCell(response) = &wait_response {
                    let runtime_cell_id = match response {
                        workx_code_mode::RuntimeResponse::Yielded { cell_id, .. }
                        | workx_code_mode::RuntimeResponse::Terminated { cell_id, .. }
                        | workx_code_mode::RuntimeResponse::Result { cell_id, .. } => cell_id,
                    };
                    ...
                    if !matches!(response, workx_code_mode::RuntimeResponse::Yielded { .. }) {
                        exec.session
                            .services
                        ...
                        exec.session
                            ...
                            .track_code_mode_tool_call(
                                workx_analytics::CodeModeToolCallFact::CellClosed {
        ...
        telemetry.finish(
            result
                .as_ref()
                .is_ok_and(workx_tools::ToolOutput::success_for_logging),
        );
        result
    }
}

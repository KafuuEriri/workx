...
use workx_analytics::CodeModeToolCallFact;
use workx_analytics::CodeModeToolCallStatus;
...
impl CodeModeToolCallGuard {
    pub(super) fn new(
        ...
        Self {
            ...
            cell_id: None,
            tool_name,
            started_at_ms: workx_analytics::now_unix_millis(),
    ...
    pub(super) fn record_code_mode_host_duration(&self, duration: Duration) {
        ...
        tracing::info!(
            target: "workx_code_mode::timing",
            {
                event.name = "workx.code_mode.host_timing",
    ...
    fn drop(&mut self) {
        self.analytics
            .track_code_mode_tool_call(CodeModeToolCallFact::Completed {
                ...
                completed_at_ms: workx_analytics::now_unix_millis(),
                status: self.status,
            });
    }
}

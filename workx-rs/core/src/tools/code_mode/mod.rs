...
use workx_code_mode::CellId;
use workx_code_mode::CodeModeNestedToolCall;
use workx_code_mode::CodeModeSession;
use workx_code_mode::CodeModeSessionProvider;
use workx_code_mode::CodeModeToolKind;
use workx_code_mode::RuntimeResponse;
...
use workx_utils_audio::estimate_audio_token_count;
use workx_utils_output_truncation::TruncationPolicy;
use workx_utils_output_truncation::formatted_truncate_text_content_items_with_policy;
use workx_utils_output_truncation::truncate_function_output_items_with_policy;
...
pub(crate) const PUBLIC_TOOL_NAME: &str = workx_code_mode::PUBLIC_TOOL_NAME;
pub(crate) const WAIT_TOOL_NAME: &str = workx_code_mode::WAIT_TOOL_NAME;
pub(crate) const DEFAULT_WAIT_YIELD_TIME_MS: u64 = workx_code_mode::DEFAULT_WAIT_YIELD_TIME_MS;
...
impl CodeModeService {
    ...
    pub(crate) fn take_unavailable_warning(&self, tool_mode: ToolMode) -> Option<String> {
        ...
        .then(|| {
            format!(
                "Code Mode is unavailable because {error}. {behavior}; enable `features.code_mode_host` and install `workx-code-mode-host`."
            )
        })
    }
    ...
    pub(crate) async fn execute(
        &self,
        mut request: workx_code_mode::ExecuteRequest,
    ) -> Result<workx_code_mode::StartedCell, String> {
        request
            .yield_time_ms
    ...
    ) -> Result<workx_code_mode::WaitOutcome, String> {
    ...
    pub(crate) fn mark_cell_ready_for_dispatch(
        ...
        originating_item_id: Option<workx_protocol::ResponseItemId>,
    ) {
        self.dispatch_broker
    ...
    pub(crate) fn cell_originating_item_id(
        &self,
        cell_id: &workx_code_mode::CellId,
    ) -> Option<workx_protocol::ResponseItemId> {
...
> {
    ...
    exec.session
        ...
        .track_code_mode_tool_call(workx_analytics::CodeModeToolCallFact::ChildStarted {

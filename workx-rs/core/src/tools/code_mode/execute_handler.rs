...
impl CodeModeExecuteHandler {
    ...
    async fn execute(
        ...
        let args =
            workx_code_mode::parse_exec_source(&code).map_err(FunctionCallError::RespondToModel)?;
        ...
        for (spec, cached_runtime) in &self.nested_tool_specs {
            ...
            let definitions =
                workx_tools::collect_code_mode_tool_definitions(std::iter::once(spec.as_ref()));
        ...
        let started_cell = exec
            ...
            .services
            .code_mode_service
            .execute(workx_code_mode::ExecuteRequest {
        ...
        exec.session
            ...
            .track_code_mode_tool_call(workx_analytics::CodeModeToolCallFact::CellStarted {
        ...
        if !matches!(response, workx_code_mode::RuntimeResponse::Yielded { .. }) {
            ...
            exec.session
                .services
                .analytics_events_client
                .track_code_mode_tool_call(workx_analytics::CodeModeToolCallFact::CellClosed {
                    thread_id: exec.session.thread_id.to_string(),
                    turn_id: exec.turn.sub_id.clone(),
                    cell_id: cell_id.to_string(),
                });

...
fn guardian_cwd_preserves_drive_shaped_local_posix_path() {
    ...
    assert_eq!(
        guardian_cwd(workx_exec_server::LOCAL_ENVIRONMENT_ID, cwd)
...
fn guardian_cwd_rejects_foreign_remote_path() {
    ...
    assert!(guardian_cwd(workx_exec_server::REMOTE_ENVIRONMENT_ID, cwd).is_err());
}
#[tokio::test]
async fn explicit_mcp_reviewer_override_takes_precedence_over_action_context() {
    ...
    tokio::select! {
        ...
        event = events.recv() => {
            let workx_protocol::protocol::EventMsg::ElicitationRequest(request) =
            ...
            assert_eq!(
                request.id,
                workx_protocol::mcp::RequestId::String(

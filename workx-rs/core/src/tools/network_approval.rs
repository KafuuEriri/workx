...
use tracing::warn;
use uuid::Uuid;
use workx_network_proxy::BlockedRequest;
...
use workx_network_proxy::NetworkRequestDisconnect;
...
use workx_sandboxing::record_network_sandbox_violation;
        ...
        let approval_decision = match session.request_approval(action, approval_context).await {
            ...
            Err(ToolError::Workx(err)) => {
                let telemetry_decision = if matches!(
                    err.details(),
                    workx_protocol::error::WorkxErrorDetails::TurnAborted
                ) {
...
pub(crate) async fn begin_network_approval(
    ...
    let execution_proxy = network
        ...
        .map_err(|err| {
            ToolError::Workx(workx_protocol::error::WorkxErr::Io(io::Error::other(

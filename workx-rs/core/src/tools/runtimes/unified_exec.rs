...
use workx_shell_command::powershell::prefix_powershell_script_with_utf8;
...
pub struct UnifiedExecRequest {
    ...
    pub shell_snapshot: Option<workx_exec_server::ShellSnapshotRequest>,
        ...
        let (mut env, managed_network_context, network_proxy_launch) = match managed_network {
            Some(network) if environment_is_remote => {
                let mut launch = network.remote_launch_config().await.map_err(|err| {
                    ToolError::Workx(WorkxErr::Io(io::Error::other(err.to_string())))
                })?;
                ...
                if !launch.proxy.enabled {
                    ...
                    let environment_info =
                        req.turn_environment
                            ...
                            .await
                            .map_err(|err| {
                                ToolError::Workx(WorkxErr::Io(io::Error::other(format!(

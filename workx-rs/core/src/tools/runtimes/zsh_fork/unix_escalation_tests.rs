...
use workx_execpolicy::PolicyParser;
...
use workx_shell_escalation::EscalationExecution;
use workx_shell_escalation::EscalationPermissions;
use workx_shell_escalation::ResolvedPermissionProfile;
...
async fn preapproved_additional_permissions_escalate_intercepted_exec() -> anyhow::Result<()> {
    ...
    let provider = CoreShellActionProvider {
        policy: Arc::new(RwLock::new(workx_execpolicy::Policy::empty())),
        ...
        stopwatch: workx_shell_escalation::Stopwatch::new(Duration::from_secs(1)),
    };

    let action = workx_shell_escalation::EscalationPolicy::determine_action(
        &provider,
    ...
    let expected = workx_shell_escalation::EscalationDecision::Escalate(
        EscalationExecution::Permissions(EscalationPermissions::ResolvedPermissionProfile(
            ResolvedPermissionProfile { permission_profile },
        )),
    );
...
async fn execve_permission_request_hook_short_circuits_prompt() -> anyhow::Result<()> {
    ...
    std::fs::create_dir_all(&turn_context.config.workx_home)
        .context("recreate workx home for hook fixtures")?;
    ...
    let config_toml_path = turn_context
        .config
        .workx_home
        .join(workx_config::CONFIG_TOML_FILE);
    ...
    let expected_hook_command =
        workx_shell_command::parse_command::shlex_join(&["/usr/bin/touch".to_string(), target_str]);
    ...
    session
        ...
        .await;
    let provider = CoreShellActionProvider {
        policy: std::sync::Arc::new(RwLock::new(workx_execpolicy::Policy::empty())),
    ...
    let action = tokio::time::timeout(
        ...
        workx_shell_escalation::EscalationPolicy::determine_action(
            &provider,
    ...
    assert!(matches!(
        action,
        workx_shell_escalation::EscalationDecision::Escalate(
            workx_shell_escalation::EscalationExecution::Unsandboxed
        )
    ));
...
"#
    ...
    assert_eq!(action, workx_shell_escalation::EscalationDecision::Run);
    Ok(())
}
...
async fn denied_reads_keep_granular_sandbox_rejection_for_escalation() -> anyhow::Result<()> {
    ...
    assert_eq!(
        action,
        workx_shell_escalation::EscalationDecision::Deny {

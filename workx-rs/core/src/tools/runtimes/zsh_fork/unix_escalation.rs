...
use workx_execpolicy::MatchOptions;
...
use workx_protocol::protocol::NetworkPolicyRuleAction;
...
use workx_shell_command::bash::parse_shell_lc_plain_commands;
use workx_shell_escalation::EscalateServer;
use workx_shell_escalation::EscalationDecision;
use workx_shell_escalation::EscalationExecution;
use workx_shell_escalation::EscalationPermissions;
use workx_shell_escalation::EscalationPolicy;
use workx_shell_escalation::EscalationPolicyFuture;
use workx_shell_escalation::EscalationSession;
use workx_shell_escalation::ExecResult;
use workx_shell_escalation::PreparedExec;
use workx_shell_escalation::ResolvedPermissionProfile;
use workx_shell_escalation::ShellCommandExecutor;
use workx_shell_escalation::ShellCommandExecutorFuture;
use workx_shell_escalation::Stopwatch;
...
pub(crate) async fn prepare_unified_exec_zsh_fork(
    ...
    let command_executor = CoreShellCommandExecutor {
        ...
        workx_linux_sandbox_exe: ctx.step_context.turn.config.workx_linux_sandbox_exe.clone(),
...
impl CoreShellActionProvider {
    ...
    async fn prompt(
            ...
            .await
        {
            ...
            Err(ToolError::Workx(err)) => Err(err.into()),
        }
    }
...
struct CoreShellCommandExecutor {
    ...
    network: Option<workx_network_proxy::NetworkProxy>,
...
impl CoreShellCommandExecutor {
    async fn run(
        ...
        for var in ["WORKX_ESCALATE_SOCKET", "EXEC_WRAPPER"] {
    ...
    fn prepare_sandboxed_exec(
        ...
        let exec_request = sandbox_manager.transform(SandboxTransformRequest {
            ...
            workx_linux_sandbox_exe: self.workx_linux_sandbox_exe.as_deref(),

...
*/
use crate::exec_env::WORKX_PERMISSION_PROFILE_ENV_VAR;
use crate::exec_env::WORKX_SESSION_ID_ENV_VAR;
use crate::exec_env::WORKX_THREAD_ID_ENV_VAR;
use crate::sandboxing::SandboxPermissions;
use crate::shell::Shell;
use crate::shell::ShellType;
use std::collections::HashMap;
use std::path::Path;
use workx_apply_patch::WORKX_APPLY_PATCH_PRESERVE_LINE_ENDINGS_ENV_VAR;
use workx_core_plugins::PLUGIN_METRICS_OUTPUT_ENV_VAR;
#[cfg(unix)]
...
pub(crate) use workx_network_proxy::is_managed_proxy_env_var;
pub(crate) use workx_network_proxy::strip_managed_proxy_env;
use workx_protocol::config_types::WindowsSandboxLevel;
use workx_protocol::shell_environment::is_non_inheritable_env_var;
...
pub(crate) fn exec_env_for_sandbox_permissions(
    ...
    // defensive cleanup for a caller that passes an environment already prepared by Workx.
...
/// PATH entries owned by Workx runtime setup.
///
...
pub(crate) fn prepare_powershell_command_for_elevated_windows_sandbox(
    ...
    prepare_powershell_command_for_elevated_windows_sandbox_with_fallback(
        ...
        |path| {
            workx_shell_command::shell_detect::fallback_powershell_shell_for_elevated_windows_sandbox(
                path,
            )
        },
    )
...
fn prepare_powershell_command_for_elevated_windows_sandbox_with_fallback(
    ...
    windows_sandbox_level: WindowsSandboxLevel,
    environment_is_remote: bool,
    find_fallback: impl FnOnce(&Path) -> Option<workx_shell_command::shell_detect::DetectedShell>,
) -> Vec<String> {
...
/// runtime-only vars like `WORKX_THREAD_ID` without pretending they came from
/// the explicit override policy.
///
/// `runtime_path_prepends` contains Workx-owned PATH entries already applied to
...
pub(crate) fn maybe_wrap_shell_lc_with_snapshot(
    ...
    for key in [
        WORKX_SESSION_ID_ENV_VAR,
        WORKX_THREAD_ID_ENV_VAR,
        WORKX_PERMISSION_PROFILE_ENV_VAR,
        WORKX_APPLY_PATCH_PRESERVE_LINE_ENDINGS_ENV_VAR,
...
fn build_override_exports(
    ...
    build_override_exports_for_keys("__WORKX_SNAPSHOT_OVERRIDE", &keys)
}
...
fn build_proxy_env_exports(env: &HashMap<String, String>) -> (String, String) {
    let mut keys = PROXY_ENV_KEYS
        .iter()
        .copied()
        .chain(workx_network_proxy::brokered_credential_env_keys(env))
    ...
    let (captures, restores) =
        build_override_exports_for_keys("__WORKX_SNAPSHOT_PROXY_OVERRIDE", &keys);
    let key = PROXY_ACTIVE_ENV_KEY;
    let proxy_blocks = (
        format!("{captures}\n__WORKX_SNAPSHOT_PROXY_ENV_SET=\"${{{key}+x}}\""),
        format!(
            "if [ -n \"$__WORKX_SNAPSHOT_PROXY_ENV_SET\" ] || [ -n \"${{{key}+x}}\" ]; then\n{restores}\nfi"
        ),
    );
    let git_blocks = build_workx_proxy_git_ssh_command_exports();
    (
...
#[cfg(target_os = "macos")]
fn build_workx_proxy_git_ssh_command_exports() -> (String, String) {
    let key = PROXY_GIT_SSH_COMMAND_ENV_KEY;
    let marker_pattern = format!("{}\\ *", WORKX_PROXY_GIT_SSH_COMMAND_MARKER.trim_end());
    (
        format!(
            "__WORKX_SNAPSHOT_PROXY_GIT_SSH_COMMAND_SET=\"${{{key}+x}}\"\n__WORKX_SNAPSHOT_PROXY_GIT_SSH_COMMAND=\"${{{key}-}}\"\ncase \"$__WORKX_SNAPSHOT_PROXY_GIT_SSH_COMMAND\" in\n  {marker_pattern}) __WORKX_SNAPSHOT_PROXY_GIT_SSH_COMMAND_LIVE_MARKED=1 ;;\n  *) __WORKX_SNAPSHOT_PROXY_GIT_SSH_COMMAND_LIVE_MARKED= ;;\nesac"
        ),
        format!(
            "case \"${{{key}-}}\" in\n  {marker_pattern}) __WORKX_SNAPSHOT_PROXY_GIT_SSH_COMMAND_AFTER_MARKED=1 ;;\n  *) __WORKX_SNAPSHOT_PROXY_GIT_SSH_COMMAND_AFTER_MARKED= ;;\nesac\nif [ -n \"$__WORKX_SNAPSHOT_PROXY_GIT_SSH_COMMAND_LIVE_MARKED\" ]; then\n  if [ -z \"${{{key}+x}}\" ] || [ -n \"$__WORKX_SNAPSHOT_PROXY_GIT_SSH_COMMAND_AFTER_MARKED\" ]; then\n    export {key}=\"$__WORKX_SNAPSHOT_PROXY_GIT_SSH_COMMAND\"\n  fi\nelif [ -n \"$__WORKX_SNAPSHOT_PROXY_GIT_SSH_COMMAND_AFTER_MARKED\" ]; then\n  if [ -n \"$__WORKX_SNAPSHOT_PROXY_GIT_SSH_COMMAND_SET\" ]; then\n    export {key}=\"$__WORKX_SNAPSHOT_PROXY_GIT_SSH_COMMAND\"\n  else\n    unset {key}\n  fi\nfi"
        ),
    )
}
...
mod prepare_powershell_command_tests {
    ...
    fn local_elevated_powershell_uses_discovered_fallback() {
        ...
        let rewritten = prepare_powershell_command_for_elevated_windows_sandbox_with_fallback(
            ...
            /*environment_is_remote*/ false,
            |_| {
                Some(workx_shell_command::shell_detect::DetectedShell {

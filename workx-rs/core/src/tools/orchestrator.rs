...
impl ToolOrchestrator {
        ...
        T: ToolRuntime<Rq, Out>,
    {
        ...
        let attempt_with_network_approval = SandboxAttempt {
            ...
            workx_linux_sandbox_exe: attempt.workx_linux_sandbox_exe,
        ...
        T: ToolRuntime<Rq, Out>,
    {
        ...
        let initial_attempt = SandboxAttempt {
            ...
            workspace_roots,
            workx_linux_sandbox_exe: turn_ctx.config.workx_linux_sandbox_exe.as_ref(),
        ...
        match first_result {
            ...
            Err(ToolError::Workx(err)) => {
                let WorkxErrorDetails::Sandbox(SandboxErr::Denied {
                    output,
                    network_policy_decision,
                }) = err.details()
                else {
                    let err = ToolError::Workx(err);
                ...
                if network_policy_decision.is_some() && network_approval_context.is_none() {
                    ...
                    return Err(ToolError::Workx(err));
                }
                ...
                let retry_workx_linux_sandbox_exe = if unsandboxed_allowed {
                    None
                } else {
                    turn_ctx.config.workx_linux_sandbox_exe.as_ref()
                };
                let retry_attempt = SandboxAttempt {
                    ...
                    sandbox_cwd: &sandbox_policy_cwd,
                    workspace_roots,
                    workx_linux_sandbox_exe: retry_workx_linux_sandbox_exe,
                    use_legacy_landlock: sandbox_config.use_legacy_landlock,
                    windows_sandbox_level: sandbox_config.windows_sandbox_level,
                    windows_sandbox_private_desktop: sandbox_config.windows_sandbox_private_desktop,
                    network_denial_cancellation_token: None,
...
fn sandbox_outcome_from_tool_error(err: &ToolError) -> Option<&'static str> {
    match err {
        ToolError::Workx(err) => match err.details() {
            WorkxErrorDetails::Sandbox(SandboxErr::Denied { .. }) => Some("denied"),
            WorkxErrorDetails::Sandbox(SandboxErr::Timeout { .. }) => Some("timed_out"),
            WorkxErrorDetails::Sandbox(SandboxErr::Signal(_)) => Some("signal"),
            _ => None,
        },

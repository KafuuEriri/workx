...
async fn verified_plugin_install_completed_requires_installed_plugin() {
    ...
    write_curated_plugin_sha(workx_home.path());
    write_plugins_feature_config(workx_home.path());

    let config = load_plugins_config(workx_home.path()).await;
    let plugins_manager =
    ...
    let refreshed_config = load_plugins_config(workx_home.path()).await;
    assert!(verified_plugin_install_completed(
        "sample@openai-api-curated",
        &refreshed_config,
        &plugins_manager,
...
async fn persist_disabled_install_request_writes_connector_config() {
    ...
    persist_disabled_install_request(&workx_home.path().abs(), &tool)
        .await
        .expect("persist connector disable");

    let contents =
        std::fs::read_to_string(workx_home.path().join(CONFIG_TOML_FILE)).expect("read config");

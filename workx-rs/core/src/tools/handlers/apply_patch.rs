...
use workx_apply_patch::ApplyPatchAction;
use workx_apply_patch::ApplyPatchFileChange;
use workx_apply_patch::ApplyPatchFileUpdateMode;
use workx_apply_patch::Hunk;
use workx_apply_patch::StreamingPatchParser;
...
use workx_protocol::protocol::PatchApplyUpdatedEvent;
use workx_sandboxing::policy_transforms::effective_file_system_sandbox_policy;
use workx_sandboxing::policy_transforms::merge_permission_profiles;
use workx_sandboxing::policy_transforms::normalize_additional_permissions;
...
fn format_update_chunks_for_progress(chunks: &[workx_apply_patch::UpdateFileChunk]) -> String {
...
fn write_permissions_for_paths(
    ...
    file_system_sandbox_policy: &workx_protocol::permissions::FileSystemSandboxPolicy,
    cwd: &AbsolutePathBuf,
...
async fn effective_patch_permissions(
    ...
    workx_protocol::permissions::FileSystemSandboxPolicy,
)> {
...
fn patch_permissions_without_path_matching(
    ...
    (
        ...
        workx_protocol::permissions::FileSystemSandboxPolicy::unrestricted(),
    )
}
...
impl ApplyPatchHandler {
    async fn handle_call(
        ...
        let args = match workx_apply_patch::parse_patch(&patch_input) {
            Ok(args) => args,
            Err(parse_error) => {
        ...
        match workx_apply_patch::verify_apply_patch_args_with_mode(
            args,
        ...
        .await
        {
            workx_apply_patch::MaybeApplyPatchVerified::Body(changes) => {
            ...
            workx_apply_patch::MaybeApplyPatchVerified::CorrectnessError(parse_error) => {
                Err(FunctionCallError::RespondToModel(format!(
                    "apply_patch verification failed: {parse_error}"
                )))
            }
            workx_apply_patch::MaybeApplyPatchVerified::ShellParseError(error) => {
                tracing::trace!("Failed to parse apply_patch input, {error:?}");
                Err(FunctionCallError::RespondToModel(
                    "apply_patch handler received invalid patch input".to_string(),
                ))
            }
            workx_apply_patch::MaybeApplyPatchVerified::NotApplyPatch => {
                Err(FunctionCallError::RespondToModel(
...
pub(crate) async fn intercept_apply_patch(
    ...
    let sandbox = turn_environment.sandbox_context(/*additional_permissions*/ None);
    match workx_apply_patch::maybe_parse_apply_patch_verified_with_mode(
        command,
        cwd,
        apply_patch_file_update_mode(turn),
        fs,
    ...
    .await
    {
        ...
        workx_apply_patch::MaybeApplyPatchVerified::NotApplyPatch => Ok(None),
    }
}

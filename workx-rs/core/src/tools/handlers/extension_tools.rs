...
use workx_tools::ConversationHistory;
use workx_tools::ExtensionTurnItem;
use workx_tools::ResponsesApiNamespaceTool;
use workx_tools::ToolCall as ExtensionToolCall;
use workx_tools::ToolEnvironment;
use workx_tools::ToolName;
use workx_tools::ToolSearchInfo;
use workx_tools::ToolSpec;
use workx_tools::TurnItemEmissionFuture;
use workx_tools::TurnItemEmitter;
...
pub(crate) struct ExtensionToolAdapter(
    Arc<dyn for<'call> workx_tools::ToolExecutor<ExtensionToolCall<'call>>>,
);

impl ExtensionToolAdapter {
    pub(crate) fn new(
        executor: Arc<dyn for<'call> workx_tools::ToolExecutor<ExtensionToolCall<'call>>>,
    ) -> Self {
        Self(executor)
    }
}
...
async fn to_extension_call(invocation: &ToolInvocation) -> ExtensionToolCall<'_> {
    ...
    let workx_turn_metadata = invocation
        .turn
        .turn_metadata_state
...
mod tests {
    ...
    use workx_extension_items::web_search::WebSearchItem;
    use workx_protocol::items::TurnItem;
    ...
    use workx_tools::ExtensionTurnItem;
    use workx_tools::ToolCallSource as ExtensionToolCallSource;
    ...
    impl<'call> workx_extension_api::ToolExecutor<workx_tools::ToolCall<'call>>
        for StubExtensionExecutor
    {
        fn tool_name(&self) -> workx_tools::ToolName {
            workx_tools::ToolName::plain("extension_echo")
        }
        ...
        fn spec(&self) -> workx_tools::ToolSpec {
            workx_tools::ToolSpec::Function(workx_tools::ResponsesApiTool {
                ...
                strict: true,
                parameters: workx_tools::parse_tool_input_schema(&json!({
                    "type": "object",
                    "properties": {
                        "message": { "type": "string" },
                    },
        ...
        fn handle<'a>(
            &'a self,
            _call: workx_tools::ToolCall<'call>,
        ) -> workx_tools::ToolExecutorFuture<'a>
        where
            'call: 'a,
        {
            Box::pin(async {
                Ok(
                    Box::new(workx_tools::JsonToolOutput::new(json!({ "ok": true })))
                        as Box<dyn workx_tools::ToolOutput>,
                )
            })
        }
    }
    ...
    struct CapturingExtensionExecutor {
        captured_call: Arc<Mutex<Option<workx_tools::ToolCall<'static>>>>,
        ...
        for CapturingExtensionExecutor
    {
        ...
        fn handle<'a>(
            &'a self,
            call: workx_tools::ToolCall<'call>,
        ) -> workx_tools::ToolExecutorFuture<'a>
        where
            'call: 'a,
        {
    ...
    impl CapturingExtensionExecutor {
        async fn handle_call(
            &self,
            call: workx_tools::ToolCall<'_>,
        ) -> Result<Box<dyn workx_tools::ToolOutput>, workx_tools::FunctionCallError> {
    ...
    async fn exposes_generic_hook_payloads() {
        ...
        let invocation = ToolInvocation {
            ...
            tool_name: workx_tools::ToolName::plain("extension_echo"),
            source: ToolCallSource::Direct,
            payload: ToolPayload::Function {
                arguments: json!({ "message": "hello" }).to_string(),
            },
        };
        let output = workx_tools::JsonToolOutput::new(json!({ "ok": true }));
    ...
    async fn image_generation_publication_preserves_extension_saved_path() {
        ...
        workx_tools::TurnItemEmitter::emit_started(
            &emitter,
            ExtensionTurnItem {
        ...
        .await;
        workx_tools::TurnItemEmitter::emit_completed(
            &emitter,
            ExtensionTurnItem {

...
use workx_protocol::protocol::FileSystemAccessMode;
use workx_protocol::protocol::FileSystemPath;
use workx_protocol::protocol::FileSystemSandboxEntry;
use workx_protocol::protocol::FileSystemSandboxPolicy;
use workx_protocol::protocol::InterAgentCommunication;
use workx_protocol::protocol::ItemCompletedEvent;
use workx_protocol::protocol::NetworkSandboxPolicy;
...
fn invocation(
    ...
    ToolInvocation {
        ...
        tool_name: workx_tools::ToolName::plain(tool_name),
...
async fn wait_for_recorded_user_input(thread: &crate::WorkxThread, expected: &[UserInput]) {
...
async fn install_role_with_model_override(turn: &mut TurnContext) -> String {
    ...
    tokio::fs::create_dir_all(&turn.config.workx_home)
        .await
        .expect("workx home should be created");
        ...
        | ResponseInputItem::CustomToolCallOutput { output, .. } => {
            let content = match output.body {
                ...
                FunctionCallOutputBody::ContentItems(items) => {
                    workx_protocol::models::function_call_output_content_items_to_text(&items)
                        .unwrap_or_default()
                }
            };
...
async fn multi_agent_v2_spawn_fork_turns_all_applies_agent_type_override() {
    ...
    let mut turn = turn;
    turn.config = Arc::new(config);
    turn.multi_agent_version = workx_protocol::protocol::MultiAgentVersion::V2;
...
async fn spawn_agent_role_service_tier_cannot_override_root_preference() {
    ...
    tokio::fs::create_dir_all(&turn.config.workx_home)
        .await
        .expect("workx home should be created");
    let role_config_path = turn.config.workx_home.as_path().join("tiered-role.toml");
    tokio::fs::write(
        &role_config_path,
        r#"model = "gpt-5.4"
service_tier = "turbo"
...
async fn spawn_agent_reapplies_runtime_sandbox_after_role_config() {
    ...
    let sandbox_runtime = core_test_support::test_workx::test_workx()
    ...
    let manager = ThreadManager::with_models_provider_and_home_for_tests(
        ...
        turn.config.workx_home.to_path_buf(),
...
async fn resume_agent_restores_closed_agent_and_accepts_send_input() {
    ...
    let thread = manager
        .resume_thread_with_history(
            ...
            AuthManager::from_auth_for_testing(WorkxAuth::from_api_key("dummy")),
            /*parent_trace*/ None,
...
async fn build_agent_spawn_config_uses_turn_context_values() {
    ...
    config.workx_linux_sandbox_exe = Some(PathBuf::from("/bin/echo"));
    turn.config = Arc::new(config);

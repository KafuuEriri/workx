...
use tracing::warn;
use workx_analytics::PluginInstallRequestSource;
use workx_analytics::PluginInstallRequested;
use workx_analytics::PluginInstallRequestedPlugin;
...
use workx_tools::DiscoverableToolAction;
use workx_tools::DiscoverableToolType;
use workx_tools::LIST_AVAILABLE_PLUGINS_TO_INSTALL_TOOL_NAME;
use workx_tools::REQUEST_PLUGIN_INSTALL_PERSIST_ALWAYS_VALUE;
use workx_tools::REQUEST_PLUGIN_INSTALL_PERSIST_KEY;
use workx_tools::REQUEST_PLUGIN_INSTALL_TOOL_NAME;
use workx_tools::RequestPluginInstallArgs;
use workx_tools::RequestPluginInstallResult;
use workx_tools::ToolName;
use workx_tools::ToolSpec;
use workx_tools::all_requested_connectors_picked_up;
use workx_tools::build_request_plugin_install_elicitation_request;
use workx_tools::filter_request_plugin_install_discoverable_tools_for_client;
use workx_tools::verified_connector_install_completed;
...
impl RequestPluginInstallHandler {
    async fn handle_call(
        ...
        if (requested_tool_type == Some(DiscoverableToolType::Plugin)
            ...
            && turn.app_server_client_name.as_deref() == Some("workx-tui")
        {
            return Err(FunctionCallError::RespondToModel(
                "plugin install requests are not available in workx-tui yet".to_string(),
            ));
        }
        ...
        let tool = if self.presentation == ToolSuggestPresentation::RecommendationContext {
            ...
            match workx_core_plugins::hydrate_selected_recommended_plugin_install_metadata(
                &plugins_config,
                auth.as_ref(),
                tool,
            )
...
async fn maybe_persist_disabled_install_request(
    ...
    if let Err(err) = persist_disabled_install_request(&turn.config.workx_home, tool).await {
        warn!(
            error = %err,
            tool_id = tool.id(),
...
async fn persist_disabled_install_request(
    workx_home: &workx_utils_absolute_path::AbsolutePathBuf,
    tool: &DiscoverableTool,
) -> anyhow::Result<()> {
...
async fn verify_request_plugin_install_completed(
    ...
    tool: &DiscoverableTool,
    auth: Option<&workx_login::WorkxAuth>,
) -> bool {
    match tool {
...
async fn refresh_missing_requested_connectors(
    ...
    match session.hard_refresh_latest_workx_apps_tools().await {
        Ok(mcp_tools) => {
        ...
        Err(err) => {
            warn!(
                "failed to refresh workx apps tools cache after plugin install request for {tool_id}: {err:#}"
            );
            None
        }
    }

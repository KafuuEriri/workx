...
use workx_tools::ToolSearchSourceInfo;
use workx_tools::ToolSpec;
use workx_tools::agent_plugin_mcp_tool_to_responses_api_tool;
use workx_tools::mcp_tool_to_responses_api_tool;
use workx_utils_image::PromptImageMode;
use workx_utils_image::load_data_url_for_prompt_uncached;
...
pub struct McpHandler {
    tool_info: ToolInfo,
    spec: Arc<ToolSpec>,
    code_mode_tool_definitions: OnceLock<Vec<workx_code_mode::ToolDefinition>>,
}
...
impl McpHandler {
    async fn handle_call(
        ...
        let truncation_policy = prepared_mcp_call
            .as_ref()
            .and_then(workx_mcp::PreparedMcpCall::output_token_limit)
...
impl CoreToolRuntime for McpHandler {
    ...
    fn cached_code_mode_definitions(&self) -> Option<&[workx_code_mode::ToolDefinition]> {
        Some(
            self.code_mode_tool_definitions
                .get_or_init(|| {
                    let mut definitions = workx_tools::collect_code_mode_tool_definitions(
    ...
    fn on_tool_result_accepted(&self, invocation: &ToolInvocation, result: &dyn ToolOutput) {
        ...
        let is_encrypted = |item: &Value| {
            item.get("_meta")
                .and_then(|meta| meta.get("workx/encryptedContent"))
...
mod tests {
    ...
    async fn mcp_pre_tool_use_payload_uses_prefixed_tool_name_and_raw_args() {
        ...
        assert_eq!(
            handler.pre_tool_use_payload(&ToolInvocation {
                ...
                tool_name: workx_tools::ToolName::namespaced("memory", "create_entities"),
    ...
    async fn mcp_pre_tool_use_payload_keeps_builtin_like_tool_names_namespaced() {
        ...
        assert_eq!(
            handler.pre_tool_use_payload(&ToolInvocation {
                ...
                tool_name: workx_tools::ToolName::namespaced("mcp__foo", "exec_command"),
                source: ToolCallSource::Direct,
                payload,
            }),
            Some(PreToolUsePayload {
    ...
    async fn mcp_post_tool_use_payload_uses_prefixed_tool_name_args_and_result() {
        ...
        let output = McpToolOutput {
            ...
            truncation_policy: workx_utils_output_truncation::TruncationPolicy::Bytes(1024),
        };
        let (session, turn) = make_session_and_context().await;
        let turn = Arc::new(turn);
        ...
        let invocation = ToolInvocation {
            ...
            tool_name: workx_tools::ToolName::namespaced("filesystem", "read_file"),
            source: ToolCallSource::Direct,
            payload,
        };
        assert_eq!(

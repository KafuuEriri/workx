...
use workx_apply_patch::MaybeApplyPatchVerified;
...
async fn invocation_for_payload(payload: ToolPayload) -> ToolInvocation {
    ...
    ToolInvocation {
        ...
        tool_name: workx_tools::ToolName::plain("apply_patch"),
...
async fn file_update_mode_follows_preserve_line_endings_feature() {
    ...
    assert_eq!(
        ...
        workx_apply_patch::ApplyPatchFileUpdateMode::NormalizeToLf
    );

    Arc::make_mut(&mut turn.config)
        .features
        .enable(workx_features::Feature::ApplyPatchPreserveLineEndings)
        .expect("feature should be enabled");
    assert_eq!(
        apply_patch_file_update_mode(&turn),
        workx_apply_patch::ApplyPatchFileUpdateMode::PreserveLineEndings
    );
}
...
*** End Patch"#;
    ...
    let action = match workx_apply_patch::maybe_parse_apply_patch_verified(
        &argv,
        &cwd,
        LOCAL_FS.as_ref(),
        /*sandbox*/ None,

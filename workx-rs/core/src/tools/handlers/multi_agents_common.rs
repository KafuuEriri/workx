...
pub(crate) fn collab_spawn_error(err: WorkxErr) -> FunctionCallError {
    match err.details() {
        WorkxErrorDetails::UnsupportedOperation(message) if message == "thread manager dropped" => {
...
pub(crate) fn collab_agent_error(agent_id: ThreadId, err: WorkxErr) -> FunctionCallError {
    match err.details() {
        WorkxErrorDetails::ThreadNotFound(id) => {

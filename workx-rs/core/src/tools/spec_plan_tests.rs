...
use workx_model_provider_info::AMAZON_BEDROCK_GPT_5_6_LUNA_MODEL_ID;
use workx_model_provider_info::AMAZON_BEDROCK_GPT_5_6_SOL_MODEL_ID;
...
use workx_protocol::openai_models::ApplyPatchToolType;
...
use workx_tools::ResponsesApiTool;
use workx_tools::ToolCall as ExtensionToolCall;
use workx_tools::ToolExecutor;
use workx_tools::ToolExposure;
...
fn plan_with_model(
    ...
    let mcp = workx_mcp::McpBinding::empty(mcp_config_for_test(&turn.config));
...
fn zsh_fork_config_for_spec_plan_tests() -> workx_tools::ZshForkConfig {
    let placeholder_exe = workx_utils_absolute_path::AbsolutePathBuf::try_from(
...
impl<'call> ToolExecutor<ExtensionToolCall<'call>> for TestNamespaceExtensionTool {
    ...
    fn spec(&self) -> ToolSpec {
        ToolSpec::Namespace(workx_tools::ResponsesApiNamespace {
            ...
            tools: vec![ResponsesApiNamespaceTool::Function(ResponsesApiTool {
                ...
                strict: false,
                defer_loading: None,
                parameters: workx_tools::JsonSchema::default(),
                output_schema: None,
            })],
        })
    }

    fn handle<'a>(&'a self, _call: ExtensionToolCall<'call>) -> workx_tools::ToolExecutorFuture<'a>
    where
        'call: 'a,
    {
        Box::pin(async {
            Ok(Box::new(workx_tools::JsonToolOutput::new(json!({}))) as Box<dyn ToolOutput>)
        })
    }
}
...
impl<'call> ToolExecutor<ExtensionToolCall<'call>> for DeferredExtensionTool {
    ...
    fn spec(&self) -> ToolSpec {
        ToolSpec::Function(ResponsesApiTool {
            ...
            strict: true,
            defer_loading: None,
            parameters: workx_tools::JsonSchema::object(
                BTreeMap::from([(
                    "message".to_string(),
                    workx_tools::JsonSchema::string(/*description*/ None),
                )]),
...
fn dynamic_tool(namespace: Option<&str>, name: &str, defer_loading: bool) -> DynamicToolSpec {
    let function = workx_protocol::dynamic_tools::DynamicToolFunctionSpec {
        name: name.to_string(),
    ...
    match namespace {
        Some(namespace) => {
            DynamicToolSpec::Namespace(workx_protocol::dynamic_tools::DynamicToolNamespaceSpec {
                name: namespace.to_string(),
                description: format!("{namespace} dynamic tools"),
                tools: vec![
                    workx_protocol::dynamic_tools::DynamicToolNamespaceTool::Function(function),
                ],
            })
        }
    ...
    for (disabled_feature, shell_type) in [
        ...
        if let Some(feature) = disabled_feature {
            ...
            config.features = crate::config::ManagedFeatures::from_configured(
                ...
                Some(workx_config::Sourced::new(
                    workx_config::FeatureRequirementsToml {
        ...
        assert_eq!(
            router
                ...
                .collect::<Vec<_>>(),
            vec![
                workx_code_mode::PUBLIC_TOOL_NAME,
                workx_code_mode::WAIT_TOOL_NAME,
                "view_image",
            ],
...
async fn internal_guardian_sessions_preserve_code_mode() {
    ...
    assert!(
        router
            .model_visible_specs()
            .iter()
            .any(|tool| tool.name() == workx_code_mode::PUBLIC_TOOL_NAME)
    );
    assert!(
        router
            .model_visible_specs()
            .iter()
            .any(|tool| tool.name() == workx_code_mode::WAIT_TOOL_NAME)
    );
}
...
async fn internal_guardian_sessions_require_managed_secondary_environments() {
    for (secondary_profile, expected_tools) in [
        (
            workx_protocol::models::PermissionProfile::workspace_write(),
            vec!["exec_command", "write_stdin", "view_image"],
        ),
        (
        ...
        primary.config_mut().permission_profile =
            workx_protocol::models::PermissionProfileSnapshot::legacy(
                workx_protocol::models::PermissionProfile::workspace_write(),
            );
        duplicate_primary_environment(&mut turn);
        let secondary_workspace_root =
            workx_utils_path_uri::PathUri::from_abs_path(&turn.config.cwd.join("secondary"));
        ...
        secondary.config_mut().permission_profile =
            workx_protocol::models::PermissionProfileSnapshot::legacy(secondary_profile);
        let turn = Arc::new(turn);
        let step_context = StepContext::for_test(Arc::clone(&turn));
...
async fn request_user_input_stays_direct_in_code_mode_only() {
    ...
    let ToolSpec::Freeform(exec) = plan.visible_spec(workx_code_mode::PUBLIC_TOOL_NAME) else {
    ...
    for guardian in [false, true] {
        for allow_login_shell in [false, true] {
            let plan = probe(|turn| {
                ...
                if guardian {
                    turn.session_source = workx_protocol::protocol::SessionSource::SubAgent(
                        workx_protocol::protocol::SubAgentSource::Other(
...
async fn disabling_shell_tools_disables_command_tools_for_all_environments() {
    let remote_environment = probe(|turn| {
        ...
        environment.environment = Arc::new(
            workx_exec_server::Environment::create_for_tests(Some(
...
async fn zsh_fork_unified_exec_hides_shell_parameter() {
    if !workx_utils_pty::conpty_supported() {
        return;
    }
    ...
    let plan = probe(|turn| {
        ...
        turn.unified_exec_shell_mode =
            workx_tools::UnifiedExecShellMode::ZshFork(zsh_fork_config_for_spec_plan_tests());
    })
    .await;
                ...
                crate::session::turn_context::TurnEnvironment::new(
                    TurnEnvironmentSelection {
                        ...
                        config: EnvironmentConfigState::Ready(
                            workx_protocol::protocol::EnvironmentConfig {
...
async fn environment_tools_follow_the_step_context() {
    ...
    turn.environments.environments.clear();
    let turn = Arc::new(turn);
    let mcp = Arc::new(workx_mcp::McpBinding::empty(mcp_config_for_test(
        &turn.config,
    )));
...
async fn candidate_model_plan_leaves_selected_model_and_inventory_unchanged() {
    ...
    let metadata = turn.turn_metadata_state.to_responses_metadata(
        ...
        "window".to_string(),
        WorkxResponsesRequestKind::Turn,
    );
...
async fn strict_namespace_ownership_requires_tool_namespace_inventory_opt_in() {
    for (enabled, second_exposure) in [
        ...
        if enabled && second_exposure != ToolExposure::Hidden {
            ...
            assert!(matches!(
                error.details(),
                WorkxErrorDetails::ToolCollision(name) if name == "shared"
            ));
        } else {
...
async fn strict_tool_collisions_reject_external_and_synthetic_duplicates() {
    let cases = [
        ...
        (
            ...
            ToolPlanInputs {
                ...
                dynamic_tools: vec![dynamic_tool(
                    /*namespace*/ None,
                    workx_tools::TOOL_SEARCH_TOOL_NAME,
                    /*defer_loading*/ false,
                )],
                ..ToolPlanInputs::default()
            },
        ...
        cases.into_iter().chain(namespace_cases)
    {
        ...
        assert!(matches!(
            error.details(),
            WorkxErrorDetails::ToolCollision(name) if name == expected_name
        ));
        assert_eq!(
            error.to_string(),
...
async fn code_mode_only_exposes_code_executor_and_hides_nested_tools() {
    ...
    assert_eq!(
        plain.namespace_function_names("workx_app"),
        &["lookup".to_string()]
    );
    ...
    assert_eq!(
        code_mode_only.namespace_function_names("workx_app"),

...
use workx_extension_api::ResponsesApiTool;
use workx_extension_api::ToolCall as ExtensionToolCall;
...
impl workx_extension_api::ToolContributor for ExtensionEchoContributor {
    fn tools(
        &self,
...
impl<'call> ToolExecutor<ExtensionToolCall<'call>> for ExtensionEchoExecutor {
    ...
    fn spec(&self) -> ToolSpec {
        ToolSpec::Namespace(ResponsesApiNamespace {
            ...
            tools: vec![ResponsesApiNamespaceTool::Function(ResponsesApiTool {
                ...
                strict: true,
                parameters: workx_extension_api::parse_tool_input_schema(&json!({
                    "type": "object",
                    "properties": {
                        "message": { "type": "string" },
                    },
    ...
    fn handle<'a>(&'a self, call: ExtensionToolCall<'call>) -> workx_tools::ToolExecutorFuture<'a>
    where
        'call: 'a,
    {
...
impl ExtensionEchoExecutor {
    async fn handle_call(
        ...
        Ok(Box::new(workx_tools::JsonToolOutput::new(json!({
            "arguments": arguments,
            "callId": call.call_id,
...
async fn build_tool_call_uses_namespace_for_registry_name() -> anyhow::Result<()> {
    ...
    let call = ToolRouter::build_tool_call(ResponseItem::FunctionCall {
        ...
        namespace: Some("mcp__workx_apps__calendar".to_string()),
        arguments: "{}".to_string(),
    ...
    assert_eq!(
        call.tool_name,
        ToolName::namespaced("mcp__workx_apps__calendar", tool_name)
    );
...
async fn specs_filter_deferred_dynamic_tools() -> anyhow::Result<()> {
    ...
    assert_eq!(
        namespace_function_names(&visible_specs, "workx_app"),
        vec![visible_tool.to_string()]
    );
    assert_eq!(
        router.deferred_tool_namespaces(),
        BTreeMap::from([("workx_app".to_string(), "Workx app tools.".to_string())])
    );
    ...
    assert!(namespace_function_names(&updated_specs, "workx_app").is_empty());
...
fn mcp_runtime(tool_info: workx_mcp::ToolInfo) -> RegisteredTool {
...
async fn extension_tool_executors_are_model_visible_and_dispatchable() -> anyhow::Result<()> {
    ...
    let router = test_tool_router(
        ...
        extension_tool_executors(
            &session,
            &workx_extension_api::ExtensionData::new(turn.sub_id.clone()),
        ),
        &turn.dynamic_tools,
    );

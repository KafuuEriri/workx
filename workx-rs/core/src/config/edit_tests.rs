...
fn keymap_bindings_edit_writes_multiple_bindings_as_array() {
    ...
    let raw = std::fs::read_to_string(workx_home.join(CONFIG_TOML_FILE)).expect("read config");
...
fn blocking_set_model_writes_through_symlink_chain() {
    ...
    let link_path = workx_home.join("config-link.toml");
    let config_path = workx_home.join(CONFIG_TOML_FILE);
...
fn blocking_set_model_replaces_symlink_on_cycle() {
    let tmp = tempdir().expect("tmpdir");
    let workx_home = tmp.path();
    let link_a = workx_home.join("a.toml");
    let link_b = workx_home.join("b.toml");
    let config_path = workx_home.join(CONFIG_TOML_FILE);
...
"#;
    std::fs::write(workx_home.join(CONFIG_TOML_FILE), original).expect("seed config");
    ...
    let updated = std::fs::read_to_string(workx_home.join(CONFIG_TOML_FILE)).expect("read config");
...
#[test]
fn blocking_set_hide_gpt_5_1_workx_max_migration_prompt_preserves_table() {
    let tmp = tempdir().expect("tmpdir");
    let workx_home = tmp.path();
    std::fs::write(
        workx_home.join(CONFIG_TOML_FILE),
...
"#,
    ...
    apply_blocking(
        workx_home,
        &[ConfigEdit::SetNoticeHideModelMigrationPrompt(
            "hide_gpt-5.1-workx-max_migration_prompt".to_string(),
            true,
        )],
    )
    .expect("persist");
    ...
    let expected = r#"[notice]
existing = "value"
"hide_gpt-5.1-workx-max_migration_prompt" = true
"#;
    assert_eq!(contents, expected);
}
...
client_id = \"eci-prd-pub-workx-123\"
...
fn blocking_replace_mcp_servers_serializes_tool_approval_overrides(output_token_limit: usize) {
    ...
    let result = apply_blocking(workx_home, &[ConfigEdit::ReplaceMcpServers(servers)]);
...
"#,
    ...
    apply_blocking(workx_home, &[ConfigEdit::ReplaceMcpServers(servers)]).expect("persist");
...
fn blocking_clear_path_noop_when_missing() {
    ...
    assert!(
        !workx_home.join(CONFIG_TOML_FILE).exists(),
...
"#;
    ...
    let mut contents =
        std::fs::read_to_string(workx_home.join(CONFIG_TOML_FILE)).expect("read config");
...
"#;
    ...
    contents = std::fs::read_to_string(workx_home.join(CONFIG_TOML_FILE)).expect("read config");

...
async fn restricted_read_implicitly_allows_helper_executables() -> std::io::Result<()> {
    ...
    let workx_home = temp_dir.path().join(".workx");
    let zsh_path = temp_dir.path().join("runtime").join("zsh");
    let arg0_root = workx_home.join("tmp").join("arg0");
    let allowed_arg0_dir = arg0_root.join("workx-arg0-session");
    let sibling_arg0_dir = arg0_root.join("workx-arg0-other-session");
    let execve_wrapper = allowed_arg0_dir.join("workx-execve-wrapper");
...
fn network_toml_overlays_unix_socket_permissions_by_path() {
    ...
    assert_eq!(
        config.unix_sockets,
        Some(workx_network_proxy::NetworkUnixSocketPermissions {
            entries: BTreeMap::from([
                (
                    "/tmp/base.sock".to_string(),
...
fn profile_network_proxy_config_keeps_proxy_disabled_for_proxy_policy() {
    ...
    assert_eq!(
        config.domains,
        Some(workx_network_proxy::NetworkDomainPermissions {
            entries: vec![workx_network_proxy::NetworkDomainPermissionEntry {
                pattern: "openai.com".to_string(),
                permission: workx_network_proxy::NetworkDomainPermission::Allow,
            }],
        })
    );
}

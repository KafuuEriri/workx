...
use workx_config::ConfigError;
use workx_config::ConfigLayerEntry;
use workx_config::ConfigLayerSource;
use workx_config::ConfigLoadError;
use workx_config::ConfigLoadOptions;
use workx_config::ConfigRequirements;
use workx_config::ConfigRequirementsToml;
use workx_config::ConfigRequirementsWithSources;
use workx_config::FilesystemDenyReadPattern;
use workx_config::LoaderOverrides;
use workx_config::RequirementSource;
use workx_config::RequirementsLayerEntry;
use workx_config::SessionThreadConfig;
use workx_config::StaticThreadConfigLoader;
use workx_config::ThreadConfigSource;
use workx_config::compose_requirements;
use workx_config::config_error_from_ignored_toml_fields;
use workx_config::config_error_from_toml;
use workx_config::config_error_from_typed_toml;
...
use workx_config::loader::load_requirements_toml;
...
use workx_protocol::config_types::EnvironmentVariablePattern;
...
async fn write_project_hook_config(
    dot_workx_folder: &Path,
    foo: Option<&str>,
    command: &str,
) -> std::io::Result<()> {
    tokio::fs::create_dir_all(dot_workx_folder).await?;
    let foo = foo
        .map(|value| format!("foo = \"{value}\"\n\n"))
        .unwrap_or_default();
    tokio::fs::write(
        dot_workx_folder.join(CONFIG_TOML_FILE),
        format!(
            r#"{foo}[hooks]
...
invalid = ["#,
    ...
    let user_layer = layers
        ...
        .expect("expected a user layer even when WORKX_HOME/config.toml is ignored");
    assert_eq!(
        user_layer.config,
...
async fn ignore_project_config_skips_project_discovery() -> std::io::Result<()> {
    let tmp = tempdir().expect("tempdir");
    let workx_home = tmp.path().join("home");
    let workspace = tmp.path().join("workspace");
    tokio::fs::create_dir_all(workx_home.as_path()).await?;
    ...
    .await?;
    let project_config_dir = workspace.join(".workx");
...
async fn returns_config_error_for_schema_error_in_user_config() {
    ...
    let err = ConfigBuilder::default()
        .workx_home(tmp.path().to_path_buf())
    ...
    let config_error = config_error_from_io(&err);
    let _guard = workx_utils_absolute_path::AbsolutePathBufGuard::new(tmp.path());
    let expected_config_error =
        workx_config::config_error_from_typed_toml::<ConfigToml>(&config_path, contents)
            .expect("schema error");
    assert_eq!(config_error, &expected_config_error);
}
...
collaboration_modes = "true""#;
    ...
    let _guard = workx_utils_absolute_path::AbsolutePathBufGuard::new(tmp.path());
    let error = workx_config::config_error_from_typed_toml::<ConfigToml>(&config_path, contents)
        .expect("schema error");
...
async fn returns_packaged_defaults_when_other_layers_are_missing() {
    ...
    let user_layer = layers
        .get_active_user_layer()
        .expect("expected a user layer even when WORKX_HOME/config.toml does not exist");
...
#[cfg(target_os = "macos")]
#[tokio::test]
async fn managed_preferences_requirements_resolve_paths_against_workx_home() -> anyhow::Result<()> {
    use base64::Engine;

    let tmp = tempdir()?;
    let workx_home = tmp.path().join("workx-home");
...
"#
    ...
    .await?;
    let expected_log_dir = AbsolutePathBuf::resolve_path_against_base("~/.workx/logs", &workx_home);
    let requirements = layers.requirements_toml();

    assert_eq!(
        requirements.sqlite_home.as_deref(),
        Some(workx_home.join("state").as_path())
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        Some(workx_home.join("models.json").as_path())
    );
...
"#,
    ...
    assert_eq!(
        config_requirements_toml
            ...
            .as_deref()
            .cloned(),
        Some(vec![workx_config::WebSearchModeRequirement::Cached])
    );
    assert_eq!(
        config_requirements_toml
        ...
        Some(workx_config::FeatureRequirementsToml {
    ...
    assert_eq!(
        ...
        Some(workx_config::ResidencyRequirement::Us)
    );
    assert_eq!(
        config_requirements
            .feature_requirements
...
async fn system_requirements_control_in_app_updates() -> anyhow::Result<()> {
    let tmp = tempdir()?;
    let workx_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&workx_home).await?;
    ...
    assert_eq!(
        ...
        Err(ConstraintError::InvalidValue {
            ...
            requirement_source: RequirementSource::MdmManagedPreferences {
                domain: "com.ronanxiao.workx".to_string(),
...
"#,
    ...
    let mut config = ConfigBuilder::default()
        ...
        .build()
        .await?;
    let config_toml = AbsolutePathBuf::from_absolute_path(workx_home.join(CONFIG_TOML_FILE))?;
...
"#,
    ...
    assert_eq!(
        ...
        vec![
            ...
            ConfigLayerSource::User {
                file: AbsolutePathBuf::from_absolute_path(workx_home.join(CONFIG_TOML_FILE))?,
                profile: None,
            },
        ]
    );
...
#[tokio::test]
async fn load_config_layers_resolves_relative_bundle_requirements_paths_against_workx_home()
-> anyhow::Result<()> {
    let tmp = tempdir()?;
    let workx_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&workx_home).await?;
...
"#;
    ...
    assert_eq!(
        ...
        Some(vec![
            FilesystemDenyReadPattern::from_input(&format!("{}/secrets/**", workx_home.display()))
                .expect("bundle requirements path should resolve against workx_home")
        ])
    );
...
async fn strict_config_rejects_unknown_cloud_config_key() {
    let tmp = tempdir().expect("tempdir");
    let workx_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&workx_home)
        .await
        .expect("create workx home");
...
async fn load_config_layers_applies_matching_remote_sandbox_config() -> anyhow::Result<()> {
    ...
    assert_eq!(
        ...
        Some(vec![
            workx_config::SandboxModeRequirement::ReadOnly,
            workx_config::SandboxModeRequirement::WorkspaceWrite,
        ])
    );
    assert!(
        layers
        ...
        ConfigLoadOptions {
            cloud_config_bundle: CloudConfigBundleLoader::new(async {
                Err(CloudConfigBundleLoadError::new(
                    workx_config::CloudConfigBundleLoadErrorCode::RequestFailed,
                    /*status_code*/ None,
...
async fn project_layers_prefer_closest_cwd() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".workx")).await?;
    tokio::fs::create_dir_all(project_root.join(".workx")).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;

    tokio::fs::write(
        project_root.join(".workx").join(CONFIG_TOML_FILE),
        r#"foo = "root"
"#,
    )
    .await?;
    tokio::fs::write(
        nested.join(".workx").join(CONFIG_TOML_FILE),
        r#"foo = "child"
"#,
    )
    .await?;
    ...
    let project_layers: Vec<_> = layers
        ...
        .filter_map(|layer| match &layer.name {
            ConfigLayerSource::Project { dot_workx_folder } => Some(dot_workx_folder),
            _ => None,
        })
        .collect();
    assert_eq!(project_layers.len(), 2);
    assert_eq!(project_layers[0].as_path(), nested.join(".workx").as_path());
    assert_eq!(
        project_layers[1].as_path(),
        project_root.join(".workx").as_path()
    );
...
-> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(worktree_root.join(".workx")).await?;
    tokio::fs::create_dir_all(worktree_child.join(".workx")).await?;
    write_linked_worktree_pointer(&repo_root, &worktree_root).await?;
    write_project_hook_config(
        &repo_root.join(".workx"),
        Some("repo-root"),
        "echo repo root hook",
    )
    .await?;
    write_project_hook_config(
        &repo_child.join(".workx"),
        Some("repo-child"),
        "echo repo child hook",
    )
    .await?;
    write_project_hook_config(
        &worktree_root.join(".workx"),
        Some("worktree-root"),
        "echo worktree root hook",
    )
    .await?;
    write_project_hook_config(
        &worktree_child.join(".workx"),
        Some("worktree-child"),
...
async fn forged_linked_worktree_does_not_inherit_repo_trust() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(attacker_root.join(".workx")).await?;
    tokio::fs::write(
    ...
    .await?;
    tokio::fs::write(
        attacker_root.join(".workx").join(CONFIG_TOML_FILE),
        r#"foo = "attacker"
"#,
    )
    .await?;
...
async fn malformed_untrusted_linked_worktree_does_not_read_root_hooks() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(worktree_root.join(".workx")).await?;
    tokio::fs::create_dir_all(repo_root.join(".workx")).await?;
    write_linked_worktree_pointer(&repo_root, &worktree_root).await?;
    tokio::fs::write(worktree_root.join(".workx").join(CONFIG_TOML_FILE), "foo =").await?;
    tokio::fs::write(repo_root.join(".workx").join(CONFIG_TOML_FILE), [0xff]).await?;
...
async fn nested_project_root_markers_do_not_redirect_regular_repo_hooks() -> std::io::Result<()> {
    ...
    assert_eq!(
        project_layers[0].hooks_config_folder(),
        Some(AbsolutePathBuf::from_absolute_path(nested.join(".workx"))?)
    );
    assert_eq!(
        project_layers[1].hooks_config_folder(),
        Some(AbsolutePathBuf::from_absolute_path(
...
#[tokio::test]
async fn project_paths_resolve_relative_to_dot_workx_and_override_in_order() -> std::io::Result<()>
{
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
...
"#;
    tokio::fs::write(project_root.join(".workx").join(CONFIG_TOML_FILE), root_cfg).await?;
    tokio::fs::write(nested.join(".workx").join(CONFIG_TOML_FILE), nested_cfg).await?;
    tokio::fs::write(
        project_root.join(".workx").join("root.txt"),
        "root instructions",
    )
    .await?;
    tokio::fs::write(
        nested.join(".workx").join("child.txt"),
        "child instructions",
    )
    .await?;
...
async fn cli_override_model_instructions_file_sets_base_instructions() -> std::io::Result<()> {
    ...
    let workx_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&workx_home).await?;
    tokio::fs::write(workx_home.join(CONFIG_TOML_FILE), "").await?;
...
#[tokio::test]
async fn project_layer_is_added_when_dot_workx_exists_without_config_toml() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    ...
    let expected_project_layer = ConfigLayerEntry::new(
        ConfigLayerSource::Project {
            dot_workx_folder: AbsolutePathBuf::from_absolute_path(project_root.join(".workx"))?,
        },
        TomlValue::Table(toml::map::Map::new()),
    );
...
#[tokio::test]
async fn workx_home_is_not_loaded_as_project_layer_from_home_dir() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let home_dir = tmp.path().join("home");
    let workx_home = home_dir.join(".workx");
    tokio::fs::create_dir_all(&workx_home).await?;
    tokio::fs::write(
        workx_home.join(CONFIG_TOML_FILE),
        r#"foo = "user"
...
#[tokio::test]
async fn workx_home_within_project_tree_is_not_double_loaded() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    let project_dot_workx = project_root.join(".workx");
    let nested_dot_workx = nested.join(".workx");

    tokio::fs::create_dir_all(&nested_dot_workx).await?;
    tokio::fs::create_dir_all(project_root.join(".git")).await?;
    tokio::fs::write(project_root.join(".git/HEAD"), "ref: refs/heads/main\n").await?;
    tokio::fs::write(
        nested_dot_workx.join(CONFIG_TOML_FILE),
        r#"foo = "child"
"#,
    )
    .await?;

    tokio::fs::create_dir_all(&project_dot_workx).await?;
    make_config_for_test(
        &project_dot_workx,
        &project_root,
        TrustLevel::Trusted,
    ...
    .await?;
    let user_config_path = project_dot_workx.join(CONFIG_TOML_FILE);
...
"#,
    ...
    let expected_project_layer = ConfigLayerEntry::new(
        ConfigLayerSource::Project {
            dot_workx_folder: AbsolutePathBuf::from_absolute_path(&nested_dot_workx)?,
        },
        child_config,
    );
    assert_eq!(vec![&expected_project_layer], project_layers);
...
"#,
    ...
    let workx_home_untrusted = tmp.path().join("home_untrusted");
    tokio::fs::create_dir_all(&workx_home_untrusted).await?;
    make_config_for_test(
        &workx_home_untrusted,
        &project_root,
        TrustLevel::Untrusted,
        /*project_root_markers*/ None,
    )
    .await?;
    let untrusted_config_path = workx_home_untrusted.join(CONFIG_TOML_FILE);
...
{untrusted_config_contents}"#
    ...
    let workx_home_unknown = tmp.path().join("home_unknown");
    tokio::fs::create_dir_all(&workx_home_unknown).await?;
    tokio::fs::write(
        workx_home_unknown.join(CONFIG_TOML_FILE),
        r#"foo = "user"
"#,
    )
    .await?;
...
async fn project_layer_ignores_unsupported_config_keys() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(&dot_workx).await?;
    ...
    // `.workx` folder. The malformed profile value below would fail typed path
    // resolution if `profiles` were not stripped before that pass runs.
    tokio::fs::write(
        dot_workx.join(CONFIG_TOML_FILE),
        r#"
model = "project-model"
...
responses_api_metadata = { workx_security_surface = "attacker" }
...
"#,
    ...
    let expected_startup_warnings = vec![format!(
        ...
        dot_workx.join(CONFIG_TOML_FILE).display(),
    ...
    // still use the project-local `.workx` folder as its relative-path base.
    assert_eq!(
...
async fn invalid_project_config_ignored_when_untrusted_or_unknown() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".workx")).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;
    tokio::fs::write(nested.join(".workx").join(CONFIG_TOML_FILE), "foo =").await?;
    ...
    for (name, trust_level) in cases {
        let workx_home = tmp.path().join(format!("home_{name}"));
        tokio::fs::create_dir_all(&workx_home).await?;
...
-> std::io::Result<()> {
    ...
    for (name, trust_level, expect_disabled) in cases {
        let workx_home = tmp.path().join(format!("home_no_config_{name}"));
        tokio::fs::create_dir_all(&workx_home).await?;
        if let Some(trust_level) = trust_level {
            make_config_for_test(
                &workx_home,
...
mod requirements_exec_policy_tests {
    ...
    use workx_config::ConfigRequirementsWithSources;
    use workx_config::RequirementSource;
    use workx_config::RequirementsExecPolicyDecisionToml;
    use workx_config::RequirementsExecPolicyParseError;
    use workx_config::RequirementsExecPolicyPatternTokenToml;
    use workx_config::RequirementsExecPolicyPrefixRuleToml;
    use workx_config::RequirementsExecPolicyToml;
    use workx_execpolicy::Decision;
    use workx_execpolicy::Evaluation;
    use workx_execpolicy::RuleMatch;
    use workx_utils_absolute_path::AbsolutePathBuf;
    ...
    fn config_stack_for_dot_workx_folder_with_requirements(
        dot_workx_folder: &Path,
        requirements: ConfigRequirements,
    ) -> ConfigLayerStack {
        let dot_workx_folder = AbsolutePathBuf::from_absolute_path(dot_workx_folder)
            .expect("absolute dot_workx_folder");
        let layer = ConfigLayerEntry::new(
            ConfigLayerSource::Project { dot_workx_folder },
...
"#;
    ...
    async fn loads_requirements_exec_policy_without_rules_files() -> anyhow::Result<()> {
        ...
        let config_stack =
            config_stack_for_dot_workx_folder_with_requirements(temp_dir.path(), requirements);

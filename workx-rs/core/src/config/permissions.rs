...
use workx_config::permissions_toml::NetworkUnixSocketPermissionToml;
use workx_config::permissions_toml::NetworkUnixSocketPermissionsToml;
...
use workx_features::NetworkProxyDomainPermissionToml;
use workx_features::NetworkProxyModeToml;
use workx_features::NetworkProxyUnixSocketPermissionToml;
use workx_network_proxy::NetworkMode;
use workx_network_proxy::NetworkProxyConfig;
#[cfg(test)]
use workx_network_proxy::NetworkUnixSocketPermission as ProxyNetworkUnixSocketPermission;
...
/// for Workx to function. These should always be added to the
/// `FileSystemSandboxPolicy` for a thread.
pub(crate) fn get_readable_roots_required_for_workx_runtime(
    workx_home: &Path,
    zsh_path: Option<&PathBuf>,
    main_execve_wrapper_exe: Option<&PathBuf>,
) -> Vec<AbsolutePathBuf> {
    let arg0_root = AbsolutePathBuf::from_absolute_path(workx_home.join("tmp").join("arg0")).ok();
...
// Adding a new `:special_path` must not make older Workx versions reject the
...
fn missing_filesystem_entries_warning(profile_name: &str) -> String {
    format!(
        "Permissions profile `{profile_name}` does not define any recognized filesystem entries for this version of Workx. Filesystem access will remain restricted. Upgrade Workx if this profile expects filesystem permissions."
    )
}
...
fn maybe_push_unknown_special_path_warning(
    ...
    push_warning(
        ...
        match subpath.as_deref() {
            Some(subpath) => format!(
                "Configured filesystem path `{path}` with nested entry `{subpath}` is not recognized by this version of Workx and will be ignored. Upgrade Workx if this path is required."
            ),
            None => format!(
                "Configured filesystem path `{path}` is not recognized by this version of Workx and will be ignored. Upgrade Workx if this path is required."
            ),
        },
    );
}

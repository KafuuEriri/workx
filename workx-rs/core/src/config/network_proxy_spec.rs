...
use workx_network_proxy::host_and_port_from_network_addr;
#[cfg(any(target_os = "windows", test))]
use workx_network_proxy::managed_proxy_ports;
use workx_network_proxy::normalize_host;
use workx_network_proxy::validate_policy_against_constraints;
    ...
    for host in hosts {
        if incoming.insert(host.clone()) {
            config.upsert_domain_permission(
                host,
                if allow {
                    workx_network_proxy::NetworkDomainPermission::Allow
                } else {
                    workx_network_proxy::NetworkDomainPermission::Deny
                },
                normalize_host,
            );
        }

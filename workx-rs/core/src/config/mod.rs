...
use workx_agent_roles::load_agent_roles;
use workx_config::CloudConfigBundleLoader;
use workx_config::ConfigLayerSource;
use workx_config::ConfigLayerStack;
use workx_config::ConfigRequirements;
use workx_config::ConfigRequirementsToml;
use workx_config::ConstrainedWithSource;
use workx_config::FeatureRequirementsToml;
use workx_config::ManagedAuthPolicy;
use workx_config::McpServerRequirement;
use workx_config::PluginRequirementsToml;
use workx_config::ProfileV2Name;
use workx_config::ResidencyRequirement;
use workx_config::SandboxModeRequirement;
use workx_config::Sourced;
use workx_config::ThreadConfigLoader;
use workx_config::config_toml::ConfigToml;
use workx_config::config_toml::DEFAULT_PROJECT_DOC_MAX_BYTES;
use workx_config::config_toml::ProjectConfig;
use workx_config::config_toml::RealtimeAudioConfig;
use workx_config::config_toml::RealtimeConfig;
use workx_config::config_toml::ThreadStoreToml;
use workx_config::config_toml::validate_model_providers;
use workx_config::loader::load_config_layers_state;
...
use workx_config::sandbox_mode_requirement_for_permission_profile;
use workx_config::types::ApprovalsReviewer;
use workx_config::types::AuthCredentialsStoreMode;
use workx_config::types::AuthKeyringBackendKind;
use workx_config::types::History;
use workx_config::types::McpServerConfig;
use workx_config::types::McpServerDisabledReason;
use workx_config::types::MemoriesConfig;
use workx_config::types::ModelAvailabilityNuxConfig;
use workx_config::types::Notice;
use workx_config::types::OAuthCredentialsStoreMode;
use workx_config::types::ResumeCwdMode;
use workx_config::types::SessionPickerViewMode;
use workx_config::types::ToolSuggestConfig;
use workx_config::types::ToolSuggestDisabledTool;
use workx_config::types::ToolSuggestDiscoverable;
use workx_config::types::TuiKeymap;
use workx_config::types::TuiNotificationSettings;
use workx_config::types::TuiPetAnchor;
use workx_config::types::UriBasedFileOpener;
use workx_config::types::WindowsSandboxModeToml;
use workx_core_plugins::PluginLoadOutcome;
use workx_core_plugins::PluginsConfigInput;
use workx_exec_server::ExecutorFileSystem;
use workx_exec_server::LOCAL_FS;
use workx_exec_server::ReadFileOptions;
use workx_features::CodeModeConfigToml;
use workx_features::CurrentTimeReminderConfigToml;
use workx_features::CurrentTimeReminderDeliveryMode;
use workx_features::CurrentTimeSource;
use workx_features::Feature;
use workx_features::FeatureConfigSource;
use workx_features::FeatureOverrides;
use workx_features::FeatureToml;
use workx_features::Features;
use workx_features::FeaturesToml;
use workx_features::MultiAgentV2ConfigToml;
use workx_features::NetworkProxyConfigToml;
use workx_features::SleepToolMode;
use workx_features::TokenBudgetConfigToml;
...
use workx_install_context::InstallContext;
use workx_login::AuthManagerConfig;
use workx_login::AuthRouteConfig;
use workx_mcp::DEFAULT_OPTIONAL_MCP_STARTUP_GRACE;
use workx_mcp::McpConfig;
use workx_mcp::McpPluginAttribution;
use workx_mcp::McpProtocolMode;
use workx_mcp::McpServerRegistration;
use workx_mcp::ResolvedMcpCatalog;
use workx_memories_read::memory_root;
use workx_model_provider::ProviderCapabilities;
use workx_model_provider_info::LEGACY_OLLAMA_CHAT_PROVIDER_ID;
use workx_model_provider_info::ModelProviderInfo;
use workx_model_provider_info::OLLAMA_CHAT_PROVIDER_REMOVED_ERROR;
use workx_model_provider_info::built_in_model_providers;
use workx_model_provider_info::merge_configured_model_providers;
use workx_models_manager::ModelsManagerConfig;
use workx_protocol::config_types::AltScreenMode;
use workx_protocol::config_types::AutoCompactTokenLimitScope;
...
use workx_protocol::config_types::ShellEnvironmentPolicy;
use workx_protocol::config_types::TrustLevel;
use workx_protocol::config_types::Verbosity;
use workx_protocol::config_types::WebSearchConfig;
use workx_protocol::config_types::WebSearchMode;
use workx_protocol::config_types::WindowsSandboxLevel;
use workx_protocol::models::ActivePermissionProfile;
use workx_protocol::models::BaseInstructionsProvenance;
...
use workx_protocol::permissions::ReadDenyMatcher;
...
use workx_rmcp_client::McpOAuthRefreshMode;
pub use workx_thread_store::ExtraConfig;
use workx_utils_absolute_path::AbsolutePathBuf;
use workx_utils_absolute_path::AbsolutePathBufGuard;
...
use crate::config::permissions::get_readable_roots_required_for_workx_runtime;
...
pub use workx_agent_roles::AgentRoleConfig;
pub use workx_config::ConfigLoadOptions;
pub use workx_config::Constrained;
pub use workx_config::ConstraintError;
pub use workx_config::ConstraintResult;
pub use workx_config::LoaderOverrides;
pub use workx_network_proxy::NetworkProxyAuditMetadata;
use workx_sandboxing::compatibility_sandbox_policy_for_permission_profile;
pub use workx_sandboxing::system_bwrap_warning;
...
fn resolve_sqlite_home_env(resolved_cwd: &Path) -> Option<AbsolutePathBuf> {
    let raw = std::env::var(workx_state::SQLITE_HOME_ENV).ok()?;
...
pub(crate) async fn test_config() -> Config {
    ...
    Config::load_from_base_config_with_overrides(
        ...
        AbsolutePathBuf::from_absolute_path(workx_home.path()).expect("temp dir should resolve"),
    )
    .await
...
impl Permissions {
    ...
    /// selection. Internal Workx-only writable roots are intentionally excluded.
...
// when Workx is still responsible for the network policy. `Disabled` means no
...
fn build_network_proxy_spec(
    ...
    network_requirements: Option<Sourced<workx_config::NetworkConstraints>>,
    permission_profile: &PermissionProfile,
...
pub struct Config {
    ...
    /// Optional external notifier command. When set, Workx will spawn this
    /// program after each completed *turn* (i.e. when the agent finishes
    /// processing a user submission). The value must be the full command
    /// broken into argv tokens **without** the trailing JSON argument - Workx
    /// appends one extra argument containing a JSON payload describing the
    /// event.
    ///
    /// Example `~/.workx/config.toml` snippet:
    ///
    /// ```toml
    /// notify = ["notify-send", "Workx"]
    /// ```
    ///
    /// which will be invoked as:
    ///
    /// ```shell
    /// notify-send Workx '{"type":"agent-turn-complete","turn-id":"12345"}'
    /// ```
    ///
    ...
    /// file (default): Use a file in the Workx home directory.
    /// keyring: Use an OS-specific keyring service.
    /// auto: Use the OS-specific keyring service if available, otherwise use a file.
    pub cli_auth_credentials_store_mode: AuthCredentialsStoreMode,

    /// Definition for MCP servers that Workx can reach out to for tool calls.
    ...
    /// keyring: Use an OS-specific keyring service.
    ///          Credentials stored in the keyring will only be readable by Workx unless the user explicitly grants access via OS-level keyring access.
    ///          https://github.com/RonanXiao/workx/blob/main/workx-rs/rmcp-client/src/oauth.rs#L2
    /// file: WORKX_HOME/.credentials.json
    ///       This file will be readable to Workx and other applications running as the same user.
    /// auto (default): keyring if available, otherwise file.
    pub mcp_oauth_credentials_store_mode: OAuthCredentialsStoreMode,

    /// Optional fixed port to use for the local HTTP callback server used during MCP OAuth login.
    ///
    /// When unset, Workx will bind to an ephemeral port chosen by the OS.
    ...
    /// Directory containing all Workx state (defaults to `~/.workx` but can be
    /// overridden by the `WORKX_HOME` environment variable).
    pub workx_home: AbsolutePathBuf,

    /// Resolved configuration shared by all Workx SQLite databases.
    pub sqlite: workx_state::SqliteConfig,

    /// Directory where Workx writes log files (defaults to `$WORKX_HOME/log`).
    pub log_dir: PathBuf,

    /// Settings that govern if and what will be written to `~/.workx/history.jsonl`.
    pub history: History,
    ...
    /// Path to the current Workx executable. This cannot be set in the config
    /// file: it must be set in code via [`ConfigOverrides`].
    pub workx_self_exe: Option<PathBuf>,

    /// Path to the `workx-linux-sandbox` executable. This must be set if
    /// [`workx_sandboxing::SandboxType::LinuxSeccomp`] is used. Note that this
    /// cannot be set in the config file: it must be set in code via
    /// [`ConfigOverrides`].
    ///
    /// When this program is invoked, arg0 will be set to `workx-linux-sandbox`.
    pub workx_linux_sandbox_exe: Option<PathBuf>,

    /// Path to the `workx-execve-wrapper` executable used for shell
    ...
    /// Whether Workx-owned clients should respect host system proxy settings.
    ...
    /// When `true`, checks for Workx updates on startup and surfaces update prompts.
    /// Set to `false` only if your Workx updates are centrally managed.
    /// Defaults to `true`.
    ...
    /// When `false`, disables analytics across Workx product surfaces in this machine.
    /// Voluntarily left as Optional because the default value might depend on the client.
    pub analytics_enabled: Option<bool>,

    /// When `false`, disables feedback collection across Workx product surfaces.
    /// Defaults to `true`.
    ...
    pub otel: workx_config::types::OtelConfig,
}
...
impl AuthManagerConfig for Config {
    fn workx_home(&self) -> PathBuf {
        self.workx_home.to_path_buf()
    }
...
impl ConfigBuilder {
    pub fn workx_home(mut self, workx_home: PathBuf) -> Self {
        self.workx_home = Some(workx_home);
        self
    }
    ...
    async fn build_inner(self) -> std::io::Result<Config> {
        ...
        let workx_home = match workx_home {
            Some(workx_home) => AbsolutePathBuf::from_absolute_path(workx_home)?,
            None => find_workx_home()?,
        };
        ...
        let config_layer_stack = load_config_layers_state(
            ...
            thread_config_loader
                .as_deref()
                .unwrap_or(&workx_config::NoopThreadConfigLoader),
        )
        .await?;
        ...
        let config_toml: ConfigToml = match merged_toml.try_into() {
            ...
            Err(err) => {
                if let Some(config_error) = workx_config::first_layer_config_error::<ConfigToml>(
                    &config_layer_stack,
                    workx_config::CONFIG_TOML_FILE,
                )
                .await
                {
                    return Err(workx_config::io_error_from_config_error(
...
impl Config {
    pub fn sqlite_config(&self) -> &workx_state::SqliteConfig {
        &self.sqlite
    }

    /// Whether Guardian may use the unmetered Workx inference endpoints.
    ...
    pub async fn to_mcp_config(
        &self,
        plugins_manager: &workx_core_plugins::PluginsManager,
    ) -> McpConfig {
    ...
    pub(crate) fn to_mcp_config_with_loaded_plugins(
        ...
        McpConfig {
            ...
            workx_home: self.workx_home.to_path_buf(),
            ...
            workx_linux_sandbox_exe: self.workx_linux_sandbox_exe.clone(),
            ...
            connector_snapshot:
                workx_connectors::ConnectorSnapshot::from_plugin_capability_summaries(
    ...
    pub async fn rebuild_preserving_session_layers(
        ...
        Self::load_config_with_layer_stack(
            ...
            refreshed_config.workx_home.clone(),
            config_layer_stack,
        )
        .await
    }
    ...
    pub async fn load_default_with_cli_overrides(
        ...
        Self::load_default_with_cli_overrides_for_workx_home(
            workx_home.to_path_buf(),
            cli_overrides,
        )
        .await
    }

    /// Load a default configuration for a specific Workx home without reading
    /// user, project, or system config layers.
    pub async fn load_default_with_cli_overrides_for_workx_home(
        workx_home: PathBuf,
        cli_overrides: Vec<(String, TomlValue)>,
    ) -> std::io::Result<Self> {
        ...
        let cli_layer = workx_config::build_cli_overrides_layer(&cli_overrides);
        workx_config::merge_toml_values(&mut merged, &cli_layer);
        let workx_home = AbsolutePathBuf::from_absolute_path_checked(workx_home)?;
        let config_toml = deserialize_config_toml_with_base(merged, &workx_home)?;
        Self::load_config_with_layer_stack(
            LOCAL_FS.as_ref(),
            config_toml,
            ConfigOverrides::default(),
    ...
    /// ignores user settings. For example, the `workx exec` subcommand is
    /// designed to use [AskForApproval::Never] exclusively.
    ///
    /// Further, [ConfigOverrides] contains some options that are not supported
    /// in [ConfigToml], such as `cwd`, `workx_self_exe`, `workx_linux_sandbox_exe`, and
...
pub async fn load_config_as_toml_with_cli_and_loader_overrides(
    ...
    load_config_as_toml_with_cli_and_load_options(workx_home, cwd, cli_overrides, loader_overrides)
        .await
}
...
pub async fn load_config_as_toml_with_cli_and_load_options(
    ...
    load_config_toml_with_layer_stack(workx_home, cwd, cli_overrides, options)
        .await
...
pub async fn load_config_toml_with_layer_stack(
    ...
    let config_layer_stack = load_config_layers_state(
        ...
        &cli_overrides,
        options,
        &workx_config::NoopThreadConfigLoader,
    )
    .await?;

    let merged_toml = config_layer_stack.effective_config();
    let cfg = deserialize_config_toml_with_base(merged_toml, workx_home).map_err(|e| {
...
pub async fn load_global_mcp_servers(
    ...
    // MCP servers defined in in-repo .workx/ folders.
...
/// Patch `WORKX_HOME/config.toml` project state to set trust level.
/// Use with caution.
...
pub fn set_default_oss_provider(workx_home: &Path, provider: &str) -> std::io::Result<()> {
    workx_config::config_toml::validate_oss_provider(provider)?;
    use toml_edit::value;
...
fn apply_managed_filesystem_constraints(
    ...
    filesystem_constraints: &workx_config::FilesystemConstraints,
) {
    for deny_read in &filesystem_constraints.deny_read {
        let deny_entry = if deny_read.contains_glob() {
            workx_protocol::permissions::FileSystemSandboxEntry {
                path: workx_protocol::permissions::FileSystemPath::GlobPattern {
                    pattern: deny_read.as_str().to_string(),
                },
                access: workx_protocol::permissions::FileSystemAccessMode::Deny,
...
fn resolve_orchestrator_feature_enabled(
    feature: Option<&workx_config::config_toml::OrchestratorFeatureToml>,
) -> bool {
...
impl Config {
    ...
    pub(crate) async fn load_config_with_layer_stack(
        ...
        let memories_root = memory_root(&workx_home);
        ...
        let forced_chatgpt_workspace_id = cfg
            ...
            .clone()
            .map(workx_config::config_toml::ForcedChatgptWorkspaceIds::into_vec)
            .map(|values| {
                values
                    .into_iter()
        ...
        let log_dir = cfg
            ...
            .unwrap_or_else(|| workx_home.join("log").to_path_buf());
        ...
        let sqlite_home = cfg
            ...
            .cloned()
            .or(sqlite_home_env)
            .unwrap_or_else(|| workx_home.clone());
            ...
            && constrained_approval_policy.value() == AskForApproval::Never
        {
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
                "`approval_policy = \"never\"` cannot be used because requirements do not allow `sandbox_mode = \"danger-full-access\"`; Workx would fall back to read-only permissions with approvals disabled. Choose an `approval_policy` based on what you need, such as `on-request`, or choose an allowed sandbox mode.",
            ));
        }
        if permission_profile_was_constrained {
        ...
        let mut helper_readable_roots = get_readable_roots_required_for_workx_runtime(
            &workx_home,
            zsh_path.as_ref(),
        ...
        let config = Self {
            ...
            workx_home,
            sqlite: workx_state::SqliteConfig::from_sqlite_home(sqlite_home),
            log_dir,
            config_layer_stack,
            history,
    ...
    pub fn bundled_skills_enabled(&self) -> bool {
        workx_config::bundled_skills_enabled_from_stack(&self.config_layer_stack)
    }
...
/// Returns the path to the Workx configuration directory, which can be
/// specified by the `WORKX_HOME` environment variable. If not set, defaults to
/// `~/.workx`.
///
/// - If `WORKX_HOME` is set, the value must exist and be a directory. The
///   value will be canonicalized and this function will Err otherwise.
/// - If `WORKX_HOME` is not set, this function does not verify that the
///   directory exists.
pub fn find_workx_home() -> std::io::Result<AbsolutePathBuf> {
    workx_utils_home_dir::find_workx_home()
}

/// Returns the path to the folder where Workx logs are stored. Does not verify

...
use workx_config::ConfigLayerEntry;
use workx_config::ConfigLayerSource;
use workx_config::ConfigLayerStack;
use workx_config::McpServerCommandMatcher;
use workx_config::McpServerIdentity;
use workx_config::McpServerRequirement;
use workx_config::McpServerValueMatcher;
use workx_config::ProfileV2Name;
use workx_config::RequirementSource;
use workx_config::Sourced;
use workx_config::config_toml::AgentRoleToml;
use workx_config::config_toml::AgentsToml;
use workx_config::config_toml::AutoReviewToml;
use workx_config::config_toml::ConfigToml;
use workx_config::config_toml::ExperimentalRequestUserInput;
use workx_config::config_toml::ProjectConfig;
use workx_config::config_toml::RealtimeConfig;
use workx_config::config_toml::RealtimeToml;
use workx_config::config_toml::RealtimeTransport;
use workx_config::config_toml::RealtimeWsMode;
use workx_config::config_toml::RealtimeWsVersion;
use workx_config::config_toml::ToolsToml;
...
use workx_config::permissions_toml::NetworkDomainPermissionToml;
use workx_config::permissions_toml::NetworkDomainPermissionsToml;
use workx_config::permissions_toml::NetworkMitmActionToml;
use workx_config::permissions_toml::NetworkMitmHookToml;
use workx_config::permissions_toml::NetworkMitmToml;
...
use workx_config::permissions_toml::WorkspaceRootsToml;
use workx_config::types::AppToolApproval;
use workx_config::types::ApprovalsReviewer;
use workx_config::types::BundledSkillsConfig;
use workx_config::types::FeedbackConfigToml;
use workx_config::types::HistoryPersistence;
use workx_config::types::McpServerEnvVar;
use workx_config::types::McpServerOAuthConfig;
use workx_config::types::McpServerToolConfig;
use workx_config::types::McpServerTransportConfig;
use workx_config::types::MemoriesConfig;
use workx_config::types::MemoriesToml;
use workx_config::types::ModelAvailabilityNuxConfig;
use workx_config::types::Notice;
use workx_config::types::NotificationCondition;
use workx_config::types::NotificationMethod;
use workx_config::types::Notifications;
use workx_config::types::OtelConfigToml;
use workx_config::types::OtelExporterKind;
use workx_config::types::ResumeCwdMode;
use workx_config::types::SandboxWorkspaceWrite;
use workx_config::types::SessionPickerViewMode;
use workx_config::types::SkillsConfig;
use workx_config::types::ToolSuggestDisabledTool;
use workx_config::types::ToolSuggestDiscoverableType;
use workx_config::types::Tui;
...
use workx_config::types::WindowsToml;
...
use workx_model_provider_info::LMSTUDIO_OSS_PROVIDER_ID;
use workx_model_provider_info::OLLAMA_OSS_PROVIDER_ID;
use workx_model_provider_info::WireApi;
use workx_models_manager::bundled_models_response;
use workx_network_proxy::NetworkMode;
...
use workx_protocol::models::BUILT_IN_PERMISSION_PROFILE_DANGER_FULL_ACCESS;
use workx_protocol::models::BUILT_IN_PERMISSION_PROFILE_READ_ONLY;
use workx_protocol::models::BUILT_IN_PERMISSION_PROFILE_WORKSPACE;
use workx_protocol::models::ManagedFileSystemPermissions;
use workx_protocol::models::PermissionProfile;
use workx_protocol::models::SandboxEnforcement;
use workx_protocol::openai_models::MultiAgentRoleMessages;
...
use workx_protocol::protocol::NetworkAccess;
use workx_protocol::protocol::RealtimeVoice;
use workx_protocol::protocol::SandboxPolicy;
use workx_utils_path_uri::LegacyAppPathString;
...
fn stdio_mcp_with_args(command: &str, args: &[&str]) -> McpServerConfig {
    McpServerConfig {
        ...
        environment_id: workx_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID.to_string(),
        enabled: true,
        required: false,
...
excluded_tool_namespaces = ["mcp__workx_apps", "multi_agent_v1"]
...
"#,
    ...
    assert_eq!(
        ...
        vec!["mcp__workx_apps".to_string(), "multi_agent_v1".to_string()]
    );
    assert_eq!(
...
name = "OpenAI"
http_headers = { "X-OpenAI-Internal-Workx-Residency" = "request-override", "x-provider-header" = "preserved" }
env_http_headers = { "x-openai-internal-codex-residency" = "WORKX_TEST_UNSET_RESIDENCY_HEADER", "x-provider-env-header" = "WORKX_TEST_UNSET_PROVIDER_HEADER" }
"#,
        )
        ...
        assert_eq!(
            static_headers.get("X-OpenAI-Internal-Workx-Residency"),
        ...
        assert_eq!(
            environment_headers
                ...
                .map(String::as_str),
            Some("WORKX_TEST_UNSET_RESIDENCY_HEADER")
        );
        assert_eq!(
        ...
        assert_eq!(
            environment_headers
                ...
                .map(String::as_str),
            Some("WORKX_TEST_UNSET_PROVIDER_HEADER")
        );
...
async fn tui_auto_recap_defaults_and_cli_overrides() -> anyhow::Result<()> {
    for (toml, override_value, expected) in [
        ...
        std::fs::write(workx_home.path().join(CONFIG_TOML_FILE), toml)?;
...
async fn network_proxy_cli_overrides_merge_toggle_with_proxy_config() -> std::io::Result<()> {
    ...
    let cwd = TempDir::new()?;
    std::fs::write(
        workx_home.path().join(CONFIG_TOML_FILE),
        r#"
...
"#,
    ...
    assert_eq!(
        ...
        workx_core_plugins::remote::RemotePluginServiceConfig::new(
...
-> std::io::Result<()> {
    ...
    requirements.network = Some(Sourced::new(
        workx_config::NetworkConstraints {
            enabled: Some(true),
            ..Default::default()
        },
...
-> std::io::Result<()> {
    ...
    let memories_root = workx_home.path().join("memories").abs();
    assert!(
        !config
            .permissions
            .file_system_sandbox_policy()
...
-> std::io::Result<()> {
    ...
    let workx_home = temp_dir.path().join("workx-home");
    ...
    for root in [&cwd, &runtime_root, &profile_root] {
        ...
        std::fs::create_dir_all(root.join(".workx"))?;
    }
                        ...
                        filesystem: Some(FilesystemPermissionsToml {
                            ...
                            entries: BTreeMap::from([(
                                ...
                                FilesystemPermissionToml::Scoped(BTreeMap::from([
                                    ...
                                    (".workx".to_string(), FileSystemAccessMode::Read),
                                ])),
                            )]),
                        }),
                        network: None,
    ...
    for root in [cwd_abs, runtime_root_abs, profile_root_abs.clone()] {
        ...
        assert!(
            !policy.can_write_path_with_cwd(&root.join(".workx"), cwd.as_path()),
            "expected .workx carveout under {root:?}, policy: {policy:?}"
        );
    }
    assert_eq!(
...
async fn empty_config_defaults_to_builtin_profile_for_trusted_project() -> std::io::Result<()> {
    ...
    if cfg!(target_os = "windows") {
        ...
        assert!(
            !policy.can_write_path_with_cwd(&cwd.path().join(".workx"), cwd.path()),
...
-> std::io::Result<()> {
    ...
    let cwd = TempDir::new()?;
    let extra_root = TempDir::new()?;
    for subpath in [".git", ".agents", ".workx"] {
...
async fn permissions_profiles_allow_unknown_special_paths() -> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Configured filesystem path `:future_special_path` is not recognized by this version of Workx and will be ignored."
        )),
        "{:?}",
...
-> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Configured filesystem path `:future_special_path` with nested entry `docs` is not recognized by this version of Workx and will be ignored."
        )),
        "{:?}",
        config.startup_warnings
    );
...
async fn permissions_profiles_allow_missing_filesystem_with_warning() -> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Permissions profile `dev` does not define any recognized filesystem entries for this version of Workx."
        )),
        "{:?}",
        config.startup_warnings
    );
                        ...
                        file_system_policy
                            ...
                            .contains(&FileSystemSandboxEntry {
                                ...
                                missing_path_behavior: Some(
                                    workx_protocol::permissions::FileSystemSandboxEntryMissingPathBehavior::Skip,
                                ),
                            }),
...
fn filter_plugin_mcp_servers_without_allowlists_does_not_filter_any_plugin() {
    ...
    let requirements = Sourced::new(
        BTreeMap::from([(
            ...
            workx_config::PluginRequirementsToml { mcp_servers: None },
        )]),
        RequirementSource::LegacyManagedConfigTomlFromMdm,
    );
...
async fn rebuild_preserving_session_layers_refreshes_requirements() -> std::io::Result<()> {
    let workx_home = TempDir::new()?;
    let user_file = AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, workx_home.path());
    let project_dot_workx =
        AbsolutePathBuf::resolve_path_against_base("project/.workx", workx_home.path());
    ...
    let requirements_toml = workx_config::ConfigRequirementsToml {
        mcp_servers: Some(mcp_requirements.clone()),
        ..Default::default()
    };
    let requirements = workx_config::ConfigRequirements {
        ...
        vec![
            ...
            ConfigLayerEntry::new(
                ConfigLayerSource::Project {
                    dot_workx_folder: project_dot_workx.clone(),
                },
                toml::toml! {
    ...
    let refreshed_config = Config::load_config_with_layer_stack(
        ...
        refreshed_toml,
        ConfigOverrides {
            cwd: Some(workx_home.path().to_path_buf()),
            ..Default::default()
        },
        workx_home.abs(),
    ...
    let thread_layer_stack = ConfigLayerStack::new(
        vec![
            ...
            ConfigLayerEntry::new(
                ConfigLayerSource::Project {
                    dot_workx_folder: project_dot_workx,
                },
                toml::toml! {
                    [mcp_servers.fresh_project]
...
-> anyhow::Result<()> {
    let workx_home = TempDir::new()?;
    let plugin_root = workx_home
        .path()
        .join("plugins/cache")
        .join("test/sample/local");
    std::fs::create_dir_all(plugin_root.join(".workx-plugin"))?;
    std::fs::write(
        plugin_root.join(".workx-plugin/plugin.json"),
        r#"{"name":"sample"}"#,
    )?;
    std::fs::write(
...
"#,
    ...
    assert_eq!(
        ...
        Some((
            workx_mcp::McpServerSource::SelectedPlugin(McpPluginAttribution::new(
                "selected-root".to_string(),
                "Selected Plugin".to_string(),
            )),
            selected,
...
async fn default_zsh_path_sets_runtime_zsh_path() -> std::io::Result<()> {
    let workx_home = TempDir::new()?;
    let default_zsh_path = workx_home.path().join("packaged-zsh");
...
#[tokio::test]
async fn sqlite_home_defaults_to_workx_home_for_workspace_write() -> std::io::Result<()> {
    let workx_home = TempDir::new()?;
    let config = Config::load_from_base_config_with_overrides(
        ConfigToml::default(),
        ConfigOverrides {
    ...
    assert_eq!(config.sqlite.home(), workx_home.path());
...
-> std::io::Result<()> {
    let workx_home = TempDir::new()?;
    let memories_root = workx_home.path().join("memories");
    let writable_root = workx_home.path().join("writable").abs();
    let config = Config::load_from_base_config_with_overrides(
        ConfigToml {
            sandbox_workspace_write: Some(SandboxWorkspaceWrite {
...
async fn config_applies_managed_auth_store_and_chatgpt_base_url() -> std::io::Result<()> {
    ...
    let config_layer_stack = ConfigLayerStack::new(
        Vec::new(),
        requirements,
        workx_config::ConfigRequirementsToml::default(),
    )?;
...
"#,
    ...
    let project_config_dir = workspace.path().join(".workx");
...
async fn managed_config_overrides_oauth_store_mode() -> anyhow::Result<()> {
    let workx_home = TempDir::new()?;
    let managed_path = workx_home.path().join("managed_config.toml");
    let config_path = workx_home.path().join(CONFIG_TOML_FILE);
    ...
    .await?;
    let cfg =
        deserialize_config_toml_with_base(config_layer_stack.effective_config(), workx_home.path())
            .map_err(|e| {
...
async fn load_global_mcp_servers_returns_empty_if_missing() -> anyhow::Result<()> {
    ...
    let servers = load_global_mcp_servers(workx_home.path()).await?;
...
async fn replace_mcp_servers_round_trips_entries() -> anyhow::Result<()> {
    let workx_home = TempDir::new()?;
    let expected_cwd = LegacyAppPathString::from_path(workx_home.path());
    ...
    let loaded = load_global_mcp_servers(workx_home.path()).await?;
...
async fn to_mcp_config_flows_mcp_2026_feature_from_config() -> std::io::Result<()> {
    ...
    let mcp_config = config.to_mcp_config(&plugins_manager).await;
    assert_eq!(mcp_config.protocol_mode, workx_mcp::McpProtocolMode::Legacy);

    let _ = config.features.enable(Feature::Mcp20260728);
    let mcp_config = config.to_mcp_config(&plugins_manager).await;
    assert_eq!(
        mcp_config.protocol_mode,
        workx_mcp::McpProtocolMode::V20260728
    );
...
"#,
    ...
    let err = load_global_mcp_servers(workx_home.path())
        .await
...
async fn replace_mcp_servers_serializes_cwd() -> anyhow::Result<()> {
    ...
    let cwd_path = PathBuf::from("/tmp/workx-mcp");
    ...
    let serialized = std::fs::read_to_string(&config_path)?;
    assert!(
        serialized.contains(r#"cwd = "/tmp/workx-mcp""#),
...
async fn replace_mcp_servers_streamable_http_serializes_oauth_resource() -> anyhow::Result<()> {
    ...
    let servers = BTreeMap::from([(
        ...
        McpServerConfig {
            ...
            oauth: Some(McpServerOAuthConfig {
                client_id: Some("eci-prd-pub-workx-123".to_string()),
                callback_url: None,
                callback_port: None,
            }),
    ...
    assert!(serialized.contains(r#"client_id = "eci-prd-pub-workx-123""#));
    ...
    assert_eq!(docs.oauth_client_id(), Some("eci-prd-pub-workx-123"));
...
async fn set_model_updates_defaults() -> anyhow::Result<()> {
    ...
    ConfigEditsBuilder::new(workx_home.path())
        .set_model(Some("gpt-5.4"), Some(ReasoningEffort::High))
        .apply()
        .await?;

    let serialized = tokio::fs::read_to_string(workx_home.path().join(CONFIG_TOML_FILE)).await?;
...
async fn for_config_writes_selected_user_config_file() -> anyhow::Result<()> {
    let workx_home = TempDir::new()?;
    let base_config = workx_home.path().join(CONFIG_TOML_FILE);
    let selected_config = workx_home.path().join("work.config.toml");
...
fn profile_v2_config_path_resolves_validated_names() -> anyhow::Result<()> {
    ...
    assert_eq!(
        resolve_profile_v2_config_path(workx_home.path(), &profile_name),
        workx_home.path().join("work.config.toml").abs()
    );
    Ok(())
}
...
impl PrecedenceTestFixture {
    ...
    fn workx_home(&self) -> AbsolutePathBuf {
        self.workx_home.abs()
    }
}
...
async fn loads_compact_prompt_from_file() -> std::io::Result<()> {
    let workx_home = TempDir::new()?;
    let workspace = workx_home.path().join("workspace");
    ...
    let config =
        Config::load_from_base_config_with_overrides(cfg, overrides, workx_home.abs()).await?;
...
async fn load_config_uses_requirements_guardian_policy_config() -> std::io::Result<()> {
    ...
    let config_layer_stack = ConfigLayerStack::new(
        Vec::new(),
        Default::default(),
        workx_config::ConfigRequirementsToml {
...
async fn load_config_rejects_missing_agent_role_config_file() -> std::io::Result<()> {
    let workx_home = TempDir::new()?;
    let missing_path = workx_home.path().join("agents").join("researcher.toml");
    let cfg = ConfigToml {
...
async fn agent_role_relative_config_file_resolves_against_config_toml() -> std::io::Result<()> {
    let workx_home = TempDir::new()?;
    let role_config_path = workx_home.path().join("agents").join("researcher.toml");
...
"#,
    ...
    let config_layer_stack = workx_config::ConfigLayerStack::new(
        vec![workx_config::ConfigLayerEntry::new(
            ConfigLayerSource::User {
                file: workx_home.path().join(CONFIG_TOML_FILE).abs(),
                profile: None,
            },
            layer_config,
        )],
...
"#
    ...
    let standalone_agents_dir = repo_root.path().join(".workx").join("agents");
...
async fn loads_legacy_split_agent_roles_from_config_toml() -> std::io::Result<()> {
    let workx_home = TempDir::new()?;
    let researcher_path = workx_home.path().join("agents").join("researcher.toml");
    let reviewer_path = workx_home.path().join("agents").join("reviewer.toml");
    tokio::fs::create_dir_all(
        researcher_path
            .parent()
            .expect("role config should have a parent directory"),
...
"#
    ...
    let home_agents_dir = workx_home.path().join("agents");
...
async fn model_catalog_json_loads_from_path() -> std::io::Result<()> {
    let workx_home = TempDir::new()?;
    let catalog_path = workx_home.path().join("catalog.json");
...
"#;
    ...
    let workx_home_temp_dir = TempDir::new().unwrap();

    Ok(PrecedenceTestFixture {
        cwd: cwd_temp_dir,
        workx_home: workx_home_temp_dir,
        cfg,
    })
}
...
async fn trace_exporter_defaults_to_none_when_log_exporter_is_set() -> std::io::Result<()> {
    ...
    cfg.otel = Some(OtelConfigToml {
        ...
        exporter: Some(OtelExporterKind::OtlpHttp {
            ...
            headers: HashMap::new(),
            protocol: workx_config::types::OtelHttpProtocol::Binary,
            tls: None,
        }),
...
async fn config_toml_priority_service_tier_uses_priority_request_value() -> std::io::Result<()> {
    ...
    let workx_home = fixture.workx_home();
...
async fn test_requirements_web_search_mode_allowlist_does_not_warn_when_unset() -> anyhow::Result<()>
{
    ...
    let requirements_toml = workx_config::ConfigRequirementsToml {
        ...
        allowed_web_search_modes: Some(vec![workx_config::WebSearchModeRequirement::Cached]),
    ...
    let requirement_source = workx_config::RequirementSource::Unknown;
    ...
    let requirements = workx_config::ConfigRequirements {
        web_search_mode: workx_config::ConstrainedWithSource::new(
            constrained,
    ...
    let config_layer_stack =
        workx_config::ConfigLayerStack::new(Vec::new(), requirements, requirements_toml)
...
projects = { "/Users/mbolin/code/workx4" = { trust_level = "trusted", foo = "bar" } , "/Users/mbolin/code/workx3" = { trust_level = "trusted" } }
model = "foo""#;
    let mut doc = initial.parse::<DocumentMut>()?;

    // Approve a new directory
    let new_project = Path::new("/Users/mbolin/code/workx2");
...
[projects."/Users/mbolin/code/workx4"]
trust_level = "trusted"
foo = "bar"

[projects."/Users/mbolin/code/workx3"]
trust_level = "trusted"
...
fn test_set_default_oss_provider() -> std::io::Result<()> {
    let temp_dir = TempDir::new()?;
    let workx_home = temp_dir.path();
    let config_path = workx_home.join(CONFIG_TOML_FILE);

    // Test setting valid provider on empty config
    set_default_oss_provider(workx_home, OLLAMA_OSS_PROVIDER_ID)?;
    let content = std::fs::read_to_string(&config_path)?;
    assert!(content.contains("oss_provider = \"ollama\""));

    // Test updating existing config
    std::fs::write(&config_path, "model = \"gpt-4\"\n")?;
    set_default_oss_provider(workx_home, LMSTUDIO_OSS_PROVIDER_ID)?;
    let content = std::fs::read_to_string(&config_path)?;
    ...
    let result = set_default_oss_provider(workx_home, "invalid_provider");
...
fn test_set_default_oss_provider_rejects_legacy_ollama_chat_provider() -> std::io::Result<()> {
    ...
    let result = set_default_oss_provider(workx_home, LEGACY_OLLAMA_CHAT_PROVIDER_ID);
    assert!(result.is_err());
    let error = result.unwrap_err();
    assert_eq!(error.kind(), std::io::ErrorKind::InvalidInput);
    assert!(
...
-> std::io::Result<()> {
    ...
    let instructions = "x".repeat(workx_utils_string::approx_bytes_for_tokens(
        /*tokens*/ 10_000,
    ));
...
"#,
    ...
    assert_eq!(
        ...
        Some(workx_config::types::WindowsSandboxModeToml::Elevated)
    );
    assert!(
...
async fn permission_profile_override_preserves_split_write_roots() -> std::io::Result<()> {
    let workx_home = TempDir::new()?;
    let cwd = workx_home.path().join("workspace");
    let outside_root = workx_home.path().join("outside-write");
...
-> std::io::Result<()> {
    let workx_home = TempDir::new()?;
    let selected_config = workx_home.path().join("default.config.toml");
    std::fs::write(
        &selected_config,
        r#"approvals_reviewer = "user"
"#,
...
async fn exact_requirements_apply_to_runtime_config() -> std::io::Result<()> {
    let workx_home = TempDir::new()?;
    let catalog_path = workx_home.path().join("required-models.json");
    let mut catalog = bundled_models_response()
        .unwrap_or_else(|err| panic!("bundled models.json should parse: {err}"));
    catalog.models = catalog.models.into_iter().take(1).collect();
    std::fs::write(
...
"#,
    ...
    let required_sqlite_home = workx_home.path().join("required-state");
    let required_log_dir = workx_home.path().join("required-logs");
...
"#,
    ...
    let config = load_with_enterprise_requirement(&workx_home, requirements).await?;
...
async fn absent_allow_login_shell_does_not_report_an_override() -> std::io::Result<()> {
    let workx_home = TempDir::new()?;
    let config = load_with_enterprise_requirement(&workx_home, "allow_login_shell = false").await?;
...
fn sqlite_home_env_conflict_reports_an_override() -> std::io::Result<()> {
    let workx_home = TempDir::new()?;
    let required = AbsolutePathBuf::try_from(workx_home.path().join("required-state"))?;
    let environment = workx_home.path().join("environment-state");
    ...
    assert_eq!(
        warnings,
        vec![format!(
            "Environment value for `$WORKX_SQLITE_HOME` is overridden by the required `sqlite_home` value {required:?} from {}.",
            RequirementSource::Unknown
        )]
    );

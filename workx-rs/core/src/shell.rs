...
pub use workx_shell_command::shell_detect::ShellType;
...
fn ultimate_fallback_shell() -> Shell {
    workx_shell_command::shell_detect::ultimate_fallback_shell().into()
}

pub fn get_shell_by_model_provided_path(shell_path: &PathBuf) -> Shell {
    workx_shell_command::shell_detect::get_shell_by_model_provided_path(shell_path).into()
}

pub fn get_shell(shell_type: ShellType) -> Option<Shell> {
    workx_shell_command::shell_detect::get_shell(shell_type).map(Into::into)
}

pub fn default_user_shell() -> Shell {
    workx_shell_command::shell_detect::default_user_shell().into()
}

#[cfg(all(test, target_os = "macos"))]
fn default_user_shell_from_path(user_shell_path: Option<PathBuf>) -> Shell {
    workx_shell_command::shell_detect::default_user_shell_from_path(user_shell_path).into()
}

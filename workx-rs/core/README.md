...
This crate implements the business logic for Workx. It is designed to be used by the various Workx UIs written in Rust.

## Wine-exec integration tests

On x86-64 Linux, run the shared suite against the Windows exec server with
`bazel test //workx-rs/core:core-all-wine-exec-test`.
...
Note that `workx-core` makes some assumptions about certain helper utilities being available in the environment. Currently, this support matrix is:
...
pointer file), the resolved `gitdir:` target, and `.workx` read-only.
...
Expects the binary containing `workx-core` to run the equivalent of `workx sandbox` when `arg0` is `workx-linux-sandbox`. See the `workx-arg0` crate for details.
...
`bwrap` is missing, it falls back to the bundled `workx-resources/bwrap`
binary shipped with Workx and Workx surfaces a startup warning through its
normal notification path instead of printing directly from the sandbox helper.
Workx also surfaces a startup warning when bubblewrap cannot create user
...
namespaces, so Workx rejects sandboxed shell commands that would enter the
...
Expects the binary containing `workx-core` to simulate the virtual
`apply_patch` CLI when `arg1` is `--workx-run-as-apply-patch`. See the
`workx-arg0` crate for details.

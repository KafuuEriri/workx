...
impl McpProcess {
    pub async fn new(workx_home: &Path) -> anyhow::Result<Self> {
        Self::new_with_env(workx_home, &[]).await
    }
    ...
    pub async fn new_with_env(
        ...
        let program = workx_utils_cargo_bin::cargo_bin("workx-mcp-server")
            .context("should find binary for workx-mcp-server")?;
        ...
        let mut process = cmd
            .kill_on_drop(true)
            .spawn()
            .context("workx-mcp-server proc should start")?;
        let stdin = process
            .stdin
            .take()
    ...
    pub async fn initialize(&mut self) -> anyhow::Result<()> {
        ...
        let originator = workx_login::default_client::originator().value;
    ...
    pub async fn send_workx_tool_call(
        &mut self,
        params: WorkxToolCallParam,
    ) -> anyhow::Result<i64> {
        let workx_tool_call_params = CallToolRequestParams::new("workx").with_arguments(
        ...
        self.send_request(
            "tools/call",
            Some(serde_json::to_value(workx_tool_call_params)?),
        )
        .await
    }
    ...
    /// Method "workx/event" with params.msg.type == "task_complete".
    pub async fn read_stream_until_legacy_task_complete_notification(
        ...
        self.read_stream_until_workx_event("task_complete").await
    }

    pub async fn read_stream_until_workx_event(
        &mut self,
        event_type: &str,
    ) -> anyhow::Result<JsonRpcNotification<CustomNotification>> {
        self.read_stream_until_workx_event_matching(event_type, |_| true)
            .await
    }

    pub async fn read_stream_until_workx_event_matching(
        &mut self,
        event_type: &str,
        predicate: impl Fn(&serde_json::Value) -> bool,
    ) -> anyhow::Result<JsonRpcNotification<CustomNotification>> {
        eprintln!("in read_stream_until_workx_event({event_type})");

        loop {
            let message = self.read_jsonrpc_message().await?;
            match message {
                JsonRpcMessage::Notification(notification) => {
                    let is_match = notification.notification.method == "workx/event"
                        && notification
                            .notification
                            .params
                            .as_ref()
...
impl Drop for McpProcess {
    fn drop(&mut self) {
        // These tests spawn a `workx-mcp-server` child process.
        //

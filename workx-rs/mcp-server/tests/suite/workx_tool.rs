...
use workx_core::spawn::WORKX_SANDBOX_NETWORK_DISABLED_ENV_VAR;
use workx_mcp_server::ExecApprovalElicitRequestParams;
use workx_mcp_server::ExecApprovalResponse;
use workx_mcp_server::PatchApprovalElicitRequestParams;
use workx_mcp_server::PatchApprovalResponse;
use workx_mcp_server::WorkxToolCallParam;
use workx_protocol::protocol::FileChange;
use workx_protocol::protocol::ReviewDecision;
use workx_shell_command::parse_command;
...
async fn test_exec_command_approval_triggers_elicitation() {
    if env::var(WORKX_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok() {
        println!(
            "Skipping test because it cannot execute when network is disabled in a Workx sandbox."
        );
        return;
    }
...
async fn exec_command_approval_triggers_elicitation() -> anyhow::Result<()> {
    ...
    // Send a "workx" tool request, which should hit the responses endpoint.
    // In turn, it should reply with a tool call, which the MCP should forward
    // as an elicitation.
    let workx_request_id = mcp_process
        .send_workx_tool_call(WorkxToolCallParam {
    ...
    assert_eq!(
        ...
        Some(create_expected_elicitation_request_params(
            ...
            workx_request_id.to_string(),
            params.workx_event_id.clone(),
            params.thread_id,
        )?)
    );
    ...
    // Verify the original `workx` tool call completes and that the file was created.
    let workx_response = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp_process.read_stream_until_response_message(RequestId::Number(workx_request_id)),
    )
    .await??;
    assert_eq!(
        JsonRpcResponse {
            jsonrpc: JsonRpcVersion2_0,
            id: RequestId::Number(workx_request_id),
            result: json!({
                "content": [
                    {
                        "text": "File created!",
...
fn create_expected_elicitation_request_params(
    command: Vec<String>,
    workdir: &Path,
    workx_mcp_tool_call_id: String,
    workx_event_id: String,
    thread_id: workx_protocol::ThreadId,
) -> anyhow::Result<serde_json::Value> {
    let expected_message = format!(
        "Allow Workx to run `{}` in `{}`?",
        shlex::try_join(command.iter().map(std::convert::AsRef::as_ref))?,
        workdir.to_string_lossy()
    );
    let workx_parsed_cmd = parse_command::parse_command(&command);
    let params_json = serde_json::to_value(ExecApprovalElicitRequestParams {
        message: expected_message,
        requested_schema: json!({"type":"object","properties":{}}),
        thread_id,
        workx_elicitation: "exec-approval".to_string(),
        workx_mcp_tool_call_id,
        workx_event_id,
        workx_command: command,
        workx_cwd: workdir.to_path_buf(),
        workx_call_id: "call1234".to_string(),
        workx_parsed_cmd,
    })?;
    Ok(params_json)
}
...
async fn patch_approval_triggers_elicitation() -> anyhow::Result<()> {
    ...
    // Send a "workx" tool request that will trigger the apply_patch command
    let workx_request_id = mcp_process
        .send_workx_tool_call(WorkxToolCallParam {
    ...
    // Verify the original `workx` tool call completes
    let workx_response = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp_process.read_stream_until_response_message(RequestId::Number(workx_request_id)),
    )
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_workx_tool_passes_base_instructions() {
    skip_if_no_network!();

    // Apparently `#[tokio::test]` must return `()`, so we create a helper
    // function that returns `Result` so we can use `?` in favor of `unwrap`.
    workx_tool_passes_base_instructions()
        .await
        .expect("workx tool should pass base instructions");
}

async fn workx_tool_passes_base_instructions() -> anyhow::Result<()> {
    ...
    // Run `workx mcp` with a specific config.toml.
    let workx_home = TempDir::new()?;
    create_config_toml(workx_home.path(), &server.uri())?;
    let skill_dir = workx_home.path().join("skills").join("demo");
    ...
    let mut mcp_process = McpProcess::new_with_env(
        workx_home.path(),
        &[("OPENAI_API_KEY", None), ("WORKX_ACCESS_TOKEN", None)],
    )
    .await?;
    ...
    .await??;
    assert_eq!(workx_response.jsonrpc, JsonRpcVersion2_0);
    assert_eq!(workx_response.id, RequestId::Number(workx_request_id));
    assert_eq!(
        workx_response.result,
        json!({
            "content": [
                {
                    "text": "Enjoy!",
            ...
            "structuredContent": {
                "threadId": workx_response
                    .result
                    .get("structuredContent")
                    .and_then(|v| v.get("threadId"))
                    .and_then(serde_json::Value::as_str)
                    .expect("workx tool response should include structuredContent.threadId"),
                "content": "Enjoy!"
            }
        })
    );
    ...
    assert_eq!(
        developer_text
            .matches("Co-authored-by: Workx <noreply@openai.com>")
            .count(),
        1
    );
    assert_eq!(
        developer_text
            .matches("Generated with [Workx](https://openai.com/workx/).")
            .count(),
        1
    );
    assert_eq!(
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_workx_tool_forwards_skills_extension_warnings() {
    skip_if_no_network!();

    workx_tool_forwards_skills_extension_warnings()
        .await
        .expect("workx tool should forward skills extension warnings");
}

async fn workx_tool_forwards_skills_extension_warnings() -> anyhow::Result<()> {
    let server =
        create_mock_responses_server(vec![create_final_assistant_message_sse_response("Enjoy!")?])
            .await;
    let workx_home = TempDir::new()?;
    create_config_toml(workx_home.path(), &server.uri())?;
    let skills_dir = workx_home.path().join("skills");
    for index in 0..200 {
    ...
    let mut mcp_process = McpProcess::new(workx_home.path()).await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp_process.initialize()).await??;
    ...
    let warning = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp_process.read_stream_until_workx_event_matching("warning", |params| {
            params["msg"]["message"]
                .as_str()
...
fn create_expected_patch_approval_elicitation_request_params(
    ...
    message_lines.push("Allow Workx to apply proposed code changes?".to_string());
    let params_json = serde_json::to_value(PatchApprovalElicitRequestParams {
        message: message_lines.join("\n"),
        requested_schema: json!({"type":"object","properties":{}}),
        thread_id,
        workx_elicitation: "patch-approval".to_string(),
        workx_mcp_tool_call_id,
        workx_event_id,
        workx_reason: reason,
        workx_grant_root: grant_root,
        workx_changes: changes,
        workx_call_id: "call1234".to_string(),
    })?;
...
/// Create a Workx config that uses the mock server as the model provider.
/// The command explicitly requests escalation so that we exercise the
/// elicitation code path.
fn create_config_toml(workx_home: &Path, server_uri: &str) -> std::io::Result<()> {

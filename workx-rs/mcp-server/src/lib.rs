...
mod patch_approval;
mod workx_tool_config;
mod workx_tool_runner;
...
pub use crate::workx_tool_config::WorkxToolCallParam;
pub use crate::workx_tool_config::WorkxToolCallReplyParam;
...
const OTEL_SERVICE_NAME: &str = "workx_mcp_server";
...
pub async fn run_main(
    ...
    reject_workload_identity(workx_login::is_workload_identity_selected())?;
...
fn reject_workload_identity(workload_identity_selected: bool) -> IoResult<()> {
    if workload_identity_selected {
        return Err(std::io::Error::new(
            ...
            "workload identity is not supported by `workx mcp-server`",
        ));
    }
    Ok(())
}

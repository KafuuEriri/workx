...
use workx_login::default_client::USER_AGENT_SUFFIX;
...
use crate::workx_tool_config::WorkxToolCallParam;
use crate::workx_tool_config::WorkxToolCallReplyParam;
use crate::workx_tool_config::create_tool_for_workx_tool_call_param;
use crate::workx_tool_config::create_tool_for_workx_tool_call_reply_param;
...
impl MessageProcessor {
    ...
    pub(crate) async fn new(
        ...
        workx_image_generation_extension::install(
            &mut extensions,
            auth_manager.clone(),
            |config: &Config| Some(config.workx_home.clone()),
        );
        let skill_providers = workx_skills_extension::SkillProviders::new()
            .with_host_provider(Arc::new(workx_skills_extension::HostSkillProvider::new()));
        workx_skills_extension::install_with_providers_and_metrics(
            &mut extensions,
            skill_providers,
            workx_otel::global(),
            |config: &Config| workx_skills_extension::SkillsExtensionConfig {
        ...
        let thread_manager = Arc::new(ThreadManager::new(
            ...
            workx_core::build_models_manager(config.as_ref(), auth_manager),
            ...
            workx_core::thread_store_from_config(config.as_ref(), state_db.clone()),
    ...
    async fn handle_initialize(
        ...
        let server_info =
            Implementation::new("workx-mcp-server", env!("CARGO_PKG_VERSION")).with_title("Workx");

        // Preserve Workx's existing non-spec `serverInfo.user_agent` field.
        ...
        if let serde_json::Value::Object(ref mut obj) = server_info_value {
            obj.insert("user_agent".to_string(), json!(get_workx_user_agent()));
        }
    ...
    async fn handle_list_tools(
        ...
        let result = rmcp::model::ListToolsResult::with_all_items(vec![
            create_tool_for_workx_tool_call_param(),
            create_tool_for_workx_tool_call_reply_param(),
        ]);
    ...
    async fn handle_call_tool(&self, id: RequestId, params: CallToolRequestParams) {
        ...
        match name.as_ref() {
            "workx" => self.handle_tool_call_workx(id, arguments).await,
            "workx-reply" => {
                self.handle_tool_call_workx_session_reply(id, arguments)
                    .await
            }
            _ => {
    ...
    async fn handle_tool_call_workx(
        &self,
        id: RequestId,
        ...
        let (initial_prompt, config): (String, Config) = match arguments {
            Some(json_val) => match serde_json::from_value::<WorkxToolCallParam>(json_val) {
                Ok(tool_cfg) => match tool_cfg.into_config(self.arg0_paths.clone()).await {
                    ...
                    Err(e) => {
                        let result = CallToolResult::error(vec![rmcp::model::ContentBlock::text(
                            format!("Failed to load Workx configuration from overrides: {e}"),
                        )]);
                        self.outgoing.send_response(id, result);
                        return;
                    }
                ...
                Err(e) => {
                    let result = CallToolResult::error(vec![rmcp::model::ContentBlock::text(
                        format!("Failed to parse configuration for Workx tool: {e}"),
                    )]);
                    self.outgoing.send_response(id, result);
                    return;
                }
            ...
            None => {
                let result = CallToolResult::error(vec![rmcp::model::ContentBlock::text(
                    "Missing arguments for workx tool-call; the `prompt` field is required.",
                )]);
                self.outgoing.send_response(id, result);
                return;
            }
        ...
        // Spawn an async task to handle the Workx session so that we do not
        // block the synchronous message-processing loop.
        task::spawn(async move {
            // Run the Workx session and stream events back to the client.
            crate::workx_tool_runner::run_workx_tool_session(
                id,
                initial_prompt,
                config,
                outgoing,
    ...
    async fn handle_tool_call_workx_session_reply(
        &self,
        ...
        // parse arguments
        let workx_tool_call_reply_param: WorkxToolCallReplyParam = match arguments {
            Some(json_val) => match serde_json::from_value::<WorkxToolCallReplyParam>(json_val) {
                Ok(params) => params,
                Err(e) => {
                    tracing::error!("Failed to parse Workx tool call reply parameters: {e}");
                    let result = CallToolResult::error(vec![rmcp::model::ContentBlock::text(
                        format!("Failed to parse configuration for Workx tool: {e}"),
                    )]);
            ...
            None => {
                tracing::error!(
                    "Missing arguments for workx-reply tool-call; the `thread_id` and `prompt` fields are required."
                );
                let result = CallToolResult::error(vec![rmcp::model::ContentBlock::text(
                    "Missing arguments for workx-reply tool-call; the `thread_id` and `prompt` fields are required.",
                )]);
        ...
        let thread_id = match workx_tool_call_reply_param.get_thread_id() {
            Ok(id) => id,
            Err(e) => {
        ...
        let workx = match self.thread_manager.get_thread(thread_id).await {
            Ok(c) => c,
            Err(_) => {
                tracing::warn!("Session not found for thread_id: {thread_id}");
                let result = crate::workx_tool_runner::create_call_tool_result_with_thread_id(
                    thread_id,
        ...
        let prompt = workx_tool_call_reply_param.prompt.clone();
        tokio::spawn({
            let outgoing = outgoing.clone();
            let active_turns = Arc::clone(&active_turns);

            async move {
                crate::workx_tool_runner::run_workx_tool_session_reply(
                    thread_id,
                    workx,
                    outgoing,
                    request_id,
    ...
    async fn handle_cancelled_notification(&self, params: rmcp::model::CancelledNotificationParam) {
        ...
        // Obtain the Workx thread from the server.
        let workx_arc = match self.thread_manager.get_thread(thread_id).await {
            Ok(c) => c,
            Err(_) => {
                tracing::warn!("Session not found for thread_id: {thread_id}");
                return;
        ...
        // Submit interrupt to Workx.
        if let Err(e) = workx_arc
            .submit(workx_protocol::protocol::Op::Interrupt)
            .await
        {
            tracing::error!("Failed to submit interrupt to Workx: {e}");
            return;
        }

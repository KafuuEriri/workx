...
pub struct ExecApprovalElicitRequestParams {
    ...
    // correlate the request with the workx tool call.
    #[serde(rename = "threadId")]
    pub thread_id: ThreadId,
    pub workx_elicitation: String,
    pub workx_mcp_tool_call_id: String,
    pub workx_event_id: String,
    pub workx_call_id: String,
    pub workx_command: Vec<String>,
    pub workx_cwd: PathBuf,
    pub workx_parsed_cmd: Vec<ParsedCommand>,
}
...
pub(crate) async fn handle_exec_approval_request(
    ...
    call_id: String,
    approval_id: String,
    workx_parsed_cmd: Vec<ParsedCommand>,
    thread_id: ThreadId,
) {
    let escaped_command =
        shlex::try_join(command.iter().map(String::as_str)).unwrap_or_else(|_| command.join(" "));
    let message = format!(
        "Allow Workx to run `{escaped_command}` in `{cwd}`?",
    ...
    let params = ExecApprovalElicitRequestParams {
        ...
        workx_mcp_tool_call_id: tool_call_id.clone(),
        workx_event_id: event_id.clone(),
        workx_call_id: call_id,
    ...
    {
        let workx = workx.clone();
        let approval_id = approval_id.clone();
        let event_id = event_id.clone();
        tokio::spawn(async move {
            on_exec_approval_response(approval_id, event_id, on_response, workx).await;
        });
    }
}
...
async fn on_exec_approval_response(
    ...
    // Try to deserialize `value` and then make the appropriate call to `workx`.

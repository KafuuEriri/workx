//! Asynchronous worker that executes a **Workx** tool-call inside a spawned
...
use workx_core::config::Config as WorkxConfig;
...
/// To adhere to MCP `tools/call` response format, include the Workx
...
/// Run a complete Workx session and stream events back to the client.
///
/// On completion (success or error) the function sends the appropriate
/// `tools/call` response so the LLM can continue the conversation.
pub async fn run_workx_tool_session(
    id: RequestId,
        ...
        .await
    {
        ...
        Err(e) => {
            let result = CallToolResult::error(vec![ContentBlock::text(format!(
                "Failed to start Workx session: {e}"
            ))]);
    ...
    run_workx_tool_session_inner(thread_id, thread, outgoing, id, active_turns).await;
}

pub async fn run_workx_tool_session_reply(
    thread_id: ThreadId,
    ...
    run_workx_tool_session_inner(thread_id, thread, outgoing, request_id, active_turns).await;
}

async fn run_workx_tool_session_inner(
    thread_id: ThreadId,
    thread: Arc<WorkxThread>,
    outgoing: Arc<OutgoingMessageSender>,
    request_id: RequestId,
            ...
            Ok(event) => {
                ...
                match event.msg {
                    ...
                    | EventMsg::DeprecationNotice(_) => {
                        ...
                        // events. Note that
                        // send(workx_event_to_notification(&event)) above has

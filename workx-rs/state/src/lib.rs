...
//! orchestration and rollout scanning live in `workx-core`.
...
pub const SQLITE_HOME_ENV: &str = "WORKX_SQLITE_HOME";

/// Errors encountered during DB operations. Tags: [stage]
pub const DB_ERROR_METRIC: &str = "workx.db.error";
/// Metrics on backfill process. Tags: [status]
pub const DB_METRIC_BACKFILL: &str = "workx.db.backfill";
/// Metrics on backfill duration. Tags: [status]
pub const DB_METRIC_BACKFILL_DURATION_MS: &str = "workx.db.backfill.duration_ms";
/// SQLite initialization attempts. Tags: [status, phase, db, error]
pub const DB_INIT_METRIC: &str = "workx.sqlite.init.count";
/// SQLite initialization latency. Tags: [status, phase, db, error]
pub const DB_INIT_DURATION_METRIC: &str = "workx.sqlite.init.duration_ms";
/// Rollout fallback attempts. Tags: [caller, reason]
pub const DB_FALLBACK_METRIC: &str = "workx.sqlite.fallback.count";
/// SQLite log batch write attempts. Tags: [status, error]
pub const LOG_WRITE_METRIC: &str = "workx.sqlite.logs.write.count";
/// SQLite log batch write latency. Tags: [status, error]
pub const LOG_WRITE_DURATION_METRIC: &str = "workx.sqlite.logs.write.duration_ms";
/// Estimated bytes in each SQLite log batch. Tags: [status, error]
pub const LOG_WRITE_BYTES_METRIC: &str = "workx.sqlite.logs.write.bytes";
/// Number of entries in each SQLite log batch. Tags: [status, error]
pub const LOG_WRITE_ENTRIES_METRIC: &str = "workx.sqlite.logs.write.entries";
/// Largest estimated entry size in each SQLite log batch. Tags: [status, error]
pub const LOG_WRITE_MAX_ENTRY_BYTES_METRIC: &str = "workx.sqlite.logs.write.max_entry_bytes";
/// SQLite log entries discarded before they can be queued. Tags: [reason]
pub const LOG_QUEUE_DROPPED_METRIC: &str = "workx.sqlite.logs.queue.dropped";

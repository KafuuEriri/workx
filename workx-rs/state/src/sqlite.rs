...
impl RuntimeDbSpec {
    fn path(self, workx_home: &Path) -> PathBuf {
        workx_home.join(self.filename)
    }
}
...
/// Resolved configuration shared by all Workx SQLite connections.
#[derive(Clone, Debug, Eq, PartialEq)]
...
impl SqliteConfig {
    ...
    /// Open a writable Workx SQLite database, creating it if necessary.
    ...
    /// Open an existing Workx SQLite database without creating or modifying it.

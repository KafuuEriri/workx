...
async fn sqlite_sink_filters_noisy_targets_without_dropping_useful_diagnostics() {
    let workx_home =
        std::env::temp_dir().join(format!("workx-state-log-db-filter-{}", Uuid::new_v4()));
    let _cleanup = scopeguard::guard(workx_home.clone(), |workx_home| {
        let _ = std::fs::remove_dir_all(workx_home);
    });
    ...
    tracing::debug!(
        target: "workx_rmcp_client::oauth",
        "dropped-workx-rmcp-client-debug"
    );
    tracing::info!(
        target: "workx_rmcp_client::oauth",
        "retained-workx-rmcp-client-info"
    );
    tracing::trace!(target: "workx_http_client::transport", "dropped-request-body");
    tracing::debug!(target: "workx_http_client::transport", "retained-request-diagnostic");
    tracing::trace!(target: "workx_api::sse", "dropped-sse-parent");
    tracing::trace!(target: "workx_api::sse::responses", "dropped-sse-payload");
    tracing::debug!(target: "workx_api::sse::responses", "retained-sse-diagnostic");
    tracing::trace!(target: "workx_state", "retained-trace");
    tracing::trace!(
        target: "workx_tui::streaming::controller",
    ...
    tracing::trace!(
        target: "workx_tui::streaming::table_holdback",
    ...
    tracing::trace!(
        target: "workx_tui::streaming::commit_tick",
        "retained-commit-tick-trace"
    );
    tracing::trace!(
        target: "workx_api::responses_websocket_timing",
    ...
    assert_eq!(
        ...
        vec![
            ...
            (
                "INFO",
                "workx_rmcp_client::oauth",
                Some("retained-workx-rmcp-client-info")
            ),
            (
                "DEBUG",
                "workx_http_client::transport",
                Some("retained-request-diagnostic")
            ),
            (
                "DEBUG",
                "workx_api::sse::responses",
                Some("retained-sse-diagnostic")
            ),
            ("TRACE", "workx_state", Some("retained-trace")),
            (
                "DEBUG",
                "workx_tui::streaming::controller",
                Some("retained-controller-debug"),
            ),
            (
                "DEBUG",
                "workx_tui::streaming::table_holdback",
                Some("retained-table-holdback-debug"),
            ),
            (
                "TRACE",
                "workx_tui::streaming::commit_tick",

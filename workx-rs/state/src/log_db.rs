...
//!
//! # async fn example(state_db: std::sync::Arc<workx_state::StateRuntime>) {
...
pub fn default_filter() -> Targets {
    Targets::new()
        ...
        .with_target("workx_rmcp_client", LevelFilter::INFO)
        ...
        .with_target("workx_otel.log_only", LevelFilter::OFF)
        .with_target("workx_otel.trace_safe", LevelFilter::OFF)
        ...
        .with_target("workx_tui::streaming::controller", LevelFilter::DEBUG)
        .with_target("workx_tui::streaming::table_holdback", LevelFilter::DEBUG)
}
    ...
    S: tracing::Subscriber + for<'a> LookupSpan<'a>,
{
    ...
    fn on_event(&self, event: &Event<'_>, ctx: tracing_subscriber::layer::Context<'_, S>) {
        ...
        // were over 30% of retained logs in measured high-fanout Workx environments.
...
mod tests {
    ...
    fn temp_workx_home() -> std::path::PathBuf {
        std::env::temp_dir().join(format!("workx-state-log-db-{}", Uuid::new_v4()))
    }
    ...
    async fn sqlite_feedback_logs_match_feedback_formatter_shape() {
        let workx_home = temp_workx_home();

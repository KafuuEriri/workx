...
ON CONFLICT(id) DO NOTHING
        ...
        .bind(
            metadata
                .thread_source
                .as_ref()
                .map(workx_protocol::protocol::ThreadSource::as_str),
        )
...
mod tests {
    ...
    async fn upsert_thread_keeps_creation_memory_mode_for_existing_rows() {
        ...
        let mut metadata = test_thread_metadata(&workx_home, thread_id, workx_home.clone());
    ...
    async fn thread_metadata_history_mode_does_not_downgrade() {
        ...
        let metadata = test_thread_metadata(&workx_home, thread_id, workx_home.clone());
    ...
    async fn list_threads_filters_by_cwd() {
        ...
        let first_cwd = workx_home.join("first");
        let second_cwd = workx_home.join("second");
        let other_cwd = workx_home.join("other");
        ...
        for (thread_id, cwd, updated_at) in [
            ...
            let mut metadata = test_thread_metadata(&workx_home, thread_id, cwd);
            metadata.updated_at =
    ...
    async fn apply_rollout_items_preserves_existing_git_branch_and_fills_missing_git_fields() {
        ...
        let items = vec![RolloutItem::SessionMeta(SessionMetaLine {
            ...
            git: Some(GitInfo {
                commit_hash: Some(workx_git_utils::GitSha::new("rollout-sha")),
        ...
        assert_eq!(
            ...
            Some("git@example.com:RonanXiao/workx.git")
        );
    }
    ...
    async fn insert_thread_if_absent_preserves_existing_metadata() {
        ...
        let mut existing = test_thread_metadata(&workx_home, thread_id, workx_home.clone());
        ...
        let mut fallback = test_thread_metadata(&workx_home, thread_id, workx_home.clone());
    ...
    async fn thread_updated_at_uses_unique_epoch_millis_and_reads_legacy_seconds() {
        ...
        let mut first = test_thread_metadata(&workx_home, first_id, workx_home.clone());
        first.updated_at = updated_at;
        first.recency_at = updated_at;
        let mut second = test_thread_metadata(&workx_home, second_id, workx_home.clone());
        ...
        let mut older = test_thread_metadata(&workx_home, older_id, workx_home.clone());
    ...
    async fn apply_rollout_items_uses_override_updated_at_when_provided() {
        ...
        let items = vec![RolloutItem::EventMsg(EventMsg::TokenCount(
            workx_protocol::protocol::TokenCountEvent {
                info: Some(workx_protocol::protocol::TokenUsageInfo {
                    total_token_usage: workx_protocol::protocol::TokenUsage {
                        input_tokens: 0,
                    ...
                    last_token_usage: workx_protocol::protocol::TokenUsage::default(),

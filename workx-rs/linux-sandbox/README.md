...
- a `workx-linux-sandbox` standalone executable for Linux that is bundled with the Node.js version of the Workx CLI
- a lib crate that exposes the business logic of the executable as `run_main()` so that
  - the `workx-exec` CLI can check if its arg0 is `workx-linux-sandbox` and, if so, execute as if it were `workx-linux-sandbox`
  - this should also be true of the `workx` multitool CLI

On Linux, Workx prefers the first `bwrap` found on `PATH`
...
the helper falls back to the bundled `workx-resources/bwrap` binary shipped
with Workx.
Workx also surfaces a startup warning when `bwrap` is missing so users know it
is falling back to the bundled helper. Workx surfaces the same startup warning
...
it cannot create the required user namespaces, so Workx rejects sandboxed shell
...
- If `bwrap` is missing, the helper falls back to the bundled
  `workx-resources/bwrap` path.
- If `bwrap` is missing, Workx also surfaces a startup warning instead of
  printing directly from the sandbox helper.
- If bubblewrap cannot create user namespaces, Workx surfaces a startup warning
  instead of waiting for a runtime sandbox failure.
- WSL2 uses the normal Linux bubblewrap path.
- WSL1 is not supported for bubblewrap sandboxing; Workx rejects sandboxed
...
- When bubblewrap is active, protected subpaths under writable roots (for
  example `.git`,
  resolved `gitdir:`, and `.workx`) are re-applied as read-only via `--ro-bind`.
...
**Notes**
- The CLI surface is `workx sandbox`; the host OS selects the sandbox backend.

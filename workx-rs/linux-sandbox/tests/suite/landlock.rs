...
use workx_core::exec::process_exec_tool_call;
...
fn workx_linux_sandbox_exe() -> PathBuf {
    let sandbox_program = PathBuf::from(env!("CARGO_BIN_EXE_workx-linux-sandbox"));
...
async fn run_cmd_output(
    ...
    timeout_ms: u64,
) -> workx_protocol::exec_output::ExecToolCallOutput {
...
async fn run_cmd_result_with_writable_roots(
    ...
    network_access: bool,
) -> Result<workx_protocol::exec_output::ExecToolCallOutput> {
...
async fn run_cmd_result_with_permission_profile_for_cwd(
    ...
    let workx_linux_sandbox_exe = Some(workx_linux_sandbox_exe());
...
fn is_bwrap_unavailable_output(output: &workx_protocol::exec_output::ExecToolCallOutput) -> bool {
...
async fn should_skip_bwrap_tests() -> bool {
    ...
    .await
    {
        ...
        Err(err) => match err.details() {
            ...
            WorkxErrorDetails::Sandbox(SandboxErr::Timeout { .. }) => true,
...
fn expect_denied(
    result: Result<workx_protocol::exec_output::ExecToolCallOutput>,
    context: &str,
) -> workx_protocol::exec_output::ExecToolCallOutput {
    match result {
        Ok(output) => {
...
async fn assert_network_blocked(cmd: &[&str]) {
    ...
    let workx_linux_sandbox_exe: Option<PathBuf> = Some(workx_linux_sandbox_exe());
...
#[tokio::test]
async fn sandbox_blocks_git_and_workx_writes_inside_writable_root() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    let dot_workx = tmpdir.path().join(".workx");
    std::fs::create_dir_all(&dot_git).expect("create .git");
    std::fs::create_dir_all(&dot_workx).expect("create .workx");

    let git_target = dot_git.join("config");
    let workx_target = dot_workx.join("config.toml");
    ...
    let workx_output = expect_denied(
        run_cmd_result_with_writable_roots(
            &[
                "bash",
                "-lc",
                &format!("echo denied > {}", workx_target.to_string_lossy()),
            ],
            &[tmpdir.path().to_path_buf()],
            LONG_TIMEOUT_MS,
            /*use_legacy_landlock*/ false,
        ...
        .await,
        ".workx write should be denied under bubblewrap",
    );
    assert_ne!(git_output.exit_code, 0);
    assert_ne!(workx_output.exit_code, 0);
}

#[tokio::test]
async fn sandbox_blocks_workx_symlink_replacement_attack() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    let tmpdir = tempfile::tempdir().expect("tempdir");
    let decoy = tmpdir.path().join("decoy-workx");
    std::fs::create_dir_all(&decoy).expect("create decoy dir");

    let dot_workx = tmpdir.path().join(".workx");
    symlink(&decoy, &dot_workx).expect("create .workx symlink");
    ...
    let workx_output = expect_denied(
        ...
        .await,
        ".workx symlink replacement should be denied",
    );
    assert_ne!(workx_output.exit_code, 0);
}

#[tokio::test]
async fn sandbox_reports_workx_symlink_build_failure_without_panicking() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    .await
    {
        Err(err) => match err.details() {
            ...
            details => panic!(".workx symlink build failure should deny: {details:?}"),
        },
        Ok(output) => panic!(".workx symlink build failure should deny: {output:?}"),
    };
...
async fn sandbox_rejects_symlinked_synthetic_mount_registry() {
    ...
    let registry = temp.path().join(format!(
        "workx-bwrap-synthetic-mount-targets-{effective_uid}"
    ));
...
registry="${{TMPDIR:-/tmp}}/workx-bwrap-synthetic-mount-targets-$(id -u)"
if touch "$registry/forged-marker" 2>/dev/null; then
  exit 22
fi
redirected_registry='{redirected_tmp}'/workx-bwrap-synthetic-mount-targets-$(id -u)
...
if grep -E '(^|[[:space:]])\.(git|workx|agents)(/|$)' status.before; then
  cat status.before
  exit 21
fi
"#,
    ...
    let mkdir_workx_output = expect_denied(
        run_cmd_result_with_cwd_and_writable_roots(
            &["mkdir", ".workx"],
            &subdir,
            std::slice::from_ref(&subdir),
        ...
        .await,
        "child .workx directory creation should be denied",
    );
    assert_ne!(mkdir_workx_output.exit_code, 0);
    assert!(!subdir.join(".workx").exists());
...
async fn sandbox_blocks_explicit_split_policy_carveouts_under_bwrap() {
    ...
    let sandbox_helper_dir = workx_linux_sandbox_exe()
        .parent()
...
async fn sandbox_starts_with_denied_tmp_without_exposing_registry() {
    ...
    let sandbox_helper = workx_linux_sandbox_exe();
...
registry="$TMPDIR/workx-bwrap-synthetic-mount-targets-$(id -u)"
test ! -e "$registry"
"#,
                ],
                cwd.clone(),

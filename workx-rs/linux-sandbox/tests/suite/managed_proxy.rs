...
use workx_network_proxy::PROXY_ATTRIBUTION_TOKEN_ENV_KEY;
use workx_network_proxy::write_attribution_frame;
...
fn linux_sandbox_command(
    ...
    let mut cmd = Command::new(env!("CARGO_BIN_EXE_workx-linux-sandbox"));
    cmd.args(args)
        .current_dir(cwd)
        .env_clear()
        .envs(env)
...
async fn unsupported_system_bwrap_falls_back_to_bundled_bwrap() {
    if option_env!("WORKX_BWRAP_SHA256")
    ...
    let Some(system_bwrap) = workx_sandboxing::find_system_bwrap_in_path() else {
    ...
    let sandbox_executable = tempdir.path().join("workx-linux-sandbox");
    let original_executable = env!("CARGO_BIN_EXE_workx-linux-sandbox");
    ...
    let resources_dir = tempdir.path().join("workx-resources");
    ...
    let output = run_linux_sandbox_direct(
        &[
            ...
            concat!(
                ...
                "fi; ",
                "if [ -n \"${WORKX_TEST_SHM_PATH:-}\" ]; then ",
                "test \"$(cat \"$WORKX_TEST_SHM_PATH\")\" = host-before; ",
                "printf sandbox-after >\"$WORKX_TEST_SHM_PATH\"; ",
                "fi; ",
                "stat -c '%d:%i' /dev",
            ),
        ],
...
async fn managed_proxy_mode_routes_through_bridge_and_blocks_direct_egress() {
    ...
    let sandbox_helper_dir = std::path::Path::new(env!("CARGO_BIN_EXE_workx-linux-sandbox"))
        .parent()
...
fn handoff_client() {
    let Ok(label) = std::env::var("WORKX_TEST_HANDOFF_LABEL") else {
        return;
    };
    ...
    let shared = std::env::var("WORKX_TEST_HANDOFF_SHARED").expect("shared directory");
    let shared = Path::new(&shared);
    let peer = std::env::var("WORKX_TEST_HANDOFF_PEER").expect("peer label");
...
async fn handoff_isolates_concurrent_endpoints_and_closes_privileged_descriptors() {
    ...
    for (label, peer, profile) in [
        ...
        env.insert("WORKX_TEST_HANDOFF_LABEL".to_string(), label.to_string());
        env.insert("WORKX_TEST_HANDOFF_PEER".to_string(), peer.to_string());
        env.insert(
            "WORKX_TEST_HANDOFF_SHARED".to_string(),

...
fn inserts_bwrap_argv0_before_command_separator() {
    ...
    apply_inner_command_argv0_for_launcher(
        &mut argv,
        /*supports_argv0*/ true,
        "/tmp/workx-arg0-session/workx-linux-sandbox".to_string(),
    );
    assert_eq!(
        argv,
        vec![
...
fn rewrites_inner_command_path_when_bwrap_lacks_argv0() {
    ...
    assert!(
        argv.windows(2)
            .any(|window| { window == ["--", "/tmp/workx-arg0-session/workx-linux-sandbox"] })
    );
}
...
fn rewrites_bwrap_helper_command_not_nested_user_command_when_current_exe_appears_later() {
    ...
    let mut argv = vec![
        ...
        "--workx-run-as-apply-patch".to_string(),
        "patch".to_string(),
    ];

...
//! - sensitive subpaths such as `.git`, `.agents`, and `.workx` remain
...
use workx_protocol::permissions::is_protected_metadata_name;
...
use workx_protocol::protocol::FileSystemSpecialPath;
use workx_protocol::protocol::WritableRoot;
    ...
    let missing_auto_metadata_read_only_project_root_subpaths: HashSet<PathBuf> =
        file_system_sandbox_policy
            ...
            .filter_map(|entry| {
                ...
                // protection consistently for `.git`, `.agents`, and `.workx`.
                ...
                if project_subpath != Path::new(".git")
                    ...
                    && project_subpath != Path::new(".workx")
                {
                    return None;
                }
...
mod tests {
    ...
    fn writable_roots_under_symlinked_ancestors_bind_real_target() {
        ...
        let real_workx = temp_dir.path().join("real-workx");
        let logical_workx = logical_home.join(".workx");
        let real_memories = real_workx.join("memories");
        let logical_memories = logical_workx.join("memories");
        ...
        std::os::unix::fs::symlink(&real_workx, &logical_workx)
            .expect("create symlinked workx home");
    ...
    fn missing_read_only_subpath_uses_empty_file_bind_data() {
        ...
        assert_empty_directory_mounted_read_only(&args.args, &workspace.join(".workx"));
    ...
    fn missing_child_git_under_parent_repo_is_mounted_read_only() {
        ...
        assert_eq!(
            synthetic_mount_target_paths(&args),
            vec![workspace.join(".workx"), dot_git, workspace.join(".agents")],
        );
        assert!(
    ...
    fn missing_project_root_metadata_carveouts_use_metadata_path_masks() {
        ...
        let dot_workx = path_to_string(&temp_dir.path().join(".workx"));
        ...
        assert_empty_directory_mounted_read_only(&args.args, Path::new(&dot_workx));
        ...
        assert!(synthetic_targets.contains(&PathBuf::from(&dot_workx)));
        assert_eq!(

...
use workx_sandboxing::find_system_bwrap_in_path;
        ...
        BubblewrapLauncher::Unavailable => {
            panic!(
                "bubblewrap is unavailable: no system bwrap was found on PATH and no bundled \
                 workx-resources/bwrap binary was found next to the Workx executable"
            )
        }
    }
}
...
mod tests {
    ...
    fn translates_fd_mounts_for_legacy_system_bubblewrap() {
        let mut argv = vec![
            ...
            "--".to_string(),
            "/usr/bin/workx-linux-sandbox".to_string(),

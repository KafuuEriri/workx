//! Trace bundle format, writer, and reducer for Workx rollouts.
//!
//! This crate owns the trace schema. Hot-path Workx code should depend on the
...
/// Shared recorder context for inference attempts within one Workx turn.
...
pub use thread::WORKX_ROLLOUT_TRACE_ROOT_ENV;
/// Request data for the canonical Workx tool boundary.
...
/// Append-only writer used by hot-path Workx instrumentation.

...
impl TraceReducer {
    ...
    pub(super) fn reduce_inference_request(
        ...
        // history, not the pre-compaction prompt. Any repeated developer/context prefix that Workx
    ...
    pub(super) fn reduce_inference_response(
        ...
        let Some((thread_id, workx_turn_id)) = self
            .rollout
            .inference_calls
            .get(inference_call_id)
            .map(|inference| (inference.thread_id.clone(), inference.workx_turn_id.clone()))
        else {
        ...
        for (offset, item) in items.into_iter().enumerate() {
            ...
            let item_id = if let Some(previous_item_id) = previous_snapshot.get(index) {
                if self.item_matches(previous_item_id, &item) {
                    ...
                    self.find_matching_snapshot_item(&previous_snapshot, &item_ids, &item)
                        .unwrap_or_else(|| {
                            self.create_conversation_item(
                                context.thread_id,
                                Some(context.workx_turn_id.to_string()),
                    ...
                    let workx_turn_id = context.workx_turn_id;
                    let thread_id = context.thread_id;
                    bail!(
                        "model conversation mismatch while reducing turn {workx_turn_id} for \

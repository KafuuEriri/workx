...
impl TraceReducer {
    ...
    /// Starts a Workx turn inside an existing thread.
    pub(super) fn start_workx_turn(
        &mut self,
        seq: RawEventSeq,
        wall_time_unix_ms: i64,
        ...
        if self.rollout.workx_turns.contains_key(&workx_turn_id) {
            bail!("duplicate workx turn start for {workx_turn_id}");
        }

        self.thread_mut(&thread_id)?;

        self.rollout.workx_turns.insert(
            workx_turn_id.clone(),
            WorkxTurn {
                workx_turn_id,
                thread_id,
    ...
    /// Marks a Workx turn terminal and validates any thread id carried by the raw event.
    pub(super) fn end_workx_turn(
        &mut self,
        seq: RawEventSeq,
        wall_time_unix_ms: i64,
        ...
        if let Some(event_thread_id) = thread_id.as_deref()
            && let Some(turn) = self.rollout.workx_turns.get(&workx_turn_id)
            && turn.thread_id != event_thread_id
        {
            bail!(
                "workx turn end for {workx_turn_id} used thread {event_thread_id}, \
                 but the turn belongs to {}",
                turn.thread_id
            );
        }

        let Some(turn) = self.rollout.workx_turns.get_mut(&workx_turn_id) else {
            bail!("workx turn end referenced unknown turn {workx_turn_id}");
        };

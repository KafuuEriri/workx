...
use crate::protocol_event::workx_turn_trace_event;
...
pub const WORKX_ROLLOUT_TRACE_ROOT_ENV: &str = "WORKX_ROLLOUT_TRACE_ROOT";
...
impl ThreadTraceContext {
    ...
    /// Starts a root thread trace from `WORKX_ROLLOUT_TRACE_ROOT`, or disables tracing.
    ///
    /// Trace startup is best-effort. A tracing failure must not make the Workx
    ...
    pub fn start_root_or_disabled(metadata: ThreadStartedTraceMetadata) -> Self {
        let Some(root) = std::env::var_os(WORKX_ROLLOUT_TRACE_ROOT_ENV) else {
    ...
    /// Emits typed Workx turn lifecycle events from protocol lifecycle events.
    pub fn record_workx_turn_event(&self, default_turn_id: &str, event: &EventMsg) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        };
        let Some(trace_event) =
            workx_turn_trace_event(context.thread_id.clone(), default_turn_id, event)
        else {
            return;
        };
    ...
    /// what Workx did while executing that boundary.
    pub fn record_tool_call_event(&self, workx_turn_id: impl Into<WorkxTurnId>, event: &EventMsg) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        };
        ...
        context.append_with_context_best_effort(workx_turn_id.into(), payload);
    }
    ...
    pub fn record_agent_result_interaction(
        &self,
        child_workx_turn_id: impl Into<WorkxTurnId>,
    ...
    pub fn record_workx_turn_started(&self, workx_turn_id: impl Into<WorkxTurnId>) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        };
        let workx_turn_id = workx_turn_id.into();
        context.append_with_context_best_effort(
            workx_turn_id.clone(),
            RawTraceEventPayload::WorkxTurnStarted {
                workx_turn_id,
                thread_id: context.thread_id.clone(),
            },
        );
    ...
    /// Builds reusable inference trace context for one Workx turn.
    ///
...
impl EnabledThreadTraceContext {
    ...
    fn append_with_context_best_effort(
        &self,
        workx_turn_id: WorkxTurnId,
        payload: RawTraceEventPayload,
    ) {
        let event_context = RawTraceEventContext {
            thread_id: Some(self.thread_id.clone()),
            workx_turn_id: Some(workx_turn_id),
        };

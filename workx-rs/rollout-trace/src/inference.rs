...
///
/// A Workx turn can create multiple attempts when auth recovery retries the
...
impl InferenceTraceContext {
    ...
    /// Builds an enabled context for all upstream attempts made by one Workx turn.
    pub fn enabled(
        writer: Arc<TraceWriter>,
        thread_id: AgentThreadId,
        workx_turn_id: WorkxTurnId,
...
impl InferenceTraceAttempt {
    ...
    pub fn record_started(&self, request: &impl Serialize) {
        ...
        append_with_context_best_effort(
            ...
            RawTraceEventPayload::InferenceStarted {
                ...
                workx_turn_id: attempt.context.workx_turn_id.clone(),
    ...
    /// trace-specific serialization rules. That keeps workx-core focused on
    ...
    /// Records a provider stream that Workx intentionally stopped consuming.
    ///
...
/// Workx received it, so this helper restores that content in the raw payload.
...
fn append_with_context_best_effort(
    ...
    let event_context = RawTraceEventContext {
        ...
        workx_turn_id: Some(context.workx_turn_id.clone()),
    };
...
mod tests {
    ...
    fn enabled_context_records_replayable_inference_attempt() -> anyhow::Result<()> {
        ...
        assert_eq!(inference.workx_turn_id, "turn-1");

//! Mapping from Workx protocol events into raw rollout-trace events.
//!
...
//! `workx-core`: this module translates the protocol surface into the smaller
//! trace vocabulary and keeps the mapping isolated inside `workx-rollout-trace`.
//!
...
use workx_protocol::protocol::PatchApplyStatus;
use workx_protocol::protocol::SubAgentActivityEvent;
...
use crate::WorkxTurnId;

pub(crate) struct WorkxTurnTraceEvent {
    pub context_turn_id: WorkxTurnId,
    pub payload: RawTraceEventPayload,
}

pub(crate) fn workx_turn_trace_event(
    thread_id: AgentThreadId,
    default_turn_id: &str,
    event: &EventMsg,
) -> Option<WorkxTurnTraceEvent> {
    match event {
        EventMsg::TurnStarted(event) => {
            let workx_turn_id = event.turn_id.clone();
            Some(WorkxTurnTraceEvent {
                context_turn_id: workx_turn_id.clone(),
                payload: RawTraceEventPayload::WorkxTurnStarted {
                    workx_turn_id,
                    thread_id,
                },
            })
        ...
        EventMsg::TurnComplete(event) => {
            ...
            Some(WorkxTurnTraceEvent {
                context_turn_id: workx_turn_id.clone(),
                payload: RawTraceEventPayload::WorkxTurnEnded {
                    workx_turn_id,
...
pub(crate) enum ToolRuntimePayload<'a> {
    ...
    CollabAgentSpawnBegin(&'a workx_protocol::protocol::CollabAgentSpawnBeginEvent),
    CollabAgentSpawnEnd(&'a workx_protocol::protocol::CollabAgentSpawnEndEvent),
    CollabAgentInteractionBegin(&'a workx_protocol::protocol::CollabAgentInteractionBeginEvent),
    CollabAgentInteractionEnd(&'a workx_protocol::protocol::CollabAgentInteractionEndEvent),
    CollabWaitingBegin(&'a workx_protocol::protocol::CollabWaitingBeginEvent),
    CollabWaitingEnd(&'a workx_protocol::protocol::CollabWaitingEndEvent),
    CollabCloseBegin(&'a workx_protocol::protocol::CollabCloseBeginEvent),
    CollabCloseEnd(&'a workx_protocol::protocol::CollabCloseEndEvent),
...
struct ExecCommandBeginTracePayload<'a> {
    ...
    command: &'a [String],
    cwd: String,
    parsed_cmd: &'a [workx_protocol::parse_command::ParsedCommand],

...
> **Privacy:** Rollout tracing is not telemetry. Workx does **not** upload or
> report these traces; it writes local bundles only when
> `WORKX_ROLLOUT_TRACE_ROOT` is set. Those local bundles can contain prompts,
...
during a Workx session. It records raw runtime evidence into a local bundle on
...
Hot-path Workx code does not try to build the final graph while the session is
...
model-visible conversation plus the runtime objects that explain how Workx got
there.
...
```mermaid
flowchart TD
    subgraph Runtime["workx-core runtime"]
...
one from `WORKX_ROLLOUT_TRACE_ROOT`; fresh spawned child threads derive their
...
Workx session fail just because diagnostic recording failed. Core emits raw
...
- `state.json`: optional reducer output written by `workx debug trace-reduce`.

`trace_id` identifies this diagnostic artifact. `rollout_id` identifies the
Workx rollout/session being observed. Keeping those separate lets us reason about
the stored trace without confusing it with the product-level session identity.

To reduce a bundle:

```bash
workx debug trace-reduce <trace-bundle>
```

By default this writes `<trace-bundle>/state.json`. Rust callers can also call
`workx_rollout_trace::replay_bundle` directly.

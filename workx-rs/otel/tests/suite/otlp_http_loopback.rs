...
use workx_otel::OtelExporter;
use workx_otel::OtelHttpProtocol;
use workx_otel::OtelProvider;
use workx_otel::OtelSettings;
use workx_otel::Result;
...
fn otlp_http_exporter_sends_metrics_to_collector() -> Result<()> {
    ...
    metrics.counter("workx.turns", /*inc*/ 1, &[("source", "test")])?;
    metrics.counter("workx.api_request", /*inc*/ 1, &[("status", "200")])?;
    metrics.record_duration(
        "workx.api_request.duration_ms",
        Duration::from_millis(100),
        &[("status", "200")],
    )?;
    metrics.counter("workx.conversation.turn.count", /*inc*/ 1, &[])?;
    metrics.record_duration(
    ...
    metrics.counter("workx.tool.call", /*inc*/ 1, &[("tool", "test")])?;
    metrics.record_duration(
        "workx.tool.call.duration_ms",
        Duration::from_millis(42),
        &[("tool", "test")],
    )?;
    metrics.histogram(
        "workx.turn.token_usage",
        /*value*/ 100,
        &[("token_type", "total")],
    )?;
    metrics.gauge_with_description(
        "workx.active",
        "Number of active Workx operations.",
        /*value*/ 1,
        &[("component", "test")],
    )?;
    metrics.shutdown()?;
...
-> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    let otel = OtelProvider::try_new(&OtelSettings {
        ...
        service_name: "workx-cli".to_string(),
        service_version: env!("CARGO_PKG_VERSION").to_string(),
        workx_home: PathBuf::from("."),
    ...
    tracing::subscriber::with_default(subscriber, || {
        ...
        tracing::event!(
            ...
            tracing::Level::INFO,
            event.name = "workx.test.log_exported",
    ...
    let body = String::from_utf8_lossy(&request.body);
    assert!(
        body.contains("workx.test.log_exported"),
...
-> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    let propagated_trace = tracing::subscriber::with_default(subscriber, || {
        ...
        tracing::event!(
            ...
            tracing::Level::INFO,
            event.name = "workx.test.trace_event",
    ...
    assert!(
        body.contains("workx.test.trace_event"),

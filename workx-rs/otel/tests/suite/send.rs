...
fn send_builds_payload_with_tags_and_histograms() -> Result<()> {
    let (metrics, exporter) =
        build_metrics_with_defaults(&[("service", "workx-cli"), ("env", "prod")])?;

    metrics.counter_with_description(
        "workx.turns",
        "Total number of Workx turns.",
        /*inc*/ 1,
    ...
    let counter = find_metric(&resource_metrics, "workx.turns").expect("counter metric missing");
    assert_eq!(counter.description(), "Total number of Workx turns.");
    ...
    let expected_counter_attributes = BTreeMap::from([
        ("service".to_string(), "workx-cli".to_string()),
    ...
    let (bounds, bucket_counts, sum, count) =
        histogram_data(&resource_metrics, "workx.tool_latency");
    ...
    let histogram_attrs = attributes_to_map(
        find_metric(&resource_metrics, "workx.tool_latency")
            ...
            .expect("workx.tool_latency histogram attributes should exist"),
    );
    ...
    let gauge = find_metric(&resource_metrics, "workx.active").expect("gauge metric missing");
    assert_eq!(gauge.description(), "Number of active Workx operations.");
...
fn histogram_uses_explicit_bucket_boundaries() -> Result<()> {
    ...
    assert_eq!(
        histogram_data(&latest_metrics(&exporter), "workx.payload_bytes"),
...
fn send_merges_default_tags_per_line() -> Result<()> {
    ...
    let resource_metrics = latest_metrics(&exporter);
    let alpha_metric =
        find_metric(&resource_metrics, "workx.alpha").expect("workx.alpha metric missing");
    ...
    let beta_metric =
        find_metric(&resource_metrics, "workx.beta").expect("workx.beta metric missing");
...
fn client_sends_enqueued_metric() -> Result<()> {
    ...
    metrics.counter("workx.turns", /*inc*/ 1, &[("model", "gpt-5.1")])?;
    metrics.shutdown()?;
...
fn shutdown_flushes_in_memory_exporter() -> Result<()> {
    ...
    metrics.counter("workx.turns", /*inc*/ 1, &[])?;
    metrics.shutdown()?;

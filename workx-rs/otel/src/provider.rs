...
impl OtelProvider {
    ...
    fn prepare_shutdown_worker(&mut self) -> io::Result<()> {
        self.prepare_shutdown_worker_with_spawner(|startup| {
            std::thread::Builder::new()
                .name("workx-otel-shutdown".to_string())
                .spawn(move || {
    ...
    pub fn workx_export_filter(meta: &tracing::Metadata<'_>) -> bool {
...
mod tests {
    ...
    fn log_export_target_excludes_trace_safe_events() {
        assert!(is_log_export_target("workx_otel.log_only"));
        assert!(is_log_export_target("workx_otel.network_proxy"));
        assert!(!is_log_export_target("workx_otel.trace_safe"));
        assert!(!is_log_export_target("workx_otel.trace_safe.debug"));
    }

    #[test]
    fn trace_export_target_only_includes_trace_safe_prefix() {
        assert!(is_trace_safe_target("workx_otel.trace_safe"));
        assert!(is_trace_safe_target("workx_otel.trace_safe.summary"));
        assert!(!is_trace_safe_target("workx_otel.log_only"));
        assert!(!is_trace_safe_target("workx_otel.network_proxy"));
    }
    ...
    fn cached_global_metrics_follow_reinstalled_provider() -> Result<(), Box<dyn Error>> {
        ...
        cached.counter("workx.after_transition", /*inc*/ 1, &[])?;
        initial.shutdown()?;
        ...
        names.sort_unstable();
        names.dedup();
        assert_eq!(names, vec!["workx.after_transition"]);
    ...
    fn statsig_disabled_metrics_are_not_exported() -> Result<(), Box<dyn Error>> {
        ...
        names.sort_unstable();
        names.dedup();
        assert_eq!(names, vec!["workx.turns"]);
    ...
    fn test_otel_settings() -> OtelSettings {
        OtelSettings {
            ...
            service_name: "workx-test".to_string(),

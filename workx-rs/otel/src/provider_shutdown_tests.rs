...
fn bounded_shutdown_survives_worker_guard_page_failure() {
    ...
    let temporary_directory = std::env::temp_dir().join(format!(
        "workx-otel-guard-page-{}-{unique_suffix}",
        std::process::id()
    ));
    ...
    subprocess
        ...
        .arg("--nocapture")
        .arg("--test-threads=1")
        .env("WORKX_OTEL_GUARD_PAGE_FAILURE_CHILD", "1")
        .env("WORKX_OTEL_GUARD_PAGE_FAILURE_OBSERVED", &observed_path);
...
fn bounded_shutdown_survives_worker_guard_page_failure_child() {
    if std::env::var_os("WORKX_OTEL_GUARD_PAGE_FAILURE_CHILD").is_none() {
        return;
    }
    ...
    let observed_path = std::env::var_os("WORKX_OTEL_GUARD_PAGE_FAILURE_OBSERVED")

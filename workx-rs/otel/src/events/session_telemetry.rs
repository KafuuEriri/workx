...
pub struct AuthEnvTelemetryMetadata {
    ...
    pub workx_api_key_env_present: bool,
    pub workx_api_key_env_enabled: bool,
...
pub struct SessionTelemetryMetadata {
    ...
    // Full canonical path when available; legacy nickname or thread ID otherwise.
...
impl SessionTelemetry {
    ...
    pub fn record_startup_phase(
        ...
        log_and_trace_event!(
            self,
            common: {
                event.name = "workx.startup_phase",
                startup.phase = phase,
    ...
    pub fn record_plugin_install_elicitation_sent(
        ...
        log_and_trace_event!(
            self,
            common: {
                event.name = "workx.plugin_install_elicitation_sent",
    ...
    pub fn record_plugin_install_suggestion(
        ...
        log_and_trace_event!(
            self,
            common: {
                event.name = "workx.plugin_install_suggestion",
                plugin_install.tool_type = tool_type,
                plugin_install.tool_id = tool_id,
                plugin_install.tool_name = tool_name,
    ...
    pub fn new(
        ...
        let agent_name = session_source
            ...
            .or_else(|| session_source.get_nickname())
            .unwrap_or_else(|| {
        ...
        Self {
            tool_result_log_config: ToolResultLogConfig::default(),
    ...
    pub fn record_responses(&self, handle_responses_span: &Span, event: &ResponseEvent) {
        ...
        match event {
            ...
            ResponseEvent::Completed {
                ...
                handle_responses_span.record(
                    "workx.usage.reasoning_output_tokens",
                    token_usage.reasoning_output_tokens,
                );
                handle_responses_span.record("workx.usage.total_tokens", token_usage.total_tokens);
            }
            _ => {}
        }
    }
    ...
    pub fn conversation_starts(
        ...
        log_and_trace_event!(
            self,
            common: {
                event.name = "workx.conversation_starts",
                provider_name = %provider_name,
                auth.env_openai_api_key_present = self.metadata.auth_env.openai_api_key_env_present,
                auth.env_workx_api_key_present = self.metadata.auth_env.workx_api_key_env_present,
                auth.env_workx_api_key_enabled = self.metadata.auth_env.workx_api_key_env_enabled,
    ...
    pub fn record_websocket_connect(
        ...
        log_and_trace_event!(
            self,
            common: {
                event.name = "workx.websocket_connect",
                duration_ms = %duration.as_millis(),
                http.response.status_code = status,
                success = success_str,
                error.message = error,
    ...
    pub fn record_websocket_request(
        ...
        log_and_trace_event!(
            self,
            common: {
                event.name = "workx.websocket_request",
                duration_ms = %duration.as_millis(),
                success = success_str,
                error.message = error,
                auth.env_openai_api_key_present = self.metadata.auth_env.openai_api_key_env_present,
    ...
    pub fn record_auth_recovery(
        ...
        log_and_trace_event!(
            self,
            common: {
                event.name = "workx.auth_recovery",
                auth.mode = mode,
                auth.step = step,
    ...
    pub fn tool_decision(
        ...
        let tool_namespace = crate::events::shared::tool_namespace(tool_name);
        match source {
            Some(source) => log_event!(
                self,
                event.name = "workx.tool_decision",
    ...
    pub fn sandbox_outcome(
        ...
        log_event!(
            self,
            event.name = "workx.sandbox_outcome",
            tool_name = %tool_name,
            call_id = %call_id,
            outcome = %outcome,
    ...
    /// Records a diagnostic representation and returns the original result. The temporary
    /// output string is truncated and released before the caller resumes async work.
    #[allow(clippy::too_many_arguments)]

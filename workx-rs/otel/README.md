...
`workx-otel` is the OpenTelemetry integration crate for Workx. It provides:

- Provider wiring for log/trace/metric exporters (`workx_otel::OtelProvider`
  and `workx_otel::provider`).
- Session-scoped business event emission via `workx_otel::SessionTelemetry`.
- Low-level metrics APIs via `workx_otel::metrics`.
- Trace-context helpers via `workx_otel::trace_context` and crate-root re-exports.
...
```rust
use workx_otel::config::OtelExporter;
use workx_otel::config::OtelHttpProtocol;
use workx_otel::config::OtelSettings;
...
let settings = OtelSettings {
    ...
    workx_home: std::path::PathBuf::from("/tmp"),
...
that member. If propagated trace context already has the named member, Workx
...
Workx-specific session events. Rich session/business events should go through
...
`workx-otel` also provides `OtelExporter::Statsig`, a shorthand for exporting OTLP/HTTP JSON metrics
to Statsig using Workx-internal defaults.

Statsig ingestion (OTLP/HTTP JSON) example:

```rust
use workx_otel::config::{OtelExporter, OtelHttpProtocol};
...
metrics.counter("workx.session_started", 1, &[("source", "tui")])?;
metrics.histogram("workx.request_latency", 83, &[("route", "chat")])?;
```
...
metrics.counter("workx.turns", 1, &[("model", "gpt-5.1")])?;

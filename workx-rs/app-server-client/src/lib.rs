...
//!
//! This crate wraps [`workx_app_server::in_process`] behind a single async API
...
//! [`InProcessClientHandle`](workx_app_server::in_process::InProcessClientHandle),
...
use tracing::warn;
pub use workx_app_server::app_server_control_socket_path;
pub use workx_app_server::in_process::DEFAULT_IN_PROCESS_CHANNEL_CAPACITY;
pub use workx_app_server::in_process::InProcessServerEvent;
use workx_app_server::in_process::InProcessStartArgs;
use workx_app_server::in_process::LogDbLayer;
pub use workx_app_server::in_process::StateDbHandle;
...
use workx_app_server_protocol::Result as JsonRpcResult;
...
use workx_config::NoopThreadConfigLoader;
use workx_core::config::Config;
pub use workx_core::otel_init::build_provider as build_otel_provider;
...
/// module exists so clients can remove a direct `workx-core` dependency
/// while legacy startup/config paths are migrated to RPCs.
pub mod legacy_core {
    pub mod config {
        pub use workx_core::config::*;

        pub mod edit {
            pub use workx_core::config::edit::*;
        }
    }
}
...
pub struct InProcessClientStartArgs {
    ...
    /// Whether auth loading should honor the `WORKX_API_KEY` environment variable.
    pub enable_workx_api_key_env: bool,
...
impl InProcessClientStartArgs {
    ...
    fn into_runtime_start_args(self) -> InProcessStartArgs {
        ...
        InProcessStartArgs {
            ...
            enable_workx_api_key_env: self.enable_workx_api_key_env,
            initialize,
...
/// - [`workx_app_server::in_process::InProcessClientHandle`], which speaks to
...
impl InProcessAppServerClient {
    ...
    pub async fn start(args: InProcessClientStartArgs) -> IoResult<Self> {
        ...
        let mut handle =
            workx_app_server::in_process::start(args.into_runtime_start_args()).await?;
...
impl AppServerClient {
    pub fn workx_home(&self, local_workx_home: &AbsolutePathBuf) -> Option<AppServerPath> {
        match self {
            Self::InProcess(_) => Some(AppServerPath::from_app_server(
                local_workx_home.display().to_string(),
            )),
            Self::Remote(client) => client.workx_home().map(AppServerPath::from_app_server),
        }
    }
...
mod tests {
    ...
    use workx_app_server_protocol::ServerNotification;
    use workx_app_server_protocol::SessionSource as ApiSessionSource;
    ...
    use workx_core::init_state_db;
    use workx_protocol::config_types::Personality;
    use workx_uds::UnixListener;
    use workx_utils_absolute_path::AbsolutePathBuf;
    ...
    async fn build_test_config_for_workx_home(workx_home: &Path) -> Config {
        ...
        {
            Ok(config) => config,
            Err(_) => Config::load_default_with_cli_overrides_for_workx_home(
    ...
    async fn start_test_client_with_capacity(
        ...
        let workx_home = TempDir::new().expect("temp dir");
        let config = Arc::new(build_test_config_for_workx_home(workx_home.path()).await);
        ...
        let client = InProcessAppServerClient::start(InProcessClientStartArgs {
            ...
            client_name: "workx-app-server-client-test".to_string(),
        ...
        write_websocket_message(
            ...
            JSONRPCMessage::Response(JSONRPCResponse {
                ...
                result: serde_json::json!({
                    "userAgent": "workx_cli_rs/9.8.7-test (Test OS; x86_64) rust",
                    "workxHome": "/server/.workx",
                }),
            }),
        )
        .await;
    ...
    fn command_execution_output_delta_notification(delta: &str) -> ServerNotification {
        ServerNotification::CommandExecutionOutputDelta(
            workx_app_server_protocol::CommandExecutionOutputDeltaNotification {
    ...
    fn agent_message_delta_notification(delta: &str) -> ServerNotification {
        ServerNotification::AgentMessageDelta(
            workx_app_server_protocol::AgentMessageDeltaNotification {
                thread_id: "thread".to_string(),
                turn_id: "turn".to_string(),
                item_id: "item".to_string(),
                delta: delta.to_string(),
    ...
    fn item_completed_notification(text: &str) -> ServerNotification {
        ServerNotification::ItemCompleted(workx_app_server_protocol::ItemCompletedNotification {
            ...
            turn_id: "turn".to_string(),
            completed_at_ms: 0,
            item: workx_app_server_protocol::ThreadItem::AgentMessage {
                id: "item".to_string(),
                text: text.to_string(),
                phase: None,
    ...
    fn turn_completed_notification() -> ServerNotification {
        ServerNotification::TurnCompleted(workx_app_server_protocol::TurnCompletedNotification {
            thread_id: "thread".to_string(),
            turn: workx_app_server_protocol::Turn {
                id: "turn".to_string(),
                items_view: workx_app_server_protocol::TurnItemsView::Full,
                items: Vec::new(),
                status: workx_app_server_protocol::TurnStatus::Completed,
                error: None,
                started_at: None,
                completed_at: Some(0),
                duration_ms: Some(1),
    ...
    async fn typed_request_reports_json_rpc_errors() {
        ...
        let err = client
            .request_typed::<ConfigRequirementsReadResponse>(ClientRequest::ThreadRead {
                ...
                params: workx_app_server_protocol::ThreadReadParams {
    ...
    async fn threads_started_via_app_server_are_visible_through_typed_requests() {
        ...
        let response: ThreadStartResponse = client
            ...
            .expect("thread/start should succeed");
        let read = client
            .request_typed::<workx_app_server_protocol::ThreadReadResponse>(
    ...
    async fn remote_typed_request_roundtrip_works() {
        ...
        assert_eq!(client.workx_home(), Some("/server/.workx"));
        let response: GetAccountResponse = client
            .request_typed(ClientRequest::GetAccount {
                request_id: RequestId::Integer(1),
                params: workx_app_server_protocol::GetAccountParams {
                    refresh_token: false,
                },
            })
            .await
    ...
    async fn remote_unix_socket_typed_request_roundtrip_works() {
        ...
        let socket_path = AbsolutePathBuf::from_absolute_path(socket_dir.path().join("workx.sock"))
            ...
            match event {
                ...
                AppServerEvent::ServerNotification(notification) => match notification.as_ref() {
                    ...
                    ServerNotification::ItemCompleted(notification)
                        if matches!(
                            &notification.item,
                            workx_app_server_protocol::ThreadItem::AgentMessage { text, .. }
                                if text == "hello"
                        ) =>
                    {
                    ...
                    ServerNotification::TurnCompleted(notification)
                        if notification.turn.status
                            == workx_app_server_protocol::TurnStatus::Completed =>
                    {

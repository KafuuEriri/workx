...
impl RemoteAppServerClient {
    ...
    pub fn workx_home(&self) -> Option<&str> {
        self.workx_home.as_deref()
    }
        ...
        S: AsyncRead + AsyncWrite + Unpin + Send + 'static,
    {
        ...
        let (pending_events, server_version, workx_home) = initialize_remote_connection(
            &mut stream,
            &endpoint,
            initialize_params,
            INITIALIZE_TIMEOUT,
    ...
    S: AsyncRead + AsyncWrite + Unpin,
{
                    ...
                    match message {
                        JSONRPCMessage::Response(response) if response.id == initialize_request_id => {
                            ...
                            workx_home = response
                                ...
                                .get("workxHome")
                                .and_then(serde_json::Value::as_str)
                                .filter(|workx_home| !workx_home.is_empty())
                                .map(str::to_string);
                            break Ok(());
                        }
    ...
    Ok((pending_events, server_version, workx_home))
}

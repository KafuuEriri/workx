...
use crate::landlock::WORKX_LINUX_SANDBOX_ARG0;
...
pub struct SandboxTransformRequest<'a> {
    ...
    pub workx_linux_sandbox_exe: Option<&'a Path>,
...
pub struct SandboxDirectSpawnTransformRequest<'a> {
    ...
    pub windows_sandbox_proxy_settings_mode: workx_windows_sandbox::WindowsSandboxProxySettingsMode,
}
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::MissingLinuxSandboxExecutable => {
                write!(f, "missing workx-linux-sandbox executable path")
            }
...
impl SandboxManager {
    ...
    pub fn transform(
        ...
        let (argv, arg0_override, pending_sandboxed_request) = match sandbox {
            ...
            SandboxType::LinuxSeccomp => {
                let pending = pending_sandboxed_request?;
                let exe = workx_linux_sandbox_exe
    ...
    pub fn transform_for_direct_spawn(
        ...
        #[cfg(target_os = "windows")]
        {
            let workx_home = workx_utils_home_dir::find_workx_home()
                .map_err(|err| SandboxTransformError::WindowsSandboxPreparation(err.to_string()))?;
            self.transform_for_direct_spawn_with_workx_home(request, workx_home.as_path())
        }
    ...
    #[cfg(target_os = "windows")]
    fn transform_for_direct_spawn_with_workx_home(
        &self,
        request: SandboxDirectSpawnTransformRequest<'_>,
        workx_home: &Path,
    ) -> Result<SandboxExecRequest, SandboxTransformError> {
...
fn wrap_windows_sandbox_exec_request_for_direct_spawn(
    ...
    workx_home: &Path,
    proxy_settings_mode: workx_windows_sandbox::WindowsSandboxProxySettingsMode,
    ...
    let helper = workx_windows_sandbox::resolve_exe_for_launch(source.as_path(), workx_home);
    *program = helper.to_string_lossy().into_owned();
    ...
    let mut wrapper_args =
        workx_windows_sandbox::create_windows_sandbox_command_args_for_permission_profile(
            inner_command,
            &native_cwd,
            workspace_roots,
            &request.env,
...
fn linux_sandbox_arg0_override(exe: &Path) -> String {
    if exe.file_name().and_then(|name| name.to_str()) == Some(WORKX_LINUX_SANDBOX_ARG0) {
        os_string_to_command_component(exe.as_os_str().to_owned())
    } else {
        WORKX_LINUX_SANDBOX_ARG0.to_string()
    }
}

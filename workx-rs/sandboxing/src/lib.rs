...
pub use workx_windows_sandbox::WindowsSandboxProxySettingsMode;

use workx_protocol::error::WorkxErr;

#[cfg(not(target_os = "linux"))]
pub fn system_bwrap_warning(
    _permission_profile: &workx_protocol::models::PermissionProfile,
) -> Option<String> {
    None
}

impl From<SandboxTransformError> for WorkxErr {
    fn from(err: SandboxTransformError) -> Self {
        match err {
            ...
            SandboxTransformError::MissingLinuxSandboxExecutable => {
                WorkxErr::LandlockSandboxExecutableNotProvided
            }
            SandboxTransformError::EnvironmentNetworkProxy(message) => {
                WorkxErr::UnsupportedOperation(message)
            }
            #[cfg(target_os = "macos")]
            ...
            SandboxTransformError::Wsl1UnsupportedForBubblewrap => {
                WorkxErr::UnsupportedOperation(crate::bwrap::WSL1_BWRAP_WARNING.to_string())
            }
            #[cfg(not(target_os = "macos"))]
            SandboxTransformError::SeatbeltUnavailable => WorkxErr::UnsupportedOperation(

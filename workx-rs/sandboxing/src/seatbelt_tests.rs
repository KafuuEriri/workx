...
use workx_network_proxy::ManagedNetworkSandboxContext;
...
use workx_network_proxy::build_config_state;
...
use workx_protocol::permissions::PROTECTED_METADATA_PATH_NAMES;
...
fn process_platform_defaults_allow_scratch_without_granting_it_to_filesystem_helpers() {
    let workspace = tempfile::Builder::new()
        .prefix("workx-seatbelt-approved-project-")
    ...
    for scratch_root in ["/private/tmp", "/private/var/tmp"] {
        let scratch = tempfile::Builder::new()
            .prefix("workx-seatbelt-process-scratch-")
...
fn explicit_unreadable_paths_are_excluded_from_full_disk_read_and_write_access() {
    let unreadable = absolute_path("/tmp/workx-unreadable");
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
    ...
    assert_eq!(
        ...
        vec![
            ...
            "-DWRITABLE_ROOT_0_EXCLUDED_0=/.workx".to_string(),
...
fn explicit_unreadable_paths_are_excluded_from_readable_roots() {
    let root = absolute_path("/tmp/workx-readable");
    let unreadable = absolute_path("/tmp/workx-readable/private");
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
        FileSystemSandboxEntry {
            path: root.into(),
...
fn preferences_access_requires_unrestricted_reads() {
    ...
    denied_path.entries.push(FileSystemSandboxEntry::new(
        absolute_path("/tmp/workx-private").into(),
        FileSystemAccessMode::Deny,
    ));
...
fn restricted_reads_cannot_read_preferences_outside_allowed_roots() {
    ...
    let marker = "workx-preferences-read-canary";
...
fn create_seatbelt_args_allowlists_explicit_unix_socket_paths_without_proxy() {
    ...
    let extra_allow_unix_sockets = vec![absolute_path("/tmp/workx-browser-use")];
    let args = create_seatbelt_command_args(CreateSeatbeltCommandArgsParams {
        command: vec!["/usr/bin/true".to_string()],
        file_system_sandbox_policy: &file_system_policy,
        network_sandbox_policy: NetworkSandboxPolicy::Restricted,
    ...
    let expected_socket_root = normalize_path_for_sandbox(Path::new("/tmp/workx-browser-use"))
...
async fn create_seatbelt_args_merges_proxy_and_explicit_unix_socket_paths() -> anyhow::Result<()> {
    ...
    let network_socket = "/tmp/workx-proxy-use";
    let explicit_socket = "/tmp/workx-browser-use";
...
#[test]
fn create_seatbelt_args_with_read_only_git_and_workx_subpaths() {
    ...
    // write to .workx/config.toml in the vulnerable root.
    ...
    assert!(
        ...
        "expected explicit writable root .git/.workx carveouts in policy:\n{policy_text}",
    );
    assert!(
    ...
    let expected_definitions = [
        ...
        format!(
            ...
            dot_workx_canonical.to_string_lossy()
        ),
        format!(
            "-DWRITABLE_ROOT_2={}",
    ...
    // Verify that .workx/config.toml cannot be modified under the generated
    // Seatbelt policy.
    let config_toml = dot_workx_canonical.join("config.toml");
    let output = Command::new(MACOS_PATH_TO_SEATBELT_EXECUTABLE)
        .args(&args)
        .current_dir(&cwd)
        .output()
    ...
    // Verify that writing a file to the folder containing .git and .workx is allowed.
...
fn seatbelt_protects_resolved_target_of_symlinked_metadata_directory() {
    ...
    let dot_workx = writable_root.join(".workx");
    ...
    symlink(&actual_config, &dot_workx).expect("create .workx symlink");
    let policy = restricted_write_policy(&[writable_root.as_path()]);
    let args = create_seatbelt_command_args(CreateSeatbeltCommandArgsParams {
        command: vec![
            "/bin/sh".to_string(),
    ...
    assert!(
        !output.status.success(),
        "resolved .workx target should remain read-only"
    );
    assert_eq!(
...
#[test]
fn create_seatbelt_args_block_first_time_dot_workx_creation_with_metadata_name_regex() {
    let tmp = TempDir::new().expect("tempdir");
    ...
    let dot_workx = repo_root.join(".workx");
    let config_toml = dot_workx.join("config.toml");
    let policy = SandboxPolicy::WorkspaceWrite {
    ...
    let shell_command: Vec<String> = [
        ...
        "bash",
        dot_workx.to_string_lossy().as_ref(),
...
fn create_seatbelt_args_for_cwd_as_git_repo() {
    ...
    let expected_dot_workx = format!(
        "-DWRITABLE_ROOT_0_EXCLUDED_1={}",
        dot_workx_canonical.to_string_lossy()
    );
    assert!(
        args.contains(&expected_dot_workx),
        "missing {expected_dot_workx}: {args:#?}"
    );
...
struct PopulatedTmp {
    ...
    /// ran `git commit` themselves, or modified .workx/config.toml to
...
fn populate_tmpdir(tmp: &Path) -> PopulatedTmp {
    ...
    fs::create_dir_all(vulnerable_root.join(".workx")).expect("create .workx");
    fs::write(
        vulnerable_root.join(".workx").join("config.toml"),
        "sandbox_mode = \"read-only\"\n",
    )
    .expect("write .workx/config.toml");
    ...
    let dot_workx_canonical = vulnerable_root_canonical.join(".workx");

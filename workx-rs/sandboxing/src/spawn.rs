...
pub async fn spawn_process(request: SpawnRequest<'_>) -> Result<SpawnedProcess> {
    ...
    if request.sandbox == SandboxType::WindowsRestrictedToken {
        #[cfg(target_os = "windows")]
        {
            ...
            let workx_home = workx_utils_home_dir::find_workx_home()
                .context("windows sandbox: failed to resolve workx_home")?;
            let empty_paths = &[];
            let overrides = windows.filesystem_overrides;

            return workx_windows_sandbox::spawn_windows_sandbox_session_for_level(
                workx_windows_sandbox::WindowsSandboxSessionRequest {
                    permission_profile: windows.permission_profile,
                    workspace_roots: windows.workspace_roots,
                    workx_home: workx_home.as_path(),
    ...
    let spawned = if tty {
        workx_utils_pty::pty::spawn_process(
            program,
            args,
            request.cwd,
            request.env,
        ...
        workx_utils_pty::pipe::spawn_process(
            program,
            args,
            request.cwd,
            request.env,
        ...
        workx_utils_pty::pipe::spawn_process_no_stdin(
            program,
            args,
            request.cwd,
            request.env,

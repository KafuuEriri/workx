...
Set a stable path to the skill CLI (default `WORKX_HOME` is `~/.workx`):

```
export WORKX_HOME="${WORKX_HOME:-$HOME/.workx}"
export IMAGE_GEN="$WORKX_HOME/skills/.system/imagegen/scripts/image_gen.py"
```
...
- Network/sandbox notes for fallback CLI mode: `references/workx-network.md`

...
const CACHE_FILE_NAME = "workx-manual.md";
const OUTLINE_FILE_NAME = "workx-manual.outline.md";
const HASH_HEADER = "x-content-sha256";
const USER_AGENT = "workx-openai-docs";
...
const tempFilePath = (cacheDir, suffix) =>
  path.join(
    cacheDir,
    `.fetch-workx-manual-${process.pid}-${Date.now()}-${Math.random()
      .toString(16)
      .slice(2)}${suffix}`
  );
...
const outlineMarkdown = (outline) => `# Workx Manual Outline\n\n${outline.text}\n`;
...
const main = async () => {
  ...
  const { outlineText, status } = await fetchWorkxManual(args);
...
export { DEFAULT_MANUAL_URL, fetchWorkxManual };

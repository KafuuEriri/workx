#!/usr/bin/env python3
"""Install a skill from a GitHub repo path into $WORKX_HOME/skills."""
...
def _workx_home() -> str:
    return os.environ.get("WORKX_HOME", os.path.expanduser("~/.workx"))


def _tmp_root() -> str:
    base = os.path.join(tempfile.gettempdir(), "workx")
    os.makedirs(base, exist_ok=True)
    return base


def _request(url: str) -> bytes:
    return github_request(url, "workx-skill-install")
...
def _default_dest() -> str:
    return os.path.join(_workx_home(), "skills")

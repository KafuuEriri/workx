...
- the user wants Workx to see the updated plugin without manually editing marketplace files
...
2. Update the plugin manifest to a single Workx cachebuster suffix:
...
outside Workx depends on a specific token:
...
```bash
workx plugin add <plugin-name>@<marketplace-name-from-marketplace-json>
```

The default personal marketplace is discovered implicitly from
`~/.agents/plugins/marketplace.json`. You do not need `workx plugin marketplace add` for that
path, and `workx plugin marketplace list` is not the right check for whether that default
marketplace exists.
...
```bash
workx plugin add <plugin-name>@<local-marketplace>
```

6. Prompt the user to use a new thread to try the updated plugin, so that Workx picks up new skills
   and tools.
...
```text
<base-version>+workx.<cachebuster>
```

Examples:

- `0.1.0` → `0.1.0+workx.local-20260519-184516`
- `0.1.0+workx.old-token` → `0.1.0+workx.local-20260519-184516`
- `1.2.3-beta.1+workx.prev` → `1.2.3-beta.1+workx.local-20260519-184516`
- `dev-build+other-tag` → `dev-build+workx.local-20260519-184516`

Replace the existing Workx cachebuster instead of appending another one. Do not keep incrementing
...
- Read the personal marketplace name with
  ...
  `workx plugin add <plugin-name>@<marketplace-name>`.
...
- Do not tell the user to run `workx plugin marketplace add` for the default personal-marketplace
  flow. That marketplace is discovered implicitly by Workx.
...
- Use `workx plugin list` when the plugin lives in a different configured marketplace and you need
...
- If a non-default local marketplace has not been configured yet, install it with
  `workx plugin marketplace add <path-to-marketplace-root>` before telling the user to run
  `workx plugin add <plugin-name>@<marketplace-name>`.

...
Run with:
    python3 -B -m unittest discover -s workx-rs/skills/tests -p 'test_*.py'
"""
...
class PluginCreatorSecurityTests(unittest.TestCase):
    ...
    def write_plugin(self, name: object) -> Path:
        manifest_path = self.plugin_root / ".workx-plugin" / "plugin.json"
    ...
    def test_cachebuster_accepts_dotted_plugin_names(self) -> None:
        ...
        self.assertEqual(manifest["version"], "1.0.0+workx.safe-token")

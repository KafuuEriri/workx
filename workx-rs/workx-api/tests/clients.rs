...
use workx_api::ResponsesClient;
use workx_api::ResponsesEndpoint;
use workx_api::ResponsesOptions;
use workx_client::HttpTransport;
use workx_client::Request;
use workx_client::RequestBody;
use workx_client::Response;
use workx_client::StreamResponse;
use workx_client::TransportError;
...
struct FlakyTransportState {
    attempts: i64,
    requests: Vec<(RequestBody, HeaderMap, workx_client::RequestCompression)>,
}
...
impl FlakyTransport {
    ...
    fn requests(&self) -> Vec<(RequestBody, HeaderMap, workx_client::RequestCompression)> {
        self.state
            .lock()
            .expect("flaky transport state mutex should not be poisoned")
            .requests
...
async fn streaming_client_retries_on_transport_error() -> Result<()> {
    ...
    assert_eq!(requests[0].2, workx_client::RequestCompression::None);
    Ok(())
}

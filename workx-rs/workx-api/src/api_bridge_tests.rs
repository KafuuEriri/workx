...
fn map_api_error_maps_server_overloaded() {
    ...
    assert!(matches!(err.details(), WorkxErrorDetails::ServerOverloaded));
}
...
fn map_api_error_preserves_retry_delay() {
    ...
    for (error, expected_code, expected_message) in [
        ...
        (
            ...
            WorkxErrorInfo::RateLimitExceeded,
...
fn map_api_error_maps_cloudflare_blocked_response_to_user_message() {
    ...
    let WorkxErrorDetails::UnexpectedStatus(err) = err.details() else {
        panic!("expected WorkxErrorDetails::UnexpectedStatus, got {err:?}");
    };
    assert_eq!(
...
fn map_api_error_maps_cyber_policy_from_400_body() {
    ...
    let WorkxErrorDetails::CyberPolicy { message } = err.details() else {
        panic!("expected WorkxErrorDetails::CyberPolicy, got {err:?}");
    };
    assert_eq!(
        message,
...
fn assert_misalignment_policy_violation_from_http_body(status: http::StatusCode) {
    ...
    let WorkxErrorDetails::MisalignmentPolicyViolation {
        message,
        misalignment,
    } = err.details()
    else {
        panic!("expected WorkxErrorDetails::MisalignmentPolicyViolation, got {err:?}");
    };
...
fn map_api_error_preserves_misalignment_details_from_403_body() {
    ...
    assert_eq!(
        ...
        &Some(MisalignmentErrorDetails {
            ...
            steer: Some(workx_protocol::protocol::MisalignmentSteer {
...
fn map_api_error_keeps_unknown_400_errors_generic() {
    ...
    let WorkxErrorDetails::InvalidRequest(message) = err.details() else {
        panic!("expected WorkxErrorDetails::InvalidRequest, got {err:?}");
    };
...
fn map_api_error_maps_usage_limit_limit_name_header() {
    ...
    headers.insert(
        ACTIVE_LIMIT_HEADER,
        http::HeaderValue::from_static("workx_other"),
    );
    headers.insert(
    ...
    let WorkxErrorDetails::UsageLimitReached(usage_limit) = err.details() else {
        panic!("expected WorkxErrorDetails::UsageLimitReached, got {err:?}");
    };
    assert_eq!(
        usage_limit
            .rate_limits
...
fn map_api_error_copies_rate_limit_reached_type_to_usage_limit_snapshot() {
    for (active_limit, expected_limit_id) in [(None, "workx"), (Some("workx_other"), "workx_other")]
    {
        let mut headers = HeaderMap::new();

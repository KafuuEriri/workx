...
use workx_protocol::auth::PlanType;
use workx_protocol::error::ConnectionFailedError;
use workx_protocol::error::RetryLimitReachedError;
use workx_protocol::error::UnexpectedResponseError;
use workx_protocol::error::UsageLimitReachedError;
use workx_protocol::error::WorkxErr;
use workx_protocol::error::WorkxErrorDetails;
use workx_protocol::protocol::MisalignmentErrorDetails;

pub fn map_api_error(err: ApiError) -> WorkxErr {
    match err {
        ApiError::ContextWindowExceeded => WorkxErr::ContextWindowExceeded,
        ApiError::QuotaExceeded => WorkxErr::QuotaExceeded,
        ApiError::UsageNotIncluded => WorkxErr::UsageNotIncluded,
        ApiError::Retryable { message, delay } => {
            let error = WorkxErr::Stream(message);
            match delay {
        ...
        ApiError::RateLimitExceeded { message, delay } => {
            let error = WorkxErr::new(WorkxErrorDetails::RateLimitExceeded(message));
            match delay {
                Some(delay) => error.with_retry_delay(delay),
                None => error,
            }
        }
        ApiError::Stream(msg) => WorkxErr::Stream(msg),
        ApiError::ServerOverloaded => WorkxErr::ServerOverloaded,
        ApiError::Api { status, message } => {
            let user_message = api_error_user_message(status, &message);
            WorkxErr::UnexpectedStatus(UnexpectedResponseError {
                status,
                body: message,
                user_message,
                url: None,
        ...
        ApiError::InvalidRequest { message } => WorkxErr::InvalidRequest(message),
        ApiError::CyberPolicy { message } => {
            WorkxErr::new(WorkxErrorDetails::CyberPolicy { message })
        }
        ApiError::MisalignmentPolicyViolation {
            message,
            misalignment,
        } => WorkxErr::new(WorkxErrorDetails::MisalignmentPolicyViolation {
            message,
            misalignment,
        }),
        ApiError::Transport(transport) => match transport {
            TransportError::Http {
                ...
                {
                    return WorkxErr::ServerOverloaded;
                }
                        ...
                        == Some(MISALIGNMENT_POLICY_VIOLATION_ERROR_CODE)
                {
                    ...
                    return WorkxErr::new(WorkxErrorDetails::MisalignmentPolicyViolation {
                        message,
                ...
                if status == http::StatusCode::BAD_REQUEST {
                    ...
                    {
                        WorkxErr::InvalidImageRequest()
                    } else {
                        WorkxErr::InvalidRequest(body_text)
                    }
                } else if status == http::StatusCode::INTERNAL_SERVER_ERROR {
                    WorkxErr::InternalServerError
                    ...
                    if let Ok(err) = serde_json::from_str::<UsageErrorResponse>(&body_text) {
                        if err.error.error_type.as_deref() == Some("usage_limit_reached") {
                            ...
                            return WorkxErr::UsageLimitReached(UsageLimitReachedError {
                            ...
                            return WorkxErr::UsageNotIncluded;
                        }
                    }

                    WorkxErr::RetryLimit(RetryLimitReachedError {
                        status,
            ...
            TransportError::RetryLimit => WorkxErr::RetryLimit(RetryLimitReachedError {
                status: http::StatusCode::INTERNAL_SERVER_ERROR,
                request_id: None,
            }),
            TransportError::Timeout => WorkxErr::RequestTimeout,
            TransportError::Connection(source) => {
                WorkxErr::ConnectionFailed(ConnectionFailedError { source })
            }
            TransportError::Network(msg) | TransportError::Build(msg) => WorkxErr::Stream(msg),
        },
        ApiError::RateLimit(msg) => WorkxErr::Stream(msg),
    }
}

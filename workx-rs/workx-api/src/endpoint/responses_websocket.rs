...
const X_WORKX_TURN_STATE_HEADER: &str = "x-codex-turn-state";
...
const RESPONSES_WEBSOCKET_TIMING_EVENT_TARGET: &str = "workx_api::responses_websocket_timing";
    ...
    loop {
        ...
        match message {
            Message::Text(text) => {
                ...
                if event.kind() == "workx.response.metadata"
                    && let Some(etag) =
                        event
                            .headers
                            .as_ref()
...
fn emit_responses_websocket_timing_event(
    ...
    // `RUST_LOG='workx_api::responses_websocket_timing=trace'`.
    tracing::event!(
    ...
    fn direct_serialization_preserves_websocket_request_payload() {
        let api_request = ResponsesApiRequest {
            ...
            access_programs: Some(
                workx_protocol::turn_input::CyberAccessProgram::DaybreakBlue.into(),
            ),

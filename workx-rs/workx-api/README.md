...
Typed clients for Workx/OpenAI APIs built on top of the generic transport in `workx-client`.
...
- Serves as the wire-level layer consumed by `workx-core`; higher layers handle auth refresh and business logic.
...
- **Compaction endpoint**
  - Input: `CompactionInput<'a>` (re-exported as `workx_api::CompactionInput`):
    - `model: &str`.
...
- **Memory summarize endpoint**
  - Input: `MemorySummarizeInput` (re-exported as `workx_api::MemorySummarizeInput`):
    - `model: String`.
...
All HTTP details (URLs, headers, retry/backoff policies, SSE framing) are encapsulated in `workx-api` and `workx-client`. Callers construct prompts/inputs using protocol types and work with typed streams of `ResponseEvent` or compacted `ResponseItem` values.

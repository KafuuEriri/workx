...
`workx-network-proxy` is Workx's local network policy enforcement proxy. It runs:
...
`workx-network-proxy` reads from Workx's merged `config.toml` (via `workx-core` config loading).
...
# bundle env vars pointing at immutable public files under $WORKX_HOME/proxy/ so common HTTPS
...
```bash
cargo run -p workx-network-proxy --
```
...
`workx-network-proxy` can be embedded as a library with a thin API:

```rust
use workx_network_proxy::{NetworkProxy, NetworkDecision, NetworkPolicyRequest};
...
When `workx-network-proxy` is embedded in managed Workx runtime, policy decisions emit structured
OTEL-compatible events with `target=workx_otel.network_proxy`.

Event name:

- `workx.network_proxy.policy_decision`
...
This section documents the protections implemented by `workx-network-proxy`, and the boundaries of

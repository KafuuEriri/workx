...
fn managed_ca_dir() -> Result<PathBuf> {
    let workx_home =
        find_workx_home().context("failed to resolve WORKX_HOME for managed MITM CA")?;
    Ok(workx_home.join(MANAGED_MITM_CA_DIR).to_path_buf())
}
...
/// Returns whether `path` points at a current Workx-generated MITM CA bundle.

...
const AUDIT_TARGET: &str = "workx_otel.network_proxy";
const POLICY_DECISION_EVENT_NAME: &str = "workx.network_proxy.policy_decision";
...
mod tests {
    ...
    const LEGACY_DOMAIN_POLICY_DECISION_EVENT_NAME: &str =
        "workx.network_proxy.domain_policy_decision";
    const LEGACY_BLOCK_DECISION_EVENT_NAME: &str = "workx.network_proxy.block_decision";
    ...
    async fn evaluate_host_policy_emits_domain_event_for_decider_allow_override() {
        ...
        assert!(event.target.starts_with("workx_otel."));
        assert_eq!(
    ...
    async fn evaluate_host_policy_emits_metadata_fields() {
        let metadata = NetworkProxyAuditMetadata {
            ...
            originator: Some("workx_cli_rs".to_string()),
        ...
        assert_eq!(event.field("originator"), Some("workx_cli_rs"));

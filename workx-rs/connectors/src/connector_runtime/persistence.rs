...
const MCP_TOOLS_CACHE_WRITE_DURATION_METRIC: &str = "workx.mcp.tools.cache_write.duration_ms";
const WORKX_APPS_TOOLS_CACHE_DIR: &str = "cache/workx_apps_tools";
pub(crate) const WORKX_APPS_TOOLS_CACHE_SCHEMA_VERSION: u8 = 4;
const WORKX_APPS_SERVER_INFO_CACHE_DIR: &str = "cache/workx_apps_server_info";
const WORKX_APPS_SERVER_INFO_CACHE_SCHEMA_VERSION: u8 = 1;
pub(crate) const WORKX_APPS_TOOLS_CACHE_MAX_BYTES: u64 = 32 * 1024 * 1024;

pub(crate) fn tools_cache_path(identity: &ConnectorRuntimeIdentity) -> PathBuf {
    cache_path_in(identity, WORKX_APPS_TOOLS_CACHE_DIR)
}

pub(crate) fn server_info_cache_path(identity: &ConnectorRuntimeIdentity) -> PathBuf {
    cache_path_in(identity, WORKX_APPS_SERVER_INFO_CACHE_DIR)
}

fn cache_path_in(identity: &ConnectorRuntimeIdentity, cache_dir: &str) -> PathBuf {
    // `workx_home` is already the parent directory. Keep it out of the
...
pub(crate) fn load_cached_connector_runtime_for_identity<T: ConnectorRuntimePayload>(
    ...
    let cache: WorkxAppsToolsDiskCache<T> = serde_json::from_slice(&bytes).ok()?;
    (cache.schema_version == WORKX_APPS_TOOLS_CACHE_SCHEMA_VERSION).then_some(
    ...
    T: ConnectorRuntimePayload,
{
    ...
    let bytes = serde_json::to_vec_pretty(&WorkxAppsToolsDiskCache {
    ...
    write_workx_apps_cache_file(&cache_path, "runtime", bytes)
}

#[instrument(level = "trace", skip_all)]
pub(crate) fn load_cached_workx_apps_server_info<T: ConnectorRuntimePayload>(
    cache_context: &ConnectorRuntimeContext<T>,
) -> Option<McpServerInfo> {
    let (bytes, _) = read_bounded_cache_file(&cache_context.server_info_cache_path()).ok()?;
    let cache: WorkxAppsServerInfoDiskCache = serde_json::from_slice(&bytes).ok()?;
    (cache.schema_version == WORKX_APPS_SERVER_INFO_CACHE_SCHEMA_VERSION)
        .then_some(cache.server_info)
}

fn write_cached_workx_apps_server_info<T: ConnectorRuntimePayload>(
    cache_context: &ConnectorRuntimeContext<T>,
    server_info: &McpServerInfo,
) -> anyhow::Result<()> {
    let cache_path = cache_context.server_info_cache_path();
    let bytes = serde_json::to_vec_pretty(&WorkxAppsServerInfoDiskCache {
        schema_version: WORKX_APPS_SERVER_INFO_CACHE_SCHEMA_VERSION,
        server_info: server_info.clone(),
    })
    .context("failed to serialize Workx Apps server info cache")?;
    write_workx_apps_cache_file(&cache_path, "server info", bytes)
}

pub(crate) fn persist_workx_apps_cache<T>(
    cache_context: &ConnectorRuntimeContext<T>,
    server_info: &McpServerInfo,
    snapshot: &ConnectorRuntimeSnapshot<T>,
) where
    T: ConnectorRuntimePayload,
{
    ...
    let server_info_result = write_cached_workx_apps_server_info(cache_context, server_info);
    if let Err(err) = &server_info_result {
        tracing::warn!("failed to write Workx Apps server info cache: {err:#}");
    }
...
fn read_bounded_cache_file(cache_path: &Path) -> anyhow::Result<(Vec<u8>, SystemTime)> {
    ...
    if metadata.len() > WORKX_APPS_TOOLS_CACHE_MAX_BYTES {
        return Err(anyhow!(
            "cache `{}` is {} bytes, exceeding the {} byte limit",
            cache_path.display(),
            metadata.len(),
            WORKX_APPS_TOOLS_CACHE_MAX_BYTES
        ));
    }
    let mut bytes = Vec::with_capacity(metadata.len() as usize);
    std::io::Read::by_ref(&mut file)
        .take(WORKX_APPS_TOOLS_CACHE_MAX_BYTES + 1)
        .read_to_end(&mut bytes)
        .with_context(|| format!("failed to read cache `{}`", cache_path.display()))?;
    if bytes.len() as u64 > WORKX_APPS_TOOLS_CACHE_MAX_BYTES {
        return Err(anyhow!(
...
fn write_workx_apps_cache_file(
    cache_path: &Path,
    cache_name: &str,
    bytes: Vec<u8>,
) -> anyhow::Result<()> {
    let parent = cache_path.parent().ok_or_else(|| {
        anyhow!(
            "Workx Apps {cache_name} cache path `{}` has no parent",
            cache_path.display()
        )
    })?;
    std::fs::create_dir_all(parent).with_context(|| {
        format!(
            "failed to create Workx Apps {cache_name} cache directory `{}`",
            parent.display()
        )
    })?;
    let mut temporary = NamedTempFile::new_in(parent).with_context(|| {
        format!(
            "failed to create temporary Workx Apps {cache_name} cache in `{}`",
            parent.display()
        )
    })?;
    temporary.write_all(&bytes).with_context(|| {
        format!(
            "failed to write temporary Workx Apps {cache_name} cache for `{}`",
            cache_path.display()
        )
    })?;
    temporary.persist(cache_path).map_err(|error| {
        anyhow!(
            "failed to atomically replace Workx Apps {cache_name} cache `{}`: {}",
            cache_path.display(),
            error.error
        )
    })?;
...
struct WorkxAppsToolsDiskCache<T> {
    schema_version: u8,
    tools: Vec<T>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
struct WorkxAppsServerInfoDiskCache {
    schema_version: u8,
    server_info: McpServerInfo,
}
...
#[cfg(test)]
pub(crate) fn write_cached_workx_apps_tools_for_test<T>(
    cache_context: &ConnectorRuntimeContext<T>,
    server_info: &McpServerInfo,
    tools: &[T],
) where
    T: ConnectorRuntimePayload,
{
    ...
    persist_workx_apps_cache(cache_context, server_info, &snapshot);
}

#[cfg(test)]
pub(crate) fn read_cached_workx_apps_tools<T>(
    cache_context: &ConnectorRuntimeContext<T>,
) -> Option<Vec<T>>
where
    T: ConnectorRuntimePayload,
...
#[cfg(test)]
pub(crate) fn write_cached_workx_apps_tools<T>(
    cache_context: &ConnectorRuntimeContext<T>,
    tools: &[T],
) -> anyhow::Result<()>
where

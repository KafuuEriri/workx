...
fn connector_install_url(name: &str, connector_id: &str) -> String {
    let chatgpt_base_url = std::env::var("WORKX_APP_SERVER_CHATGPT_BASE_URL")
...
mod tests {
    ...
    fn cache_context(workx_home: &TempDir, id: &str) -> ConnectorDirectoryCacheContext {
        ConnectorDirectoryCacheContext::new(workx_home.path().to_path_buf(), cache_key(id))
    }
    ...
    async fn list_all_connectors_uses_shared_directory_cache() -> anyhow::Result<()> {
        ...
        let cache_context = cache_context(&workx_home, "shared");
    ...
    async fn list_all_connectors_merges_and_normalizes_directory_apps() -> anyhow::Result<()> {
        ...
        let workx_home = TempDir::new()?;
        let cache_context = cache_context(&workx_home, "merged");
        let calls = Arc::new(AtomicUsize::new(0));
        let call_counter = Arc::clone(&calls);
    ...
    async fn list_all_connectors_overlaps_workspace_and_directory_requests() -> anyhow::Result<()> {
        ...
        let workx_home = TempDir::new()?;
        let cache_context = cache_context(&workx_home, "overlap");
    ...
    async fn cached_directory_connectors_reads_directory_disk_cache() -> anyhow::Result<()> {
        ...
        let workx_home = TempDir::new()?;
        let cache_context = cache_context(&workx_home, "disk");
        let calls = Arc::new(AtomicUsize::new(0));
        let call_counter = Arc::clone(&calls);
    ...
    -> anyhow::Result<()> {
        ...
        let workx_home = TempDir::new()?;
        let cache_context = cache_context(&workx_home, "disk-refresh");
        let calls = Arc::new(AtomicUsize::new(0));
        let call_counter = Arc::clone(&calls);
    ...
    async fn cached_directory_connectors_drops_stale_disk_schema() -> anyhow::Result<()> {
        ...
        clear_directory_memory_cache();
        let workx_home = TempDir::new()?;
        let cache_context = cache_context(&workx_home, "stale-schema");

...
use workx_state::BackfillStatus;
...
async fn backfill_sessions_resumes_from_watermark_and_marks_complete() {
    let dir = tempdir().expect("tempdir");
    let workx_home = dir.path().to_path_buf();
    ...
    let runtime = workx_state::StateRuntime::init(
        workx_state::SqliteConfig::new_for_testing(workx_home.as_path().abs()),
        "test-provider".to_string(),
    )
    .await
    .expect("initialize runtime");
    let first_watermark = backfill_watermark_for_path(workx_home.as_path(), first_path.as_path());
    ...
    backfill_sessions(runtime.as_ref(), workx_home.as_path(), "test-provider").await;
...
async fn backfill_sessions_normalizes_cwd_before_upsert() {
    ...
    let workx_home = dir.path().to_path_buf();
    let thread_uuid = Uuid::new_v4();
    let session_cwd = workx_home.join(".");
    let rollout_path = write_rollout_in_sessions_with_cwd(
        workx_home.as_path(),
        "2026-01-27T12-34-56",
        "2026-01-27T12:34:56Z",
...
fn write_rollout_in_sessions_with_cwd(
    ...
    let sessions_dir = workx_home.join("sessions");

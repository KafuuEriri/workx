//! Persist Workx session rollouts (.jsonl) so sessions can be replayed or inspected later.
...
use workx_protocol::protocol::GitInfo as ProtocolGitInfo;
use workx_protocol::protocol::HistoryPosition;
use workx_protocol::protocol::MultiAgentVersion;
use workx_protocol::protocol::SessionContextWindow;
...
///
/// ```ignore
/// $ jq -C . ~/.workx/sessions/rollout-2025-05-07T17-24-21-5973b6c0-94b8-487b-a530-2aeb6098ae0e.jsonl
/// $ fx ~/.workx/sessions/rollout-2025-05-07T17-24-21-5973b6c0-94b8-487b-a530-2aeb6098ae0e.jsonl
/// ```
#[derive(Clone)]
...
impl RolloutRecorder {
    /// List threads (rollout files) under the provided Workx home directory.
    #[allow(clippy::too_many_arguments)]
    ...
    async fn list_threads_with_db_fallback(
        ...
        let workx_home = config.workx_home();
        ...
        workx_state::record_fallback("list_threads", "db_error", /*telemetry_override*/ None);
...
async fn list_threads_from_files_desc(
    ...
    if let Some(search_term) = search_term {
        ...
        loop {
            ...
            filter_thread_items_by_search_term(workx_home, &mut page.items, Some(search_term))
                .await?;
...
async fn list_threads_from_files_desc_unfiltered(
    ...
    if archived {
        let root = workx_home.join(ARCHIVED_SESSIONS_SUBDIR);
        get_threads_in_root(
            root,
            page_size,
            cursor,
...
async fn list_threads_from_files_asc(
    ...
    filter_thread_items_by_search_term(workx_home, &mut all_items, search_term).await?;
...
async fn filter_thread_items_by_search_term(
    ...
    let thread_ids = items
        ...
        .filter_map(|item| item.thread_id)
        .collect::<HashSet<_>>();
    let thread_names = find_thread_names_by_ids(workx_home, &thread_ids).await?;
    items.retain(|item| {
        item.thread_id
...
fn precompute_new_rollout_path(
    ...
    // Resolve ~/.workx/sessions/YYYY/MM/DD path.
    let timestamp = OffsetDateTime::now_local()
        .map_err(|e| IoError::other(format!("failed to get local time: {e}")))?;
    let mut dir = config.workx_home().to_path_buf();
...
impl From<workx_state::ThreadsPage> for ThreadsPage {
    fn from(db_page: workx_state::ThreadsPage) -> Self {
...
fn thread_item_from_state_metadata(
    item: workx_state::ThreadMetadata,
    parent_thread_id: Option<ThreadId>,
) -> ThreadItem {
    ThreadItem {

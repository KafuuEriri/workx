...
//! existing rollout path. They must not do that at the same time for one Workx home, so they share
...
/// Try to exclude rollout compression and migration for one Workx home.
pub fn try_acquire_rollout_maintenance_lock(
    workx_home: &Path,
) -> io::Result<Option<RolloutMaintenanceGuard>> {
    let directory = workx_home.join(".tmp");

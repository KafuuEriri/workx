...
const ITEM_BYTES_METRIC: &str = "workx.rollout.persistence.item_bytes";
const APPEND_METRIC: &str = "workx.rollout.persistence.append";
const TURN_BYTES_METRIC: &str = "workx.rollout.persistence.turn_bytes";
const MEASUREMENT_ERROR_METRIC: &str = "workx.rollout.persistence.measurement_error";
...
fn rollout_item_type(item: &RolloutItem) -> String {
    match item {
        ...
        RolloutItem::RealtimeItem(item) => match &item.content {
            workx_protocol::realtime::RealtimeItemContent::RealtimeSessionStarted => {
                "realtime.session_started".to_string()
            }
            workx_protocol::realtime::RealtimeItemContent::TranscriptSegment { .. } => {
                "realtime.transcript_segment".to_string()
            }
            workx_protocol::realtime::RealtimeItemContent::BemItemPromoted { .. } => {
                "realtime.bem_item_promoted".to_string()
            }
            workx_protocol::realtime::RealtimeItemContent::RealtimeSessionClosed { .. } => {

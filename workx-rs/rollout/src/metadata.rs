...
use workx_state::BackfillState;
use workx_state::BackfillStats;
use workx_state::BackfillStatus;
use workx_state::DB_ERROR_METRIC;
use workx_state::DB_METRIC_BACKFILL;
use workx_state::DB_METRIC_BACKFILL_DURATION_MS;
use workx_state::ExtractionOutcome;
use workx_state::ThreadMetadataBuilder;
use workx_state::apply_rollout_item;
...
pub(crate) async fn backfill_sessions_with_lease(
    ...
    let metric_client = workx_otel::global();
    ...
    let sessions_root = workx_home.join(SESSIONS_SUBDIR);
    let archived_root = workx_home.join(ARCHIVED_SESSIONS_SUBDIR);
        ...
        match collect_rollout_paths(&root).await {
            Ok(paths) => {
                rollout_paths.extend(paths.into_iter().map(|path| BackfillRolloutPath {
                    watermark: backfill_watermark_for_path(workx_home, &path),
                    path,
                    archived,
                }));
            }
...
fn backfill_watermark_for_path(workx_home: &Path, path: &Path) -> String {
    path.strip_prefix(workx_home)
        .unwrap_or(path)
        .to_string_lossy()
        .replace('\\', "/")
}

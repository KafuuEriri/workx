...
use workx_file_search as file_search;
...
pub struct ThreadItem {
    ...
    pub section: Option<workx_state::ThreadSection>,
...
impl From<workx_state::Anchor> for Cursor {
    fn from(anchor: workx_state::Anchor) -> Self {
        let ts = anchor
            .ts
            .timestamp_nanos_opt()
...
pub async fn get_threads(
    ...
    let root = workx_home.join(SESSIONS_SUBDIR);
    get_threads_in_root(
        root,
        page_size,
        cursor,
...
///
/// Directory layout: `~/.workx/sessions/YYYY/MM/DD/rollout-YYYY-MM-DDThh-mm-ss-<uuid>.jsonl`
...
async fn find_thread_path_by_id_str_in_subdir(
    ...
    subdir: &str,
    id_str: &str,
    state_db_ctx: Option<&workx_state::StateRuntime>,
    ...
    let mut root = workx_home.to_path_buf();
    root.push(subdir);
    if !root.exists() {
...
pub async fn find_thread_path_by_id_str(
    ...
    find_thread_path_by_id_str_in_subdir(workx_home, SESSIONS_SUBDIR, id_str, state_db_ctx).await
}
...
pub async fn find_archived_thread_path_by_id_str(
    ...
    find_thread_path_by_id_str_in_subdir(workx_home, ARCHIVED_SESSIONS_SUBDIR, id_str, state_db_ctx)
        .await
}
...
pub async fn find_rollout_path_by_rollout_id(
    ...
    for subdir in [SESSIONS_SUBDIR, ARCHIVED_SESSIONS_SUBDIR] {
        let path = find_rollout_path_by_rollout_id_from_filenames(
            workx_home.join(subdir).as_path(),
            rollout_id,
        )
        .await?;
        if path.is_some() {

...
pub async fn append_thread_name(
    ...
    append_session_index_entry(workx_home, &entry).await
}
...
pub async fn append_session_index_entry(
    ...
    let path = session_index_path(workx_home);
...
fn session_index_path(workx_home: &Path) -> PathBuf {
    workx_home.join(SESSION_INDEX_FILE)
}

...
impl RolloutReferenceIndex {
    ...
    pub async fn scan(workx_home: &Path) -> io::Result<Self> {
        Self::scan_paths(vec![
    ...
    pub async fn scan_unarchived(workx_home: &Path) -> io::Result<Self> {
        Self::scan_paths(vec![workx_home.join(SESSIONS_SUBDIR)]).await
    }

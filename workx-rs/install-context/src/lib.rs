...
const CODE_MODE_HOST_EXECUTABLE_NAME: &str = if cfg!(windows) {
    "workx-code-mode-host.exe"
} else {
    "workx-code-mode-host"
};
const PACKAGE_METADATA_FILENAME: &str = "workx-package.json";
const PATH_DIRNAME: &str = "workx-path";
const RELEASES_DIRNAME: &str = "releases";
const RESOURCES_DIRNAME: &str = "workx-resources";
...
/// Version metadata recorded in a bundled Workx runtime package.
#[derive(Clone, Debug, Deserialize, Eq, PartialEq)]
pub struct WorkxPackageManifest {
    pub version: Version,
}
...
pub struct InstallContext {
    ...
    pub package_layout: Option<WorkxPackageLayout>,
}
...
pub enum InstallMethod {
    Standalone {
        ...
        /// such as
        /// `~/.workx/packages/standalone/releases/0.111.0-x86_64-unknown-linux-musl`.
        /// Package-layout installs use the package root that contains `bin/`,
        /// `workx-resources/`, and `workx-path/`.
    ...
    /// A Workx binary launched through the npm-managed `workx.js` shim.
    Npm,
    /// A Workx binary launched through the bun-managed `workx.js` shim.
    Bun,
    /// A Workx binary launched through the pnpm-managed `workx.js` shim.
    Pnpm,
    /// A Workx binary launched through the Vite+-managed `workx.js` shim.
    VitePlus,
    /// A Workx binary that appears to come from a Homebrew install prefix.
    Brew,
    /// Any other execution environment.
    ///
    /// This commonly covers `cargo run`, app-bundled Workx binaries, custom
    /// internal launchers, and tests that execute Workx from an arbitrary path.
    Other,
}
...
impl InstallContext {
    pub fn from_exe(
        ...
        let workx_home = workx_utils_home_dir::find_workx_home().ok();
        Self::from_exe_with_workx_home(
            is_macos,
            current_exe,
            method_override,
            workx_home.as_deref(),
    ...
    fn from_exe_with_workx_home(
        is_macos: bool,
        current_exe: Option<&Path>,
        method_override: Option<InstallMethod>,
        workx_home: Option<&Path>,
    ) -> Self {
        let package_layout = current_exe.and_then(WorkxPackageLayout::from_exe);
        let method = if let Some(method) = method_override {
            method
        } else if let Some(exe_path) = current_exe {
            install_method_from_exe(exe_path, workx_home, package_layout.as_ref(), is_macos)
        } else {
            InstallMethod::Other
        };
    ...
    pub fn current() -> &'static Self {
        INSTALL_CONTEXT.get_or_init(|| {
            ...
            let method_override = if std::env::var_os("WORKX_MANAGED_BY_VITE_PLUS").is_some() {
                Some(InstallMethod::VitePlus)
            } else if std::env::var_os("WORKX_MANAGED_BY_PNPM").is_some() {
                Some(InstallMethod::Pnpm)
            } else if std::env::var_os("WORKX_MANAGED_BY_NPM").is_some() {
                Some(InstallMethod::Npm)
            } else if std::env::var_os("WORKX_MANAGED_BY_BUN").is_some() {
    ...
    pub fn package_manifest(&self) -> Option<WorkxPackageManifest> {
    ...
    pub fn code_mode_host_program(&self) -> PathBuf {
        // prefer the one packed under workx-resources
    ...
    fn from_exe(exe_path: &Path) -> Option<Self> {
        ...
        match exe_dir.file_name() {
            ...
            Some(name) if name == OsStr::new("MacOS") => {
                ...
                // package. current_exe points inside the bundle, not at bin/workx.
                let contents = exe_dir.parent()?;
                let bundle = contents.parent()?;
                if contents.file_name()? != OsStr::new("Contents")
                    || bundle.file_name()? != OsStr::new("WorkxCLI.app")
                {
                    return None;
                }
...
fn install_method_from_exe(
    ...
    if let Some(standalone_method) = standalone_install_method(exe_path, workx_home, package_layout)
    {
...
fn standalone_install_method(
    ...
    let canonical_workx_home = canonical_absolute_path(workx_home?)?;
    ...
    let releases_root = canonical_workx_home
        .join("packages")
...
mod tests {
    ...
    const TEST_RESOURCE_NAME: &str = "workx-test-helper";
    ...
    fn code_mode_host_program_prefers_package_resource_over_legacy_binary() -> std::io::Result<()> {
        ...
        let exe_path = bin_dir.join(if cfg!(windows) { "workx.exe" } else { "workx" });
    ...
    fn detects_standalone_install_from_release_layout() -> std::io::Result<()> {
        ...
        let release_dir = workx_home
            .path()
            .join("packages/standalone/releases/1.2.3-x86_64-unknown-linux-musl");
        let resources_dir = release_dir.join(RESOURCES_DIRNAME);
        fs::create_dir_all(&resources_dir)?;
        let exe_path = release_dir.join(if cfg!(windows) { "workx.exe" } else { "workx" });
        fs::write(&exe_path, "")?;
        ...
        let context = InstallContext::from_exe_with_workx_home(
            /*is_macos*/ false,
            /*current_exe*/ Some(&exe_path),
            /*method_override*/ None,
            /*workx_home*/ Some(workx_home.path()),
        );
        assert_eq!(
            context,
            InstallContext {
...
"#,
        ...
        let package_layout = WorkxPackageLayout {
    ...
    fn code_mode_host_program_accepts_symlinks_to_files() -> std::io::Result<()> {
        ...
        fs::create_dir_all(&bin_dir)?;
        fs::write(package_dir.path().join(PACKAGE_METADATA_FILENAME), "{}")?;
        let exe_path = bin_dir.join("workx");
    ...
    #[test]
    fn standalone_package_rg_falls_back_when_workx_path_is_missing() -> std::io::Result<()> {
        let package_dir = tempfile::tempdir()?;
        let bin_dir = package_dir.path().join(BIN_DIRNAME);
        fs::create_dir_all(&bin_dir)?;
        fs::write(package_dir.path().join(PACKAGE_METADATA_FILENAME), "{}")?;
    ...
    fn bundled_file_lookups_ignore_directories() -> std::io::Result<()> {
        ...
        let exe_path = bin_dir.join(if cfg!(windows) { "workx.exe" } else { "workx" });
        fs::write(&exe_path, "")?;
        let fallback_exe_path = package_dir.path().join("fallback-workx");
    ...
    fn code_mode_host_program_is_next_to_the_executable_even_when_missing() {
        ...
        assert_eq!(
            context.code_mode_host_program_from_exe(Some(Path::new("/opt/workx/bin/workx"))),
            PathBuf::from("/opt/workx/bin").join(CODE_MODE_HOST_EXECUTABLE_NAME)
        );
    }
    ...
    fn package_manager_method_overrides_take_precedence() {
        let vite_plus_context = InstallContext::from_exe(
            /*is_macos*/ false,
            /*current_exe*/ Some(Path::new("/tmp/workx")),
    ...
    fn brew_is_detected_on_macos_prefixes() {
        let context = InstallContext::from_exe_with_workx_home(
            /*is_macos*/ true,
            /*current_exe*/ Some(Path::new("/opt/homebrew/bin/workx")),
            /*method_override*/ None,
            /*workx_home*/ None,
        );
        assert_eq!(

...
use workx_connectors::ConnectorMetadata;
use workx_connectors::ConnectorMetadataStore;
use workx_connectors::ConnectorToolSummary;
use workx_connectors::DirectoryListResponse;
use workx_connectors::merge::merge_connectors;
use workx_connectors::merge::merge_plugin_connectors;
use workx_core::config::Config;
pub use workx_core::connectors::list_accessible_connectors_from_mcp_tools;
pub use workx_core::connectors::list_accessible_connectors_from_mcp_tools_with_environment_manager;
pub use workx_core::connectors::list_accessible_connectors_from_mcp_tools_with_mcp_manager;
pub use workx_core::connectors::list_accessible_connectors_from_mcp_tools_with_options;
pub use workx_core::connectors::list_accessible_connectors_from_mcp_tools_with_options_and_status;
pub use workx_core::connectors::list_cached_accessible_connectors_from_mcp_tools;
...
const DEFAULT_APPS_PRODUCT_SKU: &str = "workx";
...
async fn connector_auth(config: &Config) -> anyhow::Result<WorkxAuth> {
    let auth_manager =
        AuthManager::shared_from_config(config, /*enable_workx_api_key_env*/ false).await?;
    ...
    anyhow::ensure!(
        ...
        "ChatGPT connectors require Workx backend auth"
    );
    Ok(auth)
}
...
pub async fn list_cached_all_connectors(
    ...
    let connectors = workx_connectors::cached_directory_connectors(&cache_context)?;
...
pub async fn list_all_connectors_with_options(
    ...
    let cache_context = connector_directory_cache_context(config, &auth);
    let connectors = workx_connectors::list_all_connectors_with_options(
        cache_context,
...
pub async fn read_connector_metadata(
    ...
    anyhow::ensure!(
        auth.uses_workx_backend(),
        "ChatGPT backend requests require Workx backend auth"
    );
    anyhow::ensure!(
        auth.get_account_id().is_some(),
        "ChatGPT account ID not available, please re-run workx login"
    );
...
mod tests {
    ...
    use serde_json::json;
    use workx_connectors::metadata::connector_install_url;
    use workx_plugin::AppConnectorId;

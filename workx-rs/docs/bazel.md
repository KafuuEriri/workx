...
This repository uses Bazel to build the Rust workspace under `workx-rs`.
...
- `rules_rs` imports third-party crates from `workx-rs/Cargo.toml` and
  `workx-rs/Cargo.lock` via `crate.from_cargo(...)` and exposes them under
  `@crates`.
- `../defs.bzl` provides `workx_rust_crate`, which wraps `rust_library`,
...
- Each crate in `workx-rs/*/BUILD.bazel` typically uses `workx_rust_crate` and
...
Workx uses BuildBuddy for a shared Bazel cache and remoted builds and tests. To use it
...
| --- | --- | --- |
| Push to `main` in `RonanXiao/workx` | Yes | Yes |
| `workflow_dispatch` in `RonanXiao/workx` | Yes | Yes |
| Same-repository pull request in `RonanXiao/workx` | Yes | Yes |
| Fork pull request into `RonanXiao/workx` | No | No; local |
...
```bash
BUILDBUDDY_API_KEY=... GITHUB_REPOSITORY=my-fork/workx \
  ./.github/scripts/run_bazel_with_buildbuddy.py \
  build --config=ci-linux //workx-rs/cli:workx
```

The wrapper selects the OpenAI host only inside GitHub Actions for a trusted
run in `RonanXiao/workx`. A missing or malformed pull request event
...
2. Create a `BUILD.bazel` that calls `workx_rust_crate` (see nearby crates for
   examples).
3. If a dependency needs special handling (compile/runtime data, additional binaries
   ...
   `workx_rust_crate` to configure it.

# Workx MCP Server Interface [experimental]

This document describes Workx's experimental MCP server interface: a JSON-RPC API that runs over the Model Context Protocol (MCP) transport to control a local Workx engine.

- Status: experimental and subject to change without notice
- Server binary: `workx mcp-server` (or `workx-mcp-server`)
- Transport: standard MCP over stdio (JSON-RPC 2.0, line-delimited)

## Overview

Workx exposes MCP-compatible methods to manage threads, turns, accounts, config, and approvals. The types live in `app-server-protocol/src/protocol/{common,v1,v2}.rs` and are consumed by the app server implementation in `app-server/`.
...
- Notifications
  ...
  - `workx/event/*` stream notifications for live agent events
...
Run Workx as an MCP server and connect an MCP client:

```bash
workx mcp-server | your_mcp_client
```

For a simple inspection UI, you can also try:

```bash
npx @modelcontextprotocol/inspector workx mcp-server
```

Use the separate `workx mcp` subcommand to manage configured MCP server launchers in `config.toml`.
...
Fetch the catalog of models available in the current Workx build with `model/list`. The request accepts optional pagination inputs:
...
- `workx/event` with the serialized Workx event payload. The shape matches `core/src/protocol.rs`'s `Event` and `EventMsg` types. Some notifications include a `_meta.requestId` to correlate with the originating request.
...
The `workx` and `workx-reply` tools return standard MCP `CallToolResult` payloads. For compatibility with MCP clients that prefer `structuredContent`, Workx mirrors the content blocks inside `structuredContent` alongside the `threadId`.

Example:

```json
{
  "content": [{ "type": "text", "text": "Hello from Workx" }],
  "structuredContent": {
    "threadId": "019bbed6-1e9e-7f31-984c-a05b65045719",
    "content": "Hello from Workx"
  }
}
```
...
When Workx needs approval to apply changes or run commands, the server issues JSON-RPC requests to the client:

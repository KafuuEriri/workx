...
These are entities exit on the workx backend. The intent of this section is to establish vocabulary and construct a shared mental model for the `Workx` core system.

0. `Model`
   - In our case, this is the Responses REST API
1. `Workx`
   - The core engine of workx
...
2. `Session`
   - The `Workx`'s current configuration and state
   - `Workx` starts with no `Session`, and it is initialized by `Op::ConfigureSession`, which should be the first message sent by the UI.
...
3. `Task`
   - A `Task` is `Workx` executing work in response to user input.
...
4. `Turn`
   - One cycle of iteration in a `Task`, consists of:
     ...
     - `Workx` then executes command(s), applies patch(es), and outputs message(s) returned by the `Model`
...
The term "UI" is used to refer to the application driving `Workx`. This may be the CLI / TUI chat-like interface that users operate, or it may be a GUI interface like a VSCode extension. The UI is external to `Workx`, as `Workx` is intended to be operated by arbitrary UI implementations.
...
Since only 1 `Task` can be run at a time, for parallel tasks it is recommended that a single `Workx` be run for each thread of work.
...
- `Submission`
  - These are messages sent on the `SQ` (UI -> `Workx`)
...
- `Event`
  - These are messages sent on the `EQ` (`Workx` -> UI)
...
sequenceDiagram
    ...
    user->>workx: Op::ConfigureSession
    workx-->>session: create session
    workx->>user: Event::SessionConfigured

//! Custom CA handling shared by Workx outbound HTTP and websocket clients.
//!
//! Workx constructs outbound reqwest clients and secure websocket connections in a few crates, but
...
//!
//! - read CA material from `WORKX_CA_CERTIFICATE`, falling back to `SSL_CERT_FILE`
...
//!   because Workx itself commonly runs spawned test processes under seatbelt, so this is not just
...
use tracing::info;
use tracing::warn;
use workx_utils_rustls_provider::ensure_rustls_crypto_provider;

pub const WORKX_CA_CERT_ENV: &str = "WORKX_CA_CERTIFICATE";
pub const SSL_CERT_FILE_ENV: &str = "SSL_CERT_FILE";
const CA_CERT_HINT: &str = "If you set WORKX_CA_CERTIFICATE or SSL_CERT_FILE, ensure it points to a PEM file containing one or more CERTIFICATE blocks, or unset it to use system roots.";
...
/// Builds a reqwest client that honors Workx custom CA environment variables.
///
/// Callers supply the baseline builder configuration they need, and this helper layers in custom
/// CA handling before finally constructing the client. `WORKX_CA_CERTIFICATE` takes precedence
...
/// easy mistake to make when adding a new outbound Workx HTTP path, and the resulting bug only
...
/// Builds a rustls client config when a Workx custom CA bundle is configured.
///
/// This is the websocket-facing sibling of [`build_reqwest_client_with_custom_ca`]. When
/// `WORKX_CA_CERTIFICATE` or `SSL_CERT_FILE` selects a CA bundle, the returned config starts from
...
/// Builds a rustls client config using native roots and any configured Workx custom CA bundle.
///
...
fn build_rustls_client_config(
    ...
    // they would get from tungstenite's default rustls connector, then layer in the Workx custom
...
fn build_reqwest_client_with_env(
    ...
    info!(
        workx_ca_certificate_configured = false,
...
trait EnvSource {
    ...
    ///
    /// `WORKX_CA_CERTIFICATE` wins over `SSL_CERT_FILE` because it is the Workx-specific override.
    ...
    fn configured_ca_bundle(&self) -> Option<ConfiguredCaBundle> {
        self.non_empty_path(WORKX_CA_CERT_ENV)
            .map(|path| ConfiguredCaBundle {
                source_env: WORKX_CA_CERT_ENV,
                path,
            })
            .or_else(|| {
...
impl ConfiguredCaBundle {
    ...
    /// Loads every certificate block from a PEM file intended for Workx CA overrides.
    ///
    /// This accepts a few common real-world variants so Workx behaves like other CA-aware tooling:
...
impl NormalizedPem {
    ...
    ///
    /// Workx only needs certificate DER bytes to seed `reqwest`'s root store, but operators may
...
mod tests {
    ...
    #[test]
    fn ca_path_prefers_workx_env() {
        let env = map_env(&[
            (WORKX_CA_CERT_ENV, "/tmp/workx.pem"),
            (SSL_CERT_FILE_ENV, "/tmp/fallback.pem"),
        ]);

        assert_eq!(
            env.configured_ca_bundle().map(|bundle| bundle.path),
            Some(PathBuf::from("/tmp/workx.pem"))
        );
    }
    ...
    fn rustls_config_uses_custom_ca_bundle_when_configured() {
        ...
        let env = map_env(&[(WORKX_CA_CERT_ENV, cert_path.to_string_lossy().as_ref())]);

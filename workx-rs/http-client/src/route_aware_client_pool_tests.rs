...
async fn request_failures_classify_https_proxy_authentication_challenges() {
    let (address, proxy) = spawn_response_server(vec![
        "HTTP/1.1 407 Proxy Authentication Required\r\n\
         Proxy-Authenticate: Basic realm=\"workx\"\r\n\
         Content-Length: 0\r\n\
         Connection: close\r\n\r\n"
            .to_string(),
    ]);
...
async fn legacy_custom_ca_fallback_is_limited_to_reqwest_default() {
    const CHILD_POLICY_ENV: &str = "WORKX_HTTP_CLIENT_POOL_INVALID_CA_TEST_POLICY";
        ...
        tracing_subscriber::fmt::layer()
            ...
            .with_filter(
                tracing_subscriber::filter::Targets::new()
                    .with_target("workx_http_client", tracing::Level::TRACE),
            ),
    );
    let _guard = tracing::subscriber::set_default(subscriber);
    tracing::debug!(target: "workx_http_client", "log capture sentinel");

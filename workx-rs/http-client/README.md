...
`workx-http-client` is the low-level HTTP transport shared by Workx crates. It is the intended
...
- custom CA handling through `WORKX_CA_CERTIFICATE` and `SSL_CERT_FILE`;
...
feature requires more than enabling reqwest's default proxy behavior: Workx must resolve platform
...
Higher-level retry, SSE, and request-attempt telemetry policy remains in `workx-client`.
...
would skip Workx's route selection for the redirect target. In `RespectSystemProxy` mode the pool
...
```rust
use workx_http_client::ClientRouteClass;
use workx_http_client::ReqwestTransport;

let client = http_client_factory.build_client(api_url, ClientRouteClass::Api)?;
let transport = ReqwestTransport::from_http_client(client);
```

If the existing wrapper surface cannot support a use case, extend `workx-http-client` rather than

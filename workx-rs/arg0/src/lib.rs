...
use workx_apply_patch::WORKX_CORE_APPLY_PATCH_ARG1;
use workx_async_utils::THREAD_STACK_SIZE_BYTES;
#[cfg(unix)]
use workx_exec_server::WORKX_ARG0_EXEC_HELPER_ARG1;
use workx_exec_server::WORKX_FS_HELPER_ARG1;
use workx_install_context::InstallContext;
use workx_sandboxing::landlock::WORKX_LINUX_SANDBOX_ARG0;
use workx_utils_home_dir::find_workx_home;
#[cfg(target_os = "windows")]
use workx_windows_sandbox::WORKX_WINDOWS_SANDBOX_ARG1;

const APPLY_PATCH_ARG0: &str = "apply_patch";
const MISSPELLED_APPLY_PATCH_ARG0: &str = "applypatch";
#[cfg(unix)]
const EXECVE_WRAPPER_ARG0: &str = "workx-execve-wrapper";
const LOCK_FILENAME: &str = ".lock";

#[derive(Clone, Debug, Default, Eq, PartialEq)]
pub struct Arg0DispatchPaths {
    /// Stable path to the current Workx executable for child re-execs.
    ///
...
pub fn arg0_dispatch() -> Option<Arg0PathEntryGuard> {
    ...
    if exe_name == EXECVE_WRAPPER_ARG0 {
        ...
        let exit_code = runtime.block_on(
            workx_shell_escalation::run_shell_escalation_execve_wrapper(file, argv),
        );
        match exit_code {
    ...
    if exe_name == WORKX_LINUX_SANDBOX_ARG0 {
        // Safety: [`run_main`] never returns.
        workx_linux_sandbox::run_main();
    } else if exe_name == APPLY_PATCH_ARG0 || exe_name == MISSPELLED_APPLY_PATCH_ARG0 {
        workx_apply_patch::main();
    }

    let argv1 = args.next().unwrap_or_default();
    #[cfg(unix)]
    if argv1 == WORKX_ARG0_EXEC_HELPER_ARG1 {
        workx_exec_server::run_arg0_exec_helper_main();
    }
    if argv1 == WORKX_FS_HELPER_ARG1 {
        workx_exec_server::run_fs_helper_main();
    }
    #[cfg(target_os = "windows")]
    if argv1 == WORKX_WINDOWS_SANDBOX_ARG1 {
        workx_windows_sandbox::run_windows_sandbox_wrapper_main();
    }
    if argv1 == WORKX_CORE_APPLY_PATCH_ARG1 {
        let patch_arg = args.next().and_then(|s| s.to_str().map(str::to_owned));
        let exit_code = match patch_arg {
            Some(patch_arg) => {
                let mut stdout = std::io::stdout();
                let mut stderr = std::io::stderr();
                let cwd = match workx_utils_absolute_path::AbsolutePathBuf::current_dir() {
                    Ok(cwd) => cwd,
                    Err(_) => std::process::exit(1),
                };
                let runtime = match tokio::runtime::Builder::new_current_thread()
                ...
                let cwd = cwd.into();
                let update_file_mode = workx_apply_patch::apply_patch_file_update_mode_from_env();
                match runtime.block_on(workx_apply_patch::apply_patch_with_options(
                    &patch_arg,
                    workx_apply_patch::ApplyPatchOptions {
                        update_file_mode,
                        ..Default::default()
                    },
                    &cwd,
                    &mut stdout,
                    &mut stderr,
                    workx_exec_server::LOCAL_FS.as_ref(),
                    /*sandbox*/ None,
                )) {
                    Ok(_) => 0,
                    Err(_) => 1,
            ...
            None => {
                eprintln!("Error: {WORKX_CORE_APPLY_PATCH_ARG1} requires a UTF-8 PATCH argument.");
                1
            }
        };
    ...
    let (path_entry_guard, updated_path_env_var) = prepare_path_env_var_with_aliases(
        ...
        std::env::var_os("PATH"),
        prepare_path_entry_for_workx_aliases,
    );
...
fn prepare_path_env_var_with_aliases(
    ...
    match prepare_aliases(path_for_aliases) {
        ...
        Err(err) => {
            // It is possible that Workx will proceed successfully even if
...
/// While we want to deploy the Workx CLI as a single executable for simplicity,
...
/// `workx-linux-sandbox` we *directly* execute
/// [`workx_linux_sandbox::run_main`] (which never returns). Otherwise we:
///
/// 1.  Load `.env` values from `~/.workx/.env` before creating any threads.
...
///     `workx-linux-sandbox` helper path (falling back to the current
    ...
    Fut: Future<Output = anyhow::Result<()>>,
{
    let paths = Arg0DispatchPaths {
        workx_self_exe: current_exe.clone(),
        workx_linux_sandbox_exe: if cfg!(target_os = "linux") {
...
fn linux_sandbox_exe_path(
    ...
    // Prefer the `workx-linux-sandbox` alias when available so callers can
    // re-exec through a path whose basename still triggers arg0 dispatch on
    // bubblewrap builds that do not support `--argv0`.
    path_entry_guard
        .and_then(|path_entry| path_entry.paths().workx_linux_sandbox_exe.clone())
        .or(current_exe)
}
...
const ILLEGAL_ENV_VAR_PREFIX: &str = "WORKX_";

/// Load env vars from ~/.workx/.env.
///
/// Security: Do not allow `.env` files to create or modify any variables
/// with names starting with `WORKX_`.
fn load_dotenv() {
    if let Ok(workx_home) = find_workx_home()
        && let Ok(iter) = dotenvy::from_path_iter(workx_home.join(".env"))
    {
        set_filtered(iter);
    }
}

/// Helper to set vars from a dotenvy iterator while filtering out `WORKX_` keys.
...
///   with the hidden `--workx-run-as-apply-patch` flag.
///
...
/// user to install a separate executable, simplifying the deployment of Workx
/// CLI.
...
fn prepare_path_entry_for_workx_aliases(
    existing_path: Option<OsString>,
    ...
    #[cfg(not(debug_assertions))]
    {
        ...
        if workx_home.starts_with(&temp_root) {
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
                format!(
                    "Refusing to create helper binaries under temporary dir {temp_root:?} (workx_home: {workx_home:?})"
                ),
            ));
        }
    }
    ...
    // Use a WORKX_HOME-scoped temp root to avoid cluttering the top-level directory.
    let temp_root = workx_home.join("tmp").join("arg0");
    ...
    for filename in &[
        ...
        #[cfg(windows)]
        {
            ...
            std::fs::write(
                ...
                format!(
                    r#"@echo off
"{exe}" {WORKX_CORE_APPLY_PATCH_ARG1} %*
"#,
                ),
            )?;
        }
    ...
    let paths = Arg0DispatchPaths {
        workx_self_exe: std::env::current_exe().ok(),
        workx_linux_sandbox_exe: {
            #[cfg(target_os = "linux")]
            {
                Some(path.join(WORKX_LINUX_SANDBOX_ARG0))
            }
...
mod tests {
    ...
    fn package_path_test_fixture() -> anyhow::Result<PackagePathTestFixture> {
        ...
        let path_dir = package_dir.join("workx-path");
        ...
        let install_context = InstallContext {
            ...
            package_layout: Some(WorkxPackageLayout {
    ...
    fn windows_batch_alias_preserves_unicode_executable_paths() -> anyhow::Result<()> {
        ...
        let alias_directory = profile.join(".workx").join("tmp").join("arg0");
    ...
    fn windows_batch_alias_preserves_cross_volume_executable_paths() {
        assert_eq!(
            super::windows_batch_executable_path(
                Path::new(r"D:\Tools\workx.exe"),
                Path::new(r"C:\Users\person\.workx\tmp\arg0"),
            ),
            r"D:\Tools\workx.exe",
        );
    }
    ...
    #[test]
    fn linux_sandbox_exe_path_prefers_workx_linux_sandbox_alias() -> std::io::Result<()> {
        let temp_dir = TempDir::new()?;
        let lock_file = create_lock(temp_dir.path())?;
        let alias_path = temp_dir.path().join("workx-linux-sandbox");
        let path_entry = Arg0PathEntryGuard::new(
            temp_dir,
            lock_file,
            Arg0DispatchPaths {
                workx_self_exe: Some(PathBuf::from("/usr/bin/workx")),
                workx_linux_sandbox_exe: Some(alias_path.clone()),
                main_execve_wrapper_exe: None,
            },
        );

        assert_eq!(
            linux_sandbox_exe_path(Some(&path_entry), Some(PathBuf::from("/usr/bin/workx"))),
            Some(alias_path),
        );
        Ok(())
    }
    ...
    fn run_main_with_arg0_guard_keeps_aliases_alive_until_main_returns() -> anyhow::Result<()> {
        let temp_dir = TempDir::new()?;
        let alias_path = temp_dir.path().join("workx-helper-alias");
        ...
        super::build_runtime()?.block_on(run_main_with_arg0_guard(
            ...
            Some(PathBuf::from("/usr/bin/workx")),
            |paths| async move {

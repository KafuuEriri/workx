...
use workx_config::RemoveMarketplaceConfigOutcome;
use workx_config::format_config_layer_source;
use workx_config::remove_user_marketplace_config;
...
pub async fn remove_marketplace(
    ...
    tokio::task::spawn_blocking(move || {
        remove_marketplace_sync(workx_home.as_path(), &config_layer_stack, request)
    })
    .await
    .map_err(|err| {
...
fn remove_marketplace_sync(
    ...
    let user_config_file = AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, workx_home);
    ...
    let destination = marketplace_install_root(workx_home).join(&marketplace_name);
    let config_outcome =
        remove_user_marketplace_config(workx_home, &marketplace_name).map_err(|err| {
...
mod tests {
    ...
    fn remove_marketplace_sync_removes_config_and_installed_root() {
        ...
        let config =
            fs::read_to_string(workx_home.path().join(workx_config::CONFIG_TOML_FILE)).unwrap();
    ...
    fn remove_marketplace_sync_preserves_shadowed_system_marketplace() {
        ...
        let user_file =
            AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, workx_home.path());
        let system_file =
            AbsolutePathBuf::resolve_path_against_base("system.toml", workx_home.path());

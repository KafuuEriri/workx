...
const REMOTE_PLUGIN_INSTALL_METADATA_FILE: &str = ".workx-remote-plugin-install.json";
...
impl PluginStore {
    pub fn new(workx_home: PathBuf) -> Self {
        Self::try_new(workx_home)
            .unwrap_or_else(|err| panic!("plugin cache root should be absolute: {err}"))
    }

    pub fn try_new(workx_home: PathBuf) -> Result<Self, PluginStoreError> {
        let root = AbsolutePathBuf::from_absolute_path_checked(workx_home.join(PLUGINS_CACHE_DIR))
            .map_err(|err| PluginStoreError::io("failed to resolve plugin cache root", err))?;
        let data_root =
            AbsolutePathBuf::from_absolute_path_checked(workx_home.join(PLUGINS_DATA_DIR))
        ...
        let workx_home = AbsolutePathBuf::from_absolute_path_checked(workx_home)
            .map_err(|err| PluginStoreError::io("failed to resolve Workx home", err))?;
...
fn plugin_manifest_for_source(
    ...
    match manifest {
        ...
        InstallManifest::Fallback(contents) => parse_plugin_manifest(
            source_path,
            &source_path.join(".workx-plugin/plugin.json"),
            contents,
        )
...
fn replace_plugin_root_atomically(
    ...
    let (source_manifest_relative_path, source_manifest_contents) = match manifest {
        ...
        InstallManifest::Fallback(contents) => (
            PathBuf::from(".workx-plugin/plugin.json"),
    ...
    if let InstallManifest::Fallback(contents) = manifest {
        ...
        let manifest_path = staged_version_root.join(".workx-plugin/plugin.json");

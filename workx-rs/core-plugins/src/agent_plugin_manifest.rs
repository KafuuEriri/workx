...
use workx_utils_plugins::AGENT_PLUGIN_SCHEMA_PREFIX;
use workx_utils_plugins::AGENT_PLUGIN_SCHEMA_URI;
use workx_utils_plugins::SUPPORTED_AGENT_PLUGIN_SCHEMA_URIS;

const WORKX_AGENT_PLUGIN_EXTENSION_NAMESPACE: &str = "com.openai";
...
pub(super) fn parse_agent_plugin_manifest_uri(
    ...
    let workx_extension = object
        .get("extensions")
        .and_then(JsonValue::as_object)
        .and_then(|extensions| extensions.get(WORKX_AGENT_PLUGIN_EXTENSION_NAMESPACE))
        .and_then(|extension| {
            if extension.is_object() {
                ...
                tracing::warn!(
                    path = %manifest_path,
                    namespace = WORKX_AGENT_PLUGIN_EXTENSION_NAMESPACE,
    ...
    if let Some(extension_contents) = workx_extension.as_deref() {
        apply_workx_agent_plugin_extension(
            &mut resolved,
            plugin_root,
            manifest_path,
            extension_contents,
...
fn apply_workx_agent_plugin_extension(

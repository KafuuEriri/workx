...
impl PluginsManager {
    ...
    fn bundled_plugin_exclusion_path(
        ...
        if self.restriction_product != Some(Product::Workx) {
            return None;
        }
        let auth = auth.filter(|auth| auth.uses_workx_backend())?;
    ...
    pub(super) fn update_sites_exclusion(
        ...
        if changed {
            let result = serde_json::to_string(&exclusions)
                ...
                .and_then(|contents| workx_utils_path::write_atomically(&path, &contents));

...
#[tokio::test]
async fn returns_fallback_plugins_when_remote_disabled_for_workx_auth() {
    let workx_home = tempdir().expect("tempdir should succeed");
    write_file(
...
"#,
    ...
    let curated_root = curated_plugins_repo_path(workx_home.path());
    write_openai_curated_marketplace(&curated_root, &["sample", "slack", "openai-developers"]);

    let plugins = load_plugins_config(workx_home.path(), workx_home.path()).await;
...
async fn returns_api_curated_fallback_plugins_for_direct_provider_auth() {
    ...
    let auth = WorkxAuth::from_api_key("test-api-key");
    let discoverable_plugins = list_discoverable_plugins(
        &plugins_manager,
        discovery_input(plugins, &[], &[], &[]),
        Some(&auth),
...
async fn returns_microsoft_fallback_plugins() {
    ...
    install_marketplace_plugin(workx_home.path(), curated_root.as_path(), "teams").await;
...
#[tokio::test]
async fn omits_openai_curated_but_keeps_configured_marketplaces_for_remote_workx_auth() {
    let workx_home = tempdir().expect("tempdir should succeed");
    let curated_root = curated_plugins_repo_path(workx_home.path());
    write_openai_curated_marketplace(&curated_root, &["slack"]);

    let bundled_marketplace_name = OPENAI_BUNDLED_MARKETPLACE_NAME;
    let bundled_marketplace_root = workx_home.path().join(format!(
...
async fn includes_openai_api_curated_when_remote_enabled_without_auth() {
    ...
    let plugins = load_plugins_config(workx_home.path(), workx_home.path()).await;
    let plugins_manager = test_plugins_manager(workx_home.path().to_path_buf());
    let discoverable_plugins = list_discoverable_plugins(
        &plugins_manager,
        discovery_input(plugins, &[], &[], &[]),
        /*auth*/ None,
...
async fn clear_cache_invalidates_cached_tool_suggest_metadata() {
    ...
    let curated_root = curated_plugins_repo_path(workx_home.path());
    write_openai_curated_marketplace(&curated_root, &["slack"]);
    let plugin_manifest = curated_root.join("plugins/slack/.workx-plugin/plugin.json");
    write_file(
        &plugin_manifest,
        r#"{
  "name": "slack",
...
"#
    ...
    install_marketplace_plugin(workx_home.path(), curated_root.as_path(), "installed").await;
...
async fn normalizes_description() {
    ...
    write_openai_curated_marketplace(&curated_root, &["installed", "slack"]);
    write_file(
        &curated_root.join("plugins/slack/.workx-plugin/plugin.json"),
        r#"{
  "name": "slack",
  "description": "  Plugin\n   with   extra   spacing  "
}"#,
...
async fn omits_installed_curated_plugins() {
    ...
    let curated_root = curated_plugins_repo_path(workx_home.path());
    write_openai_curated_marketplace(&curated_root, &["slack"]);
    install_marketplace_plugin(workx_home.path(), curated_root.as_path(), "slack").await;
...
async fn does_not_reload_marketplace_per_plugin() {
    ...
    for plugin_name in ["gmail", "openai-developers"] {
        write_file(
            &curated_root.join(format!("plugins/{plugin_name}/.workx-plugin/plugin.json")),
            &format!(
                r#"{{
  "name": "{plugin_name}",
  ...
  "interface": {{
    ...
    assert_eq!(logs.matches("gmail/.workx-plugin/plugin.json").count(), 4);
    assert_eq!(
        logs.matches("openai-developers/.workx-plugin/plugin.json")
            .count(),
        4
    );
}
...
async fn does_not_expand_local_sales_apps() {
    ...
    let sales_marketplace_root = workx_home
        .path()
...
"#
    ...
    install_marketplace_plugin(workx_home.path(), sales_marketplace_root.as_path(), "sales").await;
...
"#,
    ...
    let auth = WorkxAuth::create_dummy_chatgpt_auth_for_testing();
    let mut plugins = load_plugins_config(workx_home.path(), workx_home.path()).await;
...
async fn install_marketplace_plugin(workx_home: &Path, marketplace_root: &Path, plugin_name: &str) {
    write_curated_plugin_sha_with(workx_home, TEST_CURATED_PLUGIN_SHA);
    let config = load_plugins_config(workx_home, marketplace_root).await;
    test_plugins_manager(workx_home.to_path_buf())
        .install_plugin(
            &config,

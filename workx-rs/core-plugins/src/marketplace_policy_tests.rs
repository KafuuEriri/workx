...
fn config_layer_stack_with_user_config(
    ...
    let requirements =
        workx_config::ConfigRequirements::try_from(with_sources).expect("normalize requirements");
...
fn system_marketplace_discovery_and_install_validate_source_and_root() {
    ...
    let config_file = AbsolutePathBuf::try_from(workx_home.path().join("config.toml"))
...
"#
    ...
    let stack = ConfigLayerStack::new(
        vec![ConfigLayerEntry::new(
            ConfigLayerSource::System {
                file: AbsolutePathBuf::try_from(workx_home.path().join("system.toml")).unwrap(),
            },
            layer.config.clone(),
        )],
    ...
    assert_eq!(
        policy.validate_install(&stack, workx_home.path(), &configured_path, "company"),
        Ok(())
    );
    assert!(
        policy
            .validate_install(&stack, workx_home.path(), &configured_path, "other")
            .expect_err("unconfigured name should fail")
            .contains("must be added to config")
    );
    assert!(
        policy
            .validate_install(&stack, workx_home.path(), &other_path, "company")
...
"#,
    ...
    let marketplace_path = AbsolutePathBuf::try_from(
        marketplace_install_root(workx_home.path()).join("debug/.agents/plugins/marketplace.json"),
    )
    .expect("absolute marketplace path");

    let err = MarketplacePolicy::from_requirements(stack.requirements())
        .validate_install(&stack, workx_home.path(), &marketplace_path, "debug")
...
"#,
    ...
    let marketplace_path = AbsolutePathBuf::try_from(
        curated_plugins_repo_path(workx_home.path()).join(".agents/plugins/marketplace.json"),
    )
    .expect("absolute marketplace path");
    let policy = MarketplacePolicy::from_requirements(stack.requirements());
        ...
        for marketplace in [
            ...
            assert!(
                MarketplacePolicy::from_requirements(config.requirements())
                    .validate_install(config, workx_home.path(), &repository_manifest, marketplace)
                    .is_err()
            );
        }
    }
...
fn symlinked_marketplaces_cannot_borrow_managed_provenance() {
    let workx_home = TempDir::new().expect("create Workx home");
    let official_root = curated_plugins_repo_path(workx_home.path());
    ...
    for (path, marketplace) in [
        ...
        assert!(
            MarketplacePolicy::from_requirements(stack.requirements())
                .validate_install(&stack, workx_home.path(), &manifest, marketplace)
                .is_err()
        );
    }
}
...
"#,
    ...
    let expected_name =
        validate_marketplace_source_for_add(workx_home.path(), stack.requirements(), &source)
...
"#,
    ...
    let projected =
        policy_filtered_plugin_config(&stack, workx_home.path()).expect("project plugin config");
    assert_eq!(
    ...
    assert_eq!(
        configured_plugins_from_stack(&stack, workx_home.path())
            .into_keys()
            .collect::<Vec<_>>(),
...
"#,
    ...
    assert!(!marketplace_install_root(workx_home.path()).exists());

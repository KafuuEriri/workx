...
use crate::loader::configured_curated_plugin_ids_from_workx_home;
...
use workx_config::clear_user_plugin;
use workx_config::set_user_plugin_enabled;
use workx_config::skill_config_rules_from_stack;
use workx_config::types::PluginConfig;
use workx_config::types::ToolSuggestDisabledTool;
use workx_config::types::ToolSuggestDiscoverableType;
use workx_hooks::plugin_hook_declarations;
...
use workx_plugin::PluginIdError;
use workx_plugin::PluginTelemetryMetadata;
use workx_plugin::app_connector_ids_from_declarations;
use workx_plugin::prompt_safe_plugin_description;
...
use workx_skills::SkillRootLoader;
use workx_skills::SkillRootSnapshots;
use workx_tools::DiscoverablePluginInfo;
...
use workx_utils_plugins::PluginIdentity;
use workx_utils_plugins::PluginSkillRoot;
...
impl RemoteInstalledPluginsAuthIdentity {
    fn from_auth(auth: Option<&WorkxAuth>) -> Self {
        Self {
            auth_mode: auth.map(WorkxAuth::api_auth_mode),
            account_id: auth.and_then(WorkxAuth::get_account_id),
            chatgpt_user_id: auth.and_then(WorkxAuth::get_chatgpt_user_id),
            is_workspace_account: auth.map(WorkxAuth::is_workspace_account),
        }
    }
}
...
impl RemoteCatalogCacheRefreshRequest {
    fn has_same_cache_identity(&self, other: &Self) -> bool {
        self.service_config == other.service_config
            && self.auth.as_ref().and_then(WorkxAuth::get_account_id)
                == other.auth.as_ref().and_then(WorkxAuth::get_account_id)
            && self.auth.as_ref().and_then(WorkxAuth::get_chatgpt_user_id)
                == other.auth.as_ref().and_then(WorkxAuth::get_chatgpt_user_id)
            && self.auth.as_ref().map(WorkxAuth::is_workspace_account)
                == other.auth.as_ref().map(WorkxAuth::is_workspace_account)
    }
}
...
fn featured_plugin_ids_cache_key(
    ...
    FeaturedPluginIdsCacheKey {
        ...
        account_id: auth.and_then(WorkxAuth::get_account_id),
        chatgpt_user_id: auth.and_then(WorkxAuth::get_chatgpt_user_id),
        is_workspace_account: auth.is_some_and(WorkxAuth::is_workspace_account),
    }
}
...
fn target_curated_marketplace(auth_mode: Option<AuthMode>) -> TargetCuratedMarketplace {
    if auth_mode.is_some_and(AuthMode::uses_workx_backend) {
...
impl PluginsManager {
    ...
    pub fn new_with_options(
        ...
        // Product restrictions are enforced at marketplace admission time for a given WORKX_HOME:
        ...
        // contents of that WORKX_HOME and does not re-filter configured plugins by product, so
        // already-admitted plugins may continue exposing MCP servers/tools from shared local state.
        //
        // This assumes a single WORKX_HOME is only used by one product.
        let remote_installed_plugin_bundle_sync_gate =
            crate::remote::remote_installed_plugin_bundle_sync_gate(&workx_home);
        Self {
            workx_home: workx_home.clone(),
            store: PluginStore::new(workx_home),
    ...
    fn remote_global_catalog_active(&self, config: &PluginsConfigInput) -> bool {
        config.remote_plugin_enabled && self.auth_mode().is_some_and(AuthMode::uses_workx_backend)
    }
    ...
    pub fn cached_global_remote_discoverable_plugins_for_config(
        ...
        let Some(auth) = auth.filter(|auth| auth.uses_workx_backend()) else {
            return Vec::new();
        };
    ...
    fn begin_remote_installed_plugins_reconcile(&self, auth: Option<&WorkxAuth>) -> Option<u64> {
    ...
    async fn install_resolved_plugin(
        ...
        let plugin_version =
            if is_openai_curated_marketplace_name(&resolved.plugin_id.marketplace_name) {
                let curated_plugin_version = read_curated_plugins_sha(self.workx_home.as_path())
                    .ok_or_else(|| {
        ...
        let workx_home = self.workx_home.clone();
        ...
        let result: StorePluginInstallResult = tokio::task::spawn_blocking(move || {
            let materialized =
                materialize_marketplace_plugin_source(workx_home.as_path(), &resolved.source)
    ...
    async fn uninstall_plugin_id(&self, plugin_id: PluginId) -> Result<(), PluginUninstallError> {
        ...
        clear_user_plugin(&self.workx_home, plugin_id.as_key())
            .await
            .map_err(anyhow::Error::from)?;
    ...
    pub async fn read_plugin_detail_for_marketplace_plugin(
        ...
        let source_path = if plugin.source.is_install_materialized() && plugin.installed {
            ...
            let materialized = tokio::task::spawn_blocking(move || {
                materialize_marketplace_plugin_source(workx_home.as_path(), &source)
            })
            .await
            .map_err(|err| {
                MarketplaceError::InvalidPlugin(format!(
        ...
        let loaded_manifest =
            if workx_utils_plugins::find_plugin_manifest_path(source_path.as_path()).is_some() {
    ...
    fn start_curated_repo_sync(
        ...
        if let Err(err) = std::thread::Builder::new()
            ...
            .spawn(move || {
                match sync_openai_plugins_repo(workx_home.as_path(), http_client_factory) {
                    Ok(curated_plugin_version) => {
                        let configured_curated_plugin_ids =
                            configured_curated_plugin_ids_from_workx_home(workx_home.as_path());
    ...
    fn configured_plugin_states(&self, config: &PluginsConfigInput) -> ConfiguredPluginStates {
        let configured_plugins =
            configured_plugins_from_stack(&config.config_layer_stack, self.workx_home.as_path());
        ...
        let curated_marketplace_path = if include_openai_curated {
            match target_curated_marketplace(self.auth_mode()) {
                TargetCuratedMarketplace::OpenAi | TargetCuratedMarketplace::OpenAiWithRemote => {
                    let curated_repo_root = curated_plugins_repo_path(self.workx_home.as_path());
                    curated_repo_root.is_dir().then_some(curated_repo_root)
                }
                TargetCuratedMarketplace::OpenAiApi => {
                    let api_marketplace_path =
                        curated_plugins_api_marketplace_path(self.workx_home.as_path());

...
use workx_plugin::PluginLoadOutcome;
...
fn synced_plugin_root(workx_home: &Path, marketplace: &str, plugin_name: &str) -> AbsolutePathBuf {
    let synced_root = curated_plugins_repo_path(workx_home);
    match marketplace {
    ...
    let root = PluginStore::new(workx_home.to_path_buf()).plugin_root(
        &plugin_id,
...
fn cached_remote_plugin_root(workx_home: &Path, plugin_name: &str) -> AbsolutePathBuf {
    let plugin_id = PluginId::new(
        plugin_name.to_string(),
        REMOTE_GLOBAL_MARKETPLACE_NAME.to_string(),
    )
    .expect("plugin id");
    let root = PluginStore::new(workx_home.to_path_buf()).plugin_root(&plugin_id, "1.2.3");
    fs::create_dir_all(root.as_path()).expect("create cached remote plugin root");
    root
}
fn installed_remote_plugin_root(workx_home: &Path, plugin_name: &str) -> AbsolutePathBuf {
    let root = cached_remote_plugin_root(workx_home, plugin_name);
    let plugin_id = PluginId::new(
        plugin_name.to_string(),
        REMOTE_GLOBAL_MARKETPLACE_NAME.to_string(),
    )
    .expect("plugin id");
    PluginStore::new(workx_home.to_path_buf())
...
fn resolves_primary_runtime_scripts_from_the_installed_plugin_cache() {
    ...
    fs::create_dir_all(source_root.join(".workx-plugin")).expect("create manifest directory");
    fs::write(
...
fn roots_for(workx_home: &Path, plugins: Vec<LoadedPlugin>) -> TrustedPluginRoots {
...
fn recognizes_windows_executor_plugin_cache_root() {
    ...
    let script = PathUri::parse(
        "file:///C:/Users/user/.workx/plugins/cache/openai-primary-runtime/presentations/0.1.29/skills/presentations/container_tools/mark_artifact_operation_started.mjs",
    )
    .expect("Windows script URI");

    assert!(executor_plugin_root_matches(&script, &attribution));
}
fn assert_invalid_metrics_manifest(workx_home: &Path, root: &AbsolutePathBuf, manifest: &str) {
...
fn assert_untrusted(workx_home: &Path, config_name: &str, root: &Path) {
    assert!(
        roots_for(workx_home, vec![loaded_plugin(config_name, root, ENABLED)])
            .roots
            .is_empty()
    );
}
...
fn only_emits_safe_normalized_relative_script_paths() {
    ...
    assert_eq!(
        normalized_relative_script_path(Path::new(
            "/home/user/.workx/plugins/cache/openai-curated/sample/scripts/run.py"
        )),
        None
    );
}

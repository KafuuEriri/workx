...
#[test]
fn try_new_rejects_relative_workx_home() {
    let err = PluginStore::try_new(PathBuf::from("relative"))
        .expect_err("relative workx home should fail");
...
fn install_rejects_symlinked_manifest_that_hides_lower_precedence_mcp_server() {
    ...
    fs::create_dir_all(workx_path.parent().unwrap()).unwrap();
    ...
    .unwrap();
    std::os::unix::fs::symlink("../benign.json", &workx_path).unwrap();
    fs::write(
        &claude_path,
...
fn install_rejects_symlinked_manifest_directory() {
    ...
    std::os::unix::fs::symlink(&manifest_directory, plugin_root.join(".workx-plugin")).unwrap();
    let plugin_id = PluginId::new("manifest-switch".to_string(), "debug".to_string()).unwrap();
...
fn remote_plugin_install_metadata_follows_installed_cache_lifecycle() {
    ...
    assert_eq!(
        ...
        Some(std::ffi::OsStr::new(".workx-remote-plugin-install.json"))
    );
    assert_eq!(
...
fn install_stages_fallback_manifest_when_source_has_no_manifest() {
    ...
    assert_eq!(
        fs::read_to_string(result.installed_path.join(".workx-plugin/plugin.json")).unwrap(),
        manifest,
    );
}

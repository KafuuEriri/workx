...
use workx_config::types::PluginMcpServerConfig;
use workx_connectors::parse_plugin_app_config;
use workx_connectors::parse_plugin_app_config_value;
use workx_mcp::parse_agent_plugin_mcp_config;
use workx_mcp::parse_plugin_mcp_config;
use workx_plugin::AppDeclaration;
use workx_plugin::LoadedPlugin;
...
use workx_utils_plugins::find_plugin_manifest_path;
...
async fn load_plugins_from_layer_stack_with_scope(
    ...
    let mut configured_plugins = merge_configured_plugins_with_remote_installed(
        configured_plugins_from_stack(config_layer_stack, store.workx_home().as_path()),
        extra_plugins,
        store,
        remote_global_catalog_active,
    );
...
pub fn refresh_curated_plugin_cache(
    ...
    let store = PluginStore::try_new(workx_home.to_path_buf()).map_err(|err| err.to_string())?;
    let curated_marketplace_paths = curated_marketplace_paths_for_cache_refresh(workx_home)?;
...
fn curated_marketplace_paths_for_cache_refresh(
    ...
    let api_marketplace_path = workx_home
        .join(".tmp/plugins")
...
fn refresh_non_curated_plugin_cache_with_mode(
    ...
    for plugin_id in configured_non_curated_plugin_ids {
        ...
        let refresh_result = (|| -> Result<bool, String> {
            let materialized =
                materialize_marketplace_plugin_source_with_mode(workx_home, &source, git_mode)
                    .map_err(|err| {
...
fn configured_plugins_from_workx_home(
    workx_home: &Path,
...
pub fn configured_curated_plugin_ids_from_workx_home(workx_home: &Path) -> Vec<PluginId> {
    curated_plugin_ids_from_config_keys(configured_plugins_from_workx_home(
        workx_home,
    ...
    match &manifest_paths.hooks {
        ...
        Some(PluginManifestHooks::Inline(hooks_files)) => {
            let manifest_path = find_plugin_manifest_path(plugin_root.as_path())
                ...
                .unwrap_or_else(|| plugin_root.join(".workx-plugin/plugin.json"));
...
pub async fn load_configured_plugin_mcp_servers(
    ...
    let configured_plugins = configured_plugins_from_stack(config_layer_stack, workx_home);
...
pub(crate) async fn load_plugin_mcp_servers_from_manifest_with_format(
    ...
    if manifest_format == PluginManifestFormat::AgentPlugin {
        agent_plugin_mcp_overlay::apply_workx_env_overlay(plugin_root, &mut mcp_servers).await;
    }
...
pub fn materialize_marketplace_plugin_source(
    ...
    materialize_marketplace_plugin_source_with_mode(workx_home, source, PluginGitMode::Manual)
}
...
pub(crate) fn materialize_marketplace_plugin_source_with_mode(
    ...
    match source {
        ...
        MarketplacePluginSource::Git {
            ...
            let staging_root = workx_home.join("plugins/.marketplace-plugin-source-staging");
...
fn clone_git_plugin_source(
    ...
    let clone_cwd = match mode {
        PluginGitMode::Automatic => Some(workx_home),

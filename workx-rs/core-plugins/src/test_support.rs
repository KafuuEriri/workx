...
use workx_login::ExternalAuth;
use workx_login::ExternalAuthFuture;
use workx_login::ExternalAuthRefreshContext;
...
use workx_skills::LoadedSkills;
use workx_skills::SkillError;
use workx_skills::SkillLoadFuture;
use workx_skills::SkillMetadata;
use workx_skills::SkillRootLoadRequest;
use workx_skills::SkillRootLoader;
use workx_skills::parse_skill_frontmatter_metadata;
...
use workx_utils_plugins::migrated_command_skills_root;
...
pub(crate) fn test_plugins_manager(workx_home: PathBuf) -> PluginsManager {
    PluginsManager::new(
        workx_home,
...
pub(crate) fn test_auth_manager(auth_mode: Option<AuthMode>) -> Arc<AuthManager> {
    auth_manager_from_optional_auth(test_workx_auth(auth_mode))
}

pub(crate) async fn set_test_auth_mode(auth_manager: &AuthManager, auth_mode: Option<AuthMode>) {
    set_test_auth(auth_manager, test_workx_auth(auth_mode)).await;
}

pub(crate) async fn set_test_auth(auth_manager: &AuthManager, auth: Option<WorkxAuth>) {
...
fn test_workx_auth(auth_mode: Option<AuthMode>) -> Option<WorkxAuth> {
    auth_mode.map(|auth_mode| match auth_mode {
        AuthMode::ApiKey => WorkxAuth::from_api_key("test-api-key"),
        AuthMode::Chatgpt => WorkxAuth::create_dummy_chatgpt_auth_for_testing(),
        AuthMode::ChatgptAuthTokens => WorkxAuth::from_external_chatgpt_tokens(
            "header.e30.test",
            "test-account",
            /*chatgpt_plan_type*/ None,
        )
        .expect("test ChatGPT tokens should parse"),
        AuthMode::Headers => WorkxAuth::Headers(AuthHeaders::new(http::HeaderMap::new())),
        AuthMode::BedrockApiKey => WorkxAuth::BedrockApiKey(BedrockApiKeyAuth {
            api_key: "test-api-key".to_string(),
            region: "us-east-1".to_string(),
        }),
        AuthMode::BedrockAccessKeys => WorkxAuth::BedrockAccessKeys(BedrockAccessKeysAuth {
        ...
        AuthMode::AgentIdentity | AuthMode::PersonalAccessToken => {
            panic!("test auth mode requires a purpose-built WorkxAuth")
        }
    })
}
...
pub(crate) fn write_curated_plugin_sha_with(workx_home: &Path, sha: &str) {
    write_file(&workx_home.join(".tmp/plugins.sha"), &format!("{sha}\n"));
}

pub(crate) async fn load_plugins_config(workx_home: &Path, cwd: &Path) -> PluginsConfigInput {
    let workx_home = AbsolutePathBuf::try_from(workx_home).expect("workx home should be absolute");

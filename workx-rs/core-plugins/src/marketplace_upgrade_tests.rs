...
"#,
    ...
    assert_eq!(
        read_configured_git_marketplace(&config_reloader(workx_home.path()), "good")
...
fn one_upgrade_failure_does_not_block_another_marketplace() {
    ...
    let good_url = url::Url::from_directory_path(remote_repo.path())
        ...
        .to_string();
    let missing_url = url::Url::from_directory_path(workx_home.path().join("missing-repository"))
...
"#
    ...
    std::fs::write(workx_home.path().join(CONFIG_TOML_FILE), &config).expect("write config");
    let reload_config = config_reloader(workx_home.path());
    ...
    assert_eq!(
        ...
        vec![
            AbsolutePathBuf::try_from(marketplace_install_root(workx_home.path()).join("good"))
                .expect("installed marketplace root")
        ]
    );
    assert_eq!(
        std::fs::read_to_string(workx_home.path().join(CONFIG_TOML_FILE)).unwrap(),
        config
    );
}
...
fn automatic_marketplace_git_ignores_inherited_repository_configuration() {
    const CHILD_HOME: &str = "WORKX_MARKETPLACE_GIT_ISOLATION_CHILD_HOME";
    const CHILD_SOURCE: &str = "WORKX_MARKETPLACE_GIT_ISOLATION_CHILD_SOURCE";

    if let Some(workx_home) = std::env::var_os(CHILD_HOME) {
        let workx_home = PathBuf::from(workx_home);
        let source = std::env::var(CHILD_SOURCE).expect("read configured marketplace source");
        let config =
            format!("[marketplaces.trusted]\nsource_type = \"git\"\nsource = {source:?}\n");
        std::fs::write(workx_home.join(CONFIG_TOML_FILE), &config).expect("write config");
        let reload_config = config_reloader(&workx_home);
        let stack = reload_config().expect("load config");
            ...
            ConfiguredMarketplaceUpgradeOutcome {
                ...
                upgraded_roots: vec![
                    AbsolutePathBuf::try_from(
                        marketplace_install_root(&workx_home).join("trusted")
                    )
                    .expect("installed marketplace root"),
                ],
                errors: Vec::new(),
    ...
    let workx_home = project.join("workx-home");
    ...
    std::fs::write(
        ...
        "manual.txt filter=workx-required\n",
    )
    ...
    let output = Command::new(std::env::current_exe().expect("locate test binary"))
        ...
        .env("GIT_CONFIG_KEY_2", "filter.workx-required.smudge")
...
fn upgrade_uses_validated_source_for_git_operations() {
    ...
    let normalized_url = url::Url::from_directory_path(remote_repo.path())
        .expect("remote repository URL")
        .to_string();
    let raw_source = workx_home.path().join("missing-raw-source");
...
"#
    ...
    let install_root = marketplace_install_root(workx_home.path());

    let upgraded_root = upgrade_configured_git_marketplace(
        workx_home.path(),
        &install_root,
        &marketplace,
        &config_reloader(workx_home.path()),
...
fn up_to_date_fast_path_validates_marketplace_name() {
    ...
    let missing_source = workx_home.path().join("missing-source");
...
fn config_reloader(workx_home: &Path) -> ConfigLayerReload {
    let workx_home = workx_home.to_path_buf();
    let options = LoaderOverrides {
        system_config_path: Some(workx_home.join("system.toml")),
        managed_config_path: Some(workx_home.join("managed.toml")),
        system_requirements_path: Some(workx_home.join("requirements.toml")),
    ...
    Arc::new(move || {
        runtime.block_on(load_config_layers_state(
            LOCAL_FS.as_ref(),
            &workx_home,
            Some(AbsolutePathBuf::try_from(workx_home.join("project"))?),
            &[],
            options.clone(),
...
fn system_marketplace_stack(workx_home: &Path, system: &str, user: &str) -> ConfigLayerStack {
    let system_file = AbsolutePathBuf::try_from(workx_home.join("system.toml")).unwrap();
    std::fs::write(&system_file, system).unwrap();
    if !user.is_empty() {
        std::fs::write(workx_home.join(CONFIG_TOML_FILE), user).unwrap();
    }
    config_reloader(workx_home)().expect("load system and user config")
}
...
fn system_marketplace_downloads_and_updates_without_writing_user_config() {
    for user_config in ["", "[marketplaces.good]\nsparse_paths = []\n"] {
        ...
        let stack = system_marketplace_stack(workx_home.path(), &system, user_config);
        let reload_config = config_reloader(workx_home.path());
        let root =
            AbsolutePathBuf::try_from(marketplace_install_root(workx_home.path()).join("good"))
                .unwrap();
        ...
        assert_eq!(
            std::fs::read_to_string(workx_home.path().join("system.toml")).unwrap(),
            system
        );
        let user_file = workx_home.path().join(CONFIG_TOML_FILE);
...
fn changed_config_rolls_back_marketplace_activation() {
    ...
    let stack = system_marketplace_stack(workx_home.path(), &system, "");
    let reload_config = config_reloader(workx_home.path());
    ...
    let original_metadata = std::fs::read(root.join(".workx-marketplace-install.json")).unwrap();
    ...
    for (file, changed_config) in [
        ...
        std::fs::write(workx_home.path().join("system.toml"), &system).unwrap();
        std::fs::write(workx_home.path().join(file), changed_config).unwrap();
        let outcome = upgrade_configured_git_marketplaces(
            workx_home.path(),
            &stack,
            Some("good"),
        ...
        assert_eq!(
            std::fs::read(root.join(".workx-marketplace-install.json")).unwrap(),
            original_metadata
        );
    }
}

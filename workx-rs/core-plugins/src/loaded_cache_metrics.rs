...
pub(crate) const LOAD_DURATION: &str = "workx.plugins.loaded_cache.load.duration_ms";
pub(crate) const WAIT_DURATION: &str = "workx.plugins.loaded_cache.wait.duration_ms";
...
impl RequestOutcome {
    pub(crate) fn record(self) {
        ...
        let _ = metrics.counter(
            "workx.plugins.loaded_cache.request",
            /*inc*/ 1,
            &[("outcome", outcome)],
        );
    }
...
pub(crate) fn record_event(event: &'static str) {
    if let Some(metrics) = workx_otel::global() {
        let _ = metrics.counter(
            "workx.plugins.loaded_cache.event",
            /*inc*/ 1,
            &[("event", event)],
        );
    }

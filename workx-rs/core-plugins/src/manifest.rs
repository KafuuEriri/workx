...
use workx_utils_plugins::AGENT_PLUGIN_MANIFEST_RELATIVE_PATH;
...
pub type PluginManifest = workx_plugin::manifest::PluginManifest<AbsolutePathBuf>;
pub type PluginManifestHooks = workx_plugin::manifest::PluginManifestHooks<AbsolutePathBuf>;
pub type PluginManifestInterface = workx_plugin::manifest::PluginManifestInterface<AbsolutePathBuf>;
pub type PluginManifestMcpServers =
    workx_plugin::manifest::PluginManifestMcpServers<AbsolutePathBuf>;
pub type PluginManifestPaths = workx_plugin::manifest::PluginManifestPaths<AbsolutePathBuf>;

pub type UriPluginManifest = workx_plugin::manifest::PluginManifest<PathUri>;
...
pub(crate) fn load_plugin_manifest_with_format(plugin_root: &Path) -> Option<LoadedPluginManifest> {
    ...
    let overlay = if is_agent_plugin {
        let overlay_path = plugin_root.join(".workx-plugin/plugin.json");
...
fn resolve_raw_plugin_manifest(
    ...
    let interface = interface.and_then(|interface| {
        ...
        let interface = workx_plugin::manifest::PluginManifestInterface {
            display_name,
            short_description,
            long_description,
            developer_name,
    ...
    Ok(workx_plugin::manifest::PluginManifest {
        name,
        version,
        description,
        keywords,
        paths: workx_plugin::manifest::PluginManifestPaths {
...
fn resolve_manifest_hooks(
    plugin_root: &PathUri,
    hooks: Option<RawPluginManifestHooks>,
) -> Option<workx_plugin::manifest::PluginManifestHooks<PathUri>> {
    match hooks? {
        RawPluginManifestHooks::Path(path) => {
            resolve_manifest_path(plugin_root, "hooks", Some(&path))
                .map(|path| workx_plugin::manifest::PluginManifestHooks::Paths(vec![path]))
        }
        RawPluginManifestHooks::Paths(paths) => {
            let hooks = paths
                ...
                .collect::<Vec<_>>();
            (!hooks.is_empty()).then_some(workx_plugin::manifest::PluginManifestHooks::Paths(hooks))
        }
        RawPluginManifestHooks::Inline(hooks) => {
            Some(workx_plugin::manifest::PluginManifestHooks::Inline(vec![
                *hooks,
            ]))
        }
        RawPluginManifestHooks::InlineList(hooks) => (!hooks.is_empty())
            .then_some(workx_plugin::manifest::PluginManifestHooks::Inline(hooks)),
...
fn resolve_manifest_mcp_servers(
    plugin_root: &PathUri,
    mcp_servers: Option<RawPluginManifestMcpServers>,
) -> Option<workx_plugin::manifest::PluginManifestMcpServers<PathUri>> {
    match mcp_servers? {
        RawPluginManifestMcpServers::Path(path) => {
            resolve_manifest_path(plugin_root, "mcpServers", Some(&path))
                .map(workx_plugin::manifest::PluginManifestMcpServers::Path)
        }
        RawPluginManifestMcpServers::Object(servers) => match serde_json::to_string(&servers) {
            Ok(servers) => Some(workx_plugin::manifest::PluginManifestMcpServers::Object(
                servers,
            )),
            Err(err) => {
...
mod tests {
    ...
    use workx_plugin::PluginProvider;
    use workx_plugin::ResolvedPlugin;
    use workx_plugin::manifest::PluginManifest as GenericPluginManifest;
    use workx_plugin::manifest::PluginManifestHooks;
    use workx_plugin::manifest::PluginManifestInterface;
    use workx_plugin::manifest::PluginManifestMcpServers;
    use workx_plugin::manifest::PluginManifestPaths;
    ...
    fn write_manifest(plugin_root: &Path, version: Option<&str>, interface: &str) {
        fs::create_dir_all(plugin_root.join(".workx-plugin")).expect("create manifest dir");
        let version = version
  ...
  "interface": {{
    ...
    "composerIcon": "./assets/icon.svg"
        ...
        let manifest_contents =
            fs::read_to_string(plugin_root.join(".workx-plugin/plugin.json")).expect("manifest");

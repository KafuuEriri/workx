...
#[test]
fn legacy_workx_overlay_keeps_portable_components_fixed() {
    let tmp = tempdir().expect("tempdir");
    let plugin_root = tmp.path().join("demo-plugin");
    write_agent_plugin_manifest(
        &plugin_root,
  ...
  "description": "Portable description""#,
    ...
    fs::create_dir_all(plugin_root.join(".workx-plugin")).expect("create overlay dir");
    fs::write(
        plugin_root.join(".workx-plugin/plugin.json"),
        r#"{
  "name": "different-name",
  "version": "9.9.9",
  "description": "Workx description",
  "skills": [],
  "mcpServers": null,
  "interface": {"displayName": "Workx Demo"}
}"#,
    )
...
fn inline_openai_extension_precedes_legacy_overlay() {
  ...
  "extensions": {
    "com.openai": {
      "interface": {"displayName": "Inline Workx"}
    }
  }"#,
    );
    fs::create_dir_all(plugin_root.join(".workx-plugin")).expect("create overlay dir");
    fs::write(
        plugin_root.join(".workx-plugin/plugin.json"),
        r#"{"interface":{"displayName":"Legacy Workx"}}"#,
    )
    .expect("write overlay");

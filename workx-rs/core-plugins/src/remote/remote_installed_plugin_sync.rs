...
pub(crate) fn remote_installed_plugin_bundle_sync_gate(workx_home: &Path) -> Arc<Semaphore> {
    let plugin_cache_root = remote_plugin_cache_root(workx_home);
    let gates =
...
pub fn mark_remote_plugin_cache_mutation_in_flight(
    ...
    let key = RemotePluginCacheMutationKey {
        plugin_cache_root: remote_plugin_cache_root(workx_home),
...
async fn remove_stale_remote_plugin_caches(
    ...
    let workx_home = store.workx_home().as_path();
    for marketplace_name in [
        ...
        let marketplace_root = workx_home.join(PLUGINS_CACHE_DIR).join(marketplace_name);
        ...
        while let Some(entry) = entries.next_entry().await.map_err(|err| {
            ...
            if is_remote_plugin_cache_mutation_in_flight(workx_home, marketplace_name, &plugin_name)
            {
                continue;
            }
...
fn remote_plugin_cache_root(workx_home: &Path) -> PathBuf {
    workx_home.join(PLUGINS_CACHE_DIR)
}
...
mod tests {
    ...
    async fn sync_same_version_backfills_metadata_and_missing_version_requires_materialization() {
        ...
        let cached_manifest = workx_home
            .path()
        ...
        let metadata_path = PluginStore::new(workx_home.path().to_path_buf())
    ...
    async fn sync_all_scopes_paginates_and_reconciles_each_marketplace() {
        ...
        for (marketplace_name, plugin_name, _, _) in cached_plugins {
            ...
            assert!(
                plugin_root
                    .join("1.2.3/.workx-plugin/plugin.json")
                    .is_file()
            );
            assert_eq!(
                serde_json::from_str::<serde_json::Value>(
                    &std::fs::read_to_string(plugin_root.join(".workx-remote-plugin-install.json"))
                        .expect("read remote plugin install metadata")
                )
                .expect("parse remote plugin install metadata"),
                json!({
    ...
    async fn stale_remote_plugin_cleanup_removes_stale_marketplace_caches_and_keeps_canonical_cache()
     {
        let workx_home = tempfile::tempdir().expect("create workx home");
        let created_by_me_cached_manifest = workx_home
            .path()
            .join(PLUGINS_CACHE_DIR)
        ...
        std::fs::write(&cached_manifest, r#"{"name":"private-plugin"}"#)
            .expect("write cached plugin manifest");
        let canonical_cached_manifest = workx_home
            .path()
            .join(PLUGINS_CACHE_DIR)

...
fn catalog_cache_paths_are_isolated_by_scope_and_collection() {
    let workx_home = Path::new("/tmp/workx-home");
    ...
    .map(|scope| cache_path(workx_home, &cache_key_for_scope(scope)));
    ...
    assert!(!paths.contains(&cache_path(workx_home, &collection_key)));
}
...
fn global_catalog_cache_reuses_legacy_cache_file() {
    ...
    let auth = WorkxAuth::Headers(AuthHeaders::new(http::HeaderMap::new()));
    let legacy_cache_path = workx_home
        .path()
    ...
    workx_utils_path::write_atomically(&legacy_cache_path, &contents).expect("write legacy cache");
...
fn header_auth_does_not_cache_private_catalogs_without_a_stable_identity() {
    ...
    for scope in [RemotePluginScope::User, RemotePluginScope::Workspace] {
        ...
        let insecure_cache_path = cache_path(workx_home.path(), &insecure_cache_key);
        ...
        workx_utils_path::write_atomically(&insecure_cache_path, &contents)

...
const PERSONAL_MARKETPLACE_NAME: &str = "workx-curated";
...
pub async fn checkout_remote_plugin_share(
    ...
    let local_paths = load_share_local_paths_for_checkout(workx_home)?;
...
fn load_share_local_paths_for_checkout(
    ...
    match local_paths::load_plugin_share_local_paths(workx_home) {
        Ok(paths) => Ok(paths),

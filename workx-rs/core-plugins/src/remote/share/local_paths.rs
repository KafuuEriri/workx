...
pub(crate) fn load_plugin_share_local_paths(
    ...
    read_plugin_share_local_paths(workx_home)
}
...
pub(crate) fn record_plugin_share_local_path(
    ...
    let _guard = lock_plugin_share_local_paths()?;
    let mut mapping = read_plugin_share_local_paths_for_update(workx_home)?;
    mapping.insert(remote_plugin_id.to_string(), plugin_path);
    write_plugin_share_local_paths(workx_home, mapping)
}
...
fn read_plugin_share_local_paths(
    ...
    let path = plugin_share_local_paths_path(workx_home);
...
fn read_plugin_share_local_paths_for_update(
    ...
    match read_plugin_share_local_paths(workx_home) {
...
fn plugin_share_local_paths_path(workx_home: &Path) -> std::path::PathBuf {
    workx_home.join(PLUGIN_SHARE_LOCAL_PATHS_FILE)
}

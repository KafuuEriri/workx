...
#[tokio::test]
async fn recommended_plugins_requests_workx_suggestions_endpoint() {
    let server = MockServer::start().await;
    Mock::given(method("GET"))
        .and(path("/backend-api/ps/plugins/suggested/workx"))
    ...
    assert_eq!(
        recorded_http_client_urls(&selected_urls),
        vec![format!(
            "{}/backend-api/ps/plugins/suggested/workx?scope=GLOBAL",
            server.uri()
        )]
    );
}
...
async fn remote_catalog_cache_modes_control_refresh_and_persist_fetched_results() {
    ...
    for (mode, expire_cache, expected_refresh_needed, expected_cache_used) in [
        ...
        if expire_cache {
            let cache_path =
                std::fs::read_dir(workx_home.path().join("cache/remote_plugin_catalog"))
...
fn catalog_cache_invalidation_clears_global_collections_and_preserves_other_scopes() {
    ...
    assert_eq!(
        cached_remote_plugin_catalog_scopes(workx_home.path(), &config, Some(&auth)),
...
#[test]
fn recommended_plugins_accept_workx_suggestion_items() {

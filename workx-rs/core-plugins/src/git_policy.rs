...
impl PluginGitMode {
    ...
    pub(crate) fn command(self, git_binary: &Path) -> Command {
        ...
        command.args(["-c", workx_git_utils::SAFE_BARE_REPOSITORY_CONFIG]);
...
pub(crate) fn configure_trusted_git_repository(
    ...
    let canonical_home = AbsolutePathBuf::from_absolute_path(workx_home)
        .and_then(|path| path.canonicalize())
        .map_err(|err| format!("failed to resolve trusted Workx home: {err}"))?;
    ...
    if !staging_root.as_path().starts_with(canonical_home.as_path()) {
        return Err("trusted Git repository root escapes Workx home".to_string());
    }

...
use workx_http_client::RouteAwareRequestError;
...
const WORKX_PRODUCT_SKU: &str = "workx";
...
pub(crate) async fn fetch_and_cache_remote_plugin_catalog(
    ...
    catalog_cache::write_cached_directory_plugins(
        workx_home, config, auth, scope, /*collection*/ None, &plugins,
    );
    Ok(())
}
...
pub async fn fetch_and_cache_global_remote_plugin_catalog(
    ...
    fetch_and_cache_remote_plugin_catalog(workx_home, config, auth, RemotePluginScope::Global).await
}
...
pub fn invalidate_cached_remote_plugin_catalog_scopes(
    ...
    for scope in scopes {
        catalog_cache::remove_cached_directory_plugins(
            workx_home, config, auth, *scope, /*collection*/ None,
        );
...
pub async fn fetch_recommended_plugins(
    ...
    let mut url = Url::parse(&format!("{base_url}/ps/plugins/suggested/workx"))
...
pub async fn uninstall_remote_plugin(
    ...
    tokio::task::spawn_blocking(move || {
        remove_remote_plugin_cache(workx_home, marketplace_name, plugin_name, legacy_plugin_id)
    })
    .await
    .map_err(|err| {
...
fn remove_remote_plugin_cache(
    ...
    let store = PluginStore::try_new(workx_home.clone())
    ...
    let legacy_remote_plugin_cache_root = workx_home
...
async fn fetch_directory_plugins_for_scope_with_cache(
    ...
    if cache_mode != RemotePluginCatalogCacheMode::ForceRefetch
        && let Some(workx_home) = workx_home
        && let Some(cached) = catalog_cache::load_cached_directory_plugins(
            workx_home, config, auth, scope, collection,
        )
    ...
    if let Some(workx_home) = workx_home {
        catalog_cache::write_cached_directory_plugins(
            workx_home, config, auth, scope, collection, &plugins,
        );
    }
...
fn ensure_chatgpt_auth(auth: Option<&WorkxAuth>) -> Result<&WorkxAuth, RemotePluginCatalogError> {
...
fn authenticated_request(
    ...
    request
        ...
        .headers(workx_model_provider::auth_provider_from_auth(auth).to_auth_headers())
        .header(OAI_PRODUCT_SKU_HEADER, WORKX_PRODUCT_SKU)
}

...
use workx_config::MarketplaceAllowedSourceKind;
use workx_config::MarketplaceAllowedSourceToml;
use workx_config::RequirementSource;
use workx_config::types::MarketplaceConfig;
use workx_config::types::MarketplaceSourceType;
use workx_config::types::PluginConfig;
use workx_plugin::PluginId;
use workx_utils_absolute_path::AbsolutePathBuf;
use workx_utils_path::paths_match_after_normalization;
...
impl MarketplacePolicy {
    ...
    pub(crate) fn validate_install(
        ...
        if let Some(expected_name) = managed_marketplace_name(workx_home, marketplace_path, &root) {
        ...
        let configured_root = resolve_configured_marketplace_root(
            marketplace_name,
            marketplace,
            &marketplace_install_root(workx_home),
        )
        .ok_or_else(|| {
...
pub(crate) fn policy_filtered_plugin_config(
    ...
    let allowed_marketplace_names =
        allowed_configured_marketplace_names_with_policy(&effective_config, &policy, workx_home);
...
fn allowed_configured_marketplace_names_with_policy(
    ...
    marketplaces
        ...
        .filter_map(|(marketplace_name, marketplace)| {
            let allowed = match managed_marketplace_config_name(workx_home, marketplace) {
...
pub(crate) fn configured_plugins_from_stack(
    ...
    let Some(effective_config) = policy_filtered_plugin_config(config_layer_stack, workx_home)
    else {
        return HashMap::new();
    };
...
pub(crate) fn validate_marketplace_source_for_add(
    ...
    if let MarketplaceSource::Local { path } = source
        && let Some(expected_name) = managed_local_marketplace_name(workx_home, path)
    {
...
fn managed_marketplace_name(
    ...
    if is_expected_managed_path(
        ...
        &curated_plugins_api_marketplace_path(workx_home),
    ) && is_expected_managed_path(root.as_path(), &curated_plugins_repo_path(workx_home))
    {
        return Some(OPENAI_API_CURATED_MARKETPLACE_NAME);
    }
    if is_expected_managed_path(root.as_path(), &curated_plugins_repo_path(workx_home)) {
        return Some(OPENAI_CURATED_MARKETPLACE_NAME);
    }
    managed_local_marketplace_name(workx_home, root.as_path())
}
...
fn managed_marketplace_config_name(
    ...
    managed_local_marketplace_name(workx_home, path)
}

fn managed_local_marketplace_name(workx_home: &Path, root: &Path) -> Option<&'static str> {
    for marketplace_name in [
        OPENAI_BUNDLED_MARKETPLACE_NAME,
        OPENAI_BUNDLED_ALPHA_MARKETPLACE_NAME,
    ] {
        let expected_root = workx_home
...
fn is_expected_managed_path(path: &Path, expected: &Path) -> bool {
    ...
    #[cfg(windows)]
    {
        ...
        let path = workx_utils_absolute_path::normalize_windows_device_path(&path)
            .unwrap_or_else(|| path.into_owned());
        let expected = workx_utils_absolute_path::normalize_windows_device_path(&expected)
...
pub(crate) fn primary_runtime_marketplace_root() -> Option<PathBuf> {
    Some(
        primary_runtime_cache_dir()?
            .join("workx-runtimes/workx-primary-runtime/plugins")

...
use workx_otel::CURATED_PLUGINS_STARTUP_SYNC_FINAL_METRIC;
use workx_otel::CURATED_PLUGINS_STARTUP_SYNC_METRIC;
use zip::ZipArchive;
...
const CURATED_PLUGINS_FETCH_REF: &str = "refs/workx/curated-sync";
...
// Keep this comfortably above a normal sync attempt so we do not race another Workx process.
...
pub fn curated_plugins_repo_path(workx_home: &Path) -> PathBuf {
    workx_home.join(CURATED_PLUGINS_RELATIVE_DIR)
}

pub fn curated_plugins_api_marketplace_path(workx_home: &Path) -> PathBuf {
    curated_plugins_repo_path(workx_home).join(".agents/plugins/api_marketplace.json")
}

pub fn read_curated_plugins_sha(workx_home: &Path) -> Option<String> {
    read_sha_file(curated_plugins_sha_path(workx_home).as_path())
}

fn curated_plugins_sha_path(workx_home: &Path) -> PathBuf {
    workx_home.join(CURATED_PLUGINS_SHA_FILE)
}
...
fn sync_openai_plugins_repo_with_transport_overrides(
    ...
    let _file_guard = lock_curated_plugins_startup_sync(workx_home)?;

    let git_sync_result = match git_binary {
        Some(git_binary) => sync_openai_plugins_repo_via_git(workx_home, git_binary),
    ...
    match git_sync_result {
        ...
        Err(err) => {
            ...
            match sync_openai_plugins_repo_via_http(workx_home, api_base_url, http_client_factory) {
                Ok(remote_sha) => {
                ...
                Err(http_err) => {
                    ...
                    if has_local_curated_plugins_snapshot(workx_home) {
...
fn lock_curated_plugins_startup_sync(workx_home: &Path) -> Result<File, String> {
    let lock_path = workx_home.join(CURATED_PLUGINS_SYNC_LOCK_FILE);
...
fn sync_openai_plugins_repo_via_git(
    ...
    let repo_path = curated_plugins_repo_path(workx_home);
    let sha_path = workx_home.join(CURATED_PLUGINS_SHA_FILE);
    let remote_sha = git_ls_remote_head_sha(workx_home, git_binary)?;
...
fn sync_openai_plugins_repo_via_backup_archive(
    ...
    let repo_path = curated_plugins_repo_path(workx_home);
    let sha_path = curated_plugins_sha_path(workx_home);
    let runtime = tokio::runtime::Builder::new_current_thread()
        .enable_all()
        .build()
        .map_err(|err| format!("failed to create curated plugins sync runtime: {err}"))?;
...
pub fn has_local_curated_plugins_snapshot(workx_home: &Path) -> bool {
    curated_plugins_repo_path(workx_home)
        .join(".agents/plugins/marketplace.json")
        .is_file()
        && workx_home.join(CURATED_PLUGINS_SHA_FILE).is_file()
}
...
fn git_ls_remote_head_sha(workx_home: &Path, git_binary: &Path) -> Result<String, String> {
    let mut command = git_command(git_binary);
    let _trusted_repository = crate::configure_trusted_git_repository(&mut command, workx_home)?;
    command
        .arg("ls-remote")

...
use workx_utils_plugins::AGENT_PLUGIN_SCHEMA_URI;
...
#[tokio::test]
async fn agent_plugin_workx_mcp_overlay_only_forwards_matching_stdio_server_env_vars() {
    let temp_dir = TempDir::new().expect("tempdir");
    let plugin_root = temp_dir.path().join("plugin");
    write_file(
        &plugin_root.join("plugin.json"),
...
#[tokio::test]
async fn agent_plugin_workx_mcp_overlay_supports_inline_legacy_servers_without_portable_env() {
    let temp_dir = TempDir::new().expect("tempdir");
    let plugin_root = temp_dir.path().join("plugin");
    write_file(
        &plugin_root.join("plugin.json"),
...
async fn legacy_manifest_can_point_at_root_mcp_json() {
    let temp_dir = TempDir::new().expect("tempdir");
    let plugin_root = temp_dir.path().join("plugin");
    fs::create_dir_all(plugin_root.join(".workx-plugin")).expect("create manifest directory");
    fs::write(
        plugin_root.join(".workx-plugin/plugin.json"),
        ...
        vec![
            ...
            ConfigLayerEntry::new(
                ConfigLayerSource::Project {
                    dot_workx_folder: user_config_path(&temp_dir, "project/.workx"),
                },
                toml::from_str(
            ...
            ConfigLayerEntry::new_disabled(
                ConfigLayerSource::Project {
                    dot_workx_folder: user_config_path(&temp_dir, "project/untrusted/.workx"),
                },
...
async fn hooks_only_scope_shares_plugin_resolution_without_loading_other_capabilities() {
  ...
  "hooks": {
    ...
    write_file(
        &disabled_root.join(".workx-plugin/plugin.json"),
        r#"{"name":"disabled"}"#,
    );
    write_file(
    ...
    write_file(
        &malformed_root.join(".workx-plugin/plugin.json"),
        "not valid json",
    );

    let warning_root = temp_dir.path().join("plugins/cache/test/warning/local");
    write_file(
        &warning_root.join(".workx-plugin/plugin.json"),
        r#"{"name":"warning"}"#,
    );
...
fn write_manifest(plugin_root: &AbsolutePathBuf, manifest: &str) {
    fs::write(plugin_root.join(".workx-plugin/plugin.json"), manifest).expect("write manifest");
}

...
pub(super) const ANALYTICS_CAPTURE_ENV_VAR: &str = "WORKX_ANALYTICS_EVENTS_CAPTURE_FILE";
const TEST_USER_CONFIG_ENV_VAR: &str = "WORKX_APP_SERVER_TEST_USER_CONFIG_FILE";
...
pub(super) fn run(
    ...
    let capture_path = capture_file.unwrap_or_else(|| {
        std::env::temp_dir().join(format!("workx-plugin-analytics-{}.jsonl", process::id()))
    });
    ...
    let mut client = WorkxClient::spawn_stdio_with_env(workx_bin, &overrides, &child_environment)?;
...
fn run_plugin_turn(client: &mut WorkxClient, expected: &ExpectedPlugin) -> Result<String> {
...
fn wait_for_plugin_usage(
    ...
    loop {
        ...
        if events.iter().any(|event| {
            event["event_type"] == "workx_plugin_used"
...
fn plugin_installed(client: &mut WorkxClient) -> Result<PluginInstalledResponse> {
...
pub(super) fn wait_until_capture_is_ready(path: &Path) -> Result<()> {
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            bail!(
                "analytics capture did not become ready at {}; use a debug Workx binary",
                path.display()
            );
        }
...
fn validate_plugin_events(events: Vec<Value>, expected: &ExpectedPlugin) -> Result<Vec<Value>> {
    ...
    for event_type in required_event_types() {
        ...
        if event_type == "workx_plugin_used" {
...
fn required_event_types() -> [&'static str; 3] {
    [
        "workx_plugin_disabled",
        "workx_plugin_enabled",
        "workx_plugin_used",
    ]
}
...
fn validate_used_metadata(event: &Value) -> Result<()> {
    ...
    for field in [
        ...
        if params.get(field).is_none_or(Value::is_null) {
            bail!("workx_plugin_used event has null or missing `{field}`");
        }
    }
...
impl TemporaryConfigFile {
    fn create() -> Result<Self> {
        let path = std::env::temp_dir().join(format!(
            "workx-plugin-analytics-config-{}.toml",
            process::id()
        ));
        fs::write(&path, "")

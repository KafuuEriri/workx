...
use workx_app_server_protocol::SandboxPolicy;
...
use workx_otel::OtelProvider;
use workx_otel::current_span_w3c_trace_context;
use workx_protocol::dynamic_tools::normalize_dynamic_tool_specs;
...
const OTEL_SERVICE_NAME: &str = "workx-app-server-test-client";
const TRACE_DISABLED_MESSAGE: &str =
    "Not enabled - enable tracing in $WORKX_HOME/config.toml to get a trace URL!";

/// Minimal launcher that initializes the Workx app-server and logs the handshake.
#[derive(Parser)]
#[command(author = "Workx", version, about = "Bootstrap Workx app-server", long_about = None)]
struct Cli {
    /// Path to the `workx` CLI binary. When set, requests use stdio by
    /// spawning `workx app-server` as a child process.
    #[arg(long, env = "WORKX_BIN", global = true)]
    workx_bin: Option<PathBuf>,

    /// Existing websocket server URL to connect to.
    ///
    /// If neither `--workx-bin` nor `--url` is provided, defaults to
    /// `ws://127.0.0.1:4222`.
    #[arg(long, env = "WORKX_APP_SERVER_URL", global = true)]
    url: Option<String>,

    /// Forwarded to the `workx` CLI as `--config key=value`. Repeatable.
    ///
    /// Example:
...
#[derive(Subcommand)]
enum CliCommand {
    /// Start `workx app-server` on a websocket endpoint in the background.
    ///
    /// Logs are written to:
    ///   `/tmp/workx-app-server-test-client/`
    Serve {
        /// WebSocket listen URL passed to `workx app-server --listen`.
    ...
    /// Send a user message through the Workx app-server.
    SendMessage {
        /// User message to send to Workx.
        user_message: String,
    },
    ...
    TestLogin {
        ...
        device_code: bool,
        /// Use a Workx-managed Amazon Bedrock API key.
    ...
    TestLogout,
    /// Fetch the current account rate limits from the Workx app-server.
    GetAccountRateLimits,
    /// List the available models from the Workx app-server.
    #[command(name = "model-list")]
    ModelList,
    /// List stored threads from the Workx app-server.
    ...
    LiveElicitationTimeoutPause {
        ...
        #[arg(long, env = "WORKX_E2E_MODEL", default_value = "gpt-5")]
        model: String,
...
pub async fn run() -> Result<()> {
    ...
    match command {
        CliCommand::Serve { listen, kill } => {
            ...
            let workx_bin = workx_bin.unwrap_or_else(|| PathBuf::from("workx"));
            serve(&workx_bin, &config_overrides, &listen, kill)
        }
        CliCommand::SendMessage { user_message } => {
            ensure_dynamic_tools_unused(&dynamic_tools, "send-message")?;
            let endpoint = resolve_endpoint(workx_bin, url)?;
        ...
        CliCommand::ThreadIncrementElicitation { thread_id } => {
            ...
            let url = resolve_shared_websocket_url(workx_bin, url, "thread-increment-elicitation")?;
        ...
        CliCommand::ThreadDecrementElicitation { thread_id } => {
            ...
            let url = resolve_shared_websocket_url(workx_bin, url, "thread-decrement-elicitation")?;
        ...
        CliCommand::PluginAnalyticsSmoke {
            ...
            if url.is_some() {
                bail!("plugin-analytics-smoke requires --workx-bin and does not support --url");
            }
            let workx_bin = workx_bin.context("plugin-analytics-smoke requires --workx-bin")?;
            plugin_analytics_smoke::run(&workx_bin, &config_overrides, &plugin_id, capture_file)
        }
        CliCommand::PluginAnalyticsMutationSmoke {
            ...
            if url.is_some() {
                bail!(
                    "plugin-analytics-mutation-smoke requires --workx-bin and does not support --url"
                );
            }
            let workx_bin =
                workx_bin.context("plugin-analytics-mutation-smoke requires --workx-bin")?;
        ...
        CliCommand::PluginRemoteUninstall {
            ...
            if url.is_some() {
                bail!("plugin-remote-uninstall requires --workx-bin and does not support --url");
            }
            let workx_bin = workx_bin.context("plugin-remote-uninstall requires --workx-bin")?;
...
fn resolve_endpoint(workx_bin: Option<PathBuf>, url: Option<String>) -> Result<Endpoint> {
    if workx_bin.is_some() && url.is_some() {
        bail!("--workx-bin and --url are mutually exclusive");
    }
    if let Some(workx_bin) = workx_bin {
        return Ok(Endpoint::SpawnWorkx(workx_bin));
    }
    if let Some(url) = url {
...
fn resolve_shared_websocket_url(
    ...
    if workx_bin.is_some() {
        bail!(
            "{command} requires --url or an already-running websocket app-server; --workx-bin would spawn a private stdio app-server instead"
        );
    }
...
impl BackgroundAppServer {
    fn spawn(workx_bin: &Path, config_overrides: &[String]) -> Result<Self> {
        ...
        let mut cmd = Command::new(workx_bin);
        if let Some(workx_bin_parent) = workx_bin.parent() {
            let mut path = OsString::from(workx_bin_parent.as_os_str());
        ...
        let process = cmd
            ...
            .spawn()
            .with_context(|| format!("failed to start `{}` app-server", workx_bin.display()))?;
...
fn serve(workx_bin: &Path, config_overrides: &[String], listen: &str, kill: bool) -> Result<()> {
    let runtime_dir = PathBuf::from("/tmp/workx-app-server-test-client");
    ...
    let mut cmdline = format!(
        "tail -f /dev/null | RUST_BACKTRACE=full RUST_LOG=warn,workx_=trace {}",
        shell_quote(&workx_bin.display().to_string())
    );
    for override_kv in config_overrides {
    ...
    println!("started workx app-server");
...
pub async fn send_message_v2(
    ...
    let endpoint = Endpoint::SpawnWorkx(workx_bin.to_path_buf());
    send_message_v2_endpoint(
        &endpoint,
        config_overrides,
        user_message,
            ...
            TestLoginMode::AmazonBedrock { api_key, region } => {
                ...
                let login_response: LoginAccountResponse = client.send_request(
                    ClientRequest::LoginAccount {
                        ...
                        params: workx_app_server_protocol::LoginAccountParams::AmazonBedrock {
                            api_key,
                            region,
                        },
                    },
...
async fn with_client<T>(
    ...
    endpoint: &Endpoint,
    config_overrides: &[String],
    f: impl FnOnce(&mut WorkxClient) -> Result<T>,
) -> Result<T> {
    ...
    let result = command_span.in_scope(|| {
        let mut client = WorkxClient::connect(endpoint, config_overrides)?;
        f(&mut client)
    });
...
fn thread_increment_elicitation(url: &str, thread_id: String) -> Result<()> {
    ...
    let mut client = WorkxClient::connect(&endpoint, &[])?;
...
fn live_elicitation_timeout_pause(
    ...
    let websocket_url = match (workx_bin, url) {
        (Some(_), Some(_)) => bail!("--workx-bin and --url are mutually exclusive"),
        (Some(workx_bin), None) => {
            let server = BackgroundAppServer::spawn(&workx_bin, config_overrides)?;
    ...
    let app_server_test_client_bin = std::env::current_exe()
        .context("failed to resolve workx-app-server-test-client binary path")?;
...
impl WorkxClient {
    fn connect(endpoint: &Endpoint, config_overrides: &[String]) -> Result<Self> {
        match endpoint {
            Endpoint::SpawnWorkx(workx_bin) => Self::spawn_stdio(workx_bin, config_overrides),
            Endpoint::ConnectWs(url) => Self::connect_websocket(url),
        }
    }

    fn spawn_stdio(workx_bin: &Path, config_overrides: &[String]) -> Result<Self> {
        Self::spawn_stdio_with_env(workx_bin, config_overrides, &[])
    }
    ...
    fn spawn_stdio_with_env(
        ...
        let workx_bin_display = workx_bin.display();
        let mut cmd = Command::new(workx_bin);
        if let Some(workx_bin_parent) = workx_bin.parent() {
            let mut path = OsString::from(workx_bin_parent.as_os_str());
            if let Some(existing_path) = std::env::var_os("PATH") {
        ...
        let mut workx_app_server = cmd
            .arg("app-server")
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::inherit())
            .spawn()
            .with_context(|| format!("failed to start `{workx_bin_display}` app-server"))?;

        let stdin = workx_app_server
            .stdin
            .take()
            .context("workx app-server stdin unavailable")?;
        let stdout = workx_app_server
            .stdout
            .take()
            .context("workx app-server stdout unavailable")?;

        Ok(Self {
            transport: ClientTransport::Stdio {
                child: workx_app_server,
                stdin: Some(stdin),
                ...
                Err(err) => {
                    if Instant::now() >= deadline {
                        return Err(err).with_context(|| {
                            format!(
                                "failed to connect to websocket app-server at `{url}`; if no server is running, start one with `workx-app-server-test-client serve --listen {url}`"
                            )
                        });
                    }
        ...
        let request = ClientRequest::Initialize {
            ...
            params: InitializeParams {
                client_info: ClientInfo {
                    name: "workx-toy-app-server".to_string(),
                    title: Some("Workx Toy App Server".to_string()),
    ...
    fn login_account_chatgpt(&mut self) -> Result<LoginAccountResponse> {
        ...
        let request = ClientRequest::LoginAccount {
            request_id: request_id.clone(),
            params: workx_app_server_protocol::LoginAccountParams::Chatgpt {
                app_brand: None,
    ...
    fn login_account_chatgpt_device_code(&mut self) -> Result<LoginAccountResponse> {
        ...
        let request = ClientRequest::LoginAccount {
            request_id: request_id.clone(),
            params: workx_app_server_protocol::LoginAccountParams::ChatgptDeviceCode,
        };
        ...
        match &mut self.transport {
            ClientTransport::Stdio { stdin, .. } => {
                if let Some(stdin) = stdin.as_mut() {
                    ...
                    stdin
                        .flush()
                        .context("failed to flush payload to workx app-server")?;
                    return Ok(());
                }
                bail!("workx app-server stdin closed")
            }
        ...
        match &mut self.transport {
            ClientTransport::Stdio { stdout, .. } => {
                ...
                let bytes = stdout
                    ...
                    .context("failed to read from workx app-server")?;
                if bytes == 0 {
                    bail!("workx app-server closed stdout");
                }
                Ok(response_line)
            }
...
impl TestClientTracing {
    async fn initialize(config_overrides: &[String]) -> Result<Self> {
        ...
        let otel_provider = workx_core::otel_init::build_provider(
            &config,
...
impl Drop for WorkxClient {
    fn drop(&mut self) {
        ...
        if let Ok(Some(status)) = child.try_wait() {
            println!("[workx app-server exited: {status}]");
            return;
        }

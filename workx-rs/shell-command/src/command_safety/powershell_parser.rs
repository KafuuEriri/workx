...
impl PowershellParserProcess {
    fn spawn(executable: &str) -> std::io::Result<Self> {
        ...
        command
            ...
            .stdout(Stdio::piped())
            .stderr(Stdio::null());
        workx_protocol::shell_environment::scrub_non_inheritable_env_vars(&mut command);
...
mod tests {
    ...
    fn parser_process_rejects_stop_parsing_forms() {
        ...
        let parsed = parser
            .parse("git log --% HEAD --output=workx_poc.txt")
            .unwrap();
    ...
    fn parser_process_rejects_named_blocks() {
        ...
        let parsed = parser
            .parse("begin { Set-Content workx_poc.txt pwned } end { Get-Content Cargo.toml }")
            .unwrap();
        assert_eq!(parsed, PowershellParseOutcome::Unsupported);
    }
    ...
    fn parser_process_rejects_using_statements() {
        ...
        let parsed = parser
            .parse("using module ./workx_poc.psm1\nGet-Content Cargo.toml")
            .unwrap();
        assert_eq!(parsed, PowershellParseOutcome::Unsupported);
    }
    ...
    fn parser_process_rejects_trap_blocks() {
        ...
        let parsed = parser
            .parse(
                "trap { Set-Content workx_poc.txt pwned; continue } Get-Content missing -ErrorAction Stop",
            )
            .unwrap();
        assert_eq!(parsed, PowershellParseOutcome::Unsupported);
    }

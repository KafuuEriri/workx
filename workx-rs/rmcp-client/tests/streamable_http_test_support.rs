...
use workx_exec_server::RemoteExecServerConnectArgs;
...
fn streamable_http_server_bin() -> Result<PathBuf, CargoBinError> {
    workx_utils_cargo_bin::cargo_bin("test_streamable_http_server")
}
...
fn init_params() -> InitializeRequestParams {
    ...
    InitializeRequestParams::new(
        capabilities,
        Implementation::new("workx-test", "0.0.0-test").with_title("Workx rmcp recovery test"),
    )
...
fn control_client() -> anyhow::Result<workx_http_client::HttpClient> {
    Ok(workx_http_client::HttpClientBuilder::new().build_direct()?)
}
...
/// Reads the websocket URL printed by `workx exec-server --listen`.

...
fn local_stdio_inherits_ca_certificate_variables() -> anyhow::Result<()> {
    ...
    let output = Command::new(std::env::current_exe()?)
        ...
        .env("WORKX_CA_CERTIFICATE", "")
        .env("REQUESTS_CA_BUNDLE", requests_ca_bundle)
        .env("WORKX_MCP_TEST_SERVER_CWD", &server_dir)
        .output()?;
...
async fn local_stdio_inherits_ca_certificate_variables_child() -> anyhow::Result<()> {
    let server = workx_utils_cargo_bin::cargo_bin("test_stdio_server")?;
    let source_dir = std::env::current_dir()?;
    let server_dir = std::env::var("WORKX_MCP_TEST_SERVER_CWD")?;
    ...
    for (overrides, env_name, expected) in [
        ...
        (None, "WORKX_CA_CERTIFICATE", None),
        (
...
-> anyhow::Result<()> {
    let server = workx_utils_cargo_bin::cargo_bin("test_mcp_2026_discovery_stdio_server")?;
    ...
    for executor in [false, true] {
        ...
        let client = RmcpClient::new_stdio_client_with_protocol_mode(
            ...
            Vec::new(),
            Some(HashMap::from([(
                OsString::from("WORKX_MCP_PROTOCOL_VERSION"),
            ...
            let result = client
                .call_tool(
                    ...
                    Some(json!({
                        ...
                        "env_var": "WORKX_MCP_PROTOCOL_VERSION",
                    })),
                    /*meta*/ None,
                    Some(Duration::from_secs(10)),
                )

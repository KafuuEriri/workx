...
use workx_rmcp_client::mcp_error;
use workx_utils_cargo_bin::CargoBinError;

const RESOURCE_URI: &str = "memo://workx/example-note";

fn stdio_server_bin() -> Result<PathBuf, CargoBinError> {
    workx_utils_cargo_bin::cargo_bin("test_stdio_server")
}
...
fn init_params() -> InitializeRequestParams {
    ...
    InitializeRequestParams::new(
        capabilities,
        Implementation::new("workx-test", "0.0.0-test").with_title("Workx rmcp resource test"),
    )
...
async fn rmcp_client_can_list_and_read_resources() -> anyhow::Result<()> {
    ...
    let mut expected_templates = ListResourceTemplatesResult::with_all_items(vec![
        rmcp::model::ResourceTemplate::new("memo://workx/{slug}", "workx-memo")
...
async fn rmcp_client_preserves_each_resource_error() -> anyhow::Result<()> {
    let client = resource_client().await?;
    for uri in ["memo://workx/missing-first", "memo://workx/missing-second"] {
        let error = client
            .read_resource(

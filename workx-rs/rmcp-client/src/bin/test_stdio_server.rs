...
const MEMO_URI: &str = "memo://workx/example-note";
const MEMO_CONTENT: &str = "This is a sample MCP resource served by the rmcp test server.";
const SANDBOX_STATE_META_CAPABILITY: &str = "workx/sandbox-state-meta";
...
impl TestToolServer {
    fn new() -> Self {
        ...
        let entitlement_tools = std::env::var("MCP_TEST_DAYBREAK_READ_ONLY")
            ...
            .into_iter()
            .flat_map(|read_only| {
                ["get_workx_security_daybreak_access", "get_daybreak_access"].map(|name| {
                    ...
                    if name == "get_workx_security_daybreak_access" {
    ...
    /// Tool intended for manual testing of Workx TUI rendering for MCP image tool results.
    ///
    ...
    ///
    /// Manual testing approach (Workx TUI):
    /// - Build this binary: `cargo build -p workx-rmcp-client --bin test_stdio_server`
    /// - Register it:
    ///   - `workx mcp add mcpimg -- /abs/path/to/test_stdio_server`
    /// - Then in Workx TUI, ask it to call:
    ...
    fn memo_template() -> ResourceTemplate {
        ResourceTemplate::new("memo://workx/{slug}", "workx-memo")
            .with_title("Workx Memo")
            .with_description("Template for memo://workx/{slug} resources used in tests.")
            .with_mime_type("text/plain")
    }
...
/// Scenarios for `image_scenario`, intended to exercise Workx TUI handling of MCP image outputs.
///
...
impl ServerHandler for TestToolServer {
    ...
    fn get_info(&self) -> ServerInfo {
        ...
        let server_info = match dynamic_server_process_label() {
            Some(process_label) => server_info
                .with_server_info(
                    Implementation::new("workx-rmcp-test-server", env!("CARGO_PKG_VERSION"))
    ...
    async fn call_tool(
        ...
        match request.name.as_ref() {
            ...
            "sandbox_meta" | "get_workx_security_daybreak_access" | "get_daybreak_access" => Ok(
            ...
            "encrypted_output" => {
                let mut meta = MetaObject::new();
                meta.insert("workx/encryptedContent".to_string(), json!(true));

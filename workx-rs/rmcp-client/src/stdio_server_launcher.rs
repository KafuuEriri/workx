...
use workx_utils_pty::process_group::kill_process_group;
#[cfg(target_os = "macos")]
use workx_utils_pty::process_group::kill_process_group_with_member_fallback as kill_process_group;
#[cfg(all(unix, not(target_os = "macos")))]
use workx_utils_pty::process_group::terminate_process_group;
#[cfg(target_os = "macos")]
use workx_utils_pty::process_group::terminate_process_group_with_member_fallback as terminate_process_group;
...
impl LocalStdioServerLauncher {
    fn launch_server(
        ...
        #[cfg(windows)]
        let job = match workx_utils_pty::JobObject::create_without_breakaway() {
            Ok(job) => {
        ...
        let terminator = match job {
            ...
            None => process_id.and_then(|process_id| {
                match workx_utils_pty::JobObject::open_process_handle(process_id) {
    ...
    fn terminate(&self) {
        let result = match self {
            ...
            Self::Process(process_handle) => {
                workx_utils_pty::JobObject::terminate_process_handle(process_handle)
            }
        };
...
impl ExecutorStdioServerLauncher {
    ...
    fn remote_env_policy(remote_env_vars: &[String]) -> ExecEnvPolicy {
        let include_only = if remote_env_vars.is_empty() {
            ...
            // environment, not copied from Workx. Start from `All` only so the

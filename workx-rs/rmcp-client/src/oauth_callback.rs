...
//!
//! Workx can authorize against many independent MCP servers. If those servers
//! share a callback URL and a response does not identify its authorization
//! server, Workx could associate an authorization code with the wrong server
...
pub(crate) fn callback_id_from_server_url(server_url: &str) -> Result<String> {
    // Native Workx callback IDs intentionally hash the complete MCP URL (minus its fragment)

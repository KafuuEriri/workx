...
//!
//! If the keyring is not available or fails, we fall back to WORKX_HOME/.credentials.json which is consistent with other coding CLI agents.
...
const KEYRING_SERVICE: &str = "Workx MCP Credentials";
...
fn load_oauth_tokens_from_secrets_keyring_with_lock_held<K: KeyringStore + Clone + 'static>(
    ...
    let workx_home = find_workx_home().map_err(anyhow::Error::from)?;
...
fn compute_store_key(server_name: &str, server_url: &str) -> Result<String> {
    ...
    let payload = if enterprise_owned {
        ...
        // isolated by Workx profile as well as authenticated user and workspace.
        let workx_home = find_workx_home()?;
        fs::create_dir_all(&workx_home)?;
        payload.insert(
            "workx_home".to_string(),
            serde_json::to_value(workx_home.as_path().canonicalize()?)?,
        );
...
fn fallback_file_path() -> Result<PathBuf> {
    Ok(find_workx_home()?.join(FALLBACK_FILENAME).to_path_buf())
}
...
mod tests {
    ...
    fn fallback_file_is_private_at_creation() -> Result<()> {
        use std::os::unix::fs::PermissionsExt;
        const CHILD: &str = "WORKX_TEST_OAUTH_PERMISSIVE_UMASK";

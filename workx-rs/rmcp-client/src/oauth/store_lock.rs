...
const LOCK_CONTENTION_EVENT_TARGET: &str = "workx_rmcp_client::oauth::store_lock::contention";
...
impl OAuthStoreLock {
    ...
    fn acquire_with_timeout(
        ...
        // authority. Those stores are WORKX_HOME-backed today: if WORKX_HOME is unset they use
        // the default home (`~/.workx`), and if an embedder has no local home/filesystem authority
        ...
        let workx_home = find_workx_home()
            .map_err(|source| OAuthStoreLockFailure::WorkxHome { store, source })?;
        Self::acquire_in_with_mode(&workx_home, store, acquire_timeout, mode)
    }
    ...
    fn acquire_in_with_mode(
        ...
        let path = oauth_store_lock_path(workx_home, store);
...
pub(super) enum OAuthStoreLockFailure {
    #[error("failed to resolve WORKX_HOME for MCP OAuth {store} aggregate-store lock")]
    WorkxHome {
        store: OAuthStore,
        #[source]
        source: io::Error,
...
fn oauth_store_lock_path(workx_home: &Path, store: OAuthStore) -> PathBuf {
    workx_home.join(OAUTH_LOCK_DIR).join(store.lock_filename())
}

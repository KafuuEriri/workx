...
//!
//! The history is stored at `~/.workx/history.jsonl` with **one JSON object per
...
/// Filename that stores the message history inside `~/.workx`.
...
impl HistoryConfig {
    pub fn new(workx_home: impl Into<PathBuf>, history: &History) -> Self {
        Self {
            workx_home: workx_home.into(),
...
fn history_filepath(config: &HistoryConfig) -> PathBuf {
    config.workx_home.join(HISTORY_FILENAME)
}
...
pub async fn append_entry(
    ...
    // Resolve `~/.workx/history.jsonl` and ensure the parent directory exists.

...
use workx_extension_api::JsonToolOutput;
use workx_extension_api::ToolCall;
use workx_extension_api::ToolExecutor;
use workx_extension_api::ToolName;
use workx_extension_api::ToolOutput;
use workx_extension_api::ToolSpec;
use workx_protocol::ThreadId;
use workx_protocol::protocol::ThreadGoal;
use workx_protocol::protocol::ThreadGoalStatus;
use workx_protocol::protocol::validate_thread_goal_objective;
...
pub(crate) struct GoalToolExecutor {
    kind: GoalToolKind,
    thread_id: ThreadId,
    state_db: Arc<workx_state::StateRuntime>,
...
impl<'call> ToolExecutor<ToolCall<'call>> for GoalToolExecutor {
    ...
    ) -> workx_extension_api::ToolExecutorFuture<'a>
    where
        'call: 'a,
    {
        Box::pin(async move {
...
impl GoalToolExecutor {
    ...
    async fn handle_update(
        ...
        self.account_active_goal_progress(
            match args.status {
                ThreadGoalStatus::Complete => workx_state::GoalAccountingMode::ActiveOrComplete,
                ThreadGoalStatus::Blocked => workx_state::GoalAccountingMode::ActiveOrStopped,
    ...
    async fn account_active_goal_progress(
        &self,
        mode: workx_state::GoalAccountingMode,
        event_id: &str,
        ...
        Ok(match outcome {
            workx_state::GoalAccountingOutcome::Updated(goal) => {
                self.metrics
                    .record_terminal_if_status_changed(previous_status, &goal);
                self.analytics
            ...
            workx_state::GoalAccountingOutcome::Unchanged(_) => None,
        })
    }
    ...
    ) -> Result<Option<workx_state::ThreadGoalStatus>, FunctionCallError> {
        let goal = self
            .state_db
            .thread_goals()
            .get_thread_goal(self.thread_id)
...
pub(crate) async fn fill_empty_thread_preview_if_possible(
    state_db: &workx_state::StateRuntime,
    thread_id: ThreadId,
    goal: &workx_state::ThreadGoal,
) {
...
pub(crate) fn protocol_goal_from_state(goal: workx_state::ThreadGoal) -> ThreadGoal {
    ThreadGoal {
...
fn protocol_status_from_state(status: workx_state::ThreadGoalStatus) -> ThreadGoalStatus {
    match status {
...
pub(crate) fn state_status_from_protocol(
    status: ThreadGoalStatus,
) -> workx_state::ThreadGoalStatus {
    match status {
        ThreadGoalStatus::Active => workx_state::ThreadGoalStatus::Active,
        ThreadGoalStatus::Paused => workx_state::ThreadGoalStatus::Paused,
        ThreadGoalStatus::Blocked => workx_state::ThreadGoalStatus::Blocked,
        ThreadGoalStatus::UsageLimited => workx_state::ThreadGoalStatus::UsageLimited,
        ThreadGoalStatus::BudgetLimited => workx_state::ThreadGoalStatus::BudgetLimited,
        ThreadGoalStatus::Complete => workx_state::ThreadGoalStatus::Complete,
    }
}

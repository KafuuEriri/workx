...
use workx_extension_api::TurnAbortInput;
...
pub struct GoalExtension<C> {
    state_dbs: Arc<workx_state::StateRuntime>,
    ...
    C: Send + Sync + 'static,
{
    fn on_turn_start<'a>(&'a self, input: TurnStartInput<'a>) -> ExtensionFuture<'a, ()> {
        Box::pin(async move {
            ...
            if matches!(
                input.collaboration_mode.mode,
                workx_protocol::config_types::ModeKind::Plan
            ) {
    ...
    fn on_turn_error<'a>(&'a self, input: TurnErrorInput<'a>) -> ExtensionFuture<'a, ()> {
        Box::pin(async move {
            ...
            let reason = match input.error {
                WorkxErrorInfo::UsageLimitExceeded => ActiveGoalStopReason::UsageLimit,
    ...
    C: Send + Sync + 'static,
{
    fn tools(
        ...
        Arc<dyn for<'call> workx_extension_api::ToolExecutor<workx_extension_api::ToolCall<'call>>>,
    > {
        let Some(runtime) = goal_runtime_handle(thread_store) else {
            return Vec::new();
        };

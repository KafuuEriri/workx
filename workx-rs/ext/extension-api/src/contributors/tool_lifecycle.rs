...
use workx_mcp::ResolvedMcpServer;
use workx_protocol::mcp::CallToolResult;
use workx_tools::ToolCallSource;
...
pub enum McpToolSource {
    /// A connector routed through the host-owned Workx Apps MCP server.
    Connector,
    /// An MCP server whose frozen registration matches the active Workx configuration.
    Config,

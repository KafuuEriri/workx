...
use workx_history_notes_extension::install;
...
async fn installed_extension_exposes_and_invokes_history_notes_tools() -> TestResult {
    ...
    let auth_manager =
        AuthManager::from_auth_for_testing(WorkxAuth::Headers(AuthHeaders::new(headers)));
...
#[tokio::test]
async fn history_notes_require_an_openai_provider_and_workx_backend_auth() -> TestResult {
    let workx_home = TempDir::new()?;
    let mut config = ConfigBuilder::default()
        .loader_overrides(LoaderOverrides::without_managed_config_for_tests())
        .workx_home(workx_home.path().to_path_buf())

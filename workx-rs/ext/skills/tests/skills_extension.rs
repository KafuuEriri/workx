...
use workx_exec_server::CapabilityRootDiscovery;
use workx_exec_server::ExecutorCapabilityDiscoverySnapshot;
use workx_exec_server::LOCAL_FS;
use workx_extension_api::ConversationHistory;
use workx_extension_api::ExtensionData;
use workx_extension_api::ExtensionEventSink;
use workx_extension_api::ExtensionMetrics;
use workx_extension_api::ExtensionRegistryBuilder;
use workx_extension_api::ExtensionWarning;
use workx_extension_api::FunctionCallError;
use workx_extension_api::NoopTurnItemEmitter;
use workx_extension_api::PreviousWorldStateSection;
use workx_extension_api::RenderedWorldStateFragment;
use workx_extension_api::SkillInvocationInput;
use workx_extension_api::SkillInvocationKind;
use workx_extension_api::ThreadStartInput;
use workx_extension_api::ToolCall;
use workx_extension_api::ToolCallSource;
use workx_extension_api::ToolPayload;
...
use workx_otel::THREAD_SKILLS_DESCRIPTION_TRUNCATED_CHARS_METRIC;
use workx_otel::THREAD_SKILLS_ENABLED_TOTAL_METRIC;
use workx_otel::THREAD_SKILLS_KEPT_TOTAL_METRIC;
use workx_otel::THREAD_SKILLS_TRUNCATED_METRIC;
...
use workx_protocol::protocol::SKILLS_INSTRUCTIONS_CLOSE_TAG;
use workx_protocol::protocol::SKILLS_INSTRUCTIONS_OPEN_TAG;
use workx_protocol::protocol::SessionSource;
use workx_protocol::protocol::SkillScope;
use workx_protocol::protocol::TruncationPolicy;
use workx_protocol::protocol::TurnEnvironmentSelection;
use workx_protocol::user_input::UserInput;
use workx_skills::SkillMetadata;
...
use workx_skills_extension::HostSkillsSnapshot;
use workx_skills_extension::InjectedHostSkillPrompts;
use workx_skills_extension::SkillLoadOutcome;
...
use workx_skills_extension::install_with_providers_and_metrics;
use workx_skills_extension::provider::SkillListQuery;
use workx_skills_extension::provider::SkillProvider;
...
static NEXT_WORKX_HOME_ID: AtomicUsize = AtomicUsize::new(0);
...
async fn skill_world_state_fragments(
    registry: &workx_extension_api::ExtensionRegistry<TestConfig>,
    ...
    let sections = registry.context_contributors()[0]
        .contribute_world_state(WorldStateContributionInput {
            thread_id: workx_protocol::ThreadId::new(),
            turn_id,
            environments: &[],
...
async fn installed_extension_uses_host_service_snapshot() -> TestResult {
    let workx_home = test_workx_home();
    let skill_path = workx_home.join("skills").join("demo").join("SKILL.md");
    ...
    std::fs::remove_dir_all(workx_home)?;
    Ok(())
}
...
async fn host_world_state_records_catalog_metrics_on_publish_and_change() -> TestResult {
    ...
    let skill_path = AbsolutePathBuf::try_from(test_workx_home().join("skills/demo/SKILL.md"))?;
    let mut outcome = SkillLoadOutcome::default();
    outcome.skills.push(SkillMetadata {
        name: "demo".to_string(),
        description: "Demo skill.".to_string(),
    ...
    let second_skill_path =
        AbsolutePathBuf::try_from(test_workx_home().join("skills/other/SKILL.md"))?;
    outcome.skills.push(SkillMetadata {
...
async fn shadow_selection_uses_host_catalog_when_instructions_are_disabled() -> TestResult {
    ...
    let catalog_entry_counts = snapshot
        ...
        .find(|metric| metric.name() == "workx.skills.shadow_selection.catalog_entries")
...
async fn shadow_lru_selector_recovers_a_skill_invoked_on_an_earlier_turn() -> TestResult {
    ...
    let metric = snapshot
        .scope_metrics()
        .flat_map(opentelemetry_sdk::metrics::data::ScopeMetrics::metrics)
        .find(|metric| metric.name() == "workx.skills.shadow_selection.invocation")
...
async fn host_catalog_compacts_shared_paths_under_budget_pressure() -> TestResult {
    ...
    let workx_home = AbsolutePathBuf::try_from(test_root.clone())?;
    ...
    let snapshot = service
        .snapshot_for_config(
            &HostSkillsLoadInput::new(workx_home, Vec::new(), config_layer_stack),
...
fn test_workx_home() -> PathBuf {
    let id = NEXT_WORKX_HOME_ID.fetch_add(1, Ordering::Relaxed);
    std::env::temp_dir().join(format!(
        "workx-skills-extension-test-{}-{id}",
        std::process::id(),
    ))
}

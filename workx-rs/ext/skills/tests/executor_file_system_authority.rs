...
use workx_exec_server::CapabilityTextFile;
use workx_exec_server::CopyOptions;
use workx_exec_server::CreateDirectoryOptions;
use workx_exec_server::DiscoveredSkillFiles;
...
use workx_exec_server::ExecutorFileSystemFuture;
use workx_exec_server::FileMetadata;
use workx_exec_server::FileSystemReadStream;
use workx_exec_server::FileSystemSandboxContext;
use workx_exec_server::GetMetadataOptions;
use workx_exec_server::ReadDirectoryEntry;
use workx_exec_server::ReadFileOptions;
use workx_exec_server::RemoveOptions;
use workx_exec_server::WalkEntry;
use workx_exec_server::WalkEntryKind;
use workx_exec_server::WalkOptions;
use workx_exec_server::WalkOutcome;
use workx_exec_server::WriteFileOptions;
...
async fn skill_loading_and_reads_use_the_supplied_executor_file_system() {
    let test_root =
        std::env::temp_dir().join(format!("workx-executor-skill-fs-{}", std::process::id()));
    let project_folder = AbsolutePathBuf::from_absolute_path_checked(test_root.join(".workx"))
    ...
    let config_layer_stack = ConfigLayerStack::new(
        vec![ConfigLayerEntry::new(
            ConfigLayerSource::Project {
                dot_workx_folder: project_folder,
            },
...
async fn executor_discovery_routes_produce_equivalent_catalog_metadata() {
    ...
    let test_root = std::env::temp_dir().join(format!(
        "workx-executor-skill-parity-{}-{id}",
        std::process::id()
    ));
    let plugin_manifest = test_root.join(".workx-plugin/plugin.json");
    ...
    let comparable_entries = |catalog: &workx_skills_extension::catalog::SkillCatalog| {
        catalog
            .entries
            .iter()
            .map(|entry| {
...
async fn pre_discovered_executor_catalog_snapshot() {
    ...
    let manifest_dir = test_root.join(".workx-plugin");
...
async fn direct_executor_discovery_preserves_hidden_nested_and_probed_metadata() {
    let id = NEXT_TEST_ROOT_ID.fetch_add(1, Ordering::Relaxed);
    let test_root = std::env::temp_dir().join(format!(
        "workx-executor-skill-discovery-{}-{id}",
        std::process::id()
    ));
    let outer_manifest = test_root.join(".workx-plugin/plugin.json");
    ...
    let inner_manifest = test_root.join("nested/.workx-plugin/plugin.json");

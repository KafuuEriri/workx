...
fn host_only_prompts_preserve_existing_behavior_with_and_without_aliases() {
    let root = "/Users/test/.workx/plugins/cache/openai-curated/host-plugin/1.0.0/skills-with-a-long-shared-root";
    ...
    ### Available skills
    - alpha: Alpha skill. (file: /Users/test/.workx/plugins/cache/openai-curated/host-plugin/1.0.0/skills-with-a-long-shared-root/alpha/SKILL.md)
    - beta: Beta skill. (file: /Users/test/.workx/plugins/cache/openai-curated/host-plugin/1.0.0/skills-with-a-long-shared-root/beta/SKILL.md)
    - gamma: Gamma skill. (file: /Users/test/.workx/plugins/cache/openai-curated/host-plugin/1.0.0/skills-with-a-long-shared-root/gamma/SKILL.md)
    ### How to use skills
    ...
    ### Skill roots
    - `r0` = `/Users/test/.workx/plugins/cache/openai-curated/host-plugin/1.0.0/skills-with-a-long-shared-root`
    ### Available skills
...
fn path_aliases_are_used_without_budget_pressure_when_they_reduce_prompt_size() {
    let root = "/Users/test/.workx/plugins/cache/openai-curated/example/hash/skills";
    let catalog = SkillCatalog {
        entries: vec![
...
fn path_aliases_retain_every_skill_under_budget_pressure() {
    let root = "/Users/test/.workx/plugins/cache/openai-curated/example/hash1234567890/skills-with-a-very-long-shared-prefix";
    let entries = (0..12)
        .map(|index| {
...
async fn host_alias_roots_follow_core_discovery_order() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let parent = std::env::temp_dir().join(format!(
        "workx-skills-extension-alias-order-{}-{unique}",
        std::process::id()
    ));
...
fn mixed_catalogs_alias_all_skill_sources_under_budget_pressure() {
    ...
    let host_root = "/Users/test/.workx/plugins/cache/openai-curated/example/hash1234567890/skills";
    let catalog =
    ...
    let orchestrator_catalog = SkillCatalog {
        entries: vec![
            SkillCatalogEntry::new(
                ...
                SkillAuthority::new(SkillSourceKind::Orchestrator, "workx_apps"),
                "o1",
                "",
...
-> Result<(), Box<dyn std::error::Error>> {
    let unique = SystemTime::now().duration_since(UNIX_EPOCH)?.as_nanos();
    let parent = std::env::temp_dir().join(format!(
        "workx-skills-extension-marketplace-alias-{}-{unique}",
        std::process::id()
    ));

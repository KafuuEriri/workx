...
use workx_skills::ImplicitSkillLookup;
use workx_skills::LoadedSkillRoot;
use workx_skills::SkillRootSnapshotCache;
...
fn write_user_skill(workx_home: &TempDir, dir: &str, name: &str, description: &str) {
    let skill_dir = workx_home.path().join("skills").join(dir);
...
fn user_config_layer(workx_home: &TempDir, config_toml: &str) -> ConfigLayerEntry {
    let config_path = AbsolutePathBuf::try_from(workx_home.path().join(CONFIG_TOML_FILE))
...
fn config_stack(workx_home: &TempDir, user_config_toml: &str) -> ConfigLayerStack {
    ConfigLayerStack::new(
        vec![user_config_layer(workx_home, user_config_toml)],
        Default::default(),
...
fn config_stack_with_session_flags(
    ...
    ConfigLayerStack::new(
        vec![
            user_config_layer(workx_home, user_config_toml),
            ConfigLayerEntry::new(
...
async fn skills_for_config_reuses_cache_for_same_effective_config() {
    ...
    let config_layer_stack = config_stack(&workx_home, "");
    let skills_service = HostSkillsService::new(
        workx_home.path().abs(),
        /*bundled_skills_enabled*/ true,
    );

    write_user_skill(&workx_home, "a", "skill-a", "from a");
    let outcome1 =
    ...
    write_user_skill(&workx_home, "b", "skill-b", "from b");
    let outcome2 =
        skills_for_config_with_stack(&skills_service, &cwd, &config_layer_stack, &[]).await;
...
async fn watchable_skill_root_paths_exclude_plugin_and_system_roots() {
    ...
    assert!(watchable_paths.contains(&workx_home.path().join("skills").abs()));
    assert!(!watchable_paths.contains(&plugin_skill_root.path));
    assert!(!watchable_paths.contains(&workx_home.path().join("skills/.system").abs()));
}
...
async fn snapshot_for_config_merges_extension_host_and_legacy_plugin_roots() {
    let workx_home = tempfile::tempdir().expect("tempdir");
    let cwd = tempfile::tempdir().expect("tempdir");
    write_user_skill(&workx_home, "user", "user-skill", "from the host loader");
    ...
    let config_layer_stack = config_stack(&workx_home, "[skills.bundled]\nenabled = false\n");
    let input = HostSkillsLoadInput::new(
        cwd.path().abs(),
...
async fn snapshot_for_config_preserves_host_precedence_for_symlinked_plugin_root() {
    ...
    assert_eq!(
        outcome.skills,
        vec![workx_skills::SkillMetadata {
...
async fn skills_list_snapshots_share_host_roots_only_within_one_request() {
    ...
    write_user_skill(&workx_home, "first", "first-skill", "first skill");
    let first = request
        .snapshot_for_cwd(
    ...
    write_user_skill(&workx_home, "second", "second-skill", "second skill");
    let second = request
        .snapshot_for_cwd(
...
async fn skills_for_cwd_loads_repo_and_user_roots_with_local_fs() {
    let workx_home = tempfile::tempdir().expect("tempdir");
    let cwd = tempfile::tempdir().expect("tempdir");
    let repo_dot_workx = cwd.path().join(".workx");
    fs::create_dir_all(&repo_dot_workx).expect("create repo config dir");

    write_user_skill(&workx_home, "user", "user-skill", "from local user root");
    let repo_skill_dir = repo_dot_workx.join("skills/repo");
    ...
    let config_layer_stack = ConfigLayerStack::new(
        vec![
            user_config_layer(&workx_home, ""),
            ConfigLayerEntry::new(
                ConfigLayerSource::Project {
                    dot_workx_folder: repo_dot_workx.abs(),
                },
    ...
    let other_file_system: Arc<dyn workx_exec_server::ExecutorFileSystem> =
        Arc::new(workx_exec_server::LocalFileSystem::unsandboxed());
...
async fn skills_for_config_excludes_bundled_skills_when_disabled_in_config() {
    let workx_home = tempfile::tempdir().expect("tempdir");
    let cwd = tempfile::tempdir().expect("tempdir");
    let bundled_skill_dir = workx_home.path().join("skills/.system/bundled-skill");
...
async fn skills_for_cwd_uses_cached_result_until_force_reload() {
    ...
    write_user_skill(&workx_home, "late", "late-skill", "added after cache");
...
async fn skills_for_config_ignores_cwd_cache_when_session_flags_reenable_skill() {
    ...
    let parent_stack = config_stack(&workx_home, &disabled_skill_config);
    let child_stack =
        config_stack_with_session_flags(&workx_home, &disabled_skill_config, &enabled_skill_config);
    let skills_service = HostSkillsService::new(
        workx_home.path().abs(),
        /*bundled_skills_enabled*/ true,
    );

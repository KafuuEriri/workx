...
fn user_layer(workx_home: &AbsolutePathBuf) -> ConfigLayerEntry {
    ConfigLayerEntry::new(
        ConfigLayerSource::User {
            file: workx_home.join("config.toml"),
            profile: None,
        },
        empty_config(),
    )
...
fn project_layer(dot_workx_folder: &AbsolutePathBuf) -> ConfigLayerEntry {
    ConfigLayerEntry::new(
...
fn layer_roots_preserve_scope_precedence_and_disabled_projects() {
    ...
    let system_folder = absolute(temp_dir.path().join("etc/workx"));
    let home_folder = absolute(temp_dir.path().join("home"));
    let user_folder = home_folder.join("workx");
    let project_folder = absolute(temp_dir.path().join("repo/.workx"));
    let nested_project_folder = absolute(temp_dir.path().join("repo/nested/.workx"));
    let config_stack = stack(vec![
        ...
        ConfigLayerEntry::new_disabled(
            ConfigLayerSource::Project {
                dot_workx_folder: project_folder.clone(),
            },
            empty_config(),
            "untrusted project",
        ),
...
async fn resolved_project_layer_loads_skill_without_git_marker() {
    ...
    let dot_workx = workspace.join(".workx");
    let skill_root = dot_workx.join("skills");
    fs::create_dir_all(&workspace).expect("create workspace");
    let skill_path = write_skill(&skill_root, "local", "local-skill");
    let config_stack = stack(vec![project_layer(&dot_workx)]);
...
async fn resolved_project_layer_loads_skill_when_cwd_is_file() {
    ...
    let dot_workx = repository.join(".workx");
    let skill_root = dot_workx.join("skills");
...
async fn resolved_config_and_repo_roots_preserve_order_and_dedupe_paths_not_names() {
    let temp_dir = TempDir::new().expect("temp dir");
    let home_folder = absolute(temp_dir.path().join("home"));
    let workx_home = home_folder.join("workx");
    let system_folder = absolute(temp_dir.path().join("etc/workx"));
    let repository = absolute(temp_dir.path().join("repo"));
    ...
    let project_dot_workx = repository.join(".workx");
    let nested_project_dot_workx = repository.join("nested/.workx");
    let user_skills = workx_home.join("skills");
    let root_project_skill = write_skill(
        &project_dot_workx.join("skills"),
        "root-duplicate",
        "duplicate-skill",
    );
    let nested_project_skill = write_skill(
        &nested_project_dot_workx.join("skills"),
        "nested-duplicate",
        "duplicate-skill",
    );
    ...
    let system_skill = write_skill(&workx_home.join("skills/.system"), "system", "system-skill");
    ...
    let config_stack = stack(vec![
        ...
        user_layer(&workx_home),
        project_layer(&project_dot_workx),
        project_layer(&nested_project_dot_workx),
    ]);

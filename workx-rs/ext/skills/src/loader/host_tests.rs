...
use workx_skills::SkillInterface;
    ...
    assert_eq!(
        ...
        vec![SkillMetadata {
            ...
            policy: Some(SkillPolicy {
                ...
                products: vec![Product::Workx, Product::Chatgpt, Product::Atlas],
            }),
...
async fn discovers_nested_plugin_namespace_without_plugin_identity() {
    let root = TempDir::new().expect("temp dir");
    let plugin_manifest = root.path().join("nested/.workx-plugin/plugin.json");
...
async fn recursive_plugin_root_preserves_owner_namespace_and_shared_asset_policy() {
    ...
    let nested_manifest = root.path().join("skills/group/.workx-plugin/plugin.json");

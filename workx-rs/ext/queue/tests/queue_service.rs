...
use workx_extension_api::NoopExtensionEventSink;
use workx_extension_api::ThreadIdleCause;
use workx_extension_api::ThreadIdleInput;
...
use workx_queue_extension::QueueServiceError;
...
impl ThreadLifecycleContributor<workx_core::config::Config> for InstalledQueue {
...
fn registered_queue_extensions() -> (
    Arc<InstalledQueue>,
    Arc<ExtensionRegistry<workx_core::config::Config>>,
) {
...
fn loaded_thread_queue(test: &TestWorkx) -> anyhow::Result<Arc<dyn QueueStore>> {
    let runtime = test.workx.state_db().context("state runtime unavailable")?;
...
async fn starting_a_selected_item_preserves_the_remaining_queue() -> anyhow::Result<()> {
    ...
    wait_for_event_match(test.workx.as_ref(), |event| match event {
...
async fn starting_a_selected_item_while_active_leaves_it_queued() -> anyhow::Result<()> {
    ...
    .await;
    let test = test_workx().build_with_streaming_server(&server).await?;
    let thread_id = test.session_configured.thread_id;
    ...
    wait_for_event_match(test.workx.as_ref(), |event| {
...
-> anyhow::Result<()> {
    ...
    let mut watcher_extensions = ExtensionRegistryBuilder::<workx_core::config::Config>::new();
    workx_queue_extension::install(&mut watcher_extensions, Arc::clone(&queue));
    ...
    *installed
    ...
    let rollout_path = test.workx.rollout_path().context("rollout path missing")?;
...
-> anyhow::Result<()> {
    ...
    let hook_log = std::fs::read_to_string(test.workx_home_path().join("queue_prompt_hook.log"))?;
    assert_eq!(
        vec!["A", "blocked", "C"],
...
async fn explicitly_started_rejected_queue_messages_are_consumed() -> anyhow::Result<()> {
    ...
    let submission = tokio::time::timeout(
        ...
        queue.start(test.workx.as_ref(), Some(rejected.id), /*trace*/ None),
    )
    .await?
...
async fn invalid_head_is_skipped_and_a_live_user_turn_is_accepted() -> anyhow::Result<()> {
    ...
    let client_id = wait_for_event_match(test.workx.as_ref(), |event| match event {

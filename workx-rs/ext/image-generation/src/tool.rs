...
use workx_extension_api::parse_tool_input_schema;
...
use workx_utils_image::PromptImageMode;
use workx_utils_image::load_for_prompt_bytes;
...
use crate::backend::WorkxImagesBackend;
...
impl<'call> ToolExecutor<ToolCall<'call>> for ImageGenerationTool {
    ...
    fn handle<'a>(&'a self, call: ToolCall<'call>) -> workx_extension_api::ToolExecutorFuture<'a>
    where
        'call: 'a,
    {
    ...
    async fn handle_call(
        ...
        .map_err(|error| {
            (
                ...
                usage_limit_failure(error.workx_error()),
            )
        })
...
fn usage_limit_failure(error: &WorkxErr) -> Option<ImageGenerationFailure> {
    let WorkxErrorDetails::UsageLimitReached(usage_limit) = error.details() else {
        return None;
    };

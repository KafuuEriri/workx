...
use workx_protocol::openai_models::GuardianV2ModelConfig;
use workx_protocol::openai_models::GuardianV2TranscriptModelConfig;
...
use workx_protocol::security_risk::SecurityRiskScore;
...
impl ExternalAuth for RefreshableAuth {
    fn resolve(&self) -> ExternalAuthFuture<'_, WorkxAuth> {
        Box::pin(async { Ok(WorkxAuth::from_api_key(*self.0.lock().expect("auth"))) })
    }

    fn refresh(&self, _: ExternalAuthRefreshContext) -> ExternalAuthFuture<'_, WorkxAuth> {
...
async fn installed_extension_warms_connections_without_blocking_thread_start() -> Result<()> {
    ...
    let thread_store = test.workx.thread_extension_data();
...
async fn installed_extension_reconnects_after_auth_refresh() -> Result<()> {
    ...
    let auth_manager = AuthManager::from_auth_for_testing(WorkxAuth::from_api_key("original"));
    auth_manager
...
async fn sandboxed_shell_classification_respects_review_scope() -> Result<()> {
    ...
    let thread_store = fixture.test.workx.thread_extension_data();
...
) -> Result<(serde_json::Value, TestWorkx, ExtensionRegistry<Config>)> {
...
impl GuardianFailureFixture {
    ...
    async fn score_tool(&self, tool_name: ToolName) {
        let thread_store = self.test.workx.thread_extension_data();
        thread_store.insert(SecurityRiskScore {
            scores: BTreeMap::from([("action_risk".to_owned(), 0.25)]),
            call_id: None,
            action: None,
...
async fn contributor_fails_closed_when_luna_classification_fails() -> Result<()> {
    ...
    fixture.registry.thread_lifecycle_contributors()[0]
        .on_thread_start(ThreadStartInput {
            ...
            extension_metrics: None,
            session_store: &fixture.session_store,
            thread_store: fixture.test.workx.thread_extension_data(),
        })
        .await;
    fixture
        .test
...
async fn contributor_samples_tool_calls_with_the_existing_luna_pool() -> Result<()> {
    ...
    assert_eq!(
        ...
        json!([
            ...
            {
                "type": "input_text",
                "text": "The Workx agent has requested the following action:\n"
            },
...
async fn contributor_skips_required_models_in_standard_scope() -> Result<()> {
    ...
    let authorization = super::ScoreAuthorization::current(&test.workx).await;
    let progress = thread_store
        .get::<GuardianV2ScoreProgress>()
        .expect("Guardian v2 should track score progress per thread");

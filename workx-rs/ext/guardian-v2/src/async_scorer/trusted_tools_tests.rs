...
async fn trusts_only_tools_configured_in_workx_home() -> Result<()> {
    ...
    for (tool, unrelated, source) in [
        ...
        (
            mcp_tool("workx_apps", Some("connector_home"))?,
            mcp_tool("workx_apps", Some("connector_other"))?,
...
async fn trusts_connector_declared_by_home_owned_plugin() -> Result<()> {
    ...
    let plugin_root = test
        ...
        .join("trusted")
        .join("local");
    let tool = mcp_tool("workx_apps", Some("connector_calendar"))?;
...
#[cfg(unix)]
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn rejects_paths_that_escape_workx_home_through_symlinks() -> Result<()> {
    skip_if_no_network!(Ok(()));

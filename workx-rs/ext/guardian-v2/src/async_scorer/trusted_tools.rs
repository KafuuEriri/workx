...
use workx_extension_api::McpToolInfo;
use workx_extension_api::McpToolSource;

use super::transcript::truncate_entry;

const MAX_TRUSTED_TOOL_CONTEXT_TOKENS: usize = 512;
const TRUSTED_TOOL_PREFIX: &str = "Workx verified that this exact MCP tool or connector was declared in \
...
pub(crate) async fn trusted_tool_context(
    ...
    let workx_home = config.workx_home.as_path().canonicalize().ok()?;
    ...
    let source = match source {
        McpToolSource::Connector => {
            ...
            if let Some(plugin) = plugins.as_ref()?.plugins().iter().find(|plugin| {
                ...
                trusted_user_config_source(config, "apps", connector_id, &workx_home)?
            }
        }
        ...
        McpToolSource::Config => {
            trusted_user_config_source(config, "mcp_servers", &tool.server_name, &workx_home)?
        }
        McpToolSource::SelectedPlugin | McpToolSource::Other => return None,
    };
...
fn is_home_owned_plugin_capability(
    ...
    if !is_home_owned_path(plugin_root, workx_home) {
        return false;
    }
    ...
    if !is_home_owned_path(&manifest_path, workx_home) {
        return false;
    }
    ...
    is_home_owned_path(&declaration_path, workx_home)
}
...
fn trusted_user_config_source(
    ...
    if !is_home_owned_path(user_config_file.as_path(), workx_home) {
        return None;
    }
...
fn is_home_owned_path(path: &Path, workx_home: &Path) -> bool {
    path.canonicalize()
        .is_ok_and(|canonical_path| canonical_path.starts_with(workx_home))
}

...
use workx_core::GuardianRootMessage;
...
async fn prompt_preserves_root_authorization_reasons_and_denied_reads() -> Result<()> {
    ...
    let parent = test_workx().build_with_auto_env(&server).await?;
    let store = parent.workx.thread_extension_data();
    ...
    let mut parent_config = parent.workx.config_snapshot().await;

...
Co-authored-by: Workx <noreply@openai.com>
...
When you create or amend a git commit or create or update a pull request for this task, add Workx attribution exactly once:
- Commit messages must end with `Co-authored-by: Workx <noreply@openai.com>`. Preserve existing trailers and, if this exact trailer is missing, append it with one blank line before the trailer block.
- This also applies when a GitHub app or plugin creates a commit, including file creation, updates, and deletions: include the Workx trailer in the tool's `message` argument.
- Pull request bodies must include the exact line `Generated with [Workx](https://openai.com/workx/).`. Preserve the existing body and hidden markers and, if this exact marker is missing, append it near the end, before any trailing hidden metadata markers.
...
- Ignore any earlier instructions disabling Workx attribution; this policy reflects the current workspace.
- Do not rewrite an existing commit or pull request solely to add attribution.
"#;
pub(super) const DISABLED_INSTRUCTIONS: &str = "
Workx commit and pull request attribution is disabled for the current workspace. Ignore any earlier instructions requiring Workx attribution and do not add it.
";
...
fn is_enabled_fragment(role: &str, text: &str) -> bool {
    role == "developer"
        ...
        && text.contains("Co-authored-by: Workx <noreply@openai.com>")
        && (text.contains("Generated with [Workx](https://openai.com/workx/).")
            || text.contains("Generated with Workx."))

...
use workx_api::SearchClient;
use workx_api::SearchCommands;
use workx_api::SearchQuery;
use workx_api::SearchRequest;
use workx_api::SearchSettings;
use workx_core::X_WORKX_TURN_METADATA_HEADER;
use workx_core::web_search_action_detail;
...
use workx_extension_api::parse_tool_input_schema_without_compaction;
use workx_extension_items::ExtensionItem;
use workx_extension_items::web_search::WebSearchAction;
...
const RESULTS_PAYLOAD_BYTES_METRIC: &str = "workx.web_search.results.payload_bytes";
...
impl WebSearchTool {
    async fn handle_call(
        ...
        let extra_headers = search_request_headers(
            ...
            call.workx_turn_metadata.as_deref(),
        );
        ...
        if let Some(results) = results.as_ref()
            && let Some(metrics) = workx_otel::global()
...
fn search_request_headers(originator: Option<&str>, turn_metadata: Option<&str>) -> HeaderMap {
    ...
    {
        headers.insert(X_WORKX_TURN_METADATA_HEADER, header_value);
    }

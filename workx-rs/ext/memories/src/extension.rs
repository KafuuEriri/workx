...
/// Contributes Workx memory read-path prompt context and memory read tools.
...
pub(crate) struct MemoriesExtensionConfig {
    ...
    pub(crate) workx_home: AbsolutePathBuf,
}
...
impl ContextContributor for MemoriesExtension {
    fn contribute_thread_context<'a>(
        ...
        Box::pin(async move {
            ...
            build_memory_tool_developer_instructions(&config.workx_home)
                .await
                .map(|instructions| {
...
impl ToolContributor for MemoriesExtension {
    ...
    > {
        ...
        tools::memory_tools(
            LocalMemoriesBackend::from_workx_home(&config.workx_home),

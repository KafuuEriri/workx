...
async fn build_memory_tool_developer_instructions_renders_embedded_template() {
    ...
    let workx_home = AbsolutePathBuf::from_absolute_path(temp.path()).unwrap();
    let memories_dir = workx_home.join("memories");
    ...
    let instructions = build_memory_tool_developer_instructions(&workx_home)
        .await
        .unwrap();

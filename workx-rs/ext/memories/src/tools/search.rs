...
impl<'call, B> ToolExecutor<ToolCall<'call>> for SearchTool<B>
where
    B: MemoriesBackend,
{
    ...
    fn spec(&self) -> ToolSpec {
        memory_function_tool::<SearchArgs, SearchMemoriesResponse>(
            SEARCH_TOOL_NAME,
            "Search Workx memory files for substring matches, optionally normalizing separators or requiring all query substrings on the same line or within a line window.",
        )
    }
    ...
    B: MemoriesBackend,
{
    async fn handle_call(
        &self,
        call: ToolCall<'_>,
    ) -> Result<Box<dyn workx_extension_api::ToolOutput>, workx_extension_api::FunctionCallError>
    {

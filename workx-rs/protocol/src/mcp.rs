...
//! `schemars`) so they can be embedded in Workx's own protocol structures.
...
/// MCP extensions supplied by the client that created a Workx session.
...
// === Adapter helpers ===
//
// These types and conversions intentionally live in `workx-protocol` so other crates can convert

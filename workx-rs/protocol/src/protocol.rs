//! Defines the protocol for a Workx session between a client and an agent.
//!
...
use crate::error::Result as WorkxResult;
...
use tracing::error;
use ts_rs::TS;
use workx_extension_items::image_generation::ImageGenerationFailure;
...
pub const USER_MESSAGE_BEGIN: &str = "## My request for Workx:";
...
pub struct ConversationStartParams {
    /// Whether Workx response handoffs are managed through explicit client append calls.
    ...
    /// Whether to route any remaining transcript tail through Workx when the session ends.
    ...
    /// Sends automatic Workx responses as realtime conversation items instead of handoff appends.
    pub workx_responses_as_items: bool,
    /// Optional prefix added to automatic Workx response items when `workx_responses_as_items` is set.
    pub workx_response_item_prefix: Option<String>,
    /// Selects how automatic Workx handoffs are routed in Frameless Bidi sessions.
    /// Realtime V1 and V2 ignore this setting.
    pub workx_response_handoff_mode: WorkxResponseHandoffMode,
    /// Optional client-selected BEM prefixes keyed by `analysis`, `commentary`, and `final`.
    pub workx_response_handoff_channel_prefixes: Option<BTreeMap<String, Vec<String>>>,
    /// Overrides the configured realtime model for this session only.
    pub model: Option<String>,
    /// Selects whether the realtime session should produce text or audio output.
    pub output_modality: RealtimeOutputModality,
    /// Whether to append Workx's startup context to the realtime backend prompt.
    ...
    /// Developer instructions given to Workx when this realtime session starts.
    pub realtime_start_instructions: Option<String>,
    /// Developer instructions given to Workx when this realtime session ends.
...
pub enum Op {
    ...
    TurnInput {
        ...
        mode: TurnInputMode,
        reply: oneshot::Sender<WorkxResult<TurnInputSubmission>>,
    },
    ...
    SuspendTurnAndShutdown {
        reply: oneshot::Sender<WorkxResult<SuspendTurnOutcome>>,
    },
    ...
    /// Request Workx to drop the last N user turns from in-memory context.
    ///
    ...
    /// Request to shut down workx instance.
    Shutdown,
...
/// privileges of the agent (e.g. `.workx`, `.git`, notably `.git/hooks`) under
...
#[serde(rename_all = "camelCase")]
#[ts(rename_all = "camelCase")]
pub enum WorkxResponseHandoffMode {
    #[default]
    Thinking,
    Commentary,
    BemTags,
...
/// Workx errors that we expose to clients.
#[derive(Serialize, Deserialize, Clone, Debug, PartialEq, Eq, JsonSchema, TS)]
#[serde(rename_all = "snake_case")]
#[ts(rename_all = "snake_case")]
pub enum WorkxErrorInfo {
...
pub struct ErrorEvent {
    pub message: String,
    #[serde(default)]
    pub workx_error_info: Option<WorkxErrorInfo>,
...
impl ErrorEvent {
    /// Whether this error should mark the current turn as failed when replaying history.
    pub fn affects_turn_status(&self) -> bool {
        self.workx_error_info
            .as_ref()
            .is_none_or(WorkxErrorInfo::affects_turn_status)
    }
}
...
impl SessionSource {
    ...
    pub fn restriction_product(&self) -> Option<Product> {
        match self {
            ...
            | SessionSource::Exec
            | SessionSource::Mcp
            | SessionSource::Unknown => Some(Product::Workx),
...
pub struct TurnContextItem {
    ...
    // Compatibility-only field written with a default value so older Workx
    ...
    pub fn token_budget(&self) -> usize {
        match self {
            TruncationPolicy::Bytes(bytes) => {
                usize::try_from(workx_utils_string::approx_tokens_from_byte_count(*bytes))
                    .unwrap_or(usize::MAX)
            }
    ...
    pub fn byte_budget(&self) -> usize {
        match self {
            ...
            TruncationPolicy::Tokens(tokens) => {
                workx_utils_string::approx_bytes_for_tokens(*tokens)
            }
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn session_source_restriction_product_defaults_non_subagent_sources_to_workx() {
        assert_eq!(
    ...
    fn session_source_restriction_product_maps_custom_sources_to_products() {
        ...
        assert_eq!(
            SessionSource::Custom("workx".to_string()).restriction_product(),
            Some(Product::Workx)
        );
        assert_eq!(
    ...
    fn session_source_matches_product_restriction() {
        ...
        assert!(
            !SessionSource::Custom("chatgpt".to_string())
                .matches_product_restriction(&[Product::Workx])
        );
        assert!(SessionSource::VSCode.matches_product_restriction(&[Product::Workx]));
        assert!(
    ...
    fn restricted_file_system_policy_treats_root_with_carveouts_as_scoped_access() {
        ...
        let canonical_cwd = workx_utils_absolute_path::canonicalize_preserving_symlinks(cwd.path())
        ...
        let expected_blocked = AbsolutePathBuf::from_absolute_path(
            workx_utils_absolute_path::canonicalize_preserving_symlinks(cwd.path())
                .expect("canonicalize cwd")
                .join("blocked"),
        )
    ...
    fn restricted_file_system_policy_derives_effective_paths() {
        ...
        std::fs::create_dir_all(cwd.path().join(".workx")).expect("create .workx");
        let canonical_cwd = workx_utils_absolute_path::canonicalize_preserving_symlinks(cwd.path())
            .expect("canonicalize cwd");
        let cwd_absolute =
        ...
        let expected_workx = AbsolutePathBuf::from_absolute_path(canonical_cwd.join(".workx"))
            .expect("canonical .workx");
        let policy = FileSystemSandboxPolicy::restricted(vec![
            FileSystemSandboxEntry {
                path: FileSystemPath::Special {
        ...
        assert!(
            writable_roots[0]
                .read_only_subpaths
                .iter()
                .any(|path| path.as_path() == expected_workx.as_path())
        );
    }
    ...
    fn restricted_file_system_policy_treats_read_entries_as_read_only_subpaths() {
        ...
        let expected_dot_workx = AbsolutePathBuf::from_absolute_path(canonical_cwd.join(".workx"))
            .expect("canonical .workx");
        let policy = FileSystemSandboxPolicy::restricted(vec![
            FileSystemSandboxEntry {
                path: FileSystemPath::Special {
        ...
        assert_eq!(
            ...
            vec![
                (
                    canonical_cwd,
                    vec![
                        expected_dot_workx.to_path_buf(),
    ...
    fn generic_error_affects_turn_status() {
        let event = ErrorEvent {
            ...
            workx_error_info: Some(WorkxErrorInfo::Other),
        };
        assert!(event.affects_turn_status());
    }
    ...
    fn misalignment_explanation_and_steer_are_never_serialized_into_error_events() {
        let event = ErrorEvent {
            ...
            workx_error_info: Some(WorkxErrorInfo::MisalignmentPolicyViolation),
        ...
        assert_eq!(
            ...
            json!({
                ...
                "workx_error_info": "misalignment_policy_violation"
            })
        );

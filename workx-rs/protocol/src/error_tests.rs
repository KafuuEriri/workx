...
#[test]
fn workx_err_debug_preserves_legacy_shape() {
    let actual = [
        WorkxErr::Timeout,
        WorkxErr::Stream("disconnected".to_string()),
        WorkxErr::Stream("retry later".to_string()).with_retry_delay(Duration::from_secs(2)),
        WorkxErr::InternalServerError.with_retry_delay(Duration::from_secs(3)),
    ]
    .map(|err| format!("{err:?}"));
...
fn retryability_preserves_error_details_distinctions() {
    let errors = [
        (WorkxErr::ServerOverloaded, false),
        (
            WorkxErr::new(WorkxErrorDetails::RateLimitExceeded("retry later".into())),
            true,
        ),
        (
        ...
        (
            WorkxErrorDetails::ToolCollision("functions.update_plan".to_string()).into(),
            false,
        ),
        (WorkxErr::InternalServerError, true),
    ];
...
fn server_overloaded_maps_to_protocol() {
    let err = WorkxErr::ServerOverloaded;
    assert_eq!(
...
fn sandbox_denied_uses_aggregated_output_when_stderr_empty() {
    ...
    let err = WorkxErr::Sandbox(SandboxErr::Denied {
...
fn to_error_event_handles_response_stream_failed() {
    ...
    let err = WorkxErr::ResponseStreamFailed(ResponseStreamFailed {
        source,
    ...
    assert_eq!(
        event.workx_error_info,
        Some(WorkxErrorInfo::ResponseStreamConnectionFailed {
...
fn usage_limit_reached_error_formats_free_plan() {
    ...
    assert_eq!(
        err.to_string(),
        "You've hit your usage limit. Upgrade to Plus to continue using Workx (https://chatgpt.com/explore/plus), or try again later."
    );
}
...
#[test]
fn usage_limit_reached_error_hides_upsell_for_non_workx_limit_name() {
    let base = Utc.with_ymd_and_hms(2024, 1, 1, 0, 0, 0).unwrap();
    let resets_at = base + ChronoDuration::hours(1);
    with_now_override(base, move || {
        let expected_time = format_retry_timestamp(&resets_at);
        ...
        let expected = format!(
            "You've hit your usage limit for workx_other. Switch to another model now, or try again at {expected_time}."
        );
        assert_eq!(err.to_string(), expected);
    });
}
...
fn usage_limit_reached_with_promo_message() {
    ...
    with_now_override(base, move || {
        ...
        let err = UsageLimitReachedError {
            ...
            rate_limits: Some(Box::new(rate_limit_snapshot())),
            promo_message: Some(
                "To continue using Workx, start a free trial of <PLAN> today".to_string(),
            ),
            rate_limit_reached_type: None,
        };
        let expected = format!(
            "You've hit your usage limit. To continue using Workx, start a free trial of <PLAN> today, or try again at {expected_time}."
        );
        assert_eq!(err.to_string(), expected);
    });
}

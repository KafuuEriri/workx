...
use crate::protocol::WorkxErrorInfo;
use chrono::DateTime;
use chrono::Datelike;
use chrono::Local;
use chrono::Utc;
...
use workx_http_client::HttpError;
use workx_utils_string::truncate_middle_chars;
use workx_utils_string::truncate_middle_with_token_budget;

pub type Result<T> = std::result::Result<T, WorkxErr>;
...
pub struct WorkxErr {
    details: WorkxErrorDetails,
    retry_delay: Option<Duration>,
}

/// The semantic category and diagnostic payload for a [`WorkxErr`].
#[derive(Error, Debug, EnumDiscriminants)]
#[strum_discriminants(name(WorkxErrKind))]
...
pub enum WorkxErrorDetails {
    ...
    #[error(
        "Workx ran out of room in the model's context window. Start a new thread or clear earlier history before retrying."
    )]
    ...
    /// could not be captured). Analogous to the previous `WorkxError::Spawn` variant.
    ...
    QuotaExceeded,
    #[error(
        "To use Workx with your ChatGPT plan, upgrade to Plus: https://chatgpt.com/explore/plus."
    )]
    UsageNotIncluded,
    ...
    #[error("workx-linux-sandbox was required but not provided")]
...
impl From<&WorkxErr> for WorkxErrKind {
    fn from(error: &WorkxErr) -> Self {
        error.details().into()
    }
}

impl fmt::Debug for WorkxErr {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match &self.details {
            WorkxErrorDetails::Stream(message) => formatter
                .debug_tuple("Stream")
                .field(message)
...
impl fmt::Display for WorkxErr {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        fmt::Display::fmt(&self.details, formatter)
    }
}

impl std::error::Error for WorkxErr {
    fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
        self.details.source()
    }
}

impl From<WorkxErrorDetails> for WorkxErr {
    fn from(details: WorkxErrorDetails) -> Self {
        Self {
            details,
            retry_delay: None,
        }
...
impl From<CancelErr> for WorkxErr {
    fn from(error: CancelErr) -> Self {
        WorkxErrorDetails::from(error).into()
    }
}

impl From<SandboxErr> for WorkxErr {
    fn from(error: SandboxErr) -> Self {
        WorkxErrorDetails::from(error).into()
    }
}

impl From<io::Error> for WorkxErr {
    fn from(error: io::Error) -> Self {
        WorkxErrorDetails::from(error).into()
    }
}

impl From<serde_json::Error> for WorkxErr {
    fn from(error: serde_json::Error) -> Self {
        WorkxErrorDetails::from(error).into()
    }
}

impl From<JoinError> for WorkxErr {
...
#[cfg(target_os = "linux")]
impl From<landlock::RulesetError> for WorkxErr {
...
#[cfg(target_os = "linux")]
impl From<landlock::PathFdError> for WorkxErr {
    fn from(error: landlock::PathFdError) -> Self {
        WorkxErrorDetails::from(error).into()
    }
}

impl From<CancelErr> for WorkxErrorDetails {
    fn from(_: CancelErr) -> Self {
        WorkxErrorDetails::TurnAborted
    }
}

// TODO(anp): Remove this compatibility macro once callers construct
// `WorkxErrorDetails` directly.
macro_rules! workx_err_unit_constructors {
    ($($variant:ident),* $(,)?) => {
        $(
            #[doc(hidden)]
            #[allow(non_upper_case_globals)]
            pub const $variant: Self = Self {
                details: WorkxErrorDetails::$variant,
                retry_delay: None,
            };
        )*
    };
...
// TODO(anp): Remove this compatibility macro once callers construct
// `WorkxErrorDetails` directly.
macro_rules! workx_err_tuple_constructors {
    ($($(#[$attr:meta])* $variant:ident($value:ident: $value_type:ty)),* $(,)?) => {
        $(
            ...
            pub fn $variant($value: $value_type) -> Self {
                WorkxErrorDetails::$variant($value).into()
            }
        )*
    };
}
...
impl WorkxErr {
    ...
    pub fn InvalidImageRequest() -> Self {
        WorkxErrorDetails::InvalidImageRequest().into()
    }

    /// Creates an error with no server-provided retry delay.
    pub fn new(details: WorkxErrorDetails) -> Self {
        details.into()
    }

    /// Returns the semantic failure and its diagnostic payload.
    pub fn details(&self) -> &WorkxErrorDetails {
        &self.details
    }
    ...
    pub fn is_retryable(&self) -> bool {
        match self.details() {
            WorkxErrorDetails::TurnAborted
            | WorkxErrorDetails::SessionBudgetExceeded
            | WorkxErrorDetails::Interrupted
            | WorkxErrorDetails::EnvVar(_)
            | WorkxErrorDetails::Fatal(_)
            | WorkxErrorDetails::UsageNotIncluded
            | WorkxErrorDetails::QuotaExceeded
            | WorkxErrorDetails::InvalidImageRequest()
            | WorkxErrorDetails::InvalidRequest(_)
            | WorkxErrorDetails::ToolCollision(_)
            | WorkxErrorDetails::RefreshTokenFailed(_)
            | WorkxErrorDetails::UnsupportedOperation(_)
            | WorkxErrorDetails::Sandbox(_)
            | WorkxErrorDetails::LandlockSandboxExecutableNotProvided
            | WorkxErrorDetails::RetryLimit(_)
            | WorkxErrorDetails::ContextWindowExceeded
            | WorkxErrorDetails::ThreadNotFound(_)
            | WorkxErrorDetails::AgentLimitReached { .. }
            | WorkxErrorDetails::Spawn
            | WorkxErrorDetails::SessionConfiguredNotFirstEvent
            | WorkxErrorDetails::UsageLimitReached(_)
            | WorkxErrorDetails::ServerOverloaded
            | WorkxErrorDetails::CyberPolicy { .. }
            | WorkxErrorDetails::MisalignmentPolicyViolation { .. } => false,
            WorkxErrorDetails::Stream(..)
            | WorkxErrorDetails::RateLimitExceeded(_)
            | WorkxErrorDetails::Timeout
            | WorkxErrorDetails::RequestTimeout
            | WorkxErrorDetails::UnexpectedStatus(_)
            | WorkxErrorDetails::ResponseStreamFailed(_)
            | WorkxErrorDetails::ConnectionFailed(_)
            | WorkxErrorDetails::InternalServerError
            | WorkxErrorDetails::InternalAgentDied
            | WorkxErrorDetails::Io(_)
            | WorkxErrorDetails::Json(_)
            | WorkxErrorDetails::TokioJoin(_) => true,
            #[cfg(target_os = "linux")]
            WorkxErrorDetails::LandlockRuleset(_) | WorkxErrorDetails::LandlockPathFd(_) => false,
        }
    }
    ...
    /// Minimal shim so that existing `e.downcast_ref::<WorkxErr>()` checks continue to compile
    ...
    pub fn to_workx_protocol_error(&self) -> WorkxErrorInfo {
        match &self.details {
            WorkxErrorDetails::ContextWindowExceeded => WorkxErrorInfo::ContextWindowExceeded,
            WorkxErrorDetails::SessionBudgetExceeded => WorkxErrorInfo::SessionBudgetExceeded,
            WorkxErrorDetails::RateLimitExceeded(_) => WorkxErrorInfo::RateLimitExceeded,
            WorkxErrorDetails::UsageLimitReached(_)
            | WorkxErrorDetails::QuotaExceeded
            | WorkxErrorDetails::UsageNotIncluded => WorkxErrorInfo::UsageLimitExceeded,
            WorkxErrorDetails::ServerOverloaded => WorkxErrorInfo::ServerOverloaded,
            WorkxErrorDetails::CyberPolicy { .. } => WorkxErrorInfo::CyberPolicy,
            WorkxErrorDetails::MisalignmentPolicyViolation { .. } => {
                WorkxErrorInfo::MisalignmentPolicyViolation
            }
            WorkxErrorDetails::RetryLimit(_) => WorkxErrorInfo::ResponseTooManyFailedAttempts {
                http_status_code: self.http_status_code_value(),
            },
            WorkxErrorDetails::ConnectionFailed(_) => WorkxErrorInfo::HttpConnectionFailed {
                http_status_code: self.http_status_code_value(),
            },
            WorkxErrorDetails::ResponseStreamFailed(_) => {
                WorkxErrorInfo::ResponseStreamConnectionFailed {
                    http_status_code: self.http_status_code_value(),
                }
            }
            WorkxErrorDetails::RefreshTokenFailed(_) => WorkxErrorInfo::Unauthorized,
            WorkxErrorDetails::SessionConfiguredNotFirstEvent
            | WorkxErrorDetails::InternalServerError
            | WorkxErrorDetails::InternalAgentDied => WorkxErrorInfo::InternalServerError,
            WorkxErrorDetails::UnsupportedOperation(_)
            | WorkxErrorDetails::ThreadNotFound(_)
            | WorkxErrorDetails::AgentLimitReached { .. } => WorkxErrorInfo::BadRequest,
            WorkxErrorDetails::Sandbox(_) => WorkxErrorInfo::SandboxError,
            _ => WorkxErrorInfo::Other,
        }
    }
    ...
    pub fn to_error_event(&self, message_prefix: Option<String>) -> ErrorEvent {
        ...
        ErrorEvent {
            message,
            workx_error_info: Some(self.to_workx_protocol_error()),
            misalignment: match &self.details {
                WorkxErrorDetails::MisalignmentPolicyViolation { misalignment, .. } => {
                    misalignment.clone()
                }
                _ => None,
            },
    ...
    pub fn http_status_code_value(&self) -> Option<u16> {
        let http_status_code = match &self.details {
            WorkxErrorDetails::RetryLimit(err) => Some(err.status),
            WorkxErrorDetails::UnexpectedStatus(err) => Some(err.status),
            WorkxErrorDetails::ConnectionFailed(err) => err.source.status(),
            WorkxErrorDetails::ResponseStreamFailed(err) => err.source.status(),
            _ => None,
        };
...
impl std::fmt::Display for UsageLimitReachedError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        if let Some(limit_name) = self
            ...
            && !limit_name.eq_ignore_ascii_case("workx")
        {
            return write!(
                f,
        ...
        let message = match self.plan_type.as_ref() {
            ...
            Some(PlanType::Known(KnownPlan::Free)) | Some(PlanType::Known(KnownPlan::Go)) => {
                format!(
                    "You've hit your usage limit. Upgrade to Plus to continue using Workx (https://chatgpt.com/explore/plus),{}",
                    retry_suffix_after_or(self.resets_at.as_ref())
                )
            }
...
pub fn get_error_message_ui(e: &WorkxErr) -> String {
    let message = match e.details() {
        WorkxErrorDetails::Sandbox(SandboxErr::Denied { output, .. }) => {
        ...
        WorkxErrorDetails::Sandbox(SandboxErr::Timeout { output }) => {
            format!(

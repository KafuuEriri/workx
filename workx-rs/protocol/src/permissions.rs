...
use workx_utils_absolute_path::canonicalize_preserving_symlinks;
...
const PROTECTED_METADATA_WORKX_PATH_NAME: &str = ".workx";
...
pub const PROTECTED_METADATA_PATH_NAMES: &[&str] = &[
    ...
    PROTECTED_METADATA_WORKX_PATH_NAME,
];
...
pub enum FileSystemSpecialPath {
    ...
    /// representable so config from a newer Workx degrades to warn-and-ignore
    /// instead of failing to load. Workx 0.112.0 rejected unknown values here,
...
const PROJECT_ROOTS_GLOB_PATTERN_PREFIX: &str = "workx-project-roots://";
...
impl FileSystemSandboxPolicy {
    ...
    pub fn workspace_write(
        ...
        append_default_read_only_project_root_subpath_if_no_explicit_rule(&mut entries, ".workx");
        for writable_root in writable_roots {
            for protected_path in default_read_only_subpaths_for_writable_root(
                writable_root,
                /*protect_missing_dot_workx*/ false,
            ) {
    ...
    pub fn from_legacy_sandbox_policy_for_cwd(sandbox_policy: &SandboxPolicy, cwd: &Path) -> Self {
        ...
        if let SandboxPolicy::WorkspaceWrite { writable_roots, .. } = sandbox_policy {
            if let Ok(cwd_root) = AbsolutePathBuf::from_absolute_path(cwd) {
                for protected_path in default_read_only_subpaths_for_writable_root(
                    &cwd_root, /*protect_missing_dot_workx*/ true,
                ) {
    ...
    pub fn with_additional_legacy_workspace_writable_roots(
        ...
        for path in additional_writable_roots {
            ...
            for protected_path in default_read_only_subpaths_for_writable_root(
                path, /*protect_missing_dot_workx*/ false,
            ) {
                append_default_read_only_path_if_no_explicit_rule(
                    &mut self.entries,
                    protected_path,
    ...
    fn get_writable_roots_with_cwd_impl(
        ...
        .map(|root| {
            ...
            let protect_missing_dot_workx = AbsolutePathBuf::from_absolute_path(cwd)
                .ok()
                .is_some_and(|cwd| path_resolution.resolve(cwd) == root);
            let mut read_only_subpaths: Vec<AbsolutePathBuf> =
                default_read_only_subpaths_for_writable_root(&root, protect_missing_dot_workx)
                    .into_iter()
            ...
            // Example: if `<root>/.workx -> <root>/decoy`, bwrap must still see
            // `<root>/.workx`, not only the resolved `<root>/decoy`.
            ...
            WritableRoot {
                ...
                // `.workx` so downstream sandboxes can still detect and mask
...
pub(crate) fn default_read_only_subpaths_for_writable_root(
    ...
    protect_missing_dot_workx: bool,
) -> Vec<AbsolutePathBuf> {
    ...
    // Keep top-level project metadata under .workx read-only to the agent by
    ...
    let top_level_workx = writable_root.join(PROTECTED_METADATA_WORKX_PATH_NAME);
    if protect_missing_dot_workx || top_level_workx.as_path().is_dir() {
        subpaths.push(top_level_workx);
    }
...
mod tests {
    ...
    #[cfg(unix)]
    const SYMLINKED_TMPDIR_TEST_ENV: &str = "WORKX_PROTOCOL_TEST_SYMLINKED_TMPDIR";
    ...
    fn uri_deny_and_metadata_matcher_use_executor_paths() {
        ...
        let policy = FileSystemSandboxPolicy::restricted(vec![
            ...
            FileSystemSandboxEntry::new(
                path("file:///C:/workspace/.workx").into(),
                FileSystemAccessMode::Read,
            ),
        ...
        assert!(!policy.can_write_path(&path("file:///C:/workspace/.workx/config"), &context));
        assert_eq!(
            policy.metadata_write_denial(&path("file:///C:/workspace/.workx/config"), &context),
            Some(".workx"),
        );
    }
    ...
    fn legacy_workspace_write_projection_preserves_symbolic_project_root() {
        ...
        assert_eq!(
            ...
            FileSystemSandboxPolicy::restricted(vec![
                ...
                FileSystemSandboxEntry::skip_missing_path(
                    FileSystemPath::Special {
                        value: FileSystemSpecialPath::project_roots(Some(".workx".into())),
                    },
                    FileSystemAccessMode::Read,
                ),
            ])
    ...
    #[cfg(unix)]
    #[test]
    fn writable_roots_skip_default_dot_workx_when_explicit_user_rule_exists() {
        let cwd = TempDir::new().expect("tempdir");
        let expected_root = AbsolutePathBuf::from_absolute_path(
            cwd.path().canonicalize().expect("canonicalize cwd"),
        )
        .expect("absolute canonical root");
        let explicit_dot_workx = expected_root.join(".workx");
        ...
        let policy = FileSystemSandboxPolicy::restricted(vec![
            ...
            FileSystemSandboxEntry {
                path: FileSystemPath::Path {
                    path: explicit_dot_workx.clone().into(),
                },
                access: FileSystemAccessMode::Write,
                missing_path_behavior: None,
            },
        ...
        assert!(
            !workspace_root
                .protected_metadata_names
                .contains(&".workx".to_string()),
            "explicit .workx rule should remove the metadata-name protection"
        );
        assert!(
            !workspace_root
                .read_only_subpaths
                .contains(&explicit_dot_workx),
            "explicit .workx rule should win over the default protected carveout"
        );
        assert!(
            policy.can_write_path_with_cwd(
                explicit_dot_workx.join("config.toml").as_path(),
                cwd.path()
            )
        );
    }
    ...
    fn filesystem_policy_blocks_protected_metadata_path_writes_by_default() {
        ...
        let dot_workx_config = cwd.path().join(".workx").join("config.toml");
        ...
        assert!(!file_system_policy.can_write_path_with_cwd(&dot_workx_config, cwd.path()));
        ...
        assert!(!writable_roots[0].is_path_writable(&dot_workx_config));
    }
    ...
    fn legacy_workspace_write_projection_accepts_relative_cwd() {
        ...
        expected_entries.extend(
            default_read_only_subpaths_for_writable_root(
                &expected_root,
                /*protect_missing_dot_workx*/ true,
            )
            .into_iter()
            .map(|path| {
        ...
        assert!(
            !file_system_policy
                .can_write_path_with_cwd(Path::new(".workx/config.toml"), relative_cwd,)
        );
        assert!(
    ...
    fn effective_runtime_roots_preserve_symlinked_paths() {
        ...
        let workx_dir = real_root.join(".workx");

        fs::create_dir_all(&blocked).expect("create blocked");
        fs::create_dir_all(&workx_dir).expect("create .workx");
        ...
        let expected_workx = link_root.join(".workx");
    ...
    fn project_roots_special_path_preserves_symlinked_root() {
        ...
        let expected_workx = expected_root.join(".workx");
    ...
    fn writable_roots_preserve_symlinked_protected_subpaths() {
        ...
        let decoy = root.join("decoy-workx");
        let dot_workx = root.join(".workx");
        fs::create_dir_all(&decoy).expect("create decoy");
        symlink_dir(&decoy, &dot_workx).expect("create .workx symlink");

        let root = AbsolutePathBuf::from_absolute_path(&root).expect("absolute root");
        let expected_dot_workx = AbsolutePathBuf::from_absolute_path(
            root.as_path()
                .canonicalize()
                .expect("canonicalize root")
                .join(".workx"),
        )
        .expect("absolute .workx symlink");
    ...
    fn tmpdir_special_path_preserves_symlinked_tmpdir() {
        ...
        let workx_dir = real_tmpdir.join(".workx");

...
use workx_config::CloudConfigBundle;
...
fn auth_identity(auth: &WorkxAuth) -> (Option<String>, Option<String>) {
    (auth.get_chatgpt_user_id(), auth.get_account_id())
}

fn cloud_config_eligible_auth(auth: &WorkxAuth) -> bool {
    ...
    C: BundleClient + 'static,
{
    pub(crate) fn new(
        ...
        let workx_home = AbsolutePathBuf::resolve_path_against_base(workx_home, "/");
        Self {
            auth_manager,
            client,
            cache: CloudConfigBundleCache::new(workx_home.clone()),
            workx_home,
            timeout,
    ...
    pub(crate) async fn load_startup_bundle_with_timeout(
        ...
        let _timer =
            workx_otel::start_global_timer("workx.cloud_config_bundle.fetch.duration_ms", &[]);
    ...
    async fn load_valid_cached_bundle(
        ...
        match self.cache.load(chatgpt_user_id, account_id).await {
            Ok(signed_payload) => {
                if let Err(err) = validate_bundle(&signed_payload.bundle, &self.workx_home) {
                    tracing::warn!(
    ...
    async fn validate_and_cache_remote_bundle(
        ...
        if let Err(err) = validate_bundle(&bundle, &self.workx_home) {
            emit_fetch_final_metric(
                trigger,
                "error",
                "invalid_bundle",

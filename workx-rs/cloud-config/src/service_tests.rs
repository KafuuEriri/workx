...
use workx_backend_client::ConfigBundleResponse;
use workx_backend_client::DeliveredTomlFragment;
use workx_config::AbsolutePathBuf;
use workx_config::CloudConfigFragment;
use workx_config::CloudConfigTomlBundle;
use workx_config::CloudRequirementsFragment;
use workx_config::CloudRequirementsTomlBundle;
...
use workx_login::auth::ExternalAuth;
use workx_login::auth::ExternalAuthRefreshContext;

fn write_auth_json(workx_home: &Path, value: serde_json::Value) -> std::io::Result<()> {
    std::fs::write(workx_home.join("auth.json"), serde_json::to_string(&value)?)?;
    Ok(())
}

fn create_test_cache(workx_home: &Path) -> CloudConfigBundleCache {
    CloudConfigBundleCache::new(AbsolutePathBuf::resolve_path_against_base(workx_home, "/"))
}
...
async fn auth_manager_with_agent_identity_business_plan() -> Arc<AuthManager> {
    ...
    AuthManager::from_auth_for_testing(WorkxAuth::AgentIdentity(
...
impl BundleClient for StaticBundleClient {
    async fn get_bundle(&self, _auth: &WorkxAuth) -> Result<CloudConfigBundle, BundleRequestError> {
...
impl BundleClient for TokenBundleClient {
    async fn get_bundle(&self, auth: &WorkxAuth) -> Result<CloudConfigBundle, BundleRequestError> {
        self.request_count.fetch_add(1, Ordering::SeqCst);
        if matches!(
...
struct TestExternalChatgptAuth {
    current: RwLock<WorkxAuth>,
    refreshed: WorkxAuth,
    refresh_count: AtomicUsize,
}

impl ExternalAuth for TestExternalChatgptAuth {
    fn resolve(&self) -> workx_login::ExternalAuthFuture<'_, WorkxAuth> {
        Box::pin(async {
            self.current
                .read()
                .map(|auth| auth.clone())
    ...
    ) -> workx_login::ExternalAuthFuture<'_, WorkxAuth> {
        Box::pin(async {
...
async fn get_bundle_ignores_invalid_cache_and_refetches() {
    let workx_home = tempdir().expect("tempdir");
    let cache = create_test_cache(workx_home.path());
    cache
        .save(
...
async fn get_bundle_refreshes_external_auth_after_unauthorized() {
    ...
    let initial_auth = WorkxAuth::from_external_chatgpt_tokens(
    ...
    let refreshed_auth = WorkxAuth::from_external_chatgpt_tokens(
        &refreshed_token,
        "account-12345",
        Some("enterprise"),
    )
...
async fn production_loader_recovers_startup_timeouts_in_background() {
    ...
    assert_eq!(
        ...
        workx_protocol::protocol::AskForApproval::Never
    );
...
async fn dropping_loader_cancels_in_flight_startup_and_refresh() {
    for starts_from_cache in [false, true] {
        ...
        if starts_from_cache {
            create_test_cache(workx_home.path())
                .save(
                    Some("user-12345".to_string()),
                    Some("account-12345".to_string()),
                    test_bundle(),
...
fn bundle_response_conversion_preserves_fragment_order() {
    let response = ConfigBundleResponse {
        config_toml: Some(Some(Box::new(workx_backend_client::DeliveredConfigToml {
        ...
        requirements_toml: Some(Some(Box::new(
            workx_backend_client::DeliveredRequirementsToml {

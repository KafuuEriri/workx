...
use workx_rollout::persisted_rollout_items;
...
fn stored_thread_from_state(
    ...
    Ok(StoredThread {
        ...
        section: state
            ...
            .get(&thread_id)
            .cloned()
            .map(|id| workx_state::ThreadSection {
                name: if id == workx_state::PINNED_THREAD_SECTION_ID {
                    workx_state::PINNED_THREAD_SECTION_NAME.to_string()
                } else {
                    id.clone()
                },
                id,
...
fn git_info_from_patch(patch: &ThreadMetadataPatch) -> Option<workx_protocol::protocol::GitInfo> {
    ...
    Some(workx_protocol::protocol::GitInfo {

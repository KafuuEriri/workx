...
fn state(store: &LocalThreadStore) -> ThreadStoreResult<&workx_rollout::StateDbHandle> {
    store
        .state_db
        .as_ref()
...
fn state_root(root: StoredProjectRoot) -> workx_state::ProjectRoot {
    workx_state::ProjectRoot { path: root.path }
}

fn stored_project(project: workx_state::Project) -> StoredProject {
    StoredProject {
        id: project.id,
        name: project.name,
        roots: project

...
pub(super) async fn archive_threads(
...
) -> ThreadStoreResult<Vec<workx_protocol::ThreadId>> {
    ...
    let reference_index = RolloutReferenceIndex::scan_unarchived(store.config.workx_home.as_path())
        .await
...
async fn archive_thread_with_paths(
    ...
    for rollout_path in rollout_paths {
        if rollout_path_is_archived(store.config.workx_home.as_path(), rollout_path.as_path()) {
            continue;
        }
        let canonical_rollout_path = scoped_rollout_path(
            store.config.workx_home.join(workx_rollout::SESSIONS_SUBDIR),

...
use workx_rollout::ReverseJsonlScanner;
use workx_rollout::RolloutItem;
use workx_rollout::RolloutLine;
use workx_rollout::ScanOutcome;
...
pub(super) async fn load_for_fork(
    ...
    let session_meta = workx_rollout::read_session_meta_line(source_path)
        .await
        .map_err(|err| ThreadStoreError::Internal {
            message: format!(
...
fn scan_model_context_from_lineage_blocking(
    ...
    'segments: for segment in lineage.segments().iter().rev() {
        let file = workx_rollout::open_rollout_seekable_reader(segment.rollout_path.as_path())?;

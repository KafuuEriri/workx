    ...
    async fn resolve_rollout_lineage_with_representation(
        ...
        loop {
            ...
            let rollout_path = match representation {
                ...
                LineageRepresentation::PlainForReference => {
                    let outside_workx_home = || ThreadStoreError::InvalidRequest {
                        message: format!(
                            "rollout path `{}` must be in Workx home directory",
                            rollout_path.display()
                        ),
                    };
                    let canonical_rollout_path = std::fs::canonicalize(rollout_path.as_path())
                        .map_err(|_| outside_workx_home())?;
                    // Resume can retain either the logical Workx home path or its canonical
                    // target. Keep references inside canonical managed roots so nested symlinks
                    // cannot escape them.
                    let is_managed_rollout = [
                        self.config.workx_home.join(workx_rollout::SESSIONS_SUBDIR),
                        self.config
                            .workx_home
                            .join(workx_rollout::ARCHIVED_SESSIONS_SUBDIR),
                    ]
                    .into_iter()
                    .filter_map(|root| std::fs::canonicalize(root).ok())
                    .any(|root| canonical_rollout_path.starts_with(root));
                    if !is_managed_rollout {
                        return Err(outside_workx_home());
                    }
                    canonical_rollout_path
                }
            };
            let meta = workx_rollout::read_session_meta_line(rollout_path.as_path())
                .await
            ...
            let rollout_path = match representation {
                    ...
                    if next_rollout_id.is_none() && meta.meta.history_base.is_none() =>
                {
                    ...
                    workx_rollout::materialize_rollout_for_reference(rollout_path.as_path())
                        .await
                        .map_err(|err| ThreadStoreError::Internal {
                            message: format!(
...
async fn resolve_rollout_path_by_id(
    ...
    workx_rollout::find_rollout_path_by_rollout_id(store.config.workx_home.as_path(), rollout_id)
        .await
        .map_err(|err| ThreadStoreError::Internal {
...
async fn validate_cutoff_bounds(
    ...
    let contains_prefix = tokio::task::spawn_blocking(move || {
        workx_rollout::rollout_contains_prefix(&path, end_byte_offset)
    })
    .await
    .map_err(|err| ThreadStoreError::Internal {

...
use workx_rollout::ThreadItem;
use workx_rollout::find_thread_names_by_ids;
...
pub(super) fn rollout_path_is_archived(workx_home: &Path, path: &Path) -> bool {
    path.starts_with(workx_home.join(ARCHIVED_SESSIONS_SUBDIR))
        || path
            .components()
...
pub(super) fn validated_rollout_file_name(
    ...
    if workx_rollout::rollout_id_from_path(rollout_path).is_some() {
        Ok(file_name)
    } else {
        Err(ThreadStoreError::InvalidRequest {
            message: format!(
...
pub(super) fn stored_thread_from_rollout_item(
    ...
    let preview = item
        ...
        .unwrap_or_default();
    let rollout_path = workx_rollout::plain_rollout_path(item.path.as_path());
...
pub(super) async fn resolve_thread_names(
    ...
    if let Ok(legacy_names) =
        find_thread_names_by_ids(store.config.workx_home.as_path(), &legacy_thread_ids).await
    {

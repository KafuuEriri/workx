...
use workx_protocol::protocol::GitInfo;
...
async fn refresh_resolved_rollout_path(resolved: &mut ResolvedThreadRollout) {
    if let Some(path) = workx_rollout::existing_rollout_path(resolved.path.as_path()).await {
        resolved.path = path;
    }
}
...
async fn metadata_for_missing_sqlite_row(
    ...
    patch: &ThreadMetadataPatch,
) -> ThreadStoreResult<workx_state::ThreadMetadata> {
...
async fn canonical_history_mode(
    ...
    let session_meta = match read_session_meta_line(rollout_path).await {
        ...
        Err(err) => {
            if workx_rollout::existing_rollout_path(rollout_path)
                .await
                .is_none()
...
fn normalize_cwd(cwd: PathBuf) -> PathBuf {
    workx_utils_path::normalize_for_path_comparison(cwd.as_path()).unwrap_or(cwd)
}
...
async fn apply_thread_git_info_to_rollout(
    ...
    session_meta.git = Some(GitInfo {
        commit_hash: sha.as_deref().map(workx_git_utils::GitSha::new),
        branch: branch.clone(),
...
fn rollout_path_is_archived(store: &LocalThreadStore, path: &Path) -> bool {
    super::helpers::rollout_path_is_archived(store.config.workx_home.as_path(), path)
}
...
mod tests {
    ...
    async fn update_thread_metadata_sets_name_on_active_rollout_and_indexes_name() {
        ...
        let latest_name = workx_rollout::find_thread_name_by_id(home.path(), &thread_id)
            .await
    ...
    async fn section_moves_persist_in_sqlite_without_changing_the_rollout() {
        ...
        store
            .move_thread_to_section(MoveThreadToSectionParams {
                thread_id,
                section: Some(workx_state::PINNED_THREAD_SECTION_ID.to_string()),
        ...
        let pinned_page = store
            .list_threads(ListThreadsParams {
                ...
                cwd_filters: None,
                section: Some(Some(workx_state::PINNED_THREAD_SECTION_ID.to_string())),
                project_id: None,
                archived: false,
                search_term: None,
    ...
    async fn paginated_name_updates_use_sqlite_without_rollout_writes() {
        ...
        assert_eq!(
            workx_rollout::find_thread_name_by_id(home.path(), &thread_id)
                .await
                .expect("find thread name")
                .as_deref(),
    ...
    async fn update_thread_metadata_sets_git_info() {
                ...
                patch: ThreadMetadataPatch {
                    git_info: Some(GitInfoPatch {
                        ...
                        origin_url: Some(Some(
                            SanitizedGitUrl::try_from("https://github.com/RonanXiao/workx")
        ...
        assert_eq!(
            git_info.repository_url.as_deref(),
            Some("https://github.com/RonanXiao/workx")
        );
    }
    ...
    async fn observed_metadata_normalizes_cwd_for_list_filters() {
        ...
        let normalized_cwd = workx_utils_path::normalize_for_path_comparison(workspace.as_path())

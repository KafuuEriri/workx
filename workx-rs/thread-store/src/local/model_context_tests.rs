...
async fn loads_latest_checkpoint_with_required_turn_metadata() {
    ...
    let thread_id = workx_protocol::ThreadId::from_string(&uuid.to_string()).expect("thread id");
...
async fn loads_recent_context_after_many_empty_wake_turns() {
    ...
    let session_meta = workx_rollout::read_session_meta_line(&path)
        .await
...
async fn fork_context_excludes_items_after_frozen_cutoff() {
    ...
    let session_meta = workx_rollout::read_session_meta_line(path.as_path())
        .await
...
async fn replays_nested_archived_lineage_from_frozen_prefix() {
    ...
    let child_meta = workx_rollout::read_session_meta_line(child_path.as_path())
        .await
...
fn completed_user_message(turn_id: &str, message: &str) -> RolloutItem {
    RolloutItem::EventMsg(EventMsg::ItemCompleted(ItemCompletedEvent {
        thread_id: workx_protocol::ThreadId::from_string("00000000-0000-0000-0000-000000000000")

...
use workx_protocol::realtime::BemItemPresentation;
use workx_protocol::realtime::RealtimeItem;
use workx_protocol::realtime::RealtimeItemContent;
use workx_protocol::realtime::RealtimeSessionOutcome;
use workx_protocol::realtime::RealtimeTranscriptRole;
use workx_rollout::RolloutConfig;
use workx_rollout::RolloutItem;
use workx_rollout::RolloutLine;
use workx_rollout::RolloutRecorder;
use workx_rollout::RolloutRecorderParams;
...
/// Separate Workx and SQLite homes must work together across startup backfill,
...
async fn split_homes_support_backfill_listing_and_paginated_history() {
    ...
    let workx_home = root.path().join("workx");
    ...
    let runtime = workx_rollout::state_db::try_init(&rollout_config)
        .await
        .expect("backfill state from Workx home");
    assert!(
        runtime
    ...
    for sqlite_path in [&state_db_path, &thread_history_db_path] {
        ...
        assert!(
            !workx_home.join(filename).exists(),
            "SQLite database should not be created under Workx home"
        );
    }
}
...
async fn paginated_live_append_materializes_turn_items_and_state() {
    ...
    let pool = workx_state::open_thread_history_db(&workx_state::SqliteConfig::new_for_testing(
        home.path().abs(),
    ))
    .await
...
async fn paginated_fork_reads_compressed_shared_lineage_without_materializing() {
    ...
    assert!(matches!(
        error,
        crate::ThreadStoreError::InvalidRequest { message } if message.contains("must be in Workx home")
    ));
    store
        .shutdown_thread(source_thread_id)
        .await
...
async fn summary_items_use_final_answers_and_ignore_commentary() {
    ...
    let config = test_config(home.path());
    let thread_id = ThreadId::default();
    let runtime = workx_state::StateRuntime::init(
        config.sqlite.clone(),
...
async fn catch_up_rejects_invalid_complete_suffixes_without_advancing_state() {
    ...
    for (name, suffix) in cases {
        ...
        let pool = workx_state::open_thread_history_db(
...
async fn projection_store(workx_home: &Path) -> LocalThreadStore {
    let config = test_config(workx_home);

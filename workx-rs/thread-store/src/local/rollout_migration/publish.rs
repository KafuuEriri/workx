...
pub(super) fn migration_journal_path(workx_home: &Path, thread_id: ThreadId) -> PathBuf {
    workx_home
...
pub(super) async fn pending_migration_thread_ids(
    ...
    let mut entries = match tokio::fs::read_dir(workx_home.join(MIGRATION_JOURNAL_DIRECTORY)).await
    {
        Ok(entries) => entries,

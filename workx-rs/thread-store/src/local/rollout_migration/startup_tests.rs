...
async fn records_and_advances_checked_thread() {
    ...
    assert_eq!(
        workx_rollout::read_session_meta_line(&legacy_path)
            .await
...
async fn checks_rollouts_within_the_cursor_lookback() {
    ...
    assert_eq!(
        workx_rollout::read_session_meta_line(&older_path)
            .await
            .expect("read migrated metadata")
            .meta
            .history_mode,
...
async fn recovers_pending_migrations_after_retrying_busy_rollouts() {
    ...
    assert_eq!(
        workx_rollout::read_session_meta_line(&busy_path)
            .await
            .expect("read migrated metadata")
            .meta
            .history_mode,
...
async fn waits_for_a_live_writer_before_migrating() {
    ...
    assert_eq!(
        workx_rollout::read_session_meta_line(&path)
            .await
            .expect("read migrated metadata")
            .meta
            .history_mode,
...
async fn waits_for_rollout_maintenance_before_migrating() {
    ...
    let path = write_rollout(home.path(), thread_id, ThreadHistoryMode::Legacy);
    let store = indexed_store(home.path()).await;
    let maintenance_guard = workx_rollout::try_acquire_rollout_maintenance_lock(home.path())
...
async fn permanently_skips_failed_rollouts_without_blocking_the_cursor() {
    ...
    let archived_directory = home.path().join(workx_rollout::ARCHIVED_SESSIONS_SUBDIR);
    ...
    assert_eq!(
        workx_rollout::read_session_meta_line(&archived_path)
            .await
...
async fn treats_writer_owned_empty_rollouts_as_busy() {
    ...
    let metadata = workx_rollout::builder_from_items(items.as_slice(), &path)

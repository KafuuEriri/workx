...
use workx_state::RolloutMigrationCursor;
use workx_state::RolloutMigrationSkippedRollout;
...
pub(super) async fn migrate_rollouts_on_startup(store: &LocalThreadStore) -> ThreadStoreResult<()> {
    ...
    let paths = find_all_rollout_paths(&store.config.workx_home).await?;
    ...
    skipped_rollouts = state_db
        ...
        .await
        .map_err(migration_error)?;
    if !pending_migration_thread_ids(&store.config.workx_home)
        .await?
        .is_empty()
    {
...
async fn migrate_all_rollouts(
    ...
    let pending_thread_ids = pending_migration_thread_ids(&store.config.workx_home).await?;
...
async fn retry_busy_rollouts(
    ...
    {
        let stored_path = store.config.workx_home.join(&skipped_rollout.rollout_path);
...
async fn run_startup_migration(
    ...
    loop {
        let Some(maintenance_guard) =
            workx_rollout::try_acquire_rollout_maintenance_lock(&store.config.workx_home)
                .map_err(migration_error)?
        else {
    ...
    match outcome.status {
        ...
        RolloutMigrationStatus::Failed => {
            if outcome.thread_id.is_some_and(|thread_id| {
                migration_journal_path(&store.config.workx_home, thread_id).exists()
            }) {
                return Ok(());
            }
...
fn relative_rollout_path(store: &LocalThreadStore, path: &Path) -> String {
    path.strip_prefix(&store.config.workx_home)
        .unwrap_or(path)
        .to_string_lossy()
        .replace('\\', "/")
}
...
fn skipped_rollout_file_names(
    ...
    skipped_rollouts
        ...
        .filter_map(|skipped_rollout| {
            plain_rollout_file_name(&store.config.workx_home.join(&skipped_rollout.rollout_path))
        })
        .collect()
}

fn plain_rollout_file_name(path: &Path) -> Option<OsString> {
    workx_rollout::plain_rollout_path(path)
        .file_name()

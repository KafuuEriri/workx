...
async fn resolve(
    ...
    if let Ok(path) = live_writer::rollout_path(store, thread_id).await
        && workx_rollout::existing_rollout_path(path.as_path())
            .await
            .is_some()
            ...
            Ok(Some(metadata)) => {
                    ...
                    workx_rollout::existing_rollout_path(metadata.rollout_path.as_path()).await
                {
                    let belongs_to_thread =
                        match workx_rollout::read_session_meta_line(path.as_path()).await {
...
fn location_for_path(store: &LocalThreadStore, path: &std::path::Path) -> RolloutLocation {
    if rollout_path_is_archived(store.config.workx_home.as_path(), path) {
...
async fn resolve_path(
    ...
    let rollout_id = match workx_rollout::rollout_id_from_path(path.as_path()) {
        Some(rollout_id) => rollout_id,
        None => {
            let history_mode = workx_rollout::read_session_meta_line(path.as_path())
                .await
...
pub(super) fn rollout_id_from_path_or_legacy_thread_id(
    ...
    Ok(match workx_rollout::rollout_id_from_path(path) {
        Some(rollout_id) => rollout_id,
        None => {

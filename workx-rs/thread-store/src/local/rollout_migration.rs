...
use workx_app_server_protocol::project_rollout_line;
...
impl LocalThreadStore {
    ...
    async fn migrate_rollouts_with_progress_inner(
        ...
        let _maintenance_guard = match options.mode {
            ...
            RolloutMigrationMode::Apply => Some(
                workx_rollout::try_acquire_rollout_maintenance_lock(&self.config.workx_home)
        ...
        let mut paths = match paths {
            RolloutMigrationPaths::Discover => {
                find_all_rollout_paths(&self.config.workx_home).await?
            }
            RolloutMigrationPaths::Known(paths) => paths,
        };
        if options.mode == RolloutMigrationMode::Apply {
            let pending_thread_ids = pending_migration_thread_ids(&self.config.workx_home).await?;
        ...
        let legacy_names = if options.mode == RolloutMigrationMode::Apply {
            let thread_ids = paths
                ...
                .collect();
            workx_rollout::find_thread_names_by_ids(&self.config.workx_home, &thread_ids)
                .await
                .unwrap_or_default()
        } else {
            HashMap::new()
    ...
    async fn migrate_rollout_path(
        ...
        let metadata = loop {
            let error = match workx_rollout::read_session_meta_line(&path).await {
                Ok(metadata) => break metadata,
                Err(error) if !retried_moved_path && error.kind() == io::ErrorKind::NotFound => {
                    // A different Workx process can archive or compress this rollout after path
                    ...
                    if let Some(current_path) =
                        find_current_rollout_path(&self.config.workx_home, &path).await?
                    {
                        path = current_path;
                ...
                && let Some(thread_id) = thread_id
            {
                ...
                match workx_rollout::read_session_meta_line(&path).await {
                    Ok(metadata) => break metadata,
                    Err(error) => error,
                }
            } else {
        ...
        let journal_path = migration_journal_path(&self.config.workx_home, thread_id);
        ...
        let publish_result = async {
                ...
                || current_source_metadata.modified().ok() != source_modified
            {
                return Err(ThreadStoreError::Conflict {
                    message: "rollout changed while migration was staging it; close older Workx processes and retry".to_string(),
                });
            }
    ...
    async fn write_canonical_rollout(
        ...
        while let Some(record) = read_rollout_record(&mut source, &mut bytes).await? {
            ...
            if plan.is_none()
                && matches!(
                    &line.item,
                    RolloutItem::EventMsg(workx_protocol::protocol::EventMsg::ThreadRolledBack(_))
                )
            {
    ...
    async fn cleanup_failed_unpublished_migration(
        ...
        let Ok(metadata) = workx_rollout::read_session_meta_line(rollout_path).await else {
            return Ok(());
        };
        if metadata.meta.history_mode == ThreadHistoryMode::Paginated {
            return Ok(());
    ...
    async fn project_rollout_in_batches(
        ...
        let subagent_history_start_ordinal = workx_rollout::read_session_meta_line(rollout_path)
            .await
            .map_err(migration_error)?
            .meta
        ...
        while let Some(entry) = entries.next_entry().await.map_err(migration_error)? {
                ...
                && (name.ends_with(".jsonl") || name.ends_with(".jsonl.zst"))
            {
                ...
                if name.ends_with(".jsonl.zst")
                    && tokio::fs::try_exists(workx_rollout::plain_rollout_path(&path))
                        .await
                        .map_err(migration_error)?
                {
                    continue;
...
async fn find_all_rollout_paths(workx_home: &Path) -> ThreadStoreResult<Vec<PathBuf>> {
    let mut paths = find_rollout_paths(&workx_home.join(workx_rollout::SESSIONS_SUBDIR)).await?;
    paths.extend(
        find_rollout_paths(&workx_home.join(workx_rollout::ARCHIVED_SESSIONS_SUBDIR)).await?,
    );
    Ok(paths)
}
...
async fn find_current_rollout_path(
    ...
    let plain_path = workx_rollout::plain_rollout_path(stale_path);
    let Some(file_name) = plain_path.file_name() else {
        return Ok(None);
    };
    Ok(find_all_rollout_paths(workx_home)
        .await?
        .into_iter()
        .find(|candidate| {
            workx_rollout::plain_rollout_path(candidate).file_name() == Some(file_name)
        }))
}

...
async fn revert_keeps_thread_id_and_hides_suffix_across_repeated_reverts() {
    ...
    assert_ne!(
        workx_rollout::rollout_id_from_path(first_replacement_path.as_path()),
        Some(thread_id)
    );
    let replacement_meta = workx_rollout::read_session_meta_line(first_replacement_path.as_path())
        .await
...
async fn rollout_paths_for_thread(
    ...
    workx_rollout::RolloutReferenceIndex::scan(home)
        .await

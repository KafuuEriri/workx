...
pub(super) async fn revert(
    ...
    for segment in &mut lineage.segments {
        if segment.rollout_id() == current_rollout.rollout_id && source_meta.history_base.is_none()
        {
            segment.rollout_path =
                workx_rollout::materialize_rollout_for_reference(segment.rollout_path.as_path())
                    .await
                    .map_err(|err| ThreadStoreError::Internal {
                        message: format!(
    ...
    let forked_from_ordinal_exclusive =
        workx_rollout::forked_from_ordinal_exclusive(&source_meta, Some(source_path.as_path()))
            .map(|cutoff| {
...
async fn create_replacement_recorder(
    store: &LocalThreadStore,
    source_meta: workx_rollout::SessionMeta,
    rollout_id: ThreadId,
    history_base: Option<workx_protocol::protocol::HistoryPosition>,

...
use workx_rollout::parse_cursor;
use workx_state::ThreadFilterOptions;
...
pub(super) async fn list_threads(
    ...
    let sort_key = match params.sort_key {
        ThreadSortKey::CreatedAt => workx_rollout::ThreadSortKey::CreatedAt,
        ThreadSortKey::UpdatedAt => workx_rollout::ThreadSortKey::UpdatedAt,
        ThreadSortKey::RecencyAt => workx_rollout::ThreadSortKey::RecencyAt,
    ...
    let sort_direction = match params.sort_direction {
        SortDirection::Asc => workx_rollout::SortDirection::Asc,
        SortDirection::Desc => workx_rollout::SortDirection::Desc,
    };
    let state_db = store.state_db().await;
    let rollout_config = RolloutConfig {
        workx_home: store.config.workx_home.clone(),
        sqlite: store.config.sqlite.clone(),
        cwd: store.config.workx_home.clone(),
...
async fn list_section_threads(
    ...
    let anchor = params
        .cursor
        .as_deref()
        .map(|cursor| -> ThreadStoreResult<workx_state::Anchor> {
            ...
            let thread_id = workx_protocol::ThreadId::from_string(thread_id).map_err(|_| {
                ThreadStoreError::InvalidRequest {
                    message: format!("invalid cursor: {cursor}"),
                }
            })?;
            Ok(workx_state::Anchor {
                ts: timestamp,
                id: Some(thread_id),
            })
        })
    ...
    let normalized_cwd_filters = params.cwd_filters.as_ref().map(|filters| {
        filters
            .iter()
            .map(|cwd| workx_rollout::state_db::normalize_cwd_for_state_db(cwd))
            .collect::<Vec<_>>()
    });
    let filters = ThreadFilterOptions {
        ...
        sort_key: workx_state::SortKey::SectionPosition,
        sort_direction: match params.sort_direction {
            SortDirection::Asc => workx_state::SortDirection::Asc,
            SortDirection::Desc => workx_state::SortDirection::Desc,
        },
    ...
    let page = match params.relation_filter {
        Some(ThreadRelationFilter::DirectChildrenOf(thread_id)) => {
            state_db
                .list_threads_by_relation(
                    params.page_size,
                    workx_state::ThreadRelationFilter::DirectChildrenOf(thread_id),
                    filters,
                )
                .await
        }
        Some(ThreadRelationFilter::DescendantsOf(thread_id)) => {
            state_db
                .list_threads_by_relation(
                    params.page_size,
                    workx_state::ThreadRelationFilter::DescendantsOf(thread_id),
                    filters,
                )
                .await
        }
...
pub(super) async fn list_rollout_threads(
    state_db: Option<workx_rollout::StateDbHandle>,
    config: &RolloutConfig,
    default_model_provider_id: &str,
    params: &ListThreadsParams,
    cursor: Option<&workx_rollout::Cursor>,
    sort_key: workx_rollout::ThreadSortKey,
    sort_direction: workx_rollout::SortDirection,
) -> ThreadStoreResult<workx_rollout::ThreadsPage> {
        ...
        let relation_filter = params
            ...
            .map(|relation_filter| match relation_filter {
                ThreadRelationFilter::DirectChildrenOf(parent_thread_id) => {
                    workx_state::ThreadRelationFilter::DirectChildrenOf(parent_thread_id)
                }
                ThreadRelationFilter::DescendantsOf(ancestor_thread_id) => {
                    workx_state::ThreadRelationFilter::DescendantsOf(ancestor_thread_id)
                }
            });
        let page = workx_rollout::state_db::list_threads_db(
            state_db.as_deref(),
            &config.sqlite,
            params.page_size,
            cursor,
...
mod tests {
    ...
    async fn section_listing_uses_sqlite_metadata_without_reading_rollouts() {
        ...
        for index in 0..3 {
            ...
            workx_rollout::state_db::reconcile_rollout(
                Some(state.as_ref()),

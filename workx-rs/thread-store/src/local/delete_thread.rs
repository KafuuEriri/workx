...
use workx_rollout::RolloutReferenceIndex;
use workx_rollout::SESSIONS_SUBDIR;
use workx_rollout::find_archived_thread_path_by_id_str;
use workx_rollout::find_thread_path_by_id_str;
use workx_rollout::remove_thread_name_entries;
...
struct ThreadRollouts {
    ...
    rollout_ids: HashSet<workx_protocol::ThreadId>,
    paths: Vec<PathBuf>,
}
...
impl ThreadRollouts {
    ...
    fn add_path(&mut self, path: PathBuf) {
        if let Some(rollout_id) = workx_rollout::rollout_id_from_path(path.as_path()) {
...
async fn scan_reference_index(
    ...
    RolloutReferenceIndex::scan(store.config.workx_home.as_path())
        .await
...
fn referenced_thread_error(thread_id: workx_protocol::ThreadId) -> ThreadStoreError {
...
async fn delete_thread_after_reference_check(
    ...
    remove_thread_name_entries(store.config.workx_home.as_path(), thread_id)
        .await
        .map_err(|err| ThreadStoreError::Internal {
...
fn delete_rollout_file(store: &LocalThreadStore, rollout_path: &Path) -> ThreadStoreResult<bool> {
    let plain_path = workx_rollout::plain_rollout_path(rollout_path);
...
fn delete_rollout_path(store: &LocalThreadStore, rollout_path: &Path) -> ThreadStoreResult<bool> {
    let canonical_rollout_path = scoped_rollout_path(
        store.config.workx_home.join(SESSIONS_SUBDIR),
        rollout_path,
        "sessions",
    )
    .or_else(|_| {
        scoped_rollout_path(
            store.config.workx_home.join(ARCHIVED_SESSIONS_SUBDIR),
            rollout_path,
            "archived sessions",
        )
    })
...
mod tests {
    ...
    async fn delete_thread_without_state_db_preserves_materialized_thread_history() {
        ...
        .expect("session file");
        let pool = workx_state::open_thread_history_db(&config.sqlite)
            .await

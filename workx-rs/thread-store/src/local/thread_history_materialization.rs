...
use tracing::warn;
use workx_app_server_protocol::ThreadHistoryChangeSet;
...
pub(super) async fn materialize_to_sqlite(
    ...
    if projection_state.is_none()
        && workx_rollout::existing_rollout_path(rollout_path)
            .await
            .is_none()
    {
        return Ok(());
...
async fn read_projection_steps(
    ...
    let file =
        tokio::task::spawn_blocking(move || workx_rollout::open_rollout_seekable_reader(&path))
            .await
    ...
    for line_bytes in bytes[..complete_byte_count].split_inclusive(|byte| *byte == b'\n') {
        ...
        let line = match workx_rollout::decode_rollout_line(value) {
            Ok(line) => Some(line),
            Err(err) => {
                warn!(
                    thread_id = %thread_id,

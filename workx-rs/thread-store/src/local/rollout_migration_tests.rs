...
use workx_app_server_protocol::build_turns_from_rollout_items;
...
use workx_protocol::mcp::McpResourceOrigin;
...
fn move_to_archived(home: &Path, path: PathBuf) -> PathBuf {
    let directory = home.join(workx_rollout::ARCHIVED_SESSIONS_SUBDIR);
...
async fn indexed_store(home: &Path) -> LocalThreadStore {
    ...
    let state_db = workx_rollout::state_db::try_init(&rollout_config)
        .await
...
async fn migration_publishes_canonical_projected_history_and_is_idempotent() {
    let home = TempDir::new().expect("create Workx home");
    ...
    let replacement_histories = read_rollout(&path)
        ...
        .filter_map(|line| match line.item {
            RolloutItem::Compacted(item) => item.replacement_history.map(|items| {
                items
                    .into_iter()
                    .map(workx_rollout::ResponseItemEnvelope::into_item)
                    .collect::<Vec<_>>()
            }),
            _ => None,
        })
...
async fn migration_preserves_compressed_rollouts_during_publish_and_recovery() {
    ...
    assert_eq!(
        workx_rollout::read_session_meta_line(&compressed_path)
            .await
...
async fn migration_preserves_legacy_displayed_thread_names() {
    ...
    workx_rollout::append_thread_name(home.path(), title_thread_id, "stale index title")
        .await
        .expect("write stale legacy index name");
    workx_rollout::append_thread_name(home.path(), index_thread_id, "indexed title")
        .await
...
async fn migration_apply_conflicts_with_rollout_maintenance() {
    ...
    let _maintenance_guard = workx_rollout::try_acquire_rollout_maintenance_lock(home.path())

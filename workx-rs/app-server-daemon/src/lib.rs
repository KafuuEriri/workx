...
use managed_install::managed_workx_bin;
#[cfg(unix)]
use managed_install::managed_workx_version;
use serde::Serialize;
...
use workx_app_server_transport::app_server_control_socket_path;
...
pub struct LifecycleOutput {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub pid: Option<u32>,
    pub managed_workx_path: PathBuf,
    pub managed_workx_version: Option<String>,
...
fn ensure_supported_platform() -> Result<()> {
    Err(anyhow!(
        "workx app-server daemon lifecycle is only supported on Unix platforms"
    ))
}
...
struct Daemon {
    ...
    managed_workx_bin: PathBuf,
}

impl Daemon {
    fn from_environment() -> Result<Self> {
        let workx_home = find_workx_home().context("failed to resolve WORKX_HOME")?;
        let socket_path = app_server_control_socket_path(workx_home.as_path())?
            .as_path()
            .to_path_buf();
        let state_dir = workx_home.as_path().join(STATE_DIR_NAME);
        Ok(Self {
            socket_path,
            ...
            managed_workx_bin: managed_workx_bin(workx_home.as_path()),
        })
    }
    ...
    async fn restart(&self) -> Result<LifecycleOutput> {
        ...
        {
            return Err(anyhow!(
                "app server is running but is not managed by workx app-server daemon"
            ));
        }
    ...
    pub(crate) async fn try_restart_if_running(
        ...
        let outcome = if let Some(backend) = self.running_backend_instance(&settings).await? {
            ...
            let managed_version = if info.is_some() {
                Some(managed_workx_version(managed_workx_bin).await?)
            } else {
                None
            };
            match restart_decision(mode, info.as_ref(), managed_version.as_deref()) {
                ...
                RestartDecision::Restart => {
                    backend.stop().await?;
                    let _ = self
                        .start_managed_backend_with_bin(&settings, managed_workx_bin)
                        .await?;
        ...
        if should_reexec_updater(updater_refresh_mode, outcome) {
            crate::update_loop::reexec_managed_updater(managed_workx_bin)?;
        }
    ...
    async fn append_daemon_app_server_context(&self, context: &mut String) {
        let managed_workx_version = self
            .managed_workx_version_best_effort()
            .await
            .unwrap_or_else(|| "unknown".to_string());
        context.push_str(&format!(
            "\n\nDaemon used app-server:\n  path: {}\n  version: {managed_workx_version}",
            self.managed_workx_bin.display()
        ));
    }
    ...
    async fn bootstrap_locked(&self, options: BootstrapOptions) -> Result<BootstrapOutput> {
        ...
        let info = self.wait_until_ready().await?;
        let managed_workx_version = self.managed_workx_version_best_effort().await;
        Ok(BootstrapOutput {
            status: BootstrapStatus::Bootstrapped,
            backend: BackendKind::Pid,
            auto_update_enabled: true,
            remote_control_enabled: settings.remote_control_enabled,
            managed_workx_path: self.managed_workx_bin.clone(),
            managed_workx_version,
    ...
    async fn start_managed_backend(&self, settings: &DaemonSettings) -> Result<Option<u32>> {
        self.start_managed_backend_with_bin(settings, &self.managed_workx_bin)
            .await
    }
    ...
    async fn start_managed_backend_with_bin(
        ...
        let backend =
            backend::pid_backend(self.backend_paths_with_bin(settings, managed_workx_bin));
        backend.start().await
    }
    ...
    fn ensure_managed_workx_bin(&self) -> Result<()> {
        if self.managed_workx_bin.is_file() {
            return Ok(());
        }

        let managed_workx_path = self.managed_workx_bin.display();
        Err(anyhow!(
            "managed standalone Workx install not found at {managed_workx_path}\n\n\
             This command requires the standalone install managed by the Workx installer, because \
    ...
    #[cfg(unix)]
    async fn managed_workx_version_best_effort(&self) -> Option<String> {
        managed_workx_version(&self.managed_workx_bin).await.ok()
    }
    ...
    fn backend_paths(&self, settings: &DaemonSettings) -> BackendPaths {
        self.backend_paths_with_bin(settings, &self.managed_workx_bin)
    }
    ...
    fn backend_paths_with_bin(
        ...
        BackendPaths {
            workx_bin: managed_workx_bin.to_path_buf(),
...
mod tests {
    ...
    fn remote_control_start_output_serializes_inner_output_without_tag() {
        let lifecycle_output = LifecycleOutput {
            ...
            pid: None,
            managed_workx_path: "workx".into(),
            managed_workx_version: Some("1.2.3".to_string()),
            socket_path: "workx.sock".into(),
        ...
        assert_eq!(
            ...
            serde_json::json!({
                ...
                "backend": "pid",
                "managedWorkxPath": "workx",
                "managedWorkxVersion": "1.2.3",
                "socketPath": "workx.sock",
                "cliVersion": "1.2.3",
    ...
    async fn not_ready_context_reports_daemon_app_server_before_stderr() {
        ...
        let daemon = Daemon {
            ...
            managed_workx_bin: temp_dir.path().join("missing-workx"),
        };
        ...
        assert_eq!(
            ...
            format!(
                ...
                daemon.managed_workx_bin.display(),
                stderr_log.display()
            )
        );
    }

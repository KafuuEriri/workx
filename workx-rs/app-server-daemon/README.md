# workx-app-server-daemon

> `workx-app-server-daemon` is experimental and its lifecycle contract may
> change while the remote-management flow is still being developed.

`workx-app-server-daemon` backs the machine-readable `workx app-server`
lifecycle commands used by remote clients such as the desktop and mobile apps.
It is intended for Workx instances launched over SSH, including fresh developer
...
```sh
workx app-server daemon start
workx app-server daemon restart
workx app-server daemon enable-remote-control
workx app-server daemon disable-remote-control
workx app-server daemon stop
workx app-server daemon version
workx app-server daemon bootstrap --remote-control
```
...
$HOME/.workx/packages/standalone/current/workx app-server daemon bootstrap --remote-control
```

`bootstrap` requires the standalone managed install. It records the daemon
settings under `WORKX_HOME/app-server-daemon/`, starts app-server as a
pidfile-backed detached process, and launches a detached updater loop.

## Installation and update cases

The daemon assumes Workx is installed through `install.sh` and always launches
the standalone managed binary under `WORKX_HOME`.

| Situation | What starts | Does this daemon fetch new binaries? | Does a running app-server eventually move to a newer binary on its own? |
| --- | --- | --- | --- |
| `install.sh` has run, but only `start` is used | `start` uses `WORKX_HOME/packages/standalone/current/workx` | No | No. The managed path is used when starting or restarting, but no updater is installed. |
| `install.sh` has run, then `bootstrap` is used | The pidfile backend uses `WORKX_HOME/packages/standalone/current/workx` | Yes. Bootstrap launches a detached updater loop that runs `install.sh` hourly. | Yes, while that updater process is alive and app-server is already running. After a successful fetch, the updater restarts app-server with the refreshed binary and only then replaces its own process image. |
...
Top-level `workx remote-control` bootstraps with `--remote-control` when the
...
All mutating lifecycle commands are serialized per `WORKX_HOME`, so a concurrent
...
The daemon stores its local state under `WORKX_HOME/app-server-daemon/`:

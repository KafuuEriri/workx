...
pub enum Command {
    /// Submit a new Workx Cloud task without launching the TUI.
    Exec(ExecCommand),
    /// Show the status of a Workx Cloud task.
    Status(StatusCommand),
    /// List Workx Cloud tasks.
    List(ListCommand),
    /// Apply the diff for a Workx Cloud task locally.
    Apply(ApplyCommand),
    /// Show the unified diff for a Workx Cloud task.
    Diff(DiffCommand),
}

#[derive(Debug, Args)]
pub struct ExecCommand {
    /// Task prompt to run in Workx Cloud.
    #[arg(value_name = "QUERY")]
    pub query: Option<String>,

    /// Target environment identifier (see `workx cloud` to browse).
    ...
    /// Git branch to run in Workx Cloud (defaults to current branch).
...
pub struct StatusCommand {
    /// Workx Cloud task identifier to inspect.
...
pub struct ApplyCommand {
    /// Workx Cloud task identifier to apply.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
...
pub struct DiffCommand {
    /// Workx Cloud task identifier to display.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,

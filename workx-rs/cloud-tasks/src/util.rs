...
pub fn set_user_agent_suffix(suffix: &str) {
    if let Ok(mut guard) = workx_login::default_client::USER_AGENT_SUFFIX.lock() {
...
pub(crate) fn validate_chatgpt_base_url(input: &str) -> anyhow::Result<String> {
    let invalid_url = || {
        anyhow::anyhow!(
            "WORKX_CLOUD_TASKS_BASE_URL must use a trusted HTTPS origin on port 443, without user information, a query, or a fragment; custom backends cannot use saved ChatGPT credentials"
        )
    };
...
pub async fn build_chatgpt_headers() -> HeaderMap {
    ...
    set_user_agent_suffix("workx_cloud_tasks_tui");
    let ua = workx_login::default_client::get_workx_user_agent();
    let mut headers = HeaderMap::new();
    headers.insert(
        USER_AGENT,
        HeaderValue::from_str(&ua).unwrap_or(HeaderValue::from_static("workx-cli")),
    );
    if let Some(am) = load_auth_manager(/*chatgpt_base_url*/ None).await.0
        && let Some(auth) = am.auth().await
        && auth.uses_workx_backend()
    {
        headers.extend(workx_model_provider::auth_provider_from_auth(&auth).to_auth_headers());
    }
    headers
}
...
pub fn task_url(base_url: &str, task_id: &str) -> String {
    ...
    if let Some(root) = normalized.strip_suffix("/backend-api") {
        return format!("{root}/workx/tasks/{task_id}");
    }
    if let Some(root) = normalized.strip_suffix("/api/codex") {
        return format!("{root}/workx/tasks/{task_id}");
    }
    if normalized.ends_with("/workx") {
        return format!("{normalized}/tasks/{task_id}");
    }
    format!("{normalized}/workx/tasks/{task_id}")
}

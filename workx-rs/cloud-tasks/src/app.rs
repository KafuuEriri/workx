...
pub struct EnvironmentRow {
    ...
    pub is_pinned: bool,
    pub repo_hints: Option<String>, // e.g., "RonanXiao/workx"
}
...
use workx_cloud_tasks_client::CloudBackend;
...
pub struct AttemptView {
    ...
    pub status: workx_cloud_tasks_client::AttemptStatus,
...
pub enum AppEvent {
    ...
    DetailsMessagesLoaded {
        ...
        attempt_status: workx_cloud_tasks_client::AttemptStatus,
    },
    DetailsFailed {
        id: TaskId,
        title: String,
    ...
    AttemptsLoaded {
        id: TaskId,
        attempts: Vec<workx_cloud_tasks_client::TurnAttempt>,
    },
    /// Background completion of new task submission
    NewTaskSubmitted(Result<workx_cloud_tasks_client::CreatedTask, String>),
    ...
    ApplyFinished {
        id: TaskId,
        result: std::result::Result<workx_cloud_tasks_client::ApplyOutcome, String>,
    },
}
...
mod tests {
    use super::*;
    use chrono::Utc;
    use workx_cloud_tasks_client::CloudBackendFuture;
    use workx_cloud_tasks_client::CloudTaskError;
    ...
    impl FakeBackend {
        async fn list_tasks(
            ...
            limit: Option<i64>,
            cursor: Option<&str>,
        ) -> Result<workx_cloud_tasks_client::TaskListPage, CloudTaskError> {
            ...
            for (i, t) in titles.into_iter().enumerate() {
                out.push(TaskSummary {
                    ...
                    title: t.to_string(),
                    status: workx_cloud_tasks_client::TaskStatus::Ready,
                    updated_at: Utc::now(),
                    environment_id: env.map(str::to_string),
                    environment_label: None,
                    summary: workx_cloud_tasks_client::DiffSummary::default(),
                    is_review: false,
            ...
            Ok(workx_cloud_tasks_client::TaskListPage {
                tasks: limited,
        ...
        async fn get_task_text(
            &self,
            _id: TaskId,
        ) -> Result<workx_cloud_tasks_client::TaskText, CloudTaskError> {
            Ok(workx_cloud_tasks_client::TaskText {
                prompt: Some("Example prompt".to_string()),
                messages: Vec::new(),
                turn_id: Some("fake-turn".to_string()),
                sibling_turn_ids: Vec::new(),
                attempt_placement: Some(0),
                attempt_status: workx_cloud_tasks_client::AttemptStatus::Completed,
            })
        }
    }

    impl workx_cloud_tasks_client::CloudBackend for FakeBackend {
        fn list_tasks<'a>(
            &'a self,
            env: Option<&'a str>,
            limit: Option<i64>,
            cursor: Option<&'a str>,
        ) -> CloudBackendFuture<'a, workx_cloud_tasks_client::TaskListPage> {
        ...
        fn get_task_diff(&self, _id: TaskId) -> CloudBackendFuture<'_, Option<String>> {
            Box::pin(async {
                Err(workx_cloud_tasks_client::CloudTaskError::Unimplemented(
                    "not used in test",
                ))
            })
        }
        ...
        fn get_task_text(
            &self,
            id: TaskId,
        ) -> CloudBackendFuture<'_, workx_cloud_tasks_client::TaskText> {
        ...
        fn list_sibling_attempts(
            ...
            _task: TaskId,
            _turn_id: String,
        ) -> CloudBackendFuture<'_, Vec<workx_cloud_tasks_client::TurnAttempt>> {
        ...
        ) -> CloudBackendFuture<'_, workx_cloud_tasks_client::ApplyOutcome> {
            Box::pin(async {
                Err(workx_cloud_tasks_client::CloudTaskError::Unimplemented(
                    "not used in test",
                ))
        ...
        fn create_task<'a>(
            ...
            _qa_mode: bool,
            _best_of_n: usize,
        ) -> CloudBackendFuture<'a, workx_cloud_tasks_client::CreatedTask> {
            Box::pin(async {
                Err(workx_cloud_tasks_client::CloudTaskError::Unimplemented(
                    "not used in test",
                ))

//! Proxy-aware WebSocket connection setup shared by Workx API clients.
...
use workx_http_client::BuildCustomCaTransportError;
use workx_http_client::HttpClientFactory;
use workx_http_client::build_rustls_client_config_with_custom_ca;
...
/// Selects whether WebSocket TLS follows Workx custom-CA policy or Tungstenite defaults.
#[derive(Clone, Copy, PartialEq, Eq)]
pub enum WebSocketTlsMode {
    /// Build an explicit TLS configuration from native roots and configured Workx custom CAs.
    ExplicitWorkxTls,
...
impl WebSocketConnector {
    /// Creates a connector using native roots and any configured Workx custom CA bundle.
    pub fn new(
        http_client_factory: &HttpClientFactory,
    ) -> Result<Self, BuildCustomCaTransportError> {
        Self::new_with_tls_mode(http_client_factory, WebSocketTlsMode::ExplicitWorkxTls)
    }

    /// Creates a connector with explicit Workx TLS or the transport's existing TLS defaults.
    ///
    /// HTTPS proxy connections still build Workx TLS configuration when they establish their
    ...
    pub fn new_with_tls_mode(
        ...
        let tls_config = match tls_mode {
            WebSocketTlsMode::ExplicitWorkxTls => {

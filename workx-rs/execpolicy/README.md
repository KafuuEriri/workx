...
- From the Workx CLI, run `workx execpolicy check` subcommand with one or more policy files (for example `src/default.rules`) to check a command:

```bash
workx execpolicy check --rules path/to/policy.rules git status
```

- To opt into basename fallback for absolute program paths, pass `--resolve-host-executables`:

```bash
workx execpolicy check \
...
```bash
cargo run -p workx-execpolicy -- check --rules path/to/policy.rules git status
```

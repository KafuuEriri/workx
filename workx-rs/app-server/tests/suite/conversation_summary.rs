...
async fn get_conversation_summary_by_thread_id_reads_pathless_store_thread() -> Result<()> {
    ...
    create_config_toml_with_in_memory_thread_store(workx_home.path(), &store_id)?;
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn get_conversation_summary_by_relative_rollout_path_resolves_from_workx_home() -> Result<()>
{
    let workx_home = TempDir::new()?;
    let conversation_id = create_fake_rollout(
        workx_home.path(),
    ...
    let relative_path = rollout_path.strip_prefix(workx_home.path())?.to_path_buf();

...
"#
    ...
    let state = StateRuntime::init(
        SqliteConfig::new_for_testing(workx_home.path().abs()),
        "mock_provider".into(),
    )
    .await?;
...
fn standalone_app_server_emits_json_info_events() -> Result<()> {
    let workx_home = TempDir::new()?;
    let event = app_server_json_shutdown_event("workx-app-server", &[], workx_home.path())?;
...
async fn sqlite_log_metrics_exports_do_not_create_log_cycles() -> Result<()> {
    let quiet_period = workx_state::log_db::LogSinkQueueConfig::default().flush_interval
    ...
    for status in [200, 500] {
        ...
        let _app_server = TestAppServer::builder()
            ...
            .with_env_overrides(&[
                ...
                (
                    workx_app_server_transport::REMOTE_CONTROL_DISABLED_ENV_VAR,
                    Some("1"),
                ),
            ])
        ...
        let exports = timeout(export_timeout, async {
            loop {
                ...
                if requests.iter().any(|request| {
                    String::from_utf8_lossy(&request.body).contains(workx_state::LOG_WRITE_METRIC)
                }) {
                    break requests.len();
                }
        ...
        let expected_metrics = [
            (workx_state::LOG_WRITE_METRIC, "sum"),
            (workx_state::LOG_WRITE_DURATION_METRIC, "histogram"),
            (workx_state::LOG_WRITE_BYTES_METRIC, "histogram"),
            (workx_state::LOG_WRITE_ENTRIES_METRIC, "histogram"),
            (workx_state::LOG_WRITE_MAX_ENTRY_BYTES_METRIC, "histogram"),
        ];
        ...
        for (name, kind) in expected_metrics {
            ...
            for point in points {
                ...
                if matches!(
                    name,
                    workx_state::LOG_WRITE_BYTES_METRIC
                        | workx_state::LOG_WRITE_MAX_ENTRY_BYTES_METRIC
                ) {
        ...
        let write_count = observed_metrics[workx_state::LOG_WRITE_METRIC].0;
        ...
        let state = workx_state::StateRuntime::init(
            workx_state::SqliteConfig::new_for_testing(workx_home.path().abs()),
            "test-provider".to_string(),
        )
        .await?;
        let persisted_logs = state.query_logs(&workx_state::LogQuery::default()).await?;
        assert_eq!(
            observed_metrics[workx_state::LOG_WRITE_ENTRIES_METRIC].1,
            persisted_logs.len() as f64,
            "exported batch sizes must equal the number of persisted SQLite log rows"
        );
        assert!(
            observed_metrics[workx_state::LOG_WRITE_BYTES_METRIC].1
                >= observed_metrics[workx_state::LOG_WRITE_MAX_ENTRY_BYTES_METRIC].1,
...
async fn sqlite_log_metrics_grpc_exports_do_not_create_log_cycles(grpc_status: &str) -> Result<()> {
    ...
    Mock::given(method("POST"))
        ...
        .respond_with(move |request: &wiremock::Request| {
            let metric_name = workx_state::LOG_WRITE_METRIC.as_bytes();
            if request
                .body
...
async fn app_server_emits_structured_tool_call_timing_event() -> Result<()> {
    ...
    let mut app_server = TestAppServer::builder()
        .with_workx_home(workx_home.path())
        .with_json_logging("warn,workx_core::tools::parallel=info")
        .build_initialized()
        .await?;
    ...
    let mut tool_call = app_server
        .wait_for_json_log_event("workx.tool_call")
        .await?;
    ...
    assert_eq!(
        ...
        json!({
            ...
            "target": "workx_core::tools::parallel",
        })
    );

...
use workx_app_server::AppServerRuntimeOptions;
use workx_app_server::AppServerTransport;
use workx_app_server::AppServerWebsocketAuthSettings;
use workx_app_server::PluginStartupTasks;
use workx_app_server::RemoteControlStartupMode;
use workx_app_server::run_main_with_transport_options;
...
use workx_app_server_protocol::RemoteControlClient;
use workx_app_server_protocol::RemoteControlClientsListOrder;
use workx_app_server_protocol::RemoteControlClientsListParams;
use workx_app_server_protocol::RemoteControlClientsListResponse;
use workx_app_server_protocol::RemoteControlClientsRevokeParams;
use workx_app_server_protocol::RemoteControlClientsRevokeResponse;
use workx_app_server_protocol::RemoteControlConnectionStatus;
use workx_app_server_protocol::RemoteControlDisableResponse;
use workx_app_server_protocol::RemoteControlEnableResponse;
use workx_app_server_protocol::RemoteControlPairingStartParams;
use workx_app_server_protocol::RemoteControlPairingStartResponse;
use workx_app_server_protocol::RemoteControlPairingStatusParams;
use workx_app_server_protocol::RemoteControlPairingStatusResponse;
use workx_app_server_protocol::RemoteControlStatusChangedNotification;
use workx_app_server_protocol::RemoteControlStatusReadResponse;
...
use workx_state::RemoteControlEnrollmentRecord;
...
async fn explicit_remote_control_startup_fails_when_disabled_by_requirements() -> Result<()> {
    ...
    let managed_config_path = workx_home.path().join("managed_config.toml");
    let socket_path = workx_home.path().join("app-server.sock");
    let transport =
        AppServerTransport::from_listen_url(&format!("unix://{}", socket_path.display()))?;
    let _workx_home_guard = EnvVarGuard::set("WORKX_HOME", workx_home.path().as_os_str());
...
async fn listen_off_honors_persisted_remote_control_enable() -> Result<()> {
    let workx_home = TempDir::new()?;
    let listener = configured_remote_control_listener(workx_home.path()).await?;
...
async fn remote_control_disable_returns_disabled_status() -> Result<()> {
    let workx_home = TempDir::new()?;
    let _listener = configured_remote_control_listener(workx_home.path()).await?;
    let mut mcp = TestAppServer::builder()
        .with_workx_home(workx_home.path())
        .without_auto_env()
        .build_initialized()
...
async fn remote_control_enable_returns_connecting_status() -> Result<()> {
    let workx_home = TempDir::new()?;
    let mut backend = BlockingRemoteControlBackend::start(workx_home.path()).await?;
    let mut mcp = TestAppServer::builder()
        .with_workx_home(workx_home.path())
        .without_auto_env()
        .build_initialized()
...
async fn stdio_eof_exits_with_remote_control_connection() -> Result<()> {
    let workx_home = TempDir::new()?;
    let mut backend = ConnectedRemoteControlBackend::start(workx_home.path()).await?;
    let mut app_server = TestAppServer::builder()
        .with_workx_home(workx_home.path())
        .without_auto_env()
        .build_initialized()
...
async fn remote_control_pairing_start_returns_pairing_artifacts() -> Result<()> {
    let workx_home = TempDir::new()?;
    let mut backend = PairingRemoteControlBackend::start(workx_home.path()).await?;
    let mut mcp = TestAppServer::builder()
        .with_workx_home(workx_home.path())
        .without_auto_env()
        .build_initialized()
...
async fn remote_control_client_management_works_while_disabled() -> Result<()> {
    let workx_home = TempDir::new()?;
    let mut backend = ClientManagementRemoteControlBackend::start(workx_home.path()).await?;
    let mut mcp = TestAppServer::builder()
        .with_workx_home(workx_home.path())
        .without_auto_env()
        .build_initialized()
...
impl ConnectedRemoteControlBackend {
    async fn start(workx_home: &std::path::Path) -> Result<Self> {
        let listener = configured_remote_control_listener(workx_home).await?;
...
async fn configured_remote_control_listener(workx_home: &std::path::Path) -> Result<TcpListener> {

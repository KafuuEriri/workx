...
use workx_app_server_protocol::ChatgptAuthTokensRefreshReason;
use workx_app_server_protocol::ChatgptAuthTokensRefreshResponse;
use workx_app_server_protocol::ClientInfo;
use workx_app_server_protocol::DesktopOnboardingEntrypoint;
...
use workx_login::CLIENT_ID_OVERRIDE_ENV_VAR;
use workx_login::REFRESH_TOKEN_URL_OVERRIDE_ENV_VAR;
use workx_login::auth::BedrockAccessKeysAuth;
...
const LOGIN_ISSUER_ENV_VAR: &str = "WORKX_APP_SERVER_LOGIN_ISSUER";
const LOGIN_OPEN_APP_URL_ENV_VAR: &str = "WORKX_APP_SERVER_DEV_OPEN_APP_URL";
...
fn create_config_toml(workx_home: &Path, params: CreateConfigTomlParams) -> std::io::Result<()> {
...
fn read_config_toml(workx_home: &Path) -> Result<toml::Value> {
    Ok(toml::from_str(&std::fs::read_to_string(
        workx_home.join("config.toml"),
    )?)?)
}

fn load_file_auth(workx_home: &Path) -> Result<Option<AuthDotJson>> {
...
async fn logout_account_removes_auth_and_notifies() -> Result<()> {
    ...
    create_config_toml(workx_home.path(), CreateConfigTomlParams::default())?;
...
async fn logout_account_succeeds_when_config_reload_fails() -> Result<()> {
    ...
    assert_eq!(load_file_auth(workx_home.path())?, None);
...
async fn external_auth_refreshes_on_unauthorized() -> Result<()> {
    ...
    let thread_req = mcp
        .send_thread_start_request_with_auto_env(workx_app_server_protocol::ThreadStartParams {
            model: Some("mock-model".to_string()),
            ..Default::default()
        })
        .await?;
    let thread: workx_app_server_protocol::ThreadStartResponse =
        timeout(DEFAULT_READ_TIMEOUT, mcp.read_response(thread_req)).await??;

    let turn_req = mcp
        .send_turn_start_request(workx_app_server_protocol::TurnStartParams {
            thread_id: thread.thread.id,
            client_user_message_id: None,
            input: vec![workx_app_server_protocol::UserInput::Text {
    ...
    .await?;
    let _: workx_app_server_protocol::TurnStartResponse =
...
async fn login_amazon_bedrock_replaces_primary_auth_and_persists_provider(
    ...
    let mut mcp = TestAppServer::builder()
        ...
        .build_initialized_with_timeout(DEFAULT_READ_TIMEOUT)
        .await?;
    let mut expected_config = read_config_toml(workx_home.path())?;
    expected_config
        .as_table_mut()
    ...
    assert_eq!(
        load_file_auth(workx_home.path())?,
        Some(AuthDotJson {
    ...
    assert_eq!(read_config_toml(workx_home.path())?, expected_config);
    assert!(!workx_home.path().join(".env").exists());
    ...
    assert_eq!(
        ...
        GetAccountResponse {
            account: Some(Account::AmazonBedrock {
                uses_workx_managed_credentials: true,
            }),
    ...
    if managed_access_keys {
        ...
        assert_eq!(load_file_auth(workx_home.path())?, None);
        assert_eq!(read_config_toml(workx_home.path())?, expected_logout_config);
        assert!(!workx_home.path().join(".env").exists());
        assert_account_updated(&mut mcp, /*auth_mode*/ None).await?;
        assert_eq!(
            read_account(&mut mcp).await?,
...
async fn login_amazon_bedrock_rejects_non_bedrock_provider_override_without_changes() -> Result<()>
{
    ...
    let expected_auth = load_file_auth(workx_home.path())?;
    ...
    let mut mcp = TestAppServer::builder()
        ...
        .build_initialized_with_timeout(DEFAULT_READ_TIMEOUT)
        .await?;
    let expected_config = read_config_toml(workx_home.path())?;
    ...
    assert_eq!(load_file_auth(workx_home.path())?, expected_auth);
    assert_eq!(read_config_toml(workx_home.path())?, expected_config);
...
async fn logout_aws_managed_bedrock_clears_provider_and_restores_default_account() -> Result<()> {
    for managed_bedrock_auth in [false, true] {
        let workx_home = TempDir::new()?;
        create_config_toml(workx_home.path(), aws_managed_bedrock_config())?;
        let config_path = workx_home.path().join("config.toml");
        let config = std::fs::read_to_string(&config_path)?
            .replace(
                "model = \"mock-model\"",
        ...
        let dotenv_path = workx_home.path().join(".env");
        let aws_credentials_path = workx_home.path().join("aws-credentials");
        let dotenv = "AWS_ACCESS_KEY_ID=environment-id\nAWS_SECRET_ACCESS_KEY=environment-secret\n";
        let aws_credentials = "[workx-bedrock]\naws_access_key_id = profile-id\naws_secret_access_key = profile-secret\n";
        ...
        assert_eq!(
            ...
            GetAccountResponse {
                account: Some(Account::AmazonBedrock {
                    uses_workx_managed_credentials: false,
                }),
                requires_openai_auth: false,
            }
        );
...
async fn login_account_chatgpt_device_code_succeeds_and_notifies() -> Result<()> {
    ...
    assert_eq!(verification_url, format!("{issuer}/workx/device"));
...
async fn login_account_chatgpt_redirects_to_hosted_success_page() -> Result<()> {
    ...
    let mut mcp = TestAppServer::builder()
        ...
        .with_env_overrides(&[
            ...
            (
                ...
                Some("http://localhost:3000/workx/open-app"),
            ),
        ])
        .build_initialized_with_timeout(DEFAULT_READ_TIMEOUT)
        .await?;
    ...
    assert_eq!(
        ...
        "http://localhost:3000/workx/open-app?source=login&app_brand=chatgpt"
    );

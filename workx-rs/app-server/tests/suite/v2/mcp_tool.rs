...
use workx_app_server_protocol::CapabilityRootLocation;
...
use workx_app_server_protocol::McpElicitationSchema;
...
use workx_protocol::protocol::ThreadSource as CoreThreadSource;
        ...
        (
            ...
            json!({
                ...
                "data": {
                    ...
                    "_meta": {"_workx_apps": {"connector_auth_failure": {
...
async fn mcp_server_tool_call_round_trips_elicitation() -> Result<()> {
    mcp_server_tool_call_round_trips_elicitation_for_thread(ElicitationThread::Start {
        params: ThreadStartParams {
            model: Some("mock-model".to_string()),
            approval_policy: Some(workx_app_server_protocol::AskForApproval::UnlessTrusted),
            ..Default::default()
        },
...
-> Result<()> {
    mcp_server_tool_call_round_trips_elicitation_for_thread(ElicitationThread::Start {
        params: ThreadStartParams {
            ...
            sandbox: Some(workx_app_server_protocol::SandboxMode::DangerFullAccess),
            thread_source: Some(workx_app_server_protocol::ThreadSource::User),
            ..Default::default()
        },
        session_source: "vscode",
...
{
    assert_full_access_form_elicitation_is_declined(FullAccessElicitationCase {
        thread_source: Some(workx_app_server_protocol::ThreadSource::Feature(
...
async fn mcp_server_tool_call_declines_full_access_elicitation_for_subagent_thread() -> Result<()> {
    assert_full_access_form_elicitation_is_declined(FullAccessElicitationCase {
        thread_source: Some(workx_app_server_protocol::ThreadSource::Subagent),
        client_advertises_standard_form_input: true,
    })
    .await
}
...
struct FullAccessElicitationCase {
    thread_source: Option<workx_app_server_protocol::ThreadSource>,
...
async fn assert_full_access_form_elicitation_is_declined(
    ...
    assert_eq!(
        approval_policy,
        workx_app_server_protocol::AskForApproval::Never
    );
    assert_eq!(
        sandbox,
...
async fn mcp_server_tool_call_round_trips_elicitation_for_thread(
    ...
    let thread = match elicitation_thread {
        ...
        ElicitationThread::Resume { source, params } => {
            ...
            assert_eq!(
                thread.source,
                workx_app_server_protocol::SessionSource::from(source)
            );
            assert_eq!(
                approval_policy,
                workx_app_server_protocol::AskForApproval::Never
...
impl ServerHandler for ToolAppsMcpServer {
    ...
    async fn call_tool(
            ...
            == Some(true)
        {
            ...
            {
                approval_meta["workx_sensitive_action"] = json!(sensitive_action);
            }
            let result = context
                .peer

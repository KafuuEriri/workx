...
use workx_app_server_protocol::AppBranding;
...
use workx_app_server_protocol::AppReview;
use workx_app_server_protocol::AppScreenshot;
...
async fn list_apps_includes_plugin_apps_for_chatgpt_auth() -> Result<()> {
    ...
    let workx_home = TempDir::new()?;
    write_connectors_and_plugins_config(workx_home.path(), &server_url)?;
    write_plugin_app_fixture(workx_home.path(), "sample", "connector_sample")?;
    write_chatgpt_auth(
        workx_home.path(),
...
fn write_connectors_and_plugins_config(workx_home: &Path, base_url: &str) -> std::io::Result<()> {
    let config_toml = workx_home.join("config.toml");
    std::fs::write(
        config_toml,
        format!(
...
fn write_plugin_app_fixture(workx_home: &Path, plugin_name: &str, app_id: &str) -> Result<()> {

...
async fn start_stable_server(workx_home: &TempDir) -> Result<TestAppServer> {
...
async fn config_requirements_read_preserves_import_policy() -> Result<()> {
    for (contents, expected_table) in [
        ...
        std::fs::write(workx_home.path().join("requirements.toml"), contents)?;
        // An unrecognized ordinary user setting cannot override managed policy.
        std::fs::write(workx_home.path().join("config.toml"), ALLOW)?;
        let mut server = start_stable_server(&workx_home).await?;

...
async fn turn_start_shell_zsh_fork_executes_command_v2() -> Result<()> {
    ...
    let mut mcp = create_zsh_test_mcp_process(&workx_home, &workspace, &zsh_path).await?;
    ...
    let turn_id = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            model: Some("mock-model".to_string()),
            effort: Some(workx_protocol::openai_models::ReasoningEffort::Medium),
            summary: Some(workx_protocol::config_types::ReasoningSummary::Auto),
            ..Default::default()
        })
        .await?;
    ...
    assert!(command.starts_with(&command_packaged_zsh_path(&workx_home).display().to_string()));
...
async fn turn_start_shell_zsh_fork_subcommand_decline_marks_parent_declined_v2() -> Result<()> {
    ...
    while target_decision_index < target_decisions.len() || !saw_parent_approval {
        ...
        let is_parent_approval = approval_command
            .contains(&command_packaged_zsh_path(&workx_home).display().to_string())
...
async fn create_zsh_test_mcp_process(
    ...
    let app_server = create_test_package_app_server(workx_home, zsh_path)?;
...
fn create_test_package_app_server(workx_home: &Path, zsh_path: &Path) -> Result<PathBuf> {
    let package_dir = workx_home.join("test-package");
    let bin_dir = package_dir.join("bin");
    let package_zsh_path = packaged_zsh_path(workx_home);
    ...
    std::fs::write(package_dir.join("workx-package.json"), "{}")?;

    let app_server = bin_dir.join("workx-app-server");
    copy_with_permissions(
        &workx_utils_cargo_bin::cargo_bin("workx-app-server")?,
        &app_server,
    )?;
...
fn packaged_zsh_path(workx_home: &Path) -> PathBuf {
    workx_home
        .join("test-package")
...
fn command_packaged_zsh_path(workx_home: &Path) -> PathBuf {
    let path = packaged_zsh_path(workx_home);
...
fn find_test_zsh_path() -> Result<Option<std::path::PathBuf>> {
    let repo_root = workx_utils_cargo_bin::repo_root()?;
    let dotslash_zsh = repo_root.join("workx-rs/app-server/tests/suite/zsh");

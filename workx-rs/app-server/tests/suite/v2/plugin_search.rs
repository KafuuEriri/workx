...
use workx_app_server_protocol::PluginSearchResponse;
use workx_app_server_protocol::PluginSearchScope;
...
async fn plugin_search_stitches_local_results_into_the_first_remote_page() -> Result<()> {
    ...
    write_remote_plugin_search_config(workx_home.path(), &server, /*remote_plugin*/ true)?;
    write_chatgpt_search_auth(workx_home.path())?;
...
async fn plugin_search_hides_bundled_sites_when_remote_sites_is_effective() -> Result<()> {
    let workx_home = TempDir::new()?;
    let server = MockServer::start().await;
    let curated_root = workx_home.path().join(".tmp/plugins");
    ...
    write_installed_plugin(workx_home.path(), "openai-bundled", "sites")?;
    write_installed_plugin(workx_home.path(), "openai-curated-remote", "sites")?;
    std::fs::write(
        workx_home.path().join("config.toml"),
        format!(
            r#"chatgpt_base_url = "{}/backend-api/"
...
async fn plugin_search_returns_local_matches_for_api_key_auth() -> Result<()> {
    for remote_plugin_enabled in [false, true] {
        ...
        let repo_root = TempDir::new()?;
        let curated_root = workx_home.path().join(".tmp/plugins");
        let bundled_alpha_root = workx_home
            .path()
        ...
        write_remote_plugin_search_config(workx_home.path(), &server, remote_plugin_enabled)?;
        login_with_api_key(
            workx_home.path(),
            "sk-test-key",
            AuthCredentialsStoreMode::File,
...
-> Result<()> {
    ...
    write_installed_plugin(workx_home.path(), "openai-curated", "calendar")?;
    write_installed_plugin(workx_home.path(), "personal-tools", "local-planner")?;
    write_installed_plugin(workx_home.path(), "personal-tools", "calendar-local-only")?;
    std::fs::write(
        workx_home.path().join("config.toml"),
        format!(
            r#"chatgpt_base_url = "{}/backend-api/"
...
fn write_local_marketplace(
    ...
    for plugin in plugins {
        let plugin_manifest = root.join("plugins").join(plugin.name).join(".workx-plugin");
...
fn write_chatgpt_search_auth(workx_home: &std::path::Path) -> Result<()> {
    write_chatgpt_auth(
        workx_home,
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
...
fn write_installed_plugin(
    ...
    let plugin_manifest = workx_home
        .join("plugins/cache")

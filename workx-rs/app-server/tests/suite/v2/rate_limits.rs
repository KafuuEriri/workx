...
async fn get_account_rate_limits_returns_snapshot(
    ...
    write_chatgpt_base_url(workx_home.path(), &server_url)?;
    ...
    let response_body = json!({
        ...
        "additional_rate_limits": [
            {
                "limit_name": "workx_other",
                "metered_feature": "workx_other",
                "rate_limit": {
                    "allowed": true,
                    "limit_reached": false,
                    "primary_window": {
        ...
        rate_limit_reset_credits: Some(RateLimitResetCreditsSummary {
            ...
            credits: Some(vec![
                RateLimitResetCredit {
                    ...
                    reset_type: RateLimitResetType::WorkxRateLimits,
...
async fn get_account_rate_limits_filters_banner_by_identity(
    ...
    let server = MockServer::start().await;
    write_chatgpt_base_url(workx_home.path(), &server.uri())?;
    let banner = json!({
    ...
    let expected: GetAccountRateLimitsResponse = serde_json::from_value(json!({
        ...
        "rateLimits": snapshot, "rateLimitsByLimitId": {"workx": snapshot},
...
fn write_chatgpt_base_url(workx_home: &Path, base_url: &str) -> std::io::Result<()> {

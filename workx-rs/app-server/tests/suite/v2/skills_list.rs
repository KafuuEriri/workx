...
use workx_app_server_protocol::SkillScope;
use workx_app_server_protocol::SkillsChangedNotification;
...
fn write_cached_remote_plugin_with_skill(
    ...
    let plugin_root = workx_home.join("plugins/cache/openai-curated-remote/linear/local");
...
fn write_cached_local_curated_plugin_with_skill(
    ...
    let plugin_root = workx_home.join(format!(
...
async fn skills_list_uses_each_cwds_bundled_skills_configuration() -> Result<()> {
    ...
    for (cwd, enabled) in [(disabled_cwd.path(), false), (enabled_cwd.path(), true)] {
        ...
        std::fs::create_dir_all(cwd.join(".workx"))?;
        std::fs::write(
            cwd.join(".workx/config.toml"),
            format!("[skills.bundled]\nenabled = {enabled}\n"),
        )?;
        set_project_trust_level(workx_home.path(), cwd, TrustLevel::Trusted)?;
    }
...
async fn skills_list_runtime_enable_refreshes_shared_system_skill_cache() -> Result<()> {
    let workx_home = TempDir::new()?;
    let cwd = TempDir::new()?;
    let stale_skill_path = workx_home
        .path()
...
async fn runtime_remote_plugin_toggle_updates_local_curated_plugin_skills() -> Result<()> {
    ...
    write_cached_local_curated_plugin_with_skill(workx_home.path(), "openai-curated")?;
    std::fs::write(
        workx_home.path().join("config.toml"),
        format!(
...
async fn skills_list_loads_remote_installed_plugin_skills_from_cache() -> Result<()> {
    ...
    let expected_skill_path =
        std::fs::canonicalize(write_cached_remote_plugin_with_skill(workx_home.path())?)?;
...
async fn skills_list_skips_cwd_roots_when_environment_disabled() -> Result<()> {
    let workx_home = TempDir::new()?;
    let cwd = TempDir::new()?;
    write_skill(&workx_home, "home-skill")?;
    let repo_skill_dir = cwd.path().join(".workx/skills/repo-skill");
    ...
    let mut mcp = TestAppServer::builder()
        .with_workx_home(workx_home.path())
        .without_auto_env()
        .with_env_overrides(&[(WORKX_EXEC_SERVER_URL_ENV_VAR, Some("none"))])
        .build_initialized_with_timeout(DEFAULT_TIMEOUT)
        .await?;
...
async fn skills_list_accepts_relative_cwds() -> Result<()> {
    ...
    std::fs::create_dir_all(workx_home.path().join(&relative_cwd))?;
...
async fn skills_list_preserves_requested_cwd_order(home_plugins_enabled: bool) -> Result<()> {
    ...
    write_skill(&workx_home, "shared-skill")?;
    write_cached_local_curated_plugin_with_skill(workx_home.path(), "openai-api-curated")?;
    std::fs::write(
        workx_home.path().join("config.toml"),
        format!(
            r#"[features]
...
-> Result<()> {
    ...
    let skill_dir = cwd.path().join(".workx/skills/late-extra-skill");
    std::fs::create_dir_all(&skill_dir)?;
    std::fs::write(
        skill_dir.join("SKILL.md"),

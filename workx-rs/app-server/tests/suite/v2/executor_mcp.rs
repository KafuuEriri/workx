...
async fn selected_executor_discovers_browser_mcp_with_executor_only_bearer_token() -> Result<()> {
    ...
    let mut executor = Command::new(workx_utils_cargo_bin::cargo_bin("workx")?)
        .args(["exec-server", "--listen", "ws://127.0.0.1:0"])
        .stdout(Stdio::piped())
        .kill_on_drop(true)
        .env("WORKX_HOME", executor_home.path())
...
async fn guardian_review_does_not_discover_executor_mcp() -> Result<()> {
    ...
    let workx_bin = toml::Value::String(
        workx_utils_cargo_bin::cargo_bin("workx")?
            .to_string_lossy()
            .into_owned(),
    );
    ...
    std::fs::write(
        workx_home.path().join("environments.toml"),
        format!(
            "default = \"{EXECUTOR_ID}\"\ninclude_local = false\n\n[[environments]]\nid = \"{EXECUTOR_ID}\"\nprogram = {workx_bin}\nargs = [\"exec-server\", \"--listen\", \"stdio\"]\n[environments.env]\nWORKX_HOME = {executor_home_value}\n"
        ),
    )?;
    let mut app_server = TestAppServer::builder()
        .with_workx_home(workx_home.path())
...
async fn selected_executor_plugin_exposes_its_mcps_only_to_that_thread() -> Result<()> {
    ...
    MockResponsesConfig::new(&responses_server.uri())
        ...
        .write(workx_home.path())?;
    let executor_config: workx_config::types::McpServerConfig = serde_json::from_value(json!({
    ...
    let oauth_credentials_path = workx_home.path().join(".credentials.json");
    std::fs::write(
...
"#
    ...
    std::fs::create_dir_all(plugin.path().join(".workx-plugin"))?;
    std::fs::write(
        plugin.path().join(".workx-plugin/plugin.json"),
...
"#,
    ...
    assert_eq!(registration_request["client_name"], json!("Workx"));
    assert_eq!(

...
//! the temporary `workx_home` must not contain rollout session files or sqlite
...
use workx_app_server::in_process::InProcessClientHandle;
use workx_app_server::in_process::InProcessServerEvent;
...
use workx_app_server_protocol::ThreadSectionCreateParams;
use workx_app_server_protocol::ThreadSectionDeleteParams;
use workx_app_server_protocol::ThreadSectionListParams;
use workx_app_server_protocol::ThreadSectionUpdateParams;
...
use workx_state::PINNED_THREAD_SECTION_ID;
use workx_thread_store::CreateThreadParams as StoreCreateThreadParams;
...
async fn thread_section_operations_without_sqlite_return_method_not_found() -> Result<()> {
    ...
    create_config_toml_with_thread_store(workx_home.path(), "http://127.0.0.1:1", &store_id)?;
    let _in_memory_store = InMemoryThreadStoreId { store_id };
    let client = start_in_process_server(workx_home.path()).await?;
    ...
    assert_no_local_persistence_artifacts(workx_home.path())?;
...
async fn thread_start_defaults_to_legacy_without_history_list_support() -> Result<()> {
    ...
    let workx_home = TempDir::new()?;
    let store_id = Uuid::new_v4().to_string();
    create_config_toml_with_thread_store(workx_home.path(), &server.uri(), &store_id)?;
...
async fn thread_delete_with_non_local_thread_store_does_not_create_local_persistence() -> Result<()>
{
    ...
    let workx_home = TempDir::new()?;
    let store_id = Uuid::new_v4().to_string();
    // Plugin startup warmups may create `.tmp` under workx_home. Disable them
    ...
    let mut client = start_in_process_server(workx_home.path()).await?;
...
async fn start_in_process_server(workx_home: &Path) -> Result<InProcessClientHandle> {
    let loader_overrides = LoaderOverrides::without_managed_config_for_tests();
    let config = Arc::new(
        ConfigBuilder::default()
            .workx_home(workx_home.to_path_buf())
            .fallback_cwd(Some(workx_home.to_path_buf()))
            .loader_overrides(loader_overrides.clone())
            .build()
            .await?,
    );
...
fn assert_no_local_persistence_artifacts(workx_home: &Path) -> Result<()> {
    ...
    let mut entries = workx_home_entries(workx_home)?;
    ...
    assert_eq!(
        ...
        "non-local thread persistence should not create unexpected files in workx_home"
    );

    Ok(())
}

fn workx_home_entries(workx_home: &Path) -> Result<BTreeSet<String>> {
    Ok(std::fs::read_dir(workx_home)?
        .filter_map(|entry| {
            let entry = entry.ok()?;

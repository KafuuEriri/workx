...
use workx_app_server_protocol::QueuedSubmission;
...
use workx_app_server_protocol::ThreadQueueAddParams;
use workx_app_server_protocol::ThreadQueueAddResponse;
use workx_app_server_protocol::ThreadQueueChangedNotification;
use workx_app_server_protocol::ThreadQueueDeleteParams;
use workx_app_server_protocol::ThreadQueueDeleteResponse;
use workx_app_server_protocol::ThreadQueueListParams;
use workx_app_server_protocol::ThreadQueueListResponse;
use workx_app_server_protocol::ThreadQueueReorderParams;
use workx_app_server_protocol::ThreadQueueReorderResponse;
use workx_app_server_protocol::ThreadQueueStartParams;
use workx_app_server_protocol::ThreadQueueStartResponse;
use workx_app_server_protocol::ThreadQueueUpdateParams;
use workx_app_server_protocol::ThreadQueueUpdateResponse;
...
async fn queue_requires_experimental_handshake() -> Result<()> {
    let (mut app, workx_home, _server) = queue_app(Vec::new()).await?;
...
async fn queue_crud_preserves_identity_order_and_notifications() -> Result<()> {
    ...
    let (mut app, _workx_home, _server) = queue_app(responses).await?;
    let thread_id = app
...
async fn queue_list_returns_ordered_pages_and_lightweight_notifications() -> Result<()> {
    ...
    let (mut app, _workx_home, _server) = queue_app(vec![blocked_turn_response()?]).await?;
    let thread_id = app
        .start_thread(ThreadStartParams::default())
        .await?
        .thread
...
async fn idle_queue_dispatch_preserves_client_id() -> Result<()> {
    ...
    let (mut app, _workx_home, server) = queue_app(responses).await?;
    let thread_id = app
        .start_thread(ThreadStartParams::default())
        .await?
        .thread
...
async fn cold_thread_resume_dispatches_a_persisted_queued_submission() -> Result<()> {
    ...
    let (mut first, workx_home, _server) = queue_app(responses).await?;
    let thread_id = first
        .start_thread(ThreadStartParams::default())
        .await?
        .thread
...
async fn interrupt_preserves_queue_and_queue_start_can_resume_a_non_head_item() -> Result<()> {
    ...
    let (mut app, workx_home, _server) = queue_app(responses).await?;
    let thread_id = app
        .start_thread(ThreadStartParams::default())
        .await?
        .thread
...
async fn queue_start_while_active_returns_busy_and_preserves_the_queue() -> Result<()> {
    ...
    .await;
    let (mut app, _workx_home, _server) = queue_app_with_server(server).await?;
    let thread_id = app
        .start_thread(ThreadStartParams::default())
        .await?
        .thread

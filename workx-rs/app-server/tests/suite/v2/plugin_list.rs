...
use workx_app_server_protocol::HookTrustStatus;
use workx_app_server_protocol::HooksListParams;
use workx_app_server_protocol::HooksListResponse;
use workx_app_server_protocol::JSONRPCResponse;
use workx_app_server_protocol::LoginAccountResponse;
use workx_app_server_protocol::PluginAuthPolicy;
use workx_app_server_protocol::PluginDisabledReason;
use workx_app_server_protocol::PluginInstallParams;
use workx_app_server_protocol::PluginInstallPolicy;
use workx_app_server_protocol::PluginInstallPolicySource;
...
use workx_core::config::set_project_trust_level;
use workx_login::AuthKeyringBackendKind;
...
const TEST_ALLOW_HTTP_REMOTE_PLUGIN_BUNDLE_DOWNLOADS: &str =
    "WORKX_TEST_ALLOW_HTTP_REMOTE_PLUGIN_BUNDLE_DOWNLOADS";
...
fn write_plugins_enabled_config(workx_home: &std::path::Path) -> std::io::Result<()> {
    std::fs::write(
...
async fn plugin_list_skips_invalid_marketplace_file_and_reports_error() -> Result<()> {
    ...
    write_plugins_enabled_config(workx_home.path())?;
    let marketplace_path =
        AbsolutePathBuf::try_from(repo_root.path().join(".agents/plugins/marketplace.json"))?;
    std::fs::write(marketplace_path.as_path(), "{not json")?;

    let home = workx_home.path().to_string_lossy().into_owned();
...
async fn plugin_rpcs_reject_repository_spoofing_openai_curated() -> Result<()> {
    ...
    std::fs::create_dir_all(repository.path().join("attacker/.workx-plugin"))?;
    std::fs::write(
        repository.path().join("attacker/.workx-plugin/plugin.json"),
        r#"{"name":"attacker"}"#,
    )?;
    let marketplace_path =
    ...
    write_plugins_enabled_config(workx_home.path())?;
    let original_config = std::fs::read(workx_home.path().join("config.toml"))?;
    let mut mcp = TestAppServer::builder()
        .with_workx_home(workx_home.path())
        .build_initialized_with_timeout(DEFAULT_TIMEOUT)
        .await?;
    ...
    assert!(!workx_home.path().join("plugins/cache").exists());
    assert_eq!(
        std::fs::read(workx_home.path().join("config.toml"))?,
        original_config
    );
    Ok(())
}
...
async fn plugin_installed_includes_installed_plugins_and_explicit_install_suggestions() -> Result<()>
{
    ...
    write_installed_plugin(&workx_home, "openai-api-curated", "linear")?;
    std::fs::write(
...
async fn plugin_installed_prefers_remote_curated_conflicts_when_remote_plugin_enabled() -> Result<()>
{
    ...
    write_openai_curated_marketplace(workx_home.path(), &["linear", "calendar"])?;
    write_installed_plugin(&workx_home, "openai-curated", "linear")?;
    write_installed_plugin(&workx_home, "openai-curated", "calendar")?;
    std::fs::write(
        workx_home.path().join("config.toml"),
        format!(
...
async fn plugin_installed_hides_bundled_sites_when_remote_sites_is_effective() -> Result<()> {
    ...
    write_installed_plugin(&workx_home, "openai-bundled", "sites")?;
    write_installed_plugin(&workx_home, "openai-curated-remote", "sites")?;
    std::fs::write(
        workx_home.path().join("config.toml"),
        format!(
            r#"chatgpt_base_url = "{}/backend-api/"
...
"#,
    ...
    write_remote_plugin_test_auth(workx_home.path())?;
...
async fn plugin_installed_prefers_api_curated_conflicts_after_switching_to_api_auth() -> Result<()>
{
    let workx_home = TempDir::new()?;
    let server = MockServer::start().await;
    write_openai_api_curated_marketplace(workx_home.path(), &["linear"])?;
    write_installed_plugin(&workx_home, "openai-api-curated", "linear")?;
    let config = format!(
        r#"chatgpt_base_url = "{}/backend-api/"
...
"#,
    ...
    std::fs::write(workx_home.path().join("config.toml"), &config)?;
    write_chatgpt_auth(
        workx_home.path(),
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
    ...
    std::fs::write(workx_home.path().join("config.toml"), "invalid config")?;
    let request_id = app_server
...
async fn plugin_list_keeps_valid_marketplaces_when_another_marketplace_fails_to_load() -> Result<()>
{
    ...
    std::fs::create_dir_all(
        valid_repo_root
            .path()
            .join("plugins/valid-plugin/.workx-plugin"),
    )?;
  ...
  "plugins": [
    ...
    std::fs::write(
        valid_repo_root
            .path()
            .join("plugins/valid-plugin/.workx-plugin/plugin.json"),
    ...
    assert_eq!(
        ...
        vec![PluginMarketplaceEntry {
            ...
            plugins: vec![PluginSummary {
                ...
                availability: workx_app_server_protocol::PluginAvailability::Available,
        ...
        vec![PluginMarketplaceEntry {
            ...
            plugins: vec![
                PluginSummary {
                    ...
                    disabled_reason: None,
                    eligible_plan_types: None,
                    interface: Some(workx_app_server_protocol::PluginInterface {
...
async fn plugin_list_omitted_cwds_excludes_server_project_config() -> Result<()> {
    ...
    std::fs::create_dir_all(workx_home.path().join(".agents/plugins"))?;
    std::fs::create_dir_all(workx_home.path().join(".git"))?;
    std::fs::create_dir_all(workx_home.path().join(".workx"))?;
    std::fs::create_dir_all(project_marketplace.path().join(".agents/plugins"))?;
    write_installed_plugin(&workx_home, "home-marketplace", "home-plugin")?;
    std::fs::write(
        workx_home.path().join("config.toml"),
        "[features]\nplugins = true\n[plugins.\"home-plugin@home-marketplace\"]\nenabled = true\n",
    )?;
    std::fs::write(
        workx_home.path().join(".agents/plugins/marketplace.json"),
        r#"{
  ...
  "plugins": [
    ...
    std::fs::write(
        workx_home.path().join(".workx/config.toml"),
        format!(
            "[marketplaces.project-marketplace]\nsource_type = \"local\"\nsource = {source}\n\n[plugins.\"home-plugin@home-marketplace\"]\nenabled = false\n"
        ),
    )?;
    set_project_trust_level(workx_home.path(), workx_home.path(), TrustLevel::Trusted)?;
    let home = workx_home.path().to_string_lossy().into_owned();
    let mut mcp = TestAppServer::builder()
        .with_workx_home(workx_home.path())
        .with_env_overrides(&[
    ...
    for (cwds, expected) in [
        ...
        (
            Some(vec![AbsolutePathBuf::try_from(workx_home.path())?]),
            vec![
...
async fn plugin_list_force_refetch_waits_for_same_path_local_plugin_upgrade() -> Result<()> {
    ...
    let source_manifest = marketplace_root
        .path()
        .join("sample-plugin/.workx-plugin/plugin.json");
...
"#,
    ...
    let plugin_cache = workx_home
        .path()
        .join("plugins/cache/sample-marketplace/sample-plugin");
    let installed_manifest = plugin_cache.join("1.1.0/.workx-plugin/plugin.json");
    assert!(
...
async fn plugin_list_refreshes_plugins_from_each_cwd(
    ...
    for ((repo, name), version) in repos.iter().zip(names).zip(versions) {
        for directory in [".git", ".workx", ".agents/plugins", "sample/.workx-plugin"] {
            std::fs::create_dir_all(repo.join(directory))?;
        }
        std::fs::write(
            repo.join("sample/.workx-plugin/plugin.json"),
        ...
        std::fs::write(
            repo.join(".workx/config.toml"),
            format!("[features]\nplugins = true\n{plugin_config}"),
        )?;
        set_project_trust_level(workx_home.path(), repo, TrustLevel::Trusted)?;
        write_installed_plugin_with_version(&workx_home, name, "sample", "1.0.0")?;
    }
    ...
    for (name, version) in &expected_sources {
        let manifest = workx_home.path().join(format!(
            "plugins/cache/{name}/sample/{version}/.workx-plugin/plugin.json"
        ));
        if !force_refetch {
...
async fn plugin_catalogs_skip_invalid_project_config_and_report_cwd_error() -> Result<()> {
    ...
    for repo in [&invalid_repo, &valid_repo] {
        for directory in [".git", ".workx", ".agents/plugins"] {
            std::fs::create_dir_all(repo.join(directory))?;
        }
        set_project_trust_level(workx_home.path(), repo, TrustLevel::Trusted)?;
    }
    std::fs::write(invalid_repo.join(".workx/config.toml"), "invalid = [\n")?;
    std::fs::write(
        valid_repo.join(".workx/config.toml"),
    ...
    write_installed_plugin(&workx_home, "valid-marketplace", "sample")?;
...
async fn plugin_list_includes_install_and_enabled_state_from_config() -> Result<()> {
    ...
    std::fs::create_dir_all(repo_root.path().join(".git"))?;
    std::fs::create_dir_all(repo_root.path().join(".agents/plugins"))?;
    write_installed_plugin(&workx_home, "workx-curated", "enabled-plugin")?;
    write_installed_plugin(&workx_home, "workx-curated", "disabled-plugin")?;
    std::fs::write(
        repo_root.path().join(".agents/plugins/marketplace.json"),
        r#"{
  "name": "workx-curated",
...
[plugins."enabled-plugin@workx-curated"]
enabled = true

[plugins."disabled-plugin@workx-curated"]
enabled = false
"#,
    )?;
    ...
    assert_eq!(marketplace.name, "workx-curated");
    assert_eq!(
        marketplace
            .interface
            .as_ref()
    ...
    assert_eq!(marketplace.plugins[0].id, "enabled-plugin@workx-curated");
    ...
    assert_eq!(marketplace.plugins[1].id, "disabled-plugin@workx-curated");
    ...
    assert_eq!(
        ...
        "uninstalled-plugin@workx-curated"
    );
...
async fn plugin_list_deduplicates_sources_and_merges_enabled_state() -> Result<()> {
    let workx_home = TempDir::new()?;
    std::fs::create_dir_all(workx_home.path().join(".agents/plugins"))?;
    write_installed_plugin(&workx_home, "workx-curated", "shared-plugin")?;
    std::fs::write(
        workx_home.path().join("config.toml"),
        r#"[features]
plugins = true

[plugins."shared-plugin@workx-curated"]
enabled = true
"#,
    )?;
  ...
  "plugins": [
    ...
    std::fs::create_dir_all(workspace_enabled.path().join(".workx"))?;
    std::fs::write(
        workspace_enabled.path().join(".workx/config.toml"),
        r#"[plugins."shared-plugin@workx-curated"]
enabled = false
"#,
    )?;
    ...
    let marketplaces = response
        .marketplaces
        .iter()
        .filter(|marketplace| marketplace.name == "workx-curated")
        .map(|marketplace| {
            (
    ...
    assert_eq!(
        ...
        vec![(
            "workx-curated",
            vec![("shared-plugin@workx-curated", true, true)]
        )]
    );
    Ok(())
}
...
async fn plugin_list_returns_plugin_interface_with_absolute_asset_paths() -> Result<()> {
  ...
  "interface": {
    ...
    assert_eq!(plugin.id, "demo-plugin@workx-curated");
...
async fn plugin_list_returns_installed_git_source_interface_from_cache(
  ...
  "plugins": [
    ...
    let cached_plugin_root = workx_home.path().join("plugins/cache/debug/toolkit/local");
    std::fs::create_dir_all(cached_plugin_root.join(".workx-plugin"))?;
    std::fs::write(
        cached_plugin_root.join(".workx-plugin/plugin.json"),
        r##"{
  "name": "toolkit",
  "interface": {
    "displayName": "Toolkit",
...
"#;
    ...
    if configured_in_later_cwd {
        ...
        std::fs::write(later_repo.path().join(".workx/config.toml"), plugin_config)?;
        set_project_trust_level(workx_home.path(), later_repo.path(), TrustLevel::Trusted)?;
...
async fn app_server_startup_sync_downloads_remote_installed_plugin_bundles() -> Result<()> {
    ...
    wait_for_path_exists(&installed_path.join(".workx-plugin/plugin.json")).await?;
    let installed_plugin_manifest: serde_json::Value = serde_json::from_str(
        &std::fs::read_to_string(installed_path.join(".workx-plugin/plugin.json"))?,
    )?;
    assert_eq!(
...
async fn plugin_list_sync_upgrades_and_removes_remote_installed_plugin_bundles() -> Result<()> {
    ...
    write_installed_plugin_with_version(&workx_home, "openai-curated-remote", "linear", "1.0.0")?;
    write_installed_plugin_with_version(&workx_home, "openai-curated-remote", "stale", "1.0.0")?;
    ...
    let old_path = workx_home
        .path()
        .join("plugins/cache/openai-curated-remote/linear/1.0.0");
    let new_path = workx_home
        .path()
        .join("plugins/cache/openai-curated-remote/linear/1.2.3");
    let stale_path = workx_home
        .path()
    ...
    wait_for_path_exists(&new_path.join(".workx-plugin/plugin.json")).await?;
    let installed_plugin_manifest: serde_json::Value = serde_json::from_str(
        &std::fs::read_to_string(new_path.join(".workx-plugin/plugin.json"))?,
    )?;
    assert_eq!(
        installed_plugin_manifest["version"],
        serde_json::json!("1.2.3")
...
async fn plugin_list_includes_remote_marketplaces_when_remote_plugin_enabled(
    ...
    write_installed_plugin_with_version(&workx_home, "openai-curated-remote", "linear", "1.2.3")?;
    ...
    let cwds = if project_configuration != ProjectPluginConfiguration::None {
        ...
        std::fs::write(repo.path().join(".workx/config.toml"), config)?;
        std::fs::write(
            repo.path().join(".agents/plugins/marketplace.json"),
            r#"{"name":"disabled-local","plugins":[{"name":"sample","source":{"source":"local","path":"./sample"}}]}"#,
        )?;
        set_project_trust_level(workx_home.path(), repo.path(), TrustLevel::Trusted)?;
  ...
  "pagination": {
    ...
    Mock::given(method("GET"))
        ...
        .and(header("oai-product-sku", "workx"))
    ...
    Mock::given(method("GET"))
        ...
        .and(query_param("platform", "workx"))
        .respond_with(
    ...
    assert_eq!(
        ...
        workx_app_server_protocol::PluginAvailability::Available
    );
    assert_eq!(
    ...
    let cache_files = std::fs::read_dir(workx_home.path().join("cache/remote_plugin_catalog"))?
...
async fn plugin_list_honors_global_remote_catalog_cache_ttl() -> Result<()> {
    ...
    wait_for_cached_remote_catalog_plugin_ids(workx_home.path(), &[cached_remote_plugin_id])
        .await?;
    ...
    wait_for_remote_plugin_request_count(&server, "/ps/plugins/list", /*expected_count*/ 1).await?;
    wait_for_cached_remote_catalog_plugin_ids(workx_home.path(), &[refreshed_remote_plugin_id])
        .await?;
    sleep(Duration::from_millis(100)).await;
    wait_for_remote_plugin_request_count(&server, "/ps/plugins/list", /*expected_count*/ 1).await?;
...
async fn app_server_startup_skips_disabled_remote_plugin_catalog_scopes() -> Result<()> {
    ...
    write_remote_plugin_catalog_config(workx_home.path(), &base_url)?;
    write_chatgpt_auth(
        workx_home.path(),
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
    ...
    write_remote_plugins_disabled_config_with_base_url(workx_home.path(), &base_url)?;
    server.reset().await;
    mount_remote_plugin_list(&server, "GLOBAL", &global_body).await;
    mount_remote_plugin_list(&server, "USER", &user_body).await;
    mount_remote_plugin_list(&server, "WORKSPACE", &workspace_body).await;
...
-> Result<()> {
    ...
    write_openai_api_curated_marketplace(workx_home.path(), &["api-plugin"])?;
    login_with_api_key(
        workx_home.path(),
        "sk-test-key",
        AuthCredentialsStoreMode::File,
...
#[tokio::test]
async fn plugin_list_includes_api_curated_marketplace_for_bedrock_without_workx_auth() -> Result<()>
{
    let workx_home = TempDir::new()?;
    std::fs::write(
        workx_home.path().join("config.toml"),
...
"#,
    ...
    write_openai_curated_marketplace(workx_home.path(), &["chatgpt-plugin"])?;
    write_openai_api_curated_marketplace(workx_home.path(), &["api-plugin"])?;
    ...
    assert!(!workx_home.path().join("auth.json").exists());
    let api_curated_marketplace = response
        .marketplaces
        .iter()
        .find(|marketplace| marketplace.name == "openai-api-curated")
...
"#,
    ...
    wait_for_path_exists(
        &workx_home.path().join(
            "plugins/cache/created-by-me-remote/private-linear/1.2.3/.workx-plugin/plugin.json",
        ),
    )
    .await?;
    wait_for_remote_installed_snapshot_request(&server).await?;
...
"#,
    ...
    wait_for_path_exists(
        &workx_home
            .path()
            .join("plugins/cache/workspace-directory/no-hooks/1.2.3/.workx-plugin/plugin.json"),
    )
    .await?;
...
async fn plugin_installed_hook_trust_write_failure_stays_untrusted() -> Result<()> {
    ...
    symlink(&config_target, workx_home.path().join("config.toml"))?;
    write_remote_plugin_test_auth(workx_home.path())?;
...
async fn assert_disabled_remote_plugin_metadata(
    ...
    assert_eq!(
        plugin.availability,
        workx_app_server_protocol::PluginAvailability::DisabledByAdmin
    );
...
"#,
    ...
    std::fs::write(
        repo.path().join(".workx/config.toml"),
    ...
    write_installed_plugin(&workx_home, "local", "sample")?;
    set_project_trust_level(workx_home.path(), repo.path(), TrustLevel::Trusted)?;
...
async fn plugin_list_omits_featured_plugin_ids_without_chatgpt_auth() -> Result<()> {
    let workx_home = TempDir::new()?;
    let server = MockServer::start().await;
    write_plugin_sync_config(workx_home.path(), &format!("{}/backend-api/", server.uri()))?;
    write_openai_api_curated_marketplace(workx_home.path(), &["linear", "gmail"])?;
...
async fn plugin_list_uses_warmed_featured_plugin_ids_cache_on_first_request() -> Result<()> {
    ...
    let server = MockServer::start().await;
    write_plugin_sync_config(workx_home.path(), &format!("{}/backend-api/", server.uri()))?;
    write_openai_curated_marketplace(workx_home.path(), &["linear", "gmail"])?;
    write_remote_plugin_test_auth(workx_home.path())?;
...
async fn wait_for_cached_remote_catalog_plugin_ids(
    ...
    timeout(DEFAULT_TIMEOUT, async {
        loop {
            let plugin_ids = cached_remote_catalog_plugin_ids(workx_home)?;
...
fn cached_remote_catalog_plugin_ids(workx_home: &std::path::Path) -> Result<Vec<String>> {
    let cache_dir = workx_home.join("cache/remote_plugin_catalog");
    if !cache_dir.exists() {
        return Ok(Vec::new());
    }
...
fn write_installed_plugin(
    ...
    write_installed_plugin_with_version(workx_home, marketplace_name, plugin_name, "local")
}
...
fn write_plugin_sync_config(workx_home: &std::path::Path, base_url: &str) -> std::io::Result<()> {
    std::fs::write(
        workx_home.join("config.toml"),
        format!(
            r#"
...
fn write_remote_plugin_test_auth(workx_home: &std::path::Path) -> Result<()> {
    write_chatgpt_auth(
        workx_home,
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
...
fn write_curated_marketplace(
    ...
    let curated_root = workx_home.join(".tmp/plugins");
...
{plugins}
    ...
    for plugin_name in plugin_names {
        let plugin_root = curated_root.join(format!("plugins/{plugin_name}/.workx-plugin"));
        std::fs::create_dir_all(&plugin_root)?;
        std::fs::write(
            plugin_root.join("plugin.json"),
            format!(r#"{{"name":"{plugin_name}"}}"#),
    ...
    std::fs::create_dir_all(workx_home.join(".tmp"))?;
    std::fs::write(
        workx_home.join(".tmp/plugins.sha"),
...
fn write_plugin_share_local_path_mapping(
    ...
    std::fs::create_dir_all(workx_home.join(".tmp"))?;
    std::fs::write(
        workx_home.join(".tmp/plugin-share-local-paths-v1.json"),
        format!("{contents}\n"),
    )
}

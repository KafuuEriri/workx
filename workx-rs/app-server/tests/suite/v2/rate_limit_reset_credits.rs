...
const RATE_LIMIT_RESET_REQUEST_TIMEOUT_ENV_VAR: &str =
    "WORKX_TEST_RATE_LIMIT_RESET_REQUEST_TIMEOUT_MS";
...
async fn consume_rate_limit_reset_credit_requires_chatgpt_auth() -> Result<()> {
    ...
    let mut mcp = initialized_app_server(workx_home.path()).await?;
    ...
    assert_eq!(
        ...
        "workx account authentication required for rate limit reset credits"
    );
...
async fn consume_account_rate_limit_reset_credit_maps_backend_outcomes() -> Result<()> {
    let (workx_home, server) = chatgpt_test_context().await?;
    let cases = [
        (
            "request-reset",
            "reset",
...
async fn consume_account_rate_limit_reset_credit_rejects_empty_idempotency_key() -> Result<()> {
    let (workx_home, _server) = chatgpt_test_context().await?;
    let mut mcp = initialized_app_server(workx_home.path()).await?;
...
async fn chatgpt_test_context() -> Result<(TempDir, MockServer)> {
    ...
    Ok((workx_home, server))
}

async fn initialized_app_server(workx_home: &Path) -> Result<TestAppServer> {

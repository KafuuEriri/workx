...
const PROVIDER_RESIDENCY_ENV_VAR: &str = "WORKX_TEST_RESIDENCY_HEADER";
...
"X-OpenAI-Internal-Workx-Residency" = "eu-static"
...
async fn managed_residency_overrides_provider_headers(transport: ModelTransport) -> Result<()> {
    ...
    write_provider_config(workx_home.path(), &model_base_url, transport)?;
    std::fs::write(

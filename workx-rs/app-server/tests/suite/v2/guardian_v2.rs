...
use workx_app_server_protocol::ItemGuardianApprovalReviewStartedNotification;
...
async fn guardian_v2_routes_scoped_tool_approvals(
    ...
    let classifier_in_scope = match scope {
        ...
        GuardianToolScope::ComputerUseOnly { .. } => {
            workx_protocol::mcp::is_node_repl_backed_server(server_name)
        }
    };
        ...
        .route(
            ...
            post(
                |State(state): State<Arc<MockResponsesState>>, body: String| async move {
                    if body.contains("workx.guardian_v2.classification") {
                        state.classification_completed.notify_one();
                    }
                    if body.contains("workx.guardian_v2.classification.truncation")
    ...
    mount_analytics_capture(&analytics_server, workx_home.path()).await?;
    let root_skill = if matches!(lifecycle, ThreadLifecycle::RootTrustedSkill) {
        let path = workx_home.path().join("skills/root-trusted/SKILL.md");
    ...
    if lifecycle.has_post_tool_hook() {
        ...
        let hook_path = workx_home.path().join("guardian-post-tool-hook.py");
    ...
    mock_config.write(workx_home.path())?;
    if node_repl_review_required {
        let config = load_default_config_for_test(&workx_home).await;
        let mut model_info = workx_core::test_support::construct_model_info_offline(MODEL, &config);
        model_info.node_repl_auto_review_required = true;
        write_models_cache_with_models(workx_home.path(), vec![model_info])?;
    }
    ...
    if classifier_in_scope {
        ...
        if !lifecycle.uses_root_worker() {
            let trusted_tool_context = luna_request["input"]
                ...
                .find(|text| text.starts_with("Workx verified that this exact MCP tool"))
            ...
            assert_eq!(
                ...
                workx_home.path().join("config.toml").canonicalize()?,
            );
        }
        ...
        && sensitive_action.is_none()
    {
        if classifier_in_scope {
            wait_for_matching_analytics_event(&analytics_server, TIMEOUT, |event| {
                event["event_type"] == "workx_guardian_v2_classification"
        ...
        let approvals = events
            .iter()
            .filter(|event| event["event_type"] == "workx_guardian_v2_fast_decision")
            .collect::<Vec<_>>();
        assert_eq!(
            approvals.len(),
        ...
        for event in classification.into_iter().chain(approvals) {
            ...
            if event["event_type"] == "workx_guardian_v2_fast_decision" {
                assert_eq!(
...
async fn guardian_v2_trusts_invoked_user_skills_but_rejects_repository_forgery() -> Result<()> {
    ...
    let workx_home = TempDir::new()?;
    let explicit_skill = workx_home.path().join("skills/explicit/SKILL.md");
    let implicit_skill = workx_home.path().join("skills/implicit/SKILL.md");

...
use workx_app_server_protocol::BedrockDiscoverParams;
use workx_app_server_protocol::BedrockDiscoverResponse;
use workx_app_server_protocol::BedrockSetupParams;
use workx_app_server_protocol::BedrockSetupResponse;
...
async fn bedrock_app_server(
    ...
    let aws_config = workx_home.join("aws-config");
    let aws_credentials = workx_home.join("aws-credentials");
...
async fn setup_bedrock_profile_and_environment() -> Result<()> {
    ...
    assert_eq!(
        error.error.message,
        "Workx-managed Bedrock credentials are already configured and take priority over AWS environment credentials. Run `workx logout` and try again."
    );
...
async fn setup_bedrock_rejects_invalid_or_conflicting_credentials() -> Result<()> {
    let workx_home = TempDir::new()?;
    let mut app_server = bedrock_app_server(workx_home.path(), &[]).await?;
    ...
    assert!(!workx_home.path().join(".env").exists());
    assert!(!workx_home.path().join("config.toml").exists());

...
use workx_app_server_protocol::ProjectChangeType;
use workx_app_server_protocol::ProjectChangedNotification;
use workx_app_server_protocol::ProjectCreateParams;
use workx_app_server_protocol::ProjectCreateResponse;
use workx_app_server_protocol::ProjectDeleteParams;
use workx_app_server_protocol::ProjectDeleteResponse;
use workx_app_server_protocol::ProjectImportParams;
use workx_app_server_protocol::ProjectImportResponse;
use workx_app_server_protocol::ProjectListParams;
use workx_app_server_protocol::ProjectListResponse;
use workx_app_server_protocol::ProjectMoveParams;
use workx_app_server_protocol::ProjectMoveResponse;
use workx_app_server_protocol::ProjectReadParams;
use workx_app_server_protocol::ProjectReadResponse;
use workx_app_server_protocol::ProjectRoot;
use workx_app_server_protocol::ProjectSortKey;
use workx_app_server_protocol::ProjectUpdateParams;
use workx_app_server_protocol::ProjectUpdateResponse;
...
async fn projects_persist_and_assign_threads() -> Result<()> {
    ...
    MockResponsesConfig::new(&responses.uri())
        .enable_feature(Feature::Sqlite)
        .write(workx_home.path())?;
    let root = AbsolutePathBuf::from_absolute_path(workx_home.path())?;
    let mut server = TestAppServer::builder()
        .with_workx_home(workx_home.path())
        .build_initialized()
        .await?;
...
async fn projects_validate_filters_cursors_and_sqlite_less_assignment() -> Result<()> {
    ...
    let mut unsupported_projects = TestAppServer::builder()
        .with_workx_home(unsupported_projects_home.path())
        .build_initialized()
        .await?;

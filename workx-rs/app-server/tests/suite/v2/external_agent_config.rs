...
use workx_app_server_protocol::ExternalAgentConfigImportHistoriesReadResponse;
use workx_app_server_protocol::ExternalAgentConfigImportHistoryRecordResponse;
use workx_app_server_protocol::ExternalAgentConfigImportProgressNotification;
...
use workx_app_server_protocol::ExternalAgentImportedConnectorCandidate;
use workx_app_server_protocol::ExternalAgentImportedConnectorSource;
#[cfg(unix)]
...
fn external_agent_home(workx_home: &Path) -> PathBuf {
    workx_home.join(concat!(".", "cla", "ude"))
}
...
fn secondary_external_agent_home(workx_home: &Path) -> PathBuf {
    workx_home.join(concat!(".", "cur", "sor"))
}
...
-> Result<()> {
    ...
    let source_home = external_agent_home(workx_home.path());
    std::fs::create_dir_all(&source_home)?;
    std::fs::write(source_home.join("CLAUDE.md"), "project instructions")?;
    let home_dir = workx_home.path().display().to_string();
...
async fn external_agent_config_import_skips_repository_redirect_after_detection() -> Result<()> {
    ...
    let repo_config_dir = repo_root.join(".workx");
    let global_config = workx_home.path().join("config.toml");
    ...
    std::os::unix::fs::symlink(workx_home.path(), &repo_config_dir)?;
...
async fn external_agent_config_detect_does_not_block_configuration_reads() -> Result<()> {
    let workx_home = TempDir::new()?;
    let project_root = workx_home.path().join("repo");
    let session_dir = external_agent_home(workx_home.path()).join("projects/repo");
    ...
    assert!(
        std::fs::read_to_string(workx_home.path().join("config.toml"))?
...
async fn external_agent_config_migration_source_drives_detect_and_import() -> Result<()> {
    let workx_home = TempDir::new()?;
    let source_home = secondary_external_agent_home(workx_home.path());
    std::fs::create_dir_all(&source_home)?;
    std::fs::write(
...
async fn external_agent_config_import_source_remains_attribution_only() -> Result<()> {
    ...
    assert_eq!(completed.item_type_results[0].failures, Vec::new());
    assert_eq!(
        std::fs::read_to_string(workx_home.path().join("AGENTS.md"))?,
        "Workx guidance"
    );
...
async fn external_agent_config_secondary_source_imports_session_and_plugin_end_to_end() -> Result<()>
{
    let workx_home = TempDir::new()?;
    let source_home = secondary_external_agent_home(workx_home.path());
    let project_root = workx_home.path().join("my-project");
    std::fs::create_dir_all(&project_root)?;
    ...
    let configured_marketplace_root = workx_home.path().join("configured-marketplace");
    ...
    std::fs::create_dir_all(configured_plugin_root.join(".workx-plugin"))?;
    std::fs::write(
  ...
  "plugins": [{
    ...
    std::fs::write(
        configured_plugin_root.join(".workx-plugin/plugin.json"),
...
"#,
    ...
    assert_eq!(
        marketplace
            .path
            .as_ref()
            .map(workx_config::AbsolutePathBuf::as_path),
...
-> Result<()> {
    ...
    let mut mcp = TestAppServer::builder()
        ...
        .with_env_overrides(&[
            ...
            ("WORKX_SQLITE_HOME", Some(sqlite_home_dir.as_str())),
        ])
        .build_initialized_with_timeout(DEFAULT_TIMEOUT)
        .await?;
    ...
    let state_db = workx_state::StateRuntime::init(
        workx_state::SqliteConfig::new_for_testing(sqlite_home.path().abs()),
        "mock_provider".into(),
    )
    .await?;
...
async fn external_agent_config_detects_and_imports_project_memory_files() -> Result<()> {
    ...
    let project_cwd = workx_home.path().join("project-a");
    std::fs::create_dir_all(&source_memory)?;
    ...
    let expected_resources_root = workx_home
        .path()
    ...
    let memory_diff = workx_git_utils::diff_since_latest_init(&memory_root).await?;
    ...
    workx_memories_write::workspace::reset_memory_workspace_baseline(&memory_root).await?;
    ...
    assert_eq!(
        ...
        vec![
            (
                workx_git_utils::GitBaselineChangeStatus::Deleted,
                "extensions/external_agent_import/resources/project-a/MEMORY.md",
            ),
            (
                workx_git_utils::GitBaselineChangeStatus::Deleted,
...
async fn external_agent_config_import_reports_failed_sync_import_in_completion() -> Result<()> {
    ...
    std::fs::write(workx_home.path().join("config.toml"), "invalid = [")?;
    let home_dir = workx_home.path().display().to_string();
    let analytics_capture_file = workx_home.path().join("analytics-events.jsonl");
    ...
    let mut mcp = TestAppServer::builder()
        ...
        .with_env_overrides(&[
            ("HOME", Some(home_dir.as_str())),
            (
                "WORKX_ANALYTICS_EVENTS_CAPTURE_FILE",
    ...
    let events = timeout(DEFAULT_TIMEOUT, async {
        loop {
            ...
            if captured_events.iter().any(|event| {
                event["event_type"] == "workx_onboarding_external_agent_import_complete"
    ...
    let event = events
        .iter()
        .find(|event| {
            event["event_type"] == "workx_onboarding_external_agent_import_failure"
...
async fn external_agent_config_import_completed_tracks_analytics_event() -> Result<()> {
    ...
    let workx_home = TempDir::new()?;
    write_analytics_config(workx_home.path(), &analytics_server.uri())?;
    write_chatgpt_auth(
        workx_home.path(),
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
    ...
    let missing_session_path =
        external_agent_home(workx_home.path()).join("projects/repo/missing.jsonl");
    let project_root = workx_home.path().join("repo");
    let home_dir = workx_home.path().display().to_string();
    let mut mcp = TestAppServer::builder()
        .with_workx_home(workx_home.path())
    ...
    let event = wait_for_analytics_event(
        &analytics_server,
        DEFAULT_TIMEOUT,
        "workx_onboarding_external_agent_import_complete",
    )
    .await?;
    let event_params = &event["event_params"];
    ...
    let event = wait_for_analytics_event(
        &analytics_server,
        DEFAULT_TIMEOUT,
        "workx_onboarding_external_agent_import_failure",
    )
    .await?;
    let event_params = &event["event_params"];
    assert_eq!(event_params["import_id"], serde_json::json!(import_id));
...
async fn external_agent_config_import_reinstalls_plugins_from_known_marketplaces() -> Result<()> {
  ...
  "plugins": [
    ...
    assert_eq!(
        ...
        Some(vec![workx_app_server_protocol::PluginsMigration {
...
async fn external_agent_config_import_creates_session_rollouts() -> Result<()> {
    ...
    let session_dir = external_agent_home(workx_home.path()).join("projects/repo");
    let session_path = session_dir.join("session.jsonl");
    let manifest_dir = connector_metadata_root(workx_home.path())
...
async fn external_agent_config_import_does_not_initialize_required_mcp() -> Result<()> {
    ...
    let workx_home = TempDir::new()?;
    MockResponsesConfig::new(&server.uri()).write(workx_home.path())?;
    let mut config = std::fs::read_to_string(workx_home.path().join("config.toml"))?;
    config.push_str(
        r#"
...
fn write_analytics_config(workx_home: &std::path::Path, base_url: &str) -> std::io::Result<()> {
    std::fs::write(

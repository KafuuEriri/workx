...
fn init_marketplace_repo(root: &Path, marketplace_name: &str, marker: &str) -> Result<String> {
    run_git(root, &["init"])?;
    run_git(root, &["config", "user.email", "workx@example.com"])?;
    run_git(root, &["config", "user.name", "Workx Tests"])?;
...
fn disable_plugin_startup_tasks(workx_home: &Path) -> Result<()> {
...
fn marketplace_install_root(workx_home: &Path) -> std::path::PathBuf {
    workx_home.join(INSTALLED_MARKETPLACES_DIR)
}

fn expected_installed_root(workx_home: &Path, marketplace_name: &str) -> Result<AbsolutePathBuf> {
    AbsolutePathBuf::try_from(
        marketplace_install_root(&workx_home.canonicalize()?).join(marketplace_name),
    )
...
async fn marketplace_upgrade_all_configured_git_marketplaces() -> Result<()> {
    ...
    disable_plugin_startup_tasks(workx_home.path())?;
    ...
    let debug_root = expected_installed_root(workx_home.path(), "debug")?;
    let tools_root = expected_installed_root(workx_home.path(), "tools")?;
    ...
    for (root, expected_revision) in [
        ...
        let metadata: serde_json::Value = serde_json::from_str(&std::fs::read_to_string(
            root.as_path().join(".workx-marketplace-install.json"),
        )?)?;
        assert_eq!(
...
async fn automatic_upgrade_isolates_git_while_explicit_install_preserves_configuration(
    ...
    let plugin_root = plugin_source.path().join(".workx-plugin");
    ...
    run_git(workx_home.path(), &["init", "--quiet"])?;
    let source_path = source.path().display().to_string();
    let missing_path = workx_home.path().join("missing").display().to_string();
    ...
    for untrusted_source in [&source_path, &plugin_url] {
        ...
        assert!(run_git(workx_home.path(), &["ls-remote", untrusted_source, "HEAD"]).is_err());
    }
    ...
    let plugin_root = workx_home
        .path()
        .join("plugins/cache/trusted/toolkit/1.0.0/.workx-plugin/plugin.json");
    ...
    let marketplace_path = marketplace_install_root(workx_home.path())
    ...
    std::fs::write(
        plugin_source.path().join(".workx-plugin/plugin.json"),
    ...
    assert!(
        workx_home
            .path()
            .join("plugins/cache/trusted/toolkit/1.1.0/.workx-plugin/plugin.json")
            .is_file(),
...
async fn marketplace_upgrade_named_marketplace_only() -> Result<()> {
    ...
    assert!(
        !marketplace_install_root(workx_home.path())
            .join("debug")
            .exists()
    );
    Ok(())

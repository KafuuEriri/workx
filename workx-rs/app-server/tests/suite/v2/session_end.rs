...
async fn run_removal_session_end_test(operation: &str) -> Result<()> {
    ...
    let log_path = write_config_and_hook(workx_home.path(), &server.uri())?;
...
fn write_config_and_hook(workx_home: &Path, server_uri: &str) -> Result<std::path::PathBuf> {
    let log_path = workx_home.join("session-end.jsonl");
    let script_path = workx_home.join("session-end.py");
    std::fs::write(
        &script_path,
        format!(
            r#"import json

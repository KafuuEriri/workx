...
use workx_app_server_protocol::McpServerConnectionStatus;
...
-> Result<()> {
    let server = create_mock_responses_server_sequence_unchecked(Vec::new()).await;
    let workx_home = TempDir::new()?;
    let barrier_file = workx_home.path().join("allow-initialize");
    let pid_file = workx_home.path().join("mcp.pid");
...
async fn mcp_server_status_list_reports_disconnected_stdio_transport() -> Result<()> {
    ...
    let server = create_mock_responses_server_sequence_unchecked(Vec::new()).await;
    let workx_home = TempDir::new()?;
    let exit_file = workx_home.path().join("exit-mcp-server");
    mock_responses_config(&server.uri())
        .with_extra_config(&format!(

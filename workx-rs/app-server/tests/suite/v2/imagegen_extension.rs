...
use workx_app_server_protocol::ImageGenerationFailure;
...
async fn standalone_image_generation_returns_saved_path_hint_to_model() -> Result<()> {
    ...
    create_config_toml(workx_home.path(), &server.uri(), ImagegenTestMode::Direct)?;
...
async fn standalone_image_edit_uses_attached_model_visible_image() -> Result<()> {
    ...
    let (edit_request, _) = run_image_edit_test(|workx_home| {
        let image_path = workx_home.join("attached.png");
...
async fn run_image_edit_test(
    ...
    let workx_home = TempDir::new()?;
    let (arguments, input) = input(workx_home.path())?;
    let response_mock = responses::mount_sse_sequence(
        &server,
        vec![
            responses::sse(vec![

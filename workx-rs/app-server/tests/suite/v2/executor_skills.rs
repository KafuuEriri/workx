...
use workx_app_server_protocol::GrantedPermissionProfile;
...
"#,
    ...
    let manifest_dir = plugin_dir.join(".workx-plugin")?;
    ...
    #[cfg(unix)]
    if scenario == ExecutorSkillScenario::RestrictedDeniedReference {
        let external_reference_dir = workx_home.path().join("external-reference");
    ...
    if scenario == ExecutorSkillScenario::RestrictedVisible && !auto_env.environment().is_remote() {
        let denied_skill_dir = workx_home.path().join("denied-skill");

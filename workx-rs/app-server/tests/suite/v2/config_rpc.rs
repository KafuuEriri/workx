...
use workx_app_server_protocol::AppConfig;
use workx_app_server_protocol::AppLinkConfig;
use workx_app_server_protocol::AppLinksConfig;
use workx_app_server_protocol::AppToolApproval;
use workx_app_server_protocol::ApprovalsReviewer;
use workx_app_server_protocol::AppsConfig;
use workx_app_server_protocol::AppsDefaultConfig;
use workx_app_server_protocol::AskForApproval;
use workx_app_server_protocol::BrowserUseAccessApprovalLifetime;
use workx_app_server_protocol::BrowserUseConfig;
use workx_app_server_protocol::BrowserUseOriginPolicy;
use workx_app_server_protocol::BrowserUseOriginPolicyConfig;
use workx_app_server_protocol::BrowserUseRequirements;
use workx_app_server_protocol::CliAuthCredentialsStoreMode;
use workx_app_server_protocol::ComputerUseConfig;
use workx_app_server_protocol::ComputerUseMacosConfig;
use workx_app_server_protocol::ComputerUseMacosRequirements;
use workx_app_server_protocol::ComputerUseRequirements;
use workx_app_server_protocol::ComputerUseWindowsConfig;
use workx_app_server_protocol::ComputerUseWindowsExeConfig;
...
use workx_app_server_protocol::ConfigLayerSource;
...
use workx_app_server_protocol::ForcedChatgptWorkspaceIds;
...
use workx_app_server_protocol::ToolsV2;
...
use workx_protocol::config_types::WebSearchContextSize;
use workx_protocol::config_types::WebSearchLocation;
use workx_protocol::config_types::WebSearchToolConfig;
...
fn write_config(workx_home: &TempDir, contents: &str) -> Result<()> {
    Ok(std::fs::write(
...
async fn config_read_disables_guardian_v2_when_managed_config_requires_guardian_v1() -> Result<()> {
    let workx_home = TempDir::new()?;
    write_config(&workx_home, "[features]\nguardianv2 = true\n")?;
    std::fs::write(
        workx_home.path().join("requirements.toml"),
...
"#,
    ...
    let workx_home_path = workx_home.path().canonicalize()?;
    let user_file = AbsolutePathBuf::try_from(workx_home_path.join("config.toml"))?;
...
"#,
    ...
    set_project_trust_level(workx_home.path(), workspace.path(), TrustLevel::Trusted)?;
    let workx_home_path = workx_home.path().canonicalize()?;
    let user_file = AbsolutePathBuf::try_from(workx_home_path.join("config.toml"))?;
    ...
    assert_eq!(
        ...
        ConfigLayerSource::Project {
            dot_workx_folder: project_config.clone(),
        }
    );
    assert_eq!(
        origins
    ...
    assert_eq!(
        origins
            ...
            .name,
        ConfigLayerSource::Project {
            dot_workx_folder: project_config,
        }
    );
...
"#,
    ...
    assert_eq!(desktop.get("selected-avatar-id"), Some(&json!("workx")));
    assert_eq!(
...
async fn config_read_includes_project_layers_for_cwd() -> Result<()> {
    let workx_home = TempDir::new()?;
    write_config(&workx_home, r#"model = "gpt-user""#)?;
...
async fn config_read_respects_managed_project_root_markers() -> Result<()> {
    let workx_home = TempDir::new()?;
    write_config(&workx_home, "model_context_window = 16384\n")?;
    let workspace = TempDir::new()?;
    let ancestor_config = workspace.path().join(".workx");
    let child = workspace.path().join("child");
    let child_config = child.join(".workx");
    for dir in [
    ...
    let mut app_server = TestAppServer::builder()
        .with_workx_home(workx_home.path())
        .without_auto_env()
        .with_env_overrides(&[("WORKX_APP_SERVER_MANAGED_CONFIG_PATH", Some(&managed_path))])
        .build_initialized_with_timeout(DEFAULT_READ_TIMEOUT)
        .await?;
    ...
    let project_layers = layers
        ...
        .into_iter()
        .filter_map(|layer| {
            if let ConfigLayerSource::Project { dot_workx_folder } = layer.name {
                Some((dot_workx_folder, layer.config, layer.disabled_reason))
            } else {
                None
            }
        })
...
async fn config_value_write_replaces_value() -> Result<()> {
    ...
    let workx_home = temp_dir.path().canonicalize()?;
    write_config(
        &temp_dir,
        r#"
model = "gpt-old"
"#,
    ...
    let expected_file_path = AbsolutePathBuf::resolve_path_against_base("config.toml", workx_home);
...
"#,
    ...
    let write_id = mcp
        .send_config_value_write_request(ConfigValueWriteParams {
            file_path: Some(workx_home.path().join("config.toml").display().to_string()),
            key_path: "model".to_string(),
            value: json!("gpt-new"),
            merge_strategy: MergeStrategy::Replace,
...
async fn config_batch_write_applies_multiple_edits() -> Result<()> {
    ...
    let workx_home = tmp_dir.path().canonicalize()?;
    ...
    let batch_id = mcp
        .send_config_batch_write_request(ConfigBatchWriteParams {
            file_path: Some(workx_home.join("config.toml").display().to_string()),
            edits: vec![
                ConfigEdit {
...
"#,
    ...
    assert_eq!(
        ...
        AbsolutePathBuf::resolve_path_against_base("config.toml", &workx_home)
    );
    assert_eq!(batch_write.overridden_metadata, None);

    let persisted: toml::Value =
        toml::from_str(&std::fs::read_to_string(workx_home.join("config.toml"))?)?;
...
fn assert_layers_user_then_optional_system(
    layers: &[workx_app_server_protocol::ConfigLayer],

...
use workx_app_server_protocol::ConnectorMetadata;
...
async fn app_read_deduplicates_orders_partial_misses_and_reuses_cached_metadata() -> Result<()> {
    ...
    write_apps_config(workx_home.path(), &server_url, Some("tpp"))?;
...
"#,
    ...
    write_plugin_app(workx_home.path(), "alpha-z", "Alpha Z", "alpha")?;
    write_plugin_app(workx_home.path(), "alpha-a", "Alpha A", "alpha")?;
    write_plugin_app(
        workx_home.path(),
        "disabled",
        "Disabled Plugin",

...
async fn computer_use_scoring_follows_model_review_requirement(
    ...
    let ordinary_model = workx_core::test_support::construct_model_info_offline(MODEL, &config);
    let mut reviewed_model =
        workx_core::test_support::construct_model_info_offline(REVIEWED_MODEL, &config);
    reviewed_model.node_repl_auto_review_required = true;
    write_models_cache_with_models(workx_home.path(), vec![ordinary_model, reviewed_model])?;

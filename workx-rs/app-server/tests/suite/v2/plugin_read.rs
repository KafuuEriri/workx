...
use workx_app_server_protocol::AppInfo;
use workx_app_server_protocol::AppMetadata;
...
use workx_app_server_protocol::HookEventName;
...
use workx_app_server_protocol::ScheduledTaskSchedule;
use workx_app_server_protocol::ScheduledTaskSummary;
use workx_app_server_protocol::ScheduledTaskWeekday;
use workx_app_server_protocol::SkillInterface;
    ...
    let request_id = mcp
        .send_plugin_read_request(PluginReadParams {
            marketplace_path: Some(AbsolutePathBuf::try_from(
                workx_home.path().join("marketplace.json"),
            )?),
...
async fn plugin_read_rejects_invalid_remote_plugin_name() -> Result<()> {
    let workx_home = TempDir::new()?;
    write_remote_plugin_catalog_config(workx_home.path(), "https://example.invalid/backend-api/")?;
    let mut mcp = TestAppServer::builder()
        .with_workx_home(workx_home.path())
        .without_auto_env()
        .build_initialized_with_timeout(DEFAULT_TIMEOUT)
...
async fn plugin_read_returns_canonical_openai_curated_marketplace_name() -> Result<()> {
    let workx_home = TempDir::new()?;
    let repo_root = workx_home.path().join(".tmp/plugins");
    write_plugin_marketplace(&repo_root, "openai-curated", "demo-plugin", "./demo-plugin")?;
    std::fs::create_dir_all(repo_root.join("demo-plugin/.workx-plugin"))?;
    std::fs::write(
        repo_root.join("demo-plugin/.workx-plugin/plugin.json"),
        r#"{
  "name": "demo-plugin",
...
"#,
    ...
    write_installed_plugin(&workx_home, "openai-curated", "demo-plugin")?;
...
async fn plugin_read_returns_share_context_for_shared_local_plugin() -> Result<()> {
    ...
    std::fs::create_dir_all(repo_root.path().join("demo-plugin/.workx-plugin"))?;
    std::fs::write(
        repo_root
            .path()
            .join("demo-plugin/.workx-plugin/plugin.json"),
    ...
    write_plugin_share_local_path_mapping(workx_home.path(), "plugins_123", &plugin_path)?;
    Mock::given(method("GET"))
...
async fn plugin_read_falls_back_to_local_share_context_without_remote_auth() -> Result<()> {
    let workx_home = TempDir::new()?;
    let repo_root = TempDir::new()?;
    write_plugins_enabled_config(&workx_home)?;
    write_plugin_marketplace(
        repo_root.path(),
        "workx-curated",
        "demo-plugin",
...
async fn plugin_read_fails_on_malformed_share_mapping() -> Result<()> {
    ...
    write_plugin_source(repo_root.path(), "demo-plugin", &[])?;
    std::fs::create_dir_all(workx_home.path().join(".tmp"))?;
    std::fs::write(
        workx_home
            .path()
...
async fn plugin_read_agent_plugin_excludes_nested_skills() -> Result<()> {
    ...
    assert_eq!(
        ...
        vec![workx_app_server_protocol::SkillSummary {
...
[plugins."demo-plugin@workx-curated"]
enabled = true

[hooks.state."demo-plugin@workx-curated:hooks/hooks.json:pre_tool_use:0:0"]
enabled = false
"#,
    )?;
    write_installed_plugin(&workx_home, "workx-curated", "demo-plugin")?;
    ...
    assert_eq!(response.plugin.marketplace_name, "workx-curated");
    assert_eq!(response.plugin.marketplace_path, Some(marketplace_path));
    assert_eq!(response.plugin.summary.id, "demo-plugin@workx-curated");
    assert_eq!(response.plugin.summary.name, "demo-plugin");
    assert_eq!(
        response.plugin.description.as_deref(),
    ...
    assert_eq!(
        ...
        vec![
            workx_app_server_protocol::PluginHookSummary {
                key: "demo-plugin@workx-curated:hooks/hooks.json:pre_tool_use:0:0".to_string(),
                event_name: HookEventName::PreToolUse,
            },
            workx_app_server_protocol::PluginHookSummary {
                key: "demo-plugin@workx-curated:hooks/hooks.json:pre_tool_use:0:1".to_string(),
                event_name: HookEventName::PreToolUse,
            },
            workx_app_server_protocol::PluginHookSummary {
                key: "demo-plugin@workx-curated:hooks/hooks.json:session_start:0:0".to_string(),
...
async fn plugin_read_batches_large_app_metadata_requests() -> Result<()> {
    ...
    let workx_home = TempDir::new()?;
    write_connectors_config(workx_home.path(), &server_url)?;
    write_chatgpt_auth(
        workx_home.path(),
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
...
async fn plugin_read_stops_batching_after_app_metadata_failure() -> Result<()> {
    ...
    let workx_home = TempDir::new()?;
    write_connectors_config(workx_home.path(), &server.uri())?;
    write_chatgpt_auth(
        workx_home.path(),
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
...
fn write_plugins_enabled_config(workx_home: &TempDir) -> Result<()> {
    std::fs::write(
        workx_home.path().join("config.toml"),
        r#"[features]
plugins = true
...
fn write_connectors_config(workx_home: &std::path::Path, base_url: &str) -> std::io::Result<()> {
    std::fs::write(

...
use workx_rmcp_client::McpOAuthCallbackMode;
use workx_rmcp_client::resolve_mcp_oauth_callback_url;
...
async fn plugin_install_writes_remote_plugin_to_cloud_and_cache() -> Result<()> {
    ...
    .await;
    configure_remote_plugin_test(workx_home.path(), &server)?;
    ...
    mount_remote_plugin_install_after_cache_write(
        &server,
        REMOTE_PLUGIN_ID,
        installed_path.join(".workx-plugin/plugin.json"),
    )
    .await;
...
async fn plugin_install_uses_remote_apps_needing_auth_response() -> Result<()> {
    ...
    .await;
    configure_remote_plugin_with_apps_test(workx_home.path(), &server)?;
    mount_remote_plugin_detail_with_app_manifest(
        &server,
        REMOTE_PLUGIN_ID,
        "1.2.3",
...
async fn plugin_install_returns_invalid_request_for_missing_marketplace_file() -> Result<()> {
    ...
    let request_id = mcp
        .send_plugin_install_request(PluginInstallParams {
            marketplace_path: Some(AbsolutePathBuf::try_from(
                workx_home.path().join("missing-marketplace.json"),
            )?),
            remote_marketplace_name: None,
            install_attempt_id: None,
...
async fn plugin_install_tracks_analytics_when_marketplace_file_cannot_be_read() -> Result<()> {
    ...
    let marketplace_path = workx_home.path().join("marketplace-dir");
...
async fn plugin_install_skips_mcp_oauth_for_chatgpt_dual_surface_plugin() -> Result<()> {
    ...
    let workx_home = TempDir::new()?;
    write_connectors_config(workx_home.path(), &apps_server_url)?;
    write_chatgpt_auth(
        workx_home.path(),
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
...
"#,
    ...
    let persisted_config = std::fs::read_to_string(workx_home.path().join("config.toml"))?;
...
async fn plugin_install_skips_mcp_oauth_for_unowned_environment() -> Result<()> {
    ...
    let mut executor =
        tokio::process::Command::new(workx_utils_cargo_bin::cargo_bin("exec-server")?)
    ...
    std::fs::write(
        workx_home.path().join("environments.toml"),
        format!(
            ...
            if let Some(request) = requests.iter().find(|request| {
                request.method == "POST"
                    && request
                        .url
                        .path()
                        .ends_with("/workx/analytics-events/events")
            }) {
...
fn configure_remote_plugin_test(workx_home: &std::path::Path, server: &MockServer) -> Result<()> {
    write_remote_plugin_catalog_config(workx_home, &format!("{}/backend-api/", server.uri()))?;
    write_chatgpt_auth(
        workx_home,
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")

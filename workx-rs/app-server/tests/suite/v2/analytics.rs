...
use workx_config::types::OtelHttpProtocol;
use workx_core::config::ConfigBuilder;
use workx_core_plugins::loader::curated_plugin_cache_version;
...
async fn guardian_review_turns_and_tools_reach_analytics() -> Result<()> {
    ...
    let turns = children
        .iter()
        .filter(|event| event["event_type"] == "workx_turn_event")
        .map(|event| {
            let params = &event["event_params"];
    ...
    let tools = children
        .iter()
        .filter(|event| event["event_type"] == "workx_web_search_event")
        .collect::<Vec<_>>();
...
fn set_metrics_exporter(config: &mut workx_core::config::Config) {
...
async fn app_server_default_analytics_disabled_without_flag() -> Result<()> {
    ...
    let provider = workx_core::otel_init::build_provider(
        &config,
        SERVICE_VERSION,
        Some("workx-app-server"),
...
pub(crate) async fn mount_analytics_capture(server: &MockServer, workx_home: &Path) -> Result<()> {
    Mock::given(method("POST"))
    ...
    let body = timeout(read_timeout, async {
        loop {
            ...
            if let Some(request) = requests.iter().find(|request| {
                request.method == "POST" && request.url.path() == "/workx/analytics-events/events"
            }) {
...
pub(crate) async fn wait_for_goal_event(
    ...
    wait_for_matching_analytics_event(server, read_timeout, |event| {
        event["event_type"] == "workx_goal_event"
        ...
        loop {
            ...
            for request in &requests {
                if request.method != "POST"
                    || request.url.path() != "/workx/analytics-events/events"
                {
                    continue;
                }
...
pub(crate) fn thread_initialized_event(payload: &Value) -> Result<&Value> {
    ...
    events
        ...
        .ok_or_else(|| anyhow::anyhow!("workx_thread_initialized event should be present"))
}
...
fn write_curated_metrics_plugin(workx_home: &Path) -> Result<PathBuf> {
    let plugin_id = PluginId::parse(METRICS_PLUGIN_ID)?;
    let plugin_root = PluginStore::new(workx_home.to_path_buf()).plugin_root(
        &plugin_id,
...
"#,
    ...
    std::fs::write(
        &script_path,
        r#"test -n "$WORKX_PLUGIN_METRICS_OUTPUT"
sleep "${1:-0.3}"
printf '%s' '{"version":1,"measurements":[{"name":"findings","value":3,"dimensions":{"severity":"high"}},{"name":"files_scanned","value":17}]}' > "$WORKX_PLUGIN_METRICS_OUTPUT"
"#,
    )?;

    let curated_repo = workx_home.join(".tmp/plugins");
...
async fn assert_plugin_measurement_analytics(remote: bool, background: bool) -> Result<()> {
    ...
    skip_if_remote!(
        Ok(()),
        "trusted plugin metrics fixture uses a local Workx home cache"
    );
    skip_if_wine_exec!(Ok(()), "plugin metrics fixture is Unix-only");

    let workx_home = TempDir::new()?;
    let script_path = write_curated_metrics_plugin(workx_home.path())?.canonicalize()?;
    let mut command = vec![
        "/bin/sh".to_string(),
...
"#,
    ...
    if remote {
        assert_eq!(
            ...
            workx_exec_server::REMOTE_ENVIRONMENT_ID
        );
    }
    ...
    for measurement_name in ["findings", "files_scanned"] {
        wait_for_matching_analytics_event(&analytics_server, Duration::from_secs(10), |event| {
            event["event_type"] == "workx_plugin_measurement_event"
    ...
    let command_event =
        wait_for_matching_analytics_event(&analytics_server, Duration::from_secs(10), |event| {
            event["event_type"] == "workx_command_execution_event"
                && event["event_params"]["item_id"] == call_id
        })
        .await?;
    assert_eq!(
    ...
    for request in analytics_server
        ...
        .unwrap_or_default()
    {
        if request.method != "POST" || request.url.path() != "/workx/analytics-events/events" {
            continue;
        }
        let payload: Value = serde_json::from_slice(&request.body)?;
        let Some(events) = payload["events"].as_array() else {

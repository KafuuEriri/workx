...
use workx_app_server_protocol::PluginReconcileChangedPlugin;
use workx_app_server_protocol::PluginReconcileResponse;
...
async fn plugin_reconcile_syncs_bundles_and_reports_changes(
    ...
    let (mut app_server, workx_home) = start_app_server(&server).await?;

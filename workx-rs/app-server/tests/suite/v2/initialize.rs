...
async fn initialize_uses_client_info_name_as_originator() -> Result<()> {
    ...
    let expected_workx_home = AbsolutePathBuf::try_from(workx_home.path().canonicalize()?)?;
    ...
    let InitializeResponse {
        user_agent,
        workx_home: response_workx_home,
        platform_family,
        platform_os,
    } = to_response::<InitializeResponse>(response)?;

    assert!(user_agent.starts_with("workx_vscode/"));
    assert_eq!(response_workx_home, expected_workx_home);
...
async fn initialize_probe_does_not_override_originator() -> Result<()> {
    ...
    let message = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.initialize_with_client_info(ClientInfo {
            name: "workx_app_server_daemon".to_string(),
            title: Some("Workx App Server Daemon".to_string()),
            version: "0.1.0".to_string(),
        }),
    )
    .await??;
    ...
    assert!(user_agent.starts_with("workx_cli_rs/"));
    Ok(())
}

#[tokio::test]
async fn initialize_workx_backend_does_not_override_originator() -> Result<()> {
    let responses = Vec::new();
    let server = create_mock_responses_server_sequence_unchecked(responses).await;
    let workx_home = TempDir::new()?;
    MockResponsesConfig::new(&server.uri())
    ...
    let message = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.initialize_with_client_info(ClientInfo {
            name: "workx-backend".to_string(),
            title: Some("Workx Backend".to_string()),
            version: "0.1.0".to_string(),
        }),
    )
    .await??;
...
async fn initialize_respects_originator_override_env_var() -> Result<()> {
    ...
    let mut mcp = TestAppServer::builder()
        ...
        .without_auto_env()
        .with_env_overrides(&[(
            "WORKX_INTERNAL_ORIGINATOR_OVERRIDE",
            Some("workx_originator_via_env_var"),
        )])
        .build()
        .await?;
    ...
    assert!(user_agent.starts_with("workx_originator_via_env_var/"));
    assert_eq!(response_workx_home, expected_workx_home);
    assert_eq!(platform_family, std::env::consts::FAMILY);
    assert_eq!(platform_os, std::env::consts::OS);
    Ok(())
...
async fn initialize_rejects_invalid_client_name() -> Result<()> {
    ...
    let mut mcp = TestAppServer::builder()
        .with_workx_home(workx_home.path())
        .without_auto_env()
        .with_env_overrides(&[("WORKX_INTERNAL_ORIGINATOR_OVERRIDE", None)])
        .build()
        .await?;
...
async fn turn_start_notify_payload_includes_initialize_client_name() -> Result<()> {
    ...
    let server = create_mock_responses_server_sequence_unchecked(responses).await;
    let workx_home = TempDir::new()?;
    let notify_file = workx_home.path().join("notify.json");
    let notify_capture = cargo_bin("workx-app-server-test-notify-capture")?;

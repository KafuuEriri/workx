...
use workx_core::find_archived_thread_path_by_id_str;
...
async fn thread_archive_requires_materialized_rollout() -> Result<()> {
    ...
    mcp.initialize_with_client_info(ClientInfo {
        name: "workx_work_desktop".to_string(),
        title: None,
    ...
    assert!(
        find_thread_path_by_id_str(workx_home.path(), &thread.id, /*state_db_ctx*/ None)
            .await?
            .is_none(),
    ...
    let event = wait_for_matching_analytics_event(&server, DEFAULT_READ_TIMEOUT, |event| {
        event["event_type"] == "workx_thread_archive_event"
    ...
    assert_eq!(
        ...
        json!({
            ...
            "event_params": {
                ...
                "app_server_client": {
                    "product_client_id": "workx_work_desktop",
                    "client_name": "workx_work_desktop",
                    "client_version": "0.1.0",
                    "rpc_transport": "stdio",
                    "experimental_api_enabled": true,
                },
                "runtime": {
                    "workx_rs_version": env!("CARGO_PKG_VERSION"),
    ...
    // Verify file moved.
    let archived_directory = workx_home.path().join(ARCHIVED_SESSIONS_SUBDIR);
...
async fn thread_archive_succeeds_when_descendant_archive_fails() -> Result<()> {
    ...
    let child_rollout_path =
        find_thread_path_by_id_str(workx_home.path(), &child_id, /*state_db_ctx*/ None)
            .await?
            .expect("child rollout path");
    let archived_child_path = workx_home
        .path()
    ...
    for request in requests {
        if request.url.path() != "/workx/analytics-events/events" {
            continue;
        }
        ...
        for event in events
            .iter()
            .filter(|event| event["event_type"] == "workx_thread_archive_event")
        {
...
async fn thread_archive_succeeds_when_spawned_descendant_is_missing() -> Result<()> {
    ...
    assert!(
        find_thread_path_by_id_str(workx_home.path(), &parent_id, /*state_db_ctx*/ None)
            .await?
            .is_none(),

...
fn write_installed_marketplace(workx_home: &std::path::Path, marketplace_name: &str) -> Result<()> {
    let root = marketplace_install_root(workx_home).join(marketplace_name);
...
async fn marketplace_remove_deletes_config_and_installed_root(user_entry: bool) -> Result<()> {
    ...
    if user_entry {
        record_user_marketplace(workx_home.path(), "debug", &configured_marketplace_update())?;
    }
    write_installed_marketplace(workx_home.path(), "debug")?;
...
async fn marketplace_remove_preserves_project_marketplace(user_entry: bool) -> Result<()> {
    ...
    // TestAppServer starts in WORKX_HOME, so make it a trusted project as well.
    std::fs::create_dir_all(workx_home.path().join(".git"))?;
    std::fs::create_dir_all(workx_home.path().join(".workx"))?;
    let project_config_path = workx_home.path().join(".workx/config.toml");

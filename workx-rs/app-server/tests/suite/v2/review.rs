...
async fn review_start_exec_approval_item_id_matches_command_execution_item() -> Result<()> {
    ...
    mcp.send_response(
        request_id,
        serde_json::json!({ "decision": workx_protocol::protocol::ReviewDecision::Approved }),
    )
    .await?;
    timeout(
        DEFAULT_READ_TIMEOUT,
...
async fn review_start_with_detached_delivery_returns_new_thread_id() -> Result<()> {
    ...
    let workx_home = TempDir::new()?;
    create_config_toml(workx_home.path(), &server.uri())?;
    let colliding_skill_dir = workx_home.path().join("skills/review-agent-collision");
    ...
    let canonical_workx_home = std::fs::canonicalize(workx_home.path())?.try_into()?;
    let review_skill_path = system_cache_root_dir(&canonical_workx_home)
        .join("review-agent")
        .join("SKILL.md");
...
fn create_config_toml(workx_home: &std::path::Path, server_uri: &str) -> std::io::Result<()> {

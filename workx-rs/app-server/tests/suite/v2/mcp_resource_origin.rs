...
-> Result<()> {
    ...
    let (workx_home, mut app_server) = start_resource_test_app_server(
        &apps_server_url,
    ...
    assert!(
        model_request
            .tool_by_name("mcp__workx_apps__best_buy", "_product_search")
            .is_none()
    );
    assert!(
        model_request
            .tool_by_name("mcp__workx_apps__walmart", "_product_search")
            .is_some()
    );

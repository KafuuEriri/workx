...
async fn code_mode_model_output_uses_structured_host_timing(
    ...
    let mut host = match transport {
        "grpc" => Some(
            Command::new(workx_utils_cargo_bin::cargo_bin("workx-code-mode-host")?)
    ...
    let mut builder = TestAppServer::builder()
        ...
        .with_json_logging("workx_code_mode::timing=info,workx_core::tools::parallel=info");
    ...
    let timing = app_server
        .wait_for_json_log_event("workx.code_mode.host_timing")
        .await?;
...
async fn app_server_shares_flag_selected_grpc_code_mode_host_across_threads() -> Result<()> {
    let host_program = workx_utils_cargo_bin::cargo_bin("workx-code-mode-host")?;
    ...
    MockResponsesConfig::new(&model_server.uri())
        ...
        .write(workx_home.path())?;
    let original_config = std::fs::read_to_string(workx_home.path().join("config.toml"))?;

...
async fn account_switch_reloads_telemetry_collectors_and_preserves_trace_context() -> Result<()> {
    ...
    write_otel_config(workx_home.path(), &initial_endpoint)?;
    write_chatgpt_auth(
        workx_home.path(),
    ...
    let mut app_server = TestAppServer::builder()
        ...
        .with_json_logging("workx_app_server::otel_reloader=info")
    ...
    write_otel_config(workx_home.path(), &next_endpoint)?;
    ...
    timeout(
        TEST_TIMEOUT,
        app_server.wait_for_json_log_event("workx.app_server.otel_reloaded"),
    )
    .await??;
    app_server
        .start_thread(ThreadStartParams::default())
    ...
    assert!(
        next_metrics.contains("workx.thread.started"),
...
fn write_otel_config(workx_home: &Path, collector_endpoint: &str) -> Result<()> {
    std::fs::write(

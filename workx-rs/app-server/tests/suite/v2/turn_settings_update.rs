...
-> Result<()> {
    ...
    .await;
    let workx_home = TempDir::new()?;
    mock_config(workx_home.path(), &server.uri())?.write(workx_home.path())?;
    let mut app = TestAppServer::builder()
        .with_workx_home(workx_home.path())
        .build_initialized_with_timeout(DEFAULT_TIMEOUT)
        .await?;
...
fn mock_config(workx_home: &Path, server_uri: &str) -> Result<MockResponsesConfig> {
    ...
    let models = [MODEL_A, MODEL_B]
        ...
        .collect();
    let catalog_path = workx_home.join("step-models.json");
    std::fs::write(
        &catalog_path,

...
use workx_app_server_protocol::GitInfo;
...
use workx_app_server_protocol::ThreadSectionListResponse;
...
use workx_protocol::protocol::GitInfo as RolloutGitInfo;
use workx_rollout::state_db::reconcile_rollout;
use workx_state::PINNED_THREAD_SECTION_ID;
use workx_state::PINNED_THREAD_SECTION_NAME;
...
async fn thread_section_move_pins_and_unpins_with_filtered_recency_pagination() -> Result<()> {
    ...
    let workx_home = TempDir::new()?;
    mock_responses_config(&server.uri()).write(workx_home.path())?;
    let state_db = init_state_db(workx_home.path()).await?;
    ...
    for (filename_timestamp, timestamp, preview) in [
        ...
        reconcile_rollout(
            Some(&state_db),
            rollout_path(workx_home.path(), filename_timestamp, &thread_id).as_path(),
            "mock_provider",
            /*builder*/ None,
            &[],
            /*archived_only*/ None,
    ...
    for thread_id in [older_pinned, newer_pinned] {
        ...
        assert_eq!(
            thread.section,
            Some(workx_state::ThreadSection {
...
async fn thread_metadata_update_repairs_missing_sqlite_row_for_stored_thread() -> Result<()> {
    ...
    let workx_home = TempDir::new()?;
    mock_responses_config(&server.uri()).write(workx_home.path())?;
    let _state_db = init_state_db(workx_home.path()).await?;
...
async fn thread_metadata_update_repairs_loaded_thread_without_resetting_summary() -> Result<()> {
    ...
    let rollout_path = rollout_path(workx_home.path(), "2025-01-06T08-30-00", &thread_id);
    reconcile_rollout(
        Some(&state_db),
...
async fn thread_metadata_update_repairs_missing_sqlite_row_for_archived_thread() -> Result<()> {
    ...
    let archived_source = rollout_path(workx_home.path(), "2025-01-06T08-30-00", &thread_id);
...
async fn thread_metadata_update_can_clear_stored_git_fields() -> Result<()> {
    ...
    let thread_id = create_fake_rollout(
        ...
        Some(RolloutGitInfo {
            ...
            branch: Some("feature/sidebar-pr".to_string()),
            repository_url: Some(
                SanitizedGitUrl::try_from("git@example.com:RonanXiao/workx.git")
...
async fn init_state_db(workx_home: &Path) -> Result<Arc<StateRuntime>> {

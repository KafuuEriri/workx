...
use workx_app_server_protocol::ResponseUsageMetadata;
...
async fn auto_compaction_local_emits_started_and_completed_items() -> Result<()> {
    ...
    compaction_config(&server.uri(), AUTO_COMPACT_LIMIT).write(workx_home.path())?;
...
async fn thread_compact_start_triggers_compaction_and_returns_empty_response() -> Result<()> {
    ...
    compaction_config(&server.uri(), /*auto_compact_limit*/ 1_000_000).write(workx_home.path())?;

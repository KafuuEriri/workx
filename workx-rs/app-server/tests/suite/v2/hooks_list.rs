...
use workx_app_server_protocol::HookSource;
...
fn command_hook_hash(
    ...
    let Ok(value) = workx_config::TomlValue::try_from(identity) else {
        unreachable!("normalized hook identity should serialize to TOML");
    };
    workx_config::version_for_toml(&value)
}

fn write_user_hook_config(workx_home: &std::path::Path) -> Result<()> {
    std::fs::write(
...
fn write_plugin_hook_config(workx_home: &std::path::Path, hooks_json: &str) -> Result<()> {
    let plugin_root = workx_home.join("plugins/cache/test/demo/local");
...
fn write_project_hook_config(dot_workx_folder: &std::path::Path, command: &str) -> Result<()> {
    std::fs::create_dir_all(dot_workx_folder)?;
    std::fs::write(
        dot_workx_folder.join("config.toml"),
        format!(
            r#"[features]
hooks = true
...
async fn hooks_list_shows_discovered_hook() -> Result<()> {
    ...
    let cwd = TempDir::new()?;
    write_user_hook_config(workx_home.path())?;
...
async fn plugin_upgrade_refreshes_hook_runtime_for_loaded_session() -> Result<()> {
    ...
    let hook_log_path = workx_home.path().join("plugin-hook-versions.log");
  ...
  "plugins": [{
    ...
    MockResponsesConfig::new(&server.uri())
        ...
        .enable_feature(Feature::WorkxHooks)
    ...
    let hooks_list_id = mcp
        .send_hooks_list_request(HooksListParams {
            cwds: vec![workx_home.path().to_path_buf()],
        })
        .await?;
    let HooksListResponse { data } =
    ...
    let trust_id = mcp
        ...
        .await?;
    let _: workx_app_server_protocol::ConfigWriteResponse =
    ...
    std::fs::write(
        plugin_source.join(".workx-plugin/plugin.json"),
    ...
    let _: PluginInstallResponse =
        timeout(DEFAULT_TIMEOUT, mcp.read_response(install_id)).await??;
    assert!(!workx_home.path().join("plugins/cache/test/demo/1").exists());
    ...
    let HooksListResponse { data } =
        timeout(DEFAULT_TIMEOUT, mcp.read_response(hooks_list_id)).await??;
    let expected_hook_path = workx_home
        .path()
    ...
    let _: PluginInstallResponse =
        timeout(DEFAULT_TIMEOUT, mcp.read_response(install_id)).await??;
    assert!(!workx_home.path().join("plugins/cache/test/demo/2").exists());
...
async fn automatic_marketplace_upgrade_refreshes_hook_runtime_for_loaded_session() -> Result<()> {
    ...
    let hook_log_path = workx_home.path().join("plugin-hook-versions.log");
    let old_plugin_root = workx_home.path().join("plugins/cache/test/demo/1");
    let new_plugin_root = workx_home.path().join("plugins/cache/test/demo/2");
  ...
  "plugins": [{
    ...
    for args in [
        &["init"][..],
        &["config", "user.email", "workx@example.com"],
        &["config", "user.name", "Workx Tests"],
        &["add", "."],
...
async fn hooks_list_shows_discovered_plugin_mcp_tool_hook() -> Result<()> {
  ...
  "hooks": {
    ...
    let identity = NormalizedHookIdentity {
        ...
        group: workx_config::MatcherGroup {
            matcher: Some("Bash".to_string()),
            hooks: vec![workx_config::HookHandlerConfig::McpTool {
    ...
    assert_eq!(
        ...
        vec![HooksListEntry {
            ...
            hooks: vec![HookMetadata {
                ...
                enabled: true,
                is_managed: false,
                current_hash: workx_config::version_for_toml(&identity),
                trust_status: HookTrustStatus::Untrusted,
            }],
            warnings: Vec::new(),
            errors: Vec::new(),
...
async fn hooks_list_warms_plugin_capabilities_for_thread_start() -> Result<()> {
  ...
  "hooks": {
    ...
    let plugin_mcp_path = workx_home
        .path()
...
async fn hooks_list_shows_plugin_hook_load_warnings() -> Result<()> {
    let workx_home = TempDir::new()?;
    let cwd = TempDir::new()?;
    write_plugin_hook_config(workx_home.path(), "{ not-json")?;
...
"#,
    ...
    std::fs::create_dir_all(workspace.path().join(".workx"))?;
    std::fs::write(
        workspace.path().join(".workx/config.toml"),
        r#"[features]
hooks = true
...
"#,
    ...
    let project_config_path =
        AbsolutePathBuf::try_from(workspace.path().join(".workx/config.toml"))?;
    assert_eq!(
        data,
        vec![
            HooksListEntry {
...
async fn hooks_list_uses_root_repo_hooks_for_linked_worktrees() -> Result<()> {
    ...
    write_project_hook_config(&repo_root.join(".workx"), "echo root hook")?;
    write_project_hook_config(&worktree_root.join(".workx"), "echo worktree hook")?;
    set_project_trust_level(workx_home.path(), &repo_root, TrustLevel::Trusted)?;
    ...
    let repo_config_path =
        AbsolutePathBuf::from_absolute_path(repo_root.join(".workx/config.toml"))?;
...
async fn config_batch_write_updates_hook_trust_for_loaded_session() -> Result<()> {
    ...
    let server = create_mock_responses_server_sequence_unchecked(responses).await;
    let workx_home = TempDir::new()?;
    let hook_script_path = workx_home.path().join("user_prompt_submit_hook.py");
    let hook_log_path = workx_home.path().join("user_prompt_submit_hook_log.jsonl");
    std::fs::write(
        &hook_script_path,
        format!(
            r#"import json

...
use workx_app_server_protocol::CommandExecOutputDeltaNotification;
use workx_app_server_protocol::CommandExecOutputStream;
use workx_app_server_protocol::CommandExecParams;
use workx_app_server_protocol::CommandExecResizeParams;
use workx_app_server_protocol::CommandExecResponse;
use workx_app_server_protocol::CommandExecTerminalSize;
...
use workx_core::exec_env::WORKX_APPLY_PATCH_PRESERVE_LINE_ENDINGS_ENV_VAR;
...
async fn command_exec_without_streams_can_be_terminated() -> Result<()> {
    ...
    create_config_toml(workx_home.path(), &server.uri(), "never")?;
...
async fn command_exec_env_overrides_merge_with_server_environment_and_support_unset() -> Result<()>
{
    ...
    let command_request_id = mcp
        .send_command_exec_request(CommandExecParams {
            command: vec![
                "/bin/sh".to_string(),
                "-lc".to_string(),
                "printf '%s|%s|%s|%s|%s|%s' \"$COMMAND_EXEC_BASELINE\" \"$COMMAND_EXEC_EXTRA\" \"${RUST_LOG-unset}\" \"$WORKX_HOME\" \"$OPENAI_FEDERATION_RULE_ID\" \"$openai_identity_token_file\"".to_string(),
            ],
            process_id: None,
            tty: false,
            stream_stdin: false,
    ...
    assert_eq!(
        ...
        CommandExecResponse {
            exit_code: 0,
            stdout: format!("request|added|unset|{}||", workx_home.path().display()),
            stderr: String::new(),
        }
    );
...
async fn assert_command_exec_apply_patch_rollout(
    ...
    let mut mcp = TestAppServer::builder()
        .with_workx_home(workx_home.path())
        .without_auto_env()
        .with_env_overrides(&[(WORKX_APPLY_PATCH_PRESERVE_LINE_ENDINGS_ENV_VAR, Some("1"))])
        .build_initialized_with_timeout(DEFAULT_READ_TIMEOUT)
        .await?;
...
async fn command_exec_enforces_managed_deny_read_requirements() -> Result<()> {
    ...
    let workx_home = TempDir::new()?;
    create_config_toml(workx_home.path(), &server.uri(), "never")?;
    let denied_root = workx_home.path().join("private");
    ...
    let user_denied_root = workx_home.path().join("user-private");
    ...
    let command_request_id = mcp
        .send_command_exec_request(CommandExecParams {
            command: vec![
                "sh".to_string(),
                "-lc".to_string(),
                "printf '%s' \"${WORKX_NETWORK_PROXY_ACTIVE-unset}\"".to_string(),
            ],
            process_id: None,
            tty: false,
            stream_stdin: false,
...
async fn command_exec_permission_profile_does_not_reuse_default_network_proxy() -> Result<()> {
    ...
    let workx_home = TempDir::new()?;
    create_config_toml(workx_home.path(), &server.uri(), "never")?;
    insert_networked_permission_profile_config(workx_home.path(), Some("networked"))?;
    let mut mcp = TestAppServer::builder()
        .with_workx_home(workx_home.path())
        .without_auto_env()
        .build_initialized_with_timeout(DEFAULT_READ_TIMEOUT)
...
async fn command_exec_permission_profile_project_roots_use_command_cwd() -> Result<()> {
    let server = create_mock_responses_server_sequence_unchecked(Vec::new()).await;
    let workx_home = TempDir::new()?;
    let command_dir = workx_home.path().join("command-cwd");
...
"#,
    ...
    assert!(
        !workx_home.path().join("parent.txt").exists(),
...
-> Result<()> {
    ...
    create_config_toml(workx_home.path(), &server.uri(), "never")?;
    let marker = format!(
        "workx-command-exec-marker-{}",
        std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)?
            .as_nanos()
    );

    let (mut process, bind_addr) = spawn_websocket_server(workx_home.path()).await?;
...
"#
    ...
    insert_command_exec_config(workx_home, &inserted_config)?;
    Ok(())
}

fn insert_command_exec_config(workx_home: &Path, inserted_config: &str) -> Result<()> {

...
use workx_app_server_protocol::ActivePermissionProfile;
use workx_app_server_protocol::ApprovalsReviewer;
...
use workx_app_server_protocol::SandboxPolicy as AppSandboxPolicy;
...
use workx_app_server_protocol::ThreadActiveFlag;
...
use workx_core::ARCHIVED_SESSIONS_SUBDIR;
use workx_features::Feature;
use workx_login::REFRESH_TOKEN_URL_OVERRIDE_ENV_VAR;
...
use workx_protocol::protocol::ThreadSettingsAppliedEvent;
...
use workx_protocol::user_input::ByteRange;
use workx_protocol::user_input::TextElement;
use workx_rollout::CompactedItem;
use workx_rollout::RolloutItem;
use workx_rollout::RolloutRecorder;
use workx_rollout::append_rollout_item_to_path;
use workx_rollout::read_session_meta_line;
...
const WORKX_5_2_INSTRUCTIONS_TEMPLATE_DEFAULT: &str = "You are Workx, a coding agent based on GPT-5. You and the user share the same workspace and collaborate to achieve the user's goals.";
...
async fn thread_resume_paginated_model_context_preserves_original_metadata() -> Result<()> {
    ...
    let saved_cwd = normalized_existing_path(workx_home.path())?;
    mock_responses_config(&server.uri()).write(workx_home.path())?;
    ...
    let path = rollout_path(workx_home.path(), "2025-01-05T12-00-00", &conversation_id);
...
async fn thread_resume_preserves_persisted_approvals_reviewer() -> Result<()> {
    ...
    let config_path = workx_home.path().join("config.toml");
...
-> Result<()> {
    ...
    let live_cwd = normalized_existing_path(workx_home.path())?;
    mock_responses_config(&server.uri()).write(workx_home.path())?;
    let config_path = workx_home.path().join("config.toml");
...
async fn cold_resume_reresolves_persisted_active_permission_profile() -> Result<()> {
    ...
    for history_mode in [ThreadHistoryMode::Legacy, ThreadHistoryMode::Paginated] {
        ...
        write_dev_permission_config(&server.uri(), workx_home.path(), ":workspace")?;
        let thread_id = {
            let mut mcp = TestAppServer::builder()
                .with_workx_home(workx_home.path())
                .without_managed_config()
        ...
        write_dev_permission_config(&server.uri(), workx_home.path(), ":read-only")?;
        let mut mcp = TestAppServer::builder()
            .with_workx_home(workx_home.path())
            .without_managed_config()
            .build_initialized()
...
async fn unloaded_thread_goal_mutations_respect_parent_ownership() -> Result<()> {
    ...
    for (source, version) in [
        ...
        let path = rollout_path(workx_home.path(), TIMESTAMP, &thread_id);
        let params = json!({
            "threadId": thread_id,
...
async fn goal_first_live_thread_appears_in_state_db_thread_list() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let workx_home = TempDir::new()?;
    let workx_home_path = normalized_existing_path(workx_home.path())?;
    mock_responses_config(&server.uri()).write(&workx_home_path)?;
    let config_path = workx_home_path.join("config.toml");
    let config = std::fs::read_to_string(&config_path)?;
    std::fs::write(
        &config_path,
        config.replace("personality = true\n", "personality = true\ngoals = true\n"),
    ...
    let sqlite_home = workx_home_path
        .as_path()
        .to_str()
        .expect("test workx home should be utf-8");
    let mut mcp = TestAppServer::builder()
        .with_workx_home(&workx_home_path)
        .without_managed_config()
        .with_env_overrides(&[("WORKX_SQLITE_HOME", Some(sqlite_home))])
        .build_initialized()
        .await?;
...
async fn thread_resume_tracks_thread_initialized_analytics() -> Result<()> {
    ...
    mock_responses_config(&server.uri())
        ...
        .write(workx_home.path())?;
    mount_analytics_capture(&server, workx_home.path()).await?;
...
async fn thread_resume_running_thread_tracks_thread_originator_in_analytics() -> Result<()> {
    ...
    let start_id = mcp
        .send_thread_start_request_with_auto_env(ThreadStartParams {
            ...
            service_name: Some("workx_work_desktop".to_string()),
            ..Default::default()
        })
        .await?;
    let ThreadStartResponse { thread, .. } =
    ...
    let event = wait_for_matching_analytics_event(&server, DEFAULT_READ_TIMEOUT, |event| {
        event["event_type"] == "workx_thread_initialized"
...
fn set_session_meta_on_fake_rollout(
    ...
    let path = rollout_path(workx_home, filename_ts, thread_id);
...
async fn thread_resume_returns_rollout_history() -> Result<()> {
    ...
    append_rollout_item_to_path(
        &rollout_path(workx_home.path(), "2025-01-05T12-00-00", &conversation_id),
        &RolloutItem::EventMsg(EventMsg::ThreadSettingsApplied(settings)),
    )
    .await?;
...
async fn thread_resume_redacts_payloads_for_chatgpt_remote_clients() -> Result<()> {
    for client_name in ["workx_chatgpt_android_remote", "workx_chatgpt_ios_remote"] {
...
fn append_resume_redaction_history(
    ...
    let rollout_file_path = rollout_path(workx_home, filename_ts, conversation_id);
...
async fn thread_resume_rejects_archived_session_by_id() -> Result<()> {
    ...
    let active_rollout_path = rollout_path(workx_home.path(), filename_ts, &conversation_id);
    let archived_dir = workx_home.path().join(ARCHIVED_SESSIONS_SUBDIR);
    ...
    assert!(
        message.contains(&format!("session {conversation_id} is archived"))
            && message.contains(&format!(
                "workx unarchive {conversation_id}` to unarchive it first"
            )),
...
async fn thread_goal_set_edits_objective_without_resetting_usage() -> Result<()> {
    ...
    state_db
        ...
        .account_thread_goal_usage(
            ...
            /*token_delta*/ 50,
            workx_state::GoalAccountingMode::ActiveOnly,
...
async fn thread_goal_keeps_original_root_until_external_objective_edit() -> Result<()> {
    ...
    let get_id = mcp
        ...
        .await?;
    let _: workx_app_server_protocol::ThreadGoalGetResponse =
...
async fn thread_goal_lifecycle_emits_analytics_and_clear_deletes_goal() -> Result<()> {
    ...
    let get_id = mcp
        ...
        .await?;
    let get: workx_app_server_protocol::ThreadGoalGetResponse =
        timeout(DEFAULT_READ_TIMEOUT, mcp.read_response(get_id)).await??;
...
async fn cold_paginated_resume_omits_usage_when_its_turn_is_ambiguous() -> Result<()> {
    ...
    let path = rollout_path(workx_home.path(), filename_ts, &conversation_id);
    append_rollout_item_to_path(
        &path,
        &RolloutItem::EventMsg(EventMsg::TokenCount(TokenCountEvent {
            info: Some(TokenUsageInfo {
...
async fn thread_resume_token_usage_replay_ignores_stale_interrupted_tail_turn() -> Result<()> {
    ...
    let rollout_file_path = rollout_path(workx_home.path(), filename_ts, &conversation_id);
    let persisted_rollout = std::fs::read_to_string(&rollout_file_path)?;
...
async fn thread_resume_prefers_persisted_git_metadata_for_local_threads() -> Result<()> {
    ...
    let repo_path = workx_home.path().join("repo");
    ...
    let rollout_path = rollout_path(workx_home.path(), "2025-01-05T12-00-00", &thread_id);
...
async fn thread_resume_defers_updated_at_until_turn_start() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let workx_home = TempDir::new()?;
    let rollout = setup_rollout_fixture(workx_home.path(), &server.uri()).await?;
    ...
    let resume_id = mcp
        .send_thread_resume_request(ThreadResumeParams {
            ...
            cwd: Some(workx_home.path().to_string_lossy().to_string()),
            ..Default::default()
        })
        .await?;
    let ThreadResumeResponse { cwd, .. } =
        timeout(DEFAULT_READ_TIMEOUT, mcp.read_response(resume_id)).await??;
    assert_eq!(cwd, AbsolutePathBuf::from_absolute_path(workx_home.path())?);
...
async fn thread_resume_rejects_mismatched_path_for_running_thread_id() -> Result<()> {
    ...
    let stale_path = rollout_path(workx_home.path(), "2025-01-01T00-00-00", &stale_thread_id);
...
async fn thread_resume_replays_pending_file_change_request_approval() -> Result<()> {
    ...
    let tmp = TempDir::new()?;
    let workx_home = tmp.path().join("workx_home");
    std::fs::create_dir(&workx_home)?;
...
"#;
    ...
    let expected_file_change = ThreadItem::FileChange {
        ...
        changes: vec![workx_app_server_protocol::FileUpdateChange {
...
async fn thread_resume_with_overrides_defers_updated_at_until_turn_start() -> Result<()> {
    ...
    let RestartedThreadFixture {
        ...
        rollout_file_path,
        updated_at,
    } = start_materialized_thread_and_restart(workx_home.path(), "materialize").await?;
...
command = "workx-definitely-not-a-real-binary"
required = true"#,
        )
        .write(workx_home.path())?;
...
async fn thread_resume_supports_history_and_overrides() -> Result<()> {
    ...
    let RestartedThreadFixture {
        mut mcp, thread_id, ..
    } = start_materialized_thread_and_restart(workx_home.path(), "seed history").await?;
...
async fn thread_resume_accepts_personality_override() -> Result<()> {
    ...
    assert!(
        instructions_text.contains(WORKX_5_2_INSTRUCTIONS_TEMPLATE_DEFAULT),
...
async fn setup_rollout_fixture(workx_home: &Path, server_uri: &str) -> Result<RolloutFixture> {
    mock_responses_config(server_uri).write(workx_home)?;

...
async fn thread_list_reports_system_error_idle_flag_after_failed_turn() -> Result<()> {
    ...
    let workx_home = TempDir::new()?;
    create_runtime_config(workx_home.path(), &server.uri())?;
    let mut mcp = init_mcp(workx_home.path()).await?;
...
fn create_minimal_config(workx_home: &std::path::Path) -> std::io::Result<()> {
    let config_toml = workx_home.join("config.toml");
    std::fs::write(
        config_toml,
        r#"
model = "mock-model"
...
fn create_runtime_config(workx_home: &std::path::Path, server_uri: &str) -> std::io::Result<()> {
    MockResponsesConfig::new(server_uri).write(workx_home)
}
...
async fn thread_list_respects_cwd_filters() -> Result<()> {
    ...
    let first_target_cwd = workx_home.path().join("first-target-cwd");
    let second_target_cwd = workx_home.path().join("second-target-cwd");
    fs::create_dir_all(&first_target_cwd)?;
    fs::create_dir_all(&second_target_cwd)?;
    set_rollout_cwd(
        rollout_path(workx_home.path(), "2025-01-02T10-00-00", &first_filtered_id).as_path(),
        &first_target_cwd,
    )?;
    set_rollout_cwd(
        rollout_path(
    ...
    let mut mcp = init_mcp(workx_home.path()).await?;
    let request_id = mcp
        .send_thread_list_request(workx_app_server_protocol::ThreadListParams {
            cursor: None,
            limit: Some(10),
            sort_key: None,
            sort_direction: None,
...
"#,
    ...
    state_db
        ...
        .await?;
    let rollout_config = workx_rollout::RolloutConfig {
        workx_home: workx_home.path().to_path_buf(),
        sqlite: workx_state::SqliteConfig::new_for_testing(workx_home.path().abs()),
        cwd: workx_home.path().to_path_buf(),
        model_provider_id: "mock_provider".to_string(),
        generate_memories: false,
    };
    let repaired_page = workx_core::RolloutRecorder::list_threads(
        Some(state_db.clone()),
        &rollout_config,
        /*page_size*/ 10,
        /*cursor*/ None,
        workx_core::ThreadSortKey::CreatedAt,
        workx_core::SortDirection::Desc,
        &[],
...
async fn thread_search_returns_content_matches() -> Result<()> {
    ...
    let mut mcp = init_mcp(workx_home.path()).await?;
    let request_id = mcp
        .send_thread_search_request(workx_app_server_protocol::ThreadSearchParams {
            cursor: None,
            limit: Some(10),
            sort_key: None,
            sort_direction: None,
    ...
    for thread_id in [&older_match, &newer_match] {
        let request_id = mcp
            .send_thread_section_move_request(ThreadSectionMoveParams {
                ...
                section_id: Some(workx_state::PINNED_THREAD_SECTION_ID.to_string()),
...
"#,
    ...
    let stale_cwd = workx_home.path().join("stale-cwd");
...
async fn thread_list_relation_filters_read_spawn_graph_from_state_db() -> Result<()> {
    ...
    for (thread_id, created_at, source, model_provider) in [
        ...
        let mut builder = workx_state::ThreadMetadataBuilder::new(
            thread_id,
            workx_home.path().join(format!("{thread_id}.jsonl")),
            created_at,
            source,
        );
        builder.model_provider = Some(model_provider.to_string());
        builder.cwd = workx_home.path().to_path_buf();
...
async fn thread_list_reports_loaded_subagent_direct_input_capability() -> Result<()> {
    ...
    let parent_rollout_path = rollout_path(workx_home.path(), "2025-02-01T09-00-00", &cli_id);
    ...
    for (filename_ts, timestamp, version, capability, should_resume) in [
        ...
        let path = rollout_path(workx_home.path(), filename_ts, &thread_id);
...
async fn thread_list_sanitizes_git_info_from_existing_rollouts() -> Result<()> {
    ...
    let path = rollout_path(workx_home.path(), "2025-02-01T09-00-00", &conversation_id);
...
async fn thread_list_sort_updated_at_orders_by_mtime() -> Result<()> {
    ...
    set_rollout_mtime(
        rollout_path(workx_home.path(), "2025-01-01T10-00-00", &id_old).as_path(),
        "2025-01-03T00:00:00Z",
    )?;
    set_rollout_mtime(
        rollout_path(workx_home.path(), "2025-01-01T11-00-00", &id_mid).as_path(),
        "2025-01-02T00:00:00Z",
    )?;
    set_rollout_mtime(
        rollout_path(workx_home.path(), "2025-01-01T12-00-00", &id_new).as_path(),
...
async fn thread_list_sort_recency_at_uses_state_db_order_with_provider_filter() -> Result<()> {
    ...
    workx_core::RolloutRecorder::list_threads(
        Some(state_db.clone()),
        &rollout_config,
        /*page_size*/ 10,
        /*cursor*/ None,
        workx_core::ThreadSortKey::CreatedAt,
        workx_core::SortDirection::Desc,
        workx_core::INTERACTIVE_SESSION_SOURCES.as_slice(),
        /*model_providers*/ None,
        /*cwd_filters*/ None,
        "mock_provider",
        /*search_term*/ None,
...
async fn thread_list_updated_at_paginates_with_cursor() -> Result<()> {
    ...
    set_rollout_mtime(
        rollout_path(workx_home.path(), "2025-02-01T10-00-00", &id_a).as_path(),
        "2025-02-03T00:00:00Z",
    )?;
    set_rollout_mtime(
        rollout_path(workx_home.path(), "2025-02-01T11-00-00", &id_b).as_path(),
        "2025-02-02T00:00:00Z",
    )?;
    set_rollout_mtime(
        rollout_path(workx_home.path(), "2025-02-01T12-00-00", &id_c).as_path(),
...
async fn thread_list_backwards_cursor_can_seed_forward_delta_sync() -> Result<()> {
    ...
    set_rollout_mtime(
        rollout_path(workx_home.path(), "2025-02-01T10-00-00", &id_old).as_path(),
        "2025-02-02T00:00:00Z",
    )?;
    set_rollout_mtime(
        rollout_path(workx_home.path(), "2025-02-01T11-00-00", &id_watermark).as_path(),
        "2025-02-03T00:00:00Z",
    )?;
    ...
    set_rollout_mtime(
        rollout_path(workx_home.path(), "2025-02-01T12-00-00", &id_new).as_path(),
...
async fn thread_list_updated_at_uses_mtime() -> Result<()> {
    ...
    set_rollout_mtime(
        rollout_path(workx_home.path(), "2025-02-01T10-00-00", &thread_id).as_path(),
...
async fn thread_list_archived_filter() -> Result<()> {
    ...
    let archived_source = rollout_path(workx_home.path(), "2025-03-01T09-00-00", &archived_id);

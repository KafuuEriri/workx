...
use workx_app_server_protocol::CurrentTimeReadResponse;
...
use workx_app_server_protocol::SleepItem;
...
async fn clock_tools_emit_control_tool_analytics() -> Result<()> {
    ...
    let turn_event = wait_for_matching_analytics_event(&server, DEFAULT_READ_TIMEOUT, |event| {
        event["event_type"] == "workx_turn_event" && event["event_params"]["turn_id"] == turn.id
    })
    .await?;
    ...
    let mut control_events = events
        .iter()
        .filter(|event| event["event_type"] == "workx_control_tool_call_event")
        .collect::<Vec<_>>();

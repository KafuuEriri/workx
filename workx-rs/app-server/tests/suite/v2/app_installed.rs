...
async fn installed_apps_force_refresh_only_refreshes_tools_snapshot() -> Result<()> {
    ...
    let workx_home = configured_workx_home(fixture.base_url())?;
    let mut app_server = start_app_server(workx_home.path()).await?;
...
fn configured_workx_home(base_url: &str) -> Result<TempDir> {
...
impl InstalledAppsFixture {
    async fn start() -> Result<Self> {
        ...
        synthetic_link
            ...
            .0
            .insert("_workx_apps".to_string(), json!({ "synthetic_link": true }));

...
use workx_protocol::protocol::PLUGINS_INSTRUCTIONS_OPEN_TAG;
...
async fn managed_plugins_requirement_disables_selected_executor_plugin_capabilities(
    ...
    let config_path = fixture.workx_home.path().join("config.toml");
    ...
    std::fs::write(
        fixture.workx_home.path().join("requirements.toml"),
        "[features]\nplugins = false\n",
    )?;
    ...
    let mut app_server = TestAppServer::builder()
        .with_workx_home(fixture.workx_home.path())
        .without_auto_env()
        .build()
        .await?;
    ...
    .await?;
    let mut exec_server =
        spawn_exec_server(fixture.workx_home.path(), &fixture.exec_server_url).await?;
...
async fn selected_capability_stack_tracks_environment_availability_and_resume() -> Result<()> {
    ...
    exec_server = spawn_exec_server(fixture.workx_home.path(), &fixture.exec_server_url).await?;
    add_environment(&mut app_server, &fixture.exec_server_url).await?;
    wait_for_selected_mcp_server(&mut app_server, &thread_id).await?;
...
fn selected_capability_fixture(
    ...
    let local_skill_dir = workx_home.path().join("skills/local-deploy");
    ...
    let manifest_dir = plugin.path().join(".workx-plugin");
...
fn assert_selected_plugin_tools_absent(request: &ResponsesRequest) {
    ...
    let connector = request
        .tool_by_name("mcp__workx_apps__calendar", "connector_calendar")
...
async fn spawn_exec_server(workx_home: &std::path::Path, url: &str) -> Result<Child> {
    let mut child = Command::new(workx_utils_cargo_bin::cargo_bin("workx")?)

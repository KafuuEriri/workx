...
impl LocalWebsocketExecServer {
    pub(crate) async fn start(workx_home: &Path, exec_server_program: &Path) -> Result<Self> {
        let mut command = Command::new(exec_server_program);
        command.stdin(Stdio::null());
        command.stdout(Stdio::piped());
        command.stderr(Stdio::inherit());
        command.current_dir(workx_home);
        command.env("WORKX_HOME", workx_home);
        #[cfg(target_os = "linux")]
        command.env(
            WORKX_LINUX_SANDBOX_EXE_ENV_VAR,

...
use core_test_support::test_workx::TestEnv;
use core_test_support::test_workx::test_env;
...
use workx_app_server_protocol::ClientNotification;
...
use workx_app_server_protocol::ConfigReadParams;
use workx_app_server_protocol::ConfigValueWriteParams;
...
use workx_app_server_protocol::FsCopyParams;
use workx_app_server_protocol::FsCreateDirectoryParams;
use workx_app_server_protocol::FsGetMetadataParams;
use workx_app_server_protocol::FsReadDirectoryParams;
use workx_app_server_protocol::FsReadFileParams;
use workx_app_server_protocol::FsRemoveParams;
use workx_app_server_protocol::FsUnwatchParams;
use workx_app_server_protocol::FsWatchParams;
use workx_app_server_protocol::FsWriteFileParams;
...
use workx_app_server_protocol::JSONRPCError;
use workx_app_server_protocol::JSONRPCErrorError;
use workx_app_server_protocol::JSONRPCMessage;
use workx_app_server_protocol::JSONRPCNotification;
use workx_app_server_protocol::JSONRPCRequest;
...
use workx_app_server_protocol::ModelProviderCapabilitiesReadParams;
...
use workx_app_server_protocol::PluginSearchParams;
use workx_app_server_protocol::PluginSkillReadParams;
use workx_app_server_protocol::PluginUninstallParams;
use workx_app_server_protocol::ProcessKillParams;
use workx_app_server_protocol::ProcessSpawnParams;
use workx_app_server_protocol::ProjectImportParams;
use workx_app_server_protocol::ProjectListParams;
use workx_app_server_protocol::ProjectReadParams;
use workx_app_server_protocol::RemoteControlClientsListParams;
use workx_app_server_protocol::RemoteControlClientsRevokeParams;
use workx_app_server_protocol::RemoteControlPairingStartParams;
use workx_app_server_protocol::RemoteControlPairingStatusParams;
...
use workx_app_server_protocol::ThreadRealtimeListVoicesParams;
...
use workx_app_server_protocol::ThreadSectionMoveParams;
...
use workx_exec_server::WORKX_EXEC_SERVER_NOISE_AUTH_TOKEN_ENV_VAR;
use workx_exec_server::WORKX_EXEC_SERVER_NOISE_CHATGPT_ACCOUNT_ID_ENV_VAR;
use workx_exec_server::WORKX_EXEC_SERVER_NOISE_ENVIRONMENT_ID_ENV_VAR;
use workx_exec_server::WORKX_EXEC_SERVER_NOISE_REGISTRY_URL_ENV_VAR;
use workx_exec_server::WORKX_EXEC_SERVER_URL_ENV_VAR;
use workx_login::default_client::WORKX_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR;
...
pub struct TestAppServer {
    ...
    // removing an owned WORKX_HOME that may still be its cwd on Windows.
    ...
    _owned_workx_home: Option<TempDir>,
}

pub const DEFAULT_CLIENT_NAME: &str = "workx-app-server-tests";
pub const DISABLE_PLUGIN_STARTUP_TASKS_ARG: &str = "--disable-plugin-startup-tasks-for-tests";
const DISABLE_MANAGED_CONFIG_ENV_VAR: &str = "WORKX_APP_SERVER_DISABLE_MANAGED_CONFIG";
#[cfg(windows)]
...
impl TestAppServer {
    /// Starts building a server with a temporary WORKX_HOME and the standard
    ...
    async fn new_with_program_env_and_args(
        ...
        cmd.current_dir(workx_home);
        cmd.env("WORKX_HOME", workx_home);
        cmd.env("RUST_LOG", "warn");
        // Keep integration tests isolated from host managed configuration.
        cmd.env(
            "WORKX_APP_SERVER_MANAGED_CONFIG_PATH",
            workx_home.join("managed_config.toml"),
        );
        cmd.env_remove(WORKX_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR);
        cmd.args(args);
        ...
        let mut process = loop {
            ...
            if !process
                ...
                || retries == 2
            {
                break process.context("workx-mcp-server proc should start")?;
            }
            retries += 1;
    ...
    pub async fn send_experimental_feature_enablement_set_request(
        &mut self,
        params: workx_app_server_protocol::ExperimentalFeatureEnablementSetParams,
    ) -> anyhow::Result<i64> {
        let params = Some(serde_json::to_value(params)?);
...
impl TestAppServerBuilder {
    /// Uses this existing WORKX_HOME instead of a temporary one.
    pub fn with_workx_home(mut self, workx_home: &Path) -> Self {
        self.workx_home = Some(workx_home.to_path_buf());
        self
    }
    ...
    /// Builds a server with a temporary WORKX_HOME and automatic environment
    /// by default.
    pub async fn build(self) -> anyhow::Result<TestAppServer> {
        ...
        let (workx_home, owned_workx_home) = match workx_home {
            Some(workx_home) => (workx_home, None),
            None => {
                let owned_workx_home = TempDir::new()?;
                (
                    owned_workx_home.path().to_path_buf(),
                    Some(owned_workx_home),
                )
            }
        };
        let attribution_settings_server = if workx_home.join("auth.json").is_file() {
            let config_path = workx_home.join("config.toml");
        ...
        let (auto_env, delayed_exec_server) = match environment {
            TestAppServerEnvironment::Auto => {
                let environments_toml = workx_home.join("environments.toml");
                ensure!(
                ...
                let (auto_env, delayed_exec_server) = match exec_server_delay {
                    Some(added_delay) => {
                        ...
                        let exec_server_program =
                            workx_utils_cargo_bin::cargo_bin("exec-server")
                        ...
                        let local_websocket_exec_server =
                            LocalWebsocketExecServer::start(&workx_home, &exec_server_program)
                                .await?;
                ...
                let mut auto_env_overrides = vec![
                    (
                        WORKX_EXEC_SERVER_URL_ENV_VAR.to_string(),
                        auto_env.exec_server_url().map(str::to_string),
                    ),
                    (
                        WORKX_EXEC_SERVER_NOISE_REGISTRY_URL_ENV_VAR.to_string(),
                        None,
                    ),
                    (
                        WORKX_EXEC_SERVER_NOISE_ENVIRONMENT_ID_ENV_VAR.to_string(),
                        None,
                    ),
                    (WORKX_EXEC_SERVER_NOISE_AUTH_TOKEN_ENV_VAR.to_string(), None),
                    (
                        WORKX_EXEC_SERVER_NOISE_CHATGPT_ACCOUNT_ID_ENV_VAR.to_string(),
                        None,
                    ),
                ];
        ...
        let mut program = match program {
            ...
            None => workx_utils_cargo_bin::cargo_bin("workx-app-server")
                .context("should find binary for workx-app-server")?,
        };
        let mut owned_install_dir = None;
        if !custom_program
            && workx_utils_cargo_bin::runfiles_available()
            && let Ok(code_mode_host_program) =
                workx_utils_cargo_bin::cargo_bin("workx-code-mode-host")
        {
        ...
        app_server._owned_workx_home = owned_workx_home;
...
impl Drop for TestAppServer {
    fn drop(&mut self) {
        // These tests spawn a `workx-app-server` child process.
        //

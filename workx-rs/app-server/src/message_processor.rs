...
use workx_app_server_protocol::ExperimentalApi;
...
use workx_app_server_protocol::experimental_required_message;
...
use workx_thread_store::LocalQueueStore;
use workx_thread_store::QueueStore;
...
pub(crate) struct MessageProcessorArgs {
    ...
    pub(crate) feedback: WorkxFeedback,
...
impl MessageProcessor {
    ...
    pub(crate) fn new(args: MessageProcessorArgs) -> Self {
        ...
        let thread_store = workx_core::thread_store_from_config(config.as_ref(), state_db.clone());
        ...
        let executor_skill_provider: Arc<dyn workx_skills_extension::SkillProvider> = Arc::new(
            workx_skills_extension::ExecutorSkillProvider::new_with_restriction_product(
        ...
        let thread_manager = Arc::new_cyclic(|thread_manager| {
            ...
            let manager = ThreadManager::new(
                config.as_ref(),
                auth_manager.clone(),
                workx_core::build_models_manager(config.as_ref(), auth_manager.clone()),
    ...
    pub(crate) async fn process_request(
        ...
        Self::run_request_with_context(
            ...
            async {
                let workx_request = deserialize_client_request(request);
                let result = match workx_request {
                    Ok(workx_request) => {
    ...
    async fn handle_client_request(
        ...
        if let ClientRequest::Initialize { request_id, params } = workx_request {
    ...
    async fn dispatch_initialized_client_request(
        ...
        if let Some(reason) = workx_request.experimental_reason()
        ...
        let event_stream_ready = match &workx_request {
        ...
        let serialization_scope = workx_request.serialization_scope();
    ...
    async fn handle_initialized_client_request(
        ...
        let request_id = ConnectionRequestId {
            connection_id,
            request_id: workx_request.id().clone(),
        };
        let result: Result<Option<ClientResponsePayload>, JSONRPCErrorError> = match workx_request {
            ...
            ClientRequest::McpServerEventStreamStart { params, .. } => {
                ...
                session
                    ...
                    .await?;
                Ok(Some(
                    workx_app_server_protocol::McpServerEventStreamStartResponse {}.into(),
                ))
            }
            ClientRequest::McpServerEventStreamStop { params, .. } => {
                session
                    ...
                    .await;
                Ok(Some(
                    workx_app_server_protocol::McpServerEventStreamStopResponse {}.into(),
                ))
            }

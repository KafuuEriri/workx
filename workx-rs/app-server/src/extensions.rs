...
use workx_extension_api::AgentSpawnFuture;
use workx_extension_api::AgentSpawner;
...
use workx_extension_api::InternalSessionSpawnFuture;
use workx_extension_api::InternalSessionSpawner;
...
use workx_queue_extension::QueuedItemService;
...
pub(crate) struct ThreadExtensionDependencies {
    ...
    pub(crate) executor_skill_provider: Arc<dyn workx_skills_extension::SkillProvider>,
...
where
    S: AgentSpawner<StartThreadOptions, Spawned = NewThread, Error = WorkxErr> + 'static,
{
    ...
    if let Some(queue_service) = queue_service {
        workx_queue_extension::install(&mut builder, queue_service);
    }
    workx_history_notes_extension::install(&mut builder, auth_manager.clone());
    if let Some(state_db) = state_db {
        workx_goal_extension::install_with_backend(
            &mut builder,
            state_db,
            analytics_events_client,
            workx_otel::global(),
            ...
            |config: &Config| GoalExtensionConfig {
                enabled: config.features.enabled(workx_features::Feature::Goals),
    ...
    workx_memories_extension::install(&mut builder, workx_otel::global());
    workx_mcp_extension::install(&mut builder);
    workx_mcp_extension::install_executor_plugins(&mut builder, environment_manager);
    workx_web_search_extension::install(&mut builder, auth_manager.clone());
    workx_image_generation_extension::install(&mut builder, auth_manager, |config: &Config| {
    ...
    let skill_providers = workx_skills_extension::SkillProviders::new()
        ...
        .with_orchestrator_provider(Arc::new(
            workx_skills_extension::OrchestratorSkillProvider::new(),
        ))
...
) -> impl AgentSpawner<StartThreadOptions, Spawned = NewThread, Error = WorkxErr> {
    move |forked_from_thread_id: ThreadId,
          options: StartThreadOptions|
          -> AgentSpawnFuture<'static, NewThread, WorkxErr> {
        let thread_manager = thread_manager.clone();
        Box::pin(async move {
            let thread_manager = thread_manager.upgrade().ok_or_else(|| {
                WorkxErr::UnsupportedOperation("thread manager dropped".to_string())
            })?;
            thread_manager
...
fn internal_session_spawner(
    thread_manager: Weak<ThreadManager>,
) -> impl InternalSessionSpawner<StartThreadOptions, Spawned = NewThread, Error = WorkxErr> {
    move |parent_thread_id: ThreadId,
          options: StartThreadOptions|
          -> InternalSessionSpawnFuture<'static, NewThread, WorkxErr> {
        let thread_manager = thread_manager.clone();
        Box::pin(async move {
            let thread_manager = thread_manager.upgrade().ok_or_else(|| {
                WorkxErr::UnsupportedOperation("thread manager dropped".to_string())
...
mod tests {
    ...
    use workx_protocol::protocol::ThreadGoal as CoreThreadGoal;
    use workx_protocol::protocol::ThreadGoalStatus;
    use workx_protocol::protocol::ThreadGoalUpdatedEvent;

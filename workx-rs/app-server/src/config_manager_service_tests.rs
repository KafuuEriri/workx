...
use workx_app_server_protocol::ConfigLayerSource as ApiConfigLayerSource;
...
async fn write_value_preserves_comments_and_order() -> Result<()> {
    ...
    let original = r#"# Workx user configuration
model = "gpt-5.2"
...
"#;
    ...
    let expected = r#"# Workx user configuration
model = "gpt-5.2"
approval_policy = "on-request"
...
async fn psp_feature_configures_first_party_routing() -> Result<()> {
    ...
    assert!(config.features.enabled(workx_features::Feature::Psp));
    assert_eq!(
...
async fn batch_write_rejects_legacy_profile_selector() -> Result<()> {
    ...
    let error = service
        .batch_write(ConfigBatchWriteParams {
            edits: vec![
                workx_app_server_protocol::ConfigEdit {
...
async fn load_default_config_preserves_managed_requirements_and_selected_user_config_path() {
    ...
    assert_eq!(
        config
            ...
            .requirements()
            .managed_auth_policy(),
        workx_config::ManagedAuthPolicy {
            allowed_login_methods: Some(vec![workx_protocol::config_types::ForcedLoginMethod::Api]),
...
async fn managed_auth_policy_survives_unusable_requirements_file_changes() -> Result<()> {
    ...
    let auth_manager = workx_login::AuthManager::shared_from_config(
        &startup, /*enable_workx_api_key_env*/ false,
    )
    .await?;
    std::fs::write(
        &requirements_path,
    ...
    assert!(
        auth_manager.is_login_method_allowed(workx_protocol::config_types::ForcedLoginMethod::Api)
    );
    assert!(
        !auth_manager
            .is_login_method_allowed(workx_protocol::config_types::ForcedLoginMethod::Chatgpt)
    );
    assert_eq!(

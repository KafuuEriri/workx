...
use workx_cloud_config::cloud_config_bundle_loader;
...
/// Shared app-server entry point for loading effective Workx configuration.
#[derive(Clone)]
...
impl ConfigManager {
    ...
    pub(crate) fn workx_home(&self) -> &Path {
        self.workx_home.as_path()
    }

    pub(crate) fn user_config_path(&self) -> std::io::Result<AbsolutePathBuf> {
        self.loader_overrides.user_config_path(self.workx_home())
    }
    ...
    pub(crate) async fn load_default_config(&self) -> std::io::Result<Config> {
        ...
        let mut config = ConfigBuilder::default()
            ...
            .fallback_cwd(Some(self.workx_home.clone()))
    ...
    pub(crate) async fn load_with_cli_overrides(
        ...
        let merged_cli_overrides = cli_overrides
            ...
            .collect::<Vec<_>>();
        let mut config = workx_core::config::ConfigBuilder::default()
            .workx_home(self.workx_home.clone())
    ...
    fn apply_arg0_paths(&self, config: &mut Config) {
        config.workx_self_exe = self.arg0_paths.workx_self_exe.clone();
        config.workx_linux_sandbox_exe = self.arg0_paths.workx_linux_sandbox_exe.clone();
    ...
    #[cfg(test)]
    pub(crate) fn without_managed_config_for_tests(workx_home: PathBuf) -> Self {
        Self::new_for_tests(
            workx_home,
            Vec::new(),

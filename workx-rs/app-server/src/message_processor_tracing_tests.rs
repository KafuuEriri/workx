...
fn init_test_tracing() -> &'static TestTracing {
    ...
    TEST_TRACING.get_or_init(|| {
        ...
        let provider = SdkTracerProvider::builder()
            ...
            .build();
        let tracer = provider.tracer("workx-app-server-message-processor-tests");
...
impl TracingHarness {
    async fn new() -> Result<Self> {
        ...
        let config = Arc::new(build_test_config(workx_home.path(), &server.uri()).await?);
...
async fn build_test_config(workx_home: &Path, server_uri: &str) -> Result<Config> {
...
async fn build_test_processor(
    ...
    let auth_manager =
        AuthManager::shared_from_config(config.as_ref(), /*enable_workx_api_key_env*/ false)
            .await
    ...
    let config_manager = ConfigManager::new(
        ...
        Arc::new(workx_config::NoopThreadConfigLoader),
    );
        ...
        match envelope {
            crate::outgoing_message::OutgoingEnvelope::ToConnection {
                ...
                if matches!(
                    ...
                    workx_app_server_protocol::ServerNotification::ThreadStarted(_)
                ) {
                    return;
                }
            }
...
async fn turn_start_jsonrpc_span_parents_core_turn_spans() -> Result<()> {
    ...
    let spans = wait_for_exported_spans(harness.tracing, |spans| {
        spans.iter().any(|span| {
            ...
            span_attr(span, "workx.op") == Some("turn_input")
                && span.span_context.trace_id() == remote_trace_id
        })
    })
    .await;
    ...
    let core_turn_span =
        find_span_with_trace(&spans, remote_trace_id, "workx.op=turn_input", |span| {
            span_attr(span, "workx.op") == Some("turn_input")
        });

use workx_app_server_protocol::ConfigLayer as ApiConfigLayer;
use workx_app_server_protocol::ConfigLayerMetadata as ApiConfigLayerMetadata;
use workx_app_server_protocol::ConfigLayerSource as ApiConfigLayerSource;
use workx_config::ConfigLayer;
use workx_config::ConfigLayerMetadata;
use workx_config::ConfigLayerSource;

/// Converts a config-layer source owned by `workx-config` into the app-server wire type owned by
/// `workx-app-server-protocol`.
///
...
pub(crate) fn config_layer_source_to_api(source: ConfigLayerSource) -> ApiConfigLayerSource {
    match source {
        ...
        ConfigLayerSource::Project { dot_workx_folder } => {
            ApiConfigLayerSource::Project { dot_workx_folder }
        }
...
/// Converts config-layer metadata owned by `workx-config` into the app-server wire type owned by
/// `workx-app-server-protocol`.
///
/// The types stay separate so app-server protocol ownership does not leak into the config domain
/// crate. Because this crate owns neither type, Rust's orphan rules require an explicit conversion
...
/// Converts a config layer owned by `workx-config` into the app-server wire type owned by
/// `workx-app-server-protocol`.
///
/// The types stay separate so app-server protocol ownership does not leak into the config domain
/// crate. Because this crate owns neither type, Rust's orphan rules require an explicit conversion

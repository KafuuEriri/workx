...
use workx_analytics::PluginInstallSource;
use workx_app_server_protocol::PluginAvailability;
use workx_app_server_protocol::PluginSharePrincipalRole;
use workx_app_server_protocol::PluginShareTargetRole;
use workx_config::types::McpServerConfig;
use workx_core_plugins::OPENAI_CURATED_MARKETPLACE_NAME;
use workx_core_plugins::PluginListBackgroundTaskOptions;
use workx_core_plugins::PluginMarketplaceContext;
use workx_core_plugins::RemotePluginInstallRequest;
use workx_core_plugins::RemotePluginOperationError;
use workx_core_plugins::RemotePluginOperationErrorKind;
use workx_core_plugins::is_openai_curated_marketplace_name;
use workx_core_plugins::loader::load_configured_plugin_mcp_servers;
use workx_core_plugins::manifest::is_agent_plugin_manifest;
use workx_core_plugins::remote::REMOTE_CREATED_BY_ME_MARKETPLACE_NAME;
use workx_core_plugins::remote::REMOTE_GLOBAL_MARKETPLACE_NAME;
use workx_core_plugins::remote::REMOTE_WORKSPACE_MARKETPLACE_NAME;
use workx_core_plugins::remote::REMOTE_WORKSPACE_SHARED_WITH_ME_MARKETPLACE_NAME;
use workx_core_plugins::remote::REMOTE_WORKSPACE_SHARED_WITH_ME_PRIVATE_MARKETPLACE_NAME;
use workx_core_plugins::remote::REMOTE_WORKSPACE_SHARED_WITH_ME_UNLISTED_MARKETPLACE_NAME;
use workx_core_plugins::remote::RemoteAppTemplateUnavailableReason;
use workx_core_plugins::remote::RemotePluginCatalogCacheMode;
use workx_core_plugins::remote::RemotePluginScope;
use workx_core_plugins::remote::is_valid_remote_plugin_id;
use workx_core_plugins::remote::validate_remote_plugin_id;
use workx_core_plugins::remote_bundle::RemotePluginBundleInstallError;
use workx_mcp::McpOAuthLoginSupport;
use workx_mcp::McpRuntimeContext;
use workx_mcp::oauth_login_support;
use workx_mcp::resolve_oauth_callback;
use workx_mcp::should_retry_without_scopes;
use workx_plugin::PluginId;
use workx_plugin::PluginTelemetryMetadata;
use workx_protocol::auth::AuthMode as DomainAuthMode;
use workx_rmcp_client::McpOAuthClientRegistration;
use workx_rmcp_client::OAuthDiscoveryTimeout;
use workx_rmcp_client::StreamableHttpRedirectMode;
use workx_rmcp_client::perform_oauth_login_silent;
...
pub(crate) struct PluginRequestProcessor {
    ...
    on_effective_plugins_changed:
        Arc<dyn Fn(workx_core_plugins::EffectivePluginsChange) + Send + Sync>,
}

fn plugin_skills_to_info(
    skills: &[workx_skills::SkillMetadata],
    ...
    skills
        ...
        .map(|skill| SkillSummary {
            ...
            interface: skill.interface.clone().map(|interface| {
                workx_app_server_protocol::SkillInterface {
...
fn load_shared_plugin_ids_by_local_path(
    ...
    workx_core_plugins::remote::load_plugin_share_remote_ids_by_local_path(
...
fn convert_configured_marketplace_plugin_to_plugin_summary(
    plugin: workx_core_plugins::ConfiguredMarketplacePlugin,
    shared_plugin_ids_by_local_path: &std::collections::BTreeMap<AbsolutePathBuf, String>,
) -> PluginSummary {
...
) -> workx_core_plugins::remote::RemotePluginShareDiscoverability {
    match discoverability {
        PluginShareDiscoverability::Listed => {
            workx_core_plugins::remote::RemotePluginShareDiscoverability::Listed
        }
        PluginShareDiscoverability::Unlisted => {
            workx_core_plugins::remote::RemotePluginShareDiscoverability::Unlisted
        }
        PluginShareDiscoverability::Private => {
            workx_core_plugins::remote::RemotePluginShareDiscoverability::Private
        }
    }
}
...
) -> workx_core_plugins::remote::RemotePluginShareUpdateDiscoverability {
    match discoverability {
        PluginShareUpdateDiscoverability::Listed => {
            workx_core_plugins::remote::RemotePluginShareUpdateDiscoverability::Listed
        }
        PluginShareUpdateDiscoverability::Unlisted => {
            workx_core_plugins::remote::RemotePluginShareUpdateDiscoverability::Unlisted
        }
        PluginShareUpdateDiscoverability::Private => {
            workx_core_plugins::remote::RemotePluginShareUpdateDiscoverability::Private
        }
    }
}
...
) -> workx_core_plugins::remote::RemotePluginShareTargetRole {
    match role {
        PluginShareTargetRole::Reader => {
            workx_core_plugins::remote::RemotePluginShareTargetRole::Reader
        }
        PluginShareTargetRole::Editor => {
            workx_core_plugins::remote::RemotePluginShareTargetRole::Editor
        }
    }
}

fn plugin_share_principal_role_from_remote(
    role: workx_core_plugins::remote::RemotePluginSharePrincipalRole,
) -> PluginSharePrincipalRole {
    match role {
        workx_core_plugins::remote::RemotePluginSharePrincipalRole::Reader => {
            PluginSharePrincipalRole::Reader
        }
        workx_core_plugins::remote::RemotePluginSharePrincipalRole::Editor => {
            PluginSharePrincipalRole::Editor
        }
        workx_core_plugins::remote::RemotePluginSharePrincipalRole::Owner => {
...
) -> Vec<workx_core_plugins::remote::RemotePluginShareTarget> {
    targets
        .into_iter()
        .map(
            |target| workx_core_plugins::remote::RemotePluginShareTarget {
                principal_type: match target.principal_type {
                    PluginSharePrincipalType::User => {
                        workx_core_plugins::remote::RemotePluginSharePrincipalType::User
                    }
                    PluginSharePrincipalType::Group => {
                        workx_core_plugins::remote::RemotePluginSharePrincipalType::Group
                    }
                    PluginSharePrincipalType::Workspace => {
                        workx_core_plugins::remote::RemotePluginSharePrincipalType::Workspace
                    }
                },
...
fn plugin_share_principal_from_remote(
    principal: workx_core_plugins::remote::RemotePluginSharePrincipal,
...
impl PluginRequestProcessor {
    pub(crate) fn new(
        ...
        on_effective_plugins_changed: Arc<
            dyn Fn(workx_core_plugins::EffectivePluginsChange) + Send + Sync,
        >,
    ) -> Self {
        Self {
            auth_manager,
    ...
    async fn plugin_list_response(
        ...
        let auth_mode = auth.as_ref().map(WorkxAuth::api_auth_mode);
        if include_local
            && force_refetch
            && plugins_manager
        ...
        let use_remote_global_catalog =
            include_global_remote && auth_mode.is_some_and(DomainAuthMode::uses_workx_backend);
            ...
            match tokio::task::spawn_blocking(move || {
                ...
                Ok::<
                    (
                        ...
                        Vec<workx_app_server_protocol::MarketplaceLoadErrorInfo>,
                        Vec<workx_core_plugins::ConfiguredMarketplace>,
                    ),
                    MarketplaceError,
                >((
                    outcome
                    ...
                    outcome
                        .errors
                        .into_iter()
                        .map(|err| workx_app_server_protocol::MarketplaceLoadErrorInfo {
        ...
        if include_vertical && !config.features.enabled(Feature::RemotePlugin) {
            match workx_core_plugins::remote::fetch_openai_curated_remote_collection_marketplace(
        ...
        if !remote_sources.is_empty() {
            match workx_core_plugins::remote::fetch_remote_marketplaces(
                &remote_plugin_service_config,
                auth.as_ref(),
                &remote_sources,
                /*catalog_cache_root*/ Some(config.workx_home.as_path()),
    ...
    async fn plugin_installed_response(
        ...
        let use_remote_global_catalog = context.remote_plugins_enabled()
            && auth_mode.is_some_and(DomainAuthMode::uses_workx_backend);
    ...
    async fn load_local_installed_and_suggested_plugins(
        &self,
        plugins_manager: Arc<workx_core_plugins::PluginsManager>,
        config: &Config,
    ...
    async fn load_remote_installed_plugins(
        &self,
        plugins_manager: Arc<workx_core_plugins::PluginsManager>,
        plugins_input: &workx_core_plugins::PluginsConfigInput,
        ...
        let plugin = match read_source {
            Ok(marketplace_path) => {
                ...
                let share_context = match share_context {
                    Some(context) => {
                        let remote_plugin_service_config = remote_plugin_service_config(&config);
                        match workx_core_plugins::remote::fetch_remote_plugin_share_context(
                            &remote_plugin_service_config,
                            auth.as_ref(),
                ...
                PluginDetail {
                    ...
                    hooks: outcome
                        ...
                        .hooks
                        .into_iter()
                        .map(|hook| workx_app_server_protocol::PluginHookSummary {
                            key: hook.key,
            ...
            Err(remote_marketplace_name) => {
                ...
                let remote_detail = workx_core_plugins::remote::fetch_remote_plugin_detail(
                    &remote_plugin_service_config,
                    auth.as_ref(),
                    &remote_marketplace_name,
                    &plugin_name,
                ...
                let plugin_apps = remote_detail
                    ...
                    .iter()
                    .cloned()
                    .map(workx_plugin::AppConnectorId)
                    .collect::<Vec<_>>();
    ...
    async fn plugin_skill_read_response(
        ...
        let auth = self.auth_manager.auth().await;
        let remote_plugin_service_config = remote_plugin_service_config(&config);
        let remote_skill_detail = workx_core_plugins::remote::fetch_remote_plugin_skill_detail(
            &remote_plugin_service_config,
            auth.as_ref(),
            &remote_marketplace_name,
            &remote_plugin_id,
    ...
    async fn plugin_share_save_response(
        ...
        let remote_plugin_service_config = remote_plugin_service_config(&config);
        let access_policy = workx_core_plugins::remote::RemotePluginShareAccessPolicy {
        ...
        let result = workx_core_plugins::remote::save_remote_plugin_share(
            &remote_plugin_service_config,
            auth.as_ref(),
            config.workx_home.as_path(),
            &plugin_path,
        ...
        workx_core_plugins::remote::invalidate_cached_remote_plugin_catalog_scopes(
            config.workx_home.as_path(),
            &remote_plugin_service_config,
            auth.as_ref(),
    ...
    async fn plugin_share_update_targets_response(
        ...
        let remote_plugin_service_config = remote_plugin_service_config(&config);
        let result = workx_core_plugins::remote::update_remote_plugin_share_targets(
            &remote_plugin_service_config,
            auth.as_ref(),
            &remote_plugin_id,
    ...
    async fn plugin_share_list_response(
        ...
        let (config, auth) = self.load_plugin_share_config_and_auth().await?;
        let remote_plugin_service_config = remote_plugin_service_config(&config);
        let data = workx_core_plugins::remote::list_remote_plugin_shares(
            &remote_plugin_service_config,
            auth.as_ref(),
            config.workx_home.as_path(),
        )
    ...
    async fn plugin_share_checkout_response(
        ...
        let remote_plugin_service_config = remote_plugin_service_config(&config);
        let result = workx_core_plugins::remote::checkout_remote_plugin_share(
            &remote_plugin_service_config,
            auth.as_ref(),
            config.workx_home.as_path(),
            &remote_plugin_id,
    ...
    async fn plugin_share_delete_response(
        ...
        let remote_plugin_service_config = remote_plugin_service_config(&config);
        workx_core_plugins::remote::delete_remote_plugin_share(
            &remote_plugin_service_config,
            auth.as_ref(),
            config.workx_home.as_path(),
            &remote_plugin_id,
    ...
    async fn load_plugin_share_config_and_auth(
        &self,
    ) -> Result<(Config, Option<WorkxAuth>), JSONRPCErrorError> {
        let config = self.load_latest_config(/*fallback_cwd*/ None).await?;
        if !config.features.enabled(Feature::Plugins) {
    ...
    async fn plugin_install_response(
        ...
        let plugin_mcp_servers = load_configured_plugin_mcp_servers(
            ...
            auth.as_ref().map(WorkxAuth::auth_mode),
            &result.plugin_id,
    ...
    async fn remote_plugin_install_response(
        ...
        let is_chatgpt_auth = auth.as_ref().is_some_and(WorkxAuth::is_chatgpt_auth);
    ...
    async fn plugin_apps_needing_auth_for_install(
        ...
        auth: Option<&WorkxAuth>,
        plugin_id: &str,
        plugin_app_declarations: &[workx_plugin::AppDeclaration],
    ) -> Vec<AppSummary> {
        if plugin_app_declarations.is_empty()
            || !config
                .features
                .apps_enabled_for_auth(auth.is_some_and(WorkxAuth::is_chatgpt_auth))
        {
            return Vec::new();
        }

        let plugin_apps =
            workx_plugin::app_connector_ids_from_declarations(plugin_app_declarations);
        ...
        let (accessible_connectors, workx_apps_ready) = match accessible_connectors_result {
            Ok(status) => (status.connectors, status.workx_apps_ready),
            Err(err) => {
                warn!(
                    plugin = plugin_id,
        ...
        if !workx_apps_ready {
            warn!(
                plugin = plugin_id,
                "workx_apps MCP not ready after plugin install; skipping appsNeedingAuth check"
            );
            return Vec::new();
        }
    ...
    async fn plugin_uninstall_response(
        ...
        if workx_plugin::PluginId::parse(&plugin_id).is_err()
...
async fn load_plugin_app_summaries(
    config: &Config,
    auth: Option<&WorkxAuth>,
    plugin_apps: &[workx_plugin::AppConnectorId],
    ...
    if let Some(auth) = auth.filter(|auth| {
        config
            .features
            .apps_enabled_for_auth(auth.uses_workx_backend())
    }) {
        metadata_by_id.extend(
            workx_connectors::ConnectorMetadataStore::new(
                config.chatgpt_base_url.clone(),
                auth.get_account_id(),
    ...
    app_ids
        ...
        .map(|app_id| {
            ...
            AppSummary {
                install_url: Some(workx_connectors::metadata::connector_install_url(
                    &name, &app_id,
                )),
                id: app_id,
                name,
...
fn plugin_app_category_by_id_from_value(value: &serde_json::Value) -> HashMap<String, String> {
    workx_core_plugins::loader::plugin_app_declarations_from_value(value)
        .into_iter()
...
fn remote_plugin_share_discoverability_to_info(
    discoverability: workx_core_plugins::remote::RemotePluginShareDiscoverability,
...
fn remote_plugin_bundle_install_error_to_jsonrpc(
    err: workx_core_plugins::remote_bundle::RemotePluginBundleInstallError,
) -> JSONRPCErrorError {

...
use workx_app_server_protocol::SelectedCapabilityRoot;
use workx_app_server_protocol::ThreadHistoryMode as ApiThreadHistoryMode;
use workx_app_server_protocol::ThreadRevertParams;
use workx_app_server_protocol::ThreadRevertResponse;
use workx_app_server_protocol::ThreadRevertedNotification;
use workx_app_server_protocol::ThreadSection;
use workx_app_server_protocol::ThreadSectionAppearance;
use workx_app_server_protocol::ThreadSectionMoveParams;
use workx_app_server_protocol::ThreadSectionMoveResponse;
use workx_extension_api::ExtensionDataInit;
use workx_extension_api::ThreadIdleCause;
use workx_protocol::SanitizedGitUrl;
...
const WORKX_TUI_CLIENT_NAME: &str = "workx-tui";
...
struct ThreadRevertRuntimeSnapshot {
    config: Config,
    settings: WorkxThreadSettingsOverrides,
...
fn collect_resume_override_mismatches(
    ...
    if let Some(requested_review_policy) = request.approvals_reviewer.as_ref() {
        let active_review_policy: workx_app_server_protocol::ApprovalsReviewer =
    ...
    if let Some(requested_sandbox) = request.sandbox.as_ref() {
        ...
        let sandbox_matches = matches!(
            ...
            (
                ...
                workx_protocol::protocol::SandboxPolicy::DangerFullAccess
            ) | (
                SandboxMode::DangerFullAccess,
...
fn validate_dynamic_tools(tools: &[DynamicToolSpec]) -> Result<(), String> {
    ...
    fn validate_dynamic_tool<'a>(
        ...
        if let Err(err) = workx_tools::parse_tool_input_schema(&tool.input_schema) {
            return Err(format!(
...
impl ThreadRequestProcessor {
    ...
    pub(crate) async fn thread_rollback(
        ...
        if app_server_client_name != Some(WORKX_TUI_CLIENT_NAME) {
    ...
    async fn load_thread(
        &self,
        thread_id: &str,
    ) -> Result<(ThreadId, Arc<WorkxThread>), JSONRPCErrorError> {
    ...
    fn listener_task_context(&self) -> ListenerTaskContext {
        ListenerTaskContext {
            ...
            workx_home: self.config.workx_home.to_path_buf(),
    ...
    async fn thread_start_inner(
        ...
        if matches!(
            history_mode,
            Some(workx_app_server_protocol::ThreadHistoryMode::Paginated)
    ...
    async fn request_trace_context(
        &self,
        request_id: &ConnectionRequestId,
    ) -> Option<workx_protocol::protocol::W3cTraceContext> {
    ...
    async fn thread_start_task(
        ...
        session_start_source: Option<workx_app_server_protocol::ThreadStartSource>,
        ...
        let effective_permissions_trust_project = match &effective_permission_profile {
            workx_protocol::models::PermissionProfile::Disabled
            | workx_protocol::models::PermissionProfile::External { .. } => true,
            workx_protocol::models::PermissionProfile::Managed { .. } => {
            ...
            && effective_permissions_trust_project
        {
            ...
            let cli_overrides_for_reload = if let Err(err) =
                workx_core::config::set_project_trust_level(
                    &listener_task_context.workx_home,
        ...
        if let Ok(Some(err)) =
            workx_core::check_execpolicy_for_warnings(&config.config_layer_stack).await
        {
        ...
        let new_thread = listener_task_context
            ...
            .start_thread(StartThreadOptions {
                ...
                initial_history: match session_start_source
                    .unwrap_or(workx_app_server_protocol::ThreadStartSource::Startup)
                {
                    workx_app_server_protocol::ThreadStartSource::Startup => InitialHistory::New,
                    workx_app_server_protocol::ThreadStartSource::Clear => InitialHistory::Cleared,
                },
                history_mode,
                thread_source,
                dynamic_tools,
        ...
        let NewThread {
            ...
            Err(err) => {
                ...
                return Err(match err.details() {
                    ...
                    WorkxErrorDetails::UnsupportedOperation(message) => {
    ...
    fn build_thread_config_overrides(
        ...
        approval_policy: Option<workx_app_server_protocol::AskForApproval>,
        approvals_reviewer: Option<workx_app_server_protocol::ApprovalsReviewer>,
        ...
        ConfigOverrides {
            ...
            approval_policy: approval_policy
                .map(workx_app_server_protocol::AskForApproval::to_core),
            approvals_reviewer: approvals_reviewer
                .map(workx_app_server_protocol::ApprovalsReviewer::to_core),
            sandbox_mode: sandbox.map(SandboxMode::to_core),
            workx_linux_sandbox_exe: self.arg0_paths.workx_linux_sandbox_exe.clone(),
    ...
    ) -> Result<(ThreadSetNameResponse, Option<ThreadNameUpdatedNotification>), JSONRPCErrorError>
    {
        ...
        let thread_id = ThreadId::from_string(&thread_id)
            .map_err(|err| invalid_request(format!("invalid thread id: {err}")))?;
        let Some(name) = workx_core::util::normalize_thread_name(&name) else {
    ...
    async fn memory_reset_response_inner(&self) -> Result<MemoryResetResponse, JSONRPCErrorError> {
        ...
        clear_memory_roots_contents(&self.config.workx_home)
            .await
            .map_err(|err| {
                internal_error(format!(
                    "failed to clear memory directories under {}: {err}",
                    self.config.workx_home.display()
                ))
            })?;
    ...
    async fn thread_revert_response(
        ...
        let revert_result = self
            .thread_store
            .revert_thread(workx_thread_store::RevertThreadParams {
                thread_id,
                before_turn_id,
            })
            .await
    ...
    async fn reload_paginated_thread(
        ...
        let thread_state = self.thread_state_manager.thread_state(thread_id).await;
        self.ensure_listener_task_running(thread_id, Arc::clone(&workx_thread), thread_state)
            .await?;
        let mut thread = self
    ...
    async fn apply_thread_read_store_fields(
        ...
        if include_turns {
            if matches!(
                thread.history_mode,
                workx_app_server_protocol::ThreadHistoryMode::Paginated
            ) {
                self.thread_store
    ...
    ) -> Result<workx_app_server_protocol::TurnsPage, JSONRPCErrorError> {
    ...
    async fn thread_resume_inner(
            ...
            .await
        {
            Ok(NewThread {
                ...
                let instruction_sources = workx_thread.legacy_instruction_sources().await;
                let SessionConfiguredEvent { rollout_path, .. } = session_configured;
                ...
                thread.thread_source = workx_thread
                    .config_snapshot()
                    .await
                    .thread_source
                    .map(Into::into);
                ...
                let config_snapshot = workx_thread.config_snapshot().await;
                let (turns_backwards_cursor, items_backwards_cursor) =
                    if matches!(config_snapshot.history_mode, ThreadHistoryMode::Paginated) {
    ...
    async fn read_stored_thread_for_resume(
            ...
            && matches!(stored_thread.history_mode, ThreadHistoryMode::Paginated)
        {
            ...
            if let Some(current_path) = current_thread.rollout_path.as_ref()
                && !path_utils::paths_match_after_normalization(
                    workx_rollout::plain_rollout_path(requested_path).as_path(),
                    workx_rollout::plain_rollout_path(current_path).as_path(),
                )
            {
                return Err(invalid_request(format!(
        ...
        if stored_thread.archived_at.is_some() {
            ...
            return Err(invalid_request(format!(
                "session {thread_id} is archived. Run `workx unarchive {thread_id}` to unarchive it first."
            )));
        }
    ...
    async fn thread_fork_inner(
        ...
        let source_thread_name = source_thread
            .name
            .as_deref()
            .and_then(workx_core::util::normalize_thread_name);
        let prepared_fork = if paginated_source {
            let boundary = match (last_turn_id.as_deref(), before_turn_id.as_deref()) {
                (Some(turn_id), None) => {
                    workx_thread_store::ForkBoundary::ThroughTurn(turn_id.to_string())
                }
                (None, Some(turn_id)) => {
                    workx_thread_store::ForkBoundary::BeforeTurn(turn_id.to_string())
                }
                (None, None) => workx_thread_store::ForkBoundary::Latest,
                (Some(_), Some(_)) => unreachable!("fork boundaries are mutually exclusive"),
            };
            Some(
                self.thread_store
                    .prepare_fork(workx_thread_store::PrepareForkParams {
        ...
        let NewThread {
            ...
            Err(err) => {
                remove_pending_project_metadata(self.thread_store.as_ref(), reserved_thread_id)
                    .await;
                return Err(match err.details() {
                    WorkxErrorDetails::Io(_) | WorkxErrorDetails::Json(_) => {
...
fn deserialize_stored_thread_item(
    item: workx_thread_store::StoredThreadItem,
...
fn stored_turn_to_api_turn(
    ...
    let error = turn.error.map(|error| TurnError {
        ...
        workx_error_info: error.workx_error_info,
...
pub(super) fn core_thread_write_error(operation: &str, err: WorkxErr) -> JSONRPCErrorError {
    match err.details() {
        WorkxErrorDetails::ThreadNotFound(thread_id) => {
            invalid_request(format!("thread not found: {thread_id}"))
        }
        WorkxErrorDetails::InvalidRequest(message) => invalid_request(message.clone()),
        WorkxErrorDetails::UnsupportedOperation(message) => method_not_found(message.clone()),
...
) -> (Thread, Option<workx_thread_store::StoredThreadHistory>) {
...
fn summary_from_state_db_metadata(
    ...
    cli_version: String,
    source: String,
    _thread_source: Option<workx_protocol::protocol::ThreadSource>,
    ...
    let source = serde_json::from_str(&source)
        ...
        .unwrap_or(workx_protocol::protocol::SessionSource::Unknown);
...
fn preview_from_rollout_items(items: &[RolloutItem]) -> String {
    items
        ...
        .find_map(|item| match item {
            RolloutItem::ResponseItem(item) => match workx_core::parse_turn_item(&item.item) {
                Some(workx_protocol::items::TurnItem::UserMessage(user)) => Some(user.message()),
                _ => None,
            },
            _ => None,
        })
...
fn build_thread_from_snapshot(
    thread_id: ThreadId,
    session_id: String,
    multi_agent_version: Option<workx_protocol::protocol::MultiAgentVersion>,
    config_snapshot: &ThreadConfigSnapshot,
    path: Option<PathBuf>,
) -> Thread {

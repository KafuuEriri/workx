...
use workx_core::config::permission_profile_catalog;
use workx_hooks::HookListEntryHandler;
...
) -> Vec<workx_app_server_protocol::SkillMetadata> {
    skills
        .iter()
        .map(|skill| {
            let enabled = !disabled_paths.contains(&skill.path_to_skills_md);
            workx_app_server_protocol::SkillMetadata {
                ...
                dependencies: skill.dependencies.clone().map(|dependencies| {
                    workx_app_server_protocol::SkillDependencies {
                        tools: dependencies
                            .tools
                            .into_iter()
                            .map(|tool| workx_app_server_protocol::SkillToolDependency {
                                r#type: tool.r#type,
                                value: tool.value,
...
fn hooks_to_info(hooks: &[workx_hooks::HookListEntry]) -> Vec<HookMetadata> {
    hooks
        .iter()
...
fn errors_to_info(
    errors: &[workx_skills::SkillError],
) -> Vec<workx_app_server_protocol::SkillErrorInfo> {
    errors
        .iter()
        .map(|err| workx_app_server_protocol::SkillErrorInfo {
...
impl CatalogRequestProcessor {
                ...
                async move {
                    let config = match self.load_latest_config(Some(cwd.clone())).await {
                        ...
                        Err(error) => {
                            ...
                            return (
                                index,
                                workx_app_server_protocol::SkillsListEntry {
                                    cwd,
                                    skills: Vec::new(),
                                    errors: vec![workx_app_server_protocol::SkillErrorInfo {
                                        path: error_path,
                    ...
                    let skills_input = workx_skills_extension::HostSkillsLoadInput::new(
                        config.cwd.clone(),
    ...
    async fn hooks_list_response(
        ...
        for cwd in cwds {
                ...
                .await
            {
                ...
                Err(err) => {
                    let error_path = cwd.clone();
                    data.push(workx_app_server_protocol::HooksListEntry {
                        cwd,
                        hooks: Vec::new(),
                        warnings: Vec::new(),
                        errors: vec![workx_app_server_protocol::HookErrorInfo {
                            path: error_path,
            ...
            let hooks_enabled = config.features.enabled(Feature::WorkxHooks);
            let plugin_hooks = if hooks_enabled && config.features.enabled(Feature::Plugins) {
                let plugins_input = config.plugins_config_input();
                let plugin_outcome = plugins_manager.plugins_for_config(&plugins_input).await;
                workx_core_plugins::PluginHookLoadOutcome {
                ...
                workx_core_plugins::PluginHookLoadOutcome::default()
            };
            let hooks = workx_hooks::list_hooks(workx_hooks::HooksConfig {
    ...
    async fn skills_config_write_response_inner(
        ...
        let edits = vec![edit];
        ConfigEditsBuilder::new(&self.config.workx_home)
            .with_edits(edits)
            .apply()
            .await
            .map(|()| {

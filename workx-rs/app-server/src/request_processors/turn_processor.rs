...
use super::*;
use workx_agent_extension::AgentInvocation;
use workx_agent_extension::AgentRun;
use workx_agent_extension::AgentRunner;
...
use workx_protocol::protocol::AdditionalContextEntry as CoreAdditionalContextEntry;
use workx_protocol::protocol::AdditionalContextKind as CoreAdditionalContextKind;
use workx_protocol::protocol::TurnSettingsUpdate;
use workx_protocol::protocol::TurnSettingsUpdateOutcome;
use workx_skills::system_cache_root_dir;
...
struct ThreadSettingsBuildParams {
    ...
    sandbox_policy: Option<workx_app_server_protocol::SandboxPolicy>,
...
impl TurnRequestProcessor {
    ...
    fn review_request_from_target(
        ...
        let hint = workx_core::review_prompts::user_facing_hint(&core_target);
    ...
    async fn turn_start_inner(
        ...
        if turn_has_input && started {
            ...
            if config_snapshot.is_primary_environment_configured() {
                workx_memories_write::start_memories_startup_task(
    ...
    async fn build_thread_settings_overrides(
    ...
    ) -> Result<workx_protocol::protocol::ThreadSettingsOverrides, JSONRPCErrorError> {
        ...
        let approval_policy =
            approval_policy.map(workx_app_server_protocol::AskForApproval::to_core);
        let approvals_reviewer =
            approvals_reviewer.map(workx_app_server_protocol::ApprovalsReviewer::to_core);
        ...
        Ok(workx_protocol::protocol::ThreadSettingsOverrides {
            environments,
    ...
    async fn thread_settings_update_inner(
        ...
        if thread_settings != workx_protocol::protocol::ThreadSettingsOverrides::default() {
            self.submit_core_op(
                request_id,
                thread.as_ref(),
                ...
                let (message, data, error_type) = match reason {
                    ...
                    NotSubmittedReason::ActiveTurnNotSteerable { turn_kind } => {
                        let (message, turn_steer_error) = match turn_kind {
                            workx_protocol::protocol::NonSteerableTurnKind::Review => (
                                "cannot steer a review turn".to_string(),
                                TurnSteerRequestError::NonSteerableReview,
                            ),
                            workx_protocol::protocol::NonSteerableTurnKind::Compact => (
    ...
    async fn prepare_realtime_conversation_thread(
        ...
        request_id: &ConnectionRequestId,
        thread_id: &str,
    ) -> Result<Option<(ThreadId, Arc<WorkxThread>)>, JSONRPCErrorError> {
        ...
        self.submit_core_op(
            ...
            Op::RealtimeConversationStart(ConversationStartParams {
                ...
                flush_transcript_tail_on_session_end: params
                    ...
                    .unwrap_or(false),
                workx_responses_as_items: params.workx_responses_as_items.unwrap_or(false),
                workx_response_item_prefix: params.workx_response_item_prefix,
                workx_response_handoff_mode: params.workx_response_handoff_mode.unwrap_or_default(),
                workx_response_handoff_channel_prefixes: params
    ...
    async fn start_detached_review(
        ...
        if matches!(
            ...
            workx_protocol::protocol::ThreadHistoryMode::Paginated
        ) {
            return Err(invalid_request(
    ...
    async fn review_start_inner(
        ...
        match delivery.unwrap_or(ApiReviewDelivery::Inline).to_core() {
            ...
            CoreReviewDelivery::Detached => {
                let review_skill_path = system_cache_root_dir(&self.config.workx_home)
                    .join("review-agent")
                    .join("SKILL.md");
                let prompt = format!(

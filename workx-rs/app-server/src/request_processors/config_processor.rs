...
use workx_app_server_protocol::BrowserUseAccessApprovalLifetime;
use workx_app_server_protocol::BrowserUseOriginPolicy;
use workx_app_server_protocol::BrowserUseRequirements;
use workx_app_server_protocol::CliAuthCredentialsStoreMode;
use workx_app_server_protocol::ClientResponsePayload;
use workx_app_server_protocol::ComputerUseMacosRequirements;
use workx_app_server_protocol::ComputerUseRequirements;
use workx_app_server_protocol::ComputerUseWindowsExeRequirement;
use workx_app_server_protocol::ComputerUseWindowsRequirements;
use workx_app_server_protocol::ConfigBatchWriteParams;
use workx_app_server_protocol::ConfigReadParams;
use workx_app_server_protocol::ConfigReadResponse;
use workx_app_server_protocol::ConfigRequirements;
use workx_app_server_protocol::ConfigRequirementsReadResponse;
use workx_app_server_protocol::ConfigValueWriteParams;
use workx_app_server_protocol::ConfigWriteErrorCode;
use workx_app_server_protocol::ConfigWriteResponse;
use workx_app_server_protocol::ConfiguredHookHandler;
use workx_app_server_protocol::ConfiguredHookMatcherGroup;
use workx_app_server_protocol::ExperimentalFeatureEnablementSetParams;
use workx_app_server_protocol::ExperimentalFeatureEnablementSetResponse;
use workx_app_server_protocol::FeedbackRequirements;
...
use workx_app_server_protocol::NetworkRequirements;
use workx_app_server_protocol::NetworkUnixSocketPermission;
...
use workx_config::HookEventsToml;
use workx_config::HookHandlerConfig as CoreHookHandlerConfig;
use workx_config::ManagedHooksRequirementsToml;
use workx_config::MatcherGroup as CoreMatcherGroup;
use workx_config::ResidencyRequirement as CoreResidencyRequirement;
use workx_config::SandboxModeRequirement as CoreSandboxModeRequirement;
use workx_core::ThreadManager;
use workx_features::Feature;
use workx_features::canonical_feature_for_key;
use workx_features::feature_for_key;
...
impl ConfigRequestProcessor {
    ...
    ) -> Result<workx_core::config::Config, JSONRPCErrorError> {
        self.config_manager
    ...
    async fn write_value(
        ...
        let pending_changes = workx_core_plugins::toggles::collect_plugin_enabled_candidates(
...
fn map_requirements_toml_to_api(requirements: ConfigRequirementsToml) -> ConfigRequirements {
    ...
    ConfigRequirements {
        cli_auth_credentials_store: requirements.cli_auth_credentials_store.map(
            |mode| match mode {
                workx_config::types::AuthCredentialsStoreMode::File => {
                    CliAuthCredentialsStoreMode::File
                }
                workx_config::types::AuthCredentialsStoreMode::Keyring => {
                    CliAuthCredentialsStoreMode::Keyring
                }
                workx_config::types::AuthCredentialsStoreMode::Auto => {
                    CliAuthCredentialsStoreMode::Auto
                }
                workx_config::types::AuthCredentialsStoreMode::Ephemeral => {
        ...
        allowed_approval_policies: requirements.allowed_approval_policies.map(|policies| {
            policies
                .into_iter()
                .map(workx_app_server_protocol::AskForApproval::from)
                .collect()
        }),
        allowed_approvals_reviewers: requirements.allowed_approvals_reviewers.map(|reviewers| {
            reviewers
                .into_iter()
                .map(workx_app_server_protocol::ApprovalsReviewer::from)
                .collect()
        }),
                ...
                .map(|implementations| {
                    implementations
                        ...
                        .map(|implementation| match implementation {
                            workx_config::types::WindowsSandboxModeToml::Elevated => {
                                WindowsSandboxSetupMode::Elevated
                            }
                            workx_config::types::WindowsSandboxModeToml::Unelevated => {
...
fn map_computer_use_requirements_to_api(
    computer_use: workx_config::ComputerUseRequirementsToml,
...
fn map_browser_use_requirements_to_api(
    browser_use: workx_config::BrowserUseRequirementsToml,
...
fn map_browser_use_origin_policy_to_api(
    policy: workx_config::BrowserUseOriginPolicyToml,
...
fn map_allow_deny_requirement_to_api(
    requirement: workx_config::AllowDenyRequirementToml,
) -> AllowDenyRequirement {
    match requirement {
        workx_config::AllowDenyRequirementToml::Allow => AllowDenyRequirement::Allow,
        workx_config::AllowDenyRequirementToml::Deny => AllowDenyRequirement::Deny,
    }
}

fn map_browser_use_access_approval_lifetime_to_api(
    lifetime: workx_config::BrowserUseAccessApprovalLifetimeToml,
) -> BrowserUseAccessApprovalLifetime {
    match lifetime {
        workx_config::BrowserUseAccessApprovalLifetimeToml::Turn => {
            BrowserUseAccessApprovalLifetime::Turn
        }
        workx_config::BrowserUseAccessApprovalLifetimeToml::Thread => {
...
) -> workx_app_server_protocol::ResidencyRequirement {
    match residency {
        CoreResidencyRequirement::Us => workx_app_server_protocol::ResidencyRequirement::Us,
    }
}

fn map_network_requirements_to_api(
    network: workx_config::NetworkRequirementsToml,
) -> NetworkRequirements {
    let allowed_domains = network
        .domains
        .as_ref()
        .and_then(workx_config::NetworkDomainPermissionsToml::allowed_domains);
    let denied_domains = network
        .domains
        .as_ref()
        .and_then(workx_config::NetworkDomainPermissionsToml::denied_domains);
    let allow_unix_sockets = network
        .unix_sockets
        .as_ref()
        .map(workx_config::NetworkUnixSocketPermissionsToml::allow_unix_sockets)
...
fn map_network_domain_permission_to_api(
    permission: workx_config::NetworkDomainPermissionToml,
) -> NetworkDomainPermission {
    match permission {
        workx_config::NetworkDomainPermissionToml::Allow => NetworkDomainPermission::Allow,
        workx_config::NetworkDomainPermissionToml::Deny => NetworkDomainPermission::Deny,
    }
}

fn map_network_unix_socket_permission_to_api(
    permission: workx_config::NetworkUnixSocketPermissionToml,
) -> NetworkUnixSocketPermission {
    match permission {
        workx_config::NetworkUnixSocketPermissionToml::Allow => NetworkUnixSocketPermission::Allow,
        workx_config::NetworkUnixSocketPermissionToml::Deny => NetworkUnixSocketPermission::Deny,
    }
}
...
mod tests {
    ...
    use workx_app_server_protocol::FeedbackRequirements;
    use workx_app_server_protocol::WindowsSandboxSetupMode;
    use workx_config::AllowDenyRequirementToml;
    use workx_config::AutoReviewRequirementsToml;
    use workx_config::BrowserUseAccessApprovalLifetimeToml;
    use workx_config::BrowserUseOriginPolicyToml;
    use workx_config::BrowserUseRequirementsToml;
    use workx_config::ComputerUseMacosRequirementsToml;
    use workx_config::ComputerUseRequirementsToml;
    use workx_config::ComputerUseWindowsExeRequirementToml;
    use workx_config::ComputerUseWindowsRequirementsToml;
    use workx_config::ConfigRequirementsToml;
    use workx_config::ModelsRequirementsToml;
    use workx_config::NewThreadModelDefaultsToml;
    use workx_config::WindowsRequirementsToml;

...
mod thread_processor_behavior_tests {
    async fn forked_from_id_from_rollout(path: &Path) -> Option<String> {
        workx_core::read_session_meta_line(path)
            .await
            .ok()
    ...
    fn dynamic_tool(
        ...
        match namespace {
            Some(namespace) => {
                DynamicToolSpec::Namespace(workx_app_server_protocol::DynamicToolNamespaceSpec {
    ...
    fn validate_dynamic_tools_rejects_duplicate_name_in_same_namespace() {
        ...
        let tools = vec![DynamicToolSpec::Namespace(
            workx_app_server_protocol::DynamicToolNamespaceSpec {
        ...
        assert!(err.contains("workx_app"), "unexpected error: {err}");
        assert!(err.contains("my_tool"), "unexpected error: {err}");
    }
    ...
    fn validate_dynamic_tools_rejects_namespace_not_supported_by_responses() {
        ...
        assert!(err.contains("workx.app"), "unexpected error: {err}");
        assert!(
            err.contains("Responses API") && err.contains("^[a-zA-Z0-9_-]+$"),
            "unexpected error: {err}"
        );
    ...
    fn summary_from_stored_thread_preserves_millisecond_precision() {
        ...
        let stored_thread = StoredThread {
            ...
            thread_source: Some(workx_protocol::protocol::ThreadSource::User),
            agent_nickname: None,
            agent_role: None,
            agent_path: None,
            git_info: None,
    ...
    fn collect_resume_override_mismatches_includes_service_tier() {
        ...
        let config_snapshot = ThreadConfigSnapshot {
            ...
            approval_policy: workx_protocol::protocol::AskForApproval::OnRequest,
            approvals_reviewer: workx_protocol::config_types::ApprovalsReviewer::User,
            permission_profile: workx_protocol::models::PermissionProfile::Disabled,
            full_access: false,
    ...
    fn test_thread_metadata(
        ...
        let mut builder = ThreadMetadataBuilder::new(
            ...
            Utc::now(),
            workx_protocol::protocol::SessionSource::default(),
        );
    ...
    async fn read_summary_from_rollout_preserves_agent_nickname() -> Result<()> {
        ...
        let session_meta = SessionMeta {
            ...
            thread_source: Some(workx_protocol::protocol::ThreadSource::Subagent),
    ...
    fn summary_from_state_db_metadata_preserves_agent_nickname() -> Result<()> {
        ...
        let summary = summary_from_state_db_metadata(
            ...
            "0.0.0".to_string(),
            source,
            Some(workx_protocol::protocol::ThreadSource::Subagent),

...
//! the configured Workx executable as a subprocess, accepts only valid JSON from
//! `workx doctor --json --feedback`, derives a small set of Sentry tags, and otherwise
...
use workx_feedback::FeedbackAttachment;
...
pub(crate) struct DoctorFeedbackReport {
    /// JSON support report to upload as `workx-doctor-report.json`.
    pub(crate) attachment: FeedbackAttachment,
    /// Low-cardinality Sentry tags derived from the report status and check ids.
    pub(crate) tags: BTreeMap<String, String>,
}

/// Runs `workx --cd <workspace> doctor --json --feedback` and returns a best-effort
/// feedback attachment.
///
/// Failure to spawn Workx, finish before the timeout, or parse JSON means the
...
pub(crate) async fn doctor_feedback_report(
    ...
    let mut command = doctor_command(&executable, workspace, config.workx_home.as_path());
...
fn doctor_command(executable: &Path, cwd: &Path, workx_home: &Path) -> Command {
...
mod tests {
    ...
    fn doctor_command_keeps_missing_workspace_out_of_process_cwd() {
        let workx_home = tempdir().expect("workx home");
        let workspace = workx_home.path().join("deleted-workspace");

        let command = doctor_command(Path::new("workx"), &workspace, workx_home.path());
        let command = command.as_std();

        assert_eq!(command.get_current_dir(), Some(workx_home.path()));
        assert_eq!(
        ...
        assert_eq!(
            ...
            [("WORKX_HOME".as_ref(), Some(workx_home.path().as_os_str()))]
        );
    }

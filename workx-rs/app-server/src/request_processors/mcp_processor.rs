...
use workx_mcp::ReadResourceRequestParams;
...
impl McpRequestProcessor {
    ...
    async fn mcp_server_oauth_login_response(
        ...
        let effective_servers = workx_mcp::effective_mcp_servers(&mcp_config, auth.as_ref());
        ...
        let discovered_scopes = if scopes.is_none() && server.scopes.is_none() {
            discover_supported_scopes(
                ...
                workx_rmcp_client::OAuthDiscoveryTimeout::Requested,
                redirect_mode,
            )
            .await
        } else {
    ...
    async fn list_mcp_server_status_response(
        request_id: String,
        params: ListMcpServerStatusParams,
        mcp_config: workx_mcp::McpConfig,
        auth: Option<WorkxAuth>,
        runtime_context: McpRuntimeContext,
        mcp_manager: Arc<McpManager>,
        thread: Option<Arc<workx_core::WorkxThread>>,
        ...
        let snapshot = collect_mcp_server_status_snapshot_with_detail(
            ...
            request_id,
            runtime_context,
            mcp_manager.workx_apps_tools_cache(),
    ...
    async fn read_mcp_resource(
        ...
        if let Some(thread_id) = thread_id {
            ...
            tokio::spawn(async move {
                let origin_call_id =
                    origin_call_id.filter(|_| server == workx_mcp::WORKX_APPS_MCP_SERVER_NAME);
        ...
        let mcp_manager = self.thread_manager.mcp_manager();
        let mcp_config = mcp_manager.runtime_config(&config).await;
        let workx_apps_tools_cache = mcp_manager.workx_apps_tools_cache();
...
fn mcp_operation_error(error: anyhow::Error) -> JSONRPCErrorError {
    match workx_rmcp_client::mcp_error(&error) {

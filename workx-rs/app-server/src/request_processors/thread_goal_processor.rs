...
use workx_goal_extension::GoalServiceError;
...
impl ThreadGoalRequestProcessor {
    ...
    async fn thread_goal_set_inner(
        ...
        let persist_result = match self.thread_manager.get_thread(thread_id).await {
            Ok(thread) => match thread.rollout_path() {
                Some(path) if workx_rollout::existing_rollout_path(&path).await.is_none() => {
    ...
    async fn state_db_for_materialized_thread(
        ...
        if let Ok(thread) = self.thread_manager.get_thread(thread_id).await {
            ...
            let rollout_path = workx_rollout::find_thread_path_by_id_str(
            ...
            if matches!(access, GoalAccess::Mutate) {
                let session_meta = workx_rollout::read_session_meta_line(&rollout_path)
                    .await
                    .map_err(|err| {
    ...
    async fn reconcile_thread_goal_rollout(
        ...
        let rollout_path = match running_thread.as_ref() {
            ...
            None => workx_rollout::find_thread_path_by_id_str(
                &self.config.workx_home,
                &thread_id.to_string(),
                self.state_db.as_deref(),
            )
        ...
        if let Ok(Some(metadata)) = state_db.get_thread(thread_id).await
            && workx_rollout::plain_rollout_path(metadata.rollout_path.as_path())
                == workx_rollout::plain_rollout_path(rollout_path.as_path())
            && let Some(existing_path) =
                workx_rollout::existing_rollout_path(metadata.rollout_path.as_path()).await
            && workx_rollout::read_session_meta_line(existing_path.as_path())
                .await
...
pub(super) fn api_thread_goal_from_state(goal: workx_state::ThreadGoal) -> ThreadGoal {
    ThreadGoal {
...
fn api_thread_goal_status_from_state(status: workx_state::ThreadGoalStatus) -> ThreadGoalStatus {
    match status {

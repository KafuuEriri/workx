...
use workx_login::LoginOnboardingEntrypoint;
use workx_login::login_with_bedrock_access_keys;
use workx_model_provider::is_supported_amazon_bedrock_region;
...
const LOGIN_ISSUER_OVERRIDE_ENV_VAR: &str = "WORKX_APP_SERVER_LOGIN_ISSUER";
#[cfg(debug_assertions)]
const LOGIN_OPEN_APP_URL_OVERRIDE_ENV_VAR: &str = "WORKX_APP_SERVER_DEV_OPEN_APP_URL";
...
impl AccountRequestProcessor {
    ...
    fn current_account_updated_notification(&self) -> AccountUpdatedNotification {
        ...
        AccountUpdatedNotification {
            auth_mode: auth
                .as_ref()
                .map(WorkxAuth::api_auth_mode)
                .map(auth_mode_to_api),
            plan_type: auth.as_ref().and_then(WorkxAuth::account_plan_type),
        }
    }
            ...
            LoginAccountParams::Chatgpt {
                ...
                let login_success_page = if use_hosted_login_success_page {
                    let app_brand = match app_brand.unwrap_or_default() {
                        LoginAppBrand::Workx => LoginSuccessPageBrand::Workx,
                        LoginAppBrand::Chatgpt => LoginSuccessPageBrand::Chatgpt,
                    };
                    LoginSuccessPage::Hosted {
                        url: WORKX_OPEN_APP_URL.parse().map_err(|err| {
                            internal_error(format!("invalid Workx open app URL: {err}"))
                        })?,
                        app_brand,
                    }
                } else {
    ...
    async fn login_chatgpt_common(
        ...
        let opts = {
                ...
                && !open_app_url.trim().is_empty()
            {
                *url = open_app_url
                    .parse()
                    .map_err(|err| internal_error(format!("invalid Workx open app URL: {err}")))?;
            }
            opts
        };
    ...
    async fn get_account_rate_limits_response(
        ...
        let Some(auth) = self.auth_manager.auth().await else {
            return Err(invalid_request(
                "workx account authentication required to read rate limits",
            ));
        };
        ...
        let response = response
            .map_err(|err| internal_error(format!("failed to fetch workx rate limits: {err}")))?;
        if response.rate_limits.is_empty() {
            return Err(internal_error(
                "failed to fetch workx rate limits: no snapshots returned",
            ));
        }
        ...
        let rate_limits_by_limit_id: HashMap<_, _> = response
            ...
            .map(|snapshot| {
                let limit_id = snapshot
                    .limit_id
                    .clone()
                    .unwrap_or_else(|| "workx".to_string());
                (limit_id, snapshot)
            })
            .collect();
    ...
    async fn get_account_token_usage_response(
        ...
        let Some(auth) = self.auth_manager.auth().await else {
            return Err(invalid_request(
                "workx account authentication required to read token usage",
            ));
        };
        ...
        if let Some(thread_id) = thread_id {
            ...
            let thread_usage = match usage {
                Ok(usage) => Some(workx_app_server_protocol::ThreadUsage {
                    ...
                    groups: usage
                        ...
                        .into_iter()
                        .map(
                            |group| workx_app_server_protocol::ThreadUsageBreakdownGroup {
                                model: group.model,
    ...
    async fn get_workspace_messages_response(
        ...
        let Some(auth) = self.auth_manager.auth().await else {
            return Err(invalid_request(
                "workx account authentication required to read workspace messages",
            ));
        };
    ...
    async fn send_add_credits_nudge_email_inner(
        ...
        let Some(auth) = self.auth_manager.auth().await else {
            return Err(invalid_request(
                "workx account authentication required to notify workspace owner",
            ));
        };
...
mod tests {
    ...
    use workx_backend_client::TokenUsageProfileDailyBucket;
    use workx_backend_client::TokenUsageProfileStats;

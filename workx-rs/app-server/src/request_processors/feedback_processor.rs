...
use workx_connectors::connector_runtime_cache_path;
#[cfg(target_os = "windows")]
use workx_feedback::WINDOWS_SANDBOX_LOG_ATTACHMENT_FILENAME;
use workx_feedback::WORKX_APP_DIRECTORY_CACHE_ATTACHMENT_FILENAME;
use workx_feedback::WORKX_APPS_TOOLS_CACHE_ATTACHMENT_FILENAME;
use workx_feedback::guardian_review_failures;
...
impl FeedbackRequestProcessor {
    ...
    async fn upload_feedback_response(
        ...
        if let Some(chatgpt_user_id) = auth
            .as_ref()
            .and_then(workx_login::WorkxAuth::get_chatgpt_user_id)
        {
            tracing::info!(target: "feedback_tags", chatgpt_user_id);
        }
        if let Some(account_id) = auth
            .as_ref()
            .and_then(workx_login::WorkxAuth::get_account_id)
        {
        ...
        if include_logs {
            ...
            if let Some(sandbox_log_attachment) =
                windows_sandbox_log_attachment(&self.config.workx_home)
...
fn tool_cache_feedback_attachments(
    ...
    if directory_cache_path.is_file() {
        attachments.push(FeedbackAttachmentPath {
            ...
            attachment_filename_override: Some(
                WORKX_APP_DIRECTORY_CACHE_ATTACHMENT_FILENAME.to_string(),
            ),
        });
    }
...
#[cfg(target_os = "windows")]
fn windows_sandbox_log_attachment(workx_home: &Path) -> Option<FeedbackAttachmentPath> {
    let sandbox_log_path = workx_windows_sandbox::current_log_file_path_for_workx_home(workx_home);
    sandbox_log_path
        .is_file()
...
fn windows_sandbox_log_attachment(_workx_home: &Path) -> Option<FeedbackAttachmentPath> {
    None
}
...
mod tests {
    ...
    fn feedback_rollout(
        ...
        let mut lines = vec![RolloutLine {
            ...
            item: RolloutItem::SessionMeta(SessionMetaLine {
                meta: workx_protocol::protocol::SessionMeta {
                    cwd: tempdir.path().to_path_buf(),
                    base_instructions: Some(workx_protocol::models::BaseInstructions {
        ...
        lines.extend(turns.iter().map(|(turn_id, model, effort)| {
            RolloutLine {
                ...
                item: RolloutItem::TurnContext(TurnContextItem {
                    ...
                    current_date: None,
                    timezone: None,
                    approval_policy: workx_protocol::protocol::AskForApproval::Never,
                    approvals_reviewer: None,
                    sandbox_policy: workx_protocol::protocol::SandboxPolicy::new_read_only_policy(),
    ...
    fn tool_cache_feedback_attachments_include_existing_active_cache_files() {
        let workx_home = tempfile::tempdir().expect("create tempdir");
        let auth = WorkxAuth::create_dummy_chatgpt_auth_for_testing();
        let tools_cache_path = connector_runtime_cache_path(workx_home.path(), Some(&auth));
        ...
        assert_eq!(
            ...
            vec![
                (
                    tools_cache_path,
                    Some(WORKX_APPS_TOOLS_CACHE_ATTACHMENT_FILENAME.to_string()),
                ),
                (
                    directory_cache_path,
                    Some(WORKX_APP_DIRECTORY_CACHE_ATTACHMENT_FILENAME.to_string()),
                ),
            ]
        );
    }
    ...
    fn tool_cache_feedback_attachments_include_directory_cache_without_account_id() {
        let workx_home = tempfile::tempdir().expect("create tempdir");
        let auth = WorkxAuth::Headers(workx_login::AuthHeaders::new(HeaderMap::new()));
        let directory_cache_context = ConnectorDirectoryCacheContext::new(
            workx_home.path().to_path_buf(),
            ConnectorDirectoryCacheKey::new(
                "https://chatgpt.com/backend-api".to_string(),
    ...
    fn windows_sandbox_log_attachment_uses_current_log() {
        let workx_home = tempfile::tempdir().expect("create tempdir");
        let sandbox_dir = workx_windows_sandbox::sandbox_dir(workx_home.path());
        std::fs::create_dir_all(&sandbox_dir).expect("create sandbox dir");
        let sandbox_log_path =
            workx_windows_sandbox::current_log_file_path_for_workx_home(workx_home.path());
        std::fs::write(&sandbox_log_path, "sandbox log").expect("write sandbox log");

        let attachment = windows_sandbox_log_attachment(workx_home.path())
            .map(|attachment| (attachment.path, attachment.attachment_filename_override));

...
use workx_app_server_protocol::Project;
...
use workx_thread_store::CreateProjectParams as StoreCreateProjectParams;
use workx_thread_store::ListProjectsParams as StoreListProjectsParams;
use workx_thread_store::MoveProjectParams as StoreMoveProjectParams;
use workx_thread_store::ProjectMoveOutcome;
use workx_thread_store::ProjectSortKey as StoreProjectSortKey;
use workx_thread_store::SortDirection as StoreSortDirection;
use workx_thread_store::StoredProject;
use workx_thread_store::StoredProjectRoot;
use workx_thread_store::ThreadStore;
use workx_thread_store::ThreadStoreError;
use workx_thread_store::UpdateProjectParams as StoreUpdateProjectParams;
...
fn validate_roots(roots: Vec<ProjectRoot>) -> Result<Vec<StoredProjectRoot>, JSONRPCErrorError> {
    ...
    roots
        .into_iter()
        .map(|root| {
            let path = workx_utils_absolute_path::AbsolutePathBuf::from_absolute_path_checked(
...
fn api_project(project: StoredProject) -> Result<Project, JSONRPCErrorError> {
    Ok(Project {
        ...
        roots: project
            ...
            .map(|root| {
                Ok(ProjectRoot {
                    path: workx_utils_absolute_path::AbsolutePathBuf::from_absolute_path(root.path)
                        .map_err(|error| {

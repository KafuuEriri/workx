...
//!
//! # Relationship to `workx-app-server-client`
//!
//! This module provides the low-level runtime handle ([`InProcessClientHandle`]).
//! Higher-level callers (TUI, exec) should go through `workx-app-server-client`,
...
use workx_app_server_protocol::AgentMessageDelivery;
...
use workx_app_server_protocol::Result;
...
pub use workx_rollout::StateDbHandle;
pub use workx_state::log_db::LogDbLayer;
...
/// through `workx-app-server-client`, which adds worker-task buffering,
...
pub struct InProcessClientHandle {
    ...
    #[cfg(test)]
    _test_workx_home: Option<tempfile::TempDir>,
}
...
async fn start_uninitialized(args: InProcessStartArgs) -> IoResult<InProcessClientHandle> {
    ...
    let installation_id = resolve_installation_id(&args.config.workx_home).await?;
    let auth_manager =
        AuthManager::shared_from_config(args.config.as_ref(), args.enable_workx_api_key_env)
            .await
    ...
    let runtime_handle = tokio::spawn(async move {
        ...
        let config_manager = ConfigManager::new(
            args.config.workx_home.to_path_buf(),
            args.cli_overrides,
...
mod tests {
    ...
    async fn start_test_client_with_capacity(
        ...
        let config = Arc::new(build_test_config(workx_home.path()).await);
        let state_db = workx_rollout::state_db::try_init(config.as_ref())
            .await
        ...
        let args = InProcessStartArgs {
            ...
            initialize: InitializeParams {
                client_info: ClientInfo {
                    name: "workx-in-process-test".to_string(),
                    title: None,
        ...
        client._test_workx_home = Some(workx_home);
        client
    }

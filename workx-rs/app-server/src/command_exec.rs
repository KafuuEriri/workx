...
use workx_app_server_protocol::CommandExecResizeResponse;
...
use workx_app_server_protocol::CommandExecTerminateResponse;
use workx_app_server_protocol::CommandExecWriteParams;
use workx_app_server_protocol::CommandExecWriteResponse;
use workx_app_server_protocol::JSONRPCErrorError;
use workx_app_server_protocol::ServerNotification;
use workx_core::config::StartedNetworkProxy;
use workx_core::exec::ExecExpiration;
use workx_core::exec::ExecExpirationOutcome;
use workx_core::exec::IO_DRAIN_TIMEOUT_MS;
use workx_core::sandboxing::ExecRequest;
use workx_protocol::exec_output::bytes_to_string_smart;
use workx_sandboxing::SandboxType;
use workx_utils_pty::DEFAULT_OUTPUT_BYTES_CAP;
use workx_utils_pty::ProcessHandle;
use workx_utils_pty::SpawnedProcess;
use workx_utils_pty::TerminalSize;
    ...
    pub(crate) async fn start(
        ...
        if matches!(exec_request.sandbox, SandboxType::WindowsRestrictedToken) {
            ...
            tokio::spawn(async move {
                ...
                match workx_core::sandboxing::execute_env(exec_request, /*stdout_stream*/ None)
                    .await
                {
                    Ok(output) => {
                        outgoing
        ...
        let spawned = if tty {
            workx_utils_pty::spawn_pty_process(
                program,
                args,
                cwd.as_path(),
                &env,
            ...
            workx_utils_pty::spawn_pipe_process(program, args, cwd.as_path(), &env, &arg0, &[])
                .await
        } else {
            workx_utils_pty::spawn_pipe_process_no_stdin(
                program,
                args,
                cwd.as_path(),
                &env,
...
mod tests {
    ...
    fn windows_sandbox_exec_request() -> ExecRequest {
        ...
        ExecRequest::new(
            ...
            workx_core::exec::ExecCapturePolicy::ShellTool,
        ...
        let err = manager
            .start(StartCommandExecParams {
                ...
                request_id: ConnectionRequestId {
                    ...
                    request_id: workx_app_server_protocol::RequestId::Integer(42),
                },
    ...
    async fn windows_sandbox_non_streaming_exec_uses_execution_path() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: workx_app_server_protocol::RequestId::Integer(99),
        };
    ...
    async fn cancellation_expiration_keeps_process_alive_until_terminated() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: workx_app_server_protocol::RequestId::Integer(100),
        };
        let cwd = AbsolutePathBuf::current_dir().expect("current dir");
        ...
        let workx_app_server_protocol::ClientResponsePayload::OneOffCommandExec(response) =
            *response.result
        else {
    ...
    async fn timeout_or_cancellation_reports_cancellation_without_timeout_exit_code() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: workx_app_server_protocol::RequestId::Integer(101),
        };
    ...
    async fn windows_sandbox_process_ids_reject_write_requests() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: workx_app_server_protocol::RequestId::Integer(1),
        };
    ...
    async fn windows_sandbox_process_ids_reject_terminate_requests() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: workx_app_server_protocol::RequestId::Integer(2),
        };
        let process_id = ConnectionProcessId {
            connection_id: request_id.connection_id,
    ...
    async fn dropped_control_request_is_reported_as_not_running() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: workx_app_server_protocol::RequestId::Integer(3),
        };

...
use workx_app_server_protocol::Config as ApiConfig;
...
use workx_app_server_protocol::OverriddenMetadata;
use workx_app_server_protocol::WriteStatus;
use workx_config::CONFIG_TOML_FILE;
use workx_config::ConfigLayerEntry;
use workx_config::ConfigLayerMetadata;
use workx_config::ConfigLayerSource;
use workx_config::ConfigLayerStack;
use workx_config::ConfigRequirementsToml;
use workx_config::ShellEnvironmentPolicyFilterRepresentation;
use workx_config::config_toml::ConfigToml;
use workx_config::is_structured_feature_path;
use workx_config::merge_toml_values;
use workx_config::shell_environment_filter_entry;
use workx_config::validate_shell_environment_policy_filter_config;
use workx_core::config::deserialize_config_toml_with_base;
use workx_core::config::edit::ConfigEdit;
use workx_core::config::edit::ConfigEditsBuilder;
use workx_core::config::validate_feature_requirements_for_config_toml;
use workx_core::path_utils;
use workx_core::path_utils::SymlinkWritePaths;
use workx_core::path_utils::resolve_symlink_write_paths;
use workx_core::path_utils::write_atomically;
...
impl ConfigManager {
    ...
    async fn apply_edits(
        ...
        for (key_path, value, strategy) in edits.into_iter() {
            ...
            let preserves_network_proxy_settings = value.is_null()
                ...
                && {
                    ...
                    has_broker_settings(&user_config)
                        || !has_broker_settings(&user_layer.config)
                            && has_broker_settings(&layers.effective_config())
                };
            let parsed_value = if preserves_network_proxy_settings {
                Some(TomlValue::Boolean(false))
            } else {
                parse_value(value).map_err(|message| {
                    ConfigManagerError::write(ConfigWriteErrorCode::ConfigValidationError, message)
                })?
            };
                        ...
                        value.is_bool()
                            || matches!(strategy, MergeStrategy::Upsert)
                                && value.as_table().is_some_and(|table| {
                                    table.len() == 1 && table.contains_key("enabled")
                                })
                    });
        ...
        let user_config_toml =
            deserialize_config_toml_with_base(user_config.clone(), self.workx_home()).map_err(
                |err| {
                    ConfigManagerError::write(
                        ConfigWriteErrorCode::ConfigValidationError,
                        format!("Invalid configuration: {err}"),
    ...
    /// include any in-repo .workx/ folders because there is no cwd/project root
...
fn structured_feature_depth(segments: &[String]) -> Option<usize> {
    let depth = match segments {
        [profiles, _, _, _, ..] if profiles == "profiles" => 4,
        [_, _, ..] => 2,
        _ => return None,
    };
...
fn apply_merge(
    ...
    let preserves_structured_feature_config =
        structured_feature_depth(segments).is_some_and(|feature_depth| {
...
fn value_at_semantic_path<'a>(root: &'a TomlValue, segments: &[String]) -> Option<&'a TomlValue> {
    shell_environment_filter_entry(root, segments)
        ...
        .or_else(|| {
            ...
            if structured_feature_depth(parents) != Some(parents.len()) {
                return None;
            }
...
fn override_message(layer: &ConfigLayerSource) -> String {
    match layer {
        ...
        ConfigLayerSource::Project { dot_workx_folder } => format!(
            "Overridden by project config: {}/{CONFIG_TOML_FILE}",
            dot_workx_folder.display(),
        ),

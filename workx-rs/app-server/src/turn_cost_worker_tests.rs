...
use workx_backend_client::ApiKeyResponseCost;
...
async fn worker_starts_with_otlp_metrics_exporter_without_log_exporter() {
    ...
    Mock::given(method("POST"))
        ...
        .mount(&server)
        .await;
    let workx_home = TempDir::new().expect("temporary Workx home");
    ...
    let auth_manager = AuthManager::from_auth_for_testing(WorkxAuth::from_api_key("sk-test"));
...
async fn auth_manager_at(workx_home: &std::path::Path) -> Arc<AuthManager> {
    Arc::new(
        AuthManager::new(
...
async fn turn_cost_failures_do_not_log_response_bodies(status: u16) {
    ...
    let mut runtime = test_runtime(
        &server,
        AuthManager::from_auth_for_testing(WorkxAuth::from_api_key("sk-test")),
    )
    .await;

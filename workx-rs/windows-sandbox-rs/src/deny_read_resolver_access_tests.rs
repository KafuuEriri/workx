...
fn protected_directory_does_not_discard_accessible_deny_matches() {
    #[cfg(windows)]
    if let Some(root) = std::env::var_os("WORKX_TEST_DENY_READ_RESTRICTED_ROOT") {
    ...
    #[cfg(windows)]
    {
        ...
        let workx_home = TempDir::new().expect("restricted workx home");
        let cwd = AbsolutePathBuf::from_absolute_path(temp.path()).expect("workspace");
        let mut env = std::collections::HashMap::from([
            (
                "WORKX_TEST_DENY_READ_RESTRICTED_ROOT".to_string(),

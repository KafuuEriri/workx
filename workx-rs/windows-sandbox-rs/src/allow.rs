...
mod tests {
    ...
    #[test]
    fn denies_workx_and_agents_inside_writable_root() {
        let tmp = TempDir::new().expect("tempdir");
        let command_cwd = tmp.path().join("workspace");
        let workx_dir = command_cwd.join(".workx");
        let agents_dir = command_cwd.join(".agents");
        let _ = fs::create_dir_all(&workx_dir);
        ...
        let expected_deny: HashSet<PathBuf> = [
            dunce::canonicalize(&workx_dir).unwrap(),

...
pub(super) fn ensure_workx_app_runtime_paths_readable(
...
fn runtime_paths(local_app_data: Option<PathBuf>, user_profile: Option<PathBuf>) -> Vec<PathBuf> {
    ...
    if let Some(local_app_data) = local_app_data {
        let workx_root = local_app_data.join("OpenAI").join("Workx");
        runtime_paths.push(workx_root);
    }
    // The managed primary runtime is installed outside the LocalAppData runtime roots.
    if let Some(user_profile) = user_profile {
        runtime_paths.push(user_profile.join(".cache").join("workx-runtimes"));
    }

...
use workx_windows_sandbox::dpapi_protect;
...
pub const SANDBOX_USERS_GROUP: &str = "WorkxSandboxUsers";
const SANDBOX_USERS_GROUP_COMMENT: &str = "Workx sandbox internal group (managed)";
...
fn write_secrets(
    ...
    let secrets_dir = sandbox_secrets_dir(workx_home);
...
pub(super) fn prepare_setup_marker(workx_home: &Path, real_user: &str) -> Result<()> {
    let marker_path = sandbox_dir(workx_home).join("setup_marker.json");

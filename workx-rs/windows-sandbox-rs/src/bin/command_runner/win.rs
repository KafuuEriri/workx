...
use workx_utils_pty::JobObject;
use workx_windows_sandbox::ConsoleMode;
use workx_windows_sandbox::ErrorPayload;
use workx_windows_sandbox::ErrorStage;
use workx_windows_sandbox::ExitPayload;
use workx_windows_sandbox::FramedMessage;
use workx_windows_sandbox::IPC_PROTOCOL_VERSION;
use workx_windows_sandbox::LaunchDesktop;
use workx_windows_sandbox::LocalSid;
use workx_windows_sandbox::Message;
use workx_windows_sandbox::OutputPayload;
use workx_windows_sandbox::OutputStream;
use workx_windows_sandbox::PipeSpawnHandles;
use workx_windows_sandbox::ResizePayload;
use workx_windows_sandbox::SpawnReady;
use workx_windows_sandbox::SpawnRequest;
use workx_windows_sandbox::StderrMode;
use workx_windows_sandbox::StdinMode;
use workx_windows_sandbox::WindowsSandboxTokenMode;
use workx_windows_sandbox::allow_null_device;
use workx_windows_sandbox::create_readonly_token_with_caps_and_user_from;
use workx_windows_sandbox::create_workspace_write_token_with_caps_and_user_from;
use workx_windows_sandbox::decode_bytes;
use workx_windows_sandbox::encode_bytes;
use workx_windows_sandbox::get_current_token_for_restriction;
use workx_windows_sandbox::hide_current_user_profile_dir;
use workx_windows_sandbox::log_note;
use workx_windows_sandbox::read_frame;
use workx_windows_sandbox::read_handle_loop;
use workx_windows_sandbox::spawn_process_with_pipes;
use workx_windows_sandbox::to_wide;
use workx_windows_sandbox::token_mode_for_permission_profile;
use workx_windows_sandbox::write_frame;

// Kept in sync with workx_exec_server::WORKX_FS_HELPER_ARG1 without introducing
// a dependency cycle.
const FS_HELPER_ARG: &str = "--workx-run-as-fs-helper";
const READ_ACL_MUTEX_NAME: &str = "Local\\WorkxSandboxReadAcl";
...
struct IpcSpawnedProcess {
    ...
    conpty_owner: Option<workx_windows_sandbox::ConptyInstance>,
...
fn spawn_ipc_process(req: &SpawnRequest) -> Result<IpcSpawnedProcess> {
    let log_dir = req.workx_home.clone();
    hide_current_user_profile_dir(req.workx_home.as_path());
    ...
    let (pi, job, stdout_handle, stderr_handle, stdin_handle) = if req.tty {
        let (pi, mut conpty) = workx_windows_sandbox::spawn_conpty_process_as_user(
            h_token.raw(),
            &req.command,
            &effective_cwd,
            &req.env,

...
pub const OFFLINE_USERNAME: &str = "WorkxSandboxOffline";
pub const ONLINE_USERNAME: &str = "WorkxSandboxOnline";
const ERROR_CANCELLED: u32 = 1223;
const SECURITY_BUILTIN_DOMAIN_RID: u32 = 0x0000_0020;
const DOMAIN_ALIAS_RID_ADMINS: u32 = 0x0000_0220;
const SETUP_EXE_FILENAME: &str = "workx-windows-sandbox-setup.exe";
...
pub fn sandbox_dir(workx_home: &Path) -> PathBuf {
    workx_home.join(".sandbox")
}

pub fn sandbox_bin_dir(workx_home: &Path) -> PathBuf {
    workx_home.join(".sandbox-bin")
}

pub fn sandbox_secrets_dir(workx_home: &Path) -> PathBuf {
    workx_home.join(".sandbox-secrets")
}

pub fn setup_marker_path(workx_home: &Path) -> PathBuf {
    sandbox_dir(workx_home).join("setup_marker.json")
}

pub fn sandbox_users_path(workx_home: &Path) -> PathBuf {
    sandbox_secrets_dir(workx_home).join("sandbox_users.json")
}
...
pub fn run_setup_refresh_with_extra_read_roots(
    ...
    let deny_read_paths =
        setup_refresh_deny_read_paths(permission_profile, workspace_roots, command_cwd)?;
    let mut read_roots = gather_read_roots(command_cwd, &permissions, env_map, workx_home);
...
fn run_setup_refresh_inner(
    ...
    let payload = ElevationPayload {
        ...
        workx_home: request.workx_home.to_path_buf(),
    ...
    run_setup_singleflight(b64.clone(), || {
        run_setup_refresh_payload(&b64, request.workx_home)
    })
}

fn run_setup_refresh_payload(b64: &str, workx_home: &Path) -> Result<()> {
    let exe = find_setup_exe();
    let sbx_dir = sandbox_dir(workx_home);
    let log_path = current_log_file_path(&sbx_dir);
    let cleared_report = match clear_setup_error_report(workx_home) {
        Ok(()) => true,
        Err(err) => {
            log_note(
    ...
    cmd.arg(b64)
        ...
        .stdout(Stdio::null())
        .stderr(Stdio::null());
    let cwd = std::env::current_dir().unwrap_or_else(|_| workx_home.to_path_buf());
    log_note(
        &format!(
    ...
    if let Err(err) = clear_setup_error_report(workx_home) {
        log_note(
...
fn gather_helper_read_roots(workx_home: &Path) -> Vec<PathBuf> {
    let helper_dir = helper_bin_dir(workx_home);
...
fn gather_full_read_roots_for_permissions(
    ...
    let mut roots = gather_helper_read_roots(workx_home);
    roots.extend(
...
pub(crate) fn effective_write_roots_for_permissions(
    ...
    filter_sensitive_write_roots(write_roots, workx_home)
}
...
struct ElevationPayload {
    ...
    otel: Option<workx_otel::StatsigMetricsSettings>,
    real_user: String,
    mode: SetupMode,
    #[serde(default)]
    refresh_only: bool,
...
const ALLOW_LOCAL_BINDING_ENV_KEY: &str = "WORKX_NETWORK_ALLOW_LOCAL_BINDING";
...
const WINDOWS_SANDBOX_PROXY_PORTS_ENV_KEY: &str = "WORKX_WINDOWS_SANDBOX_PROXY_PORTS";
...
fn report_helper_failure(
    ...
    match read_setup_error_report(workx_home) {
...
fn verify_setup_completed(workx_home: &Path) -> Result<()> {
    if sandbox_setup_is_complete(workx_home) {
        Ok(())
    } else {
        Err(failure(
...
fn run_setup_exe(
    ...
    run_setup_singleflight(payload_b64.clone(), || {
        run_setup_exe_payload(&payload_b64, needs_elevation, workx_home)
    })
}
...
fn run_setup_exe_payload(
    ...
    if !needs_elevation {
        ...
        verify_setup_completed(workx_home)?;
        if let Err(err) = clear_setup_error_report(workx_home) {
            log_note(
                &format!(
...
fn run_elevated_setup_inner(
    ...
    let sbx_dir = sandbox_dir(request.workx_home);
    ...
    let payload = ElevationPayload {
        ...
        allow_local_binding: offline_proxy_settings.allow_local_binding,
        real_user: std::env::var("USERNAME").unwrap_or_else(|_| "Administrators".to_string()),
        otel: workx_otel::global_statsig_metrics_settings(),
        mode: SetupMode::Full,
        refresh_only: false,
    };
    ...
    run_setup_exe(&payload, needs_elevation, request.workx_home)
}
...
pub fn run_elevated_provisioning_setup(
    ...
    if !workx_home.is_absolute()
        || !matches!(
            workx_home.components().next(),
        ...
        )
    {
        return Err(failure(
            SetupErrorCode::OrchestratorSandboxDirCreateFailed,
            format!(
                "sandbox provisioning WORKX_HOME must be an absolute local disk path: {}",
                workx_home.display()
            ),
        ));
    }
    ...
    let payload = ElevationPayload {
        ...
        command_cwd: workx_home.to_path_buf(),
        read_roots: Vec::new(),
        write_roots: Vec::new(),
    ...
    run_setup_exe(&payload, /*needs_elevation*/ false, workx_home)
}
...
pub(crate) fn build_payload_roots(
    ...
    let mut read_roots = if let Some(roots) = overrides.read_roots.as_deref() {
        ...
        let mut read_roots = gather_helper_read_roots(request.workx_home);
...
fn filter_sensitive_write_roots(mut roots: Vec<PathBuf>, workx_home: &Path) -> Vec<PathBuf> {
    // Never grant capability write access to WORKX_HOME or anything under WORKX_HOME/.sandbox,
    // WORKX_HOME/.sandbox-bin, or WORKX_HOME/.sandbox-secrets. These locations contain sandbox
    // control/state and helper binaries and must remain tamper-resistant.
    let workx_home_key = canonical_path_key(workx_home);
    let sbx_dir_key = canonical_path_key(&sandbox_dir(workx_home));
    let sbx_dir_prefix = format!("{}/", sbx_dir_key.trim_end_matches('/'));
    let sbx_bin_dir_key = canonical_path_key(&sandbox_bin_dir(workx_home));
    let sbx_bin_dir_prefix = format!("{}/", sbx_bin_dir_key.trim_end_matches('/'));
    let secrets_dir_key = canonical_path_key(&sandbox_secrets_dir(workx_home));
...
mod tests {
    ...
    use workx_protocol::permissions::project_roots_glob_pattern;
    use workx_utils_absolute_path::AbsolutePathBuf;
    ...
    fn setup_completion_requires_ready_artifacts() {
        let workx_home = TempDir::new().expect("tempdir");
        let err = verify_setup_completed(workx_home.path())
    ...
    fn setup_exe_lookup_checks_package_resource_dir_for_bin_exe() {
        ...
        let exe = bin_dir.join("workx.exe");
        let setup_exe = resources_dir.join("workx-windows-sandbox-setup.exe");
        fs::write(&exe, b"workx").expect("write exe");
    ...
    fn offline_proxy_settings_ignore_proxy_env_when_online_identity_selected() {
        ...
        env.insert(
            "WORKX_NETWORK_ALLOW_LOCAL_BINDING".to_string(),
            "1".to_string(),
        );
    ...
    #[test]
    fn expanded_write_roots_still_drop_protected_workx_home() {
        let tmp = TempDir::new().expect("tempdir");
        let user_profile = tmp.path().join("user-profile");
        let workx_home = user_profile.join("WorkxHome");
        let documents = user_profile.join("Documents");
        fs::create_dir_all(&workx_home).expect("create workx home");
        fs::create_dir_all(&documents).expect("create documents");
        ...
        let roots = super::filter_sensitive_write_roots(roots, &workx_home);
    ...
    fn gather_read_roots_includes_helper_bin_dir() {
        ...
        let roots = gather_read_roots(&command_cwd, &permissions, &HashMap::new(), &workx_home);
        let expected =
            dunce::canonicalize(helper_bin_dir(&workx_home)).expect("canonical helper dir");
    ...
    fn effective_write_roots_match_payload_filtering_for_overrides() {
        ...
        let sandbox_root = super::sandbox_dir(&workx_home);
        fs::create_dir_all(&workx_home).expect("create workx home");
        fs::create_dir_all(&command_cwd).expect("create workspace");
        ...
        let forbidden_workx_home = dunce::canonicalize(&workx_home).expect("canonical workx home");
        ...
        assert!(!effective_write_roots.contains(&forbidden_workx_home));
    ...
    fn payload_deny_write_paths_merge_explicit_and_protected_children() {
        ...
        assert_eq!(
            [
                ...
                dunce::canonicalize(&extra_workx).expect("canonical extra .workx"),
                explicit_deny,
            ]
            .into_iter()

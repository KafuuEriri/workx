//! Internal `workx.exe --run-as-windows-sandbox` wrapper.
//!
...
pub const WORKX_WINDOWS_SANDBOX_ARG1: &str = "--run-as-windows-sandbox";

const COMMAND_CWD_FLAG: &str = "--command-cwd";
const WORKX_HOME_FLAG: &str = "--workx-home";
...
pub fn create_windows_sandbox_command_args_for_permission_profile(
    ...
    let mut args = vec![
        WORKX_WINDOWS_SANDBOX_ARG1.to_string(),
        WORKX_HOME_FLAG.to_string(),
        workx_home.to_string_lossy().into_owned(),
...
async fn run_windows_sandbox_wrapper_request(request: WindowsSandboxWrapperRequest) -> Result<i32> {
    ...
    let spawned =
        crate::spawn_windows_sandbox_session_for_level(crate::WindowsSandboxSessionRequest {
            ...
            workx_home: request.workx_home.as_path(),
...
fn parse_windows_sandbox_wrapper_args(args: Vec<String>) -> Result<WindowsSandboxWrapperRequest> {
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            WORKX_HOME_FLAG => workx_home = Some(PathBuf::from(next_flag_value(&mut args, &arg)?)),
            COMMAND_CWD_FLAG => {
    ...
    let workx_home = workx_home.ok_or_else(|| anyhow!("missing required {WORKX_HOME_FLAG}"))?;
    if !workx_home.is_absolute() {
        bail!(
            "{WORKX_HOME_FLAG} must be absolute: {}",
            workx_home.display()
        );
    }

...
use super::WORKX_HOME_FLAG;
use super::WORKX_WINDOWS_SANDBOX_ARG1;
...
fn windows_wrapper_args_round_trip() {
    ...
    let args = create_windows_sandbox_command_args_for_permission_profile(
        vec![
            "workx.exe".to_string(),
            "--workx-run-as-fs-helper".to_string(),
        ],
        &command_cwd,
        ...
        Path::new(r"C:\Users\me\.workx"),
    );

    assert_eq!(args[0], WORKX_WINDOWS_SANDBOX_ARG1);
    assert!(args.contains(&WORKX_HOME_FLAG.to_string()));
    ...
    assert_eq!(
        parsed.command,
        vec!["workx.exe", "--workx-run-as-fs-helper"]
    );

...
pub fn current_log_file_path_for_workx_home(workx_home: &Path) -> PathBuf {
    current_log_file_path(&crate::sandbox_dir(workx_home))
}
...
mod tests {
    ...
    #[test]
    fn current_log_file_path_for_workx_home_uses_sandbox_dir() {
        let workx_home = Path::new("workx-home");

        assert_eq!(
            current_log_file_path_for_workx_home(workx_home),
            current_log_file_path(&workx_home.join(".sandbox"))
        );
    }
}

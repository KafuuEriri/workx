...
# Run a suite of smoke tests against the Windows sandbox via the Workx CLI
...
def _resolve_workx_cmd() -> List[str]:
    """Resolve the Workx CLI to invoke `workx sandbox windows`.

    Prefer local builds (debug first), then fall back to PATH.
    Returns the argv prefix to run Workx.
    """
    ...
    candidates = [
        ws_root / "target" / "debug" / "workx.exe",
        ws_root / "target" / "release" / "workx.exe",
    ]
    if cargo_target:
        cargo_base = Path(cargo_target)
        candidates.extend(
            [
                cargo_base / "debug" / "workx.exe",
                cargo_base / "release" / "workx.exe",
            ]
        )
    ...
    if shutil.which("workx"):
        return ["workx"]

    raise FileNotFoundError(
        "Workx CLI not found. Build it first, e.g.\n"
        "  cargo build -p workx-cli --release\n"
        "or for debug:\n"
        "  cargo build -p workx-cli\n"
    )


WORKX_CMD = _resolve_workx_cmd()
print(WORKX_CMD)
TIMEOUT_SEC = 20

WS_ROOT = Path(os.environ["USERPROFILE"]) / "sbx_ws_tests"
OUTSIDE = (
    Path(os.environ["USERPROFILE"]) / "sbx_ws_outside"
)  # outside CWD for deny checks
EXTRA_ROOT = (
    Path(os.environ["USERPROFILE"]) / "WorkspaceRoot"
...
def run_sbx(
    ...
    # Map policy to workx CLI overrides.
...
def summarize(results: List[CaseResult]) -> int:
    ...
    for r in results:
        print(
            ...
            + (f" :: {r.detail.strip()}" if r.detail and not r.ok else "")
        )
    print("=" * 72)
...
def main() -> int:
    ...
    rc, out, err = run_sbx(
        "read-only", ["cmd", "/c", "echo nope > ro_should_fail.txt"], WS_ROOT
    )
    add(
        "RO: write in CWD denied",
        rc != 0 and assert_not_exists(target),
        f"rc={rc}, err={err}",
    )
    ...
    remove_if_exists(target)
    rc, out, err = run_sbx(
        "workspace-write", ["cmd", "/c", "echo ok > ws_ok.txt"], WS_ROOT
    )
    add(
        "WS: write in CWD allowed",
        rc == 0 and assert_exists(target),
        f"rc={rc}, err={err}",
    )
    ...
    rc, out, err = run_sbx(
        "workspace-write", ["cmd", "/c", f"echo nope > {outside_file}"], WS_ROOT
    )
    add(
        "WS: write outside workspace denied",
        rc != 0 and assert_not_exists(outside_file),
        f"rc={rc}",
    )
    ...
    add(
        "WS: write in additional root allowed",
        rc == 0 and assert_exists(extra_target),
        f"rc={rc}, err={err}",
    )
    ...
    rc, out, err = run_sbx(
        "read-only", ["cmd", "/c", "echo tempno > %TEMP%\\ro_temp_fail.txt"], WS_ROOT
    )
    if ro_temp_denied:
    ...
    rc, out, err = run_sbx(
        "workspace-write", ["cmd", "/c", "echo line2 >> append.txt"], WS_ROOT
    )
    add(
        "WS: append allowed",
        rc == 0 and target.read_text().strip().endswith("line2"),
        f"rc={rc}",
    )
    ...
    write_file(target, "line1\n")
    rc, out, err = run_sbx(
        "read-only", ["cmd", "/c", "echo line2 >> ro_append.txt"], WS_ROOT
    )
    ...
    add(
        "RO: PowerShell Set-Content denied",
        rc != 0 and assert_not_exists(target),
        f"rc={rc}",
    )
    ...
    add(
        "WS: mkdir+write allowed",
        rc == 0 and (WS_ROOT / "sub/in_sub.txt").exists(),
        f"rc={rc}",
    )

    # 11. WS: rename (EXPECTED SUCCESS on this host)
    rc, out, err = run_sbx(
        "workspace-write", ["cmd", "/c", "echo x > r.txt & ren r.txt r2.txt"], WS_ROOT
    )
    add(
        "WS: rename succeeds (expected on this host)",
        rc == 0 and (WS_ROOT / "r2.txt").exists(),
        f"rc={rc}, err={err}",
    )
    ...
    write_file(target, "x")
    rc, out, err = run_sbx(
        "workspace-write", ["cmd", "/c", "del /q delme.txt"], WS_ROOT
    )
    add(
        "WS: delete succeeds (expected on this host)",
        rc == 0 and not target.exists(),
        f"rc={rc}, err={err}",
    )
    ...
    rc, out, err = run_sbx(
        "read-only",
        ["python", "-c", "open('py_should_fail.txt','w').write('x')"],
        WS_ROOT,
    )
    add(
        "RO: python file write denied",
        rc != 0 and assert_not_exists(pyfile),
        f"rc={rc}",
    )
    ...
    remove_if_exists(pyfile)
    rc, out, err = run_sbx(
        "workspace-write", ["python", "-c", "open('py_ok.txt','w').write('x')"], WS_ROOT
    )
    add(
        "WS: python file write allowed",
        rc == 0 and assert_exists(pyfile),
        f"rc={rc}, err={err}",
    )
    ...
    rc, out, err = run_sbx(
        "workspace-write",
        ["curl", "--connect-timeout", "1", "--max-time", "2", "https://example.com"],
        WS_ROOT,
    )
    ...
    rc, out, err = run_sbx(
        ...
        [
            ...
            "-NoProfile",
            "-Command",
            "try { iwr http://neverssl.com -TimeoutSec 2 } catch { exit 1 }",
        ],
        WS_ROOT,
    )
    ...
    if have("curl"):
        with start_loopback_proxy_fixture() as (target_port, proxy_port):
            ...
            rc_direct, _out_direct, err_direct = run_sbx(
                ...
                direct_cmd,
                WS_ROOT,
                env_extra={"WORKX_HOME": str(proxy_home)},
            )
            add(
                "WS: direct loopback blocked",
                rc_direct != 0,
                f"rc={rc_direct}, err={err_direct}",
            )
    else:
        add(
            "WS: direct/proxy loopback tests (curl missing)", True, "curl not installed"
        )
    ...
    if have("curl"):
        rc, out, err = run_sbx(
            "workspace-write", ["cmd", "/c", "curl --version"], WS_ROOT
        )
    ...
    if have("rg"):
        rc, out, err = run_sbx(
            "workspace-write", ["cmd", "/c", "rg --version"], WS_ROOT
        )
    ...
    add(
        "WS: PS bytes write allowed",
        rc == 0 and (WS_ROOT / "bytes_ok.bin").exists(),
        f"rc={rc}",
    )
    ...
    add(
        "RO: PS bytes write denied",
        rc != 0 and not (WS_ROOT / "bytes_fail.bin").exists(),
        f"rc={rc}",
    )
    ...
    add(
        "WS: deep mkdir+write allowed",
        rc == 0 and (WS_ROOT / "deep/nest/f.txt").exists(),
        f"rc={rc}",
    )
    ...
    add(
        "WS: move succeeds (expected on this host)",
        rc == 0 and (WS_ROOT / "m2.txt").exists(),
        f"rc={rc}, err={err}",
    )
    ...
    remove_if_exists(target)
    rc, out, err = run_sbx(
        "read-only", ["cmd", "/c", "echo nope > cmd_ro.txt"], WS_ROOT
    )
    ...
    if make_junction(poison_cwd, OUTSIDE):
        ...
        remove_if_exists(target)
        rc, out, err = run_sbx(
            "workspace-write", ["cmd", "/c", "echo poison > poisoned.txt"], poison_cwd
        )
        add(
            "WS: junction poisoning via CWD denied",
            rc != 0 and assert_not_exists(target),
            f"rc={rc}, err={err}",
        )
    else:
        add(
            "WS: junction poisoning via CWD denied (setup skipped)",
            True,
    ...
    if make_junction(sys_link, sys_target):
        rc, out, err = run_sbx(
            "workspace-write",
            ["cmd", "/c", "echo bad > sys_link\\system32\\sbx_junc.txt"],
            WS_ROOT,
        )
        add(
            "WS: junction into Windows denied",
            rc != 0 and not sys_file.exists(),
            f"rc={rc}, err={err}",
        )
    else:
        add(
            "WS: junction into Windows denied (setup skipped)",
            True,
            "junction creation failed",
        )
    ...
    rc, out, err = run_sbx(
        "workspace-write", ["cmd", "/c", "type \\\\.\\PhysicalDrive0"], WS_ROOT
    )
    add("WS: raw device access denied", rc != 0, f"rc={rc}")

    rc, out, err = run_sbx(
        "workspace-write",
        ["cmd", "/c", "echo hi > \\\\.\\pipe\\workx_testpipe"],
        WS_ROOT,
    )
    ...
    rc, out, err = run_sbx(
        "workspace-write", ["cmd", "/c", "echo secret > ads_base.txt:stream"], WS_ROOT
    )
    ...
    rc, out, err = run_sbx(
        "workspace-write",
        ["cmd", "/c", "echo long > \\\\?\\C:\\sbx_longpath_test.txt"],
        WS_ROOT,
    )
    ...
    rc, out, err = run_sbx(
        "workspace-write", ["cmd", "/c", "echo hack > .GiT\\config"], WS_ROOT
    )
    add(
        "WS: protected path case-variation denied",
        rc != 0 and assert_not_exists(git_variation),
        f"rc={rc}",
    )

    # 34. WS: policy tamper (.workx artifacts) denied
    workx_home = Path(os.environ["USERPROFILE"]) / ".workx"
    cap_sid_target = workx_home / "cap_sid"
    rc, out, err = run_sbx(
        "workspace-write",
    ...
    rc2, out2, err2 = run_sbx(
        "workspace-write", ["cmd", "/c", "echo tamper > .workx\\policy.json"], WS_ROOT
    )
    add("WS: .workx cap_sid tamper denied", rc != 0, f"rc={rc}, err={err}")
    add("WS: .workx policy tamper denied", rc2 != 0, f"rc={rc2}, err={err2}")
    ...
    if ssh_path:
        ...
        rc, out, err = run_sbx(
            "workspace-write", ["cmd", "/c", "ssh"], WS_ROOT, env_extra=env
        )
    ...
    subprocess.Popen(
        toggle, cwd=str(race_root), stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
    )
    rc, out, err = run_sbx(
        "workspace-write", ["cmd", "/c", "echo race > flip\\race.txt"], race_root
    )
    add(
        "WS: symlink race write denied (best-effort)",
        rc != 0 and not (outside / "race.txt").exists(),
        f"rc={rc}",
    )
    ...
    rc, out, err = run_sbx(
        "workspace-write",
        ["cmd", "/c", "echo probe > deep\\redir\\system32\\audit_gap.txt"],
        WS_ROOT,
    )
    add(
        "WS: deep junction/world-writable escape denied", rc != 0, f"rc={rc}, err={err}"
    )
    ...
    if make_symlink(fake_root, Path("C:/")):
        rc, out, err = run_sbx(
            "workspace-write", ["cmd", "/c", "echo owned > workx_escape.txt"], fake_root
        )
        add("WS: workspace-root symlink poisoning denied", rc != 0, f"rc={rc}")
    else:
        add(
            "WS: workspace-root symlink poisoning denied (setup skipped)",
            True,
    ...
    if make_symlink(unc_link, other_to):
        rc, out, err = run_sbx(
            "workspace-write",
            ["cmd", "/c", "echo unc > unc_link\\unc_test.txt"],
            WS_ROOT,
        )
        add("WS: UNC link escape denied", rc != 0, f"rc={rc}")
    else:
        add(
            "WS: UNC link escape denied (setup skipped)",
            True,
            "symlink creation failed",
        )
    ...
    if make_symlink(other_drive, other_target):
        rc, out, err = run_sbx(
            "workspace-write",
            ["cmd", "/c", "echo drive > other_drive\\drive.txt"],
            WS_ROOT,
        )
        add("WS: other-drive link escape denied", rc != 0, f"rc={rc}")
    else:
        add(
            "WS: other-drive link escape denied (setup skipped)",
            True,
            "symlink creation failed",
        )
    ...
    rc, out, err = run_sbx(
        "workspace-write",
        ["cmd", "/c", f"echo leak > {outside_after_timeout}"],
        WS_ROOT,
    )
    add(
        "WS: post-timeout outside write still denied",
        rc != 0 and assert_not_exists(outside_after_timeout),
        f"rc={rc}",
    )
    ...
    rc, out, err = run_sbx(
        ...
        [
            ...
            "-NoProfile",
            "-Command",
            "Start-Process 'https://workx-invalid.local/smoke'",
        ],
        WS_ROOT,
    )
    add(

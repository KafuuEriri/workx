...
use workx_otel_trace_websocket::TraceWebSocket;
...
struct Cli {
    ...
    #[arg(
        long,
        value_name = "URL",
        default_value = workx_code_mode_host::DEFAULT_LISTEN_URL
    )]
    listen: String,
...
async fn main() -> anyhow::Result<()> {
    ...
    if let Some(trace_transport) = trace_transport.as_ref() {
        ...
        tracing::info!("workx-code-mode-host OTEL trace websocket listening on ws://{listen_addr}");
    }
    tracing::info_span!(
    ...
    let main_transport = workx_code_mode_host::run_main(&cli.listen);
...
fn build_trace_provider(endpoint: &str) -> anyhow::Result<OtelProvider> {
    OtelProvider::try_new(&OtelSettings {
        ...
        service_name: "workx-code-mode-host".to_string(),
        service_version: env!("CARGO_PKG_VERSION").to_string(),
        workx_home: PathBuf::from("/tmp"),

...
use workx_code_mode_protocol::grpc::code_mode_host_server::CodeModeHost;
...
impl CodeModeHost for GrpcCodeModeHost {
        ...
        Self: 'async_trait,
    {
        ...
        if let Some(trace) = trace.as_ref() {
            workx_otel::set_parent_from_w3c_trace_context(&open_session_span, trace);
        }
        Box::pin(
        ...
        Self: 'async_trait,
    {
        ...
        if let Some(trace) = trace.as_ref() {
            workx_otel::set_parent_from_w3c_trace_context(&execute_span, trace);
        }
        let callback_traceparent =
            workx_otel::span_w3c_trace_context(&execute_span).and_then(|trace| trace.traceparent);
        Box::pin(

...
use serde_json::json;
use workx_app_server_protocol::CollabAgentState as ApiCollabAgentState;
use workx_app_server_protocol::CollabAgentStatus as ApiCollabAgentStatus;
use workx_app_server_protocol::CollabAgentTool;
use workx_app_server_protocol::CollabAgentToolCallStatus as ApiCollabAgentToolCallStatus;
use workx_app_server_protocol::CommandAction;
use workx_app_server_protocol::CommandExecutionSource;
use workx_app_server_protocol::CommandExecutionStatus as ApiCommandExecutionStatus;
use workx_app_server_protocol::ErrorNotification;
use workx_app_server_protocol::FileUpdateChange as ApiFileUpdateChange;
use workx_app_server_protocol::ItemCompletedNotification;
use workx_app_server_protocol::ItemStartedNotification;
use workx_app_server_protocol::McpToolCallError;
use workx_app_server_protocol::McpToolCallResult;
use workx_app_server_protocol::McpToolCallStatus as ApiMcpToolCallStatus;
use workx_app_server_protocol::PatchApplyStatus as ApiPatchApplyStatus;
use workx_app_server_protocol::PatchChangeKind as ApiPatchChangeKind;
...
use workx_app_server_protocol::WebSearchAction as ApiWebSearchAction;
use workx_app_server_protocol::WebSearchItem as ApiWebSearchItem;
...
use workx_exec::AgentMessageItem;
use workx_exec::CollabAgentState;
use workx_exec::CollabAgentStatus;
use workx_exec::CollabTool;
use workx_exec::CollabToolCallItem;
use workx_exec::CollabToolCallStatus;
use workx_exec::CollectedThreadEvents;
use workx_exec::CommandExecutionItem;
use workx_exec::CommandExecutionStatus;
use workx_exec::ErrorItem;
use workx_exec::EventProcessorWithJsonOutput;
use workx_exec::ExecThreadItem;
use workx_exec::FileChangeItem;
use workx_exec::FileUpdateChange as ExecFileUpdateChange;
use workx_exec::ItemCompletedEvent;
use workx_exec::ItemStartedEvent;
use workx_exec::ItemUpdatedEvent;
use workx_exec::McpToolCallItem;
use workx_exec::McpToolCallItemError;
use workx_exec::McpToolCallItemResult;
use workx_exec::McpToolCallStatus;
use workx_exec::PatchApplyStatus;
use workx_exec::PatchChangeKind;
use workx_exec::ReasoningItem;
use workx_exec::ThreadErrorEvent;
use workx_exec::ThreadEvent;
use workx_exec::ThreadItemDetails;
use workx_exec::ThreadStartedEvent;
use workx_exec::TodoItem;
use workx_exec::TodoListItem;
use workx_exec::TurnCompletedEvent;
use workx_exec::TurnFailedEvent;
use workx_exec::TurnStartedEvent;
use workx_exec::Usage;
use workx_exec::WebSearchItem;
use workx_exec::WorkxStatus;
...
fn session_configured_produces_thread_started_event() {
    ...
    let session_configured = SessionConfiguredEvent {
        ...
        approvals_reviewer: workx_protocol::config_types::ApprovalsReviewer::User,
...
fn plan_update_emits_started_then_updated_then_completed() {
    ...
    assert_eq!(
        ...
        CollectedThreadEvents {
            ...
            status: WorkxStatus::InitiateShutdown,
        }
    );
}
...
fn token_usage_update_is_emitted_on_turn_completion() {
    ...
    let usage_update =
        processor.collect_thread_events(ServerNotification::ThreadTokenUsageUpdated(
            workx_app_server_protocol::ThreadTokenUsageUpdatedNotification {
                thread_id: "thread-1".to_string(),
                turn_id: "turn-1".to_string(),
...
fn failed_turn_clears_stale_final_message() {
    ...
    assert_eq!(collected.status, WorkxStatus::Running);
    ...
    assert_eq!(collected.status, WorkxStatus::InitiateShutdown);
...
fn model_reroute_surfaces_as_error_item() {
    ...
    let collected = processor.collect_thread_events(ServerNotification::ModelRerouted(
        workx_app_server_protocol::ModelReroutedNotification {
            thread_id: "thread-1".to_string(),
            turn_id: "turn-1".to_string(),
            from_model: "gpt-5".to_string(),
            to_model: "gpt-5-mini".to_string(),
            reason: workx_app_server_protocol::ModelRerouteReason::HighRiskCyberActivity,
        },
    ));

...
/// While we may add an `apply-patch` subcommand to the `workx` CLI multitool
/// at some point, we must ensure that the smaller `workx-exec` CLI can still
...
fn test_standalone_exec_cli_can_use_apply_patch() -> anyhow::Result<()> {
    ...
    Command::new(workx_utils_cargo_bin::cargo_bin("workx-exec")?)
        .arg(WORKX_CORE_APPLY_PATCH_ARG1)
        .arg(
            r#"*** Begin Patch
...
async fn test_apply_patch_tool() -> anyhow::Result<()> {
    ...
    use core_test_support::test_workx_exec::test_workx_exec;

    skip_if_no_network!(Ok(()));

    let test = test_workx_exec();
...
async fn shutdown_flushes_completed_turn_and_file_diff() -> anyhow::Result<()> {
    ...
    test.cmd_with_server(&server)
        ...
        .env_remove("WORKX_ANALYTICS_EVENTS_CAPTURE_FILE")
        .arg("--skip-git-repo-check")
        .arg("-s")
        .arg("danger-full-access")
        .arg("-c")
    ...
    assert!(
        event_types.iter().any(|event| event == "workx_turn_event"),

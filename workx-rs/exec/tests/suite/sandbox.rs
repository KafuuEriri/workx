...
use workx_core::spawn::StdioPolicy;
...
pub(super) async fn spawn_command_under_sandbox(
    ...
    use workx_core::exec::build_exec_request;
    ...
    let workx_linux_sandbox_exe = Some(
        core_test_support::find_workx_linux_sandbox_exe()
...
#[tokio::test]
async fn sandbox_blocks_first_time_dot_workx_creation() {
    core_test_support::skip_if_sandbox!();
    #[cfg(target_os = "linux")]
    let sandbox_env = match linux_sandbox_test_env().await {
        Some(env) => env,
    ...
    let mut child = spawn_command_under_sandbox(
        vec![
            "bash".to_string(),
            "-lc".to_string(),
            "mkdir -p .workx && echo 'sandbox_mode = \"danger-full-access\"' > .workx/config.toml"
                .to_string(),
        ],
        repo_root.clone(),
        &permission_profile,
    ...
    .await
    .expect("should spawn command creating .workx");

    let status = child.wait().await.expect("should wait for .workx command");
    assert!(
        !status.success(),
        "sandbox unexpectedly allowed first-time .workx creation: {status:?}"
    );
    let dot_workx_metadata = tokio::fs::symlink_metadata(&dot_workx).await;
    if let Ok(metadata) = dot_workx_metadata {
        assert!(
            !metadata.is_dir(),
            "{} should not be creatable as a directory",
            dot_workx.display()
        );
    } else if let Err(err) = &dot_workx_metadata {
        assert_eq!(
            err.kind(),

...
#[command(
    version,
    override_usage = "workx exec [OPTIONS] [PROMPT]\n       workx exec [OPTIONS] <COMMAND> [ARGS]"
)]
pub struct Cli {
    ...
    /// Allow running Workx outside a Git repository.
    ...
    /// Do not load `$WORKX_HOME/config.toml`; auth still uses `WORKX_HOME`.
...
struct ResumeArgsRaw {
    ...
    // so "workx resume --last <prompt>" treats the positional as a prompt, not a session id.

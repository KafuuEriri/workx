...
fn test_tracing_subscriber() -> impl tracing::Subscriber + Send + Sync {
    ...
    let tracer = provider.tracer("workx-exec-tests");
...
fn exec_default_stderr_filter_suppresses_otel_self_diagnostics() {
    ...
    tracing::subscriber::with_default(subscriber, || {
        ...
        tracing::error!(target: "workx_exec_test", "real exec error");
    });
...
fn exec_root_span_can_be_parented_from_trace_context() {
    ...
    let parent = workx_protocol::protocol::W3cTraceContext {
...
fn runtime_warnings_are_filtered_to_the_primary_thread() {
    ...
    let turn_id = "turn-1";
    let outcomes = [
        workx_app_server_protocol::WarningNotification {
            thread_id: None,
...
async fn resume_lookup_model_providers_filters_only_last_lookup() {
    let workx_home = tempdir().expect("create temp workx home");
...
fn turn_items_for_thread_returns_matching_turn_items() {
    let thread = AppServerThread {
        ...
        source: workx_app_server_protocol::SessionSource::Exec,
        ...
        name: None,
        turns: vec![
            workx_app_server_protocol::Turn {
                id: "turn-1".to_string(),
    ...
    let notification =
        ServerNotification::TurnCompleted(workx_app_server_protocol::TurnCompletedNotification {
            ...
            turn: workx_app_server_protocol::Turn {
                id: "turn-1".to_string(),
                items_view: workx_app_server_protocol::TurnItemsView::Summary,
                items: Vec::new(),
                status: workx_app_server_protocol::TurnStatus::Completed,
                error: None,
                started_at: None,
...
async fn thread_start_params_include_review_policy_when_auto_review_is_enabled() {
    ...
    assert_eq!(
        params.approvals_reviewer,
        Some(workx_app_server_protocol::ApprovalsReviewer::AutoReview)
    );
}
...
async fn thread_start_params_match_history_to_persistence() {
    ...
    assert_eq!(
        ...
        Some(workx_app_server_protocol::ThreadSource::User)
    );
...
async fn session_configured_from_thread_response_preserves_thread_source() {
    ...
    assert_eq!(
        event.thread_source,
        Some(workx_protocol::protocol::ThreadSource::User)
    );
}
...
fn sample_thread_start_response() -> ThreadStartResponse {
    ThreadStartResponse {
        ...
        approval_policy: workx_app_server_protocol::AskForApproval::OnRequest,
        approvals_reviewer: workx_app_server_protocol::ApprovalsReviewer::AutoReview,
        sandbox: workx_app_server_protocol::SandboxPolicy::WorkspaceWrite {
            writable_roots: vec![],

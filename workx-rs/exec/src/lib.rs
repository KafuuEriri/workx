...
pub use event_processor_with_jsonl_output::WorkxStatus;
...
use uuid::Uuid;
use workx_app_server_client::DEFAULT_IN_PROCESS_CHANNEL_CAPACITY;
use workx_app_server_client::EnvironmentManager;
use workx_app_server_client::ExecServerRuntimePaths;
use workx_app_server_client::InProcessAppServerClient;
use workx_app_server_client::InProcessClientStartArgs;
use workx_app_server_client::InProcessServerEvent;
...
use workx_app_server_protocol::Thread as AppServerThread;
...
use workx_app_server_protocol::ThreadItem as AppServerThreadItem;
...
use workx_config::format_config_error_with_source;
use workx_core::StateDbHandle;
use workx_core::check_execpolicy_for_warnings;
use workx_core::config::Config;
use workx_core::config::ConfigBuilder;
use workx_core::config::ConfigOverrides;
use workx_core::config::ConfigTomlLoadResult;
...
use workx_core::config::resolve_oss_provider;
use workx_core::config::resolve_profile_v2_config_path;
use workx_core::find_thread_meta_by_name_str;
use workx_core::format_exec_policy_error_with_source;
use workx_core::path_utils;
use workx_core::read_session_meta_line;
...
use workx_login::default_client::set_default_originator;
use workx_login::enforce_login_restrictions;
...
use workx_otel::set_parent_from_context;
use workx_otel::traceparent_context_from_env;
...
use workx_utils_absolute_path::canonicalize_existing_preserving_symlinks;
use workx_utils_cli::SharedCliOptions;
use workx_utils_oss::ensure_oss_provider_ready;
use workx_utils_oss::get_default_model_for_oss_provider;
...
enum StdinPromptBehavior {
    ...
    /// `workx exec` behavior for `workx exec` with piped input.
    RequiredIfPiped,
    /// Always treat stdin as the prompt, used for the explicit `workx exec -`
...
struct ExecRunArgs {
    ...
    config: Config,
    resume_approvals_reviewer_override: Option<workx_app_server_protocol::ApprovalsReviewer>,
...
pub async fn run_main(cli: Cli, arg0_paths: Arg0DispatchPaths) -> anyhow::Result<()> {
    if let Err(err) = set_default_originator("workx_exec".to_string()) {
        tracing::warn!(?err, "Failed to set workx exec originator override {err:?}");
    }
    ...
    #[allow(clippy::print_stderr)]
    let workx_home = match find_workx_home() {
        Ok(workx_home) => workx_home,
        Err(err) => {
            eprintln!("Error finding workx home: {err}");
            std::process::exit(1);
        }
    };
    let user_config_path = config_profile_v2
        .as_ref()
        .map(|profile_v2| resolve_profile_v2_config_path(&workx_home, profile_v2));
    ...
    let otel = match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        workx_core::otel_init::build_provider(
            &config,
    ...
    workx_core::otel_init::record_process_start(otel.as_ref(), "workx_exec");
    workx_core::otel_init::install_sqlite_telemetry(otel.as_ref(), "workx_exec");
    ...
    let local_runtime_paths = ExecServerRuntimePaths::from_optional_paths(
        arg0_paths.workx_self_exe.clone(),
        arg0_paths.workx_linux_sandbox_exe.clone(),
    )?;
    let state_db = workx_core::init_state_db(&config).await;
    let environment_manager = if run_loader_overrides.ignore_user_config {
        ...
        EnvironmentManager::from_workx_home(
    ...
    let in_process_start_args = InProcessClientStartArgs {
        ...
        client_name: "workx_exec".to_string(),
...
async fn load_bootstrap_config_or_exit(
    ...
    cli_kv_overrides: Vec<(String, workx_config::TomlValue)>,
...
async fn run_exec_session(args: ExecRunArgs) -> anyhow::Result<()> {
    ...
    let (initial_operation, prompt_summary) = match (command.as_ref(), prompt, images) {
        (Some(ExecCommand::Review(review_cli)), _, _) => {
            ...
            let summary = workx_core::review_prompts::user_facing_hint(&review_request.target);
    ...
    // Print the effective configuration and initial request so users can see what Workx
    // is using.
    event_processor.print_config_summary(&config, &prompt_summary, &session_configured);
    if !json_mode
        && let Some(message) =
            workx_core::config::system_bwrap_warning(config.permissions.permission_profile())
    {
        event_processor.process_warning(message);
    }

    info!("Workx initialized with event: {session_configured:?}");
        ...
        match server_event {
            ...
            InProcessServerEvent::ServerNotification(notification) => {
                ...
                if let ServerNotification::Error(payload) = &notification {
                    ...
                    && matches!(
                        payload.turn.status,
                        workx_app_server_protocol::TurnStatus::Failed
                            | workx_app_server_protocol::TurnStatus::Interrupted
                    )
                {
                    error_seen = true;
                }
...
fn thread_resume_params_from_config(
    config: &Config,
    thread_id: String,
    approvals_reviewer_override: Option<workx_app_server_protocol::ApprovalsReviewer>,
) -> ThreadResumeParams {
    let permissions = permissions_selection_from_config(config);
    let sandbox = permissions.is_none().then(|| {
        sandbox_mode_from_permission_profile(
...
fn session_configured_from_thread_response(
    ...
    thread_source: Option<workx_protocol::protocol::ThreadSource>,
    ...
    permission_profile: PermissionProfile,
    active_permission_profile: Option<workx_protocol::models::ActivePermissionProfile>,
    cwd: AbsolutePathBuf,
...
fn should_backfill_turn_completed_items(
    ...
    !thread_ephemeral && payload.turn.items_view != workx_app_server_protocol::TurnItemsView::Full
}
...
async fn parse_latest_turn_context_cwd(path: &Path) -> Option<PathBuf> {
    ...
    tokio::task::spawn_blocking(move || {
        let reader = workx_rollout::open_rollout_seekable_reader(&path).ok()?;
        let mut scanner = workx_rollout::ReverseJsonlScanner::new(reader).ok()?;
        while let Some(outcome) = scanner.scan_next::<RolloutLine>().ok()? {
            if let workx_rollout::ScanOutcome::Parsed(RolloutLine {
...
async fn resolve_resume_thread_id(
    ...
    if let Some(state_db) = state_db {
        ...
        if let Some((_, session_meta)) =
            find_thread_meta_by_name_str(&config.workx_home, session_id, Some(state_db.as_ref()))
                .await?

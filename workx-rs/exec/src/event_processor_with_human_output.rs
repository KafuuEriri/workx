...
use workx_protocol::num_format::format_with_separators;
use workx_protocol::protocol::SessionConfiguredEvent;
use workx_utils_sandbox_summary::summarize_permission_profile;

use crate::event_processor::EventProcessor;
use crate::event_processor::WorkxStatus;
    ...
    fn render_item_completed(&mut self, item: ThreadItem) {
        match item {
            ThreadItem::AgentMessage { text, .. } => {
                eprintln!(
                    "{}\n{}",
                    "workx".style(self.italic).style(self.magenta),
                    text
                );
...
impl EventProcessor for EventProcessorWithHumanOutput {
    ...
    fn process_server_notification(&mut self, notification: ServerNotification) -> WorkxStatus {
        match notification {
            ...
            ServerNotification::TurnPlanUpdated(notification) => {
                ...
                for step in notification.plan {
                    match step.status {
                        ...
                        workx_app_server_protocol::TurnPlanStepStatus::InProgress => {
                            eprintln!("  {} {}", "→".style(self.cyan), step.step);
                        }
                        workx_app_server_protocol::TurnPlanStepStatus::Pending => {
                            eprintln!(
                                "  {} {}",
                                "•".style(self.dimmed),
            ...
            ServerNotification::TurnStarted(_) => WorkxStatus::Running,

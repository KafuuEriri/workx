...
async fn command_hook_does_not_expose_configured_noise_auth_token() {
    ...
    let env = HashMap::from([
        ...
        ("WORKX_HOOK_SAFE_ENV".to_string(), "visible".to_string()),
    ]);
    let handler = ConfiguredHandler {
        builtin: false,
        event_name: HookEventName::SessionStart,
    ...
    assert_eq!(result.exit_code, Some(0), "stderr: {}", result.stderr);
    assert!(result.stdout.contains("WORKX_HOOK_SAFE_ENV=visible"));
    assert!(!result.stdout.lines().any(|line| {
        line.split_once('=').is_some_and(|(name, _)| {
            name.eq_ignore_ascii_case(WORKX_EXEC_SERVER_NOISE_AUTH_TOKEN_ENV_VAR)
        })
    }));
    assert_eq!(result.error, None);
}
...
fn build_command_replays_snapshot_before_hook_overrides_and_scrubbing() {
    ...
    let environment = vec![
        (
            OsString::from("WORKX_HOOK_SNAPSHOT"),
            OsString::from("captured"),
        ),
        (
            OsString::from("WORKX_HOOK_OVERRIDE"),
            OsString::from("captured"),
        ),
        (
            OsString::from(WORKX_EXEC_SERVER_NOISE_AUTH_TOKEN_ENV_VAR),
            OsString::from("captured-noise-token"),
        ),
        #[cfg(unix)]
        (
            OsString::from("WORKX_HOOK_NON_UNICODE"),
            non_unicode_value.clone(),
        ),
    ];
    let env = HashMap::from([
        ("WORKX_HOOK_OVERRIDE".to_string(), "configured".to_string()),
        ("WORKX_HOOK_SAFE_ENV".to_string(), "visible".to_string()),
        (
    ...
    assert_eq!(
        configured_environment_value(&command, "WORKX_HOOK_SNAPSHOT"),
        Some(Some(OsString::from("captured")))
    );
    assert_eq!(
        configured_environment_value(&command, "WORKX_HOOK_OVERRIDE"),
        Some(Some(OsString::from("configured")))
    );
    assert_eq!(
        configured_environment_value(&command, "WORKX_HOOK_SAFE_ENV"),
        Some(Some(OsString::from("visible")))
    );
    assert_eq!(
        configured_environment_value(&command, WORKX_EXEC_SERVER_NOISE_AUTH_TOKEN_ENV_VAR),
        None
    );
    #[cfg(unix)]
    assert_eq!(
        configured_environment_value(&command, "WORKX_HOOK_NON_UNICODE"),
...
async fn async_hook_result_survives_runtime_reconfiguration() {
    ...
    environment.push((
        OsString::from("WORKX_HOOK_CAPTURED_ENV"),
        OsString::from("captured"),
    ));
...
while not Path(r"{}").exists():
    time.sleep(0.01)
print(os.environ["WORKX_HOOK_CAPTURED_ENV"])
"#,

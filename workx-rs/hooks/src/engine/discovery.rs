...
use workx_config::version_for_toml;
use workx_plugin::PluginHookSource;
use workx_plugin::is_allowlisted_bundled_cleanup_hook;
...
fn config_toml_source_path(layer: &ConfigLayerEntry) -> AbsolutePathBuf {
    match &layer.name {
        ...
        ConfigLayerSource::Project { dot_workx_folder } => layer
            .hooks_config_folder()
            .unwrap_or_else(|| dot_workx_folder.clone())
...
fn append_matcher_groups(
    ...
    event_name: workx_protocol::protocol::HookEventName,
            ...
            let normalized = match handler {
                HookHandlerConfig::Command {
                    ...
                    let additional_context_limit = if matches!(
                        event_name,
                        workx_protocol::protocol::HookEventName::PreToolUse
                            | workx_protocol::protocol::HookEventName::PostToolUse
                            | workx_protocol::protocol::HookEventName::SessionStart
                            | workx_protocol::protocol::HookEventName::UserPromptSubmit
                            | workx_protocol::protocol::HookEventName::SubagentStart
                    ) {
                        additional_context_limit
                    } else {
...
mod tests {
        ...
        let source = RequirementSource::Composite {
            sources: vec![
                RequirementSource::SystemRequirementsToml {
                    file: test_path_buf("/etc/workx/requirements.toml").abs(),
                },
    ...
    fn hook_metadata_for_config_layer_source_discards_source_details() {
        let config_file = test_path_buf("/tmp/.workx/config.toml").abs();
        let dot_workx_folder = test_path_buf("/tmp/worktree/.workx").abs();

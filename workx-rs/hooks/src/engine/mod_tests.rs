...
use workx_protocol::protocol::HookOutputEntry;
...
use workx_protocol::protocol::HookTrustStatus;
...
fn bundled_cleanup_source(plugin_id: &str, server: &str, event: &str) -> PluginHookSource {
    ...
    PluginHookSource {
        ...
        source_path: plugin_root.join(".workx-plugin/plugin.json"),
...
fn local_hosted_app_cleanup_hooks_require_saved_trust() {
    let mut source = bundled_cleanup_source("browser@openai-curated-remote", "workx_apps", "Stop");
...
fn executor_stop_hook_fixture() -> (
    ...
    let source = ExecutorPluginHookSource {
        ...
        manifest_path: "file:///plugins/computer-use/.workx-plugin/plugin.json"
            .parse()
            .expect("valid plugin manifest URI"),
        source_relative_path: ".workx-plugin/plugin.json#hooks[0]".to_string(),
        hooks: HookEventsToml {
...
async fn mcp_tool_hooks_expand_event_input_and_apply_pre_tool_decisions() {
    ...
    assert_eq!(
        ...
        workx_protocol::protocol::HookExecutionMode::Sync
    );
    assert_eq!(
        *calls.lock().expect("lock MCP calls"),
        vec![HookMcpCall {

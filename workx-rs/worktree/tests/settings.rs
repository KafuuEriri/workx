...
use workx_worktree::WorktreeSettings;
...
fn default_settings_match_desktop_worktree_defaults() {
    ...
    let settings = WorktreeSettings::from_desktop_config(workx_home.path(), /*desktop*/ None)
        .expect("load default worktree settings");

    assert_eq!(
        settings,
        WorktreeSettings {
            root: workx_home.path().join("worktrees"),
...
fn desktop_settings_use_existing_root_and_retention_keys() {
    let workx_home = tempfile::tempdir().expect("create Workx home");
    let custom_root = workx_home.path().join("custom-managed-worktrees");
    ...
    let settings = WorktreeSettings::from_desktop_config(workx_home.path(), Some(&desktop))
...
fn desktop_settings_reject_invalid_root_and_retention_values() {
    ...
    for desktop in invalid_configs {
        assert!(
            WorktreeSettings::from_desktop_config(workx_home.path(), Some(&desktop)).is_err(),

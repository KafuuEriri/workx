...
use workx_worktree::WorktreeManager;
...
impl RepositoryFixture {
    fn new() -> Self {
        ...
        let workx_home = temp_root.join("workx-home");
    ...
    fn manager(&self) -> WorktreeManager {
        let settings =
            WorktreeSettings::from_desktop_config(&self.workx_home, /*desktop*/ None)
...
fn git_output(repository: &Path, args: &[&str]) -> std::process::Output {
    Command::new("git")
        ...
        .args([
            "-c",
            "user.name=Workx Worktree Test",
            "-c",
            "user.email=workx-worktree-test@example.invalid",
            "-c",
...
fn thread_binding_rejects_primary_and_unmanaged_worktrees() {
    ...
    assert!(
        !fixture.repository.join(".git/workx-thread.json").exists(),
...
fn thread_binding_writes_the_exact_desktop_owner_schema_into_git_metadata() {
    ...
    let metadata_path = checkout.join(run_git(
        &checkout,
        &["rev-parse", "--git-path", "workx-thread.json"],
    ));

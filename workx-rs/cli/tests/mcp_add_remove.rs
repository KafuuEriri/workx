...
async fn add_and_remove_server_updates_global_config() -> Result<()> {
    ...
    let mut add_cmd = workx_command(workx_home.path())?;
    add_cmd
    ...
    let mut remove_cmd = workx_command(workx_home.path())?;
    remove_cmd
    ...
    let mut remove_again_cmd = workx_command(workx_home.path())?;
    remove_again_cmd
        .args(["mcp", "remove", "docs"])
        .assert()
        .success()
...
async fn npm_server_names_round_trip_through_cli() -> Result<()> {
    ...
    workx_command(workx_home.path())?
        ...
        .assert()
        .success();
    assert!(load_global_mcp_servers(workx_home.path()).await?.is_empty());
    Ok(())
}
...
async fn add_and_login_discover_oauth_through_configured_http_proxy() -> Result<()> {
    ...
    assert!(
        load_global_mcp_servers(workx_home.path())
            .await?
            .contains_key("oauth")
    );
    ...
    let mut login = workx_command(workx_home.path())?;
    login
        .env("HTTP_PROXY", proxy.uri())
        .env("http_proxy", proxy.uri())
        .env_remove("HTTPS_PROXY")
...
async fn add_streamable_http_without_manual_token() -> Result<()> {
    ...
    assert!(!workx_home.path().join(".credentials.json").exists());
...
async fn add_persists_issuer_bound_callback_before_starting_oauth() -> Result<()> {
    ...
    let concurrent_workx_home = workx_home.path().to_path_buf();
    let concurrent_add = std::sync::Once::new();
    Mock::given(method("GET"))
        .and(path("/.well-known/oauth-authorization-server/mcp"))
        .respond_with(move |_: &wiremock::Request| {
            concurrent_add.call_once(|| {
                workx_command(&concurrent_workx_home)

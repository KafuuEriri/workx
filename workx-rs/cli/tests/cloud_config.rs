...
impl CloudManagedConfigFixture {
    async fn new() -> Result<Option<Self>> {
        ...
        std::fs::write(workx_home.path().join("config.toml"), &user_config)?;

        let bootstrap_config = load_config_toml_with_layer_stack(
            workx_home.path(),
            Some(&AbsolutePathBuf::from_absolute_path(workx_home.path())?),
            Vec::new(),
    ...
    fn command(&self, args: &[&str]) -> Result<Command> {
        let mut command = Command::new(workx_utils_cargo_bin::cargo_bin("workx")?);
        command
            .kill_on_drop(true)
            .current_dir(self.workx_home.path())
            .env("WORKX_HOME", self.workx_home.path())
    ...
    async fn output(&self, args: &[&str]) -> Result<Output> {
        let output = self.command(args)?.output().await?;
            ...
            "workx {} failed with status {}: stdout={}; stderr={}",
            args.join(" "),
            output.status,
    ...
    fn assert_user_config_unchanged(&self) -> Result<()> {
        assert_eq!(
            std::fs::read_to_string(self.workx_home.path().join("config.toml"))?,
            self.user_config
        );
        Ok(())
    }
...
async fn login_and_logout_persist_only_cloud_managed_mcp_oauth_credentials() -> Result<()> {
    ...
    let credentials_path = fixture.workx_home.path().join(".credentials.json");
...
async fn add_and_remove_preserve_cloud_managed_resources() -> Result<()> {
    ...
    let installed_root = marketplace_install_root(fixture.workx_home.path()).join("managed");
    ...
    fixture
        ...
        .await?;
    let local_servers = load_global_mcp_servers(fixture.workx_home.path()).await?;
    ...
    assert!(
        load_global_mcp_servers(fixture.workx_home.path())
            .await?
            .is_empty()
    );
    fixture

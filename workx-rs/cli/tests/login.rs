...
use workx_login::REVOKE_TOKEN_URL_OVERRIDE_ENV_VAR;
...
fn write_file_auth_config(workx_home: &Path) -> Result<()> {
    std::fs::write(
...
fn read_auth_json(workx_home: &Path) -> Result<Value> {
    let auth_json = std::fs::read_to_string(workx_home.join("auth.json"))?;
...
fn login_with_api_key_reads_stdin_and_writes_auth_json() -> Result<()> {
    ...
    write_file_auth_config(workx_home.path())?;
    ...
    let auth = read_auth_json(workx_home.path())?;
...
fn login_status_reports_auth_storage_errors() -> Result<()> {
    let workx_home = TempDir::new()?;
    write_file_auth_config(workx_home.path())?;
    std::fs::write(workx_home.path().join("auth.json"), "{invalid json")?;
...
fn login_status_validates_configured_workload_identity() -> Result<()> {
    let workx_home = TempDir::new()?;
    write_file_auth_config(workx_home.path())?;
    let missing_assertion = workx_home.path().join("missing-identity-token");

    workx_command(workx_home.path())?
        .env_remove(WORKX_ACCESS_TOKEN_ENV_VAR)
...
async fn debug_prompt_input_follows_authenticated_attribution_setting() -> Result<()> {
    ...
    for enabled in [true, false] {
        let output = workx_command(workx_home.path())?
        ...
        assert_eq!(
            prompt.contains("Co-authored-by: Workx <noreply@openai.com>"),
            enabled
        );

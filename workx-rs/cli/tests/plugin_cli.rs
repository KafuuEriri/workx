...
use workx_config::MarketplaceConfigUpdate;
use workx_config::record_user_marketplace;
...
fn workx_command(workx_home: &Path) -> Result<assert_cmd::Command> {
    ...
    cmd.env("HOME", workx_home);
    Ok(cmd)
}

fn workx_command_in(workx_home: &Path, current_dir: &Path) -> Result<assert_cmd::Command> {
    let mut cmd = workx_command(workx_home)?;
...
fn write_plugins_enabled_config(workx_home: &Path) -> Result<()> {
    std::fs::write(
...
fn write_marketplace_source_with_manifest(source: &Path, marketplace_manifest: &str) -> Result<()> {
    ...
    std::fs::create_dir_all(source.join("plugins").join("sample").join(".workx-plugin"))?;
    std::fs::write(
        source
            .join(".agents")
            .join("plugins")
...
fn setup_local_marketplace_with_implicit_system_roots() -> Result<(TempDir, TempDir, TempDir)> {
    let (workx_home, source) = setup_local_marketplace()?;

    let bundled_root = workx_home
        .path()
        .join(".tmp")
    ...
    Ok((workx_home, source, cache_home))
}
...
fn setup_custom_marketplace_under_implicit_system_root() -> Result<(TempDir, std::path::PathBuf)> {
    ...
    Ok((workx_home, custom_root))
}

fn remove_installed_plugin_config(workx_home: &Path, plugin_key: &str) -> Result<()> {
...
async fn marketplace_list_json_includes_configured_git_marketplace_source() -> Result<()> {
    let workx_home = TempDir::new()?;
    let marketplace_root = workx_home
        .path()
        .join(".tmp")
        .join("marketplaces")
        .join("debug");
    ...
    record_user_marketplace(workx_home.path(), "debug", &update)?;
...
async fn marketplace_list_includes_root_when_plugins_are_filtered_out() -> Result<()> {
    let (workx_home, source) = setup_local_marketplace_with_explicit_empty_products()?;
    let expected_row = marketplace_list_row("debug", source.path());
...
async fn marketplace_list_fails_when_configured_marketplace_snapshot_is_missing() -> Result<()> {
    let (workx_home, source) = setup_configured_marketplace_without_manifest()?;
...
async fn marketplace_list_fails_when_configured_marketplace_name_is_invalid() -> Result<()> {
    let workx_home = setup_configured_local_marketplace_with_invalid_name()?;
...
{
    let workx_home = setup_configured_local_marketplace_with_missing_source()?;
...
async fn marketplace_list_fails_when_configured_marketplace_snapshot_is_malformed() -> Result<()> {
    let (workx_home, source) = setup_configured_marketplace_with_malformed_manifest()?;
...
async fn plugin_list_available_requires_json() -> Result<()> {
    let (workx_home, _source) = setup_local_marketplace()?;
...
async fn plugin_list_excludes_unconfigured_repo_local_marketplaces() -> Result<()> {
    let (workx_home, source) = setup_unconfigured_local_marketplace()?;

    workx_command_in(workx_home.path(), source.path())?
...
async fn plugin_list_ignores_implicit_system_marketplace_roots_without_manifests() -> Result<()> {
    let (workx_home, source, cache_home) = setup_local_marketplace_with_implicit_system_roots()?;
...
async fn plugin_list_fails_for_custom_marketplace_under_system_root() -> Result<()> {
    let (workx_home, custom_root) = setup_custom_marketplace_under_implicit_system_root()?;
...
async fn plugin_list_hides_version_for_cached_but_unconfigured_plugin() -> Result<()> {
    ...
    remove_installed_plugin_config(workx_home.path(), "sample@debug")?;
...
async fn plugin_add_json_prints_install_outcome() -> Result<()> {
    ...
    let stdout = assert.get_output().stdout.as_slice();
    let actual: serde_json::Value = serde_json::from_slice(stdout)?;
    let installed_path = workx_home.path().join("plugins/cache/debug/sample/1.2.3");
...
async fn plugin_add_reinstalls_from_configured_marketplace_snapshot() -> Result<()> {
    ...
    assert!(
        workx_home
            .path()
            .join("plugins/cache/debug/sample/1.2.3/.workx-plugin/plugin.json")
            .is_file()
    );
...
impl RemoteMarketplaceFixture {
    ...
    async fn run(&self, args: &[&str]) -> Result<Output> {
        Ok(Command::new(workx_utils_cargo_bin::cargo_bin("workx")?)
            .current_dir(self.home.path())
            .env("WORKX_HOME", self.home.path())
            .env("HOME", self.home.path())
            .env_remove("OPENAI_API_KEY")
            .env_remove("WORKX_API_KEY")
            .env_remove("WORKX_ACCESS_TOKEN")
            .env("WORKX_TEST_ALLOW_HTTP_REMOTE_PLUGIN_BUNDLE_DOWNLOADS", "1")
            .args(args)
            .output()
            .await?)
    }
...
async fn remote_plugin_add_list_and_remove() -> Result<()> {
    ...
    let installed_root = canonicalize_existing_preserving_symlinks(fixture.home.path())?
        ...
        .join("sample")
        .join("1.2.3");
    let manifest = installed_root.join(".workx-plugin/plugin.json");

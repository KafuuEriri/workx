...
async fn configure_http_oauth_server(workx_home: &Path, url: &str) -> Result<()> {
    let mut servers = load_global_mcp_servers(workx_home).await?;
    servers.insert(
        "oauth".to_string(),
...
fn list_shows_empty_state() -> Result<()> {
    ...
    let mut cmd = workx_command(workx_home.path())?;
...
"#,
  ...
  "mcpServers": {
    ...
    let mut list_cmd = workx_command(workx_home.path())?;
    list_cmd
        .env(WORKX_API_KEY_ENV_VAR, "sk-test")
        .args(["mcp", "list", "--json"])
        .assert()
        .success()
        .stdout(contains(r#""name": "api-docs""#));

    let mut get_cmd = workx_command(workx_home.path())?;
    get_cmd
        .env(WORKX_API_KEY_ENV_VAR, "sk-test")
...
async fn list_discovers_local_oauth_server_through_environment_proxy() -> Result<()> {
    let workx_home = TempDir::new()?;
    configure_http_oauth_server(workx_home.path(), "http://mcp-proxy.invalid/mcp").await?;
    std::fs::write(workx_home.path().join("environments.toml"), "invalid = [")?;
    ...
    let mut command = workx_command(workx_home.path())?;
    command
...
async fn list_reports_unknown_auth_status_when_oauth_discovery_is_rate_limited() -> Result<()> {
    ...
    wiremock::Mock::given(wiremock::matchers::method("GET"))
        ...
        .mount(&server)
        .await;
    configure_http_oauth_server(workx_home.path(), &format!("{}/mcp", server.uri())).await?;
...
async fn list_and_get_render_expected_output() -> Result<()> {
    ...
    let mut add = workx_command(workx_home.path())?;
    add.args([
        "mcp",
        "add",
        "docs",
    ...
    let mut servers = load_global_mcp_servers(workx_home.path()).await?;
    ...
    let mut list_json_cmd = workx_command(workx_home.path())?;
    ...
    assert!(stdout.contains("remove: workx mcp remove docs"));

    let mut get_json_cmd = workx_command(workx_home.path())?;
    get_json_cmd
...
fn list_and_get_redact_http_headers_helper() -> Result<()> {
    let workx_home = TempDir::new()?;
    let marker = workx_home.path().join("helper-ran");
    ...
    let list_output = workx_command(workx_home.path())?
        .args(["mcp", "list", "--json"])
        .output()?;
    assert!(list_output.status.success());
    let stdout = String::from_utf8(list_output.stdout)?;
    ...
    for args in [
        ...
        let output = workx_command(workx_home.path())?.args(args).output()?;
        assert!(output.status.success());
        let stdout = String::from_utf8(output.stdout)?;
        assert!(stdout.contains("http_headers_helper"));
        assert!(stdout.contains("<redacted>"));

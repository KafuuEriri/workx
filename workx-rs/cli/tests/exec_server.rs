...
use workx_exec_server::ExecServerClient;
use workx_exec_server::NoiseChannelIdentity;
use workx_exec_server::NoiseChannelPublicKey;
use workx_exec_server::NoiseRendezvousConnectArgs;
...
fn local_exec_server_ignores_invalid_config_without_strict_config() -> Result<()> {
    let workx_home = TempDir::new()?;
    std::fs::write(workx_home.path().join("config.toml"), "not valid toml = [")?;
...
fn local_exec_server_allows_disabled_parent_lifetime_environment_variable() -> Result<()> {
    ...
    let mut cmd = workx_command(workx_home.path())?;
    cmd.env(
        workx_exec_server::WORKX_EXEC_SERVER_EXIT_ON_STDIN_CLOSE_ENV_VAR,
        "false",
    )
...
"#
    ...
    let mut command = tokio::process::Command::new(workx_utils_cargo_bin::cargo_bin("workx")?);
    command
        .env("WORKX_HOME", workx_home.path())
        .env("WORKX_API_KEY", "test-api-key")
        .env(
            workx_exec_server::WORKX_EXEC_SERVER_EXIT_ON_STDIN_CLOSE_ENV_VAR,
            "true",
        )
    ...
    let argv = vec![
        "cmd.exe",
        "/C",
        "if defined WORKX_EXEC_SERVER_EXIT_ON_STDIN_CLOSE (exit /b 1) else ping -n 61 127.0.0.1",
    ];
    #[cfg(not(windows))]
    let argv = vec![
        "/bin/sh",
        "-c",
        "[ -z \"${WORKX_EXEC_SERVER_EXIT_ON_STDIN_CLOSE+present}\" ] && exec /bin/sleep 60",
    ];
    ...
    client
        .exec(ExecParams {
            ...
            shell_snapshot: None,
            env_policy: Some(workx_exec_server::ExecEnvPolicy {
                inherit: workx_protocol::config_types::ShellEnvironmentPolicyInherit::All,
...
"#
    ...
    let workx_bin = workx_utils_cargo_bin::cargo_bin("workx")?;
    let workx_home = workx_home.path().to_path_buf();
    let subprocess = async move {
        let mut command = tokio::process::Command::new(workx_bin);
        command
...
fn local_exec_server_exits_successfully_on_sigterm() -> Result<()> {
    let workx_home = TempDir::new()?;
    let mut child = std::process::Command::new(workx_utils_cargo_bin::cargo_bin("workx")?)
        .env("WORKX_HOME", workx_home.path())

...
fn interactive_validates_config_before_requiring_terminal() -> Result<()> {
    ...
    for &(args, config_file, contents, expected_error) in cases {
        let workx_home = TempDir::new()?;
        std::fs::write(workx_home.path().join(config_file), contents)?;
...
-> Result<()> {
    let workx_home = TempDir::new()?;
    std::fs::write(workx_home.path().join("config.toml"), "")?;
    std::fs::write(
...
"#,
    ...
    let remote_only_cwd = workx_home.path().join("remote-only-working-directory");
...
fn strict_config_is_not_supported_for_cloud_command() -> Result<()> {
    ...
    cmd.args(["--strict-config", "-c", "foo=bar", "cloud", "list"])
        ...
        .failure()
        .stderr(contains(
            "`--strict-config` is not supported for `workx cloud`",
        ));

    ...
    S: tokio::io::AsyncRead + tokio::io::AsyncWrite + Unpin,
{
    ...
    let initialized_response = json!({
        "id": initialize["id"],
        "result": {
            "userAgent": "workx_cli_rs/0.0.0-test",
...
fn queue_rejects_empty_message() -> Result<()> {
    let output = std::process::Command::new(workx_utils_cargo_bin::cargo_bin("workx")?)
...
async fn run_remote_queue_command(response: QueueResponse) -> Result<(Output, Value)> {
    ...
    let server_home = workx_home.path().to_path_buf();
    ...
    let output = tokio::process::Command::new(workx_utils_cargo_bin::cargo_bin("workx")?)
        .env("WORKX_HOME", workx_home.path())
        .env("WORKX_REMOTE_TOKEN", "test-token")
        .args(remote_args)
...
async fn queue_rejects_local_daemon_that_does_not_support_queueing() -> Result<()> {
    let workx_home = tempfile::tempdir_in("/tmp")?;
    let socket_path = workx_app_server::app_server_control_socket_path(workx_home.path())?;
    ...
    assert!(!workx_home.path().join("queue_1.sqlite").exists());
    Ok(())
}

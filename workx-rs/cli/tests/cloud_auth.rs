...
async fn cloud_list_only_allows_trusted_credential_destinations() -> Result<()> {
    ...
    let command = || -> Result<assert_cmd::Command> {
        let mut command = assert_cmd::Command::new(workx_utils_cargo_bin::cargo_bin("workx")?);
        command
            .current_dir(workx_home.path())
            .env("WORKX_HOME", workx_home.path())
            .env_remove("WORKX_ACCESS_TOKEN")
            .env_remove("OPENAI_API_KEY")
            .env_remove("WORKX_CLOUD_TASKS_MODE")
    ...
    let output = command()?
        ...
        .stderr
        .clone();
    insta::assert_snapshot!(String::from_utf8(output)?, @"Error: WORKX_CLOUD_TASKS_BASE_URL must use a trusted HTTPS origin on port 443, without user information, a query, or a fragment; custom backends cannot use saved ChatGPT credentials");
    ...
    command()?
        ...
        .env("WORKX_ACCESS_TOKEN", "at-synthetic-cloud")
        .env("WORKX_AUTHAPI_BASE_URL", auth_server.uri())
        .args(["cloud", "list", "--limit", "1", "--json"])
        .assert()
        .failure()
        .stderr(contains("Not signed in. Please run 'workx login'"));

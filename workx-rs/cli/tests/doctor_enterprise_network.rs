...
async fn invalid_custom_ca_falls_back_to_system_roots() -> Result<()> {
    ...
    let certificate = workx_home.path().join("invalid-ca.pem");
    ...
    for sandbox in [None, Some("seatbelt")] {
        ...
        command
            ...
            .env("WORKX_CA_CERTIFICATE", &certificate)
            .stdin(Stdio::null());
        if let Some(sandbox) = sandbox {
            command
                .env("WORKX_SANDBOX", sandbox)
...
fn doctor_reports_macos_system_proxy_configuration_and_policy() -> Result<()> {
    let workx_home = TempDir::new()?;
    let report = doctor_report(workx_home.path())?;
...
#[cfg(target_os = "macos")]
fn doctor_report(workx_home: &Path) -> Result<Value> {
    let output = Command::new(workx_utils_cargo_bin::cargo_bin("workx")?)
        .args(["doctor", "--json"])

...
use workx_app_server_daemon::BootstrapOptions as AppServerBootstrapOptions;
use workx_app_server_daemon::LifecycleCommand as AppServerLifecycleCommand;
use workx_app_server_daemon::RemoteControlMode as AppServerRemoteControlMode;
use workx_arg0::Arg0DispatchPaths;
use workx_arg0::arg0_dispatch_or_else;
use workx_chatgpt::apply_command::ApplyCommand;
use workx_chatgpt::apply_command::run_apply_command;
use workx_cli::read_access_token_from_stdin;
use workx_cli::read_api_key_from_stdin;
use workx_cli::run_login_status;
use workx_cli::run_login_with_access_token;
use workx_cli::run_login_with_api_key;
use workx_cli::run_login_with_chatgpt;
use workx_cli::run_login_with_device_code;
use workx_cli::run_logout;
use workx_cloud_config::cloud_config_bundle_loader_for_storage;
use workx_cloud_tasks::Cli as CloudTasksCli;
use workx_exec::Cli as ExecCli;
use workx_exec::Command as ExecCommand;
use workx_exec::ReviewArgs;
use workx_execpolicy::ExecPolicyCheckCommand;
use workx_responses_api_proxy::Args as ResponsesApiProxyArgs;
use workx_rollout_trace::REDUCED_STATE_FILE_NAME;
use workx_rollout_trace::replay_bundle;
use workx_state::StateRuntime;
use workx_tui::AppExitInfo;
use workx_tui::Cli as TuiCli;
use workx_tui::ExitReason;
use workx_tui::UpdateAction;
use workx_utils_absolute_path::AbsolutePathBuf;
use workx_utils_cli::CliConfigOverrides;
use workx_utils_cli::ProfileV2Name;
use workx_utils_cli::SharedCliOptions;
...
use workx_core::build_models_manager;
use workx_core::config::Config;
use workx_core::config::ConfigBuilder;
use workx_core::config::ConfigLoadOptions;
...
use workx_core::config::resolve_profile_v2_config_path;
use workx_features::FEATURES;
use workx_features::Stage;
use workx_features::is_known_feature_key;
use workx_home::WorkxHomeUserInstructionsProvider;
use workx_login::AuthManager;
use workx_login::WorkxAuth;
use workx_login::is_workload_identity_selected;
use workx_login::read_workx_access_token_from_env;
...
use workx_terminal_detection::TerminalName;
...
#[clap(
    ...
    // `workx-x86_64-unknown-linux-musl`, but the help output should always use
    // the generic `workx` command name that users run.
    bin_name = "workx",
    override_usage = "workx [OPTIONS] [PROMPT]\n       workx [OPTIONS] <COMMAND> [ARGS]"
)]
struct MultitoolCli {
    #[clap(flatten)]
...
enum Subcommand {
    ...
    /// Manage external MCP servers for Workx.
    Mcp(McpCli),

    /// Manage Workx plugins.
    Plugin(PluginCli),

    /// Start Workx as an MCP server (stdio).
    ...
    /// Update Workx to the latest version.
    Update,

    /// Diagnose local Workx installation, config, auth, and runtime health.
    Doctor(DoctorCommand),

    /// Run commands within a Workx-provided sandbox.
    ...
    /// Apply the latest diff produced by Workx agent as a `git apply` to your local working tree.
    ...
    /// [EXPERIMENTAL] Browse tasks from Workx Cloud and apply changes locally.
...
struct ReviewCommand {
    /// Error out when config.toml contains fields that are not recognized by this version of Workx.
...
#[cfg(target_os = "macos")]
type HostSandboxArgs = workx_cli::SeatbeltCommand;
#[cfg(target_os = "linux")]
type HostSandboxArgs = workx_cli::LandlockCommand;
#[cfg(target_os = "windows")]
type HostSandboxArgs = workx_cli::WindowsCommand;
...
struct UnsupportedSandboxArgs {
    /// Layer $WORKX_HOME/<name>.config.toml on top of the base user config.
...
struct LoginCommand {
    ...
    #[arg(
        long = "with-api-key",
        help = "Read the API key from stdin (e.g. `printenv OPENAI_API_KEY | workx login --with-api-key`)"
    )]
    with_api_key: bool,

    #[arg(
        long = "with-access-token",
        help = "Read the access token from stdin (e.g. `printenv WORKX_ACCESS_TOKEN | workx login --with-access-token`)"
    )]
...
struct AppServerCommand {
    ...
    #[command(flatten)]
    code_mode_host: workx_app_server::AppServerCodeModeHostArgs,
    ...
    #[arg(
        long = "listen",
        value_name = "URL",
        default_value = workx_app_server::AppServerTransport::DEFAULT_LISTEN_URL
    )]
    listen: workx_app_server::AppServerTransport,
    ...
    #[command(flatten)]
    auth: workx_app_server::AppServerWebsocketAuthArgs,
}
...
struct ExecServerCommand {
    ...
    request_dispatch_mode: workx_exec_server::RequestDispatchMode,
    ...
    /// Use Agent Identity auth from WORKX_ACCESS_TOKEN for remote registration.
    #[arg(
    ...
    #[arg(
        ...
        env = workx_exec_server::WORKX_EXEC_SERVER_EXIT_ON_STDIN_CLOSE_ENV_VAR,
...
enum AppServerSubcommand {
    ...
    /// [internal] Generate internal JSON Schema artifacts for Workx tooling.
    #[clap(hide = true)]
...
fn run_update_action(action: UpdateAction) -> anyhow::Result<()> {
    ...
    println!("Updating Workx via `{cmd_str}`...");
    let status = {
        #[cfg(windows)]
        {
    ...
    println!("\n🎉 Update ran successfully! Please restart Workx.");
    Ok(())
}
...
fn run_update_command() -> anyhow::Result<()> {
    ...
    {
        anyhow::bail!(
            "`workx update` is not available in debug builds. Install a release build of Workx to use this command."
        );
    }

    #[cfg(not(debug_assertions))]
    {
        let Some(action) = workx_tui::get_update_action() else {
            anyhow::bail!(
                "Could not detect the Workx installation method. Please update manually: https://developers.openai.com/codex/cli/"
            );
        };
        run_update_action(action)
    }
...
async fn run_session_archive_cli_command(
    action: workx_tui::SessionArchiveAction,
    ...
    workx_tui::run_session_archive_command(
        action,
        target,
        workx_tui::SessionArchiveCommandOptions {
            cli: interactive,
            arg0_paths,
...
fn delete_action(target: &str, force: bool) -> anyhow::Result<workx_tui::SessionArchiveAction> {
    if force && workx_protocol::ThreadId::from_string(target).is_err() {
        anyhow::bail!("--force requires a session UUID; names must be confirmed interactively");
    }
    let confirmation = match force {
        true => workx_tui::DeleteConfirmation::Skip,
        false => workx_tui::DeleteConfirmation::Prompt,
    };
    Ok(workx_tui::SessionArchiveAction::Delete(confirmation))
}

async fn run_debug_app_server_command(cmd: DebugAppServerCommand) -> anyhow::Result<()> {
    match cmd.subcommand {
        DebugAppServerSubcommand::SendMessageV2(cmd) => {
            let workx_bin = std::env::current_exe()?;
            workx_app_server_test_client::send_message_v2(&workx_bin, &[], cmd.user_message, &None)
                .await
        }
    }
}
...
fn main() -> anyhow::Result<()> {
    let remote_control_disabled = workx_app_server::take_remote_control_disabled_env();
...
async fn cli_main(
    ...
    {
        anyhow::bail!("`workx agents` received conflicting remote server endpoints");
    }
    ...
    match subcommand {
        None | Some(Subcommand::Agents(_)) => {
            ...
            if open_agents_overview {
                if interactive.prompt.is_some() || !interactive.images.is_empty() {
                    anyhow::bail!("`workx agents` does not accept an initial prompt or images");
                }
                ...
                {
                    anyhow::bail!(
                        "`workx agents` cannot apply local provider or additional-directory overrides to a remote server"
                    );
                }
                if is_workload_identity_selected() {
                    anyhow::bail!(
                        "`workx agents` is unavailable while workload identity is active"
                    );
                }
                if root_remote.is_none() {
                    ...
                    #[cfg(not(unix))]
                    anyhow::bail!("`workx agents` requires `--remote` on this platform");
                }
        ...
        Some(Subcommand::Exec(mut exec_cli)) => {
            ...
            workx_exec::run_main(exec_cli, arg0_paths.clone()).await?;
        }
        Some(Subcommand::Review(ReviewCommand {
            ...
            let mut exec_cli = ExecCli::try_parse_from(["workx", "exec"])?;
            exec_cli
                .shared
                .inherit_exec_root_options(&interactive.shared);
        ...
        Some(Subcommand::McpServer(McpServerCommand { strict_config })) => {
            eprintln!(
                "warning: `workx mcp-server` is deprecated and will be removed in a future release."
            );
            reject_remote_mode_for_subcommand(
                root_remote.as_deref(),
                root_remote_auth_token_env.as_deref(),
            ...
            match subcommand {
                None => {
                    let transport = if stdio {
                        workx_app_server::AppServerTransport::Stdio
                    } else {
                        listen
                    };
                    let auth = auth.try_into_settings()?;
                    let runtime_options = workx_app_server::AppServerRuntimeOptions {
                        code_mode_host_transport: code_mode_host.into(),
                        remote_control_startup_mode: match (remote_control, remote_control_disabled)
                        {
                            (true, _) => {
                                workx_app_server::RemoteControlStartupMode::EnabledEphemeral
                            }
                            (false, true) => {
                                workx_app_server::RemoteControlStartupMode::DisabledEphemeral
                            }
                            (false, false) => {
                                workx_app_server::RemoteControlStartupMode::ResolvePersisted
                            }
                        },
                        ..Default::default()
                    };
                    workx_app_server::run_main_with_transport_options(
                        arg0_paths.clone(),
                        root_config_overrides,
                        ...
                        analytics_default_enabled,
                        transport,
                        workx_protocol::protocol::SessionSource::VSCode,
                        auth,
                        runtime_options,
                    )
                    .await?;
                ...
                Some(AppServerSubcommand::Daemon(daemon_cli)) => match daemon_cli.subcommand {
                    ...
                    AppServerDaemonSubcommand::Bootstrap(bootstrap_cli) => {
                        let output =
                            workx_app_server_daemon::bootstrap(AppServerBootstrapOptions {
                    ...
                    AppServerDaemonSubcommand::PidUpdateLoop => {
                        ...
                        workx_app_server_daemon::run_pid_update_loop(http_client_factory).await?;
                    }
                },
                Some(AppServerSubcommand::Proxy(proxy_cli)) => {
                    let socket_path = match proxy_cli.socket_path {
                        ...
                        None => {
                            let workx_home = find_workx_home()?;
                            workx_app_server::app_server_control_socket_path(&workx_home)?
                        }
                    };
                    workx_stdio_to_uds::run(socket_path.as_path()).await?;
                }
                Some(AppServerSubcommand::GenerateTs(gen_cli)) => {
                    let options = workx_app_server_protocol::GenerateTsOptions {
                        experimental_api: gen_cli.experimental,
                        ..Default::default()
                    };
                    workx_app_server_protocol::generate_ts_with_options(
                        &gen_cli.out_dir,
                ...
                Some(AppServerSubcommand::GenerateJsonSchema(gen_cli)) => {
                    workx_app_server_protocol::generate_json_with_experimental(
                        &gen_cli.out_dir,
                        gen_cli.experimental,
                    )?;
                }
                Some(AppServerSubcommand::GenerateInternalJsonSchema(gen_cli)) => {
                    workx_app_server_protocol::generate_internal_json_schema(&gen_cli.out_dir)?;
                }
            }
        }
        ...
        Some(Subcommand::Archive(cmd)) => {
            let output = run_session_archive_cli_command(
                workx_tui::SessionArchiveAction::Archive,
                cmd,
                interactive,
                root_config_overrides.clone(),
                root_remote.clone(),
        ...
        Some(Subcommand::Unarchive(cmd)) => {
            let output = run_session_archive_cli_command(
                workx_tui::SessionArchiveAction::Unarchive,
                cmd,
                interactive,
                root_config_overrides.clone(),
                root_remote.clone(),
            ...
            match login_cli.action {
                ...
                None => {
                    if login_cli.with_api_key && login_cli.with_access_token {
                        ...
                        eprintln!(
                            "The --api-key flag is no longer supported. Pipe the key instead, e.g. `printenv OPENAI_API_KEY | workx login --with-api-key`."
                        );
                        std::process::exit(1);
        ...
        Some(Subcommand::Cloud(mut cloud_cli)) => {
            ...
            workx_cloud_tasks::run_main(cloud_cli, arg0_paths.workx_linux_sandbox_exe.clone())
                .await?;
        }
        Some(Subcommand::Sandbox(mut sandbox_cli)) => {
            ...
            #[cfg(target_os = "macos")]
            workx_cli::run_command_under_seatbelt(
                sandbox_cli,
                arg0_paths.workx_linux_sandbox_exe.clone(),
                loader_overrides,
            )
            .await?;
            #[cfg(target_os = "linux")]
            workx_cli::run_command_under_landlock(
                sandbox_cli,
                arg0_paths.workx_linux_sandbox_exe.clone(),
                loader_overrides,
            )
            .await?;
            #[cfg(target_os = "windows")]
            workx_cli::run_command_under_windows_sandbox(
                sandbox_cli,
                arg0_paths.workx_linux_sandbox_exe.clone(),
                loader_overrides,
            )
            ...
            #[cfg(not(any(target_os = "macos", target_os = "linux", target_os = "windows")))]
            {
                ...
                anyhow::bail!("`workx sandbox` is not supported on this operating system");
            }
        }
        ...
        Some(Subcommand::ResponsesApiProxy(args)) => {
            ...
            tokio::task::spawn_blocking(move || workx_responses_api_proxy::run_main(args))
                .await??;
        }
...
fn profile_v2_for_subcommand<'a>(
    ...
    match subcommand {
        ...
        _ => anyhow::bail!(
            "--profile only applies to runtime commands and `workx mcp`: `workx`, `workx exec`, `workx review`, `workx resume`, `workx queue`, `workx archive`, `workx delete`, `workx unarchive`, `workx fork`, `workx mcp`, `workx sandbox`, and `workx debug prompt-input`."
        ),
    }
}
...
async fn run_exec_server_command(
    ...
    let workx_self_exe = arg0_paths
        .workx_self_exe
        .clone()
        .ok_or_else(|| anyhow::anyhow!("Workx executable path is not configured"))?;
    let runtime_paths = workx_exec_server::ExecServerRuntimePaths::new(
        workx_self_exe,
        arg0_paths.workx_linux_sandbox_exe.clone(),
    )?;
    if let Some(base_url) = cmd.remote {
        ...
        let auth_provider =
            load_exec_server_remote_auth_provider(&config, &base_url, cmd.use_agent_identity_auth)
                .await?;
        let mut remote_config = workx_exec_server::RemoteEnvironmentConfig::new(
            base_url,
            environment_id,
            auth_provider,
            ...
            async move {
                ...
                match cmd.command {
                    Some(ExecServerSubcommand::Forward { connect }) => {
                        workx_exec_server::run_remote_environment_forward_until_shutdown(
                            remote_config,
                            connect,
                            shutdown,
                        )
                    ...
                    None => {
                        workx_exec_server::run_remote_environment_until_shutdown(
                            remote_config,
                            runtime_paths,
                            shutdown,
                        )
        ...
        let http_client_factory = config
            .as_ref()
            .map(workx_core::config::Config::http_client_factory)
            .unwrap_or_else(|| {
                workx_http_client::HttpClientFactory::new(
                    workx_http_client::OutboundProxyPolicy::ReqwestDefault,
                )
            });
        let listen_url = cmd
            .listen
            .unwrap_or_else(|| workx_exec_server::DEFAULT_LISTEN_URL.to_string());
        exec_server_telemetry::run_until_shutdown(
            async move {
                workx_exec_server::run_main_with_telemetry(
                    &listen_url,
                    runtime_paths,
                    telemetry,
                    http_client_factory,
...
async fn load_exec_server_remote_auth_provider(
    config: &workx_core::config::Config,
    base_url: &str,
    use_agent_identity_auth: bool,
) -> anyhow::Result<workx_api::SharedAuthProvider> {
    if use_agent_identity_auth {
        read_workx_access_token_from_env().ok_or_else(|| {
            anyhow::anyhow!("WORKX_ACCESS_TOKEN is required when --use-agent-identity-auth is set")
        })?;
        let auth = AuthManager::shared_from_config(config, /*enable_workx_api_key_env*/ false)
            .await?
            .auth()
            .await
            .ok_or_else(|| anyhow::anyhow!("Agent Identity authentication is unavailable"))?;
        if !matches!(auth, WorkxAuth::AgentIdentity(_)) {
            anyhow::bail!(
                "WORKX_ACCESS_TOKEN did not provide permitted Agent Identity authentication"
            );
        }
        return Ok(workx_model_provider::auth_provider_from_auth(&auth));
    }

    let (auth_manager, auth) = load_exec_server_remote_auth(
        config,
        "remote exec-server registration requires ChatGPT authentication or API key authentication; run `workx login` or set WORKX_API_KEY",
    )
    .await?;
    ...
    if auth_manager.is_workload_identity_selected() {
        Ok(workx_model_provider::auth_provider_from_auth_manager(
            auth_manager,
            &auth,
        ))
    } else {
        Ok(workx_model_provider::auth_provider_from_auth(&auth))
    }
}

fn is_supported_exec_server_remote_auth(auth: &WorkxAuth) -> bool {
...
async fn load_exec_server_config(
...
) -> anyhow::Result<workx_core::config::Config> {
    ...
    if enable_workload_identity && is_workload_identity_selected() {
        ...
        .await?;
        let bootstrap_auth_config = bootstrap_auth_config(&workx_home, &bootstrap_config)?;
...
) -> anyhow::Result<(Arc<AuthManager>, workx_login::WorkxAuth)> {
    let auth_manager =
        AuthManager::shared_from_config(config, /*enable_workx_api_key_env*/ true).await?;
...
async fn enable_feature_in_config(feature: &str) -> anyhow::Result<()> {
    ...
    let workx_home = find_workx_home()?;
    ConfigEditsBuilder::new(&workx_home)
        .set_feature_enabled(feature, /*enabled*/ true)
        .apply()
        .await?;
    println!("Enabled feature `{feature}` in config.toml.");
    maybe_print_under_development_feature_warning(&workx_home, feature);
    Ok(())
}
...
fn loader_overrides_for_profile(
    ...
    match profile_v2 {
        Some(profile_v2) => {
            let workx_home = find_workx_home()?;
            Ok(loader_overrides_for_profile_at_workx_home(
                Some(profile_v2),
                &workx_home,
            ))
        }
...
fn loader_overrides_for_profile_at_workx_home(
    profile_v2: Option<&ProfileV2Name>,
    workx_home: &std::path::Path,
) -> LoaderOverrides {
    match profile_v2 {
        Some(profile_v2) => LoaderOverrides {
            user_config_path: Some(resolve_profile_v2_config_path(workx_home, profile_v2)),
...
fn maybe_print_under_development_feature_warning(workx_home: &std::path::Path, feature: &str) {
    ...
    let config_path = workx_home.join(workx_config::CONFIG_TOML_FILE);
    eprintln!(
...
async fn run_debug_prompt_input_command(
    ...
    let sandbox_mode = if shared.dangerously_bypass_approvals_and_sandbox {
        Some(workx_protocol::config_types::SandboxMode::DangerFullAccess)
    } else {
    ...
    let overrides = ConfigOverrides {
        ...
        sandbox_mode,
        cwd: shared.cwd,
        workx_self_exe: arg0_paths.workx_self_exe,
        workx_linux_sandbox_exe: arg0_paths.workx_linux_sandbox_exe,
    ...
    let auth_manager =
        AuthManager::shared_from_config(&config, /*enable_workx_api_key_env*/ false).await?;
    let mut extensions = workx_extension_api::ExtensionRegistryBuilder::new();
    workx_git_attribution::install(
        &mut extensions,
        auth_manager,
        config.chatgpt_base_url.clone(),
        config.http_client_factory(),
    );
    workx_skills_extension::install(&mut extensions, |config: &Config| {
        workx_skills_extension::SkillsExtensionConfig {
            ...
            shadow_selection_enabled: config
                .features
                .enabled(workx_features::Feature::SkillSearch),
        }
    });
    let prompt_input = workx_core::build_prompt_input(
        config,
        input,
        /*state_db*/ None,
...
async fn run_debug_clear_memories_command(
    ...
    clear_memory_roots_contents(&config.workx_home).await?;
...
fn reject_remote_mode_for_subcommand(
    ...
    if let Some(remote) = remote {
        anyhow::bail!(
            "`--remote {remote}` is only supported for interactive TUI commands, not `workx {subcommand}`"
        );
    }
    if remote_auth_token_env.is_some() {
        anyhow::bail!(
            "`--remote-auth-token-env` is only supported for interactive TUI commands, not `workx {subcommand}`"
        );
    }
    Ok(())
}
...
/// `workx --strict-config` continue to work for the TUI and for wrappers that
...
fn reject_strict_config_for_unsupported_subcommand(
    ...
    if strict_config {
        anyhow::bail!("`--strict-config` is not supported for `workx {subcommand}`");
    }
    Ok(())
}
...
async fn print_app_server_daemon_output(command: AppServerLifecycleCommand) -> anyhow::Result<()> {
    let output = workx_app_server_daemon::run(command).await?;
    println!("{}", serde_json::to_string(&output)?);
    Ok(())
}

fn updater_http_client_factory(
    config: anyhow::Result<workx_core::config::Config>,
) -> workx_http_client::HttpClientFactory {
    match config {
...
async fn print_app_server_remote_control_output(
    ...
    let output = workx_app_server_daemon::set_remote_control(mode).await?;
    println!("{}", serde_json::to_string(&output)?);
    Ok(())
}
...
async fn run_interactive_tui(
    ...
    let terminal_info = workx_terminal_detection::terminal_info();
    if terminal_info.name == TerminalName::Dumb {
        ...
        eprintln!(
            "WARNING: TERM is set to \"dumb\". Workx's interactive TUI may not work in this terminal."
        );
    ...
    if interactive.agents_overview && remote.is_none() {
        ...
        workx_app_server_daemon::run(AppServerLifecycleCommand::Start)
            .await
            .map_err(std::io::Error::other)?;
    }
    ...
    let start_tui = || {
        workx_tui::run_main(
            interactive.clone(),
            arg0_paths.clone(),
            workx_config::LoaderOverrides::default(),
        ...
        match local_state_db::backup_files_for_fresh_start(startup_error).await {
            ...
            Err(backup_err) => {
                ...
                return Ok(AppExitInfo::fatal(format!(
                    "failed to move damaged Workx local database files into a backup folder automatically: {backup_err}"
                )));
            }
        }
    }
...
fn resolve_remote_endpoint(
    remote: Option<String>,
    remote_auth_token_env: Option<String>,
) -> std::io::Result<Option<workx_tui::RemoteAppServerEndpoint>> {
    let mut remote_endpoint = remote
        .as_deref()
        .map(workx_tui::resolve_remote_addr)
        .transpose()
        .map_err(std::io::Error::other)?;
    if let Some(remote_auth_token_env) = remote_auth_token_env {
        ...
        if !workx_tui::remote_addr_supports_auth_token(endpoint) {
            return Err(std::io::Error::other(
        ...
        let auth_token = read_remote_auth_token_from_env_var(&remote_auth_token_env)
            .map_err(std::io::Error::other)?;
        let workx_tui::RemoteAppServerEndpoint::WebSocket {
            auth_token: slot, ..
        } = endpoint
        else {
            return Err(std::io::Error::other(
...
/// Build the final `TuiCli` for a `workx resume` invocation.
fn finalize_resume_interactive(
    ...
    // configuration surface area as `workx` without additional flags.
...
/// Build the final `TuiCli` for a `workx fork` invocation.
...
mod tests {
    ...
    async fn updater_http_client_factory_honors_respect_system_proxy() {
        let workx_home = tempfile::tempdir().expect("temporary Workx home");
        let config = ConfigBuilder::default()
        ...
        assert_eq!(
            ...
            workx_http_client::OutboundProxyPolicy::RespectSystemProxy
        );
    }
    ...
    fn exec_server_remote_auth_accepts_api_key_auth() {
        let auth = WorkxAuth::from_api_key("sk-test");
    ...
    #[test]
    fn profile_loader_overrides_use_explicit_workx_home() -> anyhow::Result<()> {
        let workx_home = tempfile::tempdir()?;
        let profile: ProfileV2Name = "work".parse()?;

        let overrides =
            loader_overrides_for_profile_at_workx_home(Some(&profile), workx_home.path());

        assert_eq!(
            overrides.user_config_path,
            Some(resolve_profile_v2_config_path(workx_home.path(), &profile))
        );
    ...
    fn profile_v2_is_rejected_for_config_management_subcommands() {
        assert!(profile_v2_for_args(&["workx", "--profile", "work", "features", "list"]).is_err());
    }

    #[test]
    fn profile_v2_is_allowed_for_runtime_subcommands() {
        assert_eq!(
            profile_v2_for_args(&["workx", "--profile", "work", "resume"])
                .expect("resume supports profile-v2")
                .as_deref(),
            Some("work")
        );
        assert_eq!(
            profile_v2_for_args(&["workx", "--profile", "work", "debug", "prompt-input"])
                .expect("debug prompt-input supports profile-v2")
                .as_deref(),
            Some("work")
        );
        assert_eq!(
            profile_v2_for_args(&["workx", "--profile", "work", "mcp", "list"])
                .expect("mcp supports profile-v2")
                .as_deref(),
            Some("work")
        );
        assert_eq!(
            profile_v2_for_args(&["workx", "--profile", "work", "sandbox"])
    ...
    fn import_remains_an_interactive_prompt() {
        let cli = MultitoolCli::try_parse_from(["workx", "import"]).expect("parse");
    ...
    fn profile_v2_rejects_non_plain_names_at_parse_time() {
        assert!(
            MultitoolCli::try_parse_from(["workx", "--profile", "nested/work", "resume"]).is_err()
        );
    }
    ...
    fn exec_resume_last_accepts_prompt_positional() {
        let cli =
            MultitoolCli::try_parse_from(["workx", "exec", "--json", "resume", "--last", "2+2"])
                .expect("parse should succeed");

        let Some(Subcommand::Exec(exec)) = cli.subcommand else {
            panic!("expected exec subcommand");
        };
        let Some(workx_exec::Command::Resume(args)) = exec.command else {
    ...
    fn approve_for_me_configures_interactive_mode() {
        for flag in ["--approve-for-me", "--not-so-yolo"] {
            let mut cli = MultitoolCli::try_parse_from(["workx", flag]).expect("parse flag");
    ...
    fn not_so_yolo_alias_is_hidden_from_help() {
        for args in [&["workx", "--help"][..], &["workx", "exec", "--help"][..]] {
    ...
    fn approve_for_me_defaults_propagate_from_root_to_exec() {
        let exec = finalize_exec_from_args(&["workx", "--approve-for-me", "exec", "summarize"]);
    ...
    fn later_exec_sandbox_partially_overrides_approve_for_me() {
        ...
        assert_matches!(
            exec.sandbox_mode,
            Some(workx_utils_cli::SandboxModeCliArg::ReadOnly)
        );
        assert_eq!(
            exec.config_overrides.raw_overrides,
            vec![
    ...
    fn later_resume_approval_policy_partially_overrides_approve_for_me() {
        ...
        assert_matches!(
            ...
            Some(workx_utils_cli::ApprovalModeCliArg::Never)
        );
        assert_eq!(
    ...
    fn approve_for_me_conflicts_with_explicit_interactive_permissions() {
        for conflicting_args in [
            ...
            let mut args = vec!["workx", "--approve-for-me"];
    ...
    fn default_app_server_socket_path() -> AbsolutePathBuf {
        let workx_home = find_workx_home().expect("workx home");
        workx_app_server::app_server_control_socket_path(&workx_home)
    ...
    fn debug_models_parses_bundled_flag() {
        let cli =
            MultitoolCli::try_parse_from(["workx", "debug", "models", "--bundled"]).expect("parse");
    ...
    fn plugin_marketplace_help_uses_plugin_namespace() {
        let help = help_from_args(&["workx", "plugin", "marketplace", "--help"]);
        assert!(
            help.contains("Usage: workx plugin marketplace [OPTIONS] <COMMAND>"),
            "{help}"
        );

        for (subcommand, usage) in [
            ("add", "Usage: workx plugin marketplace add"),
            ("list", "Usage: workx plugin marketplace list"),
            ("upgrade", "Usage: workx plugin marketplace upgrade"),
            ("remove", "Usage: workx plugin marketplace remove"),
        ] {
            let help = help_from_args(&["workx", "plugin", "marketplace", subcommand, "--help"]);
    ...
    fn plugin_marketplace_add_parses_under_plugin() {
        let cli =
            MultitoolCli::try_parse_from(["workx", "plugin", "marketplace", "add", "owner/repo"])
                .expect("parse");
    ...
    fn plugin_marketplace_upgrade_parses_under_plugin() {
        let cli =
            MultitoolCli::try_parse_from(["workx", "plugin", "marketplace", "upgrade", "debug"])
                .expect("parse");
    ...
    fn plugin_list_parses_under_plugin() {
        let cli =
            MultitoolCli::try_parse_from(["workx", "plugin", "list", "--marketplace", "debug"])
                .expect("parse");
    ...
    fn update_parses_as_update_subcommand() {
        let cli = MultitoolCli::try_parse_from(["workx", "update"]).expect("parse");
    ...
    fn sandbox_help_only_shows_singular_permission_profile() {
        let help = help_from_args(&["workx", "sandbox", "--help"]);
    ...
    fn sandbox_parses_permissions_profile_short_alias() {
        let cli =
            MultitoolCli::try_parse_from(["workx", "sandbox", "-P", ":workspace", "--", "echo"])
                .expect("parse");
    ...
    fn sandbox_parses_config_profile() {
        let cli =
            MultitoolCli::try_parse_from(["workx", "sandbox", "--profile", "work", "--", "echo"])
                .expect("parse");
    ...
    fn sandbox_rejects_explicit_profile_controls_without_profile() {
        let err = MultitoolCli::try_parse_from(["workx", "sandbox", "-C", "/tmp"])
    ...
    fn plugin_marketplace_remove_parses_under_plugin() {
        let cli =
            MultitoolCli::try_parse_from(["workx", "plugin", "marketplace", "remove", "debug"])
                .expect("parse");
    ...
    fn marketplace_no_longer_parses_at_top_level() {
        let add_result =
            MultitoolCli::try_parse_from(["workx", "marketplace", "add", "owner/repo"]);
        assert!(add_result.is_err());

        let upgrade_result =
            MultitoolCli::try_parse_from(["workx", "marketplace", "upgrade", "debug"]);
        assert!(upgrade_result.is_err());

        let remove_result =
            MultitoolCli::try_parse_from(["workx", "marketplace", "remove", "debug"]);
    ...
    fn sample_exit_info(conversation_id: Option<&str>, thread_name: Option<&str>) -> AppExitInfo {
        ...
        AppExitInfo {
            token_usage,
            thread_id,
            resume_hint: thread_id.map(|thread_id| workx_tui::ResumableThread {
                thread_id,
    ...
    async fn format_exit_messages_preserves_auth_env_through_tui_runner() {
        let exit_info = run_tui_with_recovery(
            || async {
                ...
                exit_info.disconnect_info = Some(workx_tui::DisconnectInfo {
                    command: vec![
                        "workx".to_string(),
                        "--remote".to_string(),
        ...
        assert_eq!(
            ...
            vec![
                ...
                "Reconnect: workx --remote wss://example.com:443/ --remote-auth-token-env WORKX_REMOTE_TOKEN resume 123e4567-e89b-12d3-a456-426614174000",
                "Stop the current turn: run workx --remote wss://example.com:443/ --remote-auth-token-env WORKX_REMOTE_TOKEN agents, select this task, and press ctrl + x.",
    ...
    fn format_exit_messages_includes_resume_hint_for_fatal_exit() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                "  workx resume 123e4567-e89b-12d3-a456-426614174000".to_string(),
            ]
        );
    }
        ...
        insta::allow_duplicates! {
            for thread_name in [None, Some("")] {
                ...
                To continue this session, run:
                  workx resume 123e4567-e89b-12d3-a456-426614174000
                ");
            }
        }
    }
    ...
    fn format_exit_messages_applies_color_when_enabled() {
        ...
        assert_eq!(
            ...
            vec![
                "Token usage: total=2 input=0 output=2",
                "To continue this session, run:",
                "  \u{1b}[36mworkx resume 123e4567-e89b-12d3-a456-426614174000\u{1b}[39m",
            ]
        );
    }
    ...
    fn format_exit_messages_names_picker_item_when_thread_has_name() {
        ...
        To continue this session, run:
          workx resume 123e4567-e89b-12d3-a456-426614174000
        Or run workx resume and select my-thread.
        ");
    }
    ...
    fn format_exit_messages_colors_commands_and_thread_name() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                "To continue this session, run:",
                "  \u{1b}[36mworkx resume 123e4567-e89b-12d3-a456-426614174000\u{1b}[39m",
                "Or run \u{1b}[36mworkx resume\u{1b}[39m and select \u{1b}[36mmy-thread\u{1b}[39m.",
            ]
        );
    }
    ...
    fn resume_model_flag_applies_when_no_root_flags() {
        let interactive =
            finalize_resume_from_args(["workx", "resume", "-m", "gpt-5.1-test"].as_ref());
    ...
    fn resume_and_fork_preserve_no_alt_screen() {
        for (command, finalize) in [
            ...
            assert!(finalize(&["workx", command, "--no-alt-screen"]).no_alt_screen);
            assert!(finalize(&["workx", "--no-alt-screen", command]).no_alt_screen);
            assert!(!finalize(&["workx", command]).no_alt_screen);
        }
    }

    #[test]
    fn resume_picker_logic_none_and_not_last() {
        let interactive = finalize_resume_from_args(["workx", "resume"].as_ref());
        assert!(interactive.resume_picker);
        assert!(!interactive.resume_last);
        assert_eq!(interactive.resume_session_id, None);
    ...
    fn resume_picker_logic_last() {
        let interactive = finalize_resume_from_args(["workx", "resume", "--last"].as_ref());
        assert!(!interactive.resume_picker);
        assert!(interactive.resume_last);
        assert_eq!(interactive.resume_session_id, None);
        assert!(!interactive.resume_show_all);
    ...
    fn resume_last_accepts_prompt_positional() {
        let interactive = finalize_resume_from_args(
            ["workx", "resume", "--last", "/compact focus on auth"].as_ref(),
        );
    ...
    fn resume_last_rejects_explicit_session_and_prompt() {
        let err =
            MultitoolCli::try_parse_from(["workx", "resume", "--last", "1234", "continue here"])
    ...
    fn resume_picker_logic_with_session_id() {
        let interactive = finalize_resume_from_args(["workx", "resume", "1234"].as_ref());
        assert!(!interactive.resume_picker);
        assert!(!interactive.resume_last);
    ...
    fn resume_with_session_id_accepts_prompt_positional() {
        let interactive =
            finalize_resume_from_args(["workx", "resume", "1234", "continue here"].as_ref());
    ...
    fn resume_all_flag_sets_show_all() {
        let interactive = finalize_resume_from_args(["workx", "resume", "--all"].as_ref());
        assert!(interactive.resume_picker);
        assert!(interactive.resume_show_all);
    }
    ...
    fn resume_include_non_interactive_flag_sets_source_filter_override() {
        let interactive =
            finalize_resume_from_args(["workx", "resume", "--include-non-interactive"].as_ref());
    ...
    fn resume_merges_option_flags() {
        ...
        assert_matches!(
            ...
            Some(workx_utils_cli::SandboxModeCliArg::WorkspaceWrite)
        );
        assert_matches!(
            interactive.approval_policy,
            Some(workx_utils_cli::ApprovalModeCliArg::OnRequest)
        );
        assert_eq!(
            interactive.cwd.as_deref(),
    ...
    fn resume_merges_bypass_hook_trust_flag() {
        let interactive = finalize_resume_from_args(
            ["workx", "resume", "--dangerously-bypass-hook-trust"].as_ref(),
        );
    ...
    fn fork_picker_logic_none_and_not_last() {
        let interactive = finalize_fork_from_args(["workx", "fork"].as_ref());
    ...
    fn fork_picker_logic_last() {
        let interactive = finalize_fork_from_args(["workx", "fork", "--last"].as_ref());
        assert!(!interactive.fork_picker);
        assert!(interactive.fork_last);
        assert_eq!(interactive.fork_session_id, None);
        assert!(!interactive.fork_show_all);
    ...
    fn fork_last_accepts_prompt_positional() {
        let interactive =
            finalize_fork_from_args(["workx", "fork", "--last", "/compact focus on auth"].as_ref());
    ...
    fn fork_last_rejects_explicit_session_and_prompt() {
        let err =
            MultitoolCli::try_parse_from(["workx", "fork", "--last", "1234", "continue here"])
                .expect_err("--last with an explicit session and prompt should be rejected");
    ...
    fn fork_picker_logic_with_session_id() {
        let interactive = finalize_fork_from_args(["workx", "fork", "1234"].as_ref());
        assert!(!interactive.fork_picker);
        assert!(!interactive.fork_last);
    ...
    fn fork_with_session_id_accepts_prompt_positional() {
        let interactive =
            finalize_fork_from_args(["workx", "fork", "1234", "continue here"].as_ref());
    ...
    fn fork_all_flag_sets_show_all() {
        let interactive = finalize_fork_from_args(["workx", "fork", "--all"].as_ref());
        assert!(interactive.fork_picker);
        assert!(interactive.fork_show_all);
    }
    ...
    fn app_server_analytics_default_disabled_without_flag() {
        let app_server = app_server_from_args(["workx", "app-server"].as_ref());
    ...
    fn app_server_remote_control_startup_flag_enables_remote_control() {
        let enabled = app_server_from_args(["workx", "app-server", "--remote-control"].as_ref());
    ...
    fn app_server_analytics_default_enabled_with_flag() {
        let app_server =
            app_server_from_args(["workx", "app-server", "--analytics-default-enabled"].as_ref());
        assert!(app_server.analytics_default_enabled);
    }

    #[test]
    fn strict_config_parses_for_supported_commands() {
        let cli = MultitoolCli::try_parse_from(["workx", "--strict-config"]).expect("parse");
        assert!(cli.interactive.strict_config);

        let cli = MultitoolCli::try_parse_from(["workx", "mcp-server", "--strict-config"])
            .expect("parse");
        assert_matches!(
            cli.subcommand,
        ...
        let cli =
            MultitoolCli::try_parse_from(["workx", "review", "--strict-config", "--uncommitted"])
                .expect("parse");
        assert_matches!(
            cli.subcommand,
            Some(Subcommand::Review(ReviewCommand {
        ...
        let cli = MultitoolCli::try_parse_from(["workx", "exec-server", "--strict-config"])
            .expect("parse");
        assert_matches!(
            cli.subcommand,
    ...
    fn exec_server_forward_requires_registration_and_destination() {
        for args in [
            ...
            assert!(
                MultitoolCli::try_parse_from(["workx", "exec-server"].into_iter().chain(args))
                    .is_err()
            );
        }
    }
    ...
    fn root_strict_config_is_supported_for_exec_server() {
        let cli = MultitoolCli::try_parse_from(["workx", "--strict-config", "exec-server"])
            .expect("parse");
    ...
    fn root_strict_config_is_rejected_for_unsupported_subcommands() {
        let cli = MultitoolCli::try_parse_from(["workx", "--strict-config", "mcp", "list"])
            .expect("parse");
        ...
        assert_eq!(
            err.to_string(),
            "`--strict-config` is not supported for `workx mcp`"
        );

        let cli = MultitoolCli::try_parse_from(["workx", "--strict-config", "remote-control"])
            .expect("parse");
        let err = reject_root_strict_config_for_subcommand(
            cli.interactive.strict_config,
            &cli.subcommand,
        ...
        assert_eq!(
            err.to_string(),
            "`--strict-config` is not supported for `workx remote-control`"
        );
    }
    ...
    fn app_server_subcommands_reject_strict_config() {
        let app_server =
            app_server_from_args(["workx", "app-server", "--strict-config", "proxy"].as_ref());
        ...
        assert_eq!(
            err.to_string(),
            "`--strict-config` is not supported for `workx app-server proxy`"
        );
    }

    #[test]
    fn reject_remote_flag_for_remote_control() {
        let cli = MultitoolCli::try_parse_from(["workx", "--remote", "unix://", "remote-control"])
            .expect("parse");
    ...
    fn remote_control_pair_parses() {
        let cli = MultitoolCli::try_parse_from(["workx", "remote-control", "pair"]).expect("parse");
        let Some(Subcommand::RemoteControl(remote_control)) = &cli.subcommand else {
            panic!("expected remote-control subcommand");
        };
    ...
    fn remote_flag_parses_for_interactive_root() {
        let cli = MultitoolCli::try_parse_from(["workx", "--remote", "unix://workx.sock"])
            .expect("parse");
        assert_eq!(cli.remote.remote.as_deref(), Some("unix://workx.sock"));
    }
    ...
    fn remote_flag_parses_for_resume_subcommand() {
        let cli =
            MultitoolCli::try_parse_from(["workx", "resume", "--remote", "unix://workx.sock"])
                .expect("parse");
        ...
        assert_eq!(remote.remote.as_deref(), Some("unix://workx.sock"));
    }
    ...
    fn read_remote_auth_token_from_env_var_reports_missing_values() {
        let err = read_remote_auth_token_from_env_var_with("WORKX_REMOTE_AUTH_TOKEN", |_| {
    ...
    fn read_remote_auth_token_from_env_var_trims_values() {
        let auth_token =
            read_remote_auth_token_from_env_var_with("WORKX_REMOTE_AUTH_TOKEN", |_| {
    ...
    fn app_server_rejects_invalid_code_mode_host_urls() {
        for endpoint in [
            ...
            let error =
                MultitoolCli::try_parse_from(["workx", "app-server", "--code-mode-host", endpoint])
    ...
    fn app_server_listen_websocket_url_parses() {
        let app_server = app_server_from_args(
            ["workx", "app-server", "--listen", "ws://127.0.0.1:4500"].as_ref(),
        );
        assert_eq!(
            app_server.listen,
            workx_app_server::AppServerTransport::WebSocket {
    ...
    fn app_server_listen_stdio_url_parses() {
        let app_server =
            app_server_from_args(["workx", "app-server", "--listen", "stdio://"].as_ref());
        assert_eq!(
            app_server.listen,
            workx_app_server::AppServerTransport::Stdio
        );
    ...
    fn app_server_stdio_flag_parses() {
        let app_server = app_server_from_args(["workx", "app-server", "--stdio"].as_ref());
    ...
    fn app_server_listen_unix_socket_url_parses() {
        let app_server =
            app_server_from_args(["workx", "app-server", "--listen", "unix://"].as_ref());
        assert_eq!(
            app_server.listen,
            workx_app_server::AppServerTransport::UnixSocket {
    ...
    fn app_server_listen_unix_socket_path_parses() {
        let app_server = app_server_from_args(
            ["workx", "app-server", "--listen", "unix:///tmp/workx.sock"].as_ref(),
        );
        assert_eq!(
            app_server.listen,
            workx_app_server::AppServerTransport::UnixSocket {
                socket_path: AbsolutePathBuf::from_absolute_path("/tmp/workx.sock")
    ...
    fn app_server_listen_off_parses() {
        let app_server = app_server_from_args(["workx", "app-server", "--listen", "off"].as_ref());
        assert_eq!(app_server.listen, workx_app_server::AppServerTransport::Off);
    }

    #[test]
    fn app_server_listen_invalid_url_fails_to_parse() {
        let parse_result =
            MultitoolCli::try_parse_from(["workx", "app-server", "--listen", "http://foo"]);
        assert!(parse_result.is_err());
    }

    #[test]
    fn app_server_proxy_subcommand_parses() {
        let app_server = app_server_from_args(["workx", "app-server", "proxy"].as_ref());
        assert!(matches!(
    ...
    fn app_server_daemon_subcommands_parse() {
        ...
        assert!(matches!(
            app_server_from_args(["workx", "app-server", "daemon", "start"].as_ref()).subcommand,
            Some(AppServerSubcommand::Daemon(AppServerDaemonCommand {
                subcommand: AppServerDaemonSubcommand::Start
            }))
        ));
        assert!(matches!(
            app_server_from_args(["workx", "app-server", "daemon", "restart"].as_ref()).subcommand,
            Some(AppServerSubcommand::Daemon(AppServerDaemonCommand {
        ...
        assert!(matches!(
            app_server_from_args(
                ["workx", "app-server", "daemon", "enable-remote-control"].as_ref()
            )
            .subcommand,
            Some(AppServerSubcommand::Daemon(AppServerDaemonCommand {
        ...
        assert!(matches!(
            app_server_from_args(
                ["workx", "app-server", "daemon", "disable-remote-control"].as_ref()
            )
            .subcommand,
            Some(AppServerSubcommand::Daemon(AppServerDaemonCommand {
        ...
        assert!(matches!(
            app_server_from_args(["workx", "app-server", "daemon", "stop"].as_ref()).subcommand,
            Some(AppServerSubcommand::Daemon(AppServerDaemonCommand {
                subcommand: AppServerDaemonSubcommand::Stop
            }))
        ));
        assert!(matches!(
            app_server_from_args(["workx", "app-server", "daemon", "version"].as_ref()).subcommand,
            Some(AppServerSubcommand::Daemon(AppServerDaemonCommand {
    ...
    fn app_server_proxy_sock_path_parses() {
        let app_server =
            app_server_from_args(["workx", "app-server", "proxy", "--sock", "workx.sock"].as_ref());
        ...
        assert_eq!(
            proxy.socket_path,
            Some(
                AbsolutePathBuf::relative_to_current_dir("workx.sock")
    ...
    fn app_server_capability_token_flags_parse() {
        ...
        assert_eq!(
            ...
            Some(workx_app_server::WebsocketAuthCliMode::CapabilityToken)
        );
        assert_eq!(
            app_server.auth.ws_token_file,
            Some(PathBuf::from("/tmp/workx-token"))
        );
    }
    ...
    fn app_server_signed_bearer_flags_parse() {
        ...
        assert_eq!(
            app_server.auth.ws_auth,
            Some(workx_app_server::WebsocketAuthCliMode::SignedBearerToken)
        );
        assert_eq!(
            app_server.auth.ws_shared_secret_file,
            Some(PathBuf::from("/tmp/workx-secret"))
        );
    ...
    fn features_enable_parses_feature_name() {
        let cli = MultitoolCli::try_parse_from(["workx", "features", "enable", "unified_exec"])
            .expect("parse should succeed");
    ...
    fn features_disable_parses_feature_name() {
        let cli = MultitoolCli::try_parse_from(["workx", "features", "disable", "shell_tool"])
            .expect("parse should succeed");
        let Some(Subcommand::Features(FeaturesCli { sub })) = cli.subcommand else {
            panic!("expected features subcommand");
        };
    ...
    fn strict_config_feature_toggle_error(args: &[&str]) -> anyhow::Error {
        let cli_args = std::iter::once("workx")

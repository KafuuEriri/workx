...
const OTEL_SERVICE_NAME: &str = "workx-exec-server";
...
pub(crate) fn init(
    config: Option<&workx_core::config::Config>,
) -> (impl Send + Sync, workx_exec_server::ExecServerTelemetry) {
    let fmt_layer = tracing_subscriber::fmt::layer()
        .with_writer(std::io::stderr)
        .with_filter(stderr_env_filter());
    let otel = match config {
        Some(config) => workx_core::otel_init::build_provider(
            config,
    ...
    workx_core::otel_init::record_process_start(provider, OTEL_SERVICE_NAME);
    ...
    let telemetry = provider
        ...
        .cloned()
        .map(workx_exec_server::ExecServerTelemetry::new)
        .unwrap_or_default();

...
use workx_state::LogRow;
use workx_state::SqliteConfig;
use workx_state::StateRuntime;
use workx_utils_absolute_path::AbsolutePathBuf;

#[derive(Debug, Parser)]
#[command(name = "workx-state-logs")]
#[command(about = "Tail Workx logs from the dedicated logs SQLite DB with simple filters")]
struct Args {
    /// Path to WORKX_HOME. Defaults to $WORKX_HOME or ~/.workx.
    #[arg(long, env = "WORKX_HOME")]
    workx_home: Option<PathBuf>,

    /// Direct path to the logs SQLite database. Overrides --workx-home.
    #[arg(long)]
    db: Option<PathBuf>,
...
async fn resolve_sqlite_config(args: &Args) -> anyhow::Result<SqliteConfig> {
    ...
    if let Some(workx_home) = args.workx_home.as_ref() {
        config_builder = config_builder.workx_home(workx_home.clone());
    }
...
mod tests {
    ...
    fn log_level_rejects_aliases_and_unknown_values() {
        assert!(Args::try_parse_from(["workx-state-logs", "--level", "warning"]).is_err());
        assert!(Args::try_parse_from(["workx-state-logs", "--level", "err"]).is_err());
        assert!(Args::try_parse_from(["workx-state-logs", "--level", "warn,error"]).is_err());
    }

    #[test]
    fn log_level_accepts_canonical_values_case_insensitively() {
        let args = Args::try_parse_from(["workx-state-logs", "--level", "WARN"])
            .expect("parse uppercase log level");

        assert_eq!(args.level, Some(LogLevelThreshold::Warn));
    }

    /// Explicit database selection must not parse an overridden Workx home.
    #[tokio::test]
    async fn direct_db_skips_workx_home_config() {
        let workx_home = tempfile::tempdir().expect("create Workx home");
        std::fs::write(workx_home.path().join("config.toml"), "model = [")
            .expect("write invalid config");
        let sqlite_home = tempfile::tempdir().expect("create SQLite home");
        let db_path = sqlite_home.path().join("logs_2.sqlite");
        let args = Args::try_parse_from([
            OsString::from("workx-state-logs"),
            OsString::from("--workx-home"),
            workx_home.path().as_os_str().to_owned(),
            OsString::from("--db"),

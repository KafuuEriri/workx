//! Implements the `workx doctor` diagnostic report.
//!
...
use workx_api::ApiError;
use workx_api::ResponsesWebsocketClient;
use workx_api::is_azure_responses_provider;
...
use workx_install_context::InstallMethod;
use workx_install_context::StandalonePlatform;
use workx_install_context::WorkxPackageLayout;
use workx_login::AuthDotJson;
use workx_login::AuthManager;
use workx_login::OPENAI_API_KEY_ENV_VAR;
use workx_login::WORKX_ACCESS_TOKEN_ENV_VAR;
use workx_login::WORKX_API_KEY_ENV_VAR;
use workx_login::WorkxAuth;
use workx_login::default_client::create_client_without_request_logging;
use workx_login::default_client::default_headers;
use workx_login::load_auth_dot_json;
use workx_model_provider::create_model_provider;
use workx_protocol::auth::AuthMode;
use workx_protocol::protocol::AskForApproval;
use workx_terminal_detection::Multiplexer;
use workx_terminal_detection::TerminalInfo;
use workx_terminal_detection::TerminalName;
use workx_terminal_detection::terminal_info;
...
/// Options for building a local Workx diagnostic report.
///
...
///
/// This is the CLI entry point for workx doctor. It does not repair issues;
...
async fn build_report(
            ...
            let (
                ...
                async {
                    run_sync_check("auth", progress.clone(), || match &auth_manager_result {
                        ...
                        .remediation(
                            "Fix the reported authentication error, then rerun workx doctor.",
                        ),
                    })
                },
            ...
            let (
                ...
                async {
                    run_sync_check("config", progress.clone(), || {
                        ...
                        .remediation("Fix the reported config error, then rerun workx doctor.")
                    })
                },
                async {
    ...
    DoctorReport {
        ...
        overall_status,
        workx_version: env!("CARGO_PKG_VERSION").to_string(),
        checks,
    }
}
...
async fn load_config(
    ...
    ConfigBuilder::default()
        ...
        .build()
        .await
        .context("failed to load Workx config")
}
...
/// JSON support report emitted by `workx doctor --json`.
///
...
fn redacted_json_report(report: &DoctorReport) -> JsonDoctorReport {
    ...
    JsonDoctorReport {
        ...
        workx_version: report.workx_version.clone(),
        checks,
    }
}
...
fn installation_check(show_details: bool) -> DoctorCheck {
    ...
    details.push(format!(
        "managed by bun: {}",
        env::var_os("WORKX_MANAGED_BY_BUN").is_some()
    ));
    details.push(format!(
        "managed by Vite+: {}",
        env::var_os("WORKX_MANAGED_BY_VITE_PLUS").is_some()
    ));
    details.push(format!(
        "managed by pnpm: {}",
        env::var_os("WORKX_MANAGED_BY_PNPM").is_some()
    ));
    push_env_path_detail(
        &mut details,
        "managed package root",
        "WORKX_MANAGED_PACKAGE_ROOT",
    );

    let path_entries = workx_path_entries();
    let mut status = CheckStatus::Ok;
    let mut summary = "installation looks consistent".to_string();
    let mut remediation = None;

    if path_entries.len() > 1 {
        details.push(format!("PATH workx entries: {}", path_entries.len()));
    }
    if show_details || path_entries.len() > 1 {
        details.extend(
            path_entries
                .iter()
                .enumerate()
                .map(|(index, path)| format!("PATH workx #{}: {path}", index + 1)),
        );
    }
    ...
    if doctor_managed_by_npm(current_exe.as_deref()) {
        match npm_global_root_check() {
            ...
            NpmRootCheck::Mismatch {
                ...
                summary =
                    "npm install -g @ronanxiao/workx would update a different install".to_string();
            ...
            NpmRootCheck::MissingPackageRoot => {
                ...
                remediation = Some(
                    "Reinstall or update Workx so the JS shim provides WORKX_MANAGED_PACKAGE_ROOT."
                        .to_string(),
                );
            }
...
fn doctor_managed_by_npm(current_exe: Option<&Path>) -> bool {
    env::var_os("WORKX_MANAGED_BY_NPM").is_some()
        && !inherited_managed_env_for_cargo_binary(current_exe)
}

fn inherited_managed_env_for_cargo_binary(current_exe: Option<&Path>) -> bool {
    if env::var_os("WORKX_MANAGED_BY_NPM").is_none()
        && env::var_os("WORKX_MANAGED_BY_BUN").is_none()
        && env::var_os("WORKX_MANAGED_BY_VITE_PLUS").is_none()
        && env::var_os("WORKX_MANAGED_BY_PNPM").is_none()
    {
        return false;
    }
...
fn describe_method_with_package_layout(
    method: &str,
    package_layout: Option<&WorkxPackageLayout>,
) -> String {
...
fn npm_global_root_check() -> NpmRootCheck {
    let Some(running_package_root) = env::var_os("WORKX_MANAGED_PACKAGE_ROOT").map(PathBuf::from)
    else {
...
fn compare_npm_package_roots(running_package_root: &Path, npm_root: &Path) -> NpmRootCheck {
    let npm_package_root = npm_root.join("@openai").join("workx");
...
fn workx_path_entries() -> Vec<String> {
    #[cfg(windows)]
    let result = run_command("where", ["workx"]);
    #[cfg(not(windows))]
    let result = run_command("which", ["-a", "workx"]);
...
fn config_check(config: &Config) -> DoctorCheck {
    let mut details = Vec::new();
    details.push(format!("WORKX_HOME: {}", config.workx_home.display()));
...
fn config_toml_details(config: &Config, details: &mut Vec<String>) {
    let config_path = config.workx_home.join(workx_config::CONFIG_TOML_FILE);
...
fn auth_check(config: &Config) -> DoctorCheck {
    let mut details = Vec::new();
    let auth_path = config.workx_home.join("auth.json");
    details.push(format!(
        "auth storage mode: {:?}",
    ...
    let env_auth_vars = [
        ...
        WORKX_API_KEY_ENV_VAR,
        WORKX_ACCESS_TOKEN_ENV_VAR,
    ]
    .into_iter()
    ...
    match load_auth_dot_json(
        ...
        Ok(Some(auth)) => {
            ...
            if status == CheckStatus::Fail {
                check =
                    check.remediation("Run workx login again or provide a supported auth env var.");
            }
            check
        }
        ...
        Ok(None) => DoctorCheck::new(
            ...
            "auth",
            CheckStatus::Fail,
            "no Workx credentials were found",
        )
        .details(details)
        .remediation("Run workx login or provide an API key through a supported auth env var."),
        ...
        .detail(err.to_string())
        .remediation("Fix auth storage access or run workx login again."),
    }
}
...
fn stored_auth_mode(auth: &workx_login::AuthDotJson) -> &'static str {
...
fn stored_auth_issues(
    ...
    match stored_auth_mode_value(auth) {
        AuthMode::ApiKey => {
            ...
            let env_key_present =
                env_var_present(OPENAI_API_KEY_ENV_VAR) || env_var_present(WORKX_API_KEY_ENV_VAR);
...
async fn state_check(config: &Config, command: &DoctorCommand) -> DoctorCheck {
    let mut details = Vec::new();
    path_readiness(&mut details, "WORKX_HOME", &config.workx_home);
    ...
    rollout_stats_details(&mut details, &config.workx_home);
...
async fn sqlite_integrity_detail(
    sqlite: &workx_state::SqliteConfig,
    details: &mut Vec<String>,
    label: &str,
    path: &Path,
    ...
    let (rows, timed_out) = match workx_state::sqlite_integrity_check(sqlite, path, deadline).await
    {
        Ok(workx_state::SqliteIntegrityCheck::Complete(rows)) => (rows, false),
        Ok(workx_state::SqliteIntegrityCheck::TimedOut(rows)) => (rows, true),
        Err(err) => {
...
fn rollout_stats_details(details: &mut Vec<String>, workx_home: &Path) {
    let active = collect_rollout_stats(&workx_home.join("sessions"));
    let archived = collect_rollout_stats(&workx_home.join("archived_sessions"));
...
fn auth_mode_name(auth: &WorkxAuth) -> &'static str {
...
fn fallback_state_check() -> DoctorCheck {
    ...
    match workx_home {
        Ok(path) => DoctorCheck::new(
            ...
            "state",
            CheckStatus::Ok,
            "WORKX_HOME was resolved without config",
        )
        .detail(format!("WORKX_HOME: {}", path.display())),
        Err(err) => DoctorCheck::new(
            "state.paths",
            "state",
            CheckStatus::Warning,
            "WORKX_HOME could not be resolved",
        )
        .detail(err.to_string()),
    }
}
...
fn provider_auth_reachability_mode_from_auth(
    ...
    if provider_base_url.is_some_and(|url| !url.trim().is_empty())
        ...
        || env_var_present(WORKX_API_KEY_ENV_VAR)
    {
        return ProviderAuthReachabilityMode::ApiKey;
    }
    if env_var_present(WORKX_ACCESS_TOKEN_ENV_VAR) {
...
fn provider_reachability_plan_from_parts(
    ...
    let endpoints = match (mode, provider_base_url) {
        ...
        (ProviderAuthReachabilityMode::Chatgpt, None) => vec![ReachabilityEndpoint {
            ...
            url: provider_url_for_path(chatgpt_base_url, "workx/responses", provider_query_params),
            required: true,
...
mod tests {
    ...
    fn compare_npm_package_roots_detects_match() {
        let running = PathBuf::from("/prefix/lib/node_modules/@ronanxiao/workx");
        let npm_root = PathBuf::from("/prefix/lib/node_modules");
        assert_eq!(
            compare_npm_package_roots(&running, &npm_root),
            NpmRootCheck::Match {
                package_root: npm_root.join("@openai").join("workx")
            }
        );
    }
    ...
    fn compare_npm_package_roots_detects_mismatch() {
        let running = PathBuf::from("/old/lib/node_modules/@ronanxiao/workx");
        let npm_root = PathBuf::from("/new/lib/node_modules");
        assert_eq!(
            compare_npm_package_roots(&running, &npm_root),
            NpmRootCheck::Mismatch {
                running_package_root: running,
                npm_package_root: npm_root.join("@openai").join("workx"),
            }
        );
    }
    ...
    fn startup_warning_counts_group_known_sources() {
        let warnings = vec![
            ...
            "[features].workx_hooks is deprecated. Use [features].hooks instead.".to_string(),
    ...
    fn config_overrides_from_interactive_preserves_global_options() {
        ...
        let arg0_paths = Arg0DispatchPaths {
            workx_self_exe: Some(PathBuf::from("/bin/workx")),
            workx_linux_sandbox_exe: Some(PathBuf::from("/bin/workx-linux-sandbox")),
            main_execve_wrapper_exe: Some(PathBuf::from("/bin/workx-execve-wrapper")),
        };
        ...
        assert_eq!(overrides.workx_self_exe, arg0_paths.workx_self_exe);
        assert_eq!(
            overrides.workx_linux_sandbox_exe,
            arg0_paths.workx_linux_sandbox_exe
        );
        assert_eq!(
    ...
    async fn mcp_check_ignores_disabled_servers() {
        let disabled_server: McpServerConfig = toml::from_str(
            r#"
                ...
                enabled = false
                required = true
                bearer_token_env_var = "WORKX_DOCTOR_DISABLED_MCP_TOKEN"
            "#,
        )
        ...
        assert!(
            check
                .details
                .iter()
                .all(|detail| !detail.contains("WORKX_DOCTOR_DISABLED_MCP_TOKEN"))
        );
        assert!(
            check
                .details
    ...
    fn provider_specific_auth_fails_when_provider_env_key_is_missing() {
        let check = provider_specific_auth_check(
            ...
            Some("Set PROVIDER_API_KEY before running Workx."),
            Vec::new(),
            |_| false,
        )
        .expect("non-OpenAI provider should produce a provider-specific check");
        ...
        assert_eq!(
            check.remediation,
            Some("Set PROVIDER_API_KEY before running Workx.".to_string())
        );
    }
    ...
    fn provider_reachability_mode_uses_api_key_auth() {
        ...
        assert_eq!(
            provider_auth_reachability_mode_from_auth(
                ...
                /*provider_env_key*/ None,
                /*provider_base_url*/ None,
                |name| name == WORKX_API_KEY_ENV_VAR,
                /*stored_auth*/ None,
            ),
            ProviderAuthReachabilityMode::ApiKey
        );
    ...
    async fn mcp_check_fails_required_missing_stdio_command() {
        let required_server: McpServerConfig = toml::from_str(
            r#"
                command = "definitely-missing-workx-doctor-mcp"
                required = true
            "#,
        )
        .expect("should deserialize required MCP config");
        ...
        assert!(check.details.iter().any(|detail| {
            detail.contains(
                "required: stdio command \"definitely-missing-workx-doctor-mcp\" is not resolvable",
            )
        }));
    }

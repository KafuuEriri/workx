...
#[cfg(target_os = "macos")]
use workx_core::spawn::WORKX_SANDBOX_ENV_VAR;
...
use workx_sandboxing::landlock::allow_network_for_proxy;
use workx_sandboxing::landlock::create_linux_sandbox_command_args_for_permission_profile;
#[cfg(target_os = "macos")]
use workx_sandboxing::seatbelt::CreateSeatbeltCommandArgsParams;
#[cfg(target_os = "macos")]
use workx_sandboxing::seatbelt::create_seatbelt_command_args;
use workx_sandboxing::with_managed_mitm_ca_readable_root;
...
pub async fn run_command_under_seatbelt(
    ...
    _workx_linux_sandbox_exe: Option<PathBuf>,
...
async fn run_command_under_sandbox(
    ...
    let sandbox_state = config_options
        ...
        .sandbox_state_json
        .as_deref()
        .map(serde_json::from_str::<workx_mcp::SandboxState>)
        .transpose()
    ...
    let workx_linux_sandbox_exe = match sandbox_state.as_ref() {
        Some(state) => {
            ...
            state
                .workx_linux_sandbox_exe
                .clone()
                .or(workx_linux_sandbox_exe)
        }
        None => workx_linux_sandbox_exe,
    };
    ...
    let network = network_proxy
        .as_ref()
        .map(workx_core::config::StartedNetworkProxy::proxy);
    ...
    let mut child = match sandbox_type {
        ...
        SandboxType::Seatbelt => {
            ...
            spawn_debug_sandbox_child(
                ...
                env,
                |env_map| {
                    env_map.insert(WORKX_SANDBOX_ENV_VAR.to_string(), "seatbelt".to_string());
        ...
        SandboxType::Landlock => {
            ...
            let workx_linux_sandbox_exe = config
                .workx_linux_sandbox_exe
                .expect("workx-linux-sandbox executable not found");
...
async fn run_command_under_windows_session(
    ...
    use workx_windows_sandbox::WindowsSandboxProxySettingsMode;
    use workx_windows_sandbox::WindowsSandboxSessionRequest;
    use workx_windows_sandbox::spawn_windows_sandbox_session_for_level;

    let empty_paths: &[AbsolutePathBuf] = &[];
    let spawned = spawn_windows_sandbox_session_for_level(WindowsSandboxSessionRequest {
        permission_profile,
        workspace_roots: workspace_roots.as_slice(),
        workx_home: config.workx_home.as_path(),
        command,
        cwd: cwd.as_path(),
        env_map: env,
    ...
    let exit_code = workx_windows_sandbox::forward_sandbox_session_stdio(spawned).await;
...
async fn spawn_debug_sandbox_child(
    ...
    if !network_sandbox_policy.is_enabled() {
        cmd.env(WORKX_SANDBOX_NETWORK_DISABLED_ENV_VAR, "1");
    }
...
async fn load_debug_sandbox_config(
    ...
    load_debug_sandbox_config_with_workx_home(
        cli_overrides,
        workx_linux_sandbox_exe,
        options,
        /*workx_home*/ None,
...
async fn load_debug_sandbox_config_with_workx_home(
    cli_overrides: Vec<(String, TomlValue)>,
    workx_linux_sandbox_exe: Option<PathBuf>,
    options: DebugSandboxConfigOptions,
    ...
    // For legacy configs, `workx sandbox` historically defaulted to read-only
    ...
    let config = build_debug_sandbox_config_with_loader_overrides(
        ...
        ConfigOverrides {
            cwd: cwd.clone(),
            workx_linux_sandbox_exe: workx_linux_sandbox_exe.clone(),
            ..Default::default()
        },
        workx_home.clone(),
        managed_requirements_mode,
...
async fn build_debug_sandbox_config_with_loader_overrides(
    ...
    if let Some(workx_home) = workx_home {
        builder = builder
            .workx_home(workx_home.clone())
            .fallback_cwd(Some(workx_home));
    }
    builder.build().await
}
...
"#;
    ...
    async fn debug_sandbox_honors_active_permission_profiles() -> anyhow::Result<()> {
        ...
        let config = load_debug_sandbox_config_with_workx_home(
            Vec::new(),
    ...
    async fn debug_sandbox_honors_config_profile_loader_overrides() -> anyhow::Result<()> {
        ...
        let docs = sandbox_paths.path().join("docs");
        let private = docs.join("private");
        let profile_path = workx_home.path().join("work.config.toml");
    ...
    async fn debug_sandbox_honors_explicit_builtin_permission_profile() -> anyhow::Result<()> {
        ...
        let config = load_debug_sandbox_config_with_workx_home(
            ...
            Some(workx_home.path().to_path_buf()),
            CloudConfigBundleLoader::default(),
            /*strict_config*/ false,
        )
        .await?;
        ...
        let actual = config
            ...
            .permission_profile()
            .file_system_sandbox_policy();
        let expected = workx_protocol::models::PermissionProfile::workspace_write()
            .file_system_sandbox_policy();
        assert!(
            expected
                .entries

...
use workx_core_plugins::ConfiguredMarketplace;
use workx_core_plugins::OPENAI_BUNDLED_MARKETPLACE_NAME;
use workx_core_plugins::PluginInstallOutcome;
...
use workx_core_plugins::allowed_configured_marketplace_names;
use workx_core_plugins::installed_marketplaces::marketplace_install_root;
use workx_core_plugins::installed_marketplaces::resolve_configured_marketplace_root;
use workx_core_plugins::marketplace::MarketplaceListError;
use workx_core_plugins::marketplace::MarketplacePluginAuthPolicy;
use workx_core_plugins::marketplace::MarketplacePluginInstallPolicy;
use workx_core_plugins::marketplace::MarketplacePluginSource;
use workx_core_plugins::marketplace::find_marketplace_manifest_path;
use workx_core_plugins::remote;
...
use workx_core_plugins::remote::RemotePluginSummary;
use workx_login::AuthManager;
use workx_login::WorkxAuth;
use workx_plugin::PluginId;
use workx_plugin::validate_plugin_segment;
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "workx plugin add",
    after_help = "Examples:\n  workx plugin add sample@debug\n  workx plugin add sample --marketplace debug"
)]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "workx plugin list",
    after_help = "Examples:\n  workx plugin list\n  workx plugin list --marketplace debug\n  workx plugin list --json\n  workx plugin list --available --json"
)]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "workx plugin remove",
    after_help = "Examples:\n  workx plugin remove sample@debug\n  workx plugin remove sample --marketplace debug"
)]
...
pub async fn run_plugin_list(
    ...
    let marketplace_sources = configured_marketplace_sources(&plugins_input, workx_home.as_path());
...
pub(crate) fn configured_marketplace_sources(
    ...
    let allowed_marketplace_names =
        allowed_configured_marketplace_names(&plugins_input.config_layer_stack, workx_home);
...
pub(crate) async fn load_cli_auth_manager(config: &Config) -> Result<Arc<AuthManager>> {
    Ok(AuthManager::shared_from_config(config, /*enable_workx_api_key_env*/ true).await?)
}
...
async fn fetch_remote_marketplaces(
    ...
    let auth = context.auth.as_ref();
    if !auth.is_some_and(WorkxAuth::uses_workx_backend) {
        ensure!(
    ...
    let result = if context.plugins_input.remote_plugin_enabled {
        remote::fetch_remote_marketplaces(
            ...
            /*catalog_cache_root*/ Some(context.workx_home.as_path()),
            cache_mode,
        )
        .await
...
pub(crate) fn configured_marketplace_snapshot_issues(
    ...
    let default_install_root = marketplace_install_root(workx_home);
    ...
    for (configured_name, marketplace) in configured_marketplaces {
        ...
        match find_marketplace_manifest_path(&root) {
            ...
            None => {
                if is_implicit_system_marketplace_root(configured_name, workx_home, &root) {
                    continue;
                }
                issues.push(ConfiguredMarketplaceSnapshotIssue {
                    marketplace_name: configured_name.clone(),

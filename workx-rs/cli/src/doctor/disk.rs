...
pub(super) fn check(config: Option<&Config>, cwd: &Path) -> DoctorCheck {
    let home = config
        .map(|config| config.workx_home.as_path().to_path_buf())
        .or_else(|| find_workx_home().ok().map(Into::into))
        .or_else(|| std::env::var_os("WORKX_HOME").map(Into::into));
...
fn check_with_paths(
    ...
    for (label, path) in [("WORKX_HOME", home.as_deref()), ("worktree", Some(cwd))] {

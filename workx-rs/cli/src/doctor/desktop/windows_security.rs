...
fn event_target(value: &str) -> Option<&'static str> {
    ...
    let name = *parts.last()?;
    let package = parts.iter().any(|part| part.starts_with("openai.workx_"));
    let trusted = package || parts.windows(2).any(|pair| pair == ["openai", "workx"]);
    match name {
        "workx-windows-sandbox-setup.exe" => Some("sandbox_setup"),
        "workx-command-runner.exe" => Some("command_runner"),
        "workx.exe" => Some("workx"),
        "workx-desktop.exe" => Some("workx_desktop"),
        "chatgpt.exe" | "electron.exe" if trusted => Some("workx_desktop"),
        "rg.exe" if trusted => Some("ripgrep"),
        _ if package => Some("workx_desktop_package"),
        _ => None,
    }
}
...
fn classify(channels: &[Option<Vec<Evidence>>]) -> DoctorCheck {
    ...
    let summary = match (status, visible) {
        ...
        (CheckStatus::Ok, _) => "no locally visible recent Workx security enforcement was found",
        (CheckStatus::Warning, false) => "security event channels could not be inspected",
        (CheckStatus::Warning, true) => "recent Workx security audit or detection requires review",
        (CheckStatus::Fail, _) => "endpoint security blocked or quarantined a Workx executable",
    };
    let mut check = desktop_check("desktop.security.enforcement", status, summary).details(details);
    if status != CheckStatus::Ok {
        check = check.remediation(
            "ask your organization's security administrator to review endpoint security events and the approved Workx application policy",
        );
    }
    check
}

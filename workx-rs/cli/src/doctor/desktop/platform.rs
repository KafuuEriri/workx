...
fn installed_windows_app() -> Result<Option<InstalledApp>, DiscoveryError> {
    const PACKAGE_FAMILY: &str = "OpenAI.Workx_2p2nqsd0c76g0";
...
pub(in crate::doctor) async fn inspect_macos_bundle(
    ...
    if metadata.get("CFBundleIdentifier").and_then(Value::as_str) != Some("com.ronanxiao.workx") {
        return Ok(None);
    }

...
const SECURITY_LOG_PREDICATE: &str = concat!(
    ...
    "(eventMessage CONTAINS[c] 'workx' OR eventMessage CONTAINS[c] 'chatgpt' ",
...
fn enforcement_check(gatekeeper: Evidence, events: Evidence) -> DoctorCheck {
    ...
    let (status, summary, remedy) = if events == Evidence::Malware {
        (
            ...
            "collect the XProtect detection and ask your security administrator to review the official Workx installation",
        )
        ...
        (
            ...
            "restart your Mac, retry Workx once, and contact support if the problem returns",
        )
        ...
        (
            ...
            "check access to macos unified security logs and rerun workx doctor",
        )
    } else {

...
fn every_source_distinguishes_audits_from_blocks() {
    for (channel, audit, block) in [
        ...
        for (id, expected) in [(audit, CheckStatus::Warning), (block, CheckStatus::Fail)] {
            let events = parse_events(&fixture(id, "workx.exe", &[]), CHANNELS[channel]);
...
fn defender_distinguishes_detection_and_remediation() {
    for (id, action, expected) in [
        ...
        let events = parse_events(
            &fixture(id, "workx.exe", &[("Action Name", action)]),
            CHANNELS[0],
        );
...
#[test]
fn only_trusted_workx_executables_are_reported() {
    for (path, expected) in [
        ("workx.exe", true),
        (r"OpenAI.Workx_2p2nqsd0c76g0\ChatGPT.exe", true),
        ("evil-workx.exe", false),
        (r"C:\Other\ChatGPT.exe", false),
        (r"OpenAI.WorkxEvil_1\ChatGPT.exe", false),
    ] {
...
fn evidence_is_bounded_redacted_and_correctly_classified() {
    ...
    let audits = fixture(/*id*/ 1122, "workx.exe", &[]).repeat(MAX_RENDERED_EVENTS);
    let secret = "private-customer-secret";
    let block = fixture(/*id*/ 1121, "workx.exe", &[("User", secret)]);

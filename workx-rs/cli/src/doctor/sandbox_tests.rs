...
async fn reports_missing_linux_sandbox_helper() {
    let home = tempfile::tempdir().expect("create Workx home");
    ...
    let arg0_paths = Arg0DispatchPaths {
        workx_linux_sandbox_exe: Some(home.path().join("missing-linux-helper")),
        ..Default::default()
    };
...
async fn reports_refresh_failure_after_setup_completed() {
    let home = tempfile::tempdir().expect("create Workx home");
    let version = workx_windows_sandbox::SETUP_VERSION;
    for (path, contents) in [
        (
            ".sandbox/setup_marker.json",
            serde_json::json!({
                "version": version,
                "offline_username": workx_windows_sandbox::OFFLINE_USERNAME,
                "online_username": workx_windows_sandbox::ONLINE_USERNAME,
            }),
        ),
        (
            ...
            serde_json::json!({
                "version": version,
                "offline": {
                    "username": workx_windows_sandbox::OFFLINE_USERNAME,
                    "password": "",
                },
                "online": {
                    "username": workx_windows_sandbox::ONLINE_USERNAME,
                    "password": "",
                },
            }),
        ),
    ...
    assert!(workx_windows_sandbox::sandbox_setup_is_complete(
        home.path()
    ));
    workx_windows_sandbox::write_setup_error_report(
        home.path(),
        &workx_windows_sandbox::SetupErrorReport {
            code: workx_windows_sandbox::SetupErrorCode::HelperFirewallPolicyIneffective,
...
async fn reports_malformed_setup_failure_reports() {
    let home = tempfile::tempdir().expect("create Workx home");
    let path = workx_windows_sandbox::setup_error_path(home.path());

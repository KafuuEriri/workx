...
pub(super) fn endpoint_check(inspection: EndpointInspection) -> DoctorCheck {
    ...
    let targets = if cfg!(target_os = "windows") {
        "signed Workx app; workx.exe; workx-windows-sandbox-setup.exe; workx-command-runner.exe; workx-code-mode-host.exe"
    } else if cfg!(target_os = "macos") {
        "signing team 2DC432GLL2 plus the installed Workx app identity, signed workx agent, and required helpers"
    } else {
        "verified Workx app, workx agent, and required helpers"
    };
    let mut check = DoctorCheck::new(
        "security.endpoint",
        "security",
        CheckStatus::Warning,
        "endpoint protection detected; Workx exclusions are unverified",
    )
    .detail(format!("endpoint products: {}", products.join(", ")))
    .detail(format!("exclusion targets: {targets}"))
    .detail("Workx exclusions: not verified")
    .remediation(
        "ask your security administrator to verify Workx exclusions and required helper allowances",
    );
    ...
    for product in products {
        let remedy = match product {
            "CrowdStrike Falcon" => {
                "CrowdStrike Falcon: Add a certificate or IOA exclusion for Workx. If sensor overhead continues, exclude the Workx agent from sensor visibility. Keep monitoring descendant processes."
            }
            "BeyondTrust Privilege Management" => {
                "BeyondTrust: Remove Workx from application blocking rules. Add allow rules for Workx helper executables. Do not grant administrator privileges."
            }
            "Microsoft Defender" => {
                "Microsoft Defender: Add a certificate or executable-path exclusion for Workx and its helpers. If Attack Surface Reduction blocks Workx, add a rule exclusion. If Controlled Folder Access blocks Workx, allow the app."
            }
            "SentinelOne" => {
                "SentinelOne: Add a signer, file-hash, or executable-path exclusion for Workx and its helpers."
            }
            "Jamf Protect" => {
                "Jamf Protect: Add an Override Threat Prevention exception for Workx app and helper signing identities. If analytics cause delays, add an Ignore System Events for Analytics exception."
            }
            _ => {
                "Add an exclusion for Workx and its helpers. Use the endpoint product instructions."
            }
        };
        check = check.issue(
            DoctorIssue::new(
                CheckStatus::Warning,
                format!("{product} can interfere with Workx. Verify Workx exclusions."),
            )
            .measured("not verified")
            .expected("Workx application and helper exclusions")
            .remedy(remedy)
            .field("Workx exclusions"),
        );
    }

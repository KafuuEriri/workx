...
fn write_footer(out: &mut String, options: HumanOutputOptions) {
    if options.show_details {
        ...
        let _ = writeln!(
            ...
            "{}",
            dim(
                "Run workx doctor without --summary for detailed diagnostics.",
                options
            )
        );
        let _ = writeln!(
...
fn header_suffix(report: &DoctorReport) -> String {
    let version = format!("v{}", report.workx_version);
    report
        .checks
        .iter()
...
fn update_note(check: &DoctorCheck, report: &DoctorReport) -> Option<DoctorNote> {
    ...
    let mut parenthetical = format!("current {}", report.workx_version);
...
mod tests {
    ...
    fn sample_report() -> DoctorReport {
        let checks = vec![
            ...
            .detail("terminal title project value: workx"),
            DoctorCheck::new(
                "state.paths",
                "state",
                CheckStatus::Ok,
...
Notes
   ...
   ✗ auth         token expired - Run `workx login`.
─────────────────────────────────────────────────────────────
...
",
    ...
    fn render_human_report_snapshot_covers_environment_rows() {
        ...
        *targets = "exclusion targets: verified Workx app and required helpers".into();
        ...
        report.checks.extend([
            ...
            .detail("running: true")
            .detail("log directory: $HOME/Library/Logs/com.ronanxiao.workx"),
            DoctorCheck::new(
...
Notes
   [!!] terminal     narrow terminal
   [XX] auth         token expired - Run `workx login`.
-------------------------------------------------------------
...
Environment
  ...
  [!!] terminal     narrow terminal
  [ok] title        default | project workx
...
",
    ...
    fn detail_value_colors_inline_statuses_and_low_signal_values() {
        let rendered = detail_value(
            "npm: no · commit unknown · integrity ok · ~/code/workx/target/debug/workx · <redacted>",
        ...
        assert!(rendered.contains("\u{1b}[38;5;117m~/code/workx/target/debug/workx"));
    ...
    fn redact_detail_preserves_env_var_names() {
        assert_eq!(
            redact_detail("auth env vars present: OPENAI_API_KEY, WORKX_API_KEY"),
            "auth env vars present: OPENAI_API_KEY, WORKX_API_KEY"
        );
    }

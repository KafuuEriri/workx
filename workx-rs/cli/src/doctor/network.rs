...
#[cfg(target_os = "macos")]
use workx_http_client::MacosSystemProxyConfiguration;
use workx_http_client::RouteAwareClientPool;
use workx_http_client::RouteAwareRequestError;
use workx_http_client::RouteFailureClass;
#[cfg(target_os = "macos")]
use workx_http_client::macos_system_proxy_configuration;
...
pub(super) fn check(config: Option<&Config>) -> DoctorCheck {
    ...
    for name in ["WORKX_CA_CERTIFICATE", "SSL_CERT_FILE"] {
...
pub(super) fn with_system_proxy_remediation(
        ...
        })
    {
        check.remediation = Some(
            "A macOS system proxy is configured but unused. If your organization requires it, ask your administrator whether to enable the under-development feature with `workx features enable respect_system_proxy`."
                .to_string(),
        );
    }
    check
...
pub(super) async fn probe_status(
    ...
    let response = if env::var("WORKX_SANDBOX").as_deref() == Ok("seatbelt") {

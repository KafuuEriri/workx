...
#[cfg(target_os = "windows")]
const WINDOWS_SETUP_REMEDIATION: &str = "run workx sandbox setup --elevated --user <end-user> --workx-home <authoritative-home> from an elevated shell";
...
pub(super) fn sandbox_check(config: &Config, arg0_paths: &Arg0DispatchPaths) -> DoctorCheck {
    ...
    push_path_detail(
        &mut details,
        "workx-linux-sandbox helper",
        arg0_paths.workx_linux_sandbox_exe.as_deref(),
    );
    push_path_detail(
        &mut details,
    ...
    if let Some(helper) = arg0_paths.workx_linux_sandbox_exe.as_deref()
        && !helper.exists()
    {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        if elevated {
            let home = config.workx_home.as_path();
            let path = workx_windows_sandbox::setup_error_path(home);
            let report = match fs::metadata(&path) {
                Err(error) if error.kind() == io::ErrorKind::NotFound => Ok(None),
                Ok(metadata) if metadata.is_file() && metadata.len() <= 64 * 1024 => {
                    workx_windows_sandbox::read_setup_error_report(home)
            ...
            if report.is_none() && workx_windows_sandbox::sandbox_setup_is_complete(home) {
                check
                    .details
                    .push("sandbox provisioning: complete".to_string());

                for username in [
                    workx_windows_sandbox::OFFLINE_USERNAME,
                    workx_windows_sandbox::ONLINE_USERNAME,
                ] {
                ...
                if let Some(report) = report {
                    let remediation = match report.code {
                        ...
                        | SetupErrorCode::HelperReadAclHelperSpawnFailed => {
                            "repair the installed Workx helpers or ask IT to allow their execution"
                        }
                        ...
                        | SetupErrorCode::HelperSandboxLockFailed => {
                            "rerun elevated setup for the authoritative Workx home or ask IT"
                        }
                        ...
                        | SetupErrorCode::HelperFirewallRuleVerifyFailed => {
                            "ask IT to allow Workx sandbox rules in managed Windows Firewall policy"
                        }
                        SetupErrorCode::OrchestratorPayloadSerializeFailed
                        | SetupErrorCode::HelperRequestArgsFailed
                        | SetupErrorCode::HelperUnknownError => {
                            "repair or reinstall the Workx CLI from an approved distribution"
                        }
                    };
                    check
                        .details

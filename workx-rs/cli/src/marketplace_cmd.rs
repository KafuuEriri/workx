...
use workx_core_plugins::PluginMarketplaceUpgradeOutcome;
...
use workx_core_plugins::marketplace::marketplace_root_dir;
use workx_core_plugins::marketplace_add::MarketplaceAddOutcome;
use workx_core_plugins::marketplace_add::MarketplaceAddRequest;
use workx_core_plugins::marketplace_add::add_marketplace;
use workx_core_plugins::marketplace_remove::MarketplaceRemoveOutcome;
use workx_core_plugins::marketplace_remove::MarketplaceRemoveRequest;
...
#[derive(Debug, Parser)]
#[command(bin_name = "workx plugin marketplace")]
...
enum MarketplaceSubcommand {
    ...
    /// List plugin marketplaces Workx is currently considering and their roots.
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "workx plugin marketplace add",
    after_help = "Examples:\n  workx plugin marketplace add ./path/to/marketplace\n  workx plugin marketplace add owner/repo --ref main\n  workx plugin marketplace add https://github.com/owner/repo --sparse plugins/foo"
)]
...
#[derive(Debug, Parser)]
#[command(bin_name = "workx plugin marketplace list")]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "workx plugin marketplace upgrade",
    after_help = "Examples:\n  workx plugin marketplace upgrade\n  workx plugin marketplace upgrade debug"
)]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "workx plugin marketplace remove",
    after_help = "Example:\n  workx plugin marketplace remove debug"
)]
...
async fn run_list(config: Config, args: ListMarketplaceArgs) -> Result<()> {
    ...
    if args.json {
        let marketplace_sources =
            configured_marketplace_sources_by_root(config.workx_home.as_path(), &plugins_input);
        let output =
...
impl JsonMarketplaceListOutput {
    fn from_marketplaces(
        marketplaces: Vec<workx_core_plugins::marketplace::Marketplace>,
...
fn configured_marketplace_sources_by_root(
    ...
    let marketplace_sources = configured_marketplace_sources(plugins_input, workx_home);
...
async fn run_upgrade(
    ...
    builder: workx_core::config::ConfigBuilder,
) -> Result<()> {
    ...
    let reload_config: workx_core_plugins::ConfigLayerReload = std::sync::Arc::new(move || {
        runtime

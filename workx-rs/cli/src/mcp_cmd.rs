...
use workx_rmcp_client::delete_oauth_tokens;
use workx_rmcp_client::perform_oauth_login;
use workx_rmcp_client::resolve_mcp_oauth_callback_url;
use workx_utils_cli::CliConfigOverrides;
use workx_utils_cli::format_env_display;
...
/// - `add`    — add a server launcher entry to `~/.workx/config.toml`
...
#[derive(Debug, clap::Parser)]
#[command(override_usage = "workx mcp add [OPTIONS] <NAME> (--url <URL> | -- <COMMAND>...)")]
pub struct AddArgs {
...
async fn perform_oauth_login_retry_without_scopes(
    name: &str,
    url: &str,
    store_mode: workx_config::types::OAuthCredentialsStoreMode,
    keyring_backend_kind: workx_config::types::AuthKeyringBackendKind,
...
async fn run_add(config_overrides: &CliConfigOverrides, add_args: AddArgs) -> Result<()> {
    ...
    let mut servers = load_global_mcp_servers(&workx_home)
        .await
        .with_context(|| format!("failed to load MCP servers from {}", workx_home.display()))?;
    ...
    ConfigEditsBuilder::new(&workx_home)
        ...
        .apply()
        .await
        .with_context(|| format!("failed to write MCP servers to {}", workx_home.display()))?;
    ...
    match login_support {
        ...
        McpOAuthLoginSupport::Unknown(_) => println!(
            "MCP server may or may not require login. Run `workx mcp login {name}` to login."
        ),
    }
...
async fn run_list(config: &Config, list_args: ListArgs) -> Result<()> {
    ...
    if entries.is_empty() {
        println!("No MCP servers configured yet. Try `workx mcp add my-tool -- my-command`.");
        return Ok(());
    }
...
async fn run_get(config: &Config, get_args: GetArgs) -> Result<()> {
    ...
    println!("  remove: workx mcp remove {}", get_args.name);

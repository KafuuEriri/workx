...
//! interactive-session layers. Direct `workx login` intentionally does less: it preserves the
...
//! into a one-shot CLI command while still producing a durable `workx-login.log` artifact that
...
use workx_login::CLIENT_ID;
use workx_login::ServerOptions;
use workx_login::is_workload_identity_selected;
use workx_login::login_with_access_token;
use workx_login::login_with_api_key;
use workx_login::logout_with_revoke;
use workx_login::run_device_code_login;
...
/// Installs a small file-backed tracing layer for direct `workx login` flows.
///
...
/// command produce a durable `workx-login.log` artifact without coupling it to the TUI's broader
/// telemetry and feedback initialization.
fn init_login_file_logging(config: &Config) -> Option<WorkerGuard> {
    let log_dir = match workx_core::config::log_dir(config) {
        Ok(log_dir) => log_dir,
        Err(err) => {
    ...
    let log_path = log_dir.join("workx-login.log");
    ...
    let env_filter = EnvFilter::try_from_default_env()
        .unwrap_or_else(|_| EnvFilter::new("workx_cli=info,workx_core=info,workx_login=info"));
    ...
    // Direct `workx login` otherwise relies on ephemeral stderr and browser output.
...
fn print_login_server_start(actual_port: u16, auth_url: &str) {
    eprintln!(
        "Starting local login server on http://localhost:{actual_port}.\nIf your browser did not open, navigate to this URL to authenticate:\n\n{auth_url}\n\nOn a remote or headless machine? Use `workx login --device-auth` instead."
    );
}
...
pub fn read_api_key_from_stdin() -> String {
    read_stdin_secret(
        "--with-api-key expects the API key on stdin. Try piping it, e.g. `printenv OPENAI_API_KEY | workx login --with-api-key`.",
...
pub fn read_access_token_from_stdin() -> String {
    read_stdin_secret(
        "--with-access-token expects the access token on stdin. Try piping it, e.g. `printenv WORKX_ACCESS_TOKEN | workx login --with-access-token`.",
...
/// `workx login` working in environments where device-code may be disabled/feature-gated.
...
pub async fn run_login_status(cli_config_overrides: CliConfigOverrides) -> ! {
    ...
    if is_workload_identity_selected() {
        match AuthManager::shared_from_config(&config, /*enable_workx_api_key_env*/ false).await {
            Ok(_) => {
    ...
    match auth_config
        .load_auth(/*enable_workx_api_key_env*/ false)
        .await
    {
...
mod tests {
    ...
    async fn clears_existing_auth_before_login() {
        let workx_home = tempdir().expect("create temporary Workx home");
        login_with_api_key(
            workx_home.path(),
            "sk-existing",

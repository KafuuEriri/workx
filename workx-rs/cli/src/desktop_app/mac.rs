...
const WORKX_BUNDLE_IDENTIFIER: &str = "com.ronanxiao.workx";
const OPENAI_APPLE_TEAM_IDENTIFIER: &str = "2DC432GLL2";
const WORKX_DMG_URL_ARM64: &str =
    "https://github.com/RonanXiao/workx/releases/latest/download/Workx.dmg";
const WORKX_DMG_URL_X64: &str =
    "https://github.com/RonanXiao/workx/releases/latest/download/Workx-latest-x64.dmg";

pub async fn run_mac_app_open_or_install(
    workspace: PathBuf,
    download_url_override: Option<String>,
) -> anyhow::Result<()> {
    if let Some(app_path) = find_existing_workx_app_path(&workx_app_search_dirs()) {
        eprintln!(
            "Opening Desktop app at {app_path}...",
            app_path = app_path.display()
        );
        open_workx_app(&app_path, &workspace).await?;
        return Ok(());
    }
    ...
    let installed_app = download_and_install_workx_to_user_applications(&download_url)
        .await
    ...
    open_workx_app(&installed_app, &workspace).await?;
    Ok(())
}
...
fn find_existing_workx_app_path(applications_dirs: &[PathBuf]) -> Option<PathBuf> {
    applications_dirs
        .iter()
        .flat_map(|dir| ["ChatGPT.app", "Workx.app"].map(|app_name| dir.join(app_name)))
        .find(|candidate| is_workx_app_bundle(candidate))
}

fn workx_app_search_dirs() -> Vec<PathBuf> {
...
fn is_workx_app_bundle(app_path: &Path) -> bool {
    if !app_path.is_dir() {
        return false;
    }
    ...
    std::process::Command::new("/usr/bin/plutil")
        ...
        .is_ok_and(|output| {
            output.status.success()
                && String::from_utf8_lossy(&output.stdout).trim() == WORKX_BUNDLE_IDENTIFIER
        })
}

async fn open_workx_app(app_path: &Path, workspace: &Path) -> anyhow::Result<()> {
    verify_workx_app_bundle(app_path).await?;
    eprintln!(
        "Opening workspace {workspace}...",
        workspace = workspace.display()
    );
    let url = workx_new_thread_url(workspace);
...
async fn verify_workx_app_bundle(app_path: &Path) -> anyhow::Result<()> {
    let requirement = format!(
        "identifier \"{WORKX_BUNDLE_IDENTIFIER}\" and anchor apple generic and certificate leaf[subject.OU] = \"{OPENAI_APPLE_TEAM_IDENTIFIER}\""
    );
    ...
    anyhow::bail!(
        "Desktop app at {} failed OpenAI signature verification (team {OPENAI_APPLE_TEAM_IDENTIFIER}, bundle {WORKX_BUNDLE_IDENTIFIER}): {}",
        app_path.display(),
        String::from_utf8_lossy(&output.stderr).trim()
    );
}

fn workx_new_thread_url(workspace: &Path) -> String {
    let workspace = workspace.as_os_str().to_string_lossy();
    let mut serializer = url::form_urlencoded::Serializer::new(String::new());
    serializer.append_pair("path", workspace.as_ref());
    let query = serializer.finish();
    format!("workx://threads/new?{query}")
}

async fn download_and_install_workx_to_user_applications(dmg_url: &str) -> anyhow::Result<PathBuf> {
    let temp_dir = Builder::new()
        .prefix("workx-app-installer-")
        .tempdir()
        .context("failed to create temp dir")?;
    let tmp_root = temp_dir.path().to_path_buf();
    let _temp_dir = temp_dir;

    let dmg_path = tmp_root.join("Workx.dmg");
    ...
    let result = async {
        let app_in_volume = find_workx_app_in_mount(&mount_point)
            .context("failed to locate Workx.app in mounted dmg")?;
        verify_workx_app_bundle(&app_in_volume)
            .await
            .context("refusing to install an unverified Desktop app")?;
        install_workx_app_bundle(&app_in_volume).await
    }
    .await;
...
async fn install_workx_app_bundle(app_in_volume: &Path) -> anyhow::Result<PathBuf> {
    for applications_dir in candidate_applications_dirs()? {
        ...
        let dest_app = applications_dir.join("Workx.app");
        if dest_app.is_dir() {
            return Ok(dest_app);
        }
        ...
        match copy_app_bundle(app_in_volume, &dest_app).await {
            ...
            Err(err) => {
                eprintln!(
                    "warning: failed to install Workx.app to {applications_dir}: {err}",
                    applications_dir = applications_dir.display()
                );
            }
        }
    ...
    anyhow::bail!("failed to install Workx.app to any applications directory");
}
...
fn find_workx_app_in_mount(mount_point: &Path) -> anyhow::Result<PathBuf> {
    let direct = mount_point.join("Workx.app");
    if direct.is_dir() {
        return Ok(direct);
    }
...
#[cfg(test)]
mod tests {
    use super::find_existing_workx_app_path;
    use super::open_workx_app;
    use super::parse_hdiutil_attach_mount_point;
    use super::verify_workx_app_bundle;
    use super::workx_new_thread_url;
    ...
    #[test]
    fn finds_chatgpt_app_with_workx_bundle_identifier() {
        let temp_dir = tempfile::tempdir().expect("create temp dir");
        let app_path = temp_dir.path().join("ChatGPT.app");
        write_app_bundle(&app_path, "com.ronanxiao.workx");

        assert_eq!(
            find_existing_workx_app_path(&[temp_dir.path().to_path_buf()]),
            Some(app_path)
        );
    }
    ...
    fn ignores_classic_chatgpt_app() {
        ...
        let workx_app_path = temp_dir.path().join("Workx.app");
        write_app_bundle(&workx_app_path, "com.ronanxiao.workx");
    ...
    #[tokio::test]
    async fn rejects_unsigned_app_with_workx_bundle_identifier() {
        let temp_dir = tempfile::tempdir().expect("create temp dir");
        let app_path = temp_dir.path().join("Workx.app");
        write_app_bundle(&app_path, "com.ronanxiao.workx");

        let err = verify_workx_app_bundle(&app_path)
            .await
    ...
    async fn rejects_valid_signature_without_openai_signing_identity() {
        ...
        let app_path = temp_dir.path().join("Workx.app");
        let contents_path = app_path.join("Contents");
        let executable_path = contents_path.join("MacOS/Workx");
        ...
        fs::write(
            contents_path.join("Info.plist"),
            r#"<?xml version="1.0"?><plist version="1.0"><dict><key>CFBundleIdentifier</key><string>com.ronanxiao.workx</string><key>CFBundleExecutable</key><string>Workx</string></dict></plist>"#,
        )
        .expect("write Info.plist");
    ...
    #[tokio::test]
    async fn refuses_to_launch_unsigned_existing_workx_app() {
        let temp_dir = tempfile::tempdir().expect("create temp dir");
        let app_path = temp_dir.path().join("Workx.app");
        write_app_bundle(&app_path, "com.ronanxiao.workx");

        let err = open_workx_app(&app_path, temp_dir.path())
            .await
    ...
    fn parses_mount_point_from_tab_separated_hdiutil_output() {
        let output = "/dev/disk2s1\tApple_HFS\tWorkx\t/Volumes/Workx\n";
        assert_eq!(
    ...
    fn parses_mount_point_with_spaces() {
        let output = "/dev/disk2s1\tApple_HFS\tWorkx Installer\t/Volumes/Workx Installer\n";
        assert_eq!(
            parse_hdiutil_attach_mount_point(output).as_deref(),
            Some("/Volumes/Workx Installer")
        );
    }

    #[test]
    fn workx_new_thread_url_encodes_workspace_path() {
        let url = url::Url::parse(&workx_new_thread_url(Path::new("/tmp/workx workspace/#1")))
        ...
        assert_eq!(
            ...
            (
                ...
                "/new".to_string(),
                vec![("path".to_string(), "/tmp/workx workspace/#1".to_string())],
            )
        );
    }
}

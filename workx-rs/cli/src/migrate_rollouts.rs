...
use workx_thread_store::RolloutMigrationMode;
use workx_thread_store::RolloutMigrationOptions;
use workx_thread_store::RolloutMigrationProgress;
use workx_thread_store::RolloutMigrationReport;
use workx_thread_store::RolloutMigrationStatus;
...
pub(crate) async fn run(
    ...
    workx_core::otel_init::record_process_start(otel.as_ref(), "workx_migrate_rollouts");
    ...
    let state_db = if mode == RolloutMigrationMode::Apply {
        Some(
            workx_rollout::state_db::try_init(&config)
                .await
...
fn print_human_report(
    ...
    if mode == RolloutMigrationMode::DryRun && counts.eligible > 0 {
        println!("Run `workx migrate-rollouts --apply` to migrate eligible sessions.");
    }
...
fn print_outcome(outcome: &workx_thread_store::RolloutMigrationOutcome) {
...
async fn thread_storage_bytes(workx_home: &Path, thread_history_db_path: &Path) -> io::Result<u64> {
    let mut bytes = 0_u64;
    let mut directories = vec![
        workx_home.join(workx_rollout::SESSIONS_SUBDIR),
        workx_home.join(workx_rollout::ARCHIVED_SESSIONS_SUBDIR),
    ];

...
pub(crate) struct SandboxSetupCommand {
    ...
    /// Windows user that will run Workx after managed deployment.
    #[arg(
        long = "user",
        value_name = "USER",
    ...
    /// Use the current Windows user as the Workx user.
    #[arg(
        long = "current-user",
    ...
    /// WORKX_HOME for the Workx user. Required with --user.
    #[arg(long = "workx-home", value_name = "DIR")]
...
impl SandboxSetupCommand {
    fn setup_level(&self) -> anyhow::Result<SandboxSetupLevel> {
        if self.elevated_sandbox_level {
            ...
            anyhow::bail!("`workx sandbox setup` currently requires --elevated");
        }
    }
}
...
async fn run_elevated(
    ...
    let config = ConfigBuilder::default()
        .workx_home(identity.workx_home.clone())
        .fallback_cwd(Some(identity.workx_home.clone()))
        .loader_overrides(super::loader_overrides_for_profile_at_workx_home(
        ...
        .build()
        .await
        .context("failed to load target user's Workx config for sandbox provisioning")?;

    workx_core::windows_sandbox::run_elevated_provisioning_setup(
        identity.workx_home.as_path(),
        identity.real_user.as_str(),
        config.permissions.network.as_ref(),
    )?;
    ConfigEditsBuilder::new(identity.workx_home.as_path())
...
fn resolve_sandbox_setup_identity(
    ...
    if cmd.current_user {
        ...
        let workx_home = match cmd.workx_home.clone() {
            Some(workx_home) => workx_home,
            None => find_workx_home()?.to_path_buf(),
        };
    ...
    let workx_home = cmd
        .workx_home
        .clone()
        .ok_or_else(|| anyhow::anyhow!("--workx-home is required with --user"))?;
...
mod tests {
    ...
    fn parses_managed_user_identity() {
        ...
        assert_eq!(
            command.workx_home.as_deref(),
            Some(std::path::Path::new(r"C:\Users\alice\.workx"))
        );
    }
    ...
    #[test]
    fn requires_workx_home_for_managed_user() {
        let err =
    ...
    fn parses_setup_from_sandbox_command_args() {
        let command = parse_setup_command(&[
            ...
            "--workx-home".to_string(),
            r"C:\Users\alice\.workx".to_string(),
        ])
        .expect("parse")

...
use workx_exec_server::FsCloseParams;
use workx_exec_server::FsOpenParams;
use workx_exec_server::FsReadBlockParams;
use workx_exec_server::FsReadBlockResponse;
...
async fn file_reads_reject_named_pipes() -> Result<()> {
    ...
    let read_path = format!(r"\\.\pipe\workx-fs-read-{}", Uuid::new_v4());
    ...
    let stream_path = format!(r"\\.\pipe\workx-fs-stream-{}", Uuid::new_v4());
...
#[cfg(unix)]
fn read_only_sandbox(path: std::path::PathBuf) -> workx_exec_server::FileSystemSandboxContext {

...
use workx_exec_server::ExecBackend;
#[cfg(unix)]
use workx_exec_server::ExecEnvPolicy;
use workx_exec_server::ExecOutputStream;
use workx_exec_server::ExecParams;
use workx_exec_server::ExecProcess;
use workx_exec_server::ExecProcessEvent;
#[cfg(any(unix, windows))]
use workx_exec_server::FileSystemSandboxContext;
use workx_exec_server::ProcessId;
use workx_exec_server::ProcessSignal;
use workx_exec_server::ReadResponse;
#[cfg(unix)]
use workx_exec_server::ShellInfo;
#[cfg(unix)]
use workx_exec_server::ShellSnapshotRequest;
use workx_exec_server::StartedExecProcess;
use workx_exec_server::WriteStatus;
#[cfg(unix)]
use workx_network_proxy::NetworkProxyConfig;
#[cfg(unix)]
use workx_network_proxy::RemoteNetworkProxyConfig;
#[cfg(unix)]
...
async fn shell_snapshot_v2_filters_profile_exports_and_stays_in_memory(
    ...
    if use_sandbox
        && let Some(warning) =
            workx_sandboxing::system_bwrap_warning(&PermissionProfile::read_only())
    {
    ...
    let command = format!(
        "export PATH='{}':\"$PATH\"; {command_prefix}printf '|%s|%s|%s|%s|%s|%s' \"$PROFILE_ALLOWED\" \"${{PROFILE_SECRET-missing}}\" \"${{PROFILE_DENIED-missing}}\" \"$PATH\" \"${{__WORKX_SHELL_SNAPSHOT_STATE_0-missing}}\" \"${{__WORKX_SHELL_SNAPSHOT_STATE_1-missing}}\"",
    ...
    if let Some(server) = context._server {
        assert!(!server.workx_home().join("shell_snapshots").exists());
    }
    Ok(())
}
...
async fn shell_snapshot_v2_remote_managed_proxy_uses_prepared_execution_context() -> Result<()> {
    ...
    std::fs::write(
        ...
        "printf '%s\\n' \"$HTTP_PROXY\" >> \"$HOME/captures\"\ntest \"$WORKX_NETWORK_PROXY_ACTIVE\" = 1 || exit 41\nexport PROFILE_ALLOWED=profile\nprofile_helper() { printf helper; }\n",
    )?;
    let policy = ExecEnvPolicy {
        inherit: ShellEnvironmentPolicyInherit::All,
        ignore_default_excludes: false,
        ...
        let started = context
            ...
            .start(ExecParams {
                ...
                argv: vec![
                    "/bin/bash".to_string(),
                    "-lc".to_string(),
                    "profile_helper; printf '|%s|%s|%s' \"$PROFILE_ALLOWED\" \"$WORKX_NETWORK_PROXY_ACTIVE\" \"$HTTP_PROXY\"".to_string(),
                ],
                cwd: cwd.clone(),
                env_policy: Some(policy.clone()),
                shell_snapshot: Some(ShellSnapshotRequest {
...
async fn shell_snapshot_v2_capture_failure_falls_back_and_retries(
    ...
    if use_remote
        && let Some(warning) =
            workx_sandboxing::system_bwrap_warning(&PermissionProfile::workspace_write())
    {
        eprintln!("skipping sandbox test: {warning}");
        return Ok(());
    }
...
async fn remote_sandboxed_process_preserves_custom_arg0() -> Result<()> {
    if let Some(warning) = workx_sandboxing::system_bwrap_warning(&PermissionProfile::read_only()) {
    ...
    let session = context
        ...
        .start(ExecParams {
            ...
            argv: vec![
                "/bin/sh".to_string(),
                "-c".to_string(),
                "printf '%s' \"$0\"; if /bin/cat \"$WORKX_TEST_DENIED_FILE\" >/dev/null 2>&1; then exit 42; fi"
                    .to_string(),
            ],
            cwd,
            shell_snapshot: None,
            ...
            env: HashMap::from([
                ...
                (
                    "WORKX_TEST_DENIED_FILE".to_string(),
...
async fn remote_tty_process_uses_configured_sandbox_helper_with_hostile_path() -> Result<()> {
    ...
    let file = workspace.path().join("allowed.txt");
    std::fs::write(&file, b"allowed")?;
    let hostile_helper = workspace.path().join("workx-linux-sandbox");

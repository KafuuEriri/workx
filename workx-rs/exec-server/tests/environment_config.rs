...
use workx_exec_server::EnvironmentConfigLayer;
use workx_exec_server::EnvironmentConfigLayerStack;
use workx_exec_server::EnvironmentConfigReadParams;
use workx_exec_server::EnvironmentConfigReadResponse;
...
async fn remote_environment_reads_projected_executor_config() -> anyhow::Result<()> {
    ...
    let workx_home =
        AbsolutePathBuf::from_absolute_path(std::fs::canonicalize(server.workx_home())?)?;
    let config_file = workx_home.join(CONFIG_TOML_FILE);
    let project = workx_home.join("project");
    let dot_workx = project.join(".workx");
    tokio::fs::create_dir_all(dot_workx.as_path()).await?;
    ...
    .await?;
    tokio::fs::write(
        dot_workx.join(CONFIG_TOML_FILE).as_path(),
        r#"
[future_environment]
...
async fn environment_config_read_rejects_empty_selectors() -> anyhow::Result<()> {
    ...
    for (config_paths, expected_message) in [
        ...
        let error = environment
            .read_environment_config(EnvironmentConfigReadParams {
                cwd: PathUri::from_abs_path(&workx_home),
                config_paths,
                requirements_paths: Vec::new(),
            })
            .await

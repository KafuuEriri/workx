...
pub(crate) struct ExecServerHarness {
    workx_home: TempDir,
    _helper_paths: TestWorkxHelperPaths,
    child: Child,
...
pub(crate) struct TestWorkxHelperPaths {
    pub(crate) workx_exe: PathBuf,
    pub(crate) workx_linux_sandbox_exe: Option<PathBuf>,
}
...
pub(crate) fn test_workx_helper_paths() -> anyhow::Result<TestWorkxHelperPaths> {
    let (helper_binary, workx_linux_sandbox_exe) = super::current_test_binary_helper_paths()?;
    Ok(TestWorkxHelperPaths {
        workx_exe: helper_binary,
...
{
    let helper_paths = test_workx_helper_paths()?;
    let workx_home = TempDir::new()?;
    let mut child = Command::new(&helper_paths.workx_exe);
    ...
    child.env("WORKX_HOME", workx_home.path());
    child.envs(env);
...
impl ExecServerHarness {
    pub(crate) fn workx_home(&self) -> &std::path::Path {

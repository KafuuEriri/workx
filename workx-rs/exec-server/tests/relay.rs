...
use workx_exec_server_protocol::ProcessSandboxType;
use workx_http_client::HttpClientFactory;
use workx_http_client::OutboundProxyPolicy;
use workx_http_client::cache_system_proxy_route_for_test;
...
async fn pending_noise_environment_connects_and_reconnects_after_ready_report() -> Result<()> {
    ...
    let (workx_exe, workx_linux_sandbox_exe) = common::current_test_binary_helper_paths()?;
    let runtime_paths = ExecServerRuntimePaths::new(workx_exe, workx_linux_sandbox_exe)?;

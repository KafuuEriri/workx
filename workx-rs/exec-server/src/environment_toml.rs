...
pub(crate) fn environment_provider_from_workx_home(
    workx_home: &Path,
) -> Result<Box<dyn EnvironmentProvider>, ExecServerError> {
    let path = workx_home.join(ENVIRONMENTS_TOML_FILE);
...
mod tests {
        ...
        let provider = TomlEnvironmentProvider::new(EnvironmentsToml {
            ...
            environments: vec![
                ...
                EnvironmentToml {
                    ...
                    args: Some(vec![
                        "dev".to_string(),
                        "workx exec-server --listen stdio".to_string(),
                    ]),
    ...
    fn load_environments_toml_reads_root_environment_list() {
        ...
        let path = workx_home.path().join(ENVIRONMENTS_TOML_FILE);
        std::fs::write(
            &path,
            r#"
default = "ssh-dev"
...
id = "ssh-dev"
program = "ssh"
args = ["dev", "workx exec-server --listen stdio"]
cwd = "/tmp"
[environments.env]
WORKX_LOG = "debug"
"#,
...
"#,
        ...
        for (index, (contents, expected)) in cases.into_iter().enumerate() {
            let path = workx_home.path().join(format!("environments-{index}.toml"));
    ...
    #[tokio::test]
    async fn environment_provider_from_workx_home_uses_present_environments_file() {
        let workx_home = tempdir().expect("tempdir");
        std::fs::write(
            workx_home.path().join(ENVIRONMENTS_TOML_FILE),
            r#"
default = "none"
include_local = false
"#,
        ...
        let provider =
            environment_provider_from_workx_home(workx_home.path()).expect("environment provider");
    ...
    #[tokio::test]
    async fn environment_provider_from_workx_home_falls_back_when_file_is_missing() {
        let workx_home = tempdir().expect("tempdir");

...
async fn snapshot_failure_retries_are_bounded_and_single_flight(
    ...
    let params = ExecParams {
        ...
        cwd: workx_utils_path_uri::PathUri::from_host_native_path(home.path())?,
        env: HashMap::from([
            (
                "HOME".to_string(),
        ...
        .flat_map(opentelemetry_sdk::metrics::data::ScopeMetrics::metrics)
    {
        match metric.name() {
            ...
            "workx.shell_snapshot.duration_ms" => {
...
fn snapshot_caches_only_unmanaged_proxy_state() {
    for (exports, expected) in [
        (
            "PROFILE_ALLOWED=profile\0HTTP_PROXY=http://127.0.0.1:4321\0WORKX_NETWORK_PROXY_ACTIVE=1\0WORKX_NETWORK_PROXY_CREDENTIAL_BROKER_ACTIVE=1\0",

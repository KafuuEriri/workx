...
impl JsonRpcConnectionEvent {
    pub(crate) fn message(message: JSONRPCMessage) -> Self {
        ...
        if let Some(trace) = &request.trace
            && !workx_otel::set_parent_from_w3c_trace_context(&request_span, trace)
        {
            warn!(
                method = request.method.as_str(),
...
fn terminate_process_tree(child_process: &mut Child, process_group_id: Option<u32>) {
    ...
    #[cfg(unix)]
    if let Err(err) = workx_utils_pty::process_group::terminate_process_group(process_group_id) {
...
fn kill_process_tree(child_process: &mut Child, process_group_id: Option<u32>) {
    ...
    #[cfg(unix)]
    if let Err(err) = workx_utils_pty::process_group::kill_process_group(process_group_id) {

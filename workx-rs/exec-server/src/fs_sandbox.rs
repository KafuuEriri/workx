...
use workx_exec_server_protocol::JSONRPCErrorError;
...
use workx_sandboxing::SandboxDirectSpawnTransformRequest;
use workx_sandboxing::SandboxExecRequest;
...
use crate::fs_helper::WORKX_FS_HELPER_ARG1;
...
impl FileSystemSandboxRunner {
    ...
    fn sandbox_exec_request(
        ...
        let helper = &self.runtime_paths.workx_self_exe;
        ...
        let command = SandboxCommand {
            ...
            args: vec![WORKX_FS_HELPER_ARG1.to_string()],
            cwd: cwd.uri.clone(),
        ...
        sandbox_manager
            .transform_for_direct_spawn(SandboxDirectSpawnTransformRequest {
                ...
                transform: SandboxTransformRequest {
                    ...
                    workx_linux_sandbox_exe: self.runtime_paths.workx_linux_sandbox_exe.as_deref(),
...
fn helper_read_roots(runtime_paths: &ExecServerRuntimePaths) -> Vec<AbsolutePathBuf> {
    let mut roots = vec![runtime_paths.workx_self_exe.clone()];
    if let Some(path) = &runtime_paths.workx_linux_sandbox_exe
        && !roots.contains(path)
    {
...
pub(crate) fn spawn_command(
    ...
    env.retain(|name, _| !workx_protocol::shell_environment::is_non_inheritable_env_var(name));
    command.env_clear();
    command.envs(env);
    command.stdin(stdin);
    ...
    unsafe {
        command.pre_exec(|| {
            workx_utils_pty::pty::close_inherited_fds_except(&[]);
            Ok(())
        });
    }
...
mod tests {
    ...
    fn helper_permissions_preserve_existing_writes() {
        let workx_self_exe = std::env::current_exe().expect("current exe");
        let runtime_paths =
            ExecServerRuntimePaths::new(workx_self_exe, /*workx_linux_sandbox_exe*/ None)
        ...
        let readable = runtime_paths.workx_self_exe.clone();
    ...
    fn helper_env_preserves_path_for_system_bwrap_discovery_without_leaking_secrets() {
        ...
        assert_eq!(
            ...
            HashMap::from([
                ...
                ("TMPDIR".to_string(), "/tmp/workx".to_string()),
    ...
    fn sandbox_exec_request_carries_helper_env() {
        ...
        let workx_self_exe = std::env::current_exe().expect("current exe");
        let runtime_paths =
            ExecServerRuntimePaths::new(workx_self_exe.clone(), Some(workx_self_exe))
                .expect("runtime paths");
        ...
        let sandbox_context = {
            ...
            crate::FileSystemSandboxContext {
                windows_sandbox_level:
                    workx_protocol::config_types::WindowsSandboxLevel::RestrictedToken,
                ..sandbox_context
            }
        };
    ...
    fn sandbox_cwd_rejects_cwd_dependent_profile_without_context_cwd() {
        ...
        let sandbox_context = workx_file_system::FileSystemSandboxContext::from_permission_profile(
    ...
    fn helper_permissions_include_only_the_helper_executable() {
        ...
        assert!(
            policy.can_read_path_with_cwd(runtime_paths.workx_self_exe.as_path(), cwd.as_path())
        );
    ...
    fn helper_permissions_include_only_linux_sandbox_alias_executable() {
        ...
        let workx_self_exe = root.path().join("bin").join("workx");
        let workx_linux_sandbox_exe = root.path().join("aliases").join("workx-linux-sandbox");
        let runtime_paths =
            ExecServerRuntimePaths::new(workx_self_exe, Some(workx_linux_sandbox_exe))
                .expect("runtime paths");
        let cwd = AbsolutePathBuf::from_absolute_path(std::env::temp_dir().as_path())
            .expect("absolute cwd");
        let mut policy = restricted_policy(Vec::new());
        let workx_parent = runtime_paths.workx_self_exe.parent().expect("workx parent");
        ...
        assert!(!policy.can_read_path_with_cwd(workx_parent.as_path(), cwd.as_path()));
    ...
    fn sandbox_context_with_cwd(
        ...
        workx_file_system::FileSystemSandboxContext::from_permission_profile_with_cwd(
            PermissionProfile::from_runtime_permissions(policy, NetworkSandboxPolicy::Restricted),
            cwd,
        )
    }

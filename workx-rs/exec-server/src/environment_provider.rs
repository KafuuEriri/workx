...
use crate::environment::WORKX_EXEC_SERVER_URL_ENV_VAR;

/// Lists the remote environment transports available to Workx.
///
...
/// Default provider backed by `WORKX_EXEC_SERVER_URL`.
#[derive(Clone, Debug)]
...
impl DefaultEnvironmentProvider {
    /// Builds a provider from an already-read raw `WORKX_EXEC_SERVER_URL` value.
    pub fn new(exec_server_url: Option<String>) -> Self {
        Self { exec_server_url }
    }

    /// Builds a provider by reading `WORKX_EXEC_SERVER_URL`.
    pub fn from_env() -> Self {
        Self::new(std::env::var(WORKX_EXEC_SERVER_URL_ENV_VAR).ok())
    }

...
use workx_network_proxy::NetworkPolicyAuditObserver;
use workx_network_proxy::NetworkPolicyDecider;
use workx_network_proxy::NetworkProxy;
use workx_network_proxy::NetworkProxyHandle;
use workx_network_proxy::NetworkProxyState;
use workx_network_proxy::RemoteNetworkProxyLaunchConfig;
use workx_network_proxy::is_managed_mitm_ca_trust_bundle_path;
#[cfg(target_os = "windows")]
use workx_network_proxy::strip_managed_proxy_env;
...
use workx_sandboxing::WindowsSandboxProxySettingsMode;
use workx_sandboxing::WindowsSandboxSpawnRequest;
...
#[cfg(unix)]
use crate::WORKX_ARG0_EXEC_HELPER_ARG1;
...
pub(crate) async fn prepare_exec_request(
    ...
    let sandbox_helper_paths = params
        .arg0
        .iter()
        .map(|_| runtime_paths.workx_self_exe.clone())
        .collect::<Vec<_>>();
    ...
    let sandbox_helper_paths = {
        ...
        if !sandbox_helper_paths.contains(&runtime_paths.workx_self_exe) {
            sandbox_helper_paths.push(runtime_paths.workx_self_exe.clone());
        }
        sandbox_helper_paths.extend(runtime_paths.workx_linux_sandbox_exe.iter().cloned());
    ...
    let (program, args) = params.arg0.as_ref().map_or_else(
        ...
        |arg0| {
            ...
            helper_args.push(WORKX_ARG0_EXEC_HELPER_ARG1.to_string());
    ...
    let transform_request = SandboxDirectSpawnTransformRequest {
        ...
        transform: SandboxTransformRequest {
            ...
            network: None,
            sandbox_policy_cwd,
            workx_linux_sandbox_exe: runtime_paths.workx_linux_sandbox_exe.as_deref(),

...
impl ExecServerTelemetry {
    ...
    pub(crate) fn process_started(&self, process_id: &str) -> ProcessMetricGuard {
        ...
        let parent = workx_otel::current_span_w3c_trace_context();
        ...
        let span = tracing::info_span!(
            parent: None,
            "workx.exec_server.process",
            otel.kind = "internal",
            process.id = process_id,
            result = tracing::field::Empty,
        );
        if let Some(parent) = parent {
            workx_otel::set_parent_from_w3c_trace_context(&span, &parent);
        }
        ProcessMetricGuard {
            telemetry: self.clone(),
            span,

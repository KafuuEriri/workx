...
use workx_config::McpServerDisabledReason;
use workx_config::RequirementSource;
use workx_config::RequirementsLayerEntry;
use workx_config::compose_requirements_for_hostname;
use workx_config::format_config_layer_source;
use workx_config::host_name;
use workx_config::loader::LocalTomlLayerStack;
use workx_config::loader::load_local_config_layers;
use workx_exec_server_protocol::EnvironmentConfigLayer;
use workx_exec_server_protocol::EnvironmentConfigLayerStack;
use workx_exec_server_protocol::EnvironmentConfigReadParams;
use workx_exec_server_protocol::EnvironmentConfigReadResponse;
...
pub(crate) async fn read_environment_config(
    ...
    let workx_home = find_workx_home().map_err(|error| {
        ReadEnvironmentConfigError::Internal(format!("failed to find Workx home: {error}"))
    })?;
    let layers = load_local_config_layers(file_system, workx_home.as_path(), &cwd)
        .await
        .map_err(|error| {
    ...
    Ok(EnvironmentConfigReadResponse {
        ...
        workx_home_dir: PathUri::from_abs_path(&workx_home),
        hostname: host_name(),
...
impl Environment {
    ...
    pub async fn discover_http_mcp_servers(
        ...
        for layer in response.config.layers {
            ...
            workx_config::merge_toml_values(&mut merged, &config);
        }

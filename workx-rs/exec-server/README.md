...
`workx-exec-server` is the library backing `workx exec-server`, a small
JSON-RPC server for spawning and controlling subprocesses through
`workx-utils-pty`.

It provides:

- a CLI entrypoint: `workx exec-server`
- a Rust client: `ExecServerClient`
- a small protocol module with shared request/response types

This crate owns the transport, protocol, and filesystem/process handlers. The
top-level `workx` binary owns hidden helper dispatch for sandboxed
filesystem operations and `workx-linux-sandbox`.

## Transport

The server speaks the exec-specific `workx-exec-server-protocol` message
...
It uses the standard Workx ChatGPT sign-in state; run `workx login` first when
remote registration needs authentication. Containerized callers that receive an
Agent Identity JWT in `WORKX_ACCESS_TOKEN` can opt into that auth path with
`--use-agent-identity-auth`; Workx then registers an Agent task and sends the
derived AgentAssertion headers on the registry request.

Alternatively, API users can instead use `WORKX_API_KEY`;
Workx sends it as a bearer token on the registration request. For example:

```sh
WORKX_API_KEY="$OPENAI_API_KEY" \
workx exec-server \
  --remote ... \
...
`workx.exec_server.relay.v1.RelayMessageFrame`; the checked-in schema is in
`src/proto/workx.exec_server.relay.v1.proto`. The relay frame carries stream
...
- `arg0`: optional argv0 override forwarded to `workx-utils-pty`.
...
hidden helper process launched from the top-level `workx` executable and
...
`HttpClientFactory` to `run_main()`. The top-level `workx exec-server` command
builds these paths from the `workx` arg0 dispatch state and resolves its HTTP
client factory from the effective Workx configuration.
...
provider from Workx auth state before starting remote mode.
